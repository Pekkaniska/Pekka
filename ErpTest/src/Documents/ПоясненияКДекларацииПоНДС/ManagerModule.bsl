	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		Для каждого СтрокаСведСчФ Из СведСчФ Цикл
			...
			Иначе
				...
				ФИО = Новый Массив;
				ФИО.Добавить(СведенияКонтрагента.СведИП_Каг.ФИО.Фамилия);
				ФИО.Добавить(СведенияКонтрагента.СведИП_Каг.ФИО.Имя);
				
				// Необязательное поле
				Если СведенияКонтрагента.СведИП_Каг.ФИО.Свойство("Отчество") Тогда
					ФИО.Добавить(СведенияКонтрагента.СведИП_Каг.ФИО.Отчество);
				КонецЕсли;
...
Функция ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, Раздел)
	...
	Для Каждого Ошибка ИЗ КодыОшибок Цикл
		...
		Если Ошибка = "4" Тогда
			...
			ИначеЕсли Раздел = 9 Тогда
				ПредставлениеОшибки = ПредставлениеОшибкиРаздела9_Код4(ГрафаОшибки);
	...
	МассивОшибок = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(МассивОшибок, Символы.ПС, Истина);
...
Функция ПредставлениеОшибкиРаздела9(КодОшибки, ГрафаОшибки) Экспорт
	...
	Возврат ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, 9);
	
КонецФункции

Функция ПредставлениеОшибкиРаздела9_Код4(ГрафаОшибки)
...
Функция КодыОшибок(КодыОшибок)
	...
	Для каждого КодОшибки Из КодыОшибок Цикл
		МассивКодовОшибок.Добавить(КодОшибки.КодОш);
	КонецЦикла;
	
	МассивКодовОшибок = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(МассивКодовОшибок, ";", Истина);
	...
	Для каждого КодОшибки Из КодыОшибок Цикл
		Если КодОшибки.Свойство("ГрафОш") Тогда
			...
			Для каждого СтрокаГрафОш Из ГрафОш Цикл
				МассивГрафОшибок.Добавить(СтрокаГрафОш);
			КонецЦикла; 
		КонецЕсли;
	КонецЦикла; 
	
	МассивГрафОшибок = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(МассивГрафОшибок, ";", Истина);
...
Функция ПериодОтраженияЗаписи(ПериодЗап)
	...
	Если ТипЗнч(ПериодЗап) = Тип("Массив") Тогда
		...
		Для каждого СтрокаПериодЗап Из ПериодЗап Цикл
			Результат.Добавить(ПериодОтраженияПодзаписи(СтрокаПериодЗап));
		КонецЦикла;
		Результат = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Результат, ";", Истина);
		Возврат Результат;
