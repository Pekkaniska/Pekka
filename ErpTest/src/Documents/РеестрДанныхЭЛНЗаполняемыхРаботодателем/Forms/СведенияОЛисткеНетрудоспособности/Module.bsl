...
Процедура ПолучитьХэшИзСервиса(Команда)
	Если Не ЗначениеЗаполнено(Параметры.ИнформацияОДокументе.Организация) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В документе не выбрана организация.'"), , "Организация");
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран сотрудник.'"), , "Сотрудник");
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(НомерЛисткаНетрудоспособности) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнен номер листка нетрудоспособности.'"), , "НомерЛисткаНетрудоспособности");
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьФорму()
	...
	Хэш = РегистрыСведений.ХэшиЭЛН.ПрочитатьХэш(НомерЛисткаНетрудоспособности);
...
Функция ЗагрузитьХэшЭЛН(Знач АдресРасшифрованногоОтветаSOAP)
	РезультатОбмена = ОбменЛисткамиНетрудоспособностиФСС.ЗагрузитьОтветСервисаФСС(СтруктураДляПолученияХэшаЭЛН(), "getPrivateLNData", АдресРасшифрованногоОтветаSOAP);
	ОбновитьФорму();
