...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	Отчеты.СверкаРасчетовСПартнерами.ДобавитьКомандуОтчета(КомандыОтчетов);
...
//+++rarus-spb_zlov 15.03.2017    
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "пкАктСверкиВзаимныхРасчетовPekkaniska";
	КомандаПечати.Представление = НСтр("ru = 'Акт сверки взаимных расчетов (Pekkaniska)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
    
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "пкАктСверкиВзаимныхРасчетовСПодписямиPekkaniska";
	КомандаПечати.Представление = НСтр("ru = 'Акт сверки взаимных расчетов с подписями (Pekkaniska)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
//---rarus-spb_zlov 15.03.2017 
КонецПроцедуры
...
//+++rarus-spb_zlov 15.03.2017    
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "пкАктСверкиВзаимныхРасчетовPekkaniska") Тогда
			
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			"пкАктСверкиВзаимныхРасчетовPekkaniska",
			НСтр("ru='Акт сверки взаимных расчетов (Pekkaniska)'"),
			СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетовPekkaniska(МассивОбъектов, ОбъектыПечати, Ложь));
            
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "пкАктСверкиВзаимныхРасчетовСПодписямиPekkaniska") Тогда
			
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			"пкАктСверкиВзаимныхРасчетовСПодписямиPekkaniska",
			НСтр("ru='Акт сверки взаимных расчетов с подписями (Pekkaniska)'"),
			СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетовPekkaniska(МассивОбъектов, ОбъектыПечати, Истина));
//---rarus-spb_zlov 15.03.2017 
	КонецЕсли;
...
//+++DubI Рарус-СПб 24.03.2017
	...
	ВидыДокументовКонтрагента.Вставить("ПриобретениеУслугПрочихАктивов",		НСтр("ru='Приход'"));
	ВидыДокументовКонтрагента.Вставить("ПриобретениеУслугПоЛизингу",            НСтр("ru='Приход'"));
//---DubI Рарус-СПб
	...
	Пока ДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДетальныеЗаписи.Номер) Тогда
				...
				Иначе
//++ Рарус Лимаренко 23.07.2018 Задача №13213
					НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ДетальныеЗаписи.Документ.Номер,Ложь,Истина);
					//НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДетальныеЗаписи.Номер,Истина);
//-- Рарус Лимаренко 23.07.2018 Задача №13213
				КонецЕсли;
			Иначе
...
//+++rarus-spb_zlov 15.03.2017    
			...
			ОбластьМакетаСтрока.Параметры.Дата = Формат(ДетальныеЗаписи.Дата, "ДФ=dd.MM.yyyy");
//---rarus-spb_zlov 15.03.2017 
...
//Рарус: Владимир Подрезов 09.10.2017 4819
//	|	ДанныеДокумента.РасшифровкаПоДоговорам КАК РасшифровкаПоДоговорам
	|	ВЫБОР КОГДА ДанныеДокумента.РасшифровкаПоДоговорам
	|		ИЛИ ДанныеДокумента.Договор <> Значение(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК РасшифровкаПоДоговорам
//Рарус: Владимир Подрезов 10.07.2017
	//+++DubI Рарус-СПб 24.03.2017
	|	, ДанныеДокумента.пкПодписант КАК пкПодписант
	//---DubI Рарус-СПб
	|ИЗ
	|	Документ.СверкаВзаиморасчетов КАК ДанныеДокумента
...
//+++rarus-spb_zlov 15.03.2017    
Функция СформироватьПечатнуюФормуАктСверкиВзаимныхРасчетовPekkaniska(МассивОбъектов, ОбъектыПечати, ВыводитьПодписи)
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СверкаВзаиморасчетов.пкАктСверкиВзаимныхРасчетовPekkaniska");
		...
		Пока ДетальныеРасчеты.Следующий() Цикл
			...
			Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
				ВыборкаПоДетальнымРасчетам = ДетальныеРасчеты.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ВыборкаПоДетальнымРасчетам.Следующий() Цикл
					ТабличныйДокумент.Вывести(пкВывестиДетальныеЗаписи(ВыборкаПоДетальнымРасчетам, ДанныеДокумента, СтруктураПредставленийУчастников, Макет, ТаблицаЗадолженности, ДетальныеРасчеты.Партнер));
				КонецЦикла; 
			ИначеЕсли ДанныеДокумента.РасшифровкаПоДоговорам Тогда
				ВыборкаПоДетальнымРасчетам = ДетальныеРасчеты.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ВыборкаПоДетальнымРасчетам.Следующий() Цикл
					ТабличныйДокумент.Вывести(пкВывестиДетальныеЗаписи(ВыборкаПоДетальнымРасчетам, ДанныеДокумента, СтруктураПредставленийУчастников, Макет, ТаблицаЗадолженности, , ДетальныеРасчеты.Договор));
				КонецЦикла; 
			Иначе
				ТабличныйДокумент.Вывести(пкВывестиДетальныеЗаписи(ДетальныеРасчеты, ДанныеДокумента, СтруктураПредставленийУчастников, Макет, ТаблицаЗадолженности));
			КонецЕсли;
		...
		Если ДанныеДокумента.РасшифровкаПоЗаказам Тогда
			...
			Если ТаблицаОстатков <> Неопределено Тогда
				...
				Если ДанныеДокумента.РасшифровкаПоПартнерам Тогда
					Для Каждого Партнер Из МассивПартнеров Цикл
						ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(Новый Структура("ДанныеДокумента, ТаблицаОстатков, МассивВалют, СтруктураПредставленийУчастников, Макет", 
							ДанныеДокумента, ТаблицаОстатков, МассивВалют, СтруктураПредставленийУчастников, Макет), Партнер));
					КонецЦикла; 
				Иначе
					ТабличныйДокумент.Вывести(ВывестиТаблицуЗадолженности(Новый Структура("ДанныеДокумента, ТаблицаОстатков, МассивВалют, СтруктураПредставленийУчастников, Макет", 
						ДанныеДокумента, ТаблицаОстатков, МассивВалют, СтруктураПредставленийУчастников, Макет)));
				КонецЕсли;	
        ...
        Партнер = ДанныеДокумента.Контрагент.Партнер;
        ...
        ОбластьПодписи = пкОбщегоНазначенияСервер.ПолучитьМакетПодписи(ДанныеДокумента.Дата, 
//++ Рарус Лимаренко 19.10.17	
		?(ЗначениеЗаполнено(ДанныеДокумента.Договор),ДанныеДокумента.Договор.Подразделение,Партнер.пкПодразделение),
//Партнер.пкПодразделение, 
//-- Рарус Лимаренко 19.10.17		
		Перечисления.пкВидыПечатныхФормДляШаблоновПодписей.АктСверки, 
			ВыводитьПодписи, 
			?(ЗначениеЗаполнено(ДанныеДокумента.пкПодписант), ДанныеДокумента.пкПодписант, Неопределено));
		//---DubI Рарус-СПб
        ...
        Если МакетПечатнойФормы <> Неопределено тогда
			//+++DubI Рарус-СПб 24.03.2017
			МакетПечатнойФормы.Параметры.НазваниеОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, 
			//											  			  "ПолноеНаименование,ИНН,ЮридическийАдрес");
														  			  "ПолноеНаименование,ИНН,КПП");
			...
			МакетПечатнойФормы.Параметры.ДолжностьК = ДанныеДокумента.ДолжностьРуководителяКонтрагента;
			МакетПечатнойФормы.Параметры.ФИОПредставителяК = ДанныеДокумента.ФИОРуководителяКонтрагента;
			//---DubI Рарус-СПб
...
КонецФункции
//---rarus-spb_zlov 15.03.2017 
//++ Рарус Лимаренко 26.02.2018 №9463
Функция пкВывестиДетальныеЗаписи(
	ВыборкаДетальныхЗаписей,
	ДанныеДокумента,
	СтруктураПредставленийУчастников,
	Макет,
	...
	ОбластьМакетаКонОстаток  = Макет.ПолучитьОбласть("СтрокаТаблицыРасчетыКонОстаток");
//++ Рарус Лимаренко 26.02.2018 №9463
	ОбластьМакетаСтрокаОбороты = Макет.ПолучитьОбласть("СтрокаТаблицыОборотыЗаПериод");	
//-- Рарус Лимаренко 26.02.2018 №9463
...
//-- Рарус Лимаренко 26.02.2018 №9463
	...
	Пока ДетальныеЗаписи.Следующий() Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДетальныеЗаписи.Номер) Тогда
				Если ИмяДокумента = "СписаниеБезналичныхДенежныхСредств" Тогда
					НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДетальныеЗаписи.Номер,Истина,Истина);
				ИначеЕсли ДетальныеЗаписи.ЭтоВходящийДокумент Тогда
					НомерНаПечать = УбратьЛидирующиеНули(ДетальныеЗаписи.Номер);
				Иначе
//++ Рарус Лимаренко 23.07.2018 Задача №13213
...
//++ Рарус Лимаренко 26.02.2018 №9463
		   ОборотыПартнера = ОборотыПартнера + ?(ТипЗнч(ОбластьМакетаСтрока.Параметры.СуммаДолгПартнера) <> Тип("Число"),0,ОбластьМакетаСтрока.Параметры.СуммаДолгПартнера);
		   ОборотыНаши = ОборотыНаши + ?(ТипЗнч(ОбластьМакетаСтрока.Параметры.СуммаНашДолг) <> Тип("Число"),0,ОбластьМакетаСтрока.Параметры.СуммаНашДолг);
//-- Рарус Лимаренко 26.02.2018 №9463
...
//++ Рарус Лимаренко 26.02.2018 №9463
	ОбластьМакетаСтрокаОбороты.Параметры.ОборотыНаши = ОборотыНаши;
	ОбластьМакетаСтрокаОбороты.Параметры.ОборотыПартнера = ОборотыПартнера;
	ТабличныйДокумент.Вывести(ОбластьМакетаСтрокаОбороты);
//-- Рарус Лимаренко 26.02.2018 №9463
