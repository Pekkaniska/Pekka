...
Функция ТекстЗапросаМатериаловИРаботВКладовых(ВоВременнуюТаблицу = Ложь, ВыбиратьПервые = 0) Экспорт
	...
	|					ИЛИ &ВсеПодразделения)
	|				И АналитикаУчетаНоменклатуры.Склад ССЫЛКА Справочник.Склады
	|				И ВЫРАЗИТЬ(АналитикаУчетаНоменклатуры.Склад КАК Справочник.Склады).ЦеховаяКладовая) КАК ПоступленияМатериалов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	...
	|		ИЛИ &ВсеОрганизации)
	|	И (ВЫРАЗИТЬ(Аналитика.Склад КАК Справочник.Склады).Подразделение В (&Подразделения)
	|		ИЛИ &ВсеПодразделения)
	...
	|	0,
	|	ВЫБОР 
	|		КОГДА ПоступленияРабот.КоличествоПриход > 0
	|			ТОГДА ПоступленияРабот.КоличествоПриход
	|		КОГДА ПоступленияРабот.КоличествоРасход < 0
	|			ТОГДА -ПоступленияРабот.КоличествоРасход
	|		ИНАЧЕ
	|			0
	|	КОНЕЦ,
	|	0
	...
	|	И РаботыВПроизводстве.Количество > 0
...
Функция ТекстЗапросаВидыЗапасовИРаботы(Запрос, ТекстыЗапроса)
	...
	|		КОГДА СпрВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияДавальца)
	...
	|		И СпрВидыЗапасов.ВладелецТовара = КлючиКомитента.Склад
	|
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ВидыЗапасовИРаботы", ТекстыЗапроса) Тогда
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	ТекстЗапроса = "
	// Расход материалов со склада на партию производства
	//   - списанных на расходы
	//   - распределенных вручную
	//      в том числе выкупленных комиссионных.
	|ВЫБРАТЬ
	|	1                                                КАК Порядок,
	...
	|		ИНАЧЕ
	|			ВидыЗапасов.РазделУчета
	|	КОНЕЦ                                            КАК РазделУчета,
	|	ВЫБОР
	|		КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов И ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	...
	|			ТОГДА ВидыЗапасов.ПодразделениеПолучательРасходов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                            КАК Подразделение,
	...
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	|			ТОГДА ВидыЗапасов.Назначение.НаправлениеДеятельности
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                            КАК КорНаправлениеДеятельности
	|ИЗ
	|	ВидыЗапасовИРаботы КАК ВидыЗапасов
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтПрочиеРасходы КАК ПрочиеРасходы
	|		ПО ПрочиеРасходы.СтатьяРасходов = ВидыЗапасов.СтатьяРасходов
	|		И ПрочиеРасходы.АналитикаРасходов = ВидыЗапасов.АналитикаРасходов
	|ГДЕ
	|	&НовоеПроизводство
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Приход материалов на партию производства
	//   - распределенных вручную.
	|ВЫБРАТЬ
	|	2                                                    КАК Порядок,
	|	ВидыЗапасов.НомерСтроки                              КАК НомерСтрокиДокумента,
	...
	|	ВидыЗапасов.КорВидДеятельностиНДС                    КАК ВидДеятельностиНДС,
	|	НЕОПРЕДЕЛЕНО                                         КАК КорВидДеятельностиНДС,
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюОтчетовКомитенту(Запрос, ТекстыЗапроса, Регистры)
	...
	|	0                                         КАК Количество,
	|	0                                         КАК СуммаВыручки,
	|	СУММА(ТаблицаВидыЗапасов.Количество)      КАК КоличествоСписано,
	|	0                                         КАК КоличествоКОформлению,
	|	0                                         КАК СуммаВыручкиКОформлению,
	|	СУММА(ТаблицаВидыЗапасов.Количество)      КАК КоличествоСписаноКОформлению,
...
// Заполняет аналитику учета номенклатуры получателя для материалов и работ, распределенных на партии производства.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.РаспределениеПроизводственныхЗатрат.ПартииПроизводства КАК ТаблицаТоваров
	|ГДЕ
	...
	|	ТаблицаТоваров.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.РаспределениеПроизводственныхЗатрат.ВидыЗапасов КАК ТаблицаТоваров
	|ГДЕ
	|	ТаблицаТоваров.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
	|	И НЕ ТаблицаТоваров.УдалитьПартияПроизводства = НЕОПРЕДЕЛЕНО
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.РаспределениеПроизводственныхЗатрат";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Запрос.Текст = ТекстВидовЗапасовДляРабот;
			Запрос.УстановитьПараметр("Ссылка", Выборка.Ссылка);
			
			ВыборкаСтрокПоРаботам = Запрос.Выполнить().Выбрать();
			Пока ВыборкаСтрокПоРаботам.Следующий() Цикл
				СтрокаСРаботой = ДокументОбъект.ВидыЗапасов[ВыборкаСтрокПоРаботам.НомерСтроки - 1];
				СтрокаСРаботой.ВидЗапасов = Неопределено;
			КонецЦикла;
