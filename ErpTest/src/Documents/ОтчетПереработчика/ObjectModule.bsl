...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ПоступлениеОтПереработчика") Тогда
		...
		Если УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПартионныйУчетВерсии22(РеквизитыПоступления.Дата) Тогда
	...
	ДоходыИРасходыСервер.ИнициализироватьПустоеЗначениеСтатьиВТЧ(ВозвратныеОтходы , "СтатьяРасходов");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ВзаиморасчетыСервер.ЗаполнитьСуммуНДСВзаиморасчетовВТабличнойЧасти(ЭтотОбъект, "Услуги");
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если СуммаДокумента > 0
			И ((НЕ ПоЗаказам И ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным)
...
Процедура ПриЗаписи(Отказ)
	...
	ОбщегоНазначенияУТ.УстановитьПометкуУдаленияКлюча(
...
Процедура ЗаполнитьНаОснованииПоступленияОтПереработчикаПартионныйУчет21(МассивПоступлений)
	...
	Если ПартионныйУчетВерсии22 Тогда
		Дата = УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ДатаПереходаНаПартионныйУчетВерсии22() - 1;
	КонецЕсли;
...
Процедура ЗаполнитьПоТоварамКПоступлениюПартионныйУчет22()
	...
	ТекстПродукция =
		...
		|ГДЕ
		|	ТоварыПолученные.АналитикаУчетаНоменклатуры.Склад = &Партнер
		|АВТОУПОРЯДОЧИВАНИЕ";
...
Процедура ЗаполнитьТабличнуюЧастьУслугиИМатериалыПоСпецификацииПродукции()
	...
	Для Каждого ТекущиеДанные Из Продукция Цикл
		...
		ДанныеСпецификации = УправлениеДаннымиОбИзделияхВызовСервера.СпецификацияИзделия(ТекущиеДанные.Номенклатура,
			ТекущиеДанные.Характеристика, Дата, Подразделение, ПараметрыВыбораСпецификаций);
	...
	Для Каждого ТекущиеДанные Из Продукция Цикл
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.Спецификация) Тогда
			...
			СтрокаПродукции.ПоляГруппировки = Новый Структура("ДокументПоступления", ТекущиеДанные.ДокументПоступления);
			ЗаполнитьЗначенияСвойств(СтрокаПродукции, ТекущиеДанные, "Номенклатура,Характеристика,Спецификация,Количество");
			МассивПродукции.Добавить(СтрокаПродукции);
	...
	ПараметрыВыборки.ПоляГруппировки = Новый Структура("ДокументПоступления");
	...
	ДанныеСпецификаций = Справочники.РесурсныеСпецификации.ДанныеСпецификацииПоСпискуНоменклатуры(МассивПродукции, ПараметрыВыборки);
	...
	Если ТаблицаЭтапы.Количество() > 0 Тогда
		...
		ТаблицаЭтапы.Свернуть("ДокументПоступления,УслугаПереработчика,ХарактеристикаУслугиПереработчика,СтатьяКалькуляции");
		...
		Для Каждого СтрокаУслуги Из ТаблицаУслуги Цикл
			...
			Если НайденныеЭтапы.Количество() = 1 Тогда
				...
				НайденныйЭтап = НайденныеЭтапы[0];
				СтрокаУслуги.Номенклатура      = НайденныйЭтап.УслугаПереработчика;
				СтрокаУслуги.Характеристика    = НайденныйЭтап.ХарактеристикаУслугиПереработчика;
				СтрокаУслуги.СтатьяКалькуляции = НайденныйЭтап.СтатьяКалькуляции;
...
Процедура ЗаполнитьТабличныеЧастиПоЭтапамПроизводства()
	...
	ТекстПродукция =
		...
		|	ВыходныеИзделияЭтапа.СчетУчета,
		|	ВыходныеИзделияЭтапа.Субконто1,
		|	ВыходныеИзделияЭтапа.Субконто2,
		|	ВыходныеИзделияЭтапа.Субконто3,
	...
	ТекстВозвратныеОтходы =
		...
		|	ПобочныеИзделияЭтапа.СчетУчета,
		|	ПобочныеИзделияЭтапа.Субконто1,
		|	ПобочныеИзделияЭтапа.Субконто2,
		|	ПобочныеИзделияЭтапа.Субконто3,
