...
Процедура ЗаполнитьВидыЗапасовПродукции(Объект) Экспорт
	...
	Запрос.Выполнить();
	ЗапасыСервер.ЗаполнитьВидыЗапасовПоУмолчанию(Запрос.МенеджерВременныхТаблиц, Объект.Продукция);
	
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ТаблицаТоваров", Объект.ВозвратныеОтходы.Выгрузить(,"НомерСтроки, Номенклатура, ВидЗапасов, КодСтроки"));
	Запрос.Выполнить();
	ЗапасыСервер.ЗаполнитьВидыЗапасовПоУмолчанию(Запрос.МенеджерВременныхТаблиц, Объект.ВозвратныеОтходы);
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|	0                                                               КАК СтоимостьУпр,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПартииПроизводства.Назначение.ВидДеятельностиНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка))
	|				<> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|			ТОГДА ПартииПроизводства.Назначение.ВидДеятельностиНДС
	...
	|		И &Подразделение = АналитикаПроизводства.Склад
	...
	|		И &Подразделение = АналитикаПроизводстваБезНазначения.Склад
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                                           КАК АналитикаФинансовогоУчета,
	|	ТаблицаТовары.КорВидДеятельностиНДС                             КАК ВидДеятельностиНДС,
	|	ВЫБОР КОГДА &ФИФОСкользящаяОценка
	|		ТОГДА ТаблицаТовары.ПартияВыпуска
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                                           КАК КорПартия,
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ПО Константы.УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|ГДЕ
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЧВозвратныеОтходы.Ссылка
	|ИЗ
	|	Документ.ОтчетПереработчика КАК ОтчетПереработчика
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.ВозвратныеОтходы КАК ТЧВозвратныеОтходы
	|	ПО ОтчетПереработчика.Ссылка = ТЧВозвратныеОтходы.Ссылка
	|
	...
	|	И ОтчетПереработчика.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И ТЧВозвратныеОтходы.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Документ.ОтчетПереработчика КАК ОтчетПереработчика
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.Материалы КАК ТЧМатериалы
	|	ПО ОтчетПереработчика.Ссылка = ТЧМатериалы.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	...
	|	И Константы.ПартионныйУчетВерсии22
	|	И ОтчетПереработчика.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И ТЧМатериалы.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Документ.ОтчетПереработчика КАК ОтчетПереработчика
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.ВидыЗапасов КАК ТЧМатериалы
	|	ПО ОтчетПереработчика.Ссылка = ТЧМатериалы.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	...
	|	Документ.ОтчетПереработчика КАК ОтчетПереработчика
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.Услуги КАК ТЧУслуги
	|	ПО ОтчетПереработчика.Ссылка = ТЧУслуги.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	...
	|	И Константы.ПартионныйУчетВерсии22
	|	И ОтчетПереработчика.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И ТЧУслуги.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И ОтчетПереработчика.УслугиПоПереработке = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	И Константы.ПартионныйУчетВерсии22
	|	И ОтчетПереработчика.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И ОтчетПереработчика.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И ОтчетПереработчика.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки)
	|	И ОтчетПереработчика.УслугиПоПереработке = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение кодов строк продукции для отчетов переработчика
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Реквизиты.Ссылка
	|ИЗ
	...
	|	И ТаблицаТовары.КодСтрокиПродукция = 0
	|	И НЕ ТаблицаТовары.УдалитьТипСтоимости = ЗНАЧЕНИЕ(Перечисление.ТипыСтоимостиВыходныхИзделий.Фиксированная)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Очистка кодов строк для отчетов переработчика без заказа
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Реквизиты.Ссылка
	|ИЗ
	|	Документ.ОтчетПереработчика КАК Реквизиты
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если Не ЗагруженыТребуемыеДанныеИзРИБ(МенеджерВременныхТаблиц, Результат.ИмяВременнойТаблицы) Тогда
		Параметры.ОбработкаЗавершена = Ложь;
		Возврат;
	КонецЕсли;
	
	СтатьяКалькуляцииМатериалов = Неопределено;
	СтатьяКалькуляцииОтходов = Неопределено;
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаПродукция.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ЗаполнитьВидыЗапасов
	|ИЗ
	|	Документ.ОтчетПереработчика.Продукция КАК ТаблицаПродукция
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыДляОбработки КАК КОбработке
	|	ПО ТаблицаПродукция.Ссылка = КОбработке.Ссылка
	|
	|ГДЕ
	|	ТаблицаПродукция.ВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	...
	|ВЫБРАТЬ
	|	ТаблицаПродукция.Ссылка
	|ПОМЕСТИТЬ ЗаполнитьКодСтрокиПродукция
	|ИЗ
	|	Документ.ОтчетПереработчика.Продукция КАК ТаблицаПродукция
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыДляОбработки КАК КОбработке
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика КАК Реквизиты
	|	ПО ТаблицаПродукция.Ссылка = Реквизиты.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	...
	|	И Реквизиты.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И Реквизиты.Проведен
	|	И ТаблицаПродукция.КодСтрокиПродукция = 0
	|	И НЕ ТаблицаПродукция.УдалитьТипСтоимости = ЗНАЧЕНИЕ(Перечисление.ТипыСтоимостиВыходныхИзделий.Фиксированная)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПродукция.Ссылка
	|
	...
	|	И Реквизиты.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И НЕ Реквизиты.ПоЗаказам
	|	И НЕ ТаблицаПродукция.КодСтроки = 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПродукция.Ссылка
	|
	...
	|	Документ.ОтчетПереработчика.Услуги КАК ТаблицаУслуги
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыДляОбработки КАК КОбработке
	|		ПО ТаблицаУслуги.Ссылка = КОбработке.Ссылка
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика КАК Реквизиты
	|		ПО ТаблицаУслуги.Ссылка = Реквизиты.Ссылка
	|ГДЕ
	|	Реквизиты.Проведен
	...
	|	ВЫБОР
	|		КОГДА ЗаполнитьВидыЗапасов.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                          КАК ТребуетсяЗаполнитьВидыЗапасов,
	|	ВЫБОР
	|		КОГДА ЗаполнитьКодСтрокиПродукция.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                          КАК ТребуетсяЗаполнитьКодСтрокиПродукция,
	|	ВЫБОР
	|		КОГДА ОчиститьКодСтроки.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                          КАК ТребуетсяОчиститьКодСтроки,
	|	ВЫБОР
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьВидыЗапасов КАК ЗаполнитьВидыЗапасов
	|		ПО ЗаполнитьВидыЗапасов.Ссылка = КОбработке.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьКодСтрокиПродукция КАК ЗаполнитьКодСтрокиПродукция
	|		ПО ЗаполнитьКодСтрокиПродукция.Ссылка = КОбработке.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ОчиститьКодСтроки КАК ОчиститьКодСтроки
	|		ПО ОчиститьКодСтроки.Ссылка = КОбработке.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЗаполнитьСуммуВзаиморасчетов КАК ЗаполнитьСуммуВзаиморасчетов
	|		ПО ЗаполнитьСуммуВзаиморасчетов.Ссылка = КОбработке.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	ВыборкаСтатья = ПакетРезультатов[5].Выбрать();
	Если ВыборкаСтатья.Следующий() Тогда
		СтатьяКалькуляцииМатериалов = ВыборкаСтатья.Ссылка;
	КонецЕсли;
	ВыборкаСтатья = ПакетРезультатов[6].Выбрать();
	Если ВыборкаСтатья.Следующий() Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ОбъектИзменен     = Ложь;
			МожноОбрабатывать = Истина;
			
			Если ДокументОбъект <> Неопределено Тогда
				
				ПартионныйУчетВерсии22 = УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПартионныйУчетВерсии22(НачалоМесяца(ДокументОбъект.Дата));
				...
				// заполнение пустой статьи калькуляции.
				
				Если МожноОбрабатывать
					И ДокументОбъект.Проведен
					И ПартионныйУчетВерсии22 Тогда
					СтруктураПоиска = Новый Структура("СтатьяКалькуляции", Справочники.СтатьиКалькуляции.ПустаяСсылка());
					НайденныеСтроки = ДокументОбъект.ВозвратныеОтходы.НайтиСтроки(СтруктураПоиска);
					Если СтатьяКалькуляцииОтходов = Неопределено И НайденныеСтроки.Количество() > 0 Тогда
						МожноОбрабатывать = Ложь;
					ИначеЕсли НайденныеСтроки.Количество() > 0 Тогда
						ОбъектИзменен = Истина;
						Для Каждого Строка Из НайденныеСтроки Цикл
							Строка.СтатьяКалькуляции = СтатьяКалькуляцииОтходов;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
				
				// Обработчик №3
				// Табличная часть "Материалы" и "Виды запасов"
				// заполнение пустой статьи калькуляции.
				...
				Если МожноОбрабатывать
					...
					И ПартионныйУчетВерсии22 Тогда
					СтруктураПоиска = Новый Структура("СтатьяКалькуляции", Справочники.СтатьиКалькуляции.ПустаяСсылка());
					НайденныеСтроки = ДокументОбъект.Материалы.НайтиСтроки(СтруктураПоиска);
					Если СтатьяКалькуляцииМатериалов = Неопределено И НайденныеСтроки.Количество() > 0 Тогда
						МожноОбрабатывать = Ложь;
					ИначеЕсли НайденныеСтроки.Количество() > 0 Тогда
						ОбъектИзменен = Истина;
						Для Каждого Строка Из НайденныеСтроки Цикл
							Строка.СтатьяКалькуляции = СтатьяКалькуляцииМатериалов;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
				...
				Если МожноОбрабатывать
					...
					И ПартионныйУчетВерсии22 Тогда
					СтруктураПоиска = Новый Структура("СтатьяКалькуляции", Справочники.СтатьиКалькуляции.ПустаяСсылка());
					НайденныеСтроки = ДокументОбъект.ВидыЗапасов.НайтиСтроки(СтруктураПоиска);
					Если СтатьяКалькуляцииМатериалов = Неопределено И НайденныеСтроки.Количество() > 0 Тогда
						МожноОбрабатывать = Ложь;
					ИначеЕсли НайденныеСтроки.Количество() > 0 Тогда
						ОбъектИзменен = Истина;
				...
				Если МожноОбрабатывать
					И ДокументОбъект.Проведен
					И ПартионныйУчетВерсии22
					И ДокументОбъект.УслугиПоПереработке = Перечисления.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете Тогда
					СтруктураПоиска = Новый Структура("СтатьяКалькуляции", Справочники.СтатьиКалькуляции.ПустаяСсылка());
					НайденныеСтроки = ДокументОбъект.Услуги.НайтиСтроки(СтруктураПоиска);
					Если СтатьяКалькуляцииМатериалов = Неопределено И НайденныеСтроки.Количество() > 0 Тогда
						МожноОбрабатывать = Ложь;
					ИначеЕсли НайденныеСтроки.Количество() > 0 Тогда
						ОбъектИзменен = Истина;
				...
				// Заполнение пустой статьи калькуляции в шапке документа если группировка затрат БезГруппировки.
				
				Если МожноОбрабатывать
					И ДокументОбъект.Проведен
					И ПартионныйУчетВерсии22
					И ДокументОбъект.УслугиПоПереработке = Перечисления.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете
					И ДокументОбъект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки
					И Не ЗначениеЗаполнено(ДокументОбъект.СтатьяКалькуляции) Тогда
					
					Если СтатьяКалькуляцииМатериалов = Неопределено Тогда
						МожноОбрабатывать = Ложь;
					Иначе
						ОбъектИзменен = Истина;
						ДокументОбъект.СтатьяКалькуляции = СтатьяКалькуляцииМатериалов;
					КонецЕсли;
					
				КонецЕсли;
				
				// Обработчик №5
				// Табличные части "Продукция" и "Возвратные отходы"
				// заполнение пустого реквизита "ВидЗапасов".
				
				Если МожноОбрабатывать
					И Выборка.ТребуетсяЗаполнитьВидыЗапасов Тогда
					ОбъектИзменен = Истина;
					ЗаполнитьВидыЗапасовПродукцииПриОбновленииИБ(ДокументОбъект);
				КонецЕсли;
				
				// Обработчик №6
				// заполнение кодов строк продукции.
				Если МожноОбрабатывать
					И Выборка.ТребуетсяЗаполнитьКодСтрокиПродукция Тогда
					
					ОбъектИзменен = Истина;
					ДокументОбъект.МаксимальныйКодСтрокиПродукция = 0;
					
					Для Каждого Строка Из ДокументОбъект.Продукция Цикл
						Строка.КодСтрокиПродукция = Строка.КодСтроки;
						ДокументОбъект.МаксимальныйКодСтрокиПродукция =
							Макс(ДокументОбъект.МаксимальныйКодСтрокиПродукция, Строка.КодСтроки);
					КонецЦикла;
				...
				// Обработчик №7
				// очистка кодов строк отчетов без заказа.
				Если МожноОбрабатывать
					И Выборка.ТребуетсяОчиститьКодСтроки Тогда
				...
				// Обработчик №8
				// заполнение ревизита СпособРаспределенияЗатратНаВыходныеИзделия.
				Если МожноОбрабатывать
					И ДокументОбъект.СпособРаспределенияЗатратНаВыходныеИзделия = Перечисления.СпособыРаспределенияЗатратНаВыходныеИзделия.ПустаяСсылка() 
				...
				Если МожноОбрабатывать
					И Выборка.ТребуетсяЗаполнитьСуммуВзаиморасчетов Тогда
...
Функция ЗагруженыТребуемыеДанныеИзРИБ(МВТ, ИмяВременнойТаблицы)
	...
	|	ТЧВозвратныеОтходы.Ссылка
	|ИЗ
	|	Документ.ОтчетПереработчика КАК ДокументОтчетПереработчика
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыДляОбработки КАК ВТДокументыДляОбработки
	|	ПО ДокументОтчетПереработчика.Ссылка = ВТДокументыДляОбработки.Ссылка
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.ВозвратныеОтходы КАК ТЧВозвратныеОтходы
	|	ПО ДокументОтчетПереработчика.Ссылка = ТЧВозвратныеОтходы.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	|ГДЕ
	|	Константы.ПартионныйУчетВерсии22
	|	И ДокументОтчетПереработчика.Дата >= Константы.ДатаПереходаНаПартионныйУчетВерсии22
	|	И ДокументОтчетПереработчика.Проведен
	|	И ТЧВозвратныеОтходы.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.Материалы КАК ТЧМатериалы
	|	ПО ДокументОтчетПереработчика.Ссылка = ТЧМатериалы.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.Услуги КАК ТЧУслуги
	|	ПО ДокументОтчетПереработчика.Ссылка = ТЧУслуги.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	...
	|	И ДокументОтчетПереработчика.Проведен
	|	И ТЧУслуги.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И ДокументОтчетПереработчика.УслугиПоПереработке = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДокументыДляОбработки КАК ВТДокументыДляОбработки
	|	ПО ОтчетПереработчика.Ссылка = ВТДокументыДляОбработки.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТДокументыДляОбработки", ИмяВременнойТаблицы);
...
Процедура ЗаполнитьВидыЗапасовПродукцииПриОбновленииИБ(Объект) Экспорт
	...
	Запрос.УстановитьПараметр("ТаблицаТоваров", Объект.Продукция.Выгрузить(,"НомерСтроки, Номенклатура, ВидЗапасов, КодСтроки"));
	Запрос.Выполнить();
	ЗаполнитьВидыЗапасовПоУмолчаниюПриОбновленииИБ(Запрос.МенеджерВременныхТаблиц, Объект.Продукция);
