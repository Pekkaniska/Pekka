...
Процедура ПриОткрытии(Отказ)
	...
	ЭтоВозврватИзЭксплуатации = (Объект.ХозяйственнаяОперация=ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратИзЭксплуатации"));
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ НЕ УТ
	Справочники.ПартииТМЦВЭксплуатации.ЗаполнитьПризнакиПартииТМЦВЭксплуатации(Объект.Товары);
	//-- НЕ УТ
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Не Копирование Тогда
		ПриДобавленииСтрокиТоваров(Элементы.Товары.ТекущиеДанные, Объект.ХозяйственнаяОперация);
...
Процедура ТоварыПартияТМЦВЭксплуатацииПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	//++ НЕ УТ
	СтруктураДействий.Вставить("ЗаполнитьПризнакиПартииТМЦВЭксплуатации");
	//-- НЕ УТ
	ДобавитьВСтруктуруДействияПриИзмененииКоличестваУпаковок(СтруктураДействий);
...
Процедура УстановитьУсловноеОформление()
	...
	// Подразделение нужно заполнять только для операции "Возврат из эксплуатации", "Оприходование (за счет расходов)".
...
Процедура ПриДобавленииСтрокиТоваров(Строка, ХозяйственнаяОперация)
	
	Если ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеИзПроизводства")
		ИЛИ ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СторноСписанияНаРасходы")
		ИЛИ ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОприходованиеЗаСчетРасходов") Тогда
		ПустаяСтатья = ПредопределенноеЗначение("ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка");
	ИначеЕсли ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратИзЭксплуатации")
		ИЛИ ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ОприходованиеЗаСчетДоходов") Тогда
	...
	Если Строка.СтатьяРасходовДоходов = Неопределено Тогда
		Строка.СтатьяРасходовДоходов = ПустаяСтатья;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораОпераций()
	...
	СписокВыбора.Добавить(Перечисления.ХозяйственныеОперации.ОприходованиеЗаСчетРасходов);
	//-- НЕ УТ
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для Каждого СтрокаТЧ Из ТаблицаТоваров Цикл
		...
		ПриДобавленииСтрокиТоваров(ТекущаяСтрока, Объект.ХозяйственнаяОперация);
...
Процедура ОбработкаВыбораПодборТМЦВЭксплуатацииНаСервере(МассивВыбранных)
	...
	Для Каждого ЭлементМассива Из МассивВыбранных Цикл
		...
		ПриДобавленииСтрокиТоваров(НоваяСтрока, Объект.ХозяйственнаяОперация);
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	Для Каждого СтрокаИсточник Из ТаблицаТовары Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ПриДобавленииСтрокиТоваров(СтрокаТЧ, Объект.ХозяйственнаяОперация);
...
Процедура УстановитьПараметрыВыбораСтатьи()
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратИзЭксплуатации
		ИЛИ Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОприходованиеЗаСчетДоходов Тогда
