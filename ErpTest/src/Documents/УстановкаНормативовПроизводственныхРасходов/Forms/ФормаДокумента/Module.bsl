...
Процедура ВеличиныБазРаспределенияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ОбщегоНазначенияУТКлиент.КешироватьТекущуюСтроку(ЭтотОбъект, "ВеличиныБазРаспределения");
...
Процедура ВеличиныБазРаспределенияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	КешСтроки = ?(НоваяСтрока, Неопределено, ВеличиныБазРаспределенияКешТекущейСтроки);
	Если Не НоваяСтрока И Не КешСтроки = Неопределено 
		И Не ТекущиеДанные.ПравилоРаспределения = КешСтроки.ПравилоРаспределения Тогда
		
		ВыбранноеПравило = ТекущиеДанные.ПравилоРаспределения;
		ТекущиеДанные.ПравилоРаспределения = КешСтроки.ПравилоРаспределения;
		...
		Если ПараметрыУдаления.ЕстьСтрокиКУдалению Тогда
			...
			ДополнительныеПараметры.Вставить("ПравилоКУдалению", КешСтроки.ПравилоРаспределения);
			...
			ПоказатьВопрос(ОписаниеОповещения, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОповещения, КешСтроки.НаименованиеПравила),
...
#Область ТекстОтнесенияРасходов
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", "на");
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	УстановитьЗаголовки();
	ЗаполнитьЗависимыеРеквизиты();
	ОбщегоНазначенияУТ.ИнициализироватьКешТекущейСтроки(ЭтотОбъект, "ВеличиныБазРаспределения");
...
Процедура ЗаполнитьСписокВыбораПравил()
	...
	СписокВыбора.Добавить("ДругиеПравила", 
		Новый ФорматированнаяСтрока(НСтр("ru = 'Другие правила...'"),,,, "заглушка"));
...
Процедура ПриИзмененииСтатьиРасходовСервер(Источник)
	...
	ТекущаяСтрока = Объект[Источник].НайтиПоИдентификатору(Элементы[Источник].ТекущаяСтрока);
...
Процедура ЗаполнитьПоДаннымБазыРаспределения(СтрокаЗаполнения, Источник)
	...
	ПравилоРаспределения = ПравилоРаспределенияСтатьиРасходов(СтрокаРасхода.СтатьяРасходов);
...
Функция ПравилоРаспределенияСтатьиРасходов(СтатьяРасходов)
	
	ПростыеБазыРаспределения = ПредставленияПростыхБаз();
	...
	ПравилоРаспределенияСтатьи = Справочники.ПравилаРаспределенияРасходов.ПустаяСсылка();
	БазаРаспределенияСтатьи = Перечисления.ТипыБазыРаспределенияРасходов.ПустаяСсылка();
		
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПравилаРаспределенияРасходов.Ссылка КАК ПравилоРаспределения,
		|	ЕСТЬNULL(ПравилаРаспределенияРасходов.БазаРаспределенияПоПартиям, ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ПустаяСсылка)) КАК БазаРаспределенияПоПартиям
		|ПОМЕСТИТЬ РеквизитыСтатьиРасходов
		|ИЗ
		|	ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиРасходов
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПравилаРаспределенияРасходов КАК ПравилаРаспределенияРасходов
		|		ПО СтатьиРасходов.ПравилоРаспределенияРасходов = ПравилаРаспределенияРасходов.Ссылка
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РеквизитыСтатьиРасходов.ПравилоРаспределения КАК ПравилоРаспределения,
		|	РеквизитыСтатьиРасходов.БазаРаспределенияПоПартиям КАК БазаРаспределенияПоПартиям
		|ИЗ
		|	РеквизитыСтатьиРасходов КАК РеквизитыСтатьиРасходов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	ИЗ
		|		Справочник.ПравилаРаспределенияРасходов.ОтборПоГруппамПродукции КАК Таблица
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РеквизитыСтатьиРасходов КАК РеквизитыСтатьиРасходов
		|			ПО Таблица.Ссылка = РеквизитыСтатьиРасходов.ПравилоРаспределения
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		Таблица.Ссылка
		|	ИЗ
		|		Справочник.ПравилаРаспределенияРасходов.ОтборПоМатериалам КАК Таблица
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РеквизитыСтатьиРасходов КАК РеквизитыСтатьиРасходов
		|			ПО Таблица.Ссылка = РеквизитыСтатьиРасходов.ПравилоРаспределения
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|		Таблица.Ссылка
		|	ИЗ
		|		Справочник.ПравилаРаспределенияРасходов.ОтборПоВидамРабот КАК Таблица
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РеквизитыСтатьиРасходов КАК РеквизитыСтатьиРасходов
		|			ПО Таблица.Ссылка = РеквизитыСтатьиРасходов.ПравилоРаспределения
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|		Таблица.Ссылка
		|	ИЗ
		|		Справочник.ПравилаРаспределенияРасходов.ОтборПоПродукции КАК Таблица
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РеквизитыСтатьиРасходов КАК РеквизитыСтатьиРасходов
		|			ПО Таблица.Ссылка = РеквизитыСтатьиРасходов.ПравилоРаспределения) КАК ВложенныйЗапрос";
	
	Запрос.УстановитьПараметр("Ссылка", СтатьяРасходов);
	
	Результат = Запрос.ВыполнитьПакет();
	
	// Данные статьи расходов.
	Выборка = Результат[Результат.Количество() - 2].Выбрать();
	Если Выборка.Следующий() Тогда
	
		ПравилоРаспределенияСтатьи = Выборка.ПравилоРаспределения;
		БазаРаспределенияСтатьи = Выборка.БазаРаспределенияПоПартиям;
	
	КонецЕсли;
	
	ПростаяБазаВПравиле = Результат[Результат.Количество() - 1].Пустой();
	
	Если Не ПростаяБазаВПравиле Тогда
		ПравилоРаспределения = ПравилоРаспределенияСтатьи;
	Иначе
		Если Не БазыСУчетомБудущихВыпусков.Получить(БазаРаспределенияСтатьи) = Неопределено Тогда
			ПравилоРаспределения = БазыСУчетомБудущихВыпусков.Получить(БазаРаспределенияСтатьи);
		Иначе
			ПравилоРаспределения = БазаРаспределенияСтатьи;
		КонецЕсли;
	КонецЕсли;	
...
Функция НаименованиеПравила(Правило)
	...
	НаименованиеПравила = СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Правило, "ПредставлениеПравила"));
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПравилаРаспределенияРасходовОтборПоГруппамПродукции.ГруппаПродукции.Наименование КАК ГруппаПродукции
		|ИЗ
		|	Справочник.ПравилаРаспределенияРасходов.ОтборПоГруппамПродукции КАК ПравилаРаспределенияРасходовОтборПоГруппамПродукции
		|ГДЕ
		|	ПравилаРаспределенияРасходовОтборПоГруппамПродукции.Ссылка = &Правило";
	
	Запрос.УстановитьПараметр("Правило", Правило);
	
	Результат = Запрос.Выполнить();
	ГруппыПродукции = Результат.Выгрузить().ВыгрузитьКолонку("ГруппаПродукции");
	
	Если ГруппыПродукции.Количество() = 0 Тогда
		Возврат НаименованиеПравила;
	КонецЕсли;
	
	ПредставлениеГруппПродукции = Справочники.ПравилаРаспределенияРасходов.ПредставлениеОтобранныхПозиций(ГруппыПродукции, 
			НСтр("ru = 'группа, группы, групп'"));
			
	Если ПустаяСтрока(СокрЛП(Сред(НаименованиеПравила, СтрНайти(НаименованиеПравила, ":") + 1))) Тогда
		НаименованиеПравила = Лев(НаименованиеПравила, СтрДлина(НаименованиеПравила) - 1);
	КонецЕсли;
	
	ШаблонНаименования = НСтр("ru = '%1 с отбором по группам продукции: %2'");
	НаименованиеПравила = СтрШаблон(ШаблонНаименования, НаименованиеПравила, ПредставлениеГруппПродукции);
...
Процедура ЗаполнитьНаСервере(ОбъектыЗаполнения, КоличествоМесяцев)
	...
	ИначеЕсли ОбъектыЗаполнения = "ПостоянныеРасходы" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, 
			"&УсловиеОтбораСтатей", 
