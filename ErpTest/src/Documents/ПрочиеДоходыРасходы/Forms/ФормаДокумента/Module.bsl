...
Процедура ДоходыПредставлениеОтраженияВРеглУчетеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыНастройки.Вставить("ИмяЭлементаКоманды", "НастроитьОтражениеВРеглУчете");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НастройкаОтраженияВРеглУчете", РеглУчетКлиент.НастройкаОтраженияВРеглУчете(Элементы.ПрочиеДоходы.ТекущиеДанные, ПараметрыНастройки.СоответствиеИменРеквизитов));
	
	ПараметрыОповедения = Новый Структура("ИмяТабличнойЧасти", "ПрочиеДоходы");
	
	ОписаниеОповещения = Новый ОписаниеОповещения("НастройкаОтраженияВРеглУчетеЗавершение", ЭтотОбъект, ПараметрыОповедения);
...
Процедура РасходыПредставлениеОтраженияВРеглУчетеНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НастройкаОтраженияВРеглУчете", РеглУчетКлиент.НастройкаОтраженияВРеглУчете(Элементы.ПрочиеРасходы.ТекущиеДанные, ПараметрыНастройки.СоответствиеИменРеквизитов));
	
	ПараметрыОповедения = Новый Структура("ИмяТабличнойЧасти", "ПрочиеРасходы");
...
Процедура НастройкаОтраженияВРеглУчетеЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Строка = Элементы[ПараметрыОповещения.ИмяТабличнойЧасти].ТекущиеДанные;
	Строка.СчетУчета = Результат.СчетУчета;
	Строка.Субконто1 = Результат.Субконто1;
	Строка.Субконто2 = Результат.Субконто2;
	Строка.Субконто3 = Результат.Субконто3;
	Строка.ПредставлениеОтраженияВРеглУчете = РеглУчетВызовСервера.ПредставлениеОтраженияВРеглУчете(Результат);
	ПредставлениеОтраженияВРеглУчете = Строка.ПредставлениеОтраженияВРеглУчете;
	
	Если Объект[ПараметрыОповещения.ИмяТабличнойЧасти].Количество() > 1
	 И (ПереключательРасшифровкиДоходыКорЧасть = 0 И ПараметрыОповещения.ИмяТабличнойЧасти = "ПрочиеДоходы"
	 ИЛИ ПереключательРасшифровкиРасходыКорЧасть = 0 И ПараметрыОповещения.ИмяТабличнойЧасти = "ПрочиеРасходы") Тогда
		Реквизиты = Новый Структура("СчетУчета, Субконто1, Субконто2, Субконто3, ПредставлениеОтраженияВРеглУчете");
		ЗаполнитьЗначенияСвойств(Реквизиты, Строка);
		ЗаполнитьЗначенияРеквизитовВТабличнойЧасти(ПараметрыОповещения.ИмяТабличнойЧасти, Реквизиты);
	КонецЕсли;
	//-- НЕ УТ
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.КорСтатьяРасходов.Имя);
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.КорСтатьяРасходовБезРазбиения.Имя);
	...
	Если Не ЭтоРасходы Тогда
		...
		Если ПереключательРасшифровкиДоходыКорЧасть = 0
		 И Элементы.ПрочиеДоходы.ТекущаяСтрока = Неопределено Тогда
			...
			//++ НЕ УТ
			ПредставлениеОтраженияВРеглУчете = РеглУчетВызовСервера.ПредставлениеОтраженияВРеглУчете(Объект.ПрочиеДоходы[0]);
			//-- НЕ УТ
		КонецЕсли;
	...
	Иначе
		...
		Если ПереключательРасшифровкиРасходыКорЧасть = 0
		 И Элементы.ПрочиеРасходы.ТекущаяСтрока = Неопределено Тогда
			...
			//++ НЕ УТ
			ПредставлениеОтраженияВРеглУчете = РеглУчетВызовСервера.ПредставлениеОтраженияВРеглУчете(Объект.ПрочиеРасходы[0]);
			//-- НЕ УТ
		КонецЕсли;
...
Процедура УправлениеФормойСервер()
	...
	Элементы.ДоходыВременнаяРазница.Видимость   = ПоддержкаПБУ18;
	Элементы.ПрочиеРасходыВременнаяРазница.Видимость  = ПоддержкаПБУ18;
	Элементы.РасходыВременнаяРазница.Видимость  = ПоддержкаПБУ18;
	//-- НЕ УТ
