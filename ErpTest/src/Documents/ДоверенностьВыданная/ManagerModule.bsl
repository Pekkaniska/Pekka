...
// Обработчик обновления УТ 2.4.4
...
Функция ТекстЗапросаРаспоряженийНаДС() Экспорт
	...
	Если ЧтениеРасчетыСКлиентами Тогда
		...
		Если ЧтениеЗаказДавальца Тогда
			ТекстЗапроса = ТекстЗапроса + СтрШаблон(ТекстШаблона,
				ТекстХО, ТекстХОПредставление, "ЗаказДавальца", ТекстРасчеты, ТекстИзмерениеРасчеты);
		КонецЕсли;
		// Отчет давальцу.
		Если ЧтениеОтчетДавальцу Тогда
			ТекстЗапроса = ТекстЗапроса + СтрШаблон(ТекстШаблона,
				ТекстХО, ТекстХОПредставление, "ОтчетДавальцу", ТекстРасчеты, ТекстИзмерениеРасчеты);
		КонецЕсли;
		//-- НЕ УТКА
