...
Функция УточнитьПериодыЗаполненияПоСотрудникам(МенеджерВременныхТаблиц, ПериодРегистрации, ИсключаемыеРегистраторы = Неопределено)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СотрудникиСУчтоненнымиПериодами", ПериодыЗарегистрированныхДанныхУчетаВремени.ВыгрузитьКолонку("Сотрудник"));
	...
	|ГДЕ
	|	НЕ Сотрудники.Сотрудник В (&СотрудникиСУчтоненнымиПериодами)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
