...
Процедура ОССуммаПриИзменении(Элемент)
	...
	//++ RARUS_IUC 20171220
	Если ЗначениеЗаполнено(Объект.пкКурсДокумента) Тогда	
		ТекущиеДанные.СуммаНУ = (ТекущиеДанные.СуммаСНДС - ТекущиеДанные.СуммаНДС) * Объект.пкКурсДокумента;
	Иначе	
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ПартнерыИКонтрагенты.ЗаголовокЭлементаПартнерВЗависимостиОтХозяйственнойОперации(
		ЭтотОбъект, "Партнер", Перечисления.ХозяйственныеОперации.ПоступлениеПредметовЛизинга);
