...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.пкКурсДокумента,
...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	Если Реквизиты <> Неопределено Тогда
		...
		//Рарус Владимир Подрезов 20.03.2017
		Если ЗначениеЗаполнено(Реквизиты.пкКурсДокумента) Тогда
			ВалютаУправленческогоУчета = Константы.ВалютаУправленческогоУчета.Получить();
			
			КурсТекущейВалюты	= Реквизиты.пкКурсДокумента;
			КурсНовойВалюты		= РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаУправленческогоУчета, Реквизиты.Период);
			Если КурсНовойВалюты.Курс <> 0 Тогда
				КоэффициентПересчетаВВалютуУпр = (КурсТекущейВалюты * КурсНовойВалюты.Кратность) / (КурсНовойВалюты.Курс);
			Иначе
				КоэффициентПересчетаВВалютуУпр = 0;
			КонецЕсли;
			...
			КурсНовойВалюты		= РаботаСКурсамиВалют.ПолучитьКурсВалюты(ВалютаУправленческогоУчета, Реквизиты.Период);
			Если КурсНовойВалюты.Курс <> 0 Тогда
				КоэффициентПересчетаВВалютуРегл = (КурсТекущейВалюты * КурсНовойВалюты.Кратность) / (КурсНовойВалюты.Курс);
			Иначе
				КоэффициентПересчетаВВалютуРегл = 0;
			КонецЕсли;
			ЗначенияПараметровПроведения.Вставить("КоэффициентПересчетаВВалютуРегл",       КоэффициентПересчетаВВалютуРегл);
		Иначе
		//Типовым образом
		//Рарус Владимир Подрезов Конец
...
Процедура ИнициализироватьКлючиАналитикиУчетаПоПартнерам(Запрос)
	...
	|		И ДанныеДокумента.ДоговорЛизинга.НаправлениеДеятельности = РегистрАналитикаУчетаПоПартнерам.НаправлениеДеятельности
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка
...
Возврат "
	...
	//|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаДт,
	//|	ВЫБОР КОГДА Операция.ВалютаВзаиморасчетов = Константа.ВалютаРегламентированногоУчета ТОГДА
	...
	//|	Выбор Когда Операция.ВалютаВзаиморасчетов Строки.СуммаНДСВзаиморасчетов КАК ВалютнаяСуммаДт,
	//-- Рарус Лимаренко 19.07.2018	
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеДокумента = Метаданные.Документы.ПоступлениеПредметовЛизинга;
