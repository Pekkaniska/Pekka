...
Процедура ПередЗаписьюПредложитьПодобратьМногооборотнуюТаруЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	РеглУчетКлиент.ПроверитьВыборОСиНМА(
		Объект,
		Новый ОписаниеОповещения("ПередЗаписьюОбработатьЗаписьОбъектаВФорме", ЭтотОбъект, ДополнительныеПараметры),
...
Процедура ТоварыКатегорияЭксплуатацииПриИзменении(Элемент)
	...
	//++ НЕ УТ
	Если ЗначениеЗаполнено(ТекущаяСтрока.КатегорияЭксплуатации) И ТекущаяСтрока.ИнвентарныйУчет Тогда
