...
Функция ПараметрыОткрытияФормыРабочегоМеста(ХозяйственнаяОперация = Неопределено) Экспорт
	...
	Иначе
		ОтборХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию);
		ОтборХозяйственныеОперации.Добавить(Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию);
	КонецЕсли;
...
//     * ЗаказНаВнутреннееПотребление - ДокументСсылка.ВнутреннееПотреблениеТоваров,
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ДопОтборы = Неопределено) Экспорт
	...
	Если ДопОтборы <> Неопределено И ДопОтборы.Свойство("Характеристика_НеПроводитьПоБУ") Тогда
		Отбор.Вставить("Характеристика_НеПроводитьПоБУ", "Характеристика_НеПроводитьПоБУ");
	КонецЕсли;
...
//-- НЕ УТКА
	...
	//Рарус Владимир Подрезов 04.07.2017
	Если ДопОтборы <> Неопределено И ДопОтборы.Свойство("Характеристика_НеПроводитьПоБУ") Тогда
		Запрос.УстановитьПараметр("Характеристика_НеПроводитьПоБУ", ДопОтборы.Характеристика_НеПроводитьПоБУ);
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"Характеристика.пкНеПроводитьПоБУ В(&Характеристика_НеПроводитьПоБУ)","ВЫБОР
		|КОГДА Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	ТОГДА ИСТИНА
		|ИНАЧЕ Характеристика.пкНеПроводитьПоБУ В (&Характеристика_НеПроводитьПоБУ)
		|КОНЕЦ");
	КонецЕсли;
	//Рарус Владимир Подрезов Конец
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ВнутреннееПотреблениеТоваровЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ВнутреннееПотреблениеТоваровЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			ИЛИ НЕ Строки.СтатьяРасходов.ПринятиеКНалоговомуУчету))
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаВЭксплуатациюБУНУ)
	//-- НЕ УТ
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаВЭксплуатацию)
	//++ НЕ УТ
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ХозОперации[0] = Перечисления.ХозяйственныеОперации.СписаниеТоваровПоТребованию;
	ХозОперации[1] = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию;
