...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВЭксплуатацию Тогда
		...
		СвойстваКатегорий = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(
			Товары.ВыгрузитьКолонку("КатегорияЭксплуатации"),
			"ИнвентарныйУчет");
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ИнвентаризацияПродукцииВЕТИС") Тогда
		...
		ИнтеграцияВЕТИСУТ.ЗаполнитьВнутреннееПотреблениеТоваровНаОснованииИнвентаризацииПродукцииВЕТИС(ЭтотОбъект, ДанныеЗаполнения,, СтандартнаяОбработка);
...
//++ Рарус Лимаренко 16.05.2018
	ДопОтборы = Новый Структура();
	ДопОтборы.Вставить("Склад", ПараметрыЗаполнения.Склад);
//-- Рарус Лимаренко 16.05.2018
	
//+++rarus-spb_zlov 27.12.2016 
	Если СтруктураЗаполнения.Свойство("НеПроводитьПоБУ") Тогда
		ДопОтборы.Вставить("Характеристика_НеПроводитьПоБУ", СтруктураЗаполнения.НеПроводитьПоБУ);
    КонецЕсли;    
//---rarus-spb_zlov 27.12.2016
...
Процедура ПроверитьЗаполнениеИнвентарныхНомеров(НепроверяемыеРеквизиты, Отказ)
	...
	Запрос.УстановитьПараметр("Таблица", Товары.Выгрузить(, "НомерСтроки, КатегорияЭксплуатации, ИнвентарныйНомер"));
