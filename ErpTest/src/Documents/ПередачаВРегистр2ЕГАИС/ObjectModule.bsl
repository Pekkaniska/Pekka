...
Процедура ЗаполнитьНаОснованииТТНВходящейЕГАИС(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеКоэффициентПересчетаНеупакованнойПродукции",
		ИнтеграцияЕГАИСПереопределяемый.ПолеКоэффициентПересчетаНеупакованнойПродукции());
...
Процедура ЗаполнитьПоВыбраннымСтрокамОстатков(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеКоэффициентПересчетаНеупакованнойПродукции", 
		ИнтеграцияЕГАИСПереопределяемый.ПолеКоэффициентПересчетаНеупакованнойПродукции("СопоставленыеПозиции"));
