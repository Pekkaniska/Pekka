	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПартионныйУчетВерсии22(НачалоМесяца(ДанныеПечати.Дата)) Тогда
			...
			Иначе
				...
				ЗапросЗатрат.Текст =
				"ВЫБРАТЬ
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК Товар,
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.НаименованиеПолное, """") КАК ТоварНаименование,
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.Код, """") КАК Код,
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.Артикул, """") КАК Артикул,
				|	ЕСТЬNULL(АналитикаЗатрат.Характеристика.НаименованиеПолное, """") КАК Характеристика,
				|	СУММА(ЕСТЬNULL(ЗатратыНаПродукцию.Количество, 0)) КАК Количество,
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.ЕдиницаИзмерения.Наименование, """") КАК ПредставлениеБазовойЕдиницыИзмерения
				|ИЗ
				...
				|		ПО (ЗатратыНаПродукцию.АналитикаУчетаНоменклатуры = АналитикаЗатрат.КлючАналитики)
				|ГДЕ
				...
				|
				|СГРУППИРОВАТЬ ПО
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.Код, """"),
				|	ЕСТЬNULL(АналитикаЗатрат.Характеристика.НаименованиеПолное, """"),
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.НаименованиеПолное, """"),
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)),
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.ЕдиницаИзмерения.Наименование, """"),
				|	ЕСТЬNULL(АналитикаЗатрат.Номенклатура.Артикул, """")
				|
				|ИМЕЮЩИЕ
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ОтчетДавальцу КАК ОтчетДавальцу
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу.ЭтапыГрафикаОплаты КАК ЭтапГрафикаОплаты
	|			ПО ОтчетДавальцу.Ссылка = ЭтапГрафикаОплаты.Ссылка
	|СГРУППИРОВАТЬ ПО
	|	ОтчетДавальцу.Ссылка
	|ИМЕЮЩИЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ОтчетДавальцу";
	МетаданныеДокумента = Метаданные.Документы.ОтчетДавальцу;
			...
			Если ДокументОбъект.ЭтапыГрафикаОплаты.Количество() = 0 Тогда
				...
				Если ДокументОбъект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным Тогда
					...
					Если ДокументОбъект.Валюта = ДокументОбъект.ВалютаВзаиморасчетов Тогда
						СуммаВзаиморасчетов = ЦенообразованиеКлиентСервер.ПолучитьСуммуДокумента(ДокументОбъект.Продукция, ДокументОбъект.ЦенаВключаетНДС);
					Иначе
						СуммаВзаиморасчетов = ДокументОбъект.Продукция.Итог("СуммаВзаиморасчетов");
					КонецЕсли;
				...
				Иначе
					...
					ПараметрыЗаполненияЭтапов.ТабличнаяЧасть = ДокументОбъект.Продукция.Выгрузить();
