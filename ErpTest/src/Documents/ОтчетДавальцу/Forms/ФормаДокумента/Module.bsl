...
Процедура НадписьЭтапыОплатыНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ИмяПоляЗаказ",                            "ЗаказДавальца");
	ПараметрыФормы.Вставить("УпрощенныйРежим",                         НЕ ПараметрыФормы.НакладнаяПоЗаказам
		ИЛИ ИспользоватьУпрощеннуюСхемуОплатыВПродажах И НЕ ИспользоватьОтчетДавальцуПоНесколькимЗаказам);
...
Процедура ПриИзмененииПартнераСервер()
	...
	ОбновитьФорму = Ложь;
	ВзаиморасчетыСервер.ЗаполнитьПорядокРасчетовВФорме(ЭтаФорма, ОтчетПоЗаказу, , ОбновитьФорму);
...
Процедура АктуализироватьНалогооблажениеНДС(Заполнить = Истина, КэшированныеЗначения = Неопределено)
	...
	ПараметрыЗаполнения = Документы.ОтчетДавальцу.ПараметрыЗаполненияНалогооблаженияНДС(Объект);
...
Функция ПоместитьТоварыВоВременноеХранилищеНаСервере()
	...
	Возврат ПоместитьВоВременноеХранилище(Объект.Продукция.Выгрузить(,"КодСтроки, Номенклатура, ЗаказДавальца, СуммаСНДС, СуммаВзаиморасчетов"),
