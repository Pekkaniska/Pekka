...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|		И НЕ(Реквизиты.БазаРаспределенияПоПартиям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ПустаяСсылка)
		|					И Реквизиты.БазаРаспределенияПоПодразделениям = ЗНАЧЕНИЕ(Перечисление.ТипыБазыРаспределенияРасходов.ПустаяСсылка))
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|	ИЗ
		|		Документ.РаспределениеПрочихЗатрат.ПартииПроизводства КАК ТаблицаТоваров
		|	ГДЕ
		|		ТаблицаТоваров.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
		|		И НЕ ТаблицаТоваров.УдалитьПартияПроизводства = НЕОПРЕДЕЛЕНО) КАК ОбъектыКОбработке";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмя = "Документ.РаспределениеПрочихЗатрат";
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуСсылокДляОбработки(
		Параметры.Очередь, ПолноеИмя, МенеджерВременныхТаблиц);
	...
	Запрос.Текст =
		...
		|	""ПеренестиПартию"" КАК Действие
		|ИЗ
		|	ВТДляОбработкиРаспределениеПрочихЗатрат КАК ОбъектыДляОбработки
		...
		|				ИСТИНА
		|			ИЗ
		|				Документ.РаспределениеПрочихЗатрат.ПартииПроизводства КАК ТаблицаПартий
		|			ГДЕ
		...
		|		ПО ОбъектыДляОбработки.Ссылка = Реквизиты.Ссылка
		|ГДЕ
		|	НЕ Реквизиты.УдалитьВариантРаспределения = ЗНАЧЕНИЕ(Перечисление.СпособыРаспределенияСтатейРасходов.ПустаяСсылка)
		|	И НЕ(Реквизиты.РаспределятьПоПартиям
		|				ИЛИ Реквизиты.РаспределятьНаСтатьи
		|				ИЛИ Реквизиты.ОставитьВНЗП)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			ВыборкаДетальная = ВыборкаДокументов.Выбрать();
			Пока ВыборкаДетальная.Следующий() Цикл
				
				Если ДокументОбъект = Неопределено Тогда
					ДокументОбъект = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(
						ВыборкаДетальная.Ссылка, ВыборкаДетальная.ВерсияДанных, Параметры.Очередь);
				КонецЕсли;
				...
				Если Не ДокументОбъект.ВерсияДанных = ВыборкаДетальная.ВерсияДанных Тогда
					
					// в процессе выполнения запросов объект изменился.
					ДокументОбъект = Неопределено;
					Прервать;
					
				КонецЕсли;
				
				Если ВыборкаДетальная.Действие = "ОбновитьРеквизиты" Тогда
					...
					Если ДокументОбъект.УдалитьВариантРаспределения = Перечисления.СпособыРаспределенияСтатейРасходов.НаДругиеСтатьиРасходов
						...
						Если ДокументОбъект.УдалитьВариантРаспределения = Перечисления.СпособыРаспределенияСтатейРасходов.НаДругиеСтатьиРасходов Тогда
							Продолжить;
						КонецЕсли;
				...
				ИначеЕсли ВыборкаДетальная.Действие = "ПеренестиПартию" Тогда
					ГруппыЗатратСервер.ПеренестиПартиюПроизводства(ДокументОбъект.ПартииПроизводства);
				ИначеЕсли ВыборкаДетальная.Действие = "ИсправитьБазыРаспределения" Тогда
					
					ДокументОбъект.БазаРаспределенияПоПартиям 			= Перечисления.ТипыБазыРаспределенияРасходов.ПустаяСсылка();
					ДокументОбъект.БазаРаспределенияПоПодразделениям 	= Перечисления.ТипыБазыРаспределенияРасходов.ПустаяСсылка();
					
				Иначе
					
					ТекстОшибки = НСтр("ru = 'Некорректная обработка данных. Объект не подпадает ни под одно условие.'");
					ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ТекстОшибки, ВыборкаДокументов.Ссылка);
	...
	Параметры.ОбработкаЗавершена = Не ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Параметры.Очередь, ПолноеИмя);
