		...
		Если Объект.Проведен Тогда
			Если ДокументБылПерезаписан Тогда
				...
				Если Объект.РаспределятьПоПартиям
					Или Объект.РаспределятьПоПодразделениям
					Или Объект.РаспределятьНаСтатьи
					Или Объект.ОставитьВНЗП Тогда
			...
			КонецЕсли;
		Иначе
			ПараметрыПередачи.Вставить("Ссылка", Неопределено);
			ПараметрыПередачи.Вставить("Состояние", 
				ПредопределенноеЗначение("Перечисление.СостоянияРаспределенияРасходов.ТребуетсяНастройкаРаспределения"));
		КонецЕсли;
...
Процедура УстановитьВидимостьСтраниц(Форма, ИмяРеквизита = Неопределено)
	...
	Если ИмяРеквизита = Неопределено Или ИмяРеквизита = "ПартииУказаныВручную" Тогда
		...
		Элементы.СтраницаПравила.Видимость = Не Объект.ПартииУказаныВручную;
...
Процедура УстановитьВидимостьЭлементов(Форма, ИмяРеквизита = Неопределено)
	...
	Если ИмяРеквизита = "НаправлениеРаспределения" Или ИмяРеквизита = Неопределено Тогда
		Если Объект.НаправлениеРаспределения = ПредопределенноеЗначение("Перечисление.НаправлениеРаспределенияПоПодразделениям.Указанные") Тогда
			Элементы.ПредставлениеУказанныхПодразделений.Видимость = Истина;
		Иначе
			Элементы.ПредставлениеУказанныхПодразделений.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьДоступностьЭлементов(Форма, ИмяРеквизита = Неопределено)
	...
	Если ИмяРеквизита = Неопределено Или ИмяРеквизита = "РаспределятьПоПартиям" Тогда
		...
		Элементы.СтраницаПравила.Доступность = Объект.РаспределятьПоПартиям;
...
Функция НедоступныеЭлементыПоФО(Период)
	...
	ПартионныйУчетВерсии22 = УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПартионныйУчетВерсии22(Период);
	...
	//++ НЕ УТКА
		И Не (ПартионныйУчетВерсии22 И НастройкиПроизводства.ИспользуетсяПроизводство21
...
Процедура ОпределитьВидимостьИСуммуРасходовПоСтатье()
	...
	Элементы.ГруппаРегл.Видимость = (Не ПоступилоРегл = 0);
	Элементы.ГруппаВР.Видимость = (Не ПоступилоВременнаяРазница = 0);
