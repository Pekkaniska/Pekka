...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "БланкФорма2НДФЛ";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	КонецЕсли;
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "БланкФорма2НДФЛ") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "БланкФорма2НДФЛ", "Форма 2НДФЛ", СформироватьПечатнуюФорму2НДФЛ(МассивОбъектов, ОбъектыПечати, Ложь));
	КонецЕсли;
	...
	Для Каждого Ссылка Из МассивОбъектов Цикл
		...
		Для Каждого ДанныеСотрудника Из СтрокиДанныхПоСотрудникам Цикл
			...
			Для Каждого СтавкаНДФЛ Из Перечисления.НДФЛСтавки Цикл
				...
				Если ЗначениеЗаполнено(ДанныеТекущегоДокумента.ФормаРеорганизации) Тогда
					ВывестиДанныеПоБуквенно(ДанныеТекущегоДокумента.ФормаРеорганизации, ТитульныйЛист, "ФормаР", 1);
				...
				ВывестиДанныеПоБуквенно(СоответствиеКодовСтран[ДанныеСотрудника.Гражданство], ТитульныйЛист, "Страна", 3);
				...
				НеобходимоеКоличествоСтраниц = Макс(Уведомления.Количество(), Окр(ВычетыПоВидам.Количество()/6,0) + ?(ВычетыПоВидам.Количество()%6, 1, 0));
				Для НомерСтраницыСотрудника = 1 По НеобходимоеКоличествоСтраниц Цикл
					...
					Для Индекс = 1 По 6 Цикл
						ИндексСтрокиВычетов = Индекс * НомерСтраницыСотрудника;
			...
			ДоходыСотрудника.Сортировать("НомерСправки, Ставка, МесяцНалоговогоПериода, КодДохода, КодВычета Убыв");
			...
			Для Каждого ДоходСотрудника Из ДоходыСотрудника Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ДоходыПоМесяцамИКодам.Сортировать("НомерСправки, Ставка, МесяцНалоговогоПериода, КодДохода, КодВычета");
...
Процедура ВывестиНаименованиеНалоговогоАгента(НазваниеАгента, ТабличныйДокумент, ИмяПоказателя, КоличествоСтрок, КоличествоЯчеек)
	...
	Иначе
		...
		МассивПодстрок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
			ЗарплатаКадры.РазбитьСтрокуНаПодСтроки(НазваниеАгента, МассивДлинПодстрок),
				"  ",
				Истина,
				Истина);
		Если МассивПодстрок.Количество() > КоличествоСтрок Тогда
			МассивПодстрок = Новый Массив;
			Для ИндексСтроки = 1 По КоличествоСтрок Цикл
				ЧастьНаименования = Сред(НазваниеАгента, КоличествоЯчеек*(ИндексСтроки - 1) + 1, КоличествоЯчеек);
