...
Процедура ПроверитьДанныеДокумента(Отказ) Экспорт
	...
	Если ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛЗаРеорганизованнуюОрганизацию Или ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетностьЗаРеорганизованнуюОрганизацию Тогда
		Если Не ЗначениеЗаполнено(ФормаРеорганизации) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.ФормаРеорганизации", , Отказ);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(НаименованиеРеорганизованнойОрганизации) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.НаименованиеРеорганизованнойОрганизации", , Отказ);
		КонецЕсли;
		Если ФормаРеорганизации <> "0" Тогда
			Если Не ЗначениеЗаполнено(ИННРеорганизованнойОрганизации) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.ИННРеорганизованнойОрганизации", , Отказ);
			...
			Если Не ЗначениеЗаполнено(КППРеорганизованнойОрганизации) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.КППРеорганизованнойОрганизации", , Отказ);
	...
	Если ТипПодписанта <> "2" И Не ЗначениеЗаполнено(СправкуПодписал) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.СправкуПодписал", , Отказ);
	КонецЕсли;
...
Функция ТекстВыходногоФайла2018(ИмяФайла)
	...
	Если ОрганизацияЭтоФизЛицо Тогда
		...
		ФорматСправка.СвНА.Значение.СвНАФЛ.Значение = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(НаборЗаписейОтправитель);
...
Функция ТекстВыходногоФайла2019(ИмяФайла)
	...
	НаборЗаписейСведенияОДоходах = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматДокумент.НДФЛ__2.Значение.СведДох.Значение);
	ФорматСведенияОВычетах = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(НаборЗаписейСведенияОДоходах.НалВычССИ.Значение.ПредВычССИ);
	СтруктураДанныхСправки.Удалить("СведДох");
