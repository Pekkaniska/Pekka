...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого Тип Из Метаданные.РегистрыБухгалтерии.Хозрасчетный.СтандартныеРеквизиты.Регистратор.Тип.Типы() Цикл
		...
		Если Не СоответствиеДоступныхТипов.Получить(МетаданныеДокумента.ПолноеИмя()) = Ложь Тогда
...
Процедура ОбработатьНачалоВыбораСубконто(ДтКт, Элемент, СтандартнаяОбработка)
	...
	ТекущиеДанные      = Элементы.Хозрасчетный.ТекущиеДанные;
	ПараметрыДокумента = ПараметрыВыбораСубконто(ЭтотОбъект, ТекущиеДанные, "Субконто" + ДтКт + "%Индекс%", "Счет" + ДтКт);
...
Процедура НастроитьОтображениеТаблицыПроводок()
	...
	Если ПолучитьФункциональнуюОпцию("ПоддержкаПБУ18", ПолучитьПараметрыФункциональныхОпцийФормы()) Тогда
		...
		Элементы.ХозрасчетныйСуммаНУКт.Заголовок   = СтрШаблон(НСтр("ru = 'Сумма Кт (%1)'"), ВалютаРУ);
	Иначе
		Элементы.ХозрасчетныйСуммаНУДт.Заголовок   = СтрШаблон(НСтр("ru = 'Сумма Дт НУ (%1)'"), ВалютаРУ);
		Элементы.ХозрасчетныйСуммаНУКт.Заголовок   = СтрШаблон(НСтр("ru = 'Сумма Кт НУ (%1)'"), ВалютаРУ);
	КонецЕсли;
