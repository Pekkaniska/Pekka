...
Процедура ЗаполнитьЭтапыГрафикаОплаты() Экспорт
	...
	Если СуммаДокумента + СуммаТары > 0 Тогда
		...
		Если НЕ РеализацияПоЗаказам
			...
			ИЛИ ЗначениеЗаполнено(ГрафикИсполненияДоговора) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		//Для ХО Реализация только регл. в ТЧ РасшифровкаПлатежа валюта взаиморасчетов всегда была равна валюте документа,
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение 
		...
		ИначеЕсли ЭтапыГрафикаОплаты.Итог("СуммаПлатежа") + ЭтапыГрафикаОплаты.Итог("СуммаЗалогаЗаТару") <> СуммаДокумента
			...
			Если ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным ИЛИ НЕ РеализацияПоЗаказам Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ИсходящаяТранспортнаяОперацияВЕТИС") Тогда
		...
		ИнтеграцияВЕТИСУТ.ЗаполнитьРеализациюТоваровУслугНаОснованииИсходящейТранспортнойОперацииВЕТИС(ЭтотОбъект, ДанныеЗаполнения,, СтандартнаяОбработка);
		ЗаполненНаОснованииДокумента = Истина;
...
Функция ЕстьЗачтенныеУпрРасчеты()
	...
	Для Каждого Стр из РасшифровкаПлатежа Цикл
		Если ТипЗнч(Стр.Заказ) = Тип("ДокументСсылка.ЗаказКлиента")
			...
			ИЛИ ТипЗнч(Стр.Заказ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			Возврат ИСТИНА;
		КонецЕсли;
	КонецЦикла;
