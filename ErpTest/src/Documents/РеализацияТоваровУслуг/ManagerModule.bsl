...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора,
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		СтрокаТаб.КоличествоУпаковок = КоличествоКОтгрузке /
			?(ЗначениеЗаполнено(ВыборкаТовары.Коэффициент), ВыборкаТовары.Коэффициент, 1);
		//-- Рарус Лимаренко 29.07.2018
...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР КОГДА &Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.ВПути)
...
Функция ТекстЗапросаВтАгентскиеУслуги(Запрос, ТекстыЗапроса)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(Соглашение.Ссылка, НЕОПРЕДЕЛЕНО) КАК Соглашение,
	|	ЕСТЬNULL(Соглашение.Валюта, &Валюта) КАК Валюта,
	...
	|	И ЕСТЬNULL(Соглашение.Ссылка, НЕОПРЕДЕЛЕНО) = ЕСТЬNULL(Услуги.Ссылка, НЕОПРЕДЕЛЕНО)
...
Функция ТекстЗапросаВтРаботыУслуги(Запрос, ТекстыЗапроса)
	...
	|			И (ТоварыЗаказа.Номенклатура = ТаблицаТовары.Номенклатура)
	|			И (ТоварыЗаказа.пкЗаявкаНаАрендуТехники = ТаблицаТовары.пкЗаявкаНаАрендуТехники)
	|			И (ТоварыЗаказа.пкДатаНачалаАренды = ТаблицаТовары.пкДатаНачалаАренды)
	|			И (ТоварыЗаказа.пкДатаОкончанияАренды = ТаблицаТовары.пкДатаОкончанияАренды)
...
Процедура УстановитьПараметрЗапросаТипыНалогообложенияНДСОблагаемые(Запрос) Экспорт
	...
	Запрос.УстановитьПараметр("ТипыНалогообложенияНДСОблагаемые", УчетНДСУПСлужебный.ТипыНалогообложенияНДСНеУчитываетсяВСтоимости());
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Услуги.Ссылка.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	Услуги.Ссылка.Контрагент КАК Контрагент,
	|	Услуги.Ссылка.Договор КАК Договор,
	|	Услуги.Ссылка.Грузоотправитель КАК Грузоотправитель,
	|	Услуги.Ссылка.Грузополучатель КАК Грузополучатель,
	...
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И Услуги.Ссылка.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
	|	И Услуги.Ссылка.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет))
	|
	...
	|ГДЕ
	|	ВидыЗапасов.Ссылка В (&Ссылка)
	|	И ВидыЗапасов.Ссылка.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	//+++rarus-spb_zlov 02.12.2016    
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.пкПечатьАктовОказанияУслуг";
	КомандаПечати.Идентификатор = "АктОказанияУслугАренда";
	КомандаПечати.Представление = НСтр("ru = 'Акт оказания услуг (аренда)'");
	КомандаПечати.Порядок = 40;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.пкПечатьАктовОказанияУслуг";
	КомандаПечати.Идентификатор = "АктОказанияУслугАрендаСПодписями";
	КомандаПечати.Представление = НСтр("ru = 'Акт оказания услуг (аренда с подписями)'");
	КомандаПечати.Порядок = 41;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.пкПечатьАктовОказанияУслуг";
	КомандаПечати.Идентификатор = "АктОказанияУслугРуб";
	КомандаПечати.Представление = НСтр("ru = 'Акт оказания услуг (руб.)'");
	КомандаПечати.Порядок = 42;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.пкПечатьАктовОказанияУслуг";
	КомандаПечати.Идентификатор = "АктОказанияУслугВал";
	КомандаПечати.Представление = НСтр("ru = 'Акт оказания услуг (вал.)'");
	КомандаПечати.Порядок = 43;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.пкПечатьСчетовФактур";
	КомандаПечати.Идентификатор = "СчетФактураАренда";
	КомандаПечати.Представление = НСтр("ru = 'Счет-фактура (аренда)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.МенеджерПечати = "Обработка.пкПечатьСчетовФактур";
	КомандаПечати.Идентификатор = "СчетФактураАрендаСПодписями";
	КомандаПечати.Представление = НСтр("ru = 'Счет-фактура (аренда с подписями)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КомандаПечати.ДополнительныеПараметры.Вставить("ПечатьВВалюте", Ложь);
	...
	КомандаПечати.Представление = НСтр("ru = 'Счет на оплату (Pekkaniska)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	...
	КомандаПечати.Представление = НСтр("ru = 'Счет на оплату аренды (Подписи)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
//	//+++rarus-spb_zlov 30.01.2017   
//    
//	ИмяМакета = "АктОказанияУслугАренда";
...
//			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслугАренда(ИмяМакета, ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, ТекущийКомплект));
//	КонецЕсли;
//    
//	ИмяМакета = "АктОказанияУслугАрендаСПодписями";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,","АктОказанияУслугАрендаСПодписями",Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
//			ТаблицаСсылок = ТекущийКомплект.Скопировать(,"Ссылка");
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслугАренда(ИмяМакета, ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, ТекущийКомплект, Истина));
//    КонецЕсли;
//        
//    ИмяМакета = "АктОказанияУслугРуб";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслуг(ИмяМакета, ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати));
//    КонецЕсли;
//        
//	ИмяМакета = "АктОказанияУслугВал";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслуг(ИмяМакета, ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, Ложь));
//    КонецЕсли;
//        
//	ИмяМакета = "СчетФактураАренда";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьСчетовФактур.СформироватьПечатнуюФормуСчетФактура(ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура("ПечатьВВалюте", Ложь), ТекущийКомплект));
//    КонецЕсли;
//        
//	ИмяМакета = "СчетФактураАрендаСПодписями";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,", "СчетФактураАрендаСПодписями",Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
//			ТаблицаСсылок = ТекущийКомплект.Скопировать(,"Ссылка");
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьСчетовФактур.СформироватьПечатнуюФормуСчетФактура(ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура("ПечатьВВалюте", Ложь), ТекущийКомплект, Истина));
//    КонецЕсли;
//  
////---rarus-spb_zlov 30.01.2017

////++ Рарус Лимаренко 13.12.2017
//	ИмяМакета = "СчетНаОплатуПекканиска";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуПекканиска(ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, ТекущийКомплект));
//	КонецЕсли;

//	ИмяМакета = "СчетНаОплатуАренды";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды("СчетНаОплатуАренды", ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, ТекущийКомплект));
//	КонецЕсли;

//	ИмяМакета = "СчетНаОплатуАрендыСПодписью";
//	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
//	Если ТекущийКомплект.Количество() > 0 Тогда
//		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
//			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
//			ИмяМакета,
//			ТекущийКомплект[0].Представление,
//			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды("СчетНаОплатуАрендыСПодписью", ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, ТекущийКомплект));
//	КонецЕсли;
//	
////-- Рарус Лимаренко 13.12.2017	
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслугАренда("АктОказанияУслугАренда", СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла;                                                                                 
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслугАренда("АктОказанияУслугАрендаСПодписями", СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла; 	
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьАктовОказанияУслуг.СформироватьПечатнуюФормуАктОказанияУслуг("АктОказанияУслуг", СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла; 
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовФактур.СформироватьПечатнуюФормуСчетФактура(СтруктураТипов, ОбъектыПечати, Новый Структура("ПечатьВВалюте", Ложь)));
	КонецЦикла;           
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовФактур.СформироватьПечатнуюФормуСчетФактура(СтруктураТипов, ОбъектыПечати, Новый Структура("ПечатьВВалюте", Ложь),,Истина));
	КонецЦикла;	
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуПекканиска(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды("СчетНаОплатуАренды",СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла;
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды("СчетНаОплатуАрендыСПодписью",СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла;
...
Функция КомплектПечатныхФорм() Экспорт
	...
	//+++rarus-spb_zlov 30.01.2017    
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "АктОказанияУслугАренда", НСтр("ru = 'Акт оказания услуг (аренда)'"), 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "АктОказанияУслугАрендаСПодписями", НСтр("ru = 'Акт оказания услуг (аренда с подписями)'"), 0);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "АктОказанияУслугРуб", НСтр("ru = 'Акт оказания услуг (руб.)'"), 0);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "АктОказанияУслугВал", НСтр("ru = 'Акт оказания услуг (вал.)'"), 0);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетФактураАренда", НСтр("ru = 'Счет-фактура (аренда)'"), 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетФактураАрендаСПодписями", НСтр("ru = 'Счет-фактура (аренда с подписями)'"), 0);
	//---rarus-spb_zlov 30.01.2017
	
	//++ Рарус Лимаренко 13.12.2017
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуПекканиска", НСтр("ru = 'Счет на оплату (Pekkaniska)'"), 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуАренды", НСтр("ru = 'Счет на оплату аренда'"), 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуАрендыСПодписью", НСтр("ru = 'Счет на оплату аренды с подписью'"), 1);
	//-- Рарус Лимаренко 13.12.2017
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДанныеДокумента.Ссылка.Дата КАК ДатаПлатежа,
	...
	|	ДанныеДокумента.Ссылка.Дата КАК ДатаПлатежа,
	|	
	|	ДанныеДокумента.ПроцентПлатежа* 100 КАК ПроцентПлатежа,
	|	
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	|			ДанныеДокумента.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию)
	|		ТОГДА
	|			СУММА(ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеРасход, 0))
	|		ИНАЧЕ
	|			СУММА(ЕСТЬNULL(Этапы.СуммаПлатежа,0))
	...
	|			ИСТИНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ
	|			Документ.РеализацияТоваровУслуг.ЭтапыГрафикаОплаты КАК Этапы
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора,
	...
	Запрос.Текст =
		...
		|									КОГДА ДокументОтгрузки ССЫЛКА Документ.ЗаказКлиента
		|										ТОГДА ВЫБОР
		|												КОГДА ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаказКлиента).НалогообложениеНДС В (ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг), ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров))
		|													ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
		|												ИНАЧЕ ВЫРАЗИТЬ(ДокументОтгрузки КАК Документ.ЗаказКлиента).НалогообложениеНДС
		|											КОНЕЦ = &НалогообложениеНДС
		|									КОГДА ДокументОтгрузки ССЫЛКА Документ.ЗаявкаНаВозвратТоваровОтКлиента
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстЗапросаВидыЗапасовСУслугами = СтрЗаменить(ТекстЗапросаВидыЗапасовСУслугами,
		"#ИмяВТЗаблокированныхРегистраторов",
		РезультатыСозданияВТ.ИмяВременнойТаблицы);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет Тогда
				...
				Пока ВыборкаПоАналитикам.Следующий() Цикл
					Если ВыборкаПоАналитикам.КоличествоВидовЗапасов <> 1 Тогда
						ТекстИсключения = НСтр("ru = 'Ошибка обработки документа %Документ% - несколько приходуемых видов запасов по одной номенклатуре.'");
			...
			Запрос.Текст = ТекстЗапросаВидыЗапасовСУслугами;
			Запрос.УстановитьПараметр("Регистратор", Выборка.Ссылка);
			...
			Если ДокументОбъект.ЭтапыГрафикаОплаты.Количество() = 0 
				...
				Если (ДокументОбъект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным
					ИЛИ НЕ ДокументОбъект.РеализацияПоЗаказам) И ДокументОбъект.СуммаВзаиморасчетов > 0 И ДокументОбъект.СуммаДокумента > 0 Тогда
