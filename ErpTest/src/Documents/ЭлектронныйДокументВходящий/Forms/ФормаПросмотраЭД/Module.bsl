...
Процедура ПроверитьФормированиеПредставленияФайлаЭлектронногоДокумента(ДлительнаяОперация, ТекущаяСтрокаДерева)
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		ПредставлениеЭлектронногоДокумента = ПолучитьИзВременногоХранилища(ДлительнаяОперация.АдресРезультата);
...
Процедура ОбновитьДанныеОВложении(ТекущаяСтрокаДерева)
	...
	ПрисоединенныйФайлПиктограмма = РаботаСФайламиСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(ТекущаяСтрокаДерева.Расширение);
...
Процедура НастроитьЭлементыОтраженияВУчете()
	...
	Если Объект.ДокументыОснования.Количество() = 0 
		...
		Если ПараметрыПроизвольногоДокумента <> Неопределено
			...
			НевозможноОтразитьВУчете = Истина;
		...
		Иначе
			...
			Если Объект.ВидЭД = Перечисления.ВидыЭД.ПрикладнойЭД Тогда
				...
				НевозможноОтразитьВУчете = ПрикладныеВиды.Найти(Объект.ПрикладнойВидЭД) = Неопределено;
			Иначе
				Если ВидЭлектронногоДокумента = Неопределено Тогда
					ВидЭлектронногоДокумента = Объект.ВидЭД;
				КонецЕсли;
				АктуальныеВидыЭД = ОбменСКонтрагентамиПовтИсп.ПолучитьАктуальныеВидыЭД();
				НевозможноОтразитьВУчете = АктуальныеВидыЭД.Найти(ВидЭлектронногоДокумента) = Неопределено;
			КонецЕсли;
		...
		Элементы.ГруппаНевозможноОтразитьВУчете.Видимость = НевозможноОтразитьВУчете;
...
Процедура ОткрытьНастройкуОтраженияВУчете()
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", Объект.НастройкаЭДО);
	ПараметрыФормы.Вставить("ОткрытьНастройкуВходящихДокументов", Истина);
...
Процедура ОбработатьВыборПодбораДокументаУчета(Знач ВыбранноеЗначение, Знач ДополнительныеПараметры) Экспорт
	...
	НастройкиПодбора = НастройкиПодбораУчетногоДокумента(СпособОбработки, Объект.НастройкаЭДО);
...
&НаСервереБезКонтекста
Функция НастройкиПодбораУчетногоДокумента(Знач СпособОбработки, Знач НастройкаЭДО)
	...
	РеквизитыНастройкиЭДО = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаЭДО, "Контрагент,Организация");
	Настройки.Вставить("Организация", РеквизитыНастройкиЭДО.Организация);
	Настройки.Вставить("Контрагент", РеквизитыНастройкиЭДО.Контрагент);
	...
	Пока СтекЭД.Количество() Цикл
		...
		Для Каждого ПодчиненныйЭД Из КоллекцияЭД Цикл
			...
			Если ПодчиненныйЭД.Ссылка = ТекущийЭД Тогда
				...
				Если ПодчиненныйЭД.СтатусЭД = Перечисления.СтатусыЭД.ПодготовленКОтправке
			...
			Если НайденТекущийЭД И (ПодчиненныйЭД.СтатусЭД = Перечисления.СтатусыЭД.ПодготовленКОтправке
				ИЛИ ПодчиненныйЭД.СтатусЭД = Перечисления.СтатусыЭД.ПодготовленоПодтверждение) Тогда
				МассивДокументов.Добавить(ПодчиненныйЭД.Ссылка);
			КонецЕсли;
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	МожноОтклонитьЭтотЭД = ЕстьПравоОбработки И ОбменСКонтрагентамиСлужебный.МожноОтклонитьЭтотЭД(СсылкаНаЭД);
	МожноАннулироватьЭтотЭД = ЕстьПравоОбработки И ОбменСКонтрагентамиСлужебный.МожноАннулироватьЭтотЭД(СсылкаНаЭД);
	...
	ИначеЕсли СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		КомандаПодписиОтправки.Видимость = ЕстьПравоОбработки И Объект.ТребуетсяПодтверждение И ТребуетсяПодпись
			И (СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.Подписан
				ИЛИ СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.Утвержден);
	...
	Элементы.ГруппаШапки.ЗаголовокСвернутогоОтображения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Состояние: %1'"), ТекстСостояния);
...
Процедура ЗаполнитьТаблицуЭП()
	...
	СоглашениеЭД = Объект.НастройкаЭДО;
	
	ПроверятьСертификатыПодписей = ЗначениеЗаполнено(СоглашениеЭД)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СоглашениеЭД, "ПроверятьСертификатыПодписей");
	
	Если ПроверятьСертификатыПодписей Тогда
		МассивОтпечатковОжидаемыхСертификатов = ОбменСКонтрагентамиСлужебный.ОтпечаткиОжидаемыхСертификатов(Объект.Ссылка);
	КонецЕсли;
		
	Если ОбменСКонтрагентамиСлужебный.ЭтоСлужебныйДокумент(ПрисоединенныйФайлСсылка) Тогда
		ЭлектронныеПодписи = ЭлектроннаяПодпись.УстановленныеПодписи(ПрисоединенныйФайлСсылка);
	Иначе
		ЭлектронныеПодписи = ОбменСКонтрагентамиСлужебный.ЭлектронныеПодписиДвухТитулов(ПрисоединенныйФайлСсылка);
	КонецЕсли;
	...
	Для Каждого Подпись Из ЭлектронныеПодписи Цикл
		...
		Если ПроверятьСертификатыПодписей Тогда
			...
			Если МассивОтпечатковОжидаемыхСертификатов.Найти(Подпись.Отпечаток) = Неопределено Тогда
...
Функция НеобходимоПодписать(СвойстваФайлаЭД)
	...
	ИначеЕсли СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий
		...
		Иначе
			...
			Запрос.УстановитьПараметр("НаправлениеЭД", СвойстваФайлаЭД.НаправлениеЭД);
