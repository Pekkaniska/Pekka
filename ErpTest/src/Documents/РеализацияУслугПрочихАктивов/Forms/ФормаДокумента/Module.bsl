...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	Если Объект.РеализацияНаОсновании
		И ((ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПодготовкаКПередачеОС2_4")
			...
			ИЛИ (ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПодготовкаКПередачеОС")
				...
				И Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности) Тогда
		Объект.РеализацияНаОсновании = Ложь;
		Объект.ДокументОснование = Неопределено;
	КонецЕсли; 
...
&НаКлиенте
Процедура РеализацияВзаимозависимомуЛицуПриИзменении(Элемент)
	
	//++ НЕ УТ
	Если Объект.РеализацияВзаимозависимомуЛицу Тогда
		РеализацияВзаимозависимомуЛицуПриИзмененииНаСервере();
	Иначе
...
&НаКлиенте
Процедура ЗаполнитьСтатьюАмортизационнойПремии(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьСтатьюАмортизационнойПремииЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ЗаполнитьВосстановлениеАмортизационнойПремии(Команда)
	...
	ЗаполнитьВосстановлениеАмортизационнойПремииНаСервере(Истина);
	//-- НЕ УТ
...
Процедура УстановитьУсловноеОформление()
	...
	//++ НЕ УТ
	ПланыВидовХарактеристик.СтатьиДоходов.УстановитьУсловноеОформлениеАналитик(УсловноеОформление, Новый Структура("Расходы"));
	//-- НЕ УТ
...
Процедура ПартнерПриИзмененииСервер()
	...
	//++ НЕ УТ
	ЗаполнитьПризнакРеализацииВзаимозависимомуЛицу();
...
&НаСервере
Процедура РеализацияВзаимозависимомуЛицуПриИзмененииНаСервере()

	Если Объект.РеализацияВзаимозависимомуЛицу Тогда
		ЗаполнитьВосстановлениеАмортизационнойПремииНаСервере();
	КонецЕсли; 
	
	НастроитьЗависимыеЭлементыФормыНаСервере("РеализацияВзаимозависимомуЛицу");

КонецПроцедуры
 
&НаСервере
Процедура ЗаполнитьПризнакРеализацииВзаимозависимомуЛицу()

	Документы.РеализацияУслугПрочихАктивов.ЗаполнитьПризнакРеализацииВзаимозависимомуЛицу(Объект);
...
&НаСервере
Процедура ЗаполнитьВосстановлениеАмортизационнойПремииНаСервере(ТолькоВыбранныеСтроки = Ложь)

	Если ТолькоВыбранныеСтроки Тогда
		КоллекцияСтрок = ОбщегоНазначенияУТКлиентСервер.ВыделенныеЭлементыКоллекции(Объект.Расходы, Элементы.Расходы.ВыделенныеСтроки);
	Иначе
		КоллекцияСтрок = Объект.Расходы;
	КонецЕсли;
	
	Документы.РеализацияУслугПрочихАктивов.ЗаполнитьВосстановлениеАмортизационнойПремии(Объект, КоллекцияСтрок);
...
Процедура ЗаполнитьСтатьюАмортизационнойПремииЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если ЗначениеЗаполнено(Результат) Тогда
        УстановитьСтатьюДоходовСервер(Результат, "Расходы");
    КонецЕсли;
...
Процедура НастроитьЭлементыФормы()
	...
	ОтображатьИтогСуммыНДС = УчетНДСУПКлиентСервер.ПродажаОблагаетсяНДС(Объект.НалогообложениеНДС);
...
Процедура ОбновитьТекстСчетаФактуры()
	...
	ТекстСчетаФактурыВыданные = ПродажиСервер.ДополнитьПредставлениеПредставлениемФискальнойОперацииВДокументеПродажи(Объект.Ссылка, СчетаФактурыВыданныеНаОсновании);
...
Процедура УстановитьВидимостьСатьиАктиваПассива()
	...
	Элементы.РасходыПредставлениеОтраженияВРеглУчете.Видимость = НЕ РеализацияВНА;
	Элементы.РасходыСтатьяАктивовПассивов.Видимость = НЕ РеализацияВНА;
	Элементы.РасходыАналитикаАктивовПассивов.Видимость = НЕ РеализацияВНА;
