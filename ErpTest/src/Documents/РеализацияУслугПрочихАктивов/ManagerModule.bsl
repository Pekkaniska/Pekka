...
//
Процедура ЗаполнитьВосстановлениеАмортизационнойПремии(Объект, КоллекцияСтрок) Экспорт
	...
	|ПОМЕСТИТЬ СписокОС
	|ИЗ
	|	РегистрСведений.ПервоначальныеСведенияОС.СрезПоследних(&КонецПериода, ОсновноеСредство В (&СписокОС)) КАК ПервоначальныеСведенияОС
	|ГДЕ
	...
	Запрос.УстановитьПараметр("ДатаНачалаУчетаГраница", Новый Граница(Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить(), ВидГраницы.Исключая));
...
//  Объект	 - ДокументОбъект.РеализацияУслугПрочихАктивов - Документ реализации.
//
Процедура ЗаполнитьПризнакРеализацииВзаимозависимомуЛицу(Объект) Экспорт
...
Функция ТекстЗапросаТаблицаПрочиеДоходы(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	Если Не ПроведениеСерверУТ.ЕстьТаблицаЗапроса("втОстаточнаяСтоимость", ТекстыЗапроса) Тогда
...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	//++ НЕ УТ
	ЗначенияПараметровПроведения.Вставить("ДатаНачалаУчетаВнеоборотныхАктивов2_4", Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить());
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	ТаблицаДокумента.СтоимостьПР КАК ПостояннаяРазница,
	|	ТаблицаДокумента.СтоимостьВР КАК ВременнаяРазница,
	|	
	|	ВЫБОР
	...
	|	0 КАК СуммаБезНДСРегл,
	|	ТаблицаДокумента.СтоимостьПР КАК ПостояннаяРазница,
	|	-(ТаблицаДокумента.СтоимостьРегл - ТаблицаДокумента.СтоимостьВР) КАК ВременнаяРазница,
	|	
	...
	|	ТаблицаДокумента.СтоимостьРегл КАК СуммаБезНДСРегл,
	|	0 КАК ПостояннаяРазница,
	|	ТаблицаДокумента.СтоимостьРегл КАК ВременнаяРазница,
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РасходыОтСписанияАктиваСОтложеннымПереходомПрав) КАК ХозяйственнаяОперация,
	|	Неопределено КАК АналитикаУчетаНоменклатуры
	|ИЗ
...
Функция ТекстЗапросаТаблицаСтоимостьОС(ТекстыЗапроса)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// В НУ ОС списываются до перехода прав.
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)   КАК ВидДвижения,
	|	&Период                                  КАК Период,
	|	
	...
	|	ТаблицаСтоимости.СтоимостьНУ             КАК СтоимостьНУ,
	|	ТаблицаСтоимости.СтоимостьПР             КАК СтоимостьПР,
	|	-(ТаблицаСтоимости.СтоимостьРегл - ТаблицаСтоимости.СтоимостьВР) КАК СтоимостьВР,
	|	0                                        КАК СтоимостьЦФ,
	|	ТаблицаСтоимости.СтоимостьНУЦФ           КАК СтоимостьНУЦФ,
	|	ТаблицаСтоимости.СтоимостьПРЦФ           КАК СтоимостьПРЦФ,
	|	-(ТаблицаСтоимости.СтоимостьЦФ - ТаблицаСтоимости.СтоимостьВРЦФ) КАК СтоимостьВРЦФ,
	|
	|	ТаблицаСтоимости.СтатьяРасходов          КАК КорСтатьяРасходов,
	|	ТаблицаСтоимости.АналитикаРасходов       КАК КорАналитикаРасходов,
	|	
	...
	|	0                                        КАК СтоимостьНУ,
	|	0                                        КАК СтоимостьПР,
	|	ТаблицаСтоимости.СтоимостьРегл           КАК СтоимостьВР,
	|	ТаблицаСтоимости.СтоимостьЦФ             КАК СтоимостьЦФ,
	|	0                                        КАК СтоимостьНУЦФ,
	|	0                                        КАК СтоимостьПРЦФ,
	|	ТаблицаСтоимости.СтоимостьЦФ             КАК СтоимостьВРЦФ,
	|
	|	ТаблицаСтоимости.СтатьяРасходов          КАК КорСтатьяРасходов,
	|	ТаблицаСтоимости.АналитикаРасходов       КАК КорАналитикаРасходов,
	|	
...
Процедура ВременнаяТаблицаВНА(ТекстыЗапроса)
	...
	|				&Период,
	|		) КАК ПараметрыЦФ
	|		ПО ТаблицаВНА.ВнеоборотныйАктив = ПараметрыЦФ.ОсновноеСредство
	|
	...
	|				&Период,
	|		) КАК ПараметрыЦФ
	|		ПО ТаблицаВНА.ВнеоборотныйАктив = ПараметрыЦФ.НематериальныйАктив
	|
...
// - заполняется реквизит СтавкаНДС в табличной части.
//
