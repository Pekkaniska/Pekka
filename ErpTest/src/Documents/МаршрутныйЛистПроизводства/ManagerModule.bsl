...
Функция ТекстЗапросаТаблицаДоступностьВидовРабочихЦентров(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ВТЗанятоЗаказом", ТекстыЗапроса) Тогда
...
Функция ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТабличнаяЧасть.Получатель              КАК Склад,
	|	&ПодразделениеДокумента                КАК Отправитель,
	|	ТабличнаяЧасть.КоличествоФакт          КАК КОформлениюНакладныхПоРаспоряжению,
	|	ВЫБОР
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказовРаботами(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТабличнаяЧасть.Назначение                    КАК Назначение,
	|	-ТабличнаяЧасть.КоличествоПоЗаказу           КАК КОбеспечению
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.РаспоряжениеСпецификация КАК ТабличнаяЧасть
	|
	|ГДЕ
	...
	|	ТабличнаяЧасть.Получатель                    КАК Подразделение,
	|	ТабличнаяЧасть.Назначение                    КАК Назначение,
	|	ТабличнаяЧасть.КоличествоФакт                КАК КОбеспечению
	|ИЗ
	|	Документ.МаршрутныйЛистПроизводства.ВыходныеИзделия КАК ТабличнаяЧасть
	|
	|ГДЕ
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	Документ.МаршрутныйЛистПроизводства КАК МаршрутныйЛистПроизводства
		|ГДЕ
		|	МаршрутныйЛистПроизводства.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Материалы.Ссылка
		|ИЗ
		|	Документ.МаршрутныйЛистПроизводства.МатериалыИУслуги КАК Материалы
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры.ПолитикиУчетаСерий КАК ПолитикиУчетаСерий
		|		ПО Материалы.Ссылка.Подразделение = ПолитикиУчетаСерий.Склад
		|			И Материалы.Номенклатура.ВидНоменклатуры = ПолитикиУчетаСерий.Ссылка
		|ГДЕ
		|	Материалы.СтатусУказанияСерий В (9, 10, 11)
		|	И ПолитикиУчетаСерий.ПолитикаУчетаСерий.УчетСерийПоFEFO");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.МаршрутныйЛистПроизводства";
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВТКОбработке";
	ДопПараметры.ДополнительныеИсточникиДанных.Вставить("МатериалыИУслуги.Номенклатура");
	...
	ДопПараметры.ИмяВременнойТаблицы = "ВТБлокировкиВидыНоменклатуры";
	...
	ДопПараметры.ИмяВременнойТаблицы = "ВТБлокировкиПолитикиУчетаСерий";
	ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
	...
	Запрос = Новый Запрос(
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.МаршрутныйЛистПроизводства.МатериалыИУслуги КАК Материалы
		|		ПО КОбработке.Ссылка = Материалы.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиВидыНоменклатуры КАК Блокировки
		|		ПО (Материалы.Номенклатура.ВидНоменклатуры = Блокировки.Ссылка)
		|ГДЕ
		|	Блокировки.Ссылка ЕСТЬ НЕ NULL 
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|		ПО (Материалы.Ссылка.Подразделение = ПолитикиУчетаСерий.Склад)
		|			И (Материалы.Номенклатура.ВидНоменклатуры = ПолитикиУчетаСерий.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиПолитикиУчетаСерий КАК Блокировки
		|		ПО (ПолитикиУчетаСерий.ПолитикаУчетаСерий = Блокировки.Ссылка)
		|ГДЕ
		|	Блокировки.Ссылка ЕСТЬ НЕ NULL 
		|;
		|
		...
		|ИЗ
		|	ВТКОбработке КАК КОбработке
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированныеСсылки КАК ВТЗаблокированныеСсылки
		...
		|ГДЕ
		|	ВТЗаблокированныеСсылки.Ссылка ЕСТЬ NULL
		|	И Материалы.СтатусУказанияСерий В (9, 10, 11)
		|	И ПолитикиУчетаСерий.ПолитикаУчетаСерий.УчетСерийПоFEFO");
	...
	ТаблицаМатериалов.Индексы.Добавить("Ссылка");
	ТаблицаМатериалов.Индексы.Добавить("Ссылка, Номенклатура");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ДокументОбъект.ВариантПриемкиТоваров.Пустая() Тогда
					
					ДокументОбъект.ВариантПриемкиТоваров = ВариантПриемкиТоваров;
					ОбъектИзменен = Истина;
					
				КонецЕсли;
				
				Если ТаблицаМатериалов.Найти(Выборка.Ссылка, "Ссылка") <> Неопределено Тогда
			
					Для каждого Строка Из ДокументОбъект.МатериалыИУслуги Цикл
						
						Если Строка.СтатусУказанияСерий = 9
							ИЛИ Строка.СтатусУказанияСерий = 10
							ИЛИ Строка.СтатусУказанияСерий = 11 Тогда
							
							СтруктураПоиска.Ссылка = Выборка.Ссылка;
							СтруктураПоиска.Номенклатура = Строка.Номенклатура;
							
							Если ТаблицаМатериалов.НайтиСтроки(СтруктураПоиска).ВГраница() <> -1 Тогда
								
								Если Строка.СтатусУказанияСерий = 10 Тогда
									Строка.СтатусУказанияСерий = 6;
								ИначеЕсли Строка.СтатусУказанияСерий = 11 Тогда
									Строка.СтатусУказанияСерий = 25;
								ИначеЕсли Строка.СтатусУказанияСерий = 9 Тогда
