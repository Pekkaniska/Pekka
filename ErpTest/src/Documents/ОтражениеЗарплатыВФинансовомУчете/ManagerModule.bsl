	...
	Если СтруктураДанных.Свойство("РезультатОтраженияЗарплаты", РезультатОтраженияЗарплаты) Тогда
		...
		Если РезультатОтраженияЗарплаты.Свойство("НачисленныйНДФЛ") И РезультатОтраженияЗарплаты.НачисленныйНДФЛ.Количество() > 0 
			...
			Если Параметры.Сводно Тогда
				...
				ТаблицаИтогиНДФЛ.Свернуть("ПодразделениеПредприятия, ВидОперации, РегистрацияВНалоговомОргане", "Сумма");
			...
			Иначе
				...
				ТаблицаИтогиНДФЛ.Свернуть("ФизическоеЛицо, ПодразделениеПредприятия, ВидОперации, РегистрацияВНалоговомОргане", "Сумма");
		...
		Если РезультатОтраженияЗарплаты.Свойство("УдержаннаяЗарплата")
			И РезультатОтраженияЗарплаты.УдержаннаяЗарплата.Количество() > 0 
...
#Область ТекстЗапросаНачисленоСдельно
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		(
	|			ВЫБРАТЬ ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.ПустаяСсылка) КАК ТипНалога,				ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда) КАК СчетКт	// (Кт 70)
	...
	|			ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.ПФРСПревышения),			ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПФР_ОПС)							// (Кт 69.02.8)
...
#Область ТаблицаДокумента
	...
	|ГДЕ
	|	НЕ СтатьиРасходов.ОплатаСдельныхРабот
	|	ИЛИ НачисленнаяЗарплатаИВзносы.СпособОтраженияЗарплатыВБухучете = ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// ПФРСтраховаяЧасть
	...
	|ГДЕ
	|	НЕ СтатьиРасходов.ОплатаСдельныхРабот
	|	ИЛИ СтатьиРасходов.РежимУчетаВзносовНаТравматизм = 1
	|	ИЛИ НачисленнаяЗарплатаИВзносы.СпособОтраженияЗарплатыВБухучете = ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция ТекстЗапросаТаблицаДвиженияДоходыРасходыПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	|											ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.КомпенсацияЕжегодногоОтпускаОценочныеОбязательства),
	|											ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ЕжегодныйОтпускОценочныеОбязательства))
	|		ИЛИ НачисленнаяЗарплата.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.КомпенсацияЕжегодногоОтпускаОценочныеОбязательства)
...
Процедура ЗаполнитьПодразделениеПредприятияНачисленоСдельно(ДанныеДляОтражения, СтруктураПараметров)
	...
	СписокСпособов = ДанныеДляОтражения.НачисленнаяЗарплатаИВзносы.ВыгрузитьКолонку("СпособОтраженияЗарплатыВБухучете");
	...
	Для Каждого Строка Из ДанныеДляОтражения.НачисленнаяЗарплатаИВзносы Цикл
		...
		Если (ЗначениеЗаполнено(Строка.СпособОтраженияЗарплатыВБухучете) И Не ОплатаСдельныхРабот[Строка.СпособОтраженияЗарплатыВБухучете])
			Или (Не ЗначениеЗаполнено(Строка.СпособОтраженияЗарплатыВБухучете) И Строка.ВидОперации <> Перечисления.ВидыОперацийПоЗарплате.НачисленоСдельноДоход) Тогда
			Продолжить;
		КонецЕсли;
...
// Заполняет поле "Способ отражения" для строк с видом операции "Начислено (сдельно)".
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	И ДД.РежимУчетаВзносовНаТравматизм = 0
		...
		Попытка
			...
			Для Каждого Строка Из ДокументОбъект.НачисленнаяЗарплатаИВзносы Цикл
				Если Не ЗначениеЗаполнено(Строка.СпособОтраженияЗарплатыВБухучете)
					...
					Строка.СпособОтраженияЗарплатыВБухучете = СпособОтражения;
				КонецЕсли;
			КонецЦикла;
