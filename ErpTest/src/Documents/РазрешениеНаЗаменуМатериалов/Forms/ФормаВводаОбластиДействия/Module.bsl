...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
		...
		Иначе
			...
			Элементы.ПоЗаказамПояснение.Заголовок = НСтр("ru = 'Указание дополнительных сведений позволит сузить область действия разрешения до заказа на производство, подразделения и (или) спецификации.'");
		КонецЕсли;
	Иначе
...
Процедура ЗавершитьРедактирование()
	...
	Если ИспользоватьИзделие И Не ЗначениеЗаполнено(Изделие) Тогда
		ТекстОшибки = Нстр("ru = 'Поле ""Изделие"" не заполнено'");
