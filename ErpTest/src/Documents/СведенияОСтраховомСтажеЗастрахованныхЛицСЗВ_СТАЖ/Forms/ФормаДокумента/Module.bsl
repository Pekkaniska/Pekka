...
&НаКлиенте
Процедура ДосрочноеНазначениеПенсииОснованиеВыслугиЛетНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура("ОтчетныйПериод", Объект.ОтчетныйПериод);
	ОткрытьФорму("ОбщаяФорма.ПодборОснованийДосрочногоНазначенияПенсийПФР", ПараметрыФормы, Элементы.ДосрочноеНазначениеПенсии);
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл 
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.ТретийПараметрИсчисляемогоСтажа) И ЗначениеЗаполнено(Выборка.ЗамещениеГосударственныхМуниципальныхДолжностей) Тогда 
				...
				Иначе 
					ПредыдущаяСтрока = ЗаписиОСтаже[ЗаписиОСтаже.Количество()-1];
				...
				Если ДатыВыходаНаПенсию <> Неопределено Тогда 
					СтрокаОтклонения.ДатаОкончанияПериода = Мин(Выборка.ДатаОкончанияПериода, ДатыВыходаНаПенсию[Выборка.Сотрудник]);
				КонецЕсли;
				Продолжить;
			КонецЕсли;
		...
		Для Индекс = ИндексПервойСтрокиСотрудника По ИндексПоследнейСтрокиСотрудника Цикл 
		    СтрокиСтажаПоСотруднику.Добавить(ЗаписиОСтаже[Индекс]);
		КонецЦикла;
