...
Процедура ЗаполнитьТарифныеСтавки(ВыделенныеСтроки = Неопределено, ОтменятьИсправления = Ложь)
	...
	Если ВыделенныеСтроки = Неопределено Тогда
		Для Каждого СтрокаТаблицы Из Объект.ЗначенияПоказателей Цикл
			...
			Если Не СтрокаТаблицы.ФиксТарифнаяСтавка Тогда
				...
				МассивСотрудников.Добавить(СтрокаТаблицы.Объект);
			КонецЕсли;
		КонецЦикла;
	Иначе
	...
	Запрос.УстановитьПараметр("Период", ДатыДокумента.ДатаОкончания);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Сотрудники.Ссылка КАК Сотрудник,
	|	&Период КАК Период
...
Процедура ЗаполнитьОтработанноеВремя(ВыделенныеСтроки = Неопределено, ОтменятьИсправления = Ложь)
	...
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("МассивСотрудников", МассивСотрудников);
	Запрос.УстановитьПараметр("ДатаНачала",        ДатыДокумента.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания",     ДатыДокумента.ДатаОкончания);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Сотрудники.Ссылка КАК Сотрудник,
	...
	ПараметрыВТ.ДатаНачала         = ДатыДокумента.ДатаНачала;
	ПараметрыВТ.ДатаОкончания      = ДатыДокумента.ДатаОкончания;
	ПараметрыВТ.МесяцДатаНачала    = НачалоМесяца(ДатыДокумента.ДатаНачала);
	ПараметрыВТ.МесяцДатаОкончания = КонецМесяца(ДатыДокумента.ДатаОкончания);
	ПараметрыВТ.ДатаАктуальности   = ДатыДокумента.ДатаНачала;
	ПараметрыВТ.Организация        = Объект.Организация;
	
	УчетРабочегоВремениРасширенный.СоздатьВТДанныеУчетаВремениИСостоянийСотрудников(Запрос.МенеджерВременныхТаблиц, Истина, ПараметрыВТ);
...
Функция ДатыДокумента()
	...
	Если Элементы.ПериодСтраницы.Видимость Тогда
		...
		ИначеЕсли Элементы.ПериодСтраницы.ТекущаяСтраница = Элементы.ПериодСтраница Тогда
