...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени(
			"Документ.СписаниеБезналичныхДенежныхСредств.ФормаДокумента.Проведение");
	КонецЕсли;
	// }Рарус ilkond 2017.04.10 techlab@rarus.ru
...
Процедура БанковскийСчетПриИзмененииСервер(СтруктураРеквизитов, ПересчитыватьСуммы)
	...
	//++ Рарус Лимаренко 10.07.2018 Заявка №10396
	ВидимостьВладельцаОбъектаРасчетов = Элементы.РасшифровкаПлатежаЗаказ.Видимость;
...
Процедура ПодотчетноеЛицоПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.ПодотчетноеЛицо) Тогда
		...
		Объект.Подразделение = Объект.ПодотчетноеЛицо.пкПодразделение;
		//---rarus-spb_zlov 17.04.2017
	КонецЕсли;
...
//+rarus_spb_z.dmitriy 2018.09.20
&НаСервере
...
//Рарус Владимир Подрезов 15.03.2017
	...
	Иначе
		Элементы.БанковскийСчетКонтрагента.АвтоОтметкаНезаполненного = Неопределено;
	КонецЕсли;
...
//++ Рарус Лимаренко 10.07.2018 Заявка №10396
	Если ПереключательРасшифровки Тогда
		ВидимостьВладельцаОбъектаРасчетов = Элементы.РасшифровкаПлатежаЗаказ.Видимость;
	Иначе
		ВидимостьВладельцаОбъектаРасчетов = Элементы.РасшифровкаБезРазбиенияЗаказ.Видимость;
	КонецЕсли;
	Элементы.РасшифровкаПлатежаОрганизация.Видимость = ВидимостьВладельцаОбъектаРасчетов;
	Элементы.РасшифровкаБезРазбиенияОрганизация.Видимость = ВидимостьВладельцаОбъектаРасчетов;
//-- Рарус Лимаренко 10.07.2018
...
Процедура УстановитьВозможныеЗначенияСчетаПолучателя(ИзменятьСчетПолучатель = Истина)
	...
	|	И БанковскиеСчета.ТипСчета НЕ В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыБанковскихСчетов.Транзитный))
	|");
