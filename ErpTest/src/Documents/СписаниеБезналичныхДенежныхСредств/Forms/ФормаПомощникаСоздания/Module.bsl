	...
	Если Параметры.Свойство("Основание") Тогда
		Если ЗначениеЗаполнено(Параметры.Основание)Тогда
			...
			ИначеЕсли ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыПеречислением") Тогда
				...
				Организация                 = ОрганизацияДляЗаполнения;
...
Процедура СоздатьДокументыСписаниеНаСервере(ФизическоеЛицо, ЛицевойСчет, Сумма, ВедомостьПоВыплатеЗП, ИндексСтроки)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		...
		//++ НЕ УТ
		СтрокаРасшифровкиПоВедомости = ДокументОснование.РасшифровкаПлатежа.НайтиСтроки(Новый Структура("Ведомость",ВедомостьПоВыплатеЗП));
		Если СтрокаРасшифровкиПоВедомости.Количество() Тогда
			ДокументСписаниеОбъект.СтатьяДвиженияДенежныхСредств = СтрокаРасшифровкиПоВедомости[0].СтатьяДвиженияДенежныхСредств;
		КонецЕсли;
		//-- НЕ УТ
		//++ НЕ УТ
	Иначе // Заполнение по ведомости на выплату зарплаты перечислением
		...
		ДокументСписаниеОбъект.Подразделение          = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ВедомостьПоВыплатеЗП, "Подразделение").Подразделение;
