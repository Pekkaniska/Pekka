...
Функция ПодобратьРеквизитыПлатежаВБюджет(ИсключаяРеквизиты = "") Экспорт
	...
	|		СписаниеБезналичныхДенежныхСредств.СчетУчета
	|	ИНАЧЕ СписаниеРасшифровкаПлатежа.СчетУчета
	|	КОНЕЦ КАК СчетУчета,
	|	ВЫБОР КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеречислениеВБюджет) ТОГДА
	|		СписаниеБезналичныхДенежныхСредств.Субконто1
	|	ИНАЧЕ СписаниеРасшифровкаПлатежа.Субконто1
	|	КОНЕЦ КАК Субконто1,
	|	ВЫБОР КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеречислениеВБюджет) ТОГДА
	|		СписаниеБезналичныхДенежныхСредств.Субконто2
	|	ИНАЧЕ СписаниеРасшифровкаПлатежа.Субконто2
	|	КОНЕЦ КАК Субконто2,
	|	ВЫБОР КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеречислениеВБюджет) ТОГДА
	|		СписаниеБезналичныхДенежныхСредств.Субконто3
	|	ИНАЧЕ СписаниеРасшифровкаПлатежа.Субконто3
	|	КОНЕЦ КАК Субконто3,
	//-- НЕ УТ
	...
	Если Не Результат.Пустой() Тогда
		...
		//++ НЕ УТ
		Если ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
			ЗаполнитьЗначенияСвойств(РасшифровкаПлатежа[0], РеквизитыПлатежа, "СчетУчета, Субконто1, Субконто2, Субконто3");
		КонецЕсли;
		//-- НЕ УТ
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипОснования = Тип("Структура") И ДанныеЗаполнения.Свойство("ЗаказПоставщику") Тогда
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения.ЗаказПоставщику) = Тип("ДокументСсылка.ОтчетОператораСистемыПлатон") Тогда
			ЗаполнитьПоОтчетуОператораСистемыПлатон(ДокОснование, КОплате, ДанныеЗаполнения);
	...
	ИначеЕсли ТипОснования = Тип("Структура")
		...
		Для каждого СтрокаМассива Из ДанныеЗаполнения.ДокументОснование Цикл
			...
			ИначеЕсли ТипЗнч(СтрокаМассива)= Тип("ДокументСсылка.ОтчетОператораСистемыПлатон") Тогда	 
				ЗаполнитьПоОтчетуОператораСистемыПлатон(СтрокаМассива, 0, ДанныеЗаполнения);
	...
	ИначеЕсли ТипОснования = Тип("СправочникСсылка.ДоговорыЛизинга") Тогда
		ЗаполнитьПоДоговоруЛизинга(ДанныеЗаполнения, ДанныеЗаполнения);
		
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк")
		ИЛИ ТипОснования = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыПеречислением")
		ИЛИ (ТипОснования = Тип("Структура") И (ДанныеЗаполнения.Свойство("МассивВедомостей"))) Тогда
	...
	Если ПлатежиПо275ФЗ Тогда
		ИГК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчет, "ГосударственныйКонтракт.Код");
		НазначениеПлатежа = СтрЗаменить(НазначениеПлатежа, "<ИГК>", ИГК);
	КонецЕсли;
	//-- НЕ УТ
...
//+++rarus-spb_zlov 25.11.2016   
	Статья = Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка();
	
	Если ЭтотОбъект.РасшифровкаПлатежа.Количество() > 0 Тогда
		Статья = ЭтотОбъект.РасшифровкаПлатежа[0].СтатьяДвиженияДенежныхСредств;    
	КонецЕсли;    
	
	Если пкОбщегоНазначенияСервер.ПроверитьУсловияПроведенияПлатежаБезЗаявки(ЭтотОбъект.ХозяйственнаяОперация, ЭтотОбъект.БанковскийСчет, Статья) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("РасшифровкаПлатежа.ЗаявкаНаРасходованиеДенежныхСредств");
	КонецЕсли;    
//---rarus-spb_zlov 25.11.2016
	...
	//++ НЕ УТ
	Если ДенежныеСредстваСервер.ОперацияПоЗарплате(ХозяйственнаяОперация) И ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		
		Если СуммаДокумента <> РасшифровкаПлатежа.Итог("Сумма") Тогда
			Текст = НСтр("ru = 'Сумма по строкам табличной части не равна сумме документа. Документ не может быть проведен.'");
			Если ФлагОбменСбанками Тогда
				ДенежныеСредстваСервер.ДобавитьОшибкуЗаполнения(ОшибкиЗаполнения, Текст);
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				Текст,
				ЭтотОбъект,
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет И НДФЛПоВедомостям Тогда
		ДенежныеСредстваСервер.ПроверитьПеречислениеНДФЛпоВедомостям(ЭтотОбъект, РасшифровкаБезРазбиения, Отказ);
	КонецЕсли;
	
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаЗаймаСотруднику 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") И ПроведеноБанком Тогда
		...
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
			Если ТипНалога = Перечисления.ТипыНалогов.НДФЛ И НДФЛПоВедомостям Тогда
				ДенежныеСредстваСервер.РассчитатьИЗарегистрироватьПеречисленныйНДФЛ(ЭтотОбъект, Отказ);
			Иначе
				ДенежныеСредстваСервер.ЗарегистрироватьПеречислениеНалоговИВзносов(, ЭтотОбъект, Отказ);
			КонецЕсли;
		КонецЕсли;
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаЗаймаСотруднику Тогда
			Для Каждого Строка Из РасшифровкаПлатежа Цикл
				ЗаймыСотрудникам.ЗарегистрироватьПредоставлениеЗайма(
					Движения, Строка.ДоговорЗаймаСотруднику, Строка.Сумма, ДатаПроведенияБанком, Организация, ПодотчетноеЛицо, Отказ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ЗаполнитьПоОтчетуОператораСистемыПлатон(Знач ДокументОснование, Знач СуммаКОплате, ДанныеЗаполнения)
...
Процедура ЗаполнитьПоДоговоруЛизинга(
...
Процедура ЗаполнитьПоМассивуОснований(ДанныеЗаполнения, МассивОснований)
	...
	Если МассивОснований.Количество() Тогда
		...
		Если Метаданные.ОпределяемыеТипы.ОбъектРасчетов.Тип.СодержитТип(ТипЗнч(МассивОснований[0].ОбъектОплаты)) Тогда
...
Процедура ПроверитьЗаполнениеДоговоровГОЗ(Отказ, РасшифровкаБезРазбиения)
	...
	Для каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
		...
		Если ЗначениеЗаполнено(СтрокаРасшифровки.Заказ) Тогда
			...
			Если ТипЗнч(СтрокаРасшифровки.Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
