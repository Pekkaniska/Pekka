...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	Если Реквизиты <> Неопределено Тогда
		...
		//++ НЕ УТ
		Значения.Вставить("ОбособленноеПодразделение", Реквизиты.ОбособленноеПодразделение);
		Значения.Вставить("ОплатаЗаОбособленноеПодразделениеНаОтдельномБалансе",
		...
		ЗаполнитьЗначенияСвойств(ПараметрыПервичногоДокумента, Реквизиты);
		Значения.Вставить("РеквизитыПервичногоДокумента", УчетУСНСервер.РеквизитыПервичногоДокумента(Реквизиты.Ссылка, ПараметрыПервичногоДокумента));
		Значения.Вставить("Содержание", УчетУСНСервер.СодержаниеДохода(
...
Функция ТекстЗапросаТаблицаРасчетыПоФинансовымИнструментам(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаРасшифровкаПлатежа.ТипПлатежаПоЛизингу = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ОбеспечительныйПлатеж) ТОГДА
	|			&Ссылка
	|		ИНАЧЕ
	|			НЕОПРЕДЕЛЕНО
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	И ДанныеДокумента.Ссылка.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|	И ДанныеДокумента.Ссылка.Контрагент.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
	|	И ДанныеДокумента.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаЛизингодателю)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СписаниеДС.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК СписаниеДС
	|ГДЕ
	|	(СписаниеДС.Ссылка.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНалоговогоАгента)
	|			ИЛИ СписаниеДС.Ссылка.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате))
	|	И СписаниеДС.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
	//-- НЕ УТ
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	"ВЫБРАТЬ
	//++ НЕ УТ
	|	СписаниеДС.СчетУчета КАК СчетУчета,
	|	СписаниеДС.Субконто1 КАК Субконто1,
	|	СписаниеДС.Субконто2 КАК Субконто2,
	|	СписаниеДС.Субконто3 КАК Субконто3,
	|	ВЫБОР
	|		КОГДА СписаниеДС.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНалоговогоАгента)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.НДС_НалоговыйАгент)
	|		КОГДА СписаниеДС.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.НДС_ВвозимыеТовары)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ТипНалога,
	//-- НЕ УТ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			//++ НЕ УТ
			Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаЛизингодателю Тогда
				Партнер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент, "Партнер");
				Для каждого СтрокаРасшифровки Из ДокументОбъект.РасшифровкаПлатежа Цикл
					СтрокаРасшифровки.Партнер = Партнер;
				КонецЦикла;
			КонецЕсли;
			
			Если Выборка.ТипНалога <> Неопределено Тогда
				ДокументОбъект.ТипНалога = Выборка.ТипНалога;
				Для Каждого СтрокаРасшифровкиПлатежа Из ДокументОбъект.РасшифровкаПлатежа Цикл
					ЗаполнитьЗначенияСвойств(СтрокаРасшифровкиПлатежа, Выборка);
				КонецЦикла;
			КонецЕсли;
			//-- НЕ УТ
