...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	Если Регистры = Неопределено Тогда
		ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(НСтр("ru = 'Модернизация ОС'"));
	КонецЕсли;
...
Процедура ВременнаяТаблицаПоТабличнойЧасти(ТекстыЗапроса)
	...
	|			ТОГДА МодернизацияОСОС.ОсновноеСредство.НаправлениеДеятельности
	|		ИНАЧЕ МодернизацияОСОС.Ссылка.НаправлениеДеятельности
...
Процедура ВременнаяТаблицаСчетаУчетаОС(ТекстыЗапроса)
	...
	|		ПО ОсновныеСредства.ОсновноеСредство = ПараметрыЦелевогоФинансирования.ОсновноеСредство";
...
Процедура ПараметрыАмортизацииОСБухгалтерскийУчет(ТекстыЗапроса, Регистры)
	...
	|					И ОсновноеСредство В (ВЫБРАТЬ Т.ОсновноеСредство ИЗ ОсновныеСредства КАК Т)) КАК ПараметрыАмортизацииОСБУ
...
Функция ПрочиеРасходы(ТекстыЗапроса, Регистры)
	...
	|	-ОсновныеСредства.СуммаКапитальныхВложенийВключаемыхВРасходыНУ КАК ВременнаяРазница,
	|	
	...
	|	И ОсновныеСредства.СуммаКапитальныхВложенийВключаемыхВРасходыНУ > 0
	|";
...
Функция ВременнаяТаблицаДанныеДокумента()
	...
	|			ТОГДА ТабличнаяЧасть.ОсновноеСредство.НаправлениеДеятельности
...
Функция ПечатьОС3(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Запрос.УстановитьПараметр("СчетаНакопленияСтоимостиОС",
		Обработки.НастройкаОтраженияДокументовВРеглУчете.ДоступныеСчетаУчетаРасходов().СчетаОС);
