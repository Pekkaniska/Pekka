...
Процедура НастроитьЗависимыеЭлементыФормыНаКлиентеНаСервере(Форма, ИзмененныеРеквизиты = Неопределено)
	...
	Если ОбновитьВсе 
		...
		ИначеЕсли Объект.ВидОбъектаНалогаНаИмущество = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогаНаИмущество.ДвижимоеИмуществоПринятоеС2013") Тогда
			...
			Если Объект.Дата < '201801010000' ИЛИ Форма.ОсвобождениеОтНалогообложенияДвижимогоИмущества Тогда
...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(Знач ИзмененныеРеквизиты = Неопределено)
	...
	Если ОбновитьВсе Или СтруктураИзмененныхРеквизитов.Свойство("Дата") Тогда
		...
		Элементы.ВидОбъектаНалогаНаИмущество.Видимость = Объект.Дата < '201901010000';
	...
	Если ОбновитьВсе 
		...
		ПараметрыВыбораОС.Добавить(Новый ПараметрВыбора("Отбор.ВидНалога", ПредопределенноеЗначение("Перечисление.ВидыИмущественныхНалогов.НалогНаИмущество")));
		ПараметрыВыбораОС.Добавить(Новый ПараметрВыбора("Контекст", "БУ, МФУ, УУ"));
		
		Если Объект.Дата < '201901010000' Тогда
		...
		ИначеЕсли Объект.ВидОбъектаНалогаНаИмущество = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогаНаИмущество.ДвижимоеИмуществоПринятоеДо2013") Тогда
			...
			ПараметрыВыбораОС.Добавить(Новый ПараметрВыбора("Отбор.НедвижимоеИмущество", Ложь));
		...
		ИначеЕсли Объект.ВидОбъектаНалогаНаИмущество = ПредопределенноеЗначение("Перечисление.ВидыОбъектовНалогаНаИмущество.ДвижимоеИмуществоПринятоеС2013") Тогда
			...
			Если Объект.Дата >= '201801010000' 
				И Объект.ПостановкаНаУчетВНалоговомОргане <> ПредопределенноеЗначение("Перечисление.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане") Тогда
				
				Если ОбщаяНалоговаяСтавка <> 0 Тогда
					ТекстВыбора = СтрШаблон(НСтр("ru='Подлежит налогообложению, применяется общая ставка %1%%'"), ОбщаяНалоговаяСтавка);
				Иначе
...
Процедура ПолучитьСтавкуНалогаНаИмущество()
	...
	ТекстВыбора = СтрШаблон(НСтр("ru='Подлежит налогообложению, применяется ставка для движимого имущества %1%%'"), НалоговаяСтавкаДляДвижимогоИмущества);
