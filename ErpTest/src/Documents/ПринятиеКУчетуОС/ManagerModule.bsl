...
Функция ДанныеДокументаИУчета(ТекстыЗапроса)
	...
	ВнеоборотныеАктивыСлужебный.ТекстЗапросаВтСчетаВложенияВоВнеоборотныеАктивы(Ложь, ТекстыЗапроса);
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ВнеоборотныеАктивыСлужебный.ТекстЗапросаВтСчетаВложенияВоВнеоборотныеАктивы(Ложь, Неопределено));
...
Функция АмортизационнаяПремияДтКВ()
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	ВЫРАЗИТЬ(втДанныеСчетаКапитализации.СуммаНУ*(втДанныеДокумента.ПроцентКапитальныхВложенийВключаемыхВРасходыНУ/100) КАК ЧИСЛО(31,2)) КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|	
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуОС КАК ТаблицаОбъекта
	...
	|	ЕСТЬNULL(УчетнаяПолитикаОрганизаций.УчетнаяПолитика.ВключатьВСоставНалоговыхРасходовЛизинговыеПлатежи, ЛОЖЬ) КАК ВключатьВСоставНалоговыхРасходовЛизинговыеПлатежи,
	|	ЕСТЬNULL(УчетнаяПолитикаОрганизаций.ПрименяетсяУСНДоходыМинусРасходы, ЛОЖЬ) КАК ПрименяетсяУСНДоходыМинусРасходы,
	|	НЕ ЗаблокированныеУчетныеПолитикиРС.Организация ЕСТЬ NULL ИЛИ НЕ ЗаблокированныеУчетныеПолитикиСпр.Ссылка ЕСТЬ NULL КАК ЧитаемыеДанныеЗаблокированы
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированныеУчетныеПолитикиРС КАК ЗаблокированныеУчетныеПолитикиРС
	|		ПО (ЗаблокированныеУчетныеПолитикиРС.Организация = УчетнаяПолитикаОрганизаций.Организация)
	|			И (ЗаблокированныеУчетныеПолитикиРС.Период = УчетнаяПолитикаОрганизаций.Период)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированныеУчетныеПолитикиСпр КАК ЗаблокированныеУчетныеПолитикиСпр
	|		ПО (ЗаблокированныеУчетныеПолитикиСпр.Ссылка = УчетнаяПолитикаОрганизаций.УчетнаяПолитика)";
	...
	ПараметрыЗаблокированныхСсылок = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок(
					Параметры.Очередь, "Справочник.УчетныеПолитикиОрганизаций", МенеджерВременныхТаблиц);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТДляОбработки", ПараметрыОбработки.ИмяВременнойТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТЗаблокированныеУчетныеПолитикиРС", ПараметрыЗаблокированныхДанных.ИмяВременнойТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТЗаблокированныеУчетныеПолитикиСпр", ПараметрыЗаблокированныхСсылок.ИмяВременнойТаблицы);
 	...
 	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
 		...
 		Попытка
			...
			ИспользуетсяСтоимостьНУ = ДанныеОбъекта.СпособПоступления = Перечисления.СпособыПоступленияАктивов.ПоДоговоруЛизинга
					И Выборка.ВключатьВСоставНалоговыхРасходовЛизинговыеПлатежи;
					
			Если ИспользуетсяСтоимостьНУ Тогда
				
				СписокОС = ДанныеОбъекта.ОС.Выгрузить(, "ОсновноеСредство").ВыгрузитьКолонку("ОсновноеСредство");
				СтоимостьОС = Документы.ПоступлениеПредметовЛизинга.СтоимостьОСПоступившихВЛизинг(
								СписокОС, ДанныеОбъекта.Организация, ДанныеОбъекта.Дата);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ПринятиеКУчетуОС КАК ТаблицаДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуОС.ОС КАК ТаблицаОС
	|		ПО ТаблицаДокумента.Ссылка = ТаблицаОС.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константы КАК Константы
	|		ПО (Константы.ИспользоватьЛизинг)
	|ГДЕ
	|	ТаблицаОС.СтоимостьНУ = 0
	|	И ТаблицаДокумента.СпособПоступления = ЗНАЧЕНИЕ(Перечисление.СпособыПоступленияАктивов.ПоДоговоруЛизинга)
	...
	|		ПО ТаблицаДокумента.Организация = ОрганизацииУСНДоходыМинусРасходы.Организация
	|			И ТаблицаДокумента.Дата >= ОрганизацииУСНДоходыМинусРасходы.Период
	...
	|		ПО ТаблицаДокумента.Организация = ОстальныеОрганизации.Организация
	|			И ТаблицаДокумента.Дата >= ОстальныеОрганизации.Период
	|			И (НЕ ОстальныеОрганизации.ПрименяетсяУСНДоходыМинусРасходы)
	|ГДЕ
	|	ТаблицаДокумента.ПорядокУчетаНУ = ЗНАЧЕНИЕ(Перечисление.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Ссылка
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(ЕСТЬNULL(ОрганизацииУСНДоходыМинусРасходы.Период, ДАТАВРЕМЯ(1,1,1))) > МАКСИМУМ(ЕСТЬNULL(ОстальныеОрганизации.Период, ДАТАВРЕМЯ(1,1,1)))";
