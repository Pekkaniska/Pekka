...
&НаСервере
Процедура ЗаполнитьВидыРасчетовОсновногоОтпуска(ПервичнаяИнициализацияФормы)
...
Процедура ЗаполнитьВидыРасчетовДополнительныхОтпусков(ПервичнаяИнициализацияФормы)
	...
	Если Не ПервичнаяИнициализацияФормы Тогда
		...
		Для Каждого ДополнительныйОтпуск Из Объект.ДополнительныеОтпуска Цикл
			...
			ИначеЕсли ДополнительныйОтпуск.ВидОтпуска = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.ОтпускНаСанаторноКурортноеЛечение") Тогда
...
Процедура ПоказатьНадписьДней(Форма, Объект)
	...
	Форма.НадписьДней = ТекстНадписи;
	Форма.НадписьДнейКомпенсации = ТекстНадписи;
КонецПроцедуры	
