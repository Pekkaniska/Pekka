...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	СобытияФормВЕТИСКлиентПереопределяемый.ОбработкаВыбораПодборНоменклатуры(
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормВЕТИСПереопределяемый.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(ЭтотОбъект);
...
Процедура ПриСозданииЧтенииНаСервере(ОбновитьКомандыОснования = Истина)
	...
	ИнтеграцияВЕТИСПереопределяемый.НастроитьПодключаемоеОборудование(ЭтотОбъект);
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ХозяйствующийСубъект")
		...
		Если Не ЗначениеЗаполнено(Форма.ПредприятиеСоответствие)
			...
			И ЗначениеЗаполнено(Форма.ВладелецСкладИзОснованияДляОтбораПредприятий) Тогда
...
Процедура НастроитьВидимостьДоступностьЭлементовПоСтроке(ТекущиеДанные)
	...
	Элементы.ТоварыНадписьПроизводители.ТолькоПросмотр          = ЭлементыТолькоПросмотр;
	Элементы.ТоварыНадписьПроизводственныеПартии.ТолькоПросмотр = ЭлементыТолькоПросмотр;
	...
	Элементы.ГруппаСрокГодности.Видимость = НЕ ТипЖивыеЖивотные И ОтображатьСрокГодностиПоТочностиДаты;
	...
	Элементы.ГруппаУпаковкиШтрихкоды.Видимость = НЕ ТипЖивыеЖивотные;
...
Процедура НоменклатураПриИзменении()
	...
	ОформленоЧерезWEB = СтатусВЕТИС = ПредопределенноеЗначение("Перечисление.СтатусыОбработкиИнвентаризацийТоваровВЕТИС.ВыполненЧерезWeb");
...
Функция ТребуетсяУказаниеИдентификатораПартии(ИдентификаторСтроки)
	...
	Если ЗначениеЗаполнено(ДанныеСтроки.Номенклатура) Тогда
		Возврат ИнтеграцияВЕТИСПереопределяемый.ТребуетсяУказаниеИдентификатораПартии(ДанныеСтроки.Номенклатура);
	Иначе
		Возврат Ложь;
	КонецЕсли;
