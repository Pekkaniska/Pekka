...
// Устарело. Рекомендуется получать данные в переопределении со всеми вспомогательными полями,
//  требуемыми для заполнения прикладных документов
//  
Функция СтруктураЗапросаДанныеПроизводственнойОперации() Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ПроизводственнаяОперация.Дата КАК Дата,
	|
	...
	|
	|	(НЕ ПроизводственнаяОперация.Проведен) КАК ЕстьОшибкиПроведен,
	|
	|	ПроизводственнаяОперация.Комментарий КАК Комментарий
	|ИЗ
	|	Документ.ПроизводственнаяОперацияВЕТИС КАК ПроизводственнаяОперация
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХозяйствующиеСубъектыВЕТИС.Предприятия КАК ХозяйствующиеСубъектыВЕТИСПредприятия
	|		 ПО ПроизводственнаяОперация.ХозяйствующийСубъект = ХозяйствующиеСубъектыВЕТИСПредприятия.Ссылка
	|			И ПроизводственнаяОперация.Предприятие        = ХозяйствующиеСубъектыВЕТИСПредприятия.Предприятие
	|
	|ГДЕ
	|	ПроизводственнаяОперация.Ссылка = &Ссылка
	|;
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.ПроизводственнаяОперацияВЕТИС.Сырье КАК Сырье
	|ГДЕ
	|	Сырье.Ссылка = &Ссылка
	|;
	|";
	...
	СтруктураЗапроса = Новый Структура("Реквизиты,Товары,Сырье",ТекстыЗапросов[0],ТекстыЗапросов[1],ТекстыЗапросов[2]);
...
Функция ЗапросНаОформлениеПроизводственнойПартииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Для каждого ДанныеСтроки Из ДанныеДокумента.Сырье Цикл
		...
		СведенияОСырье = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ПроизводственнаяОперация, "rawBatch");
	...
	Для каждого ДанныеСтроки Из ДанныеДокумента.Товары Цикл
		...
		Продукция = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ПроизводственнаяОперация, "productiveBatch");
	...
	Для каждого ДанныеСтроки Из ДанныеДокумента.Товары Цикл
		...
		СведенияВСД = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "vetDocument");
	...
	Для Каждого ЭлементДанных Из ДанныеДокумента.Объект.stockEntryList.stockEntry Цикл
		...
		Если ДанныеЗаписиСкладскогоЖурнала <> Неопределено 
			И (ДанныеЗаписиСкладскогоЖурнала.Объект.Статус = Перечисления.СтатусыВерсионныхОбъектовВЕТИС.Создан
				ИЛИ ДанныеЗаписиСкладскогоЖурнала.Объект.Статус = Перечисления.СтатусыВерсионныхОбъектовВЕТИС.СозданВРезультатеПроизводственнойОперации) Тогда
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = ИнтеграцияВЕТИСПереопределяемый.ТекстЗапросаДвижениеСерийТоваров(
		Метаданные.Документы.ПроизводственнаяОперацияВЕТИС.Имя);
...
Функция ТипДокумента()
	...
	ТипДокумента = Метаданные.Документы.ПроизводственнаяОперацияВЕТИС;
