...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора)
	...
	Если УчетНДСУПКлиент.ЗаконченоРедактированиеЗаписиКнигиПокупокНаОсновании(РезультатВыбора, ИсточникВыбора) Тогда
...
Процедура ТекстЗаписьКнигиПокупокОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ДобавитьЗаписьКнигиПокупок" И пкНесколькоКонтрагентов() Тогда
		СтандартнаяОбработка = Ложь;
		
		пкСформироватьЗаписиПоНесколькимКонтрагентам();
	...
	//Рарус Владимир Подрезов Конец
		ПараметрыРегистрации = УчетНДСУПКлиентСервер.ПараметрыРегистрацииЗаписейКнигиПокупок();
		ПараметрыРегистрации.Ссылка = Объект.Ссылка;
		ПараметрыРегистрации.Организация = Объект.Организация;
		
		УчетНДСУПКлиент.ОбработкаНавигационнойСсылкиЗаписиКнигиПокупокНаОсновании(
			ЭтаФорма, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка, ПараметрыРегистрации);
	//Рарус Владимир Подрезов 09.03.2017
	КонецЕсли;
	//Рарус Владимир Подрезов Конец
...
Процедура ОбновитьТекстЗаписиКнигиПокупок()
	...
	ЗаписиНаОсновании = УчетНДСУП.ЗаписиКнигиПокупокНаОсновании(ПараметрыРегистрации);
	ТекстЗаписьКнигиПокупок = ЗаписиНаОсновании.Представление;
...
//Рарус Владимир Подрезов 09.03.2017
&НаКлиенте
Функция пкНесколькоКонтрагентов()
	...
	Для Каждого текСтрока Из Объект.ПрочиеРасходы Цикл
		Если текСтрока.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС") Тогда
			Продолжить;
		КонецЕсли;
		...
		Если тКонтрагент = ""
			...
			И тДата = "" 
		Тогда
			тКонтрагент	= текСтрока.Контрагент;
			тСтавка 	= текСтрока.СтавкаНДС;
			тНомер 		= текСтрока.НомерВходящегоДокумента;
			тДата	 	= текСтрока.ДатаВходящегоДокумента;
			Продолжить;
		КонецЕсли;
		Если тКонтрагент <> текСтрока.Контрагент
			ИЛИ тСтавка <> текСтрока.СтавкаНДС
			ИЛИ тНомер <> текСтрока.НомерВходящегоДокумента
			ИЛИ тДата <> текСтрока.ДатаВходящегоДокумента 
		Тогда
			ОдинКонтрагент = Ложь;
			Прервать;
		КонецЕсли;
	...
	Возврат Не ОдинКонтрагент;
	
КонецФункции

&НаКлиенте
Процедура пкСформироватьЗаписиПоНесколькимКонтрагентам()
	
	Если Модифицированность ИЛИ Объект.Ссылка.Пустая() ИЛИ Не Объект.Проведен Тогда
		ПоказатьПредупреждение(, НСтр("ru='Вводить на основании можно только проведенного документа!'"));
		Возврат;
	КонецЕсли;
	
	СписокДокументов = пкСформироватьЗаписиПоКонтрагентам();
	
	ОбновитьТекстЗаписиКнигиПокупок();

	//Для Каждого текСтрока Из СписокДокументов Цикл
	//	
	//	ПараметрыФормы = Новый Структура("Ключ", текСтрока.Значение);
...
&НаСервере
Функция пкСформироватьЗаписиПоКонтрагентам()
	
	СписокДокументов = Новый СписокЗначений;
	
	тПрочиеРасходы = Объект.ПрочиеРасходы.Выгрузить(,"Контрагент, Отменено, НомерВходящегоДокумента, ДатаВходящегоДокумента, СтавкаНДС, СуммаНДС, Сумма");
	РасходыПоКонтрагентам = тПрочиеРасходы.СкопироватьКолонки();
	Для Каждого текСтрока Из тПрочиеРасходы Цикл
		...
		НоваяСтрока = РасходыПоКонтрагентам.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, текСтрока);
		
	КонецЦикла;
	РасходыПоКонтрагентамСтавки = РасходыПоКонтрагентам.Скопировать();
	РасходыПоКонтрагентамСтавки.Свернуть("Контрагент, НомерВходящегоДокумента, ДатаВходящегоДокумента, СтавкаНДС", "СуммаНДС, Сумма");
	РасходыПоКонтрагентам.Свернуть("Контрагент, НомерВходящегоДокумента, ДатаВходящегоДокумента", "СуммаНДС, Сумма");
	
	Для Каждого текСтрока Из РасходыПоКонтрагентам Цикл
		
		тОбъект = Документы.ЗаписьКнигиПокупок.СоздатьДокумент();

		тОбъект.ДокументОснование		= Объект.Ссылка;
		тОбъект.ДокументРасчетов		= Объект.Ссылка;
		тОбъект.Организация				= Объект.Организация;
		тОбъект.Валюта					= Объект.Валюта;
		тОбъект.Подразделение			= Объект.Подразделение;
		тОбъект.КодВидаОперации			= "23";
		тОбъект.СпособКорректировкиНДС	= Перечисления.СпособыКорректировкиНДС.Скорректировать;
		
		тОбъект.Контрагент				= текСтрока.Контрагент;
		тОбъект.ПредъявленСчетФактура	= Истина;
		тОбъект.ДатаПолучения			= текСтрока.ДатаВходящегоДокумента;
		тОбъект.ДатаСчетаФактуры		= текСтрока.ДатаВходящегоДокумента;
		тОбъект.НомерСчетаФактуры		= текСтрока.НомерВходящегоДокумента;
		
		тОбъект.Дата					= Объект.Дата;
		тОбъект.УстановитьНовыйНомер();
		
		СтрокаДокументыОплаты = тОбъект.ДокументыОплаты.Добавить();
		СтрокаДокументыОплаты.ДатаОплаты		= Объект.Дата;
		СтрокаДокументыОплаты.ДокументОплаты	= Объект.Ссылка;
		
		НайденныеСтроки = РасходыПоКонтрагентамСтавки.НайтиСтроки(Новый Структура("Контрагент, НомерВходящегоДокумента, ДатаВходящегоДокумента",
			текСтрока.Контрагент, 
			текСтрока.НомерВходящегоДокумента, 
			текСтрока.ДатаВходящегоДокумента));
		
		Для Каждого текСтрокаЦенности Из НайденныеСтроки Цикл
			
			НоваяСтрока = тОбъект.Ценности.Добавить();
			...
			НоваяСтрока.СтавкаНДС	= текСтрокаЦенности.СтавкаНДС;
			НоваяСтрока.СчетУчета	= ПланыСчетов.Хозрасчетный.НДСпоПриобретеннымУслугам;
			НоваяСтрока.Субконто1	= текСтрока.Контрагент;
			НоваяСтрока.Сумма		= текСтрокаЦенности.Сумма - текСтрокаЦенности.СуммаНДС;
			НоваяСтрока.СуммаНДС	= текСтрокаЦенности.СуммаНДС;
			
		КонецЦикла;
		
		Попытка
			тОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
			СписокДокументов.Добавить(тОбъект.Ссылка);
		Исключение
