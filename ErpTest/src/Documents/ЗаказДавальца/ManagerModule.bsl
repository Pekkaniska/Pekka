...
// Заполняет служебный реквизит ДоговорНеОбязателен для заказов, введенных до перехода на версию 2.4.1.
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ЗаказДавальца КАК Реквизиты
	|ГДЕ
	|	Реквизиты.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	...
	|	Документ.ЗаказДавальца КАК Реквизиты
	|ГДЕ
	|	Реквизиты.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	Документ.ЗаказДавальца КАК ДокументДляОбработки
	|ГДЕ
	...
	|ИЗ (ВЫБРАТЬ
	|		ЗаказДавальцаЭтапыГрафикаОплаты.Ссылка,
	|		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказДавальцаЭтапыГрафикаОплаты.НомерСтроки) КАК Строк,
	|		ЗаказДавальцаЭтапыГрафикаОплаты.ВариантОплаты,
	|		ЗаказДавальцаЭтапыГрафикаОплаты.ДатаПлатежа
	|	ИЗ
	|		Документ.ЗаказДавальца.ЭтапыГрафикаОплаты КАК ЗаказДавальцаЭтапыГрафикаОплаты
	|	ГДЕ 
	|		ЗаказДавальцаЭтапыГрафикаОплаты.Ссылка.Проведен
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ЗаказДавальцаЭтапыГрафикаОплаты.Ссылка,
	|		ЗаказДавальцаЭтапыГрафикаОплаты.ВариантОплаты,
	...
	|	
	|	ИМЕЮЩИЕ
	|		КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗаказДавальцаЭтапыГрафикаОплаты.НомерСтроки) > 1) КАК ВложенныйЗапрос
	|");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеДокумента = Метаданные.Документы.ЗаказДавальца;
	...
	|	ОбъектыДляОбработки.Ссылка.ДоговорНеОбязателен   КАК ДоговорНеОбязателен,
	|	ОбъектыДляОбработки.Ссылка.Договор               КАК Договор,
	...
	|	ВЫБОР КОГДА
	|		ОбъектыДляОбработки.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|		И НЕ ОбъектыДляОбработки.ДоговорНеОбязателен
	|		ТОГДА
	|			ИСТИНА
	|		ИНАЧЕ
	|			ЛОЖЬ
	|		КОНЕЦ
	|	КАК                                       ТребуетсяУказаниеДоговорНеОбязателен,
	|	ВЫБОР КОГДА
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Выборка.ТребуетсяУказаниеДоговорНеОбязателен Тогда
				ДокументОбъект.ДоговорНеОбязателен = Истина;
			...
			Если ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ПустаяСсылка() Тогда
