...
Процедура ПередЗаписьюПредложитьПодобратьМногооборотнуюТаруЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Объект.ПорядокРасчетов <> ПредопределенноеЗначение("Перечисление.ПорядокРасчетов.ПоНакладным")
		...
		НадписьЭтапыОплаты = ЭтапыОплатыКлиентСервер.НадписьЭтапыОплаты(ЭтаФорма, Истина, СуммаЗаказано);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(
		Истина, "Документ.ЗаказДавальца.Форма.ФормаДокумента.Событие.ОбработкаВыбора");
...
Процедура ВидЦеныПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦены) И Объект.Продукция.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидЦеныПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Перезаполнить цены выпускаемой продукции?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ТипПроизводственногоПроцессаПриИзменении(Элемент)
	...
	Если Объект.ТипПроизводственногоПроцесса <> ТипПроизводственногоПроцессаДоИзменения Тогда
		...
		Для каждого Строка Из Объект.Продукция Цикл
			...
			Если Строка.Отменено Или Строка.Спецификация.Пустая() Тогда
				Продолжить;
			КонецЕсли;
			
			Идентификаторы.Добавить(Строка.ПолучитьИдентификатор());
			СпецификацииКУдалению.Добавить(Строка.Спецификация);
			
		КонецЦикла;
		
		Если Идентификаторы.Количество() > 0 Тогда
			
			ДополнительныеПараметры = Новый Структура("Идентификаторы, СпецификацииКУдалению", Идентификаторы, СпецификацииКУдалению);
			
			ТекстВопроса = НСтр("ru='Спецификации в табличной части ""Продукция"" будут перезаполнены, состав материалов по измененным спецификациям будет очищен. Продолжить?'");
			ОписаниеОповещения = Новый ОписаниеОповещения("ТипПроизводственногоПроцессаПриИзмененииЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ТипПроизводственногоПроцессаПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ТипПроизводственногоПроцессаПриИзмененииНаСервере(ДополнительныеПараметры);
...
Процедура ПродукцияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(
		Истина, "Документ.ЗаказДавальца.Форма.ФормаДокумента.Элементы.Продукция.Выбор");
	...
	Если Элемент.ТекущийЭлемент = Элементы.ПродукцияВариантОбеспечения И Не ТолькоПросмотр Тогда
		...
		Иначе
			...
			Если ОбеспечениеКлиент.ПроверитьЗаполнение(
				...
				Иначе
					Ошибки = ОбеспечениеКлиентСервер.ОшибкиКонтроляОтгрузкиИОбеспечения(ПараметрыФормы, "Продукция", НСтр("ru = 'Продукция'"));
...
Процедура ПродукцияПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не НоваяСтрока Тогда
		...
		Если Не КешСтроки = Неопределено 
			И Не ТекущиеДанные.Спецификация = КешСтроки.Спецификация Тогда
			
			СпецификацииКУдалению = СпецификацииКУдалению(Элемент.ВыделенныеСтроки, КешСтроки.Спецификация);
			Если СпецификацииКУдалению.Количество() Тогда
				
				ДополнительныеПараметры = Новый Структура;
				ДополнительныеПараметры.Вставить("СпецификацииКУдалению", СпецификацииКУдалению);
				ДополнительныеПараметры.Вставить("ТекущиеДанные", ТекущиеДанные);
				ДополнительныеПараметры.Вставить("КешСтроки", КешСтроки);
...
Процедура ПродукцияЗаполнитьЦеныПоВидуЦен(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "Продукция", НСтр("ru='Выпускаемая продукция'")) Тогда
...
Процедура ПродукцияЗаполнитьОсновнойСпецификацией(Команда)
	...
	ИсходныеСпецификации = Новый Массив;
	КешСпецификаций = Новый Соответствие;
	
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		
		ИсходнаяСпецификация = Объект.Продукция.НайтиПоИдентификатору(ВыделеннаяСтрока).Спецификация;
		
		// Формирование кеша спецификаций по строкам.
		КешСпецификаций.Вставить(ВыделеннаяСтрока, ИсходнаяСпецификация);
		
		Если Не ИсходныеСпецификации.Найти(ИсходнаяСпецификация) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		// Формирование общего списка спецификаций перед заполнением.
		ИсходныеСпецификации.Добавить(ИсходнаяСпецификация);
		
	КонецЦикла;
	
	Если ВыделенныеСтроки.Количество() Тогда
		
		// Заполнение основными спецификациями.
		ЗаполнитьОсновнуюСпецификациюВТЧПродукция(ВыделенныеСтроки);
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		СпецификацияПослеИзменения = Объект.Продукция.НайтиПоИдентификатору(ВыделеннаяСтрока).Спецификация;
		Индекс = ИсходныеСпецификации.Найти(СпецификацияПослеИзменения);
		
		СтруктураПоиска.Спецификация = КешСпецификаций[ВыделеннаяСтрока];
		Если Индекс = Неопределено И Объект.Материалы.НайтиСтроки(СтруктураПоиска).Количество() = 0 Тогда
			// Удаление спецификаций из контроля с отсутствующими материалами.
			Индекс = ИсходныеСпецификации.Найти(КешСпецификаций[ВыделеннаяСтрока]);
		КонецЕсли;
		
		Если Не Индекс = Неопределено Тогда
			ИсходныеСпецификации.Удалить(Индекс);
		КонецЕсли;
		
	КонецЦикла;
	
	Если ИсходныеСпецификации.Количество() Тогда
		
		// Контроль наличия в ТЧ Материалы строк со спецификациями, которые были заменены.
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("СпецификацииКУдалению", ИсходныеСпецификации);
		ДополнительныеПараметры.Вставить("КешСпецификаций", КешСпецификаций);
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПродукцияЗаполнитьОсновнойСпецификациейЗавершение", 
			ЭтотОбъект, 
			ДополнительныеПараметры);
		
		ПоказатьВопрос(ОписаниеОповещения, 
			НСтр("ru='В табличной части ""Материалы"" присутствуют строки по измененным спецификациям. Они будут удалены. Продолжить?'"),
			РежимДиалогаВопрос.ДаНет);
...
Процедура ПродукцияЗаполнитьОсновнойСпецификациейЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Нет Тогда
		...
		Для Каждого СтрокаКеша Из ДополнительныеПараметры.КешСпецификаций Цикл
			
			СтрокаПродукции = Объект.Продукция.НайтиПоИдентификатору(СтрокаКеша.Ключ);
			СтрокаПродукции.Спецификация = СтрокаКеша.Значение;
...
Процедура СостояниеОбеспечения(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(
		Истина, "Документ.ЗаказДавальца.Форма.ФормаДокумента.СостояниеОбеспечения");
...
Процедура ПриИзмененииДоговораСервер()
	...
	АктуализироватьНалогооблажениеНДС();
...
Процедура ПриИзмененииОрганизацииСервер(КэшированныеЗначения)
	...
	АктуализироватьНалогооблажениеНДС(Истина, КэшированныеЗначения);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ИмяТабличнойЧасти)
	...
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьЗаполнитьСклад",	ПараметрыЗаполненияСкладаВСтроке(Ложь));
...
Процедура ОбработатьШтрихкодыСервер(СтруктураПараметровДействия, КэшированныеЗначения)
	...
	Если СтруктураПараметровДействия.ИмяТЧ = "Продукция" И СтруктураПараметровДействия.ТекущаяСтрока <> Неопределено Тогда
		...
		МассивДанныхПоНоменклатуре.Добавить(Документы.ЗаказДавальца.ДанныеПоНоменклатуре(ТекущаяСтрока));
...
Процедура МатериалыЗаполнитьПоСпецификациямНаСервере()
	...
	Для Каждого СтрТЧ Из Объект.Продукция Цикл
		...
		МассивДанных.Добавить(Документы.ЗаказДавальца.ДанныеПоНоменклатуре(СтрТЧ));
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если СтрокаТовара.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Спецификация содержит материал с типом ""Многооборотная тара"" (%1). Добавление тары в заказ давальцу не допускается.'"),
			СтрокаТовара.Номенклатура);
...
&НаСервере
Процедура ЗаполнитьОсновнуюСпецификациюВТЧПродукция(ИдентификаторыСтрок)
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		МассивДанныхПоНоменклатуре.Добавить(Документы.ЗаказДавальца.ДанныеПоНоменклатуре(ДанныеСтроки));
...
Процедура ПродукцияЗаполнитьСобственныеМатериалыНаСервере(Идентификаторы, Знач МассивСтрок = Неопределено, ПересчитатьСуммы = Ложь, КэшированныеЗначения = Неопределено)
	...
	Если МассивСтрок = Неопределено Тогда
		...
		Если Не ТипЗнч(Идентификаторы) = Тип("Массив") Тогда
			СписокИдентификаторов.Добавить(Идентификаторы);
		Иначе
			СписокИдентификаторов = Идентификаторы;
		КонецЕсли;
		
		Для Каждого Идентификатор Из СписокИдентификаторов Цикл
			
			СтрокаПродукции = Объект.Продукция.НайтиПоИдентификатору(Идентификатор);
	...
	Документы.ЗаказДавальца.ЗаполнитьСтоимостьСобственныхМатериалов(МассивСтрок, ТаблицаСпецификаций, Объект.Дата, Объект.Материалы.Выгрузить());
...
//	Спецификация - СправочникСсылка.РесурснаяСпецификация - конкретная спецификация, которую надо проверить.
//
&НаКлиенте
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	АктуализироватьНалогооблажениеНДС(Ложь);
...
&НаСервере
Процедура АктуализироватьНалогооблажениеНДС(Заполнить = Истина, КэшированныеЗначения = Неопределено)
	
	ПараметрыЗаполнения = Документы.ЗаказДавальца.ПараметрыЗаполненияНалогооблаженияНДС(Объект);
...
&НаСервере
Процедура ТипПроизводственногоПроцессаПриИзмененииНаСервере(ДополнительныеПараметры = Неопределено)
	
	ЗаполнитьПараметрыВыбораСпецификаций();
	
	Если ДополнительныеПараметры <> Неопределено Тогда
		
		ЗаполнитьОсновнуюСпецификациюВТЧПродукция(ДополнительныеПараметры.Идентификаторы);
		ОчиститьМатериалыПоСпецификациям(Объект.Материалы, ДополнительныеПараметры.СпецификацииКУдалению);
...
Процедура ЗаполнитьДатыПоступления(Форма, ДатаПоступления, Знач МассивВыделенныхСтрок)
	...
	Если Не ЗначениеЗаполнено(ДатаПоступления) Тогда
		ДатаПоступления = ТекущаяДатаСеансаСервер();
	КонецЕсли;
	...
	Если ТипЗнч(Таблица) <> Тип("ТаблицаЗначений") Тогда
		Возврат Новый Структура(
			...
			ОбеспечениеКлиентСервер.ОшибкиКонтроляОтгрузкиИОбеспечения(
				Таблица, "Продукция", НСтр("ru = 'Продукция'")));
	КонецЕсли;
...
Процедура МатериалыПерезаполнитьПоПотребностямПроизводстваНаСервере(КэшированныеЗначения)
	...
	Иначе
		...
		ТекстСообщения =
			НСтр("ru = 'В заполнении материалов по фактической потребности производства нет необходимости.'") + Символы.ПС +
...
Процедура ОбновитьПредставлениеНазначения(ШаблонНазначенияМатериалы = Неопределено)
	...
	ПредставлениеНазначения = Справочники.Назначения.ПредставлениеНазначения(ШаблонНазначенияМатериалы, РеквизитыДокумента);
