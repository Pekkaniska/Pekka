...
&НаСервере
Процедура ПерезаполнитьНачисленияСотрудникаНаСервере(Сотрудники, СохранятьИсправления)
...
Процедура УстановитьДоступностьСуммыВычета()
	...
	СуммаВычетаНеРедактируется = ?(НЕ ЗначениеЗаполнено(КодДоходаНДФЛ) ИЛИ НЕ ЗначениеЗаполнено(Объект.КодВычетаНДФЛ), Истина, ВычетПоНормативу);
...
Процедура ПодробнееОРасчетеУдержаний(Команда)
	РасчетЗарплатыРасширенныйКлиент.ОткрытьФормуПодробнееОРасчетеУдержаний(Объект, "ВыплатаБывшимСотрудникам", ЭтаФорма, ОписаниеДокумента());
КонецПроцедуры
...
Процедура ЗаполнитьКВыплатеПоСтроке(Форма, ДанныеСтроки)
	...
	Если Форма.РасчетНДФЛНарастающимИтогомСНачалаГода Тогда
		...
		Для Каждого СтрокаНДФЛ Из СтрокиНДФЛ Цикл
			ДанныеСтроки.НДФЛ = ДанныеСтроки.НДФЛ + СтрокаНДФЛ.Налог - СтрокаНДФЛ.ЗачтеноАвансовыхПлатежей;
		КонецЦикла;
		ДанныеСтроки.НДФЛ = Макс(0, ДанныеСтроки.НДФЛ);
...
Функция СведенияОбНДФЛ(ФизическоеЛицо = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ВидВыплаты", Объект.ВидВыплаты);
	...
	|	ВидыВыплатБывшимСотрудникам.КодДоходаНДФЛ КАК КодДохода,
	|	ВидыВыплатБывшимСотрудникам.КодДоходаНДФЛ.СтавкаНалогообложенияРезидента КАК СтавкаНалогообложенияРезидента,
	...
	|ГДЕ
	|	ВидыВыплатБывшимСотрудникам.Ссылка = &ВидВыплаты";
	
	Запрос.Выполнить();
	
	СведенияОДоходах = Документы.ВыплатаБывшимСотрудникам.ДанныеДляПроведениеНДФЛ(Запрос.МенеджерВременныхТаблиц);
...
Процедура РассчитатьНДФЛИВзносыНаСервере(ФизическоеЛицо = Неопределено, ВыбраннаяСтрока = Неопределено)
	...
	Иначе
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ВидВыплаты, "КодДоходаНДФЛ, КодДоходаСтраховыеВзносы");
...
Процедура ОбновитьДанныеНДФЛНаСервере(АдресВременногоХранилища)
	...
	Если ФизическиеЛицаКРасчету.Количество() Тогда
		РассчитатьНДФЛИВзносыНаСервере(ФизическиеЛицаКРасчету);
	Иначе
