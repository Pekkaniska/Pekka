...
//+++rarus-spb_pavelk Сформируем документ Операция(регламентированный учет) по 007 счету
	ДобавитьДокументОперация();
//---rarus-spb_pavelk
...
//+++rarus-spb_pavelk Сформируем документ Операция(регламентированный учет) по 007 счету
Процедура ДобавитьДокументОперация()
	...
	Если Результат.Пустой() Тогда 
		...
		СтрЗаполнениеДвижений = НовДокумент.ЗаполнениеДвижений.Добавить();
		СтрЗаполнениеДвижений.Документ = Ссылка;
		НовДокумент.СуммаОперации = Задолженность.Итог("Сумма");
		НовДокумент.Ответственный = Пользователи.ТекущийПользователь();
		НовДокумент.ТиповаяОперация = Ссылка;
		НовДокумент.Записать(РежимЗаписиДокумента.Запись);
		
		ДвиженияХозрасчетный = НовДокумент.Движения.Хозрасчетный;
		Для Каждого СтрЗадолженность Из Задолженность Цикл
			НовДвижение = ДвиженияХозрасчетный.Добавить();
			НовДвижение.Активность = Ложь;
			НовДвижение.СчетДт = ПланыСчетов.Хозрасчетный.СписаннаяВУбытокЗадолженностьДебиторов;
			НовДвижение.СубконтоДт.Контрагенты = Контрагент;
			Если ЗначениеЗаполнено(СтрЗадолженность.Заказ) Тогда
				//+++rarus-spb_pavelk
				//НовДвижение.СубконтоДт.Договоры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрЗадолженность.Заказ, "Договор");
				Если ТипЗнч(СтрЗадолженность.Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда 
					НовДвижение.СубконтоДт.Договоры = СтрЗадолженность.Заказ;
				Иначе
					НовДвижение.СубконтоДт.Договоры = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрЗадолженность.Заказ, "Договор");
				КонецЕсли;
				//---rarus-spb_pavelk
				НовДвижение.СубконтоДт.ДокументыРасчетовСКонтрагентами = Ссылка;
			КонецЕсли;	
			НовДвижение.Сумма = НовДокумент.СуммаОперации;
