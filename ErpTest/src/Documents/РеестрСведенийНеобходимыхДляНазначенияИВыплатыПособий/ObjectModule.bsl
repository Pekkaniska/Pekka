...
Процедура ПроверитьПовторноВыбранныеИНепроведенныеЗаявления(Отказ)
	...
	Запрос.УстановитьПараметр("Заявления", СведенияНеобходимыеДляНазначенияПособий.Выгрузить(,"НомерСтроки, Заявление, ПервичныйДокумент"));
...
Процедура ПроверитьТребованияФормата(Отказ)
	...
	УстановитьПривилегированныйРежим(Истина);
	РеквизитыЗаявлений = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивЗаявлений, "ВидПособия,СпособВыплатыПособия,АдресПолучателяФактический,АдресПолучателяРегистрации,СтатусНалогоплательщикаПолучателя");
	УстановитьПривилегированныйРежим(Ложь);
	
	ТипАдресаФакт = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица");
	...
	ТипАдресаРегистрации = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица");
	...
	Для Каждого Строка Из СведенияНеобходимыеДляНазначенияПособий Цикл
		...
		Если РеквизитыЗаявления.СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.ПочтовымПереводом Тогда
			ТекстОшибки = ПроверитьАдрес(РеквизитыЗаявления.АдресПолучателяФактический, ТипАдресаФакт, ПредставлениеАдресаФакт);
		...
		Если РеквизитыЗаявления.СтатусНалогоплательщикаПолучателя = СтатусРезидент Тогда
			ТекстОшибки = ПроверитьАдрес(РеквизитыЗаявления.АдресПолучателяРегистрации, ТипАдресаРегистрации, ПредставлениеАдресаРегистрации);
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
...
Функция ПроверитьАдрес(ЗначениеАдреса, ТипАдреса, ЗаголовокПоля)
	Если Не ЗначениеЗаполнено(ЗначениеАдреса) Тогда
		Возврат СтрШаблон(НСтр("ru = 'Не заполнен %1'"), НРег(ЗаголовокПоля));
	КонецЕсли;
	
	ОписаниеАдреса = УчетПособийСоциальногоСтрахования.ОписаниеАдреса(ЗначениеАдреса, ТипАдреса);
	Если Дата > ДатаПереходаАдресовНаGUID() Тогда
		Если Не ЗначениеЗаполнено(ОписаниеАдреса.ИдентификаторОбъектаФИАС) Тогда
			Возврат СтрШаблон(НСтр("ru = 'В служебных данных адреса ""%1"" не заполнен идентификатор объекта ФИАС (GUID). Возможно, адрес введен некорректно, либо не обновлен классификатор адресов.'"), ЗаголовокПоля);
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ОписаниеАдреса.КодКЛАДР) Тогда
			Возврат СтрШаблон(НСтр("ru = 'В служебных данных адреса ""%1"" не заполнен код КЛАДР. Возможно, адрес введен некорректно, либо не обновлен классификатор адресов.'"), ЗаголовокПоля);
		КонецЕсли;
	КонецЕсли;
