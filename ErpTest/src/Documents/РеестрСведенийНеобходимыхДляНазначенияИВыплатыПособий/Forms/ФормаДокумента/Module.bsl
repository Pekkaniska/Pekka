...
Процедура ОбновитьУведомленияОПрекращенииОтпускаПоУходу(Форма, СтрокаСведений)
	Если СтрокаСведений <> Неопределено
		...
		Форма.Элементы.УведомлениеОПрекращенииОтпускаПоУходуСтраницы.ТекущаяСтраница = Форма.Элементы.УведомлениеОПрекращенииОтпускаПоУходуВидимаяСтраница;
	...
	Иначе
		Форма.Элементы.УведомлениеОПрекращенииОтпускаПоУходуСтраницы.ТекущаяСтраница = Форма.Элементы.УведомлениеОПрекращенииОтпускаПоУходуНевидимаяСтраница;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьПостановкуНаУчетВРанниеСрокиБеременности(СтрокаСведений)
	Если Элементы.СтраницыДанныеСтроки.ТекущаяСтраница = Элементы.СтраницаНетрудоспособность Тогда
...
Процедура СведенияНеобходимыеДляНазначенияПособийОбновитьКэшФормы(ОтборСтрок)
	...
	|		ПО ТаблицаЗаявлений.Заявление = ШапкаЗаявления.Ссылка
	|			И (ШапкаЗаявления.Проведен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Для Каждого СтрокаТаблицы Из ТаблицаЗаявлений Цикл
		...
		КэшСтроки = Новый Структура("СписокВыбораПервичногоДокумента, ПредставлениеУведомления, ПодробностиУведомления");
