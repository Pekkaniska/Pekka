...
Процедура ТиповыеДокументы(Команда)
	...
	Если ТекСтрока.ЗаказОтПодразделения Тогда
		ПоказатьПредупреждение(Неопределено, "ТД может быть введен только на основании заказа от контрагента!");
		Возврат;
	КонецЕсли;
	
	уатТиповыеДокументыКлиент.ВвестиТиповойДокумент(Новый ОписаниеОповещения("ТиповыеДокументыЗавершение", ЭтотОбъект), ТекСтрока.Ссылка);
КонецПроцедуры
