...
Процедура ИнициализироватьВтСуммыДокументовВВалютахУчета(Запрос, ТекстыЗапроса) Экспорт
	...
	|	Начисления.Ссылка.Дата КАК Дата,
	|	Начисления.Ссылка.Валюта КАК ВалютаДокумента,
	|	Начисления.Ссылка.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
...
//++ Рарус Лимаренко 19.12.2018
		//|	СУММА(ТаблицаНачислений.СуммаСНДС) * &КоэффициентПересчетаВВалютуУпр КАК Сумма
		|	СУММА(ТаблицаНачислений.СуммаВзаиморасчетов) * &КоэффициентПересчетаВВалютуУпр КАК Сумма
//-- Рарус Лимаренко 19.12.2018	
	|ИЗ
...
//++ Рарус Лимаренко 19.12.2018
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаНачислений.СуммаВзаиморасчетов * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(31,2))
//-- Рарус Лимаренко 19.12.2018
	|	КОНЕЦ КАК СуммаУпр,
	|	ВЫБОР
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ПО 
	|		ТаблицаАналитикУчетаПартий.НомерСтроки = Начисления.НомерСтроки
	|ГДЕ
	|	Начисления.Ссылка = &Ссылка
	|";
...
#Область ТекстЗачетОбеспечительногоПлатежа // (Дт <76.Л> :: Кт <76.ОЛП>)
	...
	|	ТОГДА
	|		ЕСТЬNULL(Суммы.СуммаБезНДСРегл, Строки.СуммаСНДС - Строки.СуммаНДС)
	|	КОГДА НЕ Операция.ЗакупкаПодДеятельность В (&ТипыНеОблагаемыеНДС)
	|	ТОГДА
	|		ЕСТЬNULL(Суммы.СуммаБезНДСРегл + Суммы.СуммаНДСРегл, Строки.СуммаСНДС)
	|	КОНЕЦ) КАК Сумма,
	|
	|	(ВЫБОР КОГДА Операция.ЗакупкаПодДеятельность В (&ТипыНеОблагаемыеНДС)
	|		ИЛИ НЕ СтатьиНДСРаспределяется.Ссылка ЕСТЬ NULL
	|	ТОГДА
	|		ЕСТЬNULL(Суммы.СуммаБезНДСУпр, (Строки.СуммаСНДС - Строки.СуммаНДС) / КурсВалютыУпрУчета.Курс)
	|	КОГДА НЕ Операция.ЗакупкаПодДеятельность В (&ТипыНеОблагаемыеНДС)
...
#Область ТекстНДС // НДС  (Дт <19.ХХ> :: Кт <76.Л, 76.ОЛП, 76.В>)
	...
	|	КОНЕЦ КАК СубконтоДт3,
	|	
//++ Рарус Лимаренко 19.07.2018 Задача№ 13214
	//|	0 КАК ВалютнаяСуммаДт,
	|	Строки.СуммаНДСВзаиморасчетов КАК ВалютнаяСуммаДт,
//-- Рарус Лимаренко 19.07.2018 Задача№ 13214
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ДанныеДокумента.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ПриобретениеУслугПоЛизингу.Начисления КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.УдалитьТипНачисления <> ЗНАЧЕНИЕ(Перечисление.УдалитьТипыНачисленийПоЛизингу.ПустаяСсылка)
	|	И ДанныеДокумента.ТипНачисления = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	Документ.ПриобретениеУслугПоЛизингу КАК Операция
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеУслугПоЛизингу.Начисления КАК Строки
	|		ПО Строки.Ссылка = Операция.Ссылка
	|ГДЕ
	|	Операция.Проведен
	|	И Строки.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеДокумента = Метаданные.Документы.ПриобретениеУслугПоЛизингу;
	...
	Выборка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, ПолноеИмяОбъекта, ДопПараметрыОбработки);
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ДокументОбъект.Партнер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.Контрагент, "Партнер");
			
			Для каждого СтрокаНачисление Из ДокументОбъект.Начисления Цикл
				
				Если НЕ ЗначениеЗаполнено(СтрокаНачисление.СтавкаНДС) Тогда
					СтрокаНачисление.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
				КонецЕсли;
				
				Если СтрокаНачисление.ТипНачисления <> Перечисления.ТипыПлатежейПоЛизингу.ПустаяСсылка() Тогда
					Продолжить;
				КонецЕсли;
				
				Если СтрокаНачисление.УдалитьТипНачисления = Перечисления.УдалитьТипыНачисленийПоЛизингу.УслугаПоЛизингу Тогда
					СтрокаНачисление.ТипНачисления = Перечисления.ТипыПлатежейПоЛизингу.ЛизинговыйПлатеж;
				ИначеЕсли СтрокаНачисление.УдалитьТипНачисления = Перечисления.УдалитьТипыНачисленийПоЛизингу.ЗачетОбеспечительногоПлатежа Тогда
					СтрокаНачисление.ТипНачисления = Перечисления.ТипыПлатежейПоЛизингу.ОбеспечительныйПлатеж;
				ИначеЕсли СтрокаНачисление.УдалитьТипНачисления = Перечисления.УдалитьТипыНачисленийПоЛизингу.ВыкупПредметаЛизинга Тогда
					СтрокаНачисление.ТипНачисления = Перечисления.ТипыПлатежейПоЛизингу.ВыкупПредметаЛизинга;
				ИначеЕсли СтрокаНачисление.УдалитьТипНачисления = Перечисления.УдалитьТипыНачисленийПоЛизингу.АрендныеОбязательства Тогда
					СтрокаНачисление.ТипНачисления = Перечисления.ТипыПлатежейПоЛизингу.АрендныеОбязательства;
				КонецЕсли;
			КонецЦикла;
