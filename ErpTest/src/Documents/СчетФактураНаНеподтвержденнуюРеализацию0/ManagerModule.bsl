...
Функция ТоварыПоДокументыОснованию(ДокументОснование) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		|		ВЫРАЗИТЬ((ВидыЗапасов.СуммаСНДС - ВидыЗапасов.СуммаНДС) * &КоэффициентПересчетаВвалютуРегл КАК ЧИСЛО(31,2))) КАК Сумма,
		|	0 КАК СуммаНДС,
		|	0 КАК СуммаСНДС,
		...
		|	ЕСТЬNULL(
		|		СуммыДокументовВВалютеРегл.БазаНДСРегл, 
		|		ВЫРАЗИТЬ((Товары.СуммаСНДС - Товары.СуммаНДС) * &КоэффициентПересчетаВвалютуРегл КАК ЧИСЛО(31,2))) КАК Сумма,
		|	0 КАК СуммаНДС,
		|	0 КАК СуммаСНДС,
		|	ВЫБОР 
		|		КОГДА Товары.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.Партнером)
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		...
		|	ЕСТЬNULL(
		|		СуммыДокументовВВалютеРегл.БазаНДСРегл, 
		|		ВЫРАЗИТЬ((Услуги.СуммаСНДС - Услуги.СуммаНДС) * &КоэффициентПересчетаВвалютуРегл КАК ЧИСЛО(31,2))) КАК Сумма,
		|	0 КАК СуммаНДС,
		|	0 КАК СуммаСНДС,
		|	ВЫБОР 
		|		КОГДА Услуги.Номенклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.Партнером)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.АгентскаяУслуга)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга)
		|	КОНЕЦ КАК ТипЗапасов,
		|	НЕОПРЕДЕЛЕНО КАК НомерГТД
