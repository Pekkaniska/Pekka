...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДокОснование) Тогда
		...
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ДоговорЗаймаСотруднику") Тогда
			ДенежныеСредстваСервер.ЗаполнитьПоДоговоруЗаймаСотруднику(ДокОснование, ДанныеЗаполнения, РасшифровкаПлатежа, Истина,
		...
		ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВыбытиеДенежныхДокументов") Тогда
			Документы.ВыбытиеДенежныхДокументов.ЗаполнитьПоОснованию(ЭтотОбъект, ДокОснование);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПогашениеЗаймаСотрудником Тогда
		Если Не ЗначениеЗаполнено(ПодотчетноеЛицо) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Сотрудник"" не заполнено.'"), , "ПодотчетноеЛицо", "Объект", Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		ДенежныеСредстваСервер.ЗаполнитьВладельцаОбъектаРасчета(ЭтотОбъект,
			ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратДенежныхСредствОтДругойОрганизации);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		...
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочееПоступлениеДенежныхСредств Тогда
			...
			ДенежныеСредстваСервер.СформироватьВозмещениеРасходовПоСоциальномуСтрахованию(
			,
			ЭтотОбъект,
			Отказ);
...
Процедура ПроверитьЗаполнениеПоВозвратуНеперечисленнойЗарплаты(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Если ЗначениеЗаполнено(ПодтверждениеЗачисленияЗарплаты) И ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНеперечисленнойЗарплатыПоЗарплатномуПроекту Тогда
		...
		Запрос.УстановитьПараметр("Ссылка", ПодтверждениеЗачисленияЗарплаты);
	...
	ИначеЕсли ЗначениеЗаполнено(ДокументВыдачи) И ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратНеперечисленныхДС Тогда
		
		СуммаВозврата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументВыдачи, "СуммаДокумента");
	...
	Если СуммаДокумента <> СуммаВозврата Тогда
