...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Сч = 1 По Бюджет.ШиринаТаблицы Цикл
		ШиринаКолонок.Добавить(Бюджет.Область(,Сч,,Сч).ШиринаКолонки);
	КонецЦикла;
	
	ХранилищеНастроекДанныхФорм.Сохранить(ЭтаФорма.ИмяФормы, "ШиринаКолонок_" + Объект.Ссылка.УникальныйИдентификатор(), ШиринаКолонок);
	
	ВысотаСтрок = Новый Массив;
	Для Сч = 1 По Бюджет.ВысотаТаблицы Цикл
		ВысотаСтрок.Добавить(Бюджет.Область(Сч,,Сч,).ВысотаСтроки);
	КонецЦикла;
	
	ХранилищеНастроекДанныхФорм.Сохранить(ЭтаФорма.ИмяФормы, "ВысотаСтрок_" + Объект.Ссылка.УникальныйИдентификатор(), ВысотаСтрок);
...
Процедура БюджетПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если Не Область.СодержитЗначение Тогда
		...
		Область.Текст = "";
		ПоказатьПредупреждение(, НСтр("ru = 'Редактирование ячейки не предусмотрено'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыгрузитьВExcel(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Перед выгрузкой в эксель документ следует записать'"));
		Возврат;
	КонецЕсли;
	
	Если Не МожноВыгрузитьТекущуюТаблицу(Бюджет.ТекущаяОбласть.Верх, ИДТаблицы, КолонкиПоИзмерениям) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно выгрузить текущую таблицу в однородный структурированный файл'"));
		Возврат;
	КонецЕсли;
...
Функция ВозможностьЧтенияПрисоединенногоФайла(ПрисоединенныйФайл)
	...
	ИначеЕсли НРЕГ(РеквизитыФайла.Расширение) <> "xls"
		И НРЕГ(РеквизитыФайла.Расширение) <> "xlsx" Тогда
		Результат.ЧтениеВозможно = Ложь;
		Результат.ТекстОшибки    = НСтр("ru = 'Файл не является файлом формата Excel, выбор запрещен.'");
	КонецЕсли;
...
Функция ПодготовитьПараметрыОтчета(Заполнение = Ложь, Расшифровка = Неопределено)
	...
	ПараметрыДанных = Документы.ЭкземплярБюджета.ПараметрыВыводаВидаБюджетаВТабличныйДокумент(Объект, СписокАналитикШапки, ВалютаСценария,
								ПолучитьИзВременногоХранилища(ПараметрыВыводаВидаБюджета), РежимФормирования,
...
Процедура СкопироватьПоПериодамПоРасшифровке(Расшифровка, Значение, ИсключаяПериоды = Неопределено)
	...
	НачалоПериода  = Расшифровка["Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность)];
	...
	Для Каждого Период Из МассивПериодов Цикл
		...
		Если Период = Расшифровка["Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность)]
		...
		Для Каждого Периодичность Из Периодичности Цикл
			РабочаяРасшифровка.Вставить("Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность),
		...
		ИзмененияЭкземпляраБюджета = БюджетнаяОтчетностьВыводСервер.РассчитатьСвязанныеСтатьиБюджетов(СтруктураПараметровВывода, АдресДанныхЯчеек, 
																							ПараметрыОтчета, РасшифровкаСвязанных, РезультатОтражения);
		...
		Для Строка = АктивнаяОбласть.Верх По АктивнаяОбласть.Низ Цикл
			Для Колонка = АктивнаяОбласть.Лево По АктивнаяОбласть.Право Цикл
				...
				Если ОбластьЗначения.СодержитЗначение Тогда
					...
					Период = Расшифровка["Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(МинимальнаяПериодичность)];
		...
		Для Колонка = АктивнаяОбласть.Лево По АктивнаяОбласть.Право Цикл
			...
			Если ОбластьЗначения.СодержитЗначение Тогда
				...
				ИзмененияЭкземпляраБюджета = БюджетнаяОтчетностьВыводСервер.РассчитатьСвязанныеСтатьиБюджетов(ПараметрыВыводаВидаБюджета, АдресДанныхЯчеек, 
																									ПараметрыОтчета, ОбластьЗначения.Имя, РезультатОтражения);
				ЗагрузитьРезультатРасчетаИзменений(, ИзмененияЭкземпляраБюджета);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПересчитатьНаСервере()
	...
	Если ДанныеЯчеек <> Неопределено Тогда
		ИзмененияЭкземпляраБюджета = БюджетнаяОтчетностьВыводСервер.РассчитатьСвязанныеСтатьиБюджетов(ПараметрыВыводаВидаБюджета, ДанныеЯчеек, 
																							ПараметрыОтчета, Неопределено, Неопределено, Истина);
		ЗагрузитьРезультатРасчетаИзменений(, ИзмененияЭкземпляраБюджета);
		ПараметрыОтчета = ПодготовитьПараметрыОтчета();
	КонецЕсли;
...
Функция СоздатьПрисоединенныйФайлXLS97(ИДТаблицы, КолонкиПоИзмерениям)
	...
	Исключение
		СообщениеОбОшибке =
			НСтр("ru = 'Не удалось выгрузить бюджет Excel. 
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru= 'Бюджетирование.Ввод экземпляров бюджета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Предупреждение, , ,
			НСтр("ru = 'Не удалось установить автофильтр при выгрузке бюджета в Excel'"));
	КонецПопытки;
...
Функция ЗагрузитьИзExcelНаКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Исключение
		СообщениеОбОшибке = 
			НСтр("ru = 'Не удалось загрузить данные из файла Excel. 
	...
	Исключение
		COMОбъект.Quit();
		COMОбъект = 0;
		СообщениеОбОшибке = НСтр("ru = 'Не удалось прочитать данные из файла. Подробности: %1'");
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение СообщениеОбОшибке;
	КонецПопытки;
...
Процедура УстановитьДоступностьЭлементовПослеПроверкиЗаполнения()
	...
	Элементы.ГруппаКоманднаяПанельБюджет.Доступность = МожноОбновитьБюджет(Ложь);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция РазрешеноРедактированиеАналитики(ТекущаяОбласть, АдресДанныхРасшифровки, Статус)
		...
		Для Каждого СтрокаДанных Из СтрокиДанных Цикл
			...
			Для Каждого Расшифровка Из СтруктураАналогРасшифровки Цикл
				...
				Если НайденныеСтроки.Количество() > 1 Тогда
					ВызватьИсключение НСтр("ru = 'Ошибка в структуре вида бюджета'")
...
Процедура ОтразитьИзмененияВДанных(ИндексСтроки, ИмяКолонки = "")
	...
	Иначе
		...
		ПараметрыОтчета = БюджетнаяОтчетностьКлиентСервер.ПараметрыПериода(Объект);
		ИзмененияЭкземпляраБюджета = БюджетнаяОтчетностьВыводСервер.РассчитатьСвязанныеСтатьиБюджетов(ПараметрыВыводаВидаБюджета, АдресДанныхЯчеек, 
																		ПараметрыОтчета, Расшифровка, РезультатОтражения, , ЕстьИзмененияВСвязанныхЯчейках);
...
Процедура ДобавитьПримечание(Команда)
	...
	ПоказатьВводСтроки(Оповещение, Примечание, НСтр("ru = 'Примечание'"), 500, Истина);
...
Процедура ДобавитьСтроку(Команда)
	...
	ПараметрыРедактирования = ПреобразоватьПараметрыРедактированияВСтруктуру(ПараметрыРедактирования, МассивСтрок, Бюджет.ТекущаяОбласть.Верх);
	Если ПараметрыРедактирования <> Неопределено Тогда
		СтандартнаяОбработка = Ложь;
		ДобавитьСтрокуВРежимеТаблицы(ПараметрыРедактирования);
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Операция не доступна, аналитика не динамическая.'"));
	КонецЕсли;
...
Процедура УдалитьСтроку(Команда)
	...
	ПодчиненныеСтроки = ПолучитьПодчиненныеСтроки(МассивСтрок);
	...
	РезультатУдаления = УдалитьСтрокуНаСервере(МассивСтрок, МассивРасшифровокПоСтрокам, ПараметрыВыводаВидаБюджета);
	
	Если РезультатУдаления <> Неопределено Тогда
		ПоказатьПредупреждение(, РезультатУдаления);
	КонецЕсли;
...
&НаСервере
Функция ПолучитьПодчиненныеСтроки(МассивСтрок, Знач ДеревоЭлементов=Неопределено)
...
&НаСервереБезКонтекста
Функция ПодчиненныеЭлементыРекурсивно(СтрокаОтчета, МассивПодчиненныхЭлементов, НачальныйИндекс = Неопределено)
	...
	Для Каждого СтрокаДерева Из СтрокаОтчета.Строки Цикл
		...
		ПодчиненныеЭлементыРекурсивно(СтрокаДерева, МассивПодчиненныхЭлементов)
	КонецЦикла;
...
Функция ДобавитьСтрокуНаСервере(МассивСтрок, АдресПараметровВидаБюджета)
	...
	Если Не ПараметрыРедактирования.Количество() Тогда
		Для Каждого ТекущаяСтрока Из МассивСтрок Цикл
			...
			ПодчиненныеЭлементыРекурсивно(СтрокаОтчета, МассивПодчиненныхЭлементов);
			...
			ПодчиненныеЭлементыРекурсивно(РодительСтроки, МассивПодчиненныхЭлементов, РодительСтроки.Строки.Индекс(СтрокаОтчета));
			ПараметрыРедактирования = ПараметрыРедактированияСтроки(МассивПодчиненныхЭлементов, АдресПараметровВидаБюджета, Истина);
			Если ПараметрыРедактирования.Количество() Тогда
				Прервать;
			КонецЕсли;
...
&НаСервере
Функция УдалитьСтрокуНаСервере(МассивСтрок, МассивРасшифровокПоСтрокам, АдресПараметровВидаБюджета)
	
	ОбъектИПараметрыРасчета = БюджетнаяОтчетностьКлиентСервер.СтруктураПоОбъектуЭкземпляраБюджета(ЭтаФорма);
	ДанныеЯчеек = ПолучитьИзВременногоХранилища(АдресДанныхЯчеек);
	ДеревоЭлементов = ПолучитьИзВременногоХранилища(АдресПараметровВидаБюджета).ДеревоВывода;
	
	МинимальнаяУдаляемаяСтрока = Неопределено; УдаленоСтрок = 0;
	...
	Для Сч = 0 По МассивСтрок.ВГраница() Цикл
		...
		НайденныеСтроки = ДеревоЭлементов.Строки.НайтиСтроки(Новый Структура("ЭлементОтчета", ЭлементМассива), Истина);
		МожноУдалитьСтроку = Не (НайденныеСтроки.Количество() > 0 И НайденныеСтроки[0].Строки.Количество() > 0) И МассивСтрок.Количество() = 1;
		...
		ПараметрыРедактирования = ПараметрыРедактированияСтроки(
										ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ЭлементМассива), АдресПараметровВидаБюджета, Ложь);
		...
		Если ЗначенияАналитик = Неопределено И МассивРасшифровок.Количество() > 0 И НайденныеСтроки.Количество() > 0 Тогда
			Если МассивРасшифровок[0].Свойство("ЗначенияАналитик") И МассивРасшифровок[0].ЗначенияАналитик.Количество() > 0 Тогда
		...
		Для Каждого Расшифровка Из МассивРасшифровок Цикл
			...
			РезультатОтражения = БюджетнаяОтчетностьКлиентСервер.ОтразитьИзменениеЯчейки(ОбъектИПараметрыРасчета, Расшифровка, 0, "");
			...
			Для Каждого ЗначениеПериодичности Из Перечисления.Периодичность Цикл
				ПериодСтрокой = "Период" + ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(ЗначениеПериодичности);
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				Если ЭтоПустаяСтрока ИЛИ НЕ МожноУдалитьСтроку Тогда
					ИмяОбласти = "";
					Если ЗначениеЗаполнено(НайденнаяСтрока.ИмяОбластиСумма) Тогда
						ИмяОбласти = НайденнаяСтрока.ИмяОбластиСумма;
					ИначеЕсли ЗначениеЗаполнено(НайденнаяСтрока.ИмяОбластиКоличество) Тогда
						ИмяОбласти = НайденнаяСтрока.ИмяОбластиКоличество;
					ИначеЕсли ЗначениеЗаполнено(НайденнаяСтрока.ИмяОбластиЗначение) Тогда
						ИмяОбласти = НайденнаяСтрока.ИмяОбластиЗначение;
					КонецЕсли;
		...
		Если Не ЭтоПустаяСтрока И МожноУдалитьСтроку Тогда
			...
			Если МинимальнаяУдаляемаяСтрока = Неопределено Тогда
				МинимальнаяУдаляемаяСтрока = ОписаниеСтроки.НомерСтроки - 1;
			Иначе
				МинимальнаяУдаляемаяСтрока = Мин(МинимальнаяУдаляемаяСтрока, ОписаниеСтроки.НомерСтроки - 1);
			КонецЕсли;
...
Процедура СместитьЯчейкиДанныхЯчеек(ДанныеЯчеек, НомерВерхнейСтроки, КоличествоСтрок)
	...
	Для Каждого СтрокаДанныхЯчеек Из ДанныеЯчеек Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДанныхЯчеек.ИмяОбластиСумма) Тогда
			ИмяОбласти = СтрокаДанныхЯчеек.ИмяОбластиСумма;
		ИначеЕсли ЗначениеЗаполнено(СтрокаДанныхЯчеек.ИмяОбластиКоличество) Тогда
			ИмяОбласти = СтрокаДанныхЯчеек.ИмяОбластиКоличество;
		ИначеЕсли ЗначениеЗаполнено(СтрокаДанныхЯчеек.ИмяОбластиЗначение) Тогда
			ИмяОбласти = СтрокаДанныхЯчеек.ИмяОбластиЗначение;
		КонецЕсли;
...
Процедура ДобавитьСтрокуВРежимеТаблицы(ПараметрыРедактированияОбласти)
	...
	НайденнаяСтрока = ФинансоваяОтчетностьКлиентСервер.ПодчиненныйЭлемент(ДеревоЭлементовБюджета, "ИДГруппировки", 
			ПараметрыРедактированияОбласти.Расшифровка.ИДГруппировки);
	...
	ОткрытьФорму("Документ.ЭкземплярБюджета.Форма.ВводСтрокиСАналитикой", ПараметрыФормы,,,,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДобавитьСтрокуВБюджетЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВставки = НСтр("ru = 'Дубли строк'") Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Строка с такими значениями уже есть в бюджете'"));
	Иначе
		Модифицированность = Истина;
	КонецЕсли;
...
Функция ВставитьОбластьНаСервере(СоответствиеИзмерений, ДополнительныеПараметры)
	...
	Для Каждого СтрокаСтруктурыЯчеек Из НайденныеСтроки Цикл
		...
		Если СтрокиПроверки.Количество() Тогда
			Возврат НСтр("ru = 'Дубли строк'"); // дубли строк
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаСтруктурыЯчеек Из НайденныеСтроки Цикл
		...
		Для Каждого СтрокаПериодов Из ТаблицаПериодов Цикл
			...
			ПериодСтрокой = ";";
			СтруктураПоискаПроверка = Новый Структура;
			...
			Для Каждого Периодичность Из Периодичности Цикл
				ПериодичностьСтрокой = ФинансоваяОтчетностьКлиентСерверПовтИсп.ПериодичностьСтрокой(Периодичность);
				СтруктураПоискаПроверка.Вставить("Период" + ПериодичностьСтрокой, СтрокаПериодов["Период" + ПериодичностьСтрокой]);
				
				Если ДанныеЯчеек.НайтиСтроки(СтруктураПоискаПроверка).Количество() > 0 Тогда
					СтруктураПоиска.Вставить("Период" + ПериодичностьСтрокой, СтрокаПериодов["Период" + ПериодичностьСтрокой]);
					ПериодСтрокой = ПериодСтрокой + "Период" + ПериодичностьСтрокой
						+"{" + Строка(СтрокаПериодов["Период" + ПериодичностьСтрокой]) + "};";
				Иначе
					Если СтруктураПоиска.Свойство("Период" + ПериодичностьСтрокой) Тогда
						СтруктураПоиска.Удалить("Период" + ПериодичностьСтрокой);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			
			Если ДобавленныеПериоды.Получить(ПериодСтрокой) = Неопределено Тогда
				ДобавленнаяСтрока = БюджетнаяОтчетностьВыводСервер.ДобавитьСтрокуДанныхЯчеек(
					ДанныеЯчеек,
	...
	КэшДопРеквизиты = ФинансоваяОтчетностьСервер.КэшДополнительныеРеквизиты(Объект.ВидБюджета);
	КэшРеквизитов   = БюджетнаяОтчетностьРасчетКэшаСервер.КэшРеквизитовЭлементовВидаБюджета(Объект.ВидБюджета);
	...
	ГруппировкаДерева = ФинансоваяОтчетностьКлиентСервер.ПодчиненныйЭлемент(ДеревоДляВыводаОтчета, "ИДГруппировки", Расшифровка.ИДГруппировки);
	...
	БюджетнаяОтчетностьВыводСервер.ВывестиТаблицуВОтчет(ПараметрыВывода, МассивСтрок, НомерТаблицы, СтруктураИзмерений, Уровень);
	...
	Для Строка = ОбластьПриемникДопВерх По ОбластьПриемникДопНиз Цикл
		Для Колонка = ОбластьПриемникДопЛево По ОбластьПриемникДопПраво Цикл
			...
			Если Расшифровка.Свойство("ИндексСтрокиДанных") Тогда
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ИмяОбластиСумма) Тогда
					СтрокаДанных.ИмяОбластиСумма = ОбластьДокумента.Имя;
				ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.ИмяОбластиКоличество) Тогда
					СтрокаДанных.ИмяОбластиКоличество = ОбластьДокумента.Имя;
				ИначеЕсли ЗначениеЗаполнено(СтрокаДанных.ИмяОбластиЗначение) Тогда
...
Функция ПодготовитьВспомогательныеДанныеБюджета()
	...
	Для Каждого ЭлементУО Из УсловноеОформление.Элементы Цикл
		...
		СтилиСтрокДерева = СтильУровней[КлючТаблицы];
		Если СтилиСтрокДерева = Неопределено Тогда
			СтилиСтрокДерева = Новый Соответствие;
			СтильУровней.Вставить(КлючТаблицы, СтилиСтрокДерева);
		КонецЕсли;
		
		СтилиСтрокДерева.Вставить(КлючУровня, Новый Структура("ЦветФона", ЦветФона));
...
Процедура ПолучитьДоступностьЭлементовПоБюджетнойЗадаче()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ВызватьИсключение НСтр("ru='Создание документа возможно только из бюджетной задачи.'");
	КонецЕсли;
