...
Процедура ЗаполнитьБюджетПоДаннымИнформационнойБазы(Объект) Экспорт
	...
	КэшРеквизитовЭлементовБюджета.Вставить("КэшДопРеквизитов", ФинансоваяОтчетностьСервер.КэшДополнительныеРеквизиты(Объект.ВидБюджета));
	КэшРеквизитовЭлементовБюджета.Вставить("КэшРеквизитов", БюджетнаяОтчетностьРасчетКэшаСервер.КэшРеквизитовЭлементовВидаБюджета(Объект.ВидБюджета));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	"ВЫБРАТЬ
		...
		|ИЗ
		|	Документ.ЭкземплярБюджета КАК Таблица
		|ГДЕ
		...
		|	Таблица.Ссылка
		|ИЗ
		|	Документ.ЭкземплярБюджета.АналитикаСтатейБюджетов КАК Таблица
		|ГДЕ
		|	(Таблица.Аналитика1 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика2 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика3 В (&ПустыеЗначенияДляОбработки)
		...
		|	Таблица.Ссылка
		|ИЗ
		|	Документ.ЭкземплярБюджета.АналитикаЗначений КАК Таблица
		|ГДЕ
		|	(Таблица.Аналитика1 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика2 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика3 В (&ПустыеЗначенияДляОбработки)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяМетаданных = "Документ.ЭкземплярБюджета";
	МетаданныеОбъекта = Метаданные.Документы.ЭкземплярБюджета;
	...
	Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
		...
		РеквизитыПоиска.Добавить("АналитикаСтатейБюджетов.Аналитика" + Сч);
		РеквизитыПоиска.Добавить("АналитикаЗначений.Аналитика" + Сч);
	КонецЦикла;
