...
Процедура УстановитьДоступностьЭлементовПоСтатусу(УстанавливатьТолькоПросмотр)
	...
	//++ НЕ УТ
	Элементы.ДоговорСЗаказчиком.ТолькоПросмотр = ТолькоПросмотрЭлементов;
	Элементы.ТипПлатежаФЗ275.ТолькоПросмотр = ТолькоПросмотрЭлементов;
	//-- НЕ УТ
...
Процедура ЗаполнитьРеквизитыГОЗ()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеДенежныхСредствНаДругойСчет Тогда
		...
		РеквизитыСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийБанковскийСчет, "ОтдельныйСчетГОЗ, ГосударственныйКонтракт");
		Объект.ПлатежиПо275ФЗ = РеквизитыСчета.ОтдельныйСчетГОЗ;
		
		Если Объект.ПлатежиПо275ФЗ Тогда
			
			ДоговорыСЗаказчиком = Справочники.ГосударственныеКонтракты.ДоговорыПоГосударственномуКонтракту(РеквизитыСчета.ГосударственныйКонтракт);
