...
Процедура СоздатьВТПерсональныеДанныеСотрудников(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ОкончаниеОтчетногоПериода, ФизическиеЛица = Неопределено) Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
	               |		КОГДА &Период >= ДАТАВРЕМЯ(2017, 1, 1)
...
Функция СформироватьВыходнойФайл(Ссылка)
	...
	Если ВыборкаПоДаннымВзносов.Следующий()  
		...
		ФорматОДВ_1.Страховая.Значение.ИС2__ЗадолженностьНаНачало = ВыборкаПоДаннымВзносов.ЗадолженностьНаНачалоСтраховая;
		...
		ФорматОДВ_1.Страховая.Значение.ИС2__ЗадолженностьНаКонец = ВыборкаПоДаннымВзносов.ЗадолженностьНаКонецСтраховая;
		
		ФорматОДВ_1.Накопительная.Значение.ИС2__ЗадолженностьНаНачало = ВыборкаПоДаннымВзносов.ЗадолженностьНаНачалоНакопительная;
		...
		ФорматОДВ_1.Накопительная.Значение.ИС2__ЗадолженностьНаКонец = ВыборкаПоДаннымВзносов.ЗадолженностьНаКонецНакопительная;
		
		ФорматОДВ_1.ТарифСВ.Значение.ИС2__ЗадолженностьНаНачало = ВыборкаПоДаннымВзносов.ЗадолженностьНаНачалоТарифСВ;
		...
		ФорматОДВ_1.ТарифСВ.Значение.ИС2__ЗадолженностьНаКонец = ВыборкаПоДаннымВзносов.ЗадолженностьНаКонецТарифСВ;
	...
	Пока ВыборкаУплаченныхВзносов.Следующий() Цикл 
		...
		НаборЗаписейТипУплата.ИС2__Год.Значение =ВыборкаУплаченныхВзносов.Год;
	...
	ФорматЗЛ = Новый Структура("ЗЛ", ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_КОРР.ЗЛ));
	ФорматЗЛ.ЗЛ.Значение.Удалить("СтажевыйПериод");
	ФорматЗЛ.ЗЛ.Значение.Удалить("ДатаУвольнения");
	ФорматСЗВ_КОРР.Удалить("ЗЛ");
	
	ФорматДанныеЗЛ = Новый Структура("ДанныеЗЛ", ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_КОРР.ДанныеЗЛ));
	ФорматСЗВ_КОРР.Удалить("ДанныеЗЛ");
	
	ФорматСуммы = Новый Структура("Суммы", ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_КОРР.Суммы));
	ФорматСЗВ_КОРР.Удалить("Суммы");
	
	ФорматВыплатыДТ = Новый Структура("ВыплатыДТ", ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_КОРР.ВыплатыДТ));
	ФорматСЗВ_КОРР.Удалить("ВыплатыДТ");
	
	ФорматСтажевыйПериод = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_КОРР.СтажевыйПериод.Значение);
	ФорматСЗВ_КОРР.Удалить("СтажевыйПериод");
	
	НаборЗаписейДатаУвольнения = Новый Структура("ДатаУвольнения", ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_КОРР.ДатаУвольнения));
	...
	Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		НаборЗаписейКорректируемыйПериод.Страхователь.Значение.Удалить("ИС2__Наименование");
		...
		// Данные ЗЛ
		НаборЗаписейДанныеЗЛ = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматДанныеЗЛ);
		...
		Для Каждого КодМесяца Из СписокМесяцев Цикл 
			...
			НаборЗаписейСуммы = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСуммы);
		...
		Для Каждого КодМесяца Из СписокМесяцев Цикл 
			СтруктураПоиска.КодМесяца = КодМесяца;
			НаборЗаписейВыплатыДТ = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматВыплатыДТ);
...
Процедура ВывестиЛьготныйСтаж(ВыборкаПоРаботникам, УзелСтажевыйПериод, ФорматЛьготныйСтаж, ПараметрыСтажаСПравомНаДосрочнуюПенсию)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВыборкаПоРаботникам.ТерриториальныеУсловияКод) Тогда
			...
			ПерсонифицированныйУчет.ВписатьСтавкуВНаборДанныхТеррУсловий(ВыборкаПоРаботникам.ТерриториальныеУсловия, ВыборкаПоРаботникам.ТерриториальныеУсловияСтавка, НаборЗаписейЛьготныйСтаж.ИС2__ТУ);
		Иначе
			НаборЗаписейЛьготныйСтаж.ИС2__ТУ.НеВыводитьВФайл = Истина;	
		КонецЕсли;	
	...
	Если ВыводитьЛьготныйСтаж Тогда
		УзелЛьготныйСтаж = ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелСтажевыйПериод, "ИС2__ЛьготныйСтаж","");
...
Функция СформироватьВыходнойФайл2019(Ссылка)
	...
	Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		ВписатьЗаписиОСтажеВНаборДанных2019(УзелФормы, ФорматСтажевыйПериод, ВыборкаПоРаботникам);
...
Процедура ВывестиЛьготныйСтаж2019(ВыборкаПоРаботникам, УзелСтажевыйПериод, ФорматЛьготныйСтаж, ПараметрыСтажаСПравомНаДосрочнуюПенсию)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВыборкаПоРаботникам.ТерриториальныеУсловияКод) Тогда
			...
			ПерсонифицированныйУчет.ВписатьСтавкуВНаборДанныхТеррУсловий2019(ВыборкаПоРаботникам.ТерриториальныеУсловия, ВыборкаПоРаботникам.ТерриториальныеУсловияСтавка, НаборЗаписейЛьготныйСтаж.ИС4__ТУ);
		Иначе
			НаборЗаписейЛьготныйСтаж.ИС4__ТУ.НеВыводитьВФайл = Истина;	
		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(ВыборкаПоРаботникам.ОснованиеВыслугиЛетКод) Тогда
			...
			ПерсонифицированныйУчет.ВписатьСтавкуВНаборДанныхИсчисляемогоСтажа2019(ВыборкаПоРаботникам.ТретийПараметрВыслугиЛет, НаборЗаписейВыслугаЛет);
			Если ЗначениеЗаполнено(ВыборкаПоРаботникам.ТретийПараметрИсчисляемогоСтажа) Тогда	
				НаборЗаписейВыслугаЛет.ВС2__ВыработкаВЧасах.НеВыводитьВФайл = Истина;
				НаборЗаписейВыслугаЛет.ВС2__ВыработкаКалендарная.НеВыводитьВФайл = Истина;
			ИначеЕсли Не ЗначениеЗаполнено(ВыборкаПоРаботникам.ПервыйПараметрВыслугиЛет) И Не ЗначениеЗаполнено(ВыборкаПоРаботникам.ВторойПараметрВыслугиЛет) Тогда
...
Функция ЗапросПоШапкеДляПечати(СписокДокументов)
	...
	|	ДанныеОКорректировкеСведений.Формат2019,
...
Функция ПечатнаяФормаСЗВ_КОРР(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		Макет = ?(ВыборкаПоШапкеДокумента.Формат2019, 
			...
			УправлениеПечатью.МакетПечатнойФормы("Документ.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР.ПФ_MXL_ФормаСЗВ_КОРР"));
		...
		Если ВыборкаПоРаботникам.НайтиСледующий(Новый Структура("Ссылка", ВыборкаПоШапкеДокумента.Ссылка)) Тогда
			...
			Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
				...
				Если Не ВыборкаПоШапкеДокумента.Формат2019 Тогда 
					...
					ПерсонифицированныйУчет.ВывестиДанныеПоБуквенно(НомерДляПечати, ОбластьШапка, "НомСтр", 3);
				КонецЕсли;
