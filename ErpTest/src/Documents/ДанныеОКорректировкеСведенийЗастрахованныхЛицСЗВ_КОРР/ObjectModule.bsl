...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЗначениеЗаполнено(ИмяФайлаДляПФР) Тогда
		ИмяФайлаДляПФР = Документы.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР.ИмяФайла(Организация, Дата);
	Иначе		
		...
		КонецЕсли;	
		ИмяФайлаДляПФР = Документы.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР.ИмяФайла(Организация, Дата, УИДИзИмениФайла);
	КонецЕсли;
...
Процедура ПроверитьДанныеДокумента(Отказ = Ложь) Экспорт 
	...
	Если КорректируемыйПериод >= '20140101' Тогда 
		ПроверяемыйПериод = КорректируемыйПериод;
		ПерсонифицированныйУчет.ПроверитьЗаписиОСтаже(МенеджерВременныхТаблиц, Ссылка, ПроверяемыйПериод, Отказ, Ложь, Истина, Истина);
	КонецЕсли;
