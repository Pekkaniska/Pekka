...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ВидЦены) Тогда
		...
		ПродажиСервер.ЗаполнитьЦены(ТаблицаЗаказы, Новый Структура("ЭтоПродукция", Истина),
			Новый Структура("Дата, Валюта, ВидЦены", ТекущаяДатаСеанса(), ПараметрыЗаполнения.Валюта, ВидЦены),
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса)
	...
	|		И &Партнер = АналитикаПоПартнеруУчетСырьяПоНазначениям.Склад
	//++ НЕ УТ
	...
	|		И &Партнер = АналитикаПоПартнеруБезНазначения.Склад
	//++ НЕ УТ
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|	И ТаблицаТовары.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|
	// Передача продукции от партнера
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТаблицаТовары.НомерСтроки                                КАК НомерСтрокиДокумента,
...
Функция ТекстЗапросаТаблицаТоварыПолученныеОтПереработчика(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА &ПоступлениеПоЗаказам ИЛИ НЕ &ПартионныйУчетВерсии22
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	ПоступлениеОтПереработчикаТовары.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2
	|	И ПоступлениеОтПереработчикаТовары.ЗаказПереработчику.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПоступлениеОтПереработчикаВозвратныеОтходы.Ссылка
	|ИЗ
	|	Документ.ПоступлениеОтПереработчика.ВозвратныеОтходы КАК ПоступлениеОтПереработчикаВозвратныеОтходы
	|ГДЕ
	|	ПоступлениеОтПереработчикаВозвратныеОтходы.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2
	|	И ПоступлениеОтПереработчикаВозвратныеОтходы.ЗаказПереработчику.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	//-- НЕ УТКА
	|
	...
	|ГДЕ
	|	ПоступлениеОтПереработчика.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.ПоступлениеОтПереработчика";
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВТДокументыДляОбработки";
	//++ НЕ УТКА
	ДопПараметры.ДополнительныеИсточникиДанных.Вставить("Товары.ЗаказПереработчику");
	ДопПараметры.ДополнительныеИсточникиДанных.Вставить("ВозвратныеОтходы.ЗаказПереработчику");
	//-- НЕ УТКА
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТЧТовары.ЗаказПереработчику КАК ЗаказПереработчику
	|	ИЗ
	|		Документ.ПоступлениеОтПереработчика.Товары КАК ТЧТовары
	|	ГДЕ
	|		ТЧТовары.ЗаказПереработчику.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства)
	|		И ТЧТовары.Ссылка = &Ссылка
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	|		ТЧВозвратныеОтходы.ЗаказПереработчику КАК ЗаказПереработчику
	|	ИЗ
	|		Документ.ПоступлениеОтПереработчика.ВозвратныеОтходы КАК ТЧВозвратныеОтходы
	|	ГДЕ
	|		ТЧВозвратныеОтходы.ЗаказПереработчику.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства)
	|		И ТЧВозвратныеОтходы.Ссылка = &Ссылка) КАК ЗаказыПереработчику
	|СГРУППИРОВАТЬ ПО
	...
	//++ НЕ УТКА
	|	ПоступлениеОтПереработчика.Товары.(
	|		ЗаказПереработчику КАК ЗаказПереработчику
	|	) КАК Товары,
	|	ПоступлениеОтПереработчика.ВозвратныеОтходы.(
	|		ЗаказПереработчику КАК ЗаказПереработчику
	|	) КАК ВозвратныеОтходы,
	//-- НЕ УТКА
	|	ПоступлениеОтПереработчика.ВерсияДанных КАК ВерсияДанных
	|ИЗ
	|	ВТДокументыДляОбработки КАК КОбработке
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеОтПереработчика КАК ПоступлениеОтПереработчика
	|		ПО КОбработке.Ссылка = ПоступлениеОтПереработчика.Ссылка");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.ИсточникДанных = Выборка.Товары;
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "ЗаказПереработчику");
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			
			ЭлементБлокировки = Блокировка.Добавить("Документ.ЗаказПереработчику");
			ЭлементБлокировки.ИсточникДанных = Выборка.ВозвратныеОтходы;
			ЭлементБлокировки.ИспользоватьИзИсточникаДанных("Ссылка", "ЗаказПереработчику");
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			//-- НЕ УТКА
			...
			Если Объект <> Неопределено Тогда
				...
				//++ НЕ УТКА
				ЗапросПоЗаказам.УстановитьПараметр("Ссылка", Выборка.Ссылка);
				ЗаказыСГруппировкойПоЭтапам = ЗапросПоЗаказам.Выполнить().Выгрузить();
				
				Для каждого СтрокаОтходы Из Объект.Товары Цикл
					Если ТипЗнч(СтрокаОтходы.Распоряжение) = Тип("ДокументСсылка.ЭтапПроизводства2_2")
						И ЗаказыСГруппировкойПоЭтапам.Найти(СтрокаОтходы.ЗаказПереработчику) <> Неопределено Тогда
						СтрокаОтходы.Распоряжение = СтрокаОтходы.ЗаказПереработчику;
						ОбъектИзменен = Истина;
					КонецЕсли;
				КонецЦикла;
				
				Для каждого СтрокаОтходы Из Объект.ВозвратныеОтходы Цикл
					Если ТипЗнч(СтрокаОтходы.Распоряжение) = Тип("ДокументСсылка.ЭтапПроизводства2_2")
						И ЗаказыСГруппировкойПоЭтапам.Найти(СтрокаОтходы.ЗаказПереработчику) <> Неопределено Тогда
						СтрокаОтходы.Распоряжение = СтрокаОтходы.ЗаказПереработчику;
						ОбъектИзменен = Истина;
