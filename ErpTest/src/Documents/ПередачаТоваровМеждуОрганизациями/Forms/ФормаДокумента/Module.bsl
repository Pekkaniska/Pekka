...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_СчетФактураПолученныйНалоговыйАгент"
		И Объект.НалогообложениеНДС = ПредопределенноеЗначение("Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя") Тогда
...
Процедура РассчитатьИтоговыеПоказателиПередачиТоваровМеждуОрганизациями(Форма)
	...
	Если Не УчетНДСУПКлиентСервер.ПродажаОблагаетсяНДС(Форма.Объект.НалогообложениеНДС) Тогда
