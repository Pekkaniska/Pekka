...
// 				* УчетНДСУП.АктуализироватьСчетаФактурыВыданныеПриПроведении()
// 				* УчетНДСУП.АктуализироватьСчетаФактурыВыданныеПриУдаленииПроведения()
//
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	УчетНДСУПСлужебный.УстановитьПараметрыЗапросаПроведенияПоЖурналуСчетаФактурыВыставленного(Реквизиты, Запрос);
...
Функция ТекстЗапросаВтПрошлыеКорректировки(Запрос, ТекстыЗапроса)
	...
	ЗапросИсходныхКорректировок = Новый Запрос;
	ЗапросИсходныхКорректировок.МенеджерВременныхТаблиц = Запрос.МенеджерВременныхТаблиц;
	ЗапросИсходныхКорректировок.УстановитьПараметр("СчетФактураОснование",Запрос.Параметры.СчетФактураОснование);
	ЗапросИсходныхКорректировок.УстановитьПараметр("Ссылка",Запрос.Параметры.Ссылка);
	ЗапросИсходныхКорректировок.УстановитьПараметр("Корректировочный",Запрос.Параметры.Корректировочный);
	ЗапросИсходныхКорректировок.Текст = 
	"ВЫБРАТЬ
	|	МИНИМУМ(КорректировкаРеализации.Дата) КАК ДатаИсходнойКорректировки,
	|	КорректировкаРеализации.ДокументОснование КАК Документ
	|ПОМЕСТИТЬ ВтИсходныеКорректировки
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданныйДокументыОснования
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации КАК КорректировкаРеализации
	|		ПО СчетФактураВыданныйДокументыОснования.ДокументОснование = КорректировкаРеализации.Ссылка
	|ГДЕ
	|	СчетФактураВыданныйДокументыОснования.Ссылка В (&СчетФактураОснование, &Ссылка)
	|	И &Корректировочный
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ";
	ЗапросИсходныхКорректировок.Выполнить();
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтИсходныеКорректировки КАК ИсходныеКорректировки
	|		ПО КорректировкаРеализации.ДокументОснование = ИсходныеКорректировки.Документ
	|ГДЕ
	|	КорректировкаРеализации.Проведен
	|	И КорректировкаРеализации.Дата >= ЕСТЬNULL(ИсходныеКорректировки.ДатаИсходнойКорректировки, ДАТАВРЕМЯ(1, 1, 1))
	|	И &Исправление";
...
Функция ТекстЗапросаВтТаблицаСуммДокументов(Запрос, ТекстыЗапроса)
	...
	"ВЫБРАТЬ
	|	ДанныеОснований.ВидЦенности  КАК ВидЦенностиПродажи,
	...
	|		ПО ДанныеОснований.Регистратор = ДокументыОснования.Документ
	|ГДЕ
	|	НЕ ДанныеОснований.ИсправлениеОшибок
	|	И НЕ ДанныеОснований.КорректировкаПоСогласованиюСторон
	|	И ДанныеОснований.ТипСчетаФактуры = &ИдентификаторМетаданных
	|	И Не &Корректировочный";
	
	ТекстЗапроса = ТекстДанныеДокументовОснований 
...
Функция ТекстЗапросаВтТаблицаКорректировки(Запрос, ТекстыЗапроса, ПрошлыеКорректировки = Ложь)
	...
	|	И (ДанныеОснований.ИсправлениеОшибок ИЛИ ДанныеОснований.КорректировкаПоСогласованиюСторон)";
...
#Область ТекстВыставленныеСчетаФактурыНаРеализациюКорректировочный
	...
	|	ТаблицаДанных.СуммаПоСчетуФактуреУвеличениеКомиссия КАК СуммаПоСчетуФактуреРазницаУвеличениеКомиссия,
	|	-ТаблицаДанных.СуммаНДСУменьшениеКомиссия КАК СуммаНДСРазницаУменьшениеКомиссия,
...
#Область ТекстПолученныеСчетаФактуры
	...
	|	
	|	СУММА(ВЫБОР 
	|			КОГДА ТаблицаСуммДокументов.Комиссия
	|				ТОГДА ТаблицаСуммДокументов.СуммаПоСчетуФактуреЖурнал
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаПоСчетуФактуреКомиссия,
	|	СУММА(ВЫБОР 
	|			КОГДА ТаблицаСуммДокументов.Комиссия
	|				ТОГДА ТаблицаСуммДокументов.СуммаНДСЖурнал
	|			ИНАЧЕ 0
...
#Область ТекстКорректировка
	...
	|	КОГДА ТаблицаДанных.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	...
	|		ВтТаблицаСуммДокументов КАК ТаблицаСуммДокументов
	|	ГДЕ
	|		НЕ ТаблицаСуммДокументов.Комиссия
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ВтТаблицаКорректировки КАК ТаблицаКорректировки
	|	ГДЕ
	|		НЕ ТаблицаКорректировки.Комиссия
	|
	|	) КАК ТаблицаДанных
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументыОснования КАК ДанныеОснований
...
//++ Рарус Лимаренко 28.11.2018
		...
		+ пкМодификацияСервер.пкТекстЗапросаДанныхТабличнойЧастиДляПечатиСчетаФактуры(ПараметрыПечати);
//-- Рарус Лимаренко 28.11.2018
...
//+++rarus-spb_zlov 13.01.2017    
		|//ДанныеАренды
//---rarus-spb_zlov 13.01.2017 
//Рарус Владимир Подрезов 29.03.2017
		|//ДанныеАктиваДоходов
		|//ДанныеКлючаСтрокиРабот
//Рарус Владимир Подрезов Конец
        ...
        Если ИмяОбъекта = "РеализацияТоваровУслуг" Тогда
			ТекстЗамены =
			"	ТаблицаДокумента.пкДатаНачалаАренды    		КАК пкДатаНачалаАренды,
			|	ТаблицаДокумента.пкДатаОкончанияАренды 		КАК пкДатаОкончанияАренды,
			|	ТаблицаДокумента.пкТехника             		КАК пкТехника,
			|	ТаблицаДокумента.НомерСтрокиТранспортировки КАК НомерСтрокиТранспортировки,
			|	ТаблицаДокумента.пкПримечаниеДляПечати 		КАК пкПримечаниеДляПечати,";
        Иначе
			ТекстЗамены =
            "    НЕОПРЕДЕЛЕНО                          КАК пкДатаНачалаАренды,
            |    НЕОПРЕДЕЛЕНО                          КАК пкДатаОкончанияАренды,
            |    НЕОПРЕДЕЛЕНО                          КАК пкТехника,
            |    0                                     КАК НомерСтрокиТранспортировки,
            |    """"                                  КАК пкПримечаниеДляПечати,";
        КонецЕсли; 
		ТекстОбщейТаблицыТовары = СтрЗаменить(ТекстОбщейТаблицыТовары, "//ДанныеАренды", ТекстЗамены);
        
//---rarus-spb_zlov 13.01.2017 
//Рарус Владимир Подрезов 29.03.2017
        Если ИмяОбъекта = "РеализацияУслугПрочихАктивов" Тогда
			ТекстЗамены =
        ...
        КонецЕсли; 
		ТекстОбщейТаблицыТовары = СтрЗаменить(ТекстОбщейТаблицыТовары, "//ДанныеАктиваДоходов", ТекстЗамены);
		
		Если ИмяОбъекта = "РеализацияТоваровУслуг" Тогда
			ТекстЗамены =
			"		ВЫБОР КОГДА ТаблицаДокумента.Номенклатура = &пкРемонтАрендованногоОборудования
			|			ТОГДА ТаблицаДокумента.НомерСтроки
			|			ИНАЧЕ 0
			|		КОНЕЦ КАК КлючСтрокиРаботы,
			|		ТаблицаДокумента.пкЗамененнаяМодель			КАК пкЗамененнаяМодель,";
        Иначе

			ТекстЗамены =
			"	   0					   КАК КлючСтрокиРаботы,	
			|	 НЕОПРЕДЕЛЕНО 						   КАК пкЗамененнаяМодель,";						
        КонецЕсли; 
		ТекстОбщейТаблицыТовары = СтрЗаменить(ТекстОбщейТаблицыТовары, "//ДанныеКлючаСтрокиРабот", ТекстЗамены);
//Рарус Владимир Подрезов Конец
		...
		Если ИмяОбъекта = "КорректировкаРеализации" Или ИмяОбъекта = "ВозвратТоваровОтКлиента" Тогда
			ТекстЗамены =
...
//+++rarus-spb_zlov 13.01.2017    
    |   ТаблицаТоваров.пкДатаНачалаАренды                        КАК пкДатаНачалаАренды,
    |   ТаблицаТоваров.пкДатаОкончанияАренды                     КАК пкДатаОкончанияАренды,
    |   ТаблицаТоваров.пкТехника                                 КАК пкТехника,
    |   ТаблицаТоваров.НомерСтрокиТранспортировки                КАК НомерСтрокиТранспортировки,
    |   ТаблицаТоваров.пкПримечаниеДляПечати                     КАК пкПримечаниеДляПечати,
//---rarus-spb_zlov 13.01.2017 
	
//Рарус Владимир Подрезов 29.03.2017
	|   ТаблицаТоваров.АналитикаДоходов							 КАК АналитикаДоходов,
	|	ТаблицаТоваров.КлючСтрокиРаботы							 КАК КлючСтрокиРаботы,
//Рарус Владимир Подрезов Конец
//++ Рарус Лимаренко 28.11.2018
	|	ТаблицаТоваров.пкЗамененнаяМодель						  КАК пкЗамененнаяМодель,
//-- Рарус Лимаренко 28.11.2018
...
//Рарус Владимир Подрезов 29.03.2017
	Запрос.УстановитьПараметр("пкРемонтАрендованногоОборудования", Константы.пкРемонтАрендованногоОборудования.Получить());
//Рарус Владимир Подрезов Конец	
	Запрос.Выполнить();
...
Функция ТекстЗапросаВтДанныеОснований(Запрос, ТекстыЗапроса)
	...
	|	ИЗ
	|		Документ.СчетФактураВыданный.ДокументыОснования КАК ДанныеДокумента
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетДавальцу КАК ШапкаОснования
	|			ПО ДанныеДокумента.ДокументОснование = ШапкаОснования.Ссылка
	|	ГДЕ
	|		ДанныеДокумента.Ссылка = &Ссылка
	|	
...
Функция ТекстЗапросаТаблицаОснований()
	...
	|		ПО ТаблицаОснований.ИсходныйДокумент = СчетФактураВыданныйДокументыОснования.Ссылка
	|ГДЕ
	|	ТаблицаОснований.Ссылка В(&МассивОбъектов)
...
Функция ПараметрыЗапросаСчетаФактурыКОформлению() Экспорт
