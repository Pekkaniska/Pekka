...
#Область ТекстЗапроса
	...
	|	ВыходныеИзделия.Номенклатура,
	|	ВыходныеИзделия.Характеристика,
	|	ВыходныеИзделия.Этап.Подразделение КАК Подразделение,
	...
	|	ВозвратныеОтходы.Этап.Подразделение КАК Подразделение,
...
Процедура ЗаполнитьМатериалыТрудозатратыПоНормативам(Объект, Спецификация, ИменаТЧ = "") Экспорт
	...
	Если Найти(ИменаТЧ, "МатериалыИУслуги") > 0 Или ИменаТЧ = "" Тогда
		Для Каждого ТекСтрока Из ТабличныеЧасти.МатериалыИУслуги Цикл
			ЗаполнитьЗначенияСвойств(Объект.МатериалыИУслуги.Добавить(), ТекСтрока);
		КонецЦикла;
...
Функция ТаблицаЛимитовПоСпецификации(Объект)
	...
	|	И НЕ Отходы.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПолуфабрикатыПроизводимыеВПроцессе)
	|
	|СГРУППИРОВАТЬ ПО
	|	Отходы.Номенклатура,
	|	Отходы.Характеристика
