...
Процедура СформироватьДвиженияНДС(ТаблицаИзменений) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			УчетНДСУП.ОтразитьЗаписиРегистраЗаданий(Выборка.СчетФактура, ДополнительныеСвойства);
			
			//++ НЕ УТ
			ВнеоборотныеАктивы.СформироватьЗаписиРегистровЗаданий(Выборка.СчетФактура, ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы);
			//-- НЕ УТ
