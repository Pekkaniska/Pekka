...
Процедура ХозяйственнаяОперацияПриИзмененииНаСервере()
	...
	// Нужно округлить количество, если до этого оно не округлялось.
	Если ХозОперацияДоИзменения = Перечисления.ХозяйственныеОперации.ПеремещениеПолуфабрикатов
		И Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПеремещениеПолуфабрикатов Тогда
...
&НаСервере
Процедура АктуализироватьНалогообложениеНДС(Запонить = Истина)
...
Процедура УстановитьИспользованиеПлановойСтоимости()
	...
	Если ИспользоватьПлановуюСтоимость <> ПараметрыУчетнойПолитики.УчетГотовойПродукцииПоПлановойСтоимости Тогда
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	Для Индекс = 0 По КоличествоИндексов Цикл
		...
		Если Не (ЗаполнениеПоПриемке И НакладнаяЯвляетсяРаспоряжением) И МассивЗаказов.Найти(Строка.Распоряжение) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
