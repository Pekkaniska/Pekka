...
Функция ДанныеДляПроверкиВводаНаОсновании(Распоряжения, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВПроизводство
		...
		ТекстыЗапросов.Добавить(
			...
			|	И ЗаказМатериаловВПроизводство.ХозяйственнаяОперация = &ХозОперация
			//++ НЕ УТКА
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
		...
		ТекстыЗапросов.Добавить(
			...
			//-- НЕ УТКА
			|");
		ТекстыЗапросов.Добавить(РегистрыНакопления.ЗаказыМатериаловВПроизводство.ТекстЗапросаОстатки("ВтОстаткиКОформлению",
	...
	Иначе
		...
		ТекстыЗапросов.Добавить(
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	1");
		ТекстыЗапросов.Добавить(РегистрыНакопления.РаспоряженияНаПередачуИзПроизводства.ТекстЗапросаОстатки("ВтОстаткиКОформлению",
			Новый Структура("Распоряжение, Операция", "Распоряжения", "ХозОперация"), "Количество <> 0"));
		ТекстыЗапросов.Добавить(
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ОстаткиКОформлению.Распоряжение КАК РаспоряженияКОформлению
			|ИЗ
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратМатериаловИзКладовой
		...
		Запрос = Новый Запрос();
		Запрос.Текст = РегистрыНакопления.ТоварыКПоступлению.ТекстЗапросаОсталосьОформитьПоОрдерам();
	...
	Если ПараметрыЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВПроизводство
		...
		Запрос.УстановитьПараметр("Склад", ПараметрыЗаполнения.Отправитель);
	Иначе
		Запрос.УстановитьПараметр("Склад", ПараметрыЗаполнения.Получатель);
	КонецЕсли;
	...
	Если ПараметрыЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВПроизводство
		...
		КлючевыеПоля = СтрЗаменить(ПараметрыЗаполнения.КлючевыеПоля, "Назначение, НазначениеОтправителя", "НазначениеОтправителя");
	Иначе
		КлючевыеПоля = СтрЗаменить(ПараметрыЗаполнения.КлючевыеПоля, "Назначение, НазначениеОтправителя", "Назначение");;
	КонецЕсли;
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Команда = СозданиеНаОснованииПереопределяемый.ДобавитьКомандуСоздатьНаОснованииБизнесПроцессЗадание(КомандыСозданияНаОсновании);
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	//++ НЕ УТКА
	ТекстЗапросаТаблицаЭтаповПроизводства(Запрос, ТекстыЗапроса, Регистры);
	//-- НЕ УТКА
...
Процедура ИнициализироватьКлючиАналитикиНоменклатуры(Запрос)
	...
	|			И ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)  = Ключи.СтатьяКалькуляции
	|			И ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)         = Ключи.Назначение
	|			И ТаблицаВидыЗапасов.ВидЗапасов.ВладелецТовара         = Ключи.Склад
	|	ГДЕ
	|		Ключи.Номенклатура ЕСТЬ NULL
	|		И ТаблицаВидыЗапасов.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|			И &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеПолуфабрикатовМеждуФилиалами)
...
Функция ТекстЗапросаТаблицаДвиженияНоменклатураНоменклатура(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ТОГДА Строки.ВидЗапасовПолучателя.ТипЗапасов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                             КАК КорТипЗапасов,
	|	ВЫБОР КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|		ТОГДА Строки.ВидЗапасовПолучателя
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ                                             КАК КорВидЗапасов,
	|
...
//++ НЕ УТКА
Функция ТекстЗапросаТаблицаЭтаповПроизводства(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "ТаблицаЭтаповПроизводства";
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаТовары.Распоряжение КАК Ссылка
	|ИЗ
	|	Документ.ДвижениеПродукцииИМатериалов.Товары КАК ТаблицаТовары
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ТИПЗНАЧЕНИЯ(&Получатель) = ТИП(Справочник.СтруктураПредприятия)
	|	И &ПоРаспоряжениям
	|	И ТаблицаТовары.Распоряжение Ссылка Документ.ЭтапПроизводства2_2";
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|	Аналитика.Номенклатура.ГруппаАналитическогоУчета КАК ГруппаПродукцииПолуфабриката,
	|	ТаблицаВидыЗапасов.Количество                 КАК Количество,
	|	&ХозяйственнаяОперация                        КАК ХозяйственнаяОперация,
	|	ВЫБОР
...
// заполняет новый реквизит ВидЗапасовПолучателя.
//
	...
	|ИЗ
	|	Документ.ДвижениеПродукцииИМатериалов.ВидыЗапасов КАК Передачи
	|ГДЕ
	|	Передачи.ВидЗапасовПолучателя = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|	И Передачи.Ссылка.Проведен
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Документ.ДвижениеПродукцииИМатериалов.ВидыЗапасов КАК Передачи
	|ГДЕ
	|	Передачи.ВидЗапасовПолучателя.РеализацияЗапасовДругойОрганизации
	|	И Передачи.Ссылка.Проведен
	|
	|ОБЪЕДИНИТЬ
	|
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументДвижениеПродукцииИМатериалов.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ДвижениеПродукцииИМатериалов КАК ДокументДвижениеПродукцииИМатериалов
	|ГДЕ
	|	ДокументДвижениеПродукцииИМатериалов.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	КОбработке.Ссылка.ВариантПриемкиТоваров КАК ВариантПриемкиТоваров
	...
	|	&ВариантПриемкиКонстанта КАК РассчитанныйВариантПриемки
	|ИЗ
	...
	|	АналитикаУчетаНоменклатурыВидЗапасов.АналитикаУчетаНоменклатуры,
	|	АналитикаУчетаНоменклатурыВидЗапасов.ВидЗапасов,
	|	АналитикаУчетаНоменклатурыВидЗапасов.ВидЗапасовОприходование
	|ИЗ
	|	АналитикаУчетаНоменклатурыВидЗапасов КАК АналитикаУчетаНоменклатурыВидЗапасов
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Попытка
			...
			МассивВидовЗапасов = ДокументОбъект.ВидыЗапасов.Выгрузить().ВыгрузитьКолонку("ВидЗапасовПолучателя");
			ВидыЗапасовРеализацияЗапасовДругойОрганизации = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивВидовЗапасов, "РеализацияЗапасовДругойОрганизации");
			Для Каждого Стр Из ВидыЗапасовРеализацияЗапасовДругойОрганизации Цикл
			...
			Если ВыборкаПоДокументам.ТребуетсяУказаниеВариантаПриемки Тогда
				ОбъектИзменен = Истина;
				ДокументОбъект.ВариантПриемкиТоваров = ВыборкаПоДокументам.РассчитанныйВариантПриемки;
			КонецЕсли;
