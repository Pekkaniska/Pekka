...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД") Тогда
		...
		Если ДанныеФайла = Неопределено Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПрисоединенныйФайлИмяФайла",, Отказ);
...
Функция РазрешеноПодписыватьФайлЭлектронногоДокумента(ФайлЭлектронногоДокумента)
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	ЕстьВозможностьПодписания = ЕстьПравоОбработки И ТребуетсяПодпись
		...
		И РазрешеноПодписыватьФайлЭлектронногоДокумента(ПрисоединенныйФайлСсылка);
...
Процедура ЗаполнитьТаблицуЭП()
	...
	СвойстваЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Ссылка, "НастройкаЭДО");
	...
	Если ЗначениеЗаполнено(ПрисоединенныйФайлСсылка) Тогда
		...
		Для Каждого ТекСтрока Из ЭлПодписи Цикл
			...
			Если НоваяСтрока.ОтветнаяПодпись Тогда
...
Функция ЕстьСоглашение(ОбъектДокумент)
	...
	ПараметрыОпределенияНастройки = Новый Структура("ВыводитьСообщения,ВидЭД", ВыводитьСообщение, Объект.ВидЭД);
	НастройкиЭД = ОбменСКонтрагентамиСлужебный.ОпределитьНастройкиОбменаЭДПоИсточнику(ОбъектДокумент, ПараметрыОпределенияНастройки); 
...
Процедура ОбновитьДанныеОВложении()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайлСсылка,
			"НаименованиеФайла,Расширение,Редактирует");
		...
		ПрисоединенныйФайлПиктограмма = РаботаСФайламиСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(
