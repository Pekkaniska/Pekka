...
Функция ВыгрузитьДокумент(УникальныйИдентификатор = Неопределено) Экспорт
	...
	Для Каждого ЭлементВыгрузки Из СодержаниеВыгрузки Цикл
		...
		Исключение
			ИмяСобытия = НСтр("ru = 'Уведомление о контролируемых сделках.Выгрузка документа.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ЭлектронноеПредставление502(ОсновныеСведенияВыгрузки, УникальныйИдентификатор = Неопределено)
	...
	Контекст = КонтекстВыполненияВыгрузки502(ОсновныеСведенияВыгрузки);
	
	ВременныйПакетФайлов = Новый Массив;
	
	ИдФайл = Контекст.ИдФайлПерв;
	
	ФайлВыгрузки = НовыйФайлВыгрузки(Контекст, ИдФайл);
	...
	Для Каждого Лист1А Из ЛистыУведомления.ЛистыРаздела1А Цикл
		ТекстБлокаСделки = ТекстПоСделке(Контекст, ЛистыУведомления, Лист1А);
...
Функция КонтекстВыполненияВыгрузки502(ОсновныеСведенияВыгрузки)
...
Функция НовыйФайлВыгрузки(Контекст, ИдентификаторФайла)
	...
	Если ОсновныеСведенияВыгрузки.ЭтоПБОЮЛ Тогда
		ЗаполнитьСведенияПБОЮЛ(Контекст, ТекстФайла);
	Иначе
...
Процедура ЗаполнитьСведенияПБОЮЛ(Контекст, ТекстФайла)
...
Функция ТекстПоСделке(Контекст, ЛистыУведомления, Лист1А);
	...
	ЗаполнитьСведенияОСделке502(Контекст, ТекстСделки, Лист1А);
	
	ЗаполнитьСведенияОДоходахРасходах502(Контекст, ТекстСделки, Лист1А);
...
Процедура ЗаполнитьСведенияОСделке502(Контекст, ТекстСделки, Сведения)
...
Процедура ЗаполнитьСведенияОДоходахРасходах502(Контекст, ТекстСделки, Сведения)
	...
	Для Каждого ТипПредмета Из ТипыПредметовСделок Цикл
		...
		Для Каждого Лист1Б Из ТипПредмета.Значение Цикл
			...
			Если ТипПредСд = 1 Тогда
				...
				КонецЕсли;
				Если Контекст.ОсновныеСведенияВыгрузки.ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012() Тогда
...
Процедура ЗаполнитьСведенияОбУчастникахФизЛицах502(Контекст, ТекстСделки, Листы3, ДанныеРаздела3, Сделка)
	...
	Для Каждого Лист3 Из СписокЛистов3 Цикл
		...
		Иначе
			...
			АдресСтруктурой = РаботаСАдресами.СведенияОбАдресе(ФизЛицо.КонтактнаяИнформацияЗначенияПолей);
			...
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Индекс", ПоляАдреса.Индекс);
			УстановитьАтрибут(Шаблон_АдрРФ, "КодРегион", ПоляАдреса.РегионКод);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Район", ПоляАдреса.Район);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Город", ПоляАдреса.Город);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "НаселПункт", ПоляАдреса.НаселенныйПункт);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Улица", ПоляАдреса.Улица);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Дом", ПоляАдреса.Дом);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Корпус", ПоляАдреса.Корпус);
			УстановитьНеобязательныйАтрибут(Шаблон_АдрРФ, "Кварт", ПоляАдреса.Квартира);
...
Процедура ЗаполнитьСведенияОПредметеСделки(Параметры, ПрототипУзлаВыгрузки, Листы1Б, Сделка)
	...
	Для Каждого ТипПредмета Из ТипыПредметовСделок Цикл
		...
		Для Каждого Лист1Б Из ТипПредмета.Значение Цикл
			...
			Если Параметры.ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012() Тогда
				КодПоОКП = Лист1Б.Строка043КодПоОКП;
...
Процедура ЗаполнитьСведенияОбУчастникахФизЛицах(ПрототипУзлаВыгрузки, Листы3, ДанныеРаздела3, Сделка)
	...
	Для Каждого Лист3 Из СписокЛистов3 Цикл
		...
		Если ПустаяСтрока(ФизЛицо.Строка030ИНН) Тогда
			...
			ПоляАдреса.Вставить("РегионКод", "КодРегион");
...
Процедура ОтсечьНезаполненныеНеобязательныеУзлы(Узел)
	...
	Если ТипЗнч(Узел) <> Тип("ДеревоЗначений") Тогда
		...
		И УзелПуст(Узел)
		И ?(Найти(Узел.Обязательность, "О") <> 0 ИЛИ СтрНайти(Узел.Обязательность, "O") <> 0, ИмеютсяАналогичныеСоседниеУзлы(Узел), Истина) Тогда
			УдалитьУзел(Узел);
		КонецЕсли;
	КонецЕсли;
