...
Процедура ПодготовитьДанныеАвтоматическогоЗаполнения(Уведомление) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЛизинг") Тогда
		ДобавитьСделкиПоступленияУслугПоЛизингу(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок);
	КонецЕсли;
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьВЖурналРегистрации = НСтр("ru = 'Выполнение операции записи уведомления по контролируемым сделкам'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
...
Процедура СформироватьКонтролируемыеСделкиУведомления(Уведомление) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КонтролируемыеСделки.Соглашение.КодНаименованияСделки, """") = """"
	|			ТОГДА ВЫБОР
	|					КОГДА КонтролируемыеСделки.ТипПредметаСделки = ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.РаботаУслуга)
	|						ТОГДА ""019""
	|					ИНАЧЕ ""015""
	|				КОНЕЦ
	|		ИНАЧЕ ЕСТЬNULL(КонтролируемыеСделки.Соглашение.КодНаименованияСделки, """")
	...
	КлючСделки = Новый Структура("Контрагент, Договор, ПредметСделки, Строка121СтороныВзаимозависимыПоКодексу, 
...
Функция ПолучитьВременныеТаблицыДляЗаполненияУведомления(Уведомление) Экспорт
	...
	|				ИЛИ НЕ Контрагенты.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицоНеРезидент)
	...
	|	ВЫБОР
	|		КОГДА НЕ Организации.ИностраннаяОрганизация
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	...
	|	КонтролируемыеСделкиОрганизаций.Соглашение КАК Соглашение,
	...
	|	ВЫБОР
	|		КОГДА (КонтролируемыеСделкиОрганизаций.Контрагент ССЫЛКА Справочник.Контрагенты
	|					И УчетнаяПолитикаОрганизации.ЯвляетсяПлательщикомНалогаНаПрибыль <> ЕСТЬNULL(ДанныеПоКонтрагентамПоПериодам.ЯвляетсяПлательщикомНалогаНаПрибыль, ЛОЖЬ)
	|				ИЛИ КонтролируемыеСделкиОрганизаций.Контрагент ССЫЛКА Справочник.Организации
	|					И УчетнаяПолитикаОрганизации.ЯвляетсяПлательщикомНалогаНаПрибыль <> ЕСТЬNULL(ДанныеПоОрганизациямПоПериодам.ЯвляетсяПлательщикомНалогаНаПрибыль, ЛОЖЬ))
	...
	|	ВЫБОР
	|		КОГДА (ЕСТЬNULL(УчетнаяПолитикаОрганизации.ЯвляетсяПлательщикомНДПИ, ЛОЖЬ)
	|					И КонтролируемыеСделкиОрганизаций.ТипКонтролируемойСделки = ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ПолученДоход)
	|					И ЕСТЬNULL(ВзаимозависимыеЛицаПоПериодам.ВзаимозависимыеЛица, ЛОЖЬ)
	|				ИЛИ (КонтролируемыеСделкиОрганизаций.Контрагент ССЫЛКА Справочник.Контрагенты
	|						И ЕСТЬNULL(ДанныеПоКонтрагентамПоПериодам.ЯвляетсяПлательщикомНДПИ, ЛОЖЬ)
	|					ИЛИ КонтролируемыеСделкиОрганизаций.Контрагент ССЫЛКА Справочник.Организации
	|						И ЕСТЬNULL(ДанныеПоОрганизациямПоПериодам.ЯвляетсяПлательщикомНДПИ, ЛОЖЬ))
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(УчетнаяПолитикаОрганизации.ЗарегистрированВОЭЗ, ЛОЖЬ) <> ВЫБОР
	|					КОГДА КонтролируемыеСделкиОрганизаций.Контрагент ССЫЛКА Справочник.Контрагенты
	|						ТОГДА ЕСТЬNULL(ДанныеПоКонтрагентамПоПериодам.ЗарегистрированВОЭЗ, ЛОЖЬ)
	|					ИНАЧЕ ДанныеПоОрганизациямПоПериодам.ЗарегистрированВОЭЗ
	|				КОНЕЦ
	|				И УчастникиКонтролируемыхСделок.КонтрагентЗарегистрированВРФ
	|				И ЕСТЬNULL(ВзаимозависимыеЛицаПоПериодам.ВзаимозависимыеЛица, ЛОЖЬ)
	|			ТОГДА ИСТИНА
	...
	|					И КонтролируемыеСделкиОрганизаций.ТипКонтролируемойСделки = ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ПолученДоход)
	|					И ЕСТЬNULL(ВзаимозависимыеЛицаПоПериодам.ВзаимозависимыеЛица, ЛОЖЬ)
	|				ИЛИ ЕСТЬNULL(ДанныеПоКонтрагентамПоПериодам.ЯвляетсяПлательщикомЕНВД, ЛОЖЬ)
	|					И КонтролируемыеСделкиОрганизаций.ТипКонтролируемойСделки = ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ОсуществленРасход)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеПоКонтрагентамПоПериодам.ЯвляетсяПлательщикомЕСХН, ЛОЖЬ)
	|				И ЕСТЬNULL(ВзаимозависимыеЛицаПоПериодам.ВзаимозависимыеЛица, ЛОЖЬ)
	|			ТОГДА ВЫБОР
	|					КОГДА ВзаимозависимыеЛицаПоПериодам.НевзаимозависимыйПосредник
	|							И НЕ &УчитыватьСпецрежимДляПосредников
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияОбОрганизацииДляКонтролируемыхСделокПоПериодам КАК УчетнаяПолитикаОрганизации
	|		ПО КонтролируемыеСделкиОрганизаций.Организация = УчетнаяПолитикаОрганизации.Организация
	|			И КонтролируемыеСделкиОрганизаций.Период >= УчетнаяПолитикаОрганизации.НачалоПериода
	|			И КонтролируемыеСделкиОрганизаций.Период <= УчетнаяПолитикаОрганизации.ОкончаниеПериода
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПоКонтрагентамПоПериодам КАК ДанныеПоКонтрагентамПоПериодам
	|		ПО КонтролируемыеСделкиОрганизаций.Контрагент = ДанныеПоКонтрагентамПоПериодам.Контрагент
	|			И КонтролируемыеСделкиОрганизаций.Период >= ДанныеПоКонтрагентамПоПериодам.НачалоПериода
	|			И КонтролируемыеСделкиОрганизаций.Период <= ДанныеПоКонтрагентамПоПериодам.ОкончаниеПериода
	|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияОбОрганизацииДляКонтролируемыхСделокПоПериодам КАК ДанныеПоОрганизациямПоПериодам
	|		ПО (КонтролируемыеСделкиОрганизаций.Контрагент ССЫЛКА Справочник.Организации)
	|			И КонтролируемыеСделкиОрганизаций.Период >= ДанныеПоОрганизациямПоПериодам.НачалоПериода
	|			И КонтролируемыеСделкиОрганизаций.Период <= ДанныеПоОрганизациямПоПериодам.ОкончаниеПериода
	|			И КонтролируемыеСделкиОрганизаций.Контрагент = ДанныеПоОрганизациямПоПериодам.Организация
	|ГДЕ
...
Функция ПолучитьТекстЗапросаПоКонтролируемымСделкам() Экспорт
	...
	|	КонтролируемаяСделка.СпособОпределенияЦеныСделки,
	|	ВЫБОР
	...
	|			ПО Листы1А.Контрагент = ДанныеКонтактнойИнформации.Ссылка
	...
	|	ЕСТЬNULL(ИсторияКППКонтрагента.КПП, Контрагенты.КПП) КАК Строка060КПП,
	|	ВЫБОР
	|		КОГДА Контрагенты.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
	|				ИЛИ Контрагенты.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|			ТОГДА """"
	...
	|		ПО Контрагенты.Ссылка = КонтактнаяИнформацияКонтрагента.Ссылка
	...
	|		ПО Контрагенты.Ссылка = ИсторияНаименованийКонтрагента.Ссылка
	...
	|ВЫБРАТЬ
	|	Контрагенты.Ссылка КАК Контрагент,
	|	ЕСТЬNULL(ДанныеПоКонтрагентамКонтролируемыхСделок.КодВидаДеятельностиФизическогоЛица, """") КАК Строка020КодВидаДеятельности,
	|	Контрагенты.ИНН КАК Строка030ИНН,
	|	ЕСТЬNULL(ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо, ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)) КАК ФизическоеЛицо,
	|	ЕСТЬNULL(ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо.ДатаРождения, ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)) КАК ДатаРождения,
	|	ЕСТЬNULL(ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо.МестоРождения, """") КАК МестоРождения,
	...
	|ИЗ
	|	КонтагентыКонтролируемыхСделок КАК Контрагенты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПоКонтрагентамКонтролируемыхСделок КАК ДанныеПоКонтрагентамКонтролируемыхСделок
	|		ПО Контрагенты.Ссылка = ДанныеПоКонтрагентамКонтролируемыхСделок.Контрагент
	|		ЛЕВОЕ СОЕДИНЕНИЕ ГражданствоФизическихЛиц КАК ГражданствоФизическихЛиц
	|		ПО (ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо = ГражданствоФизическихЛиц.ФизическоеЛицо)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица.КонтактнаяИнформация КАК КонтактнаяИнформацияФизлица
	|		ПО (ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо = КонтактнаяИнформацияФизлица.Ссылка)
	...
	|		ПО (ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо = КонтактнаяИнформацияЗаРФ.Ссылка)
	...
	|		ПО (ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо = ВидыДокументовФизическихЛиц.Физлицо)
	...
	|		ПО (ДанныеПоКонтрагентамКонтролируемыхСделок.ФизическоеЛицо = ФИОФизЛиц.ФизическоеЛицо)
	|ГДЕ
...
#Область ПодготовкаДанныхАвтоматическогоЗаполнения
...
Функция ПолучитьВременныеТаблицыДляЗаполненияСделок(Уведомление)
	...
	Запрос.УстановитьПараметр("СписокКодовСтранОфшоров", КонтролируемыеСделкиПовтИсп.ПереченьКодовСтранОфшоров());
	...
	|	УчетнаяПолитикаОрганизацийСрезПоследних.УчетнаяПолитика.ЯвляетсяПлательщикомНДПИ КАК ЯвляетсяПлательщикомНДПИ,
	|	УчетнаяПолитикаОрганизацийСрезПоследних.Организация.ЗарегистрированВОЭЗ КАК ЗарегистрированВОЭЗ,
	|	УчетнаяПолитикаОрганизацийСрезПоследних.УчетнаяПолитика.ПрименяетсяЕНВД КАК ЯвляетсяПлательщикомЕНВД,
	|	УчетнаяПолитикаОрганизацийСрезПоследних.ПлательщикНалогаНаПрибыль КАК ЯвляетсяПлательщикомНалогаНаПрибыль
	|ПОМЕСТИТЬ СведенияОбОрганизацииДляКонтролируемыхСделок
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&НачалоПериода, Организация В (&СписокОрганизаций)) КАК УчетнаяПолитикаОрганизацийСрезПоследних
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	УчетнаяПолитикаОрганизаций.УчетнаяПолитика.ЯвляетсяПлательщикомНДПИ,
	|	УчетнаяПолитикаОрганизаций.Организация.ЗарегистрированВОЭЗ,
	|	УчетнаяПолитикаОрганизаций.УчетнаяПолитика.ПрименяетсяЕНВД,
	|	УчетнаяПолитикаОрганизаций.ПлательщикНалогаНаПрибыль
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетнаяПолитикаОрганизаций
	|ГДЕ
	|	УчетнаяПолитикаОрганизаций.Период > &НачалоПериода
	|	И УчетнаяПолитикаОрганизаций.Период <= &ОкончаниеПериода
	...
	|		ПО Контрагенты.СтранаРегистрации = СтраныСЛьготнымНалогообложением.Страна
	|ГДЕ
...
Функция СтруктураПараметровЗапросаКДокументамИсточникам(Уведомление)
	...
	СтруктураВозврата.Вставить("ВалютаРегламентированногоУчета", Константы.ВалютаРегламентированногоУчета.Получить());
...
Процедура ДобавитьСделкиРеализацииТоваровУслуг(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ПО ТоварыРеализации.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	|ГДЕ
	|	ВЫБОР
...
Процедура ДобавитьСделкиВозвратаТоваровОтПокупателя(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	ВЫБОР
	|		КОГДА ВозвратТоваровОтКлиента.Контрагент ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ВзаимозависимыеЛица,
	|	ВЫБОР
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	|			ПО Аналитика.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	|		ПО ТоварыВозврата.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыКонтролируемыхСделок КАК ДокументыКонтролируемыхСделок
...
Процедура ДобавитьСделкиКорректировкиРеализации(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаУчета
	|				ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	|				ПО АналитикаУчета.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	|			ПО КорректировкаРеализацииВидыЗапасовОприходование.АналитикаУчетаНоменклатуры = АналитикаУчета.КлючАналитики
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	|				ПО АналитикаУчета.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	|			ПО КорректировкаРеализацииВидыЗапасовСписание.АналитикаУчетаНоменклатуры = АналитикаУчета.КлючАналитики
...
Процедура ДобавитьСделкиПриобретенияТоваровУслуг(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ПриобретениеТоваровУслуг.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	|					ИЛИ ПриобретениеТоваровУслуг.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.Импорт)
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
	...
	|	ЕСТЬNULL(ТоварыПоступления.НомерГТД.СтранаПроисхождения, ЕСТЬNULL(ТоварыКОформлениюТаможенныхДеклараций.НомерГТД.СтранаПроисхождения, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка))) КАК СтранаПроисхождения,
	|	ДокументыКонтролируемыхСделок.ВзаимозависимыеЛица,
	|	ДокументыКонтролируемыхСделок.ИностранныйКонтрагент,
	|	ДокументыКонтролируемыхСделок.ЗарегистрированВСтранеСЛьготнымНалогообложением
	|ПОМЕСТИТЬ ТоварыКонтролируемыхСделок
	...
	|		ПО ТоварыПоступления.Ссылка = ТоварыКОформлениюТаможенныхДеклараций.ДокументПоступления
	|			И ТоварыПоступления.ВидЗапасов = ТоварыКОформлениюТаможенныхДеклараций.ВидЗапасов
	|			И ТоварыПоступления.АналитикаУчетаНоменклатуры = ТоварыКОформлениюТаможенныхДеклараций.АналитикаУчетаНоменклатуры
...
Процедура ДобавитьСделкиПоступленияУслугПрочихАктивов(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ПриобретениеУслугПрочихАктивов.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
...
Процедура ДобавитьСделкиВозвратаТоваровПоставщику(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	ВЫБОР
	|		КОГДА ВозвратТоваровПоставщику.Контрагент ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ВзаимозависимыеЛица,
	|	ВЫБОР
...
Процедура ДобавитьСделкиКорректировкиПоступления(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ КорректировкаПриобретения.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Корректировка,
...
Процедура ДобавитьСделкиОтчетаКомитентуОСписании(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	|			ПО Аналитика.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	|		ПО ТоварыКомитента.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьСделкиОтчетаКомиссионераОПродажахЗависимымЛицам(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	|			ПО Аналитика.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	|		ПО ТоварыКомиссионера.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ДобавитьСделкиПередачиТоваровМеждуОрганизациями_ОрганизацияОтправитель(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	               |			ПО Аналитика.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	               |		ПО ПередачаТоваровМеждуОрганизациями.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПередачаТоваровМеждуОрганизациями.Ссылка,
	               |	Аналитика.Номенклатура,
...
Процедура ДобавитьСделкиПередачиТоваровМеждуОрганизациями_ОрганизацияПолучатель(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	               |			ПО Аналитика.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	               |		ПО ПередачаТоваровМеждуОрганизациямиВидыЗапасов.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ПередачаТоваровМеждуОрганизациямиВидыЗапасов.Ссылка,
	               |	Аналитика.Номенклатура,
...
Процедура ДобавитьСделкиОтчетПоКомиссииМеждуОрганизациями(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	               |			ПО Аналитика.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	               |		ПО ПередачаТоваровКомитентаЗависимымЛицам.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
...
Процедура ДобавитьСделкиВозвратаТоваровМеждуОрганизациями(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |		ПО ВозвратТоваровМеждуОрганизациями.ДокументПоступления = ПередачаТоваровМеждуОрганизациями.Ссылка
	               |ГДЕ
...
Процедура ДобавитьСделкиНачисленияКредитовИДепозитов(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	НачисленияКредитовИДепозитовНачисления.Ссылка.ХозяйственнаяОперация КАК ПредметСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.ИнойОбъектГражданскихПрав) КАК ТипПредметаСделки,
...
#Область ДобавлениеСделокЛизинга

Процедура ДобавитьСделкиПоступленияУслугПоЛизингу(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	
	Запрос = ПодготовитьЗапросКДокументамИсточникам(СтруктураПараметров);
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПриобретениеУслугПоЛизингу.Ссылка КАК РасчетныйДокумент,
	|	ВЫБОР
	|		КОГДА ВзаимозависимыеЛицаПоПериодам.Контрагент ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВзаимозависимыеЛицаПоПериодам КАК ВзаимозависимыеЛицаПоПериодам
	|		ПО (ВзаимозависимыеЛицаПоПериодам.Организация = &Организация)
	|			И (ВзаимозависимыеЛицаПоПериодам.Контрагент = ПриобретениеУслугПоЛизингу.Контрагент)
	|			И ПриобретениеУслугПоЛизингу.Дата >= ВзаимозависимыеЛицаПоПериодам.НачалоПериода
	|			И ПриобретениеУслугПоЛизингу.Дата <= ВзаимозависимыеЛицаПоПериодам.ОкончаниеПериода
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИностранныеКонтрагенты КАК ИностранныеКонтрагенты
	|		ПО (ИностранныеКонтрагенты.Контрагент = ПриобретениеУслугПоЛизингу.Контрагент)
	|ГДЕ
	|	ПриобретениеУслугПоЛизингу.Организация В(&СтруктураОрганизации)
	|	И ПриобретениеУслугПоЛизингу.Дата >= &НачалоПериода
	|	И ПриобретениеУслугПоЛизингу.Дата <= &ОкончаниеПериода
	|	И ПриобретениеУслугПоЛизингу.Проведен = ИСТИНА
	|	И ПриобретениеУслугПоЛизингу.Контрагент В
	|			(ВЫБРАТЬ
	|				КонтролируемыеКонтрагенты.Контрагент
	|			ИЗ
	|				КонтролируемыеКонтрагенты КАК КонтролируемыеКонтрагенты)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка КАК РасчетныйДокумент,
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка.Дата КАК Период,
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка.Организация,
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка.Контрагент,
	|	ПриобретениеУслугПоЛизингуНачисления.Содержание КАК ПредметСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.ИнойОбъектГражданскихПрав) КАК ТипПредметаСделки,
	|	ПриобретениеУслугПоЛизингуНачисления.Содержание КАК НаименованиеПредметаСделки,
	|	ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка) КАК СтранаПроисхожденияПредметаСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперацииКонтролируемыхСделок.ПолучениеУслуг) КАК ХозяйственнаяОперация,
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка.Валюта,
	|	&ЕдиницаИзмеренияШтука КАК ЕдиницаИзмерения,
	|	1 КАК Количество,
	|	ПриобретениеУслугПоЛизингуНачисления.СтавкаНДС,
	|	ПриобретениеУслугПоЛизингуНачисления.СуммаСНДС - ПриобретениеУслугПоЛизингуНачисления.СуммаНДС КАК СуммаБезНДС,
	|	ПриобретениеУслугПоЛизингуНачисления.СуммаНДС,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ОсуществленРасход) КАК ТипКонтролируемойСделки,
	|	ЛОЖЬ КАК ТоварМировойБиржевойТорговли,
	|	ВЫБОР
	|		КОГДА ПриобретениеУслугПоЛизингуНачисления.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК Грузоотправитель,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК Грузополучатель,
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка.Договор КАК Договор,
	|	НЕОПРЕДЕЛЕНО КАК Соглашение,
	|	ПриобретениеУслугПоЛизингуНачисления.Ссылка КАК РегистраторРегистраСумм,
	|	1 КАК МножительСуммовыхДанных,
	|	ПриобретениеУслугПоЛизингуНачисления.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ПОМЕСТИТЬ ТаблицаСделок
	|ИЗ
	|	ДокументыКонтролируемыхСделок КАК ДокументыКонтролируемыхСделок
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеУслугПоЛизингу.Начисления КАК ПриобретениеУслугПоЛизингуНачисления
	|		ПО ДокументыКонтролируемыхСделок.РасчетныйДокумент = ПриобретениеУслугПоЛизингуНачисления.Ссылка
	|ГДЕ
	|	(ДокументыКонтролируемыхСделок.ВзаимозависимыеЛица
...
Процедура ДобавитьСделкиВыкупаВозвратнойТарыКлиентом(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
	|			ПО ВозвратнаяТараДляВыкупа.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
...
Функция ПолучитьОсновныеСведенияУведомленияДляВыгрузки(Уведомление) Экспорт
	...
	РеквизитыУведомления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Уведомление, 
		"Организация, НомерКорректировки, ВерсияУведомления, КодМестаПредставления, ОтчетныйГод, КодФормыРеорганизации, ИННРеорганизованнойОрганизации, КППРеорганизованнойОрганизации");
	...
	Если ЗначениеЗаполнено(ПараметрыОрганизации.РегистрацияВНалоговомОргане) Тогда
		...
		Иначе
			ОКАТО = СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыОрганизации.РегистрацияВНалоговомОргане, "КодПоОКТМО"));
			Если СтрДлина(ОКАТО) > 0 Тогда
				Пока СтрДлина(ОКАТО)< 11 Цикл
					ОКАТО = ОКАТО + "0";
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	...
	Если ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012() Тогда
	...
	Если ОрганизацияФизЛицо Тогда
		...
		Если КадровыеДанныеФизЛиц.Количество()>0 Тогда
			...
			Если ДанныеФизЛица.Страна = Справочники.СтраныМира.Россия
...
Процедура ПроверитьЛисты1Б(ОсновныеСведения, ЛистыУведомления, Ошибки)
	...
	Для Каждого Лист Из ЛистыУведомления.ЛистыРаздела1Б Цикл
		...
		Если ЗначениеЗаполнено(Лист.Строка020ТипПредмета) Тогда
			...
			Если ОсновныеСведения.ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012() Тогда
...
Функция ВерсияУведомления(Уведомление) Экспорт
	...
	ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012();
...
Функция ВерсияУведомленияПоОтчетномуГоду(ОтчетныйГод) Экспорт
	...
	Если ОтчетныйГод >= Дата(2017, 1, 1) Тогда
		Возврат ВерсияУведомления_2017();
	Иначе
		Возврат ВерсияУведомления_2012();
	КонецЕсли;
