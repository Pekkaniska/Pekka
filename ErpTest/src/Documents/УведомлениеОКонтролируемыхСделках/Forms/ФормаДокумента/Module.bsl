...
Процедура ПредставлениеСделокНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыОткрытия.Вставить("КлючВарианта", "КонтролируемыеСделки");
	ПараметрыОткрытия.Вставить("Уведомление", Объект.Ссылка);
...
Процедура КомандаЗаполнитьУведомление(Команда)
	...
	Если Объект.Ссылка <> ПредопределенноеЗначение("Документ.УведомлениеОКонтролируемыхСделках.ПустаяСсылка") Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ВопросЗаполненитьУведомлениеЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
&НаКлиенте
Процедура ВопросЗаполненитьУведомлениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьСписокВерсийУведомления()
	...
	Элементы.ВерсияУведомления.СписокВыбора.Добавить(
		Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012(),
		НСтр("ru = 'ММВ-7-13/524@ от 27 июля 2012'"));
	
	Элементы.ВерсияУведомления.СписокВыбора.Добавить(
		Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2017(),
