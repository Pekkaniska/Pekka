...
Функция ТекстЗапросаВтТаблицаРасхожденияТоваров(Запрос, ТекстыЗапроса) Экспорт
	...
	|			ТаблицаРасхождения.Назначение.НаправлениеДеятельности 
	|		ИНАЧЕ
	...
	//++ НЕ УТ
	|		И ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаПоставщика.СтатьяКалькуляции
	//-- НЕ УТ
...
Функция ТекстЗапросаВтАналитикаНулевыхОстатков(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	Документ.КорректировкаПриобретения.Расхождения КАК ТаблицаРасхождения
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций.Остатки(НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)) КАК ОстаткиТоваров
...
Функция ТекстЗапросаВтНетВТаблицеТовары(Запрос, ТекстыЗапроса)
	...
	|		И (ВЫБОР
	|			КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ТОГДА ВтАналитикаНулевыхОстатков.ВидЗапасов = ТаблицаРасхождения.ВидЗапасов
	|			ИНАЧЕ ИСТИНА КОНЕЦ)
	|ГДЕ
	|	ТаблицаРасхождения.Ссылка = &Ссылка
	|	И ТаблицаРасхождения.ВариантОтражения В (ЗНАЧЕНИЕ(Перечисление.ВариантыОтраженияКорректировокПоступлений.УменьшитьЗакупкуУменьшитьСкладскиеОстатки),
	...
	|		И НЕ ВтАналитикаНулевыхОстатков.АналитикаУчетаНоменклатуры ЕСТЬ NULL
...
Функция ТекстЗапросаТаблицаРасчетыСПоставщиками(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ТаблицаРасхождения.СуммаВзаиморасчетовСНДС
	|	КОНЕЦ)																	КАК Сумма,
	|	СУММА(ВЫБОР КОГДА ТаблицаРасхождения.СуммаВзаиморасчетовСНДС < 0 ТОГДА
	|		-ТаблицаРасхождения.СуммаВзаиморасчетовСНДС
	|	ИНАЧЕ
	|		ТаблицаРасхождения.СуммаВзаиморасчетовСНДС
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Расхождения.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР 
	...
	|	ПО 
	|		ТаблицаАналитикУчетаПартий.НомерСтроки = Расхождения.НомерСтроки
	|	 	И ТаблицаАналитикУчетаПартий.ИмяТабличнойЧасти = ""Расхождения""
	|ГДЕ
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.КорректировкаПриобретения КАК Реквизиты
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаПриобретения.Товары КАК Строки
	|	ПО Реквизиты.Ссылка = Строки.Ссылка
	|ГДЕ
	|	Реквизиты.ДокументОснование Ссылка Документ.ПриобретениеУслугПрочихАктивов
