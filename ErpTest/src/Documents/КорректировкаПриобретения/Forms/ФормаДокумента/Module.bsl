...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РеглУчетВызовСервера.ЗаполнитьПредставлениеСчетаРеглУчетаВТЧ(Объект.Расхождения);
	//-- НЕ УТ
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.Товары.ТекущиеДанные <> Неопределено Тогда
		...
		//++ НЕ УТ
		Если Поле = Элементы.ТоварыСчетРеглУчетаАктивовПассивов Тогда
...
Процедура НастроитьОтражениеВРеглУчетеАктивов(Команда)
	...
	ДопПараметры.Вставить("СоответствиеИменРеквизитов", СоответствиеИменРеквизитовДляОтраженияВРеглУчете("Активов"));
	ДопПараметры.Вставить("ИмяЭлементаКоманды", "НастроитьОтражениеВРеглУчетеАктивов");
...
Процедура НастроитьОтражениеВРеглУчетеПассивов(Команда)
	...
	//++ НЕ УТ
	ДопПараметры = Новый Структура;
	ДопПараметры.Вставить("СоответствиеИменРеквизитов", СоответствиеИменРеквизитовДляОтраженияВРеглУчете("Пассивов"));
	ДопПараметры.Вставить("ИмяЭлементаКоманды", "НастроитьОтражениеВРеглУчетеПассивов");
	РеглУчетКлиент.ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассива(ЭтаФорма,,, ДопПараметры);
	//-- НЕ УТ
	Возврат;
...
Процедура УстановитьУсловноеОформление()
	...
	РеглУчетВызовСервера.УстановитьУсловноеОформлениеСчетаУчетаПриСовместномИспользованииСтатей(ЭтаФорма, "Расхождения", ТипыСтатей);
	//-- НЕ УТ
...
Функция СоответствиеИменРеквизитовДляОтраженияВРеглУчете(ВидСтатьи)
	
	СтруктураВозврата = Новый Соответствие;
	СтруктураВозврата.Вставить("СчетУчета", "СчетУчета"+ВидСтатьи);
	СтруктураВозврата.Вставить("Субконто1","Субконто1"+ВидСтатьи);
	СтруктураВозврата.Вставить("Субконто2", "Субконто2"+ВидСтатьи);
	СтруктураВозврата.Вставить("Субконто3", "Субконто3"+ВидСтатьи);
	СтруктураВозврата.Вставить("АналитикаАктивовПассивов", "Аналитика"+ВидСтатьи);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	//++ НЕ УТ
	Элементы.ТоварыГруппаСчетРеглУчета.Видимость = КорректировкаПриобретенияУслугПрочихАктивов;
	Элементы.РасхожденияГруппаСчетРеглУчета.Видимость = КорректировкаПриобретенияУслугПрочихАктивов;
	
	НастройкаОтраженияВРеглУчете = Новый Структура;
	НастройкаОтраженияВРеглУчете.Вставить("СчетУчета", Объект.СчетУчетаАктивов);
	НастройкаОтраженияВРеглУчете.Вставить("Субконто1", Объект.Субконто1Активов);
	НастройкаОтраженияВРеглУчете.Вставить("Субконто2", Объект.Субконто2Активов);
	НастройкаОтраженияВРеглУчете.Вставить("Субконто3", Объект.Субконто3Активов);
	Элементы.НастроитьОтражениеВРеглУчетеАктивов.Заголовок = РеглУчетВызовСервера.ПредставлениеОтраженияВРеглУчете(НастройкаОтраженияВРеглУчете);
	
	НастройкаОтраженияВРеглУчете = Новый Структура;
	НастройкаОтраженияВРеглУчете.Вставить("СчетУчета", Объект.СчетУчетаПассивов);
	НастройкаОтраженияВРеглУчете.Вставить("Субконто1", Объект.Субконто1Пассивов);
	НастройкаОтраженияВРеглУчете.Вставить("Субконто2", Объект.Субконто2Пассивов);
	НастройкаОтраженияВРеглУчете.Вставить("Субконто3", Объект.Субконто3Пассивов);
	Элементы.НастроитьОтражениеВРеглУчетеПассивов.Заголовок = РеглУчетВызовСервера.ПредставлениеОтраженияВРеглУчете(НастройкаОтраженияВРеглУчете);
	//-- НЕ УТ
...
Процедура УстановитьВидимостьЭлементовПоОснованию()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаНеВыполнятьПроверкуСопоставленнойНоменклатурыПоставщика", "Видимость", Элементы.ТоварыНоменклатура.Видимость);
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыЗаполнитьНоменклатуруПоНоменклатуреПоставщика", "Видимость", Элементы.ТоварыНоменклатура.Видимость);
