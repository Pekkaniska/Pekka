...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстЗапроса = ТекстЗапросаДанныеКОбработке(Ложь);
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПланПроизводстваПродукция.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ПланПроизводства.Продукция КАК ПланПроизводстваПродукция
		|ГДЕ
		|	ПланПроизводстваПродукция.Спецификация <> ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)
		|	И ПланПроизводстваПродукция.ДатаВыпуска <> ДАТАВРЕМЯ(1, 1, 1)
		|	И ПланПроизводстваПродукция.ДатаЗапуска = ДАТАВРЕМЯ(1, 1, 1)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	Документ.ПланПроизводства КАК ПланПроизводства
		|ГДЕ
		|	ПланПроизводства.ВидПлана <> ЗНАЧЕНИЕ(Справочник.ВидыПланов.ПустаяСсылка)
		|	И ПланПроизводства.ТипПроизводственногоПроцесса = ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	И ПланПроизводства.СпособРасчетаПотребностейВМатериалах = ЗНАЧЕНИЕ(Перечисление.СпособыРасчетаМатериалов.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	ПланПроизводства.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДопПараметры.ДополнительныеИсточникиДанных.Вставить("Сценарий");
	ДопПараметры.ДополнительныеИсточникиДанных.Вставить("ВидПлана");
	
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуСсылокДляОбработки(
		Параметры.Очередь, ПолноеИмяДокумента, МенеджерВременныхТаблиц, ДопПараметры);
	...
	|	ДокументПланПроизводства.ВерсияДанных КАК ВерсияДанных,
	|	ДокументПланПроизводства.ВидПлана КАК ВидПлана,
	|	ДокументПланПроизводства.Проведен КАК Проведен,
	|	ДокументПланПроизводства.Сценарий КАК Сценарий
	|ПОМЕСТИТЬ ВТКОбработке
	|ИЗ
	|	ВТСсылкиДляОбработки КАК КОбработке
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПланПроизводства КАК ДокументПланПроизводства
	|		ПО КОбработке.Ссылка = ДокументПланПроизводства.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	ВидПлана,
	...
	|	КОбработке.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Сценарии.Календарь, ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|			ТОГДА Сценарии.Календарь
	...
	|		ПО КОбработке.Сценарий = Сценарии.Ссылка
	...
	|ИЗ
	|	ВТКОбработке КАК КОбработке
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПланПроизводства.Продукция КАК ПланПроизводстваПродукция
	|		ПО КОбработке.Ссылка = ПланПроизводстваПродукция.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТЗаблокировано_НормативнаяДлительность КАК ВТЗаблокировано_НормативнаяДлительность
	|		ПО (ПланПроизводстваПродукция.Спецификация = ВТЗаблокировано_НормативнаяДлительность.Спецификация)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТЗаблокировано_КалендарныеГрафики КАК ВТЗаблокировано_КалендарныеГрафики
	|		ПО Календари.Календарь = ВТЗаблокировано_КалендарныеГрафики.Календарь
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КОбработке.Ссылка
	|ИЗ
	|	ВТЗаблокировано_ПланыПроизводства КАК ВТЗаблокировано_ПланыПроизводства
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПланПроизводства КАК ДокументПланПроизводства
	|		ПО ВТЗаблокировано_ПланыПроизводства.Ссылка = ДокументПланПроизводства.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКОбработке КАК КОбработке
	|		ПО (ДокументПланПроизводства.ВидПлана = КОбработке.ВидПлана)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|	
	...
	|	КОбработке.Сценарий                                       КАК Сценарий,
	|	КОбработке.Сценарий.ИспользоватьДляПланированияМатериалов КАК ИспользоватьДляПланированияМатериалов,
	|	КОбработке.Сценарий.СпособРасчетаПотребностейВМатериалах  КАК СпособРасчетаПотребностейВМатериалах,
	|	КОбработке.ВидПлана                                       КАК ВидПлана,
	|	КОбработке.ВидПлана.ТипПроизводственногоПроцесса          КАК ТипПроизводственногоПроцесса,
	|	КОбработке.Проведен                                       КАК Проведен
	|ИЗ
	|	ВТКОбработке КАК КОбработке
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗаблокированныеСсылки
	|		ПО КОбработке.Ссылка = ВТЗаблокированныеСсылки.Ссылка
	|ГДЕ
	|	ВТЗаблокированныеСсылки.Ссылка ЕСТЬ NULL";
	
	БлокировкиНормативнаяДлительность = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
		"РегистрСведений.НормативнаяДлительностьПроизводства",
		МенеджерВременныхТаблиц);
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"ВТЗаблокировано_НормативнаяДлительность",
		БлокировкиНормативнаяДлительность.ИмяВременнойТаблицы);
	
	БлокировкиКалендарныеГрафики = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
		"РегистрСведений.КалендарныеГрафики",
		МенеджерВременныхТаблиц);
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"ВТЗаблокировано_КалендарныеГрафики",
		БлокировкиКалендарныеГрафики.ИмяВременнойТаблицы);
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"ВТЗаблокировано_ПланыПроизводства",
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки = Блокировка.Добавить(ПолноеИмяДокумента);
			ЭлементБлокировки.УстановитьЗначение("ВидПлана", Выборка.ВидПлана);
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(ДокументОбъект.НачалоПериодаЗамещения) Тогда
					
					ДокументОбъект.НачалоПериодаЗамещения = ДокументОбъект.НачалоПериода;
					ОбъектИзменен = Истина;
					
				КонецЕсли;
				
				Если НЕ ДокументОбъект.Сценарий.Пустая()
					И ДокументОбъект.СпособРасчетаПотребностейВМатериалах.Пустая()
					И Выборка.ИспользоватьДляПланированияМатериалов Тогда
					
					ДокументОбъект.СпособРасчетаПотребностейВМатериалах = Выборка.СпособРасчетаПотребностейВМатериалах;
					ОбъектИзменен = Истина;
					
				КонецЕсли;
				
				Если НЕ ДокументОбъект.ВидПлана.Пустая()
					И ДокументОбъект.ТипПроизводственногоПроцесса.Пустая() Тогда
					
					ДокументОбъект.ТипПроизводственногоПроцесса = Выборка.ТипПроизводственногоПроцесса;
					ОбъектИзменен = Истина;
				...
				Если ДокументОбъект.ТипПроизводственногоПроцесса <> Перечисления.ТипыПроизводственныхПроцессов.Разборка Тогда
					...
					Для каждого Строка Из ДокументОбъект.Продукция Цикл
						...
						Если ЗначениеЗаполнено(Строка.Спецификация)
							И ЗначениеЗаполнено(Строка.ДатаВыпуска)
					...
					Если СтрокиОбработать.ВГраница() <> -1 Тогда
						
						Планирование.ПолучитьДлительностьСпецификацийИЗаполнитьДатыПоГрафику(
							СтрокиОбработать,
							"ДатаВыпуска",
							"ДатаЗапуска",
							ДокументОбъект.Сценарий,
							Ложь,
							ДокументОбъект.СпособРасчетаПотребностейВМатериалах);
						ОбъектИзменен = Истина;
				...
				Если ДокументОбъект.Проведен И ДокументОбъект.Замещающий Тогда
					
					ДокументОбъект.ОбновитьЗамещениеПлана(РежимЗаписиДокумента.Запись, Истина);
					
				КонецЕсли;
				
				Если ДокументОбъект.Проведен И Не ДокументОбъект.Замещающий Тогда
					
					ЗамещенныеСтроки = ДокументОбъект.Продукция.НайтиСтроки(Новый Структура("Замещен", Истина));
					Для каждого Строка Из ЗамещенныеСтроки Цикл
						Строка.Замещен = Ложь;
						ОбъектИзменен = Истина;
					КонецЦикла;
				...
				Если ЗначениеЗаполнено(ДокументОбъект.Подразделение) 
					И КомплекснаяАвтоматизация Тогда
				...
				Если ОбъектИзменен Или ДокументОбъект.Проведен Тогда
