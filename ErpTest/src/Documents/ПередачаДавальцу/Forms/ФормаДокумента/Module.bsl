...
Процедура ВидЦеныПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидЦены) И Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ВидЦеныПриИзмененииЗавершение", ЭтотОбъект), НСтр("ru = 'Перезаполнить цены продукции?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ЗаполнитьЦены(Команда)
	...
	Если ПродажиКлиент.НеобходимоЗаполнениеЦенПоВидуЦен(Объект, "Товары", НСтр("ru='Выпущенная продукция'")) Тогда
