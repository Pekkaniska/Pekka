...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТипПроизводственногоПроцесса = Метаданные().Реквизиты.ТипПроизводственногоПроцесса.ЗначениеЗаполнения;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(РазмещениеВыпуска)
		ИЛИ РазмещениеВыпуска = Перечисления.СпособыПривязкиОперацийПроизводства.КНачалу Тогда
	...
	// с типо процесса "Без спецификаций"
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ДопСвойства = ОбеспечениеСервер.ДополнительныеСвойстваДляЗаписиНабораОбеспечениеЗаказов(ДополнительныеСвойства);
	Движения.ДвижениеТоваров.ДополнительныеСвойства.Вставить("ОбеспечениеЗаказовДополнительныеСвойства", ДопСвойства);
...
Процедура ЗаполнитьДанныеСпецификацииВСтроках(ТолькоНезаполненные = Ложь)
	...
	Для каждого Строка Из Продукция Цикл
		...
		СтруктураПодбора.Вставить("Подразделение", Подразделение);
		СтруктураПодбора.Вставить("НачалоПроизводства", НачатьНеРанее);
		СтруктураПодбора.Вставить("Номенклатура", Строка.Номенклатура);
		СтруктураПодбора.Вставить("Характеристика", Строка.Характеристика);
		
		МассивДанных.Добавить(СтруктураПодбора);
...
Процедура ВыполнитьКонтрольЗакрытияЗаказа(Отказ)
	...
	ТекстЗапроса = Документы.ЗаказНаПроизводство2_2.ТекстЗапросаКонтрольЗакрытияЗаказа();
