...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЗ
	|		Документ.ЗаказНаПроизводство2_2 КАК Реквизиты
	|	ГДЕ
	|		Реквизиты.Проведен
	|		И Реквизиты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья)
	|		И Реквизиты.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|		И НЕ Реквизиты.ДоговорНеОбязателен
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		Реквизиты.Распоряжение.ВариантОбособления <> ЗНАЧЕНИЕ(Перечисление.ВариантыОбособленияМатериаловВПроизводстве.ЭтапПроизводства)
	|		И Реквизиты.НазначениеПолуфабрикаты = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|		И НЕ Реквизиты.Распоряжение.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ГДЕ
	|		Реквизиты.Распоряжение.ВариантОбособления <> ЗНАЧЕНИЕ(Перечисление.ВариантыОбособленияМатериаловВПроизводстве.ЭтапПроизводства)
	|		И Реквизиты.НазначениеПолуфабрикаты = Реквизиты.НазначениеМатериалы
	|		И НЕ Реквизиты.Распоряжение.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	//Обработчик СписатьНаРасходы
	|	ВЫБРАТЬ
	|		ТаблицаЗаказов.Ссылка КАК Ссылка
	|	ИЗ
	|		Документ.ЗаказНаПроизводство2_2 КАК ТаблицаЗаказов
	|	ГДЕ
	...
	|						ИСТИНА
	|					ИЗ
	|						Документ.ЗаказНаПроизводство2_2.Продукция КАК ТабличнаяЧастьПродукция
	|					ГДЕ
	|						ТабличнаяЧастьПродукция.Ссылка = ТаблицаЗаказов.Ссылка
	|						И ТабличнаяЧастьПродукция.СписатьНаРасходы)
	|		И ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ЭтапПроизводства2_2 КАК ТаблицаДокументовЭтап
	|						ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ТабличнаяЧастьВыходныеИзделия
	|						ПО
	|							ТаблицаДокументовЭтап.Ссылка = ТабличнаяЧастьВыходныеИзделия.Ссылка
	|								И ТаблицаДокументовЭтап.Проведен
	|								И ТаблицаДокументовЭтап.НомерСледующегоЭтапа                 = 0
	|								И ТаблицаДокументовЭтап.Распоряжение                         = ТаблицаЗаказов.Ссылка
	|								И ТабличнаяЧастьВыходныеИзделия.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|								И ТабличнаяЧастьВыходныеИзделия.СписатьНаРасходы
	|								И НЕ ТабличнаяЧастьВыходныеИзделия.Отменено
	|				ГДЕ
	|					ИСТИНА В
	|						(ВЫБРАТЬ ПЕРВЫЕ 1
	|							ИСТИНА
	...
	|						ГДЕ
	|							ТабличнаяЧастьПродукция.Ссылка           = ТаблицаЗаказов.Ссылка
	|							И ТабличнаяЧастьПродукция.Номенклатура   = ТабличнаяЧастьВыходныеИзделия.Номенклатура
	|							И ТабличнаяЧастьПродукция.Характеристика = ТабличнаяЧастьВыходныеИзделия.Характеристика
	|							И ТабличнаяЧастьПродукция.Назначение     = ТабличнаяЧастьВыходныеИзделия.Назначение
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.ЗаказНаПроизводство2_2";
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.Распоряжение КАК Ссылка
	|ПОМЕСТИТЬ ВтОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	ВЫБОР
	|			КОГДА Таблица.Распоряжение.ВариантОбособления <> ЗНАЧЕНИЕ(Перечисление.ВариантыОбособленияМатериаловВПроизводстве.ЭтапПроизводства)
	|					И (Таблица.НазначениеПолуфабрикаты = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|						ИЛИ Таблица.НазначениеПолуфабрикаты = Таблица.НазначениеМатериалы)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ
	...
	|			ИЗ
	|				ВТСсылкиДляОбработки КАК СсылкиДляОбработки)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|	ТаблицаДокументовЭтап.Ссылка                           КАК Этап,
	|	ТабличнаяЧастьВыходныеИзделия.Номенклатура             КАК Номенклатура,
	|	ТабличнаяЧастьВыходныеИзделия.Характеристика           КАК Характеристика,
	|	ТабличнаяЧастьВыходныеИзделия.Спецификация             КАК Спецификация,
	|	ВЫБОР
	|		КОГДА ТабличнаяЧастьВыходныеИзделия.Получатель = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			ТОГДА ТаблицаДокументовЭтап.Подразделение
	|		ИНАЧЕ ТабличнаяЧастьВыходныеИзделия.Получатель               
	|	КОНЕЦ                                                  КАК Получатель,
	|	ТабличнаяЧастьВыходныеИзделия.СписатьНаРасходы         КАК СписатьНаРасходы,
	|	ТабличнаяЧастьВыходныеИзделия.СтатьяРасходов           КАК СтатьяРасходов,
	|   ТабличнаяЧастьВыходныеИзделия.АналитикаРасходов        КАК АналитикаРасходов,
	|	ТабличнаяЧастьВыходныеИзделия.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
	|	ТабличнаяЧастьВыходныеИзделия.Упаковка                 КАК Упаковка,
	|	ТабличнаяЧастьВыходныеИзделия.КоличествоУпаковок       КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ТаблицаКорректировкиПоЭтапам
	|ИЗ
	|	ВТСсылкиДляОбработки КАК ВТСсылкиДляОбработки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2 КАК ТаблицаЗаказов
	|		ПО ВТСсылкиДляОбработки.Ссылка = ТаблицаЗаказов.Ссылка
	|			И (ТаблицаЗаказов.Проведен)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ТаблицаДокументовЭтап
	|		ПО ТаблицаЗаказов.Ссылка = ТаблицаДокументовЭтап.Распоряжение
	|			И (ТаблицаДокументовЭтап.НомерСледующегоЭтапа = 0)
	|			И (ТаблицаДокументовЭтап.Проведен)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ТабличнаяЧастьВыходныеИзделия
	|		ПО (ТаблицаДокументовЭтап.Ссылка = ТабличнаяЧастьВыходныеИзделия.Ссылка)
	|			И (ТабличнаяЧастьВыходныеИзделия.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|			И (ТабличнаяЧастьВыходныеИзделия.СписатьНаРасходы)
	|			И (НЕ ТабличнаяЧастьВыходныеИзделия.Отменено)
	|ГДЕ
	...
	|				И ТабличнаяЧастьПродукция.Номенклатура   = ТабличнаяЧастьВыходныеИзделия.Номенклатура
	|				И ТабличнаяЧастьПродукция.Характеристика = ТабличнаяЧастьВыходныеИзделия.Характеристика
	|				И ТабличнаяЧастьПродукция.Назначение     = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|				И НЕ ТабличнаяЧастьПродукция.Отменено)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	//Обработчик СписатьНаРасходы - соответствующие строки в заказе НЕ БУДУТ обработаны 
	|ВЫБРАТЬ
	|	ТаблицаДокументовЭтап.Распоряжение                                КАК Заказ,
	|	ТабличнаяЧасть.Номенклатура                                       КАК Номенклатура,
	|	ТабличнаяЧасть.Характеристика                                     КАК Характеристика,
	|	ТабличнаяЧасть.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины 
	|		= ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук) КАК Штучный,
	|	ТабличнаяЧасть.Назначение                                         КАК Назначение,
	|	ТабличнаяЧасть.Спецификация                                       КАК Спецификация,
	|	ТабличнаяЧасть.Упаковка                                           КАК Упаковка,
	|	ТабличнаяЧасть.КоличествоУпаковок * 
	|		ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковкиСписатьНаРасходы, 1) КАК Количество
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ТаблицаДокументовЭтап
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ТабличнаяЧасть
	|		ПО ТаблицаДокументовЭтап.Ссылка = ТабличнаяЧасть.Ссылка
	|			И НЕ ТабличнаяЧасть.Отменено
	|ГДЕ
	|	ТабличнаяЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|	И НЕ ТабличнаяЧасть.СписатьНаРасходы
	|	И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				ТаблицаКорректировкиПоЭтапам КАК ТаблицаКорректировкиПоЭтапам
	|			ГДЕ
	|				ТаблицаКорректировкиПоЭтапам.Этап             = ТабличнаяЧасть.Ссылка
	|				И ТаблицаКорректировкиПоЭтапам.Номенклатура   = ТабличнаяЧасть.Номенклатура
	|				И ТаблицаКорректировкиПоЭтапам.Характеристика = ТабличнаяЧасть.Характеристика)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	//Обработчик СписатьНаРасходы - соответствующие строки в заказе БУДУТ обработаны 
	|ВЫБРАТЬ
	...
	|	ТабличнаяЧасть.Спецификация                                       КАК Спецификация,
	|	ТабличнаяЧасть.Спецификация 
	|		= ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)     КАК СпецификацияНеЗаполнена,
	...
	|		ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковкиСписатьНаРасходы, 1) КАК Количество
	|ИЗ
	|	ТаблицаКорректировкиПоЭтапам КАК ТабличнаяЧасть
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА НЕ ВтОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется.Ссылка ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется
	|ИЗ
	|	ВТСсылкиДляОбработки КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется КАК ВтОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется
	|		ПО (Таблица.Ссылка = ВтОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется.Ссылка)");
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
							   "&ТекстЗапросаКоэффициентУпаковкиСписатьНаРасходы",
							   Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки("ТабличнаяЧасть.Упаковка",
																							  		 "ТабличнаяЧасть.Номенклатура"));
	
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	МассивРезультатов    = Запрос.ВыполнитьПакет();
	ИндексВыборки        = МассивРезультатов.ВГраница();
	
	ИсключаемыеСтроки    = МассивРезультатов[ИндексВыборки - 2].Выгрузить();
	ИсключаемыеСтроки.Индексы.Добавить("Заказ, Номенклатура, Характеристика");
	
	ОбрабатываемыеСтроки = МассивРезультатов[ИндексВыборки - 1].Выгрузить();
	ОбрабатываемыеСтроки.Индексы.Добавить("Заказ");
	ОбрабатываемыеСтроки.Индексы.Добавить("Заказ, Номенклатура, Характеристика, СпецификацияНеЗаполнена");
	
	Выборка              = МассивРезультатов[ИндексВыборки].Выбрать();
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья
					И Объект.Договор.Пустая()
					И Не Объект.ДоговорНеОбязателен Тогда
				...
				Если Выборка.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется И НЕ Объект.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется Тогда
					Объект.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеиспользуется = Истина;
					ОбъектИзменен = Истина;
				КонецЕсли;
				
				ТипыНоменклатуры = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
										Объект.Продукция.ВыгрузитьКолонку("Номенклатура"), "ТипНоменклатуры");
				...
				Для каждого Строка Из Объект.Продукция Цикл
					...
					Если (ЗначениеЗаполнено(Строка.Номенклатура)
						...
						И ТипыНоменклатуры[Строка.Номенклатура] <> Перечисления.ТипыНоменклатуры.Работа) Тогда
				...
				ОбработчикОбновлениеСписатьНаРасходы(
					Выборка, Объект, ИсключаемыеСтроки, ОбрабатываемыеСтроки, ОбъектИзменен);
...
Процедура ОбработчикОбновлениеСписатьНаРасходы(Выборка, Объект, ИсключаемыеСтроки, ОбрабатываемыеСтроки, ОбъектИзменен)
	
	МассивОбСтрок = ОбрабатываемыеСтроки.НайтиСтроки(Новый Структура("Заказ", Выборка.Ссылка));
	Если МассивОбСтрок.Количество() > 0 Тогда
		
		ТаблицаПродукция = Объект.Продукция.Выгрузить();
		ТаблицаПродукция.Колонки.Подразделение.Имя = "ПодразделениеИсходное";
		ТаблицаПродукция.Колонки.Добавить("Подразделение"       , Новый ОписаниеТипов("СправочникСсылка.СтруктураПредприятия"));
		ТаблицаПродукция.Колонки.Добавить("Штучный"             , Новый ОписаниеТипов("Булево"));
		ТаблицаПродукция.Колонки.Добавить("КоэффициентУпаковки" , ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 7));
		ТаблицаПродукция.Колонки.Добавить("Направление"         , ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(1, 0));
		ТаблицаПродукция.Колонки.Добавить("НеОбработать"        , Новый ОписаниеТипов("Булево"));
		ТаблицаПродукция.Индексы.Добавить("Номенклатура, Характеристика, НеОбработать, Спецификация, Назначение");
		ТаблицаПродукция.Индексы.Добавить("Номенклатура, Характеристика, НеОбработать, Спецификация");
		ТаблицаПродукция.Индексы.Добавить("Номенклатура, Характеристика, НеОбработать, Назначение");
		ТаблицаПродукция.Индексы.Добавить("Номенклатура, Характеристика, НеОбработать");
		
		СтруктураОтбора = Новый Структура("Заказ, Номенклатура, Характеристика");
		ЗаполнитьЗначенияСвойств(СтруктураОтбора, МассивОбСтрок[0]);
		
		#Область Исключения_строк
		
		//1. Исключения строк из списка обрабатываемых
		МассивИскСтрок = ИсключаемыеСтроки.НайтиСтроки(СтруктураОтбора);
		Если МассивИскСтрок.Количество() > 0 Тогда
			
			ИндексИскСтроки = МассивИскСтрок.ВГраница();
			Пока ИндексИскСтроки >= 0 Цикл
				
				СтрокаИсключаемая = МассивИскСтрок[ИндексИскСтроки];
				ИндексИскСтроки   = ИндексИскСтроки - 1;
				
				//1.1 Назначение + Спецификация
				СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать, Спецификация, Назначение");
				СтруктураОтбораТЧ.НеОбработать = Ложь;
				ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаИсключаемая);
				
				ОбработчикОбновлениеСписатьНаРасходы_ИсключитьСтрокиИзОбрабатываемых(
					ТаблицаПродукция, СтрокаИсключаемая, СтруктураОтбораТЧ, 1); 
				
				Если СтрокаИсключаемая.Количество = 0 Тогда
					ИсключаемыеСтроки.Удалить(СтрокаИсключаемая);
					Продолжить;	
				КонецЕсли;	

				//1.2 Назначение
				СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать, Назначение");
				СтруктураОтбораТЧ.НеОбработать = Ложь;
				ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаИсключаемая);
				...
				//1.3 <пустое назначение> + Спецификация
				СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать, Спецификация, Назначение");
				СтруктураОтбораТЧ.НеОбработать = Ложь;
				СтруктураОтбораТЧ.Назначение   = Справочники.Назначения.ПустаяСсылка();
				ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаИсключаемая,, "Назначение");
				...
				//1.4 <пустое назначение> + <пустая спецификация>
				СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать, Спецификация, Назначение");
				СтруктураОтбораТЧ.НеОбработать = Ложь;
				СтруктураОтбораТЧ.Назначение   = Справочники.Назначения.ПустаяСсылка();
				СтруктураОтбораТЧ.Спецификация = Справочники.РесурсныеСпецификации.ПустаяСсылка();
				ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаИсключаемая,, "Спецификация, Назначение");
		...
		//2.1 Спецификация заполнена в этапе и в заказе
		СтруктураОтбора.Вставить("СпецификацияНеЗаполнена", Ложь);
		
		МассивОбСтрок  = ОбрабатываемыеСтроки.НайтиСтроки(СтруктураОтбора);
		ИндексОбСтроки = МассивОбСтрок.ВГраница();
		Пока ИндексОбСтроки >= 0 Цикл
			
			СтрокаОбрабатываемая = МассивОбСтрок[ИндексОбСтроки];
			ИндексОбСтроки       = ИндексОбСтроки - 1;	
			
			СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать, Спецификация");
			СтруктураОтбораТЧ.НеОбработать = Ложь;
			ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаОбрабатываемая);
			
			ОбработчикОбновлениеСписатьНаРасходы_ОбработатьСтроки(
				ТаблицаПродукция, СтрокаОбрабатываемая, СтруктураОтбораТЧ, -1);
			
			Если СтрокаОбрабатываемая.Количество = 0 Тогда
				ОбрабатываемыеСтроки.Удалить(СтрокаОбрабатываемая);
			КонецЕсли;		
			
		КонецЦикла;
		
		//2.2 Спецификация не заполнена в этапе
		СтруктураОтбора.Вставить("СпецификацияНеЗаполнена", Истина);
		...
		Пока ИндексОбСтроки >= 0 Цикл
			...
			//Спецификация не заполнена в заказе
			СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать, Спецификация");
			СтруктураОтбораТЧ.НеОбработать = Ложь;
			СтруктураОтбораТЧ.Спецификация = Справочники.РесурсныеСпецификации.ПустаяСсылка();
			ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаОбрабатываемая);
			...
			//Остальные совпадающие строки по номенклатуре и характеристике
			СтруктураОтбораТЧ = Новый Структура("Номенклатура, Характеристика, НеОбработать");
			СтруктураОтбораТЧ.НеОбработать = Ложь;
			ЗаполнитьЗначенияСвойств(СтруктураОтбораТЧ, СтрокаОбрабатываемая);
		...
		//Сворачивание таблицы с сохранением очередности строк
		ОбработчикОбновлениеСписатьНаРасходы_СвернутьТаблицуПродукции(ТаблицаПродукция);
		
		Объект.Продукция.Загрузить(ТаблицаПродукция);
...
Процедура ОбработчикОбновлениеСписатьНаРасходы_ИсключитьСтрокиИзОбрабатываемых(ТаблицаПродукция,
																			   СтрокаИсключаемая,
																			   СтруктураОтбораТЧ,
																			   НаправлениеОбрабатываемое)

	МассивСтрокТЧ = ТаблицаПродукция.НайтиСтроки(СтруктураОтбораТЧ);
	Если МассивСтрокТЧ.Количество() > 0 Тогда
		
		ИндексСтроки = МассивСтрокТЧ.ВГраница();
		Пока ИндексСтроки >= 0 Цикл
			
			СтрокаПродукция = МассивСтрокТЧ[ИндексСтроки];
			
			Если СтрокаПродукция.Направление = 0 Тогда
				...
				КоэффициентУпаковки  = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьКоэффициентУпаковки(
											СтрокаПродукция.Упаковка, КэшированныеЗначения, СтрокаПродукция.Номенклатура).Коэффициент;
				
				СтрокаПродукция.КоэффициентУпаковки = КоэффициентУпаковки;							
				СтрокаПродукция.Количество          = СтрокаПродукция.КоличествоУпаковок 
														* СтрокаПродукция.КоэффициентУпаковки;
				
				СтрокаПродукция.Штучный             = СтрокаИсключаемая.Штучный;							
				
			КонецЕсли;	
			
			КоличествоРаспределено = Мин(СтрокаПродукция.Количество, СтрокаИсключаемая.Количество);
			
			Если СтрокаПродукция.Количество > КоличествоРаспределено Тогда
				
				НоваяСтрокаПродукция = ТаблицаПродукция.Добавить();
				НоваяСтрокаПродукция.Направление   = НаправлениеОбрабатываемое;
				НоваяСтрокаПродукция.Подразделение = СтрокаПродукция.ПодразделениеИсходное;
				НоваяСтрокаПродукция.Количество    = СтрокаПродукция.Количество - КоличествоРаспределено;
				ЗаполнитьЗначенияСвойств(НоваяСтрокаПродукция, СтрокаПродукция,, "Направление, Подразделение, Количество");
				
			КонецЕсли;	
			
			СтрокаПродукция.Направление   = НаправлениеОбрабатываемое;
			СтрокаПродукция.Количество    = КоличествоРаспределено;
			СтрокаПродукция.Подразделение = СтрокаПродукция.ПодразделениеИсходное;
			СтрокаПродукция.НеОбработать  = Истина;
			
			СтрокаИсключаемая.Количество = СтрокаИсключаемая.Количество - КоличествоРаспределено;
			Если СтрокаИсключаемая.Количество = 0 Тогда
				Прервать;	
			КонецЕсли;	
...
Процедура ОбработчикОбновлениеСписатьНаРасходы_ОбработатьСтроки(ТаблицаПродукция,
	...
	Если МассивСтрокТЧ.Количество() > 0 Тогда
		...
		Пока ИндексСтроки >= 0 Цикл
			...
			Если СтрокаПродукция.Направление = 0 Тогда
				...
				СтрокаПродукция.Штучный             = СтрокаОбрабатываемая.Штучный;							
									
			КонецЕсли;	
			
			КоличествоРаспределено = Мин(СтрокаПродукция.Количество, СтрокаОбрабатываемая.Количество);
			...
			ЗаполнитьЗначенияСвойств(СтрокаПродукция, СтрокаОбрабатываемая, 
				"Подразделение, СписатьНаРасходы, СтатьяРасходов, АналитикаРасходов, АналитикаАктивовПассивов"); 
			
			СтрокаПродукция.Направление  = НаправлениеОбрабатываемое;
			СтрокаПродукция.Количество   = КоличествоРаспределено;
			СтрокаПродукция.НеОбработать = Истина;
			
			СтрокаОбрабатываемая.Количество = СтрокаОбрабатываемая.Количество - КоличествоРаспределено;
			Если СтрокаОбрабатываемая.Количество = 0 Тогда
				Прервать;	
			КонецЕсли;	
...
Процедура ОбработчикОбновлениеСписатьНаРасходы_СвернутьТаблицуПродукции(ТаблицаПродукция)
	
	МассивКолонокСуммирования = Новый Массив;
	МассивКолонокСуммирования.Добавить("КоличествоУпаковок");
	МассивКолонокСуммирования.Добавить("Количество");
	
	ТаблицаПродукцияШтучный = ТаблицаПродукция.СкопироватьКолонки(); 
	
	КолонкиГруппировки = "";
	КолонкиТаблицы     = ТаблицаПродукция.Колонки;
	Для каждого КолонкаТаблицы Из КолонкиТаблицы Цикл
		
		ИмяКолокни = КолонкаТаблицы.Имя;
		
		Если МассивКолонокСуммирования.Найти(ИмяКолокни) = Неопределено
		   И ИмяКолокни <> "НомерСтроки" 
		   И ИмяКолокни <> "ПодразделениеИсходное" Тогда
			КолонкиГруппировки = КолонкиГруппировки + ", " + ИмяКолокни;	
		КонецЕсли;	
		
	КонецЦикла;
	
	КолонкиГруппировки = Сред(КолонкиГруппировки, 3);
	
	СтруктураОтбора    = Новый Структура(КолонкиГруппировки);
	
	ТаблицаПродукция.Индексы.Добавить(КолонкиГруппировки);
	
	КоличествоСтрок = ТаблицаПродукция.Количество();
	ИндексСтроки    = КоличествоСтрок - 1; 
	Пока ИндексСтроки >= 0 Цикл
		
		СтрокаПродукция = ТаблицаПродукция[ИндексСтроки];
		...
		МассивСтрокТП = ТаблицаПродукция.НайтиСтроки(СтруктураОтбора);
		Если МассивСтрокТП.Количество() > 1 Тогда
			
			МассивСтрокТП.Удалить(МассивСтрокТП.Получить(СтрокаПродукция));
			
			ИндексСтрокиТП = МассивСтрокТП.ВГраница();
			Пока ИндексСтрокиТП >= 0 Цикл
				
				СтрокаСворачиваемая = МассивСтрокТП[ИндексСтрокиТП];
				
				Для каждого ИмяКолонки Из МассивКолонокСуммирования Цикл
					СтрокаПродукция[ИмяКолонки] = СтрокаПродукция[ИмяКолонки] + СтрокаСворачиваемая[ИмяКолонки];	
				КонецЦикла;	
				
				ТаблицаПродукция.Удалить(СтрокаСворачиваемая);
				КоличествоСтрок = КоличествоСтрок - 1;
				
				ИндексСтрокиТП = ИндексСтрокиТП - 1;
		...
		Если ЗначениеЗаполнено(СтрокаПродукция.Упаковка) Тогда
			...
			Если СтрокаПродукция.Штучный Тогда
				
				КоличествоШтучный = СтрокаПродукция.Количество / СтрокаПродукция.КоэффициентУпаковки;
				
				СтрокаПродукция.КоличествоУпаковок = Цел(КоличествоШтучный);
				
				Если СтрокаПродукция.КоличествоУпаковок > 0 Тогда
				
					СтруктураДействий    = Новый Структура("ПересчитатьКоличествоЕдиниц");
					КэшированныеЗначения = Новый Структура("КоэффициентыУпаковок", Новый Соответствие);
					
					ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьКоличествоЕдиницВСтрокеТЧ(СтрокаПродукция, 
																							 СтруктураДействий, 
				...
				Иначе
					...
					СтрокаПродукция.Подразделение            = СтрокаПродукция.ПодразделениеИсходное;
					СтрокаПродукция.СписатьНаРасходы         = Ложь;
					СтрокаПродукция.СтатьяРасходов           = Неопределено;
					СтрокаПродукция.АналитикаРасходов        = Неопределено;
					СтрокаПродукция.АналитикаАктивовПассивов = Неопределено;
					
				КонецЕсли;																			 
																						 
				Если КоличествоШтучный > СтрокаПродукция.КоличествоУпаковок Тогда
					
					СтрокаПродукцияШтучный = ТаблицаПродукцияШтучный.Добавить();
					
					ЗаполнитьЗначенияСвойств(СтрокаПродукцияШтучный, СтрокаПродукция,, 
						"Подразделение, КоличествоУпаковок, СписатьНаРасходы, СтатьяРасходов, АналитикаРасходов, АналитикаАктивовПассивов");
					
					СтрокаПродукцияШтучный.Подразделение      = СтрокаПродукция.ПодразделениеИсходное;
					СтрокаПродукцияШтучный.КоличествоУпаковок = КоличествоШтучный - СтрокаПродукция.КоличествоУпаковок;
					
				КонецЕсли;	
																						 
			Иначе
				
				СтрокаПродукция.КоличествоУпаковок	= СтрокаПродукция.Количество / СтрокаПродукция.КоэффициентУпаковки;
	...
	Если ТаблицаПродукцияШтучный.Количество() > 0 Тогда
		
		ТаблицаПродукцияШтучный.Свернуть(КолонкиГруппировки, СтрСоединить(МассивКолонокСуммирования, ","));
		
		Для каждого СтрокаПродукцияШтучный Из ТаблицаПродукцияШтучный Цикл
			
			ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаПродукцияШтучный);
			
			МассивСтрок = ТаблицаПродукция.НайтиСтроки(СтруктураОтбора);
			
			Если МассивСтрок.Количество() = 0 Тогда
				
				СтрокаПродукция = ТаблицаПродукция.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаПродукция, СтрокаПродукцияШтучный);
				
			Иначе
				
				СтрокаПродукция = МассивСтрок[0];
				СтрокаПродукция.КоличествоУпаковок = СтрокаПродукция.КоличествоУпаковок 
														+ СтрокаПродукцияШтучный.КоличествоУпаковок;
	...
	МассивСтрокУдаления = ТаблицаПродукция.НайтиСтроки(Новый Структура("КоличествоУпаковок", 0));
	Если МассивСтрокУдаления.Количество() > 0 Тогда
		
		ИндексСтроки = МассивСтрокУдаления.ВГраница();
		Пока ИндексСтроки >= 0 Цикл
			
			ТаблицаПродукция.Удалить(МассивСтрокУдаления[ИндексСтроки]);
			ИндексСтроки = ИндексСтроки - 1;	
