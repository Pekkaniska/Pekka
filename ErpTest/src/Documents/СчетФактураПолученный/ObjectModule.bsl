	...
	Если ЭтотОбъект.Корректировочный Тогда
		Для каждого СтрокаОснование Из ЭтотОбъект.ДокументыОснования Цикл
			Если ТипЗнч(СтрокаОснование.ДокументОснование) = Тип("ДокументСсылка.КорректировкаПриобретения")
				ИЛИ ТипЗнч(СтрокаОснование.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
...
#Область КорректировкаПриобретения
	...
	Если ЕстьКорректировки И ЕстьИсправления Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В документах основания нельзя одновременно указывать корректировки приобретения с операциями: ""%1"" и ""%2""'"),
			...
			Иначе
				...
				Пока ВыборкаПоИсходнымСчетамФактуры.Следующий() Цикл
					...
					Пока ВыборкаПоДокументам.Следующий() Цикл
						...
						Если РеквизитыОснований.Найти(ВыборкаПоДокументам.ИсходныйСчетФактура) = Неопределено Тогда
							СтрокаСРеквизитами = РеквизитыОснований.Добавить();
...
Процедура ПроверитьЗаполнениеДокументовОснований(Отказ)
	...
	Для Каждого СтрокаОснование Из ДокументыОснования Цикл
		...
		Если МассивОснований.Найти(СтрокаОснование.ДокументОснование) <> Неопределено 
			И Не ТипЗнч(СтрокаОснование.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
