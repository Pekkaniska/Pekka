...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("ru='Список ""Сырье и материалы"" будет перезаполнен. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
