...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//RS YRN +++ 21.01.2018
	Если Объект.РасшифровкаПлатежа.Количество() = 1 И ТипЗнч(Объект.РасшифровкаПлатежа[0].Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		пкОбъектРасчетов_ОсновнойДоговор = Объект.РасшифровкаПлатежа[0].Заказ.пкОсновнойДоговорСПоставщиком;
		пкОбъектРасчетов_Статус          = Объект.РасшифровкаПлатежа[0].Заказ.Статус;
		пкОбъектРасчетов                 = Объект.РасшифровкаПлатежа[0].Заказ;
	КонецЕсли;
	//RS
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		//+++rarus-spb_zlov 17.04.2017
		ФормаОплаты = "Безналичная";
		ФормаОплатыПриИзменении(Неопределено);
		//---rarus-spb_zlov 17.04.2017
	КонецЕсли;
	
	//Рарус Владимир Подрезов 25.04.2017
	пкПодразделение	= Объект.Подразделение;
	пкКтоЗаявил		= Объект.КтоЗаявил;
	пкСтатус		= Объект.Статус;
	пкЭтоНовый		= Объект.Ссылка.Пустая();
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//Рарус Владимир Подрезов 25.04.2017
	ТекстИзменений = пкПроверитьИзмененияКлючевыхРеквизитов(ПараметрыЗаписи);
	Если ТекстИзменений <> "" Тогда
		ПоказатьПредупреждение(,ТекстИзменений);
		пкПодразделение	= Объект.Подразделение;
		пкКтоЗаявил		= Объект.КтоЗаявил;
		пкСтатус		= Объект.Статус;
		пкЭтоНовый		= Ложь;
...
Процедура НадписьОплатаНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму(
		//+++rarus-spb_pavelk 31\10\2016
		//"Отчет.ОплатаЗаявокНаРасходованиеДенежныхСредств.Форма",
		"Отчет.пкОплатаЗаявокНаРасходованиеДенежныхСредств.Форма",
		//---rarus-spb_pavelk
		ПараметрыФормы,
		ЭтаФорма);
...
Процедура ТипНалогаПриИзмененииНаСервере()
	...
	//++ НЕ УТ
	Если ИспользоватьНачислениеЗарплатыУТ И Объект.ТипНалога <> Перечисления.ТипыНалогов.НДФЛ Тогда
		ОчиститьВедомостиВРасшифровкеПлатежа(Объект);
	КонецЕсли;
...
Процедура ЗаполнитьПодтверждающиеДокументы(Команда)
	...
	ОбщегоНазначенияУТКлиент.ПроверитьВозможностьЗаполненияТабличнойЧасти(
		Оповещение, ЭтаФорма, Объект.ПодтверждающиеДокументы, Новый Структура("ТипПлатежаФЗ275", НСтр("ru= 'Тип платежа 275-ФЗ'")), Ложь);
	//-- НЕ УТ
	Возврат;
...
Процедура ЗаполнитьПодтверждающиеДокументыНаСервере()
	...
	Объект.ПодтверждающиеДокументы.Загрузить(ПолучитьИзВременногоХранилища(
		ДенежныеСредстваСервер.ПодтверждающиеДокументыФЗ275(Объект.ТипПлатежаФЗ275, УникальныйИдентификатор, ДоговорКонтрагента)));
...
Процедура ФормаОплатыЗаявкиПриИзмененииСервер()
	...
	НастроитьЗависимыеЭлементыФормыНаСервере("ФормаОплаты, ФормаОплатыЗаявки");
	...
	Для каждого СтрокаРасшифровки Из Объект.РасшифровкаПлатежа Цикл 
		...
		ИначеЕсли ЗначениеЗаполнено(СтрокаРасшифровки.Заказ) Тогда
			...
			Если (ТипЗнч(СтрокаРасшифровки.Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
				...
				ВладельцыПодтверждающихДокументов.Добавить(СтрокаРасшифровки.Заказ);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если Объект.ПлатежиПо275ФЗ И Объект.РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		ОбъектРасчетов = Объект.РасшифровкаПлатежа[0].Заказ;
		...
		ИначеЕсли ТипЗнч(ОбъектРасчетов) = Тип("ДокументСсылка.ЗаказПоставщику")
			...
			Или ТипЗнч(ОбъектРасчетов) = Тип("ДокументСсылка.ПоступлениеТоваров")
...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(ИзмененныйРеквизит = "")
	...
	//Рарус Владимир Подрезов 25.04.2017
	Если (РольДоступна("ПолныеПрава") ИЛИ РольДоступна("пкПолныеПрава") ИЛИ РольДоступна("пкИзменениеЗаявокНаРасходованиеДСКОплате")) Тогда
		Элементы.Подразделение.ТолькоПросмотр											= Ложь;
		Элементы.Подразделение.Доступность												= Истина;
		Элементы.Заявитель.ТолькоПросмотр												= Ложь;
		Элементы.Заявитель.Доступность													= Истина;
		Элементы.СтатьяДвиженияДенежныхСредств.ТолькоПросмотр							= Ложь;
		Элементы.СтатьяДвиженияДенежныхСредств.Доступность								= Истина;
		Элементы.РасшифровкаБезРазбиения.ТолькоПросмотр									= Ложь;
		Элементы.РасшифровкаБезРазбиенияСтатьяДвиженияДенежныхСредств.ТолькоПросмотр	= Ложь;
		Элементы.РасшифровкаБезРазбиенияСтатьяДвиженияДенежныхСредств.Доступность		= Истина;
		Элементы.ЖелательнаяДатаПлатежа.ТолькоПросмотр									= Ложь;
		Элементы.ЖелательнаяДатаПлатежа.Доступность										= Истина;
		Элементы.БанковскийСчетКонтрагента.ТолькоПросмотр								= Ложь;
		Элементы.БанковскийСчетКонтрагента.Доступность									= Истина;
	КонецЕсли;
	//Рарус Владимир Подрезов Конец
...
Процедура УстановитьПараметрыВыбораДоговораЛизинга()
	...
	Элементы.РасшифровкаБезРазбиенияДоговорЛизинга.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
...
//Рарус Владимир Подрезов 25.04.2017
&НаСервере
Функция пкПроверитьИзмененияКлючевыхРеквизитов(ПараметрыЗаписи)
	
	ТекстИзменений = "";
	РазделительСтр = "";
	
	Если Не пкЭтоНовый И ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если Объект.Ссылка.Проведен И Объект.Ссылка.Статус = Перечисления.СтатусыЗаявокНаРасходованиеДенежныхСредств.НеСогласована Тогда
			Если пкКтоЗаявил <> Объект.КтоЗаявил Тогда
				ТекстИзменений = ТекстИзменений + РазделительСтр + НСтр("ru='- Заявитель'");
				РазделительСтр = Символы.ПС;
			КонецЕсли;
			Если пкПодразделение <> Объект.Подразделение Тогда
				ТекстИзменений = ТекстИзменений + РазделительСтр + НСтр("ru='- Подразделение'");
				РазделительСтр = Символы.ПС;
			КонецЕсли;
		КонецЕсли;
		
		Если ТекстИзменений <> "" Тогда
			ТекстИзменений = НСтр("ru='Изменены ключевые реквизиты для согласования:'") + РазделительСтр + ТекстИзменений + РазделительСтр + НСтр("ru='Процесс согласования будет перезапущен!'");
		КонецЕсли;
