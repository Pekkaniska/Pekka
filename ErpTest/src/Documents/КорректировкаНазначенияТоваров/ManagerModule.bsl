...
Функция КонтролироватьПревышениеПотребностиПоЗаказу(Назначение) Экспорт
	...
	Иначе
		...
		// В этапе производства есть экономия материалов, когда для закрытого заказа создаются резервы (нет потребности, но
		// есть резерв).
		Если ЗначениеЗаполнено(РеквизитыНазначения.Заказ)
			И ТипЗнч(РеквизитыНазначения.Заказ) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			КонтролироватьПревышениеПотребности = Ложь;
		КонецЕсли;
...
Функция ТоварыОрганизацийПоОтбору(ТаблицаОтбора, ВидОперации, Организация, КорректировкаНазначения)
	...
	|			И ВТТаблицаОтбора.Склад = КлючиАналитики.Склад
	|ГДЕ
	|	НЕ КлючиАналитики.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	|		ТаблицаСторно.АналитикаУчетаНоменклатуры.Склад          КАК Склад,
...
Функция ВременнаяТаблицаОбеспечениеЗаказовОстаткиНазначенийСУчетомКорректировки(ВидОперации, ЕстьОтборПоОрганизации)
	...
	ТекстЗапроса =
		...
		|
//++ НЕ УТКА
		|	ЕСТЬNULL(ВЫРАЗИТЬ(Набор.Заказ КАК Документ.ЗаказДавальца).Партнер,
		|		ЕСТЬNULL(Набор.Назначение.Партнер, ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)))             КАК Партнер,
		|	ЕСТЬNULL(ВЫРАЗИТЬ(Набор.Назначение.Заказ КАК Документ.ЗаказДавальца).Договор,
		|		ЕСТЬNULL(Набор.Назначение.Договор, ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка))) КАК Договор,
//-- НЕ УТКА
...
//++ НЕ УТКА
		...
		// При использовании давальческих назначений необходимо
		// проверить что оба назначения относятся к одной версии производства.
		// Или заказ задан, но не является Заказом давальца.
		|	(НЕ Набор.Назначение.Заказ ССЫЛКА Документ.ЗаказДавальца)
		// Производство 2.1.
		|	ИЛИ (ВЫРАЗИТЬ(&Назначение КАК Справочник.Назначения).ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21)
		|		И Набор.Назначение.ТипНазначения = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21))
		// Производство 2.2.
		|	ИЛИ (ВЫРАЗИТЬ(&Назначение КАК Справочник.Назначения).ТипНазначения В (ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалы22),
		|																			ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалыПодЭтап22))
		|		И Набор.Назначение.ТипНазначения В (ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалы22),
		|											ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалыПодЭтап22)))
//-- НЕ УТКА
		|
		|СГРУППИРОВАТЬ ПО
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Попытка
			...
			Если ВыборкаПоДокументам.ОбрабатыватьАналитикуОприходование Тогда
				...
				МестаУчета = РегистрыСведений.АналитикаУчетаНоменклатуры.МестаУчета(Перечисления.ХозяйственныеОперации.КорректировкаОбособленногоУчета,
					Неопределено,
					Неопределено,
					Неопределено);
				...
				РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ДокОбъект.Товары,
					МестаУчета,
					ИменаПолей,
					,
