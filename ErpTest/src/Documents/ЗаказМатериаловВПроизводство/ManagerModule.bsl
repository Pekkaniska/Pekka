...
Процедура ЗарегистрироватьДанныеКОбработкеДляГенерацииНазначений(Параметры) Экспорт
	...
	|		ПО ЗаказМатериаловВПроизводство.Ссылка = Назначения.Заказ
	|			И (ВЫБОР
	|				КОГДА ЕСТЬNULL(ЗаказМатериаловВПроизводство.НаправлениеДеятельности.УчетЗатрат, ИСТИНА)
	|					ТОГДА ЗаказМатериаловВПроизводство.НаправлениеДеятельности
	|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|			КОНЕЦ = Назначения.НаправлениеДеятельности)
	|ГДЕ
	|	ЗаказМатериаловВПроизводство.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
