...
Процедура ЗаполнитьЭтапыГрафикаОплаты() Экспорт
	...
	Если СуммаДокумента + СуммаТары > 0 Тогда
		...
		Если ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным
			ИЛИ НЕ ПоступлениеПоЗаказам ИЛИ ЗначениеЗаполнено(ГрафикИсполненияДоговора) Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС") Тогда
		...
		ИнтеграцияВЕТИСУТ.ЗаполнитьПриобретениеТоваровУслугНаОснованииВходящейТранспортнойОперацииВЕТИС(ЭтотОбъект, ДанныеЗаполнения,, СтандартнаяОбработка);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		//Для совместимости версий < 2.4
		//Для ХО Закупка только регл. в ТЧ РасшифровкаПлатежа валюта взаиморасчетов всегда была равна валюте документа,
		//а не валюте взаиморасчетов документа.
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщикаРеглУчет 
			И ЕстьЗачтенныеУпрРасчеты() Тогда
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение 
		...
		ИначеЕсли ЭтапыГрафикаОплаты.Итог("СуммаПлатежа") + ЭтапыГрафикаОплаты.Итог("СуммаЗалогаЗаТару") <> СуммаДокумента
			...
			Если ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным ИЛИ НЕ ПоступлениеПоЗаказам Тогда
...
Функция ПолучитьСуммуЗалоговойТары() Экспорт
...
Функция ЕстьЗачтенныеУпрРасчеты()
	
	Для Каждого Стр из РасшифровкаПлатежа Цикл
		Если ТипЗнч(Стр.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику")
			ИЛИ ТипЗнч(Стр.Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			ИЛИ ТипЗнч(Стр.Заказ) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
			Возврат ИСТИНА;
		КонецЕсли;
	КонецЦикла;
