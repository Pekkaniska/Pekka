...
Процедура ИнициализироватьКлючиАналитикиУчетаПоПартнерам(Запрос)
	...
	|	ВЫБОР 
	|		КОГДА &РасчетыПоЗаказам
	|			ТОГДА ЕСТЬNULL(Этапы.Заказ.НаправлениеДеятельности, &НаправлениеДеятельности)
	|		ИНАЧЕ
	|			&НаправлениеДеятельности
	|	КОНЕЦ  		 КАК НаправлениеДеятельности 
	|ИЗ
...
//+++rarus-spb_zlov 23.01.2017    
    ...
    |        ПО КурсыВалютСрезПоследних.Валюта = ПриобретениеТоваровУслуг.ВалютаВзаиморасчетов
    |ГДЕ
...
//+++rarus-spb_zlov 23.01.2017    
    ...
    Коэффициенты = пкОбщегоНазначенияСервер.ПолучитьКоэффициентыПересчетаВалютыПоПоступлению(Запрос.Параметры.Валюта, Запрос.Параметры.ВалютаВзаиморасчетов, Запрос.Параметры.Период, Запрос.Параметры.Ссылка);
//---rarus-spb_zlov 23.01.2017 
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ПО 
	|		ТаблицаАналитикУчетаПартий.НомерСтроки = Товары.НомерСтроки
	|	 	И ТаблицаАналитикУчетаПартий.ИмяТабличнойЧасти = ""Товары""
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И &ХозяйственнаяОперация В (
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЗ
	|		Документ.ПриобретениеТоваровУслуг КАК ПриобретениеТоваровУслуг
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг.ЭтапыГрафикаОплаты КАК ЭтапГрафикаОплаты
	|				ПО ПриобретениеТоваровУслуг.Ссылка = ЭтапГрафикаОплаты.Ссылка
	|	ГДЕ
	|		ПриобретениеТоваровУслуг.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПриемНаКомиссию)
		...
		Попытка
			...
			Если ДокументОбъект.ЭтапыГрафикаОплаты.Количество() = 0 
				...
				Если ДокументОбъект.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным
					ИЛИ НЕ ДокументОбъект.ПоступлениеПоЗаказам Тогда
