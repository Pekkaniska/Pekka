...
// Приводит документы к новому формату:
//  - заполняет сумму документа
//  - заполняет вид наряда:
//          - если производство по заказам и ручное распределение -> Персональный
//          - если производство по заказам и автораспределение -> Бригадный
//          - если производство без заказов 2.1 -> БригадныйБезЗаказа21 (с автораспределеним или без)
//          - если производство по прочим работам -> ПрочиеРаботы (с автораспределеним или без)
//          - если производство по ремонтам -> Ремонт (с автораспределеним или без).
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Производство Тогда
				
				Если ДокументОбъект.УдалитьПоРаспоряжениям И ДокументОбъект.Автораспределение Тогда
					
					ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.БригадныйПоЗаказу21;
					ОбъектИзменен = Истина;
					
				ИначеЕсли ДокументОбъект.УдалитьПоРаспоряжениям И Не ДокументОбъект.Автораспределение Тогда
					
					ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.БригадныйПоЗаказу21;
					ЗаполнитьСотрудниковВТаблицеВидовРабот(ДокументОбъект);
					ОбъектИзменен = Истина;
					
				ИначеЕсли Не ДокументОбъект.УдалитьПоРаспоряжениям И ДокументОбъект.Автораспределение Тогда
					
					ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.БригадныйБезЗаказа21;
					ОбъектИзменен = Истина;
					
				ИначеЕсли Не ДокументОбъект.УдалитьПоРаспоряжениям И Не ДокументОбъект.Автораспределение Тогда
			...
			ИначеЕсли ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Ремонт Тогда
				
				Если Не ДокументОбъект.Автораспределение И ДокументОбъект.Работники.Количество() > 0 Тогда
					ЗаполнитьСотрудниковВТаблицеВидовРабот(ДокументОбъект);
					ОбъектИзменен = Истина;
				КонецЕсли;
				
			ИначеЕсли ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.ПрочиеРаботы Тогда
			...
			Если ДокументОбъект.СуммаДокумента <> ДокументОбъект.ВидыРабот.Итог("Сумма") Тогда
				ДокументОбъект.СуммаДокумента = ДокументОбъект.ВидыРабот.Итог("Сумма");
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(ДокументОбъект.Исполнители) Тогда
				Если ДокументОбъект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Персональный Тогда
					ФизическиеЛица = ДокументОбъект.ВидыРабот.ВыгрузитьКолонку("Работник");
					ФизическиеЛица = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ФизическиеЛица);
					ДокументОбъект.Исполнители = ЗарплатаКадры.КраткийСоставФизЛиц(ФизическиеЛица, ДокументОбъект.Дата);
				Иначе
					ДокументОбъект.Исполнители = ДокументОбъект.Бригада;
				КонецЕсли;
				ОбъектИзменен = Истина;
			КонецЕсли;
...
Процедура ЗаполнитьСотрудниковВТаблицеВидовРабот(ДокументОбъект)
	
	Отбор = Новый Структура("УдалитьКлючСтроки");
	УдаляемыеСтроки = Новый Массив;
	
	Для Каждого ВидРабот Из ДокументОбъект.ВидыРабот Цикл
		
		Если Не ЗначениеЗаполнено(ВидРабот.УдалитьКлючСтроки) Тогда
			Продолжить;
		КонецЕсли;
		
		УдаляемыеСтроки.Добавить(ВидРабот);
		
		Отбор.УдалитьКлючСтроки = ВидРабот.УдалитьКлючСтроки;
		
		НайденныеСтроки = ДокументОбъект.Работники.НайтиСтроки(Отбор);
		
		Для Каждого Строка Из НайденныеСтроки Цикл
			
			НоваяСтрока = ДокументОбъект.ВидыРабот.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВидРабот, , "УдалитьКлючСтроки");
			
			НоваяСтрока.Работник = Строка.Работник;
			НоваяСтрока.Сумма = Строка.Сумма;
			НоваяСтрока.Количество = Строка.УдалитьКоличество;
	...
	Для Каждого Строка Из УдаляемыеСтроки Цикл
		ДокументОбъект.ВидыРабот.Удалить(Строка);
	КонецЦикла;
