...
Процедура ПриОткрытии(Отказ)
	...
	пкУстановитьВидимостьДоступностьЭлементовДоставки();
...
Процедура ЗаполнитьТМЦНаСервере()
	...
	Запрос = Новый Запрос(
		...
		//++ НЕ УТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НаработкиТМЦВЭксплуатации.СрезПоследних(&Период, ) КАК Наработки
		|		ПО ПартииТМЦВЭксплуатации.ИнвентарныйНомер = Наработки.ИнвентарныйНомер
		//-- НЕ УТ
		|ГДЕ
		|	НЕ ПартииТМЦВЭксплуатации.ПометкаУдаления
		|	И ВЫБОР
		//++ НЕ УТ
		|		КОГДА ПартииТМЦВЭксплуатации.СпособПогашенияСтоимостиБУ = ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимостиТМЦ.ПоНаработке)
		|				И ЕСТЬNULL(Наработки.Значение, 0) >= ЕСТЬNULL(Наработки.ПредельныйОбъем, 1)
		|			ТОГДА ИСТИНА
		//-- НЕ УТ
		|		КОГДА КОНЕЦПЕРИОДА(ПартииТМЦВЭксплуатации.ДатаЗавершенияЭксплуатации, МЕСЯЦ) = &Период
		//++ НЕ УТ
		|				И ПартииТМЦВЭксплуатации.СпособПогашенияСтоимостиБУ <> ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимостиТМЦ.ПоНаработке)
		//-- НЕ УТ
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ
...
//Рарус Владимир Подрезов Закупки 2.4.3 12.08.2016
#Область ПоляЦенаИСумма

&НаКлиенте
Процедура ТоварыпкЦенаПланПриИзменении(Элемент)
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	
	ТекущаяСтрока.пкСуммаСНДС = ТекущаяСтрока.Количество * ТекущаяСтрока.пкЦенаПлан;
...
//Рарус Владимир Подрезов Закупки 2.6.3 12.08.2016
#Область ПоляФормыДляДоставки

&НаКлиенте
Процедура пкСпособМестоДоставкиПеревозчикПриИзменении(Элемент)
...
&НаКлиенте
Процедура пкАдресДоставкиПриИзменении(Элемент)
	
	ИмяРеквизитаАдресаДоставки = пкПолучитьИмяРеквизитаАдресаДоставки(Элемент);
...
&НаКлиенте
Процедура пкАдресДоставкиОчистка(Элемент, СтандартнаяОбработка)
	
	пкАдресДоставкиПриИзменении(Элемент);
	
КонецПроцедуры

&НаКлиенте
Процедура пкАдресДоставкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаКлиенте
Процедура пкАдресДоставкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ДопИнфо 			= Объект.пкДополнительнаяИнформацияПоДоставке;
	ЭлементыФормы		= Элементы;
	ИмяЭлементаФормы	= Элемент.Имя;
	
	Если Не ЗначениеЗаполнено(ДопИнфо) Тогда
	ИначеЕсли ЭлементыФормы.Найти("пкАдресПоставщика") <> Неопределено Тогда
		
		ДопИнфоИзменено = Не пкДопИнфоЕстьВСпискеВыбораЭлемента(ЭлементыФормы.пкАдресПоставщика, ДопИнфо);
		
	ИначеЕсли ЭлементыФормы.Найти("пкАдресПункта") <> Неопределено Тогда
		
		ДопИнфоИзменено = Не пкДопИнфоЕстьВСпискеВыбораЭлемента(ЭлементыФормы.пкАдресПункта, ДопИнфо);
		
	Иначе
		
		ДопИнфоИзменено = Не пкДопИнфоЕстьВСпискеВыбораЭлемента(ЭлементыФормы.пкАдресДоставкиПолучателя, ДопИнфо);
		Если ЭлементыФормы.Найти("пкАдресДоставкиПолучателя1") <> Неопределено
			И Не ДопИнфоИзменено Тогда
			ДопИнфоИзменено = Не пкДопИнфоЕстьВСпискеВыбораЭлемента(ЭлементыФормы.пкАдресДоставкиПолучателя1, ДопИнфо);
		КонецЕсли;
	КонецЕсли;
		
	Если ЭлементыФормы.Найти("пкАдресДоставкиПеревозчика") <> Неопределено
		И Не ДопИнфоИзменено Тогда
		ДопИнфоИзменено = Не пкДопИнфоЕстьВСпискеВыбораЭлемента(ЭлементыФормы.пкАдресДоставкиПеревозчика, ДопИнфо);
	КонецЕсли;
	
	ДопИнфоИзмененоПользователем = ДопИнфоИзменено;
	
	Если ИмяЭлементаФормы = "пкАдресДоставкиПолучателя" Тогда
		Если ДопИнфоИзмененоПользователем
			И ВыбранноеЗначение.Свойство("ДополнительнаяИнформацияПоДоставке") Тогда
			Объект.пкАдресДоставки						= ВыбранноеЗначение.пкАдресДоставки;
			Объект.пкАдресДоставкиЗначенияПолей			= ВыбранноеЗначение.пкАдресДоставкиЗначенияПолей;
		Иначе
			Объект.пкАдресДоставки						= ВыбранноеЗначение.пкАдресДоставки;
			Объект.пкАдресДоставкиЗначенияПолей			= ВыбранноеЗначение.пкАдресДоставкиЗначенияПолей;
			Объект.пкДополнительнаяИнформацияПоДоставке	= ВыбранноеЗначение.ДополнительнаяИнформацияПоДоставке;
		КонецЕсли;
	ИначеЕсли ИмяЭлементаФормы = "пкАдресДоставкиПеревозчика" Тогда
		Если ДопИнфоИзмененоПользователем 
			И ВыбранноеЗначение.Свойство("ДополнительнаяИнформацияПоДоставке") Тогда
			Объект.пкАдресДоставки						= ВыбранноеЗначение.пкАдресДоставки;
			Объект.пкАдресДоставкиЗначенияПолей			= ВыбранноеЗначение.пкАдресДоставкиЗначенияПолей;
		Иначе
			...
			Объект.пкДополнительнаяИнформацияПоДоставке	= ВыбранноеЗначение.ДополнительнаяИнформацияПоДоставке;
		КонецЕсли;
		Объект.пкАдресДоставкиПеревозчика 				= ВыбранноеЗначение.пкАдресДоставки;
		Объект.пкАдресДоставкиПеревозчикаЗначенияПолей 	= ВыбранноеЗначение.пкАдресДоставкиЗначенияПолей;
		
	ИначеЕсли ИмяЭлементаФормы = "пкАдресДоставкиПолучателя1" Тогда
		Если ДопИнфоИзмененоПользователем
			И ВыбранноеЗначение.Свойство("ДополнительнаяИнформацияПоДоставке") Тогда
			Объект.пкАдресДоставки						= ВыбранноеЗначение.пкАдресДоставки;
			Объект.пкАдресДоставкиЗначенияПолей			= ВыбранноеЗначение.пкАдресДоставкиЗначенияПолей;
		Иначе
			...
			Объект.пкДополнительнаяИнформацияПоДоставке	= ВыбранноеЗначение.ДополнительнаяИнформацияПоДоставке;
		КонецЕсли;
	ИначеЕсли ИмяЭлементаФормы = "пкАдресДоставкиПолучателя2" Тогда
...
&НаКлиенте
Процедура пкОсобыеУсловияПеревозкиПриИзменении(Элемент)
...
&НаСервере
Процедура пкОсобыеУсловияПеревозкиПриИзмененииСервер()
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция пкПолучитьИмяРеквизитаАдресаДоставки(Элемент)
	
	Если СтрНайти(Элемент.Имя, "пкАдресДоставкиПеревозчика") > 0 Тогда
		ИмяРеквизитаАдреса = "пкАдресДоставкиПеревозчика";
	Иначе
		ИмяРеквизитаАдреса = "пкАдресДоставки";
	КонецЕсли;
...
&НаКлиенте
Процедура пкУстановитьВидимостьДоступностьЭлементовДоставки()
	
	СпособДоставки			= Объект.пкСпособДоставки;
	ОсобыеУсловияПеревозки	= Объект.пкОсобыеУсловияПеревозки;
	ЭлементыФормы			= Элементы;
	
	ЭлементыФормы.пкОсобыеУсловияПеревозкиОписание.Доступность = ОсобыеУсловияПеревозки;
	ЭлементыФормы.пкОсобыеУсловияПеревозкиОписание1.Доступность = ОсобыеУсловияПеревозки;
	ЭлементыФормы.пкОсобыеУсловияПеревозкиОписаниеДоставкаПеревозчиком.Доступность = ОсобыеУсловияПеревозки;
	...
	Если СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчика")
		...
		ЭлементыФормы.пкСтраницыПеревозчик.ТекущаяСтраница = ЭлементыФормы.пкСтраницаПеревозчик;
	Иначе
		ЭлементыФормы.пкСтраницыПеревозчик.ТекущаяСтраница = ЭлементыФормы.пкСтраницаПеревозчикПусто;
	КонецЕсли;
	...
	Если (СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.ДоКлиента")
		...
		ЭлементыФормы.пкСтраницыДоставки.ТекущаяСтраница = ЭлементыФормы.пкСтраницаДоставкаДоПолучателя;
		
	ИначеЕсли СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчика") Тогда
		ЭлементыФормы.пкСтраницыДоставки.ТекущаяСтраница = ЭлементыФормы.пкСтраницаДоставкаПеревозчиком;
		
	ИначеЕсли СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчикаПоАдресу") Тогда
		ЭлементыФормы.пкСтраницыДоставки.ТекущаяСтраница = ЭлементыФормы.пкСтраницаДоставкаПеревозчикомПоАдресу;
	...
	ИначеЕсли СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.НашимиСиламиСАдресаОтправителя")		
		...
		ЭлементыФормы.пкСтраницыДоставки.ТекущаяСтраница = ЭлементыФормы.пкСтраницаДоставкаНашимиСилами;
		Если СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчикаДоПунктаПередачи") Тогда
			ЭлементыФормы.пкСтраницыАдресПередачи.ТекущаяСтраница = ЭлементыФормы.пкСтраницаАдресПеревозчика;
		Иначе
			ЭлементыФормы.пкСтраницыАдресПередачи.ТекущаяСтраница = ЭлементыФормы.пкСтраницаАдресПоставщика;
		КонецЕсли;
		
	Иначе
		
		ЭлементыФормы.пкСтраницыДоставки.ТекущаяСтраница = ЭлементыФормы.пкСтраницаСамовывоз;
	...
	Если СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.ДоКлиента")
		...
		Или СпособДоставки = ПредопределенноеЗначение("Перечисление.СпособыДоставки.СиламиПеревозчикаДоПунктаПередачи") Тогда
		ЭлементыФормы.пкСтраницаДоставка.Картинка = БиблиотекаКартинок.Комментарий;
	Иначе
		ЭлементыФормы.пкСтраницаДоставка.Картинка = Новый Картинка;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция пкДопИнфоЕстьВСпискеВыбораЭлемента(Элемент, ДопИнфо)
