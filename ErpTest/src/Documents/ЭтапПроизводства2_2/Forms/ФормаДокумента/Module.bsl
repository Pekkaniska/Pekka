...
Процедура Трудозатраты_ЗаполнитьСтатьюКалькуляцииВВыделенныхСтроках(Команда)
	...
	Если Элементы.Трудозатраты.ВыделенныеСтроки.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Необходимо выбрать строки, для которых нужно заполнить статью калькуляции.'");
		ПоказатьПредупреждение(,ТекстПредупреждения);
		Возврат;
	КонецЕсли;
	...
	ДополнительныеПараметры = Новый Структура("Идентификаторы", МассивСтрок);
	ОписаниеОповещения = Новый ОписаниеОповещения("Трудозатраты_ЗаполнитьСтатьюКалькуляцииВВыделенныхСтрокахЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Функция ФорматированнаяСтрокаНадписьСпецификация()
	...
	Если ПравоДоступа("Использование", Метаданные.Обработки.РедактированиеПоследовательностиЭтаповПроизводства) Тогда
		МассивСтрок.Добавить("  ");
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'показать этапы'"),,,, "#ПоказатьЭтапы"));
	КонецЕсли;
...
Функция ФорматированнаяСтрокаПланироватьНеРанее(ПланироватьНеРанее, ФорматДатыГрафика)
	...
	Если НЕ ЗначениеЗаполнено(ПланироватьНеРанее) Тогда
		...
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(НСтр("ru = '<указать>'"),,,,"#РедактироватьЗначение"));
	...
	Иначе
		...
		МассивСтрок.Добавить("  ");
		МассивСтрок.Добавить(
			Новый ФорматированнаяСтрока(НСтр("ru = 'изменить'"),,,,"#РедактироватьЗначение"));
