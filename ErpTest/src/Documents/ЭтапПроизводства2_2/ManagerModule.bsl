...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса, ИмяРегистра)
	...
	|		И ВидыЗапасов.Получатель = КлючиНаСкладе.Склад
	...
	|		И ВидыЗапасов.Получатель = КлючиНаСкладеБезНазначения.Склад
...
Процедура ИнициализироватьДанныеДокументаОтложенно(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	ТекстЗапросаТаблицаРезервыТоваровОрганизацийОтложенно(Запрос, ТекстыЗапроса, Регистры);
...
Функция ТекстЗапросаВтРезервыТоваровОрганизаций(Запрос, ТекстыЗапроса)
	
	ИмяРегистра = "ВтРезервыТоваровОрганизаций";
	...
	|ПОМЕСТИТЬ ВтРезервыТоваровОрганизаций
	|ИЗ
...
Функция ТекстЗапросаТаблицаТоварыОрганизацийОтложенно(Запрос, ТекстыЗапроса, Регистры)
	...
	|		И Аналитика.Склад = АналитикаПолучателя.Склад
...
Функция ТекстЗапросаТаблицаРезервыТоваровОрганизацийОтложенно(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеСерверУТ.ЕстьТаблицаЗапроса("ВтРезервыТоваровОрганизаций", ТекстыЗапроса) Тогда
		ТекстЗапросаВтРезервыТоваровОрганизаций(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	|ИЗ
	|	РегистрНакопления.РезервыТоваровОрганизаций КАК Движения
	|
	|ГДЕ
	|	Движения.Регистратор = &Ссылка
	...
	|	Резервы.ЭтоСторно                             КАК ЭтоСторно
	|ИЗ
	|	ВтРезервыТоваровОрганизаций КАК Резервы
	|	
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	|	Порядок
...
Функция ТекстЗапросаВтСебестоимостьТоваровОтложенно(Запрос, ТекстыЗапроса)
	...
	|		И Аналитика.Склад = АналитикаПолучателяБезНазначения.Склад
	...
	|		И ВидыЗапасов.ВидЗапасов.ВладелецТовара = АналитикаКомитента.Склад
...
Процедура ИнициализироватьКлючиАналитикиНоменклатурыОтложенно(Запрос)
	...
	Если Запрос.Параметры.Свойство("ТаблицаВидыЗапасов") Тогда
		...
		|			И (РСАналитикаУчетаНоменклатуры.Склад = ВидыЗапасов.ВладелецТовара)
...
Процедура ЗаполнитьПараметрыОбеспеченияВПакетномРежиме(Этапы, СтрокиДляЗаполненияОбеспечения, ВариантыОбеспечения)
	...
	Попытка
		...
		СтруктураДействийНовыхСтрок = Новый Структура("ПересчитатьКоличествоУпаковок");
		СтруктураДействийИсходныхСтрок = Новый Структура();
		...
		Для Индекс = 0 По Этапы.ВГраница() Цикл
			...
			ПараметрыДействияПроверитьЗаполнитьНазначение = ОбеспечениеПроизводстваКлиентСервер.ПараметрыДействияПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(
				ДокументОбъект);
			СтруктураДействийНовыхСтрок.Вставить("ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства", ПараметрыДействияПроверитьЗаполнитьНазначение);
			СтруктураДействийИсходныхСтрок.Вставить("ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства", ПараметрыДействияПроверитьЗаполнитьНазначение);
			...
			Для каждого КодСтроки Из НайденнаяСтрока.КодыСтрок Цикл
				...
				Если СтрокиОбеспечение.Количество() = 1 Тогда
					...
					ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ИсходнаяСтрока, СтруктураДействийИсходныхСтрок, КэшированныеЗначения);
				...
				Иначе
					...
					Для каждого ДанныеОбеспечения Из СтрокиОбеспечение Цикл
						...
						ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрока, СтруктураДействийНовыхСтрок, КэшированныеЗначения);
...
Функция РассчитатьРасходМатериалов(МенеджерВременныхТаблиц, ДатаРасхода = '00010101') Экспорт
	...
	СтруктураОтбораРасход = Новый Структура("Ссылка, Номенклатура, Характеристика, Серия, СтатьяКалькуляции");
	Для Каждого Расход Из РасходМатериаловИРаботКОбеспечению Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураОтбораРасход, Расход);
		
		ОстаткиОтгруженных = ОтгруженныеМатериалы.НайтиСтроки(СтруктураОтбораРасход);
		Для Каждого Остаток Из ОстаткиОтгруженных Цикл
			
			Если Остаток.Количество = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			Количество = Мин(Остаток.Количество, Расход.Количество);
			
			НовыйРасход = РасходМатериаловИРабот.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйРасход, Расход);
			НовыйРасход.Количество = Количество;
			
			Расход.Количество	= Расход.Количество - Количество;
			Остаток.Количество	= Остаток.Количество - Количество;
	...
	СтруктураОтбораЭкономия = Новый Структура("Ссылка, Номенклатура, Характеристика, Серия");
	Для Каждого Экономия Из ЭкономияМатериалов Цикл
		
		ЗаполнитьЗначенияСвойств(СтруктураОтбораЭкономия, Экономия);
		
		ОстаткиОтгруженных = ОтгруженныеМатериалы.НайтиСтроки(СтруктураОтбораЭкономия);
		Для Каждого Остаток Из ОстаткиОтгруженных Цикл
			...
			Количество = Мин(Остаток.Количество, Экономия.Количество);
			
			Экономия.Количество	= Экономия.Количество - Количество;
			Остаток.Количество	= Остаток.Количество - Количество;
	...
	СтруктураОтбораРасход = Новый Структура("Ссылка, Номенклатура, Характеристика, Серия");
	Для Каждого Расход Из РасходМатериаловИРаботКОбеспечению Цикл
		...
		ЗаполнитьЗначенияСвойств(СтруктураОтбораРасход, Расход);
		СтруктураОтбораРасход.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
		...
		Для Каждого Остаток Из ОстаткиОтгруженных Цикл
			...
			НовыйРасход.СтатьяКалькуляции	= Остаток.СтатьяКалькуляции;
			НовыйРасход.Количество			= Количество;
	...
	СтруктураОтбораЭкономия = Новый Структура("Ссылка, Номенклатура, Характеристика");
	Для Каждого Экономия Из ЭкономияМатериалов Цикл
		
		Если Экономия.Количество = 0 Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПроверитьЗаполнитьНазначения(Объект, СхемаОбособленияКешированноеЗначение = Неопределено, ТолькоВРеквизитах = Ложь) Экспорт
	...
	//+rarus_spb_b.mihail 2018.11.26
	// Обход ошибки при обновлении
	//Если СхемаОбособления.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеИспользуется Тогда
	Если СхемаОбособления.ОбособлениеПолуфабрикатовПоНазначениюЭтаповНеИспользуется = Истина Тогда
	//-rarus_spb_b.mihail 2018.11.26
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	"ВЫБРАТЬ
	|	Изделия.Ссылка КАК Ссылка,
	|	ЭтапыПотребители.Ссылка КАК Потребитель
	...
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|// Обработчик заполнения нового реквизита ""ПорядокРаботыВидовРабочихЦентров"" в документе.
	|ВЫБРАТЬ
	|	ВидыРЦ.Ссылка КАК Ссылка
	|ИЗ
	...
	|	И Этапы.ПорядокРаботыВидовРабочихЦентров = ЗНАЧЕНИЕ(Перечисление.ПорядокРаботыВидовРабочихЦентров.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВидыРЦ.Ссылка
	|
	...
	|ОБЪЕДИНИТЬ
	|
	|// Обработчик обновления таблицы последователей.
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ПО Полуфабрикаты.Ссылка = СвязиЭтапов.Ссылка
	|			И Полуфабрикаты.Потребитель = СвязиЭтапов.Этап
	|ГДЕ
	|	СвязиЭтапов.Этап ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ
	|
	...
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ДокументыСНезаполненнойВалютой
	|ГДЕ
	|	ДокументыСНезаполненнойВалютой.Валюта = ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|// Обработчик заполнения нового реквизита ""НомерНаПечать"" в документе.
	|ВЫБРАТЬ
	|	ДокументыСНезаполненнымНомеромНаПечать.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ДокументыСНезаполненнымНомеромНаПечать
	|ГДЕ
	|	ДокументыСНезаполненнымНомеромНаПечать.НомерНаПечать = """"
	|
	|ОБЪЕДИНИТЬ
	|
	|// Обработчик заполнения нового реквизита ""Тип производственного процесса"" в документе.
	|ВЫБРАТЬ
	|	ТаблицаДокумента.Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.ТипПроизводственногоПроцесса = ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	...
	|	И ТаблицаДокумента.НомерСледующегоЭтапа = 0
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|		ИЛИ ТаблицаДокумента.ЗаказПереработчику <> ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка)
	|			И ТаблицаДокумента.ЗаказПереработчику.ВариантПриемкиТоваров <> ТаблицаДокумента.ВариантПриемкиТоваров
	|
	|ОБЪЕДИНИТЬ
	|
	|// Обработчик очистки вида запасов для работ
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументыСЗаполненнымВидомЗапасовДляРабот.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ДокументыСЗаполненнымВидомЗапасовДляРабот
	|ГДЕ
	|	ДокументыСЗаполненнымВидомЗапасовДляРабот.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга)
	|
	|ОБЪЕДИНИТЬ
	|
	|// Обработчик снятия флага ""Отгружать одной датой""
	|ВЫБРАТЬ
	|	ДокументЭтап.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ДокументЭтап
	|ГДЕ
	|	ДокументЭтап.НеОтгружатьЧастями
	|	И (ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ТабличнаяЧастьЭтап
	|				ГДЕ
	|					ТабличнаяЧастьЭтап.Ссылка               = ДокументЭтап.Ссылка
	|					И ДокументЭтап.ЗаказПереработчику       = ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка)
	|					И ТабличнаяЧастьЭтап.ДатаОтгрузки      <> ДокументЭтап.ДатаОтгрузки
	|					И ТабличнаяЧастьЭтап.ВариантОбеспечения В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить), ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно)))
	|			ИЛИ 
	|		ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.ЭтапПроизводства2_2 КАК ШапкаЭтап
	|						ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ТабличнаяЧастьЭтап
	|						ПО
	|							ШапкаЭтап.Ссылка = ТабличнаяЧастьЭтап.Ссылка
	|				ГДЕ
	|					ШапкаЭтап.ЗаказПереработчику            = ДокументЭтап.ЗаказПереработчику
	|					И ШапкаЭтап.ЗаказПереработчику         <> ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка)
	|					И ШапкаЭтап.НеОтгружатьЧастями
	|					И ТабличнаяЧастьЭтап.ДатаОтгрузки      <> ШапкаЭтап.ДатаОтгрузки
	|					И ТабличнаяЧастьЭтап.ВариантОбеспечения В (ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить), ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))))
	|
	|ОБЪЕДИНИТЬ
	|
	|// Установка признака УдалитьЯвляетсяНазначениемПродукции по ТЧ ВыходныеИзделия
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Реквизиты.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК Таблица
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО Таблица.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
	|ГДЕ
	|	НЕ Реквизиты.ЯвляетсяНазначениемПродукции
	|	И Аналитика.Назначение = Реквизиты.Назначение
	|	И Аналитика.Склад ССЫЛКА Справочник.СтруктураПредприятия
	|
	|ОБЪЕДИНИТЬ
	|
	|// Установка признака УдалитьЯвляетсяНазначениемПродукции по ТЧ ПобочныеИзделия
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Реквизиты.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК Таблица
	...
	|ОБЪЕДИНИТЬ
	|
	|// Заполнение статуса указания серий отправитель в ТЧ ОбеспечениеМатериаламиИРаботами
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК Таблица
	|ГДЕ
	|	Таблица.СтатусУказанияСерий <> 0
	|	И Таблица.СтатусУказанияСерийОтправитель = 0
	|	И Таблица.СтатусУказанияСерийПолучатель = 0
	|
	|ОБЪЕДИНИТЬ
	|
	|// Заполнение статуса указания серий получатель в ТЧ ЭкономияМатериалов
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	...
	|ОБЪЕДИНИТЬ
	|
	|// Заполнение пустого реквизита ПланироватьНеРанее значением Неопределено
	|ВЫБРАТЬ
	|	Этапы.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Этапы
	|ГДЕ
	|	(Этапы.ПланироватьНеРанее = ДАТАВРЕМЯ(1, 1, 1)
	|			ИЛИ Этапы.ПланироватьНеРанее = ЗНАЧЕНИЕ(Документ.ЭтапПроизводства2_2.ПустаяСсылка))
	|
	|ОБЪЕДИНИТЬ
	|
	|// Корректировка реквизита НазначениеПродукция по схеме обособления
	|ВЫБРАТЬ
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.НазначениеПродукция <> Таблица.Распоряжение.НазначениеПродукция
	|	И Таблица.Распоряжение.ТипПроизводственногоПроцесса = ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.Разборка)
	|
	|ОБЪЕДИНИТЬ
	|
	|// Заполнение нового реквизита НазначениеПолуфабрикаты
	|ВЫБРАТЬ
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.НазначениеПолуфабрикаты = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|	И Таблица.НазначениеМатериалы <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	|// Заполнение нового реквизита Назначение в ТЧ ОбеспечениеМатериаламиИРаботами
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК Таблица
	...
	|ОБЪЕДИНИТЬ
	|
	|// Очистка даты фактического окончания у документов в статусе Начат
	|ВЫБРАТЬ
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Начат)
	|	И Таблица.ФактическоеОкончаниеЭтапа <> ДАТАВРЕМЯ(1,1,1)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВтЗапланированаРазборка.Ссылка
	|ИЗ
	...
	|ОБЪЕДИНИТЬ
	|
	// Заполнение нового реквизита ПартияПроизводства
	|ВЫБРАТЬ
	|	Таблица.Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ
	|
	// очистка секунд в дате начала этапа
	|ВЫБРАТЬ
	|	Таблица.Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.ФактическоеНачалоЭтапа <> НАЧАЛОПЕРИОДА(Таблица.ФактическоеНачалоЭтапа, МИНУТА)
	|
	|ОБЪЕДИНИТЬ
	|
	// очистка секунд в дате окончания этапа
	|ВЫБРАТЬ
	|	Таблица.Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.ФактическоеОкончаниеЭтапа <> НАЧАЛОПЕРИОДА(Таблица.ФактическоеОкончаниеЭтапа, МИНУТА)
	|
	|ОБЪЕДИНИТЬ
	|
	// Заполнение реквизита статья расходов дефолтным значением статьи расходов
	|ВЫБРАТЬ
	|	ТаблицаЭтапов.Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ТаблицаЭтапов
	|ГДЕ
	|	ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	...
	|						ПО
	|							ЭтапПроизводства2_2.Ссылка          = ТабличнаяЧасть.Ссылка
	|								И ЭтапПроизводства2_2.Ссылка    = ТаблицаЭтапов.Ссылка
	...
	|ОБЪЕДИНИТЬ
	|
	// Заполнение нового реквизита ДатаРеестраДокументов
	|ВЫБРАТЬ
	|	Таблица.Ссылка
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Таблица
	|ГДЕ
	|	Таблица.ДатаРеестраДокументов = ДАТАВРЕМЯ(1,1,1)
	|");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДополнительныеПараметрыВыборкиДанных.ДополнительныеИсточникиДанных.Вставить("ЗаказПереработчику");
	
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуСсылокДляОбработки(
		Параметры.Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметрыВыборкиДанных);
	...
	|	ОбъектыДляОбработки.Ссылка.ЗаказПереработчику КАК ЗаказПереработчику,
	|	ОбъектыДляОбработки.Ссылка.Распоряжение КАК ЗаказНаПроизводство,
	|
	|	ВЫБОР КОГДА ОбъектыДляОбработки.Ссылка.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			КОГДА ОбъектыДляОбработки.Ссылка.ВариантПриемкиТоваров <> ЕСТЬNULL(ОбъектыДляОбработки.Ссылка.ЗаказПереработчику.ВариантПриемкиТоваров,
	|																		ОбъектыДляОбработки.Ссылка.ВариантПриемкиТоваров)
	|				ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ТребуетсяУказаниеВариантаПриемки,
	|	
	|	МАКСИМУМ(НЕ ОбеспечениеМатериаламиИРаботами.Ссылка ЕСТЬ NULL) КАК РазличаютсяДатыОтгрузки,
	|	МАКСИМУМ(НЕ ЭтапПроизводства2_2ВыходныеИзделия.Ссылка ЕСТЬ NULL) КАК НужноОчищатьВидЗапасов,
	|	МАКСИМУМ(НЕ ОбеспечениеСтатусСерий.Ссылка ЕСТЬ NULL) КАК ЗаполнитьСтатусСерийОбеспечение,
	|	МАКСИМУМ(НЕ ЭкономияСтатусСерий.Ссылка ЕСТЬ NULL) КАК ЗаполнитьСтатусСерийЭкономия,
	...
	|		ПО ОбъектыДляОбработки.Ссылка = ЭтапПроизводства2_2ВыходныеИзделия.Ссылка
	|			И (ЭтапПроизводства2_2ВыходныеИзделия.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга))
	|		
	|		// Обработчик снятия флага ""Отгружать одной датой""
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ОбеспечениеМатериаламиИРаботами
	|		ПО ОбъектыДляОбработки.Ссылка = ОбеспечениеМатериаламиИРаботами.Ссылка
	|		 И ОбъектыДляОбработки.Ссылка.ЗаказПереработчику = ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка)
	|		 И ОбъектыДляОбработки.Ссылка.НеОтгружатьЧастями
	|		 И ОбеспечениеМатериаламиИРаботами.ДатаОтгрузки <> ОбъектыДляОбработки.Ссылка.ДатаОтгрузки
	|		 И ОбеспечениеМатериаламиИРаботами.ВариантОбеспечения В(
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
	|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
	|
	|		// Обработчик заполнения реквизита СтатусУказанияСерийОтправитель
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ОбеспечениеСтатусСерий
	|		ПО ОбъектыДляОбработки.Ссылка = ОбеспечениеСтатусСерий.Ссылка
	|		 И ОбеспечениеСтатусСерий.СтатусУказанияСерий <> 0
	|		 И ОбеспечениеСтатусСерий.СтатусУказанияСерийОтправитель = 0
	|		 И ОбеспечениеСтатусСерий.СтатусУказанияСерийПолучатель = 0
	|
	|		// Обработчик заполнения реквизита СтатусУказанияСерийПолучатель
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ЭкономияМатериалов КАК ЭкономияСтатусСерий
	|		ПО ОбъектыДляОбработки.Ссылка = ЭкономияСтатусСерий.Ссылка
	|		 И ЭкономияСтатусСерий.СтатусУказанияСерий <> 0
	|		 И ЭкономияСтатусСерий.СтатусУказанияСерийОтправитель = 0
	|		 И ЭкономияСтатусСерий.СтатусУказанияСерийПолучатель = 0
	|
	|		// Обработчик заполнения реквизита ПартияПроизводства
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|		ПО СпрПартииПроизводства.Документ = ОбъектыДляОбработки.Ссылка.Распоряжение
	|		 И СпрПартииПроизводства.Код = ОбъектыДляОбработки.Ссылка.НомерПартииЗапуска
	...
	|	ОбъектыДляОбработки.Ссылка.ВариантПриемкиТоваров,
	|	ОбъектыДляОбработки.Ссылка.ЗаказПереработчику.ВариантПриемкиТоваров,
	|	ОбъектыДляОбработки.Ссылка.ЗаказПереработчику,
	...
	|;
	|
	// установка признака УдалитьЯвляетсяНазначениемПродукции
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК Реквизиты
	|			ПО ОбъектыДляОбработки.Ссылка = Реквизиты.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ПобочныеИзделия КАК Таблица
	|			ПО Таблица.Ссылка = Реквизиты.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|			ПО Таблица.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
	|	ГДЕ
	|		НЕ Реквизиты.ЯвляетсяНазначениемПродукции
	|		И Аналитика.Назначение = Реквизиты.Назначение
	|		И Аналитика.Склад Ссылка Справочник.СтруктураПредприятия) КАК ДД
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка
	|;
	...
	|ИЗ
	|	ВТОбъектыДляОбработки КАК ТаблицаЭтапов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ВыходныеИзделия КАК ТабличнаяЧасть
	|		ПО ТаблицаЭтапов.Ссылка = ТабличнаяЧасть.Ссылка
	|			И (ТабличнаяЧасть.СтатьяРасходов = НЕОПРЕДЕЛЕНО)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	ВыборкаСтрокДляОчисткиВидовЗапасов = РезультатЗапроса[1].Выбрать();
	ВыборкаЯвляетсяНазначениемПродукции = РезультатЗапроса[2].Выбрать();
	
	ТаблицаНастроекСтрок = РезультатЗапроса[3].Выгрузить();
	ТаблицаНастроекСтрок.Индексы.Добавить("Ссылка");
	
	ОтборСсылки = Новый Структура("Ссылка");
	
	КэшированныеЗначения = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	СтруктураДействийПересчитатьКоличествоУпаковок = Новый Структура("ПересчитатьКоличествоУпаковок");
	
	ЗапросСинхронностиЭтаповПоЗаказуПереработчику = Новый Запрос();
	ЗапросСинхронностиЭтаповПоЗаказуПереработчику.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Этапы.Ссылка КАК Поле
		|ИЗ
		|	Документ.ЭтапПроизводства2_2 КАК Этапы
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК Обеспечение
		|		ПО Обеспечение.Ссылка = Этапы.Ссылка
		|		 И Этапы.НеОтгружатьЧастями
		|		 И Обеспечение.ДатаОтгрузки <> Этапы.ДатаОтгрузки
		|		 И Обеспечение.ВариантОбеспечения В(
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно))
		|ГДЕ
		|	Этапы.ЗаказПереработчику = &ЗаказПереработчику
		|		И НЕ Обеспечение.Ссылка ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	Этапы.ЗаказПереработчику = &ЗаказПереработчику
		|ИМЕЮЩИЕ
		|	МИНИМУМ(Этапы.НеОтгружатьЧастями) <> МАКСИМУМ(Этапы.НеОтгружатьЧастями)";
	ЗапросТребуетсяРазборка = Новый Запрос(
		"ВЫБРАТЬ
		|	Таблица.ЭтапПотребитель КАК Ссылка,
		|	Таблица.Номенклатура КАК Номенклатура,
		|	Таблица.Характеристика КАК Характеристика,
		...
		|		Таблица.ОбработатьПоСпецификации
		|		И Таблица.Ссылка.Проведен
		|		И Таблица.ЭтапПотребитель = (&Ссылка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		Таблица.ОбработатьПоСпецификации
		|		И Таблица.Ссылка.Проведен
		|		И Таблица.ЭтапПотребитель В (&Ссылка)) КАК Таблица
		|
		|СГРУППИРОВАТЬ ПО
		|	Таблица.ЭтапПотребитель,
		|	Таблица.Характеристика,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВтЗапланированаРазборка.Ссылка КАК Ссылка,
		|	ВтЗапланированаРазборка.Номенклатура КАК Номенклатура,
		|	ВтЗапланированаРазборка.Характеристика КАК Характеристика,
		|	ВтТребуетсяРазборка.Количество - ВтЗапланированаРазборка.КоличествоПроизвести КАК Количество
		|ИЗ
		|	ВтЗапланированаРазборка КАК ВтЗапланированаРазборка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТребуетсяРазборка КАК ВтТребуетсяРазборка
		|		ПО ВтЗапланированаРазборка.Ссылка = ВтТребуетсяРазборка.Ссылка
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ВыборкаДокументов.ЗаказНаПроизводство);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			
			Если ЗначениеЗаполнено(ВыборкаДокументов.ЗаказПереработчику) Тогда // блокировка для чтения заказа переработчику и всех его этапов.
				
				ЭлементБлокировки = Блокировка.Добавить(ПолноеИмяОбъекта);
				ЭлементБлокировки.УстановитьЗначение("ЗаказПереработчику", ВыборкаДокументов.ЗаказПереработчику);
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
				
				ЭлементБлокировки = Блокировка.Добавить("Документ.ЗаказПереработчику");
				ЭлементБлокировки.УстановитьЗначение("Ссылка", ВыборкаДокументов.ЗаказПереработчику);
				ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			...
			ЗаполнитьКодыСтрокОбеспечения = Ложь;
			
			// Обработчик заполнения нового реквизита ""ПорядокРаботыВидовРабочихЦентров"" в документе.
			Если ДокументОбъект.ПланироватьРаботуВидовРабочихЦентров
				И ДокументОбъект.ВидыРабочихЦентров.Количество() > 1
				И ДокументОбъект.ПорядокРаботыВидовРабочихЦентров.Пустая() Тогда
				
				ДокументОбъект.ПорядокРаботыВидовРабочихЦентров = Перечисления.ПорядокРаботыВидовРабочихЦентров.Независимо;
				
				УправлениеДаннымиОбИзделиях.ЗаполнитьЕдиницуИзмеренияИДлительностьЭтапаББВ(ДокументОбъект);
			...
			// Обработчик заполнения нового реквизита ""Валюта"" в документе.
			Если Не ЗначениеЗаполнено(ДокументОбъект.Валюта) И ДокументОбъект.Валюта <> ВалютаУправленческогоУчета Тогда
				
				ДокументОбъект.Валюта = ВалютаУправленческогоУчета;
				ОбъектИзменен = Истина;
				
			КонецЕсли;
			
			// Обработчик заполнения нового реквизита ""Тип производственного процесса"" в документе.
			Если Не ЗначениеЗаполнено(ДокументОбъект.ТипПроизводственногоПроцесса) Тогда
				
				ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Сборка;
				ОбъектИзменен = Истина;
				
			КонецЕсли;
			
			// Обработчик заполнения нового реквизита ""Способ распределения затрат на выходные изделия"" в документе.
			Если Не ЗначениеЗаполнено(ДокументОбъект.СпособРаспределенияЗатратНаВыходныеИзделия)
				И ДокументОбъект.НомерСледующегоЭтапа = 0 Тогда
			...
			ДокументОбъект.ЗаполнитьПоследователей();
			...
			// Обработчик заполнения нового реквизита ""НомерНаПечать"" в документе.
			Если Не ЗначениеЗаполнено(ДокументОбъект.НомерНаПечать) Тогда
				
				ДокументОбъект.НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДокументОбъект.Номер);
				ОбъектИзменен = Истина;
				
			КонецЕсли;
			
			ОтборСсылки.Ссылка = ВыборкаДокументов.Ссылка;
			
			Если ВыборкаДокументов.НужноОчищатьВидЗапасов Тогда
				Пока ВыборкаСтрокДляОчисткиВидовЗапасов.НайтиСледующий(ОтборСсылки) Цикл
					
					ДокументОбъект.ВыходныеИзделия[ВыборкаСтрокДляОчисткиВидовЗапасов.НомерСтроки - 1].ВидЗапасов = Справочники.ВидыЗапасов.ПустаяСсылка();	
					ОбъектИзменен = Истина;
			...
			// установка признака УдалитьЯвляетсяНазначениемПродукции
			Если ВыборкаЯвляетсяНазначениемПродукции.НайтиСледующий(ОтборСсылки) Тогда
				ДокументОбъект.ЯвляетсяНазначениемПродукции = Истина;
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			// Проверка, что все еще нужно снять флаг "Отгружать одной датой".
			Если ДокументОбъект.НеОтгружатьЧастями Тогда
				
				НужноСнятьФлагНеОтгружатьЧастями = Ложь;
				
				Если ДокументОбъект.ЗаказПереработчику.Пустая() Тогда
					
					Если ВыборкаДокументов.РазличаютсяДатыОтгрузки Тогда
						
						ОтборСтрок = Новый Структура("ВариантОбеспечения", Перечисления.ВариантыОбеспечения.Отгрузить);
						СтрокиОтгрузить = ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Выгрузить(ОтборСтрок, "ДатаОтгрузки");
						СтрокиОтгрузить.Свернуть("ДатаОтгрузки");
						
						ОтборСтрок = Новый Структура("ВариантОбеспечения", Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно);
						СтрокиОтгрузитьОбособленно = ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Выгрузить(ОтборСтрок, "ДатаОтгрузки");
						СтрокиОтгрузитьОбособленно.Свернуть("ДатаОтгрузки");
						
						НужноСнятьФлагНеОтгружатьЧастями = СтрокиОтгрузить.Количество() > 1
							Или СтрокиОтгрузитьОбособленно.Количество() > 1
							Или СтрокиОтгрузить.Количество() = 1
								И СтрокиОтгрузить[0].ДатаОтгрузки <> ДокументОбъект.ДатаОтгрузки
							Или СтрокиОтгрузитьОбособленно.Количество() = 1
								И СтрокиОтгрузитьОбособленно[0].ДатаОтгрузки <> ДокументОбъект.ДатаОтгрузки;
						
					КонецЕсли;
					
				Иначе
					
					ЗапросСинхронностиЭтаповПоЗаказуПереработчику.УстановитьПараметр("ЗаказПереработчику", ВыборкаДокументов.ЗаказПереработчику);
					НужноСнятьФлагНеОтгружатьЧастями = Не ЗапросСинхронностиЭтаповПоЗаказуПереработчику.Выполнить().Пустой();
					
				КонецЕсли;
				
				Если НужноСнятьФлагНеОтгружатьЧастями Тогда
			...
			Если ВыборкаДокументов.ТребуетсяУказаниеВариантаПриемки Тогда
				
				Если ЗначениеЗаполнено(ВыборкаДокументов.ЗаказПереработчику) Тогда
					
					ДокументОбъект.ВариантПриемкиТоваров = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыборкаДокументов.ЗаказПереработчику,
			...
			// Заполнение статуса указания серий отправитель в ТЧ ОбеспечениеМатериаламиИРаботами
			Если ВыборкаДокументов.ЗаполнитьСтатусСерийОбеспечение Тогда
				
				Для каждого Строка Из ДокументОбъект.ОбеспечениеМатериаламиИРаботами Цикл
					
					Если Строка.СтатусУказанияСерий <> 0
						И Строка.СтатусУказанияСерийОтправитель = 0
						И Строка.СтатусУказанияСерийПолучатель = 0 Тогда
						
						Строка.СтатусУказанияСерийОтправитель = Строка.СтатусУказанияСерий;
						ОбъектИзменен = Истина;
			...
			// Заполнение статуса указания серий получатель в ТЧ ЭкономияМатериалов
			Если ВыборкаДокументов.ЗаполнитьСтатусСерийЭкономия Тогда
				
				Для каждого Строка Из ДокументОбъект.ЭкономияМатериалов Цикл
			...
			// Заполнение пустого реквизита ПланироватьНеРанее значением Неопределено
			Если ДокументОбъект.ПланироватьНеРанее = '00010101'
				ИЛИ ДокументОбъект.ПланироватьНеРанее = Документы.ЭтапПроизводства2_2.ПустаяСсылка() Тогда
			...
			// Корректировка реквизита НазначениеПродукция по схеме обособления
			Если ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка Тогда
				
				ДанныеЗаказа = УправлениеПроизводствомПовтИсп.ДанныеПроизводственногоЗаказа(
											ВыборкаДокументов.ЗаказНаПроизводство);
				
				СхемаОбособления = Документы.ЗаказНаПроизводство2_2.СхемаОбособления(
											ДанныеЗаказа.Ссылка,
											ДанныеЗаказа.НазначениеПродукция);
				
				Если ДокументОбъект.НазначениеПродукция <> СхемаОбособления.НазначениеПродукция Тогда
					ДокументОбъект.НазначениеПродукция = СхемаОбособления.НазначениеПродукция;
					ОбъектИзменен = Истина;
				КонецЕсли;
			...
			// Очистка даты фактического окончания у документов в статусе Начат
			Если ДокументОбъект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.Начат
				И ДокументОбъект.ФактическоеОкончаниеЭтапа <> '00010101' Тогда
				
				ДокументОбъект.ФактическоеОкончаниеЭтапа = '00010101';
				ОбъектИзменен = Истина;
				
			КонецЕсли;
			
			// Заполнение нового реквизита НазначениеПолуфабрикаты и реквизита Назначение в ТЧ ОбеспечениеМатериаламиИРаботами
			ОбновленоНазначение = Документы.ЭтапПроизводства2_2.ПроверитьЗаполнитьНазначения(ДокументОбъект);
			ОбъектИзменен = (ОбъектИзменен ИЛИ ОбновленоНазначение);
			
			Если ДокументОбъект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка Тогда
			
				ЗапросТребуетсяРазборка.УстановитьПараметр("Ссылка", ДокументОбъект.Ссылка);
				ТребуетсяРазборка = ЗапросТребуетсяРазборка.Выполнить().Выгрузить();
				
				Для каждого СтрокаТребуется Из ТребуетсяРазборка Цикл
					
					ОтборТребуетсяРазборка = Новый Структура("Номенклатура,Характеристика");
					ЗаполнитьЗначенияСвойств(ОтборТребуетсяРазборка, СтрокаТребуется);
					
					НайденныеСтроки = ДокументОбъект.ОбеспечениеМатериаламиИРаботами.НайтиСтроки(ОтборТребуетсяРазборка);
					Для каждого СтрокаОбеспечение Из НайденныеСтроки Цикл
						
						Если СтрокаОбеспечение.Производится
							ИЛИ НЕ (СтрокаОбеспечение.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно
									ИЛИ СтрокаОбеспечение.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно) Тогда
							Продолжить;
						КонецЕсли;
						
						Если СтрокаОбеспечение.Количество > СтрокаТребуется.Количество Тогда
							
							НоваяСтрока = ДокументОбъект.ОбеспечениеМатериаламиИРаботами.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОбеспечение,, "КодСтроки,Количество,Производится,Спецификация");
							НоваяСтрока.Производится = Истина;
							
							НоваяСтрока.Количество = СтрокаТребуется.Количество;
							СтрокаОбеспечение.Количество = СтрокаОбеспечение.Количество - СтрокаТребуется.Количество;
							
							ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(НоваяСтрока,       СтруктураДействийПересчитатьКоличествоУпаковок, КэшированныеЗначения);
							ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаОбеспечение, СтруктураДействийПересчитатьКоличествоУпаковок, КэшированныеЗначения);
							
							ОбъектИзменен = Истина;
							
							ЗаполнитьКодыСтрокОбеспечения = Истина;
							СтрокаТребуется.Количество = 0;
							
						Иначе
							
							СтрокаОбеспечение.Производится = Истина;
							ОбъектИзменен = Истина;
							
							СтрокаТребуется.Количество = СтрокаТребуется.Количество - СтрокаОбеспечение.Количество;
							
						КонецЕсли;
						
						Если СтрокаТребуется.Количество > 0 Тогда
			...
			Если ЗаполнитьКодыСтрокОбеспечения Тогда
				ЗаказыСервер.УстановитьКлючВСтрокахТабличнойЧасти(ДокументОбъект, "ОбеспечениеМатериаламиИРаботами", "МаксимальныйКодСтрокиОбеспечение");
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			// Заполнение нового реквизита ПартияПроизводства
			Если Не ЗначениеЗаполнено(ДокументОбъект.ПартияПроизводства) Тогда
				ОбъектИзменен = Истина;
				ДокументОбъект.ПартияПроизводства = ВыборкаДокументов.ПартияПроизводства;
			КонецЕсли;
			
			// Очистка секунд в дате начала этапа
			Если ДокументОбъект.ФактическоеНачалоЭтапа <> НачалоМинуты(ДокументОбъект.ФактическоеНачалоЭтапа) Тогда
				ДокументОбъект.ФактическоеНачалоЭтапа = НачалоМинуты(ДокументОбъект.ФактическоеНачалоЭтапа);
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			// Очистка секунд в дате окончания этапа
			Если ДокументОбъект.ФактическоеОкончаниеЭтапа <> НачалоМинуты(ДокументОбъект.ФактическоеОкончаниеЭтапа) Тогда
				ДокументОбъект.ФактическоеОкончаниеЭтапа = НачалоМинуты(ДокументОбъект.ФактическоеОкончаниеЭтапа);
				ОбъектИзменен = Истина;
			КонецЕсли;
			
			// Заполнение реквизита статья расходов дефолтным значением статьи расходов
			НастройкиСтрок = ТаблицаНастроекСтрок.НайтиСтроки(Новый Структура("Ссылка", ВыборкаДокументов.Ссылка));
			Если НастройкиСтрок.Количество() > 0 Тогда
				
				Для каждого НастройкаСтроки Из НастройкиСтрок Цикл
					
					СтрокаТЧ = ДокументОбъект[НастройкаСтроки.ИмяТЧ][НастройкаСтроки.ИндексСтроки];
					СтрокаТЧ.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.ПустаяСсылка();
			...
			// Заполнение нового реквизита ДатаРеестраДокументов
			Если Не ЗначениеЗаполнено(ДокументОбъект.ДатаРеестраДокументов) Тогда
				ОбъектИзменен = Истина;
				ОсновнаяЗапись = Документы.ЭтапПроизводства2_2.ОсновнаяЗаписьРеестраДокументов(ДокументОбъект);
				ЗаполнитьЗначенияСвойств(ДокументОбъект, ОсновнаяЗапись);
			КонецЕсли;
...
Функция ДанныеОбновленыНаНовуюВерсиюПрограммы(МетаданныеИОтбор) Экспорт
	...
	Если Не МетаданныеИОтбор.ЭтоНовый Тогда
		
		Заказы = Новый Массив;
		
		Если ТипЗнч(МетаданныеИОтбор.Отбор) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			
			Заказы.Добавить(МетаданныеИОтбор.Отбор);
			
		ИначеЕсли ТипЗнч(МетаданныеИОтбор.Отбор) = Тип("Массив") И МетаданныеИОтбор.Отбор.Количество() > 0 Тогда
			
			Заказы = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(МетаданныеИОтбор.Отбор);
		...
		Запрос.Текст =
			...
			|	Документ.ЭтапПроизводства2_2 КАК Этапы
			|ГДЕ
			|	Этапы.ЗаказПереработчику В(
			|		ВЫБРАТЬ РАЗЛИЧНЫЕ
			|			Этапы.ЗаказПереработчику КАК ЗаказПереработчику
			|		ИЗ
			|			Документ.ЭтапПроизводства2_2 КАК Этапы
			|		ГДЕ
			|			Этапы.Ссылка В(&Ссылки)
			|				И Этапы.ЗаказПереработчику <> ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка))
			|;
			|
			|////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	Этапы.ЗаказПереработчику КАК Ссылка
			|ИЗ
			|	Документ.ЭтапПроизводства2_2 КАК Этапы
			|ГДЕ
			|	Этапы.Ссылка В(&Ссылки)
		...
		Этапы  = Пакет[0].Выгрузить().ВыгрузитьКолонку("Ссылка");
		Заказы = Пакет[1].Выгрузить().ВыгрузитьКолонку("Ссылка");
		
		ДанныеСвязанныхОбъектовОбновлены = Не
			(ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Неопределено, "Документ.ЭтапПроизводства2_2", Этапы)
				Или ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Неопределено, "Документ.ЗаказПереработчику", Заказы));
