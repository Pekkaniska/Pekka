...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ИспытательныйСрокСотрудников") Тогда
		...
		Модуль.ПриСозданииДокументаУвольнения(ЭтотОбъект, Элементы.ГруппаСостояниеСотрудника);
	КонецЕсли;
...
Процедура ОснованиеУвольненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль.УточнитьПараметрыВыбораОснованияУвольненияСГосударственнойСлужбы(Элемент, Объект.Сотрудник, Объект.ДатаУвольнения);
	КонецЕсли;
...
Процедура МесяцНачалаРасчетовСтрокойПриИзменении(Элемент)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаПриИзменении(ЭтаФорма, "Объект.МесяцНачалаРасчетов", "МесяцНачалаРасчетовСтрокой", Модифицированность);
...
Процедура МесяцНачалаРасчетовСтрокойНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаНачалоВыбора(ЭтаФорма, ЭтаФорма, "Объект.МесяцНачалаРасчетов", "МесяцНачалаРасчетовСтрокой", , Оповещение);
...
Процедура МесяцНачалаРасчетовСтрокойРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаРегулирование(ЭтаФорма, "Объект.МесяцНачалаРасчетов", "МесяцНачалаРасчетовСтрокой", Направление, Модифицированность);
...
Процедура ЗавершитьИнициализациюФормы(НовыйДокумент)
	...
	ЗарплатаКадрыКлиентСервер.ЗаполнитьМесяцПоДате(ЭтаФорма, "Объект.МесяцНачалаРасчетов", "МесяцНачалаРасчетовСтрокой");
...
Процедура ЗаполнитьВидРасчетаКомпенсацииУдержанияОтпуска(ПервичнаяИнициализацияФормы)
	...
	ИначеЕсли ИспользоватьУдержания(Объект.ПризнакКомпенсацииУдержанияОтпуска) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ВидРасчетаКомпенсацииУдержанияОтпуска) Или (Не ПервичнаяИнициализацияФормы И ПараметрыВыбораИзменились) Тогда
			...
			ДополнительныйОтбор = Новый Структура("ВидОтпуска", ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.Основной"));
...
Процедура ЗаполнитьНачисления(ПериодЗаполнения, ИсключаемыеРегистраторы, МенеджерРасчета, ТаблицаНачислений)
	...
	Если Объект.НачислитьЗарплату Тогда
		...
		Если Объект.Проведен И Объект.СотрудникиПерерасчет.Количество() = 0 Тогда
			ПерерасчетЗарплаты.ЗаполнитьСотрудниковПерерасчетаДокумента(Объект);
		КонецЕсли;
...
Процедура ОбновитьДанныеПоСотруднику()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ИспытательныйСрокСотрудников") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ИспытательныйСрокСотрудников");
		Модуль.СостояниеСотрудникаПриИзменении(ЭтотОбъект);
	КонецЕсли;
...
Функция ПериодЗаполненияДокумента()
	...
	Если Объект.НачислитьЗарплату Тогда
		ПериодЗаполнения.ДатаНачала = Мин(Объект.МесяцНачалаРасчетов, НачалоМесяца(Объект.ДатаУвольнения));
	Иначе
		ПериодЗаполнения.ДатаНачала = НачалоМесяца(Объект.ДатаУвольнения);
	КонецЕсли;
	
	ПериодЗаполнения.ДатаОкончания = Мин(КонецДня(Объект.ДатаУвольнения), КонецМесяца(Объект.ПериодРегистрации));
