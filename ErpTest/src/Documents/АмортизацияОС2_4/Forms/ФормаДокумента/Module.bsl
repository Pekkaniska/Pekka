...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("УправлениеПредприятием") 
		...
		//-- НЕ УТКА
		Тогда
		Элементы.ФормаПроводкиМеждУчета.Видимость = Ложь;
	КонецЕсли; 
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ ЗавершениеРаботы Тогда
		...
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Отменить расчет и закрыть форму'"));
...
Процедура ПроводкиРеглУчета(Команда)
	...
	Если Модифицированность Тогда
		ЗаписатьИВыполнитьДействие(
			НСтр("ru = 'Проводки регламентированного учета'"), 
			"ОткрытьПроводкиРегламентированногоУчета");
	Иначе
		ОткрытьПроводкиРегламентированногоУчета();
	КонецЕсли; 
...
Процедура ПроводкиМеждУчета(Команда)
	...
	Если Модифицированность Тогда
		ЗаписатьИВыполнитьДействие(
			НСтр("ru = 'Проводки международного учета'"),
			"ОткрытьПроводкиМеждународногоУчета");
	Иначе
		//++ НЕ УТКА
		ОткрытьПроводкиМеждународногоУчета();
		//-- НЕ УТКА
...
Процедура ПослеРасчетаАмортизации()
	...
	ВыполненоСОшибками = ПослеРасчетаАмортизацииНаСервере();

	Оповестить("Запись_АмортизацияОС2_4");
	ОповеститьОбИзменении(Объект.Ссылка);
	ВнеоборотныеАктивыКлиент.ОповеститьОРасчетеАмортизации(ВыполненоСОшибками, Объект.Дата);
	
	Если НЕ ВыполненоСОшибками Тогда
...
Процедура ВыполнитьДействиеПослеРасчета()
	...
	Если ДействиеПослеРасчета = "ЗакрытьФорму" Тогда
		Закрыть();
	ИначеЕсли ДействиеПослеРасчета = "ОткрытьПроводкиРегламентированногоУчета" Тогда
		ОткрытьПроводкиРегламентированногоУчета();
	//++ НЕ УТКА
...
&НаСервере
Функция ПослеРасчетаАмортизацииНаСервере()

	ВыполняетсяРасчет = Ложь;
	
	ВыполненоСОшибками = ПолучитьИзВременногоХранилища(АдресХранилища) <> Ложь;
...
&НаКлиенте
Процедура ОткрытьПроводкиРегламентированногоУчета()
...
&НаКлиенте
Процедура ОткрытьПроводкиМеждународногоУчета()

	ПараметрыОтбора = Новый Структура("Регистратор", Объект.Ссылка);
	ПараметрыФормы = Новый Структура("Отбор", ПараметрыОтбора);
	ОткрытьФорму("РегистрБухгалтерии.Международный.Форма.ПроводкиМеждународногоУчета", ПараметрыФормы,, Объект.Ссылка);
