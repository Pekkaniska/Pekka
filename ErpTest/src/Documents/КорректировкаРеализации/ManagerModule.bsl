...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Запрос.Параметры.ПериодПродажи) Тогда
		Если Запрос.Параметры.ПериодПродажи < НачалоМесяца(Реквизиты.Период) Тогда
...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(Запрос, ТекстыЗапроса)
	...
	|	(ВЫБОР
	|		КОГДА ДанныеДокумента.ДокументОснование.ВернутьМногооборотнуюТару
	|		 И АналитикаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ДанныеДокумента.ДокументОснование.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиентуРеглУчет)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ДанныеДокумента.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	|			ТОГДА 
...
Функция ТекстЗапросаВтТаблицаВидыЗапасовОприходование(Запрос, ТекстыЗапроса)
	...
	|			КОНЕЦ
	|		ИНАЧЕ
	|			ВЫРАЗИТЬ((ВидыЗапасов.СуммаНДС * &КоэффициентПересчетаВВалютуУпр) КАК ЧИСЛО(31,2))
	|	КОНЕЦ КАК НДСУпр,
	|
	|	ВидыЗапасов.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
	|	ВЫБОР КОГДА ВидыЗапасов.СуммаСНДС <> 0 ТОГДА
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|ВЫБРАТЬ
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Дата КАК Период,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка КАК Ссылка,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.ВидКорректировки КАК ХозяйственнаяОперация,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Контрагент КАК Контрагент,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Договор КАК Договор,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Грузоотправитель КАК Грузоотправитель,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Грузополучатель КАК Грузополучатель,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Организация КАК Организация,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.Подразделение КАК Подразделение,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовКорректировкаВыручки.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)
	|			ТОГДА ВидыЗапасовКорректировкаВыручки.Ссылка
	|		ИНАЧЕ ВидыЗапасовКорректировкаВыручки.Ссылка.ДокументОснование
	|	КОНЕЦ КАК ДокументРеализации,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовКорректировкаВыручки.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ВидыЗапасовКорректировкаВыручки.Ссылка
	|	КОНЕЦ КАК ДокументКорректировкиРеализации,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовКорректировкаВыручки.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ИсправлениеОшибок,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовКорректировкаВыручки.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК КорректировкаПоСогласованиюСторон,
	|	ЛОЖЬ КАК РеализацияВРозницу,
	|	ВидыЗапасовКорректировкаВыручки.Ссылка.НалогообложениеНДС КАК НалогообложениеНДС,
	...
	|	ВидыЗапасовКорректировкаВыручки.СтавкаНДС КАК СтавкаНДС,
	|	ВидыЗапасовКорректировкаВыручки.КодТНВЭД КАК КодТНВЭД,
	|	ВидыЗапасовКорректировкаВыручки.ВидЗапасов КАК ВидЗапасов,
	|	ВидыЗапасовКорректировкаВыручки.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ИЗ
	|	Документ.КорректировкаРеализации.ВидыЗапасовКорректировкаВыручки КАК ВидыЗапасовКорректировкаВыручки
	|ГДЕ
	|	ВидыЗапасовКорректировкаВыручки.Ссылка В (&Ссылка)
	|	И ВидыЗапасовКорректировкаВыручки.Ссылка.ВидКорректировки В (
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|	И (ВЫБОР
	|		КОГДА ВидыЗапасовКорректировкаВыручки.Ссылка.ДокументОснование.ВернутьМногооборотнуюТару
	|			ТОГДА ВидыЗапасовКорректировкаВыручки.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ИНАЧЕ
	|			ИСТИНА
	|		КОНЕЦ)
	|
	...
	|
	|ВЫБРАТЬ
	|	ВидыЗапасовОприходование.Ссылка.Дата КАК Период,
	|	ВидыЗапасовОприходование.Ссылка КАК Ссылка,
	|	ВидыЗапасовОприходование.Ссылка.ВидКорректировки КАК ХозяйственнаяОперация,
	|	ВидыЗапасовОприходование.Ссылка.Контрагент КАК Контрагент,
	|	ВидыЗапасовОприходование.Ссылка.Договор КАК Договор,
	|	ВидыЗапасовОприходование.Ссылка.Грузоотправитель КАК Грузоотправитель,
	|	ВидыЗапасовОприходование.Ссылка.Грузополучатель КАК Грузополучатель,
	|	ВидыЗапасовОприходование.Ссылка.Организация КАК Организация,
	|	ВидыЗапасовОприходование.Ссылка.Подразделение КАК Подразделение,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовОприходование.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)
	|			ТОГДА ВидыЗапасовОприходование.Ссылка
	|		ИНАЧЕ ВидыЗапасовОприходование.Ссылка.ДокументОснование
	|	КОНЕЦ КАК ДокументРеализации,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовОприходование.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ВидыЗапасовОприходование.Ссылка
	|	КОНЕЦ КАК ДокументКорректировкиРеализации,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовОприходование.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ИсправлениеОшибок,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовОприходование.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК КорректировкаПоСогласованиюСторон,
	|	ЛОЖЬ КАК РеализацияВРозницу,
	|	ВидыЗапасовОприходование.Ссылка.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ВидыЗапасовОприходование.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
	|	ВидыЗапасовОприходование.АналитикаУчетаНоменклатуры.Характеристика КАК Характеристика,
	|	ВидыЗапасовОприходование.СтавкаНДС КАК СтавкаНДС,
	|	ВидыЗапасовОприходование.КодТНВЭД КАК КодТНВЭД,
	...
	|ГДЕ
	|	ВидыЗапасовОприходование.Ссылка В (&Ссылка)
	|	И ВидыЗапасовОприходование.Ссылка.ВидКорректировки В (
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|	И (ВЫБОР
	|		КОГДА ВидыЗапасовОприходование.Ссылка.ДокументОснование.ВернутьМногооборотнуюТару
	|			ТОГДА ВидыЗапасовОприходование.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ИНАЧЕ
	|			ИСТИНА
	|		КОНЕЦ)
	|
	...
	|
	|ВЫБРАТЬ
	|	ВидыЗапасовСписание.Ссылка.Дата КАК Период,
	|	ВидыЗапасовСписание.Ссылка КАК Ссылка,
	|	ВидыЗапасовСписание.Ссылка.ВидКорректировки КАК ХозяйственнаяОперация,
	|	ВидыЗапасовСписание.Ссылка.Контрагент КАК Контрагент,
	|	ВидыЗапасовСписание.Ссылка.Договор КАК Договор,
	|	ВидыЗапасовСписание.Ссылка.Грузоотправитель КАК Грузоотправитель,
	|	ВидыЗапасовСписание.Ссылка.Грузополучатель КАК Грузополучатель,
	|	ВидыЗапасовСписание.Ссылка.Организация КАК Организация,
	|	ВидыЗапасовСписание.Ссылка.Подразделение КАК Подразделение,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСписание.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)
	|			ТОГДА ВидыЗапасовСписание.Ссылка
	|		ИНАЧЕ ВидыЗапасовСписание.Ссылка.ДокументОснование
	|	КОНЕЦ КАК ДокументРеализации,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСписание.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ВидыЗапасовСписание.Ссылка
	|	КОНЕЦ КАК ДокументКорректировкиРеализации,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСписание.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ИсправлениеОшибок,
	|	ВЫБОР
	|		КОГДА ВидыЗапасовСписание.Ссылка.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК КорректировкаПоСогласованиюСторон,
	|	ЛОЖЬ КАК РеализацияВРозницу,
	|	ВидыЗапасовСписание.Ссылка.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ВидыЗапасовСписание.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
	|	ВидыЗапасовСписание.АналитикаУчетаНоменклатуры.Характеристика КАК Характеристика,
	|	ВидыЗапасовСписание.СтавкаНДС КАК СтавкаНДС,
	|	ВидыЗапасовСписание.КодТНВЭД КАК КодТНВЭД,
	|	ВидыЗапасовСписание.ВидЗапасов КАК ВидЗапасов,
	|	ВидыЗапасовСписание.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ИЗ
	|	Документ.КорректировкаРеализации.ВидыЗапасовСписание КАК ВидыЗапасовСписание
	|ГДЕ
	|	ВидыЗапасовСписание.Ссылка В (&Ссылка)
	|	И ВидыЗапасовСписание.Ссылка.ВидКорректировки В (
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок),
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон))
	|	И (ВЫБОР
	|		КОГДА ВидыЗапасовСписание.Ссылка.ДокументОснование.ВернутьМногооборотнуюТару
	|			ТОГДА ВидыЗапасовСписание.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|		ИНАЧЕ
	|			ИСТИНА
	|		КОНЕЦ)
	|";
...
Функция ТекстЗапросаПоИсходнымДаннымАктВыполненныхРабот()
	...
	|	ЕСТЬNULL(ТоварыЗаказа.Ссылка.Назначение, ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК Назначение,
...
Функция ТекстЗапросаПоИсходнымДаннымРеализацияУслугПрочихАктивов()
	...
	|	ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)         КАК КодТНВЭД,
	|
	|	ВЫРАЗИТЬ(ТаблицаДоходы.Сумма / ТаблицаДоходы.Количество КАК ЧИСЛО(31,2)) КАК Цена,
	|
