...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ИсходящаяТранспортнаяОперацияВЕТИС") Тогда
		...
		ИнтеграцияВЕТИСУТ.ЗаполнитьВозвратТоваровМеждуОрганизациямиНаОснованииИсходящейТранспортнойОперацииВЕТИС(ЭтотОбъект, ДанныеЗаполнения,, СтандартнаяОбработка);
...
Процедура ЗаполнитьВидЗапасовПолучателя()
	Для Каждого Запас Из ВидыЗапасов Цикл
		Если ЗначениеЗаполнено(Запас.ВидЗапасовПолучателя) И Не Запас.ПоТоварамКОформлению Тогда
	...
	Пока ИндексОстатка < СтрокОстатков Цикл
		...
		Пока ИндексЗапаса < СтрокЗапасов Цикл
			...
			ИначеЕсли Распределить > 0. И Распределить < Запас.Количество Тогда
				...
				НовыйЗапас.СуммаНДС =
					ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(НовыйЗапас.СуммаСНДС, Запас.СтавкаНДС, ЦенаВключаетНДС);
...
Функция ОстаткиПереданныхТоваров()
	...
	Запрос.УстановитьПараметр("ДатаОстатков", КонецМесяца(Дата));
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("ОрганизацияПолучатель", ОрганизацияПолучатель);
...
#Область ТекстЗапроса
	...
	|		 ИЛИ СпрВидыЗапасов.Организация <> &Организация
