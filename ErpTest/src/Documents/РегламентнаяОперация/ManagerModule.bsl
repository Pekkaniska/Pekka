...
Функция РассчитатьРегламентныеОперации(Период, МассивОпераций, Организация = Неопределено, Отказ = Ложь, 
	...
	Для Каждого Строка Из ТаблицаОпераций Цикл
		...
		Если ПроводитьДокументы Тогда
			...
			СообщенияПроверки = УниверсальныеМеханизмыПартийИСебестоимости.ПроверитьЗаполнениеОбъектаСПерехватомСообщений(РеглОперация);
			
			Если НЕ ЗначениеЗаполнено(СообщенияПроверки) Тогда
				Попытка
				...
				Исключение
					...
					Если РеглОперация.ДополнительныеСвойства.Свойство("СписокОшибок") Тогда
						ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РезультатРасчета.СписокОшибок, РеглОперация.ДополнительныеСвойства.СписокОшибок);
					КонецЕсли; 
...
Функция ПодготовитьДанныеЗакрытияГода(СтруктураШапки, Отказ) Экспорт
	...
	//++ RARUS_IUC 20180119 
	Запрос.УстановитьПараметр("пкГоловнаяОрганизация",   СтруктураШапки.Организация.ГоловнаяОрганизация);
	//-- RARUS_IUC 20180119                
...
Процедура ЗаполнитьПроводку(Проводка, Выборка, КурсоваяРазница, ПараметрыСтатьиДДС = Неопределено)
	...
	Если КорСчет.ВидыСубконто.Найти(КорВидСубконто, "ВидСубконто") <> Неопределено Тогда
...
Процедура РасчетКурсовыхРазницФО(ПараметрыРасчета, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДебетКредит = ?(Проводка.СчетДт = ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиНеЕНВД, "Дт", "Кт");
...
Функция ТекстЗапросаРасчетКурсовыхРазницФО()
	...
	|		НЕ Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрибылиИУбытки)) И НЕ Счет.Забалансовый,
	|		,
	|		Организация = &Организация) КАК Остатки
	|ГДЕ
...
Функция ТекстЗапросаТаблицаОС(НомераТаблиц)
	...
	|			И КОНЕЦПЕРИОДА(ДокВводОстатков24.Дата, МЕСЯЦ) < &КонДата)
