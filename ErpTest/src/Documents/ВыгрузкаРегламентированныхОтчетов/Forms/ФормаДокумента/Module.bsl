	...
	Иначе
		...
		Если НЕ ФайлВыгрузкиЗаписан Тогда
			...
			КодировкаФайлаВыгрузки = ?(Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсия400
				...
				ИЛИ Объект.Выгрузки.Получить(0).Тип = ВерсииФорматовВыгрузки.ПеречислениеВерсииФорматовВыгрузкиВерсияМинобороны, "UTF-8", "cp866"));
...
Процедура СформироватьТекстыВыгрузки(Команда)
	...
	СтруктДопПараметров = Новый Структура("УдалитьВыгрузкуПояснительнойЗапискиКДекларацииПоЕНВД");
