...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ОсновноеСредство) Тогда
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ОсновноеСредство, "ГруппаОС,АмортизационнаяГруппа,НаправлениеДеятельности");
		Если ЗначенияРеквизитов <> Неопределено Тогда
			ГруппаОС = ЗначенияРеквизитов.ГруппаОС;
			АмортизационнаяГруппа = ЗначенияРеквизитов.АмортизационнаяГруппа;
...
Процедура ИнициализацияФормыПриСозданииНаСервере()
	...
	Элементы.ОССтоимостьБУ1.Заголовок = СтрШаблон(НСтр("ru = 'Стоимость регл. (%1)'"), Строка(ВалютаРегл));
	Элементы.ОССтоимостьУУ.Заголовок  = СтрШаблон(НСтр("ru = 'Стоимость упр. (%1)'"), Строка(ВалютаУпр));
...
Процедура ЗаполнитьПараметрыВыбораОС()
	...
	ПараметрВыбора = Новый СвязьПараметраВыбора("Отбор.ГруппаОС", "ГруппаОС");
	СвязиПараметровВыбораОС.Добавить(ПараметрВыбора);
	
	ПараметрВыбора = Новый СвязьПараметраВыбора("Отбор.АмортизационнаяГруппа", "АмортизационнаяГруппа");
	СвязиПараметровВыбораОС.Добавить(ПараметрВыбора);
	
	ПараметрВыбора = Новый СвязьПараметраВыбора("Отбор.НаправлениеДеятельности", "НаправлениеДеятельности");
	СвязиПараметровВыбораОС.Добавить(ПараметрВыбора);
...
Процедура НастроитьЗависимыеЭлементыФормыНаКлиентеНаСервере(Форма, ИзмененныеРеквизиты)
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ХозяйственнаяОперация")
		...
		Элементы.ОССтоимостьБУ.Видимость = Объект.ОтражатьВРеглУчете И Форма.ПлательщикНалогаНаПрибыль;
		Элементы.ОССтоимостьБУ1.Видимость = Объект.ОтражатьВРеглУчете И НЕ Форма.ПлательщикНалогаНаПрибыль;
...
Процедура ПроверитьВыборОсновногоСредства()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ОсновноеСредство) Тогда
		ГруппаОС = ПредопределенноеЗначение("Перечисление.ГруппыОС.ПустаяСсылка");
	...
	Если РезультатПроверки.ПроблемныеОС.Количество() <> 0 Тогда
		...
		Если РезультатПроверки.ОтличаетсяНаправлениеДеятельности Тогда
			ТекстВопроса = НСтр("ru = 'Из табличной части будут удалены основные средства,
                                 ...
                                 |Продолжить?'");
		ИначеЕсли РезультатПроверки.ОтличаетсяГруппаОС
			ИЛИ РезультатПроверки.ОтличаетсяАмортизационнаяГруппа Тогда
	...
	Иначе
		...
		ГруппаОС = РезультатПроверки.ГруппаОС;
		АмортизационнаяГруппа = РезультатПроверки.АмортизационнаяГруппа;
		НаправлениеДеятельности = РезультатПроверки.НаправлениеДеятельности;
