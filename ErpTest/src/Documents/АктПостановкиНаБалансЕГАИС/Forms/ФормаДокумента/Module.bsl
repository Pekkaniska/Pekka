...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияЕГАИСПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(ЭтотОбъект);
	...
	ГосударственныеИнформационныеСистемыПереопределяемый.УстановитьСвязиПараметровВыбораСХарактеристикой(ЭтотОбъект, "ТоварыСерия");
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИнтеграцияЕГАИС.СохранитьНастройкиВыбораАлкогольнойПродукции(ЭтотОбъект, Объект.Ссылка);
...
Процедура ТоварыАлкогольнаяПродукцияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СобытияФормЕГАИСКлиентПереопределяемый.ОткрытьФормуВыбораАлкогольнойПродукции(
		Элемент,
		ИнтеграцияЕГАИСКлиентПереопределяемый.РеквизитыНоменклатурыДляВыбораАлкогольнойПродукции(ТекущиеДанные.Номенклатура),
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ИнтеграцияЕГАИС.ВосстановитьНастройкиВыбораАлкогольнойПродукции(ЭтотОбъект, Объект.Ссылка);
...
Процедура Подключаемый_ОбработаныНеизвестныеШтрихкоды(ДанныеШтрихкодов, ДополнительныеПараметры) Экспорт
	...
	ШтрихкодированиеНоменклатурыЕГАИСКлиентПереопределяемый.ОткрытьФормуСчитыванияАкцизнойМаркиПриНеобходимости(ЭтотОбъект, ДанныеДляОбработки);
...
Процедура УправлениеДоступностьюЭлементовФормы(Форма)
	...
	ИнтеграцияЕГАИСКлиентСервер.УправлениеДоступностьюЭлементовФормы(Форма, ДоступныеДляЧерновика, ЭтоЧерновик);
...
Функция ПодготовленныеДанныеПартионнойПродукцииТСД()
	...
	Иначе
		...
		Возврат ШтрихкодированиеНоменклатурыЕГАИСКлиентПереопределяемый.ПодготовитьДанныеДляОбработкиТаблицыТоваровПолученнойИзТСД(
				ЭтотОбъект, ЗагрузкаДанныхТСД.ПартионнаяАлкогольнаяПродукция, КэшированныеЗначения,
				ПараметрыЗаполнения);
...
Процедура ОбработатьПолученныеДанныеТСД()
	...
	Если ЗагрузкаДанныхТСД.Упаковки.Количество() > 0
		Или ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() > 0 
...
Процедура ОбработатьДанныеТСДНаСервере(ДанныеПартионнойПродукции, КэшированныеЗначения)
	...
	ПараметрыСканированияАкцизныхМарок = АкцизныеМаркиКлиентСервер.ПараметрыСканированияАкцизныхМарок(ЭтотОбъект);
	ВложенныеШтрихкоды = ПолучитьИзВременногоХранилища(ЗагрузкаДанныхТСД.АдресВложенныеШтрихКоды);
	
	Для Каждого МаркируемаяАлкогольнаяПродукция Из ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция Цикл
		
		Если ВложенныеШтрихкоды <> Неопределено Тогда
			
			ПараметрыПоиска = Новый Структура;
			ПараметрыПоиска.Вставить("ЗначениеШтрихкода", МаркируемаяАлкогольнаяПродукция.ДанныеШтрихкода.Штрихкод);
		...
		РезультатОбработкиШтрихкода = Документы.АктПостановкиНаБалансЕГАИС.ОбработатьШтрихкодАкцизнойМарки(ЭтотОбъект, 
		                                                                                                   МаркируемаяАлкогольнаяПродукция.ДанныеШтрихкода, 
	...
	Если ЗагрузкаДанныхТСД.Упаковки.Количество() > 0 Тогда
		...
		РезультатОбработкиШтрихкода = Документы.АктПостановкиНаБалансЕГАИС.ОбработатьШтрихкодУпаковки(ЭтотОбъект,
		                                                                                              ДанныеШтрихкода, 
		                                                                                              ВложенныеШтрихкоды, 
		                                                                                              ПараметрыСканированияАкцизныхМарок);
...
Процедура Подключаемый_ПослеОбработкиШтрихкодаТСД()
	...
	Если ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента <> ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукцияДопУказание.Количество() Тогда
		...
		АкцизныеМаркиЕГАИСКлиент.ЗавершитьОбработкуВводаШтрихкода(ЭтотОбъект, РезультатОбработкиШтрихкода, КэшированныеЗначения);
...
Функция РезультатОбработкиШтрихкодаДопУказаниеТСД(ДанныеШтрихКода, КэшированныеЗначения)
	...
	ПараметрыСканированияАкцизныхМарок = АкцизныеМаркиКлиентСервер.ПараметрыСканированияАкцизныхМарок(ЭтотОбъект);
	АкцизныеМаркиЕГАИС.АкцизныеМаркиВДокументе(ЭтотОбъект, ПараметрыСканированияАкцизныхМарок);
	
	РезультатОбработкиШтрихкода = АкцизныеМаркиЕГАИС.РезультатОбработкиШтрихкодаАкцизнойМарки(ЭтотОбъект,
