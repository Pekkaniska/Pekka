...
Функция СформироватьВыходнойФайл(Ссылка)
	...
	Атрибуты.Вставить("xmlns", "http://пф.рф/ВС/СЗВ-ИСХ/2018-02-13");
	...
	Если ВыборкаПоДаннымДНП.Количество() > 0 Тогда 
		...
		ФорматОснование = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматОснованияДНП.Основание.Значение);
		...
		Пока ВыборкаПоДаннымДНП.Следующий() Цикл 
			...
			НаборЗаписейОУТ.Код = ВРег(СокрЛП(ВыборкаПоДаннымДНП.ОснованиеВыслугиЛетКод));
			НаборЗаписейОУТ.ПозицияСписка = ВРег(СокрЛП(ВыборкаПоДаннымДНП.КодПозицииСпискаКод));
			ЗарплатаКадры.ДобавитьИнформациюВДерево(ЗарплатаКадры.ДобавитьУзелВДеревоXML(УзелСпискаОснованияДНП, "Основание", ""), НаборЗаписейОснование);
		КонецЦикла;
	...
	НаборЗаписейДатаУвольнения = Новый Структура("ДатаУвольнения", ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_ИСХ.ДатаУвольнения));
	ФорматСЗВ_ИСХ.Удалить("ДатаУвольнения");
	
	Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		
		НаборЗаписейСЗВ_ИСХ = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ФорматСЗВ_ИСХ);
		...
		Для Каждого КодМесяца Из СписокМесяцев Цикл 
			...
			Если ВыборкаПоДоходамНаВредныхИТяжелыхРаботах.НайтиСледующий(СтруктураПоиска) Тогда
				...
				НаборЗаписейПериод.ИС2__ДопТарифП1.Значение = ВыборкаПоДоходамНаВредныхИТяжелыхРаботах.ОблагаетсяВзносамиЗаЗанятыхНаПодземныхИВредныхРаботах;
				НаборЗаписейПериод.ИС2__ДопТарифП2_18.Значение = ВыборкаПоДоходамНаВредныхИТяжелыхРаботах.ОблагаетсяВзносамиЗаЗанятыхНаТяжелыхИПрочихРаботах;
		...
		Если УзелВыплатыДТ <> Неопределено Тогда
			...
			НаборЗаписейВсего.ИС2__ДопТарифП1.Значение = ВсегоДопТарифП1;
			НаборЗаписейВсего.ИС2__ДопТарифП2_18.Значение = ВсегоДопТарифП2_18;
			ЗарплатаКадры.ДобавитьИнформациюВДерево(УзелВсего, НаборЗаписейВсего);
		КонецЕсли;
...
Функция ЗапросПоШапкеДляПечати(СписокДокументов)
	...
	|	СведенияОЗаработкеСтаже.Формат2019 КАК Формат2019,
...
Функция ПечатнаяФормаСЗВ_ИСХ(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоШапкеДокумента.СледующийПоЗначениюПоля("Ссылка") Цикл 
		...
		Макет = ?(ВыборкаПоШапкеДокумента.Формат2019,
			...
			УправлениеПечатью.МакетПечатнойФормы("Документ.СведенияОЗаработкеСтажеЗастрахованныхЛицСЗВ_ИСХ.ПФ_MXL_ФормаСЗВ_ИСХ"));
