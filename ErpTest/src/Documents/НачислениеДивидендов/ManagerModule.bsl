...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	|	СрезРегистрацийВНО.РегистрацияВНО КАК СубконтоКт3,
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ СрезРегистрацийВНО КАК СрезРегистрацийВНО
	|			ПО Операция.Организация.ГоловнаяОрганизация = СрезРегистрацийВНО.Организация
	|				И (НАЧАЛОПЕРИОДА(Операция.Дата, МЕСЯЦ) = СрезРегистрацийВНО.ДатаОтражения)
	...
	|	НЕОПРЕДЕЛЕНО,
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка),
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСБюджетом),
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.НачислениеДивидендов КАК Операция
	|			ЛЕВОЕ СОЕДИНЕНИЕ СрезРегистрацийВНО КАК СрезРегистрацийВНО
	|			ПО Операция.Организация = СрезРегистрацийВНО.Организация
	|				И (НАЧАЛОПЕРИОДА(Операция.Дата, МЕСЯЦ) = СрезРегистрацийВНО.ДатаОтражения)
	|		ПО ДокументыКОтражению.Ссылка = Операция.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.НачислениеДивидендов.ЮридическиеЛица КАК Строки
	|		ПО (Строки.Ссылка = Операция.Ссылка)
