...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	Если Реквизиты <> Неопределено Тогда
		...
		ПараметрыПервичногоДокумента.Номер = Реквизиты.Номер;
		ПараметрыПервичногоДокумента.Дата = Реквизиты.Период;
...
Функция ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	СУММА(ВЫРАЗИТЬ(ДанныеДокумента.СуммаДокумента * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(31,2)))
	|ИЗ
	|	Документ.РасходныйКассовыйОрдер КАК ДанныеДокумента
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеДокумента = Метаданные.Документы.РасходныйКассовыйОрдер;
