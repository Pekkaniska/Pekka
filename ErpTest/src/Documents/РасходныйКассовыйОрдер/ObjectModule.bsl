...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетОператораСистемыПлатон") Тогда
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ДепонированиеЗарплаты") Тогда
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("СправочникСсылка.ДоговорыЛизинга") Тогда
	...
	Если Константы.ИспользоватьНачислениеЗарплаты.Получить()
		И ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыЧерезКассу Тогда
		
		ПараметрыВыплатыЗП = Новый Структура;
		ПараметрыВыплатыЗП.Вставить("МассивВедомостей", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Ведомость));
		ПараметрыВыплатыЗП.Вставить("Организация", Организация);
		ПараметрыВыплатыЗП.Вставить("ИсключатьОплаченные", Истина);
		ПараметрыВыплатыЗП.Вставить("ОтображатьДепонированные", Истина);
		
		ВыплатаЗаработнойПлаты.Загрузить(ДенежныеСредстваСервер.ДанныеРаботниковПоВедомостям(ПараметрыВыплатыЗП));
	КонецЕсли;
	//-- НЕ УТ
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаЗаймаСотруднику Тогда
			Для Каждого Строка Из РасшифровкаПлатежа Цикл
				ЗаймыСотрудникам.ЗарегистрироватьПредоставлениеЗайма(
					Движения, Строка.ДоговорЗаймаСотруднику, Строка.Сумма, Дата, Организация, ПодотчетноеЛицо, Отказ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ЗаполнитьПоВедомостямНаВыплатуЗарплаты(Знач ДанныеКоманды, ДанныеЗаполнения)
	...
	Если ДанныеЗаполнения.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыРаботнику") Тогда
		...
		ДанныеЗаполнения.Вставить("СуммаДокумента", ДенежныеСредстваСервер.ДанныеРаботниковПоВедомостям(СтруктураПараметров));
...
Процедура ЗаполнитьПоДепонированию(Знач ДокументОснование, Знач СуммаКОплате, ДанныеЗаполнения)
