...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.УстановкаЗначенийНефинансовыхПоказателей) Тогда
		...
		КомандаСоздатьНаОсновании.Представление = ОбщегоНазначенияУТ.ПредставлениеОбъекта(Метаданные.Документы.УстановкаЗначенийНефинансовыхПоказателей);
...
Функция ТипКолонки(Измерение)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Неверный шаблон: неизвестное измерение """ + Измерение.ИмяИзмерения + """'");
	КонецЕсли;
...
Функция РедактируемыеКолонки(ПоказательШаблон, МенеджерВТ)
	...
	Если ТипЗнч(ПоказательШаблон) = Тип("СправочникСсылка.НефинансовыеПоказателиБюджетов") Тогда
		Если ПоказательШаблон.ПоПериодам Тогда
			ДобавитьПолеШапки(СписокПолей, "НомерПодПериода", НСтр("ru = '№ подпериода'"), ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(3, 0));
		КонецЕсли;
		Возврат СписокПолей;
	КонецЕсли;
	...
	Запрос.МенеджерВременныхТаблиц = МенеджерВТ;
	Запрос.УстановитьПараметр("НомерПодпериода", НСтр("ru = '№ подпериода'")); 
	Запрос.УстановитьПараметр("Ссылка", ПоказательШаблон);
...
Функция ВидыПоказателейПоИмениКолонки(ДеревоКолонокШапки, СвойстваПоказателей)
	...
	ЗаполнитьВидыПоказателейПоИмениКолонкиРекурсивно(ВидыПоказателейЭлементовФормы, ДеревоКолонокШапки.Строки, СвойстваПоказателей);
...
Процедура ОтразитьПоказателиРедактируемыеВШапке(СтруктураОписанияВвода, Элементы, ДобавленныеЭлементы, ЗначенияПоказателей) Экспорт
	...
	Для Каждого Показатель Из СтруктураОписанияВвода.ПоказателиРедактируемыеВШапке Цикл
		...
		Если Показатель.ВидПоказателя = Перечисления.ВидыНефинансовыхПоказателей.Количественный Тогда
			НовыйЭлемент.ОграничениеТипа = ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 3);
	...
	Для Каждого ИмяРеквизита Из СтруктураОписанияВвода.КолонкиЗначенийТабличнойЧасти Цикл
		...
		Иначе
			...
			Если ВидПоказателя = Перечисления.ВидыНефинансовыхПоказателей.Количественный Тогда
				ТипДанныхПоказателя = ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(15, 3);
...
Процедура ТаблицаЗначенияНефинансовыхПоказателей(Ссылка, ДополнительныеСвойства, Регистры)
	...
	|	ДанныеТабличныхЧастей.Аналитика1 КАК Аналитика1,
	|	ДанныеТабличныхЧастей.Аналитика2 КАК Аналитика2,
	|	ДанныеТабличныхЧастей.Аналитика3 КАК Аналитика3,
	|	ДанныеТабличныхЧастей.Аналитика4 КАК Аналитика4,
	|	ДанныеТабличныхЧастей.Аналитика5 КАК Аналитика5,
	|	ДанныеТабличныхЧастей.Аналитика6 КАК Аналитика6,
	...
	|	ТаблицаЗначенияПоказателей.НефинансовыйПоказатель.Периодичность КАК Периодичность,
	|	ТаблицаЗначенияПоказателей.ТипПериода КАК ТипПериода,
	|	ТаблицаЗначенияПоказателей.Период КАК Период,
	|	ТаблицаЗначенияПоказателей.Аналитика1 КАК Аналитика1,
	|	ТаблицаЗначенияПоказателей.Аналитика2 КАК Аналитика2,
	|	ТаблицаЗначенияПоказателей.Аналитика3 КАК Аналитика3,
	|	ТаблицаЗначенияПоказателей.Аналитика4 КАК Аналитика4,
	|	ТаблицаЗначенияПоказателей.Аналитика5 КАК Аналитика5,
	|	ТаблицаЗначенияПоказателей.Аналитика6 КАК Аналитика6,
	|	ТаблицаЗначенияПоказателей.Сценарий КАК Сценарий,
	|	ТаблицаЗначенияПоказателей.Организация КАК Организация,
	|	ТаблицаЗначенияПоказателей.Подразделение КАК Подразделение,
	|	ТаблицаЗначенияПоказателей.НомерПодПериода КАК НомерПодПериода,
	|	ТаблицаЗначенияПоказателей.НефинансовыйПоказатель КАК НефинансовыйПоказатель,
	|	ТаблицаЗначенияПоказателей.Значение КАК ЗначениеПоказателя,
	|	ТаблицаЗначенияПоказателей.Валюта КАК Валюта
	|ИЗ
	|	ЗначенияПоказателей КАК ТаблицаЗначенияПоказателей
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УстановкаЗначенийНефинансовыхПоказателей КАК Шапка
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	"ВЫБРАТЬ
		...
		|ИЗ
		|	Документ.УстановкаЗначенийНефинансовыхПоказателей КАК Таблица
		|ГДЕ
		...
		|	Таблица.Ссылка
		|ИЗ
		|	Документ.УстановкаЗначенийНефинансовыхПоказателей.СтрокиДокумента КАК Таблица
		|ГДЕ
		|	(Таблица.Аналитика1 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика2 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика3 В (&ПустыеЗначенияДляОбработки)
		...
		|	Таблица.Ссылка
		|ИЗ
		|	Документ.УстановкаЗначенийНефинансовыхПоказателей.КолонкиДокумента КАК Таблица
		|ГДЕ
		|	(Таблица.Аналитика1 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика2 В (&ПустыеЗначенияДляОбработки)
		|			ИЛИ Таблица.Аналитика3 В (&ПустыеЗначенияДляОбработки)
		...
		|	Таблица.Ссылка
		|ИЗ
		|	Документ.УстановкаЗначенийНефинансовыхПоказателей.ЗначенияКолонок КАК Таблица
		|ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяМетаданных = "Документ.УстановкаЗначенийНефинансовыхПоказателей";
	МетаданныеОбъекта = Метаданные.Документы.УстановкаЗначенийНефинансовыхПоказателей;
	...
	Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
		...
		РеквизитыПоиска.Добавить("СтрокиДокумента.Аналитика" + Сч);
		РеквизитыПоиска.Добавить("КолонкиДокумента.Аналитика" + Сч);
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если НЕ Объект = Неопределено Тогда
				...
				Для каждого СтрокаТабличнойЧасти Из Объект.ЗначенияКолонок Цикл
					Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Аналитика)
					   И НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Значение) Тогда
