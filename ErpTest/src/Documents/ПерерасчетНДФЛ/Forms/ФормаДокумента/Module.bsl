...
Функция ДанныеПоНДФЛ(Сотрудники = Неопределено)
	...
	ИсчисленныйНалог = УчетНДФЛ.ИсчисленныйНалогФизическихЛицПоРезультатамРасчета(ДанныеОНалогах);
	...
	ИсчисленныйНалогФизическиеЛица = ФизическиеЛицаРаботавшиеВОрганизации.ВыгрузитьКолонку("ФизическоеЛицо");
	ТекущиеОстаткиВзаиморасчетов = ВзаиморасчетыССотрудниками.КорректировкиВыплаты(Объект.Организация, Объект.МесяцПерерасчета, ИсчисленныйНалогФизическиеЛица, Объект.Ссылка);
	
	КорректировкиВыплаты = РасчетЗарплаты.РекомендуемыеКорректировкиВыплаты(ИсчисленныйНалог, ТекущиеОстаткиВзаиморасчетов);
