...
Процедура ИнициализироватьКлючиАналитикиНоменклатуры(Запрос)
	...
	|			И Аналитика.Склад = ЗаказДавальца.Подразделение
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса)
	...
	|		И &Подразделение = АналитикаВПодразделенииБезСерии.Склад
	...
	|		И АналитикаДавальца.Склад = ЗаказДавальца.Подразделение
	...
	|		И АналитикаДавальцаБезНазначения.Склад = ЗаказДавальца.Подразделение
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|		И ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаВПодразделении.СтатьяКалькуляции
	//-- НЕ УТ	
	|		И ТаблицаТовары.Подразделение = АналитикаВПодразделении.Склад
	|		И ВЫБОР
	|			КОГДА ТаблицаТовары.СтатусУказанияСерий = 14
	|			ТОГДА ТаблицаТовары.Серия = АналитикаВПодразделении.Серия
	|			ИНАЧЕ &ПустаяСерия = АналитикаВПодразделении.Серия
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ТОГДА ВЫБОР КОГДА &ИспользуетсяНазначение
	|			ТОГДА ТаблицаТовары.АналитикаУчетаВПодразделении
	|			ИНАЧЕ ТаблицаТовары.АналитикаУчетаВПодразделенииБезНазначения
	|			КОНЕЦ
	|		ИНАЧЕ ВЫБОР КОГДА &ИспользуетсяНазначение
	|			ТОГДА ТаблицаТовары.АналитикаУчетаНоменклатуры
	|			ИНАЧЕ ТаблицаТовары.АналитикаУчетаНоменклатурыБезНазначения
	|			КОНЕЦ
	|	КОНЕЦ) КАК АналитикаУчетаНоменклатуры,
...
Функция ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)  КАК ВидДвижения,
	|	&Дата                                   КАК Период,
	|	ТаблицаТовары.Распоряжение.Распоряжение КАК ДокументПоступления,
	|	ТаблицаТовары.Номенклатура              КАК Номенклатура,
	|	ТаблицаТовары.Характеристика            КАК Характеристика,
	...
	|	КОНЕЦ                                   КАК Назначение,
	|	ТаблицаТовары.Склад                     КАК Склад,
	|	ТаблицаТовары.Распоряжение.Подразделение КАК Отправитель,
	...
	|			ТОГДА &Ссылка
	//++ НЕ УТКА
	|		ИНАЧЕ ТаблицаТовары.Распоряжение.Распоряжение
	//-- НЕ УТКА
	...
	|			ТОГДА &Подразделение
	//++ НЕ УТКА
	|		ИНАЧЕ ТаблицаТовары.Распоряжение.Подразделение
	//-- НЕ УТКА
...
// Регистрирует данные для обработчика обновления УП 2.4.1
//
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.ВыпускПродукции";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			СтрокаКОбработке = Запрос.Выполнить().Выбрать();
