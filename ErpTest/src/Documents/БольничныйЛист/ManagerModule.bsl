...
Функция РеквизитыДляПроведенияПустаяСтруктура()
	...
	РеквизитыДляПроведенияПустаяСтруктура = Новый Структура("Ссылка, Организация, ПериодРегистрации, ДокументРассчитан, ПорядокВыплаты, Дата, ПланируемаяДатаВыплаты,
		...
		| ДатаОкончанияРодственник2, РаспределениеПоТерриториямУсловиямТруда, РаспределениеРезультатовНачислений, ДоходПолученНаТерриторииРФ");
...
Функция ДанныеДляРеестраЭЛН(Больничный = Неопределено, Реестр = Неопределено) Экспорт
	...
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Больничные = Запрос.Выполнить().Выгрузить();
	Больничные.Свернуть("Ссылка");
	
	ДанныеДляРасчетаСреднегоЗаработкаПоВсемБольничным = ДанныеДокументовДляРасчетаСреднегоЗаработкаФСС(Больничные.ВыгрузитьКолонку("Ссылка"));
	...
	Запрос.УстановитьПараметр("БазыДляРасчетаСреднегоЗаработка", БазыДляРасчетаСреднегоЗаработка);
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ХэшиЭЛН КАК ХэшиЭЛН
	|		ПО (БольничныйЛист.НомерЛисткаНетрудоспособности = ХэшиЭЛН.НомерЛисткаНетрудоспособности)
	|			И (ХэшиЭЛН.Хэш <> """")
...
Процедура ЗаполнитьФлагПостановкиНаУчетВРанниеСрокиБеременности(ДокументОбъект, ДанныеИзСервиса)
	ФлагПостановки = ОбменЛисткамиНетрудоспособностиФСС.ЗначениеИзСтрокиXMLПоТипу(ДанныеИзСервиса.PREGN12W_FLAG, Тип("Булево"));
