...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИспользуетсяРасчетЗарплаты Тогда
		Если ИмяСобытия = "ВведеныДанныеЛисткаНетрудоспособности" И Источник = ЭтотОбъект Тогда
		...
		ИначеЕсли ИмяСобытия = "ИзмененСтажФизическогоЛица" И Источник.ВладелецФормы = ЭтотОбъект Тогда
...
Функция ЗагрузитьОтветСервисаФСС(Знач АдресРасшифрованногоОтветаSOAP)
	...
	Если ОбменВыполненУспешно Тогда
		...
		ОбновитьФормуПослеЗагрузкиЭЛН(Ложь);
...
Функция ЗагрузитьИзФайлаНаСервере(АдресДвоичныхДанных)
	...
	Если ФайлЗагружен Тогда
		ЗначениеВРеквизитФормы(Документ, "Объект");
		ОбновитьФормуПослеЗагрузкиЭЛН(Истина);
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьСсылкиНачислений(ПервичнаяИнициализацияФормы)
	...
	Если Объект.ДоплачиватьДоДенежногоСодержания Или Объект.ДоплачиватьДоСреднегоЗаработка Тогда
		ДополнительныеПараметрыВыбора = Документы.БольничныйЛист.ДополнительныеПараметрыВыбораНачислений(Объект, "ВидРасчетаДоплаты");
...
Процедура НачатьПерезаполнениеИРасчетНачисленийНаКлиенте(ВыводитьСообщения = Ложь)
	...
	Если ВыводитьСообщения Тогда
		ЗарплатаКадрыРасширенныйКлиент.ПодключитьОбработчикОжиданияАвтоматическогоРасчета(ЭтотОбъект, "ПерезаполнитьИРассчитатьНачисленияСВыводомСообщений");
	Иначе
		ЗарплатаКадрыРасширенныйКлиент.ПодключитьОбработчикОжиданияАвтоматическогоРасчета(ЭтотОбъект, "ПерезаполнитьИРассчитатьНачисленияБезВыводаСообщений");
	КонецЕсли;
...
Процедура УчетПособийОбновитьСведенияОПрямыхВыплатах()
	Элементы.СтраницаПилотныйПроектФСС.Видимость = ИспользуетсяРасчетЗарплаты
		И ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(Объект.Организация, Объект.ПериодРегистрации);
