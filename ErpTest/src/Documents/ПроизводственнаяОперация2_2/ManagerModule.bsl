...
Процедура Разбить(Ссылка, ЗначенияЗаполнения, БлокироватьДанные = Истина) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Производственные операции'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|		ПО Документ.Ссылка = Таблица.Ссылка
		|ГДЕ
		|	Документ.Исполнитель ССЫЛКА Справочник.Бригады
		|	И Документ.Исполнитель <> ЗНАЧЕНИЕ(Справочник.Бригады.ПустаяСсылка)
		|	И Таблица.Ссылка ЕСТЬ НЕ NULL
		|	И Документ.Исполнитель <> Таблица.Исполнитель
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	Документ.ПроизводственнаяОперация2_2 КАК Документ
		|ГДЕ
		|	Документ.Операция <> ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|	И НЕ Документ.НаОснованииМаршрутнойКарты
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|	Документ.ПроизводственнаяОперация2_2 КАК Документ
		|ГДЕ
		|	Документ.НачалоВыполнения = ДАТАВРЕМЯ(1, 1, 1)
		|	И Документ.Статус В (
		...
		|		ПО Документ.Ссылка = Таблица.Ссылка
		|ГДЕ
		|	Документ.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПроизводственныхОпераций.НеВыполнена)
		|	И Таблица.Ссылка ЕСТЬ НЕ NULL 
		|
		|ОБЪЕДИНИТЬ
		|
		...
		|	Документ.ПроизводственнаяОперация2_2 КАК Документ
		|ГДЕ
		|	Документ.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|	И Документ.Этап <> ЗНАЧЕНИЕ(Документ.ЭтапПроизводства2_2.ПустаяСсылка)");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДопПараметры.ДополнительныеИсточникиДанных.Вставить("Операция");
	...
	Запрос = Новый Запрос(
		...
		|	ЕСТЬNULL(ТехнологическиеОперации.Наименование, """") КАК НаименованиеОперации,
		|	ЕСТЬNULL(Этапы.Организация, НЕОПРЕДЕЛЕНО)            КАК Организация
		|ИЗ
		|	ВТСсылкиДляОбработки КАК СсылкиДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводственнаяОперация2_2 КАК ТаблицаОбъекта
		|		ПО СсылкиДляОбработки.Ссылка = ТаблицаОбъекта.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ТехнологическиеОперации КАК ТехнологическиеОперации
		|		ПО (ТаблицаОбъекта.Операция = ТехнологическиеОперации.Ссылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК Этапы
		|		ПО (ТаблицаОбъекта.Этап = Этапы.Ссылка)");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ТипЗнч(ДокументОбъект.Исполнитель) = Тип("СправочникСсылка.Бригады")
					И НЕ ДокументОбъект.Исполнитель.Пустая() Тогда
					
					ОчиститьРеквизитВШапке = Ложь;
					
					Для каждого Строка Из ДокументОбъект.Трудозатраты Цикл
						
						Если ЗначениеЗаполнено(Строка.Исполнитель)
							И Строка.Исполнитель <> ДокументОбъект.Исполнитель Тогда
							
							ОчиститьРеквизитВШапке = Истина;
							Прервать;
					...
					Если ОчиститьРеквизитВШапке Тогда
						ДокументОбъект.Исполнитель = Справочники.Бригады.ПустаяСсылка();
					Иначе
						Для каждого Строка Из ДокументОбъект.Трудозатраты Цикл
							Строка.Исполнитель = ДокументОбъект.Исполнитель;
						КонецЦикла;
					КонецЕсли;
				...
				Если НЕ ДокументОбъект.Операция.Пустая()
					И НЕ ДокументОбъект.НаОснованииМаршрутнойКарты Тогда
					
					ДокументОбъект.НаОснованииМаршрутнойКарты = Истина;
					ДокументОбъект.Наименование = Выборка.НаименованиеОперации;
					
					ОбъектИзменен = Истина;
					
				КонецЕсли;
				
				Если ДокументОбъект.НачалоВыполнения = '00010101'
					И (ДокументОбъект.Статус = Перечисления.СтатусыПроизводственныхОпераций.Выполняется
						ИЛИ ДокументОбъект.Статус = Перечисления.СтатусыПроизводственныхОпераций.Выполнена) Тогда
					
					ДокументОбъект.НачалоВыполнения = ?(ДокументОбъект.ОкончаниеВыполнения = '00010101',
						ДокументОбъект.Дата,
						ДокументОбъект.ОкончаниеВыполнения);
					
					Если ДокументОбъект.Статус = Перечисления.СтатусыПроизводственныхОпераций.Выполнена Тогда
						
						ДокументОбъект.КоличествоФакт = ДокументОбъект.Количество;
				...
				Если ДокументОбъект.Статус = Перечисления.СтатусыПроизводственныхОпераций.НеВыполнена
					И (ДокументОбъект.ВыходныеИзделия.Количество() <> 0
						ИЛИ ДокументОбъект.МатериалыИРаботы.Количество() <> 0
						ИЛИ ДокументОбъект.Трудозатраты.Количество() <> 0) Тогда
					
					ДокументОбъект.Статус = Перечисления.СтатусыПроизводственныхОпераций.Выполнена;
					ДокументОбъект.ТребуетПовторения = Истина;
					ДокументОбъект.НачалоВыполнения = ДокументОбъект.ОкончаниеВыполнения;
					ДокументОбъект.КоличествоФакт = ДокументОбъект.Количество;
				...
				Если ДокументОбъект.Организация.Пустая() И Выборка.Организация <> Неопределено Тогда
