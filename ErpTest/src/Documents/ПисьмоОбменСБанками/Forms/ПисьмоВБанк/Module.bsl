...
Процедура СтатусНажатие(Элемент)
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыОбменСБанками.НеПодтвержден") Тогда
		...
		Если ТестовыйРежим Тогда
			ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("http://194.54.14.120:9080/icdk");
		Иначе
			ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("https://sbi.sberbank.ru:9443/ic/dcb");
		КонецЕсли;
	КонецЕсли;
...
Процедура Отправить(Команда)
	...
	Если Объект.ДляВалютногоКонтроля И Не ЗначениеЗаполнено(Объект.Основание) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Основание", , Отказ);
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Объект.Банк) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.Банк", , Отказ);
	КонецЕсли;
	...
	Если ОбщийРазмер > МаксимальныйРазмерВложений Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПрисоединенныеФайлыТаблица", , Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.Тема) Тогда
		ТекстСообщения = НСтр("ru = 'Необходимо заполнить тему письма'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.Тема", , Отказ);
	КонецЕсли;
	
	Если ПустаяСтрока(Объект.Текст) Тогда
		ТекстСообщения = НСтр("ru = 'Текст письма не может быть пустым'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.Текст", , Отказ);
	КонецЕсли;
...
Процедура ПослеВыбораФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ОписаниеФайла Из ПомещенныеФайлы Цикл
		...
		СтруктураФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ОписаниеФайла.Имя);
		...
		ПараметрыФайла = ПараметрыФайла();
		ПараметрыФайла.Размер = Размер;
		ПараметрыФайла.Хранение = ОписаниеФайла.Хранение;
		ПараметрыФайла.Наименование = СтруктураФайла.Имя;
		МассивДополнения.Добавить(ПараметрыФайла);
	
	КонецЦикла;
	
	Для Каждого ОписаниеФайла Из МассивДополнения Цикл
		
		НовСтрока = ПрисоединенныеФайлыТаблица.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтрока, ОписаниеФайла);
		НовСтрока.Сохранен = Ложь;
		НовСтрока.Размер = НовСтрока.Размер;
...
Функция ПараметрыФайла()
	...
	ВозвращаемоеЗначение.Вставить("Размер");
	ВозвращаемоеЗначение.Вставить("Хранение");
