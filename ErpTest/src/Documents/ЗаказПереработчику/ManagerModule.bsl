...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ЗаказПереработчику КАК ТаблицаДокументы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику.ВозвратныеОтходы КАК ТЧВозвратныеОтходы
	|		ПО ТаблицаДокументы.Ссылка = ТЧВозвратныеОтходы.Ссылка
	|ГДЕ
	|	ТаблицаДокументы.Проведен
	|	И &ПартионныйУчетВерсии22
	|	И ТаблицаДокументы.Дата >= &ДатаПереходаНаПартионныйУчетВерсии22
	...
	|ИЗ
	|	Документ.ЗаказПереработчику КАК ТаблицаДокументы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику.Материалы КАК ТЧМатериалы
	|		ПО ТаблицаДокументы.Ссылка = ТЧМатериалы.Ссылка
	|ГДЕ
	|	ТаблицаДокументы.Проведен
	|	И &ПартионныйУчетВерсии22
	|	И ТаблицаДокументы.Дата >= &ДатаПереходаНаПартионныйУчетВерсии22
	...
	|ИЗ
	|	Документ.ЗаказПереработчику КАК ТаблицаДокументы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику.Услуги КАК ТЧУслуги
	|		ПО ТаблицаДокументы.Ссылка = ТЧУслуги.Ссылка
	|ГДЕ
	|	ТаблицаДокументы.Проведен
	|	И &ПартионныйУчетВерсии22
	|	И ТаблицаДокументы.Дата >= &ДатаПереходаНаПартионныйУчетВерсии22
	|	И ТЧУслуги.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И ТаблицаДокументы.УслугиПоПереработке = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|	И &ПартионныйУчетВерсии22
	|	И ЗаказПереработчику.Дата >= &ДатаПереходаНаПартионныйУчетВерсии22
	|	И ЗаказПереработчику.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И ЗаказПереработчику.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки)
	|	И ЗаказПереработчику.УслугиПоПереработке = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	...
	|ОБЪЕДИНИТЬ
	|
	|// Обработчик снятия флага ""Отгружать одной датой"". Все этапы одного заказа переработчику.
	|ВЫБРАТЬ
	|	Заказ.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ЗаказПереработчику КАК Заказ
	|ГДЕ
	|	Заказ.НеОтгружатьЧастями
	|		И Заказ.Ссылка В(
	|		
	|		ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			ОбеспечениеМатериаламиИРаботами.Ссылка.ЗаказПереработчику КАК Ссылка
	|		ИЗ
	|			Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ОбеспечениеМатериаламиИРаботами
	|		ГДЕ
	|			ОбеспечениеМатериаламиИРаботами.Ссылка.ЗаказПереработчику <> ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка)
	|				И ОбеспечениеМатериаламиИРаботами.Ссылка.НеОтгружатьЧастями
	|				И ОбеспечениеМатериаламиИРаботами.ДатаОтгрузки <> ОбеспечениеМатериаламиИРаботами.Ссылка.ДатаОтгрузки
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ПО КОбработке.Ссылка.УдалитьСклад = Склады.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	ЗапросСинхронностиЭтаповПоЗаказуПереработчику.Текст =
		...
		|	ИЗ(
		|		ВЫБРАТЬ
		|			Этапы.НеОтгружатьЧастями КАК НеОтгружатьЧастями
		|		ИЗ
		|			Документ.ЭтапПроизводства2_2 КАК Этапы
		|		ГДЕ
		|			Этапы.ЗаказПереработчику = &ЗаказПереработчику
		...
		|		ГДЕ
		|			Заказы.Ссылка = &ЗаказПереработчику) КАК Набор
		|		
		|ИМЕЮЩИЕ
		|	МИНИМУМ(Набор.НеОтгружатьЧастями) <> МАКСИМУМ(Набор.НеОтгружатьЧастями)";
	//-- НЕ УТКА
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("ЗаказПереработчику", Выборка.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			...
			Если Объект <> Неопределено Тогда
				...
				Если МожноОбрабатывать
					И Объект.Проведен
					И ПартионныйУчетВерсии22 И ДатаПереходаНаПартионныйУчетВерсии22 <= Объект.Дата Тогда
				...
				Если МожноОбрабатывать
					И Объект.Проведен
					И ПартионныйУчетВерсии22 И ДатаПереходаНаПартионныйУчетВерсии22 <= Объект.Дата
					И Объект.УслугиПоПереработке = Перечисления.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете Тогда
					СтруктураПоиска = Новый Структура("СтатьяКалькуляции", Справочники.СтатьиКалькуляции.ПустаяСсылка());
				...
				Если МожноОбрабатывать
					...
					И Не ЗначениеЗаполнено(Объект.СтатьяКалькуляции) Тогда
					
					Если СтатьяКалькуляцииМатериалов = Неопределено Тогда
						МожноОбрабатывать = Ложь;
					Иначе
						ОбъектИзменен = Истина;
						Объект.СтатьяКалькуляции = СтатьяКалькуляцииМатериалов;
					КонецЕсли;
				...
				Если МожноОбрабатывать
					И НЕ Объект.ОбосабливатьПоНазначениюПродукции Тогда
					
					ОбособлениеПоНазначениюПродукции = Истина;
					Индекс = 0;
					Пока ОбособлениеПоНазначениюПродукции И Индекс < Объект.Продукция.Количество() Цикл
						СтрокаПродукции = Объект.Продукция[Индекс];
						Если ЗначениеЗаполнено(СтрокаПродукции.Назначение) Тогда
							Материалы = Объект.Материалы.НайтиСтроки(Новый Структура("НомерГруппыЗатрат", СтрокаПродукции.НомерГруппыЗатрат));
					...
					Если ОбособлениеПоНазначениюПродукции Тогда
						Объект.ОбосабливатьПоНазначениюПродукции = Истина;
						ОбъектИзменен = Истина;
					КонецЕсли;
				...
				Если МожноОбрабатывать
					И Объект.ВариантПриемкиТоваров = Перечисления.ВариантыПриемкиТоваров.ПустаяСсылка() Тогда
					ОбъектИзменен = Истина;
				...
				//++ НЕ УТКА
				// Обработчик №8.
				// Снятие флага ""Отгружать одной датой"".
				
				ЗапросСинхронностиЭтаповПоЗаказуПереработчику.УстановитьПараметр("ЗаказПереработчику", Выборка.Ссылка);
				Если МожноОбрабатывать
					...
					И Не ЗапросСинхронностиЭтаповПоЗаказуПереработчику.Выполнить().Пустой() Тогда
					ОбъектИзменен = Истина;
				...
				Если МожноОбрабатывать
					И Не ПустаяСтрока(Объект.АдресДоставкиЗначенияПолей) Тогда
					НовыеЗначенияПолей = "";
				...
				Если МожноОбрабатывать
					И Объект.СпособРаспределенияЗатратНаВыходныеИзделия = Перечисления.СпособыРаспределенияЗатратНаВыходныеИзделия.ПустаяСсылка() Тогда
				...
				Если Выборка.СтрокДляГруппировки > 1 Тогда
					ЭтапыОплаты = Объект.ЭтапыГрафикаОплаты.Выгрузить();
				...
				Если МожноОбрабатывать Тогда
					...
					СкладШапкиЗаполнен = ЗначениеЗаполнено(Объект.УдалитьСклад);
					
					СкладШапки = ?(СкладШапкиЗаполнен И Не Выборка.ЭтоГруппаСкладов,
								   Объект.УдалитьСклад, Неопределено);
				...
				Если МожноОбрабатывать Тогда
					...
					Иначе
						ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(Объект.Ссылка);
					КонецЕсли;
...
Функция ЗагруженыТребуемыеДанныеИзРИБ(МВТ, ИмяВременнойТаблицы)
	...
	|		ПО ТаблицаДокументы.Ссылка = ВТДокументыДляОбработки.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику.ВозвратныеОтходы КАК ТЧВозвратныеОтходы
	|		ПО ТаблицаДокументы.Ссылка = ТЧВозвратныеОтходы.Ссылка
	|ГДЕ
	|	ТаблицаДокументы.Дата >= &ДатаПереходаНаПартионныйУчетВерсии22
	...
	|	ТаблицаДокументы.Дата >= &ДатаПереходаНаПартионныйУчетВерсии22
	|	И ТаблицаДокументы.Проведен
	|	И ТаблицаДокументы.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И ТаблицаДокументы.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки)
	|	И ТаблицаДокументы.УслугиПоПереработке = ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете)";
...
Функция ДанныеОбновленыНаНовуюВерсиюПрограммы(МетаданныеИОтбор) Экспорт
	...
	Если Не МетаданныеИОтбор.ЭтоНовый Тогда
		...
		Если ТипЗнч(МетаданныеИОтбор.Отбор) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
		...
		Запрос.Текст =
			...
			|	Документ.ЭтапПроизводства2_2 КАК Этапы
			|ГДЕ
			|	Этапы.ЗаказПереработчику В(&Ссылки)";
		Запрос.УстановитьПараметр("Ссылки", Заказы);
		Этапы = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
		
		ДанныеСвязанныхОбъектовОбновлены = Не ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Неопределено,
			"Документ.ЭтапПроизводства2_2", Этапы);
