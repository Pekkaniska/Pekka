...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИменаПолейДляОткрытияУточнения = "ТоварыНоменклатура,ТоварыХарактеристика,ТоварыСерия,ТоварыИдентификаторПартии,"
		+"ТоварыНоменклатураЕдиницаИзмерения,ТоварыКоличество,ТоварыЕдиницаИзмеренияВЕТИС,ТоварыКоличествоВЕТИС,"
		+"ТоварыКоличествоРасхождений,ТоварыВозвращаемоеКоличествоВЕТИС";
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьОбщуюФорму") Тогда
		...
		ИначеЕсли ИмяТабличнойЧасти = "УпаковкиВЕТИС" Тогда
			...
			ИнтеграцияВЕТИСКлиент.ОткрытьФормуРедактированияУпаковокШтрихкодовПоСтроке(ЭтотОбъект, Объект, ТекущаяСтрока.ИдентификаторСтроки, ОписаниеОповещения, РедактированиеФормыНедоступно);
			Возврат;
...
Процедура ЗаписатьДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ВходящаяТранспортнаяОперацияВЕТИС.Форма.ФормаДокумента.Записать");
...
Процедура ПровестиДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ВходящаяТранспортнаяОперацияВЕТИС.Форма.ФормаДокумента.Провести");
...
Процедура ПровестиИЗакрыть(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ВходящаяТранспортнаяОперацияВЕТИС.Форма.ФормаДокумента.ПровестиИЗакрыть");
		...
		Если ЕстьДополнительнаяСтрока Тогда
			...
			Если ЕстьГиперссылка Тогда
				ПредставлениеСтрокиУсловий = Новый ФорматированнаяСтрока(ПредставлениеСтрокиУсловий, Новый Шрифт(,,,,Истина),
					ЦветаСтиля.ЦветГиперссылкиГИСМ,, "ОткрытьФормуРегионализации");
			Иначе
...
Процедура ОткрытьПодборСерий(Текст = "", СтандартнаяОбработка)
	...
	Если ПараметрыУказанияСерий <> Неопределено
		И ИнтеграцияВЕТИСКлиент.ДляУказанияСерийНуженСерверныйВызов(ЭтаФорма, ПараметрыУказанияСерий.Товары, Текст, СтандартнаяОбработка) Тогда
...
Процедура ЗаполнитьСлужебныеРеквизитыСерий(Объект, ПараметрыУказанияСерий)
	...
	Если ПараметрыУказанияСерий <> Неопределено Тогда							
		ГосударственныеИнформационныеСистемыПереопределяемый.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.Товары);
		ГосударственныеИнформационныеСистемыПереопределяемый.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий.ТоварыУточнение);
	КонецЕсли;
...
Функция ПредставлениеПериодаВЕТИС(ИмяПериода, ДанныеПериода)
	...
	ТолькоПросмотр = (ИмяПериода = "ДатаПроизводства");
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ГосударственныеИнформационныеСистемыПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтотОбъект, "ТоварыБезРазбиенияХарактеристика");
	...
	ГосударственныеИнформационныеСистемыПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтотОбъект, "ТоварыБезРазбиенияСерия");
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ГрузополучательХозяйствующийСубъект")
		...
		Объект.ТорговыйОбъект = ПредставленияГрузополучателя.ТорговыйОбъект.Ссылка;
	...
	Если СтруктураРеквизитов.Свойство("ТоварыВСД")
		...
		Элементы.ТоварыБезРазбиенияДобавитьБумажныйВСД.Видимость = Не ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент);
		Элементы.ТоварыБезРазбиенияЗаполнитьИзВСД.Видимость      = ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент);
		Элементы.ТоварыДобавитьБумажнуюВСД.Видимость             = Не ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент);
		Элементы.ЗаполнитьИзВСД.Видимость                        = ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент);
		
		Элементы.ТоварыБезРазбиенияПродукция.ТолькоПросмотр = Не ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент);
		Элементы.ТоварыБезРазбиенияПродукция.ПодсказкаВвода = ?(Не ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент),
			НСтр("ru = 'Укажите ВСД'"), "");
		Элементы.ТоварыБезРазбиенияПродукция.АвтоОтметкаНезаполненного = ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокумент);
	...
	Если СтруктураРеквизитов.Свойство("СвязанныеДокументыВозврата")
		...
		РеквизитПоказываем = Объект.ГрузополучательПредприятие <> Объект.ГрузоотправительПредприятие
			И Не Объект.ГашениеНаСВХ И Объект.Товары.Итог("ВозвращаемоеКоличествоВЕТИС") <> 0;
	...
	Если СтруктураРеквизитов.Свойство("ТоварыПродукция")
		Или ИнициализацияТоваровБезРазбиения Тогда
	...
	Если СтруктураРеквизитов.Свойство("УпаковкиВЕТИС") Или ИнициализацияТоваровБезРазбиения Тогда
	...
	Если СтруктураРеквизитов.Свойство("ТоварыНоменклатура")
		Или ИнициализацияТоваровБезРазбиения Тогда
		...
		Элементы.ТоварыСопоставлениеНоменклатура.ЦветТекста = ?(ОсобыйТекст, ЦветТекстаПоля("ЦветТекстаПроблемаЕГАИС"), ЦветТекстаПоля("ЦветТекстаПоля"));
	...
	Если СтруктураРеквизитов.Свойство("ТоварыХарактеристика")
		Или ИнициализацияТоваровБезРазбиения Тогда
		
		Если Не ТекущиеДанные.ХарактеристикиИспользуются Тогда
			ПредставлениеХарактеристика = НСтр("ru = '<характеристики не используются>'");
		ИначеЕсли ТекущиеДанные.ВыбраноХарактеристика > 1 Тогда
			ПредставлениеХарактеристика = СтрШаблон(НСтр("ru = '<Выбрано (%1)>'"), ТекущиеДанные.ВыбраноХарактеристика);
		Иначе
			ПредставлениеХарактеристика = ТекущиеДанные.СопоставлениеХарактеристика;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ТекущиеДанные.Характеристика) И ТекущиеДанные.ХарактеристикиИспользуются И ТекущиеДанные.ВыбраноХарактеристика <= 1 Тогда
			...
			ЦветТекста = ЦветТекстаПоля("ЦветТекстаПроблемаЕГАИС");
		...
		Элементы.ТоварыБезРазбиенияХарактеристика.ТолькоПросмотр = ТекущиеДанные.ЕстьУточнения Или Не ТекущиеДанные.ХарактеристикиИспользуются;
		Элементы.ТоварыПредставлениеХарактеристика.ТолькоПросмотр = ТекущиеДанные.ЕстьУточнения Или Не ТекущиеДанные.ХарактеристикиИспользуются;
		
	КонецЕсли;
	
	Если СтруктураРеквизитов.Свойство("ТоварыСерия")
		Или ИнициализацияТоваровБезРазбиения Тогда
		
		Если ТекущиеДанные.ВыбраноСерия > 1 Тогда
			ПредставлениеСерия = СтрШаблон(НСтр("ru = '<Выбрано (%1)>'"), ТекущиеДанные.ВыбраноСерия);
		Иначе
			ПредставлениеСерия = Неопределено;
		КонецЕсли;
		...
		Если ИспользоватьСерииНоменклатуры Тогда
			...
			Если Не ТекущиеДанные.ЕстьУточнения И Не ЗначениеЗаполнено(ТекущиеДанные.Серия) Тогда
				...
				ПараметрыОформленияСерий = ИнтеграцияВЕТИСКлиентСерверПереопределяемый.ПараметрыОформленияСерииПоДаннымСтроки(ТекущиеДанные);
				
				Элементы.ТоварыБезРазбиенияСерия.Видимость = ЗначениеЗаполнено(ТекущиеДанные.Серия) И Не ПараметрыОформленияСерий.Свойство("Текст");
				Элементы.ТоварыПредставлениеСерия.Видимость = Не ЗначениеЗаполнено(ТекущиеДанные.Серия) Или ПараметрыОформленияСерий.Свойство("Текст");
				
				НастраиваемыйЭлемент = ?(Элементы.ТоварыБезРазбиенияСерия.Видимость, Элементы.ТоварыБезРазбиенияСерия, Элементы.ТоварыПредставлениеСерия);
				
				Если ПараметрыОформленияСерий.Свойство("Текст") Тогда
					ПредставлениеСерия = ПараметрыОформленияСерий.Текст;
				КонецЕсли;
				
				Если ПараметрыОформленияСерий.Свойство("ИмяЦветаТекста") Тогда
					ПараметрыОформленияСерий.Вставить("ЦветТекста", ЦветТекстаПоля(ПараметрыОформленияСерий.ИмяЦветаТекста));
				КонецЕсли;
				
				ЗаполнитьЗначенияСвойств(НастраиваемыйЭлемент, ПараметрыОформленияСерий);
	...
	Если СтруктураРеквизитов.Свойство("ТоварыКоличествоВЕТИС")
		Или ИнициализацияТоваровБезРазбиения Тогда
	...
	Если СтруктураРеквизитов.Свойство("СрокГодности")
		Или ИнициализацияТоваровБезРазбиения Тогда
	...
	Если СтруктураРеквизитов.Свойство("ТоварыВозвращаемоеКоличествоВЕТИС")
		...
		РеквизитПоказываем = Не Объект.ГашениеНаСВХ;
	...
	Если Инициализация Тогда
		Элементы.ТоварыБезРазбиенияВетеринарноСопроводительныйДокументНаВозврат.Видимость = ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокументНаВозврат);
		Элементы.ТоварыСпискомВетеринарноСопроводительныйДокументНаВозврат.Видимость      = ЗначениеЗаполнено(ТекущиеДанные.ВетеринарноСопроводительныйДокументНаВозврат);
	КонецЕсли;
...
Процедура ОбновитьДанные(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ИмяОбновляемогоПоля = "ТранспортноеСредство" И Результат <> Неопределено И ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Объект.Маршрут.Количество() > 0
			И Не (ЗначениеЗаполнено(Объект.Маршрут[0].НомерТранспортногоСредства)
				Или ЗначениеЗаполнено(Объект.Маршрут[0].НомерАвтомобильногоПрицепа)
				Или ЗначениеЗаполнено(Объект.Маршрут[0].НомерАвтомобильногоКонтейнера)) Тогда
				
			ЗаполнитьЗначенияСвойств(Объект.Маршрут[0], Результат, "НомерТранспортногоСредства,НомерАвтомобильногоПрицепа,НомерАвтомобильногоКонтейнера,ТипТранспорта");
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения)
	...
	КонецЕсли;
	Если РеквизитыВлияющиеНаВСД.Свойство(ИмяОбновляемогоПоля) И ЗначениеЗаполнено(Объект.Товары.Получить(0).ДанныеВСД.ВетеринарноСопроводительныйДокумент) Тогда
		
		ПодключаемыеКомандыВЕТИСКлиентСервер.УправлениеВидимостьюКомандПодключенныхКОбъекту(ЭтотОбъект, Объект);
		ПустойВСД = ПредопределенноеЗначение("Справочник.ВетеринарноСопроводительныйДокументВЕТИС.ПустаяСсылка");
...
Процедура ОбновитьДанныеВСД(МассивВСД, ИдентификаторСтроки, ДобавлениеНовыхСтрок, КэшированныеЗначения)
	...
	Пока ТекущиеДанныеВСД.Следующий() Цикл
		...
		ИзменяемаяСтрока.ЕстьОтклонения = Ложь;
		ИзменяемаяСтрока.ЕстьУточнения = ВыборкаПартии.Количество() > 1;
		ИзменяемаяСтрока.КоличествоРасхождений = 0;
...
Процедура ОбновитьДанныеУточнений(ИзменяемаяСтрока, КоллекцияУточнений, КэшированныеЗначения)
	...
	ИзменяемаяСтрока.ЕстьУточнения = КоллекцияУточнений.Количество() > 1;
...
Процедура ЗаполнитьМаршрутВозврата()
	Если Объект.Товары.Итог("ВозвращаемоеКоличествоВЕТИС") > 0 И Объект.МаршрутВозврата.Количество() = 0 Тогда
		Для каждого ПунктМаршрута Из Объект.Маршрут Цикл
			Если Объект.ГрузоотправительПредприятие = ПунктМаршрута.Предприятие Тогда
				Продолжить;
			КонецЕсли;
		...
		КонецЦикла;	
		НовыйПунктМаршрутаВозврата = Объект.МаршрутВозврата.Вставить(0);
		НовыйПунктМаршрутаВозврата.Предприятие = Объект.ГрузополучательПредприятие;
		НовыйПунктМаршрутаВозврата.СПерегрузкой = Истина;
		ЗаполнитьЗначенияСвойств(НовыйПунктМаршрутаВозврата, Объект, ПоляТранспортногоСредства());
	КонецЕсли;
	Если Объект.Товары.Итог("ВозвращаемоеКоличествоВЕТИС") = 0 И Объект.МаршрутВозврата.Количество() > 0 Тогда
		Объект.МаршрутВозврата.Очистить();
	КонецЕсли;
...
Процедура ЗаполнитьСлужебныеРеквизитыТоваров(ТекущаяСтрока, ОбновляемыеПоля = Неопределено, ДополнительныеПараметры = Неопределено)
	...
	Если Инициализация Или СтруктураПолей.Свойство("ТоварыУточнение") Тогда
		...
		Если ДополнительныеПараметры.Свойство("Объект") Тогда
			ТаблицаДляПоиска = ?(ТекущаяСтрока.ЕстьУточнения, "ТоварыУточнение", "Товары");
