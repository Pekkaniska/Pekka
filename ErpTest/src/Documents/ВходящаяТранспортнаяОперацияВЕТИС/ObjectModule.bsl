	...
	Для каждого СтрокаТовары Из Товары Цикл
		...
		Если СтрокаТовары.ЕстьУточнения Тогда
			...
			Для каждого СтрокаУточнения Из ТоварыУточнения Цикл
				ИтогоКоличество = ИтогоКоличество + СтрокаУточнения.Количество + СтрокаУточнения.КоличествоВЕТИС;
			КонецЦикла;
			
			Если ИтогоКоличество <> 0 И Не ЗначениеЗаполнено(ТоварыУточнения.Получить(0).Номенклатура) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ТоварыУточнение.Количество() > 0 Тогда
		ТаблицаУточнений = ТоварыУточнение.Выгрузить(, "ИдентификаторСтроки,Номенклатура,Характеристика,Серия");
...
Процедура ЗаполнитьДанныеПродукции(ВыборкаВСД)
	...
	Пока ВыборкаВСД.Следующий() Цикл 
		...
		СтрокаТЧ.ЕстьУточнения = ВыборкаПартии.Количество() > 1;
	...
	Если НЕ ВсяПродукцияСопоставлена Тогда
		...
		Для Каждого СтрокаТЧ Из Товары Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) ИЛИ СтрокаТЧ.ЕстьУточнения Тогда
				Продолжить;
			КонецЕсли;
