...
Функция ОбработатьОтветНаЗапросВозможностиПеревозкиПартииВРамкахРегионализации(ДанныеДокумента, ХозяйствующийСубъект, Предприятие, ПараметрыОбмена, ДополнительныеПараметры = Неопределено) Экспорт
	...
	РезультатДобавленияЗаписей = ИнтеграцияВЕТИС.ДобавитьЗаписиВПротоколОбмена(ХозяйствующийСубъект, ДанныеДокумента);
...
Функция ПоследовательностьОпераций(ДокументСсылка) Экспорт
	...
	ИнтеграцияВЕТИС.ДобавитьОперациюВПоследовательность(Таблица, -3,
		Входящий,
		Перечисления.ВидыОперацийВЕТИС.ОтветНаЗапросПакетногоГашенияВСД);
...
//	ВспомогательныеДанные.Вставить("ТипыТранспорта",        ИнтеграцияВЕТИСПовтИсп.ТипыТранспорта());
	...
	ДокументXDTO = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПараметрыЗапросаВЕТИС.ПространствоИмен, ПараметрыЗапросаВЕТИС.ТипВЕТИС);
...
Функция ЗапросПроверкиВозможностиПеревозкиВРамкахРегионализацииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Результат = ИнтеграцияГосИС.ВыполнитьПакетЗапросов(Запрос, СписокЗапросов);
	...
	ДокументXDTO = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, ИмяТипа);
	
	ИнтеграцияВЕТИС.ЗаполнитьСвойствоXDTO(ДокументXDTO, "localTransactionId", Шапка.Идентификатор, СообщениеXML);
	
	ДокументXDTO.initiator = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "initiator");
	ИнтеграцияВЕТИС.ЗаполнитьСвойствоXDTO(ДокументXDTO.initiator, "login", НастройкаОбмена.ЛогинПользователя, СообщениеXML);
	
	Пока Товары.Следующий() Цикл
		
		cargoType = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "cargoType");
	...
	ДокументXDTO.shipmentRoute = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "shipmentRoute");
	
	Пока Маршруты.Следующий() Цикл
		
		routePoint = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO.shipmentRoute, "routePoint");
		routePoint.sqnId = Маршруты.НомерСтроки;
		
		Если ЗначениеЗаполнено(Маршруты.Предприятие) Тогда
			routePoint.enterprise = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(routePoint, "enterprise");
			routePoint.enterprise.guid = Маршруты.ПредприятиеИдентификатор;
		Иначе
			routePoint.location = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(routePoint, "location");
	...
	Если РезультатДобавленияЗаписей.ВходящееСообщение.НовоеСообщение Тогда
		...
		Если ДанныеДокумента.Объект <> Неопределено Тогда
			...
			Если СтрокиДляГенерацииСерий.Количество() > 0 Тогда
				ПараметрыУказанияСерий = ГосударственныеИнформационныеСистемыПереопределяемый.ПараметрыУказанияСерий(
			...
			Если ДокументОбъект.Товары.Количество() > 1
				...
				Для Каждого СтрТовары из ДокументОбъект.Товары Цикл
					Если Не ЗначениеЗаполнено(СтрТовары.ЗаписьСкладскогоЖурнала) Тогда
						ИзменитьСтатус = Ложь;
						Прервать;
					КонецЕсли;
				КонецЦикла;
...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	...
	ИменаРеквизитов = ГосударственныеИнформационныеСистемыПереопределяемый.ИменаРеквизитовДляЗаполненияПараметровУказанияСерий(
						ТипДокумента);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ТипДокумента = ТипДокумента();
	ПараметрыУказанияСерий = ГосударственныеИнформационныеСистемыПереопределяемый.ПараметрыУказанияСерий(ТипДокумента,
																										Объект);
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	ТипДокумента = ТипДокумента();
	ТекстЗапроса = ГосударственныеИнформационныеСистемыПереопределяемый.ТекстЗапросаЗаполненияСтатусовУказанияСерий(
					ТипДокумента, ПараметрыУказанияСерий);
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = ИнтеграцияВЕТИСПереопределяемый.ТекстЗапросаДвижениеСерийТоваров(
		Метаданные.Документы.ВходящаяТранспортнаяОперацияВЕТИС.Имя);
...
Процедура ОбработатьВСДПакетом(СообщенияXML, ДокументСсылка, ДанныеДокумента, Знач НастройкаОбмена, Знач ВспомогательныеДанные)
	...
	Для каждого НомерПакета Из НомераПакетов Цикл
		...
		Если ТекущийНомерПакета <> НомерПакета Тогда
			...
			ДокументXDTO = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(
...
Функция ЗапросНаОформлениеВходящейПартииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	Для каждого ДанныеСтроки Из ТоварыОбрабатываемыеПоОтдельности Цикл
		...
		Если ЗначениеЗаполнено(Шапка.НомерАктаНесоответствия) Тогда
			...
			АктНесоответствия = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "discrepancyReport");
...
Функция ДанныеВходящейПартии(ДокументСсылка, РазделитьПоПакетам = Ложь)
	...
	Результат = ИнтеграцияГосИС.ВыполнитьПакетЗапросов(Запрос, СписокЗапросов, Истина);
...
Процедура ЗагрузитьСведенияВСДПоРезультатамЗапросаВЕТИС(ДанныеДокумента, ПараметрыОбмена, Предприятие, ХозяйствующийСубъект, ДокументОбъект)
	...
	Для Каждого ДанныеВСД Из ЗагруженныеВСД Цикл
		...
		ДанныеСтроки = ДокументОбъект.Товары.Найти(ВСДСсылка,
...
Функция ПолучитьДанныеДляПечатнойФормыАктаОНесоответствии(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВходящаяТранспортнаяОперацияВЕТИСТоварыУточнение.КоличествоВЕТИС ЕСТЬ NULL
	|			ТОГДА ВходящаяТранспортнаяОперацияВЕТИСТовары.КоличествоВЕТИС - ВходящаяТранспортнаяОперацияВЕТИСТовары.ВозвращаемоеКоличествоВЕТИС
	|		ИНАЧЕ ВходящаяТранспортнаяОперацияВЕТИСТоварыУточнение.КоличествоВЕТИС
	|	КОНЕЦ КАК КоличествоФакт,
	|	ПРЕДСТАВЛЕНИЕ(ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.Продукция) КАК ПродукцияПланПредставление,
	...
	|	ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.ЕдиницаИзмеренияВЕТИС КАК ЕдиницаИзмеренияПлан,
	|	ПРЕДСТАВЛЕНИЕ(ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.ЕдиницаИзмеренияВЕТИС) КАК ЕдиницаИзмеренияПланПредставление,
	|	ПРЕДСТАВЛЕНИЕ(ВходящаяТранспортнаяОперацияВЕТИСТовары.ЕдиницаИзмеренияВЕТИС) КАК ЕдиницаИзмеренияФактПредставление,
	|	ПРЕДСТАВЛЕНИЕ(ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.ГрузоотправительХозяйствующийСубъект) КАК ВладелецГруза,
	|	ПРЕДСТАВЛЕНИЕ(ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.ГрузоотправительХозяйствующийСубъект.ИНН) КАК ВладелецГрузаИНН,
	...
	|	ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.СрокГодностиСтрока КАК СрокГодностиСтрокаПлан,
	|	ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.СрокГодностиТочностьЗаполнения КАК СрокГодностиТочностьЗаполненияПлан,
	|	ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.СрокГодностиНачалоПериода КАК СрокГодностиНачалоПериодаПлан,
	|	ВходящаяТранспортнаяОперацияВЕТИСТовары.ВетеринарноСопроводительныйДокумент.СрокГодностиКонецПериода КАК СрокГодностиКонецПериодаПлан,
	...
	|ИЗ
	|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ВходящаяТранспортнаяОперацияВЕТИСТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВходящаяТранспортнаяОперацияВЕТИС.ТоварыУточнение КАК ВходящаяТранспортнаяОперацияВЕТИСТоварыУточнение
	|		ПО ВходящаяТранспортнаяОперацияВЕТИСТовары.Ссылка = ВходящаяТранспортнаяОперацияВЕТИСТоварыУточнение.Ссылка
	|			И ВходящаяТранспортнаяОперацияВЕТИСТовары.ИдентификаторСтроки = ВходящаяТранспортнаяОперацияВЕТИСТоварыУточнение.ИдентификаторСтроки
	|ГДЕ
	|	ВходящаяТранспортнаяОперацияВЕТИСТовары.Ссылка В(&Ссылка)
	|ИТОГИ ПО
	|	Ссылка
	...
	|	ЕСТЬNULL(ВходящаяТранспортнаяОперацияВЕТИСУпаковкиВЕТИС.УпаковкаВЕТИС, ЗНАЧЕНИЕ(Справочник.УпаковкиВЕТИС.ПустаяСсылка)) КАК УпаковкаВЕТИС,
	|	ПРЕДСТАВЛЕНИЕ(ЕСТЬNULL(ВходящаяТранспортнаяОперацияВЕТИСУпаковкиВЕТИС.УровеньУпаковки, """")) КАК УровеньУпаковкиПредставление,
	|	ПРЕДСТАВЛЕНИЕ(ЕСТЬNULL(ВходящаяТранспортнаяОперацияВЕТИСУпаковкиВЕТИС.УпаковкаВЕТИС, """")) КАК УпаковкаВЕТИСПредставление,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВходящаяТранспортнаяОперацияВЕТИС.УпаковкиВЕТИС КАК ВходящаяТранспортнаяОперацияВЕТИСУпаковкиВЕТИС
	...
	|	ЕСТЬNULL(ВетеринарноСопроводительныйДокументВЕТИСУпаковкиВЕТИС.УпаковкаВЕТИС, ЗНАЧЕНИЕ(Справочник.УпаковкиВЕТИС.ПустаяСсылка)),
	...
	|ИЗ
	|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ИсходныеВСД
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВетеринарноСопроводительныйДокументВЕТИС.УпаковкиВЕТИС КАК ВетеринарноСопроводительныйДокументВЕТИСУпаковкиВЕТИС
	...
	|ИЗ
	|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ИсходныеВСД
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВетеринарноСопроводительныйДокументВЕТИС.ПроизводственныеПартии КАК ВетеринарноСопроводительныйДокументВЕТИСПроизводственныеПартии
	...
	|ГДЕ
	|	ИсходныеВСД.Ссылка В(&Ссылка)
	|	И НЕ ИсходныеВСД.ЕстьУточнения
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ИсходныеВСД.Ссылка,
	|	ИСТИНА,
	|	ЕСТЬNULL(ВходящаяТранспортнаяОперацияВЕТИСПроизводственныеПартии.ИдентификаторПартии, """")
	|ИЗ
	|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ИсходныеВСД
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВходящаяТранспортнаяОперацияВЕТИС.ТоварыУточнение КАК ВходящаяТранспортнаяОперацияВЕТИСПроизводственныеПартии
	|		ПО ИсходныеВСД.Ссылка = ВходящаяТранспортнаяОперацияВЕТИСПроизводственныеПартии.Ссылка
	|			И ИсходныеВСД.ИдентификаторСтроки = ВходящаяТранспортнаяОперацияВЕТИСПроизводственныеПартии.ИдентификаторСтроки
	|ГДЕ
	|	ИсходныеВСД.Ссылка В(&Ссылка)
	|	И ВходящаяТранспортнаяОперацияВЕТИСПроизводственныеПартии.Ссылка В(&Ссылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	ИдентификаторСтроки,
	...
	|ИЗ
	|	Документ.ВходящаяТранспортнаяОперацияВЕТИС.Товары КАК ИсходныеВСД
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВетеринарноСопроводительныйДокументВЕТИС.Производители КАК ВетеринарноСопроводительныйДокументВЕТИСПроизводители
...
Функция ТипДокумента()
	...
	ТипДокумента = Метаданные.Документы.ВходящаяТранспортнаяОперацияВЕТИС;
