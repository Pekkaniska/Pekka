...
Функция СтатусПослеПередачиДанных(ДокументСсылка, Операция, СтатусОбработки) Экспорт
	...
	Если Операция = Перечисления.ВидыОперацийВЕТИС.ЗапросНаОформлениеТранспортнойПартии Тогда
		...
		СтатусыБазовыйПроцесс.ОшибкаДействия.Добавить(Перечисления.ДальнейшиеДействияПоВзаимодействиюВЕТИС.Оформите);
...
Функция ИменаРеквизитовДляЗаполненияПараметровУказанияСерий() Экспорт
	...
	ИменаРеквизитов = 
		ГосударственныеИнформационныеСистемыПереопределяемый.ИменаРеквизитовДляЗаполненияПараметровУказанияСерий(
			ТипДокумента);
...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ПараметрыУказанияСерий = 
		ГосударственныеИнформационныеСистемыПереопределяемый.ПараметрыУказанияСерий(
			ТипДокумента, Объект);
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерий(ПараметрыУказанияСерий) Экспорт
	...
	ТипДокумента = ТипДокумента();
	ТекстЗапроса = 
		ГосударственныеИнформационныеСистемыПереопределяемый.ТекстЗапросаЗаполненияСтатусовУказанияСерий(
...
Функция ЗапросНаОформлениеТранспортнойПартииXML(ДокументСсылка, ПараметрыПередачи, ДополнительныеПараметры)
	...
	ВспомогательныеДанные.Вставить("ХранилищеВременныхДат", Новый Соответствие);
		...
		СведенияОПоставке = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ДокументXDTO, "delivery");
...
Процедура ЗагрузитьСведенияВСДПоРезультатамЗапросаВЕТИС(ДанныеДокумента, ИзмененныеЗаписиЖурнала, ПараметрыОбмена, Предприятие, ХозяйствующийСубъект, ДокументОбъект)
	...
	Для Каждого ДанныеВСД Из ЗагруженныеВСД Цикл
		...
		ПараметрыОтбора.Вставить("ОсобыеОтметки",              ВСДОбъект.ОсобыеОтметки);
		...
		Для Каждого СтрокаТЧ Из НайденныеСтроки Цикл
			...
			Если ЗначениеЗаполнено(СтрокаТЧ.ВетеринарноСопроводительныйДокумент) 
				ИЛИ ИзмененныеЗаписиЖурнала.Найти(СтрокаТЧ.ЗаписьСкладскогоЖурнала) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если ДокументОбъект.Товары.Найти(ВСДСсылка, "ВетеринарноСопроводительныйДокумент") <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
...
Функция ТекстЗапросаТаблицаДвиженияСерийТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = ИнтеграцияВЕТИСПереопределяемый.ТекстЗапросаДвижениеСерийТоваров(
		Метаданные.Документы.ИсходящаяТранспортнаяОперацияВЕТИС.Имя);
...
Функция ОбновитьДвиженияЗаписиСкладскогоЖурналаВРезервеВЕТИС(ДокументСсылка) Экспорт
...
Функция ТипДокумента()
	...
	ТипДокумента = Метаданные.Документы.ИсходящаяТранспортнаяОперацияВЕТИС;
