...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользуетсяКомиссияИлиПереработка = ИнтеграцияВЕТИСПереопределяемый.ИспользуетсяКомиссияПриЗакупкахИлиПереработкаДавальческогоСырья();
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Иначе
		...
		СобытияФормВЕТИСКлиентПереопределяемый.ОбработкаВыбораПодборНоменклатуры(ОписаниеОповещения, ВыбранноеЗначение,ИсточникВыбора);
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		ЗаполнитьРеквизитыПоУмолчаниюНовойСтрокиТовары(Элементы.Товары.ТекущиеДанные, Копирование);
...
Процедура ЗаписатьДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ИсходящаяТранспортнаяОперацияВЕТИС.Форма.ФормаДокумента.Записать");
...
Процедура ПровестиДокумент(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ИсходящаяТранспортнаяОперацияВЕТИС.Форма.ФормаДокумента.Провести");
...
Процедура ПровестиИЗакрыть(Команда)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Документ.ИсходящаяТранспортнаяОперацияВЕТИС.Форма.ФормаДокумента.ПровестиИЗакрыть");
...
Процедура ОткрытьПодборТоваров(Команда)
	...
	ПараметрыПодбора = ИнтеграцияВЕТИСКлиентСерверПереопределяемый.ПараметрыФормыПодбораТоваров(ЭтаФорма,
																								ПараметрыУказанияСерий);
...
Процедура РасчетСтрокиТоварыНаСервере(ДанныеСтроки, Действия, ДополнительныеПараметры = Неопределено)
	...
	Перем ПараметрыДополненияСтатусаВЕТИС, ПараметрыРегионализацииПоВидамНоменклатуры, ЭтоНоваяСтрока, 
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, ИдентификаторСтроки = Неопределено, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ПанельВСДСвернута")
		...
		Элементы.ГруппаСтрокаТовары.Видимость = Не Форма.ПанельВСДСвернута;
	...
	Если СтруктураРеквизитов.Свойство("ГрузоотправительХозяйствующийСубъект")
		...
		Если Не ЗначениеЗаполнено(Форма.ГрузоотправительСопоставлениеХСДляОтбораОснований)
		...
		Если Не ЗначениеЗаполнено(Форма.ГрузоотправительСопоставлениеПредприятияДляОтбораОснований)
			И Не ЗначениеЗаполнено(Объект.ГрузоотправительПредприятие)
			И ЗначениеЗаполнено(Форма.ГрузоотправительИзОснованияДляОтбораПредприятий) Тогда
		...
		Объект.ТорговыйОбъект = ИнформацияГрузоотправителя.ТорговыйОбъект.Ссылка;
	...
	Если СтруктураРеквизитов.Свойство("ГрузополучательХозяйствующийСубъект")
		...
		Если Не ЗначениеЗаполнено(Форма.ГрузополучательСопоставлениеХСДляОтбораОснований)
		...
		Если Не ЗначениеЗаполнено(Форма.ГрузополучательСопоставлениеПредприятияДляОтбораОснований)
			И Не ЗначениеЗаполнено(Объект.ГрузополучательПредприятие)
			И ЗначениеЗаполнено(Форма.ГрузополучательИзОснованияДляОтбораПредприятий) Тогда
	...
	Если СтруктураРеквизитов.Свойство("Товары") И ТекущиеДанные<> Неопределено Тогда
		Элементы.ГруппаЖивотныеНаТерриторииТС.Видимость = ТекущиеДанные.ТипЖивыеЖивотные;
		Элементы.ТоварыПредставлениеУпаковкиШтрихкоды.Видимость = НЕ ТекущиеДанные.ТипЖивыеЖивотные;
		Элементы.ТоварыТипПроисхождения.Видимость = ТекущиеДанные.ТребуетсяТипПроисхождения;
	КонецЕсли;
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения, ИдентификаторСтрокиТовары)
	...
	Если НЕ(ИдентификаторСтрокиТовары = Неопределено) Тогда 
	...
	Если ДополнительныеПараметры.Свойство("ТоварыЗаписьСкладскогоЖурналаДобавление") 
		...
		ИнтеграцияВЕТИС.ЗаполнитьСопоставленныеТовары(Объект.Товары,
													...
													Истина);
		ГосударственныеИнформационныеСистемыПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтотОбъект,
																									Объект.Товары);
...
Процедура ОбновитьДанные(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СтруктураРеквизитовИзменяемыхНаСервере().Свойство(ИмяОперации) Тогда
		ИмяОперации = Новый Структура(ИмяОперации+",Объект",,Объект);
		Если ИзменяемаяСтрока = Неопределено Тогда
			ИдентификаторСтроки = Неопределено;
		Иначе 
			ИдентификаторСтроки = ИзменяемаяСтрока.ПолучитьИдентификатор();
		КонецЕсли;
		ОбновитьДанныеНаСервере(Результат, ИмяОперации, КэшированныеЗначения, ИдентификаторСтроки);
		Возврат;
	КонецЕсли;
	...
	Если ИмяОперации = "ТоварыНоменклатура" Тогда
		...
		СобытияФормВЕТИСКлиентПереопределяемый.ПриИзмененииНоменклатуры(
			ЭтотОбъект, 
			ИзменяемаяСтрока, 
			КэшированныеЗначения,
			ПараметрыЗаполнения, 
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормВЕТИСПереопределяемый.УстановитьУсловноеОформлениеСерийНоменклатуры(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьРеквизитыПоУмолчаниюНовойСтрокиТовары(СтрокаТабличнойЧасти, Копирование = Ложь)
...
Процедура ПерезаполнитьПоОснованиюНаСервере(ОбработатьТабличнуюЧасть = Истина, ОбработатьШапку = Истина)
	...
	Если ОбработатьШапку Тогда
		...
		ДанныеТТН = ИнтеграцияВЕТИСПереопределяемый.ДанныеТТНДляТранспортнойОперацииПоДокументуОснованию(Объект.ДокументОснование);
...
Процедура ЗаполнитьСтрокуПоДаннымЗаписиСкладскогоЖурнала(СтрокаТабличнойЧасти)
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.КоличествоВЕТИС)
			И(СтрокаТабличнойЧасти.Продукция = Выборка.Продукция ИЛИ НЕ ЗначениеЗаполнено(Выборка.Продукция)) Тогда
			РеквизитыДляЗаполнения = "Цель, НизкокачественнаяПродукция, ПериодНахожденияЖивотныхНаТерриторииТС, КоличествоПериодовНахожденияЖивотныхНаТерриторииТС, ТипПроисхождения";
		Иначе
			РеквизитыДляЗаполнения = "Продукция, Цель, ЕдиницаИзмеренияВЕТИС, НизкокачественнаяПродукция, ПериодНахожденияЖивотныхНаТерриторииТС, КоличествоПериодовНахожденияЖивотныхНаТерриторииТС, ТипПроисхождения, КоличествоВЕТИС";
		КонецЕсли;
...
Процедура ОбработкаРезультатаПодбораНоменклатурыНаСервере(ВыбранноеЗначение, ДополнительныеПараметры)
	...
	ПараметрыЗаполнения.ДанныеЗаполнения.Вставить("БлагополучиеМестности",Элементы.ТоварыБлагополучиеМестности.СписокВыбора[0].Значение);
...
Процедура ПодготовитьЗаполнитьУстановитьВидимостьСерий()
	...
	ПараметрыУказанияСерий = 
		ГосударственныеИнформационныеСистемыПереопределяемый.ПараметрыУказанияСерийФормыОбъекта(
