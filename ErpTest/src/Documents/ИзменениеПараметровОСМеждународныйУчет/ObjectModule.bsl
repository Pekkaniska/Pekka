...
Процедура ПриЗаписи(Отказ)
	...
	Если Не Отказ И ДополнительныеСвойства.РежимЗаписи <> РежимЗаписиДокумента.Проведение Тогда
		...
		Документы.ИзменениеПараметровОСМеждународныйУчет.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства, "РеестрДокументов,ДокументыПоОС");
...
Процедура ПроверитьВозможностьИзмененияПараметров(Отказ=Ложь)
	...
	Для Каждого Строка Из ОсновныеСредства Цикл
		...
		Иначе
			ТекстОшибки = ТекстОшибки
				+ СтрЗаменить(НСтр("ru='Объект принят к учету в организации ""%1""'"), "%1", Данные.Организация);
		КонецЕсли;
