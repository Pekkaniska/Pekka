...
Процедура ИнициализироватьКлючиАналитикиНоменклатуры(Запрос)
	...
	|		ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)	КАК СтатьяКалькуляции
	|	ГДЕ
	|		НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов) КАК ТаблицаТовары
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		ПО СпрСклады.Ссылка = АналитикаПоПолучателю.Склад
	|
	...
	|			И АналитикаПоПолучателюБезНазначения.Склад = АналитикаПоПолучателю.Склад
	...
	|		И &Подразделение = АналитикаПоПодразделению.Склад
	...
	|		И АналитикаПоПодразделению.Склад = АналитикаПоПодразделениюБезНазначения.Склад
	...
	|	ВЫБОР
	|		КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|			ТОГДА ПартииПроизводства.ВидЗапасов
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ																				КАК ВидЗапасов,
	|	ВЫБОР
	|		КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПолучателю
	|		ПО АналитикаНоменклатурыПоПолучателю.КлючАналитики = ПартииПроизводства.АналитикаУчетаНоменклатуры
	|
	...
	|			И АналитикаНоменклатурыПоПолучателюБезНазначения.Склад = АналитикаНоменклатурыПоПолучателю.Склад
	...
	|			И АналитикаНоменклатурыПоПартии.Склад = &Подразделение
	...
	|			И АналитикаНоменклатурыПоПартииБезНазначения.Склад = &Подразделение
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПолучателю
	|		ПО АналитикаНоменклатурыПоПолучателю.КлючАналитики = &АналитикаУчетаНоменклатуры
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПолучателюБезНазначения
	|		ПО АналитикаНоменклатурыПоПолучателюБезНазначения.Номенклатура = АналитикаНоменклатурыПоПолучателю.Номенклатура
	|			И АналитикаНоменклатурыПоПолучателюБезНазначения.Характеристика = АналитикаНоменклатурыПоПолучателю.Характеристика
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// приход возвратных отходов (товаров) по правилам
	|ВЫБРАТЬ
	|	&Дата																				КАК Период,
	|	&Ссылка																				КАК ДокументДвижения,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)												КАК ВидДвижения,
	...
	|			ТОГДА АналитикаНоменклатурыПоПартииБезНазначения.КлючАналитики
	|	КОНЕЦ																				КАК АналитикаУчетаНоменклатуры,
	|	ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22
	|				И &ФИФОСкользящаяОценка
	...
	|		ПО АналитикаПоПолучателю.КлючАналитики = ПартииПроизводства.АналитикаУчетаНоменклатуры
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПартии
	|		ПО АналитикаНоменклатурыПоПартии.Номенклатура = &Номенклатура
	|			И АналитикаНоменклатурыПоПартии.Характеристика = &Характеристика
	|			И АналитикаНоменклатурыПоПартии.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			И АналитикаНоменклатурыПоПартии.Склад = &Подразделение
	|			И АналитикаНоменклатурыПоПартии.Назначение = &Назначение
	|			И АналитикаНоменклатурыПоПартии.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПартииБезНазначения
	|		ПО АналитикаНоменклатурыПоПартииБезНазначения.Номенклатура = &Номенклатура
	|			И АналитикаНоменклатурыПоПартииБезНазначения.Характеристика = &Характеристика
	...
	|			И АналитикаНоменклатурыПоПартииБезНазначения.Склад = &Подразделение
	|			И АналитикаНоменклатурыПоПартииБезНазначения.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|			И АналитикаНоменклатурыПоПартииБезНазначения.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|ГДЕ
	|	ПартииПроизводства.Ссылка = &Ссылка
	|	И ПартииПроизводства.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
	|	И НЕ &КоличествоПоПравилу = 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// приход возвратных отходов (работ) по правилам
	|ВЫБРАТЬ
	|	&Дата																				КАК Период,
	|	&Ссылка																				КАК ДокументДвижения,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)												КАК ВидДвижения,
	...
	|	ВЫБОР
	|		КОГДА &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|			ТОГДА АналитикаНоменклатурыПоПравилам.КлючАналитики
	|		ИНАЧЕ АналитикаНоменклатурыПоПравиламБезНазначения.КлючАналитики
	|	КОНЕЦ																				КАК АналитикаУчетаНоменклатуры,
	|	ВЫБОР
	|		КОГДА &ПартионныйУчетВерсии22
	|				И &ФИФОСкользящаяОценка
	...
	|	НЕОПРЕДЕЛЕНО																		КАК АналитикаФинансовогоУчета,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(АналитикаНоменклатурыПоПравилам.Назначение.ВидДеятельностиНДС,ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) <>
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|			ТОГДА АналитикаНоменклатурыПоПравилам.Назначение.ВидДеятельностиНДС
	|		ИНАЧЕ &ВыпускПодДеятельность
	|	КОНЕЦ																				КАК ВидДеятельностиНДС,
	|	НЕОПРЕДЕЛЕНО																		КАК КорРазделУчета,
	|	НЕОПРЕДЕЛЕНО																		КАК КорОрганизация,
	...
	|	КОНЕЦ																				КАК СтоимостьУпр
	|ИЗ
	|	РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПравилам
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНоменклатурыПоПравиламБезНазначения
	|		ПО АналитикаНоменклатурыПоПравиламБезНазначения.Номенклатура = &Номенклатура
	|			И АналитикаНоменклатурыПоПравиламБезНазначения.Характеристика = &Характеристика
	|			И АналитикаНоменклатурыПоПравиламБезНазначения.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			И АналитикаНоменклатурыПоПравиламБезНазначения.Склад = &Подразделение
	|			И АналитикаНоменклатурыПоПравиламБезНазначения.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|			И АналитикаНоменклатурыПоПравиламБезНазначения.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|ГДЕ
	|	НЕ &КоличествоПоПравилу = 0
	|	И АналитикаНоменклатурыПоПравилам.Номенклатура = &Номенклатура
	|	И АналитикаНоменклатурыПоПравилам.Характеристика = &Характеристика
	|	И АналитикаНоменклатурыПоПравилам.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	И АналитикаНоменклатурыПоПравилам.Склад = &Подразделение
	|	И АналитикаНоменклатурыПоПравилам.Назначение = &Назначение
	|	И АналитикаНоменклатурыПоПравилам.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	И &ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)";
...
Процедура ЗарегистрироватьДанныеКОбновлениюКлючейАналитики(Параметры) Экспорт
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
	|		ПО Реквизиты.Номенклатура = Аналитика.Номенклатура
	|		И Реквизиты.Характеристика = Аналитика.Характеристика
	|		И ВЫБОР
	|			КОГДА Реквизиты.СтатусУказанияСерий = 14
	|				ТОГДА Реквизиты.Серия
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.Пустаяссылка)
	|		КОНЕЦ = Аналитика.Серия
	|		И Реквизиты.Получатель = Аналитика.Склад
...
// Обработчик обновления документа, по реквизитам генерирует недостающие ключи аналитики учета номенклатуры в ИБ.
//
Процедура СгенерироватьКлючиАналитикиНоменклатуры(Параметры) Экспорт
	
	МетаданныеОбъекта = Метаданные.Документы.РаспределениеВозвратныхОтходов;
	...
	Выборка = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(Параметры.Очередь, ПолноеИмяОбъекта, ДополнительныеПараметрыВыборкиДанных);
	Пока Выборка.Следующий() Цикл
		НачатьТранзакцию();
		Попытка
...
// заполняет ТЧ ВидыЗапасовВыходныеИзделия на основании ТЧ ВыходныеИзделия
// заполняет ТЧ ВидыЗапасовПобочныеИзделия на основании ТЧ ПобочныеИзделия.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеВозвратныхОтходов.ВидыЗапасов КАК ТаблицаВидовЗапасов
	|		ПО РаспределениеВозвратныхОтходов.Ссылка = ТаблицаВидовЗапасов.Ссылка
	|ГДЕ
	|	РаспределениеВозвратныхОтходов.Проведен
	|	И НЕ РаспределениеВозвратныхОтходов.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|	И НЕ РаспределениеВозвратныхОтходов.УдалитьВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|	И ТаблицаВидовЗапасов.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Перенос партии производства в новый реквизит (таблица ПартииПроизводства)
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаТоваров.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.РаспределениеВозвратныхОтходов.ПартииПроизводства КАК ТаблицаТоваров
	|ГДЕ
	|	ТаблицаТоваров.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
	|	И НЕ ТаблицаТоваров.УдалитьПартияПроизводства = НЕОПРЕДЕЛЕНО
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Перенос партии производства в новый реквизит (таблица ВидыЗапасов)
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаТоваров.Ссылка КАК Ссылка
	|ИЗ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеВозвратныхОтходов КАК Реквизиты
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.РаспределениеВозвратныхОтходов.ПартииПроизводства КАК ТаблицаПартий
	|			ГДЕ
	|				ОбъектыДляОбработки.Ссылка = ТаблицаПартий.Ссылка
	|				И ТаблицаПартий.ПартияПроизводства = ЗНАЧЕНИЕ(Справочник.ПартииПроизводства.ПустаяСсылка)
	...
	|	ВЫБОР
	|		КОГДА ПеренестиПартию.Ссылка ЕСТЬ НЕ NULL
	|			ТОГДА ПеренестиПартию.ВерсияДанных
	|		ИНАЧЕ Реквизиты.ВерсияДанных
	|	КОНЕЦ									КАК ВерсияДанных,
	|	ПеренестиПартию.Ссылка ЕСТЬ НЕ NULL		КАК ПеренестиПартию,
	|	Реквизиты.Номенклатура.ТипНоменклатуры	КАК ТипНоменклатуры
	|ИЗ
	|	ВТОбъектыДляОбработки КАК ВТОбъектыДляОбработки
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.РаспределениеВозвратныхОтходов КАК Реквизиты
	|		ПО ВТОбъектыДляОбработки.Ссылка = Реквизиты.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК ПеренестиПартию
	|		ПО ПеренестиПартию.Ссылка = ВТОбъектыДляОбработки.Ссылка
	|		И ПеренестиПартию.НомерУсловия = 1
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЗаполнитьЗначенияСвойств(ПараметрыАналитики, ДокументОбъект);
			ПараметрыАналитики.Склад = ДокументОбъект.Получатель;
			ДокументОбъект.АналитикаУчетаНоменклатуры = РегистрыСведений.АналитикаУчетаНоменклатуры.ЗначениеКлючаАналитики(ПараметрыАналитики);
			
			СуммаИтог = 0;
			ДокументОбъект.ВидыЗапасов.Очистить();
			
			Если Не Выборка.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
				
				Если Не ДокументОбъект.КоличествоПоПравилу = 0 Тогда
					НоваяСтрока = ДокументОбъект.ВидыЗапасов.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ДокументОбъект);
					НоваяСтрока.ВидЗапасов	= ДокументОбъект.УдалитьВидЗапасов;
					НоваяСтрока.Количество	= ДокументОбъект.КоличествоПоПравилу;
					НоваяСтрока.Сумма		= ДокументОбъект.КоличествоПоПравилу * ДокументОбъект.Сумма / ДокументОбъект.Количество;
					СуммаИтог				= СуммаИтог + НоваяСтрока.Сумма;
				КонецЕсли;
				
				Для Каждого ТекСтрока Из ДокументОбъект.ПартииПроизводства Цикл
					
					НоваяСтрока = ДокументОбъект.ВидыЗапасов.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
					НоваяСтрока.АналитикаУчетаНоменклатуры = ДокументОбъект.АналитикаУчетаНоменклатуры;
					НоваяСтрока.ВидЗапасов = ДокументОбъект.УдалитьВидЗапасов;
					НоваяСтрока.Получатель = ДокументОбъект.Получатель;
					
					Если ТекСтрока.НомерСтроки = ДокументОбъект.ПартииПроизводства.Количество() Тогда
						НоваяСтрока.Сумма	= ДокументОбъект.Сумма - СуммаИтог;
					Иначе
						НоваяСтрока.Сумма	= ТекСтрока.Количество * ДокументОбъект.Сумма / ДокументОбъект.Количество;
						СуммаИтог			= СуммаИтог + НоваяСтрока.Сумма;
					КонецЕсли;
			...
			Если Выборка.ПеренестиПартию Тогда
				ГруппыЗатратСервер.ПеренестиПартиюПроизводства(ДокументОбъект.ПартииПроизводства);
				ГруппыЗатратСервер.ПеренестиПартиюПроизводства(ДокументОбъект.ВидыЗапасов);
			КонецЕсли;
