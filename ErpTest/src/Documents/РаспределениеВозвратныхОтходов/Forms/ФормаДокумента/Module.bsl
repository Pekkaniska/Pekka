...
Процедура ВалютаПриИзмененииСервер(КэшированныеЗначения)
	...
	// Пересчитаем цену
	Объект.Цена = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(Объект.Цена,
...
Процедура ПрочиеРасходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Не Копирование Тогда
		ПриДобавлениСтрокиПрочиеРасходы();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПриДобавлениСтрокиПрочиеРасходы()
...
Процедура ПрочиеРасходыСтатьяРасходовПриИзменении(Элемент)
	...
	ПрочиеРасходыСтатьяРасходовПриИзмененииНаСервере(КэшированныеЗначения);
...
Процедура ПрочиеРасходыСтатьяРасходовНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрочиеРасходыСтатьяРасходовВыборЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ПрочиеРасходыСтатьяРасходовВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ПрочиеРасходыСтатьяРасходовПриИзмененииНаСервере(КэшированныеЗначения)
	
	СтрокаТаблицы = Объект.ПрочиеРасходы.НайтиПоИдентификатору(Элементы.ПрочиеРасходы.ТекущаяСтрока);
	СтруктураДействий = Новый Структура;
	ДобавитьВСтруктуруДействияПриСтатьиРасходов(СтруктураДействий, СтрокаТаблицы);
...
Процедура ПрочиеРасходыЗаполнитьПодразделение(Команда)
	...
	Если Объект.ПрочиеРасходы.Количество() = 0 ИЛИ НЕ ЗначениеЗаполнено(ВыделенныеСтроки) Тогда
		ТекстПредупреждения = НСтр("ru = 'Для выполнения команды требуется выделить строки табличной части.'");
...
Процедура ПрочиеРасходыЗаполнитьСтатьюРасходов(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрочиеРасходыЗаполнитьСтатьюРасходовЗавершение", ЭтотОбъект, Новый Структура("ВыделенныеСтроки", ВыделенныеСтроки));
...
&НаКлиенте
Процедура ПрочиеРасходыЗаполнитьСтатьюРасходовЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		Для Каждого Строка Из ВыделенныеСтроки Цикл
			СтрокаТаблицы = Объект.ПрочиеРасходы.НайтиПоИдентификатору(Строка);
			Если СтрокаТаблицы <> Неопределено Тогда
				СтрокаТаблицы.СтатьяРасходов = Результат.СтатьяРасходов;
				СтрокаТаблицы.АналитикаРасходов = Результат.АналитикаРасходов;
			КонецЕсли;
		КонецЦикла;
		ПрочиеРасходыПослеЗавершенияОбработки(ВыделенныеСтроки, КэшированныеЗначения);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПрочиеРасходыЗаполнитьСтатьюАктивовПассивов(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПрочиеРасходыЗаполнитьСтатьюАктивовПассивовЗавершение", ЭтотОбъект, Новый Структура("ВыделенныеСтроки", ВыделенныеСтроки));
...
&НаКлиенте
Процедура ПрочиеРасходыЗаполнитьСтатьюАктивовПассивовЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		Для Каждого Строка Из ВыделенныеСтроки Цикл
			СтрокаТаблицы = Объект.ПрочиеРасходы.НайтиПоИдентификатору(Строка);
			Если СтрокаТаблицы <> Неопределено Тогда
				СтрокаТаблицы.СтатьяРасходов = Результат.Статья;
				СтрокаТаблицы.АналитикаАктивовПассивов = Результат.Аналитика;
			КонецЕсли;
		КонецЦикла;
		ПрочиеРасходыПослеЗавершенияОбработки(ВыделенныеСтроки, КэшированныеЗначения);
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьВСтруктуруДействияПриСтатьиРасходов(СтруктураДействий, СтрокаТаблицы)
	...
	ИначеЕсли ТипЗнч(СтрокаТаблицы.СтатьяРасходов) = Тип("ПланВидовХарактеристикСсылка.СтатьиРасходов") Тогда
...
&НаСервере
Процедура ЗаполнитьСлужебныеРеквизитыПрочиеРасходы()
...
&НаСервере
Процедура ПрочиеРасходыПослеЗавершенияОбработки(ВыделенныеСтроки, КэшированныеЗначения)
...
Процедура ЗаполнитьСлужебныеРеквизитыФормы()
	...
	ЗаполнитьСлужебныеРеквизитыПрочиеРасходы();
