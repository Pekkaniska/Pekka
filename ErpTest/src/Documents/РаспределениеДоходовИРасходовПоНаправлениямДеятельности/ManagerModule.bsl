...
Процедура СоздатьДокументыЗаПериод(Период, Организация = Неопределено, Отказ = Ложь) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РегламентнаяОперация КАК ВТДокументы
	|		ПО ВТОрганизации.Организация = ВТДокументы.Организация
	|
	|ГДЕ
	|	ВТДокументы.ТипОперации В (
	...
	|	И ВТДокументы.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	...
	//++ НЕ УТ
	ДокументыСписания = РезультатЗапроса[Индекс - 1].Выбрать();
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	//++ НЕ УТ
	РассчетныеДоли = НалоговыйУчет.ДолиСписанияРасходов(Реквизиты.Период, Реквизиты.Организация, Ложь, Истина);
	ДоляТранспортныхРасходов = РассчетныеДоли.ДоляТранспортныхРасходов;
	//-- НЕ УТ
...
Процедура РезультатРаспределенияПоБазе(Запрос, ТекстыЗапроса, ДополнительныеСвойства)
	...
	//++ НЕ УТ
	ЗапросРаспределения.УстановитьПараметр("ВидыРасходовНормируемые", Запрос.Параметры.ВидыРасходовНормируемые);
	//-- НЕ УТ
