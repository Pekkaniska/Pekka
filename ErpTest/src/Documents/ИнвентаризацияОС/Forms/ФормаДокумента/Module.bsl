...
Процедура ЗаполнитьКэшированныеСчетаУчета()
	...
	СчетаГруппы08.Добавить(ПланыСчетов.Хозрасчетный.ПодготовкаКВводуВЭксплуатацию);
	СчетаГруппы08.Добавить(ПланыСчетов.Хозрасчетный.ПриобретениеЗемельныхУчастков);
	СчетаГруппы08.Добавить(ПланыСчетов.Хозрасчетный.ПриобретениеОбъектовПриродопользования);
...
Процедура ИнициализироватьСтруктуруДействий()
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(Объект.Дата) Тогда
		Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.Перемещение, "ПеремещениеОС2_4");
		Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.Списание, "СписаниеОС2_4");
		Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.ПринятиеКУчету, "ПринятиеКУчетуОС2_4");
	Иначе
		Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.Перемещение, "ПеремещениеОС");
		Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.Списание, "СписаниеОС");
		Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.ПринятиеКУчету, "ПринятиеКУчетуОС");
	КонецЕсли;
	Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.ВыбытиеАрендованныхОС, "ВыбытиеАрендованныхОС");
	Соответствие.Вставить(Перечисления.ДействияНаОснованииИнвентаризации.Операция, "ОперацияБух");
...
Процедура ЗаполнитьТабличнуюЧасть(ЗаполнитьПоСпискуОС=Ложь, Знач Строки=Неопределено)
	...
	|			ТОГДА ВЫБОР
	|						КОГДА втДанныеДокументаИСостояние.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету)
	|							ТОГДА ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетЗабалансовогоУчета, 
	|								ЕСТЬNULL(СчетаБухгалтерскогоУчета.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)))
	|						ИНАЧЕ ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетУчета, 
	|								ЕСТЬNULL(СчетаБухгалтерскогоУчета.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)))
	|					КОНЕЦ
	...
	|			ТОГДА ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетЗабалансовогоУчета, 
	|						ЕСТЬNULL(СчетаБухгалтерскогоУчета.СчетУчета, ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)))
	|		ИНАЧЕ ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетУчета, СчетаБухгалтерскогоУчета.СчетУчета)
	|	КОНЕЦ КАК СчетУчетаПоДаннымУчета,
	|	ВЫБОР
	|		КОГДА НЕ ЕСТЬNULL(ПараметрыЦелевогоФинансирования.ПрименениеЦелевогоФинансирования, ЛОЖЬ)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетУчетаЦФ, ПараметрыЦелевогоФинансирования.СчетУчета)
	|	КОНЕЦ КАК СчетУчетаЦФПоДаннымУчета,
...
Процедура ЗаполнитьСлужебныеРеквизитыТабличнойЧасти(Знач Строки=Неопределено)
	...
	|	НЕ СостоянияАрендодателя.ОсновноеСредство ЕСТЬ NULL  КАК АрендодательОрганизация,
	|	ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетУчета, СчетаБухгалтерскогоУчета.СчетУчета) КАК СчетУчетаПоДаннымУчета,
	|	ВЫБОР
	|		КОГДА НЕ ЕСТЬNULL(ПараметрыЦелевогоФинансирования.ПрименениеЦелевогоФинансирования, ЛОЖЬ)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(СчетаБухгалтерскогоУчета.ГруппаФинансовогоУчета.СчетУчетаЦФ, ПараметрыЦелевогоФинансирования.СчетУчета)
...
Процедура ЗаполнитьДействие(Строка, Подразделение, КэшированныеЗначенияДокументовНаОсновании)
	...
	Если Строка.ЭтоСчетГруппы08 Тогда
		...
		Если Строка.СтоимостьПоДаннымУчета <> Строка.СтоимостьФактическая Тогда
			Строка.Действие = ПредопределенноеЗначение("Перечисление.ДействияНаОснованииИнвентаризации.Операция");
		КонецЕсли;
	...
	ИначеЕсли Строка.НаличиеПоДаннымУчета И Строка.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОС.ПринятоКУчету") Тогда
		...
		Если Строка.НаличиеФактическое
			...
			Строка.Действие = ПредопределенноеЗначение("Перечисление.ДействияНаОснованииИнвентаризации.Списание");
	...
	ИначеЕсли Строка.НаличиеПоДаннымУчета И Строка.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету") Тогда
		Если Не Строка.НаличиеФактическое И Не Строка.АрендодательОрганизация Тогда
			...
			Строка.Действие = ПредопределенноеЗначение("Перечисление.ДействияНаОснованииИнвентаризации.ВыбытиеАрендованныхОС");
	...
	ИначеЕсли Не Строка.НаличиеПоДаннымУчета Тогда
		...
		Если Строка.НаличиеФактическое
			...
			Строка.Действие = ПредопределенноеЗначение("Перечисление.ДействияНаОснованииИнвентаризации.ПринятиеКУчету");
		...
		ИначеЕсли Строка.НаличиеФактическое
			...
			Строка.Действие = ПредопределенноеЗначение("Перечисление.ДействияНаОснованииИнвентаризации.Перемещение");
