...
Функция ДанныеУчета(Организация, Подразделение, Дата, СписокНМА) Экспорт
	...
	|	ВЫБОР
	|		КОГДА СостояниеНМА.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.ПринятКУчету)
	|				И МестоУчетаНМА.Организация = &Организация
	|				И МестоУчетаНМА.Подразделение = &Подразделение
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК НаличиеПоДаннымУчета,
	|	ЕСТЬNULL(СостояниеНМА.Состояние, ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.НеПринятКУчету)) КАК Состояние
	|ИЗ
...
Функция ЗаполнитьДанныеУчета(Объект, КоллекцияСтрок, ЗаполнитьСлужебныеРеквизиты = Ложь) Экспорт
	...
	Для каждого ДанныеСтроки Из КоллекцияСтрок Цикл
		...
		ВнеоборотныеАктивыКлиентСервер.ЗаполнитьСтрокуИнвентаризацииПоДаннымУчета(ДанныеСтроки, ДанныеУчета);
		...
		ВнеоборотныеАктивыКлиентСервер.ЗаполнитьДействиеНаОснованииИнвентаризации(
			ДанныеСтроки, 
			ДанныеУчета, 
			Объект.Организация, 
			Объект.Подразделение);
	...
	Пока Док.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Пока Док.Следующий() Цикл
			...
			Если НЕ ОбщегоНазначенияБПВызовСервера.ПроверитьВыводТабличногоДокумента(ТабличныйДокумент, СтрокаСПодвалом) Тогда
				...
				СтоимостьФактическаяПрописью  = РаботаСКурсамиВалют.СформироватьСуммуПрописью(
													СтоимостьФактическаяПоСтранице, ВалютаРегламентированногоУчета);
		...
		Итоги.Параметры.СтоимостьФактическаяПрописью        = ОбщегоНазначенияБПВызовСервера.СформироватьСуммуПрописью(
																	СтоимостьФактическаяПоОписи, ВалютаРегламентированногоУчета);
