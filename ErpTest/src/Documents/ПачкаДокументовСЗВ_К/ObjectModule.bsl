	...
	Пока ВыборкаЗаписиСтажа.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаЗаписиСтажа.Сотрудник) Тогда 
			Если Не ВыборкаЗаписиСтажа.СотрудникРаботаетВОрганизации Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаЗаписиСтажа.НомерСтрокиСотрудник - 1, "ЧН=0; ЧГ=0") + "].Сотрудник",,Отказ);
				Продолжить;
			КонецЕсли;
...
Процедура СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписейСтажа, Отказ) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Сотрудники[" + Формат(ВыборкаЗаписейСтажа.НомерСтрокиСотрудник - 1, "ЧН=0; ЧГ=0") + "].Сотрудник", ,Отказ);
