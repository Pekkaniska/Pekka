...
//+++rarus-spb_zlov 08.01.2017  
    ...
    ТаблицаТоваровСКорректировками = Новый ТаблицаЗначений();
    ТаблицаТоваровСКорректировками.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
    ТаблицаТоваровСКорректировками.Колонки.Добавить("СуммаСНДС",    Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Любой)));
    ТаблицаТоваровСКорректировками.Колонки.Добавить("Отменено",     Новый ОписаниеТипов("Булево"));
        
    Для Каждого Строка ИЗ Товары Цикл
        НоваяСтрока = ТаблицаТоваровСКорректировками.Добавить();
        ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
    КонецЦикла;  
        
    Для Каждого Строка ИЗ пкКорректировкиТоваров Цикл
        НоваяСтрока = ТаблицаТоваровСКорректировками.Добавить();
        ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
        НоваяСтрока.Отменено = Ложь;
    КонецЦикла;  
    
    Запрос.УстановитьПараметр("Товары", ТаблицаТоваровСКорректировками);
//---rarus-spb_zlov 08.01.2017 
...
Функция ПолучитьСуммуКорректирующихСтрок(ТолькоЗалогЗаТару = Ложь) Экспорт
    ...
    ТаблицаКорректировка.Колонки.Добавить("СуммаСНДС",    Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Любой)));
    ТаблицаКорректировка.Колонки.Добавить("Отменено",     Новый ОписаниеТипов("Булево"));
    ...
    Запрос.УстановитьПараметр("Товары", ТаблицаКорректировка);
	Запрос.УстановитьПараметр("ВернутьМногооборотнуюТару", ВернутьМногооборотнуюТару);
	Запрос.УстановитьПараметр("ТребуетсяЗалогЗаТару", ТребуетсяЗалогЗаТару);
	Запрос.УстановитьПараметр("ТолькоЗалогЗаТару", ТолькоЗалогЗаТару);
...
//Рарус Владимир Подрезов 30.08.2017
	пкСуммаКорректировки = ПолучитьСуммуКорректирующихСтрок();
...
//+++rarus-spb_pavelk 25/10/2016
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.пкЗаказНаряд") Тогда
		пкЗаполнитьДокументНаОснованииЗаказНаряда(ДанныеЗаполнения);
//---rarus-spb_pavelk
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(Соглашение) ИЛИ НЕ Документы.ЗаказКлиента.ЗначениеРеквизитаОбъектаТипаБулево(Соглашение, "ИспользуютсяДоговорыКонтрагентов") Тогда
...
//+++rarus-spb_pavelk 25/10/2016
Процедура пкЗаполнитьДокументНаОснованииЗаказНаряда(Знач Основание)
	
	Перем КэшированныеЗначения; //используется механизмом обработки изменения реквизитов ТЧ
	
	Если Основание.ПлатныйРемонт И НЕ Основание.ТехникаКлиента Тогда 
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Основание, , "Номер,Дата");
		пкЗаказНаряд = Основание;
		Статус = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению;
		Приоритет = Справочники.Приоритеты.ПолучитьПриоритетПоУмолчанию(Приоритет);
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
		Партнер = Основание.Клиент;
		ПартнерыИКонтрагенты.ЗаполнитьКонтрагентаПартнераПоУмолчанию(Партнер, Контрагент);
		СпособДоставки = Перечисления.СпособыДоставки.Самовывоз;
		
		Работа = Константы.пкРемонтАрендованногоОборудования.Получить();
		СуммаРаботы		= Основание.Работы.Итог("Сумма") + Основание.Материалы.Итог("Сумма");
//Рарус Владимир Подрезов 24.03.2017
		СуммаНДСРаботы	= Основание.Работы.Итог("СуммаНДС") + Основание.Материалы.Итог("СуммаНДС");
		СуммаСНДСРаботы	= Основание.Работы.Итог("СуммаСНДС") + Основание.Материалы.Итог("СуммаСНДС");
//Рарус Владимир Подрезов Конец
...
//Рарус Владимир Подрезов Конец
		...
		НовСтрока.Склад					= Основание.Склад;
...
//Рарус Владимир Подрезов 24.03.2017
		...
		НовСтрока.пкТехника				= Основание.Техника;
		НовСтрока.ВариантОбеспечения	= Перечисления.ВариантыОбеспечения.Отгрузить;
		НовСтрока.пкЗаказНаряд			= Основание;
//Рарус Владимир Подрезов Конец
		
//Рарус Владимир Подрезов 24.03.2017
//Закомментированно: не сходятся копейки после пересчета, заполняем как есть в Основании
		//СтруктураПересчетаЦены  = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаЦеныСкидкиВПродажахВТЧ(ЭтотОбъект, ЭтотОбъект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию"));
		//СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ЭтотОбъект);
		//СтруктураДействий = Новый Структура;
