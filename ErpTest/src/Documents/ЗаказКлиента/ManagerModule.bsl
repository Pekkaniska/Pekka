...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Документы.ДвижениеПродукцииИМатериалов.ДобавитьКомандуСоздатьПередачуПродукцииНаСкладНаОсновании(КомандыСозданияНаОсновании, Ложь);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Документы.ЗаказПереработчику.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	//-- НЕ УТ
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	//+++rarus-spb_zlov 13.02.2017 
	ДатаРасчетов = пкОбщегоНазначенияСервер.ПолучитьДатуЗакрытияПоЗаказу(Реквизиты.ЗаказКлиента);
	
	Если ЗначениеЗаполнено(Реквизиты.ЗаказКлиента.пкДатаЗакрытия) Тогда
		ДатаЗакрытия = Реквизиты.ЗаказКлиента.пкДатаЗакрытия;
	Иначе
		ДатаЗакрытия = ДатаРасчетов;
	КонецЕсли;
	
	ДатаЗапускаСистемы  = Константы.пкДатаЗапускаСистемы.Получить();
	СчетПрошлогоПериода = ?(Реквизиты.ЗаказКлиента.Дата < ДатаЗапускаСистемы, Истина, Ложь);
	
	Запрос.УстановитьПараметр("ДатаЗакрытия",        ДатаЗакрытия);
	Запрос.УстановитьПараметр("ДатаРасчетов",        ДатаРасчетов);
	Запрос.УстановитьПараметр("СчетПрошлогоПериода", СчетПрошлогоПериода);
	Запрос.УстановитьПараметр("Договор",             Реквизиты.ЗаказКлиента.Договор);
	Запрос.УстановитьПараметр("Закрыт",              Реквизиты.ЗаказКлиента.пкЗакрыт);
	//---rarus-spb_zlov 13.02.2017
...
Функция ТекстЗапросаТаблицаЗаказыКлиентов(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
		...
		//+++rarus-spb_zlov 08.01.2017
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|   ЗаказКлиентапкКорректировкиТоваров.НомерСтроки,
		|   ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
		|   НАЧАЛОПЕРИОДА(&Период, ДЕНЬ),
		|   &Ссылка,
		|   ЗаказКлиентапкКорректировкиТоваров.Номенклатура,
		|   НЕОПРЕДЕЛЕНО,
		|   НЕОПРЕДЕЛЕНО,
		|   ЗаказКлиентапкКорректировкиТоваров.КодСтроки,
		|   НЕОПРЕДЕЛЕНО,
		|   ЗаказКлиентапкКорректировкиТоваров.Количество,
		|   0,
		|   ЗаказКлиентапкКорректировкиТоваров.СуммаСНДС,
		|   НЕОПРЕДЕЛЕНО
		|ИЗ
...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
		...
		|
		|ВЫБРАТЬ
		|	МАКСИМУМ(ЗаказКлиентапкКорректировкиТоваров.НомерСтроки),
		|	КОНЕЦПЕРИОДА(ЗаказКлиентапкКорректировкиТоваров.Ссылка.ДатаОтгрузки, ДЕНЬ),
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	&Номер,
		...
		|	И НЕ &РасчетыПоНакладным
		|	И НЕ &ГрафикИсполненияВДоговоре
		|	И (ЗаказКлиентапкКорректировкиТоваров.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
		|			ИЛИ &ТребуетсяЗалогЗаТару
		|			ИЛИ НЕ &ВернутьМногооборотнуюТару)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|			ИЛИ &ТребуетсяЗалогЗаТару
		|			ИЛИ НЕ &ВернутьМногооборотнуюТару)
		|	И &ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту), ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности))
		|
		|СГРУППИРОВАТЬ ПО
		|	КОНЕЦПЕРИОДА(ЗаказКлиентапкКорректировкиТоваров.Ссылка.ДатаОтгрузки, ДЕНЬ)
		|
		...
		|	&ДопустимаяСуммаЗадолженности,
		|	ВЫБОР
		|		КОГДА &СчетПрошлогоПериода
		|			ТОГДА -1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.СуммаОстаток, 0)
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	-ДокументЗаказКлиента.СуммаДокумента - 1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеОстаток, 0),
		|	ДокументЗаказКлиента.СуммаДокумента - 1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОтгрузкеОстаток, 0),
		|	-ДокументЗаказКлиента.СуммаДокумента - 1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.ОтгружаетсяОстаток, 0),
		|	ИСТИНА,
		|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПланированиеПоЗаказуКлиента),
		|	&Валюта,
		|	ЗНАЧЕНИЕ(Перечисление.ВариантыОплатыКлиентом.ПустаяСсылка)
		...
		|					И &Закрыт) КАК РасчетыСКлиентамиОстатки
		|		ПО ДокументЗаказКлиента.Ссылка = РасчетыСКлиентамиОстатки.ЗаказКлиента
		|ГДЕ
		|	ДокументЗаказКлиента.Ссылка = &ЗаказКлиента
		|	И (-ДокументЗаказКлиента.СуммаДокумента - 1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеОстаток, 0) <> 0
		|			ИЛИ ДокументЗаказКлиента.СуммаДокумента - 1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОтгрузкеОстаток, 0) <> 0
		|			ИЛИ -ДокументЗаказКлиента.СуммаДокумента - 1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.ОтгружаетсяОстаток, 0) <> 0
		|			ИЛИ ВЫБОР
		|				КОГДА &СчетПрошлогоПериода
		|					ТОГДА -1 * ЕСТЬNULL(РасчетыСКлиентамиОстатки.СуммаОстаток, 0)
		|				ИНАЧЕ 0
		|			КОНЕЦ <> 0)
		|	И ДокументЗаказКлиента.пкЗакрыт
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	&ДопустимаяСуммаЗадолженности,
		|	ВЫБОР
		|		КОГДА РасчетыСКлиентамиОстаткиПоДоговорам.СуммаОстаток > РасчетыСКлиентамиОстаткиПоЗаказам.СуммаОстаток
		|			ТОГДА РасчетыСКлиентамиОстаткиПоЗаказам.СуммаОстаток
		|		ИНАЧЕ -1 * РасчетыСКлиентамиОстаткиПоДоговорам.СуммаОстаток
		|	КОНЕЦ,
		|	0,
		|	0,
		|	0,
		...
		|					И &СчетПрошлогоПериода
		|					И &Закрыт) КАК РасчетыСКлиентамиОстаткиПоДоговорам
		|		ПО ДокументЗаказКлиента.Договор = РасчетыСКлиентамиОстаткиПоДоговорам.ЗаказКлиента
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентами.Остатки(
		|				&ДатаРасчетов,
		|				ЗаказКлиента = &ЗаказКлиента
		|					И &СчетПрошлогоПериода
		|					И &Закрыт) КАК РасчетыСКлиентамиОстаткиПоЗаказам
		|		ПО ДокументЗаказКлиента.Ссылка = РасчетыСКлиентамиОстаткиПоЗаказам.ЗаказКлиента
		|ГДЕ
		|	ВЫБОР
		|			КОГДА РасчетыСКлиентамиОстаткиПоДоговорам.СуммаОстаток > РасчетыСКлиентамиОстаткиПоЗаказам.СуммаОстаток
		|				ТОГДА РасчетыСКлиентамиОстаткиПоЗаказам.СуммаОстаток
		|			ИНАЧЕ -1 * РасчетыСКлиентамиОстаткиПоДоговорам.СуммаОстаток
		|		КОНЕЦ <> 0
		|	И ДокументЗаказКлиента.Ссылка = &ЗаказКлиента
		|	И ДокументЗаказКлиента.пкЗакрыт
		|	И &СчетПрошлогоПериода
		//Рарус Конец
		|УПОРЯДОЧИТЬ ПО
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуПекканиска(СтруктураТипов, ОбъектыПечати, Новый Структура));
	КонецЦикла;
	...
	//		Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуПекканиска(ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура, ТекущийКомплект));
	//КонецЕсли;
	...
	//		ИмяМакета,
	//		ТекущийКомплект[0].Представление,
	//		Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды("СчетНаОплатуАренды",ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура, ТекущийКомплект));
	//КонецЕсли;                                                                           "СчетНаОплатуАрендыСПечатьюМосква", СтруктураТипов, ОбъектыПечати, ПараметрыПечати))
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды(ИмяМакета,СтруктураТипов, ОбъектыПечати, Новый Структура));
	КонецЦикла;
	...
	//		ИмяМакета,
	//		ТекущийКомплект[0].Представление,
	//		Обработки.пкПечатьСчетовНаОплату.СформироватьПечатнуюФормуСчетНаОплатуАренды("СчетНаОплатуАрендыСПодписью", ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура, ТекущийКомплект));
	//КонецЕсли;
	//-- Рарус Лимаренко 22.11.2017
...
Функция КомплектПечатныхФорм() Экспорт
	...
	//++ Рарус Лимаренко 22.11.2017
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуПекканиска", НСтр("ru = 'Счет на оплату (Пекканиска)'"), 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуАренды", НСтр("ru = 'Счет на оплату аренды'"), 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "СчетНаОплатуАрендыСПодписью", НСтр("ru = 'Счет на оплату аренды (Подписи)'"), 1);
	//-- Рарус Лимаренко 22.11.2017
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИЛИ ДокументДляОбработки.АдресДоставкиПеревозчика <> """"
