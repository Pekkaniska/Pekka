...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ЭтоНовый И НЕ Параметры.Копирование Тогда
		...
		Если ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПереданныхВАрендуОС
			Или ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПереданныхВАрендуПредметовЛизингаНаБалансе Тогда
		...
		ИначеЕсли ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПредметовЛизингаНаБалансе Тогда
	...
	Элементы.ДекорацияЗаголовокПР.Видимость = ПлательщикНалогаНаПрибыль И ПоддержкаПБУ18;
	Элементы.ДекорацияЗаголовокВР.Видимость = ПлательщикНалогаНаПрибыль И ПоддержкаПБУ18;
	...
	АналитикаРасходовАмортизационнойПремииОбязательна =
		...
		И (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяРасходовАмортизационнойПремии, "КонтролироватьЗаполнениеАналитики") = Истина);
	АналитикаРасходовНалогОбязательна =
		ЗначениеЗаполнено(СтатьяРасходовНалог)
		И (ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяРасходовНалог, "КонтролироватьЗаполнениеАналитики") = Истина);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТребуетсяНастройкаОтраженияРасходовПоНалогам Тогда
		
		РеквизитыПроверкиСтатейРасходов = РеквизитыПроверкиСтатейРасходов + ", СтатьяРасходовНалог, АналитикаРасходовНалог";
	...
	Если ВключитьАмортизационнуюПремиюВСоставРасходов Тогда
		РеквизитыПроверкиСтатейРасходов = РеквизитыПроверкиСтатейРасходов + ", СтатьяРасходовАмортизационнойПремии, АналитикаРасходовАмортизационнойПремии";
	Иначе
	...
	Если ТипОперации <> Перечисления.ТипыОперацийВводаОстатков.ОстаткиПредметовЛизингаНаБалансе
		И ТипОперации <> Перечисления.ТипыОперацийВводаОстатков.ОстаткиПереданныхВАрендуПредметовЛизингаНаБалансе Тогда
		НепроверяемыеРеквизиты.Добавить("ДоговорЛизинга");
	КонецЕсли;
...
Процедура ОсновноеСредствоПриИзмененииНаСервере()
	...
	СтруктураРеквизитов.Вставить("АмортизационнаяГруппа");
...
Процедура СтатьяРасходовНалогПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтатьяРасходовНалог) Тогда
...
Процедура СтатьяРасходовНалогПриИзмененииНаСервере()
	...
	АналитикаРасходовНалогОбязательна =
		(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяРасходовНалог, "КонтролироватьЗаполнениеАналитики") = Истина);
...
Процедура ОбновитьСвойстваЭлементовФормы(ИзмененныеРеквизиты=Неопределено)
	...
	Если ОбновитьВсе 
		...
		ТребуетсяНастройка = ВнеоборотныеАктивы.ТребуетсяНастройкаОтраженияРасходовПоНалогам(
									Организация, 
									Дата, 
									ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОсновноеСредство),
									ГруппаОС, 
	...
	Если ОбновитьВсе Или Реквизиты.Свойство("ТипОперации") Тогда
		
		Элементы.ДоговорЛизинга.Видимость = (
			ТипОперации=Перечисления.ТипыОперацийВводаОстатков.ОстаткиПредметовЛизингаНаБалансе
			Или ТипОперации=Перечисления.ТипыОперацийВводаОстатков.ОстаткиПереданныхВАрендуПредметовЛизингаНаБалансе);
		
		ТребуетсяАрендатор =
			(ТипОперации=Перечисления.ТипыОперацийВводаОстатков.ОстаткиПереданныхВАрендуОС
			Или ТипОперации=Перечисления.ТипыОперацийВводаОстатков.ОстаткиПереданныхВАрендуПредметовЛизингаНаБалансе);
	...
	Если ОбновитьВсе Тогда
		...
		ВидимостьЭлементов = ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПредметовЛизингаНаБалансе
