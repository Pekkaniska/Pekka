...
Функция РасчетыПоДоговорамЛизинга()
	...
	|	Строки.Договор.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|	
	...
	|	Строки.Договор.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчетаКт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	|	
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |	ВводОстатковВнеоборотныхАктивов.Ссылка КАК Ссылка
	                      |ИЗ
	                      |	Документ.ВводОстатковВнеоборотныхАктивов.ОС КАК ВводОстатковВнеоборотныхАктивовОС
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов КАК ВводОстатковВнеоборотныхАктивов
	                      |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций КАК ОрганизацииУСНДоходыМинусРасходы
	                      |			ПО ВводОстатковВнеоборотныхАктивов.Организация = ОрганизацииУСНДоходыМинусРасходы.Организация
	                      |				И ВводОстатковВнеоборотныхАктивов.Дата >= ОрганизацииУСНДоходыМинусРасходы.Период
	                      ...
	                      |			ПО ВводОстатковВнеоборотныхАктивов.Организация = ОстальныеОрганизации.Организация
	                      |				И ВводОстатковВнеоборотныхАктивов.Дата >= ОстальныеОрганизации.Период
	                      |				И (НЕ ОстальныеОрганизации.ПрименяетсяУСНДоходыМинусРасходы)
	                      |		ПО ВводОстатковВнеоборотныхАктивовОС.Ссылка = ВводОстатковВнеоборотныхАктивов.Ссылка
	                      |ГДЕ
	                      |	ВводОстатковВнеоборотныхАктивовОС.ПорядокУчетаНУ = ЗНАЧЕНИЕ(Перечисление.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации)
	                      |
	                      |СГРУППИРОВАТЬ ПО
	                      |	ВводОстатковВнеоборотныхАктивов.Ссылка
	                      |
	                      |ИМЕЮЩИЕ
 		...
 		Попытка
			...
			Если Выборка.ПрименяетсяУСНДоходыМинусРасходы Тогда
				Для каждого СтрокаТабличнойЧасти Из ДанныеОбъекта.ОС Цикл
					Если СтрокаТабличнойЧасти.ПорядокУчетаНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации Тогда
			...
			ПартнерыКонтрагентов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ДанныеОбъекта.РасчетыПоДоговорамЛизинга.ВыгрузитьКолонку("Контрагент"), "Партнер");
			...
			Для каждого СтрокаТЧ Из ДанныеОбъекта.РасчетыПоДоговорамЛизинга Цикл
				...
				Если СтрокаТЧ.Партнер = Справочники.Партнеры.ПустаяСсылка() Тогда
					СтрокаТЧ.Партнер = ПартнерыКонтрагентов.Получить(СтрокаТЧ.Контрагент);
				КонецЕсли;
