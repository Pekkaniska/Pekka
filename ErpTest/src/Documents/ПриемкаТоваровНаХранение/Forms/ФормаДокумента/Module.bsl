...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Константы.ПорядокОформленияВыкупаПриСписанииТоваров.Получить() = Перечисления.НастройкиОформленияВыкупаПриСписанииТоваров.ПриПеремещении Тогда
		...
		ОбщегоНазначенияУТКлиентСервер.ДобавитьПараметрВыбора(Элементы.Склад, "ЦеховаяКладовая", Ложь);
...
Процедура ЗакрытьЗаказ(Команда)
	...
	Если ЗначениеЗаполнено(Объект.ЗаказПоставщику) Или СписокЗаказов.Количество() > 0 Тогда
		...
		Если Модифицированность Или Не Объект.Проведен Тогда
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ЗакрытьЗаказЗавершение", ЭтотОбъект), НСтр("ru = 'Документ не проведен. Провести документ?'"), РежимДиалогаВопрос.ДаНет);
            Возврат;
		КонецЕсли;
...
Процедура УстановитьПараметрыВыбораТоварыСклад()
	...
	Если Константы.ПорядокОформленияВыкупаПриСписанииТоваров.Получить() = Перечисления.НастройкиОформленияВыкупаПриСписанииТоваров.ПриПеремещении Тогда
		...
		ОбщегоНазначенияУТКлиентСервер.ДобавитьПараметрВыбора(Элементы.ТоварыСклад, "ЦеховаяКладовая", Ложь);
