...
Процедура ДанныеСпецификацииПоСпискуПродукции(ТаблицаПродукции, Параметры, МенеджерВременныхТаблиц) Экспорт
	...
	Для Каждого ТекСтрока Из ТаблицаПродукции Цикл
		...
		ЗаполнитьЗначенияСвойств(ДанныеПоНоменклатуре, ТекСтрока);
		ДанныеПоНоменклатуре.ПоляГруппировки = Новый Структура("НомерГруппыЗатрат, Подразделение",
																ТекСтрока.НомерГруппыЗатрат,
																ТекСтрока.Подразделение);
		
		МассивПродукции.Добавить(ДанныеПоНоменклатуре);
		
	КонецЦикла;
	
	ПараметрыВыборки = Справочники.РесурсныеСпецификации.ПараметрыВыборкиДанных();
	ПараметрыВыборки.ГруппироватьДанные = Истина;
	ПараметрыВыборки.ПоляГруппировки = Новый Структура("НомерГруппыЗатрат, Подразделение");
	
	ДанныеПродукции =
		Справочники.РесурсныеСпецификации.ДанныеСпецификацииПоСпискуНоменклатуры(МассивПродукции, ПараметрыВыборки);
	...
	Для Каждого ТекСтрока Из ДанныеПродукции Цикл
		...
		Запрос.УстановитьПараметр("НомерГруппыЗатрат", ТекСтрока.ПоляГруппировки.НомерГруппыЗатрат);
		Запрос.УстановитьПараметр("Подразделение", ТекСтрока.ПоляГруппировки.Подразделение);
...
Процедура ЗаполнитьНазначениеМатериаловПоОбособленнымОстаткам(МенеджерВременныхТаблиц, Параметры)
	...
	Запрос.Текст =
		...
		|			И (КлючиАналитики.Склад = МатериалыИУслуги.Склад)
...
Функция ТекстЗапросаМатериалы()
	Возврат
		...
		|	&МатериалыИУслуги КАК МатериалыИУслуги
		|ГДЕ
		|	НЕ МатериалыИУслуги.СпособПолученияПолуфабриката =
		|		ЗНАЧЕНИЕ(Перечисление.СпособыПолучениеМатериаловЭтапаПроизводства.ПроизвестиНаЭтапе)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(СпрСклады.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК Подразделение,
		|	МатериалыИУслуги.Номенклатура,
		|	МатериалыИУслуги.Характеристика,
		|	МатериалыИУслуги.СтатьяКалькуляции,
		|	МатериалыИУслуги.Упаковка,
		...
		|	ПО МатериалыИУслуги.Склад = СпрСклады.Ссылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Процедура ИсключитьВозвратныеОтходыИзВыходныхИзделий(ТаблицаПродукции, ДанныеПродукции)
	...
	Для Каждого ТекСтруктура Из ДанныеПродукции Цикл
		...
		ЗаполнитьЗначенияСвойств(Отборы, ТекСтруктура.ПоляГруппировки);
...
Процедура ПеренестиСерииУказываемыеОтдельно(Документ, ИмяТЧ)
	...
	Для Каждого ТекСтрока Из Документ[ИмяТЧ] Цикл
		Если (ТекСтрока.СтатусУказанияСерий = 3
			...
			ЗаполнитьЗначенияСвойств(Документ[ИмяТЧ + "Серии"].Добавить(), ТекСтрока);
...
Процедура ИнициализироватьКлючиАналитикиНоменклатуры(Запрос)
	...
	|		АналитикаУчета.Номенклатура							КАК Номенклатура,
	|		АналитикаУчета.Характеристика						КАК Характеристика,
	|		ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)		КАК Назначение,
	|		ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)	КАК Серия,
	|		ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)	КАК СтатьяКалькуляции
	|	ИЗ
	...
	|		ПО ТаблицаТовары.АналитикаУчетаНоменклатуры = АналитикаУчета.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК СпрВидыЗапасов
	|		ПО ТаблицаТовары.ВидЗапасов = СпрВидыЗапасов.Ссылка
	|	ГДЕ
	|		ТаблицаТовары.Ссылка = &Ссылка
	|		И СпрВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)) КАК ТаблицаТовары
	...
	ЗапросАналитики.УстановитьПараметр("УчитыватьСебестоимостьТоваровПоВидамЗапасов",
...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(Запрос, ТекстыЗапроса)
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитики
	|		ПО ТаблицаДокумента.АналитикаУчетаНоменклатуры = КлючиАналитики.КлючАналитики
	|
...
Функция ТекстЗапросаВтТовары(Запрос, ТекстыЗапроса)
	...
	|		ПО КлючиАналитикиПолучателя.КлючАналитики = ТаблицаТовары.АналитикаУчетаНоменклатуры
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклады
	|		ПО СпрСклады.Ссылка = КлючиАналитикиПолучателя.Склад
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиОтправителя
	|		ПО КлючиАналитикиПолучателя.Номенклатура = КлючиАналитикиОтправителя.Номенклатура
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитикиОтправителя.Характеристика
	|		И КлючиАналитикиПолучателя.Серия = КлючиАналитикиОтправителя.Серия
	|		И Операция.Подразделение = КлючиАналитикиОтправителя.Склад
	|		И КлючиАналитикиПолучателя.Назначение = КлючиАналитикиОтправителя.Назначение
	|		И КлючиАналитикиПолучателя.СтатьяКалькуляции = КлючиАналитикиОтправителя.СтатьяКалькуляции
	|
	...
	|	НЕОПРЕДЕЛЕНО										КАК КорАналитикаФинансовогоУчета,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КлючиАналитики.Назначение.ВидДеятельностиНДС,ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) <>
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	...
	|		ПО ТаблицаТовары.АналитикаУчетаНоменклатуры = КлючиАналитики.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклады
	|		ПО СпрСклады.Ссылка = КлючиАналитики.Склад
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК СпрНазначения
	|		ПО СпрНазначения.Ссылка = КлючиАналитики.Назначение
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца КАК ЗаказыДавальца
	|		ПО СпрНазначения.Заказ = ЗаказыДавальца.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиПолучателя
	|		ПО КлючиАналитикиПолучателя.Номенклатура = КлючиАналитики.Номенклатура
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитики.Характеристика
	|		И КлючиАналитикиПолучателя.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		И КлючиАналитикиПолучателя.Склад = ЗаказыДавальца.Подразделение
	|		И КлючиАналитикиПолучателя.Назначение = КлючиАналитики.Назначение
	|		И КлючиАналитикиПолучателя.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиОтправителя
	|		ПО КлючиАналитики.Номенклатура = КлючиАналитикиОтправителя.Номенклатура
	|		И КлючиАналитики.Характеристика = КлючиАналитикиОтправителя.Характеристика
	|		И КлючиАналитики.Серия = КлючиАналитикиОтправителя.Серия
	|		И Операция.Подразделение = КлючиАналитикиОтправителя.Склад
	|		И КлючиАналитики.Назначение = КлючиАналитикиОтправителя.Назначение
	|		И КлючиАналитики.СтатьяКалькуляции = КлючиАналитикиОтправителя.СтатьяКалькуляции
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаАналитикУчетаПартий КАК ТаблицаАналитикУчетаПартий
	|		ПО ТаблицаАналитикУчетаПартий.НомерСтроки		= ТаблицаТовары.НомерСтроки
	|		И ТаблицаАналитикУчетаПартий.ИмяТабличнойЧасти	= ""ВидыЗапасовВыходныеИзделия""
	...
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитикиОтправителя.Характеристика
	|		И КлючиАналитикиПолучателя.Серия = КлючиАналитикиОтправителя.Серия
	|		И ТаблицаТовары.Подразделение = КлючиАналитикиОтправителя.Склад
	|		И КлючиАналитикиПолучателя.Назначение = КлючиАналитикиОтправителя.Назначение
	|		И КлючиАналитикиПолучателя.СтатьяКалькуляции = КлючиАналитикиОтправителя.СтатьяКалькуляции
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СпрПартииПроизводства.Назначение.ВидДеятельностиНДС,ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) <>
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|			ТОГДА СпрПартииПроизводства.Назначение.ВидДеятельностиНДС
	|		ИНАЧЕ СпрПартииПроизводства.ВидДеятельностиНДС
	|	КОНЕЦ												КАК ВидДеятельностиНДС,
	|	НЕОПРЕДЕЛЕНО										КАК НомерГТД,
	|	НЕОПРЕДЕЛЕНО										КАК Спецификация,
	|	ТаблицаТовары.СтатьяКалькуляции						КАК СтатьяКалькуляции,
	...
	|		ПО КлючиАналитикиПолучателя.Номенклатура = КлючиАналитики.Номенклатура
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитики.Характеристика
	|		И КлючиАналитикиПолучателя.Серия = КлючиАналитики.Серия
	|		И КлючиАналитикиПолучателя.Склад = ТаблицаТовары.Подразделение
	|		И КлючиАналитикиПолучателя.Назначение = КлючиАналитики.Назначение
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитики
	|		ПО КлючиАналитики.КлючАналитики = ТаблицаТовары.АналитикаУчетаНоменклатуры
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклады
	|		ПО КлючиАналитики.Склад = СпрСклады.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиПолучателя
	|		ПО КлючиАналитикиПолучателя.Номенклатура = КлючиАналитики.Номенклатура
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитики.Характеристика
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|	КлючиНоменклатурыПоОтправителю.Склад						КАК Склад,
	|	КлючиНоменклатурыПолучатель.Склад							КАК Получатель,
	...
	|			И КлючиНоменклатурыПоОтправителюБезНазначения.Склад = КлючиНоменклатурыПоОтправителю.Склад
	...
	|			И КлючиНоменклатурыПолучательБезНазначения.Склад = КлючиНоменклатурыПолучатель.Склад
	...
	|			И СпрВидыЗапасов.ВладелецТовара = АналитикаКомитента.Склад
...
Функция ТекстЗапросаТаблицаТоварыНаСкладах(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И НЕ (ВЫРАЗИТЬ(ТаблицаТовары.Получатель КАК Справочник.Склады).ИспользоватьОрдернуюСхемуПриПоступлении
	|		И ВЫРАЗИТЬ(ТаблицаТовары.Получатель КАК Справочник.Склады).ДатаНачалаОрдернойСхемыПриПоступлении <= &Дата)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ТИПЗНАЧЕНИЯ(ТаблицаСерий.Получатель) = ТИП(Справочник.Склады)
	|	И НЕ (ВЫРАЗИТЬ(ТаблицаСерий.Получатель КАК Справочник.Склады).ИспользоватьОрдернуюСхемуПриПоступлении
	|		И ВЫРАЗИТЬ(ТаблицаСерий.Получатель КАК Справочник.Склады).ДатаНачалаОрдернойСхемыПриПоступлении <= &Дата)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	И ТИПЗНАЧЕНИЯ(МатериалыИРаботы.Склад) = ТИП(Справочник.Склады)
	|	И НЕ (ВЫРАЗИТЬ(МатериалыИРаботы.Склад КАК Справочник.Склады).ИспользоватьОрдернуюСхемуПриПоступлении
	|		И ВЫРАЗИТЬ(МатериалыИРаботы.Склад КАК Справочник.Склады).ДатаНачалаОрдернойСхемыПриОтгрузке <= &Дата)";
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			И (втВидыЗапасов.ЦеховаяКладовая
	|				ИЛИ втВидыЗапасов.Работа)
	|			ТОГДА ВтВидыЗапасов.СуммаУпр
	|		ИНАЧЕ 0
...
Функция ТекстЗапросаТаблицаВыпускПродукции(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТабличнаяЧасть.Получатель					КАК СтатьяРасходов,
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК КлючиАналитикиПолучателя
	|		ПО ТабличнаяЧасть.АналитикаУчетаНоменклатуры = КлючиАналитикиПолучателя.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиПолучателяБезНазначения
	|		ПО КлючиАналитикиПолучателя.Номенклатура = КлючиАналитикиПолучателяБезНазначения.Номенклатура
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитикиПолучателяБезНазначения.Характеристика
	|		И ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) = КлючиАналитикиПолучателяБезНазначения.Назначение
	|		И КлючиАналитикиПолучателя.Серия = КлючиАналитикиПолучателяБезНазначения.Серия
	|		И КлючиАналитикиПолучателя.Склад = КлючиАналитикиПолучателяБезНазначения.Склад
	|		И КлючиАналитикиПолучателя.СтатьяКалькуляции = КлючиАналитикиПолучателяБезНазначения.СтатьяКалькуляции
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиОтправителя
	|		ПО КлючиАналитикиПолучателя.Номенклатура = КлючиАналитикиОтправителя.Номенклатура
	|		И КлючиАналитикиПолучателя.Характеристика = КлючиАналитикиОтправителя.Характеристика
	|		И КлючиАналитикиПолучателя.Серия = КлючиАналитикиОтправителя.Серия
	|		И &Подразделение = КлючиАналитикиОтправителя.Склад
	|		И КлючиАналитикиПолучателя.Назначение = КлючиАналитикиОтправителя.Назначение
	|		И КлючиАналитикиПолучателя.СтатьяКалькуляции = КлючиАналитикиОтправителя.СтатьяКалькуляции
	|
	...
	|		И КлючиАналитикиОтправителя.Склад = КлючиАналитикиОтправителяБезНазначения.Склад
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказов(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И ТИПЗНАЧЕНИЯ(ТаблицаТовары.Склад) = ТИП(Справочник.Склады)
	...
	|			ИЛИ НЕ (ВЫРАЗИТЬ(ТаблицаТовары.Получатель КАК Справочник.Склады).ИспользоватьОрдернуюСхемуПриПоступлении
	|				И ВЫРАЗИТЬ(ТаблицаТовары.Получатель КАК Справочник.Склады).ДатаНачалаОрдернойСхемыПриПоступлении <=
	|					&Дата)
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказовРаботами(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "
	// Обеспечение выходными изделиями (работами)
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)	КАК ВидДвижения,
	|	&Дата									КАК Период,
	|	ТаблицаТовары.Номенклатура				КАК Номенклатура,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.НаправлениеВыпуска =
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	|			ТОГДА &Подразделение
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Обеспечение побочными изделиями (работами)
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)	КАК ВидДвижения,
	|	&Дата									КАК Период,
	|	ТаблицаТовары.Номенклатура				КАК Номенклатура,
...
Функция ТекстЗапросаТаблицаСвободныеОстатки(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = 
	// Поступление выходных изделий на получателя.
	"ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)	КАК ВидДвижения,
	|	&Дата									КАК Период,
	|	ТаблицаТовары.Получатель				КАК Склад,
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Поступление побочных изделий на получателя.
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)	КАК ВидДвижения,
	|	&Дата									КАК Период,
	|	ТаблицаТовары.Получатель				КАК Склад,
...
Функция ТекстЗапросаТаблицаТоварыКОтгрузке(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Дата							КАК Период,
	|	ТаблицаТовары.Склад				КАК Склад,
	|	ТаблицаТовары.Подразделение		КАК Получатель,
	...
	|ГДЕ
	|	ТаблицаТовары.Ссылка = &Ссылка
	|	И НЕ ТаблицаТовары.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	И ТаблицаТовары.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке
	|	И ТаблицаТовары.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <= &Дата
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Приход И расход ПО накладным на неордерном складе для доставки (приход/расход ПО "К отгрузке").
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения,
	|	&Дата							КАК Период,
	|	ТаблицаТовары.Склад				КАК Склад,
...
Функция ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|	КОНЕЦ КАК Назначение,
	|	ТаблицаТовары.Получатель               КАК Склад,
	|	ТаблицаТовары.Подразделение            КАК Отправитель,
	|	0                                      КАК КОформлениюНакладныхПоРаспоряжению,
	|	0                                      КАК КОформлениюПоступленийПоОрдерам,
	|	ТаблицаТовары.Количество               КАК КОформлениюОрдеров,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции) КАК ХозяйственнаяОперация
...
Процедура СформироватьПартииПроизводства(Объект) Экспорт
	...
	Запрос.УстановитьПараметр("ПартионныйУчетВерсии22",
		УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПартионныйУчетВерсии22(Реквизиты.Дата));
...
Функция СформироватьПечатнуюФормуНакладной(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, 
		"&КодАртикул", 
		?(КолонкаКодов = "", """""", "ТаблицаТовары.Номенклатура." + КолонкаКодов));
			...
			Пока ВыборкаПоПолучателям.Следующий() Цикл
				...
				ОбластьМакета.Параметры.ПредставлениеОрганизации =
					ФормированиеПечатныхФорм.ОписаниеОрганизации(
						СведенияОбОрганизации, "ПолноеНаименование,ИНН,ЮридическийАдрес,Телефоны");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Пока ВыборкаПоПодразделениям.Следующий() Цикл
			...
			Пока ВыборкаПоСкладам.Следующий() Цикл
				...
				ОбластьЗаголовок.Параметры.Организация = ДанныеПечати.Организация;
				ШтрихкодированиеПечатныхФорм.ВывестиШтрихкодВТабличныйДокумент(ТабДокумент,
																				Макет,
																				ОбластьЗаголовок,
...
Процедура ЗарегистрироватьДанныеКОбновлениюКлючейАналитики(Параметры) Экспорт
	...
	|			КОГДА ТаблицаТовары.НаправлениеВыпуска =
	|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
	|				ТОГДА Реквизиты.Подразделение
	|			ИНАЧЕ ТаблицаТовары.Получатель
	|		КОНЕЦ = Аналитика.Склад
	|		И ТаблицаТовары.Назначение = Аналитика.Назначение
...
Процедура СгенерироватьКлючиАналитикиНоменклатуры(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ПроизводствоБезЗаказа";
	МетаданныеДокумента = Метаданные.Документы.ПроизводствоБезЗаказа;
	...
	|		И ВложенныйЗапрос.СтатьяКалькуляции = Аналитика.СтатьяКалькуляции
	|
	|ГДЕ
...
// Обработчик обновления УП 2.4.3
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ВидыЗапасовВыходныеИзделия КАК ВидыЗапасов
	|			ПО ПроизводствоБезЗаказа.Ссылка = ВидыЗапасов.Ссылка
	|	ГДЕ
	|		ПроизводствоБезЗаказа.Проведен
	|		И НЕ ПроизводствоБезЗаказаВыходныеИзделия.Номенклатура.ТипНоменклатуры =
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|		И НЕ ПроизводствоБезЗаказаВыходныеИзделия.УдалитьВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|		И ВидыЗапасов.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа КАК ПроизводствоБезЗаказа
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ПобочныеИзделия КАК ПроизводствоБезЗаказаПобочныеИзделия
	|			ПО (ПроизводствоБезЗаказаПобочныеИзделия.Ссылка = ПроизводствоБезЗаказа.Ссылка)
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ВидыЗапасовПобочныеИзделия КАК ВидыЗапасов
	|			ПО ПроизводствоБезЗаказа.Ссылка = ВидыЗапасов.Ссылка
	|	ГДЕ
	|		ПроизводствоБезЗаказа.Проведен
	|		И НЕ ПроизводствоБезЗаказаПобочныеИзделия.Номенклатура.ТипНоменклатуры =
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|		И НЕ ПроизводствоБезЗаказаПобочныеИзделия.УдалитьВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|		И ВидыЗапасов.АналитикаУчетаНоменклатуры ЕСТЬ NULL
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ГДЕ
	|		ПроизводствоБезЗаказаВыходныеИзделия.КодСтроки = 0
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// заполнение вида запасов получателя
	|	ВЫБРАТЬ
	|		Реквизиты.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа КАК Реквизиты
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ВидыЗапасовМатериалыИРаботы КАК Материалы
	|			ПО (Реквизиты.Ссылка = Материалы.Ссылка)
	|	ГДЕ
	|		Реквизиты.Проведен
	|		И Материалы.ВидЗапасовПолучателя = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Перенос отправителя материалов/работ в склад/подразделение
	|	ВЫБРАТЬ
	|		Материалы.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.МатериалыИРаботы КАК Материалы
	|	ГДЕ
	|		НЕ Материалы.УдалитьОтправитель = НЕОПРЕДЕЛЕНО
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение подразделения затрат в видах запасов материалов
	|	ВЫБРАТЬ
	|		Материалы.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.ВидыЗапасовМатериалыИРаботы КАК Материалы
	|	ГДЕ
	|		Материалы.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение реквизита "Способ распределения затрат на изделия" значением по умолчанию "по долям стоимости".
	|	ВЫБРАТЬ
	|		ТаблицаДокумента.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа КАК ТаблицаДокумента
	|	ГДЕ
	|		ТаблицаДокумента.СпособРаспределенияЗатратНаВыходныеИзделия = ЗНАЧЕНИЕ(Перечисление.СпособыРаспределенияЗатратНаВыходныеИзделия.ПустаяСсылка)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение номера группы затрат в побочных изделий
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ТаблицаТоваров.НомерГруппыЗатрат = 0
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение номера группы затрат в материалах
	|	ВЫБРАТЬ
	|		ТаблицаТоваров.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.МатериалыИРаботы КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Реквизиты
	|			ПО Реквизиты.Ссылка = ТаблицаТоваров.Ссылка
	|	ГДЕ
	|		ТаблицаТоваров.НомерГруппыЗатрат = 0
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение номера группы затрат в трудозатратах
	|	ВЫБРАТЬ
	|		ТаблицаТоваров.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.Трудозатраты КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Реквизиты
	|			ПО Реквизиты.Ссылка = ТаблицаТоваров.Ссылка
	|	ГДЕ
	|		ТаблицаТоваров.НомерГруппыЗатрат = 0
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение номера группы затрат в видах запасов выходных изделий
	|	ВЫБРАТЬ
	|		ТаблицаТоваров.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.ВидыЗапасовВыходныеИзделия КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Реквизиты
	|			ПО Реквизиты.Ссылка = ТаблицаТоваров.Ссылка
	|	ГДЕ
	|		Реквизиты.Проведен
	|		И ТаблицаТоваров.НомерГруппыЗатрат = 0
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение номера группы затрат в видах запасов побочных изделий
	|	ВЫБРАТЬ
	|		ТаблицаТоваров.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.ВидыЗапасовПобочныеИзделия КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Реквизиты
	|			ПО Реквизиты.Ссылка = ТаблицаТоваров.Ссылка
	|	ГДЕ
	|		Реквизиты.Проведен
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	// Заполнение номера группы затрат в видах запасов материалов
	|	ВЫБРАТЬ
	|		ТаблицаТоваров.Ссылка
	|	ИЗ
	|		Документ.ПроизводствоБезЗаказа.ВидыЗапасовМатериалыИРаботы КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК Реквизиты
	|			ПО Реквизиты.Ссылка = ТаблицаТоваров.Ссылка
	|	ГДЕ
	|		Реквизиты.Проведен
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстЗапроса =
		...
		|	ПроизводствоБезЗаказа.ВерсияДанных	КАК ВерсияДанных,
		|	1									КАК НомерУсловия
		|ПОМЕСТИТЬ ВТУсловия
		|ИЗ
		...
		|	ПО ОбъектыДляОбработки.Ссылка = ПроизводствоБезЗаказа.Ссылка
		|
		|ГДЕ
		|	ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = ТаблицаВыходныеИзделия.Ссылка
		|				И НЕ ТаблицаВыходныеИзделия.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|				И НЕ ТаблицаВыходныеИзделия.УдалитьВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ИСТИНА
		|		ИЗ
		|			Документ.ПроизводствоБезЗаказа.ВидыЗапасовВыходныеИзделия КАК ВыходныеИзделияВЗ
		|		ГДЕ
		|			ОбъектыДляОбработки.Ссылка = ВыходныеИзделияВЗ.Ссылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|				ИСТИНА
		|			ИЗ
		|				Документ.ПроизводствоБезЗаказа.ПобочныеИзделия КАК ТаблицаПобочныеИзделия
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = ТаблицаПобочныеИзделия.Ссылка
		|				И НЕ ТаблицаПобочныеИзделия.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|				И НЕ ТаблицаПобочныеИзделия.УдалитьВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ИСТИНА
		|		ИЗ
		|			Документ.ПроизводствоБезЗаказа.ВидыЗапасовПобочныеИзделия КАК ПобочныеИзделияВЗ
		|		ГДЕ
		|			ОбъектыДляОбработки.Ссылка = ПобочныеИзделияВЗ.Ссылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|				ИСТИНА
		|			ИЗ
		|				Документ.ПроизводствоБезЗаказа.ВидыЗапасовМатериалыИРаботы КАК МатериалыИРаботыВЗ
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = МатериалыИРаботыВЗ.Ссылка
		|				И МатериалыИРаботыВЗ.ВидЗапасовПолучателя = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = ТаблицаВыходныеИзделия.Ссылка
		|				И ТаблицаВыходныеИзделия.КодСтроки = 0)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|				ИСТИНА
		|			ИЗ
		|				Документ.ПроизводствоБезЗаказа.МатериалыИРаботы КАК ТаблицаМатериалыИРаботы
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = ТаблицаМатериалыИРаботы.Ссылка
		|				И НЕ ТаблицаМатериалыИРаботы.УдалитьОтправитель = НЕОПРЕДЕЛЕНО)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|				ИСТИНА
		|			ИЗ
		|				Документ.ПроизводствоБезЗаказа.ВидыЗапасовМатериалыИРаботы КАК ТаблицаМатериалыИРаботы
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = ТаблицаМатериалыИРаботы.Ссылка
		|				И ТаблицаМатериалыИРаботы.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		// Заполнение номера группы затрат в побочных изделиях
		|ВЫБРАТЬ
		|	ОбъектыДляОбработки.Ссылка			КАК Ссылка,
		|	ПроизводствоБезЗаказа.ВерсияДанных	КАК ВерсияДанных,
		|	9									КАК НомерУсловия
		...
		|				Документ.ПроизводствоБезЗаказа.ПобочныеИзделия КАК ТаблицаТоваров
		|			ГДЕ
		|				ОбъектыДляОбработки.Ссылка = ТаблицаТоваров.Ссылка
		|				И ТаблицаТоваров.НомерГруппыЗатрат = 0)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|	ОбъектыДляОбработки.Ссылка							КАК Ссылка,
		|	ВЫБОР
		|		КОГДА ПродукцияВЗНеОчищен.Ссылка ЕСТЬ НЕ NULL
		|			И ВЗПродукцииПустые.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА ПродукцияВЗНеОчищен.ВерсияДанных
		|		КОГДА ВОВЗНеОчищен.Ссылка ЕСТЬ НЕ NULL
		|			И ВЗВОПустые.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА ВОВЗНеОчищен.ВерсияДанных
		|		КОГДА МатериалыИРаботыВЗ.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА МатериалыИРаботыВЗ.ВерсияДанных
		|		КОГДА ПродукцияКодСтрокиНеЗаполнен.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА ПродукцияКодСтрокиНеЗаполнен.ВерсияДанных
		|		КОГДА МатериалыИРаботыОтправитель.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА МатериалыИРаботыОтправитель.ВерсияДанных
		|		КОГДА МатериалыИРаботыВЗПодразделение.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА МатериалыИРаботыВЗПодразделение.ВерсияДанных
		|		КОГДА НомерГруппыЗатратВИ.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА НомерГруппыЗатратВИ.ВерсияДанных
		|		КОГДА НомерГруппыЗатратПИ.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА НомерГруппыЗатратПИ.ВерсияДанных
		|		КОГДА НомерГруппыЗатратМ.Ссылка ЕСТЬ НЕ NULL
		|			ТОГДА НомерГруппыЗатратМ.ВерсияДанных
		|		ИНАЧЕ ПроизводствоБезЗаказа.ВерсияДанных
		|	КОНЕЦ												КАК ВерсияДанных,
		|	ПродукцияВЗНеОчищен.Ссылка ЕСТЬ НЕ NULL
		|		И ВЗПродукцииПустые.Ссылка ЕСТЬ НЕ NULL			КАК ВыходныеИзделияВЗ,
		|	ВОВЗНеОчищен.Ссылка ЕСТЬ НЕ NULL
		|		И ВЗВОПустые.Ссылка ЕСТЬ НЕ NULL				КАК ПобочныеИзделияВЗ,
		|	МатериалыИРаботыВЗ.Ссылка ЕСТЬ НЕ NULL				КАК МатериалыИРаботыВЗ,
		|	ПродукцияКодСтрокиНеЗаполнен.Ссылка ЕСТЬ НЕ NULL	КАК ВыходныеИзделияКодСтроки,
		|	МатериалыИРаботыОтправитель.Ссылка ЕСТЬ НЕ NULL		КАК МатериалыИРаботыОтправитель,
		|	МатериалыИРаботыВЗПодразделение.Ссылка ЕСТЬ НЕ NULL	КАК МатериалыИРаботыВЗПодразделение,
		|	НомерГруппыЗатратОтходы.Ссылка ЕСТЬ НЕ NULL			КАК НомерГруппыЗатратОтходы,
		|	НомерГруппыЗатратМатериалы.Ссылка ЕСТЬ НЕ NULL		КАК НомерГруппыЗатратМатериалы,
		|	НомерГруппыЗатратТруд.Ссылка ЕСТЬ НЕ NULL			КАК НомерГруппыЗатратТруд,
		|	НомерГруппыЗатратВИ.Ссылка ЕСТЬ НЕ NULL				КАК НомерГруппыЗатратВИ,
		|	НомерГруппыЗатратПИ.Ссылка ЕСТЬ НЕ NULL				КАК НомерГруппыЗатратПИ,
		|	НомерГруппыЗатратМ.Ссылка ЕСТЬ НЕ NULL				КАК НомерГруппыЗатратМ
		|ИЗ
		|	ВТОбъектыДляОбработки КАК ОбъектыДляОбработки
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК ПроизводствоБезЗаказа
		|	ПО ОбъектыДляОбработки.Ссылка = ПроизводствоБезЗаказа.Ссылка
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК ПродукцияВЗНеОчищен
		|	ПО ОбъектыДляОбработки.Ссылка = ПродукцияВЗНеОчищен.Ссылка
		|	И ПродукцияВЗНеОчищен.НомерУсловия = 1
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК ВЗПродукцииПустые
		|	ПО ОбъектыДляОбработки.Ссылка = ВЗПродукцииПустые.Ссылка
		|	И ВЗПродукцииПустые.НомерУсловия = 2
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК ВОВЗНеОчищен
		|	ПО ОбъектыДляОбработки.Ссылка = ВОВЗНеОчищен.Ссылка
		|	И ВОВЗНеОчищен.НомерУсловия = 3
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК ВЗВОПустые
		|	ПО ОбъектыДляОбработки.Ссылка = ВЗВОПустые.Ссылка
		|	И ВЗВОПустые.НомерУсловия = 4
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК МатериалыИРаботыВЗ
		|	ПО ОбъектыДляОбработки.Ссылка = МатериалыИРаботыВЗ.Ссылка
		|	И МатериалыИРаботыВЗ.НомерУсловия = 5
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК ПродукцияКодСтрокиНеЗаполнен
		|	ПО ОбъектыДляОбработки.Ссылка = ПродукцияКодСтрокиНеЗаполнен.Ссылка
		|	И ПродукцияКодСтрокиНеЗаполнен.НомерУсловия = 6
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК МатериалыИРаботыОтправитель
		|	ПО ОбъектыДляОбработки.Ссылка = МатериалыИРаботыОтправитель.Ссылка
		|	И МатериалыИРаботыОтправитель.НомерУсловия = 7
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК МатериалыИРаботыВЗПодразделение
		|	ПО ОбъектыДляОбработки.Ссылка = МатериалыИРаботыВЗПодразделение.Ссылка
		|	И МатериалыИРаботыВЗПодразделение.НомерУсловия = 8
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК НомерГруппыЗатратОтходы
		|	ПО ОбъектыДляОбработки.Ссылка = НомерГруппыЗатратОтходы.Ссылка
		|	И НомерГруппыЗатратОтходы.НомерУсловия = 9
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК НомерГруппыЗатратМатериалы
		|	ПО ОбъектыДляОбработки.Ссылка = НомерГруппыЗатратМатериалы.Ссылка
		|	И НомерГруппыЗатратМатериалы.НомерУсловия = 10
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК НомерГруппыЗатратТруд
		|	ПО ОбъектыДляОбработки.Ссылка = НомерГруппыЗатратТруд.Ссылка
		|	И НомерГруппыЗатратТруд.НомерУсловия = 11
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК НомерГруппыЗатратВИ
		|	ПО ОбъектыДляОбработки.Ссылка = НомерГруппыЗатратВИ.Ссылка
		|	И НомерГруппыЗатратВИ.НомерУсловия = 12
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК НомерГруппыЗатратПИ
		|	ПО ОбъектыДляОбработки.Ссылка = НомерГруппыЗатратПИ.Ссылка
		|	И НомерГруппыЗатратПИ.НомерУсловия = 13
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТУсловия КАК НомерГруппыЗатратМ
		|	ПО ОбъектыДляОбработки.Ссылка = НомерГруппыЗатратМ.Ссылка
		|	И НомерГруппыЗатратМ.НомерУсловия = 14
		|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			// Аналитика учета номенклатуры. ВыходныеИзделия
			ПроизвольныйСклад =
			"ВЫБОР
			|	КОГДА Коллекция.НаправлениеВыпуска =
			|					ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
			|		ТОГДА НЕОПРЕДЕЛЕНО
			...
			РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ДокументОбъект.ВыходныеИзделия,
																			МестаУчета,
																			ИменаПолей);
			
			// Код строки. ВыходныеИзделия
			Если Выборка.ВыходныеИзделияКодСтроки Тогда
				ЗаказыСервер.УстановитьКлючВСтрокахТабличнойЧасти(ДокументОбъект,
																"ВыходныеИзделия",
																"МаксимальныйКодСтрокиВыходныеИзделия");
			КонецЕсли;
			
			// Аналитика учета номенклатуры. ПобочныеИзделия
			ПроизвольныйСклад =
			"ВЫБОР
			|	КОГДА Коллекция.НаправлениеВыпуска =
			|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию)
			...
			РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ДокументОбъект.ПобочныеИзделия,
																			МестаУчета,
																			ИменаПолей);
			
			// Виды запасов. ВыходныеИзделия
			Если Выборка.ВыходныеИзделияВЗ Тогда
				ДокументОбъект.ВидыЗапасовВыходныеИзделия.Очистить();
				Для Каждого ТекСтрока Из ДокументОбъект.ВыходныеИзделия Цикл
					Если Не ТекСтрока.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа
						И Не ТекСтрока.УдалитьВидЗапасов = Справочники.ВидыЗапасов.ПустаяСсылка() Тогда
						НоваяСтрока = ДокументОбъект.ВидыЗапасовВыходныеИзделия.Добавить();
			...
			// Виды запасов. ПобочныеИзделия
			Если Выборка.ПобочныеИзделияВЗ Тогда
				ДокументОбъект.ВидыЗапасовПобочныеИзделия.Очистить();
				Для Каждого ТекСтрока Из ДокументОбъект.ПобочныеИзделия Цикл
					Если Не ТекСтрока.Номенклатура.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа
						И Не ТекСтрока.УдалитьВидЗапасов = Справочники.ВидыЗапасов.ПустаяСсылка() Тогда
						НоваяСтрока = ДокументОбъект.ВидыЗапасовПобочныеИзделия.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
						НоваяСтрока.ВидЗапасов = ТекСтрока.УдалитьВидЗапасов;
						ТекСтрока.УдалитьВидЗапасов = Справочники.ВидыЗапасов.ПустаяСсылка();
					КонецЕсли;
			...
			// Виды запасов получателя. МатериалыИРаботы
			Если Выборка.МатериалыИРаботыВЗ Тогда
				Для Каждого Строка Из ДокументОбъект.ВидыЗапасовМатериалыИРаботы Цикл
					Если Не ЗначениеЗаполнено(Строка.ВидЗапасовПолучателя) Тогда
						Строка.ВидЗапасовПолучателя = Строка.ВидЗапасов;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			
			// Перенос отправителя материалов в склад/подразделение.
			Если Выборка.МатериалыИРаботыОтправитель Тогда
				ПодразделенияКладовых = Новый Соответствие;
				Для Каждого Строка Из ДокументОбъект.МатериалыИРаботы Цикл
					
					Если Строка.УдалитьОтправитель = Неопределено Тогда
						Продолжить;
					КонецЕсли;
					
					Если ТипЗнч(Строка.УдалитьОтправитель) = Тип("СправочникСсылка.СтруктураПредприятия") Тогда
						Строка.Подразделение = Строка.УдалитьОтправитель;
					ИначеЕсли ТипЗнч(Строка.УдалитьОтправитель) = Тип("СправочникСсылка.Склады") Тогда
						
						Строка.Склад = Строка.УдалитьОтправитель;
						
						Если ПодразделенияКладовых[Строка.УдалитьОтправитель] = Неопределено Тогда
							ПодразделенияКладовых.Вставить(Строка.УдалитьОтправитель,
														ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Строка.УдалитьОтправитель,
																									"Подразделение"));
						КонецЕсли;
						
						Строка.Подразделение = ПодразделенияКладовых[Строка.УдалитьОтправитель];
						
					КонецЕсли;
					Строка.УдалитьОтправитель = Неопределено;
					
				КонецЦикла;
			КонецЕсли;
			
			// Обработчик заполнения нового реквизита ""Способ распределения затрат на выходные изделия"" в документе.
			Если Не ЗначениеЗаполнено(ДокументОбъект.СпособРаспределенияЗатратНаВыходныеИзделия) Тогда
			...
			Если Выборка.НомерГруппыЗатратОтходы
				ИЛИ Выборка.НомерГруппыЗатратМатериалы
				ИЛИ Выборка.НомерГруппыЗатратТруд Тогда
				
				НомераГруппЗатрат = Новый Соответствие;
				Для Каждого ТекПродукция Из ДокументОбъект.ВыходныеИзделия Цикл
					Если НомераГруппЗатрат[ТекПродукция.УдалитьАналитикаУчетаПартийПроизводства] = Неопределено Тогда
						НомераГруппЗатрат.Вставить(
							ТекПродукция.УдалитьАналитикаУчетаПартийПроизводства,
							ТекПродукция.НомерГруппыЗатрат);
					КонецЕсли;
				КонецЦикла;
				
				// Заполнение номера группы затрат в побочных изделиях.
				Если Выборка.НомерГруппыЗатратОтходы Тогда
					ЗаполнитьНомерГруппыЗатратПриПереходеНаНовуюВерсию(ДокументОбъект, "ПобочныеИзделия", НомераГруппЗатрат);
				КонецЕсли;
				
				// Заполнение номера группы затрат в материалах.
				Если Выборка.НомерГруппыЗатратМатериалы Тогда
					ЗаполнитьНомерГруппыЗатратПриПереходеНаНовуюВерсию(ДокументОбъект, "МатериалыИРаботы", НомераГруппЗатрат);
				КонецЕсли;
				
				// Заполнение номера группы затрат в трудозатратах.
				Если Выборка.НомерГруппыЗатратТруд Тогда
					ЗаполнитьНомерГруппыЗатратПриПереходеНаНовуюВерсию(ДокументОбъект, "Трудозатраты", НомераГруппЗатрат);
				КонецЕсли;
				
			КонецЕсли;
			
			// Заполнение номера группы затрат в видах запасов выходных изделий.
			Если Выборка.НомерГруппыЗатратВИ Тогда
				ЗаполнитьДополнительныеПоляВидовЗапасовВыходныеИзделия(ДокументОбъект);
			КонецЕсли;
			
			// Заполнение номера группы затрат в видах запасов побочных изделий.
			Если Выборка.НомерГруппыЗатратПИ Тогда
				ЗаполнитьДополнительныеПоляВидовЗапасовПобочныеИзделия(ДокументОбъект);
			КонецЕсли;
			
			// Заполнение подразделения затрат
			// и номера группы затрат в видах запасов материалов.
			Если Выборка.НомерГруппыЗатратМ
				ИЛИ Выборка.МатериалыИРаботыВЗПодразделение Тогда
				ЗаполнитьДополнительныеПоляВидовЗапасовМатериалыИРаботы(ДокументОбъект);
			КонецЕсли;
...
Процедура ЗаполнитьДополнительныеПоляВидовЗапасовВыходныеИзделия(ДокументОбъект)
	
	ПоляГруппировки = "АналитикаУчетаНоменклатуры, НаправлениеВыпуска, Получатель,
				|Спецификация, АналитикаРасходов, АналитикаАктивовПассивов,
				|ИдентификаторСтроки, НомерГруппыЗатрат, ПередатьДавальцу,
				|КодСтроки";
	ПоляПоказателей = "ДоляСтоимости, Количество";
	
	ТаблицаТоваров = ДокументОбъект.ВыходныеИзделия.Выгрузить(, ПоляГруппировки + ", " + ПоляПоказателей);
	ТаблицаТоваров.Свернуть(ПоляГруппировки, ПоляПоказателей);
	
	ТаблицаВидовЗапасов = ДокументОбъект.ВидыЗапасовВыходныеИзделия.Выгрузить(, "АналитикаУчетаНоменклатуры, ВидЗапасов,
																				|НомерГТД, Количество");
	ТаблицаВидовЗапасов.Свернуть("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД", "Количество");
	ТаблицаВидовЗапасов.Индексы.Добавить("АналитикаУчетаНоменклатуры");
	...
	Для Каждого ТекТовар Из ТаблицаТоваров Цикл
		...
		КоличествоТоваров = ТекТовар.Количество;
		ДоляСтоимостиТоваров = ТекТовар.ДоляСтоимости;
		
		ЗаполнитьЗначенияСвойств(ОтборТоваров, ТекТовар);
		
		Для Каждого СтрокаЗапасов Из ТаблицаВидовЗапасов.НайтиСтроки(ОтборТоваров) Цикл
			...
			НоваяСтрока.НаправлениеВыпуска			= ТекТовар.НаправлениеВыпуска;
			НоваяСтрока.Получатель					= ТекТовар.Получатель;
			НоваяСтрока.Спецификация				= ТекТовар.Спецификация;
			НоваяСтрока.АналитикаРасходов			= ТекТовар.АналитикаРасходов;
			НоваяСтрока.АналитикаАктивовПассивов	= ТекТовар.АналитикаАктивовПассивов;
			НоваяСтрока.ИдентификаторСтроки			= ТекТовар.ИдентификаторСтроки;
			НоваяСтрока.ПередатьДавальцу			= ТекТовар.ПередатьДавальцу;
			НоваяСтрока.КодСтроки					= ТекТовар.КодСтроки;
			НоваяСтрока.НомерГруппыЗатрат			= ТекТовар.НомерГруппыЗатрат;
			НоваяСтрока.Количество					= Количество;
...
Процедура ЗаполнитьДополнительныеПоляВидовЗапасовПобочныеИзделия(ДокументОбъект)
	
	ПоляГруппировки = "АналитикаУчетаНоменклатуры, Подразделение, НаправлениеВыпуска,
						|ПодразделениеПолучатель, Получатель, СтатьяКалькуляции, АналитикаРасходов,
						|АналитикаАктивовПассивов, ИдентификаторСтроки, НомерГруппыЗатрат,
						|ПередатьДавальцу";
	ПоляПоказателей = "Количество, Сумма";
	ТаблицаТоваров = ДокументОбъект.ПобочныеИзделия.Выгрузить(, ПоляГруппировки + ", " + ПоляПоказателей);
	ТаблицаТоваров.Свернуть(ПоляГруппировки, ПоляПоказателей);
	
	ТаблицаВидовЗапасов = ДокументОбъект.ВидыЗапасовПобочныеИзделия.Выгрузить(, "АналитикаУчетаНоменклатуры, ВидЗапасов,
																				|НомерГТД, Количество");
	ТаблицаВидовЗапасов.Свернуть("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД", "Количество");
	ТаблицаВидовЗапасов.Индексы.Добавить("АналитикаУчетаНоменклатуры");
	...
	Для Каждого ТекТовар Из ТаблицаТоваров Цикл
		...
		Для Каждого СтрокаЗапасов Из ТаблицаВидовЗапасов.НайтиСтроки(ОтборТоваров) Цикл
			...
			НоваяСтрока.Подразделение				= ТекТовар.Подразделение;
			НоваяСтрока.НаправлениеВыпуска			= ТекТовар.НаправлениеВыпуска;
			НоваяСтрока.ПодразделениеПолучатель		= ТекТовар.ПодразделениеПолучатель;
			НоваяСтрока.Получатель					= ТекТовар.Получатель;
			НоваяСтрока.СтатьяКалькуляции			= ТекТовар.СтатьяКалькуляции;
			НоваяСтрока.АналитикаРасходов			= ТекТовар.АналитикаРасходов;
			НоваяСтрока.АналитикаАктивовПассивов	= ТекТовар.АналитикаАктивовПассивов;
			НоваяСтрока.ИдентификаторСтроки			= ТекТовар.ИдентификаторСтроки;
			НоваяСтрока.ПередатьДавальцу			= ТекТовар.ПередатьДавальцу;
...
Процедура ЗаполнитьДополнительныеПоляВидовЗапасовМатериалыИРаботы(ДокументОбъект)
	
	ПоляГруппировки = "АналитикаУчетаНоменклатуры, Подразделение, СтатьяКалькуляции,
					|НомерГруппыЗатрат";
	ПоляПоказателей = "Количество";
	ТаблицаТоваров = ДокументОбъект.МатериалыИРаботы.Выгрузить(, ПоляГруппировки + ", " + ПоляПоказателей);
	ТаблицаТоваров.Свернуть(ПоляГруппировки, ПоляПоказателей);
	
	ТаблицаВидовЗапасов = ДокументОбъект.ВидыЗапасовМатериалыИРаботы.Выгрузить(, "АналитикаУчетаНоменклатуры, ВидЗапасов,
																				|НомерГТД, Количество");
	ТаблицаВидовЗапасов.Свернуть("АналитикаУчетаНоменклатуры, ВидЗапасов, НомерГТД", "Количество");
	ТаблицаВидовЗапасов.Индексы.Добавить("АналитикаУчетаНоменклатуры");
	
	ТипыЗапасов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ТаблицаВидовЗапасов.ВыгрузитьКолонку("ВидЗапасов"),
															"ТипЗапасов");
	
	ДокументОбъект.ВидыЗапасовМатериалыИРаботы.Очистить();
	...
	Для Каждого ТекТовар Из ТаблицаТоваров Цикл
		...
		Для Каждого СтрокаЗапасов Из ТаблицаВидовЗапасов.НайтиСтроки(ОтборТоваров) Цикл
			...
			НоваяСтрока = ДокументОбъект.ВидыЗапасовМатериалыИРаботы.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаЗапасов);
			
			Если ТипыЗапасов[СтрокаЗапасов.ВидЗапасов] = Перечисления.ТипыЗапасов.КомиссионныйТовар Тогда
				НоваяСтрока.ВидЗапасовПолучателя = Справочники.ВидыЗапасов.ВидЗапасовДокумента(ДокументОбъект.Организация,,
																								ТекТовар);
			Иначе
...
Процедура ЗаполнитьНомерГруппыЗатратПриПереходеНаНовуюВерсию(ДокументОбъект, ИмяТЧ, НомераГруппЗатрат)
	
	Для Каждого ТекСтрока Из ДокументОбъект[ИмяТЧ] Цикл
		ТекСтрока.НомерГруппыЗатрат = НомераГруппЗатрат[ТекСтрока.УдалитьАналитикаУчетаПартийПроизводства];
	КонецЦикла;
