...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("Содержание", УчетУСНСервер.СодержаниеДохода(Перечисления.ХозяйственныеОперации.ОтчетКомитенту));
	//-- НЕ УТ
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ОтчетКомитенту.Ссылка КАК ОтчетКомитенту,
...
// Обработчик обновления УП 2.4.4.
//
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ОбъектыДляОбработки.Следующий() Цикл
		...
		Попытка
			...
			ВзаиморасчетыСервер.ЗаполнитьИдентификаторыСтрокВТабличнойЧасти(Объект.Товары);
