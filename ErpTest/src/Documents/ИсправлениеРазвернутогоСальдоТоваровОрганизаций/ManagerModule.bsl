...
Функция ТекстЗапросаВтТаблицаИсправлений(Запрос, ТекстыЗапроса)
	...
	|		ИНАЧЕ ВЫБОР КОГДА КлючиАналитикиУчетаНоменклатуры.Склад.ЦеховаяКладовая
...
Функция ТекстЗапросаТаблицаТоварыПринятые(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаИсправлений.ВидЗапасовОприходования КАК ВидЗапасов,
	|	ТаблицаИсправлений.НомерГТДОприходования КАК НомерГТД,
	|	ТаблицаИсправлений.КоличествоПолучено КАК Получено,
	|	ВидыЗапасов.Договор КАК Договор,
	|	ВидыЗапасов.ВладелецТовара КАК Владелец
	|ИЗ
	|	ВтТаблицаИсправлений КАК ТаблицаИсправлений
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидыЗапасов
	|		ПО ТаблицаИсправлений.ВидЗапасовОприходования = ВидыЗапасов.Ссылка
	|ГДЕ
	...
	|	ТаблицаИсправлений.ВидЗапасовСписания,
	|	ТаблицаИсправлений.НомерГТДСписания,
	|	ТаблицаИсправлений.КоличествоПолучено,
	|	ВидыЗапасов.Договор,
	|	ВидыЗапасов.ВладелецТовара
	|ИЗ
	|	ВтТаблицаИсправлений КАК ТаблицаИсправлений
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидыЗапасов
	|		ПО ТаблицаИсправлений.ВидЗапасовСписания = ВидыЗапасов.Ссылка
	|ГДЕ
	|	ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|
	|УПОРЯДОЧИТЬ ПО
