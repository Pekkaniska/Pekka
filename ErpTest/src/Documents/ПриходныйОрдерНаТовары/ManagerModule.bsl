...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ПриходныйОрдерНаТовары КАК ПриходныйОрдерНаТовары
	|ГДЕ
	|	ПриходныйОрдерНаТовары.Распоряжение ССЫЛКА Документ.ЗаказПереработчику
	|	И ВЫРАЗИТЬ(ПриходныйОрдерНаТовары.Распоряжение КАК Документ.ЗаказПереработчику).Партнер <> ПриходныйОрдерНаТовары.Отправитель";
	...
	|	Документ.ПриходныйОрдерНаТовары КАК ПриходныйОрдерНаТовары
	|ГДЕ
	|	ПриходныйОрдерНаТовары.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции)
	...
	|	Документ.ПриходныйОрдерНаТовары КАК ПриходныйОрдерНаТовары
	|ГДЕ
	|	ПриходныйОрдерНаТовары.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
	...
	|ИЗ
	|	Документ.ПриходныйОрдерНаТовары КАК ПриходныйОрдерНаТовары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеСырьяОтДавальца.Товары КАК ДокументРаспоряжение
	|	ПО ПриходныйОрдерНаТовары.Распоряжение = ДокументРаспоряжение.Ссылка
	|ГДЕ
	|	НЕ ПриходныйОрдерНаТовары.ЗакрыватьГрафикПоступления
	|	И ДокументРаспоряжение.Ссылка.ПоступлениеПоЗаказам
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	РезультатВТСоглашения = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Параметры.Очередь,
																														...
																														МенеджерВременныхТаблиц); 
	//++ НЕ УТ
	РезультатВТЗаказыПереработчику = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Параметры.Очередь,
	...
	//++ НЕ УТКА
	|	ИЛИ ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2
	//-- НЕ УТКА
	|	)
	|		И ОбъектыДляОбработки.Ссылка.СкладскаяОперация = ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ПриемкаПоПрочемуОприходованию) КАК ТребуетсяИзменениеСкладскойОперации,
	//-- НЕ УТ
	//++ НЕ УТ
	|	ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику
	|		И ВЫРАЗИТЬ(ОбъектыДляОбработки.Ссылка.Распоряжение КАК Документ.ЗаказПереработчику).Партнер <> ОбъектыДляОбработки.Ссылка.Отправитель КАК ТребуетсяИзменениеОтправителя,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ИЛИ (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
	|		И ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику)
	|	ИЛИ (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства)
	|		И ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику)
	|	ИЛИ (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
	|		И ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ПоступлениеОтПереработчика)
	|	ИЛИ (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства)
	|		И ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ПоступлениеОтПереработчика)
	//-- НЕ УТ
	//++ НЕ УТКА
	|	ИЛИ (ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2
	|		И ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции))
	|	ИЛИ (ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2
	|		И ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства))
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	|			КОГДА ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказНаПроизводство
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад)
	|			КОГДА ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказДавальца
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтДавальца)
	|			КОГДА ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2
	|				И (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукции)
	|				ИЛИ ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства))
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
	//-- НЕ УТКА
	//++ НЕ УТ
	|			КОГДА ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика)
	|			КОГДА ((ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
	|					И ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику)
	|				ИЛИ (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства)
	|					И ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику)
	|				ИЛИ (ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)
	...
	|		И (
	|			(РаспоряжениеВозврат.ЗаявкаНаВозвратТоваровОтКлиента <> ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента.ПустаяСсылка)
	|			И РаспоряжениеВозврат.Склад.ИспользоватьОрдернуюСхемуПриПоступлении
	|			И РаспоряжениеВозврат.Дата >= РаспоряжениеВозврат.Склад.ДатаНачалаОрдернойСхемыПриПоступлении
	|			И РаспоряжениеВозврат.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным))
	//++ НЕ УТ
	|			ИЛИ
	|
	|			(РаспоряжениеПоступлениеОтПереработчика.Ссылка.ПоступлениеПоЗаказам
	|			И РаспоряжениеПоступлениеОтПереработчика.Склад.ИспользоватьОрдернуюСхемуПриПоступлении
	|			И РаспоряжениеПоступлениеОтПереработчика.Ссылка.Дата >= РаспоряжениеПоступлениеОтПереработчика.Склад.ДатаНачалаОрдернойСхемыПриПоступлении
	|			И РаспоряжениеПоступлениеОтПереработчика.Ссылка.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным))
	//-- НЕ УТ
	//++ НЕ УТКА
	|
	|			ИЛИ
	|
	|			(РаспоряжениеПоступлениеСырьяОтДавальца.Ссылка.ПоступлениеПоЗаказам
	|			И РаспоряжениеПоступлениеСырьяОтДавальца.Склад.ИспользоватьОрдернуюСхемуПриПоступлении
	|			И РаспоряжениеПоступлениеСырьяОтДавальца.Ссылка.Дата >= РаспоряжениеПоступлениеСырьяОтДавальца.Склад.ДатаНачалаОрдернойСхемыПриПоступлении
	|			И РаспоряжениеПоступлениеСырьяОтДавальца.Ссылка.ВариантПриемкиТоваров = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным))
	//-- НЕ УТКА
	|
	|		)
	|	ТОГДА ИСТИНА
	...
	//++ НЕ УТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНеобработанныеЗаказыПереработчику КАК НеобработанныеЗаказыПереработчику
	|		ПО (НеобработанныеЗаказыПереработчику.Ссылка = ОбъектыДляОбработки.Ссылка.Распоряжение)
	//-- НЕ УТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента КАК РаспоряжениеВозврат
	|		ПО ОбъектыДляОбработки.Ссылка.Распоряжение = РаспоряжениеВозврат.Ссылка
	//++ НЕ УТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеОтПереработчика.Товары КАК РаспоряжениеПоступлениеОтПереработчика
	|		ПО ОбъектыДляОбработки.Ссылка.Распоряжение = РаспоряжениеПоступлениеОтПереработчика.Ссылка
	//-- НЕ УТ
	//++ НЕ УТКА
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеСырьяОтДавальца.Товары КАК РаспоряжениеПоступлениеСырьяОтДавальца
	|		ПО ОбъектыДляОбработки.Ссылка.Распоряжение = РаспоряжениеПоступлениеСырьяОтДавальца.Ссылка
	//-- НЕ УТКА
	|ГДЕ
	|	НеобработанныеЗаписиРеестра.Ссылка ЕСТЬ NULL
	|	И НеобработанныеСоглашения.Ссылка ЕСТЬ NULL
	//++ НЕ УТ
	|	И НеобработанныеЗаказыПереработчику.Ссылка ЕСТЬ NULL
	//-- НЕ УТ
	|
	|СГРУППИРОВАТЬ ПО
	...
	//++ НЕ УТ
	|	ОбъектыДляОбработки.Ссылка.Распоряжение ССЫЛКА Документ.ЗаказПереработчику
	|		И ВЫРАЗИТЬ(ОбъектыДляОбработки.Ссылка.Распоряжение КАК Документ.ЗаказПереработчику).Партнер <> ОбъектыДляОбработки.Ссылка.Отправитель,
	//-- НЕ УТ
	|	(ОбъектыДляОбработки.Ссылка.ХозяйственнаяОперация В (&ОперацииДляЗамены)
	|		И ОбъектыДляОбработки.Ссылка.СкладскаяОперация <> ЗНАЧЕНИЕ(Перечисление.СкладскиеОперации.ВозвратНепринятыхТоваров))
	//++ НЕ УТ
	...
	//++ НЕ УТ
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТНеобработанныеЗаказыПереработчику", РезультатВТЗаказыПереработчику.ИмяВременнойТаблицы);
	//-- НЕ УТ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			//++ НЕ УТ
			Если Выборка.ТребуетсяИзменениеОтправителя Тогда
			...
			Если Выборка.ТребуетсяИзменениеСкладскойОперации Тогда
				ДокументОбъект.СкладскаяОперация = СкладыКлиентСервер.СкладскаяОперацияПриемкиПоХозяйственнойОперации(ДокументОбъект.ХозяйственнаяОперация);
				ОбъектИзменен = Истина;
			КонецЕсли;
			//-- НЕ УТ
			...
			Если Выборка.ТребуетсяИзменениеОтправителя Тогда
				...
				ДокументОбъект.Отправитель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Распоряжение, "Партнер");
				ОбъектИзменен = Истина;
