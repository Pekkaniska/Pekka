...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ВводОстатковЛокализация.ТекстОтраженияВРеглУчете();
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат ВводОстатковЛокализация.ТекстЗапросаВТОтраженияВРеглУчете();
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбраннаяОперация) Тогда
		...
		//++ НЕ УТ
		СоответствиеТиповОперацийФормам.Вставить(ПредопределенноеЗначение("Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН"), "ФормаРасходыПриУСН");
		СоответствиеТиповОперацийФормам.Вставить(ПредопределенноеЗначение("Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам"), "ФормаРасходыПриУСН");
		СоответствиеТиповОперацийФормам.Вставить(ПредопределенноеЗначение("Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам"), "ФормаРасходыПриУСН");
		//-- НЕ УТ
...
Функция ТекстЗапросаТаблицаДенежныеСредстваНаличные(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаКассы.СуммаУпр <> 0 ТОГДА
	|			ТаблицаКассы.СуммаУпр
...
Функция ТекстЗапросаТаблицаДенежныеСредстваБезналичные(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА ТаблицаБанковскиеСчета.СуммаУпр <> 0 ТОГДА
	|			ТаблицаБанковскиеСчета.СуммаУпр
...
Функция ТекстЗапросаТаблицаДенежныеСредстваУПодотчетныхЛиц(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА РасчетыСПодотчетниками.СуммаУпр <> 0
	|			ТОГДА РасчетыСПодотчетниками.СуммаУпр
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И Товары.Ссылка.ОтражатьВОперативномУчете
	...
	|ГДЕ
	|	Товары.Ссылка.ОтражатьВОперативномУчете
...
Функция ТекстЗапросаТаблицаРасчетыПоФинансовымИнструментам(Запрос, ТекстыЗапроса, Регистры)
	...
	|	КОНЕЦ                                           КАК Сумма,
	|	ВЫБОР 
	|		КОГДА РасчетыПоКредитамДепозитам.СуммаУпр <> 0
	|			ТОГДА РасчетыПоКредитамДепозитам.СуммаУпр
	...
	|				КОНЕЦ
	|	КОНЕЦ                                           КАК СуммаУпр,
	|	ВЫБОР КОГДА РасчетыПоКредитамДепозитам.СуммаРегл > 0
...
Функция ТекстЗапросаТаблицаДенежныеСредстваВПути(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ВЫБОР
	|		КОГДА РасчетыПоЭквайрингу.СуммаУпр <> 0
	|			ТОГДА РасчетыПоЭквайрингу.СуммаУпр
...
Процедура УстановитьПараметрыФормированияРегистровУчетаПартийНДС(Запрос)
	...
	ИмяРегистраУчетаПартийНДС = УниверсальныеМеханизмыПартийИСебестоимости.ИмяРегистраУчетаПартийНДС(Запрос.Параметры.Организация, Запрос.Параметры.Период);
...
Функция ТекстЗапросаТаблицаРасходыПриУСН(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И Операция.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН)
	|
	|СГРУППИРОВАТЬ ПО
	|	Операция.Дата,
...
Функция ТекстЗапросаТаблицаРасходыПриУСНМатериалы(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ТОГДА ТаблицаРасходыПриУСН.ДокументВозникновенияРасходов
	...
	|ГДЕ
	|	Операция.Ссылка = &Ссылка
	|	И Операция.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам)
	|
	|СГРУППИРОВАТЬ ПО
	|	Операция.Дата,
	|	Операция.Организация,
...
Функция ТекстЗапросаТаблицаРасходыПриУСНТовары(Запрос, ТекстыЗапроса, Регистры)
	...
	|ГДЕ
	|	Операция.Ссылка = &Ссылка
	|	И Операция.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам)
	|
	|СГРУППИРОВАТЬ ПО
	|	Операция.Дата,
	|	Операция.Организация,
...
Процедура СформироватьДокументыВводаОстатковПоДаннымОперативногоУчета(Параметры, АдресХранилища) Экспорт
	...
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиВКассах, 
		"Кассы");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиНаБанковскихСчетах, 
		"БанковскиеСчета");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов, 
		"РасчетыСПартнерами");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиПередПоставщиками, 
		"РасчетыСПартнерами");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиПодотчетников, 
		"РасчетыСПодотчетниками");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ПерерасходыПодотчетныхСредств, 
		"РасчетыСПодотчетниками");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов, 
		"РасчетыСПартнерами");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам, 
		"РасчетыСПартнерами");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасчетовМеждуОрганизациямиПоАвансам, 
		"РасчетыМеждуОрганизациями");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасчетовМеждуОрганизациямиПоРеализациям, 
		"РасчетыМеждуОрганизациями");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров, 
		"Товары");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
	...
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		...
		"Товары");
	СоответствиеТиповОперацийЗаполняемымТабличнымЧастям.Вставить(
		Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоДоговорамКредитовИДепозитов, 
	...
	Для каждого ТипОперации из СоответствиеТиповОперацийЗаполняемымТабличнымЧастям Цикл
		...
		Пока ВыборкаОстатков.Следующий() Цикл
			...
			Если КлючевыеПоляОтличаются(ДокументВводаНачальныхОстатков, ВыборкаОстатков) Тогда
				...
				ДокументВводаНачальныхОстатков.ОтражатьВБУиНУ = ОтражатьВБУиНУ;
				ДокументВводаНачальныхОстатков.ОтражатьВУУ    = ОтражатьВУУ;
				ДокументВводаНачальныхОстатков.Комментарий    = НСтр("ru = '#Ввод остатков по данным оперативного учета.'");
				
				Если ТипОперации.Ключ = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходов Тогда
					ДокументВводаНачальныхОстатков.ВидДеятельностиНДС = Справочники.Организации.НалогообложениеНДС(
...
Функция ОстаткиПоТипуОперации(Дата, ТипОперации, МассивОрганизаций, ДополнительныйОтбор = Неопределено, ВводОстатков22 = Ложь) Экспорт
	...
	ИначеЕсли ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов
		...
		Возврат ОстаткиПоРасчетамСПартнерами(ДатаОстатков, ТипОперации, МассивОрганизаций, ВводОстатков22);
...
Функция КлючевыеПоляРеглУчетаПоТипуОперации(ТипОперации) Экспорт
	...
	Если ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТоваровПереданныхНаКомиссию
			ИЛИ ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТоваровПолученныхНаКомиссию Тогда
...
Функция ОстаткиПоРасчетамСПартнерами(ДатаОстатков, ТипОперации, МассивОрганизаций, ВводОстатков22)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		...
		|			&Дата,
		|			(&ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов)
		|				ИЛИ &ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов))
		...
		|			&Дата,
		|			(&ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиПередПоставщиками)
		|				ИЛИ &ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам))
		|			И АналитикаУчетаПоПартнерам В (ВЫБРАТЬ КлючАналитики ИЗ ВтКлючиАналитики)) КАК ДанныеПоОстаткам
...
Функция ОстаткиПоТоварам(ДатаОстатков, ТипОперации, МассивОрганизаций, ДопОтбор)
	...
	|		КОГДА УчетныеПолитикиОрганизаций.ПрименяетсяУчетНДСПоФактическомуИспользованию
	...
	|		ИНАЧЕ СебестоимостьТоваровОстатки.ВидЗапасов.НалогообложениеНДС
	|	КОНЕЦ КАК НалогообложениеНДС,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ДанныеАналитики.Склад КАК Справочник.СтруктураПредприятия), АналитикаПодразделение.Ссылка) КАК Подразделение,
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		Документ.ВводОстатков.ДетализацияПартий КАК ТаблицаДетализацияПартий
	|	ПО
	|		ВводОстатков.Ссылка = ТаблицаДетализацияПартий.Ссылка
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваров
	|	ПО
	|		ВводОстатков.Ссылка = СебестоимостьТоваров.Регистратор
	|		И СебестоимостьТоваров.Партия = ВводОстатков.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ДетализацияПартийТоваровДляНДСиУСН КАК ДетализацияПартийТоваровДляНДСиУСН
	|	ПО
	|		ВводОстатков.Ссылка = ДетализацияПартийТоваровДляНДСиУСН.Регистратор
	|		И ДетализацияПартийТоваровДляНДСиУСН.ДокументПоступления = ВводОстатков.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ПартииТоваровОрганизаций КАК ПартииТоваровОрганизаций
	|	ПО
	|		ВводОстатков.Ссылка = ПартииТоваровОрганизаций.Регистратор
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ПартииТоваровПереданныеНаКомиссию КАК ПартииТоваровПереданныеНаКомиссию
	|	ПО
	|		ВводОстатков.Ссылка = ПартииТоваровПереданныеНаКомиссию.Регистратор
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрНакопления.ПартииПроизводственныхЗатрат КАК ПартииПроизводственныхЗатрат
	|	ПО
	|		ВводОстатков.Ссылка = ПартииПроизводственныхЗатрат.Регистратор
	|
	|ГДЕ
	|	ВводОстатков.ТипОперации В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров),
	...
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиТоваровПолученныхНаКомиссию),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиМатериаловПереданныхПереработчикам))
	// Документ не имеет данных специфических для нового ввода остатков 2.4
	|	И (ТаблицаДетализацияПартий.Ссылка ЕСТЬ NULL
	|		И ВводОстатков.Партия = ЗНАЧЕНИЕ(Документ.ПервичныйДокумент.ПустаяСсылка))
	// Документ имеет движения свойственные вводу остатков 2.2
	|	И (НЕ СебестоимостьТоваров.Регистратор ЕСТЬ NULL
	|		ИЛИ НЕ ДетализацияПартийТоваровДляНДСиУСН.Регистратор ЕСТЬ NULL
	|		ИЛИ НЕ ПартииТоваровОрганизаций.Регистратор ЕСТЬ NULL
	|		ИЛИ НЕ ПартииТоваровПереданныеНаКомиссию.Регистратор ЕСТЬ NULL
	|		ИЛИ НЕ ПартииПроизводственныхЗатрат.Регистратор ЕСТЬ NULL)
	|	И НЕ ВводОстатков.ВводОстатков22
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		Документ.ВводОстатков.РасчетыСПартнерами КАК ТаблицаРасчетыСПартнерами 
	|	ПО
	|		ВводОстатков.Ссылка = ТаблицаРасчетыСПартнерами.Ссылка
	|		И (ТаблицаРасчетыСПартнерами.ДокументРасчетов <> ЗНАЧЕНИЕ(Документ.ПервичныйДокумент.ПустаяСсылка)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ТаблицаРасчетыСПартнерами.ОбъектРасчетов) = ТИП(Документ.ПервичныйДокумент))
	|ГДЕ
	|	ВводОстатков.ТипОперации В (
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиКлиентов),
	|		ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиЗадолженностиПередПоставщиками))
	|	И ТаблицаРасчетыСПартнерами.Ссылка ЕСТЬ NULL
	|	И НЕ ВводОстатков.ВводОстатков22
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВтЗаблокированныеСебестоимостьТоваров";
	...
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВтЗаблокированныеДетализацияПартийТоваровДляНДСиУСН";
	РезультатыСозданияВТ = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Параметры.Очередь,
	...
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВтЗаблокированныеПартииТоваровОрганизаций";
	РезультатыСозданияВТ = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Параметры.Очередь,
	...
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВтЗаблокированныеПартииТоваровПереданныеНаКомиссию";
	РезультатыСозданияВТ = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Параметры.Очередь,
	...
	ДопПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДопПараметры.ИмяВременнойТаблицы = "ВтЗаблокированныеПартииПроизводственныхЗатрат";
	РезультатыСозданияВТ = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Параметры.Очередь,
	...
	|ИЗ
	|	ВТДляОбработки КАК ОбъектыДляОбработки
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтЗаблокированныеСебестоимостьТоваров КАК ВтЗаблокированныеСебестоимостьТоваров
	|	ПО
	|		ОбъектыДляОбработки.Ссылка = ВтЗаблокированныеСебестоимостьТоваров.Регистратор
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтЗаблокированныеСебестоимостьТоваров КАК ВтЗаблокированныеДетализацияПартийТоваровДляНДСиУСН
	|	ПО
	|		ОбъектыДляОбработки.Ссылка = ВтЗаблокированныеДетализацияПартийТоваровДляНДСиУСН.Регистратор
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтЗаблокированныеСебестоимостьТоваров КАК ВтЗаблокированныеПартииТоваровОрганизаций
	|	ПО
	|		ОбъектыДляОбработки.Ссылка = ВтЗаблокированныеПартииТоваровОрганизаций.Регистратор
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтЗаблокированныеСебестоимостьТоваров КАК ВтЗаблокированныеПартииТоваровПереданныеНаКомиссию
	|	ПО
	|		ОбъектыДляОбработки.Ссылка = ВтЗаблокированныеПартииТоваровПереданныеНаКомиссию.Регистратор
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтЗаблокированныеСебестоимостьТоваров КАК ВтЗаблокированныеПартииПроизводственныхЗатрат
	|	ПО
	|		ОбъектыДляОбработки.Ссылка = ВтЗаблокированныеСебестоимостьТоваров.Регистратор
	|ГДЕ
	|	ВтЗаблокированныеСебестоимостьТоваров.Регистратор ЕСТЬ NULL
	|	И ВтЗаблокированныеДетализацияПартийТоваровДляНДСиУСН.Регистратор ЕСТЬ NULL
	|	И ВтЗаблокированныеПартииТоваровОрганизаций.Регистратор ЕСТЬ NULL
	|	И ВтЗаблокированныеПартииТоваровПереданныеНаКомиссию.Регистратор ЕСТЬ NULL
	|	И ВтЗаблокированныеПартииПроизводственныхЗатрат.Регистратор ЕСТЬ NULL
	|";
	...
	Пока ОбъектыДляОбработки.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Регистратор", ОбъектыДляОбработки.Ссылка);
			...
			ЭлементБлокировки = Блокировка.Добавить("РегистрНакопления.ПартииТоваровПереданныеНаКомиссию.НаборЗаписей");
			ЭлементБлокировки.УстановитьЗначение("Регистратор", ОбъектыДляОбработки.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
