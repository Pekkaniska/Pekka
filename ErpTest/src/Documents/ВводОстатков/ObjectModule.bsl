...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++ НЕ УТ
	УчетУСНСервер.ОтразитьПрочиеРасходыПриУСН(ДополнительныеСвойства, Движения, Отказ);
	УчетУСНСервер.ОтразитьРасходыПриУСНМатериалы(ДополнительныеСвойства, Движения, Отказ);
	УчетУСНСервер.ОтразитьРасходыПриУСНТовары(ДополнительныеСвойства, Движения, Отказ);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Если Не Отказ И ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТМЦВЭксплуатации Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ НЕ УТ
	Если ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН
		ИЛИ ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам
		ИЛИ ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиТоваровПереданныхНаКомиссию Тогда
		ПараметрыЗаполненияНДС = Документы.ВводОстатков.ИнициализироватьПараметрыНалогообложенияНДС(ЭтотОбъект);
		УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(НалогообложениеНДС, ПараметрыЗаполненияНДС);
	КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ПараметрыЗаполненияНДСПродажи = Документы.ВводОстатков.ИнициализироватьПараметрыНалогообложенияНДС(ЭтотОбъект);
	УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(НалогообложениеНДС, ПараметрыЗаполненияНДСПродажи);
	...
	Если ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиСобственныхТоваров 
		...
		Если УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПартионныйУчетВерсии21(ДатаПроверки) Тогда
...
Процедура ПроверитьЗаполнениеТМЦВЭксплуатации(НепроверяемыеРеквизиты, Отказ)
	...
	НепроверяемыеРеквизиты.Добавить("ТМЦВЭксплуатации.ИнвентарныйНомер");
	...
	Если Не Результат.Пустой() Тогда
		...
		ШаблонОшибкаПустойИнвентарныйНомер = НСтр("ru='Не заполнена колонка ""Инвентарный номер""
			|в строке %НомерСтроки% списка ""ТМЦ в эксплуатации""'");
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаТЧ = ТМЦВЭксплуатации[Выборка.НомерСтроки-1];
			Если Выборка.ИнвентарныйУчет И (ПустаяСтрока(СтрокаТЧ.ИнвентарныйНомер) Или Выборка.ЕстьОшибкиДубль) Тогда
...
Процедура ПроверитьЗаполнениеРасходовПриУСН(Отказ)
	...
	Для ТекИндекс = 0 По РасходыПриУСН.Количество()-1 Цикл
		
		Строка = РасходыПриУСН[ТекИндекс];
		...
		Если Строка.СуммаОплаты > Строка.СуммаРасходов Тогда
			...
			ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", РасходыПриУСН[ТекИндекс].НомерСтроки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ТекстОшибки,
					ЭтотОбъект,
					ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("РасходыПриУСН", РасходыПриУСН[ТекИндекс].НомерСтроки, "СуммаОплаты"),
					,
					Отказ);
		КонецЕсли;
