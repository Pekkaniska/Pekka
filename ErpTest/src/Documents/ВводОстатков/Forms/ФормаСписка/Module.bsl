...
Функция СоответствиеТиповОпераций()
	...
	//++ НЕ УТ
	ТипыОперацийРасходыУСН = Новый СписокЗначений;
	ТипыОперацийРасходыУСН.Добавить(Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН);
	ТипыОперацийРасходыУСН.Добавить(Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам);
	ТипыОперацийРасходыУСН.Добавить(Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	СоответствиеТиповОперацийВводаОстатков.Вставить("РасходыПоУСН", ТипыОперацийРасходыУСН);
	//-- НЕ УТ
...
Процедура ОбновитьСтатусыТиповОпераций(ТипОперации = Неопределено)
	...
	Запрос.Текст =
		...
		//++ НЕ УТ
		|		КОГДА ПеречислениеТипыОперацийВводаОстатков.Ссылка В (&ТипыОперацийРасходыПоУСН)
		|			ТОГДА &РазделРасходыПоУСН
		//-- НЕ УТ
		...
		//++ НЕ УТ
		|		КОГДА ПеречислениеТипыОперацийВводаОстатков.Ссылка В (&ТипыОперацийРасходыПоУСН)
		|			ТОГДА &ПорядокРазделРасходыПоУСН
		//-- НЕ УТ
		|		КОГДА ПеречислениеТипыОперацийВводаОстатков.Ссылка В (&ТипыОперацийПрочее)
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ТипыОперацийРасходыПоУСН",              ТипыОпераций["РасходыПоУСН"]);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("РазделРасходыПоУСН",                    НСтр("ru='Расходы по УСН'"));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ПорядокРазделРасходыПоУСН",                    5);
	//-- НЕ УТ
...
Процедура ИнициализироватьТаблицуПоясненийКОперациям()
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		//++ НЕ УТ
		|		КОГДА ПеречислениеТипыОперацийВводаОстатков.Ссылка В (
		|				ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН),
		|				ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам),
		|				ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам))
		|			ТОГДА &ЗаголовокРасходыПоУСН
		//-- НЕ УТ
		|	КОНЕЦ КАК Раздел
		|ИЗ
		|	Перечисление.ТипыОперацийВводаОстатков КАК ПеречислениеТипыОперацийВводаОстатков
	...
	//++ НЕ УТ
	ЗаголовокРасходыПоУСН = НСтр("ru='Расходы по УСН'");
	//-- НЕ УТ
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ЗаголовокРасходыПоУСН", ЗаголовокРасходыПоУСН);
	//-- НЕ УТ
	...
	Пока ВыборкаРазделы.Следующий() Цикл
		...
		//++ НЕ УТ
		ИначеЕсли ВыборкаРазделы.Раздел = ЗаголовокРасходыПоУСН Тогда
			НоваяСтрока.Пояснение = НСтр("ru='Вводятся остатки расходов, ожидающих признания для целей УСН'")
		//-- НЕ УТ
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			//++ НЕ УТ 
			ИначеЕсли ВыборкаДетальныеЗаписи.ОперацияВводаОстатков = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН Тогда
				НоваяСтрока.Пояснение = НСтр("ru='Вводятся данные о прочих расходах к признанию для целей УСН'")
			ИначеЕсли ВыборкаДетальныеЗаписи.ОперацияВводаОстатков = Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам Тогда
				НоваяСтрока.Пояснение = НСтр("ru='Вводятся данные о расходах на приобретение материалов к признанию для целей УСН'")
			ИначеЕсли ВыборкаДетальныеЗаписи.ОперацияВводаОстатков = Перечисления.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам Тогда
				НоваяСтрока.Пояснение = НСтр("ru='Вводятся данные о расходах на приобретение товаров к признанию для целей УСН'")
			//-- НЕ УТ
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
