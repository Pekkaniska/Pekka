...
Процедура ЗаполнитьНалогообложениеНДС()
	...
	ПараметрыЗаполненияНДС = Документы.ВводОстатков.ИнициализироватьПараметрыНалогообложенияНДС(Объект);
	УчетНДСУП.ЗаполнитьНалогообложениеНДСПродажи(Объект.НалогообложениеНДС, ПараметрыЗаполненияНДС);
...
Процедура ОбработкаПодбораТоваровИзЗаказаПереработчику(АдресТоваровВХранилище)
	...
	Для Каждого СтрокаТоваров Из СтруктураТаблиц.Товары Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаТЧТовары, СтрокаТоваров);
		СтрокаТЧТовары.СтавкаНДС = СтавкаНДСНоменклатуры(СтрокаТЧТовары.Номенклатура, Объект.Дата);
