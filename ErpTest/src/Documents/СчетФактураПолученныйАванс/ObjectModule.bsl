...
Процедура ЗаполнитьАвансы()
	...
	Для каждого Строка Из ВыданныеАвансы Цикл
		...
		Ставки.Вставить("ОсновнаяСтавка", УчетНДСУП.СтавкаНДСПоУмолчанию(КонецПериода, Истина));
		Ставки.Вставить("ЛьготнаяСтавка", Перечисления.СтавкиНДС.НДС10_110);
		...
		Для каждого Ставка Из Ставки Цикл
			...
			Если Строка["СуммаАвансаРегл" + Ставка.Ключ] <> 0 Тогда
				...
				ПроцентНДС = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(НоваяСтрока.СтавкаНДС);
				НоваяСтрока.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(НоваяСтрока.Сумма, ПроцентНДС);
