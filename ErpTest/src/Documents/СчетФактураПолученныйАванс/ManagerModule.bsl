...
Функция ДобавитьКомандуСоздатьНаОснованииИсправительныйСчетФактура(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетФактураПолученныйАванс) Тогда
		...
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Обработчик = "УчетНДСУПСлужебныйКлиент.ИсправительныйСчетФактураПолученныйАванс";
...
Функция ПолучитьВыданныеАвансы(Организация, НачалоПериода, КонецПериода, РасчетныйДокумент = Неопределено) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.РасчетыСПоставщикамиПоСрокам.Обороты(
	|			&НачалоПериода, 
	|			&ОкончаниеПериода, 
	|			Запись,
	|			(РасчетныйДокумент = &РасчетныйДокумент 
...
// Процедура заполняет новый реквизит НалогообложениеНДС значением по умолчанию и заполняет дату отражения в регл учете
//
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.СчетФактураПолученныйАванс";
	МетаданныеДокумента = Метаданные.Документы.СчетФактураПолученныйАванс;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументОбъект.НалогообложениеНДС.Пустая() Тогда
			ДокументОбъект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДокументОбъект.ДатаОтраженияВРеглУчете) Тогда
			ДокументОбъект.ДатаОтраженияВРеглУчете = 
				?(ДокументОбъект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя,
				ДокументОбъект.ДатаСоставления,
				ДокументОбъект.Дата);
		КонецЕсли;
