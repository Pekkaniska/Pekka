...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПланированиеПроизводства.ДозаполнитьНазначениеВСпецификации(ЭтотОбъект);
	...
	ПодготовитьПараметрыПроверкиЗагрузкиОборудования(ДополнительныеСвойства);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Для Каждого Строка Из ЭтотОбъект.МатериалыИУслуги Цикл
		...
		ИначеЕсли Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить
			ИЛИ Строка.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов
...
Процедура ЗаполнитьПоЗаказуМатериаловВПроизводство(Заказ)
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Товары.НачалоОтгрузки = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА Товары.Ссылка.ЖелаемаяДатаОтгрузки
	|		ИНАЧЕ Товары.НачалоОтгрузки
	|	КОНЕЦ КАК ДатаПотребности,
	|	Товары.НомерСтроки КАК НомерСтроки,
	|	ЗНАЧЕНИЕ(ПЕречисление.СпособыПривязкиОперацийПроизводства.КНачалу) КАК РазмещениеВыпуска,
	|	&НачатьНеРанее КАК НачатьНеРанее
	...
	Для Каждого СтрокаДобавляемыйЭтап Из ТаблицаЭтапов Цикл
		...
		Для Каждого СтрокаМатериал Из НайденныеСтрокиМатериалы Цикл
			...
			Если НоваяСтрокаМатериал.ПроизводитсяВПроцессе Тогда 
				...
				Если СтрокаМатериал.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа
					И НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.НеТребуется Тогда
					НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно;
				ИначеЕсли НЕ НоваяСтрокаМатериал.ЗаказатьНаСклад 
					И НоваяСтрокаМатериал.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.НеТребуется Тогда
					НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.НеТребуется;
				ИначеЕсли НоваяСтрокаМатериал.ЗаказатьНаСклад 
					И НоваяСтрокаМатериал.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.Обособленно Тогда
					НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно;
				КонецЕсли;
				
			Иначе
				
				Если НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно Тогда
					НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно;
				ИначеЕсли НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить
					ИЛИ НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.СоСклада
					ИЛИ НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ИзЗаказов Тогда
					НоваяСтрокаМатериал.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется;
				КонецЕсли;
...
Процедура ПодготовитьПараметрыПроверкиЗагрузкиОборудования(ДополнительныеСвойства)
