...
Функция ПараметрыВыбораСпецификаций(Объект) Экспорт
	...
	ПараметрыВыбораСпецификаций.ИсключитьПроизводствоНаСтороне = Объект.ПроизводствоПоЗаказу И ТипЗнч(Объект.Заказ) = Тип("ДокументСсылка.ЗаказДавальца");
...
Функция ТекстЗапросаЗаказовКОбеспечению() Экспорт
	...
	|	ДанныеУчета.КоличествоОборот           КАК Количество,
	|	
	...
	|				ИЛИ ВЫРАЗИТЬ(Распоряжение КАК Документ.ЗаказНаПроизводство).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство.Создан)
	|			)
	|		) КАК ДанныеУчета
	|		
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство.МатериалыИУслуги КАК ТМатериалыИУслуги
	|	ПО ДанныеУчета.Распоряжение = ТМатериалыИУслуги.Ссылка
	...
	|
	|ГДЕ
	|	ДанныеУчета.КоличествоОборот > 0
	|	И НЕ ТМатериалыИУслуги.ПроизводитсяВПроцессе";
...
Функция ТекстЗапросаТаблицаДвижениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = 
		...
		// В движениях участвуют все отходы, кроме тех, которые передаются напрямую в подразделение (поле склад имеет тип
		// значения СправочникСсылка.Склады).
		|ВЫБРАТЬ
		|	ВЫБОР 
		|		КОГДА ТабличнаяЧасть.ДатаВыпуска > НАЧАЛОПЕРИОДА(ТабличнаяЧасть.ДатаВыпуска, ДЕНЬ) ТОГДА
		|			ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ТабличнаяЧасть.ДатаВыпуска, ДЕНЬ), ДЕНЬ, 1)
...
Функция ТекстЗапросаТаблицаОбеспечениеЗаказовРаботами(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = 
		...
		|
		|	И (&Статус В(ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство.КПроизводству),
		|				 ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаПроизводство.Закрыт))
		|		ИЛИ ТабличнаяЧасть.ПроизводитсяВПроцессе)
		|
		|	И ТабличнаяЧасть.Назначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
...
Функция ПолучитьРезультатЗапросаПоОстаткамПродукцииКОбеспечениюДавальцу(ЗаказДавальца, ТекущийДокумент = Неопределено) Экспорт
	...
	ТекстЗапроса =
		...
		|			Назначение = &Назначение) КАК ТаблицаОстатков
		|		
		|	ОБЪЕДИНИТЬ ВСЕ
		|		
		|	ВЫБРАТЬ
		...
		|	
		|	МИНИМУМ(ВЫБОР КОГДА ЗаказТовары.Ссылка.ЖелаемаяДатаОтгрузки = ДАТАВРЕМЯ(1,1,1) ТОГДА
...
Функция СоздатьВтСпособыПолученияМатериалов(Объект, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	Иначе
		...
		ТекстЗапроса = ПроизводствоСервер.ТекстЗапросаВТСпособыПолученияМатериаловПустая();
		Запрос = Новый Запрос(ТекстЗапроса);
