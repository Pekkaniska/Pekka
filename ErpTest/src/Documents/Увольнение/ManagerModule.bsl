		...
		Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
			...
			Для Каждого СтрокаПериодовРегистрации Из РеквизитыДляПроведения.ПериодыРегистрации Цикл
				...
				Иначе
					...
					Начисления = РасчетЗарплатыРасширенный.СтрокиТаблицыДокументаПоПериодуРегистрации(ДанныеДляПроведения.Начисления, ПериодРегистрации, ЭтоПервыйПериод, СтрокаПериодовРегистрации.Сотрудники, "ПериодРегистрации");
			...
			Для Каждого СтрокаПериодовРегистрации Из РеквизитыДляПроведения.ПериодыРегистрации Цикл
				...
				Иначе
					...
					Если СтрокаПериодовРегистрации.ЭтоПервыйПериод Тогда
						Запрос.Текст = Запрос.Текст + Символы.ПС
							+ "ГДЕ Т.ПериодРегистрации <= &ПериодРегистрации"
					Иначе
						Запрос.Текст = Запрос.Текст + Символы.ПС
							+ "ГДЕ Т.ПериодРегистрации = &ПериодРегистрации"
					КонецЕсли;
					...
					НачисленияПоСотрудникам = РасчетЗарплатыРасширенный.СтрокиТаблицыДокументаПоПериодуРегистрации(ДанныеДляПроведения.НачисленияПоСотрудникам, ПериодРегистрации, ЭтоПервыйПериод, СтрокаПериодовРегистрации.Сотрудники, "ПериодРегистрации");
					...
					УдержанияЗаймов = РасчетЗарплатыРасширенный.СтрокиТаблицыДокументаПоПериодуРегистрации(ДанныеДляПроведения.УдержанияЗаймов, ПериодРегистрации, ЭтоПервыйПериод, СтрокаПериодовРегистрации.ФизическиеЛица, "НачалоМесяцаДатаОкончания", "ФизическоеЛицо");
					...
					ОтработанноеВремяПоСотрудникам = РасчетЗарплатыРасширенный.СтрокиТаблицыДокументаПоПериодуРегистрации(ДанныеДляПроведения.ОтработанноеВремяПоСотрудникам, ПериодРегистрации, ЭтоПервыйПериод, СтрокаПериодовРегистрации.Сотрудники, "ПериодРегистрации");
				...
				ДатаОперацииПоНалогам = УчетНДФЛРасширенный.ДатаОперацииПоДокументу(РеквизитыДляПроведения.Дата, ПериодРегистрацииНалогов);
				...
				ОтражениеЗарплатыВБухучетеРасширенный.СформироватьДвиженияБухучетНачисленияУдержанияПоСотрудникам(
							Движения, Отказ, РеквизитыДляПроведения.Организация, ПериодРегистрацииНалогов,
		...
		Для Каждого ЭлементКоллекции Из ПроверяемаяКоллекция Цикл
			...
			Если ЭлементКоллекции.НачислитьЗарплату Тогда
				...
				Если Не ПустаяСтрока(ТекстСообщения) Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ДокументОбъект.Ссылка, ПутьКПолю, , Отказ);
