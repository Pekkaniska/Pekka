...
Функция ОписаниеСоставаДокумента() Экспорт
	Возврат ЗарплатаКадрыСоставДокументов.ОписаниеСоставаДокументаФизическоеЛицоВШапке("ФизическоеЛицо", "Сотрудник");
КонецФункции
...
Функция ФиксируемыеРеквизиты(Объект)
	...
	Шаблон = ФиксацияВторичныхДанныхВДокументах.ОписаниеФиксируемогоРеквизита();
	Шаблон.ОснованиеЗаполнения = "Сотрудник";
	Шаблон.ИмяГруппы = "НомерКартыМИР";
	Шаблон.ФиксацияГруппы = Истина;
	
	ФиксацияВторичныхДанныхВДокументах.ДобавитьФиксируемыйРеквизит(ФиксируемыеРеквизиты, Шаблон, "НомерКартыМИР", Ложь);
...
Функция РезультатЗапросаДанныхДляПечатиЗаявлений(МассивСсылок)
	...
	|	ЕСТЬNULL(ХэшиЭЛН.Хэш, """") <> """" КАК ЭтоЭЛН
	|ИЗ
	|	Документ.ЗаявлениеСотрудникаНаВыплатуПособия КАК Заявление
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ХэшиЭЛН КАК ХэшиЭЛН
	|		ПО Заявление.НомерЛисткаНетрудоспособности = ХэшиЭЛН.НомерЛисткаНетрудоспособности
...
Функция ПоследнийНомерКартыМИР(Сотрудник, Дата, Ссылка) Экспорт
...
Процедура ОбновитьДокумент(ДокументОбъект) Экспорт
	...
	Если ДокументОбъект.ВерсияДокумента < 2 Тогда
		...
		Если ДокументОбъект.СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.ЧерезБанк
			И ЗначениеЗаполнено(ДокументОбъект.НомерКартыМИР) Тогда
			ДокументОбъект.СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.НаКартуМИР;
		КонецЕсли;
		// Поле СпособВыплатыПособия теперь заполняется еще и на основании поля НомерКартыМИР, которое, для этого, становится фиксируемым.
		ПоследнийНомерКартыМИР = ПоследнийНомерКартыМИР(ДокументОбъект.Сотрудник, ДокументОбъект.Дата, ДокументОбъект.Ссылка);
		Если ДокументОбъект.НомерКартыМИР <> ПоследнийНомерКартыМИР Тогда
			ФиксацияВторичныхДанныхВДокументах.ЗафиксироватьРеквизитОбъекта(ДокументОбъект, "НомерКартыМИР");
	...
	Если ДокументОбъект.ВерсияДокумента < 4 Тогда
		...
		Если ЗначениеЗаполнено(ДокументОбъект.АдресПолучателяРегистрации) Тогда
			...
			ОписаниеАдреса = УчетПособийСоциальногоСтрахования.ОписаниеАдреса(ДокументОбъект.АдресПолучателяРегистрации, ТипАдреса);
		...
		Если ЗначениеЗаполнено(ДокументОбъект.АдресПолучателяФактический) Тогда
			...
			ОписаниеАдреса = УчетПособийСоциальногоСтрахования.ОписаниеАдреса(ДокументОбъект.АдресПолучателяФактический, ТипАдреса);
