...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Перерасчет Тогда
		ПроверитьРеквизитыИзвещенияИзФСС(Отказ);
	КонецЕсли;
	...
	Если СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.НаКартуМИР Тогда
		Если ЗначениеЗаполнено(НомерКартыМИР)
			...
			ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Текст, "НомерКартыМИР");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьРеквизитыНеТребующиеПроверки(ПроверяемыеРеквизиты)
	...
	Если Не (СпособВыплатыПособия = Перечисления.СпособыВыплатыПособия.НаКартуМИР) Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ПроверяемыеРеквизиты, "НомерКартыМИР");
	КонецЕсли;
...
Процедура ПроверитьРеквизитыИзвещенияИзФСС(Отказ)
	ЗаполненРеквизитНомер = ЗначениеЗаполнено(ИзвещениеИзФССНомер);
	ЗаполненРеквизитДата  = ЗначениеЗаполнено(ИзвещенияИзФССДата);
	Если ЗаполненРеквизитНомер И Не ЗаполненРеквизитДата Тогда
		Текст = Нстр("ru = 'В группе ""Извещение о представлении недостающих сведений в ФСС"" не заполнена Дата, но заполнен Номер.'");
		ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Текст, "ИзвещенияИзФССДата");
	ИначеЕсли ЗаполненРеквизитДата И Не ЗаполненРеквизитНомер Тогда
		Текст = Нстр("ru = 'В группе ""Извещение о представлении недостающих сведений в ФСС"" не заполнен Номер, но заполнена Дата.'");
		ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Текст, "ИзвещениеИзФССНомер");
	КонецЕсли;
КонецПроцедуры
...
Функция ОбновитьВторичныеДанныеДокумента() Экспорт
	...
	Если ЗначениеЗаполнено(АдресПолучателяРегистрации) Тогда
		...
		ОписаниеАдреса = УчетПособийСоциальногоСтрахования.ОписаниеАдреса(АдресПолучателяРегистрации, ТипАдреса);
	...
	Если ЗначениеЗаполнено(АдресПолучателяФактический) Тогда
		...
		ОписаниеАдреса = УчетПособийСоциальногоСтрахования.ОписаниеАдреса(АдресПолучателяФактический, ТипАдреса);
...
Процедура ЗаполнитьДанныеСотрудника(ПараметрыФиксации, РеквизитыДокумента, РайонныйКоэффициентЗаполненИзОснования)
	...
	// Номер карты МИР и способ выплаты пособия.
	ПоследнийНомерКартыМИР = Документы.ЗаявлениеСотрудникаНаВыплатуПособия.ПоследнийНомерКартыМИР(Сотрудник, Дата, Ссылка);
	Если ЗначениеЗаполнено(ПоследнийНомерКартыМИР) Тогда
		РеквизитыДокумента.Вставить("НомерКартыМИР", ПоследнийНомерКартыМИР);
