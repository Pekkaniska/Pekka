...
Процедура НомерКартыМИРПриИзменении(Элемент)
	Если Объект.НомерКартыМИР <> "" Тогда
		...
		Объект.НомерКартыМИР = СокрЛП(СтрЗаменить(СтрЗаменить(СтрЗаменить(СтрЗаменить(
			...
			Символ(150), "")); // Тире.
	КонецЕсли;
	ФиксацияВторичныхДанныхВДокументахКлиентСервер.ЗафиксироватьИзменениеРеквизита(ЭтотОбъект, "НомерКартыМИР");
КонецПроцедуры
...
Процедура ОбновитьВидимостьДоступность()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаИзвещениеИзФСС",  "Видимость",   Объект.Перерасчет);
