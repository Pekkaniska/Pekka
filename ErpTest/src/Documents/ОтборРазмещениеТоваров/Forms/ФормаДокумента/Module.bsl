...
//++ Рарус IA 01.11.2016	
&НаКлиенте
Процедура пкТоварыРазмещениеЯчейкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//ОтборСписка.ПравоеЗначение = Форма.Элементы.ТоварыРазмещение.ТекущиеДанные.Ячейка;
	//ОтборСписка.Использование = Истина;

	//ПараметрыФормы = Новый Структура;
	//ПараметрыФормы.Вставить("ФиксированныеНастройки", НастройкиКомп);
	//ПараметрыФормы.Вставить("ВыборГруппИЭлементов", ИспользованиеГруппИЭлементов.Элементы); // Здесь устанавливается возможность выбора не только элементов, но групп

	// 
	//ФормаСписка = ПолучитьФорму("Спровочник.СкладскиеЯчейки.ФормаСписка");
	//ФормаСписка.ФильтрПоТовару = Элементы.Список.ТекущаяСтрока;
	//ФормаСписка.Открыть();

	Список = пкПолучитьСписокЯчеек(Элементы.ТоварыРазмещение.ТекущиеДанные.Номенклатура); 
	
	Если Список.Количество()>0 Тогда
		СтандартнаяОбработка = Ложь;	
		
		//ПараметрыОтбора = Новый Структура("Ссылка", Список); 
		//ПараметрыФормы = Новый Структура("Отбор", ПараметрыОтбора); 
		//ОткрытьФорму("Справочник.СкладскиеЯчейки.Форма.ФормаВыбора", ПараметрыФормы, Элемент, ЭтаФорма);
		
		ФормаВыбора=ПолучитьФорму("Справочник.СкладскиеЯчейки.Форма.ФормаВыбора", , Элемент, ЭтаФорма);
	    ФормаВыбора.Открыть();
		
		//Отбор = ФормаВыбора.Список.Отбор;
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(ФормаВыбора.Список, "Ссылка", Список, ВидСравненияКомпоновкиДанных.ВСписке,, Истина);
...
&НаСервере
Функция пкПолучитьСписокЯчеек(Номенклатура)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	НоменклатурапкСкладскиеЯчейки.Ссылка = &Ссылка
	               |	И НоменклатурапкСкладскиеЯчейки.Ячейка.Владелец = &Владелец";
				   
	Запрос.УстановитьПараметр("Ссылка", Номенклатура);			   
	Запрос.УстановитьПараметр("Владелец", Объект.Склад);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Список.Добавить(Выборка.Ячейка);
	КонецЦикла; 	
...
//++ Рарус Лимаренко 12.12.2017

&НаСервере
Функция ПоместитьВХТоварыОтбор(Адрес)
	
	АдресВх = ПоместитьВоВременноеХранилище(Объект.ТоварыОтбор.Выгрузить(),Адрес);
	Возврат АдресВх; 

КонецФункции

&НаСервере
Процедура ПолучитьЗаполнитьСтаруюТЧ(АдресВХ)
	СтараяТЧ = ПолучитьИзВременногоХранилища(АдресВХ);
	Если СтараяТЧ <> Неопределено Тогда
		Объект.ТоварыОтбор.Загрузить(СтараяТЧ);
	КонецЕсли;
КонецПроцедуры
//-- Рарус Лимаренко 12.12.2017
