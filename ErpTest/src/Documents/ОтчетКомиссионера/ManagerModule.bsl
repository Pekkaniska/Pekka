...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("Содержание", УчетУСНСервер.СодержаниеДохода(
															Перечисления.ХозяйственныеОперации.ОтчетКомиссионера,
															НСтр("ru = 'Отражение дохода при удержании комиссионного вознаграждения.'"),
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ОтчетКомиссионера.СтавкаНДСВознаграждения КАК СтавкаНДС,
	|	ОтчетКомиссионера.Услуга КАК Услуга,
	|	"""" КАК ИдентификаторСтроки
	|ИЗ
	|	Документ.ОтчетКомиссионера КАК ОтчетКомиссионера
	|ГДЕ
