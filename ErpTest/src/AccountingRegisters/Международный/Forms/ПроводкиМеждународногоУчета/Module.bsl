...
Процедура СуммаПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.НаборЗаписей.ТекущиеДанные;
	ТекущаяСтрока.СуммаПредставления = РассчитатьСуммуПредставления(ТекущаяСтрока.Сумма, ТекущаяСтрока.Период);
...
Процедура ЗаполнитьПроводкиАвтоматическиСервер()
	...
	Для Каждого Проводка Из ПроводкиДокумента Цикл
		Если Проводка.Активность Тогда
...
Функция ОтразитьВМеждународномУчетеАвтоматически()
	...
	ПроводкиДокумента.Сортировать("Период,Организация");
...
Процедура ПолучитьСостояниеОтраженияДокумента()
	...
	Элементы.РучнаяКорректировкаПроводок.Видимость = НЕ ДокументПараллельногоУчета;
	...
	Если ТаблицаОтраженияДокумента.Количество() > 0 Тогда 
		...
		Если СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияВМеждународномУчете.ОтраженоВУчетеВручную Тогда
			...
			Состояние = НСтр("ru = 'Документ отражен в международном учете вручную.'");
		...
		ИначеЕсли СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияВМеждународномУчете.ОжидаетсяОтражениеВРеглУчете Тогда
			...
			Состояние = НСтр("ru = 'Ожидается отражение в регл. учете'");
		...
		ИначеЕсли СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияВМеждународномУчете.ОтсутствуютПравилаОтраженияВУчете Тогда
			...
			Состояние = НСтр("ru = 'Отсутствуют правила отражения в международном учете.'");
			Элементы.Состояние.ЦветТекста = ЦветаСтиля.ПоясняющийОшибкуТекст;
		...
		ИначеЕсли СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияВМеждународномУчете.ОтраженоВУчете Тогда
			...
			Состояние = НСтр("ru = 'Документ отражен в международном учете.'");
			Элементы.Состояние.ЦветТекста = ЦветаСтиля.ТекстИнформационнойНадписи;
		...
		ИначеЕсли СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияВМеждународномУчете.КОтражениюВУчете Тогда
			Если ЕстьПроводки Тогда
				ТекстНеактуальныеПроводки = НСтр("ru = 'Проводки могут быть неактуальными.'");
				Состояние = НСтр("ru = 'Ожидается повторное отражение документа в международном учете.'") + " " + ТекстНеактуальныеПроводки;
				Элементы.Состояние.ЦветТекста = ЦветаСтиля.ИзмененноеЗначениеРеквизитаЦвет;
				Элементы.КартинкаВнимание.Видимость = Истина;
			Иначе
				Состояние = НСтр("ru = 'Ожидается отражение документа в международном учете.'");
				Элементы.Состояние.ЦветТекста = ЦветаСтиля.ТекстИнформационнойНадписи;
			КонецЕсли;
			
		ИначеЕсли СтатусОтраженияДокумента = Перечисления.СтатусыОтраженияВМеждународномУчете.КОтражениюВУчетеВручную Тогда
			Если ЕстьПроводки Тогда
				Состояние = НСтр("ru = 'Скорректируйте проводки документа или подтвердите актуальность текущих проводок'");
				Элементы.Состояние.ЦветТекста = ЦветаСтиля.ИзмененноеЗначениеРеквизитаЦвет;
				Элементы.КартинкаВнимание.Видимость = Истина;
				Элементы.ПодтвердитьАктуальностьПроводок.Видимость = Истина;
			Иначе
				Состояние = НСтр("ru = 'Требуется ручное отражение документа в международном учете.'");
				Элементы.Состояние.ЦветТекста = ЦветаСтиля.ТекстИнформационнойНадписи;
			КонецЕсли;
		КонецЕсли;
	Иначе
		...
		Иначе
			Состояние = НСтр("ru = 'Документ не отражается в международном учете или отсутствует учетная политика для организации.'");
			Элементы.ЗаполнитьПроводкиАвтоматически.Видимость = Ложь;
			Элементы.РучнаяКорректировкаПроводок.Видимость = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Элементы.ФормаЗаписатьИЗакрыть.Видимость = НЕ Элементы.ПодтвердитьАктуальностьПроводок.Видимость;
...
Функция КомментарийКОтражениюДокумента(ТаблицаОтраженияДокумента)
	...
	Комментарий = "";
	ТаблицаКомментариев = ТаблицаОтраженияДокумента.Скопировать(,"Комментарий");
	ТаблицаКомментариев.Свернуть("Комментарий");
...
Процедура УстановитьЗаголовкиСумм()
	...
	ЗаголовокСуммы = НСтр("ru = 'Сумма (%ФункциональнаяВалюта%)'");
	Элементы.Сумма.Заголовок = СтрЗаменить(ЗаголовокСуммы, "%ФункциональнаяВалюта%", ВалютыМеждународногоУчета.Функциональная);
	ЗаголовокСуммыПредставления = НСтр("ru = 'Сумма представления (%ВалютаПредставления%)'");
...
Процедура ПроверитьПравилаОтраженияВУчете(Команда)
	...
	Если Не НеобходимаНастройкаПравил Тогда
		...
		ТекстСообщения = НСтр("ru = 'Настроены все необходимые правила отражения. Попробовать отразить документ в учете?'");
...
&НаСервереБезКонтекста
Функция РассчитатьСуммуПредставления(Сумма, Дата)
	
	Возврат МеждународныйУчетОбщегоНазначения.РассчитатьСуммуПредставления(Сумма, Дата);
