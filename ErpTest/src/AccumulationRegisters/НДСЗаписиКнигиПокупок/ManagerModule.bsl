...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(НДСЗаписиКнигиПокупок.Регистратор) В (ТИП(Документ.СчетФактураПолученныйАванс), ТИП(Документ.СчетФактураВыданныйАванс))
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Движения.СчетУчетаНДС КАК СчетУчетаНДС,
	...
	|	ВЫБОР
	|		КОГДА Движения.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВычетНДССПолученногоАванса)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ Движения.РегламентнаяОперация
	|	КОНЕЦ КАК РегламентнаяОперация,
	|	Движения.ДокументОтгрузки КАК ДокументОтгрузки,
