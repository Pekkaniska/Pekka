...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ПО (РегламентнаяОперация.Ссылка = ПрочиеРасходы.Регистратор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПрочиеРасходы КАК ПрочиеРасходы_ВидДвиженияРасход
	|		ПО (ПрочиеРасходы_ВидДвиженияРасход.Регистратор = ПрочиеРасходы.Регистратор)
	|			И (ПрочиеРасходы_ВидДвиженияРасход.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход))
	|			И (ПрочиеРасходы_ВидДвиженияРасход.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РаспределениеРБП))
	|ГДЕ
	...
	|	И ПрочиеРасходы.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РаспределениеРБП)
	|	И ПрочиеРасходы_ВидДвиженияРасход.Регистратор ЕСТЬ NULL";
	СписокЗапросов.Добавить(ТекстЗапроса);
	//-- НЕ УТ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			//++ НЕ УТ
			НадоДобавитьРасход = Истина;
			Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.РегламентнаяОперация") Тогда
				Для Каждого Запись Из НаборЗаписей Цикл
					Если Запись.ВидДвижения = ВидДвиженияНакопления.Расход 
						И Запись.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РаспределениеРБП Тогда
						НадоДобавитьРасход = Ложь;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			//-- НЕ УТ
			...
			Для Сч = 0 По ВсегоЗаписей Цикл
				...
				//++ НЕ УТ
				Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.РегламентнаяОперация")
					И Запись.ВидДвижения = ВидДвиженияНакопления.Приход 
					И Запись.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РаспределениеРБП
					И НадоДобавитьРасход Тогда
					
					ЗаписьРасход = НаборЗаписей.Добавить();
					ЗаполнитьЗначенияСвойств(ЗаписьРасход, Запись);
					ЗаписьРасход.ВидДвижения = ВидДвиженияНакопления.Расход;
					ЗаписьРасход.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.РасходыНаПлатон;
					ЗаписьРасход.АналитикаРасходов = Неопределено;
					ЗаписьРасход.ПостояннаяРазница = 0;
