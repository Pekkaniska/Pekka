...
Процедура ОбновитьСтатусВДвиженияхПоПотребностям(ПланПроизводства, Движения)
	...
	//++ НЕ УТКА
	ДанныеДвижений.Вставить("ПланыЗанятостиВидовРабочихЦентров", МассивРезультатов[3].Выгрузить());
	//-- НЕ УТКА
...
Процедура ПересчитатьПродукцию(МенеджерВременныхТаблиц, Движения)
	...
	//++ НЕ УТКА
	СтруктураРезультатов.Вставить("ПланыЗанятостиВидовРабочихЦентров", МассивРезультатов[8].Выбрать());
	//-- НЕ УТКА
...
Функция ТекстЗапросаПересчетПродукции()
	...
	|	РегистрНакопления.ПланыЗанятостиВидовРабочихЦентров КАК ПланыЗанятостиВидовРабочихЦентров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКоличествоПартий КАК ВТКоличествоПартий
	|		ПО ПланыЗанятостиВидовРабочихЦентров.ДатаПроизводства = ВТКоличествоПартий.ДатаПроизводства
...
Процедура РазузловатьИДобавитьДвижения(ДанныеСпецификации, ПараметрыСпецификации, Изделия, Движения, БольшеНуля)
	...
	Для каждого Данные Из ДанныеСпецификации.ДанныеСпецификаций Цикл
		...
		Если Данные.Спецификация = ЗначенияЗаполнения.СпецификацияПродукции Тогда
			...
			ДобавитьДвиженияВходящиеИзделияВПланыПотребленияМатериалов(ЗначенияЗаполнения, Движения, Данные.ВходящиеИзделия, Знак);
		...
		//++ НЕ УТКА
		ДобавитьДвиженияВПланыЗанятостиВидовРабочихЦентров(ЗначенияЗаполнения, Движения, Данные.ВидыРабочихЦентров, Знак, ПараметрыСпецификации.ПовторноеРазузлование);
		//-- НЕ УТКА
...
//++ НЕ УТКА
Процедура ДобавитьДвиженияВходящиеИзделияВПланыПотребленияМатериалов(ЗначенияЗаполнения, Движения, ТаблицаИсточник, Знак)
...
//++ НЕ УТКА
Процедура ДобавитьДвиженияВПланыЗанятостиВидовРабочихЦентров(ЗначенияЗаполнения, Движения, ТаблицаИсточник, Знак, ПовторноеРазузлование)
	...
	Для каждого Строка Из ТаблицаИсточник Цикл
		...
		НоваяСтрока.ПодразделениеИсполнитель = Строка.ПодразделениеЭтапа;
		НоваяСтрока.КоличествоНаЕдиницуПродукции = Строка.ЗанятостьНаЕдиницуПартииВыпуска;
		Если ПовторноеРазузлование Тогда
			НоваяСтрока.Занятость = Строка.Занятость * Знак;
		Иначе
			НоваяСтрока.Занятость = Строка.ЗанятостьНаЕдиницуПартииВыпуска * ЗначенияЗаполнения.ЕдиницПартийИзделий * Знак;
		КонецЕсли;
...
Функция АктуальныеДвиженияОчереди(ПланПроизводства, МенеджерВременныхТаблиц, ИсключитьСторно)
	...
	//++ НЕ УТКА
	Результат.Вставить("ПланыЗанятостиВидовРабочихЦентров", МассивРезультатов[4].Выгрузить());
	//-- НЕ УТКА
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстЗапроса = ТекстЗапроса + 
		...
		|ГДЕ
		|	ПланыПроизводства.Регистратор ССЫЛКА Документ.ПланПроизводства
		|	И ПланыПроизводства.ДатаВыпускаПолуфабриката <> ДАТАВРЕМЯ(1, 1, 1)
		...
		|		ПО ПланыПроизводства.Номенклатура = ПланыПотребленияМатериалов.Номенклатура
		|			И ПланыПроизводства.Характеристика = ПланыПотребленияМатериалов.Характеристика
		|			И ПланыПроизводства.ПланПроизводства = ПланыПотребленияМатериалов.ПланПроизводства
		|			И ПланыПроизводства.Назначение = ПланыПотребленияМатериалов.Назначение
		|			И ПланыПроизводства.ДатаВыпускаПолуфабриката = ПланыПотребленияМатериалов.УдалитьДатаВыпускаПолуфабриката
		|ГДЕ
		|	ПланыПроизводства.ДатаВыпускаПолуфабриката <> ДАТАВРЕМЯ(1, 1, 1)
		|	И ПланыПотребленияМатериалов.ПланПроизводства ЕСТЬ НЕ NULL 
		|	И ПланыПотребленияМатериалов.УдалитьДатаВыпускаПолуфабриката <> ПланыПотребленияМатериалов.Период
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	ДополнительныеПараметры.ПолноеИмяРегистра = "РегистрНакопления.ПланыПроизводства";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуРегистраторовРегистраДляОбработки(Параметры.Очередь, "Документ.ПланПроизводства", ПолноеИмяРегистра, МенеджерВременныхТаблиц);
	...
	Если Не Результат.ЕстьЗаписиВоВременнойТаблице Тогда
		...
		Возврат;
	КонецЕсли;
	Если Не ОбновлениеИнформационнойБазы.ОбработкаДанныхЗавершена(Неопределено, "Справочник.РесурсныеСпецификации") Тогда
		Параметры.ОбработкаЗавершена = Ложь;
		Возврат;
	КонецЕсли;
	...
	ТекстЗапроса = 
		...
		|	СсылкиДляОбработки.Регистратор.Сценарий    КАК Сценарий,
		|	ПланыДляЗамещения.Регистратор ЕСТЬ НЕ NULL КАК ПолныйПересчет
		|ИЗ
		|	&ВТДляОбработкиСсылка КАК СсылкиДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПланыДляЗамещения КАК ПланыДляЗамещения
		|		ПО СсылкиДляОбработки.Регистратор = ПланыДляЗамещения.Регистратор
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ПланыПроизводства.Назначение КАК Назначение,
		|	ПланыПроизводства.ДатаВыпускаПолуфабриката КАК ДатаВыпускаПолуфабриката,
		|	ПланыПотребленияМатериалов.Период КАК ДатаВыпускаПолуфабрикатаНовая
		|ИЗ
		|	РегистрНакопления.ПланыПроизводства КАК ПланыПроизводства
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПланыПотребленияМатериалов КАК ПланыПотребленияМатериалов
		|		ПО ПланыПроизводства.Номенклатура = ПланыПотребленияМатериалов.Номенклатура
	...
	ДатыВыпускаПФ = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
	ДатыВыпускаПФ.Индексы.Добавить("Номенклатура, Характеристика, ПланПроизводства, Назначение, ДатаВыпускаПолуфабриката");
	ДатыВыпускаПФ.Индексы.Добавить("ПланПроизводства");
	СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, ПланПроизводства, Назначение, ДатаВыпускаПолуфабриката");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Выборка.ПолныйПересчет Тогда
				
				Документы.ПланПроизводства.ЗаписатьОчередьФоновогоПроведения(Регистратор, Выборка.Сценарий, Истина);
				РегистрыНакопления.ПланыПроизводства.ВыполнитьПроведение(Новый Структура("ДокументСсылка", Регистратор));
				
			КонецЕсли;
			
			Движения = ДвиженияПланаПроизводстваПоРегистру(Регистратор, "ПланыПроизводства");
			НаборИзменен = Ложь;
			
			ЗаменитьДатыВыпуска = (ДатыВыпускаПФ.Найти(Регистратор, "ПланПроизводства") <> Неопределено);
			Если ЗаменитьДатыВыпуска Тогда
					
				Для каждого Строка Из Движения Цикл
					Если Строка.ДатаВыпускаПолуфабриката = '00010101' Тогда
						Продолжить;
					КонецЕсли;
					
					ЗаполнитьЗначенияСвойств(СтруктураПоиска, Строка);
					НайденныеСтроки = ДатыВыпускаПФ.НайтиСтроки(СтруктураПоиска);
					Если НайденныеСтроки.Количество() > 0 Тогда
						Строка.ДатаВыпускаПолуфабриката = НайденныеСтроки[0].ДатаВыпускаПолуфабрикатаНовая;
					КонецЕсли;
				КонецЦикла;
				НаборИзменен = Истина;
			...
			ЗаполнитьТипПроцесса = Движения.Найти(
				Перечисления.ТипыПроизводственныхПроцессов.ПустаяСсылка(),
				"ТипПроизводственногоПроцесса") <> Неопределено;
			Если ЗаполнитьТипПроцесса Тогда
				
				Движения.ЗаполнитьЗначения(
					ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ТипПроизводственногоПроцесса"),
					"ТипПроизводственногоПроцесса");
				
				Движения.ЗаполнитьЗначения(Истина, "ПовторноеРазузлование"); // Связано с заполнением нового регистра накопления ПланыВыпускаИзделий (версия 2.4.3)
			...
			Если КомплекснаяАвтоматизация Тогда
				МассивСтрокСПустымПодразделением = Движения.НайтиСтроки(Новый Структура("Подразделение",Справочники.СтруктураПредприятия.ПустаяСсылка()));
				
				Если МассивСтрокСПустымПодразделением.Количество()<>Движения.Количество() Тогда
			...
			Если НЕ Выборка.ПолныйПересчет Тогда
				ЗаполнитьДатуЗапускаВыпускаВПланахПроизводства(Регистратор, Движения);
				НаборИзменен = Истина;
			КонецЕсли;
