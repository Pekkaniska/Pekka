...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Результат <> Неопределено
		...
		ОбработатьДанныеСканераШтрихкодов(СобытияФормЕГАИСКлиентПереопределяемый.ПреобразоватьДанныеСоСканераВСтруктуру(Результат.Параметр));
...
Процедура ПоясняющийТекстОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "СкопироватьШтриховойКодВБуферОбмена" Тогда
		...
		ИнтеграцияЕГАИСКлиент.СкопироватьШтрихКодВБуферОбмена(Элементы.БуферОбмена, ЗначениеШтрихкода);
...
Процедура ПоказатьНеоднозначноеСоответствиеНетВДанныхОснования()
	...
	Префикс = СтрШаблон(НСтр("ru = 'Невозможно добавить алкогольную продукцию ""%1"" с акцизной маркой'"), АлкогольнаяПродукция);
	...
	Элементы.ДекорацияОшибкаДобавленияАлкогольнойПродукции.Заголовок = Новый ФорматированнаяСтрока(Префикс, " ", СтрокаШтриховойКод, " ", ТекстСОшибкой);
...
Процедура ВывестиТекстСохранениеВыбораPDF417()
	...
	Элементы.ЗапомнитьВыбор.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(ТекстЗаголовка, , ЦветаСтиля.ЕГАИССтатусОбработкиПередается);
...
Процедура ВывестиТекстИдентификацияАкцизнойМарки()
	...
	МассивСтрок.Добавить(
		Новый ФорматированнаяСтрока(
			ИнтеграцияЕГАИСКлиентСервер.ПредставлениеШтрихкода(ЗначениеШтрихКода),
			Новый Шрифт(,,,,Истина),
			ЦветаСтиля.ЦветГиперссылкиГИСМ,,
			"СкопироватьШтриховойКодВБуферОбмена"));
	МассивСтрок.Добавить(" ");
...
Процедура ПоказатьНетМаркированнойПродукцииВОснованииDataMatrix()
	...
	Префикс = НСтр("ru = 'Невозможно добавить алкогольную продукцию с акцизной маркой'");
	СтрокаШтриховойКод = Новый ФорматированнаяСтрока(ИнтеграцияЕГАИСКлиентСервер.ПредставлениеШтрихкода(ЗначениеШтрихКода),
	                                                 Новый Шрифт(,,,,Истина),
	                                                 ЦветаСтиля.ЦветГиперссылкиГИСМ,
	                                                 ,
...
Функция РезультатИзмененияНоменклатурыПоДаннымСканирования(ДанныеСопоставления)
	...
	Если Не ИнтеграцияЕГАИСПереопределяемый.НоменклатураЯвляетсяАлкогольнойМаркируемойПродукцией(ДанныеСопоставления.Номенклатура) Тогда
