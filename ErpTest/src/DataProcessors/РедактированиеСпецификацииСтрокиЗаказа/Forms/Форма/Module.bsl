...
Функция ПараметрыВыбораОбеспечения()
	...
	ТаблицаОформлено = РегистрыНакопления.ЗаказыМатериаловВПроизводство.ТаблицаОформлено(ТаблицаОтбора,, Истина);
...
Процедура ЗаполнитьСпособПолученияМатериаловПоУмолчаниюНаСервере(ТекущиеДанные)
	...
	СпособПолученияМатериала = ОбеспечениеПроизводства.СпособПолученияМатериалаПоУмолчанию(
		ДанныеЗаказа, 
		ДанныеМатериала, 
		МенеджерВременныхТаблиц);
...
Процедура ДополнитьПараметрыНаСервере(ПараметрыФормы, КлючСвязиЭтапы)
	...
	Если Не ЗначениеЗаполнено(Объект.Этапы) Тогда
		Возврат;
	КонецЕсли;
