...
Функция ТаблицаСозданныхДокументов(Параметры, СозданныеДокументы) Экспорт
	...
	Если Параметры.ОформлятьВыкупы Тогда
		...
		|	ВыкупПринятыхНаХранениеТоваров.МестоХранения КАК Склад,
		|	ВыкупПринятыхНаХранениеТоваров.МестоХранения.Представление КАК СкладПредставление,
		...
		Если Не ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(ПоляШапки, ВыборкаПоОтрицательнымОстаткам) Тогда
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				ИначеЕсли ХозяйственныеОперацииВыкупа().Найти(ВыборкаПоОтрицательнымОстаткам.ХозяйственнаяОперация) <> Неопределено Тогда
					...
					ЕстьНомерГТДВСтрокеТоваров = Ложь;
					//-- НЕ УТ
				Иначе
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	Если НЕ (ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.ТоварыОрганизаций)
		...
		//++ НЕ УТ
			Или ПравоДоступа("Добавление", Метаданные.Документы.ВыкупПринятыхНаХранениеТоваров) 
