...
Процедура ВходящиеЭДПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Для каждого ТекущаяСтрока Из Строки Цикл
		...
		ИначеЕсли Данные.КонтрольОтраженияВУчете = 1 Тогда
			Данные.ОтражениеВУчете = НСтр("ru = 'Создать'");
		ИначеЕсли Данные.КонтрольОтраженияВУчете = 2 Тогда
			Данные.ОтражениеВУчете = НСтр("ru = 'Провести'");
		КонецЕсли;
...
Функция ТекстЗапросаКоличестваЭлементовПоРазделуБезОтбора(Знач Раздел) 
	...
	ИначеЕсли Раздел = "ТребуетсяПригласить" Тогда
		...
		|	ГДЕ
		|		ОсновнаяТаблица.СтатусПодключения В (ЗНАЧЕНИЕ(Перечисление.СтатусыУчастниковОбменаЭД.ТребуетсяПригласить), ЗНАЧЕНИЕ(Перечисление.СтатусыУчастниковОбменаЭД.Ошибка))) КАК ВложенныйЗапрос";
	...
	ИначеЕсли Раздел = "ЖдемСогласия" Тогда
		...
		|		Справочник.СоглашенияОбИспользованииЭД КАК ОсновнаяТаблица
		|	ГДЕ
		|		ОсновнаяТаблица.СтатусПодключения = ЗНАЧЕНИЕ(Перечисление.СтатусыУчастниковОбменаЭД.ОжидаемСогласия)) КАК ВложенныйЗапрос";
	...
	ИначеЕсли Раздел = "ТребуетсяСогласие" Тогда
		...
		|		Справочник.СоглашенияОбИспользованииЭД КАК ОсновнаяТаблица
		|	ГДЕ
		|		ОсновнаяТаблица.СтатусПодключения = ЗНАЧЕНИЕ(Перечисление.СтатусыУчастниковОбменаЭД.ТребуетсяСогласие)) КАК ВложенныйЗапрос";
