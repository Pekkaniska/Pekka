...
//	Страница1.Параметры.ДатаВвода = ВыборкаПоТабличнымЧастям.ДатаВводаВЭксплуатацию;
	Страница1.Параметры.ДатаВвода = пкПолучитьДатуПервогоПринятияКУчету(ВыборкаПоТабличнымЧастям.НомерИнвентарный, ВыборкаПоТабличнымЧастям.ДатаВводаВЭксплуатацию);
...
//Рарус Владимир Подрезов 20.03.2017
	Страница2_Шапка.Параметры.ДатаВводаВЭксплуатацию	= пкПолучитьДатуПервогоПринятияКУчету(ВыборкаПоТабличнымЧастям.НомерИнвентарный, ВыборкаПоТабличнымЧастям.ДатаВводаВЭксплуатацию);
//Рарус Владимир Подрезов Конец
...
//Рарус Владимир Подрезов 20.03.2017
		Страница2_Таблица1_Строка.Параметры.ДатаВводаВЭксплуатацию	= пкПолучитьДатуПервогоПринятияКУчету(ВыборкаПоТабличнымЧастям.НомерИнвентарный, ВыборкаПоТабличнымЧастям.ДатаВводаВЭксплуатацию);
//Рарус Владимир Подрезов Конец
...
//Рарус Владимир Подрезов 16.03.2017
Функция пкПолучитьДатуПервогоПринятияКУчету(тНомерИнвентарный, тДатаВводаВЭксплуатацию)
	
	Если Не ЗначениеЗаполнено(тНомерИнвентарный) Тогда
		Возврат тДатаВводаВЭксплуатацию;
	КонецЕсли;
	...
	|	МИНИМУМ(ЕСТЬNULL(ПервоначальныеСведенияОСБухгалтерскийУчет.Период, ДАТАВРЕМЯ(1, 1, 1))) КАК Период
	|ИЗ
	...
	|		ПО (ПервоначальныеСведенияОСБухгалтерскийУчет.ОсновноеСредство = ОбъектыЭксплуатации.Ссылка)
	|ГДЕ
	|	ОбъектыЭксплуатации.Код = &Код
	|
	|СГРУППИРОВАТЬ ПО
	|	ОбъектыЭксплуатации.Ссылка";
	ЗапросО.УстановитьПараметр("Код", тНомерИнвентарный);
	РезЗапроса = ЗапросО.Выполнить().Выбрать();
	Если НЕ РезЗапроса.Следующий() Тогда
		Возврат тДатаВводаВЭксплуатацию;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РезЗапроса.Период) Тогда
		Возврат РезЗапроса.Период;
	Иначе
		Возврат тДатаВводаВЭксплуатацию;
	КонецЕсли;
