...
Процедура ПриОткрытии(Отказ)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоСкладам, 
		"Номенклатура", 
		?(ЭтаФорма.Элементы.СписокРасширенныйПоискНоменклатура.ТекущиеДанные=Неопределено, Неопределено, ЭтаФорма.Элементы.СписокРасширенныйПоискНоменклатура.Ссылка), 
		Истина);
		
	// Отбор по сегменту у списка номенклатуры.
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоЯчейкам, 
		"Номенклатура", 
		?(ЭтаФорма.Элементы.СписокРасширенныйПоискНоменклатура.ТекущиеДанные=Неопределено, Неопределено, ЭтаФорма.Элементы.СписокРасширенныйПоискНоменклатура.Ссылка), 
		Истина);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоЯчейкам, 
		"Склад", 
		?(ЭтаФорма.Элементы.ДанныеПоСкладам.ТекущиеДанные=Неопределено, Неопределено, ЭтаФорма.Элементы.ДанныеПоСкладам.ТекущиеДанные.Склад), 
		Истина);
	
	// Отбор по сегменту у списка номенклатуры.
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоАналогам, 
		"Номенклатура", 
		?(ЭтаФорма.Элементы.СписокРасширенныйПоискНоменклатура.ТекущиеДанные=Неопределено, Неопределено, ЭтаФорма.Элементы.СписокРасширенныйПоискНоменклатура.Ссылка), 
		Истина);
	...
	УстановитьФильтрОтбораСТатуса();	
		
КонецПроцедуры

&НаСервере
Процедура УстановитьФильтрОтбораСТатуса()

	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокЗаказовПоставщику, "Статус", Перечисления.СтатусыЗаказовПоставщикам.Закрыт, ВидСравненияКомпоновкиДанных.НеРавно,,Истина);
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоСкладам, 
		"Номенклатура", 
		?(Элемент.ТекущиеДанные=Неопределено,Неопределено,Элемент.ТекущиеДанные.Ссылка), 
		Истина);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		...
		Истина);
	Исключение	
		Сообщить("Проблема при работе с данными по ячейкам. Проверьте права соответсвующего регистра!");	
	КонецПопытки;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		...
		Истина);
	Исключение	
		Сообщить("Проблема при работе с данными по аналогам. Проверьте права соответсвующего регистра!");	
	КонецПопытки;
КонецПроцедуры
...
&НаКлиенте
Процедура ДанныеПоСкладамПриАктивизацииСтроки(Элемент)
	ДанныеПоСкладамПриАктивизацииСтрокиНаСервере();
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоЯчейкам, 
		"Номенклатура", 
		?(ЭтаФорма.Элементы[ИмяСпискаНоменклатуры].ТекущиеДанные=Неопределено, Неопределено, ЭтаФорма.Элементы[ИмяСпискаНоменклатуры].ТекущиеДанные.Ссылка), 
		Истина);
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ЭтаФорма.ДанныеПоЯчейкам, 
		"Склад", 
		?(Элемент.ТекущиеДанные=Неопределено,Неопределено,Элемент.ТекущиеДанные.Склад), 
		Истина);
...
&НаСервереБезКонтекста
Процедура ДанныеПоСкладамПриАктивизацииСтрокиНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ПоставщикОтборЗаказыПриИзменении(Элемент)
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокЗаказовПоставщику, "Партнер",  ПоставщикОтборЗаказы, ВидСравненияКомпоновкиДанных.Равно,, ЗначениеЗаполнено(ПоставщикОтборЗаказы));
КонецПроцедуры

&НаСервере
Процедура ПоставщикОтборЗаказыПриИзмененииНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ДанныеПоСкладамКОтгрузкеНажатие(Элемент, СтандартнаяОбработка)
	
КонецПроцедуры

&НаСервере
Процедура ДанныеПоСкладамКОтгрузкеНажатиеНаСервере()
	
КонецПроцедуры

&НаКлиенте
Процедура ДанныеПоСкладамВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ФормаРасшифровки=ПолучитьФорму("Обработка.пкАРМСклад.Форма.ФормаРасшифровкиКОтгрузке",,ЭтаФорма);
	ИмяСпискаНоменклатуры = ПодборТоваровКлиентСервер.ИмяСпискаНоменклатурыПоВариантуПоиска(ЭтаФорма);
	ФормаРасшифровки.НоменклатураРасшифровки=ЭтаФорма.Элементы[ИмяСпискаНоменклатуры].ТекущиеДанные.Ссылка;
	ФормаРасшифровки.СкладРасшифровки=Элемент.ТекущиеДанные.Склад;
	ФормаРасшифровки.Открыть();
	
КонецПроцедуры

&НаСервере
Процедура ДанныеПоСкладамВыборНаСервере()
