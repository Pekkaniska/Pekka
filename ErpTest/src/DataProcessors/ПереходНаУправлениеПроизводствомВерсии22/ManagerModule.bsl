...
Процедура СформироватьЭтапыПроизводства22(Заказ21Объект, КлючСвязиПродукция, Заказ22Объект, Результат)
	...
	Распоряжения = ОбщегоНазначенияУТКлиентСервер.Массив(Заказ22Объект.Ссылка);
	ДанныеЗаполнения.Вставить("Распоряжения", Распоряжения);
	
	ДанныеСпецификаций = ДанныеСпецификацийЗаказа21(Заказ21Объект, КлючСвязиПродукция, Заказ22Объект);
	ДанныеЗаполнения.Вставить("ДанныеСпецификаций", ДанныеСпецификаций);
	
	ВариантыОбеспечения = Новый Массив;
	
	РезультатФормирования = Документы.ЭтапПроизводства2_2.РезультатФормированияЭтапов();
	...
	ЗаполнитьНовыеСсылкиВТаблицеЭтапов(
				Заказ21Объект,
				КлючСвязиПродукция,
				ДанныеСпецификаций);
				
	Результат.ОбработаноЭтапов = Результат.ОбработаноЭтапов + РезультатФормирования.СформированоДокументов;
...
Функция МатериалыИУслугиЗаказа21(Заказ21Объект, ВыпускающийЭтап, КоллекцияЭтапов, СтруктураИзделия)
	...
	Для каждого СтрокаЭтап Из КоллекцияЭтапов Цикл
		...
		Для каждого ДанныеСтроки Из НайденныеСтроки Цикл
			...
			ИначеЕсли НоваяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить
			...
			Если НЕ СтрокаЭтап.ТребуетсяКорректировкаНазначения
				И НоваяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно
...
Процедура ДобавитьДанныеСпецификацииЦепочкиЭтапов(Результат, Заказ21Объект, КоллекцияЭтапов, СтруктураИзделия)
	...
	ДанныеСпецификации.Вставить("НазначениеПродукция", СтруктураИзделия.Назначение);
	...
	ДанныеСпецификации.Вставить("ИспользоватьНесколькоВыходныхИзделий", Истина);
	
	ДанныеСпецификации.Вставить("ПартияВыпуска", ПроизводствоКлиентСервер.Дробь(ВыпускающийЭтап.Количество22, 1));
	
	ДанныеСпецификации.Вставить("КоличествоПартийВыпуска", ВыпускающийЭтап.Количество22);
	ДанныеСпецификации.Вставить("ЕдиницаИзмеренияПартииВыпуска", Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка());
...
Процедура РазузловатьСпецификациюСтрокиЗаказа21(Результат, Заказ21Объект, КлючСвязиЭтапы, СтруктураИзделия)
	...
	ДобавитьДанныеСпецификацииЦепочкиЭтапов(
				Результат, 
				Заказ21Объект, 
				КоллекцияЭтапов, 
				СтруктураИзделия);
	...
	Для каждого СтрокаЭтап Из КоллекцияЭтапов Цикл
		...
		Для каждого СтрокаПолуфабрикат Из КоллекцияПолуфабрикатов Цикл
			...
			СтруктураПолуфабриката = СтруктураДанныхПолуфабриката(
				СтрокаПолуфабрикат, СтруктураИзделия, КлючСвязиРодитель);
...
Функция СтруктураДанныхПолуфабриката(СтруктураПолуфабриката, СтруктураРодитель, КлючСвязиРодитель)
	...
	Результат.Вставить("Спецификация", СтруктураПолуфабриката.Спецификация);
	Результат.Вставить("СпецификацияПредставление", "");
	
	Результат.Вставить("ЭтапПотребитель", СтруктураПолуфабриката.КлючСвязиЭтапПотребляющийПолуфабрикат);
...
Функция ОбособленныеМатериалыЭтапа(Этап22Объект, СтрокаЭтап, Заказ21Объект)
	...
	Для каждого СтрокаМатериал Из Этап22Объект.ОбеспечениеМатериаламиИРаботами Цикл
		...
		Если СтрокаМатериал.ВариантОбеспечения <> Перечисления.ВариантыОбеспечения.Обособленно
			ИЛИ СтрокаМатериал.Производится ИЛИ ЭтоРабота(СтрокаМатериал.Номенклатура, Заказ21Объект) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УточитьКоличествоПолуфабрикатаКПроизводству(Заказ21Объект, Строка22, СтрокаЭтап)
	...
	Если СтрокаЭтапВыпуска <> Неопределено
		...
		Строка21.СпособПолученияМатериала = Перечисления.СпособыПолученияМатериаловВСпецификации.ОбеспечиватьОбособленно;
...
Процедура УточнитьОбеспечениеДляОтгруженныхМатериалов(Заказ21Объект, ТекущаяСтрока, ДанныеСтроки, СтрокаЭтап)
	...
	Если ЭтоРабота(ТекущаяСтрока.Номенклатура, Заказ21Объект) Тогда
		...
		Если ДанныеСтроки.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.ОтгрузитьОбособленно Тогда
			ТекущаяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Обособленно;
		Иначе
			ТекущаяСтрока.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется;
		КонецЕсли;
...
Функция ЗаказаноОбособленно21(ТаблицаТовары)
	...
	Запрос = Новый Запрос(
		...
		|	ОбеспечениеЗаказов.КЗаказуОстаток КАК Количество
		|ИЗ
		...
		|					Т.Назначение
		|				ИЗ
		|					ВТТовары КАК Т)) КАК ОбеспечениеЗаказов
		|ГДЕ
		|	ОбеспечениеЗаказов.КЗаказуОстаток < 0");
...
Процедура УточитьОбеспечениеОбособленныхМатериалов(Этап22Объект, СтрокаЭтап, Заказ21Объект)
	...
	Документы.ЭтапПроизводства2_2.ЗаблокироватьРегистрыОбеспеченияПотребностей(ОбособленныеМатериалы);
...
Функция ЕстьОстатокОбособленногоТовара(ТаблицаТоваров)
	...
	Запрос = Новый Запрос(
		...
		|		ОбеспечениеЗаказов.НаличиеПодЗаказОстаток КАК Количество
		|	ИЗ
		|		РегистрНакопления.ОбеспечениеЗаказов.Остатки(
		|				,
		|				(Номенклатура, Характеристика, Склад, Назначение) В
