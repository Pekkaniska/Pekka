...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.ТипОбеспечения = Перечисления.ТипыОбеспечения.Производство Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Подразделения, "Номенклатура",   Параметры.Номенклатура,   Истина);
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(Подразделения, "Характеристика", Параметры.Характеристика, Истина);
		...
		Если Параметры.ГрупповоеЗаполнение Тогда
			...
			Элементы.ПодразделенияСпецификация.Видимость = Ложь;
			
		Иначе
			
			Если ЗначениеЗаполнено(Параметры.Характеристика) Тогда
				
				ШаблонТекста = НСтр("ru = 'Выбор подразделения (Номенклатура: %1, Характеристика: %2)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура, Параметры.Характеристика);
				
			Иначе
				
				ШаблонТекста = НСтр("ru = 'Выбор подразделения (Номенклатура: %1)'");
				Заголовок = СтрШаблон(ШаблонТекста, Параметры.Номенклатура);
...
Процедура УстановитьТекстЗапросаСпискаПодразделений()
	...
	СвойстваСписка.ТекстЗапроса =
		...
		|		ПО СтруктураПредприятия.Ссылка = ОсновныеСпецификации.Подразделение
