...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Если ТипЗнч(ТекущиеДанные.Ссылка) <> Тип("ДокументСсылка.ПриобретениеТоваровУслуг")
		//++ НЕ УТ
		И ТипЗнч(ТекущиеДанные.Ссылка) <> Тип("ДокументСсылка.ПриемкаТоваровНаХранение")
		//-- НЕ УТ
		Тогда
...
Процедура ОформитьПоПриемке(Команда)
	...
	Если ТекущиеДанные.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи") Тогда
		ИмяФормыЗаполнения = "Документ.ПриемкаТоваровНаХранение.ФормаОбъекта";
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	ИначеЕсли ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") Тогда
		ПараметрыЗаполнения = Новый Структура("Ключ, ЗаполнятьПоОрдеру", ТекущиеДанные.Ссылка, Истина);
		ОткрытьФорму("Документ.ПриемкаТоваровНаХранение.ФормаОбъекта", ПараметрыЗаполнения, ЭтаФорма);
	//-- НЕ УТ
	КонецЕсли;
...
Функция ОпределитьНаличиеОформленныхНакладных(РаспоряжениеСсылка,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриемкаТоваровНаХранение.Товары КАК ДокументПоступленияТовары
	|		ПО ДокументПоступления.Ссылка = ДокументПоступленияТовары.Ссылка
	|ГДЕ
	|	&ЗаполнениеПоОрдерам = ИСТИНА
	|	И ДокументПоступленияТовары.Склад = &Склад
...
Функция НастройкиФормыПереоформленияНакладных(РаспоряжениеСсылка,
	...
	Если ХозОперацииПоступленияТоваров.Найти(ХозяйственнаяОперация) <> Неопределено
		...
		НастройкиФормы.ИмяФормыНакладной = "Документ.ПоступлениеТоваров.Форма.ФормаДокумента";
...
Функция ТекстЗапросаСпискаРаспоряженийНаПриемку()
	...
	|				ИЛИ ДокументПоступления ССЫЛКА Справочник.СоглашенияСПоставщиками {(Склад) КАК Склад}) КАК Таблица
	|ГДЕ
	...
	|	ИЗ ВтОстаткиТоварыКПоступлению КАК ТаблицаОстатков) КАК Таблица
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументПоступления,
	|	Склад
	...
	|ГДЕ
	|	(Таблица.ДокументПоступления, Таблица.Склад) В
	|			(ВЫБРАТЬ
	...
	|		ИНАЧЕ РеестрДокументов.Контрагент
	...
	|		КОГДА ТаблицаРаспоряжений.ДокументПоступления ССЫЛКА Справочник.ДоговорыКонтрагентов
	|			ТОГДА ВЫРАЗИТЬ(ТаблицаРаспоряжений.ДокументПоступления КАК Справочник.ДоговорыКонтрагентов).ХозяйственнаяОперация
	|		ИНАЧЕ РеестрДокументов.ХозяйственнаяОперация
	|	КОНЕЦ КАК ХозяйственнаяОперация,
	|	ТаблицаРаспоряжений.ХозяйственнаяОперацияПриемки КАК ХозяйственнаяОперацияПриемки,
