...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	Если ЗагрузитьПробегИРасходГСМ Тогда // загружаем пробег и расход ГСМ
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТССКАУТ(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
	
	Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // загружаем дополнительные сведения
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияПоТССКАУТ(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьСКАУТ.Получить() Тогда
		ТекстСообщения = "Открытие возможно только тогда, когда включено использование системы GPS - СКАУТ.";
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	// Выполним загрузку
	Если Не Отказ Тогда 
		Состояние("Выполняется загрузка данных системы GPS СКАУТ",,"Операция может занять длительное время");
		КодВозврата = ВыполнитьЗагрузкуСервер(ТекстОшибки);
		
		Если КодВозврата = 0 Тогда 
			Состояние("Загрузка данных системы GPS СКАУТ завершена успешно");
		КонецЕсли;
	КонецЕсли;
