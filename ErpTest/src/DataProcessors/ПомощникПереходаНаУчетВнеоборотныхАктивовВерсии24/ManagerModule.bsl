...
Процедура ЗаполнитьГруппыПроверокДляРегистрации(ТаблицаГруппПроверок) Экспорт
	...
	ОписаниеГруппыПроверок.КонтекстВыполнения = Документы.ВводОстатковВнеоборотныхАктивов2_4.ПустаяСсылка();
...
Процедура ТекстЗапросаПринятыеКУчетуАктивы(ПараметрыОбработчика, ТекстыЗапроса)
	...
	|					И &ОтборОбъектыПроблемОС) КАК СостоянияОС
	//++ НЕ УТКА
	|			ЛЕВОЕ СОЕДИНЕНИЕ ПринятыеКУчетуОС_МУ КАК ПринятыеКУчетуОС_МУ
	|			ПО ПринятыеКУчетуОС_МУ.ВнеоборотныйАктив = СостоянияОС.ОсновноеСредство
	//-- НЕ УТКА
	|	ГДЕ
	|		СостоянияОС.Состояние В (ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету), ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету))
	|
	//++ НЕ УТКА
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АрендованныеОС.ВнеоборотныйАктив КАК ВнеоборотныйАктив,
	|	АрендованныеОС.Организация КАК Организация
	|ПОМЕСТИТЬ АрендованныеОС
	|ИЗ
	|	ПринятыеКУчетуОС КАК АрендованныеОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПринятыеКУчетуОС КАК СобственныеОС
	|		ПО (СобственныеОС.ВнеоборотныйАктив = АрендованныеОС.ВнеоборотныйАктив)
	|			И (СобственныеОС.Организация <> АрендованныеОС.Организация)
	|			И (СобственныеОС.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету))
	|ГДЕ
	|	АрендованныеОС.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету)
	|	И СобственныеОС.ВнеоборотныйАктив ЕСТЬ NULL
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ВнеоборотныйАктив,
	|	Организация
	...
	|	СостоянияНМА.Организация КАК Организация,
	|	МАКСИМУМ(СостоянияНМА.ОтражатьВРеглУчете) КАК ОтражатьВРеглУчете,
	|	МАКСИМУМ(СостоянияНМА.ОтражатьВУпрУчете) КАК ОтражатьВУпрУчете
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	СостоянияНМА.ВидАктива,
	|	СостоянияНМА.ВнеоборотныйАктив,
...
Процедура ТекстЗапросаСчетаУчета(ПараметрыОбработчика, ТекстыЗапроса)
	...
	|
	|	ЕСТЬNULL(МАКСИМУМ(ВЫБОР
	|			КОГДА СчетаУчета.СчетУчета.Забалансовый
	|				ТОГДА СчетаУчета.СчетУчета
	|			ИНАЧЕ NULL
	|		КОНЕЦ), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)) КАК СчетЗабалансовогоУчета,
...
Процедура ТекстЗапросаДатыПринятияКУчету(ПараметрыОбработчика, ТекстыЗапроса)
	...
	|	НАЧАЛОПЕРИОДА(Состояния.ДатаСостояния, ДЕНЬ) КАК ДатаПринятияКУчету,
	...
	|	ЕСТЬNULL(ПараметрыАмортизацииОСНалоговыйУчет.ПРДляВычисленияАмортизации, ЕСТЬNULL(ВводОстатков.ПервоначальнаяСтоимостьПР, 0)) КАК ПервоначальнаяСтоимостьПР,
	|	НЕОПРЕДЕЛЕНО КАК Подразделение,
	|	НЕОПРЕДЕЛЕНО КАК ПорядокСписанияНИОКРНУ,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(Состояния.ДатаСостояния, ДЕНЬ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|			ТОГДА ЕСТЬNULL(ПринятиеКУчетуОС.ВключитьАмортизационнуюПремиюВСоставРасходов, ВводОстатков.ВключитьАмортизационнуюПремиюВСоставРасходов)
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВключитьАмортизационнуюПремиюВСоставРасходов,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(Состояния.ДатаСостояния, ДЕНЬ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|			ТОГДА ЕСТЬNULL(ПринятиеКУчетуОС.СтатьяРасходовАмортизационнойПремии, ВводОстатков.СтатьяРасходовАмортизационнойПремии)
	|		ИНАЧЕ NULL
	|	КОНЕЦ КАК СтатьяРасходовАмортизационнойПремии,
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(Состояния.ДатаСостояния, ДЕНЬ) = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
	|			ТОГДА ЕСТЬNULL(ПринятиеКУчетуОС.АналитикаРасходовАмортизационнойПремии, ВводОстатков.АналитикаРасходовАмортизационнойПремии)
	|		ИНАЧЕ NULL
	...
	|	ИЗ
	|		РегистрСведений.СостоянияОСОрганизаций КАК Состояния
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОСОрганизаций КАК СостоянияПринятПовторно
	|			ПО СостоянияПринятПовторно.ОсновноеСредство = Состояния.ОсновноеСредство
	|				И СостоянияПринятПовторно.Период > Состояния.Период
	|				И СостоянияПринятПовторно.Период <= &Период
	|				И (СостоянияПринятПовторно.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов
	|					ИЛИ СостоянияПринятПовторно.Регистратор ССЫЛКА Документ.ПоступлениеАрендованныхОС
	|					ИЛИ СостоянияПринятПовторно.Регистратор ССЫЛКА Документ.ПоступлениеПредметовЛизинга
	|					ИЛИ СостоянияПринятПовторно.Регистратор ССЫЛКА Документ.ПринятиеКУчетуОС)
	|	ГДЕ
	|		Состояния.Активность
	|		И Состояния.Период <= &Период
	|		И (Состояния.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов
	|			ИЛИ Состояния.Регистратор ССЫЛКА Документ.ПоступлениеАрендованныхОС
	|			ИЛИ Состояния.Регистратор ССЫЛКА Документ.ПоступлениеПредметовЛизинга
	|			ИЛИ Состояния.Регистратор ССЫЛКА Документ.ПринятиеКУчетуОС)
	|		И СостоянияПринятПовторно.ОсновноеСредство ЕСТЬ NULL
	|	
	|	СГРУППИРОВАТЬ ПО
	|		Состояния.ОсновноеСредство) КАК СостоянияДатаПервого
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОСОрганизаций КАК Состояния
	...
	|		ПО (ПараметрыАмортизацииОСНалоговыйУчет.ОсновноеСредство = Состояния.ОсновноеСредство)
	|			И (ПараметрыАмортизацииОСНалоговыйУчет.Регистратор = Состояния.Регистратор)
	|			И (Состояния.Регистратор ССЫЛКА Документ.ПринятиеКУчетуОС)
	...
	|	ЛОЖЬ КАК ВключитьАмортизационнуюПремиюВСоставРасходов,
	...
	|	ИЗ
	|		РегистрСведений.СостоянияНМАОрганизаций КАК Состояния
	|	ГДЕ
	|		Состояния.Активность
	|		И Состояния.Период <= &Период
	|		И Состояния.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.ПринятКУчету)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		Состояния.НематериальныйАктив,
	|		Состояния.Организация) КАК СостоянияДатаПервого
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияНМАОрганизаций КАК Состояния
	|		ПО (Состояния.Период = СостоянияДатаПервого.Период)
	|			И (Состояния.НематериальныйАктив = СостоянияДатаПервого.НематериальныйАктив)
	|			И (Состояния.Организация = СостоянияДатаПервого.Организация)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуНМА КАК ПринятиеКУчету
	|		ПО (ПринятиеКУчету.Ссылка = Состояния.Регистратор)
	...
	|		Состояния.Активность
	|		И Состояния.Период <= &Период
	|		И Состояния.Состояние В (ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.ПринятКУчету))
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыАмортизацииОСБухгалтерскийУчет КАК ПараметрыАмортизации
	...
	|	МАКСИМУМ(ДатыПринятияКУчету.ДатаПринятияКУчетуБУ) КАК ДатаПринятияКУчетуБУ,
	|	МАКСИМУМ(ДатыПринятияКУчету.ДатаПринятияКУчетуУУ) КАК ДатаПринятияКУчетуУУ,
	|	МАКСИМУМ(ДатыПринятияКУчету.ВариантРаздельногоУчетаНДС) КАК ВариантРаздельногоУчетаНДС,
	|	МАКСИМУМ(ДатыПринятияКУчету.ДатаМодернизации) КАК ДатаМодернизации,
	...
	|	МАКСИМУМ(ДатыПринятияКУчету.ВключитьАмортизационнуюПремиюВСоставРасходов) КАК ВключитьАмортизационнуюПремиюВСоставРасходов,
	...
	|		ДатыПринятияКУчету_БУ.ВключитьАмортизационнуюПремиюВСоставРасходов КАК ВключитьАмортизационнуюПремиюВСоставРасходов,
...
Процедура СоздатьГруппыФинансовогоУчета(ПараметрыОбработчика) 
	...
	|	СчетаУчетаРазличные КАК СчетаУчета
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыФинансовогоУчетаВнеоборотныхАктивов КАК ГруппыФинансовогоУчета
	...
	|			И (НЕ ГруппыФинансовогоУчета.ПометкаУдаления)
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НовыйОбъект.ВидАктива = Перечисления.ВидыВнеоборотныхАктивов.ОсновноеСредство 
			ИЛИ НовыйОбъект.ВидАктива = Перечисления.ВидыВнеоборотныхАктивов.ИнвестиционноеИмущество Тогда
			
			Если ЗначениеЗаполнено(НовыйОбъект.СчетУчета)
				И (НовыйОбъект.СчетУчета = ПланыСчетов.Хозрасчетный.ОСвОрганизации
					ИЛИ ПланыСчетов.Хозрасчетный.ОСвОрганизации.ПринадлежитЭлементу(НовыйОбъект.СчетУчета)) Тогда
					
				Если НЕ ЗначениеЗаполнено(НовыйОбъект.СчетУчетаВыбытия) Тогда
					НовыйОбъект.СчетУчетаВыбытия = ПланыСчетов.Хозрасчетный.ВыбытиеОС;
				КонецЕсли;
				
				Если НЕ ЗначениеЗаполнено(НовыйОбъект.СчетУчетаРезерваДооценки) Тогда
					НовыйОбъект.СчетУчетаРезерваДооценки = ПланыСчетов.Хозрасчетный.ПриростСтоимостиИмуществаПоПереоценке;
				КонецЕсли; 
				
				Если НЕ ЗначениеЗаполнено(НовыйОбъект.СчетЗабалансовогоУчета) Тогда
					НовыйОбъект.СчетЗабалансовогоУчета = ПланыСчетов.Хозрасчетный.АрендованныеОсновныеСредства;
				КонецЕсли; 
...
Процедура ОпределитьГруппыФинансовогоУчета(ПараметрыОбработчика)
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ГруппыФинансовогоУчетаВнеоборотныхАктивов КАК СпрГруппыФинансовогоУчета
	...
	|					ИЛИ СчетаУчета.СчетУчетаРезерваДооценки = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка))
	|				И (&УсловиеОписание)
	|				И (НЕ СпрГруппыФинансовогоУчета.ПометкаУдаления)
	|
	|	СГРУППИРОВАТЬ ПО
...
Процедура ТекстЗапросаПараметрыУчетаСобственныхОС(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ПараметрыУчетаОС_БУ.АдресМестонахожденияАрендатора                             КАК АдресМестонахожденияАрендатора,
	...
	|	ПараметрыУчетаОС_БУ.ДоговорЛизинга                                             КАК ДоговорЛизинга,
	...
	|	ПараметрыУчетаОС_БУ.НачислятьАмортизациюБУ                                     КАК НачислятьАмортизациюБУ,
	|	ПараметрыУчетаОС_БУ.НачислятьАмортизациюНУ                                     КАК НачислятьАмортизациюНУ,
...
Процедура ТекстЗапросаИзменениеПараметровУчетаСобственныхОС(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ПараметрыУчетаОС_БУ.ОсновноеСредство.НаправлениеДеятельности КАК НаправлениеДеятельности,
...
Процедура ТекстЗапросаПараметрыУчетаОС_БУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ПервоначальныеСведенияБУ.ПоказательНаработки КАК ПоказательНаработки,
	|	ПервоначальныеСведенияБУ.ПорядокПогашенияСтоимости КАК ПорядокПогашенияСтоимости,
	|	ПервоначальныеСведенияБУ.ГруппаОС КАК ГруппаОС,
	|	ПервоначальныеСведенияБУ.КодПоОКОФ КАК КодПоОКОФ,
	|	ПервоначальныеСведенияБУ.ШифрПоЕНАОФ КАК ШифрПоЕНАОФ,
	|	ПервоначальныеСведенияБУ.АмортизационнаяГруппа КАК АмортизационнаяГруппа,
	|	ПервоначальныеСведенияБУ.ДоговорЛизинга КАК ДоговорЛизинга,
	|	ПервоначальныеСведенияБУ.РезервПереоценкиСтоимости КАК РезервПереоценкиСтоимости,
	|	ПервоначальныеСведенияБУ.РезервПереоценкиАмортизации КАК РезервПереоценкиАмортизации
	...
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК ПервоначальныеСведенияБУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Организация
	...
	|	ПервоначальныеСведенияНУ.АмортизацияДо2002 КАК АмортизацияДо2002,
	|	ПервоначальныеСведенияНУ.АмортизацияДо2009 КАК АмортизацияДо2009,
	|	ПервоначальныеСведенияНУ.МетодНачисленияАмортизации КАК МетодНачисленияАмортизации,
	|	ПервоначальныеСведенияНУ.Организация КАК Организация,
	|	ПервоначальныеСведенияНУ.ПервоначальнаяСтоимостьНУ КАК ПервоначальнаяСтоимостьНУ,
	|	ПервоначальныеСведенияНУ.ПорядокВключенияСтоимостиВСоставРасходов КАК ПорядокВключенияСтоимостиВСоставРасходов,
	|	ПервоначальныеСведенияНУ.СтоимостьДо2002 КАК СтоимостьДо2002,
	|	ПервоначальныеСведенияНУ.ФактическийСрокИспользованияДо2009 КАК ФактическийСрокИспользованияДо2009
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК ПервоначальныеСведенияНУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПервоначальныеСведенияНУ.ОсновноеСредство,
	|	ПервоначальныеСведенияНУ.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	НачислениеАмортизацииБУ.Организация КАК Организация,
	|	НачислениеАмортизацииБУ.НачислятьАмортизацию КАК НачислятьАмортизацию
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК НачислениеАмортизацииБУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НачислениеАмортизацииБУ.ОсновноеСредство,
	|	НачислениеАмортизацииБУ.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачислениеАмортизацииНУ.ОсновноеСредство КАК ОсновноеСредство,
	|	НачислениеАмортизацииНУ.Организация КАК Организация,
	|	НачислениеАмортизацииНУ.НачислятьАмортизацию КАК НачислятьАмортизацию
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК НачислениеАмортизацииНУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НачислениеАмортизацииНУ.ОсновноеСредство,
	|	НачислениеАмортизацииНУ.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПараметрыАмортизацииБУ.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
	|	ПараметрыАмортизацииБУ.ОбъемПродукцииРабот КАК ОбъемПродукцииРабот,
	...
	|	ПараметрыАмортизацииБУ.КоэффициентАмортизации КАК КоэффициентАмортизации,
	...
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПараметрыАмортизацииБУ.ОсновноеСредство,
	|	ПараметрыАмортизацииБУ.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПараметрыАмортизацииНУ.ОсновноеСредство КАК ОсновноеСредство,
	|	ПараметрыАмортизацииНУ.Организация КАК Организация,
	|	ПараметрыАмортизацииНУ.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
	|	ПараметрыАмортизацииНУ.КоэффициентАмортизации КАК КоэффициентАмортизации,
	|	ПараметрыАмортизацииНУ.ПРДляВычисленияАмортизации КАК ПРДляВычисленияАмортизации
	|ПОМЕСТИТЬ ПараметрыАмортизацииОСНалоговыйУчет
	|ИЗ
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК ПараметрыАмортизацииНУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПараметрыАмортизацииНУ.ОсновноеСредство,
	|	ПараметрыАмортизацииНУ.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачислениеАмортизацииСпециальныйКоэффициент.ОсновноеСредство КАК ОсновноеСредство,
	|	НачислениеАмортизацииСпециальныйКоэффициент.Организация КАК Организация,
	|	НачислениеАмортизацииСпециальныйКоэффициент.СпециальныйКоэффициент КАК СпециальныйКоэффициент
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК НачислениеАмортизацииСпециальныйКоэффициент
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НачислениеАмортизацииСпециальныйКоэффициент.ОсновноеСредство,
	|	НачислениеАмортизацииСпециальныйКоэффициент.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СпособыОтраженияРасходов.ПередаватьРасходыВДругуюОрганизацию КАК ПередаватьРасходыВДругуюОрганизацию,
	|	СпособыОтраженияРасходов.ОрганизацияПолучательРасходов КАК ОрганизацияПолучательРасходов,
	|	СпособыОтраженияРасходов.СчетПередачиРасходов КАК СчетПередачиРасходов
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						СобственныеОС КАК СписокАктивов)) КАК СпособыОтраженияРасходов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СпособыОтраженияРасходов.ОсновноеСредство,
	|	СпособыОтраженияРасходов.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|						СобственныеОС.Организация
	|					ИЗ
	|						СобственныеОС КАК СобственныеОС)) КАК ГрафикиАмортизации
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ГрафикиАмортизации.ОсновноеСредство,
	|	ГрафикиАмортизации.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПараметрыАмортизацииОСБухгалтерскийУчет.ОсновноеСредство КАК ОсновноеСредство,
	|	МАКСИМУМ(НАЧАЛОПЕРИОДА(ПараметрыАмортизацииОСБухгалтерскийУчет.Период, ДЕНЬ)) КАК Период
	...
	|		ПО СписаниеОС.Ссылка = ПараметрыАмортизацииОСБухгалтерскийУчет.Регистратор
	|ГДЕ
	|	ПараметрыАмортизацииОСБухгалтерскийУчет.Период < &Период
	|	И (ПараметрыАмортизацииОСБухгалтерскийУчет.ОсновноеСредство, ПараметрыАмортизацииОСБухгалтерскийУчет.Организация) В
	|			(ВЫБРАТЬ
	|				СобственныеОС.ВнеоборотныйАктив,
	|				СобственныеОС.Организация
	|			ИЗ
	|				СобственныеОС КАК СобственныеОС)
	|	И ТИПЗНАЧЕНИЯ(ПараметрыАмортизацииОСБухгалтерскийУчет.Регистратор) В (
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ПервоначальныеСведенияБУ.ДоговорЛизинга <> ЗНАЧЕНИЕ(Справочник.ДоговорыЛизинга.ПустаяСсылка)
	...
	|	ПервоначальныеСведенияБУ.ДоговорЛизинга                              КАК ДоговорЛизинга,
	|	ПервоначальныеСведенияБУ.СпособПоступления                           КАК СпособПоступления,
	|	ПервоначальныеСведенияБУ.ПорядокПогашенияСтоимости                   КАК ПорядокУчетаБУ,
	|	ПервоначальныеСведенияНУ.ПорядокВключенияСтоимостиВСоставРасходов    КАК ПорядокУчетаНУ,
	|	ПараметрыЦелевогоФинансирования.ПрименениеЦелевогоФинансирования     КАК ПрименениеЦелевогоФинансирования,
	...
	|	ЕСТЬNULL(ДатыПринятияКУчету.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) КАК НалогообложениеНДС,
	...
	|	ЕСТЬNULL(ПервоначальныеСведенияБУ.ПервоначальнаяСтоимость, 0)        КАК ПервоначальнаяСтоимостьБУ,
	|	ЕСТЬNULL(ПервоначальныеСведенияНУ.ПервоначальнаяСтоимостьНУ, 0)      КАК ПервоначальнаяСтоимостьНУ,
	|	ЕСТЬNULL(ДатыПринятияКУчету.ПервоначальнаяСтоимостьПР, 0)            КАК ПервоначальнаяСтоимостьПР,
	
	// Накопленная амортизация
	|	ВЫБОР 
	|		КОГДА ПервоначальныеСведенияБУ.ПорядокПогашенияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НачислениеИзносаПоЕНАОФ)
	...
	|	НачислениеАмортизацииБУ.НачислятьАмортизацию
	|		И ПервоначальныеСведенияБУ.ПорядокПогашенияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НачислениеИзносаПоЕНАОФ) КАК НачислятьИзносБУ,
	|
	|	ПервоначальныеСведенияБУ.СпособНачисленияАмортизации                 КАК МетодНачисленияАмортизации,
	|	ПараметрыАмортизацииБУ.СтоимостьДляВычисленияАмортизации             КАК СтоимостьДляВычисленияАмортизации,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПоследнееИзменениеПараметровАмортизации.Период, ДатыПринятияКУчету.ДатаМодернизации) <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ПараметрыАмортизацииНУ.ПРДляВычисленияАмортизации
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                                КАК ПРДляВычисленияАмортизации,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПоследнееИзменениеПараметровАмортизации.Период, ДатыПринятияКУчету.ДатаМодернизации) <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ПараметрыАмортизацииБУ.СрокИспользованияДляВычисленияАмортизации
	|		ИНАЧЕ ПараметрыАмортизацииБУ.СрокПолезногоИспользования
	|	КОНЕЦ                                                                КАК СрокИспользованияБУ,
	|	ПараметрыАмортизацииНУ.СрокПолезногоИспользования                    КАК СрокИспользованияНУ,
	|	ГрафикиАмортизации.ГрафикАмортизации                                 КАК ГрафикАмортизации,
	
	|	ПервоначальныеСведенияБУ.ПоказательНаработки                         КАК ПоказательНаработки,
	|	ПараметрыАмортизацииБУ.ОбъемПродукцииРабот                           КАК ОбъемНаработки,
	|	ПараметрыАмортизацииБУ.ОбъемПродукцииРаботДляВычисленияАмортизации   КАК ОбъемНаработкиОстаточный,
	
	|	ПараметрыАмортизацииБУ.КоэффициентАмортизации                        КАК КоэффициентАмортизацииБУ,
	|	ПараметрыАмортизацииБУ.КоэффициентУскорения                          КАК КоэффициентУскоренияБУ,
	|	НачислениеАмортизацииСпециальныйКоэффициент.СпециальныйКоэффициент   КАК СпециальныйКоэффициентНУ,
	
	// Сведения об использовании до 2009 года
	|	ЕСТЬNULL(ПервоначальныеСведенияНУ.АмортизацияДо2009, 0)                  КАК НакопленнаяАмортизацияНУДо2009,
	|	ЕСТЬNULL(ПервоначальныеСведенияНУ.ФактическийСрокИспользованияДо2009,0)  КАК СрокИспользованияНУДо2009,
	...
	// Передача расходов
	|	ЕСТЬNULL(СпособыОтраженияРасходов.ПередаватьРасходыВДругуюОрганизацию, ЛОЖЬ) КАК ПередаватьРасходыВДругуюОрганизацию,
	|	СпособыОтраженияРасходов.ОрганизацияПолучательРасходов               КАК ОрганизацияПолучательРасходов,
	|	СпособыОтраженияРасходов.СчетПередачиРасходов                        КАК СчетПередачиРасходов,
	
	// Амортизационная премия
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(НАЧАЛОПЕРИОДА(ДатыПринятияКУчету.ДатаПринятияКУчетуБУ, МЕСЯЦ), ДАТАВРЕМЯ(1, 1, 1)) = &ПериодНачалоМесяца 
	...
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(НАЧАЛОПЕРИОДА(ДатыПринятияКУчету.ДатаПринятияКУчетуБУ, МЕСЯЦ), ДАТАВРЕМЯ(1, 1, 1)) = &ПериодНачалоМесяца
	|			ТОГДА ДатыПринятияКУчету.ВключитьАмортизационнуюПремиюВСоставРасходов 
	|		ИНАЧЕ 0 
	|	КОНЕЦ                                                                КАК ВключитьАмортизационнуюПремиюВСоставРасходов,
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(НАЧАЛОПЕРИОДА(ДатыПринятияКУчету.ДатаПринятияКУчетуБУ, МЕСЯЦ), ДАТАВРЕМЯ(1, 1, 1)) = &ПериодНачалоМесяца
	|				И ДатыПринятияКУчету.ВключитьАмортизационнуюПремиюВСоставРасходов 
	|			ТОГДА ЕСТЬNULL(АмортизационнаяПремия.СуммаНУОстатокДт, 0)
	|		ИНАЧЕ 0 
	...
	|ИЗ
	|	СобственныеОС КАК ПринятыеКУчетуОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОСБухгалтерскийУчет КАК ПервоначальныеСведенияБУ
	|		ПО (ПервоначальныеСведенияБУ.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (ПервоначальныеСведенияБУ.Организация = ПринятыеКУчетуОС.Организация)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОСНалоговыйУчет КАК ПервоначальныеСведенияНУ
	|		ПО (ПервоначальныеСведенияНУ.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (ПервоначальныеСведенияНУ.Организация = ПринятыеКУчетуОС.Организация)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ МестонахождениеОСБухгалтерскийУчет КАК МестонахождениеОС
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачислениеАмортизацииОСБухгалтерскийУчет КАК НачислениеАмортизацииБУ
	|		ПО (НачислениеАмортизацииБУ.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (НачислениеАмортизацииБУ.Организация = ПринятыеКУчетуОС.Организация)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачислениеАмортизацииОСНалоговыйУчет КАК НачислениеАмортизацииНУ
	|		ПО (НачислениеАмортизацииНУ.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (НачислениеАмортизацииНУ.Организация = ПринятыеКУчетуОС.Организация)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыАмортизацииОСБухгалтерскийУчет КАК ПараметрыАмортизацииБУ
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыАмортизацииОСНалоговыйУчет КАК ПараметрыАмортизацииНУ
	|		ПО (ПараметрыАмортизацииНУ.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (ПараметрыАмортизацииНУ.Организация = ПринятыеКУчетуОС.Организация)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачислениеАмортизацииОССпециальныйКоэффициентНалоговыйУчет КАК НачислениеАмортизацииСпециальныйКоэффициент
	|		ПО (НачислениеАмортизацииСпециальныйКоэффициент.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (НачислениеАмортизацииСпециальныйКоэффициент.Организация = ПринятыеКУчетуОС.Организация)
	|
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ СпособыОтраженияРасходовПоАмортизацииОСБухгалтерскийУчет КАК СпособыОтраженияРасходов
	|		ПО (СпособыОтраженияРасходов.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (СпособыОтраженияРасходов.Организация = ПринятыеКУчетуОС.Организация)
	|
	...
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ГрафикиАмортизацииОСБухгалтерскийУчет КАК ГрафикиАмортизации
	|		ПО (ГрафикиАмортизации.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (ГрафикиАмортизации.Организация = ПринятыеКУчетуОС.Организация)
	|
	...
	|			И (ПервоначальныеСведенияБУ.ПорядокПогашенияСтоимости <> ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НачислениеИзносаПоЕНАОФ)
	|					И НачисленнаяАмортизация.Счет = ГруппыФинансовогоУчета.СчетУчетаАмортизации
	|				ИЛИ ПервоначальныеСведенияБУ.ПорядокПогашенияСтоимости = ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НачислениеИзносаПоЕНАОФ)
...
Процедура ТекстЗапросаПараметрыУчетаПереданныхВАрендуОС(ТекстыЗапроса)
	...
	|	ЕСТЬNULL(ВводОстатков.Контрагент, ПередачаОСАрендатору.Арендатор) КАК Арендатор,
	|	ЕСТЬNULL(ВводОстатков.ПодразделениеАрендатора, ПередачаОСАрендатору.ПодразделениеАрендатора) КАК ПодразделениеАрендатора,
	|	ЕСТЬNULL(ВводОстатков.ЗалоговаяСтоимость, ПередачаОСАрендаторуОС.ЗалоговаяСтоимость) КАК ЗалоговаяСтоимость
	...
	|	ИЗ
	|		РегистрСведений.МестонахождениеОСБухгалтерскийУчет КАК ПереданныеВАренду
	|	ГДЕ
	|		ПереданныеВАренду.Активность
	|		И ПереданныеВАренду.Период <= &Период
	|		И (ПереданныеВАренду.Регистратор ССЫЛКА Документ.ПередачаОСАрендатору
	|				ИЛИ ПереданныеВАренду.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов
	|				ИЛИ ПереданныеВАренду.Регистратор ССЫЛКА Документ.ВозвратОСОтАрендатора)
	...
	|		ПО (МестонахождениеОСБухгалтерскийУчет.Организация = ПереданныеВАрендуПоследнее.Организация)
	|			И (МестонахождениеОСБухгалтерскийУчет.ОсновноеСредство = ПереданныеВАрендуПоследнее.ОсновноеСредство)
	|			И (МестонахождениеОСБухгалтерскийУчет.Период = ПереданныеВАрендуПоследнее.Период)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаОСАрендатору КАК ПередачаОСАрендатору
	|		ПО (ПередачаОСАрендатору.Ссылка = МестонахождениеОСБухгалтерскийУчет.Регистратор)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаОСАрендатору.ОС КАК ПередачаОСАрендаторуОС
	|		ПО (ПередачаОСАрендаторуОС.Ссылка = МестонахождениеОСБухгалтерскийУчет.Регистратор)
	|			И (ПередачаОСАрендаторуОС.ОсновноеСредство = МестонахождениеОСБухгалтерскийУчет.ОсновноеСредство)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов.ОС КАК ВводОстатков
	|		ПО (ВводОстатков.Ссылка = МестонахождениеОСБухгалтерскийУчет.Регистратор)
	|			И (ВводОстатков.ОсновноеСредство = МестонахождениеОСБухгалтерскийУчет.ОсновноеСредство)
	|			И (ВводОстатков.Ссылка.ТипОперации В (
	...
	|
	|ГДЕ
	|	(НЕ ПередачаОСАрендатору.Ссылка ЕСТЬ NULL
	|			ИЛИ НЕ ВводОстатков.Ссылка ЕСТЬ NULL)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
...
Процедура ТекстЗапросаПараметрыУчетаОС_УУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	Иначе	
		...
		// Восстановительная стоимость
		|	ОстаткиПоСчетамУчета.СуммаОстатокДт                        КАК ТекущаяСтоимость,
		
		// Первоначальная стоимость
		|	ОстаткиПоСчетамУчета.СуммаОстатокДт                        КАК ПервоначальнаяСтоимость,
		...
		|ИЗ
		|	ПараметрыУчетаОС_БУ КАК ПараметрыУчетаОС_БУ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОстаткиПоСчетамУчетаОС_УУ КАК ОстаткиПоСчетамУчета
		...
		|
		|ГДЕ
		|	НЕ ПараметрыУчетаОС_БУ.ПорядокУчетаБУ В (
		...
		|	И ОстаткиПоСчетамУчета.СуммаОстатокДт > 0
		|	И (ПараметрыУчетаОС_БУ.СрокИспользованияБУ 
		|		- РАЗНОСТЬДАТ(ПараметрыУчетаОС_БУ.ДатаПринятияКУчету, &Период, МЕСЯЦ)) > 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство,
		|	Организация";
...
Процедура ТекстЗапросаОстаткиПоСчетамУчетаОС_УУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	Если ПараметрыОбработчика.ИспользоватьДанныеМеждународногоУчета Тогда
		...
		|	ОстаткиНачалоПериода.СуммаОстатокДт * &КоэффициентПересчетаИзВалютыВВалюту  КАК СуммаОстатокДт,
		|	ОстаткиНачалоПериода.СуммаОстатокКт * &КоэффициентПересчетаИзВалютыВВалюту  КАК СуммаОстатокКт
...
Процедура ТекстЗапросаПараметрыУчетаАрендованныхОС_БУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	ТекстЗапросаСведенияОбАрендодателе(ТекстыЗапроса);
	...
	|	ПервоначальныеСведенияБУ.РезервПереоценкиСтоимости КАК РезервПереоценкиСтоимости,
	|	ПервоначальныеСведенияБУ.РезервПереоценкиАмортизации КАК РезервПереоценкиАмортизации
	|ПОМЕСТИТЬ ПервоначальныеСведенияОСБухгалтерскийУчет_Аренда
	|ИЗ
	|	РегистрСведений.ПервоначальныеСведенияОСБухгалтерскийУчет.СрезПоследних(
	|			&Период,
	|			НЕ Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов2_4
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						АрендованныеОС КАК СписокАктивов)) КАК ПервоначальныеСведенияБУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|	МестонахождениеОС.Местонахождение КАК Местонахождение,
	|	МестонахождениеОС.АдресМестонахождения КАК АдресМестонахождения
	|ПОМЕСТИТЬ МестонахождениеОСБухгалтерскийУчет_Аренда
	|ИЗ
	|	РегистрСведений.МестонахождениеОСБухгалтерскийУчет.СрезПоследних(
	|			&Период,
	|			НЕ Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов2_4
	...
	|						СписокАктивов.Организация
	|					ИЗ
	|						АрендованныеОС КАК СписокАктивов)) КАК МестонахождениеОС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Организация
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковАрендованныхОС)
	|	КОНЕЦ КАК ХозяйственнаяОперация,
	|	ПринятыеКУчетуОС.ВнеоборотныйАктив КАК ОсновноеСредство,
	|	ПринятыеКУчетуОС.Организация КАК Организация,
	...
	|	ПервоначальныеСведенияБУ.ДоговорЛизинга КАК ДоговорЛизинга,
	|	СведенияОбАрендованныхОС.ЗалоговаяСтоимость КАК ЗалоговаяСтоимость,
	|	ЕСТЬNULL(СведенияОбАрендованныхОС.Арендодатель, НЕОПРЕДЕЛЕНО) КАК Контрагент,
	...
	|ИЗ
	|	АрендованныеОС КАК ПринятыеКУчетуОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияОСБухгалтерскийУчет_Аренда КАК ПервоначальныеСведенияБУ
	|		ПО (ПервоначальныеСведенияБУ.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|		ЛЕВОЕ СОЕДИНЕНИЕ МестонахождениеОСБухгалтерскийУчет_Аренда КАК МестонахождениеОС
	|		ПО (МестонахождениеОС.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (МестонахождениеОС.Организация = ПринятыеКУчетуОС.Организация)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ГруппыФинансовогоУчета КАК ГруппыФинансовогоУчета
	|		ПО (ГруппыФинансовогоУчета.ВнеоборотныйАктив = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДатыПринятияКУчету КАК ДатыПринятияКУчету
	|		ПО (ДатыПринятияКУчету.ВнеоборотныйАктив = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|		ЛЕВОЕ СОЕДИНЕНИЕ СведенияОбАрендованныхОС КАК СведенияОбАрендованныхОС
	|		ПО (СведенияОбАрендованныхОС.ОсновноеСредство = ПринятыеКУчетуОС.ВнеоборотныйАктив)
	|			И (СведенияОбАрендованныхОС.Организация = ПринятыеКУчетуОС.Организация)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Организация";
...
Процедура ТекстЗапросаСведенияОбАрендодателе(ТекстыЗапроса)

	ТекстЗапроса =
	"ВЫБРАТЬ
	|	АрендованныеОСПоследнее.ОсновноеСредство КАК ОсновноеСредство,
	|	АрендованныеОСПоследнее.Организация КАК Организация,
	|	ЕСТЬNULL(ВводОстатков.Контрагент, ЕСТЬNULL(ПоступлениеАрендованныхОС.Ссылка.Арендодатель, ПоступлениеПредметовЛизинга.Ссылка.Контрагент)) КАК Арендодатель,
	|	ЕСТЬNULL(ВводОстатков.ПервоначальнаяСтоимостьБУ, ЕСТЬNULL(ПоступлениеАрендованныхОС.ЗалоговаяСтоимость, ПоступлениеПредметовЛизинга.Сумма)) КАК ЗалоговаяСтоимость
	|ПОМЕСТИТЬ СведенияОбАрендованныхОС
	|ИЗ
	|	(ВЫБРАТЬ
	|		МАКСИМУМ(АрендованныеОС.Период) КАК Период,
	|		АрендованныеОС.ОсновноеСредство КАК ОсновноеСредство,
	|		АрендованныеОС.Организация КАК Организация
	|	ИЗ
	|		РегистрСведений.СостоянияОСОрганизаций КАК АрендованныеОС
	|	ГДЕ
	|		АрендованныеОС.Активность
	|		И АрендованныеОС.Период <= &Период
	|		И (АрендованныеОС.Регистратор ССЫЛКА Документ.ПоступлениеАрендованныхОС
	|				ИЛИ АрендованныеОС.Регистратор ССЫЛКА Документ.ПоступлениеПредметовЛизинга
	|				ИЛИ АрендованныеОС.Регистратор ССЫЛКА Документ.ВводОстатковВнеоборотныхАктивов
	|				ИЛИ АрендованныеОС.Регистратор ССЫЛКА Документ.ВыбытиеАрендованныхОС)
	|		И (АрендованныеОС.ОсновноеСредство, АрендованныеОС.Организация) В
	|				(ВЫБРАТЬ
	|					ПринятыеКУчетуОС.ВнеоборотныйАктив,
	|					ПринятыеКУчетуОС.Организация
	|				ИЗ
	...
	|		АрендованныеОС.Организация) КАК АрендованныеОСПоследнее
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОСОрганизаций КАК СостоянияОСОрганизаций
	|		ПО (СостоянияОСОрганизаций.ОсновноеСредство = АрендованныеОСПоследнее.ОсновноеСредство)
	|			И (СостоянияОСОрганизаций.ОсновноеСредство = АрендованныеОСПоследнее.ОсновноеСредство)
	|			И (СостоянияОСОрганизаций.Период = АрендованныеОСПоследнее.Период)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеАрендованныхОС.ОС КАК ПоступлениеАрендованныхОС
	|		ПО (ПоступлениеАрендованныхОС.Ссылка = СостоянияОСОрганизаций.Регистратор)
	|			И (ПоступлениеАрендованныхОС.ОсновноеСредство = СостоянияОСОрганизаций.ОсновноеСредство)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеПредметовЛизинга.ОС КАК ПоступлениеПредметовЛизинга
	|		ПО (ПоступлениеПредметовЛизинга.Ссылка = СостоянияОСОрганизаций.Регистратор)
	|			И (ПоступлениеПредметовЛизинга.ОсновноеСредство = СостоянияОСОрганизаций.ОсновноеСредство)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВыбытиеАрендованныхОС.ОС КАК ВыбытиеАрендованныхОС
	|		ПО (ВыбытиеАрендованныхОС.Ссылка = СостоянияОСОрганизаций.Регистратор)
	|			И (ВыбытиеАрендованныхОС.ОсновноеСредство = СостоянияОСОрганизаций.ОсновноеСредство)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов.ОС КАК ВводОстатков
	|		ПО (ВводОстатков.Ссылка = СостоянияОСОрганизаций.Регистратор)
	|			И (ВводОстатков.ОсновноеСредство = СостоянияОСОрганизаций.ОсновноеСредство)
	|			И (ВводОстатков.Ссылка.ТипОперации В (
	...
	|ГДЕ
	|	(НЕ ПоступлениеАрендованныхОС.Ссылка ЕСТЬ NULL
	|			ИЛИ НЕ ПоступлениеАрендованныхОС.Ссылка ЕСТЬ NULL
	|			ИЛИ НЕ ПоступлениеПредметовЛизинга.Ссылка ЕСТЬ NULL
	|			ИЛИ НЕ ВводОстатков.Ссылка ЕСТЬ NULL)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
...
Процедура ТекстЗапросаПараметрыУчетаНМА(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ПараметрыУчетаНМА_БУ.НачислятьАмортизациюБУ                                           КАК НачислятьАмортизациюБУ,
	|	ПараметрыУчетаНМА_БУ.НачислятьАмортизациюНУ                                           КАК НачислятьАмортизациюНУ,
	|	ПараметрыУчетаНМА_УУ.НачислятьАмортизацию                                             КАК НачислятьАмортизациюУУ,
...
Процедура ТекстЗапросаПараметрыУчетаНМА_БУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|					СписокАктивов.ВнеоборотныйАктив
	|				ИЗ
	|					ПринятыеКУчетуНМА КАК СписокАктивов)) КАК ПервоначальныеСведенияБУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НематериальныйАктив
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПервоначальныеСведенияНУ.НематериальныйАктив КАК НематериальныйАктив,
	|	ПервоначальныеСведенияНУ.АмортизацияДо2002 КАК АмортизацияДо2002,
	|	ПервоначальныеСведенияНУ.АмортизацияДо2009 КАК АмортизацияДо2009,
	|	ПервоначальныеСведенияНУ.ДатаПриобретения КАК ДатаПриобретения,
	|	ПервоначальныеСведенияНУ.МетодНачисленияАмортизации КАК МетодНачисленияАмортизации,
	|	ПервоначальныеСведенияНУ.НачислятьАмортизацию КАК НачислятьАмортизацию,
	|	ПервоначальныеСведенияНУ.Организация КАК Организация,
	|	ПервоначальныеСведенияНУ.ПервоначальнаяСтоимостьНУ КАК ПервоначальнаяСтоимостьНУ,
	|	ПервоначальныеСведенияНУ.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
	|	ПервоначальныеСведенияНУ.СтоимостьДо2002 КАК СтоимостьДо2002,
	|	ПервоначальныеСведенияНУ.ФактическийСрокИспользованияДо2009 КАК ФактическийСрокИспользованияДо2009,
	|	ПервоначальныеСведенияНУ.ПорядокУчетаНУ КАК ПорядокУчетаНУ
	...
	|					СписокАктивов.ВнеоборотныйАктив
	|				ИЗ
	|					ПринятыеКУчетуНМА КАК СписокАктивов)) КАК ПервоначальныеСведенияНУ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НематериальныйАктив
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачислениеАмортизацииСпециальныйКоэффициент.НематериальныйАктив КАК НематериальныйАктив,
	|	НачислениеАмортизацииСпециальныйКоэффициент.Организация КАК Организация,
	|	НачислениеАмортизацииСпециальныйКоэффициент.СпециальныйКоэффициент КАК СпециальныйКоэффициент
	|ПОМЕСТИТЬ НачислениеАмортизацииНМАСпециальныйКоэффициентНалоговыйУчет
	|ИЗ
	...
	|					СписокАктивов.Организация
	|				ИЗ
	|					ПринятыеКУчетуНМА КАК СписокАктивов)) КАК НачислениеАмортизацииСпециальныйКоэффициент
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НематериальныйАктив,
	|	Организация
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СпособыОтраженияРасходов.НематериальныйАктив КАК НематериальныйАктив,
	|	СпособыОтраженияРасходов.Организация КАК Организация,
	|	СпособыОтраженияРасходов.СтатьяРасходов КАК СтатьяРасходов,
	|	СпособыОтраженияРасходов.АналитикаРасходов КАК АналитикаРасходов,
	|	СпособыОтраженияРасходов.СпособОтраженияРасходовЗаданДокументом КАК СпособОтраженияРасходовЗаданДокументом,
	...
	|	СпособыОтраженияРасходов.ОрганизацияПолучательРасходов КАК ОрганизацияПолучательРасходов,
	|	СпособыОтраженияРасходов.СчетПередачиРасходов КАК СчетПередачиРасходов
	|ПОМЕСТИТЬ СпособыОтраженияРасходовПоАмортизацииНМАБухгалтерскийУчет
	|ИЗ
	...
	|					СписокАктивов.Организация
	|				ИЗ
	|					ПринятыеКУчетуНМА КАК СписокАктивов)) КАК СпособыОтраженияРасходов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НематериальныйАктив,
	|	Организация
	...
	|	ДатыПринятияКУчету.ДатаПринятияКУчетуБУ                                               КАК ДатаПринятияКУчету,
	|	ВЫБОР 
	|		КОГДА ПервоначальныеСведенияНУ.ДатаПриобретения <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ПервоначальныеСведенияНУ.ДатаПриобретения
	...
	|
	|	ВЫБОР 
	|		КОГДА ПервоначальныеСведенияБУ.НачислятьАмортизацию
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокУчетаСтоимостиВнеоборотныхАктивов.НачислятьАмортизацию)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ПорядокУчетаСтоимостиВнеоборотныхАктивов.НеНачислятьАмортизацию)
	|	КОНЕЦ                                                                                 КАК ПорядокУчетаБУ,
	|
	|	ВЫБОР 
	|		КОГДА ПринятыеКУчетуНМА.ВнеоборотныйАктив.ВидОбъектаУчета = ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовУчетаНМА.РасходыНаНИОКР)
	|			ТОГДА ВЫБОР ДатыПринятияКУчету.ПорядокСписанияНИОКРНУ
	...
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокУчетаСтоимостиВнеоборотныхАктивов.НеНачислятьАмортизацию) 
	|				КОНЕЦ
	|		КОГДА НЕ ПервоначальныеСведенияНУ.НачислятьАмортизацию
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокУчетаСтоимостиВнеоборотныхАктивов.НеНачислятьАмортизацию) 
	|	ИНАЧЕ
	|		ВЫБОР ПервоначальныеСведенияНУ.ПорядокУчетаНУ
	...
	|	ПервоначальныеСведенияБУ.СпособНачисленияАмортизации                                  КАК МетодНачисленияАмортизацииБУ,
	|	ПервоначальныеСведенияБУ.Коэффициент                                                  КАК КоэффициентБУ,
	|	ПервоначальныеСведенияБУ.СрокПолезногоИспользования                                   КАК СрокИспользованияБУ,
	|	ПервоначальныеСведенияБУ.ОбъемПродукцииРаботДляВычисленияАмортизации                  КАК ОбъемНаработки,
	
	|	ПервоначальныеСведенияНУ.НачислятьАмортизацию                                         КАК НачислятьАмортизациюНУ,
	|	ПервоначальныеСведенияНУ.МетодНачисленияАмортизации                                   КАК МетодНачисленияАмортизацииНУ,
	|	ПервоначальныеСведенияНУ.АмортизацияДо2009                                            КАК АмортизацияДо2009,
	|	НачислениеАмортизацииСпециальныйКоэффициент.СпециальныйКоэффициент                    КАК СпециальныйКоэффициентНУ,
	|	ПервоначальныеСведенияНУ.СрокПолезногоИспользования                                   КАК СрокИспользованияНУ,
	|	ПервоначальныеСведенияНУ.ФактическийСрокИспользованияДо2009                           КАК СрокИспользованияНУДо2009,
	...
	|	ПринятыеКУчетуНМА КАК ПринятыеКУчетуНМА
	|
	// ПервоначальныеСведенияБУ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияНМАБухгалтерскийУчет КАК ПервоначальныеСведенияБУ
	|		ПО (ПервоначальныеСведенияБУ.НематериальныйАктив = ПринятыеКУчетуНМА.ВнеоборотныйАктив)
	|
	// ПервоначальныеСведенияНУ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПервоначальныеСведенияНМАНалоговыйУчет КАК ПервоначальныеСведенияНУ
	|		ПО (ПервоначальныеСведенияНУ.НематериальныйАктив = ПринятыеКУчетуНМА.ВнеоборотныйАктив)
	|
	// НачислениеАмортизацииСпециальныйКоэффициент
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачислениеАмортизацииНМАСпециальныйКоэффициентНалоговыйУчет КАК НачислениеАмортизацииСпециальныйКоэффициент
	|		ПО (НачислениеАмортизацииСпециальныйКоэффициент.НематериальныйАктив = ПринятыеКУчетуНМА.ВнеоборотныйАктив)
	|			И (НачислениеАмортизацииСпециальныйКоэффициент.Организация = ПринятыеКУчетуНМА.Организация)
	|
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СпособыОтраженияРасходовПоАмортизацииНМАБухгалтерскийУчет КАК СпособыОтраженияРасходов
	|		ПО (СпособыОтраженияРасходов.НематериальныйАктив = ПринятыеКУчетуНМА.ВнеоборотныйАктив)
	|			И (СпособыОтраженияРасходов.Организация = ПринятыеКУчетуНМА.Организация)
	|
	// Прочие таблицы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ГруппыФинансовогоУчета КАК ГруппыФинансовогоУчета
...
Процедура ТекстЗапросаПараметрыУчетаНМА_УУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	ИмяТаблицы = "ПараметрыУчетаНМА_УУ";
	...
	Если ПараметрыОбработчика.ИспользоватьДанныеМеждународногоУчета Тогда
		...
		|			И (НачисленнаяАмортизация.Счет = ПервоначальныеСведения.СчетАмортизации)
		|ГДЕ
		|	ПервоначальныеСведения.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.ПринятКУчету)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	НематериальныйАктив,
		|	Организация";
	...
	Иначе	
		...
		|	ПараметрыУчетаНМА_БУ.ПорядокУчетаБУ                                                   КАК ПорядокУчета,
		|	ПараметрыУчетаНМА_БУ.СпособПоступления                                                КАК СпособПоступления,
		|	0                                                                                     КАК ЛиквидационнаяСтоимость,
		
		// Стоимость
		|	ОстаткиПоСчетамУчета.СуммаОстатокДт                                                   КАК ТекущаяСтоимостьУУ,
		|	ОстаткиПоСчетамУчета.СуммаОстатокДт                                                   КАК ПервоначальнаяСтоимостьУУ,
		|	0                                                                                     КАК НакопленнаяАмортизацияУУ,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОстаткиПоСчетамУчетаНМА_УУ КАК ОстаткиПоСчетамУчета
		...
		|
		|ГДЕ
		|	ОстаткиПоСчетамУчета.СуммаОстатокДт > 0
		|	И (ПараметрыУчетаНМА_БУ.СрокИспользованияБУ 
		|		- РАЗНОСТЬДАТ(ПараметрыУчетаНМА_БУ.ДатаПринятияКУчету, &Период, МЕСЯЦ)) > 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	НематериальныйАктив,
		|	Организация";
...
Процедура ТекстЗапросаИзменениеПараметровУчетаНМА(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ПараметрыУчетаНМА_БУ.НематериальныйАктив.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ПараметрыУчетаНМА_БУ.СтатьяРасходов                           КАК СтатьяРасходов,
	|	ПараметрыУчетаНМА_БУ.АналитикаРасходов                        КАК АналитикаРасходов,
	|	ИСТИНА                                                        КАК ПередаватьРасходыВДругуюОрганизацию,
	|	ПараметрыУчетаНМА_БУ.ОрганизацияПолучательРасходов            КАК ОрганизацияПолучательРасходов,
...
Функция ТекстЗапросаВложенияВоВнеоборотныеАктивы(ПараметрыОбработчика)
	...
	|	ВложенияСуммы.СуммаРегл                КАК СуммаРегл,
	|	ВложенияСуммы.СуммаПР                  КАК СуммаПР,
	|	ВложенияСуммы.СуммаВР                  КАК СуммаВР,
	|	ВложенияСуммы.Сумма                    КАК Сумма,
	|	ВложенияСуммы.СуммаБезНДС              КАК СуммаБезНДС
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов) КАК ХозяйственнаяОперация,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов) КАК ТипОперации,
	|
	|	&Период                                КАК Дата,
...
Функция ТекстЗапросаПроверитьВложенияВоВнеоборотныеАктивы(ПараметрыОбработчика)
	...
	|	И (ВложенияСуммы.СуммаРегл <> ЕСТЬNULL(ВложенияСуммы24.СуммаРегл, 0)
	|		ИЛИ ВложенияСуммы.СуммаПР <> ЕСТЬNULL(ВложенияСуммы24.СуммаПР, 0)
	|		ИЛИ ВложенияСуммы.СуммаВР <> ЕСТЬNULL(ВложенияСуммы24.СуммаВР, 0)
	|		ИЛИ ВложенияСуммы.Сумма <> ЕСТЬNULL(ВложенияСуммы24.Сумма, 0)
	|		ИЛИ ВложенияСуммы.СуммаБезНДС <> ЕСТЬNULL(ВложенияСуммы24.СуммаБезНДС, 0))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|					ИЛИ Аналитика ССЫЛКА Справочник.НематериальныеАктивы) КАК ТаблицаОстатки
	|	ГДЕ
...
Процедура ТекстЗапросаЛишниеВложенияВоВнеоборотныеАктивы(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	NULL                                    КАК СсылкаДубль,
	|	ДанныеДокумента.Организация             КАК Организация,
	|	ДанныеДокумента.ТипОперации             КАК ХозяйственнаяОперация,
	|	ДанныеДокумента.Проведен                КАК Проведен,
	...
	|
	|	И ДанныеДокумента.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов)
	|
...
Процедура ТекстЗапросаВложенияВоВнеоборотныеАктивы_УУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	И &ВедетсяУправленческийУчетВНА
	|	И (&ИспользоватьДанныеМеждународногоУчета
	|				И КОНЕЦПЕРИОДА(ТаблицаДвижения.Период, МЕСЯЦ) > ЕСТЬNULL(КОНЕЦПЕРИОДА(ДатыПринятияКУчету.ДатаПринятияКУчетуУУ, МЕСЯЦ), ДАТАВРЕМЯ(1, 1, 1))
	|			ИЛИ НЕ &ИспользоватьДанныеМеждународногоУчета
	|				И КОНЕЦПЕРИОДА(ТаблицаДвижения.Период, МЕСЯЦ) > ЕСТЬNULL(КОНЕЦПЕРИОДА(ДатыПринятияКУчету.ДатаПринятияКУчетуБУ, МЕСЯЦ), ДАТАВРЕМЯ(1, 1, 1)))
	|
	...
	|			ИЛИ ТаблицаДвижения.Регистратор ССЫЛКА Документ.ВозвратОСОтАрендатора
	...
	|			ИЛИ ТаблицаДвижения.Регистратор ССЫЛКА Документ.ПередачаОСАрендатору
...
Процедура ТекстЗапросаВложенияВоВнеоборотныеАктивы_БУ(ТекстыЗапроса, ПараметрыОбработчика)
	...
	ВнеоборотныеАктивыСлужебный.ТекстЗапросаВтСчетаВложенияВоВнеоборотныеАктивы(Истина, ТекстыЗапроса);
	...
	|	И (ОстаткиПоСчетам.СуммаОборот > 0
	|			ИЛИ ОстаткиПоСчетам.СуммаПРОборот > 0
	|			ИЛИ ОстаткиПоСчетам.СуммаВРОборот > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ВнеоборотныйАктив,
	|	Организация";
...
Процедура ТекстЗапросаОбнулитьРегистрПрочиеРасходы(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|	ВЫБОР 
	|		КОГДА ТаблицаОстатки.СтатьяРасходов.ВариантРаспределенияРасходовУпр = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|			ТОГДА ТаблицаОстатки.СуммаУпрОстаток
	|		ИНАЧЕ 0
	|	КОНЕЦ                                      КАК СуммаБезНДС,
	|	ВЫБОР 
	...
	|			ТОГДА 0 
	|		КОГДА ТаблицаОстатки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			ТОГДА -ТаблицаОстатки.СуммаУпр 
	|		ИНАЧЕ ТаблицаОстатки.СуммаУпр 
	|	КОНЕЦ КАК СуммаБезНДС,
	|	ВЫБОР 
	|		КОГДА ТаблицаОстатки.СтатьяРасходов.ВариантРаспределенияРасходовРегл <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|			ТОГДА 0 
...
Процедура ТекстЗапросаОбнулитьРегистрПрочиеАктивыПассивы(ТекстыЗапроса, ПараметрыОбработчика)
	...
	|		И ВводОстатков.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов)
...
Процедура ПеренестиДанныеВДокументыВводаОстатков(РезультатЗапроса, ПараметрыОбработчика, ИмяЭтапа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументОбъект <> Неопределено
			...
			И Выборка.ХозяйственнаяОперация <> Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВложенийВоВнеоборотныеАктивы 
			ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
		...
		Если ДанныеСтроки <> Неопределено Тогда
			...
			Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковОсновныхСредств
				ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПредметовЛизингаНаБалансе 
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВложенийВоВнеоборотныеАктивы
			...
			ИЛИ Выборка.ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов
		...
		ИначеЕсли Выборка.ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
...
Функция ПолучитьСуществующийДокумент(ХозяйственнаяОперация, ДанныеЗаполнения, ПараметрыОбработчика)
	...
	ИскатьТолькоПоВыбранномуАктиву = 
		ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВложенийВоВнеоборотныеАктивы 
			ИЛИ ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов;
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов
		И ПараметрыОбработчика.РежимПереносаДанных <> "Проверить" Тогда
...
Функция СоздатьНовыйДокумент(ХозяйственнаяОперация, ДанныеЗаполнения, Дата, ПараметрыОбработчика)
	...
	Если ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
...
Функция КлючевыеРеквизитыОтличаются(СтарыеРеквизиты, НовыеРеквизиты, КлючевыеРеквизиты)
	...
	ИначеЕсли НовыеРеквизиты.ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
		...
		Если СтарыеРеквизиты.ПрочиеРасходы.Количество() <> 0 Тогда
			ЗначенияТекущихРеквизитов.Аналитика = СтарыеРеквизиты.ПрочиеРасходы[0].Аналитика;
		КонецЕсли; 
...
Функция ХозяйственнаяОперацияДокумента(ДокументОбъект)
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ВводОстатков") Тогда
		Возврат ДокументОбъект.ТипОперации;
	Иначе
...
Процедура ПроверитьВложенияВоВнеоборотныеАктивы(РезультатЗапроса, ПараметрыОбработчика, ИмяЭтапа)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
		...
		Иначе
			...
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма БУ'"), Выборка.СуммаРегл, Выборка.СуммаРегл24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма ПР'"), Выборка.СуммаПР, Выборка.СуммаПР24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма ВР'"), Выборка.СуммаВР, Выборка.СуммаВР24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма с НДС (упр)'"), Выборка.Сумма, Выборка.Сумма24, СписокСтрок);
			ПроверитьСуммуВложений(НСтр("ru = 'Сумма без НДС (упр)'"), Выборка.СуммаБезНДС, Выборка.СуммаБезНДС24, СписокСтрок);
...
Процедура ПроверитьСуммуВложений(ПредставлениеПоля, Сумма22, Сумма24, СписокСтрок)

	Если Сумма22 <> Сумма24 Тогда
		ТекстСтроки = СтрШаблон(НСтр("ru = '%1: %2 - %3'"), 
								ПредставлениеПоля,
								Формат(Сумма22, "ЧН=0"), 
								Формат(Сумма24, "ЧН=0"));
		СписокСтрок.Добавить(ТекстСтроки);
...
Процедура ПроверитьКорректностьИсходныхДанных(ИсходныеДанные, ПараметрыОбработчика, ИмяЭтапа)
	...
	Если ИсходныеДанные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковОсновныхСредств
		...
		ИЛИ ИсходныеДанные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПредметовЛизингаНаБалансе 
		...
		Если ИсходныеДанные.Местонахождение <> ИсходныеДанные.МестонахождениеУУ
			...
			ТекстПроблемы = НСтр("ru = 'Отличается местонахождение по бухгалтерскому учету (%1) от местонахождения по международному учету (%2).
                                  |На момент перехода местонахождение не должно отличаться.
		...
		Если ИсходныеДанные.СуммаКапитальныхВложенийВключаемыхВРасходыНУОстаток <> 0
			И НЕ ИсходныеДанные.ВключитьАмортизационнуюПремиюВСоставРасходов Тогда
			
			ТекстПроблемы = НСтр("ru = 'Обнаружен остаток амортизационной премии %1 %2 на счете КВ, который должен быть включен в расходы до перехода.
                                  |Рекомендуется исправить данные учета и повторно перенести данные.'");
			
			ТекстПроблемы = СтрШаблон(ТекстПроблемы, 
									ИсходныеДанные.СуммаКапитальныхВложенийВключаемыхВРасходыНУОстаток, 
	...
	ИначеЕсли ИсходныеДанные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковАрендованныхОС
	...
	Для каждого КлючИЗначение Из СтруктураРеквизитов Цикл
		...
		Если СтрНачинаетсяС(КлючИЗначение.Ключ, "Удалить")
			ИЛИ (КлючИЗначение.Ключ = "НалогообложениеНДС" 
				...
				И НЕ ПараметрыОбработчика.ИспользоватьРаздельныйУчетПоНалогообложению Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗарегистрироватьПроблемыПриЗаписиДокумента(ДокументОбъект, ИзмененныеСтроки, ТекстОшибкиЗаписи, ПараметрыОбработчика)
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
...
Функция ПараметрыОбработчика(Параметры, АдресХранилища)
	...
	ПараметрыОбработчика.Вставить("ВалютаУпр", Константы.ВалютаУправленческогоУчета.Получить());
		
	// Параметры переноса данных международного учета
	ПараметрыОбработчика.Вставить("ИспользоватьДанныеМеждународногоУчета", Ложь);
	ПараметрыОбработчика.Вставить("КоэффициентПересчетаИзВалютыВВалюту", 1);
	//++ НЕ УТКА
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДокументыВнеоборотныхАктивовМеждународныйУчет") 
		И Константы.УчетВФункциональнойВалюте.Получить() = Перечисления.ВидыУчетаВФункциональнойВалюте.ВВалютеУпр Тогда
		...
		КоэффициентПересчетаИзВалютыВВалюту = РаботаСКурсамиВалютУТ.ПолучитьКоэффициентПересчетаИзВалютыВВалюту(
												...
												Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить());
		ПараметрыОбработчика.Вставить("КоэффициентПересчетаИзВалютыВВалюту", КоэффициентПересчетаИзВалютыВВалюту);
...
Функция НовыйЗапрос(ПараметрыОбработчика, ТекстЗапроса = Неопределено)
	...
	Запрос.УстановитьПараметр("КоэффициентПересчетаИзВалютыВВалюту", ПараметрыОбработчика.КоэффициентПересчетаИзВалютыВВалюту);
...
Функция ДобавитьОписаниеНовойПроверки(Наименование, Идентификатор, ДетализацияДоОрганизации, ТаблицаПроверок) 
	...
	ОписаниеПроверки.КонтекстВыполнения = Документы.ВводОстатковВнеоборотныхАктивов2_4.ПустаяСсылка();
	...
	ОписаниеПроверки.Важность = Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.Ошибка;
...
Функция ПараметрыДокумента(ХозяйственнаяОперация)
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковНМАиРасходовНаНИОКР Тогда
		...
		КлючевыеРеквизиты = 
			"ХозяйственнаяОперация,Организация,
			|Местонахождение,ОтражатьВРеглУчете,ОтражатьВУпрУчете";
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПрочихАктивовПассивов Тогда
		...
		КлючевыеРеквизиты = 
			"ТипОперации,Организация,Подразделение,НаправлениеДеятельности,
