...
Процедура СоздатьВТКоэффициентыНормированияПоВремени(ИмяВТБазовыеЗаписиПериоды, ВедущиеЗаписиКаскадноНормируемыхНачислений = Неопределено, МенеджерДанныхУчетаВремени = Неопределено, ИмяВТРезультат = "ВТКоэффициентаНормированияПоВремени")
	...
	ОписаниеНаборовВидовВремени.ПолеУчитыватьРабочееВремяДляОтклонений = "Ссылка.СпособВыполненияНачисления = ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ПоОтдельномуДокументуДоОкончательногоРасчета)";
	...
	|						ТОГДА ВремяПоБазовымЗаписям.Часов
	|					ИНАЧЕ ВремяПоБазовымЗаписям.Дней
	|				КОНЕЦ / БазовыеЗаписиВремяИтог.Время КАК ЧИСЛО(31, 8))
...
Процедура РазбитьЗаписиНормируемыеКаскадно(ЗаписиНормируемыеКаскадно)
	...
	Пока ИндексСледущейОбрабатываемойСтроки <= ИндексПоследнейОбрабатываемойСтроки Цикл
		...
		Если ТекущаяСтрока.ПериодДействияНачало < ТекущаяСтрока.БазовыйПериодОсновнойЗаписиНачало 
			И ТекущаяСтрока.ПериодДействияКонец > ТекущаяСтрока.БазовыйПериодОсновнойЗаписиНачало Тогда
		...
		КонецЕсли;
		Если ТекущаяСтрока.ПериодДействияКонец > ТекущаяСтрока.БазовыйПериодОсновнойЗаписиКонец
			И ТекущаяСтрока.ПериодДействияНачало < ТекущаяСтрока.БазовыйПериодОсновнойЗаписиКонец Тогда
