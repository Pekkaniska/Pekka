...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Справочник.РесурсныеСпецификации.Форма.ПодборМатериаловПоСпецификации" Тогда
		...
		ЗагрузитьРезультатПодбораПоСпецификации(ВыбранноеЗначение, "ОбеспечениеМатериаламиИРаботами");
...
Процедура ЭтапыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ОбщегоНазначенияУТКлиент.КешироватьТекущуюСтроку(ЭтотОбъект, "Этапы");
	
	Если НоваяСтрока Тогда
		
		НоваяСтрока = Элементы.Этапы.ТекущиеДанные;
		НоваяСтрока.ЭтоНовый = Истина;
		НоваяСтрока.Ссылка = ПолучитьСсылкуНовогоДокумента();
		ПриДобавленииНовогоДокумента(НоваяСтрока.Ссылка);
...
Процедура ЭтапыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если (ЭтапыКешТекущейСтроки.Подразделение <> ТекущиеДанные.Подразделение
			ИЛИ ЭтапыКешТекущейСтроки.НаименованиеРедактирование <> ТекущиеДанные.НаименованиеРедактирование
...
Процедура ВыходныеИзделияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ВыходныеИзделияСчетРеглУчетаАктивовПассивов
		И ПроверитьЗаполнениеЭтапаВСтроке(ТекущиеДанные) Тогда
		СтандартнаяОбработка = Ложь;
		ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассиваСтрокиТЧ("ВыходныеИзделия", ВыбраннаяСтрока);
	КонецЕсли;
...
Процедура ПобочныеИзделияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ПобочныеИзделияСчетРеглУчетаАктивовПассивов
		И ПроверитьЗаполнениеЭтапаВСтроке(ТекущиеДанные) Тогда
		ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассиваСтрокиТЧ("ПобочныеИзделия", ВыбраннаяСтрока);
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ПобочныеИзделияУпаковкаПриИзменении(Элемент)
	...
	Иначе
		СтруктураДействий.Вставить("ПроверитьЗаполнитьЦенуПобочногоВыпуска", ПараметрыЗаполненияЦенПобочногоВыпуска(ОбъектЭтап));
	КонецЕсли;
...
Процедура ЭкономияМатериаловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ЭкономияМатериаловСчетРеглУчетаАктивовПассивов
		И ПроверитьЗаполнениеЭтапаВСтроке(ТекущиеДанные) Тогда
		ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассиваСтрокиТЧ("ЭкономияМатериалов", ВыбраннаяСтрока);
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ПобочныеИзделия_ЗаполнитьЦены(Команда)
	...
	ВыделенныеСтроки = ПолучитьВыделенныеВСпискеСтроки("ПобочныеИзделия");
	...
	ЦеныРассчитаны = ЗаполнитьЦеныВыделенныхСтрокПоВидуЦенСервер(ВидЦены, Валюта, Идентификаторы);
...
Процедура РасходМатериаловИРабот_ЗаполнитьСтатьюКалькуляции(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("РасходМатериаловИРабот_ЗаполнитьСтатьюКалькуляцииЗавершение", ЭтотОбъект, ВыделенныеСтроки);
	ОткрытьФорму("Справочник.СтатьиКалькуляции.ФормаВыбора", ПараметрыФормы,,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура РасходМатериаловИРабот_ЗаполнитьСтатьюКалькуляцииЗавершение(РезультатВыбора, ВыделенныеСтроки) Экспорт
	...
	Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		ДанныеСтроки.СтатьяКалькуляции = РезультатВыбора;
	КонецЦикла;
...
Процедура Трудозатраты_ЗаполнитьСтатьюКалькуляцииВВыделенныхСтроках(Команда)
	...
	Отбор = Новый Структура("ТипЗатрат", ПредопределенноеЗначение("Перечисление.ТипыЗатрат.ОплатаТруда"));
	ПараметрыФормы = Новый Структура("Отбор", Отбор);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("Трудозатраты_ЗаполнитьСтатьюКалькуляцииВВыделенныхСтрокахЗавершение", ЭтотОбъект, Новый Структура("Идентификаторы", ВыделенныеСтроки));
	ОткрытьФорму("Справочник.СтатьиКалькуляции.ФормаВыбора", ПараметрыФормы, ЭтаФорма,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура Трудозатраты_ЗаполнитьСтатьюКалькуляцииВВыделенныхСтрокахЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		Для каждого ИдентификаторСтроки Из ДополнительныеПараметры.Идентификаторы Цикл
			...
			ДанныеСтроки = Объект.Трудозатраты.НайтиПоИдентификатору(ИдентификаторСтроки);
			ДанныеСтроки.СтатьяКалькуляции = ВыбранноеЗначение;
...
Процедура ПроверитьПроизводственныйПроцесс(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Для каждого Строка Из Строки Цикл
		...
		Если НЕ ЗначениеЗаполнено(Строка.НомерЭтапа) Тогда
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение", НСтр("ru = 'Номер этапа'"), Строка.НомерСтроки, ИмяСписка);
		...
		Если НЕ ЗначениеЗаполнено(Строка.НаименованиеРедактирование) Тогда
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение", НСтр("ru = 'Наименование'"), Строка.НомерСтроки, ИмяСписка);
		...
		Если НЕ ЗначениеЗаполнено(Строка.Подразделение) Тогда
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение", НСтр("ru = 'Подразделение'"), Строка.НомерСтроки, ИмяСписка);
		...
		Если Строка.ИспользоватьПооперационноеУправление Тогда
			...
			Если Не ЗначениеЗаполнено(Строка.КоэффициентМаршрутнойКарты)
				...
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение", НСтр("ru = 'Коэффициент'"), Строка.НомерСтроки, ИмяСписка);
...
Процедура ПроверитьПоследовательностьЭтапов(Отказ)
	...
	Если Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Ремонт
...
Процедура ПроверитьВыходныеИзделия(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Для каждого Строка Из Объект.ВыходныеИзделия Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.Ссылка) Тогда
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение", НСтр("ru = 'Этап производства'"), Строка.НомерСтроки, ИмяСписка);
		...
		Если ЗначениеЗаполнено(Строка.Ссылка) И ВыпускающиеЭтапы.Найти(Строка.Ссылка) = Неопределено Тогда
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Корректность", НСтр("ru = 'Этап производства'"), Строка.НомерСтроки, ИмяСписка);
...
Процедура ПроверитьДолиСтоимостиВыходныхИзделий(Отказ)
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение", НСтр("ru = 'Доля стоимости'"), Выборка.НомерСтроки, ИмяСписка);
...
Функция ВыгрузитьДанныеДляПереносаВДокументы()
	...
	Для каждого ИмяРеквизита Из СписокРеквизитов Цикл
		Реквизиты.Вставить(ИмяРеквизита, Объект[ИмяРеквизита]);
	КонецЦикла;
	...
	СписокТЧ = СписокВложенныхТабличныхЧастей();
	Для каждого ИмяТЧ Из СписокТЧ Цикл
		ТабличнаяЧасть = Объект[ИмяТЧ].Выгрузить();
...
Функция ЗначенияЗаполненияПоЭтапуПроизводства(Строка, ДанныеОбработки)
	...
	Результат.Вставить("СпособЗаполнения", "РедактированиеЭтаповПроизводства");
...
Процедура СтатьяРасходовПриИзмененииСервер(ИмяТЧ, КэшированныеЗначения)
	...
	ОбъектЭтап = ОбъектЭтапПоСсылке(ЭтаФорма, СтрокаТабличнойЧасти.Ссылка);
	УправлениеПроизводством.СтатьяРасходовПриИзменении(СтрокаТабличнойЧасти, ОбъектЭтап, КэшированныеЗначения);
...
&НаКлиенте
Процедура ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассиваСтрокиТЧ(ИмяТЧ, ВыбраннаяСтрока)
	
	ТекущиеДанные = Элементы[ИмяТЧ].ТекущиеДанные;
	ОбъектЭтап = ОбъектЭтапПоСсылке(ЭтаФорма, ТекущиеДанные.Ссылка);
	
	ПараметрыНастройки = Новый Структура;
	ПараметрыНастройки.Вставить("СоответствиеИменРеквизитов", Новый Соответствие);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НастройкаОтраженияВРеглУчете", РеглУчетКлиент.НастройкаОтраженияВРеглУчете(ТекущиеДанные, ПараметрыНастройки.СоответствиеИменРеквизитов));
	ПараметрыФормы.НастройкаОтраженияВРеглУчете.Организация = ОбъектЭтап.Организация;
	
	РеглУчетКлиент.ОткрытьНастройкуОтраженияВРеглУчетеАктиваПассиваСтрокиТЧ(ЭтаФорма, ИмяТЧ, ВыбраннаяСтрока, ПараметрыФормы);
...
Процедура ЗаполнитьРасходМатериаловИРаботПоДаннымОбеспеченияНаСервере()
	...
	КонецЦикла;
	Обработки.РедактированиеПоследовательностиЭтаповПроизводства.ЗаполнитьРасходМатериаловИРаботПоДаннымОбеспечения(Объект, МассивСсылок);
...
&НаСервере
Функция ПолучитьНормативыПоСпецификации(ПереченьДанных, ПолучатьМатериалыПоТребованию)
...
Функция ПолучитьНормативыИзВременногоХранилища()
	...
	Если ДанныеСпецификации.Свойство("МатериалыИУслуги") Тогда
		Таблица = ДанныеСпецификации.МатериалыИУслуги.Скопировать(, "Этап,Номенклатура,Характеристика,Количество");
		Таблица.Свернуть("Этап,Номенклатура,Характеристика", "Количество");
		Результат.Вставить("ОбеспечениеМатериаламиИРаботами", Таблица);
	КонецЕсли;
	
	Если ДанныеСпецификации.Свойство("Трудозатраты") Тогда
		Таблица = ДанныеСпецификации.Трудозатраты.Скопировать(, "Этап,ВидРабот,Количество");
...
Процедура ЗаполнитьТрудозатратыПоСпецификацииНаСервере()
	...
	Если ДанныеСпецификации.Свойство("Трудозатраты") Тогда
		...
		Для каждого Строка Из ДанныеСпецификации.Трудозатраты Цикл
			...
			ОбъектЭтап = ОбъектЭтапПоСсылке(ЭтаФорма, СоответствиеЭтапов[Строка.Этап]);
			Если Не ОбъектЭтап.ТолькоПросмотр И Не ОбъектЭтап.ПроизводствоНаСтороне Тогда
				НоваяСтрока.Ссылка = ОбъектЭтап.Ссылка;
			КонецЕсли;
...
Функция ПараметрыВыбораАналогов(Форма, ТекущиеДанные = Неопределено)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ПараметрыФормы.Подразделение = ТекущиеДанные.Подразделение;
	Иначе
		ПараметрыФормы.Подразделение = ПредопределенноеЗначение("Справочник.СтруктураПредприятия.ПустаяСсылка");
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеТрудозатраты()
	...
	УправлениеПроизводством.УстановитьУсловноеОформлениеТрудозатраты(ЭтаФорма, "Объект", "Трудозатраты");
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ТипПроизводственногоПроцесса")
		...
		Если Не ЗначениеЗаполнено(ПартияПроизводства) Тогда
			ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Создание этапов производства (%1)'"), НРег(ТипПроизводственногоПроцесса));
		Иначе
			ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Редактирование этапов производства (%1)'"), НРег(ТипПроизводственногоПроцесса));
		КонецЕсли;
	...
	Если СтруктураРеквизитов.Свойство("ТипПроизводственногоПроцесса")
		...
		Если ЗначениеЗаполнено(Объект.ОсновноеИзделиеНоменклатура) Тогда
			Если (ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Сборка")
				...
				Форма.ИнформационнаяНадписьОсновноеИзделие = Новый ФорматированнаяСтрока(НСтр("ru = 'перейти к выпуску'"),,,, "ПерейтиКВыпуску");
			ИначеЕсли ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка") Тогда
				...
				Форма.ИнформационнаяНадписьОсновноеИзделие = Новый ФорматированнаяСтрока(НСтр("ru = 'перейти к обеспечению'"),,,, "ПерейтиКОбеспечению");
			КонецЕсли;
		КонецЕсли;
	...
	Если СтруктураРеквизитов.Свойство("ИспользоватьПооперационноеУправление")
		...
		Элементы.ЭтапыГруппаМаршрутнаяКарта.Видимость = Форма.ИспользоватьПооперационноеУправление;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ОбщегоНазначенияУТ.ИнициализироватьКешТекущейСтроки(ЭтотОбъект, "Этапы");
...
&НаКлиенте
Функция ПроверитьЗаполнениеЭтапаВСтроке(ТекущиеДанные)

	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ПоказатьПредупреждение(, Нстр("ru = 'Команда не может быть выполнена, необходимо выбрать этап производства!'"));
	Возврат Ложь;
...
Функция СписокРеквизитов()
	...
	Список.Добавить("Распоряжение");
	Список.Добавить("НазначениеПродукция");
	Список.Добавить("ПартияПроизводства");
	
	Список.Добавить("ХозяйственнаяОперация");
	Список.Добавить("НаправлениеДеятельности");
	Список.Добавить("Партнер");
	Список.Добавить("Договор");
	
	Список.Добавить("Спецификация");
	Список.Добавить("ТипПроизводственногоПроцесса");
...
Функция ПоместитьМатериалыДляПодбораПоСпецификацииВХранилище()
	...
	Таблица = Объект.ОбеспечениеМатериаламиИРаботами.Выгрузить(, "Этап,Номенклатура,Характеристика, Количество");
...
&НаСервере
Процедура ЗагрузитьРезультатПодбораПоСпецификации(АдресВХранилище, ИмяТЧ)
	...
	ТаблицаРезультатПодбора = ПолучитьИзВременногоХранилища(АдресВХранилище);
	
	Для каждого Строка Из ТаблицаРезультатПодбора Цикл
...
&НаСервере
Функция ЗаполнитьЦеныВыделенныхСтрокПоВидуЦенСервер(ВидЦены, Валюта, Идентификаторы)
...
&НаКлиентеНаСервереБезКонтекста
Функция ПараметрыЗаполненияЦенПобочногоВыпуска(ОбъектЭтап)
...
Процедура ОбработкаСтрокиПриНачалеРедактирования(ИмяТЧ, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		КонецЕсли;
		Если Копирование И ТекущиеДанные.ТолькоПросмотр Тогда
