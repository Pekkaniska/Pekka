...
Процедура Пункт3_1НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.уатПечатьТТН1208.Форма.ФормаВыборГруппыТоваров",,Элемент,,,, Новый ОписаниеОповещения("Пункт3_1НачалоВыбораЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
Функция ПолучитьДанныеДляТТНПриложение4(ЗапросТовары, Шапка)
	...
	Для Каждого ТекСтр Из Объект.Документ.Выработка Цикл
		Если ЗначениеЗаполнено(ТекСтр.ПутЛист) Тогда
			...
			Если НЕ ДанныеДокумента.Свойство("Пункт10_3") Тогда
				ДанныеДокумента.Вставить("Пункт10_3", уатОбщегоНазначения.уатФИОСотрудникаПолноеСтрокой(ТекСтр.ПутЛист.Водитель1, Объект.Документ.Дата));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Иначе
		...
		СтоимостьВРублях =  уатОбщегоНазначенияТиповые.ПересчитатьИзВалютыВВалюту(Объект.Документ.Стоимость.Итог("Сумма"), ,
...
Функция ПечатьТТНПриложение4НаСервере(ТабДокумент)
	...
	МакетТТНПриложение4 = УправлениеПечатью.МакетПечатнойФормы("Обработка.уатПечатьТТН1208.ПФ_MXL_ТТНПриложение4");
