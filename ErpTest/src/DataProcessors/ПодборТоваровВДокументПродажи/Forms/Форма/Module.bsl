...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//пкМодификацияСервер.ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка);	
	Форма = ЭтаФорма;
...
//++ Рарус Лимаренко 11.01.2018
	ДокументПеремещение=Ложь;	
	Попытка
		ДокументПеремещение = ТипЗнч(Форма.Параметры.Документ)=Тип("ДокументСсылка.ПеремещениеТоваров");		
	Исключение	
	КонецПопытки;
//-- Рарус Лимаренко 11.01.2018
		
	ДобавляемыеРеквизиты = Новый Массив;
    ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("пкСкладскаяЯчейка", Новый ОписаниеТипов("СправочникСсылка.СкладскиеЯчейки")));
    ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("пкДанныеПоЯчейкам", Новый ОписаниеТипов("ТаблицаЗначений")));
    ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ТекСкладФормы", Новый ОписаниеТипов("СправочникСсылка.Склады")));
	...
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ВНаличииОстаток", Новый ОписаниеТипов("Число"), "пкДанныеПоЯчейкам", "В наличии остаток", ложь));
    ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ТолькоВНаличииПоЯчейкеПользователя", Новый ОписаниеТипов("Булево")));
	Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
	
	Если ДокументЗаказНаряд ИЛИ ДокументПеремещение Тогда 
		
		ЭтаФорма.пкСкладскаяЯчейка=ПолучитьЗначениеРеквизита(ПользователиКлиентСервер.ТекущийПользователь(), "пкСкладскаяЯчейка");
		...
		НоваяКолонкаДинамическогоСписка.ПутьКДанным = "пкДанныеПоЯчейкам.Ячейка";
	 	...
	 	НоваяКолонкаДинамическогоСписка             = Элементы.Добавить("ВНаличииОстаток", Тип("ПолеФормы"), Элементы.пкДанныеПоЯчейкам);
		НоваяКолонкаДинамическогоСписка.Вид         = ВидПоляФормы.ПолеВвода;   
		НоваяКолонкаДинамическогоСписка.ПутьКДанным = "пкДанныеПоЯчейкам.ВНаличииОстаток";
		
		НовыйГруппаЯчейкаПользователя  = Элементы.Добавить("ГруппаЯчейкаПользователя", Тип("ГруппаФормы"), Элементы.СтраницаФильтры);
		НовыйГруппаЯчейкаПользователя.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		НовыйГруппаЯчейкаПользователя.ОтображатьЗаголовок = Ложь;
		НовыйГруппаЯчейкаПользователя.Группировка   = ГруппировкаПодчиненныхЭлементовФормы.Горизонтальная;
		
		НовыйФлажок  = Элементы.Добавить("ТолькоВНаличииПоЯчейкеПользователя", Тип("ПолеФормы"), Элементы.ГруппаЯчейкаПользователя);
		НовыйФлажок.Вид         = ВидПоляФормы.ПолеФлажка;   
		НовыйФлажок.ПутьКДанным = "ТолькоВНаличииПоЯчейкеПользователя";
		НовыйФлажок.Заголовок 	= "Только по ячейке "+Строка(ЭтаФорма.пкСкладскаяЯчейка);
		НовыйФлажок.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
   		НовыйФлажок.УстановитьДействие("ПриИзменении", "ТолькоВНаличииПоЯчейкеПользователя_ПриИзменении"); 
		
		НовыйПолеЯчейка  = Элементы.Добавить("ЯчейкаОтбора", Тип("ПолеФормы"), Элементы.ГруппаЯчейкаПользователя);
		НовыйПолеЯчейка.Вид         = ВидПоляФормы.ПолеВвода;   
		НовыйПолеЯчейка.ПутьКДанным = "пкСкладскаяЯчейка";
		НовыйПолеЯчейка.Заголовок 	= "";
		НовыйПолеЯчейка.Видимость = Не ЗначениеЗаполнено(ЭтаФорма.пкСкладскаяЯчейка);
		НовыйПолеЯчейка.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
   		НовыйПолеЯчейка.УстановитьДействие("ПриИзменении", "ТолькоВНаличииПоЯчейкеПользователя_ПриИзменении"); 
...
//++ Рарус IA 2017.03.15
	Попытка
	ЭтаФорма.Элементы.пкДанныеПоЯчейкам.Видимость=ЭтаФорма.Элементы.ОстаткиТоваров.Видимость;
	Исключение
	КонецПопытки;
//-- Рарус IA 2017.03.15
...
//++ Рарус IA 2017.03.15
	Если ЭтаФорма.ТекущаяСтрокаХарактеристик <> неопределено и Элемент.ТекущиеДанные<>Неопределено Тогда
		Если ЭтаФорма.ТекущаяСтрокаХарактеристик.Характеристика <> Элемент.ТекущиеДанные.Ссылка Тогда
		ЗаполнитьОстаткиЯчеек(Элемент.ТекущиеДанные.Ссылка);
		КонецЕсли;
	КонецЕсли;
//-- Рарус IA 2017.03.15
...
//++ Рарус IA 2017.03.15
	Если ЭтаФорма.ТекущаяСтрокаНоменклатуры <> неопределено и Элемент.ТекущиеДанные<>Неопределено Тогда
		Если ЭтаФорма.ТекущаяСтрокаНоменклатуры.Номенклатура <> Элемент.ТекущиеДанные.Ссылка Тогда
		ЗаполнитьОстаткиЯчеек();
		КонецЕсли;
	КонецЕсли;
//-- Рарус IA 2017.03.15
...
Процедура ОстаткиТоваровПриАктивизацииСтроки(Элемент)
	ДокументЗаказНаряд=Ложь;	
	Попытка
		ДокументЗаказНаряд = ТипЗнч(ЭтаФорма.Параметры.Документ)=Тип("ДокументСсылка.пкЗаказНаряд");		
	Исключение	
	КонецПопытки;	
	...
	ТекСклад=?(Элемент.ТекущиеДанные=Неопределено,Неопределено,Элемент.ТекущиеДанные.Склад); 
	ТекСкладФормыСохраним=ЭтаФорма.ТекСкладФормы;	
	ЭтаФорма.ТекСкладФормы=ТекСклад;
	Если ЭтаФорма.ТекСкладФормы <> ТекСкладФормыСохраним Тогда
		ЗаполнитьОстаткиЯчеек();
	КонецЕсли
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьОстаткиЯчеек(ТекХарактеристика=Неопределено);
	...
	ИмяСпискаНоменклатуры=ПодборТоваровКлиентСервер.ИмяСпискаНоменклатурыПоВариантуПоиска(ЭтаФорма);
	ТекНоменклатура = ?(ЭтаФорма.Элементы[ИмяСпискаНоменклатуры].ТекущиеДанные=Неопределено, Неопределено, ЭтаФорма.Элементы[ИмяСпискаНоменклатуры].ТекущиеДанные.Ссылка);
	Если ТекНоменклатура=Неопределено и ЭтаФорма.ТекущаяСтрокаНоменклатуры<>Неопределено Тогда 
	ТекНоменклатура = ЭтаФорма.ТекущаяСтрокаНоменклатуры.Номенклатура;
	КонецЕсли;
	ТекСклад = ?(ЭтаФорма.Элементы.ОстаткиТоваров.ТекущиеДанные=Неопределено, ЭтаФорма.Склады, ЭтаФорма.Элементы.ОстаткиТоваров.ТекущиеДанные.Склад);	
	ЗаполнитьОстаткиЯчеекСервер(ТекНоменклатура,ТекСклад,ТекХарактеристика,ЭтаФорма.пкСкладскаяЯчейка,ЭтаФорма.ТолькоВНаличииПоЯчейкеПользователя);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьОстаткиЯчеекСервер(ТекНоменклатура,ТекСклад,ТекХарактеристика=Неопределено, ТекпкСкладскаяЯчейка=неопределено, текТолькоВНаличииПоЯчейкеПользователя=Ложь);
		...
		|				И Ячейка.Владелец В (&Склад) и (&ТолькоВНаличииПоЯчейкеПользователя=Ложь или Ячейка=&Ячейка)
		|				И (&Характеристика = НЕОПРЕДЕЛЕНО
		|					ИЛИ Характеристика = &Характеристика)) КАК ТоварыВЯчейкахОстатки";
		ЗапросЯчеек.УстановитьПараметр("Номенклатура", ТекНоменклатура);
		ЗапросЯчеек.УстановитьПараметр("Характеристика", ТекХарактеристика);
		ЗапросЯчеек.УстановитьПараметр("Склад", ТекСклад);
		ЗапросЯчеек.УстановитьПараметр("Ячейка", ТекпкСкладскаяЯчейка);
		ЗапросЯчеек.УстановитьПараметр("ТолькоВНаличииПоЯчейкеПользователя", текТолькоВНаличииПоЯчейкеПользователя);
		ЗапросЯчеекРезультат=ЗапросЯчеек.Выполнить().Выгрузить();
		ЗначениеВДанныеФормы(ЗапросЯчеекРезультат,ЭтаФорма.пкДанныеПоЯчейкам); 
КонецПроцедуры
...
&НаКлиенте
Процедура ТолькоВНаличииПоЯчейкеПользователя_ПриИзменении(Элемент)
	
	Если Элемент.Имя="ЯчейкаОтбора" и ЗначениеЗаполнено(ЭтаФорма.пкСкладскаяЯчейка) Тогда
		ЭтаФорма.ТолькоВНаличииПоЯчейкеПользователя = Истина;
	КонецЕсли; 
	
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(ЭтаФорма.СписокНоменклатура, "ВключитьОтборВЯчейкеСвободныеОстаткиОстатки", ЭтаФорма.ТолькоВНаличииПоЯчейкеПользователя);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(ЭтаФорма.СписокНоменклатура, "ЯчейкаОтбора", ЭтаФорма.пкСкладскаяЯчейка);
	Если ИспользоватьХарактеристикиНоменклатуры Тогда
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(ЭтаФорма.СписокХарактеристики, "ВключитьОтборВЯчейкеСвободныеОстаткиОстатки", ЭтаФорма.ТолькоВНаличииПоЯчейкеПользователя);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(ЭтаФорма.СписокХарактеристики, "ЯчейкаОтбора", ЭтаФорма.пкСкладскаяЯчейка);
	КонецЕсли;
	ТолькоВНаличииПоЯчейкеПользователяПриИзмененииНаСервере();
	
	Если ЭтаФорма.ТекущаяСтрокаХарактеристик <> неопределено Тогда
		ЗаполнитьОстаткиЯчеек(ЭтаФорма.ТекущаяСтрокаХарактеристик.Характеристика);
	Иначе
		ЗаполнитьОстаткиЯчеек();
	КонецЕсли;
...
&НаКлиенте
Процедура ЯчейкеПользователя_ПриИзменении(Элемент)
	
	
	ТолькоВНаличииПоЯчейкеПользователя_ПриИзменении(Элементы.ТолькоВНаличииПоЯчейкеПользователя);
	
КонецПроцедуры

&НаСервере
Процедура ТолькоВНаличииПоЯчейкеПользователяПриИзмененииНаСервере()
	
	ПодборТоваровСервер.УстановитьОтборТолькоВНаличииПоЯчейкеПользователя(ЭтаФорма);
