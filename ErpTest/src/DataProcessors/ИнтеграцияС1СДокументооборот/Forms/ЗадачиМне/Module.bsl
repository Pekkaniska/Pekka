...
//++ Рарус Лимаренко 07.03.2018
	Если Не Поле.Имя = "ЗадачиКомментарий" Тогда
//-- Рарус Лимаренко 07.03.2018
...
Процедура СгруппироватьПоКолонкеНаСервере()
	...
	Если ЗначениеЗаполнено(РежимГруппировки) Тогда
		...
		Для каждого СтрокаГруппировки из ТаблицаГруппировок Цикл
			...
			КонецЦикла;
//++ Рарус Лимаренко 25.07.2018 Задача №11205
			//СтрокаДерева.Строки.Сортировать("СрокИсполнения УБЫВ, Задача");
			Если ЭтаФорма.СортировкаПоСрокам  Тогда
			     СтрокаДерева.Строки.Сортировать("Записана УБЫВ, Задача");
			Иначе	
			     СтрокаДерева.Строки.Сортировать("СрокИсполнения УБЫВ, Задача");
			КонецЕсли;		
//-- Рарус Лимаренко 25.07.2018 Задача №11205
...
//++ Рарус Лимаренко 25.07.2018 Задача №11205
			//СтрокаДерева.Строки.Сортировать("СрокИсполнения УБЫВ, Задача");
			Если ЭтаФорма.СортировкаПоСрокам  Тогда
			     Дерево.Строки.Сортировать("Записана УБЫВ, Задача");
			Иначе	
			     Дерево.Строки.Сортировать("СрокИсполнения УБЫВ, Задача");
			КонецЕсли;		
//-- Рарус Лимаренко 25.07.2018 Задача №11205
...
//++ Рарус Лимаренко 11.03.2018
	Если НЕ ЗадачаXDTO.parentBusinessProcess.objectId.type = "DMBusinessProcessIssuesSolution" Тогда
	...
	Иначе
		...
		Ответ = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(Прокси, ЗадачаXDTO.parentBusinessProcess.objectId.type, ЗадачаXDTO.parentBusinessProcess.objectId.id, Условия);
		ЗаполнитьОбъектныйРеквизит(СтрокаЗадачи,Ответ.objects[0].issueTask,"Процесс");
	КонецЕсли;	   
//-- Рарус Лимаренко 11.03.2018
...
//++ Рарус Лимаренко 07.03.2018
&НаКлиенте
Процедура ЗадачиКомментарийОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка) //++ Рарус Лимаренко 07.03.2018

	
	СтрокаЗадачи = Элементы.Задачи.ТекущиеДанные;
	ПредметСсылка = Неопределено;

	ЗаписатьКомментарий(Текст,СтрокаЗадачи.ЗадачаТип,СтрокаЗадачи.ЗадачаID);
...
&НаСервере
Процедура ЗаписатьКомментарий(Текст,Тип, Идентификатор) //++ Рарус Лимаренко 07.03.2018
	...
	Задача.executionComment = Текст;
	
	ИнтеграцияС1СДокументооборот.ЗаписатьОбъект(Прокси, Задача);
...
//++ Рарус Лимаренко 25.07.2018 Задача №11205
&НаКлиенте
Процедура СортировкаПоСрокамПриИзменении(Элемент) //Рарус Лимаренко 25.07.2018 Задача №11205
	ОбновитьСписокЗадачЧастично();		
КонецПроцедуры
//-- Рарус Лимаренко 25.07.2018 Задача №11205
