...
Процедура СформироватьОтчет()
	...
	ИначеЕсли ТипЗнч(АктОРасхождениях) = Тип("ДокументСсылка.АктОРасхожденияхПослеПриемки") Тогда
		...
		ИначеЕсли ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПриемкаТоваровНаХранение Тогда
			...
			ПраваДоступаНаТаблицы.Вставить("ЧтениеСписаниеПринятыхНаХранениеТоваров",
				ПравоДоступа("Чтение", Метаданные.Документы.СписаниеПринятыхНаХранениеТоваров));
...
Процедура ВывестиСтрокиДействийПоАктуПослеПриемки(СтрокаДействия, Макет, ТипОснованияАктаОРасхождении, ПраваДоступаНаТаблицы)
	...
	Иначе
		...
		Если ПраваДоступаНаТаблицы.ЧтениеСписаниеПринятыхНаХранениеТоваров Тогда
			...
			СформироватьДокументыСписанияПринятыхНаХранениеТоваров(СтрокаДействия, Макет, Описание);
...
Функция ТекстШапки(ТипДокумента, Описание , ХозяйственнаяОперация = Неопределено)
	...
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХранения" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Отгрузка товаров с хранения"" для довозврата недовозращенного товара'");
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХраненияПоПриемке" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Отгрузка товаров с хранения"" для возврата перепоставленного товара'");
	ИначеЕсли ТипДокумента = "ИсправлениеОтгрузкаТоваровСХранения" Тогда
		ТекстШапки = НСтр("ru = 'Отразить расхождения в документе ""Отгрузка товаров с хранения""'");
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранениеДопоставкаПоПриемке" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Приемка товаров на хранение"" для поступления недопоставленного товара'");
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранение" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Приемка товаров на хранение"" для поступления перепоставленного товара'");
	ИначеЕсли ТипДокумента = "СписаниеПринятыхНаХранениеТоваров" Тогда
		ТекстШапки = НСтр("ru = 'Документ ""Списание принятых на хранение товаров"" для списания непризнанных недостач'");
	//-- НЕ УТ
	...
	Если ТипДокумента = "СписаниеНедостачТоваров"
		...
		Или ТипДокумента = "СписаниеПринятыхНаХранениеТоваров"
		//-- НЕ УТ
...
Функция ТекстЛегенды(ТипДокумента, ХозяйственнаяОперация = Неопределено)
	...
	//++ НЕ УТ
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХранения" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Отгрузка товаров с хранения"" по недостачам, отмеченным к допоставке поставщику. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и допоставить"".'");
	ИначеЕсли ТипДокумента = "ИсправлениеОтгрузкаТоваровСХранения" Тогда
		Возврат НСтр("ru = 'Необходимо скорректировать документ ""Отгрузка товаров с хранения"", отразив в нем количество по факту. Следует выполнить для строк акта о расхождениях с вариантами отработки: ""Оформить и вернуть"", ""Оформить"", ""Оформить и допоставить"".'");
	ИначеЕсли ТипДокумента = "ОтгрузкаТоваровСХраненияПоПриемке" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Отгрузка товаров с хранения"" по перепоставленному товару, отмеченному к возврату поставщику. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".'");
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранение" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Приемка товаров на хранение"" по излишкам, отмеченным к возврату поставщиком. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и вернуть"".'");
	ИначеЕсли ТипДокумента = "ПриемкаТоваровНаХранениеДопоставкаПоПриемке" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Приемка товаров на хранение"" по недостачам, для которых требуется допоставка. Оформляется на строки акта о расхождениях с вариантом отработки ""Оформить и ожидать допоставку"".'");
	ИначеЕсли ТипДокумента = "СписаниеПринятыхНаХранениеТоваров" Тогда
		Возврат НСтр("ru = 'Необходимо оформить документ ""Списание принятых на хранение товаров"" для списания на прочие расходы непризнанных недостач. Оформляется на строки акта о расхождениях с вариантом отработки ""По вине сторонней компании"" или ""За наш счет"".'");
	//-- НЕ УТ
	ИначеЕсли ТипДокумента = "ПриобретениеТоваровУслугДопоставкаПоПоступлению" Тогда
...
Функция ТекстЗапросаАктОРасхожденияхПослеПриемкиТоваровНаХранение(ПраваДоступаНаТаблицы, СоответствиеНаборовДанных)
	...
	Если ПраваДоступаНаТаблицы.ЧтениеСписаниеПринятыхНаХранениеТоваров Тогда
		...
		ТекстЗапроса = ТекстЗапроса + "
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СписаниеПринятыхНаХранениеТоваров.Основание     КАК Основание,
		|	СписаниеПринятыхНаХранениеТоваров.Ссылка        КАК Списание,
		|	СписаниеПринятыхНаХранениеТоваров.МестоХранения КАК Склад
		|ИЗ
		|	Действия КАК Действия
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеПринятыхНаХранениеТоваров КАК СписаниеПринятыхНаХранениеТоваров
		|		ПО (СписаниеПринятыхНаХранениеТоваров.Проведен)
		|			И СписаниеПринятыхНаХранениеТоваров.Основание = &АктОРасхождениях
		|			И СписаниеПринятыхНаХранениеТоваров.МестоХранения = Действия.Склад
		|";
...
&НаСервере
Процедура СформироватьДокументыСписанияПринятыхНаХранениеТоваров(СтрокаДействия, Макет, Описание)
	
	Если НеВыводитьВОтчет("ВывестиДокументСписаниеПринятыхНаХранениеТоваров", СтрокаДействия.Основание) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СкладСписания Из МассивСкладовСписания Цикл
		...
		ОформленныеДокументы = ОснованияСписанияНедостач.НайтиСтроки(ОтборПоСкладу);
		
		Если ОформленныеДокументы.Количество() = 0 Тогда
			СтруктураОформляемогоДокумента = Новый Структура("ОформляемыйДокумент, Склад",
				Документы.СписаниеПринятыхНаХранениеТоваров.ПустаяСсылка(), СкладСписания);
			
			МассивОформленныхДокументов.Добавить(СтруктураОформляемогоДокумента);
		Иначе
			Для Каждого ОформленныйДокумент Из ОформленныеДокументы Цикл
				МассивОформленныхДокументов.Добавить(ОформленныйДокумент.Списание);
			КонецЦикла;
		...
		Отбор = Новый Структура("ВидДокумента, Склад", "СписаниеПринятыхНаХранениеТоваров", СкладСписания);
		...
		Если ДокументыПоАкту.Количество() = 0 Тогда
			...
			НоваяСтрока.ВидДокумента         = "СписаниеПринятыхНаХранениеТоваров";
