...
Процедура НазадЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	СписокОшибок = УдалитьЗаказыНаСервере(СформированныеЗаказы.ВыгрузитьЗначения());
...
Процедура НастройкиВидимостиПриИзмененииТипаПроцесса(Форма)
	...
	Форма.Элементы.ФормированиеЗаказовКонтекстноеМенюУстановитьДату.Заголовок = ?(Форма.ЭтоРазборка,
		НСтр("ru = 'Установить дату запуска'"), НСтр("ru = 'Установить дату выпуска'"));
...
Процедура ИнициализироватьКомпоновщикНастроек()
	...
	Если ИспользоватьУправлениеПроизводством2_2 Тогда
		Если Объект.РежимРасчетаПотребностейПоПлану = 0
			...
			СхемаКомпоновкиДанных = Обработки.ФормированиеЗаказовНаПроизводствоПоПлану.ПолучитьМакет(
				"МакетКомпоновки2_2" + ?(ЭтоРазборка, "Разборка", "Сборка"));
		Иначе
			СхемаКомпоновкиДанных = Обработки.ФормированиеЗаказовНаПроизводствоПоПлану.ПолучитьМакет(
				"МакетКомпоновки2_2НарастающимИтогом" + ?(ЭтоРазборка, "Разборка", "Сборка"));
			СхемаКомпоновкиДанныхКоличествоВПланах = Обработки.ФормированиеЗаказовНаПроизводствоПоПлану.ПолучитьМакет(
				"МакетКомпоновкиКоличествоВПланах" + ?(ЭтоРазборка, "Разборка", "Сборка"));
			СхемаКомпоновкиДанныхКоличествоВЗаказах = Обработки.ФормированиеЗаказовНаПроизводствоПоПлану.ПолучитьМакет(
				"МакетКомпоновки2_2КоличествоВЗаказах" + ?(ЭтоРазборка, "Разборка", "Сборка"));
		КонецЕсли;
	Иначе
		Если Объект.РежимРасчетаПотребностейПоПлану = 0
			Или Объект.РежимРасчетаПотребностейПоПлану = 1 Тогда
...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(ЗначениеНастроек) = Тип("Структура") Тогда
		...
		Если ЗначениеНастроек.Свойство("НачалоПериода")
			Или ЗначениеНастроек.Свойство("ОкончаниеПериода") Тогда
			Объект.РежимРасчетаПотребностейПоПлану = 1;
			Объект.ПериодНачало = ЗначениеНастроек.НачалоПериода;
			Объект.ПериодОкончание = ЗначениеНастроек.ОкончаниеПериода;
		КонецЕсли;
		
		Если ЗначениеНастроек.Свойство("НачалоПериодаЗаказа")
			Или ЗначениеНастроек.Свойство("ОкончаниеПериодаЗаказа") Тогда
			Объект.РежимРасчетаПотребностейПоПлану = 0;
			Объект.ПериодНачало = ЗначениеНастроек.НачалоПериодаЗаказа;
			Объект.ПериодОкончание = ЗначениеНастроек.ОкончаниеПериодаЗаказа;
		КонецЕсли;
...
Функция УдалитьЗаказыНаСервере(СсылкиНаУдаление)
	...
	Возврат ОбщегоНазначенияУТ.УдалитьДокументы(СсылкиНаУдаление);
