...
Процедура ОжидатьЗавершениеПоиска()
	...
	ОбработкаЗавершенияПоиска = Новый ОписаниеОповещения("НайтиТорговыеПредложенияЗавершение",
		ЭтотОбъект, ФоновоеЗаданиеПоиска);
...
&НаКлиенте
Процедура ЗагрузитьСледующие(Команда)
...
Процедура НайтиТорговыеПредложения(СледующаяСтраница = Ложь, ОбновитьРегионы = Ложь)
	...
	ФоновоеЗаданиеПоиска = НайтиТорговыеПредложенияВФоне(ИнтернетПоддержкаПодключена, СледующаяСтраница, ОбновитьРегионы);
	...
	Иначе
		...
		НайтиТорговыеПредложенияПродолжение(Истина, Неопределено);
...
Процедура НайтиТорговыеПредложенияПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗаданиеПоиска.Статус = "Выполнено" Тогда
		НайтиТорговыеПредложенияЗавершение(ФоновоеЗаданиеПоиска, ФоновоеЗаданиеПоиска);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция НайтиТорговыеПредложенияВФоне(ИнтернетПоддержкаПодключена, СледующаяСтраница = Ложь, ОбновитьРегионы = Ложь)
	...
	Задание.ИмяПроцедуры = "ТорговыеПредложенияСлужебный.ПоискТорговыхПредложенияПоОтбору";
...
Процедура НайтиТорговыеПредложенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не Отказ И Результат.Статус = "Выполнено" Тогда
		Если ЗначениеЗаполнено(Результат.АдресРезультата)
			...
			И ФоновоеЗаданиеПоиска.ИдентификаторЗадания = ДополнительныеПараметры.ИдентификаторЗадания Тогда
	...
	КонецЕсли;
	Элементы.ДекорацияСостояние.Заголовок = СостояниеПоиска;
	Элементы.ЗагрузитьСледующие.Доступность = (Страницы.Страница < Страницы.КоличествоСтраниц - 1);
	
	// Позиционирование на текущей строке списка.
	Если ТекущиеДанныеСписка <> Неопределено Тогда
		СтрокиПоиска = Список.НайтиСтроки(Новый Структура("Идентификатор", ТекущиеДанныеСписка.Идентификатор));
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", "по запросу");
	...
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = ЭлементУсловногоОформления.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СписокНаличие.Имя);
	ОтборЭлемента = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
