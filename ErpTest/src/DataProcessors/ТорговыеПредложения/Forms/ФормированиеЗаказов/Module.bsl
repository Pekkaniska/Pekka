...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ТорговыеПредложения_ИзменитьКоличествоВЗаказе"
		...
		ИзменитьКоличествоВЗаказе(Параметр.Идентификатор, Количество, ЭтоРазница);
	...
	Если Не РежимЗапросаЦен Тогда
		...
		Для ТекИндекс = 0 По Товары.Количество()-1 Цикл
			...
			Если ТекущаяСтрока.Пометка
				...
				И (НЕ ЕстьРаботаСНоменклатурой
				   Или (Не ЗначениеЗаполнено(ТекущаяСтрока.ВидНоменклатуры) И ИспользоватьВидыНоменклатуры)) Тогда
				
				Если ЕстьРаботаСНоменклатурой Тогда
					ТекстШаблонаОшибки = НСтр("ru = 'В строке %1 не удалось создать номенклатуру автоматически, создайте или выбирите вручную.'");
			...
			Если ТекущаяСтрока.Пометка
				...
				Если ЕстьРаботаСНоменклатурой Тогда
					ТекстШаблонаОшибки = НСтр("ru = 'В строке %1 не удалось создать характеристику автоматически, создайте или выбирите вручную.'");
					ТекстОшибки = СтрШаблон(ТекстШаблонаОшибки, ТекИндекс + 1);
				Иначе
					ТекстШаблонаОшибки = НСтр("ru='Не заполнено поле %1 в строке %2.'");
...
Процедура ИзменитьКоличествоВЗаказе(Идентификатор, Количество, ЭтоРазница = Ложь, ДополнительныеПараметры = Неопределено, Отказ = Ложь)
	...
	Иначе
		...
		ТорговоеПредложение = ТорговыеПредложенияСлужебный.ПолучитьТорговоеПредложение(Идентификатор, Валюта, Отказ);
		...
		Если Не ЗначениеЗаполнено(СтрокаТовары.ЕдиницаИзмерения) Тогда
			...
			Если ЗначениеЗаполнено(ТорговоеПредложение.УпаковкаНаименование) Тогда
				СтрокаТовары.ПредставлениеЕдиницыИзмерения = ТорговоеПредложение.УпаковкаНаименование
					+ ?(ЗначениеЗаполнено(ПараметрыЕдиницыИзмерения.Представление),
						" (" + ПараметрыЕдиницыИзмерения.Представление + ")", "");
			Иначе
...
Процедура ИнициализацияСпискаВыбораАдреса(Элемент, ПолеАдреса, ПоказыватьПредупреждение = Истина)
	...
	Если СпособыДоставкиСамовывоз.НайтиПоЗначению(ТекущиеДанные.СпособДоставки) <> Неопределено Тогда // Самовывоз.
		...
		СтрокаКонтрагента = Товары.НайтиСтроки(Новый Структура("ИдентификаторКонтрагента", ТекущиеДанные.ИдентификаторКонтрагента))[0];
		ПараметрыКонтрагента = Новый Структура;
		ПараметрыКонтрагента.Вставить("Ссылка", СтрокаКонтрагента.Контрагент);
		ПараметрыКонтрагента.Вставить("ИНН", СтрокаКонтрагента.ИННКонтрагента);
		ПараметрыКонтрагента.Вставить("КПП", СтрокаКонтрагента.КППКонтрагента);
		
		АдресаСамовывоза = ПолучитьАдресаКонтрагента(ПараметрыКонтрагента, Отказ);
		Если Не Отказ ИЛИ АдресаСамовывоза <> Неопределено Тогда
...
&НаСервереБезКонтекста
Функция ПолучитьАдресаКонтрагента(ПараметрыМетода, Отказ)
	
	Возврат ТорговыеПредложения.ПолучитьАдресаСамовывозаОрганизации(ПараметрыМетода, Отказ);
...
Процедура ЗаполнитьПоПараметрам(Отказ)
	...
	Если Параметры.Свойство("АдресТоваровВХранилище", АдресТоваровВХранилище) И ЗначениеЗаполнено(Параметры.АдресТоваровВХранилище) Тогда
		...
		Для каждого ЭлементКоллекции Из ТаблицаТоваров Цикл
			ИзменитьКоличествоВЗаказе(ЭлементКоллекции.Идентификатор, ЭлементКоллекции.Количество,,, Отказ);
		КонецЦикла;
	...
	ИначеЕсли Параметры.Свойство("СписокТоваров") Тогда
		...
		Для каждого СоставПозиции Из Параметры.СписокТоваров Цикл
			...
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("Номенклатура", СоставПозиции.Номенклатура);
			ДополнительныеПараметры.Вставить("Характеристика", СоставПозиции.Характеристика);
			ДополнительныеПараметры.Вставить("ЕдиницаИзмерения", СоставПозиции.ЕдиницаИзмерения);
			
			
			ИзменитьКоличествоВЗаказе(СоставПозиции.Идентификатор, СоставПозиции.Количество,, ДополнительныеПараметры, Отказ);
...
Функция ПолучитьВидНоменклатурыИзКатегории(Знач ИдентификаторКатегорий)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СоответствиеВидовНоменклатурыРаботаСНоменклатурой.ИдентификаторКатегории = &ИдентификаторКатегории";
	
	Запрос.УстановитьПараметр("ИдентификаторКатегории", ИдентификаторКатегорий);
	РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	
	Если РезультатЗапроса.Количество() > 0 Тогда
		Результат = РезультатЗапроса[0].ВидНоменклатуры;
	КонецЕсли;
