...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ТорговыеПредложения_ПослеЗаписи"
		...
		ОбновитьСтатистикуСинхронизации();
...
Процедура СопоставитьНоменклатуруСервисаНажатие(Элемент)
	...
	ОчиститьСообщения();
	ОткрытьФорму("РегистрСведений.СоответствиеПоискаРаботаСНоменклатурой.Форма.Сопоставление");
...
&НаКлиенте
Процедура ОбновитьСтатистикуСинхронизации()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбновитьСтатистикуСинхронизацииЗавершение", ЭтотОбъект);
	ТорговыеПредложенияКлиент.ОбновитьСтатистикуСинхронизации(ОповещениеОЗавершении, ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСтатистикуСинхронизацииЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ОбновитьДанныеФормы(АдресРезультата = Неопределено)
	...
	ИспользоватьСопоставление1СНоменклатура = Подсистема1СНоменклатураСуществует И ПолучитьФункциональнуюОпцию("ИспользоватьСервисРаботаСНоменклатурой");
	Элементы.СопоставлениеНоменклатурыГруппа.Видимость = Подсистема1СНоменклатураСуществует;
	Элементы.СопоставлениеНоменклатурыГруппа.Доступность = ИспользоватьСопоставление1СНоменклатура;
	...
	Иначе
		КоличествоСопоставленнойНоменклатуры = 0;
		ПроцентСопоставленоНоменклатуры = 0;
		КоличествоНесопоставленнойНоменклатуры = КоличествоТоваров;
	КонецЕсли;
	...
	Иначе
		Элементы.СопоставлениеНоменклатуры.РасширеннаяПодсказка.Заголовок = "";
		Элементы.СопоставлениеНоменклатуры.Поведение = ПоведениеОбычнойГруппы.Обычное;
		Элементы.СопоставлениеНоменклатуры.ОтображатьЗаголовок = Ложь;
	КонецЕсли;
	...
	Элементы.СинхронизироватьТорговыеПредложения.Доступность = Не ЕстьБлокирующиеОшибки;
