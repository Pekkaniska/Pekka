...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТорговыеПредложения.ОбновитьСведенияРегионаПоиска(РегионПоиска,
		Элементы.ФормаВыборРегиона.Заголовок, ИдентификаторыРегионов);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ТорговыеПредложения_ИзменитьКоличествоВЗаказе"
		...
		ИдентификаторКонтрагента = СтрЗаменить(Параметр.ПоставщикИдентификатор, "/", "_");
...
Процедура СписокПриАктивизацииСтроки(Элемент)
	...
	ВидимостьПодсказки = ПустаяСтрока(ИнформацияПоТовару) И Не ПустаяСтрока(ТекущиеДанные.НаименованиеНоменклатуры);
	...
	Для каждого СтрокаКонтрагента Из Контрагенты Цикл
		...
		Если СтрокаКонтрагента.Идентификатор = Идентификатор Тогда
			...
			Если ТекущиеДанные.Пометка Тогда
				...
				ТекущиеДанные.Сумма = ТекущиеДанные.Количество * ТекущиеДанные["Цена_" + СтрокаКонтрагента.Идентификатор];
				...
				ТекущиеДанные.НаименованиеНоменклатуры = ТекущиеДанные["НаименованиеНоменклатуры_" + СтрокаКонтрагента.Идентификатор];
			Иначе
...
&НаСервере
Функция НайтиТорговыеПредложенияВФоне(Знач Задание, Знач ИдентификаторФормы, Отказ)
...
Процедура ЗаполнитьПараметрыЗапроса(ПараметрыЗапроса, Отказ)
	...
	Если ПоискПоШтрихКоду Тогда
		Штрихкоды = СписокЗначение.Скопировать();
		ТорговыеПредложенияПереопределяемый.ДобавитьСлужебныеКолонкиТовары(Штрихкоды, Истина);
	КонецЕсли;
	
	НомераСтрокПоиска.Очистить(); // Соответствие номеров строк поиска и номеров строк списка.
	Для каждого СтрокаСписка Из СписокЗначение Цикл
		...
		Если ПоискПоНаименованию И ПоискПоХарактеристике И ЗначениеЗаполнено(СтрокаСписка.Характеристика) Тогда
			НоваяСтрока.Наименование = НоваяСтрока.Наименование + ?(ПустаяСтрока(НоваяСтрока.Наименование), "", " ")
		...
		Если ПоискПоШтрихКоду Тогда
			...
			СтрокаШтрихкоды = Штрихкоды.НайтиСтроки(СтруктураПоиска);
		...
		Если ПоискЗаполнен Тогда
			...
			СтрокаИдентификаторовПоиска.НомерСтрокиСписка = СписокЗначение.Индекс(СтрокаСписка);
...
Процедура НайтиТорговыеПредложенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ТорговыеПредложенияКлиент.ОбработатьРезультатФоновогоЗадания(Результат, ДополнительныеПараметры, Отказ);
...
Процедура ОткрытьФормуОформленияЗаказов(РежимЗапросаЦен = Ложь)
	...
	Для каждого СтрокаСписка Из ВыбранныеСтроки Цикл
		...
		СоставПозиции.Вставить("Номенклатура",     СтрокаСписка.Номенклатура);
		СоставПозиции.Вставить("Характеристика",   СтрокаСписка.Характеристика);
		СоставПозиции.Вставить("Количество",       СтрокаСписка.Количество);
		СоставПозиции.Вставить("Упаковка",         СтрокаСписка.Упаковка);
		СоставПозиции.Вставить("ЕдиницаИзмерения", СтрокаСписка.ЕдиницаИзмерения);
		
		// Данные сервиса.
		СоставПозиции.Вставить("ИдентификаторКонтрагента", ИдентификаторКонтрагента);
		СоставПозиции.Вставить("НаименованиеКонтрагента",  СтрокаКонтрагента.Наименование);
		СоставПозиции.Вставить("ИННКонтрагента",           СтрокаКонтрагента.ИНН);
		СоставПозиции.Вставить("КППКонтрагента",           СтрокаКонтрагента.КПП);
		СоставПозиции.Вставить("Контрагент",               СтрокаКонтрагента.Контрагент);
		СоставПозиции.Вставить("Идентификатор",   СтрокаСписка["Идентификатор_"   + ИдентификаторКонтрагента]);
		СоставПозиции.Вставить("НаименованиеНоменклатуры",  СтрокаСписка["НаименованиеНоменклатуры_" + ИдентификаторКонтрагента]);
		СоставПозиции.Вставить("СрокПоставки",    СтрокаСписка["СрокПоставки_"    + ИдентификаторКонтрагента]);
		СоставПозиции.Вставить("Цена",            СтрокаСписка["Цена_"            + ИдентификаторКонтрагента]);
		СоставПозиции.Вставить("Сумма",            СтрокаСписка.Сумма);
		СоставПозиции.Вставить("КонтекстСтроки",  СтрокаСписка.КонтекстСтроки);
...
Процедура АвтовыборПоСценарию(Режим)
	...
	Для каждого СтрокаСписка Из Список Цикл
		...
		Если ЗначениеЗаполнено(НайденныйПоставщик) Тогда
			...
			СтрокаСписка.НаименованиеНоменклатуры = СтрокаСписка["НаименованиеНоменклатуры_" + НайденныйПоставщик];
...
Процедура ИнформацияПоТоваруНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ПоискПоНаименованию И ЗначениеЗаполнено(ТекущиеДанные.НаименованиеНоменклатуры) Тогда
...
Функция РеквизитыНоменклатуры(Знач Номенклатура)
	...
	Реквизиты = ТорговыеПредложения.НовыйПараметрыНоменклатуры();
...
Процедура НайтиТорговыеПредложения()
	...
	Если Список.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Отсутствуют товары для поиска.'"));
		Возврат;
	КонецЕсли;
	...
	Задание.ИмяПроцедуры = "ТорговыеПредложенияСлужебный.ПоискТорговыхПредложенийПоСписку";
	
	Отказ = Ложь;
	ДлительнаяОперация = НайтиТорговыеПредложенияВФоне(Задание, УникальныйИдентификатор, Отказ);
	Если Отказ Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не найдены параметры поиска.'"));
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура СохранитьРегионПоиска()
