...
Процедура РемонтыОбъединитьРемонтыВОдинЗаказ(Команда)
	...
	СписокВыбора.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("РемонтыОбъединитьРемонтыВОдинЗаказЗавершение", ЭтотОбъект, Новый Структура("ВыделенныеСтроки, НайденныеСтроки, ОбъектЭксплуатации", ВыделенныеСтроки, НайденныеСтроки, ОбъектЭксплуатации)), НСтр("ru='Выберите заказ на ремонт'"));
КонецПроцедуры
...
Процедура УдалитьЗаказыНаРемонт(Отказ=Ложь)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Планирование ремонтных работ.Удаление ранее созданных документов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Для Каждого ЗаказНаРемонт Из Объект.ЗаказыНаРемонт Цикл
		...
		Если АктуальныеКодыЗаказовНаРемонт.Получить(ЗаказНаРемонт.КодЗаказаНаРемонт) <> Неопределено Тогда // Для данного заказа есть ремонты
			...
			Если Не ЗначениеЗаполнено(ЗаказНаРемонт.ДатаНачала) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Дата начала'")) + АдресОшибки,,
			...
			Если Не ЗначениеЗаполнено(ЗаказНаРемонт.Длительность) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Длительность'")) + АдресОшибки,,
			...
			Если Не ЗначениеЗаполнено(ЗаказНаРемонт.Подразделение) Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					СтрЗаменить(ШаблонОшибки, "%Поле%", НСтр("ru='Подразделение'")) + АдресОшибки,,
