	...
	Для Каждого ЗаказНаРемонт Из ЗаказыНаРемонт Цикл
		...
		Если Не ПроведениеВыполнено Тогда
			...
			Исключение
				...
				ВызватьИсключение НСтр("ru='Ошибка при попытке создания документа заказа на ремонт:'") + " " + ИнформацияОбОшибке().Описание;
			КонецПопытки;
		КонецЕсли;
...
Процедура СформироватьТекстЗапросаРасчетаПланаРемонтныхРабот(ТекстЗапроса, СтруктураПакета)
	...
	СтруктураПакета.Вставить("ПредыдущиеРемонты", 6);
	СтруктураПакета.Вставить("Наработки", 7);
	СтруктураПакета.Вставить("ОбъектыПланирования", 8);
	СтруктураПакета.Вставить("РемонтныеЦиклы", 9);
	СтруктураПакета.Вставить("ПрочиеРемонты", 10);
	СтруктураПакета.Вставить("НаработкиПослеРемонтов", 11);
...
Функция СледующийРемонтПоПравилу(РемонтируемыйОбъект, Правило, ДатаПредыдущего, Выборки)
	...
	Если Правило.ПоИнтервалуНаработки Тогда
		...
		Если ДатаПредыдущего < ТекущаяДатаСеанса() Тогда
		...
		Если НаработкаСПоследнегоРемонта >= Правило.ИнтервалНаработки Тогда
			СтруктураРемонта.ДатаНачала = НачалоДня(ТекущаяДатаСеанса());
		Иначе
	...
	Если СтруктураРемонта.ДатаНачала < ТекущаяДатаСеанса() Тогда
		СтруктураРемонта.ДатаНачала = НачалоДня(КонецДня(ТекущаяДатаСеанса())+1);
