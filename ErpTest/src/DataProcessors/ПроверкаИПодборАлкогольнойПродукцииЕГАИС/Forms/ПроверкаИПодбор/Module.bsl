...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеВидимостьюКомандУпаковок();
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если РежимПроверкиСохраненный <> Неопределено Тогда
		...
		СформироватьПредставлениеНастроек();
	КонецЕсли;
...
Процедура ОпределитьТипУпаковки(СтрокаДерева, ВключаяПодчиненные)
	...
	Для Каждого ПодчиненнаяСтрока Из СтрокаДерева.ПолучитьЭлементы() Цикл
		...
		Справки2.Вставить(ПодчиненнаяСтрока.Справка2, Истина);
		Серии.Вставить(ПодчиненнаяСтрока.Серия, Истина);
...
&НаСервере
Процедура ПересчитатьХешСуммыВсехУпаковок(ИдентификаторСтрокиУпаковки = Неопределено)

	Если НЕ ПроверятьНеобходимостьПеремаркировки Тогда
		Возврат;
	КонецЕсли;
	
	Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
	...
	Если ИдентификаторСтрокиУпаковки = Неопределено Тогда
...
Процедура ПроверитьНеобходимостьПеремаркировки(ТаблицаХешСумм, ЭтоВыборочнаяПроверка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокаДерева <> Неопределено Тогда
			...
			ТребуетсяПеремаркировка = Выборка.ТребуетсяПеремаркировка 
			                          И Не СтрокаДерева.СтатусПроверки = Перечисления.СтатусыПроверкиНаличияАлкогольнойПродукции.Отсутствует;
			
			Если ЭтоВыборочнаяПроверка Тогда
				
				Если СтрокаДерева.ТребуетсяПеремаркировка 
					И Не ТребуетсяПеремаркировка Тогда
				...
				ИначеЕсли Не СтрокаДерева.ТребуетсяПеремаркировка 
					И ТребуетсяПеремаркировка Тогда
			...
			Иначе
				...
				Если ТребуетсяПеремаркировка Тогда
...
Функция ПолучитьПустоеЗначениеОпределяемогоТипа(ИмяТипа)
	...
	Возврат ИнтеграцияЕГАИС.ПустоеЗначениеОпределяемогоТипа(Метаданные.ОпределяемыеТипы[ИмяТипа].Имя);
...
Процедура СформироватьПредставлениеСодержимогоУпаковки(СтрокаДерева)
	...
	Если СтрокаДерева.ТипУпаковки = ПредопределенноеЗначение("Перечисление.ТипыУпаковок.МаркированныйТовар") Тогда
		...
		СтрокаДерева.ПредставлениеСодержимоеУпаковки = СтрокаДерева.АлкогольнаяПродукция;
...
Процедура ЗагрузитьДанныеДокумента(СоответствиеШтрихкодовСтрокДерева, ПомещатьВоВременноеХранилище)
	...
	СохраненнаяНастройкаДетализацииСтруктурыХранения = Неопределено;
	...
	Если ТипЗнч(СохраненныеНастройки) = Тип("Структура")
		...
		СохраненнаяНастройкаДетализацииСтруктурыХранения = СохраненныеНастройки.ДетализацияСтруктурыХранения;
	...
	Если ПроверкаНеПоДокументу Тогда
		...
		Если ЭтоАдресВременногоХранилища(Параметры.АдресДанныхПроверяемойАлкогольнойПродукции) Тогда
			...
			ЗагрузитьДеревоМаркированнойПродукции(ПроверяемыеДанные.ДеревоУпаковок, 
			                                      СоответствиеШтрихкодовСтрокДерева, 
			                                      СохраненнаяНастройкаДетализацииСтруктурыХранения);
	...
	ИначеЕсли ТипЗнч(ПроверяемыйДокумент) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
		...
		ЗагрузитьДеревоМаркированнойПродукции(ДанныеПроверяемогоДокумента.ДеревоУпаковок,
		                                      СоответствиеШтрихкодовСтрокДерева,
		                                      СохраненнаяНастройкаДетализацииСтруктурыХранения);
	...
	Если ПомещатьВоВременноеХранилище Тогда
		...
		Иначе
			АдресСоответствиеШтрихкодыИдентификаторыСтрок = ПоместитьВоВременноеХранилище(СоответствиеШтрихкодовСтрокДерева, АдресСоответствиеШтрихкодыИдентификаторыСтрок);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьДеревоМаркированнойПродукции(ДеревоПоДаннымДокумента, СоответствиеШтрихкодовСтрокДерева, СохраненнаяНастройкаДетализацииСтруктурыХранения)
	...
	Для каждого СтрокаПоДаннымДокумента Из ДеревоПоДаннымДокумента.Строки Цикл
		...
		ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(СтрокаПоДаннымДокумента,
			ДетализацияСтруктурыХраненияДанныхДокумента, КоллекцияСтрокПриемника, СохраненнаяНастройкаДетализацииСтруктурыХранения, СоответствиеШтрихкодовСтрокДерева);
		
	КонецЦикла;
	
	Если СохраненнаяНастройкаДетализацииСтруктурыХранения <> Неопределено Тогда
		
		Если КоллекцияСтрокПриемника.Количество() = 0 Тогда
		
			ДетализацияСтруктурыХранения = СохраненнаяНастройкаДетализацииСтруктурыХранения;
		...
		ИначеЕсли ДетализацияСтруктурыХраненияДанныхДокумента = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная
			...
			ДетализацияСтруктурыХранения = ДетализацияСтруктурыХраненияДанныхДокумента;
	...
	Если ИдентификаторСтрокиБутылкиБезУпаковки = - 1
		И ДетализацияСтруктурыХранения <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
...
&НаСервере
Процедура ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(СтрокаИсточника, ДетализацияДанныхИсточника, КоллекцияСтрокПриемника, ДетализацияДанныхПриемника, СоответствиеШтрихкодовСтрокДерева)
	
	Если СтрокаИсточника.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаАлкогольнойПродукции.БутылкиБезКоробки Тогда
		
		Если ДетализацияДанныхИсточника <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки
			ИЛИ (ДетализацияДанныхПриемника <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки
				И ДетализацияДанныхПриемника <> Неопределено) Тогда
		...
		Иначе
			...
			Для Каждого ПодчиненнаяСтрокаИсточника Из СтрокаИсточника.Строки Цикл
				...
				ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(ПодчиненнаяСтрокаИсточника,
					ДетализацияДанныхИсточника, КоллекцияСтрокПриемника, ДетализацияДанныхПриемника, СоответствиеШтрихкодовСтрокДерева);
	...
	ИначеЕсли СтрокаИсточника.Родитель = Неопределено
		...
		И (ДетализацияДанныхИсточника <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки
			ИЛИ (ДетализацияДанныхПриемника <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки
				И ДетализацияДанныхПриемника <> Неопределено)) Тогда
	...
	Для Каждого ПодчиненнаяСтрокаИсточника Из СтрокаИсточника.Строки Цикл
		...
		ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(ПодчиненнаяСтрокаИсточника,
			ДетализацияДанныхИсточника, КоллекцияСтрокДобавленнойСтроки, ДетализацияДанныхПриемника, СоответствиеШтрихкодовСтрокДерева);
...
Функция ВосстановитьСохраненныеРезультатыПроверки(ДанныеПроверки)
	...
	Если ДанныеПроверки.Свойство("СохраненВыборПоАлкогольнойПродукции") Тогда
		СохраненВыборПоАлкогольнойПродукции = ДанныеПроверки.СохраненВыборПоАлкогольнойПродукции;
	КонецЕсли;
	
	Если ДанныеПроверки.Свойство("ДанныеВыбораПоАлкогольнойПродукции") Тогда
		ДанныеВыбораПоАлкогольнойПродукции = ДанныеПроверки.ДанныеВыбораПоАлкогольнойПродукции;
	КонецЕсли;
...
Процедура ИнициализироватьДанныеФормы(РежимОчистки = Ложь)
	...
	ЦветТекстаАвто  = ЦветаСтиля.ЦветТекстаПоля;
...
Процедура ПересчитатьВсеИтогиФормы()
	...
	СформироватьПредставлениеНастроек();
	АкцизныеМаркиКлиентСервер.ОтобразитьСохраненныйВыборПоАлкогольнойПродукции(ЭтотОбъект);
...
Функция ДанныеРезультатовПроверки()
	...
	ДанныеРезультатовСканированияАлкогольнойПродукции.Вставить("СохраненВыборПоАлкогольнойПродукции",      СохраненВыборПоАлкогольнойПродукции);
	ДанныеРезультатовСканированияАлкогольнойПродукции.Вставить("СохраненВыборПоАлкогольнойПродукции",      СохраненВыборПоАлкогольнойПродукции);
	ДанныеРезультатовСканированияАлкогольнойПродукции.Вставить("ДанныеВыбораПоАлкогольнойПродукции",       ДанныеВыбораПоАлкогольнойПродукции);
...
Функция РезультатСканированияАлкогольнойПродукции(СтатусПроверки, УникальныйИдентификаторФормыВладельца)
	...
	Если ПриЗавершенииСохранятьРезультатыПроверки Тогда
		РезультатПроверки.Вставить("АдресДанныхРезультатаПроверки", ПоместитьВоВременноеХранилище(ДанныеРезультатовПроверки(), УникальныйИдентификаторФормыВладельца));
	КонецЕсли;
...
Функция ПараметрыСканированияАкцизныхМарок()
	...
	ПараметрыСканированияАкцизныхМарок.ДанныеВыбораПоАлкогольнойПродукции             = ДанныеВыбораПоАлкогольнойПродукции;
	...
	ПараметрыСканированияАкцизныхМарок.ИспользуетсяСоответствиеШтрихкодовСтрокДерева  = Истина;
...
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		АкцизныеМаркиЕГАИСКлиент.ОбработатьДанныеШтрихкода(ОповещениеПриЗавершении,
		                                                   ЭтотОбъект, ДанныеШтрихкода, ПараметрыСканированияАкцизныхМарок());
...
Процедура ОбработатьНеНайденныйВДеревеШтрихКод(ДанныеШтрихкода, ТекущаяСтрокаДерева, ОбработкаДанныхТСД)
	...
	Если РежимПодбораСуществующихУпаковок
		...
		Если ПараметрыСканированияАкцизныхМарок.ДоступныеСтатусы.Найти(ПредопределенноеЗначение("Перечисление.СтатусыАкцизныхМарок.ПустаяСсылка")) = Неопределено Тогда
			...
			АкцизныеМаркиЕГАИСКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, ДанныеШтрихкода);
		...
		Если ОшибкаКонтроля Тогда
			...
			ОткрытьФорму("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ИнформацияОНевозможностиДобавленияОтсканированного", ПараметрыОткрытияФормы,
			             ЭтотОбъект, УникальныйИдентификатор,,,ОповещениеЗакрытияФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	Если ДанныеШтрихкода.МаркируемаяАлкогольнаяПродукция = Истина
...
Процедура ПроверитьШтрихкодВДеревеМаркированнойПродукции(ДанныеШтрихкода, ОбработкаДанныхТСД = Ложь)
	...
	ИначеЕсли РежимПодбораСуществующихУпаковок 
		...
		Если Результат.ЕстьОшибки Тогда
			...
			АкцизныеМаркиЕГАИСКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, Результат.АдресХранилищаДереваУпаковки);
...
Процедура ПроверитьШтрихкодВТаблицеНеМаркированнойПродукции(ДанныеШтрихкода, ШтрихкодОбработан, ОбработкаДанныхТСД)
	...
	ИначеЕсли ДанныеШтрихкода.МаркируемаяАлкогольнаяПродукция = Истина  Тогда
...
Процедура ОбработатьОтсканированныйШтрихкод(ДанныеШтрихкода, ОбработкаДанныхТСД = Ложь)
	...
	АкцизныеМаркиКлиентСервер.ОбработатьСохраненныйВыборДанныхПоАлкогольнойПродукции(ЭтотОбъект, ДанныеШтрихкода);
...
&НаКлиенте
Процедура ДобавитьНовуюУпаковку(ЗначениеШтрихкода, ТипУпаковки, СпозиционироватьсяНаДобавленной = Истина, ОбработкаДанныхТСД = Ложь)
...
Процедура ПроверитьНаДубльВСтрокеДерева(СтрокаДерева, СоответствиеШтрихкодовСтрокДерева, ЕстьОшибки)
	...
	Если СоответствиеШтрихкодовСтрокДерева.Получить(СтрокаДерева.ЗначениеШтрихкода) <> Неопределено Тогда
...
&НаСервере
Функция РезультатДобавленияСуществующейУпаковкиВДеревоМаркированнойПродукции(ДанныеШтрихкода, СоответствиеШтрихкодовСтрокДерева, ОбработкаДанныхТСД)
	...
	Иначе
		ДеревоУпаковок = Справочники.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкодыУпаковок(ДанныеШтрихкода.ШтрихкодУпаковки, ОрганизацияЕГАИС).ДеревоУпаковок;
	КонецЕсли;
	...
	ЭтоУпаковка = ИнтеграцияЕГАИСКлиентСервер.ЭтоУпаковка(ДанныеШтрихкода.ТипУпаковки);
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		ЭтоСтрокаСоСчитаннымШтрихКодом = СтрокаДерева.ЗначениеШтрихкода = ДанныеШтрихкода.Штрихкод;
...
&НаСервере
Процедура УправлениеВидимостьюКомандУпаковок()
	
	КомандыВидимы = ДетализацияСтруктурыХранения <> ПредопределенноеЗначение("Перечисление.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки");
...
Функция УпаковкаНеСодержитсяВДанныхДокумента(ТекущаяСтрокаДерева, Детализация)
	...
	ИначеЕсли ИнтеграцияЕГАИСКлиентСервер.ЭтоУпаковка(ТекущаяСтрокаДерева.ТипУпаковки) Тогда
...
Процедура Подключаемый_ПолученыДанныеИзТСД(ТаблицаТоваров, ДополнительныеПараметры) Экспорт
	...
	Если ЗагрузкаДанныхТСД.ЕстьОшибкиВДеревеУпаковок Тогда
		...
		АкцизныеМаркиЕГАИСКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтотОбъект, ЗагрузкаДанныхТСД.АдресДереваУпаковок);
...
Функция СледующаяОперацияОбработкиДанныхТСД(Результат)
	...
	Если ЗагрузкаДанныхТСД.ТекущаяОперация = "" Тогда
		...
		ИначеЕсли ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() > 0 Тогда
			
			СледующаяОперация = "МаркируемаяАлкогольнаяПродукция";
		...
		ИнтеграцияЕГАИСКлиент.ОповеститьОНачалеОбработкиДанныхТСД();
	...
	ИначеЕсли ЗагрузкаДанныхТСД.ТекущаяОперация = "МаркируемаяАлкогольнаяПродукция" Тогда
		
		Если ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента <> ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() Тогда
...
Процедура ВыполнитьСледующуюОперациюОбрабткиДанныхТСД(СледующаяОперация)
	...
	ИначеЕсли СледующаяОперация = "МаркируемаяАлкогольнаяПродукция" Тогда 
		
		Если ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() <> ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента Тогда
			
			ПараметрыСканирования = ПараметрыСканированияАкцизныхМарок();
			ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента = ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента + 1;
			ДанныеШтрихКода = ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция[ЗагрузкаДанныхТСД.ИндексОбрабатываемогоЭлемента - 1].ДанныеШтрихКода;
			
			УчитыватьДанныеВыбора = Ложь;
			ДанныеВыбора          = ПараметрыСканирования.ДанныеВыбораПоАлкогольнойПродукции;
			...
			Если Не ИнтеграцияЕГАИСКлиентСервер.ЭтоУпаковка(ДанныеШтрихКода.ТипУпаковки)
				И ПараметрыСканирования.ЗапрашиватьНоменклатуру
				...
				Или Не ЗначениеЗаполнено(ДанныеШтрихкода.АлкогольнаяПродукция)) Тогда
					...
					ПараметрыВыбораНоменклатуры.Вставить("ЗначениеШтрихкода",             ДанныеШтрихкода.Штрихкод);
	...
	ИначеЕсли СледующаяОперация = "ОкончаниеПроверки" Тогда
		...
		ИнтеграцияЕГАИСКлиент.ОповеститьОбОкончанииОбработкиДанныхТСД();
...
&НаСервере
Функция РезультатПроверкиДанныхТСД(МассивСчитанныхДанных, СоответствиеШтрихкодовСтрокДерева, ПараметрыСканированияАкцизныхМарок)
	...
	ПараметрыПроверки.Вставить("ПараметрыСканированияАкцизныхМарок", ПараметрыСканированияАкцизныхМарок);
...
#Область СохраненныйВыборПоАлкогольнойПродукции
...
&НаСервере
Процедура СформироватьПредставлениеНастроек()
	
	ПредставлениеНастроек = Новый ФорматированнаяСтрока(ПредставлениеУровняДетализации(), " " ,ПредставлениеРежимаПроверки());
	
КонецПроцедуры

&НаСервере
Функция ПредставлениеРежимаПроверки()
	...
	Если Не РежимПросмотра Тогда
		СтрокаДействие = Новый ФорматированнаяСтрока(СтрокаДействие,, ЦветаСтиля.ЦветГиперссылкиГИСМ,, "ИзменитьРежимПроверки");
	КонецЕсли;
...
&НаСервере
Функция ПредставлениеУровняДетализации()
	
	Если Не ЗначениеЗаполнено(ДетализацияСтруктурыХранения) Тогда
		
		ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная;
		
	КонецЕсли;
	
	ТекстВступление = НСтр("ru = 'Детализация'");
	СтрокаДействие = НРег(Строка(ДетализацияСтруктурыХранения));
	
	Если Не РежимПросмотра Тогда
		СтрокаДействие = Новый ФорматированнаяСтрока(СтрокаДействие,, ЦветаСтиля.ЦветГиперссылкиГИСМ,, "ИзменитьДетализацию");
	КонецЕсли;
...
Процедура ИзменитьРежимДетализации(НовыйРежимДетализации, СоответствиеШтрихкодовСтрокДерева)
	...
	Если ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная
		И НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.КоробаСБутылками Тогда
		
		ТребуетсяПересчет = Истина;
		ПонизитьУровеньДетализацииДоКоробаСБутылками();
		
	ИначеЕсли (ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная
		Или ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.КоробаСБутылками)
		И НовыйРежимДетализации = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
		
		ТребуетсяПересчет = Истина;
		ПонизитьУровеньДетализацииДоБутылки();
		ИдентификаторТекущейПроверяемойУпаковки = - 1;
		
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки
		И НовыйРежимДетализации <> Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
	...
	Если ТребуетсяПересчет Тогда
		...
		СоответствиеШтрихкодовСтрокДерева = Новый Соответствие;
		ДополнитьСоответствиеШтрихкодовСтрокДереваДляКоллекции(ДеревоМаркированнойПродукции.ПолучитьЭлементы(), 
		                                                       СоответствиеШтрихкодовСтрокДерева);
	...
	Если ТипЗнч(ПроверяемыйДокумент) <> Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
		...
		ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить("Обработка.ПроверкаИПодборАлкогольнойПродукцииЕГАИС.Форма.ПроверкаИПодбор", 
