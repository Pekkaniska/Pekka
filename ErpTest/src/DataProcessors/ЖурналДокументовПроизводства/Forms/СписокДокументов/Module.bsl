...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыРазмещения.Источники = Новый ОписаниеТипов(ТЗХозОперацииИТипыДокументов.ВыгрузитьКолонку("ТипДокумента"));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ПроизводствоБезЗаказа_Запись"
		ИЛИ ИмяСобытия = "РаспределениеВозвратныхОтходов_Запись"
		...
		ИЛИ ИмяСобытия = "Запись_РаспределениеПрочихЗатрат" Тогда
...
Процедура СписокУстановитьПометкуУдаления(Команда)
	...
	Строки = Элементы.СписокДокументыПроизводства.ВыделенныеСтроки;
	
	//++ НЕ УТКА
	ЕстьНеобрабатываемые = Ложь;
	СтрокиКУдалению = Новый Массив;
	Для Каждого ТекСтрока Из Строки Цикл
		ТекущиеДанные = Элементы.СписокДокументыПроизводства.ДанныеСтроки(ТекСтрока);
		Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			СтрокиКУдалению.Добавить(ТекСтрока);
			ЕстьНеобрабатываемые = Истина;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ТекСтрока Из СтрокиКУдалению Цикл
		Строки.Удалить(Строки.Найти(ТекСтрока));
	КонецЦикла;
	
	Если ЕстьНеобрабатываемые Тогда
		ТекстСообщения = НСтр("ru = 'Пометка этапов на удаление осуществляется из рабочего места ""Структура заказа на производство""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	//-- НЕ УТКА
	
	Если Не Строки.Количество() = 0 Тогда
		ОбщегоНазначенияУТКлиент.УстановитьПометкуУдаления(Элементы.СписокДокументыПроизводства, Заголовок);
		ОбновитьГиперссылкуКОформлению();
	КонецЕсли;
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтаФорма, Элементы.СписокДокументыПроизводства);
