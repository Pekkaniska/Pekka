...
Процедура УстановитьУсловноеОформление()
	...
	"ПолуфабрикатыИМатериалы.Характеристика");
...
Процедура ЗаполнитьТаблицуПродукцииНаСервере()
	...
	|				ГДЕ
	|					ОсновныеСпецификации.Номенклатура      = ВТНоменклатураЗаказа.Номенклатура
	|					И ОсновныеСпецификации.Характеристика  В (ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка), ВТНоменклатураЗаказа.Характеристика)
	|					И ОсновныеСпецификации.Подразделение   = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|					И НЕ ОсновныеСпецификации.Спецификация = ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)))
	|");
	
	Запрос.УстановитьПараметр("Заказ", Заказ);
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("ПоЗаказуДавальца", ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказДавальца"));
	//-- НЕ УТКА
...
Процедура СформироватьЗаказыНаСервере()
	...
	РеквизитыШапки = Новый Структура("Дата, ЖелаемаяДатаОтгрузки, Организация, Подразделение, ЦеховаяКладовая, НаправлениеДеятельности,
