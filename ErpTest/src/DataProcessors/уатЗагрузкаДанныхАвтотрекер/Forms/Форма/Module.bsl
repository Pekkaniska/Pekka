...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	Если ЗагрузитьПробегИРасходГСМ Тогда // загружаем пробег и расход ГСМ
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТСАвтотрекер(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
	
	Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // загружаем дополнительные сведения
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияПоТСАвтотрекер(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьАвтотрекер.Получить() Тогда
		ТекстСообщения = "Открытие возможно только тогда, когда включено использование системы GPS - Автотрекер.";
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	Если Не Отказ Тогда 
		...
		Состояние("Выполняется загрузка данных системы GPS Автотрекер",,"Операция может занять длительное время");
		КодВозврата = ВыполнитьЗагрузкуСервер(ТекстОшибки);
		
		Если КодВозврата = 0 Тогда 
			Состояние("Загрузка данных системы GPS Автотрекер завершена успешно");
		КонецЕсли;
