...
#Область ПроверкиТабличнойЧастиТовары
	...
	ИначеЕсли Объект.Товары.Итог("КоличествоУпаковок") = 0 И
			Объект.ВариантОформленияДокументов <> Перечисления.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не введено ни одной строки для оформления документов продажи в список ""Товары"". Оформление только документов'"),
			,
			"Объект.Товары",
			,
			ЕстьОшибкиТовары);
...
#Область ПроверкиРеквизитовОплатыИОтгрузкиИТабличнойЧастиЭтапыоплаты
	...
	Если Объект.ВариантОформленияДокументов <> Перечисления.ВариантыОформленияДокументовПродажи.КоммерческоеПредложение И
		...
		Если (ПорядокРасчетов <> Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов ИЛИ НЕ ГрафикИсполненияВДоговоре) 
...
Процедура НадписьЭтапыОплатыНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("УпрощенныйРежим",                ИспользоватьУпрощеннуюСхемуОплаты ИЛИ ОплатаУказываетсяВРеализации);
...
Процедура ОткрытьПодборСерий(Текст = "", ТекущиеДанные = Неопределено)
	...
	Если ИспользоватьРасширеннуюФормуПодбораКоличестваИВариантовОбеспечения
		И Не ЭтоРеализация(ТекущиеДанные.ВариантОформления) Тогда
...
// СозданиеРеализацииТоваровИУслугАктаВыполненныхРабот
	...
	ИначеЕсли Объект.ВариантОформленияДокументов = Перечисления.ВариантыОформленияДокументовПродажи.РеализацияТоваровУслуг И НЕ Отказ Тогда
		...
		Если Объект.СоздаватьСчетНаОплату И НЕ Отказ
			...
			ЗаписатьСчетНаОплату(МассивНайденныхДокументов, ОснованиеОбъект, ОснованиеОбъект.ЭтапыГрафикаОплаты.Выгрузить(), Отказ);
...
Процедура СоздатьРасходныеОрдера(Товары, МассивНайденныхДокументов, МассивРеализаций, ОснованиеОбъект, БезПроведения, Отказ)
	...
	Пока СтрокаТаблицы <> Неопределено И ЗаблокироватьДокумент(СтрокаТаблицы.Документ) Цикл
		...
		ДокументОбъект = СтрокаТаблицы.Документ.ПолучитьОбъект();
		Если ДокументОбъект <> Неопределено И ДокументОбъект.ПометкаУдаления Тогда
			ДокументОбъект.УстановитьПометкуУдаления(Ложь);
		КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормыДатОтгрузки()
	...
	Элементы.ДекорацияДатаОтгрузки.Видимость = ((НЕ ИспользоватьПострочнуюОтгрузкуВЗаказеКлиента И Объект.НеОтгружатьЧастями) 
		ИЛИ Объект.УпрощенноеОбеспечение ИЛИ  Объект.ОтображатьРекомендацииКПокупке) И Не ЭтоКоммерческоеПредложение;
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураЗаполнитьХарактеристики.Вставить("ЗаполнитьПризнакХарактеристикиИспользуются", Новый Структура("Номенклатура", "ХарактеристикиИспользуются"));
...
Функция ПараметрыФормыЗапросаКоличестваИСерий()
	...
	Если ТекущаяСтрока.ВариантОформления =
		ПредопределенноеЗначение("Перечисление.ВариантыОформленияДокументовПродажи.ЗаказКлиента") Тогда
		ПараметрыФормы.Вставить("ПараметрыУказанияСерий", ПараметрыУказанияСерий.Заказ);
	КонецЕсли;
