...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	Если ЗагрузитьПробегИРасходГСМ Тогда // загружаем пробег и расход ГСМ
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТСWialon(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
	
	Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // загружаем дополнительные сведения
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияПоТСWialon(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьWialon.Получить() Тогда
		ТекстСообщения = "Открытие возможно только тогда, когда включено использование системы GPS - Wialon.";
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	// Выполним загрузку
	Если Не Отказ Тогда 
		Состояние("Выполняется загрузка данных системы GPS Wialon",,"Операция может занять длительное время");
		КодВозврата = ВыполнитьЗагрузкуСервер(ТекстОшибки);
		
		Если КодВозврата = 0 Тогда 
			Состояние("Загрузка данных системы GPS Wialon завершена успешно");
		КонецЕсли;
	КонецЕсли;
