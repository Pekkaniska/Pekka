...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокТипов.Добавить(Тип("ДокументСсылка.ПоступлениеДенежныхДокументов"));
	СписокТипов.Добавить(Тип("ДокументСсылка.ВыбытиеДенежныхДокументов"));
...
Процедура УстановитьУсловноеОформление()
	...
	ОбщегоНазначенияУТ.УстановитьУсловноеОфорлениеПоляСКлючемРеестраДокументов(ЭтотОбъект,
		Элементы.СписокРаспоряженийМОЛ.Имя,
		"СписокРаспоряжений.МОЛ");
...
Процедура УстановитьОтбор()
	...
	Если ЗначениеЗаполнено(ОтборМОЛ) Тогда
		КлючиРеестраДокументов = Справочники.КлючиРеестраДокументов.КлючиПоЗначениям(ОтборМОЛ);
		Если КлючиРеестраДокументов.Количество() Тогда
			МОЛКлюч = КлючиРеестраДокументов[0];
		Иначе
			МОЛКлюч = Неопределено;
		КонецЕсли;
	Иначе
		МОЛКлюч = Неопределено;
	КонецЕсли;
