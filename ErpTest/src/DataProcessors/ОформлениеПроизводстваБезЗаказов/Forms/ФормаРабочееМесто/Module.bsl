...
Процедура ИзменениеМоделиЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Элементы[ДополнительныеПараметры].Обновить();
...
Функция СформироватьДокументыПоПараметрам(ПараметрыДокументов)
	...
	// Данные шапки.
	ДатаДокумента = Мин(КонецМесяца(ПериодРегистрации), ТекущаяДатаСеанса());
	...
	// Получим основные спецификации
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаИзделия.Подразделение                           КАК Подразделение,
	|	ТаблицаИзделия.Номенклатура                            КАК Номенклатура,
	|	ТаблицаИзделия.Характеристика                          КАК Характеристика,
	|	НАЧАЛОПЕРИОДА(ТаблицаИзделия.НачалоПроизводства, ДЕНЬ) КАК НачалоПроизводства
	|ПОМЕСТИТЬ СписокНоменклатуры
	|ИЗ
	|	&ТаблицаИзделия КАК ТаблицаИзделия
	|";
	
	Запрос.УстановитьПараметр("ТаблицаИзделия", ТаблицаИзделияПоСпецификации);
	
	СтруктураКлючаПоискаСтроки = Новый Структура;
	СтруктураКлючаПоискаСтроки.Вставить("Подразделение", "Подразделение");
...
Процедура КОформлениюПродукцияОбновить(Команда)
	...
	КОформлениюОбщиеОбновить("КОформлениюПродукция");
...
Процедура КОформлениюРаботыОбновить(Команда)
	...
	КОформлениюОбщиеОбновить("КОформлениюРаботы");
...
Процедура КОформлениюОбщиеОформитьСогласноНастройке(ИмяТЧ)
	...
	Для Каждого ЭлКоллекции Из ВыделенныеСтроки Цикл
		...
		СтруктураИзделия = Новый Структура("Организация, ГруппировкаЗатрат,
											...
											|Количество, ОтражатьЗатратыДокументом, ОшибкаВНастройкахМодели");
...
&НаКлиенте
Процедура КОформлениюОбщиеОбновить(ИмяТЧ)
	
	// СтандартныеПодсистемы.ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(,
		"Обработка.ОформлениеПроизводстваБезЗаказов.Форма.ФормаРабочееМесто.Команда." + ИмяТЧ + "Обновить");
	// Конец СтандартныеПодсистемы.ЗамерПроизводительности
...
Процедура КОформлениюОбщиеНастроитьМодель(ИмяТЧ)
	...
	// СтандартныеПодсистемы.ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(,
		"Обработка.ОформлениеПроизводстваБезЗаказов.Форма.ФормаРабочееМесто.Команда." + ИмяТЧ + "НастроитьМодель");
	// Конец СтандартныеПодсистемы.ЗамерПроизводительности
