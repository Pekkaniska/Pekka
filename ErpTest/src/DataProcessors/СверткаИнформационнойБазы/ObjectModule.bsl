...
Перем СебестоимостьБезВидовЗапасов;
...
Функция ТипыДокументовВводаОстатков(ОбрабатыватьЗаказы)
	...
	МассивТипов.Добавить("РегистрацияЗемельныхУчастков");
	МассивТипов.Добавить("РегистрацияТранспортныхСредств");
	...
	МассивТипов.Добавить("РегистрацияПорядкаНалогообложенияПоНалогуНаИмущество");
	МассивТипов.Добавить("РегистрацияНаработокТМЦВЭксплуатации");
...
Функция ДокументыВводаОстатковПодлежащиеОбработке(Действие)
	...
	// Добавление документов СФПолуч и СФ на аванс, введенных на основании ввода остатков
	Если Действие = "проведение" Тогда
...
Процедура СформироватьДокументыВводаОстатков(ПовторныйЗапуск = Ложь) Экспорт
	...
	ОбработатьРегистрациюТСИЗемельныхУчастков();
	...
	ОбработатьДокументыСтавкаНалогаНаИмуществоПоОС();
					...
					Пока Выборка.СледующийПоЗначениюПоля("ОбъектРасчетов") Цикл
						...
						Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам 
							...
							Если ИзменилисьДанныеДляСФАванс Тогда
								...
								ДокОст.Записать(РежимЗаписиДокумента.Запись);
								СформироватьСчетФактуруНаАванс(ДокОст, Сумма, ДатаПлатежа, ДатаРасчетногоДокумента, НомерРасчетногоДокумента, НомерСФПоПорядку);
								КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
...
Процедура СформироватьСчетФактуруНаАванс(ДокОст, Сумма, ДатаПлатежа, ДатаРасчетногоДокумента, НомерРасчетногоДокумента, НомерСФПоПорядку)
	...
	Если ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиПоАвансамПоставщикам 
		...
		СтрокаСФ.СуммаНДС = Окр(Сумма * 18 / 118);
		СтрокаСФ.СтавкаНДС = Перечисления.СтавкиНДС.НДС18_118;
		СФАванс.НалогообложениеНДС = ДокОст.НалогообложениеНДС;
		...
		СФАванс.Контрагент = ДокОст.Контрагент;
...
Процедура СформироватьДокументыВводаОстатковТоварыСобственные()
	...
	|	(&СебестоимостьБезВидовЗапасов ИЛИ ТабРег.ВидЗапасов = ТабРегСебестоимость.ВидЗапасов)  И
	...
	|";
	ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
	Запрос.УстановитьПараметр("СебестоимостьБезВидовЗапасов", СебестоимостьБезВидовЗапасов);
	Запрос.Выполнить();
	...
	|ТаблицаОстатковПоПартиям.Склад КАК ГруппаФинансовогоУчета,
	...
	|	И  ТаблицаОстатковТоварыСвод.ГруппаФинансовогоУчета = ТаблицаОстатковПоПартиям.ГруппаФинансовогоУчета
	...
	|	ТаблицаОстатковСвод.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	...
	|	ТоварыЕстьСчетФактура.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	...
	|	ТаблицаОстатковТоварыСвод.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета, 
	...
	|	ТаблицаОстатковСвод.ГруппаФинансовогоУчета,
	...
	|	КОНЕЦ 					КАК ДатаСФ,
	|	ЕСТЬNULL(ДокОст.Ссылка.Помещение, ЗНАЧЕНИЕ(Справочник.СкладскиеПомещения.ПустаяСсылка)) КАК Помещение,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ТоварыПеребор.Склад КАК Справочник.Склады).ИспользоватьСкладскиеПомещения, ЛОЖЬ) КАК ИспользоватьСкладскиеПомещения,
	...
	|	ТоварыПеребор.Склад,
	|	ТоварыПеребор.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	...
	|	И ДокОст.ВидЗапасов.ГруппаФинансовогоУчета = ТоварыПеребор.ГруппаФинансовогоУчета
	...
	|	ТоварыПеребор.ГруппаФинансовогоУчета,
	|	ТоварыПеребор.НомерГТД,
	|	ЕСТЬNULL(ДокОст.Ссылка.Помещение, ЗНАЧЕНИЕ(Справочник.СкладскиеПомещения.ПустаяСсылка)), 
	|	ВЫБОР КОГДА СФПолуч.Ссылка ЕСТЬ NULL ТОГДА NULL
	|	ИНАЧЕ СФПолуч.Ссылка.ДатаСоставления 
	|	КОНЕЦ,
	|	ДокОст.Ссылка";
	...
	Если ОбновленаТаблицаПомещений Тогда
		ТаблицаОстаткиНаСкладахПоПомещениям.Свернуть("Склад, Помещение, Номенклатура, Характеристика, ГруппаФинансовогоУчета","Количество");
	КонецЕсли;
	Если ОбновленаТаблицаСерий Тогда
		ТаблицаОстаткиНаСкладахПоСериям.Свернуть("Склад, Помещение, Номенклатура, Характеристика, Серия, ГруппаФинансовогоУчета","Количество");
	КонецЕсли;
	...
	|ТоварыБезСчетаФактуры.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	...
	|	И  ТаблицаОстатковТоварыСвод.ГруппаФинансовогоУчета = ТоварыБезСчетаФактуры.ГруппаФинансовогоУчета
	...
	|	И  ТаблицаОстатковТовары.ГруппаФинансовогоУчета = ТоварыБезСчетаФактуры.ГруппаФинансовогоУчета
	...
	|	И  ТоварыБезСчетаФактуры.ГруппаФинансовогоУчета = ТаблицаОстатковПоПартиямСвернутая.ГруппаФинансовогоУчета
...
Процедура СформироватьДокументыВводаОстатковТоварыКомиссионные()
	...
	|ТабРег.ВидЗапасов.НалогообложениеНДС КАК НалогообложениеНДС,
	|ЕстьNULL(ТабРег.АналитикаУчетаНоменклатуры.Склад.ИспользоватьСкладскиеПомещения, ЛОЖЬ) КАК ИспользоватьСкладскиеПомещения,
	|Сумма(ТабРег.КоличествоОстаток) КАК Количество,
	...
	|	И (&СебестоимостьБезВидовЗапасов ИЛИ ТабРег.ВидЗапасов = ТабСтоимость.ВидЗапасов)
	|СГРУППИРОВАТЬ ПО
	|ТабРег.АналитикаУчетаНоменклатуры,
	|ТабРег.Организация ,
	|ТабРег.НомерГТД ,
	...
	|ЕстьNULL(ТабРег.АналитикаУчетаНоменклатуры.Склад.ИспользоватьСкладскиеПомещения, ЛОЖЬ),
	|ТабРег.ВидЗапасов.НалогообложениеНДС
	...
	|ТабРег.Организация КАК Организация,
	|ТабРег.НомерГТД КАК НомерГТД,
	|ЕстьNULL(ТабРегПартии.АналитикаУчетаПартий.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПоФактическомуИспользованию)) КАК НалогообложениеНДС,
	|Сумма(ТабРег.Количество) КАК Количество,
	...
	|	ТабРег.Партнер,
	|	ТабРег.Соглашение,
	|	ЕстьNULL(ТабРегПартии.АналитикаУчетаПартий.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПоФактическомуИспользованию))
	|";
...
Процедура СформироватьДокументыВводаОстатковВозвратнаяТара()
	...
	|ТабРег.Партнер КАК Партнер,
	|ЕстьNULL(ТабРегТовары.АналитикаУчетаНоменклатуры.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)) КАК Склад,
	...
	|	ЕстьNULL(ТабРегТовары.АналитикаУчетаНоменклатуры.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))";
...
Процедура ОбработатьОстаткиТоваровУПартнеров()
	...
	Запрос.Текст = "
	|ВЫБРАТЬ 
	|ЕстьNULL(ТабРегОбороты.Регистратор, ЗНАЧЕНИЕ(Документ.ПоступлениеТоваров.ПустаяСсылка)) КАК ДокументПоступление,
	|ТабРег.Номенклатура КАК Номенклатура,
	|ТабРег.Характеристика КАК Характеристика,
	...
	|УПОРЯДОЧИТЬ ПО 
	|	ЕстьNULL(ТабРегОбороты.Регистратор, ЗНАЧЕНИЕ(Документ.ПоступлениеТоваров.ПустаяСсылка)),
	|	ТабРег.Номенклатура,
	|	ТабРег.Характеристика,
	|	ТабРег.Назначение,
	|	ТабРег.Серия,
...
Процедура СформироватьДокументыВводаОстатковАналогиВПроизводстве()
	...
	|ИЗ РегистрСведений.АналогиВПроизводстве.СрезПоследних(&ГраницаОст) КАК ТабРег
							...
							Пока ВыборкаСпец.Следующий() Цикл
								...
								Пока Выборка.Следующий() Цикл
									...
									Если ЗначениеЗаполнено(Выборка.Материал) Тогда
										...
										СтрокаМатериал.Количество = Выборка.КоличествоМатериала;
									...
									Если ЗначениеЗаполнено(Выборка.Аналог) Тогда
										...
										СтрокаАналог.Количество = Выборка.КоличествоАналога;
		...
		Для Каждого СтрокаТЧ Из ДокОст.Продукция Цикл
			...
			Для Каждого СтрокаТаб Из СтрокиПродукция Цикл
				Если СтрокаТаб.КодСтроки = СтрокаТЧ.КодСтроки
					...
					Если СтрокаТаб.Количество <> СтрокаТЧ.Количество Тогда
						СтрокаТЧ.Количество = СтрокаТаб.Количество;
						СтрокаТЧ.Цена = СтрокаТаб.Цена;
						СтрокаТЧ.Сумма = Окр(СтрокаТаб.Цена * СтрокаТаб.Количество,2);
					КонецЕсли;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
Процедура СформироватьДокументыОСНМАМеждународныйУчет()
	...
	|	И ДетальнаяЗапись.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.ПринятКУчету)
	|ГДЕ СведенияНМА.Состояние <> ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.Списан)
	|ИТОГИ ПО 
	|	СведенияНМА.Организация,
	|	СведенияНМА.СчетУчета,
	|	СведенияНМА.СчетАмортизации";
...
Процедура СформироватьДокументыВводаОстатковЗаказыПереработчику()
	...
	|	ТабРег.КодСтроки,
	|	ТабРег.АналитикаУчетаНоменклатуры.Склад,
	|	ТабРег.КоличествоОстаток
	|ИЗ
	|	РегистрНакопления.ТоварыПолученныеОтПереработчика.Остатки(&ГраницаОст, Распоряжение ССЫЛКА Документ.ЗаказПереработчику) КАК ТабРег
	|ГДЕ ТабРег.КоличествоОстаток>0 
	|;
	|////////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура СформироватьДокументыВводаОстатковПоступлениеОтПереработчика()
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|		ИНАЧЕ	ВЫРАЗИТЬ(ТабРегПартии.СтоимостьОстаток / ТабРегПартии.КоличествоОстаток КАК ЧИСЛО(15,2)) 
		|	КОНЕЦ КАК Цена,
		|	ТабРег.Количество
...
Процедура СформироватьДокументыВводаОстатковПередачаПереработчику()
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	ТабРег.ВидЗапасов.Организация КАК Организация,
	|	ТабРег.ВидЗапасов.НалогообложениеНДС КАК ВидДеятельностиНДС,
	|	ВЫРАЗИТЬ(ТабРег.АналитикаУчетаНоменклатуры.Склад КАК Справочник.Партнеры) КАК Партнер,
	|	ТабРег.ВидЗапасов,
	|	ТабРег.АналитикаУчетаНоменклатуры,
	|	ТабРег.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
	|	ТабРег.АналитикаУчетаНоменклатуры.Характеристика КАК Характеристика,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСебестоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСебестоимость.СтоимостьОстаток / ТабСебестоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСебестоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСебестоимость.СтоимостьРеглОстаток / ТабСебестоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК ЦенаРегл,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСебестоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСебестоимость.СтоимостьУпрОстаток / ТабСебестоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК ЦенаУпр,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСебестоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСебестоимость.СтоимостьБезНДСОстаток / ТабСебестоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК ЦенаБезНДС,
	|	ТабРег.КоличествоОстаток КАК Количество,
	|	ТабСебестоимость.КоличествоОстаток КАК КоличествоПартии
	...
	|	И (&СебестоимостьБезВидовЗапасов ИЛИ ТабРег.ВидЗапасов = ТабСебестоимость.ВидЗапасов)
	|ГДЕ ТабРег.КоличествоОстаток > 0
	|ИТОГИ ПО
	|	ТабРег.ВидЗапасов.Организация,
	|	ТабРег.ВидЗапасов.НалогообложениеНДС,
	|	ВЫРАЗИТЬ(ТабРег.АналитикаУчетаНоменклатуры.Склад КАК Справочник.Партнеры)
	|";
	ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
	Запрос.УстановитьПараметр("СебестоимостьБезВидовЗапасов", СебестоимостьБезВидовЗапасов);
	Результат = Запрос.Выполнить();
		...
		Пока ВыборкаНалогообложение.Следующий() Цикл
			...
			Пока ВыборкаКонтрагент.Следующий() Цикл
				...
				Если НЕ ЗначениеЗаполнено(ДокОст.ВидДеятельностиНДС) Тогда
					ДокОст.ВидДеятельностиНДС = Справочники.Организации.ЗакупкаПодДеятельность(ДокОст.Организация,,ДокОст.Дата);
				КонецЕсли;
				ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиМатериаловПереданныхПереработчикам;
	...
	КонецЦикла;    //ВыборкаОрганизация
	ВывестиСообщениеСформированыДокументы("ВводОстатков", 
									Перечисления.ТипыОперацийВводаОстатков.ОстаткиМатериаловПереданныхПереработчикам);
...
Процедура СформироватьДокументыВводаОстатковМатериалыВПроизводстве()
	...
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСтоимость.СтоимостьОстаток / ТабСтоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСтоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСтоимость.СтоимостьРеглОстаток / ТабСтоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК ЦенаРегл,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСтоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСтоимость.СтоимостьУпрОстаток / ТабСтоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК ЦенаУпр,
	|	ВЫБОР КОГДА ЕстьNULL(ТабСтоимость.КоличествоОстаток, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ТабСтоимость.СтоимостьБезНДСОстаток / ТабСтоимость.КоличествоОстаток КАК Число(15,2))
	|	КОНЕЦ КАК ЦенаБезНДС,
	|	ТабРег.КоличествоОстаток КАК Количество,
	|	ТабСтоимость.КоличествоОстаток КАК КоличествоПартии
	...
	|	И ТабРег.Подразделение = ТабСтоимость.АналитикаУчетаНоменклатуры.Склад
	|ГДЕ ТабРег.КоличествоОстаток > 0
	|ИТОГИ ПО 
	|	ТабРег.Организация,
	...
	Пока ВыборкаОрганизации.Следующий() Цикл
		...
		Пока ВыборкаНалогообложение.Следующий() Цикл
			...
			Пока ВыборкаПодразделение.Следующий() Цикл
				ДокОст = Документы.ВводОстатков.СоздатьДокумент();
				ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, ВыборкаПодразделение);
				ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиМатериаловПереданныхВПроизводство;
				Выборка = ВыборкаПодразделение.Выбрать();
				Пока Выборка.Следующий() Цикл
					Стр = ДокОст.Товары.Добавить();
					ЗаполнитьЗначенияСвойств(Стр, Выборка);
	...
	КонецЦикла;
	ВывестиСообщениеСформированыДокументы("ВводОстатков", 
									Перечисления.ТипыОперацийВводаОстатков.ОстаткиМатериаловПереданныхВПроизводство);
...
Процедура ОбработатьРегистрациюТСИЗемельныхУчастков()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	|ИЗ РегистрСведений.РегистрацияЗемельныхУчастков.СрезПоследних(&ГраницаОст) КАК ТабСрез
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацияЗемельныхУчастков КАК ТабРегистрация
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацияЗемельныхУчастков КАК ТабСнятие
	...
	|	ТабСрез.ОсновноеСредство КАК ОсновноеСредство,
	|	ТабРегистрация.Регистратор КАК ДокументРегистрации
	|ИЗ РегистрСведений.РегистрацияТранспортныхСредств.СрезПоследних(&ГраницаОст) КАК ТабСрез
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацияТранспортныхСредств КАК ТабРегистрация
	|	ПО ТабРегистрация.Организация = ТабСрез.Организация И ТабРегистрация.ОсновноеСредство = ТабСрез.ОсновноеСредство
	|		И ТабРегистрация.ВидЗаписи = ЗНАЧЕНИЕ(Перечисление.ВидЗаписиОРегистрации.Регистрация)
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацияТранспортныхСредств КАК ТабСнятие
	|	ПО ТабСнятие.Организация = ТабСрез.Организация И ТабСнятие.ОсновноеСредство = ТабСрез.ОсновноеСредство
	|		И ТабСнятие.ВидЗаписи = ЗНАЧЕНИЕ(Перечисление.ВидЗаписиОРегистрации.СнятиеСРегистрационногоУчета)
	|		И ТабСнятие.Период > ТабРегистрация.Период
	|ГДЕ ТабСнятие.Регистратор ЕСТЬ NULL И ТабРегистрация.Регистратор ЕСТЬ НЕ NULL
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Для Каждого СтрокаТЧ Из ДокОст.ОС Цикл
			Если НЕ ВыборкаОбъекты.НайтиСледующий(Новый Структура("ОсновноеСредство", СтрокаТЧ.ОсновноеСредство)) Тогда
				МассивУдаляемыхСтрок.Добавить(СтрокаТЧ);
			КонецЕсли;
			ВыборкаОбъекты.Сбросить();
		КонецЦикла;
		Для Каждого СтрокаУд Из МассивУдаляемыхСтрок Цикл
			ДокОст.ОС.УдалитьСтроку(СтрокаУд);
		КонецЦикла;
		ПровестиИлиЗаписатьДокумент(ДокОст);
	КонецЦикла;
	СообщениеСвертки(НСтр("ru='Обработаны Регистрация транспортных средств и Регистрация земельных участков'")); 
...
Процедура СформироватьДокументыВводаОстатковТоварыПринятые()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ 
	|ТабТоварыПрин.Номенклатура КАК Номенклатура,
	|ТабТоварыПрин.Характеристика КАК Характеристика,
	|ТабТоварыПрин.Серия КАК Серия,
	|ТабТоварыОрг.АналитикаУчетаНоменклатуры.Склад КАК Склад,
	|ТабТоварыОрг.Организация КАК Организация,
	|ТабТоварыПрин.НомерГТД КАК НомерГТД,
	|ТабТоварыПрин.Владелец КАК Партнер,
	|ТабТоварыОрг.ВидЗапасов.Контрагент КАК Контрагент,
	|ТабТоварыПрин.Договор.ВалютаВзаиморасчетов КАК Валюта,
	|ТабТоварыПрин.Договор КАК Договор,
	|ТабТоварыПрин.Номенклатура.СтавкаНДС КАК СтавкаНДС,
	|Сумма(ТабТоварыПрин.ПолученоОстаток) КАК КоличествоПолучено,
	...
	|ИЗ РегистрНакопления.ТоварыОрганизаций.Остатки(&ГраницаОст, 
	|								ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)) КАК ТабТоварыОрг
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыПринятые.Остатки(&ГраницаОст) КАК ТабТоварыПрин
	|ПО  ТабТоварыПрин.Номенклатура = ТабТоварыОрг.АналитикаУчетаНоменклатуры.Номенклатура
	|	И ТабТоварыПрин.Характеристика = ТабТоварыОрг.АналитикаУчетаНоменклатуры.Характеристика
	|	И ТабТоварыПрин.Серия = ТабТоварыОрг.АналитикаУчетаНоменклатуры.Серия
	|	И ТабТоварыПрин.Назначение = ТабТоварыОрг.АналитикаУчетаНоменклатуры.Назначение
	|	И ТабТоварыПрин.НомерГТД = ТабТоварыОрг.НомерГТД
	|	И ТабТоварыПрин.ВидЗапасов = ТабТоварыОрг.ВидЗапасов
	|	И ТабТоварыПрин.Договор = ТабТоварыОрг.ВидЗапасов.Договор
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров.Остатки(&ГраницаОст, 
	...
	|	И (&СебестоимостьБезВидовЗапасов ИЛИ ТабТоварыОрг.ВидЗапасов = ТабСтоимость.ВидЗапасов)
	|СГРУППИРОВАТЬ ПО
	|ТабТоварыПрин.Номенклатура,
	|ТабТоварыПрин.Характеристика,
	|ТабТоварыПрин.Серия,
	|ТабТоварыОрг.АналитикаУчетаНоменклатуры.Склад,
	|ТабТоварыОрг.Организация,
	|ТабТоварыПрин.НомерГТД,
	|ТабТоварыПрин.Владелец,
	|ТабТоварыОрг.ВидЗапасов.Контрагент,
	|ТабТоварыПрин.Договор.ВалютаВзаиморасчетов,
	|ТабТоварыПрин.Договор
	|ИМЕЮЩИЕ Сумма(ТабТоварыОрг.КоличествоОстаток) > 0 И Сумма(ТабТоварыПрин.ПолученоОстаток) > 0
				...
				Пока Выборка.СледующийПоЗначениюПоля("Контрагент") Цикл
					Пока Выборка.СледующийПоЗначениюПоля("Договор") Цикл
						...
						Пока Выборка.Следующий() Цикл
							НоваяСтрока = ДокОст.Товары.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
							НоваяСтрока.Количество = Мин(Выборка.КоличествоПолучено, Выборка.КоличествоПоОрганизации);
							НоваяСтрока.КоличествоУпаковок = НоваяСтрока.Количество;
							Цена = 0; 
							ЦенаРегл = 0;
							Если Выборка.Сумма > 0 Тогда
							...
							НоваяСтрока.СуммаНДС = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(НоваяСтрока.Сумма, ТекПроцентНДС, Ложь);
...
Процедура СформироватьДокументыВводаОстатковНДСПредъявленный()
	...
	Пока ВыборкаОрганизации.Следующий() Цикл
		...
		Пока ВыборкаВидДеятельности.Следующий() Цикл
			...
			Пока ВыборкаВидЦенности.Следующий() Цикл
				...
				ДокОст.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиНДСПоПриобретеннымЦенностям;
				...
				Пока Выборка.Следующий() Цикл
					...
					Если ЗначениеЗаполнено(Выборка.ДокументПоступления) Тогда
						Стр.ДокументПоступления = ПреобразоватьСсылкуВПервичныйДокумент(Выборка.ДокументПоступления, Выборка.ДокументПоступленияДата, Выборка.ДокументПоступленияНомер);
					ИначеЕсли ЗначениеЗаполнено(Выборка.ДокументРеализации) Тогда
						Стр.ДокументРеализации = ПреобразоватьСсылкуВПервичныйДокумент(Выборка.ДокументРеализации, Выборка.ДокументРеализацииДата, Выборка.ДокументРеализацииНомер);
					КонецЕсли;
				КонецЦикла;
				ДокОст.Записать(РежимЗаписиДокумента.Запись);
				КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
	...
	Если КоличествоСформированныхДокументов > 1 Тогда
		ВывестиСообщениеСформированыДокументы("ВводОстатков", 
									Перечисления.ТипыОперацийВводаОстатков.ОстаткиНДСПоПриобретеннымЦенностям);
	КонецЕсли;

КонецПроцедуры

Функция ПреобразоватьСсылкуВПервичныйДокумент(ДокументСсылка, ДокументДата, ДокументНомер)
	...
	КонецЕсли;
	СтруктураРеквизитовШапки = Новый Структура("Организация, Контрагент, Партнер, Договор, Валюта, ДатаВходящегоДокумента, НомерВходящегоДокумента");
	...
	ПервичныйДок.Дата = ДокументДата;
	...
	Если ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика
		...
		|ГДЕ ДокументОснование = &ДокСсылка";
...
Процедура СформироватьДокументыВводаОстатковРасходыПриУСН()
	...
	|ВЫБРАТЬ 
	|	Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоТоварам) КАК ТипОперации,
	...
	|	ВЫБРАТЬ
	|	Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиРасходовУСНПоМатериалам) КАК ТипОперации,
	|	ДокументВозникновенияРасходов КАК ДокументВозникновенияРасходовСсылка,
	|	ДокументВозникновенияРасходов.Дата КАК ДокументВозникновенияРасходовДата,
	|	ДокументВозникновенияРасходов.Номер КАК ДокументВозникновенияРасходовНомер,
	|	Партия,
	...
	|	ВЫБРАТЬ
	|	Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПрочихРасходовУСН) КАК ТипОперации,
	|	ДокументВозникновенияРасходов КАК ДокументВозникновенияРасходовСсылка,
	|	ДокументВозникновенияРасходов.Дата КАК ДокументВозникновенияРасходовДата,
	|	ДокументВозникновенияРасходов.Номер КАК ДокументВозникновенияРасходовНомер,
	|	NULL,
	...
	|) КАК Свод
	|ИТОГИ ПО Организация, ТипОперации";
	ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
	КоличествоСформированныхДокументов = 1;
	Результат = Запрос.Выполнить();
	Если Результат.Пустой() Тогда
	...
	ВыборкаОрганизации = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаОрганизации.Следующий() Цикл
		ВыборкаТипОперации = ВыборкаОрганизации.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаТипОперации.Следующий() Цикл
			ДокОст = Документы.ВводОстатков.СоздатьДокумент();
			ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, ВыборкаОрганизации);
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(Выборка.ДокументВозникновенияРасходовСсылка) Тогда
					Стр.ДокументВозникновенияРасходов = ПреобразоватьСсылкуВПервичныйДокумент(Выборка.ДокументВозникновенияРасходовСсылка, Выборка.ДокументВозникновенияРасходовДата, Выборка.ДокументВозникновенияРасходовНомер);
				КонецЕсли;
	...
	КонецЦикла;
	Если КоличествоСформированныхДокументов > 1 Тогда
		ВывестиСообщениеСформированыДокументы("ВводОстатков", НСтр("ru='Остатки расходов по УСН'"));
	КонецЕсли;
...
Процедура СформироватьДокументыВводаОстатковВнеоборотныхАктивов()
	Если (Константы.ИспользоватьВнеоборотныеАктивы2_2.Получить() = Истина
		И Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить() > ЭтотОбъект.ДатаСверткиИБ)
	...
	// 1. Лизинг
	|	ВЫБОР КОГДА ПервоначальныеСведенияОС.ДоговорЛизинга <> ЗНАЧЕНИЕ(Справочник.ДоговорыЛизинга.ПустаяСсылка) ТОГДА
	|		ВЫБОР КОГДА ВЫРАЗИТЬ(МестонахождениеОС.Арендатор КАК Справочник.Организации) <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|			ИЛИ ВЫРАЗИТЬ(МестонахождениеОС.Арендатор КАК Справочник.Контрагенты) <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковПереданныхВАрендуПредметовЛизингаНаБалансе)
	|		КОГДА СостоянияОС.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКЗабалансовомуУчету) ТОГДА
	...
	|	СтоимостьОС.Арендатор КАК Контрагент,
	...
	|	ПорядокУчетаОСБУ.СтатьяРасходов КАК СтатьяРасходовБУ,
	|	ПорядокУчетаОСБУ.АналитикаРасходов КАК АналитикаРасходовБУ,
	...
	|	ПараметрыАмортизацииОСБУ.ОбъемПродукцииРаботДляВычисленияАмортизации КАК СрокИспользованияБУ,
	...
	|	ПараметрыАмортизацииОСУУ.ОбъемПродукцииРаботДляВычисленияАмортизации КАК СрокИспользованияУУ,
	...
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОСОрганизаций.СрезПоследних(&ГраницаОст) КАК СостоянияОС
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиАмортизацииОСБухгалтерскийУчет.СрезПоследних(&ГраницаОст) КАК ГрафикиАмортизацииОСБУ
	|ПО ГрафикиАмортизацииОСБУ.ОсновноеСредство = СтоимостьОС.ОсновноеСредство
	|		И (ГрафикиАмортизацииОСБУ.Организация = СтоимостьОС.Организация
	|			ИЛИ ГрафикиАмортизацииОСБУ.Организация = МестонахождениеОС.Организация)
	...
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НачислениеАмортизацииОССпециальныйКоэффициентНалоговыйУчет.СрезПоследних(&ГраницаОст) КАК СпецКоэфНУ
	|ПО СпецКоэфНУ.ОсновноеСредство = СтоимостьОС.ОсновноеСредство
	|		И (СпецКоэфНУ.Организация = СтоимостьОС.Организация
	|			ИЛИ СпецКоэфНУ.Организация = МестонахождениеОС.Организация)
	...
	|		ЛОЖЬ
	|	ИНАЧЕ ИСТИНА КОНЕЦ,
	|	ЕстьNULL(ПорядокУчетаОСБУ.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПоФактическомуИспользованию)), 
							...
							Пока Выборка.СледующийПоЗначениюПоля("НалогообложениеНДС") Цикл
								Пока Выборка.СледующийПоЗначениюПоля("Контрагент") Цикл
									...
									Пока Выборка.Следующий() Цикл
										...
										КонецЕсли;
										Стр.ПервоначальнаяСтоимостьУУ = Выборка.ПервоначальнаяСтоимостьБУ;
	...
	|	ПорядокУчетаНМАБУ.АналитикаРасходов КАК АналитикаРасходовБУ,
	|	ПорядокУчетаНМАБУ.СтатьяРасходов КАК СтатьяРасходовБУ,
	...
	|ГДЕ (ПорядокУчетаНМАУУ.Состояние <> ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.Списан)
	|	ИЛИ ПорядокУчетаНМАБУ.Состояние <> ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийНМА.Списан))
	|ИТОГИ ПО
...
Процедура СформироватьДокументыНаработкаТМЦВЭксплуатации()
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РегистрацияНаработокТМЦВЭксплуатации.Наработки КАК ТЧДок
	|	ПО ТЧДок.Ссылка = ТабРег.Регистратор И ТЧДок.ПартияТМЦВЭксплуатации.ИнвентарныйНомер = ТабСрез.ИнвентарныйНомер
	...
	Для Каждого СтрТаб Из ТабРезультат Цикл
		Если ДокОст = Неопределено Или
			...
			Если ДокОст <> Неопределено Тогда
				...
				КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
			КонецЕсли;
			ДокОст = Документы.РегистрацияНаработокТМЦВЭксплуатации.СоздатьДокумент();
	...
	ВывестиСообщениеСформированыДокументы("РегистрацияНаработокТМЦВЭксплуатации");

КонецПроцедуры

Процедура ОбработатьДокументыСтавкаНалогаНаИмуществоПоОС()
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	Ставки.Организация,
	|	Ставки.ОсновноеСредство,
	|	ЕстьNULL(Детали.Период, &ДатаОст) КАК Период,
	|	Детали.Регистратор
	|ИЗ РегистрСведений.СтавкиНалогаНаИмуществоПоОтдельнымОсновнымСредствам.СрезПоследних(&ГраницаОст) КАК Ставки
	|ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияОСОрганизаций.СрезПоследних(&ГраницаОст) КАК СостоянияОС
	|ПО Ставки.Организация = СостоянияОС.Организация
	|	И Ставки.ОсновноеСредство = СостоянияОС.ОсновноеСредство
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНалогаНаИмуществоПоОтдельнымОсновнымСредствам КАК Детали
	|ПО Ставки.Организация = Детали.Организация
	|	И Ставки.ОсновноеСредство = Детали.ОсновноеСредство
	|	И Детали.Период < &ДатаОст
	|ГДЕ СостоянияОС.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияОС.СнятоСУчета) 
	|	И СостоянияОС.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияОС.НеПринятоКУчету)
	|	И Детали.Период IS NOT NULL
	|УПОРЯДОЧИТЬ ПО
	|	Ставки.Организация,
	|	Ставки.ОсновноеСредство,
	|	ЕстьNULL(Детали.Период, &ДатаОст) УБЫВ
	|ИТОГИ ПО 
	|	Ставки.Организация,
	|	Ставки.ОсновноеСредство
	|";
	...
	Пока ВыборкаОрганизация.Следующий() Цикл
		...
		Пока ВыборкаОС.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				ДокОст = Выборка.Регистратор.ПолучитьОбъект();
				Если ДокОст = Неопределено Тогда
					Продолжить;
				КонецЕсли;
...
Процедура СформироватьДокументыВводаОстатковВзаиморасчетыЛизинг()
	...
	|ИЗ РегистрНакопления.РасчетыПоФинансовымИнструментам.Остатки(&ГраницаОст, Договор ССЫЛКА Справочник.ДоговорыЛизинга) КАК ТабРег
	|;
	|///////////////////////////////////////////
	|ВЫБРАТЬ
	|ТабРег.АналитикаУчетаПоПартнерам.Организация КАК Организация,
	|ТабРег.АналитикаУчетаПоПартнерам.Партнер КАК Партнер,
	|ТабРег.АналитикаУчетаПоПартнерам.Контрагент КАК Контрагент,
	|ВЫБОР КОГДА ТабРег.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ПустаяСсылка)
	|	ИЛИ ТабРег.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.ПустаяСсылка)
	|	ИЛИ ТабРег.ТипСуммы IS NULL ТОГДА
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.АрендныеОбязательства)
	|ИНАЧЕ
	|		ТабРег.ТипСуммы
	|КОНЕЦ КАК ТипЗадолженности,
	|ВЫБОР КОГДА ТабРег.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ЛизинговыйПлатеж) ТОГДА
	|		ЕСТЬNULL(ПорядокОтражения.СчетУчетаРасчетовСЛизингодателемЛизинговыеУслуги, ВЫРАЗИТЬ(ТабРег.Договор КАК Справочник.ДоговорыЛизинга).ГруппаФинансовогоУчета.СчетУчетаРасчетовСЛизингодателемЛизинговыеУслуги)
	|КОГДА ТабРег.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ВыкупПредметаЛизинга) ТОГДА
	|		ЕСТЬNULL(ПорядокОтражения.СчетУчетаРасчетовСЛизингодателемВыкупПредметаЛизинга, ВЫРАЗИТЬ(ТабРег.Договор КАК Справочник.ДоговорыЛизинга).ГруппаФинансовогоУчета.СчетУчетаРасчетовСЛизингодателемВыкупПредметаЛизинга)
	|КОГДА ТабРег.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ОбеспечительныйПлатеж) ТОГДА
	|		ЕСТЬNULL(ПорядокОтражения.СчетУчетаРасчетовСЛизингодателемОбеспечительныйПлатеж, ВЫРАЗИТЬ(ТабРег.Договор КАК Справочник.ДоговорыЛизинга).ГруппаФинансовогоУчета.СчетУчетаРасчетовСЛизингодателемОбеспечительныйПлатеж)
	|ИНАЧЕ 
	|		ЕСТЬNULL(ПорядокОтражения.СчетУчетаРасчетовСЛизингодателемАрендныеОбязательства, ВЫРАЗИТЬ(ТабРег.Договор КАК Справочник.ДоговорыЛизинга).ГруппаФинансовогоУчета.СчетУчетаРасчетовСЛизингодателемАрендныеОбязательства)
	|КОНЕЦ КАК СчетУчета,
	|ВЫБОР КОГДА ТабРег.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоЛизингу.ОбеспечительныйПлатеж) ТОГДА
	|	ИСТИНА
	|ИНАЧЕ ЛОЖЬ КОНЕЦ КАК СальдоДебетовое,
	|ТабРег.Договор КАК ДоговорЛизинга,
	|ВЫРАЗИТЬ(ТабРег.РасчетныйДокумент КАК Документ.СписаниеБезналичныхДенежныхСредств) КАК ДокументАванса,
	|ТабРег.СуммаОстаток КАК Сумма,
	|ТабРег.СуммаРеглОстаток КАК СуммаРегл,
	|ТабРег.СуммаУпрОстаток КАК СуммаУпр,
	|ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВводОстатковВзаиморасчетовПоДоговорамЛизинга) КАК ХозяйственнаяОперация
	|ИЗ РегистрНакопления.РасчетыПоФинансовымИнструментам.Остатки(&ГраницаОст, Договор В (ВЫБРАТЬ Договор ИЗ ОткрытыеДоговоры)) КАК ТабРег
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ПорядокОтраженияРасчетовСПартнерами КАК ПорядокОтражения
	|ПО
	|		ПорядокОтражения.Организация = ТабРег.АналитикаУчетаПоПартнерам.Организация
	|		И ПорядокОтражения.ГруппаФинансовогоУчета = ВЫРАЗИТЬ(ТабРег.Договор КАК Справочник.ДоговорыЛизинга).ГруппаФинансовогоУчета 
	|ГДЕ ТабРег.СуммаОстаток <> 0
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		Пока Выборка.СледующийПоЗначениюПоля("Партнер") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Контрагент") Цикл
				...
				Пока Выборка.Следующий() Цикл
					НоваяСтрока = ДокОст.РасчетыПоДоговорамЛизинга.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
					Если Выборка.Сумма < 0 Тогда
						НоваяСтрока.Сумма = (-1) * Выборка.Сумма;
					КонецЕсли;
					...
					Если ДокОст.РасчетыПоДоговорамЛизинга.Количество() >= КоличествоСтрокВДокументеВводаОстатков Тогда
						ДокОст.Записать(РежимЗаписиДокумента.Запись);
						КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
						ДокОст = Документы.ВводОстатковВнеоборотныхАктивов2_4.СоздатьДокумент();
						ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, Выборка, Ложь);
					КонецЕсли;
				КонецЦикла;
				Если ДокОст.РасчетыПоДоговорамЛизинга.Количество()>0 Тогда
					ДокОст.Записать(РежимЗаписиДокумента.Запись);
					КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла
	КонецЦикла;
	ВывестиСообщениеСформированыДокументы("ВводОстатковВнеоборотныхАктивов2_4", Перечисления.ХозяйственныеОперации.ВводОстатковВзаиморасчетовПоДоговорамЛизинга);
...
СебестоимостьБезВидовЗапасов = НЕ Константы.УчитыватьСебестоимостьТоваровПоВидамЗапасов.Получить();
#КонецОбласти
