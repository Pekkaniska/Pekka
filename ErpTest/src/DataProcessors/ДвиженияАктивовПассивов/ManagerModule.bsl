...
Процедура ДобавитьДвиженияДокумента(ОбъектДокумента) Экспорт
	...
	Если ТипЗнч(ОбъектДокумента.Движения) = Тип("КоллекцияДвижений") Тогда
		ТаблицыДвиженийДокумента = ВыгрузитьНаборыДвижений(ОбъектДокумента);
	КонецЕсли;
	...
	Если ЗапросДвижений <> Неопределено Тогда
		...
		Если ТипЗнч(ОбъектДокумента.Движения) = Тип("КоллекцияДвижений") Тогда
			...
			НаборПрочиеАктивыПассивы.Записывать = Истина;
		ИначеЕсли ТипЗнч(ОбъектДокумента.Движения) = Тип("Структура") Тогда
			ОбъектДокумента.Движения.Вставить("ПрочиеАктивыПассивы", ДвиженияАктивовПассивов);
		КонецЕсли;
...
//
Процедура ОбновитьДвиженияДокумента(ОбъектДокумента) Экспорт
	
	ДобавитьДвиженияДокумента(ОбъектДокумента);
	ОбъектДокумента.Движения.ПрочиеАктивыПассивы.ДополнительныеСвойства.Вставить("ФормированиеУправленческогоБаланса");
	ОбъектДокумента.Движения.ПрочиеАктивыПассивы.Записать();
...
Процедура ДобавитьТекстЗапросаРасчетыПоФинансовымИнструментам(Запрос, ТаблицаДвижений)
	...
	|ИЗ
	|	ПервичныеДвижения КАК Т
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыЛизинга КАК ДанныеДоговора
	|	ПО Т.Договор = ДанныеДоговора.Ссылка
	|ГДЕ
	|	Т.СуммаУпр <> 0
	|	И Т.Договор ССЫЛКА Справочник.ДоговорыЛизинга
	|	//&Отбор
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Период,
...
Процедура ДобавитьТекстЗапросаПрочиеРасходы(Запрос, ТаблицаДвижений)
	...
	|ВЫБРАТЬ
	|	Т.Регистратор КАК Регистратор,
	|	ВЫБОР КОГДА (Т.Сумма - Т.СуммаУпр) > 0
	|		ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	КОНЕЦ КАК ВидДвижения,
	|	Т.Период КАК Период,
	...
	|	ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|	СУММА(Т.Сумма - Т.СуммаУпр) КАК Сумма,
	|	СУММА(Т.Сумма - Т.СуммаУпр) КАК СуммаБаланса,
	|	ВЫБОР КОГДА ТИПЗНАЧЕНИЯ(Т.Регистратор) = ТИП(Документ.РасчетКурсовыхРазниц)
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.РасчетыСПартнерами)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.РасчетСебестоимости)
	|	КОНЕЦ КАК ВидИсточника,
...
Процедура ДобавитьТекстЗапросаПрочиеРасходыНезавершенногоПроизводства(Запрос, ТаблицаДвижений)
	...
	|	Т.Организация КАК Организация,
	|	Т.Подразделение КАК Подразделение,
	|	ЕСТЬNULL(Назначение.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельности,
	|	ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НезавершенноеПроизводство) КАК Статья,
	|	ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|	СУММА(Т.Стоимость) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.Стоимость ИНАЧЕ Т.Стоимость КОНЕЦ) КАК СуммаБаланса,
	...
	|	Т.Организация,
	|	Т.Подразделение,
	|	ЕСТЬNULL(Назначение.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)),
	|	Т.РасчетСебестоимости";
...
Процедура ДобавитьТекстЗапросаВременныхТаблицКонтроляБаланса(Запрос)
	...
	Для Каждого ИмяТаблицы Из Запрос.Параметры.ТаблицыДвиженийНаправлений Цикл
		...
		Если ИмяТаблицы = "ПрочиеАктивыПассивы" Тогда
			ШаблонЗапросаКонтроляБаланса = СтрЗаменить(ШаблонЗапросаКонтроляБаланса,
				...
				"ВЫБОР КОГДА Т.ВидИсточника = ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.ПустаяСсылка)
				|		ТОГДА НЕОПРЕДЕЛЕНО
		...
		Если ФормироватьФинансовыйРезультат Тогда
			Если ИмяТаблицы = "ДенежныеСредстваБезналичные"
				...
				ТекстЗапроса = СтрШаблон(ШаблонЗапросаДенежныхСредств, ИмяТаблицы);
	...
	Если ФормироватьФинансовыйРезультат Тогда
		...
		Запрос.Параметры.ТекстыЗапросов.Добавить(ТекстЗапросаКонтрольБаланса);
		Запрос.Параметры.ВсеТаблицы.Добавить("КонтрольБаланса");
		...
		КонецЕсли;
		Запрос.Параметры.ТекстыЗапросов.Добавить(ТекстЗапросаДенежныхСредств);
		Запрос.Параметры.ВсеТаблицы.Добавить("ДенежныеСредства");
	КонецЕсли;
	//-- НЕ УТ
КонецПроцедуры
//++ НЕ УТ
Процедура ДобавитьТекстЗапросаДвиженийДенежныхСредств(Запрос)
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.РасчетБаланса) КАК ВидИсточника,
	...
	Запрос.Параметры.ВсеТаблицы.Добавить("ДвиженияДенежныхСредств");
...
Процедура ДобавитьТекстЗапросаДвиженийВводаВыводаПоНаправлению(Запрос)
	...
	Запрос.Параметры.ТекстыЗапросов.Добавить(ТекстЗапросаВводВыводПоНД);
	Запрос.Параметры.ВсеТаблицы.Добавить("ИспользуемыеНаправления");
	Запрос.Параметры.ВсеТаблицы.Добавить("ДвиженияВводВыводПоНД");
...
Процедура ДобавитьТекстЗапросаДвиженийАктивовПассивов(Запрос)
	...
	Если ПолучитьФункциональнуюОпцию("ФормироватьФинансовыйРезультат") Тогда
		...
		|	Т.ВидИсточника КАК ВидИсточника,
		|	НЕОПРЕДЕЛЕНО КАК Источник,
		|	&ВидОтложенногоРасчета = ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.РасчетСебестоимости) КАК РасчетСебестоимости,
		|	Т.Регистратор КАК ДокументДвижения
		|ИЗ
...
Функция ИнициализироватьПараметрыДвижений(ПараметрыДвижений, ПараметрыОтладки = Неопределено)
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ДатаНачалаУчетаВНА2_4", Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить());
	...
	КонецЕсли;
	ЭтоРасчетСебестоимости = ВидОтложенногоРасчета = Перечисления.ВидыИсточниковУправленческогоБаланса.РасчетСебестоимости;
...
Функция ВыгрузитьНаборыДвижений(ОбъектДокумента) Экспорт
	
	ДвиженияДокумента = ОбъектДокумента.Движения;
	ЭтоНовый = ОбъектДокумента.ДополнительныеСвойства.ЭтоНовый;
	...
	Если НЕ ЭтоНовый Тогда
		Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
			ВыгрузитьРасчетыПоСрокам(ОбъектДокумента.Ссылка, ТаблицыДвижений);
...
Процедура ДобавитьПростойТекстЗапросаПоРегистру(Запрос, ТаблицаДвижений, ПараметрыЗапроса)
	...
	ИначеЕсли ТипЗнч(ТаблицаДвижений) = Тип("ТаблицаЗначений") Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&ПервичныйОтбор", ПервичныйОтбор);
		ПервичныйОтбор = "";
		Запрос.Параметры.ТекстыЗапросов.Добавить(ТекстЗапроса);
	...
	|	СУММА(ВЫБОР КОГДА &ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -Т.ИмяРесурса ИНАЧЕ Т.ИмяРесурса КОНЕЦ) КАК СуммаБаланса,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.ДвиженияБаланса) КАК ВидИсточника,
	|	&Источник КАК Источник,
	|	ЛОЖЬ КАК РасчетСебестоимости
	|
	...
	Если НЕ ПустаяСтрока(ПервичныйОтбор) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&Отбор", "//&Отбор" + ПервичныйОтбор);
	КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ОсновныеСредства", ИмяСтатьиАП);
	...
	Иначе
		...
		Запрос.Параметры.ТаблицыДвиженийНаправлений.Добавить(ИмяВременнойТаблицы);
		Запрос.Параметры.ВсеТаблицы.Добавить(ИмяВременнойТаблицы);
