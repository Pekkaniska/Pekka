	...
	Для Каждого ЭлементАрхива Из Архив.Элементы Цикл
		...
		Иначе
			...
			ИначеЕсли СтрНайти(ИмяФайла, Объект.КонфигурацияИсточник + Объект.КонфигурацияПриемник + "_ЗУП") = 1 Тогда
				...
				ПредставлениеЭтапа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Загрузка кадровых и зарплатных данных из конфигурации %1'"), КонфигурацияИсточника.Представление);
			ИначеЕсли СтрНайти(ИмяФайла, Объект.КонфигурацияИсточник + Объект.КонфигурацияПриемник + "_Н") = 1 Тогда
				ИсточникНайден     = Истина;
			...
			ИначеЕсли СтрНайти(ИмяФайла, Объект.КонфигурацияИсточник + Объект.КонфигурацияПриемник + "_С") = 1 Тогда
				ИсточникНайден     = Истина;
			...
			ИначеЕсли СтрНайти(ИмяФайла, Объект.КонфигурацияИсточник + Объект.КонфигурацияПриемник + "_О") = 1 Тогда
				ИсточникНайден     = Истина;
...
Процедура ОбработатьПроверкуСуществованияФайлаДанных(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Существует Тогда
		...
		РазмерФайлаДанных = Файл.Размер();
		Элементы.ДекорацияДатаВыгрузкиДанных.Заголовок = НСтр("ru='Дата выгрузки'") + ": " + ДатаФайлаДанных + ";";
		Элементы.ДекорацияРазмерФайла.Заголовок        = НСтр("ru='Размер файла'") + ": " + Окр(РазмерФайлаДанных/1024,0) + " Кб.";
