...
Процедура ЗагрузитьДанные(ПараметрыЗагрузки, АдресХранилища, АдресДополнительногоРезультата) Экспорт
	...
	Для Каждого ЭтапЗагрузки Из ПараметрыЗагрузки.ЭтапыЗагрузки Цикл
		...
		// Чтение протокола обмена.
		ЧтениеПротокола = Новый ЧтениеТекста(ОбработкаОбмена.ИмяФайлаПротоколаОбмена, КодировкаТекста.ANSI);
		СтрокаПротокола = ЧтениеПротокола.ПрочитатьСтроку();
		Пока СтрокаПротокола <> Неопределено Цикл
			СтрокаПротокола = СокрЛП(СтрокаПротокола);
			Если ЗначениеЗаполнено(СтрокаПротокола) Тогда
				РезультатВыполнения.ПротоколОбмена.ДобавитьСтроку(СтрокаПротокола);
			КонецЕсли;
			СтрокаПротокола = ЧтениеПротокола.ПрочитатьСтроку();
		КонецЦикла;
		
		ЧтениеПротокола.Закрыть();
