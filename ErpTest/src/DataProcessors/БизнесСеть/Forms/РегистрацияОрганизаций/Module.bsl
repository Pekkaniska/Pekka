...
Процедура ВыполнитьОтключениеОрганизаций(РежимУдаленияВСервисе)
	...
	Для Каждого ВыделеннаяСтрока Из МассивСсылок Цикл
		
		ВсеЗарегистрированы = Мин(ВсеЗарегистрированы, Элементы.Список.ДанныеСтроки(ВыделеннаяСтрока).Регистрация);
	...
	Если Не ВсеЗарегистрированы Тогда
		...
		Если РежимУдаленияВСервисе Тогда
			ТекстСообщения = НСтр("ru = 'Удаление организаций в сервисе не выполнено. Выбрана незарегистрированная организация.'");
		Иначе
...
Процедура ВыполнитьОтключениеОрганизацииОдиночное(РежимУдаленияВСервисе)
	...
	Если Не ОрганизацияЗарегистрирована Тогда
		...
		Если РежимУдаленияВСервисе Тогда
			ТекстСообщения = НСтр("ru = 'Удаление организаций в сервисе не выполнено. Организация не зарегистрирована.'");
		Иначе
...
Процедура УстановкаВидимостиЭлементовПодсистемыТорговыеПредложения()
	...
	МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Торговая площадка'"),,,, "https://portal.1c.ru/applications/60"));
