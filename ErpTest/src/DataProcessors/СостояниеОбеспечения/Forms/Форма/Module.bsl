...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ОповеститьЗаказОбИзменении И СтрокаТовары <> Неопределено И ТипЗнч(СтрокаТовары.Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
...
//++ НЕ УТ
	Если ДоступныеДокументы.Найти(Документы.ЗаказМатериаловВПроизводство) <> Неопределено 
//++ НЕ УТКА
		Или ДоступныеДокументы.Найти(Документы.ЭтапПроизводства2_2) <> Неопределено 
//-- НЕ УТКА
		Тогда
...
//++ НЕ УТКА
		ДоступнаГруппировкаТоваров = ДоступнаГруппировкаТоваров
...
//++ НЕ УТКА
	ИначеЕсли ТипЗнч(ДанныеЗаполнения.Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") И РежимГруппировкиМатериалов <> "СгруппироватьПоЗаказам" Тогда
		...
		Шаблон = НСтр("ru = 'Заказ на производство %1'");
...
//++ НЕ УТКА
	Если ТипЗнч(ДанныеЗаполнения.Заказ) = Тип("ДокументСсылка.ЗаказНаПроизводство") И РежимГруппировкиМатериалов <> "СгруппироватьПоЗаказам" Тогда
		...
		Если РежимГруппировкиМатериалов = "СгруппироватьПоИзделиям" Тогда
			Шаблон = НСтр("ru = '%1 (Строка %2: %3)'");
		Иначе
			Шаблон = НСтр("ru = '%1 (Строка %2: %3, %4)'");
		КонецЕсли;
...
Функция СтрокиСпискаДляОбработки(ПараметрыВыполненияКоманды, ПараметрыЗаполненияОбеспечения)
	...
	Если ПараметрыВыполненияКоманды.ТипКоманды = "Строка" Тогда // гиперссылка в колонке "Рекомендация".
		...
		Если ПараметрыЗаполненияОбеспечения.НеОтгружатьЧастями Тогда
			...
			ТаблицаОтбораСтрок = СтрокиГруппировкиСпискаСвязанные(ГруппировкаСтрок);
		...
		Иначе
			...
			ТаблицаОтбораСтрок.Колонки.Добавить("КодСтроки", ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(10, 0));
...
Функция ЭтоСвязанныйСдвиг(СтрокаТовары)
	...
	Возврат ЭтоСвязанныйСдвиг;
	
КонецФункции

&НаСервере
Функция СтрокиГруппировкиСпискаСвязанные(ГруппировкаСтрок)
...
Процедура ДобавитьСтрокиЗаданнойАналитики(Таблица, ТаблицаСтрок)
	...
	Таблица.Сортировать("Номенклатура, Характеристика, Склад, Подразделение, Назначение, СтрокаДобавлена УБЫВ", Новый СравнениеЗначений());
	Ключ = ОбеспечениеКлиентСервер.КлючНоменклатураХарактеристикаПодразделениеНазначениеСклад();
...
Функция ИндексыСтрокПоДокументу(ИндексыПоДокументам, СтрокаТовары)
	...
	//++ НЕ УТКА
	Если ТипЗнч(СтрокаТовары.Этап) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
		Заказ = СтрокаТовары.Этап;
	КонецЕсли;
	//-- НЕ УТКА
...
//++ НЕ УТ
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказПереработчику) Тогда
		Фильтр.Добавить(Документы.ЗаказПереработчику);
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказМатериаловВПроизводство) Тогда
...
//++ НЕ УТКА
	...
	Если ПравоДоступа(Право, Метаданные.Документы.ЭтапПроизводства2_2) Тогда
		Фильтр.Добавить(Документы.ЭтапПроизводства2_2);
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказДавальца) Тогда
		Фильтр.Добавить(Документы.ЗаказДавальца);
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказНаПроизводство)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыМатериаловСУчетомКорректировок) Или Право = "Изменение") Тогда
		
		Фильтр.Добавить(Документы.ЗаказНаПроизводство);
		
	КонецЕсли;
	
	Если ПравоДоступа(Право, Метаданные.Документы.ЗаказНаРемонт) Тогда
