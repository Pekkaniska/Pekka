...
//++ НЕ УТ
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		...
		Если Заказы.Количество() = 1 Тогда
			...
			РеквизитыЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Заказы[0].Значение, "Договор, Валюта, УслугиПоПереработке");
...
//-- НЕ УТКА
	...
	Если Заказы.Количество() = 1 Тогда
		Элементы.ЗакрытиеЗаказовДекорацияНеудача.Заголовок =
			СтрШаблон(НСтр("ru='Указанн%1 %2 обработан. Нажмите ""Закрыть"" для завершения.'"), ОкончаниеИП, ТекстИП);
		
		Элементы.ВсеЗаказыЗакрытыДекорация.Заголовок =
			СтрШаблон(НСтр("ru='Указанн%1 %2 уже закрыт или не требует закрытия. Нажмите ""Закрыть"" для завершения.'"),
			ОкончаниеИП, ТекстИП);
	Иначе
		Элементы.ЗакрытиеЗаказовДекорацияНеудача.Заголовок =
			СтрШаблон(НСтр("ru='Все %1 обработаны. Нажмите ""Закрыть"" для завершения.'"), ТекстИП);
		
		Элементы.ВсеЗаказыЗакрытыДекорация.Заголовок =
			СтрШаблон(НСтр("ru='Все %1 уже закрыты или не требуют закрытия. Нажмите ""Закрыть"" для завершения.'"), ТекстИП);
	КонецЕсли;
...
Процедура ЗаполнитьОтклоненияПоЗаказам(ТаблицаТоваровПриход, ТаблицаТоваровРасход)
	...
	ИначеЕсли ТипЗнч(Заказы[0].Значение) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление") Тогда
		...
		//++ НЕ УТКА
		ИспользуетсяСтатусЗакрыт = ИспользуетсяСтатусЗакрыт Или ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеРемонтами");
		//-- НЕ УТКА
