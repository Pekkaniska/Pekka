...
Процедура ЗаполнитьМесяцИсправления() Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		ТекМесяц = КонецМесяца(ВыборкаДетальныеЗаписи.Период);
		МесяцыКонтроля = Новый Массив;
		
		Пока ТекМесяц <= КонецМесяца(ТекущаяДатаСеанса()) Цикл
...
Процедура СформироватьНовыеРезервы(ПоНовымНастройкам = Ложь) Экспорт
	...
	Для Каждого Организация Из Организации Цикл
		...
		ДокументОбъект.ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.Вставить("МенеджерВременныхТаблиц", Новый МенеджерВременныхТаблиц);
		...
		ДокументОбъект.ДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаТоварыОрганизаций", ТаблицаТоварыОрганизаций);
		
		НачатьТранзакцию();
		Попытка
			
			Отказ = Ложь;
			ЗапасыСервер.ПодготовитьЗаписьТоваровОрганизаций(ДокументОбъект, РежимЗаписиДокумента.Проведение, ПараметрыЗаполненияВидовЗапасов);
			ЗапасыСервер.СформироватьРезервыПоТоварамОрганизаций(ДокументОбъект, Отказ, ПараметрыЗаполненияВидовЗапасов);
	...
	Пока ВыборкаОрганизацииСОтрицательнымОстатком.Следующий() Цикл
		...
		Попытка
			...
			Для Каждого ТипЗачета Из ТипыЗачетаОстатков Цикл
				ТаблицаОстатковКРаспределению = 
					РезультатЗапроса[ТипЗачета.НомерПакетаОстаткиКРаспределению].Выгрузить();
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				ПодробноеПредставлениеОшибки);
			ТекстСообщения = НСтр(
				"ru = 'Не удалось сформировать исправления развернутого сальдо. Подробности в журнале регистрации.'");
			ВызватьИсключение ТекстСообщения;
...
Процедура ЗаполнитьРаспоряженияНаОформлениеОприходований() Экспорт
	...
	|	И КлючиАналитикиУчетаНоменклатуры.Склад ССЫЛКА Справочник.Склады
...
Функция ЗапросИсточникиПокрытияРазвернутогоСальдо(МенеджерВременныхТаблиц)
	...
	|	ТоварыПринятыеОстатки.ПолученоОстаток КАК КоличествоПолученоОстаток
	|ПОМЕСТИТЬ ТоварыПринятыеОстатки
	|ИЗ
	|	РегистрНакопления.ТоварыПринятые.Остатки(
	|			&ГраницаИсправления,
	|			(ВидЗапасов, Владелец, Договор, Назначение, Номенклатура, НомерГТД, Серия, Характеристика) В
	|				(ВЫБРАТЬ
	|					ОстаткиИсточников.ВидЗапасов,
	|					ОстаткиИсточников.ВладелецТовара,
	|					ОстаткиИсточников.Договор,
	|					ОстаткиИсточников.Назначение,
	|					ОстаткиИсточников.Номенклатура,
	|					ОстаткиИсточников.НомерГТД,
	|					ОстаткиИсточников.Серия,
	|					ОстаткиИсточников.Характеристика
	|				ИЗ
	|					ОстаткиИсточников)) КАК ТоварыПринятыеОстатки
	|ГДЕ
	|	ТоварыПринятыеОстатки.ПолученоОстаток > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТоварыПринятыеОстатки.НомерГТД КАК НомерГТД,
	|	ТоварыПринятыеОстатки.КоличествоПолученоОстаток КАК Количество,
	|	ЛОЖЬ КАК ТребуетсяРезерв
	|ИЗ
	|	ТоварыПринятыеОстатки КАК ТоварыПринятыеОстатки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидыЗапасов
	|		ПО ТоварыПринятыеОстатки.ВидЗапасов = ВидыЗапасов.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ОтрицательныеОстатки.Серия КАК Серия,
	|	ОтрицательныеОстатки.Назначение КАК Назначение,
	|	ОтрицательныеОстатки.ВидЗапасов КАК ВидЗапасов,
	|	ОтрицательныеОстатки.Организация КАК Организация,
	|	ОтрицательныеОстатки.ВладелецТовара КАК Владелец,
	|	ОтрицательныеОстатки.НомерГТД КАК НомерГТД,
	|	ОтрицательныеОстатки.КоличествоОстаток КАК Количество
	|ИЗ
	|	ОтрицательныеОстатки КАК ОтрицательныеОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ВидыЗапасов
	|		ПО ОтрицательныеОстатки.ВидЗапасов = ВидыЗапасов.Ссылка
	|ГДЕ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТоварыПринятыеОстатки";
...
Функция ТипыЗачетаОстатковРазвернутогоСальдо()
	...
	ТипЗачета.НомерПакетаОстаткиКРаспределению = 7;
	ТипЗачета.НомерПакетаОтрицательныеОстатки = 8;
	ТипЗачета.ИмяРеквизитаКоличествоВТЧ = "Количество";
	ТипЗачета.СтруктураПоиска = "АналитикаУчетаНоменклатуры, Организация, Владелец";
	
	ТипЗачета = ТипыЗачетаОстатков.Добавить();
	ТипЗачета.ИмяТипаЗачета = "ПоТоварамПринятым";
	ТипЗачета.НомерПакетаОстаткиКРаспределению = 9;
	ТипЗачета.НомерПакетаОтрицательныеОстатки = 10;
	ТипЗачета.ИмяРеквизитаКоличествоВТЧ = "КоличествоПолучено";
	ТипЗачета.СтруктураПоиска = "Номенклатура, Характеристика, Серия, Назначение, Организация, Владелец";
