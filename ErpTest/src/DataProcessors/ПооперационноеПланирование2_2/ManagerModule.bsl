...
Процедура ЗаполнитьПараметрыПланирования(ПараметрыПланирования, Отказ)
	...
	Если Не Отказ Тогда
		...
		ИнициализироватьТаблицуОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц);
...
Процедура УдалитьОперацииНеактуальныхЭтапов(ПараметрыПланирования)
	...
	|	И ОчередьПроизводственныхОпераций.Этап ЕСТЬ NULL
...
Процедура ЗафиксироватьСозданныеПартииОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц)
	...
	|				ТОГДА ВЫБОР
	|						КОГДА (ВЫРАЗИТЬ(втФиксированныеПартии.Создано / втФиксированныеПартии.Коэффициент КАК ЧИСЛО(10, 0))) >= втФиксированныеПартии.Создано / втФиксированныеПартии.Коэффициент
	|							ТОГДА ВЫРАЗИТЬ(втФиксированныеПартии.Создано / втФиксированныеПартии.Коэффициент КАК ЧИСЛО(10, 0))
	|						ИНАЧЕ (ВЫРАЗИТЬ(втФиксированныеПартии.Создано / втФиксированныеПартии.Коэффициент КАК ЧИСЛО(10, 0))) + 1
	|					КОНЕЦ * втФиксированныеПартии.Коэффициент - (втФиксированныеПартии.Выполняется + втФиксированныеПартии.Выполнено + втФиксированныеПартии.Пропущено - втФиксированныеПартии.ТребуетПовторения)
	|			ИНАЧЕ 0
	...
	|	ВЫБОР
	|		КОГДА (ВЫРАЗИТЬ(ПартииОпераций.Создано / ПартииОпераций.Коэффициент КАК ЧИСЛО(10, 0))) >= ПартииОпераций.Создано / ПартииОпераций.Коэффициент
	|			ТОГДА ВЫРАЗИТЬ(ПартииОпераций.Создано / ПартииОпераций.Коэффициент КАК ЧИСЛО(10, 0))
	|		ИНАЧЕ (ВЫРАЗИТЬ(ПартииОпераций.Создано / ПартииОпераций.Коэффициент КАК ЧИСЛО(10, 0))) + 1
	|	КОНЕЦ * ПартииОпераций.Коэффициент КАК Количество,
	...
	НомераПартий = РезультатЗапроса[РезультатЗапроса.ВГраница() - 2].Выгрузить();
	...
	ИнтервалыПартий = РезультатЗапроса[РезультатЗапроса.ВГраница()].Выгрузить();
...
Процедура ИнициализироватьТаблицуОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ВЫРАЗИТЬ(Очередь.Операция.РабочийЦентр КАК Справочник.ВидыРабочихЦентров).ПараллельнаяЗагрузка
	|						ТОГДА Очередь.Операция.ВремяШтучное + Очередь.Операция.ВремяПЗ
	|					ИНАЧЕ Очередь.Операция.ВремяШтучное * (Очередь.Запланировано - Очередь.Выполнено - Очередь.Выполняется - Очередь.Пропущено + Очередь.ТребуетПовторения) + Очередь.Операция.ВремяПЗ
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВЫРАЗИТЬ(Очередь.Операция.РабочийЦентр КАК Справочник.РабочиеЦентры).ВидРабочегоЦентра.ПараллельнаяЗагрузка
	|					ТОГДА Очередь.Операция.ВремяШтучное + Очередь.Операция.ВремяПЗ
	|				ИНАЧЕ Очередь.Операция.ВремяШтучное * (Очередь.Запланировано - Очередь.Выполнено - Очередь.Выполняется - Очередь.Пропущено + Очередь.ТребуетПовторения) + Очередь.Операция.ВремяПЗ
	|			КОНЕЦ
	|	КОНЕЦ * ВЫБОР
	|		КОГДА Очередь.ВремяВыполненияЕдИзм = ЗНАЧЕНИЕ(Перечисление.ЕдиницыИзмеренияВремени.Сутки)
	|			ТОГДА 86400
	|		КОГДА Очередь.ВремяВыполненияЕдИзм = ЗНАЧЕНИЕ(Перечисление.ЕдиницыИзмеренияВремени.Час)
	|			ТОГДА 3600
	|		КОГДА Очередь.ВремяВыполненияЕдИзм = ЗНАЧЕНИЕ(Перечисление.ЕдиницыИзмеренияВремени.Минута)
	|			ТОГДА 60
	|		ИНАЧЕ 1
...
Процедура МодифицироватьТаблицуОпераций(ПараметрыПланирования)
	...
	ПараметрыПланирования.ТаблицаОпераций.Индексы.Добавить("Этап, ИдентификаторОперации"); 
	ПараметрыПланирования.ТаблицаОпераций.Индексы.Добавить("ВидРабочегоЦентра, ПриоритетЭтапа, Размещать, Размещена, ЗависитОтПредшествующих"); 
	ПараметрыПланирования.ТаблицаОпераций.Индексы.Добавить("Этап"); 
...
Процедура ИзменитьГрафикиРаботыОборудования(ПараметрыПланирования, Знач ДанныеМодели)
	...
	ТаблицаГрафикиРаботы = ДанныеМодели.ГрафикиРаботы.Выгрузить();
...
Процедура ПроверитьДанныеПредшествующихПартий(Операция, ПартияОперации, ИзменятьДату = Истина)
	...
	ПредшествующиеОперации = Операция.ДанныеПредшествующихПартий.Скопировать(, "Этап, Операция, ИдентификаторОперации");
	...
	Если НЕ ПроблемаЗафиксирована Тогда
		...
		Если ЗначениеЗаполнено(Пользователь) Тогда
			...
			Текст = Текст + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '(пользователь: %1 )'"), ПользовательПредставление);
		КонецЕсли;
		НоваяСтрока.Текст = Текст;
...
Процедура ЗафиксироватьОшибкуНеЗаданоВремяОперации(ПараметрыПланирования, Знач Операция)
	...
	Если Не ПроблемаЗафиксирована Тогда
		...
		НоваяСтрока.Свойства = Новый Структура("Этап", Операция.Этап);
		НоваяСтрока.Ключ = Ключ;
