...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = РаботаСНоменклатуройКлиент.ОписаниеОповещенийПодсистемы().ЗагрузкаНоменклатуры Тогда	
				
		ПроставитьПризнакЗагрузки(Элементы.Список.ТекущийРодитель, Параметр.СозданныеОбъекты);
...
Процедура ЗагрузитьНоменклатуру(ИдентификаторыНоменклатуры, ЗакрытьФормуПослеСоздания = Ложь)
	...
	РаботаСНоменклатуройКлиент.ЗагрузитьНоменклатуруИХарактеристики(Оповещение, ИдентификаторыНоменклатуры, ЭтотОбъект);
...
Процедура ВывестиНоменклатуру(ЭлементыСписка, ПереченьНоменклатуры, ЭтоЗапросПорцииДанных = Ложь)
	...
	Для каждого ЭлементКоллекции Из ПереченьНоменклатуры Цикл
		...
		Если РежимПросмотраСписка = "Группировка" 
			...
			ТехническаяСтрока.Наименование = "РаботаСНоменклатурой_ТехническаяСтрока";
		КонецЕсли;
...
Процедура ВыбратьНоменклатуруИзСписка()
	...
	ДанныеНоменклатуры.НаименованиеНоменклатуры    = ТекущиеДанные.Наименование;
...
&НаСервере
Процедура ПроставитьПризнакЗагрузки(Знач ТекущийРодитель, Знач Идентификаторы)
	
	Если ТипЗнч(Идентификаторы) <> Тип("Массив") Тогда
		Идентификаторы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Идентификаторы);
	КонецЕсли;
	
	// Получение режима загрузки характеристик.
	
	НоменклатураДляРежимаЗагрузки = Новый Массив;
	
	Для каждого ЗагруженныйЭлемент Из Идентификаторы Цикл
		Если НоменклатураДляРежимаЗагрузки.Найти(ЗагруженныйЭлемент.Номенклатура) = Неопределено Тогда
			НоменклатураДляРежимаЗагрузки.Добавить(ЗагруженныйЭлемент.Номенклатура);
		КонецЕсли;
	КонецЦикла;
	
	РежимыЗагрузки = РаботаСНоменклатурой.РежимыЗагрузкиИКоличествоХарактеристик(НоменклатураДляРежимаЗагрузки);
	
	// Установка режима загрузки характеристик.
	
	Для Каждого ЗагруженныйЭлемент Из Идентификаторы Цикл
		
		ЗагруженныйЭлемент.Вставить("РежимЗагрузкиХарактеристик");
		ЗагруженныйЭлемент.Вставить("КоличествоХарактеристик", 0);
		
		СтрокаДанных = РежимыЗагрузки.Найти(ЗагруженныйЭлемент.ИдентификаторНоменклатуры, "ИдентификаторНоменклатуры");
		
		Если СтрокаДанных <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ЗагруженныйЭлемент, СтрокаДанных);
		КонецЕсли;
	...
	Если ТекущийРодитель = Неопределено Тогда
		...
		// Режимы просмотра: Линейный, БезХарактеристик, Группировка с позицией в корне.
		
		ПроставитьПризнакЗагрузкиЭлементам(Идентификаторы, Список.ПолучитьЭлементы());
		
	Иначе
		
		// Режим просмотра Группировка с развернутыми характеристиками.
		
		СтрокаТекущийРодитель = Список.НайтиПоИдентификатору(ТекущийРодитель);
		
		Если СтрокаТекущийРодитель = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		ЭлементыРодителя = СтрокаТекущийРодитель.ПолучитьЭлементы();
		
		ПроставитьПризнакЗагрузкиЭлементам(Идентификаторы, ЭлементыРодителя);
		ПроставитьПризнакЗагрузкиЭлементам(Идентификаторы, Список.ПолучитьЭлементы());
...
&НаСервере
Процедура ПроставитьПризнакЗагрузкиЭлементам(Идентификаторы, ЭлементыСписка)
	
	Для Каждого ЗагруженныйЭлемент Из Идентификаторы Цикл
		
		Если ЗагруженныйЭлемент.РежимЗагрузкиХарактеристик 
			= Перечисления.РежимыЗагрузкиХарактеристикРаботаСНоменклатурой.НеЗагружатьХарактеристики Тогда
			
			// Если характеристики не поддерживаются.
			
			Для каждого СтрокаСписка Из ЭлементыСписка Цикл
				Если СтрокаСписка.ИдентификаторНоменклатуры = ЗагруженныйЭлемент.ИдентификаторНоменклатуры Тогда
					Если ЗначениеЗаполнено(СтрокаСписка.ИдентификаторХарактеристики) Тогда
						СтрокаСписка.Загружена = 3;
					Иначе
						СтрокаСписка.Загружена = 1;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		...
		// Если характеристики поддерживаются.
		
		Для каждого СтрокаСписка Из ЭлементыСписка Цикл
			Если ЭтоСтрокаХарактеристики(ЗагруженныйЭлемент) И ЭтоСтрокаХарактеристики(СтрокаСписка) Тогда
				
				// Если была загружена номенклатура с характеристикой и встретилась номенклатура с характеристикой.
				
				Если ЗагруженныйЭлемент.ИдентификаторХарактеристики = СтрокаСписка.ИдентификаторХарактеристики Тогда
					СтрокаСписка.Загружена = 1;
				КонецЕсли;
				
			ИначеЕсли ЭтоСтрокаХарактеристики(ЗагруженныйЭлемент) И ЭтоСтрокаНоменклатуры(СтрокаСписка) Тогда 
				
				// Если была загружена номенклатура с характеристикой и встретилась номенклатура.
				
				Если ЗагруженныйЭлемент.ИдентификаторНоменклатуры = СтрокаСписка.ИдентификаторНоменклатуры Тогда
					Если ЗагруженныйЭлемент.КоличествоХарактеристик = СтрокаСписка.КоличествоХарактеристик Тогда
						СтрокаСписка.Загружена = 1; // загружены все характеристики
					Иначе
						СтрокаСписка.Загружена = 2;	// загружены не все характеристики
					КонецЕсли;
				КонецЕсли;
				
			ИначеЕсли ЭтоСтрокаНоменклатуры(ЗагруженныйЭлемент) И ЭтоСтрокаНоменклатуры(СтрокаСписка) Тогда
				
				// Если была загружена только номенклатура и встретилась номенклатура.
...
&НаСервере
Функция ЭтоСтрокаХарактеристики(СтрокаДанных)
	
	Возврат ЗначениеЗаполнено(СтрокаДанных.ИдентификаторХарактеристики);
	
КонецФункции

&НаСервере
Функция ЭтоСтрокаНоменклатуры(СтрокаДанных)
	
	Возврат ЗначениеЗаполнено(СтрокаДанных.ИдентификаторНоменклатуры)
		И Не ЗначениеЗаполнено(СтрокаДанных.ИдентификаторХарактеристики);
...
Процедура УстановитьВидимостьДоступность()
	...
	ЕстьВозможностьЗагрузкиИПривязки = ТекущиеДанные <> Неопределено И ТекущиеДанные.Загружена <> 1;
	...
	Если ЭтоПривязкаНоменклатуры  Тогда
		...
		// В режиме привязкки действует правила как и при загрузке.
...
Процедура ПолучитьХарактеристики(ТекущиеДанные)
	...
	РаботаСНоменклатуройКлиент.ПолучитьДанныеХарактеристикСервиса(Оповещение, ПараметрыМетода, ЭтотОбъект, 
		ИдентификаторЗаданияРаботыСНоменклатурой, Элементы.ДекорацияКореньКатегорий);
...
Процедура СписокПриСменеТекущегоРодителя(Элемент)
	...
	Если Родитель.КоличествоХарактеристик > 0 Тогда
