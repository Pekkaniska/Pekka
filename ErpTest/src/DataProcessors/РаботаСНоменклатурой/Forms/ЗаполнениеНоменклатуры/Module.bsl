...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоПервичноеЗаполнение             = НоменклатураСсылка.Пустая();
...
Процедура ОбновлятьВсеАвтоматически(Команда)
	...
	РаботаСНоменклатуройКлиент.ПоказатьВопросПриВключенииАвтообновленияНоменклатуры(НоменклатураСсылка, ИдентификаторНоменклатуры, ЭтотОбъект);
...
Функция ПоместитьДанныеВХранилище()
	...
	Если ЗначениеЗаполнено(ДанныеИнтерактивногоЗаполнения) Тогда
		РаботаСНоменклатурой.УстановитьРежимОбновленияНоменклатуры(НоменклатураСсылка, ИдентификаторНоменклатуры, Ложь);
	КонецЕсли;
...
Процедура ЗаполнитьНоменклатуру()
	...
	Попытка
		...
		РаботаСНоменклатуройПереопределяемый.ЗаполнитьНоменклатуру(
			НоменклатураСсылка, ОбновляемыеРеквизиты.Выгрузить(Новый Структура("Пометка, ЭтоДополнительныйРеквизит", Истина, Ложь)));

		Если ВедетсяУчетВидовНоменклатуры Тогда
			РаботаСНоменклатуройПереопределяемый.ЗаполнитьДополнительныеРеквизитыНоменклатуры(
				НоменклатураСсылка, ОбновляемыеРеквизиты.Выгрузить(Новый Структура("Пометка, ЭтоДополнительныйРеквизит", Истина, Истина)));
		КонецЕсли;	
...
Процедура ПолучитьДанныеПоНоменклатуре()
	...
	РаботаСНоменклатуройКлиент.ПолучитьДанныеНоменклатурыСервиса(
		ПолучитьНоменклатуруЗавершение, Идентификатор, ЭтотОбъект, ИдентификаторЗадания);
...
Процедура СформироватьТаблицуДополнительныхРеквизитов(СтрокаДанных)
	...
	Для каждого ТекущийРеквизит Из СтрокаДанных.ДополнительныеРеквизиты Цикл
		...
		НоваяСтрока.НеСопоставлен                = ?(ВидНоменклатурыБудетСоздан, Ложь, Истина);
		НоваяСтрока.Пометка                      = ?(ВидНоменклатурыБудетСоздан, Истина, Ложь);
...
Функция ТекстНеСопоставленногоВида(КатегорииОтличаются = Ложь)
	...
	Иначе
		...
		Если КатегорииОтличаются Тогда
			...
			МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(ПредставлениеВидаНоменклатуры,,,,"ОткрытьВидНоменклатуры"));
			МассивПодстрок.Добавить(" " + НСтр("ru = 'сопоставлен с другой категорией сервиса 1С:Номенклатура'") + ".");
...
Процедура СформироватьПодсказкуКВидуНоменклатуры(Данные, РеквизитХраненияПодсказки)
	...
	РаботаСНоменклатуройСлужебныйКлиентСервер.СформироватьПодсказкуКВидуНоменклатуры(Данные, РеквизитХраненияПодсказки);
