...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоПервоначальноеЗаполнение = ВидНоменклатуры.Пустая();
...
Процедура ОбновлятьВсеАвтоматически(Команда)
	...
	РаботаСНоменклатуройКлиент.ПоказатьВопросПриВключенииАвтообновленияВидаНоменклатуры(
		ВидНоменклатуры, ИдентификаторыКатегорий.ВыгрузитьЗначения(), ЭтотОбъект);
...
Процедура Готово(Команда)
	...
	Если ЗначениеЗаполнено(АдресДанныхИнтерактивногоЗаполнения) Тогда
		...
		Если Не ЭтоПервоначальноеЗаполнение Тогда
...
Процедура ЗакрытьСРасширеннымиПараметрами()
	...
	ДанныеДляСозданияРеквизитов = 
		?(ДополнительныеРеквизиты.Количество() > 0 И ЭтоПервоначальноеЗаполнение, АдресДанныхКатегории, АдресДанныхКатегории);
...
Процедура ПолучитьДанныеПоКатегорииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьТаблицыФормы(Результат.АдресРезультата);
...
Процедура СоздатьРеквизитыАвтоматическиНаСервере()
	...
	ЗаполнитьСоответствияРеквизитов(ОбъединенныеРеквизиты);	
...
&НаСервере
Процедура ЗаполнитьСоответствияРеквизитов(ОбъединенныеРеквизиты)
	...
	РаботаСНоменклатурой.ЗаполнитьСоответствиеДополнительныхРеквизитовВидаНоменклатуры(
		ОбъединенныеРеквизиты.ДополнительныеРеквизиты, СоответствиеРеквизитовИЗначений);

	РаботаСНоменклатурой.ЗаполнитьСоответствиеДополнительныхРеквизитовВидаНоменклатуры(
		ОбъединенныеРеквизиты.Характеристики.ДополнительныеРеквизиты, СоответствиеРеквизитовИЗначений);
...
Процедура НастроитьФормуПриСоздании()
	...
	Элементы.ДополнительныеРеквизитыСоздатьРеквизит.Видимость                 = Не ЭтоПервоначальноеЗаполнение;
	Элементы.ДополнительныеРеквизитыСоздатьРеквизитыАвтоматически.Видимость   = Не ЭтоПервоначальноеЗаполнение;
	Элементы.ДополнительныеРеквизитыЗаполнитьРеквизитыАвтоматически.Видимость = Не ЭтоПервоначальноеЗаполнение;
	Элементы.ДополнительныеРеквизитыРеквизитВидаНоменклатуры.Видимость        = Не ЭтоПервоначальноеЗаполнение;
	Элементы.ДополнительныеРеквизитыСопоставленоЗначений.Видимость            = Не ЭтоПервоначальноеЗаполнение;
	Элементы.РежимЗагрузкиХарактеристик.Видимость                             = Не ЭтоПервоначальноеЗаполнение;
...
Процедура ЗаписатьСоответствиеРеквизитов()
	...
	РеквизитыДляЗаписи = Новый Массив;
	
	Для каждого ЭлементКоллекции Из ДополнительныеРеквизиты Цикл
		Если ЗначениеЗаполнено(ЭлементКоллекции.РеквизитВидаНоменклатуры) Тогда
			РеквизитыДляЗаписи.Добавить(ЭлементКоллекции);
		КонецЕсли;
	КонецЦикла;	
...
Процедура ПолучитьДанныеПоКатегории()
	...
	РаботаСНоменклатуройКлиент.ПолучитьДанныеКатегорийСервиса(
		ПолучитьНоменклатуруЗавершение, ИдентификаторыКатегорий.ВыгрузитьЗначения(), ЭтотОбъект, ИдентификаторЗадания);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТаблицыФормы(АдресРезультата)
	
	Если НЕ ЭтоАдресВременногоХранилища(АдресРезультата) Тогда
		Возврат;
	КонецЕсли;
	
	АдресДанныхКатегории = АдресРезультата;
	...
	ЗаполнитьСоответствияРеквизитовИЗначений(ОбъединенныеДанныеПоКатегориям);
...
Процедура НастроитьВидимостьРежимаЗагрузки()
	...
	Если ИспользуютсяХарактеристикиВСервисе Тогда
		Если ЭтоПервоначальноеЗаполнение Тогда
...
&НаСервере
Процедура ЗаполнитьСоответствияРеквизитовИЗначений(ОбъединенныеДанныеПоКатегориям)
	
	СоответствиеРеквизитовИЗначений = РаботаСНоменклатурой.СоответствиеРеквизитовИЗначенийПоВидуНоменклатуры(ВидНоменклатуры);
...
Процедура УправлениеВидимостьюДоступностью(Форма)
	...
	Элементы.Далее.Видимость                           = ЭтоСтраницаОсновныхРеквизитов;
	Элементы.Далее.КнопкаПоУмолчанию                   = ЭтоСтраницаОсновныхРеквизитов;

	Элементы.Назад.Видимость                           = ЭтоСтраницаДополнительныхРеквизитов И Не ЭтоОсобыйРежим;
	Элементы.Готово.КнопкаПоУмолчанию                  = ЭтоСтраницаДополнительныхРеквизитов;
