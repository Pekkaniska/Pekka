...
Процедура СерииНомерПриИзменении(Элемент)
	...
	ПодобратьСерию(Элемент, "Номер","ГоденДо,НомерКиЗГИСМ,ДатаПроизводства,ПроизводительЕГАИС,Справка2ЕГАИС,
		|ПроизводительВЕТИС,ЗаписьСкладскогоЖурналаВЕТИС,ИдентификаторПартииВЕТИС");
	...
	Если Не ОтменаРедактирования Тогда
		...
		Если Не НастройкиИспользованияСерий.ИспользоватьКоличествоСерии Тогда
			...
			Если ЗначениеЗаполнено(ТекущиеДанные.Номер)
				...
				Или ЗначениеЗаполнено(ТекущиеДанные.ЗаписьСкладскогоЖурналаВЕТИС) Тогда
...
Процедура СерииПроизводительВЕТИСПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ПроизводительВЕТИС) Тогда
		ПодобратьСерию(Элемент,"ПроизводительВЕТИС","ГоденДо,Номер,НомерКиЗГИСМ,ДатаПроизводства,ПроизводительЕГАИС,
			|Справка2ЕГАИС,ЗаписьСкладскогоЖурналаВЕТИС,ИдентификаторПартииВЕТИС");
	Иначе
		ТекущиеДанные.Серия = Неопределено;
	КонецЕсли;
...
Процедура СерииИдентификаторПартииВЕТИСПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторПартииВЕТИС) Тогда
		ПодобратьСерию(Элемент,"ИдентификаторПартииВЕТИС","ГоденДо,Номер,НомерКиЗГИСМ,ДатаПроизводства,ПроизводительЕГАИС,
			|Справка2ЕГАИС,ПроизводительВЕТИС,ЗаписьСкладскогоЖурналаВЕТИС");
	Иначе
		ТекущиеДанные.Серия = Неопределено;
	КонецЕсли;
КонецПроцедуры
...
Процедура СерииЗаписьСкладскогоЖурналаВЕТИСПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ЗаписьСкладскогоЖурналаВЕТИС) Тогда
		ПодобратьСерию(Элемент,"ЗаписьСкладскогоЖурналаВЕТИС","ГоденДо,Номер,НомерКиЗГИСМ,ДатаПроизводства,ПроизводительЕГАИС,
			|Справка2ЕГАИС,ПроизводительВЕТИС,ИдентификаторПартииВЕТИС");
	Иначе
		ТекущиеДанные.Серия = Неопределено;
	КонецЕсли;
	...
	ИначеЕсли СписокВыбора.Количество() = 2 Тогда
		Если СокрЛП(СписокВыбора[0].Значение.Номер) <> СокрЛП(СписокВыбора[1].Значение.Номер)
			...
			Или (СписокВыбора[0].Значение.ИдентификаторПартииВЕТИС <> СписокВыбора[1].Значение.ИдентификаторПартииВЕТИС
				И ЗначениеЗаполнено(СписокВыбора[0].Значение.ИдентификаторПартииВЕТИС)) Тогда
