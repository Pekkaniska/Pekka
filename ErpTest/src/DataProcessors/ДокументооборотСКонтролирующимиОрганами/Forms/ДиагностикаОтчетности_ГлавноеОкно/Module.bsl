...
Функция ПоместитьИнформациюОбУчетнойЗаписиВОтчет()
	...
	Если ДополнительныеРеквизитыУчетнойЗаписи <> Неопределено Тогда
		...
		РегНомерФСС 				= ДополнительныеРеквизитыУчетнойЗаписи.РегНомерФСС;
	КонецЕсли;
	...
	РезультатыДиагностики.Отчет.Абонент.Вставить("РегНомерФСС", РегНомерФСС);
...
Функция ПолучитьИнформациюФайлаАвтонастройки()
	...
	Если ДополнительныеРеквизитыУчетнойЗаписи <> Неопределено Тогда 
		...
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("РегНомерФСС", ДополнительныеРеквизитыУчетнойЗаписи.РегНомерФСС);
	...
	Иначе
		...
		ДанныеАвтонастройки.СведенияРегистрации.Вставить("РегНомерФСС", "");
...
Функция ПолучитьСписокИзменившихсяРеквизитовПодключенияК1СОтчетности(ОрганизацияСсылка, УчетнаяЗаписьОбменаСсылка)
	...
	Если ДополнительныеРеквизитыУчетнойЗаписи <> Неопределено Тогда
		...
		РегНомерФССИсходный 				= ДополнительныеРеквизитыУчетнойЗаписи.РегНомерФСС;
	...
	Если РегНомерФССИзменился Тогда 
		ДобавитьЗаписьИзменения(Перечисления.ПараметрыПодключенияК1СОтчетности.РегНомерФСС, РегНомерФССИсходный, РегНомерФСС, Изменения);
	КонецЕсли;
...
Функция ВыполнитьПроверкуПодключений() //2
	...
	Для Каждого Направление Из РезультатыДиагностики.Автонастройка.Получатели Цикл
		...
		ИначеЕсли Направление.Значение = "ФТС" Тогда 
			...
			ВерсияФормированияВыгрузки = Неопределено;
			ВерсияОтправкиПортала = Неопределено;
			Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФТС(ВерсияФормированияВыгрузки, ВерсияОтправкиПортала);
...
Функция ОтправитьОтчетНаСервере(ОтчетДиагностики, ОписаниеОшибки = "", Знач ТолькоСформировать = Ложь)
	...
	Отчет_Абонент_УчетнаяЗапись.Криптопровайдер = ?(ОтчетДиагностики.Абонент.УчетнаяЗапись.КриптопровайдерXML = "CryptoPro CSP",
	...
	Отчет_Абонент.РегНомерФСС = ОтчетДиагностики.Абонент.РегНомерФСС;
