...
Процедура ОбработатьВыбранныеНаправления(Результат)
	...
	РегНомерФСС 		 = Результат.РегНомерФСС;
	ДополнительныйКодФСС = Результат.ДополнительныйКодФСС;
...
Процедура ЗаполнитьЗаявлениеДаннымиИз1СКонтрагент()
	...
	ЗаполнятьРегНомерФСС 			= НЕ ЗначениеЗаполнено(РегНомерФСС) И НЕ ИПСдаетОтчетностьЗаСебя ИЛИ ЗаполнятьПринудительно;
	...
	// Рег номер ФСС
	Если ЗаполнятьРегНомерФСС 
		И РеквизитыОрганизации1СКонтрагент.РегистрацияВФСС <> Неопределено 
		И ЗначениеЗаполнено(РеквизитыОрганизации1СКонтрагент.РегистрацияВФСС.РегистрационныйНомерФСС) Тогда
		
		РегНомерФСС = РеквизитыОрганизации1СКонтрагент.РегистрацияВФСС.РегистрационныйНомерФСС;
		ДанныеОрганизации.РегистрационныйНомерФСС = РегНомерФСС;
...
Процедура ИзменитьОформление1СЭДО()
	...
	Элементы.Недоступно1СЭДО.Видимость 	= ОтображатьПодключениеЭДО И РежимРаботыСКлючами = 1 И ДоступнаЭлектроннаяПодписьВМоделиСервиса;
	Элементы.ПодключитьЭДО.Видимость 	= ОтображатьПодключениеЭДО И РежимРаботыСКлючами = 2;
	Элементы.ОткрытьПараметрыПодключения.Видимость = ОтображатьПодключениеЭДО И РежимРаботыСКлючами = 2;
...
Процедура ЗаполнитьЗаявлениеДаннымиИзСкопированногоЗаявления_НеподключенныеНаправления(Реквизит)
	...
	Если ПризнакПоддержкиФСС И НЕ СдаватьВФСС Тогда
		
		ЭтоОбособленноеПодразделение = ЭтоЮридическоеЛицо И (ПризнакОбособленногоПодразделения ИЛИ ДанныеОрганизации.ПризнакОбособленногоПодразделения);
...
Процедура ЗаполнитьЗаявлениеДаннымиИзСертификата(ВыполняемоеОповещение)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗаполнитьСведенияИзСертификатаДляРеквизитов_ПослеОпределенияРегНомераФСС", 
		ЭтотОбъект, 
		ДополнительныеПараметры);
	
	ОпределитьРегНомерФСС(ОписаниеОповещения);
		
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСведенияИзСертификатаДляРеквизитов_ПослеОпределенияРегНомераФСС(РегНомерФССИзСертификата, ВходящийКонтекст) Экспорт
	...
	// РегНомерФСС
	РегНомерФСССтороннегоСертификата = РегНомерФССИзСертификата;
	Если НЕ ЗначениеЗаполнено(РегНомерФСС) ИЛИ ЗаполнятьПринудительно Тогда
		
		Если ЗначениеЗаполнено(РегНомерФССИзСертификата) Тогда
			
			РегНомерФСС = РегНомерФССИзСертификата;
...
&НаКлиенте
Процедура ОпределитьРегНомерФСС(ВыполняемоеОповещение)
	
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("ВыполняемоеОповещение", ВыполняемоеОповещение);
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ОпределитьРегНомерФСС_ПослеПолученияBase64", 
		ЭтотОбъект, 
		ДополнительныеПараметры);
...
&НаКлиенте
Процедура ОпределитьРегНомерФСС_ПослеПолученияBase64(Результат, ВходящийКонтекст) Экспорт
	...
	РегНомерФССИзСертификата = КонтекстЭДОКлиент.ПолучитьНомерФССИзСертификата(ДвДанныеСертификата);
	
	ВыполнитьОбработкуОповещения(ВходящийКонтекст.ВыполняемоеОповещение, РегНомерФССИзСертификата);
...
Процедура ИзменитьОформлениеСпособаПодписания(Форма)
	...
	Элементы.ПодсказкаПояснениеПоБумажномуВиду1.РасширеннаяПодсказка.Заголовок = ПодсказкаДляБумажногоВида();
	Элементы.ПодсказкаПояснениеПоБумажномуВиду.РасширеннаяПодсказка.Заголовок  = ПодсказкаДляБумажногоВида();
...
&НаКлиентеНаСервереБезКонтекста
Функция ПодсказкаДляБумажногоВида()

	Возврат НСтр("ru = 'Встреча с представителем обслуживающей организации требуется для
                  |удостоверения личности заявителя и оформления документов, связанных с подключением.'");
КонецФункции
...
Функция ПодсказкаДляЭлектронногоПодписания(ДобавлятьОтступы = Истина)
	...
	Результат = Новый ФорматированнаяСтрока(НСтр("ru = 'Подписать заявление эл. подписью можно, если у организации для выбранного сотрудника 
                                                  ...
                                                  |При использовании эл. подписи встреча с представителем обслуживающей организации не потребуется.
