...
Процедура ПодписатьЗаявлениеИОтправить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ФайлСканПаспорт) Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Укажите скан-копию паспорта владельца сертификата на первом шаге мастера'"), ,"СканПаспорт");
...
Процедура ПроверитьЗаполнениеЗаявления(МастерДалее)
	...
	Если СдаватьВФСС И ИзменитьСоставКонтролирующихОрганов Тогда
		...
		Если ПустаяСтрока(РегНомерФСС) Тогда
			...
			ТекстОшибки 		= НСтр("ru = 'Заполните регистрационный номер в ФСС'");
			ВывестиОшибку(ТекстОшибки);
		...
		ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(РегНомерФСС, 10, Истина) Тогда
			...
			ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.РегНомерФСС");
			ТекстОшибки 		= НСтр("ru = 'Регистрационный номер в ФСС должен состоять из 10 цифр'");
			ВывестиОшибку(ТекстОшибки);
		...
		Если ПризнакОбособленногоПодразделения и ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЭтоЮрЛицо(Организация) и СдаватьВФСС И ЕстьРеквизитДопКодФССУОрганизации() Тогда
			Если ПустаяСтрока(ДополнительныйКодФСС) Тогда 
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ДополнительныйКодФСС");
				ТекстОшибки 		= НСтр("ru = 'Заполните дополнительный код ФСС у организации'");
				ВывестиОшибку(ТекстОшибки);
			...
			ИначеЕсли НЕ КонтекстЭДОКлиент.ПроверитьЦифровойКодЗаданнойДлины(ДополнительныйКодФСС, 10, Истина) Тогда
				...
				ИзмененныйРеквизит 	= ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ДополнительныйКодФСС");
				ТекстОшибки 		= НСтр("ru = 'Дополнительный код ФСС должен состоять из 10 цифр'");
				ВывестиОшибку(ТекстОшибки);
...
Процедура СравнитьТекущиеРеквизитыПодключенияСИсходными()
	...
	// Регистрационный номер в ФСС. Отслеживаем только если ФСС был подключен или подключается сейчас.
...
Функция ИзменилисьРеквизитыТребующиеПереизданияСертификата()
	...
	Возврат КраткоеНаименованиеИзменилось И ИзменитьРеквизитыПодключенияК1СОтчетности
			...
			ИЛИ ИзменитьСоставКонтролирующихОрганов И СдаватьВФССИзменился И СдаватьВФСС;
...
Процедура УстановитьГалкуИзменитьВладельцаСертификата()
	...
	Иначе
		...
		ЭтоИПиНеРуководитель = НЕ ЭтоЮридическоеЛицо <> ВладелецЭЦПТипИсходный = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.Руководитель");
			
		// Проверяем, изменились ли данные владельца ЭЦП только если нашли владельца ЭП в базе
		Если (ПодразделениеОтличается ИЛИ ДолжностьОтличается ИЛИ ВладелецЭЦПИзменилисьПаспортныеДанные ИЛИ ЭтоИПиНеРуководитель) И ЗначениеЗаполнено(ВладелецЭЦП) Тогда
...
Процедура ПрочитатьРегистрДополнительныеРеквизитыУчетнойЗаписи(УчетнаяЗапись)
	...
	Запрос.Текст = 
		...
		|	ДополнительныеРеквизитыУчетнойЗаписи.РегНомерФСС КАК РегНомерФСС,
...
Функция ЕстьРеквизитДопКодФССУОрганизации()
 Возврат НЕ ЭтоРеквизитНеХранящийсяВБазе(ПредопределенноеЗначение("Перечисление.ПараметрыПодключенияК1СОтчетности.ДополнительныйКодФСС"));
КонецФункции
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СканПаспорт.Видимость = БезбумажноеПродлениеВозможно(Форма);
