...
Процедура ПолучитьСертификатыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	МодульДокументооборотСМинобороныКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотСМинобороныКлиент");
	МодульДокументооборотСМинобороныСлужебныйВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотСМинобороныСлужебныйВызовСервера");
	...
	Для Каждого ЭлементСертификат Из СписокСертификаты Цикл
		...
		Если Отпечатки.Количество() > 0 И Отпечатки.НайтиПоЗначению(ЭлементСертификат.Отпечаток) <> Неопределено 
			...
			СвойстваСертификата = МодульДокументооборотСМинобороныСлужебныйВызовСервера.ПолучитьСвойстваСертификата(ЭлементСертификат.Выгрузить());
			
			НовыйСертификат.АлгоритмПубличногоКлюча = СтрЗаменить(СвойстваСертификата.АлгоритмПубличногоКлюча, "GOST R", ЛокализованноеИмяГОСТ());
	...
	ТекДата = МодульДокументооборотСМинобороныВызовСервера.ТекущаяДатаНаСервере();
...
Процедура ОтобразитьТаблицуСертификатов()
	...
	МодульДокументооборотСМинобороныВызовСервера = ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотСМинобороныВызовСервера");
	ТекущаяДата = МодульДокументооборотСМинобороныВызовСервера.ТекущаяДатаНаСервере();
