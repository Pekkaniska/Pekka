...
Функция ИнициализацияФормы()
	...
	ЗаявлениеНаИзменениеДоступно = ДанныеЗаполнены И НЕ СертификатНедоступенИлиИстек
		И ВидКонтролирующегоОргана <> Перечисления.ТипыКонтролирующихОрганов.ФСС;
	...
	Иначе
		...
		Если ДанныеЗаполнены И (СертификатНедоступенИлиИстек
			...
			Элементы.ДекорацияОшибка.Заголовок = ?(СертификатНедоступенИлиИстек,
				НСтр("ru = 'Сертификат недействителен, потребуется переиздание сертификата 1С-Отчетности.'"),
				НСтр("ru = 'Чтобы подключить новое направление, потребуется переиздание сертификата 1С-Отчетности.'"));
...
&НаСервере
Функция УдалосьЗаполнитьНастройкиФСС(НовыйДокументЗаявление, ДанныеОрганизации)
	...
	Если СтрокаПустая(НовыйДокументЗаявление.РегНомерФСС) Тогда
		...
		Если ПустаяСтрока(РегНомерФСС) Тогда
			...
			Элементы.ДекорацияОшибка.Заголовок = НСтр("ru = 'Заполните регистрационный номер в ФСС в настройках организации.'");
			
			Возврат Ложь;
			
		ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьЦифровойКодЗаданнойДлины(РегНомерФСС, 10, Истина) Тогда
			
			Элементы.ДекорацияОшибка.Заголовок = НСтр("ru = 'Регистрационный номер в ФСС в настройках организации должен состоять из 10 цифр.'");
	...
	Если СтрокаПустая(НовыйДокументЗаявление.ДополнительныйКодФСС) Тогда
		...
		Если ПризнакОбособленногоПодразделения И ЭтоЮридическоеЛицо Тогда
			...
			Если ПустаяСтрока(ДополнительныйКодФСС) Тогда 
				...
				Элементы.ДекорацияОшибка.Заголовок = НСтр("ru = 'Заполните дополнительный код ФСС в настройках организации.'");
				
				Возврат Ложь;
				
			ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьЦифровойКодЗаданнойДлины(ДополнительныйКодФСС, 10, Истина) Тогда
				
				Элементы.ДекорацияОшибка.Заголовок = НСтр("ru = 'Дополнительный код ФСС в настройках организации должен состоять из 10 цифр.'");
...
Функция ДанныеЗаполненыПолностью()
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС Тогда
		...
		Возврат УдалосьЗаполнитьНастройкиФСС(ЗаявлениеАбонента, ДанныеОрганизации);
