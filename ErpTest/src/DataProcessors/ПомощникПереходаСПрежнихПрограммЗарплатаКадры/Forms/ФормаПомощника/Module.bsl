...
Процедура ИнициализацияПомощника()
	...
	Объект.МесяцНачалаЭксплуатации = НачалоМесяца(ТекущаяДата());
	...
	БылПутьКИБ = "";
	ВыгрузкаИзV8 = Истина;
	УстановитьВидимостьГруппаОшибкиПроверкиПодкючения(ЭтаФорма, Ложь);
...
Процедура УстановитьВидимостьГруппаОшибкиПроверкиПодкючения(Форма, Видимость)
...
Функция ВыгрузкаИзИнформационнойБазыV8()
	...
	ПараметрыПравил.Вставить("ОбластьВыгрузки", "");
...
Функция РезультатПроверкиПодключенияКИБV8()
	...
	Если ЗначениеЗаполнено(ИнформацияОПрограмме.МинимальнаяВерсияКонфигурацииОписание) Тогда
		...
		ИБИсточник = "";
		УдалитьВременныеФайлыИзКаталога(КаталогВыгрузкиЗагрузки);
		ТекстСообщения = НСтр("ru = 'Версия выбранной программы для переноса данны: %1'");
...
Процедура ПроверкаПодключенияКИнформационнойБазе()
	...
	Если Не ПустаяСтрока(Результат) Тогда
		...
		Отмена = Истина;
		УстановитьВидимостьГруппаОшибкиПроверкиПодкючения(ЭтаФорма, Истина);
