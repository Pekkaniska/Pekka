...
Функция УчетнаяПолитикаЗаполнена()
	...
	Если Не УчетнаяПолитикаЗаполнена Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнена Учетная политика!'"), ,"УчетнаяПолитика");
	КонецЕсли;
...
Процедура ИзменитьОтражениеХозяйственныхОперацийВУчетнойПолитикеСервер(МассивСтрок, УчетнаяПолитика, ОтражатьВМеждународномУчете)
	...
	Если ТаблицаУдаляемыхЗаписей.Количество() > 0 Тогда
		...
		ОтразитьПодчиненныеШаблоныПроводокВУчетнойПолитике(ТаблицаУдаляемыхЗаписей, УчетнаяПолитика);
...
&НаСервереБезКонтекста
Процедура ОтразитьПодчиненныеШаблоныПроводокВУчетнойПолитике(ОтражаемыеОперации, УчетнаяПолитика)
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		...
		ИзменитьОтражениеШаблоновПроводокВУчетнойПолитике(ШаблоныПроводокДляОтражения, УчетнаяПолитика, Истина);
	КонецЕсли;
