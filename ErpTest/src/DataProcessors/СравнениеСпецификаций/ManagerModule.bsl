...
//++ НЕ УТКА
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
		...
		ДобавитьРеквизит(НСтр("ru = 'Тип'"), Реквизиты.ТипПроизводственногоПроцесса, ТаблицаРеквизитов, ПолеЗначения);
		
		Если Реквизиты.ВыпускПроизвольнымиПорциями Тогда
			ДобавитьРеквизит(НСтр("ru = 'Выпуск'"), НСтр("ru = 'произвольными порциями'"), ТаблицаРеквизитов, ПолеЗначения);
		Иначе
			ДобавитьРеквизит(НСтр("ru = 'Выпуск'"), НСтр("ru = 'кратными партиями'"), ТаблицаРеквизитов, ПолеЗначения);
		КонецЕсли;
		
		ДобавитьРеквизит(НСтр("ru = 'Оптимальная партия'"), Реквизиты.ОптимальнаяПартияВыпуска, ТаблицаРеквизитов, ПолеЗначения);
		ДобавитьРеквизит(НСтр("ru = 'Минимальная партия'"), Реквизиты.МинимальнаяПартияВыпуска, ТаблицаРеквизитов, ПолеЗначения);
		
		ДобавитьРеквизит(НСтр("ru = 'Ограничен срок пролеживания'"), Реквизиты.ОграниченСрокПролеживанияВыходныхИзделий, ТаблицаРеквизитов, ПолеЗначения);
		
		Если Реквизиты.ОграниченСрокПролеживанияВыходныхИзделий Тогда
			ДобавитьРеквизит(НСтр("ru = 'Максимальный срок пролеживания'"), Реквизиты.МаксимальныйСрокПролеживанияВыходныхИзделий, ТаблицаРеквизитов, ПолеЗначения);
		Иначе
			ДобавитьРеквизит(НСтр("ru = 'Максимальный срок пролеживания'"), 0, ТаблицаРеквизитов, ПолеЗначения);
		КонецЕсли;
...
//++ НЕ УТКА
	ДобавитьСвойство(НСтр("ru = 'Одновременно производимое количество единиц/партий изделий'"), ДанныеЭтапа.ОдновременноПроизводимоеКоличествоЕдиницПартийИзделий, ТаблицаСвойств, ПолеЗначения);
	
	ДобавитьСвойство(НСтр("ru = 'Маршрутная карта'"), ДанныеЭтапа.МаршрутнаяКартаПредставление, ТаблицаСвойств, ПолеЗначения);
	
	ДобавитьСвойство(НСтр("ru = 'Планировать работу видов рабочих центров'"), ДанныеЭтапа.ПланироватьРаботуВидовРабочихЦентров, ТаблицаСвойств, ПолеЗначения);
//-- НЕ УТКА
...
Процедура ПолучитьДанныеСпецификаций(ДанныеСравнения, СписокСпецификаций)
	...
	|	КОНЕЦ КАК ЕдИзм,
	|	ДанныеТЧ.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ДанныеТЧ.ДоляСтоимости КАК ДоляСтоимости,
	...
	|	ДанныеТЧ.НоменклатураВходящегоИзделия КАК Номенклатура,
	|	ДанныеТЧ.ХарактеристикаВходящегоИзделия КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ДанныеТЧ.УпаковкаВходящегоИзделия <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(ДанныеТЧ.УпаковкаВходящегоИзделия.Представление, """")
	|		ИНАЧЕ ЕСТЬNULL(ДанныеТЧ.НоменклатураВходящегоИзделия.ЕдиницаИзмерения.Представление, """")
	|	КОНЕЦ КАК ЕдИзм,
	|	ДанныеТЧ.КоличествоУпаковокВходящегоИзделия КАК КоличествоУпаковок,
	|	ЕСТЬNULL(ДанныеТЧ.НоменклатураВходящегоИзделия.Представление, """") КАК НоменклатураПредставление,
	|	ЕСТЬNULL(ДанныеТЧ.ХарактеристикаВходящегоИзделия.Представление, """") КАК ХарактеристикаПредставление
	|ИЗ
	...
	|	КОНЕЦ КАК ЕдИзм,
	|	ДанныеТЧ.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ДанныеТЧ.ПроизводитсяВПроцессе КАК ПроизводитсяВПроцессе,
	|	ДанныеТЧ.ИсточникПолученияПолуфабриката КАК ИсточникПолученияПолуфабриката,
	|	ДанныеТЧ.СтатьяКалькуляции КАК СтатьяКалькуляции
	|ИЗ
	...
	Пока ВыборкаЭтапов.Следующий() Цикл
		...
		ДобавитьДанныеПоНоменклатуре(ДанныеТЧ, ДанныеСравненияЭтапа.ТаблицаВходящихИзделий, ПолеЗначения);
