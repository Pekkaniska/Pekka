...
Процедура КомандаУместитьВГрафикРаботы(Команда)
	...
	Если НЕ ТребуетсяРасчетРасписания(РЦ_РабочийЦентр) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Нет маршрутных листов, которые требуется уместить в график работы.'"),, НСтр("ru = 'Информация'"));
		Возврат;
	КонецЕсли;
...
Процедура ВывестиГрафикЗанятостиРабочегоЦентра(ДанныеТекущейОбластиДанных, ВыбранныйПериодРасписания)
	...
	Если ПараллельнаяЗагрузка Тогда
		...
		ОбластьДелениеШкалы.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка, %1'"), ЕдиницаИзмеренияЗагрузки);
...
Процедура ПоказатьПрочиеСведенияВГрафикеЗанятости(Расшифровка)
	...
	Если Расшифровка.ВидОбласти = "НерабочееВремя" Тогда
		...
		// Нерабочий период
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Нерабочий период'"), Новый Шрифт(,, Истина)));
		
	ИначеЕсли Расшифровка.ВидОбласти = "СвободноеВремя" Тогда
		
		// Свободный период
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Свободный период'"), Новый Шрифт(,, Истина)));
		Если ПараллельнаяЗагрузка Тогда
			МассивИнформации.Добавить("; ");
			МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Загрузка:'"), Новый Шрифт(,, Истина)));
			МассивИнформации.Добавить(" ");
			МассивИнформации.Добавить(Формат(Расшифровка.Загрузка, "ЧН=0; ЧГ=0"));
			МассивИнформации.Добавить(" " + ЕдиницаИзмеренияЗагрузки + "; ");
			
			МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Свободная загрузка:'"), Новый Шрифт(,, Истина)));
			МассивИнформации.Добавить(" ");
	...
	ИначеЕсли Расшифровка.ВидОбласти = "Переналадка" Тогда
		...
		// Занято с - по -
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Переналадка с'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
		МассивИнформации.Добавить(Формат(Расшифровка.Начало, "ДФ=ЧЧ:мм"));
		МассивИнформации.Добавить(" ");
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'по'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
		МассивИнформации.Добавить(Формат(Расшифровка.Окончание, "ДФ=ЧЧ:мм"));
		МассивИнформации.Добавить("; ");
		
		// Вариант наладки
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Вариант наладки:'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
...
Процедура ПоказатьСведенияОМЛВГрафикеЗанятости(ДанныеРасшифровки, ИндексМЛДляКоторогоНужноПоказатьИнформацию = 0, ЗаполнитьВыбор = Ложь)
	...
	// Этап
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Этап:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	МассивИнформации.Добавить(Расшифровка.ЭтапСпецификация);
	МассивИнформации.Добавить("; ");
	
	// Запланировано
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Запланировано:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	МассивИнформации.Добавить(Формат(Расшифровка.Количество, "ЧГ=0"));
	МассивИнформации.Добавить(" ед/партий; ");
	
	// Объем работ
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Объем работ:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	...
	КонецЕсли; 
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Занято с'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	...
	// Номенклатура
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Номенклатура:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
	...
	Если ЗначениеЗаполнено(Расшифровка.НазначениеПредставление) Тогда
		МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Назначение:'"), Новый Шрифт(,, Истина)));
		МассивИнформации.Добавить(" ");
	...
	// Маршрутный лист
	МассивИнформации.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Маршрутный лист:'"), Новый Шрифт(,, Истина)));
	МассивИнформации.Добавить(" ");
...
Процедура УместитьВГрафикРаботыЗавершение(РезультатВопроса, РабочийЦентр) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		Если УместитьВГрафикРаботы(РабочийЦентр) Тогда
			ПриИзмененииРасписания();
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Не удалось уместить маршрутные листы в график.'"),, НСтр("ru = 'Ошибка'"));
		КонецЕсли; 
	КонецЕсли; 
