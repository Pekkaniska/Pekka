...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ПустаяСтрока(ТекстОповещенияПриОткрытии) Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Заполнение обеспечения'"),, ТекстОповещенияПриОткрытии, КартинкаОповещенияПриОткрытии);
	КонецЕсли; 
...
Функция ЗаполнитьОбеспечениеВУстановленномПорядке(ВыбранноеЗначение)
	...
	ПараметрыДокумента.Вставить("ТаблицаДляПроведения", ТаблицаДляПроведения);
...
Функция ЗаписатьИзменения()
	...
	ИначеЕсли СписокСозданныхДокументов.Количество() > 1 Тогда
		Если ИспользуютсяКорректировки Тогда
			ПоказатьОповещениеПользователя(НСтр("ru = 'Создание:'"),, НСтр("ru = 'Созданы корректировки заказа материалов'"));
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение:'"),, НСтр("ru = 'Внесены изменения в заказы на производство'"));
		КонецЕсли; 
	КонецЕсли;
