...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ЭлектронноеВзаимодействие.ОбменСБанками
	Команды["ПисьмаВБанк"].Заголовок = ОбменСБанкамиКлиентСервер.ТекстСсылкиПерепискаСБанками();
	// Конец ЭлектронноеВзаимодействие.ОбменСБанками
	
	Если Команды["ПисьмаВБанк"].Заголовок = "" Тогда
		Элементы.ДокументыВалютногоКонтроляПисьмаВБанк.Видимость = Ложь;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьСостояниеОбменСБанками" Тогда
		ОбновитьСостояниеОбменСБанкамиНаСервере();
	КонецЕсли;
	// Конец ЭлектронноеВзаимодействие.ОбменСБанками
...
Процедура СписокПлатежейВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СписокПлатежейСостояние" Тогда
		
		МассивСсылок = ПроверитьПолучитьВыделенныеВСпискеСсылки(ВыбраннаяСтрока);
		// ЭлектронноеВзаимодействие.ОбменСБанками
		Если МассивСсылок.Количество() Тогда
			ОбменСБанкамиКлиент.ПриНажатииНаГиперссылку(МассивСсылок[0], СтандартнаяОбработка);
		КонецЕсли;
		// Конец ЭлектронноеВзаимодействие.ОбменСБанками
	Иначе
...
Процедура ЗаполнитьКомандыСозданияДокументов()
	...
	//++ НЕ УТ
	Операции.Добавить(Перечисления.ХозяйственныеОперации.ОплатаЛизингодателю, НСтр("ru= 'Оплата лизингодателю'"));
	//-- НЕ УТ
	ГруппыОпераций.Добавить(Операции); // Оплата
	...
	Для каждого ИмяСписка Из МассивСписков Цикл
		...
		Если ИмяСписка = "СписокПлатежей" Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				...
				ЗначениеЗаполнено(БанковскийСчетКлюч));
		Иначе
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				ЭтаФорма[ИмяСписка],
				"БанковскийСчет",
...
&НаСервере
Процедура ОбновитьСостояниеОбменСБанкамиНаСервере()
