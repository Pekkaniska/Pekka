...
Процедура ОбработкаВыбора()
	...
	Если Элементы.СпособВыбора.ТекущаяСтраница = Элементы.ПрошлыеСмены Тогда
...
Процедура ПрочитатьСмены(ТочкаОтсчета, Количество)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "1", Количество + 1);
...
&НаСервере
Функция ФизическиеЛицаПодразделения()
	...
	Возврат ДанныеСотрудников.Скопировать(Новый Структура("МестоВСтруктуреПредприятия", ПодразделениеОтбор),"ФизическоеЛицо").ВыгрузитьКолонку("ФизическоеЛицо");
