...
Процедура ИнициализироватьСмены()
	...
	Если ДоступнаПрокрутка Тогда
		...
		Если НачалоДня(ДатаНачалаСписка) = ТекущаяДата Тогда
			Элементы.ПерейтиВНачалоСписка.Заголовок = СтрШаблон(НСтр("ru = '%1 сегодня'"), Формат(ДатаНачалаСписка, "ДЛФ=DD"));
		Иначе
...
Функция СоздатьИПолучитьВыбранныеЗадания()
	...
	Если ВыбранныеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не отмечены смены для выбора'"),, "ВидимыеСмены");
		Возврат Неопределено;
	КонецЕсли;
