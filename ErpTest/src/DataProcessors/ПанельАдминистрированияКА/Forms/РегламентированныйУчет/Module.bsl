...
Функция ЗапуститьВводНачальныхОстатков(УникальныйИдентификатор, ПараметрыЗапуска)
	...
	ВыполняемыйМетод = "Документы.ВводОстатков.СформироватьДокументыВводаОстатковПоДаннымОперативногоУчета";
...
Функция ВедениеУУНаПланеСчетовТолькоСДаты()
	...
	Если УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ДатаПереходаНаПартионныйУчетВерсии22() <> Дата(1,1,1) Тогда
		Результат = Истина;
	КонецЕсли;
	
	Если Константы.ИспользоватьВнеоборотныеАктивы2_4.Получить() 
		И Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить() <> Дата(1,1,1) Тогда
		Результат = Истина;
	КонецЕсли;
...
Функция ДоступенУУНаПланеСчетовХозрасчетный()
	...
	ИспользоватьУправленческийУчетОрганизаций = УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.УправленческийУчетОрганизаций();
