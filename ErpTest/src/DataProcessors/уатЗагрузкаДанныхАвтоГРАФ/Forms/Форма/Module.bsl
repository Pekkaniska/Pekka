...
Функция ВыполнитьЗагрузкуСервер(ТекстОшибки)
	...
	Если ЗагрузитьПробегИРасходГСМ Тогда // загружаем пробег и расход ГСМ
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьМестоположениеПоТСАвтоГРАФ(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
	
	Если КодВозврата=0 И ЗагрузитьДополнительныеСведения Тогда // загружаем дополнительные сведения
		КодВозврата = уатЗащищенныеФункцииСервер_проф.ЗагрузитьДополнительныеСведенияПоТСАвтоГРАФ(Объект.ДатаС, Объект.ДатаПо, МассивТС, ТекстОшибки);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Константы.уатИспользоватьАвтоГРАФ.Получить() Тогда
		ТекстСообщения = "Открытие возможно только тогда, когда включено использование системы GPS - АвтоГРАФ.";
...
Процедура ВыполнитьЗагрузку(Команда)
	...
	Если Не ЗагрузитьПробегИРасходГСМ И Не ЗагрузитьДополнительныеСведения Тогда 
		ТекстОшибки = "Необходимо выбрать хотя бы один параметр для загрузки (пробег и расход/дополнительные сведения)";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,,"ЗагрузитьПробегИРасходГСМ",,Отказ);
	КонецЕсли;
	
	// Выполним загрузку
	Если Не Отказ Тогда 
		Состояние("Выполняется загрузка данных системы GPS АвтоГРАФ",,"Операция может занять длительное время");
		КодВозврата = ВыполнитьЗагрузкуСервер(ТекстОшибки);
		
		Если КодВозврата = 0 Тогда 
			Состояние("Загрузка данных системы GPS АвтоГРАФ завершена успешно");
		КонецЕсли;
	КонецЕсли;
