...
Процедура НоваяАрхитектураРасчетовВключениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Иначе
			Отказ = Ложь;
			ПроверитьВозможностьВключенияНовойАрхитектурыВзаиморасчетов(Отказ);
			Если Отказ Тогда
				ОткрытьФорму("Отчет.АнализРазвернутогоСальдо.Форма", Новый Структура("КлючВарианта, СформироватьПриОткрытии", "Основной", Истина));
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьЗаявкиНаРасходованиеДенежныхСредств"
		...
		//++ НЕ УТ
		Элементы.СправочникМоделиБюджетированияОткрытьСписок.Доступность = НаборКонстант.ИспользоватьЗаявкиНаРасходованиеДенежныхСредств;
		//-- НЕ УТ
...
Функция ЗаполнитьРегистрыРасчетовНаСервере()
	...
	РегистрыСведений.ЗаданияКЗакрытиюМесяца.СоздатьЗаписьРегистра(НачалоМесяца(ТекущаяДатаСеанса()), , ,
		Перечисления.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоРасчетамСПартнерамиИПереоценкаРасчетов);
	
	НаименованиеЗадания = НСтр("ru = 'Заполнение регистров расчетов в новой архитектуре'");
...
&НаСервере
Процедура ПроверитьВозможностьВключенияНовойАрхитектурыВзаиморасчетов(Отказ)
	...
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоДокументам.Остатки КАК Расчеты
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	Расчеты.Валюта
	|ИМЕЮЩИЕ 
	|	СУММА(Расчеты.ПредоплатаОстаток) <> 0
	|	И СУММА(Расчеты.ДолгОстаток)<> 0
	|;
	|ВЫБРАТЬ
	|	Расчеты.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	Расчеты.АналитикаУчетаПоПартнерам.Организация КАК Организация,
	...
	|	СУММА(Расчеты.ДолгОстаток) КАК ДолгОстаток
	|ИЗ
	|	РегистрНакопления.РасчетыСПоставщикамиПоДокументам.Остатки КАК Расчеты
	|
	|СГРУППИРОВАТЬ ПО
	|	Расчеты.АналитикаУчетаПоПартнерам,
	...
	РасчетыСКлиентами = МассивРезультатов[0].Выгрузить();
	РасчетыСПоставщиками = МассивРезультатов[1].Выгрузить();
	
	Если РасчетыСКлиентами.Количество() > 0 ИЛИ РасчетыСПоставщиками.Количество() > 0 Тогда
