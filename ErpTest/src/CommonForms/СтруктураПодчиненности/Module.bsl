...
//++ Рарус Лимаренко 18.12.2018	
			//Если Не Метаданные.КритерииОтбора.СвязанныеДокументы.Состав.Содержит(Реквизит) Тогда
			//	Продолжить;
			//КонецЕсли;

			Если Не (Метаданные.КритерииОтбора.пкСвязанныеДокументы.Состав.Содержит(Реквизит)
				ИЛИ Метаданные.КритерииОтбора.СвязанныеДокументы.Состав.Содержит(Реквизит))	Тогда
				Продолжить;
			КонецЕсли;
//-- Рарус Лимаренко 18.12.2018
...
Процедура ДополнитьКэшМетаданных(МетаданныеОбъекта, ИмяОбъекта, КэшРеквизитовОбъекта)
	...
	Если РеквизитыДокумента = Неопределено Тогда
		...
		Иначе
			...
			РеквизитыДокумента.Вставить("СуммаДокумента", "NULL");
			//Рарус Владимир Подрезов 25.01.2018
...
//++ Рарус Лимаренко 07.12.2018
	...
	ИЛИ Метаданные.КритерииОтбора.пкСвязанныеДокументы.Тип.СодержитТип(ТипЗнч(ЗначениеКритерияОтбора)))  Тогда
//-- Рарус Лимаренко 07.12.2018
	...
	//+++rarus-spb_zlov 25.07.2016
	Запрос.Текст = Запрос.Текст + "
	|ОБЪЕДИНИТЬ ВСЕ
	|" + СтрЗаменить(Запрос.Текст, "КритерийОтбора.СвязанныеДокументы", "КритерийОтбора.пкСвязанныеДокументы");
	//---rarus-spb_zlov 25.07.2016
