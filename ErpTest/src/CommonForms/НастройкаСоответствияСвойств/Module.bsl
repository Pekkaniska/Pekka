...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОписаниеФункций = УправлениеДаннымиОбИзделиях.ОписаниеФункцийАвтовыбораЗначенияСвойстваНоменклатуры();
					...
					ИЛИ ДанныеСвойстваПродукции.Свойство.ТипЗначения.Типы().Количество() = 1
						И (ДанныеСвойстваПродукции.Свойство.ТипЗначения.СодержитТип(Тип("Число"))
							...
							ИЛИ ДанныеСвойстваПродукции.Свойство.ТипЗначения.СодержитТип(Тип("Дата"))
								И ДанныеСвойстваМатериала.Свойство.ТипЗначения.СодержитТип(Тип("Дата"))
						)
				) Тогда
	...
	Для каждого ТекущиеДанные Из СоответствиеСвойств Цикл
		...
		ИначеЕсли ТекущиеДанные.СписокДоступныхСвойств.Количество() = 1 Тогда
			...
			ТекущиеДанные.СпособПодбораЗначения          = Перечисления.СпособыПодбораЗначенияСвойстваНоменклатуры.ПоСвойству;
...
Функция ПараметрыФормыРедактированияФормулы(Формула)
	...
	ПараметрыФормы.Вставить("Операнды", АдресХранилищаДереваОперандов);
...
Процедура СформироватьДанныеДляПодбораПоФормуле(СвойстваПродукции)
	...
	ГруппаПродукция = РаботаСФормулами.ДобавитьОперанд(ДеревоОперандов, "", НСтр("ru = 'Свойства продукции'"));
	...
	Для каждого Свойство Из СвойстваПродукции Цикл
		Если Свойство.ТипЗначения.СодержитТип(Тип("Число")) 
			...
			РаботаСФормулами.ДобавитьОперанд(ГруппаПродукция, Свойство.Представление);
		КонецЕсли;
	КонецЦикла;
