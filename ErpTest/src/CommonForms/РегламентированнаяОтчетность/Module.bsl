...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о возврате налога"
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о зачете налога"
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление об освобождении от уплаты страховых взносов"
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Уведомление об открытии/закрытии счета"
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Уведомление об изменении реквизитов счета"
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Уведомление о наличии счета" Тогда
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Декларация по транспортному налогу (годовая)" Тогда
		ОписаниеТрНал = ОписаниеРегОтчета(Ссылка);
		Если ОписаниеТрНал.ВыбраннаяФорма = "ФормаОтчета2019Кв1" Тогда
			ПоказатьПредупреждение(, НСтр(
			"ru = 'Печать со штрихкодом PDF417 формы, утвержденной приказом ФНС России от 05.12.2016 № ММВ-7-21/668@
			|(в редакции приказа ФНС России от 26.11.2018 № ММВ-7-21/664@) будет реализована в будущих версиях программы.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ТекстСообщенияОНепройденномКонтролеНомеровКорректировок(ВыбОрганизация,
	...
	Если ТипЗнч(ОбъектОтчет) <> Тип("ОтчетОбъект") Тогда
