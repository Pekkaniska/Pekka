...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("НетКритическихОшибок", НетКритическихОшибок);
	
	Если НЕ ТипЗнч(НетКритическихОшибок) = Тип("Булево") Тогда
	    НетКритическихОшибок = Ложь;
	КонецЕсли; 
	...
	Элементы.ТекстРезультатаПроверки.Заголовок = ?(НетКритическихОшибок, 
													"Выгрузка и отправка возможны, но рекомендуется обратить внимание!",  
													"Обнаружены ошибки, препятствующие отправке декларации!");
...
Процедура ОбработкаРасшифровкиСтруктуры(Расшифровка, СтандартнаяОбработка)
	...
	ФормаРеглОтчета = ПолучитьФорму(СтрФормаРеглОтчета, ПараметрыФормы, , ПараметрыФормы.мСохраненныйДок);
		
	Если НЕ ФормаРеглОтчета.Открыта() Тогда
