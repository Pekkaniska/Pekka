...
Процедура ФормированиеБюджетныхЗадач(МодельБюджетирования = Неопределено, ПроцессПодготовкиБюджетов=Неопределено) Экспорт
	...
	ВсеЭтапы = Результаты[2].Выгрузить();
	ВсеЭтапы.Индексы.Добавить("Модель");
	
	ВсеЗадачи = Результаты[3].Выгрузить();
	...
	Пока ВыборкаМоделейБюджетирования.Следующий() Цикл
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru = 'Не удалось создать бюджетные задачи по модели: %1 по причине: %2'", КодОсновногоЯзыка);
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки, ВыборкаМоделейБюджетирования.Наименование, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(НСтр("ru = 'БюджетныйПроцесс.СозданиеБюджетныхЗадач'", КодОсновногоЯзыка),
...
Процедура СформироватьБюджетыПоНастройкамДействияВФоне(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ТекстОшибки = НСтр("ru = 'Не удалось создать документы по бюджетной задаче: %1%, по причине: %2%'", КодОсновногоЯзыка);
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Интерактивное создание документов для бюджетных задач'", КодОсновногоЯзыка),
...
//  ТекстОшибки - Текст - Возвращаемый параметр, при возникновении ошибки создания документов,
//                текст ошибки будет установлен в данный параметр.
//
// Возвращаемое значение:
//  Структура - структура со свойствами:
//    * ИмяФормы - Строка - имя формы объекта созданного документа
//    * ПараметрыФормы - Структура - Параметры для открытия формы объекта созданного документа
//        ** Ключ - ДокументСсылка.ЭкземплярБюджета, ДокументСсылка.УстановкаЗначенийНефинансовыхПоказателей,
//                  ДокументСсылка.ПланПродаж - Ссылка на созданный документ. Также в качестве ключа могут
//                  выступать ссылки типа ДокументСсылка.ПланПроизводства, ДокументСсылка.ПланСборкиРазборки,
...
//    * Уникальность - Булево - Признак открытия формы с уникальным идентификатором.
//
...
Функция ПараметрыВводаБюджетовПоНастройкамДействия(Знач ТаблицаДействий,
	...
	"ВЫБРАТЬ
		...
		|
		|ВЫБРАТЬ
		|	ТаблицаВидыБюджетов.ВидБюджета,
		|	ТаблицаВидыБюджетов.КлючСвязи,
		|	ТаблицаВидыБюджетов.НомерСтроки,
	...
	Если ВерныеНастройкиДействия
		...
		ТекстСообщенияЖР = НСтр("ru = 'Неверные настройки действия. Создание задачи не возможно'", КодЯзыка);
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Создание бюджетных задач'", КодЯзыка),
	...
	Если ВерныеНастройкиДействия И КоличествоЗаписейКалендарныхГрафиков = 0 Тогда
		...
		ТекстСообщенияЖР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Календарный график %1 не запланирован за период с %2 по %3. Создание задач не возможно'", КодЯзыка),
			График,
			Формат(НачалоПериода, "ДЛФ=D"),
			Формат(КонецПериода, "ДЛФ=D"));
		...
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	Результат = РезультатыВыполнения[РезультатыВыполнения.Количество()-1].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
...
//
// Параметры:
//	Процесс - Документ.ПроцессПодготовкиБюджетов - процесс, реквизит задачи.
//
// Возвращаемое значение:
...
//
// Параметры:
// 	Процесс - Документ.ПроцессПодготовкиБюджетов - Процесс подготовки бюджетов задачи.
//
// Возвращаемое значение:
	...
	Если НЕ ФоновоеЗаданиеЗапущено Тогда
		Попытка
			...
			Если ЗначениеЗаполнено(Процесс) Тогда
				НаименованиеЗадачи = НСтр("ru = 'Создание документов по процессу: %1%'", КодОсновногоЯзыка);
				НаименованиеЗадачи = СтрЗаменить(НаименованиеЗадачи, "%1%", Строка(Процесс));
			Иначе
				НаименованиеЗадачи = НСтр("ru = 'Создание документов без процесса'", КодОсновногоЯзыка);
			КонецЕсли;
...
//
// Параметры:
// 	МодельБюджетирования  - Справочник.МоделиБюджетирования - отбор задач по модели бюджетирования.
//                 
// 	Период  - Дата - отбор задач по периоду
//                 
//	ЭтапыПодготовкиБюджетов  - Справочник.ЭтапыПодготовкиБюджетов, Массив - отбор задач по этапу (массиву этапов) 
//	                           только ссылка, без групп, этап указанный в задаче
//	                           реквизите "ЭтапПодготовкиБюджетов" или "ЭтапПодготовкиБюджетовОснование".
//
//
// Возвращаемое значение:
//	ТаблицаЗначений с колонкой "Ссылка" - тип Задача.БюджетнаяЗадача.
//
...
//
// Параметры:
//	Процесс - Документ.ПроцессПодготовкиБюджетов - Процесс подготовки бюджетов (реквизит задачи)
//	ЗадачиПервойОчереди - Массив - Массив задач (ЗадачаСсылка.БюджетнаяЗадача), которые будут обрабатываться в первую очередь, 
...
//
// Параметры:
// 	Задача - Задача.БюджетнаяЗадача - задача, для которой необходимо создать документы.
Процедура СозданиеДокументовДляЗадачи(Задача) 
	...
	Пока ПродолжитьОбработку Цикл
		...
		Параметры.Вставить("ВерсияДанных", Задача.ВерсияДанных);
...
Процедура ДополнитьПараметрыСледующегоФормируемогоДокументаБюджетнойЗадачи(Знач Задача, Параметры, ПродолжитьОбработку)
	...
	Попытка
		...
		Если ЗадачаОбъект.ВерсияДанных <> Параметры.ВерсияДанных Тогда
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
			ВызватьИсключение НСтр("ru = 'Задача была изменена, создание документов прервано'", КодОсновногоЯзыка);
		КонецЕсли;
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ТекстОшибки = НСтр("ru = 'Не удалось создать документ для задачи %1 по причине: %2'", КодОсновногоЯзыка);
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки, ЗадачаОбъект, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'БюджетныйПроцесс.СозданиеДокументовДляЗадачи'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
...
Процедура СозданиеЧтениеДокументаБюджетнойЗадачиПоПараметрам(Параметры, ДокументСсылка, ЗаписатьДокументВЗадачу, ПродолжитьОбработку)
	...
	Попытка
		...
		Если МассивДокументы = Неопределено Тогда
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
			ВызватьИсключение НСтр("ru = 'Документы не были сформированы'", КодОсновногоЯзыка);
		КонецЕсли;
		...
		Если ЗадачаОбъект.ПометкаУдаления Тогда
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
			ВызватьИсключение НСтр("ru = 'Задача была помечена на удаление, создание документов будет отменено'",КодОсновногоЯзыка);
		КонецЕсли;
		Если НЕ ЗадачаОбъект.КСозданиюДокументов Тогда
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
			ВызватьИсключение НСтр("ru = 'Задача была изменена, создание документов не требуется'", КодОсновногоЯзыка);
		КонецЕсли;
...
Процедура ЗаписьНайденногоДокументаВБюджетнуюЗадачуВОтдельнойТранзакции(Параметры, ДокументСсылка, ПродолжитьОбработку)
	...
	ЭтапыПодготовкиБюджетов  = Параметры.ЭтапыПодготовкиБюджетов;
	ПараметрыНовогоДокумента = Параметры.ПараметрыНовогоДокумента; 
	ЗадачаЗаблокированаТекущимСеансом = Ложь;
	...
	Исключение
		...
		КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
		ТекстОшибки = НСтр("ru = 'Не удалось записать документ в задачу %1 по причине: %2'", КодОсновногоЯзыка);
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
