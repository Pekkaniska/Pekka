...
Процедура ПроверитьВыполнениеСозданияДокументовКлиент()
	...
	Если НЕ РезультатПроверки.Выполнено Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработкаЗакрытияФормыБлокировкиСозданиеДокументов", ЭтаФорма);
		ФормаДлительнойОперации = ОткрытьФорму("Задача.БюджетнаяЗадача.Форма.БлокировкаЗадачиФормированиеДокументов",
			Новый Структура("ТекстСообщения", НСтр("ru = 'Идет формирование документов'")), 
...
Функция ЧтениеИБлокировкаЗадачи()
	...
	Исключение
		...
		ТекстОшибкиПодробный = НСтр("ru = 'Не удалось записать задачу по причине: %1'", КодОсновногоЯзыка);
