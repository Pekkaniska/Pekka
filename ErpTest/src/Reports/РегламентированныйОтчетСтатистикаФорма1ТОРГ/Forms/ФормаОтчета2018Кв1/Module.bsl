...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
			...
			Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1. Открывается форма для выбора значения, подождите...'"), ЭтаФорма.Заголовок), , , БиблиотекаКартинок.Таблица);
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	КонецЦикла;
	Для Стр = 121 По 135 Цикл
		ПФ = "_" + Формат((Стр-120), "ЧГ=");
		ПараметрыВыгрузки.Вставить("П000501000103"+ПФ, ТабДокумент[Мкт.Область(Стр, 5, Стр, 5).Имя]);
		ПараметрыВыгрузки.Вставить("ПД00501000102"+ПФ, Мкт.Область(Стр, 4, Стр, 4).Текст);
	КонецЦикла;
	Для Стр = 137 По 162 Цикл
		ПФ = "_" + Формат((Стр-136), "ЧГ=");
...
Функция ЗагрузитьСпискиВыбораНаСервере()
	...
	Элементы.ПолеТабличногоДокументаФормаОтчета.ОтображениеСостояния.Текст = НСтр("ru = 'Загружаются списки выбора...'");
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		НСтр("ru = 'Выполняется формирование списков выбора. Пожалуйста, подождите...'"));
...
Процедура ЗагрузитьСпискиВыбора() Экспорт 
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000004000101", 250, , "Выбор муниципального образования",
		КоллекцияСписковВыбора["СправочникОКТМО"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000004000102", 250, , "Выбор муниципального образования",
		КоллекцияСписковВыбора["СправочникОКТМО"]);
...
&НаКлиенте
Процедура Подключаемый_ПроверитьВыполнениеЗаданияАвтозаполнения()
	...
	Попытка
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(Элементы.ПолеТабличногоДокументаФормаОтчета, "ФормированиеОтчета");
			Элементы.ПолеТабличногоДокументаФормаОтчета.ОтображениеСостояния.Текст = НСтр("ru = 'Выполняется автоматическое заполнение отчета. Пожалуйста, подождите...'");
			ДлительныеОперацииКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
			ПодключитьОбработчикОжидания(
				"Подключаемый_ПроверитьВыполнениеЗаданияАвтозаполнения", 
				ПараметрыОбработчикаОжидания.ТекущийИнтервал, 
				Истина);
		КонецЕсли;
	Исключение
...
Процедура ЗаполнитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатВыполнения.ЗаданиеВыполнено Тогда
		ДлительныеОперацииКлиент.ИнициализироватьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжидания);
		ПодключитьОбработчикОжидания("Подключаемый_ПроверитьВыполнениеЗаданияАвтозаполнения", 1, Истина);
	КонецЕсли;
