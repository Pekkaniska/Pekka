...
Процедура ВставитьДополнительнуюСтроку()
	...
	ВставитьДопСтроку(СтруктураРеквизитовФормы.НаимТекущегоРаздела, ИмяГруппы, ИмяОбластиПреемника);
...
&НаСервере
Процедура ВставитьДопСтроку(Знач ИмяРаздела, Знач ИмяГруппы, Знач ИмяОбластиПреемника)
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Для НомерСтроки2Уровня = 1 По КоличествоСтрок2Уровня Цикл
				...
				Если Не (Показатели.ВидДокумента.Значение = "06" 
					...
					И Не ВРег(Показатели.СтранаРазгрузки.Значение) = НСтр("ru='RU'")) Тогда
					Продолжить;
				КонецЕсли;
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Для НомерСтроки2Уровня = 1 По КоличествоСтрок2Уровня Цикл
				...
				Если Не (Не ВРег(Показатели2.СтранаПогрузки.Значение) = НСтр("ru='RU'") 
					И ВРег(Показатели2.СтранаРазгрузки.Значение) = НСтр("ru='RU'")) Тогда
					Продолжить;
				КонецЕсли;
	...
	Для каждого СтраницаРаздела Из ДанныеРеестра.Строки Цикл
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл
			...
			Для НомерСтроки2Уровня = 1 По КоличествоСтрок2Уровня Цикл
				...
				Если СтруктураРезультата.ТаблицаОКСМ = Неопределено Тогда
					СтруктураРезультата.ТаблицаОКСМ = УправлениеКонтактнойИнформацией.ТаблицаКлассификатора();	
				КонецЕсли;
				
				Настройки.ЭтоОшибка	= СтруктураРезультата.ТаблицаОКСМ.Найти(ВРег(ПоказательКода.ЗначениеСтр), "КодАльфа2") = 
					Неопределено;
				ДобавитьЭлементВСтруктуруОшибок(Настройки);
