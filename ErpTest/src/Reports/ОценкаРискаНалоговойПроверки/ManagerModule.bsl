...
Процедура НалоговаяНагрузка(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	ПриложениеПриказа.Вставить("ПриложениеПриказа", "https://its.1c.ru/db/garant#content:71287400:hdoc");
...
Процедура СоздатьВТПрочихДоходовИРасходов(МенеджерВременныхТаблиц)
	...
	ИначеЕсли Метаданные.ПланыВидовХарактеристик.Найти("СтатьиДоходов") <> Неопределено Тогда
		...
		ИмяРеквизитаВидПрочихДоходов= "ПрочиеДоходыИРасходы.ВидПрочихДоходов";
		ИмяРеквизитаВидПрочихРасходов= "ПрочиеДоходыИРасходы.ВидПрочихРасходов";
	КонецЕсли;
...
Процедура СреднемесячныйУровеньЗарплаты(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	Адрес = ПолучитьАдресВВидеСоответствия(Организация, КонецПериода);
	...
	ИначеЕсли Адрес.Получить("КодРегиона") = "" Тогда
		...
		ДанныеЗаполненияМакета.Вставить("ЮридическийАдрес", НСтр("ru='выбрать регион из КЛАДР (ФИАС).'"));
	...
	ГодПроверки = Формат(Число(Год(НачалоПериода)), "ЧГ=");
	ОбластьГода = МакетСреднемесячнойЗарплаты.Область("Год");
	
	ОбластьКолонкиГода = МакетСреднемесячнойЗарплаты.НайтиТекст(Строка(ГодПроверки), , ОбластьГода, Ложь);
	Если НЕ ЗначениеЗаполнено(ОбластьКолонкиГода) И Число(ГодПроверки) < 2013 Тогда
...
Функция ПолучитьАдресВВидеСоответствия(Организация, ДатаАктуальности)
	...
	КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(МассивОбъектов, , ВидыКИ, ДатаАктуальности);
...
Функция ПолучитьПредельноеЗначениеСреднейЗарплаты(Организация, МакетСреднемесячнойЗарплаты, НомерКолонкиРегиона)
	...
	Если ЗначениеЗаполнено(Группа_2) И РезультатПоиска <> Неопределено Тогда
...
Процедура ПриближениеКПредельнымЗначениямСпецРежима(ПараметрыОтчета, ДанныеУчетнойПолитики, РезультатПроверкиКритериев,
	...
	Если ГодРегистрации = Год(КонецПериода)
		...
		И УчетУСНСервер.НалоговыйПериодПропущен(Организация, КонецПериода, ДатаРегистрации) Тогда
	...
	Для НомерСтрокиУчетнойПолитики = 0 По КоличествоЗаписейУчетнойПолитики - 1 Цикл
		...
		Если ТекущаяСтрокаУчетнойПолитики.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная Тогда
			...
			Если Год(ТекущаяСтрокаУчетнойПолитики.Период) = ГодРегистрации
				И УчетУСНСервер.НалоговыйПериодПропущен(Организация, ТекущаяСтрокаУчетнойПолитики.Период, ДатаРегистрации) Тогда
...
Процедура УровеньРентабельности(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	ПриложениеПриказа = Новый Структура("ПриложениеПриказа", "https://its.1c.ru/db/garant#content:71287398:hdoc");
...
Функция ДанныеРасчетаУСН(Параметры)
	...
	КонтекстРасчета = УчетУСНСервер.НовыйКонтекстРасчетаНалога(Параметры.Организация, Параметры.КонецПериода);
...
Функция ДанныеОКВЭД(Организация)
	...
	Результат = Новый Структура("КодОКВЭД1, КодОКВЭД, НаименованиеОКВЭД", "", "", "");
