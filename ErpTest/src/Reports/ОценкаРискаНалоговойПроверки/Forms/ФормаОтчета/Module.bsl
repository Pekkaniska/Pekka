...
Процедура РезультатОтчетаОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Расшифровка) = Тип("Строка") Тогда
		...
		ИначеЕсли Расшифровка = "УчетнаяПолитика" Тогда
			...
			ОткрытьФорму("РегистрСведений.УчетнаяПолитикаОрганизаций.ФормаСписка", ПараметрыФормы, ЭтаФорма);
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Оценка риска налоговой проверки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
