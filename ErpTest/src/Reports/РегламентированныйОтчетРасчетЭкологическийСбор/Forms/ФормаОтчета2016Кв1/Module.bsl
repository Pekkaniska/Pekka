...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(СтруктураРеквизитовФормы.Организация) Тогда 
		СтрокаСведений = "ИННЮЛ, КППЮЛ, АдрЮР, ОКАТО, ОКТМО, НаимЮЛПол, ОКОПФ, ОГРН, ТелОрганизации";
	...
	Иначе
		СтрокаСведений = "ИННФЛ, ОГРНИП, АдрМЖ, ОКАТО, ОКТМО, ФИО, ТелДом";
		Сведения = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(СтруктураРеквизитовФормы.Организация, ДатаПодписи, СтрокаСведений);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.КодОрганаРПН) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Не задан территориальный орган Росприроднадзора!", ИмяПоляТаблДокумента, "ОрганРПН");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки)
	...
	Узел_Вещество = ПолучитьПодчиненныйЭлемент(Узел_Док, "RPT_CALC_FACT");
