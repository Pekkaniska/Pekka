...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(Объект.Организация) Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Сообщение по форме ЕНВД-3 можно создавать только для организаций");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДанныеПоРегистрацииВИФНС()
	...
	Если ЗначениеЗаполнено(Реквизиты.Представитель) Тогда
		Документы.УведомлениеОСпецрежимахНалогообложения.УстановитьПредставителяПоФизЛицу(Объект, Реквизиты.Представитель, Титульный, "ИНН_ПРЕДСТАВИТЕЛЯ");
...
Процедура ЗагрузитьПодготовленныеДанные(Контейнер)
	...
	Контейнер.ДопСтроки.Колонки.Добавить("П_КПП1");
	...
	Для Каждого Стр Из СтраницыЛиста2 Цикл 
		...
		Стр.П_КПП1 = ТитульныйЛист[0].П_КПП;
	КонецЦикла;
