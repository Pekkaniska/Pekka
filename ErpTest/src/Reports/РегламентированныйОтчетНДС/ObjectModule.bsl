...
Процедура ПоказатьПериод(ИмяФормы, СтруктураДанныхТитульный, СтруктураРеквизитовФормы) Экспорт
	...
	Если (НЕ ЗначениеЗаполнено(ПериодОтчета)
		...
		ПериодОтчета = СоответствиеНомераМесяцаКодуОтчетногоПериода[
...
Функция СформироватьСтруктуруПараметров(ИмяФормы,
	...
	ПараметрыВыгрузки.Вставить("ДанныеСегментированы",
		СохраненныеДанные.Свойство("УспешноеАвтозаполнение")
...
Функция СформироватьИдФайл(ИмяФормы, ПараметрыВыгрузки)
	...
	Возврат ПараметрыВыгрузки.ПрефиксИмениФайлаВыгрузки
		+ "_" + ?(ЗначениеЗаполнено(ПараметрыВыгрузки.КодИФНСПосредника), ПараметрыВыгрузки.КодИФНСПосредника,
...
Процедура УстановитьПризнакиВыгрузкиРазделов8и9(ИмяФормы,
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел4,
			"П0004М1003003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел4,
			"П0004М2008003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
	...
	Для Каждого ЛистРаздела Из Раздел5 Цикл
		...
		Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеЛиста,
				"П000500005003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
			
			ЗаполненаРаздел5Строка050 = ЗаполненаРаздел5Строка050
									ИЛИ ДанныеЛиста["П000500005003_" + Формат(НомерСтроки, "ЧГ=")] > 0;
			ЗаполненаРаздел5Строка070 = ЗаполненаРаздел5Строка070
									ИЛИ ДанныеЛиста["П000500007003_" + Формат(НомерСтроки, "ЧГ=")] > 0;
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел6,
			"П0006М1003003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел6,
			"П0006М1004003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел6,
			"П0006М2010003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
	...
	НомерСтроки = 1;
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел6,
			"П0006М3012003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
...
Процедура ПроверитьВыгрузкуПоНовомуАлгоритмуНаСервере(ИмяФормы, СтруктураРеквизитовФормы) Экспорт
	...
	НаименованиеОтчета = СтруктураРеквизитовФормы.НаименованиеОтчета
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв1") Тогда
		РегламентированнаяОтчетностьПроверкаНДС.
			ВыявитьОшибкиЗаполненияПрепятствующиеФормированиюЭлектронногоПредставления_ФормаОтчета2017Кв1(
...
Процедура ЗаполнитьСодержательнуюЧасть(ИмяФормы, ДанныеОтчета, СтруктураРеквизитовФормы, УзелРодитель)
	...
	П100100004003 = ?(ЗначениеСуммыЗаполнено(ИмяФормы, Раздел1.П000100005003),
					  -Раздел1.П000100005003,
					  Раздел1.П000100004003);
...
Функция ЗначениеПоказателяЗаполнено(ИмяФормы, ЗначениеПоказателя, ЯвляетсяСуммой)
	...
	Иначе
		ПоказательЯвляетсяСуммой = (ЯвляетсяСуммой.Формат = "N"
								  И ЯвляетсяСуммой.МаксРазмерность >= 10
								  И ЯвляетсяСуммой.МинРазмерность = 99999);
	КонецЕсли;
	
	Возврат ?(ПоказательЯвляетсяСуммой,
			  ЗначениеСуммыЗаполнено(ИмяФормы, ЗначениеПоказателя),
...
Процедура КомпоновщикXML_ЗакрытьУзел(ИмяФормы, КомпоновщикXML)
	...
	Если КомпоновщикXML.НовыйУзелЗакрыт Тогда
		...
		КомпоновщикXML.ЗаписьТекста.Записать(Символы.ПС + КомпоновщикXML.Табуляция
											 + "</" + КомпоновщикXML.СтэкУзлов[0] + ">");
