...
Функция СформироватьЗаголовок(ПараметрыОтчета)
	...
	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Детализация по %1'"), ТекстПроИтоги);
	КонецЕсли;
	
	Если ПараметрыОтчета.ВыводитьЕдиницуИзмерения Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СокрЛП("" + ЗаголовокОтчета.Параметры.ТекстПроИтоги + Символы.ПС + НСтр("ru = 'Единица измерения: рубль (код по ОКЕИ 383)'"));
	КонецЕсли;
...
Процедура ВывестиШапкуТаблицы(ПараметрыОтчета, СтруктураПараметров, Выборка, ШиринаВыводимогоРаздела)
	...
	НачалоЗаголовка.Параметры.ОписаниеСчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Главная книга. Счет %1 ""%2""'"),
