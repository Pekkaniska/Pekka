...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ИменаМакетовТелаКомпоновки = Новый Массив;
	Для каждого Макет из МакетКомпоновки.Тело Цикл
		Если ТипЗнч(Макет) = Тип("ТаблицаМакетаКомпоновкиДанных") Тогда
			МакетТаблицы = Макет;
		Иначе
			ИменаМакетовТелаКомпоновки.Добавить(Макет.Макет);
		КонецЕсли;
	КонецЦикла;
	...
	МакетШапкиТаблицы = БухгалтерскиеОтчетыВызовСервера.ПолучитьМакетШапки(МакетКомпоновки, , ?(МакетКомпоновки.Тело.Количество() > 1, "Подвал", "Заголовок"));
	
	МакетыШапкиОтчета = Новый Массив;
	МакетыШапкиОтчета.Добавить(МакетШапкиТаблицы.Имя);

	Для Каждого Колонка Из МакетТаблицы.Колонки Цикл
	...
	ЗаполнитьМакетыРесурсовПодвалаОтчета(МакетТаблицы, МакетыРесурсовПодвалаОтчета, СоответствиеМакетовКолонкамОтчета);
	ЗаполнитьМакетыРесурсовГруппировки(МакетТаблицы.Строки, МакетыРесурсовСчета, СоответствиеМакетовКолонкамОтчета, "Счет");
	...
	ЗаполнитьМакетыРесурсовГруппировки(МакетТаблицы.Строки, МакетРесурсовГруппировкиВалюта, СоответствиеМакетовКолонкамОтчета, "Валюта", Истина);
	...
	Если ПараметрыОтчета.ПоказательВалютнаяСумма Тогда
		Для Каждого Макет Из МакетКомпоновки.Макеты Цикл 
			...
			Если МакетыШапкиОтчета.Найти(Макет.Имя) = Неопределено И ИменаМакетовТелаКомпоновки.Найти(Макет.Имя) = Неопределено Тогда
...
Процедура ДоработатьНаборДанных_ПоСубконто(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		СчетаДляОтдельногоЗапроса = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаДляОтдельногоЗапроса, СчетаПодзапроса);
...
Процедура ДоработатьНаборДанных_ПоСубконтоРазвернутое(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	// Вычтем из них сами счета с развернутым сальдо.
	СчетаДляОтдельногоЗапроса = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаДляОтдельногоЗапроса, СписокВсехСчетовРазвернутоеСальдо.ВыгрузитьЗначения());
	// Из оставшихся субсчетов вычитаем счета с подходящим составом субконто.
	СчетаДляОтдельногоЗапроса = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаДляОтдельногоЗапроса, СчетаСПодходящимСоставомСубконто.ВыгрузитьЗначения());
	...
	Если СчетаДляОтдельногоЗапроса.Количество() > 0 Тогда
		...
		// Взводим флаг, хотя формально разворота по субконто может и не быть
		ЕстьРазворотПоСубконто = Истина;
