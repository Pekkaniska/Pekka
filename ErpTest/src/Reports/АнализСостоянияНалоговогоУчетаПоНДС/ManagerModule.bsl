...
Функция ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос)
	...
	|	КлючиАналитикиУчетаПоПартнерам.Договор КАК ДоговорКонтрагента,
	|	РасчетыСКлиентами.РасчетныйДокумент КАК СчетФактура,
	|	РасчетыСКлиентами.Регистратор КАК Реализация
	|ПОМЕСТИТЬ ВТРеализацииДокументыОплаты
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоДокументам.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Регистратор, , ) КАК РасчетыСКлиентами
	...
	|	И РасчетыСКлиентами.ПредоплатаРеглОборот <> 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Реализация
	|;
	...
	|	-(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток
	|		- РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * &ПроцентНДС / (100 + &ПроцентНДС) КАК ЧИСЛО(31,2))) КАК СуммаБезНДС,
	|	-(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * &ПроцентНДС / (100 + &ПроцентНДС) КАК ЧИСЛО(31,2))) КАК СуммаНДС
	...
	|			И РасчетыСКлиентами.АналитикаУчетаПоПартнерам.Организация = ДанныеРасчетныхДокументов.Документ
	|ГДЕ
	|	РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток < РасчетыСКлиентами.ПредоплатаРеглНачальныйОстаток
	|;
	|
...
Функция ПолучитьНаборДанныхВычетСАвансовПолученныхУпрощенныйУчетНДС(Запрос)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&НачалоСледующегоПериода, 
	|											Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСПоАвансамИПредоплатам), 
...
Функция ПолучитьНаборДанныхВычетАвансыВыданныеУпрощенныйУчетНДС(Запрос)
	...
	|			И РасчетыСПоставщиками.АналитикаУчетаПоПартнерам.Организация = ДанныеРасчетныхДокументов.Документ
	|ГДЕ
	...
	|				И Авансы.СтавкаНДС = СоставКнигиПродажВЧастиАвансов.СтавкаНДС
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
