...
Функция ТекстЗапроса(ВалютаОтчета)
	...
	|	ВЫБОР КОГДА СУММА(Расчеты.Оплачено)+СУММА(Расчеты.Поставлено) = 0 
	|		ТОГДА СУММА(Расчеты.Зачтено)+СУММА(Расчеты.ПлановаяПоставка)
	|		ИНАЧЕ СУММА(Расчеты.Оплачено)+СУММА(Расчеты.Поставлено)
	...
	|		0                                                              КАК Зачтено,
	|		0                                                              КАК Оплачено,
	|		РасчетыПоСрокамОстатки.ДолгРеглНачальныйОстаток                КАК ДолгПоставщика,
