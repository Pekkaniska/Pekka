...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2015_1(Данные, УникальныйИдентификатор)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Данные.СтруктураРеквизитов._ОписПричНепред) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не заполнено описание причины непредоставления", "СведенияОДокументах", "_ОписПричНепред"));
		КонецЕсли;
		Если Данные.СтруктураРеквизитов._КодПрич <> 20 И Данные.СтруктураРеквизитов._КодПрич <> 10 Тогда
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указан код причины", "СведенияОДокументах", "_КодПрич"));
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Данные.СтруктураРеквизитов._СрокПредст) И Данные.СтруктураРеквизитов._КодПрич = 10 Тогда
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указан срок предоставления", "СведенияОДокументах", "_СрокПредст"));
		КонецЕсли;
		
		ЕстьДок = Ложь;
		НепрЗап = Ложь;
		Для Каждого Стр Из Данные.СтруктураРеквизитов._ПорНомДок Цикл 
			Если ЗначениеЗаполнено(Стр.НомДок) Тогда 
				ЕстьДок = Истина;
				Если Не НепрЗап Тогда 
					Если СтрДлина(Стр.НомДок) <> 4 
						Или Сред(Стр.НомДок, 2, 1) <> "." 
						Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Лев(Стр.НомДок, 1))
						Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Прав(Стр.НомДок, 2))Тогда 
						
						ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Неправильно заполнен список документов (не соответствует формату Х.ХХ)", "СведенияОДокументах", "_ПорНомДок1"));
						НепрЗап = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Если Не ЕстьДок Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не заполнен список документов", "СведенияОДокументах", "_ПорНомДок1"));
		КонецЕсли;
		...
		Для Каждого Стр Из Данные.СтруктураРеквизитов._ПриложенныеФайлы.Строки Цикл 
			Если Стр.Строки.Количество() = 0 Тогда 
				ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("К документу " + Стр.Документ + " не приложены файлы", "ПриложенныеДокументы", "_ПриложенныеФайлы"));
				Прервать;
			КонецЕсли;
		КонецЦикла;
