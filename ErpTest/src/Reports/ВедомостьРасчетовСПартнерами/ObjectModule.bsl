...
Функция ТекстЗапросаРасчетыСКлиентами(ВалютаОтчета, ТипРасчетов)
	...
	|	Расчеты.НашДолгНачальныйОстаток                                        КАК НашДолгНачальныйОстаток,
	|	Расчеты.УвеличениеДолга                                                КАК УвеличениеДолга,
	|	Расчеты.УменьшениеДолга                                                КАК УменьшениеДолга,
	|	Расчеты.НашДолгКонечныйОстаток                                         КАК НашДолгКонечныйОстаток,
	|	Расчеты.ДолгПартнераНачальныйОстаток                                   КАК ДолгПартнераНачальныйОстаток,
	|	Расчеты.ДолгПартнераКонечныйОстаток                                    КАК ДолгПартнераКонечныйОстаток,
	...
	|		РасчетыПоСрокам.ДолгРеглОстаток                                    КАК ДолгПартнераКонечныйОстаток,
	...
	|		0                                                                     КАК ДолгПартнераКонечныйОстаток,
	|		0                                                                     КАК НашДолгКонечныйОстаток,
	|		РасчетыПоСрокам.ДолгРеглПриход + РасчетыПоСрокам.ПредоплатаРеглРасход КАК УвеличениеДолга,
	|		РасчетыПоСрокам.ДолгРеглРасход + РасчетыПоСрокам.ПредоплатаРеглПриход КАК УменьшениеДолга,
	|		РасчетыПоСрокам.Регистратор                                           КАК Регистратор
	|	ИЗ
	|		РегистрНакопления.РасчетыСКлиентамиПоСрокам.Обороты({(&НачалоПериода)}, {(&КонецПериода)},Регистратор , ) КАК РасчетыПоСрокам
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаУчета
	|			ПО РасчетыПоСрокам.АналитикаУчетаПоПартнерам = АналитикаУчета.КлючАналитики
	|	ГДЕ
	|		АналитикаУчета.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
...
Функция ТекстЗапросаРасчетыСПоставщиками(ВалютаОтчета, ТипРасчетов)
	...
	|		0                                                   КАК УвеличениеДолга,
	|		0                                                   КАК УменьшениеДолга,
	|		РасчетыПоСрокам.ПредоплатаРеглОстаток               КАК ДолгПартнераКонечныйОстаток,
	|		РасчетыПоСрокам.ДолгРеглОстаток                     КАК НашДолгКонечныйОстаток,
	|		1                                                   КАК ТипРасчетов,
	|		Неопределено                                        КАК Регистратор

	|	ИЗ
	|		РегистрНакопления.РасчетыСПоставщикамиПоСрокам.Остатки({(&КонецПериода)}, ) КАК РасчетыПоСрокам
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаУчета
	|			ПО РасчетыПоСрокам.АналитикаУчетаПоПартнерам = АналитикаУчета.КлючАналитики
	|	ГДЕ
	|		АналитикаУчета.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	...
	|		АналитикаУчета.Договор                                                                                        КАК Договор,
	|		АналитикаУчета.НаправлениеДеятельности                                                                        КАК НаправлениеДеятельности,
	|		РасчетыСПоставщикамиПоСрокамОбороты.ОбъектРасчетов                                                            КАК ОбъектРасчетов,
	|		РасчетыСПоставщикамиПоСрокамОбороты.Валюта                                                                    КАК Валюта,
	|		РасчетыСПоставщикамиПоСрокамОбороты.РасчетныйДокумент                                                         КАК РасчетныйДокумент,
	|		РасчетыСПоставщикамиПоСрокамОбороты.ДатаВозникновения                                                         КАК ДатаВозникновения,
	|		РасчетыСПоставщикамиПоСрокамОбороты.ДатаПлановогоПогашения                                                    КАК ДатаПлановогоПогашения,
	|		0                                                                                                             КАК ДолгПартнераНачальныйОстаток,
	|		0                                                                                                             КАК НашДолгНачальныйОстаток,
	|		РасчетыСПоставщикамиПоСрокамОбороты.ДолгРеглРасход + РасчетыСПоставщикамиПоСрокамОбороты.ПредоплатаРеглПриход КАК УвеличениеДолга,
	|		РасчетыСПоставщикамиПоСрокамОбороты.ДолгРеглПриход + РасчетыСПоставщикамиПоСрокамОбороты.ПредоплатаРеглРасход КАК УменьшениеДолга,
	|		0                                                                                                             КАК ДолгПартнераКонечныйОстаток,
	|		0                                                                                                             КАК НашДолгКонечныйОстаток,
	|		1                                                                                                             КАК ТипРасчетов,
	|		РасчетыСПоставщикамиПоСрокамОбороты.Регистратор                                                               КАК Регистратор
	|	ИЗ
	|		РегистрНакопления.РасчетыСПоставщикамиПоСрокам.Обороты({(&НачалоПериода)}, {(&КонецПериода)}, Регистратор, ) КАК РасчетыСПоставщикамиПоСрокамОбороты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаПоПартнерам КАК АналитикаУчета
	|			ПО РасчетыСПоставщикамиПоСрокамОбороты.АналитикаУчетаПоПартнерам = АналитикаУчета.КлючАналитики
	|	ГДЕ
	|		АналитикаУчета.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.НашеПредприятие)
	|	{ГДЕ
	|		АналитикаУчета.Организация.* КАК Организация,
	...
	|					ОтборПоСегментуПартнеров
	|				ГДЕ
	|					ОтборПоСегментуПартнеров.ИспользуетсяОтборПоСегментуПартнеров = &ИспользуетсяОтборПоСегментуПартнеров)) КАК Поле4}) КАК Расчеты
	|ГДЕ
	|	(&ОтборыВыбранныхПолей)
	|	И &ТипРасчетов";
