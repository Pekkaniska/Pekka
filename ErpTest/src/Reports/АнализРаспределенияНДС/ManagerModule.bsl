...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	ВыводитьОбщийЭкспорт = 
		НЕ УчетНДСУПСлужебный.УпрощенныйПорядокВычетаНДСПоНесырьевомуЭкспорту(ПараметрыОтчета.НачалоПериода)
		И ПолучитьФункциональнуюОпцию("УдалитьИспользоватьНДС0");
		
	ВыводитьЭкспортНесырьевыхТоваров = 
		УчетНДСУПСлужебный.УпрощенныйПорядокВычетаНДСПоНесырьевомуЭкспорту(ПараметрыОтчета.КонецПериода)
...
Процедура ВывестиВОтчетДанныеПоРаспределениюКосвенныхРасходов(ПериодБазы, ПараметрыОтчета, ДокументРезультат, Макет)
	...
	|		ПО 
	|			ПартииПрочихРасходов.ДокументПоступленияРасходов = СчетФактураПолученныйНА.Ссылка
	...
	|	КОНЕЦ = &ПериодБазы
	// ++ НЕ УТ
	|	И НЕ (ЕСТЬNULL(ПартииПрочихРасходов.СтатьяРасходов.ВариантРаспределенияРасходовРегл,ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка)) 
	|			= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|			ИЛИ ПартииПрочихРасходов.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию))
	// -- НЕ УТ
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ВывестиВОтчетДанныеПоРаспределениюОС(ПериодБазы, ПараметрыОтчета, ДокументРезультат, Макет)
	...
	|	РегистрНакопления.ПартииПрочихРасходов КАК ПартииПрочихРасходов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученный
	|		ПО ПартииПрочихРасходов.ДокументПоступленияРасходов = СчетФактураПолученный.ДокументОснование
	|			И СчетФактураПолученный.Ссылка.Проведен
	|			И НЕ СчетФактураПолученный.Ссылка.Исправление
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученныйНалоговыйАгент КАК СчетФактураПолученныйНА
	|		ПО ПартииПрочихРасходов.ДокументПоступленияРасходов = СчетФактураПолученныйНА.Ссылка
	|			И СчетФактураПолученныйНА.Проведен
	|			И НЕ СчетФактураПолученныйНА.Исправление
	...
	|		ИНАЧЕ КОНЕЦПЕРИОДА(ПартииПрочихРасходов.Период, МЕСЯЦ)
	|	КОНЕЦ = &ПериодБазы
	|	И (ЕСТЬNULL(ПартииПрочихРасходов.СтатьяРасходов.ВариантРаспределенияРасходовРегл,ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.ПустаяСсылка))
	|			= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|			ИЛИ ПартииПрочихРасходов.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию))
	|
	|СГРУППИРОВАТЬ ПО
	|	ПартииПрочихРасходов.Регистратор,
	|	ЕСТЬNULL(СчетФактураПолученный.Ссылка,СчетФактураПолученныйНА.Ссылка),
	|	ПартииПрочихРасходов.ДокументПоступленияРасходов,
	|	ВЫБОР
	|		КОГДА ПартииПрочихРасходов.АналитикаРасходов ССЫЛКА Справочник.ОбъектыЭксплуатации
	|			ТОГДА ""ОС""
	...
	|						= ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|						ИЛИ ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию))) КАК ПартииПрочихРасходовОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученный
	|		ПО ПартииПрочихРасходовОстатки.ДокументПоступленияРасходов = СчетФактураПолученный.ДокументОснование
	|			И СчетФактураПолученный.Ссылка.Проведен
	|			И НЕ СчетФактураПолученный.Ссылка.Исправление
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученныйНалоговыйАгент КАК СчетФактураПолученныйНА
	|		ПО ПартииПрочихРасходовОстатки.ДокументПоступленияРасходов = СчетФактураПолученныйНА.Ссылка
	|			И СчетФактураПолученныйНА.Проведен
	|			И НЕ СчетФактураПолученныйНА.Исправление
