...
Функция РеквизитыСравнения()
	...
	ОписаниеРаздела.Реквизиты.Добавить(ОписаниеРеквизита("ОбъемНаработки", НСтр("ru='Предполагаемый объем наработки (продукции или выполненных работ)'"),, "ЧДЦ=2"));
	ОписанияРазделов.Добавить(ОписаниеРаздела);
...
Функция ТекстЗапросаДанныеБУ()
	...
	|		ИНАЧЕ СчетаУчета.СчетУчета
	|	КОНЕЦ КАК СчетУчета,
	|	ВЫБОР
	|		КОГДА СчетаУчета.ГруппаФинансовогоУчета <> ЗНАЧЕНИЕ(Справочник.ГруппыФинансовогоУчетаВнеоборотныхАктивов.ПустаяСсылка)
	|			ТОГДА СчетаУчета.ГруппаФинансовогоУчета.СчетУчетаАмортизации 
	|		ИНАЧЕ СчетаУчета.СчетНачисленияАмортизации
