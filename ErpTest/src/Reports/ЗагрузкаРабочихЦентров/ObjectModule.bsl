...
Процедура УстановитьРасширенноеОформлениеСерии(Серия) Экспорт
	...
	Если СтрНайти(Серия.Текст, НСтр("ru= 'Высокая загрузка, выше доступного времени'")) <> 0
		ИЛИ СтрНайти(Серия.Текст, НСтр("ru= 'Высокая загрузка, ниже доступного времени'")) <> 0 Тогда
		
		Серия.Цвет = WebЦвета.ЛососьСветлый;
		
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru= 'Доступное время'")) Тогда
		
		Серия.Цвет = WebЦвета.СветлоСерый;
		
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru= 'Нормальная загрузка'")) Тогда
		
		Серия.Цвет = WebЦвета.СинийСоСтальнымОттенком;
		
	ИначеЕсли СтрНайти(Серия.Текст, НСтр("ru= 'Низкая загрузка'")) > 0 Тогда
