...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Инфо.Вставить("Наименование", НСтр("ru = 'Расчеты с Клиентами БУ'"));
	Инфо.Вставить("Версия",       "1.0");
	Инфо.Вставить("ВерсияБСП",    "1.2.1.4");
	Инфо.Вставить("Информация",    НСтр("ru = 'Отчет предназначен для анализа расхождений данных регистра Расчеты с Клиентами по документам и данных на счетах БУ'"));
	...
	Команда.Представление = НСтр("ru = 'Расчеты с Клиентами БУ'");
	Команда.Идентификатор = "пкОткрытьОтечРасчетыСКлиентамиБУ";
...
//Рарус Владимир Подрезов
	ПараметрСчет013 = НастройкиОтчета.ПараметрыДанных.Элементы.Найти("Счет013");
	Если ПараметрСчет013 <> Неопределено Тогда
		Счет013 = ПланыСчетов.Хозрасчетный.НайтиПоКоду("013");
		Если Не ЗначениеЗаполнено(Счет013) Тогда
			тСообщение = Новый СообщениеПользователю;
			тСообщение.Текст = НСтр("ru='Не удалось определить счет 013 в плане счетов!'");
			тСообщение.Сообщить();
		Иначе
			ПараметрСчет013.Значение		= Счет013;
			ПараметрСчет013.Использование	= Истина;
		КонецЕсли;
	КонецЕсли;
