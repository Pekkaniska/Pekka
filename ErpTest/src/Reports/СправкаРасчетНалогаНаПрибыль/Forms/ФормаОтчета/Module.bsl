...
Функция ПодготовитьПараметрыОтчета()
	...
	ИначеЕсли Отчет.НаборПоказателейОтчета = 3 Тогда
		...
		ПараметрыОтчета.Вставить(
			"НазваниеНабораПоказателейОтчета",
			НСтр("ru = 'бухгалтерский учет с постоянными
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru = 'Справка-расчет налога на прибыль'") + " " + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Процедура ИнициализацияКомпоновщикаНастроек()
	...
	БухгалтерскиеОтчетыВызовСервера.ИнициализацияКомпоновщикаНастроек(ЭтаФорма, ОрганизацияИзменилась, "РасчетНалогаНаПрибыль");
