...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Если ПараметрыОтчета.РазвернутоеСальдо Тогда
		...
		МакетПодвалаТаблицы = МакетКомпоновки.Макеты[МакетТаблицы.Строки[0].МакетПодвала.Макет];
		...
		Для Каждого МакетРесурсаПодвала Из МакетТаблицы.Строки[0].МакетПодвала.МакетРесурсов Цикл
		...
		ЗаполнитьМакетыРесурсовГруппировки(МакетТаблицы.Строки, МакетыРесурсов, СоответствиеМакетовКолонкамОтчета, , Истина, Истина);
	...
	Для Каждого СтрокаГруппировка Из ПараметрыОтчета.Группировка Цикл
		...
		Если НайденаВалюта Тогда // Нужны все последующие группировки.
			...
			ИменаМакетовРесурсов = ЗаполнитьМакетыРесурсовГруппировки(МакетТаблицы.Строки, 
	...
	// Добавим в макеты шапки еще и макеты, появившиеся в результате вывода параметров:
	Для каждого Макет из МакетКомпоновки.Тело Цикл
		Если Макет = МакетТаблицы Тогда
			Продолжить;
		КонецЕсли;
		МакетыШапкиОтчета.Добавить(Макет.Макет);
	КонецЦикла;
