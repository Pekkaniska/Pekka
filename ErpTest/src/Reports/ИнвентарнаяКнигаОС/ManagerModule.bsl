...
Процедура СформироватьСтраницыИнвентарнойКнигиОС(ПараметрыОтчета)
	...
	|				ИНАЧЕ СчетаБухгалтерскогоУчетаОС.СчетНачисления" + ?(ИДКонфигурации = "БАУ" ИЛИ ИДКонфигурации = "БАУКОРП", "Износа", "Амортизации") + " 
	...
	|							КОНЕЦПЕРИОДА(ПринятиеКУчетуОС.Дата, МЕСЯЦ) КАК ДатаДвиженийБухгалтерии,
	...
	|							КОНЕЦПЕРИОДА(РазукомплектацияОС.Дата, МЕСЯЦ) КАК ДатаДвиженийБухгалтерии,
	...
	|						ВЫБРАТЬ
	|							ПередачаОС.Ссылка,
	|							ПередачаОС.ДатаПередачи,
	|							""ПередачаОС"",
	|							"""",
	|							ПередачаОС.СобытиеПередачиОС
	|						ИЗ
	|							Документ.ПодготовкаКПередачеОС КАК ПередачаОС 
	|						ГДЕ 
	|							ПередачаОС.ПередачаВыполнена
	|						
	|						ОБЪЕДИНИТЬ ВСЕ
	|						
	|						ВЫБРАТЬ
	...
	|							NULL
	|						ИЗ
	|							Документ.ПодготовкаКПередачеОС2_4 КАК ПередачаОС 
	|						
	|						ОБЪЕДИНИТЬ ВСЕ
	|						
	|						ВЫБРАТЬ
	...
	|						ГДЕ
	|							НЕ СписаниеОС.ЧастичнаяЛиквидация
	|						
	|						ОБЪЕДИНИТЬ ВСЕ
	|						
	|						ВЫБРАТЬ
	...
	|							НЕ СписаниеОС.ЧастичнаяЛиквидация
	|						
	|						) КАК ТаблицаДокументы
	|						ПО СобытияОСОрганизаций.Регистратор = ТаблицаДокументы.Ссылка И СобытияОСОрганизаций.Событие = ЕСТЬNULL(ТаблицаДокументы.Событие, СобытияОСОрганизаций.Событие)
	|				
	|				ГДЕ
	|" + ОтборПоПериоду + "
	...
	|					ПО ДвиженияОС.ДатаДвиженийБухгалтерии >= ПервоначальныеСведенияОСБухгалтерскийУчет.Период И ДвиженияОС.ОсновноеСредство = ПервоначальныеСведенияОСБухгалтерскийУчет.ОсновноеСредство
