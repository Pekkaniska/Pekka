...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Справка-расчет резервов по сомнительным долгам" + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(Отчет.СНачалаГода,НачалоГода(Отчет.НачалоПериода),Отчет.НачалоПериода), Отчет.КонецПериода);
...
Процедура ИнициализацияКомпоновщикаНастроек()
	...
	БухгалтерскиеОтчетыВызовСервера.ИнициализацияКомпоновщикаНастроек(ЭтаФорма, ОрганизацияИзменилась, "СомнительныеДолги");
