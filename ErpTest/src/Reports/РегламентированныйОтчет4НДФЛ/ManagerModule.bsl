...
Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	...
	Если ЭкземплярРеглОтчета.ВыбраннаяФорма = "ФормаОтчета2014кв1" Тогда
		
		ДанныеДекларации = ПрочитатьДанныеДекларации(ДанныеОтчета, Период, "ОКТМО, ПредполагаемаяСуммаДоходов");
...
Функция ФормаПоУмолчанию(Знач Период) Экспорт
	...
	Для Каждого Строка Из ТаблицаФормОтчета Цикл
		...
		Если (Строка.ДатаНачалоДействия <= Период) И (Период <= Строка.ДатаКонецДействия Или Не ЗначениеЗаполнено(Строка.ДатаКонецДействия)) Тогда
			ВыбраннаяФорма = Строка.ФормаОтчета;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ВыбраннаяФорма = Неопределено И ТаблицаФормОтчета.Количество() > 0 Тогда
		ВыбраннаяФорма = ТаблицаФормОтчета[0].ФормаОтчета;
	КонецЕсли;
...
//
Процедура СоздатьЗаполнитьОтчет(РегламентированныйОтчет, Организация, Период, НомерКорректировки, ДанныеЗаполнения) Экспорт
	
	СтруктураРеквизитовФормы = СтруктураРеквизитовФормы(Организация, Период);
	...
	КлючиПоказателей = КлючиПоказателейДекларации(Период);
	...
	РегламентированныйОтчет.ДеревоНастройкиСтраниц = Новый ХранилищеЗначения(ДеревоСтраницОтчета(Период));
	Попытка
...
Функция НоваяСтруктураДанныхОтчета(Организация, Период, СтруктураРеквизитовФормы)
	...
	СвойстваРазделовДекларации = СвойстваРазделовДекларации(Период);
...
Функция ПрочитатьДанныеДекларации(ДанныеОтчета, Период, Знач Показатели) Экспорт
	...
	Если ТипЗнч(Показатели) = Тип("Строка") Тогда
		Показатели = СтрРазделить(Показатели, ",", Ложь);
	КонецЕсли;
...
Функция КлючиПоказателейДекларации(Период)
...
Функция СтруктураРеквизитовФормы(Организация, Период, ПараметрыОткрытойФормы = Неопределено) Экспорт
	
	ПараметрыФормы = НовыеПараметрыФормы(Организация, Период);
	Если ПараметрыОткрытойФормы <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыФормы, ПараметрыОткрытойФормы);
	КонецЕсли;
	
	Если ПараметрыФормы.мВыбраннаяФорма = "ФормаОтчета2014кв1" Тогда
...
Функция СвойстваРазделовДекларации(Период) Экспорт
...
Функция ДеревоСтраницОтчета(Период) Экспорт
...
Функция НовыеПараметрыФормы(Организация, Период, мСохраненныйДок = Неопределено, мСкопированаФорма = Неопределено)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("мВыбраннаяФорма", ФормаПоУмолчанию(Период));
