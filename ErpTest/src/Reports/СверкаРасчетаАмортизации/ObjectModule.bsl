...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ЗначенияОтбораДанных.НачалоПериода <> Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить() Тогда
		
		ТекстСообщения = НСтр("ru = 'Начало периода должно совпадать с датой начала учета версии 2.4 (%1)'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, Формат(Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить(), "ДЛФ=D"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ); 
...
Функция СформироватьВнешниеНаборыДанных(ЗначенияОтбораДанных, СписокПроблем)
	...
	ДобавитьОтсутствующийНабор("ПараметрыАмортизацииОС_Регл_24", СхемаКомпоновкиДанных.НаборыДанных.ПараметрыАмортизации, ВнешниеНаборыДанных);
	...
	ДобавитьОтсутствующийНабор("ПараметрыАмортизацииНМА_Регл_24", СхемаКомпоновкиДанных.НаборыДанных.ПараметрыАмортизации, ВнешниеНаборыДанных);
...
Процедура ДобавитьИнформационноеСообщениеВШапку(ТабДок, ЗначенияОтбораДанных, СписокПроблем)
	...
	ПроверитьПорядокУчетаВНАВУпрУчете(ЗначенияОтбораДанных, СписокПроблем);
...
Процедура ДобавитьАмортизацию24(ИмяОбъектов, ЗначенияОтбораДанных, ВнешниеНаборыДанных, СписокПроблем)
	...
	ПараметрыАмортизации_Регл = Неопределено;
	ПараметрыАмортизации_Упр = Неопределено;
	
	ТекущийПериод = ЗначенияОтбораДанных.НачалоПериода;
	Пока ТекущийПериод <= ЗначенияОтбораДанных.ОкончаниеПериода Цикл
		
		ПараметрыРасчетаАмортизации = ВнеоборотныеАктивы.НовыеПараметрыРасчетаАмортизации();
		...
		ПараметрыРасчетаАмортизации.ОбъектыУчета = ИмяОбъектов;
		...
		ПараметрыРасчетаАмортизации.ЭтапРасчета = ЭтапРасчета;
		
		Если ИмяОбъектов = "ОС" Тогда
			Документы.АмортизацияОС2_4.РассчитатьАмортизацию(ПараметрыРасчетаАмортизации, Неопределено);
		Иначе
			Документы.АмортизацияНМА2_4.РассчитатьАмортизацию(ПараметрыРасчетаАмортизации, Неопределено);
		КонецЕсли; 
		...
		Если НЕ ПараметрыРасчетаАмортизации.ЕстьОшибки Тогда
			...
			Если СписокПараметровАмортизации <> Неопределено Тогда
				...
				Если ПараметрыАмортизации_Регл = Неопределено Тогда
					ПараметрыАмортизации_Регл = СписокПараметровАмортизации.ПараметрыАмортизации_Регл;
				Иначе
				...
				Если ЗначенияОтбораДанных.СверкаРасчетаУпр Тогда
					Если ПараметрыАмортизации_Упр = Неопределено Тогда
						ПараметрыАмортизации_Упр = СписокПараметровАмортизации.ПараметрыАмортизации_Упр;
					Иначе
	...
	Если ПараметрыАмортизации_Регл <> Неопределено Тогда
		...
		ОбработатьТаблицуПараметровАмортизации24(ИмяОбъектов, ПараметрыАмортизации_Регл);
		ОчиститьНеиспользуемыеПараметры("БУ", ИмяОбъектов, "_24", ПараметрыАмортизации_Регл);
		
		ВнешниеНаборыДанных.Вставить("ПараметрыАмортизации" + ИмяОбъектов + "_Регл_24", ПараметрыАмортизации_Регл);
	
	КонецЕсли; 
	
	Если ПараметрыАмортизации_Упр <> Неопределено Тогда
		
		ОбработатьТаблицуПараметровАмортизации24(ИмяОбъектов, ПараметрыАмортизации_Упр);
		ОчиститьНеиспользуемыеПараметры("УУ", ИмяОбъектов, "_24", ПараметрыАмортизации_Упр);
		
		ВнешниеНаборыДанных.Вставить("ПараметрыАмортизации" + ИмяОбъектов + "_Упр_24", ПараметрыАмортизации_Упр);
...
Процедура ДобавитьАмортизациюМеждународный(ИмяОбъектов, ЗначенияОтбораДанных, ВнешниеНаборыДанных, СписокПроблем)
	...
	Пока ТекущийПериод <= ЗначенияОтбораДанных.ОкончаниеПериода Цикл
		...
		Для каждого Организация Из ЗначенияОтбораДанных.СписокОрганизаций Цикл
			...
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = '- не удалось расчитать амортизацию по причине: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				СписокПроблем.Добавить(ТекстСообщения);
			КонецПопытки; 
...
Процедура СоздатьЗаданияКРасчетуАмортизации(ЗначенияОтбораДанных, МенеджерВременныхТаблиц)
	...
	|	И ЗаданияКРасчетуАмортизации.Организация В(&ОкончаниеПериода)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура НастроитьПоФункциональнымОпциям(КомпоновщикНастроекФормы)
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьУчетВнеоборотныхАктивовПоНаправлениямДеятельности") Тогда
		...
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "ПустаяЯчейкаЦФ1");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "ПустаяЯчейкаЦФ2");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "ПустаяЯчейкаЦФ3");
		
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияЦФ_22");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияНУЦФ_22");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияПРЦФ_22");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияВРЦФ_22");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияУУЦФ_22");
		
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияЦФ_24");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияНУЦФ_24");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияПРЦФ_24");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияВРЦФ_24");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "АмортизацияУУЦФ_24");
		
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "РасхождениеЦФ");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "РасхождениеНУЦФ");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "РасхождениеПРЦФ");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "РасхождениеВРЦФ");
		КомпоновкаДанныхСервер.УдалитьВыбранноеПолеИзВсехНастроекОтчета(КомпоновщикНастроекФормы, "РасхождениеУУЦФ");
...
Процедура ПроверитьНаличиеДокументовВПроверяемомПериоде(ЗначенияОтбораДанных, СписокПроблем)
	...
	ТипыДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.ВозвратОСОтАрендатора"));
	ТипыДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.ВозвратОСОтАрендатора2_4"));
	...
	ТипыДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.ПередачаОСАрендатору"));
	ТипыДокументов.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных("Документ.ПередачаОСАрендатору2_4"));
...
Процедура ПроверитьКорректностьПерехода(ЗначенияОтбораДанных, СписокПроблем)
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьДокументыВнеоборотныхАктивовМеждународныйУчет")
		И Константы.УчетВФункциональнойВалюте.Получить() = Перечисления.ВидыУчетаВФункциональнойВалюте.ВВалютеУпр
		И Константы.ВалютаФункциональная.Получить() <> Константы.ВалютаУправленческогоУчета.Получить() Тогда
		
		ТекстСообщения = НСтр("ru = '- валюта международного учета не совпадает с валютой упр. учета - сверка расчета в упр. учете невозможна'");
		СписокПроблем.Добавить(ТекстСообщения);
...
Процедура ПроверитьПорядокУчетаВНАВУпрУчете(ЗначенияОтбораДанных, СписокПроблем)

	Если НЕ ЗначенияОтбораДанных.СверкаРасчетаУпр Тогда
		Возврат;
	КонецЕсли; 
	
	Если Константы.ПорядокУчетаВНАВУпрУчете.Получить() = Перечисления.ПорядокУчетаВНАВУпрУчете.ПоСтандартамРегл Тогда
		ТекстСообщения = НСтр("ru = '- амортизация в управленческом учете может не совпадать, т.к. учет внеоборотных активов в управленческом учете ведется по стандартам регламентированного учета'");
		СписокПроблем.Добавить(ТекстСообщения);
	КонецЕсли; 
...
Функция ПолучитьЗначенияОтбораДанных(НастройкиОсновнойСхемы)
	...
	ЗначенияОтбораДанных.Вставить("СписокОрганизаций", ЗначениеОтбора("Организация", ЭлементыОтбора));
	...
	Если ЗначенияОтбораДанных.СписокОрганизаций.Количество() = 0 Тогда
		ЗначенияОтбораДанных.СписокОрганизаций = Обработки.ПомощникПереходаНаУчетВнеоборотныхАктивовВерсии24.СписокРазрешенныхОрганизаций();
	КонецЕсли;
	...
	СверкаРасчетаУпр = ПолучитьФункциональнуюОпцию("ИспользоватьДокументыВнеоборотныхАктивовМеждународныйУчет")
						И Константы.УчетВФункциональнойВалюте.Получить() = Перечисления.ВидыУчетаВФункциональнойВалюте.ВВалютеУпр
						И Константы.ВалютаФункциональная.Получить() = Константы.ВалютаУправленческогоУчета.Получить();
	//-- НЕ УТКА
