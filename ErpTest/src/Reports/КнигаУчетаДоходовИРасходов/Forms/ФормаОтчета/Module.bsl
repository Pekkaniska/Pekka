...
Процедура УправлениеФормой(Форма)
	...
	Элементы.РежимПечатиНДС.Доступность = Форма.ПрименяетсяУСНДоходыМинусРасходы;
...
Процедура ОбновитьНастройки()
	...
	РасходыНаНДСПоОплатеПоставщику = Ложь;
	
	Если (Отчет.РежимПечатиНДС = 1)
		И (НЕ ПрименяетсяУСНДоходыМинусРасходы ИЛИ РасходыНаНДСПоОплатеПоставщику) Тогда
		Отчет.РежимПечатиНДС = 2;
	КонецЕсли;
...
Процедура ЗаполнитьСведенияОНалоговомПериоде()
	...
	МинимальныйПериод = НачалоКвартала(ДатаРегистрации);
	
	Если Отчет.КонецПериода < МинимальныйПериод Тогда
		...
		ПериодНарастающимИтогом = НачалоГода(Отчет.НачалоПериода) = НачалоКвартала(Отчет.НачалоПериода);
		
		Отчет.НачалоПериода  = ?(ПериодНарастающимИтогом, НачалоГода(Отчет.КонецПериода), НачалоКвартала(Отчет.КонецПериода));
		Отчет.КонецПериода   = КонецКвартала(МинимальныйПериод);
	КонецЕсли;
	
	НалоговыйПериодРасширен = УчетУСНСервер.НалоговыйПериодРасширен(Отчет.Организация,
		Отчет.КонецПериода, ДатаРегистрации);
	НалоговыйПериодПропущен = УчетУСНСервер.НалоговыйПериодПропущен(Отчет.Организация,
		Отчет.КонецПериода, ДатаРегистрации);
	...
	НачалоНалоговогоПериода = ?(НалоговыйПериодРасширен, НачалоДня(ДатаРегистрации), НачалоГода(Отчет.КонецПериода));
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ПрименяетсяУСНДоходыМинусРасходы", ПрименяетсяУСНДоходыМинусРасходы);
...
Функция СторноКУДиРПоТекущимЗаписямДокумента(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Свойство("ДокументВозникновенияРасходов") Тогда
		ДополнительныеПараметрыСторнирования.Вставить("ДокументВозникновенияРасходов", ДополнительныеПараметры.ДокументВозникновенияРасходов);
	КонецЕсли;
	...
	КонецЕсли;
	Возврат УчетУСНСервер.СторноКУДиРПоТекущимЗаписямДокумента(ДополнительныеПараметры.Регистратор, ДополнительныеПараметрыСторнирования);
КонецФункции
