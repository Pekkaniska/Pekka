...
//++ Рарус Лимаренко 21.06.2018
	ЭтотОбъект.Отчет.Менеджер = неопределено;	
	Если РольДоступна("пкПросмотрОтчетаSalesmanReportПоМенеджеру") И НЕ РольДоступна("пкПросмотрОтчетаSalesmanReport") 
		И НЕ РольДоступна("ПолныеПрава") Тогда
		ЭтотОбъект.Отчет.Менеджер = Пользователи.ТекущийПользователь();
		Элементы.Менеджер.Видимость = Ложь;
		Элементы.Менеджер.Доступность = Ложь;	
	КонецЕсли;	
//-- Рарус Лимаренко 21.06.2018
