...
#Область ПодключаемыеСтандартныйПериод
...
#Область ПодключаемыеСписокСКнопкойПодбор
...
#Область ОбработчикиСобытийЭлементовТаблицыОтборы
...
Процедура ПоказатьРезультатЗаполненияНастроек()
	...
	Для Каждого Узел Из Отборы.ПолучитьЭлементы() Цикл
		Элементы.Отборы.Развернуть(Узел.ПолучитьИдентификатор(), Истина);
	КонецЦикла;
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Для Каждого СвойстваНастройки Из ВыводимыеНастройки Цикл
		...
		Если (СвойстваНастройки.Тип = "Отбор")
...
Функция НайтиПользовательскуюНастройкуЭлемента(ИдентификаторЭлемента) Экспорт
	...
	Если ИдентификаторЭлемента = "Отборы" Тогда
...
Процедура ЗарегистрироватьЭлементыПараметров(Информация)
	...
	СтрокаРаздел.ИндексКартинки = СтрокаРаздел.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("ПараметрыДанных");
...
Процедура ЗарегистрироватьЭлементыОтборов(Информация, НаборСтрокВарианта = Неопределено, НаборСтрокФормы = Неопределено)
	...
	Если НаборСтрокФормы = Неопределено Тогда
		...
		СтрокаРаздел.ИндексКартинки = СтрокаРаздел.ИндексКартинки = ОтчетыКлиентСервер.ИндексКартинки("Отборы");
...
#Область ОчисткаОтНесуществующихПолей
