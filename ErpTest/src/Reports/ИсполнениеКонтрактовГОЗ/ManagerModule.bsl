...
Процедура ВывестиСтрокиТаблицы(Результат, ПараметрыОтчета, Макет, ЦелевыеПараметры = Неопределено)
	...
	Для каждого Правило Из ПравилаРасчетаПоказателей Цикл
		...
		КодПоказателя = Правило.КодПоказателя;
		// Определим уровень группировки исходя из кода ячейки
		Если Прав(КодПоказателя, 2) = "00" Тогда
			Уровень = 1;
		ИначеЕсли Прав(КодПоказателя, 1) = "0" Тогда 
			Уровень = 2;
		Иначе
			Уровень = 3;
		КонецЕсли;
		...
		Если (Раздел = "1" ИЛИ Раздел = "2" ИЛИ Раздел = "3") И Уровень <> 3 Тогда
			Продолжить;
		КонецЕсли;
		
		Если Раздел = "4" ИЛИ Раздел = "5" Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПолучениеДанных(ПараметрыОтчета, ПравилаРасчетаПоказателей, ЦелевыеПараметры = Неопределено)
	...
	Если ПараметрыОтчета.Детализация = "ДоРегистратора" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Авто", "Регистратор");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ХозрасчетныйОбороты.Регистратор", "НЕОПРЕДЕЛЕНО");
	КонецЕсли;
	Если ПараметрыОтчета.ЕстьДанныеНаПлановуюДатуЗавершения Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Авто", "Период");
	КонецЕсли;
	Если НЕ ПараметрыОтчета.ЕстьДанныеНаПлановуюДатуЗавершения И НЕ ПараметрыОтчета.Детализация = "ДоРегистратора" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ХозрасчетныйОбороты.Период", "ДАТАВРЕМЯ(1,1,1)");
	КонецЕсли;
...
Функция ТаблицаПравилРасчетаПоказателей(ПараметрыОтчета)
	...
	СчетаУчетаРасчетыПоОплатеТруда = СчетаУчетаРасчетыПоОплатеТруда();
...
#Область ФинансированиеКонтракта
	...
	ОтборОборотов.КорСчет = СчетаУчетаДенеженыеСредстваНаСчетах;
...
#Область РаспределениеСредствКонтракта
	...
	ОтборОборотов = ОтборОборотов();
	ОтборОборотов.Счет = СчетаУчетаПроизводство;
	ОтборОборотов.Субконто1 = Перечисления.ТипыЗатратРегл.Работы;
	ОтборОборотов.НаправлениеДеятельности = "Контракт";
	ОтборОборотов.КорСчет = СчетаУчетаПроизводство;
	ОтборОборотов.КорНаправлениеДеятельности = "Контракт";
	ОтборОборотов.ТипИтогов =  Перечисления.ТипыИтогов.ОборотДт;
	...
	ОтборОборотов.НаправлениеДеятельности = "Контракт";
	ОтборОборотов.КорСчет = СчетаУчетаПроизводство;
	ОтборОборотов.КорСубконто1 = Перечисления.ТипыЗатратРегл.Работы;
	ОтборОборотов.КорНаправлениеДеятельности = "Контракт";
	ОтборОборотов.ТипИтогов =  Перечисления.ТипыИтогов.ОборотКт;
	ДобавитьОбороты(НовоеПравило, ОтборОборотов, ПараметрыОтчета.Детализация);
...
#Область ОтгрузкаПродукцииВыполнениеРабот
	...
	НовоеПравило.ГруппаПоказателей2 = "300";
	НовоеПравило.КодПоказателя = "300";
	...
	// 301 "Себестоимость реализованной продукции"
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "300";
	...
	// 302 "Управленческие расходы"
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "300";
	НовоеПравило.ГруппаПоказателей2 = "302";
	НовоеПравило.КодПоказателя = "302";
	...
	// 303 "Коммерческие расходы"
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "300";
	НовоеПравило.ГруппаПоказателей2 = "303";
	НовоеПравило.КодПоказателя = "303";
	...
	// 304 "Проценты по кредитам банка"
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "300";
	НовоеПравило.ГруппаПоказателей2 = "304";
	НовоеПравило.КодПоказателя = "304";
	...
	// 305 "НДС с выручки от продаж"
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "300";
	НовоеПравило.ГруппаПоказателей2 = "305";
	НовоеПравило.КодПоказателя = "305";
	...
	// 306 "Прибыль контракта"
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "300";
	НовоеПравило.ГруппаПоказателей2 = "306";
	НовоеПравило.КодПоказателя = "306";
	НовоеПравило.Пассивный     = Истина;
	...
	НовоеПравило = ПравилаРасчетаПоказателей.Добавить(); 
	НовоеПравило.ГруппаПоказателей1 = "500";
	НовоеПравило.ГруппаПоказателей2 = "500";
	НовоеПравило.КодПоказателя = "500";
...
Функция СчетаУчетаДенеженыеСредстваНаСчетах()
	...
	Результат = Новый Массив;
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РасчетныеСчета);
...
Функция СчетаУчетаМатериалыНаСкадах()
	...
	|	ПланСчетов.Хозрасчетный КАК Хозрасчетный
	|ГДЕ
	|	Хозрасчетный.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЗаготовлениеИПриобретениеМЦ))
	|	И НЕ Хозрасчетный.ЗапретитьИспользоватьВПроводках
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция КорСчетаУчетаПрочихПроизводственныхЗатрат()
	...
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПоставщикамиИПодрядчиками);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РезервыПредстоящихРасходов);
