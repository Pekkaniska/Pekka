...
#Область ТекстЗапроса
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ВТПродукция.ПартияПродукции) = ТИП(Документ.ЭтапПроизводства2_2)
		|			ТОГДА КОНЕЦПЕРИОДА(ВЫРАЗИТЬ(ВТПродукция.ПартияПродукции КАК Документ.ЭтапПроизводства2_2).ФактическоеОкончаниеЭтапа, ДЕНЬ)
		|		ИНАЧЕ КОНЕЦПЕРИОДА(ВЫРАЗИТЬ(ВТПродукция.ПартияПродукции КАК Документ.ПроизводствоБезЗаказа).Дата, ДЕНЬ)
		|	КОНЕЦ КАК Период,
		|	ВТПродукция.Организация КАК Организация
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ КалькуляцииПродукции КАК КалькуляцииПродукции
		|		ПО ЗатратыВРазрезе.Продукция = КалькуляцииПродукции.Продукция
		|			И ЗатратыВРазрезе.ХарактеристикаПродукции = КалькуляцииПродукции.ХарактеристикаПродукции
		|			И ЗатратыВРазрезе.Назначение = КалькуляцииПродукции.НазначениеПродукции
		|			И ЗатратыВРазрезе.ПартияПродукции = КалькуляцииПродукции.Партия
		|			И ЗатратыВРазрезе.Организация = КалькуляцииПродукции.Организация
		|			И ЗатратыВРазрезе.Серия = КалькуляцииПродукции.Серия
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ИНАЧЕ МножителиПартийИПолуфабрикатов.СтоимостьРегл
		...
		|			ИНАЧЕ ЕСТЬNULL(ПлановыеМатериальныеЗатраты.СтоимостьРегл, 0)
		...
		|			ИНАЧЕ ЕСТЬNULL(ПлановыеПрочиеЗатраты.СтоимостьРегл, 0)
		|		КОНЕЦ * МножителиПолуфабрикатов.Числитель / МножителиПолуфабрикатов.Знаменатель),
		|	0,
		|	ПлановыеПрочиеЗатраты.Подразделение,
		|	ПлановыеПрочиеЗатраты.СтатьяКалькуляции,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВложенныйЗапрос.ПартияСпецификация КАК ПартияСпецификация,
		|	ВложенныйЗапрос.Подразделение КАК Подразделение
		|ПОМЕСТИТЬ ВыпускающиеПодазделения
		|ИЗ
		|	(ВЫБРАТЬ
		|		МножителиПолуфабрикатов.ПартияСпецификацияПолуфабриката КАК ПартияСпецификация,
		|		ЕСТЬNULL(РесурсныеСпецификацииВыходныеИзделия.Этап.Подразделение, ЭтапПроизводства2_2.Подразделение) КАК Подразделение
		|	ИЗ
		|		МножителиПолуфабрикатов КАК МножителиПолуфабрикатов
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РесурсныеСпецификации.ВыходныеИзделия КАК РесурсныеСпецификацииВыходныеИзделия
		|			ПО МножителиПолуфабрикатов.ПартияСпецификацияПолуфабриката = РесурсныеСпецификацииВыходныеИзделия.Ссылка
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ЭтапПроизводства2_2
		|			ПО МножителиПолуфабрикатов.ПартияСпецификацияПолуфабриката = ЭтапПроизводства2_2.Ссылка
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		МножителиПродукции.ПартияСпецификацияПродукции,
		|		ЕСТЬNULL(РесурсныеСпецификацииВыходныеИзделия.Этап.Подразделение, ЭтапПроизводства2_2.Подразделение)
		|	ИЗ
		|		МножителиПродукции КАК МножителиПродукции
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РесурсныеСпецификации.ВыходныеИзделия КАК РесурсныеСпецификацииВыходныеИзделия
		|			ПО МножителиПродукции.ПартияСпецификацияПродукции = РесурсныеСпецификацииВыходныеИзделия.Ссылка
		|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ЭтапПроизводства2_2
		|			ПО МножителиПродукции.ПартияСпецификацияПродукции = ЭтапПроизводства2_2.Ссылка) КАК ВложенныйЗапрос
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ФактическиеДанные.Полуфабрикат КАК Полуфабрикат,
		|		ФактическиеДанные.ХарактеристикаПолуфабриката КАК ХарактеристикаПолуфабриката,
		|		МАКСИМУМ(ВЫРАЗИТЬ(ФактическиеДанные.КоличествоВыпуск КАК ЧИСЛО(15, 3))) КАК КоличествоВыпуск,
		...
		|	ИЗ
		|		МножителиПолуфабрикатов КАК МножителиПолуфабрикатов
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВыпускающиеПодазделения КАК ВыпускающиеПодазделения
		|			ПО МножителиПолуфабрикатов.ПартияСпецификацияПолуфабриката = ВыпускающиеПодазделения.ПартияСпецификация
		|	ГДЕ
		|		НЕ МножителиПолуфабрикатов.Полуфабрикат = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|		
		|	СГРУППИРОВАТЬ ПО
		...
		|		МножителиПродукции КАК МножителиПродукции
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВыпускающиеПодазделения КАК ВыпускающиеПодазделения
		|			ПО МножителиПродукции.ПартияСпецификацияПродукции = ВыпускающиеПодазделения.ПартияСпецификация
		|	
		|	СГРУППИРОВАТЬ ПО
		|		МножителиПродукции.Калькуляция,
		|		МножителиПродукции.Продукция,
		...
		|		МножителиПродукции.Серия,
		|		МножителиПродукции.Продукция,
		|		МножителиПродукции.ХарактеристикаПродукции) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.ХарактеристикаПродукции,
		|	ВложенныйЗапрос.Продукция,
