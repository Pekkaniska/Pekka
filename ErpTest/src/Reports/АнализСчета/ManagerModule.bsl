...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВидСубконто1ТолькоОбороты", СвойстваСчета.ВидСубконто1ТолькоОбороты);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВидСубконто2ТолькоОбороты", СвойстваСчета.ВидСубконто2ТолькоОбороты);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ВидСубконто3ТолькоОбороты", СвойстваСчета.ВидСубконто3ТолькоОбороты);
	...
	// Позиция направления деятельности: 0 - Направление деятельности не нужно выводить, 
	// -2 - Направление деятельности нужно выводить всегда в 1-м уровне в группировке;
	// остальные значения - выводить на указанной позиции.
	ПозицияНаправленияДеятельности = 0;
	...
	Для Каждого СтрокаГруппировки Из ПараметрыОтчета.ГруппировкаКор Цикл
		Если СтрокаГруппировки.Использование Тогда
			...
			ИндексНаправленияДеятельности = (СтрНайти(СтрокаГруппировки.ПоСубконто,"+!") + 1) / 2;
			
			Если ИндексНаправленияДеятельности >= 1 Тогда
				// Скорректируем индекс убрав отключенные перед ним группировки.
				КоличествоОтключенныхГруппировок = СтрЧислоВхождений(Лев(СтрокаГруппировки.ПоСубконто, ИндексНаправленияДеятельности), "-");
				ИндексНаправленияДеятельности    = ИндексНаправленияДеятельности - КоличествоОтключенныхГруппировок;
			КонецЕсли;
			
			// Если в таблице "Направление деятельности" находятся на разных позициях
			// или оно отключена в некоторых строках таблицы,
			// то всегда выводится первым.
			Если ИндексНаправленияДеятельности <> ПозицияНаправленияДеятельности Тогда
				
				Если ИндексНаправленияДеятельности >= 1 Тогда
					
					Если ПозицияНаправленияДеятельности = 0 И СтрокаГруппировки.НомерСтроки > 1 Тогда
						// В предыдущей группировке Направление деятельности было отключено.
						ПозицияНаправленияДеятельности = -2;
					Иначе
						// Изменилась позиция Направления деятельности.
						ПозицияНаправленияДеятельности = ?(ПозицияНаправленияДеятельности = 0, ИндексНаправленияДеятельности, -2);
					КонецЕсли;
					
				Иначе
					// В строке отключено направление деятельности, надо проверить не было ли оно включено в другой строке.
					ПозицияНаправленияДеятельности = ?(ПозицияНаправленияДеятельности > 0, -2, ИндексНаправленияДеятельности);
				КонецЕсли;
				
			КонецЕсли;
		
			// Группировку по подразделению выводим:
			//  - если включена группировка по направлению деятельности, то выводим после группировки по НД;
			//  - если группировка по НД не включена, то выводим по установленном индексу.
			...
			Если ИндексПодразделения >= 1 Тогда
				// Скорректируем индекс убрав отключенные группировки.
				КоличествоОтключенныхГруппировок = СтрЧислоВхождений(Лев(СтрокаГруппировки.ПоСубконто, ИндексПодразделения), "-");
				ИндексПодразделения              = ИндексПодразделения - КоличествоОтключенныхГруппировок;
			КонецЕсли;

			// Если в таблице подразделения находятся на разных позициях,
			// и группировка по НД находиться на разных позициях,
			// то подразделение всегда выводится после групировки по НД,
			// иначе выводиться первым.
			Если ИндексПодразделения <> ПозицияПодразделения Тогда
				
				Если ИндексПодразделения >= 1 Тогда
					
					Если ПозицияПодразделения = 0 И СтрокаГруппировки.НомерСтроки > 1 Тогда
						// В предыдущих группировках подразделение было отключено.
						ПозицияПодразделения = -1;
					Иначе
						ПозицияПодразделения = ?(ПозицияПодразделения = 0, ИндексПодразделения, -1);
					КонецЕсли;
					
				Иначе
					ПозицияПодразделения = ?(ПозицияПодразделения > 0, -1, ИндексПодразделения);
				КонецЕсли;
	...
	Если ПозицияНаправленияДеятельности = -2 Тогда
	...
	ИначеЕсли ПозицияНаправленияДеятельности <> 0 Тогда
	...
	Для Индекс = 1 По КоличествоКорГруппировок Цикл 
		...
		ПолеГруппировки = Структура.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
		ПолеГруппировки.Использование  = Истина;
		Если Индекс = ПозицияНаправленияДеятельности Тогда
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("КорНаправлениеДеятельности");
		ИначеЕсли Индекс = ПозицияПодразделения Тогда
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("КорПодразделение");
		Иначе
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ГруппировкаКор Цикл 
		...
		Для НомерСубконто = 1 По КоличествоСубконто Цикл
			Если Сред(СтрокаТаблицы.ПоСубконто, НомерСубконто*2, 1)="0"
				Или Сред(СтрокаТаблицы.ПоСубконто, НомерСубконто*2, 1)="!" Тогда
				Продолжить;
			КонецЕсли;
	...
	ПостояннаяЧастьЗапроса = СтрЗаменить(ПостояннаяЧастьЗапроса, "ХозрасчетныйОбороты.НаправлениеДеятельностиКор КАК", "NULL КАК");
	...
	МакетШапкиОтчета = БухгалтерскиеОтчетыВызовСервера.ПолучитьМакетШапки(МакетКомпоновки, , ?(МакетКомпоновки.Тело.Количество() > 2, "Подвал", "Заголовок"));
	...
	КоличествоКорГруппировок = КоличествоКорГруппировок + ?(ЕстьКорНД, 1, 0);
	...
	МакетГруппировкиНаправлениеДеятельностиЗаголовок = БухгалтерскиеОтчетыВызовСервера.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "НаправлениеДеятельности", , "Заголовок");
	МакетГруппировкиНаправлениеДеятельностиПодвал    = БухгалтерскиеОтчетыВызовСервера.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "НаправлениеДеятельности", , "Подвал");
	...
	Для Каждого Макет Из МакетКомпоновки.Макеты Цикл 
		...
		Если МакетГруппировкиСчетЗаголовок.Найти(Макет) <> Неопределено 
			...
			ИЛИ МакетГруппировкиПериодЗаголовок.Найти(Макет) <> Неопределено
			ИЛИ МакетГруппировкиНаправлениеДеятельностиЗаголовок.Найти(Макет) <> Неопределено Тогда
		...
		Если МакетГруппировкиСчетПодвал.Найти(Макет) <> Неопределено 
			...
			ИЛИ МакетГруппировкиПериодПодвал.Найти(Макет) <> Неопределено
			ИЛИ МакетГруппировкиНаправлениеДеятельностиПодвал.Найти(Макет) <> Неопределено Тогда
