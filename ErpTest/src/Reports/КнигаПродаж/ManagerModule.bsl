...
Процедура ОпределитьПараметрыФормированияОтчета(СтруктураПараметров)
	...
	СтруктураПараметров.Вставить("Постановление1137_2019", ДатаПроверкиПараметровУчета >='20190101');
...
Процедура СформироватьОсновнойРаздел(СтруктураПараметров, Результат);
	...
	Если СтруктураПараметров.КонецПериода >= '20190101' Тогда
		Макет = ПолучитьОбщийМакет("КнигаПродаж2019");
	...
	Пока ИтогПоПериодам.Следующий() Цикл
		...
		Пока ИтогПоПериодамКорректировки.Следующий() Цикл;
			...
			Если ИтогПоПериодамКорректировки.КонецНалоговогоПериода >= '20190101' Тогда
				Макет = ПолучитьОбщийМакет("ДополнительныйЛистКнигиПродаж2019");
