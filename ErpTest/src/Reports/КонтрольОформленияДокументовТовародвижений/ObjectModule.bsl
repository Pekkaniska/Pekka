...
//-- НЕ УТКА
	...
	|	И ПередачаМатериаловВПроизводство.Ссылка.Дата <= &Период
	|{ГДЕ
	|	(&СтрокаВидОперацииПередачаМатериаловВПроизводство) КАК ВидОперации,
	|	(&СтрокаТипОтклоненияОформлениеФинансовыхДокументов) КАК ТипОтклонения,
...
Функция ТекстЗапросаПереработкаСырьяУПереработчика()
	...
	|	Назначение.*}
	|ИЗ
	|	РегистрНакопления.ТоварыКОтгрузке.Остатки(&ПериодГраница, 
	|	ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказПереработчику),ТИП(Документ.ПередачаСырьяПереработчику)) 
...
Функция ТекстЗапросаКонтрольОформленияДокументовТовародвижений()
	...
	//++ НЕ УТКА
	ПравоЧтениеПередачаМатериаловВПроизводство	= ПравоЧтениеПередачаМатериаловВПроизводство
	...
	//++ НЕ УТКА
	ПравоЧтениеВозвратМатериаловИзПроизводства  = ПравоЧтениеВозвратМатериаловИзПроизводства
													Или ПравоДоступа("Чтение", Метаданные.Документы.ДвижениеПродукцииИМатериалов);
	//-- НЕ УТКА
	
	ПравоЧтениеВыпускПродукции					= ПравоДоступа("Чтение", Метаданные.Документы.ВыпускПродукции);
	ПравоЧтениеВыпускПродукции					= ПравоЧтениеВыпускПродукции
													Или ПравоДоступа("Чтение", Метаданные.Документы.ДвижениеПродукцииИМатериалов);
