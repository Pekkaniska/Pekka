...
Функция ТекстЗапросаЗаказМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета)
	...
	|	И ЗаказМатериаловВПроизводствоТовары.НачалоОтгрузки < &ДатаОкончания  %ТекстУсловияОтборДокумент%
	|
	|СГРУППИРОВАТЬ ПО
	...
	|				И ВЫРАЗИТЬ(Распоряжение КАК Документ.ЗаказМатериаловВПроизводство).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовМатериаловВПроизводство.КВыполнению) %ТекстУсловияОтборРаспоряжение%) КАК ЗаказыМатериаловВПроизводствоОстатки
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"%ТекстУсловияОтборРаспоряжение%",ТекстУсловияОтборРаспоряжение);
	ОбработатьТекстПодзапросаПоВидуСобытия(ТекстЗапроса,СтруктураФормированияОтчета.ТекущийНомерПодзапросаПоВидамСобытий);
...
Функция ТекстЗапросаПередачаМатериаловВПроизводство(Запрос,СтруктураФормированияОтчета)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОтгрузке.Остатки(, ДокументОтгрузки ССЫЛКА Документ.ДвижениеПродукцииИМатериалов %ТекстУсловияОтборДокументОтгрузки%) КАК ТоварыКОтгрузкеОстатки
