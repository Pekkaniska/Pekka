...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ЗаполнитьЗначенияПоУмолчанию Тогда
		...
		ТабличныйДокумент.Области.ПоМесту.Значение = ?(ОрганизацияКрупнейшийНалогоплательщик(), "213", "270");
...
Процедура ПроверитьКС_1_37(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По КоличествоСтраниц Цикл
		...
		Раздел2 = мДанныеОтчета.Раздел2[НомерСтраницы - 1].Данные;
		Показатели = ЗаполнитьПоказатели("Р2С150, Р2С245, Р2С250", Раздел2);
		Если Не ЗначениеЗаполнено(Показатели.Р2С245.Значение) Тогда
			Продолжить;
		КонецЕсли;
		Настройки.Вставить("ДопСведенияПараметры", Новый Структура("Номер", НомерСтраницы));
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", НомерСтраницы);
		РасчетнаяСумма = Округлить(Показатели.Р2С150.Значение - Показатели.Р2С245.Значение);
		ЭтоОшибка = Не Показатели.Р2С250.Значение = РасчетнаяСумма;
		Настройки.Расшифровка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Настройки.ШаблонРасшифровки, 
