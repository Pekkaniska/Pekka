...
Процедура ПриОбновленииСоставаПользовательскихНастроекНаСервере(СтандартнаяОбработка)
	...
	//++ RARUS_IUC 20150115
	//ИзменитьПрограмноСозданныйРеквизит();
	//-- RARUS_IUC 20180115
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	Если НастройкиОтчета.ВыполнятьЗамеры И ЗначениеЗаполнено(НастройкиОтчета.КлючЗамеров) Тогда
		...
		ПараметрыФормированияОтчета.Вставить("КомментарийКлючевойОперации", НРег(пкМодификацияСервер.СтруктураВСтроку(Разрезы, "=", "/")));
	КонецЕсли;
...
//++ RARUS_IUC 20180115
Процедура ИзменитьПрограмноСозданныйРеквизит()
	
	ЭлементРегион = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("Регион");
	
	Если ЭлементРегион = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	РегионИндентификатор = ЭлементРегион.ИдентификаторПользовательскойНастройки;
	
	ПолеФормыРегион = Элементы["ЗначениеПараметраНастроек_Значение_" + СтрЗаменить(РегионИндентификатор, "-", "")];
	ПолеФормыРегион.КнопкаВыбора = Истина;
	ПолеФормыРегион.КнопкаВыпадающегоСписка = Ложь;
	ПолеФормыРегион.РежимВыбораИзСписка = Ложь; 	
