...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыРасшифровки") Тогда
		...
		Если ПараметрыРасшифровки.Отбор.Свойство("НаправлениеДеятельности") И ЗначениеЗаполнено(ПараметрыРасшифровки.Отбор.НаправлениеДеятельности) Тогда
			...
			Направления.ЗагрузитьЗначения(ПараметрыРасшифровки.Отбор.НаправлениеДеятельности);
		КонецЕсли;
		...
		Заголовок = НСтр("ru = 'Расшифровка показателя:'")+ " " + ПроизводныйПоказатель.НаименованиеДляПечати;
...
&НаСервере
Процедура ЗаполнитьВалютуОтчета()
	
	Если ЗначениеЗаполнено(Отчет.ВидОтчета) Тогда
		Реквизиты = ОбщегоНазначенияУТВызовСервера.ЗначенияРеквизитовОбъекта(Отчет.ВидОтчета,"ВидПоказателей");
		МеждународнаяОтчетностьСервер.ЗаполнитьВалютуОтчета(Элементы.Ресурс, Реквизиты.ВидПоказателей);
	КонецЕсли;
