...
Процедура ОбработкаМодифицированности(НачальноеЗначениеПолей, СтруктураМодифицированности)
	...
	Иначе
		...
		ИНН = СокрЛП(Запись.П000010000305);
		КПП = СокрЛП(Запись.П000010000306);
		НаименованиеПолное = СокрЛП(Запись.П000010000304);
		...
		Если НЕ Запись.П000010000304 = НаименованиеПолное  Тогда		
			Запись.П000010000304 = НаименованиеПолное;
			НужноОбработатьЗапись = Истина;		
		КонецЕсли;
		
		Если НЕ Запись.П000010000306 = КПП  Тогда			
