...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Таблица.Спецификация КАК Спецификация
	|ИЗ
	|	РегистрСведений.НормативнаяДлительностьПроизводства КАК Таблица
	|ГДЕ
	|	Таблица.ПлановаяДлительность <> Таблица.МаксимальнаяДлительность И Таблица.МаксимальнаяДлительность = 0
	|");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Для каждого Запись Из Набор Цикл
				Если Запись.ПлановаяДлительность <> Запись.МаксимальнаяДлительность И Не ЗначениеЗаполнено(Запись.МаксимальнаяДлительность) Тогда
					Запись.МаксимальнаяДлительность = Запись.ПлановаяДлительность;
					НаборИзменен = Истина;
				КонецЕсли;
			КонецЦикла;
