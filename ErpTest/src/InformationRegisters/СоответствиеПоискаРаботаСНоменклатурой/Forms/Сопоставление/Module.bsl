...
Процедура ОбновитьСтатистику()
	...
	Элементы.ОтборВсяНоменклатура.Заголовок = НСтр("ru = 'Вся номенклатура'")
		+ ?(Статистика.Количество > 0, " (" + Формат(Статистика.Количество, "ЧН=0") + ")", "");
	
	Элементы.ОтборТочноеСовпадение.Заголовок = НСтр("ru = 'Точное совпадение'")
		+ ?(Статистика.ТочноеСовпадение > 0, " (" + Формат(Статистика.ТочноеСовпадение, "ЧН=0") + ")", "");
		
	Элементы.ОтборНеточноеСовпадение.Заголовок  = НСтр("ru = 'Неточное совпадение'")
		+ ?(НеточноеСовпадение > 0, " (" + Формат(НеточноеСовпадение, "ЧН=0") + ")", "");
		
	Элементы.СохранитьСопоставление.Заголовок = НСтр("ru = 'Сохранить сопоставление'")
...
&НаКлиенте
Процедура СкрыватьСопоставленныеПриИзменении(Элемент)
...
Процедура ВариантыПоискаПометкаПриИзменении(Элемент)
	...
	Для каждого ЭлементКоллекции Из ВариантыПоиска Цикл
...
Процедура ОбновитьОтборСписка()
	...
	Если СкрыватьСопоставленные Тогда
		ОтборЭлемента = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Подтверждено");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура ИзменитьРеквизитыСписка(Знач Отбор, НовыеРеквизиты = Неопределено, Отказ = Ложь, ТекстСообщения = "");
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Изменение сопоставления 1С:Номенклатура.'"),
			...
			НСтр("ru = 'Ошибка изменения сопоставления 1С:Номенклатура для публикации торговых предложений.'"), "БизнесСеть");
...
Процедура НайтиСоответствияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Статистика.Количество = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Отсутствует номенклатура для поиска и сопоставления.'"));
	КонецЕсли;
