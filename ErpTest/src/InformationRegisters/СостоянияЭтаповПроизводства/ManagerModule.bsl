...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// Условный перевод состояния операций ОжидаетЗавершения -> ОжидаетНазначения
	ТекстыЗапроса.Добавить(
		"ВЫБРАТЬ
		|	СостоянияЭтаповПроизводства.Этап       КАК Ссылка,
		|	ЭтапПроизводства.Подразделение         КАК Подразделение,
		|	ЭтапПроизводства.Статус                КАК Статус,
		|	ЭтапПроизводства.ПроизводствоНаСтороне КАК ПроизводствоНаСтороне,
		|	ЭтапПроизводства.МаршрутнаяКарта       КАК МаршрутнаяКарта
		...
		|		ПО СостоянияЭтаповПроизводства.Этап = ЭтапПроизводства.Ссылка
		|ГДЕ
		|	СостоянияЭтаповПроизводства.СостояниеОпераций = ЗНАЧЕНИЕ(Перечисление.СостоянияОперацийЭтапаПроизводства.ОжидаетЗавершения)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Подразделение,
		|	Статус,
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаВТСостоянияОпераций());
	...
	ТекстыЗапроса.Добавить(
		...
		|ИЗ
		|	ВТРеквизитыДокумента КАК ВТРеквизитыДокумента
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСостоянияОпераций КАК ВТСостоянияОпераций
		|		ПО ВТРеквизитыДокумента.Ссылка = ВТСостоянияОпераций.Ссылка
		|ГДЕ
		|	ВТСостоянияОпераций.Состояние <> ЗНАЧЕНИЕ(Перечисление.СостоянияОперацийЭтапаПроизводства.ОжидаетЗавершения)");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	БлокировкиЭтапы = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
		"Документ.ЭтапПроизводства2_2",
		МенеджерВременныхТаблиц);
	
	БлокировкиПодразделения = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
		"Справочник.СтруктураПредприятия",
		МенеджерВременныхТаблиц);
	
	БлокировкиОчередь = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
		"РегистрСведений.ОчередьПроизводственныхОпераций",
		МенеджерВременныхТаблиц);
	
	БлокировкиСмены = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ 
		|	ВТДляОбработки.Этап КАК Этап
		|ПОМЕСТИТЬ ВТБлокировки
		|ИЗ
		|	ВТДляОбработки КАК ВТДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиЭтапы КАК БлокировкиЭтапы
		|		ПО ВТДляОбработки.Этап = БлокировкиЭтапы.Ссылка
		|ГДЕ
		|	БлокировкиЭтапы.Ссылка ЕСТЬ НЕ NULL 
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|		ПО ВТДляОбработки.Этап = ЭтапПроизводства2_2.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиПодразделения КАК БлокировкиПодразделения
		|		ПО ЭтапПроизводства2_2.Подразделение = БлокировкиПодразделения.Ссылка
		|ГДЕ
		|	БлокировкиПодразделения.Ссылка ЕСТЬ НЕ NULL
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ВТДляОбработки КАК ВТДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиОчередь КАК БлокировкиОчередь
		|		ПО ВТДляОбработки.Этап = БлокировкиОчередь.Этап
		|ГДЕ
		|	БлокировкиОчередь.Этап ЕСТЬ НЕ NULL 
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|		ПО ВТДляОбработки.Этап = ОперацииКСозданиюСменныхЗаданий.Этап
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиСмены КАК БлокировкиСмены
		|		ПО ОперацииКСозданиюСменныхЗаданий.СменноеЗадание = БлокировкиСмены.СменноеЗадание
		|ГДЕ
		|	БлокировкиСмены.СменноеЗадание ЕСТЬ НЕ NULL
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Этап
		|;
		...
		|	ЕСТЬNULL(
		|		СостоянияЭтаповПроизводства.СостояниеОпераций,
		|		ЗНАЧЕНИЕ(Перечисление.СостоянияОперацийЭтапаПроизводства.ПустаяСсылка)) КАК СостояниеОпераций
		|ИЗ
		|	ВТДляОбработки КАК ВТДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2 КАК ЭтапПроизводства2_2
		|		ПО ВТДляОбработки.Этап = ЭтапПроизводства2_2.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СостоянияЭтаповПроизводства КАК СостоянияЭтаповПроизводства
		|		ПО ВТДляОбработки.Этап = СостоянияЭтаповПроизводства.Этап
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировки КАК ВТБлокировки
		|		ПО ВТДляОбработки.Этап = ВТБлокировки.Этап
		|ГДЕ
		|	ВТБлокировки.Этап ЕСТЬ NULL";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТБлокировкиЭтапы",         БлокировкиЭтапы.ИмяВременнойТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТБлокировкиПодразделения", БлокировкиПодразделения.ИмяВременнойТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТБлокировкиОчередь",       БлокировкиОчередь.ИмяВременнойТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТБлокировкиСмены",         БлокировкиСмены.ИмяВременнойТаблицы);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.Подразделение);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			...
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОперацииКСозданиюСменныхЗаданий");
			ЭлементБлокировки.УстановитьЗначение("Этап", Выборка.Этап);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			...
			ПараметрыРасчета = ПараметрыРасчетаКонструктор(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Выборка.Этап),
				,
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("СостояниеОпераций"));
				
			ВыборкаСостояние = ВыборкаИзмененийСостояния(ПараметрыРасчета);
			
			Набор = РегистрыСведений.СостоянияЭтаповПроизводства.СоздатьНаборЗаписей();
			Набор.Отбор.Этап.Установить(Выборка.Этап);
			
			Если ВыборкаСостояние.Следующий()
				И ВыборкаСостояние.СостояниеОпераций <> Выборка.СостояниеОпераций Тогда
				
				Набор.Прочитать();
				Набор[0].СостояниеОпераций = ВыборкаСостояние.СостояниеОпераций;
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось обновить состояние операций документа %1 по причине: %2'"),
				Выборка.Этап,
