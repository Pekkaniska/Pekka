...
Процедура ИзменитьСтатусОпераций(Знач Операции, Знач НовыйСтатус) Экспорт
	...
	Попытка
		...
		Если ДанныеЗаблокированы Тогда
			...
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстИсключения, ?(Операции.Количество()=1, НСтр("ru = 'операция находится'"), НСтр("ru = 'операции находятся'")));
	...
	Исключение
		...
		ОтменитьТранзакцию();
 		...
 		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Выполнение операций'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПриПереходеСРегистрацииОтклоненийНаРегистрациюФакта(Знач ПараметрыЗадания, АдресХранилища) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Изменение способа управления операциями'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
