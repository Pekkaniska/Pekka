...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектВладелец, "ЮрФизЛицо");
	ОрганизацияЯвляетсяИП = (ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель);
	Если ОрганизацияЯвляетсяИП Тогда
		МассивПараметровВыбора = Новый Массив;
		МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.СистемаНалогообложения", Перечисления.СистемыНалогообложения.Упрощенная));
		Элементы.НаборЗаписейУчетнаяПолитика.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбора);
	КонецЕсли;
	//-- НЕ УТ
	...
	Для Каждого ЗаписьНабора Из Параметры.МассивЗаписей Цикл
		//++ НЕ УТ
		Если (ОрганизацияЯвляетсяИП И ЗаписьНабора.ПрименяетсяУСН)
		 ИЛИ (НЕ ОрганизацияЯвляетсяИП И ЗначениеЗаполнено(ЗаписьНабора.Организация)) Тогда
		//-- НЕ УТ
