...
Процедура ОбновитьДвиженияПоОборотнымРегистрам(МассивДокументовСКлиентами, МассивДокументовСПоставщиками)
	...
	Если ВсеДокументы.Количество() > 0 Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ФормироватьУправленческийБаланс") Тогда
			УправленческийУчетПроведениеСервер.ОбновитьДвиженияАктивовПассивовПоРасчетамСПартнерами(ВсеДокументы);
		КонецЕсли;
	КонецЕсли;
...
// 	МассивДокументов - Массив - Документы, по которым нужно рассчитать суммы.
//
			...
			Пока ВыборкаПоТипуЗаписи.Следующий() Цикл
				...
				Если СуммыПоДокументу.Количество() = 1 Тогда
					Если ВыборкаПоТипуЗаписи.ЭтоСторно Тогда
						ПредоплатаРегл = ?(СуммыПоДокументу[0].ПредоплатаРегл < 0, СуммыПоДокументу[0].ПредоплатаРегл, 0);
						ПредоплатаУпр  = ?(СуммыПоДокументу[0].ПредоплатаУпр < 0, СуммыПоДокументу[0].ПредоплатаУпр, 0);
						ДолгРегл       = ?(СуммыПоДокументу[0].ДолгРегл < 0, СуммыПоДокументу[0].ДолгРегл, 0);
						ДолгУпр        = ?(СуммыПоДокументу[0].ДолгУпр < 0, СуммыПоДокументу[0].ДолгУпр, 0);
				...
				Пока Выборка.Следующий() Цикл
					...
					Если ВыборкаПоДокументу.ВалютаВзаиморасчетов = Валюты.ВалютаРегламентированногоУчета
						 ИЛИ ВыборкаПоДокументу.Валюта = Валюты.ВалютаРегламентированногоУчета Тогда
					...
					Если ВыборкаПоДокументу.ВалютаВзаиморасчетов = Валюты.ВалютаУправленческогоУчета
						 ИЛИ ВыборкаПоДокументу.Валюта = Валюты.ВалютаУправленческогоУчета Тогда
...
Функция ТекстЗапросаДокументовДляПересчета() Экспорт
	...
	|		ВЫБОР КОГДА СуммыВзаиморасчетов.ПредоплатаРегл > 0 
	|				ТОГДА СуммыВзаиморасчетов.ПредоплатаРегл 
	|			ИНАЧЕ 0 
	|		КОНЕЦ 
	|		+ ВЫБОР КОГДА СуммыВзаиморасчетов.ДолгРегл > 0
	|				ТОГДА СуммыВзаиморасчетов.ДолгРегл 
	|			ИНАЧЕ 0 
	|		КОНЕЦ                                              КАК СуммаУвеличенияРеглРасчеты,
	|		ВЫБОР КОГДА СуммыВзаиморасчетов.ПредоплатаУпр > 0 
	|				ТОГДА СуммыВзаиморасчетов.ПредоплатаУпр 
	|			ИНАЧЕ 0 
	|		КОНЕЦ 
	|		+ ВЫБОР КОГДА СуммыВзаиморасчетов.ДолгУпр > 0
...
Функция ТекстЗапросаСуммВзаиморасчетов(ИмяВТ = "")
	...
	|	СУММА(ВЫБОР КОГДА СуммыДокументовВВалютеРегл.СуммаБезНДСРегл >= 0 
	|			ТОГДА СуммыДокументовВВалютеРегл.СуммаБезНДСРегл + СуммыДокументовВВалютеРегл.СуммаНДСРегл
	|		ИНАЧЕ 0 
	|	КОНЕЦ)                                               КАК СуммаУвеличенияРегл,
	|	СУММА(ВЫБОР КОГДА СуммыДокументовВВалютеРегл.СуммаБезНДСУпр >= 0 
	...
	|	И СуммыДокументовВВалютеРегл.ТипРасчетов <> ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.ПустаяСсылка)
	|	И (СуммыДокументовВВалютеРегл.Валюта <> &ВалютаРеглУчета ИЛИ СуммыДокументовВВалютеРегл.Валюта <> &ВалютаУпрУчета)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		-СуммыРасчетов.СуммаОбщая                                              КАК СуммаПредоплата,
	|		СуммыРасчетов.СуммаОбщая                                               КАК СуммаДолг,
	...
	|		КОНЕЦ * (-СуммыРасчетов.СуммаОбщая)                                 КАК ПредоплатаРегл,
	...
	|		КОНЕЦ * (-СуммыРасчетов.СуммаОбщая)                                  КАК ПредоплатаУпр,
	|		ВЫБОР КОГДА СуммыРасчетов.СуммаВзаиморасчетов = 0
	|			ТОГДА КурсыВалют.КоэффициентРегл
	|			ИНАЧЕ СуммыРасчетов.СуммаВзаиморасчетовРегл / СуммыРасчетов.СуммаВзаиморасчетов
	|		КОНЕЦ * СуммыРасчетов.СуммаОбщая                                  КАК ДолгРегл,
	|		ВЫБОР КОГДА СуммыРасчетов.СуммаВзаиморасчетов = 0 ИЛИ СуммыРасчетов.СуммаВзаиморасчетовУпр = 0
	|			ТОГДА КурсыВалют.КоэффициентУпр
	|			ИНАЧЕ (СуммыРасчетов.СуммаВзаиморасчетовРегл / СуммыРасчетов.СуммаВзаиморасчетов)
	|					/ (СуммыРасчетов.СуммаВзаиморасчетовРегл / СуммыРасчетов.СуммаВзаиморасчетовУпр)
	|		КОНЕЦ * СуммыРасчетов.СуммаОбщая                                  КАК ДолгУпр,
	|		ВЫБОР КОГДА СуммыРасчетов.СуммаВзаиморасчетов = 0
	|			ТОГДА КурсыВалют.КоэффициентРегл
	|			ИНАЧЕ СуммыРасчетов.СуммаВзаиморасчетовРегл / СуммыРасчетов.СуммаВзаиморасчетов
	...
	|					ТОГДА ДанныеРегистра.Предоплата
	|				КОГДА НЕ ДанныеРегистра.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РезервированиеАвансаКлиента)
	|					И ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА -ДанныеРегистра.Предоплата
	|			ИНАЧЕ 0
	|		КОНЕЦ                                                          КАК СуммаПредоплата,
...
Функция СуммыДокументовДляПострочногоРаспределения(МассивРегистраторов = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПоВсемДокументам", ЗначениеЗаполнено(МассивРегистраторов));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаРегистров = Метаданные.РегистрыСведений.СуммыДокументовВВалютеРегл.ПолноеИмя();
	...
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных,
		"Договор",
	...
	|		И (СуммыДокументовВВалютеРегл.СуммаБезНДС + СуммыДокументовВВалютеРегл.СуммаНДС 
	...
	|		И (СуммыДокументовВВалютеРегл.СуммаБезНДС + СуммыДокументовВВалютеРегл.СуммаНДС
	|		<> СуммыДокументовВВалютеРегл.СуммаБезНДСУпр + СуммыДокументовВВалютеРегл.СуммаНДСУпр)))
	...
	Если ОбновляемыеДанные.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				// Обход некорретное заполнение идентификаторов строк в документе.
				...
				ДокументССуммойВзаиморасчетов = ДокументССуммойВзаиморасчетов(Выборка.Регистратор);
					...
					Для Каждого ЗаписьНовогоНабора Из НовыйНаборЗаписей Цикл
						...
						Если ИмеющаясяЗаписьКопии <> Неопределено Тогда
							...
							Если НЕ ДокументБезБазыНДСУпр И НовыйНаборЗаписей.Колонки.Найти("БазаНДСУпр") <> Неопределено Тогда
								...
								КонецЕсли;
							КонецЕсли;
							Если ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациями") 
								И ИмеющаясяЗаписьКопии.АналитикаУчетаПоПартнерам <> ЗаписьНовогоНабора.АналитикаУчетаПоПартнерам Тогда
								ИмеющаясяЗапись.АналитикаУчетаПоПартнерам = ЗаписьНовогоНабора.АналитикаУчетаПоПартнерам;
								ОбъектИзменен = Истина;
							КонецЕсли;
		...
		Для СчетчикВидДвижения = 0 По 1 Цикл
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				...
				Если ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте Тогда
					...
					Запись.СуммаНДСУпр = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(Запись.БазаНДСУпр, Запись.СтавкаНДС, Ложь);
					Запись.СуммаБезНДСУпр = СуммаСНДСУпр - Запись.СуммаНДСУпр;
					
				Иначе
					
					Запись.СуммаНДСУпр = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(СуммаСНДСУпр, Запись.СтавкаНДС);
					Запись.СуммаБезНДСУпр = СуммаСНДСУпр - Запись.СуммаНДСУпр;
...
//++ НЕ УТ
	...
	#Область ПоступлениеПредметовЛизинга
	
	МассивТекстов.Добавить("ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаДокумента.Ссылка,
	|	NULL
	|ИЗ
	|	Документ.ПоступлениеПредметовЛизинга КАК ТаблицаДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеПредметовЛизинга.ОС КАК ТаблицаОС
	|			ПО ТаблицаОС.Ссылка = ТаблицаДокумента.Ссылка
	|ГДЕ
	|	ТаблицаДокумента.Проведен
	|	И &ВалютаРегламентированногоУчета = ТаблицаДокумента.Валюта");
	МассивТекстов.Добавить(ТекстОбъединить());
	
	#КонецОбласти
	
	#Область ПриобретениеУслугПоЛизингу
	
	МассивТекстов.Добавить("ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаДокумента.Ссылка,
	|	NULL
	|ИЗ
	|	Документ.ПриобретениеУслугПоЛизингу КАК ТаблицаДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриобретениеУслугПоЛизингу.Начисления КАК ТЧНачисления
	|			ПО ТЧНачисления.Ссылка = ТаблицаДокумента.Ссылка
	|ГДЕ
	|	ТаблицаДокумента.Проведен
	|	И ТаблицаДокумента.Валюта = &ВалютаРегламентированногоУчета");
	МассивТекстов.Добавить(ТекстОбъединить());
...
Функция СуммыКлиентовВВалютеРегл()
	...
	Иначе
		...
		|	ДанныеРегистра.Валюта КАК Валюта,
		|	ДанныеРегистра.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	МАКСИМУМ(ДанныеРегистра.ОбъектРасчетов.ПорядокОплаты) КАК ПорядокОплаты,
		|	
...
Функция СуммыПоставщиковВВалютеРегл()
	...
	Иначе
		...
		|					ИЛИ Поступление.ХозяйственнаяОперация В (&ХозОперацииЗакупкаВСтранахЕАЭС)
...
Функция ДокументССуммойВзаиморасчетов(ДокументСсылка)
	...
	Если ТипДокумента = Тип("ДокументСсылка.АктВыполненныхРабот")
		//++ НЕ УТ
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ПоступлениеДенежныхДокументов")
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ОтчетОператораСистемыПлатон")
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ОтчетПереработчика")
		ИЛИ ТипДокумента = Тип("ДокументСсылка.ПриобретениеУслугПоЛизингу")
		//-- НЕ УТ
		//++ НЕ УТКА
