...
Процедура ОбработкаМодифицированности(НачальноеЗначениеПолей, СтруктураМодифицированности)
	...
	Иначе
		...
		Если НЕ Запись.П000020000303 = НаименованиеПолное  Тогда		
			Запись.П000020000303 = НаименованиеПолное;
			НужноОбработатьЗапись = Истина;		
		КонецЕсли; 
		
		Если НЕ Запись.П000020000305 = КПП  Тогда		
		...
		Поставщик = ОбъектОтчета(ИмяФормыВладельца).ОпределитьПоставщика(ИНН, КПП, НаименованиеПолное);
		
		Если НЕ Запись.Поставщик = Поставщик Тогда																	
		    Запись.Поставщик = Поставщик;
			НужноОбработатьЗапись = Истина;
		КонецЕсли;
		
		Если НЕ Запись.П000020000306 = НаименованиеПолное  Тогда			
			Запись.П000020000306 = НаименованиеПолное;
			НужноОбработатьЗапись = Истина;			
		КонецЕсли;
		
		Если НЕ Запись.П000020000308 = КПП  Тогда			
