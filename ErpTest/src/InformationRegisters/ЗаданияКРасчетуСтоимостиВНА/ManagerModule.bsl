...
Функция УвеличитьНомерЗадания() Экспорт
	...
	Попытка
		...
		Константы.НомерЗаданияКРасчетуСтоимостиВНА.Установить(НомерДоРасчета + 1);
...
Процедура ЗафиксироватьРасчет(Организация, НачалоРасчета, НомерЗадания, ЗаданияКРасчету) Экспорт
	...
	Попытка
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(МИНИМУМ(СтоимостьОС.Период), ДАТАВРЕМЯ(1, 1, 1)) КАК Месяц,
		...
		|ГДЕ
		|	(СтоимостьОС.Организация, СтоимостьОС.АналитикаКапитализацииРасходов) В
		|			(ВЫБРАТЬ
		|				КэшГраниц.Организация,
		...
		|				КэшГраниц КАК КэшГраниц)
		|
		|	И СтоимостьОС.Период >= &НачалоСледующегоМесяца
		...
		|
		|	И СтоимостьОС.ВариантПримененияЦелевогоФинансирования В (
		...
		|	СтоимостьОС.АналитикаКапитализацииРасходов
		|
		|ИМЕЮЩИЕ
		|	ЕСТЬNULL(МИНИМУМ(СтоимостьОС.Период), ДАТАВРЕМЯ(1, 1, 1)) <> ДАТАВРЕМЯ(1, 1, 1)";
		...
		Пока Выборка.Следующий() Цикл
			Набор = РегистрыСведений.ЗаданияКРасчетуСтоимостиВНА.СоздатьНаборЗаписей();
...
Процедура ЗаблокироватьРегистрЗаданий(НомерЗаданияДоРасчета, Организации)
	...
	ВнеоборотныеАктивы.ЗаблокироватьРегистрЗаданий(
		НомерЗаданияДоРасчета, Организации, "РегистрСведений.ЗаданияКРасчетуСтоимостиВНА");
