...
Функция ВыполнитьКонтрольПередРасчетомОчереди(ДанныеЭтапа, УдалениеПроведения = Ложь, Отказ = Ложь)
	...
	Если ЕстьОшибка Тогда
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Ошибка расчета очереди производственных операций. %1'"), ТекстСообщения);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|		ПО Документ.Ссылка = Очередь.Этап
		|ГДЕ
		|	Документ.Проведен
		|	И Документ.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Завершен)
		|	И Документ.МаршрутнаяКарта <> ЗНАЧЕНИЕ(Справочник.МаршрутныеКарты.ПустаяСсылка)
		|	И Документ.МаршрутнаяКарта.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыМаршрутныхКарт.Действует)
		|	И (Документ.Подразделение.ИспользоватьСменныеЗадания
		|			ИЛИ Документ.Подразделение.ИспользоватьПооперационноеУправление
		|			ИЛИ Документ.Подразделение.ИспользоватьПооперационноеПланирование)
		|	И Очередь.Этап ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	ОчередьПроизводственныхОпераций.ВремяВыполненияНормативное = 0
		|	И ОчередьПроизводственныхОпераций.ВремяВыполнения <> 0");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	БлокировкиОперацииСпр = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
		"Справочник.ТехнологическиеОперации",
		МенеджерВременныхТаблиц);
	
	БлокировкиОперацииДок = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь,
	...
	ТекстЗапроса = 
		...
		|		ПО (ЭтапПроизводства2_2.МаршрутнаяКарта = ТехнологическиеОперации.Владелец)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиОперацииСпр КАК БлокировкиОперации
		|		ПО (ТехнологическиеОперации.Ссылка = БлокировкиОперации.Ссылка)
		|ГДЕ
		|	БлокировкиОперации.Ссылка ЕСТЬ НЕ NULL
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ИЗ
		|	ВТДляОбработки КАК ВТДляОбработки
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводственнаяОперация2_2 КАК ПроизводственныеОперации
		|		ПО ВТДляОбработки.Этап = ПроизводственныеОперации.Этап
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБлокировкиОперацииДок КАК БлокировкиОперации
		|		ПО (ПроизводственныеОперации.Ссылка = БлокировкиОперации.Ссылка)
		|ГДЕ
		|	БлокировкиОперации.Ссылка ЕСТЬ НЕ NULL 
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТДляОбработки.Этап КАК Ссылка
		|ИЗ
		|	ВТДляОбработки КАК ВТДляОбработки
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТБлокировкиОперацииСпр", БлокировкиОперацииСпр.ИмяВременнойТаблицы);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВТБлокировкиОперацииДок", БлокировкиОперацииДок.ИмяВременнойТаблицы);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Этап", Выборка.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			...
			ЭлементБлокировки = Блокировка.Добавить("Документ.ПроизводственнаяОперация2_2");
			ЭлементБлокировки.УстановитьЗначение("Этап", Выборка.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
			...
			Набор = РегистрыСведений.ОчередьПроизводственныхОпераций.СоздатьНаборЗаписей();
			Набор.Отбор.Этап.Установить(Выборка.Ссылка);
			
			ДанныеЭтапа = Документы.ЭтапПроизводства2_2.ДанныеДляРасчетаОчередиОпераций(Выборка.Ссылка);
		
			Если ДанныеЭтапа.ТребуетсяРассчитать Тогда
							
				ОчередьОпераций = ОчередьОперацийВнутриЗамераВремени(Выборка.Ссылка, ДанныеЭтапа);
				
				Набор.Загрузить(ОчередьОпераций);
