...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Запись.ИсходныйКлючЗаписи.Пустой() Тогда
		...
		СоотвДанныеДатчиков = ItobОбщегоНазначенияКлиентСервер.ПреобразоватьСтрокуВДанныеДатчиков(Запись.ДанныеДатчиков);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.Период = '00010101' Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан период'"));
		Отказ = Истина;
	
	КонецЕсли;
	
	Если ТекущийОбъект.Терминал.Пустая() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан терминал'"));
		Отказ = Истина;
	...
	Для каждого СтрокаТаб Из ТабДатчики Цикл
		...
		Если СтрокаТаб.КодДатчика = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Код датчика должен быть больше нуля'"));
			Отказ = Истина;
		
		КонецЕсли;
		
		Если СтрокаТаб.КодДатчика > 255 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Код датчика должен меньше 256'"));
			Отказ = Истина;
