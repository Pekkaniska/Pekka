...
Процедура РассчитатьПотребности(Калькуляция, ПараметрыРасчета)
	...
	Если Не ОшибкаВРасчете Тогда
		Если ЕстьЗаданияВОчереди(Калькуляция) Тогда
			РассчитатьПотребности(Калькуляция, ПараметрыРасчета);
		Иначе
...
Процедура ОбработатьПродукциюПоЗаказу(Калькуляция, МенеджерВременныхТаблиц, Множители, Движения, ПараметрыРасчета)
	...
	Запрос.Текст = 
		...
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КорАналитика
		|		ПО Себестоимость.КорАналитикаУчетаНоменклатуры = КорАналитика.КлючАналитики
		|
...
Процедура РазузловатьСпецификациюИзделий(Калькуляция, Множители, Движения, ПараметрыРасчета)
	...
	Потребности = Справочники.РесурсныеСпецификации.ДанныеСпецификацииПоПотребности(
		ПотребностиКРазузлованию,
		ПараметрыВыборкиДанных);
	
	Если Потребности.ЕстьОшибка Тогда
		
		ПараметрыРасчета.ОшибкаРазузлования = Потребности.ОшибкаТекст;
		Возврат;
	...
	Множители.Колонки.Добавить("КлючСвязи", Новый ОписаниеТипов("Число"));
	Множители.ЗаполнитьЗначения(-1, "КлючСвязи");
	
	РазузлованныеОбъекты = Новый Массив;
	КоличествоПолуфабрикатов = Потребности.ДанныеСпецификаций.Количество();
	Индекс = 0;
	...
	Множители.Индексы.Добавить("ПартияСпецификацияПолуфабриката, Полуфабрикат, ХарактеристикаПолуфабриката, РасчетЗавершен, КлючСвязи");
	
	// Обработка результата разузлования.
	// Расчет множителей полуфабрикатов.
	Пока Индекс < КоличествоПолуфабрикатов Цикл
		
		ДанныеСпецификации = Потребности.ДанныеСпецификаций[Индекс];
		...
		Для Каждого ВыходноеИзделие Из ДанныеСпецификации.ВыходныеИзделия Цикл
			...
			Если ВыходноеИзделие.КлючСвязиПолучатель = 0 Тогда
			...
			Иначе
				...
				ОтборПолуфабриката.Вставить("ХарактеристикаПолуфабриката", 		ВыходноеИзделие.Характеристика);
				ОтборПолуфабриката.Вставить("РасчетЗавершен", 					Ложь);
				ОтборПолуфабриката.Вставить("КлючСвязи", 						ВыходноеИзделие.КлючСвязиПолучатель);
				
				СтрокиМножителей = Множители.НайтиСтроки(ОтборПолуфабриката);
				
				Для Каждого СтрокаСПФ Из СтрокиМножителей Цикл
					
					СтрокаСПФ.Период = ДанныеСпецификации.Окончание;
					СтрокаСПФ.ПартияСпецификацияПолуфабриката = ДанныеСпецификации.Спецификация;					
					СтрокаСПФ.Числитель = СтрокаСПФ.Числитель * ДоляСтоимости;
			...
			Для Каждого Множитель Из СтрокиМножителей Цикл
				...
				Множитель.КлючСвязи = ДанныеСпецификации.КлючСвязи;
				...
				Для Каждого СтрокаМатериала Из ДанныеСпецификации.МатериалыИУслуги Цикл
					...
					НовыйМножитель.Порядок = Множитель.Порядок + ВыходноеИзделие.КлючСвязиПолучатель + 1;
					
					НовыйМножитель.КлючСвязи = ДанныеСпецификации.КлючСвязи;
...
Функция ПотребностиДляПолученияДанныхСпецификаций(Изделия, ПараметрыРасчета)
	...
	Для Каждого Изделие Из Изделия Цикл
		...
		Иначе
			...
			НоваяПотребность.Обособленно = Истина;
...
Процедура ЗаполнитьДатыПроизводства(Потребности, ПериодКалькуляции)
	...
	Индекс = Потребности.ДанныеСпецификаций.Количество() - 1;
	...
	Пока Индекс >= 0 Цикл
		...
		Иначе
			...
			ДанныеСпецификации.Окончание = ДанныеСпецификации.ДатаВыпуска - ОбщаяДлительность;
			ДанныеСпецификации.Начало = ДанныеСпецификации.Окончание - ТекущаяДлительность * 86400;
...
Функция ТаблицаПотребностей()
	
	Результат = Новый ТаблицаЗначений;
	
	Результат.Колонки.Добавить("Распоряжение", Новый ОписаниеТипов("ДокументСсылка.ПлановаяКалькуляция2_2"));
...
Процедура ЗаписатьСостояниеКалькуляции(Калькуляция, Состояние)
	...
	МенеджерЗаписи = РегистрыСведений.СостоянияПлановыхКалькуляций.СоздатьМенеджерЗаписи();
	МенеджерЗаписи.Калькуляция = Калькуляция;
...
Функция СобытиеЖурналаРегистрацииОшибкаРасчета() Экспорт
	...
	Возврат НСтр("ru = 'Фоновое проведение калькуляции продукции.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура РассчитатьМножителиПоФактическимДанным(ПараметрыРазузлования, ДатаКалькуляции)
	...
	Для Каждого ВыборкаПолуфабрикатов Из ПараметрыРазузлования.КРазузлованию Цикл
		...
		Отборы.Знаменатель = ВыборкаПолуфабрикатов.Знаменатель;
