...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДополнительныеПараметры.ПолноеИмяРегистра = "РегистрСведений.ГрафикПлатежей";
	...
	|	Договоры.Ссылка КАК ОбъектОплаты
	|ИЗ
	|	Справочник.ДоговорыЛизинга КАК Договоры
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ГрафикПлатежей КАК ГрафикПлатежей
	|	ПО
	...
	|	ПО
	|		ГрафикОплатЛизинга.ВариантГрафика.Владелец = Договоры.Ссылка
	|		И ГрафикОплатЛизинга.ВариантГрафика.Используется
	|ГДЕ
	|	ГрафикПлатежей.ОбъектОплаты ЕСТЬ NULL
	|	И Договоры.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыДоговоровКонтрагентов.Закрыт)
	|	И НЕ ГрафикОплатЛизинга.ВариантГрафика ЕСТЬ NULL
	//-- НЕ УТ
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Неопределено, "РегистрНакопления.РасчетыПоФинансовымИнструментам") Тогда
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru = 'Не удалось заполнить график платежей по причине: %Причина%'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбъектыОплаты.Количество() Тогда
