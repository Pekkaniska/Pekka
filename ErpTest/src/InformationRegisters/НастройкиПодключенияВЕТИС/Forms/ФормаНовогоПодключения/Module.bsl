...
Процедура НадписьИнформацияРеквизитыПодключенияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьЗаявкуВетисAPI" Тогда
		Если ИнтеграцияВЕТИСВызовСервера.РежимРаботыСТестовымКонтуромВЕТИС() Тогда
			ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("https://t2-aplms.vetrf.ru/pub/");
		Иначе
			ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("https://aplms.vetrf.ru/pub/");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьИменаСтраниц()
	...
	Если ИнтеграцияВЕТИСПереопределяемый.ИспользуетсяКомиссияПриЗакупкахИлиПереработкаДавальческогоСырья() Тогда
...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	Форма.Элементы.ГруппаПолучениеНачальныхОстатков.Видимость = Форма.ПолучитьСведенияПредприятий;
	Форма.Элементы.ГруппаПолучениеВСДокументов.Видимость      = Форма.ПолучитьСведенияПредприятий;
...
Процедура ОчиститьСведенияХозяйствующегоСубъекта()
	...
	КартинкаЗаполнениеКлассификаторов  = БиблиотекаКартинок.ОжидаетВыполненияВЕТИС;
	КартинкаПолучениеХозСубъекта       = БиблиотекаКартинок.ОжидаетВыполненияВЕТИС;
	КартинкаПолучениеПредприятий       = БиблиотекаКартинок.ОжидаетВыполненияВЕТИС;
	КартинкаПолучениеПользователей     = БиблиотекаКартинок.ОжидаетВыполненияВЕТИС;
	КартинкаПолучениеНачальныхОстатков = БиблиотекаКартинок.ОжидаетВыполненияВЕТИС;
	КартинкаПолучениеВСДокументов      = БиблиотекаКартинок.ОжидаетВыполненияВЕТИС;
...
Процедура ОбработатьРезультатВыполненияФоновогоЗадания()
	...
	Иначе
		...
		Для Каждого ДанныеПредприятия Из РезультатВыполнения.Предприятия Цикл
			НоваяСтрокаПредприятие = Предприятия.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаПредприятие, ДанныеПредприятия, , "GLN");
			
			Если ТипЗнч(ДанныеПредприятия.GLN) = Тип("Массив") Тогда
				Если ДанныеПредприятия.GLN.Количество() > 0 Тогда
					НоваяСтрокаПредприятие.GLN = ДанныеПредприятия.GLN[0];
				КонецЕсли;
			Иначе
				НоваяСтрокаПредприятие.GLN = ДанныеПредприятия.GLN;
			КонецЕсли;
...
Процедура НачатьПолучениеВетеринарноСопроводительныхДокументов()
	...
	Попытка
		РезультатПолученияВСДокументов = ЗаявкиВЕТИСВызовСервера.ПодготовитьЗапросВетеринарноСопроводительныхДокументов(
			...
			ПараметрыОтбора, КоличествоЭлементов,
			ЭтотОбъект.УникальныйИдентификатор, НастройкаПодключения());
	Исключение
		РезультатПолученияВСДокументов = Неопределено;
		ТекстОшибки = СформироватьТекстОшибки(ЗаголовокОшибки, КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ИнициализироватьРеквизиты()
	...
	ПерезаписыватьСуществующиеСведения = ОбщегоНазначенияКлиентСервер.РежимОтладки();
...
Функция ЭтоКлючAPIПроверитьНаСервере()
	...
	Возврат ПользователиВЕТИСКлиентСервер.ЭтоУчетнаяЗапись(БезопасноеХранилище.КлючAPI,,,, Ложь);
