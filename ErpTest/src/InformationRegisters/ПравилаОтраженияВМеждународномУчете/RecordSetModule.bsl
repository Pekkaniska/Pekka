...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Пока ВыборкаПоНеОтражаемымОперациям.Следующий() Цикл
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'Хозяйственная операция, используемая в шаблоне проводки ""%ШаблонПроводки%"", исключена из учетной политики.'");
	...
	Пока ВыборкаПоИгнорируемымСчетам.Следующий() Цикл
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'Счет регл. учета, указанный в соответствии ""%СоответствиеСчетов%"", игнорируется в учетной политике.'");
