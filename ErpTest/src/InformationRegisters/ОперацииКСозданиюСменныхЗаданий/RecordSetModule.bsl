...
Процедура КонтрольНазначенияОпераций(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КодОшибки = "ЗаданиеНеПроведено" Тогда
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Документ %1 не проведен.'"), Выборка.СменноеЗадание);
			КлючДанных = Выборка.СменноеЗадание;
			
		ИначеЕсли Выборка.КодОшибки = "СтатусЗаданияНеФормируется" Тогда
			
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Документ %1 не находится в статусе ""Формируется"".'"), Выборка.СменноеЗадание);
			КлючДанных = Выборка.СменноеЗадание;
			
		ИначеЕсли Выборка.КодОшибки = "НекорректныйВидРЦ" Тогда
			
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Операция %1 не может быть назначена заданному РЦ (виду РЦ).'"), Выборка.Операция);
