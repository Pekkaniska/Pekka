...
//
// Параметры:
//  Этапы - Массив, ДокументСсылка.ЭтапПроизводства2_2 - документы по которым необходимо выполнить расчет.
//
Процедура РассчитатьОперации(Этапы) Экспорт
	...
	Попытка
		...
		Для каждого Этап Из МассивЭтапов Цикл
			...
			Набор.Загрузить(РассчитатьДанныеРегистра(Этап));
...
//  Параметры - Структура - параметры назначения (состав см. функцию ПараметрыНазначенияОпераций).
//
...
Процедура УдалитьОперацииЗадания(СменноеЗадание) Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			РассчитатьОперации(Выборка.Этап);
...
Процедура РегистрВДокументы(СменноеЗадание, СтатусДокументов) Экспорт
	...
	Попытка
		...
		ДанныеЗаполнения.Вставить("Статус", СтатусДокументов);
		ДанныеЗаполнения.Вставить("КлючОперации");
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, Выборка,
				"СменноеЗадание, Подразделение, Исполнитель, ВидРабочегоЦентра, РабочийЦентр, Количество, Участок");
			
			ЗаполнитьЗначенияСвойств(КлючОперации, Выборка, "Этап,Операция,ИдентификаторОперации");
			ДанныеЗаполнения.КлючОперации = КлючОперации;
		...
		Для каждого Этап Из Этапы Цикл
			...
			РегистрыСведений.ОчередьПроизводственныхОпераций.РассчитатьОчередь(Этап, Ложь);
...
Процедура ДокументыВРегистр(СменноеЗадание) Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			...
			|					ТОГДА Операции.КоличествоФакт
			|				ИНАЧЕ Операции.Количество
			|			КОНЕЦ * Очередь.ВремяВыполненияНормативное / Очередь.Запланировано КАК ЧИСЛО(10, 1))) КАК ВремяВыполнения,
			...
			|	И Операции.Проведен
			|	И Операции.НаОснованииМаршрутнойКарты
			|
			|СГРУППИРОВАТЬ ПО
			|	Очередь.ВремяВыполненияЕдИзм,
...
Процедура ИсключитьОперацииСНарушениемПорядкаНазначения(СменноеЗадание) Экспорт
	...
	Попытка
		...
		РассчитатьОперации(МассивРезультатов[1].Выгрузить().ВыгрузитьКолонку("Этап"));
...
Процедура СкорректироватьОперацииСНарушениемПорядкаНазначения(СменноеЗадание) Экспорт
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ДоступноКНазначению > Нормативы.Количество Тогда
				...
				Если Нормативы.Количество + Запись.Количество > ДоступноКНазначению Тогда
					...
					Запись.ВремяВыполнения = (Выборка.ВремяВыполнения/Выборка.Количество) * Запись.Количество;
		...
		Если ЭтапыПересчет.ВГраница() <> -1 Тогда
			РассчитатьОперации(ЭтапыПересчет);
		КонецЕсли;
...
//		* КлючОперации - РегистрСведенийКлючЗаписи.
//		* Исполнитель - СправочникСсылка.Бригады, СправочникСсылка.ФизическиеЛица.
//		* ВидРабочегоЦентра - СправочникСсылка.ВидыРабочихЦентров.
//		* РабочийЦентр - СправочникСсылка.РабочиеЦентры.
//		* Количество - Число.
...
Процедура ДобавитьВЗадание(Операции, СменноеЗадание) Экспорт
	...
	Попытка
		...
		Если РевизитыЗадания.Статус = Перечисления.СтатусыСменныхЗаданий.Формируется Тогда
			...
			Для каждого Операция Из Операции Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Операция,
					"Исполнитель, ВидРабочегоЦентра, РабочийЦентр, Количество, ВремяВыполнения");
			...
			Запрос = Новый Запрос(
				...
				|	ВложенныйЗапрос.ВремяВыполненияЕдИзм КАК ВремяВыполненияЕдИзм,
				...
				|		ОперацииРегистр.ВремяВыполнения КАК ВремяВыполнения,
				|		ОперацииРегистр.ВремяВыполненияЕдИзм КАК ВремяВыполненияЕдИзм,
				...
				|		ОперацииНазначение.ВремяВыполнения,
				|		Очередь.ВремяВыполненияЕдИзм,
				|		ЛОЖЬ,
				|		ЛОЖЬ,
				|		ЛОЖЬ,
				...
				|	ВложенныйЗапрос.ВремяВыполненияЕдИзм,
		...
		Иначе
			...
			ДанныеЗаполнения = Новый Структура;
			ДанныеЗаполнения.Вставить("СпособЗаполнения", "ЗаполнитьПоОперации");
			ДанныеЗаполнения.Вставить("Статус", Перечисления.СтатусыПроизводственныхОпераций.Создана);
			ДанныеЗаполнения.Вставить("Участок", РевизитыЗадания.Участок);
			ДанныеЗаполнения.Вставить("КлючОперации");
			ДанныеЗаполнения.Вставить("СменноеЗадание");
			ДанныеЗаполнения.Вставить("Подразделение");
			ДанныеЗаполнения.Вставить("Исполнитель");
			...
			Для каждого Операция Из Операции Цикл
				...
				ЗаполнитьЗначенияСвойств(КлючОперации, Операция.КлючОперации,
...
Функция РассчитатьДанныеРегистра(Этап)
	...
	Для каждого Операция Из ОперацииОбработать Цикл
		...
		Для Индекс = 0 По Предшественники.ВГраница() Цикл
			...
			Время = Окр(Операция.ВремяВыполненияВсего * Дробь.Числитель/Дробь.Знаменатель, 1);
			...
			Если КоличествоНазначено > 0 Тогда
				...
				Иначе
					...
					Время = Окр(Время * (Количество - КоличествоНазначено)/Количество, 1);
			...
			Если ВГраница = -1
				...
				НазначениеПоДатам.Добавить(
					Новый Структура("НазначатьНеРанее, Количество, ВремяВыполнения",
						Предшественник.Дата, Количество, Время));
				
			Иначе
				
				НазначениеПоДатам[ВГраница].Количество = НазначениеПоДатам[ВГраница].Количество + Количество;
				НазначениеПоДатам[ВГраница].Время      = НазначениеПоДатам[ВГраница].Время + Время;
		...
		Для Индекс = 0 По НазначениеПоДатам.ВГраница() Цикл
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(НоваяОперация, НазначениеПоДатам[Индекс],
					"НазначатьНеРанее, Количество, ВремяВыполнения");
				...
				Операция.ВремяВыполнения = Операция.ВремяВыполнения - НоваяОперация.ВремяВыполнения;
...
Функция ТекстЗапросаОперацииДляРасчета()
	...
	Возврат
		...
		|	МАКСИМУМ(ОперацииПредш.Запланировано)                    КАК КоличествоВсегоПредш
		...
		|	МАКСИМУМ(ВложенныйЗапрос.ВремяВыполненияВсего) КАК ВремяВыполненияВсего,
		...
		|		ВЫРАЗИТЬ((Очередь.Запланировано + Очередь.ТребуетПовторения - Очередь.Создано) * Очередь.ВремяВыполненияНормативное / Очередь.Запланировано КАК ЧИСЛО(10, 1)) КАК ВремяВыполнения,
		|		Очередь.ВремяВыполненияЕдИзм КАК ВремяВыполненияЕдИзм,
		|		Очередь.Распоряжение КАК Распоряжение,
		|		Очередь.НомерОперации КАК НомерОперации,
		|		Очередь.НомерСледующейОперации КАК НомерСледующейОперации,
		|		Очередь.Запланировано КАК КоличествоВсего,
		|		Очередь.ВремяВыполненияНормативное КАК ВремяВыполненияВсего,
		|		0 КАК Порядок
		|	ИЗ
		...
		|			ИНАЧЕ ПроизвОперация.Количество
		|		КОНЕЦ)
		|ИЗ
		...
		|	И ПроизвОперация.НаОснованииМаршрутнойКарты
...
Процедура СоздатьВТОперацииНазначение(МенеджерВременныхТаблиц, Операции, Параметры)
	...
	ТекстЗапроса =
		...
		|	ЕСТЬNULL(ОперацииКСозданиюСменныхЗаданий.ВремяВыполнения, 0) КАК ВремяВыполнения,
		|	ОперацииКСозданиюСменныхЗаданий.ВремяВыполненияЕдИзм     КАК ВремяВыполненияЕдИзм,
		...
		|	-&СтарыеЗаписиВремяВыполнения         КАК ВремяВыполнения,
		|	СтарыеЗаписи.ВремяВыполненияЕдИзм     КАК ВремяВыполненияЕдИзм,
		...
		|	&НовыеЗаписиВремяВыполнения,
		|	НовыеЗаписи.ВремяВыполненияЕдИзм,
	...
	Если Параметры.Количество <> Неопределено Тогда
		...
		Если Параметры.НазначитьЧасть Тогда
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СтарыеЗаписиВремяВыполнения",
				"ВЫБОР
				|	КОГДА &Количество >= СтарыеЗаписи.Количество
				|		ТОГДА СтарыеЗаписи.ВремяВыполнения
				|	ИНАЧЕ СтарыеЗаписи.ВремяВыполнения * &Количество/СтарыеЗаписи.Количество
				|КОНЕЦ");
			
		Иначе
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СтарыеЗаписиВремяВыполнения",
				"СтарыеЗаписи.ВремяВыполнения");
			
		КонецЕсли;
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "НовыеЗаписи.Количество", "&Количество");
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&НовыеЗаписиВремяВыполнения",
			"НовыеЗаписи.ВремяВыполнения * &Количество/НовыеЗаписи.Количество");
...
Процедура СоздатьВТОперацииУдаление(МенеджерВременныхТаблиц, Операции)
	...
	Запрос = Новый Запрос(
		...
		|	-ОперацииКСозданиюСменныхЗаданий.ВремяВыполнения         КАК ВремяВыполнения,
		|	ОперацииКСозданиюСменныхЗаданий.ВремяВыполненияЕдИзм     КАК ВремяВыполненияЕдИзм,
		|	ОперацииКСозданиюСменныхЗаданий.МожноНазначать           КАК МожноНазначать,
		|	ОперацииКСозданиюСменныхЗаданий.НужноНазначать           КАК НужноНазначать,
		|	ОперацииКСозданиюСменныхЗаданий.НарушенПорядокНазначения КАК НарушенПорядокНазначения,
...
Процедура ИзменитьОперации(МенеджерВременнныхТаблиц)
	...
	Пока ВыборкаЭтап.Следующий() Цикл
		...
		РассчитатьОперации(ВыборкаЭтап.Этап);
...
Функция ТекстЗапросаИзменитьОперации()
	...
	Возврат
		...
		|		ВТОперации.ВремяВыполнения,
		|		ВТОперации.ВремяВыполненияЕдИзм,
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(ИнформацияОбОшибке)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Операции к созданию сменных заданий'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура УдалитьОперацииЭтапа(Этап) Экспорт
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяРегистра = "РегистрСведений.ОперацииКСозданиюСменныхЗаданий";
	...
	Запрос = Новый Запрос(
		...
		|	РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК ОперацииКСозданиюСменныхЗаданий
		|ГДЕ
		|	ОперацииКСозданиюСменныхЗаданий.СменноеЗадание <> ЗНАЧЕНИЕ(Документ.СменноеЗадание.ПустаяСсылка)
		|	И ОперацииКСозданиюСменныхЗаданий.Порядок = 0");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ДанныеКОбработке.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("СменноеЗадание", ДанныеКОбработке.СменноеЗадание);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			
			Блокировка.Заблокировать();
			
			Запрос.УстановитьПараметр("СменноеЗадание", ДанныеКОбработке.СменноеЗадание);
			Выборка = Запрос.Выполнить().Выбрать();
			
			Набор = РегистрыСведений.ОперацииКСозданиюСменныхЗаданий.СоздатьНаборЗаписей();
			Набор.Отбор.СменноеЗадание.Установить(ДанныеКОбработке.СменноеЗадание);
		...
		Исключение
			...
			ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(
				ИнформацияОбОшибке(),
				ДанныеКОбработке.СменноеЗадание);
