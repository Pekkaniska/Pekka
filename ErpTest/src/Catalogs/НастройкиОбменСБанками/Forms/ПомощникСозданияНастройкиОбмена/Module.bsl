		...
		Если ИспользуетсяВнешнийМодуль Тогда
			Если КомпонентаОбработка = 0 Тогда // Компонента
				Если НЕ ЗначениеЗаполнено(ИмяВнешнегоМодуля) Тогда
					ТекстСообщения = НСтр("ru = 'Загрузите внешнюю компоненту из файла.'");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ВнешняяКомпонента");
					Возврат;
				КонецЕсли;
			...
			Иначе //внешняя обработка
				Если НЕ ЗначениеЗаполнено(ДополнительнаяОбработка) Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ВнешняяОбработка");
					Возврат;
				КонецЕсли;
...
Процедура ПолучитьНастройкиОбменаПослеПолученияИдентификатораСессии(ИдентификаторСессии, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		ПараметрыОжидания.ВыводитьСообщения = НЕ ПробнаяОперация;
		ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
...
Процедура ПослеПолученияДвоичныхДанныхСертификатаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбменСБанкамиСлужебныйКлиент.АутентификацияПоСертификатуСбербанк(
		Оповещение, ИмяВнешнегоМодуля, ДанныеСертификата, НастройкаОбмена);
...
&НаКлиенте
Процедура ВыполнитьБазовуюАутентификациюСбербанк()
	...
	ОбменСБанкамиСлужебныйКлиент.ВыполнитьБазовуюАутентификациюСбербанк(
		Оповещение, ИмяВнешнегоМодуля, УникальныйИдентификатор, НастройкаОбмена, ЛогинПароль, ТекстПояснения);
...
&НаСервереБезКонтекста
Функция ЗаданиеПоПолучениюИдентификатораВКСбербанк(Знач URLВК)
...
Процедура ПолучитьТипыПисемПоТикетуЗапроса(Знач НастройкаОбмена, Знач ИдентификаторОрганизации, Знач КлючСессии, Знач Тикет, ТипыПисем, Ожидать, Ошибка)
	...
	ИначеЕсли Ответ = "<!--REQUESTID DUBLIC-->" ИЛИ Ответ = "<!--REQUESTID_DUBLIC-->" Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , , , Ошибка);
	Иначе
		ТипыПисем = ОбменСБанкамиСлужебныйВызовСервера.ТипыПисемСбербанк(Ответ, КлючСессии);
...
Процедура ПолучитьТипыПисемЧерезТокенСбербанк()
	...
	ЗапросТиповПисем = ОбменСБанкамиСлужебныйВызовСервера.ЗапросТиповПисемСбербанк(ИдентификаторОрганизации);
...
Процедура ОпределитьИдентификаторВКСбербанк()
	...
	Результат = ЗаданиеПоПолучениюИдентификатораВКСбербанк(URLВК);
...
Процедура ПослеПолученияОтветаНаЗапросТиповПисемЧерезТокенСбербанк(Ответ, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Ответ = "<!--REQUESTID DUBLIC-->" ИЛИ Ответ = "<!--REQUESTID_DUBLIC-->" Тогда
		...
		ОбработатьРезультатПолученияНастроекССервераБанка(Ложь);
	Иначе
		ТипыПисемСсылкаСбербанк = ОбменСБанкамиСлужебныйВызовСервера.ТипыПисемСбербанк(Ответ, УникальныйИдентификатор);
...
Процедура ОтправитьЗапросТиповПисемСбербанк(Знач НастройкаОбмена, Знач ИдентификаторОрганизации, Знач КлючСессии, ТикетОбменаСбербанк, Ошибка)
	...
	ДанныеЗапрос = ОбменСБанкамиСлужебныйВызовСервера.ЗапросТиповПисемСбербанк(ИдентификаторОрганизации);
...
Процедура ПолучитьИдентификаторВнешнейКомпоненты(Оповещение)
	...
	Результат = ЗапускЗаданияПоПолучениюИнформацииОВнешнейКомпоненте(URLВК);
...
&НаСервереБезКонтекста
Функция ЗапускЗаданияПоПолучениюИнформацииОВнешнейКомпоненте(Знач URLИнфоФайла)
