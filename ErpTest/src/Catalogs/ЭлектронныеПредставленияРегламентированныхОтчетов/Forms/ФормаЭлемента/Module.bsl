...
Процедура Отправить(Команда)
	...
	Если СтрНачинаетсяС(Объект.Наименование, "Декларация по НДС") Тогда
...
Функция ОпределитьПериодФСРАР(ДеревоXML)
	...
	Если УзелФормаОтч.Свойства().Получить("ПризПериодОтч") = Неопределено Тогда
		...
		Если НомФорм = "13" ИЛИ НомФорм = "14" ИЛИ НомФорм = "15" Тогда
...
Функция ТекстСообщенияОНепройденномКонтролеНомеровКорректировок()
	...
	Если ((ТипЗнч(Объект.ВидОтчета) = Тип("СправочникСсылка.ВидыОтправляемыхДокументов")
		И СокрЛП(Объект.ВидОтчета.Источник) = "РегламентированныйОтчетНДС")
		ИЛИ ((ТипЗнч(Объект.ВидОтчета) = Тип("СправочникСсылка.РегламентированныеОтчеты")
		ИЛИ ТипЗнч(Объект.ВидОтчета) = Тип("СправочникСсылка.УдалитьРегламентированныеОтчеты"))
		И СокрЛП(Объект.ВидОтчета.ИсточникОтчета) = "РегламентированныйОтчетНДС"))
