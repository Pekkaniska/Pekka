...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	"ВЫБРАТЬ
	|	УдалитьНастройкиЯндексКассы.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.УдалитьНастройкиЯндексКассы КАК УдалитьНастройкиЯндексКассы
	|ГДЕ
	|	УдалитьНастройкиЯндексКассы.НастройкаБЭД = ЗНАЧЕНИЕ(Справочник.НастройкиЯндексКассы.ПустаяСсылка)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Справочник.УдалитьНастройкиЯндексКассы";
	...
	РеквизитыДляЗаполнения.Добавить("СтатьяДвиженияДенежныхСредствОплаты");
	РеквизитыДляЗаполнения.Добавить("СтатьяДвиженияДенежныхСредствВозвраты");
	РеквизитыДляЗаполнения.Добавить("СтатьяРасходов");
	РеквизитыДляЗаполнения.Добавить("БанковскийСчет");
	РеквизитыДляЗаполнения.Добавить("АналитикаРасходов");
	РеквизитыДляЗаполнения.Добавить("Подразделение");
	РеквизитыДляЗаполнения.Добавить("Эквайер");
	...
	"ВЫБРАТЬ
	|	УдалитьСтатусОбменовСЯндексКассой.ДатаПоследнегоУспешногоОбмена КАК ДатаПоследнегоУспешногоОбмена,
	|	УдалитьСтатусОбменовСЯндексКассой.НастройкаЯндексКассы КАК НастройкаЯндексКассы,
	|	УдалитьСтатусОбменовСЯндексКассой.Организация КАК Организация
	|ИЗ
	|	РегистрСведений.УдалитьСтатусОбменовСЯндексКассой КАК УдалитьСтатусОбменовСЯндексКассой";
	
	СтатусыОбменов = Запрос.Выполнить().Выгрузить();
	...
	Пока Выборка.Следующий() Цикл
		...
		// Запись нового настройки и корректировка старой.
		
		СтараяНастройка = Выборка.Ссылка.ПолучитьОбъект();
		...
		НоваяНастройка = Справочники.НастройкиЯндексКассы.СоздатьЭлемент();
		
		ЗаполнитьЗначенияСвойств(НоваяНастройка, СтараяНастройка, 
			"Организация, СДоговором, ИдентификаторМагазина, ИдентификаторВитрины, 
			|	ОтправкаЧековЧерезЯндекс, Недействительна, ПометкаУдаления, Наименование");
		
		Для каждого ЭлементКоллекции Из РеквизитыДляЗаполнения Цикл
			
			НоваяСтрока = НоваяНастройка.ДополнительныеНастройки.Добавить();
			
			НоваяСтрока.Настройка = ЭлементКоллекции;
			НоваяСтрока.Значение = СтараяНастройка[ЭлементКоллекции];
			
		КонецЦикла;
		
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НоваяНастройка);	

		СтараяНастройка.НастройкаБЭД = НоваяНастройка.Ссылка;
		
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(СтараяНастройка);	
		
		// Корректировка регистра.
		
		ТекущийСтатус = СтатусыОбменов.Найти(СтараяНастройка.Ссылка, "НастройкаЯндексКассы");
		
		Если ТекущийСтатус <> Неопределено Тогда
			
			НаборЗаписей = РегистрыСведений.СтатусОбменовСЯндексКассой.СоздатьНаборЗаписей();
			
			НаборЗаписей.Отбор.НастройкаЯндексКассы.Установить(НоваяНастройка.Ссылка);
			
			НоваяЗапись = НаборЗаписей.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяЗапись, ТекущийСтатус, , "НастройкаЯндексКассы");
			
			НоваяЗапись.НастройкаЯндексКассы = НоваяНастройка.Ссылка;
