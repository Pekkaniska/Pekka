...
Процедура СистемаНалогообложенияПриИзменении(Элемент)
	...
	Если ПрименяетсяУСН Тогда
		...
		Объект.РаздельныйУчетТоваровПоНалогообложениюНДС = Ложь;
	...
	Иначе
		...
		Если Объект.ПрименяетсяЕНВД Тогда
			Объект.РаздельныйУчетТоваровПоНалогообложениюНДС = Истина;
		КонецЕсли;
...
Процедура РаздельныйУчетТоваровПоНалогообложениюНДСПриИзменении(Элемент)
	...
	Если Объект.МетодОценкиСтоимостиТоваров <> ПредопределенноеЗначение("Перечисление.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка")
	 ИЛИ НЕ Объект.РаздельныйУчетТоваровПоНалогообложениюНДС Тогда
		Объект.РаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС = Ложь;
	КонецЕсли;
	
	Если НЕ Объект.РаздельныйУчетТоваровПоНалогообложениюНДС Тогда
...
Процедура РаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДСПриИзменении(Элемент)
	...
	Если Объект.МетодОценкиСтоимостиТоваров <> ПредопределенноеЗначение("Перечисление.МетодыОценкиСтоимостиТоваров.ФИФОСкользящаяОценка")
	 ИЛИ НЕ Объект.РаздельныйУчетТоваровПоНалогообложениюНДС
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.РаздельныйУчетПостатейныхПроизводственныхЗатратПоНалогообложениюНДС.Доступность =
		...
		И Объект.РаздельныйУчетТоваровПоНалогообложениюНДС;
	...
	Элементы.УчетНДСПоФактическомуИспользованию.Доступность =
		Объект.РаздельныйУчетТоваровПоНалогообложениюНДС И НЕ Объект.ПрименяетсяОсвобождениеОтУплатыНДС;
	Элементы.Учитывать5ПроцентныйПорог.Доступность = Элементы.УчетНДСПоФактическомуИспользованию.Доступность;
	
	Элементы.ГруппаВариантУчетаНДСПриИзмененииВидаДеятельности.Доступность = Объект.РаздельныйУчетТоваровПоНалогообложениюНДС;
	
	Элементы.ГруппаПравилаСписанияНДСПоТоварам.Доступность =
		Объект.РаздельныйУчетТоваровПоНалогообложениюНДС
...
Процедура ЗаполнитьПояснениеЛизинговыеПлатежи(Форма)
	...
	Если Форма.Объект.ВключатьВСоставНалоговыхРасходовЛизинговыеПлатежи Тогда
		...
		Если Форма.Объект.ПрименяетсяПБУ18 Тогда
			ТекстПодсказки = 
