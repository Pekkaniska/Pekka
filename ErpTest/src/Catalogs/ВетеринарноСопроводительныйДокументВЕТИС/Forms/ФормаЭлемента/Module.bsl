...
&НаСервере
Функция ПродукцияПриИзмененииНаСервере()
	...
	Если ТекущаяТочностьДоступна = Неопределено Тогда
		...
		Объект.ДатаПроизводстваТочностьЗаполнения = "";
...
КонецФункции // ПродукцияПриИзмененииНаСервере() 
...
Процедура ОткрытьЧерезБраузер(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Идентификатор)Тогда 
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(ИнтеграцияВЕТИСКлиентСервер.ПутьКСерверуСИнформациейПоВСД()+Объект.Идентификатор);
	Иначе 
