...
Процедура СоздатьОбновитьКлючРеестра(СправочникОбъект, ЭтоНовый) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СправочникОбъект, "ИНН") Тогда
		СтруктураПолейОбъекта.ИНН = ?(ЗначениеЗаполнено(СправочникОбъект.ИНН),СправочникОбъект.ИНН,"");
	Иначе
	...
	Если Не ЭтоНовый Тогда
		...
		|	КлючиРеестраДокументов.Наименование КАК Наименование,
		|	КлючиРеестраДокументов.Ключ КАК Ключ,
		|	КлючиРеестраДокументов.ИНН КАК ИНН,
		|	КлючиРеестраДокументов.Организация КАК Организация,
		...
		Если Выборка.Следующий() Тогда
			...
			Если ОбщегоНазначенияУТКлиентСервер.СтруктурыРавны(СтруктураПолейОбъекта, Выборка) Тогда
				Возврат;
			КонецЕсли;
...
Процедура ЗарегистрироватьКлючиДляУдаленияДублей(Параметры)Экспорт
	
	РезультатЗапроса = ДублирующиеКлючи(Ложь);
	
	КлючиДляОбновления = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Ссылка");
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, КлючиДляОбновления);
	...
	|	РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|ГДЕ                                   
	|	(РеестрДокументов.МестоХранения В (&КлючиДляОбновления)
	|			ИЛИ РеестрДокументов.Контрагент В (&КлючиДляОбновления))";
	Запрос.УстановитьПараметр("КлючиДляОбновления", КлючиДляОбновления);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюУдалитьДубли(Параметры) Экспорт
	
	//Обрабатываем не порциями, удаление каждого клайстера дублей делаем в отдельной транзакции
	                             
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыВыборкиДанныхДляОбработки();
	ДополнительныеПараметры.ВыбиратьПорциями    = Ложь;
	ДополнительныеПараметры.ИмяВременнойТаблицы = "ВТДубли";
	
	ПолноеИмяОбъекта = "Справочник.КлючиРеестраДокументов";
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	РезультатСозданияВТ = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуСсылокДляОбработки(Параметры.Очередь,
		ПолноеИмяОбъекта,
	...
	|	ВТДубли КАК ВТДубли
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиРеестраДокументов КАК КлючиРеестраДокументов
	|		ПО ВТДубли.Ссылка = КлючиРеестраДокументов.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ключ,
	|	Ссылка
	...
	УдалитьДубли(Запрос.Выполнить(), Истина);
