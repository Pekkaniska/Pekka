...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ПлатежиПо275ФЗ Тогда
		Если Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПоставщиком И ВариантПлатежаГОЗ = 1 Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтатьяДвиженияДенежныхСредств) Тогда
		ПоказатьОповещениеПользователя(
			...
			"Не заполнена статья движения денежных средств!",
			БиблиотекаКартинок.МониторПортала1СИТСНегативноеСобытие,
...
//+++rarus-spb_zlov 20.11.2016    
    Объект.Менеджер = пкОбщегоНазначенияСервер.ЗначениеРеквизитаОбъекта(Объект.Партнер, "ОсновнойМенеджер");
//---rarus-spb_zlov 20.11.2016 
...
Процедура ЗаполнитьПодтверждающиеДокументы(Команда)
	...
	ОбщегоНазначенияУТКлиент.ПроверитьВозможностьЗаполненияТабличнойЧасти(
		Оповещение, ЭтаФорма, Объект.ПодтверждающиеДокументы,, Ложь);
	//-- НЕ УТ
...
Процедура ТипДоговораПриИзмененииСервер()
	...
	УправлениеЭлементамиГосКонтрактов();
	Элементы.ГруппаЗаполнениеРаздела7.Видимость = (Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем);
	//-- НЕ УТ
...
//++ RARUS_IUC 20180119
&НаКлиенте
Процедура пкЕдИзмКЦПриИзмерении(Элемент)
	
	ТекущиеДанные = Элементы["пкТарифы"].ТекущиеДанные;
	
	Если НЕ ЗначениеЗаполнено(ТекущиеДанные.ЕдиницаИзмеренияКраткосрочнойЦены) Тогда
		ТекущиеДанные.ЕдиницаИзмеренияКраткосрочнойЦены = пкПолучитьЕдХрНаСервере(ТекущиеДанные.Модель);	
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция пкПолучитьЕдХрНаСервере(Номенклатура)
	Возврат Номенклатура.ЕдиницаИзмерения;
КонецФункции

&НаКлиенте
Процедура пкЕдИзмДЦПриИзмерении(Элемент)
	
	ТекущиеДанные = Элементы["пкТарифы"].ТекущиеДанные;
	
	Если НЕ ЗначениеЗаполнено(ТекущиеДанные.ЕдиницаИзмеренияДолгосрочнойЦены) Тогда
		ТекущиеДанные.ЕдиницаИзмеренияДолгосрочнойЦены = пкПолучитьЕдХрНаСервере(ТекущиеДанные.Модель);	
	КонецЕсли;
...
Процедура ЗаполнитьПодтверждающиеДокументыСервер()
	...
	Объект.ПодтверждающиеДокументы.Загрузить(ПолучитьИзВременногоХранилища(
		ДенежныеСредстваСервер.ПодтверждающиеДокументыФЗ275(Объект.ТипПлатежаФЗ275, УникальныйИдентификатор)));
