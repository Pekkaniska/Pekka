...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ПлатежиПо275ФЗ Или Не ДоговорСУчастникомГОЗ Тогда
		ДоговорыСЗаказчиками.Очистить();
	КонецЕсли;
	//-- НЕ УТ
	...
	//++ НЕ УТ
	ИначеЕсли ПлатежиПо275ФЗ И ЗначениеЗаполнено(БанковскийСчет) Тогда
		ИдентификаторПлатежа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчет, "ГосударственныйКонтракт.Код");
	//-- НЕ УТ
	Иначе
	...
	ПустойШаблон = ТипДоговора <> Перечисления.ТипыДоговоров.СПереработчиком И ТипДоговора <> Перечисления.ТипыДоговоров.СДавальцем;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДоговорИспользуетсяПриПриемке Тогда
		...
		Документы.РегистраторГрафикаДвиженияТоваров.РегистраторПоРаспоряжению(Ссылка, Истина);
	...
	ОбщегоНазначенияУТ.УстановитьПометкуУдаленияКлюча("Документ.РегистраторГрафикаДвиженияТоваров",
														Ссылка,
														ДополнительныеСвойства,
														ПометкаУдаления,
														Ложь);
...
//++ Рарус Лимаренко 06.03.2018 Заявка №
...
//Рарус Владимир Подрезов 20.01.2018 5683
	Если ТипДоговора = Перечисления.ТипыДоговоров.СПоставщиком Тогда
		Если Не (пкМодификацияСервер.ПроверитьВхождениеПользователяВГруппупкУправлениеСогласованиемДоговоровСПоставщикамиРарус() ИЛИ РольДоступна("ПолныеПрава")) Тогда
			Статус							= Перечисления.СтатусыДоговоровКонтрагентов.НеСогласован;
			Согласован						= Ложь;
			пкПройденПроцессСогласования	= Ложь;
			//Менеджер						= Пользователи.ТекущийПользователь();
		КонецЕсли;
	КонецЕсли;
