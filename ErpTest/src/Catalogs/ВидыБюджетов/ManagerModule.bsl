...
Функция ВидБюджетаЗаполненПравильно(ВидБюджета) Экспорт
	...
	РезультатПроверки = РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.ПолучитьЗначениеКэша(ВидБюджета, ВидЭлементаКэша);
	...
	ПроверитьОдинаковыеВидыАналитикВИзмеренияхБюджета(ВидБюджета);
	...
	РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.УстановитьЗначениеКэша(ВидБюджета, ВидЭлементаКэша, Не ЕстьОшибки);
...
Процедура ПроверитьДублиСтрокСтатейБюджетовПоАналитике(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	ДеревоЭлементов = БюджетнаяОтчетностьРасчетКэшаСервер.ДеревоЭлементовБюджета(ВидБюджета);
...
Процедура ПроверитьОтсутствиеИзмеренийАналитикВРежимеВвода(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	ДеревоЭлементов = БюджетнаяОтчетностьРасчетКэшаСервер.ДеревоЭлементовБюджета(ВидБюджета, Истина);
...
Процедура ПроверитьНеПересечениеПериодов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого СтрокаСтруктуры Из СтруктураЯчеек Цикл
		...
		Если МассивВышестоящихПериодовКолонки.Количество()
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", 
								НСтр("ru = 'Период не может быть и в строках и в колонках!'"), "");
			Прервать;
		...
		Если НеверныйПорядокПериодов(МассивВышестоящихПериодовКолонки)
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "",
								НСтр("ru = 'Больший период не может входить в меньший!'"), "");
			Прервать;
	...
	Для Каждого СтрокаСтруктуры Из СтруктураЯчеек Цикл
		...
		Если ЕстьНеделяВДекадах(МассивВышестоящихПериодовСтроки)
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "",
									НСтр("ru = 'Неделя не может входить в декаду!'"), "");
			Возврат;
...
Процедура ПроверитьНаличиеЦиклов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Пока НайденныеСтроки.Количество() Цикл
		...
		Если СталоСтрок = ВсегоСтрок Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "",
				НСтр("ru = 'Обнаружены циклические ссылки в формулах!'"), "");
...
Процедура ПроверитьНаличиеИсточниковДанных(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Если Не ИсточникиДанных.Количество() Тогда
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", 
			НСтр("ru = 'Не выбраны статьи бюджетов или показатели бюджетов в структуре вида бюджета'"), "");
	КонецЕсли;
	...
	Если РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Документ Тогда
		...
		Если Не ЕстьИсточникДокумент Тогда
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках, "", 
				НСтр("ru = 'Для ввода данных должны быть выбраны статьи бюджета'"), "");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьСоответствиеВалютыСтатьям(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	Для Каждого КлючиИЗначение Из Структура Цикл
		Для Каждого СтрокаОшибки Из КлючиИЗначение.Значение Цикл
			ТекстОшибки = НСтр("ru = 'Для %1 ""%2"" недоступен учет по валюте'");
			
			Если КлючиИЗначение.Ключ = "Статьи" Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'статьи'"), 
																					СтрокаОшибки.СтатьяПоказатель);
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, НСтр("ru = 'показателя'"), 
																					СтрокаОшибки.СтатьяПоказатель);
			КонецЕсли;
...
Процедура ПроверитьНаличиеСтрокКолонок(ВидБюджета, СообщенияОбОшибках)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Если Не НайденнаяСтрока.Строки.Количество() Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,
				"",
				НСтр("ru = 'Строки таблицы не могут быть пустыми'"),
				"");
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Если Не НайденнаяСтрока.Строки.Количество() Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СообщенияОбОшибках,
				"",
				НСтр("ru = 'Колонки таблицы не могут быть пустыми'"),
				"");
...
Процедура ПроверитьПересечениеАналитическихРазрезов(ВидБюджета, СообщенияОбОшибках, РежимФормирования)
	...
	ДеревоЭлементов = БюджетнаяОтчетностьРасчетКэшаСервер.ДеревоЭлементовБюджета(ВидБюджета, Ложь);
...
Процедура ПроверитьОдинаковыеВидыАналитикВИзмеренияхБюджета(ВидБюджета)
	
	Если ВидБюджета.ОсновнойСпособВвода <> Перечисления.СпособыРедактированияБюджета.ВСписке Тогда
		Возврат;
	КонецЕсли;
	
	ЕстьОшибка = Ложь;
	
	ПараметрыВидаБюджета = БюджетнаяОтчетностьРасчетКэшаСервер.ПолучитьПараметрыВыводаВидаБюджета(
		ВидБюджета,
...
Функция ВидБюджетаИспользуетГраницуФактическихДанных(ВидБюджета) Экспорт
	...
	РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.УстановитьЗначениеКэша(ВидБюджета, ВидЭлементаКэша, ИспользуетГраницуФактическихДанных);
...
Процедура НастроитьГруппуЭлементовПериода(ВидБюджета, Элементы, ГраницаФактДанных) Экспорт
	...
	Иначе
		...
		Элементы.НадписьВидБюджета.Заголовок = НСтр("ru = 'Фиксированный бюджет'");
	...
	Если ВидБюджетаИспользуетГраницуФактическихДанных(ВидБюджета) Тогда
		...
		Элементы.ГраницаФактДанных.Заголовок = НСтр("ru = 'Граница факт.данных: %1'");
...
// Очищает кэш вспомогательных данных.
// 
Процедура ОчиститьКэшВспомогательныхДанных(Параметры) Экспорт
	...
	Исключение
		...
		ТекстСообщения = НСтр("ru = 'Не удалось очистить кэш вспомогательных данных по причине: %Причина%'");
...
Процедура ЗарегистрироватьДанныеДляОчисткиКэшВспомогательныхДанных(Параметры) Экспорт
	
	Возврат; // Кэш очищается целиком
