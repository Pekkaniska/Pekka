...
Процедура РекомендацииПоПовышениюСкоростиРасчета(Команда)
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Вид бюджета должен быть записан для расчета рекомендаций'"));
		Возврат;
	КонецЕсли;
...
Процедура ЭлементыОтчетаПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, СтрокаПриемникОтражение, Поле)
	...
	Если Элементы.ЭлементыОтчетаОтображение.ТолькоПросмотр Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(,НСтр("ru = 'Вид бюджета используется в документах и недоступен для редактирования'"));
		Возврат;
	КонецЕсли;
...
Процедура ПосмотретьВидОтчета(Команда)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Или Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПосмотретьВидОтчетаЗавершение", ЭтотОбъект), 
			НСтр("ru = 'Просмотр вида отчета возможен только после записи данных. Записать?'"), 
...
Процедура СоздатьСтатьюБюджетов(Команда)
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Можно добавлять только статьи бюджетов и показатели бюджетов'"), 60);
		Возврат;
	КонецЕсли;
...
Процедура СоздатьЭкземплярБюджетаФрагмент()
	...
	Если Не ПроверитьВидБюджетаНаСервере() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Обнаружены ошибки'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьВидБюджетаФрагмент()
	...
	Если Не ПроверитьВидБюджетаНаСервере() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Обнаружены ошибки'"));
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Ошибок нет'"));
	КонецЕсли;
...
Процедура Сгруппировать(Команда)
	...
	Таблица = ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(СтрокаПроверки, ВидЭлемента("ТаблицаСложная"));
	Если Таблица = Неопределено Тогда
		Таблица = ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(СтрокаПроверки, ВидЭлемента("ТаблицаПоказателиВКолонках"));
		Если Таблица = Неопределено Тогда
			Таблица = ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(СтрокаПроверки, ВидЭлемента("ТаблицаПоказателиВСтроках"));
		КонецЕсли;
	КонецЕсли;
	
	Если Таблица <> Неопределено И ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(СтрокаПроверки, ВидЭлемента("Строки")) = Неопределено Тогда
		ЕстьОшибки = Истина;
	Иначе
	...
	Для Каждого СтрокаДерева Из СтрокиДерева Цикл
		...
		Если Не НоваяСтрока = Неопределено Тогда
			Если НоваяСтрока.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НастройкаЯчеек Тогда
				...
				Если СложнаяТаблицаПривилегированно.НайтиСтроки(Новый Структура("Родитель",СтрокаДерева.ЭлементОтчета)).Количество() > 0 Тогда
					ЕстьПривилегированныйРежим = Истина;
				КонецЕсли;
			Иначе
				ЕстьПривилегированныйРежим = Ложь;
				Если ЗначениеЗаполнено(НоваяСтрока.ПривилегированныйРежим) И Не НоваяСтрока.ПривилегированныйРежим Тогда
					Если ОперандыФормулыПривилегированно.НайтиСтроки(Новый Структура("Родитель",СтрокаДерева.ЭлементОтчета)).Количество() > 0 Тогда
						ЕстьПривилегированныйРежим = Истина;
					КонецЕсли;
				Иначе
					ЕстьПривилегированныйРежим = ?(ЗначениеЗаполнено(НоваяСтрока.ПривилегированныйРежим),НоваяСтрока.ПривилегированныйРежим, Ложь)
				КонецЕсли;
			КонецЕсли;
			НоваяСтрока.ПривилегированныйРежимКартинка = Не Число(ЕстьПривилегированныйРежим);
		КонецЕсли;
		
		ВывестиДеревоЭлементовВДеревоОтображения(СтрокаДерева, ТекущийРодитель, ГлубинаУровней, ПараметрыПривилегированногоПолучения);
	КонецЦикла;
	...
	Для Каждого НовыйЭлементВидаБюджета Из НовыеЭлементы Цикл
		...
		Если ВидЭлемента = ВидЭлемента("Измерение") Тогда
			...
			Если ТребуетсяФормаВыбораЗначений Тогда
					...
					КонецЕсли;
					ОткрытьФорму(ИмяФормыВыбора, ПараметрыПодбора, ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
				Продолжить;
			КонецЕсли;
...
Процедура ПриВыбореСвойствИзмерения(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Значение Из Результат Цикл
			...
			Если НайденныйЭлемент.ЭтоАналитикаПрочее Тогда
				Если Значение.РазрешитьРедактирование Тогда
					Если Значение.ВсеЗначения Тогда
						НайденныйЭлемент.НаименованиеДляПечати = НСтр("ru = '<все %1>'");
					Иначе
						НайденныйЭлемент.НаименованиеДляПечати = НСтр("ru = '<добавляемые и прочие %1>'");
					КонецЕсли;
				...
				Иначе
					НайденныйЭлемент.НаименованиеДляПечати = НСтр("ru = '<прочие %1>'");
					НайденныйЭлемент.НаименованиеДляПечати = 
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НайденныйЭлемент.НаименованиеДляПечати, Нрег(Значение.СтатьяПоказательТипИзмерения));
				КонецЕсли;
		...
		Если НоваяСтрока.ЭтоАналитикаПрочее Тогда
			Если Значение.РазрешитьРедактирование Тогда
				Если Значение.ВсеЗначения Тогда
					НоваяСтрока.НаименованиеДляПечати = НСтр("ru = '<все %1>'");
					НоваяСтрока.ИсточникВсеЭлементы = Истина;
				Иначе
					НоваяСтрока.НаименованиеДляПечати = НСтр("ru = '<добавляемые и прочие %1>'");
				КонецЕсли;
			...
			Иначе
				НоваяСтрока.НаименованиеДляПечати = НСтр("ru = '<прочие %1>'");
				НоваяСтрока.НаименованиеДляПечати = 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НоваяСтрока.НаименованиеДляПечати, Нрег(НоваяСтрока.СтатьяПоказательТипИзмерения));
			КонецЕсли;
		КонецЕсли;
...
Процедура АналитикиШапкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Элементы.АналитикиШапки.ТолькоПросмотр Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Не рекомендуется изменять аналитики, не пройдя проверку на разблокирование реквизитов формы'"));
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьКэшСтатейИПоказателей(СтрокаДереваЭлементов, ТзКэшСтатейИПоказателей)
	...
	Для Каждого ЭлементДляПроверки Из МассивЭлементовДляПроверки Цикл
		...
		Для Каждого Стр Из ЭлементДляПроверки.ОперандыФормулы Цикл
			...
			Если ТипЗнч(ЭлементДляПроверки.ОперандыФормулы) = Тип("ТаблицаЗначений") И ЗначениеЗаполнено(Стр.АдресСтруктурыЭлемента) Тогда
			...
			Если ДанныеОперанд.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
				НоваяСтрока.СтатьяПоказатель = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеОперанд, "СтатьяБюджетов");
			ИначеЕсли ДанныеОперанд.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
				НоваяСтрока.СтатьяПоказатель = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеОперанд, "ПоказательБюджетов");
			ИначеЕсли ДанныеОперанд.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НефинансовыйПоказатель Тогда
				НоваяСтрока.СтатьяПоказатель = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеОперанд, "НефинансовыйПоказатель");
			КонецЕсли;
			НоваяСтрока.ПривилегированныйРежим = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеОперанд, "ПривилегированныйРежим");
