...
Процедура ПроверитьИспользованиеПараметровПроизводства(Параметры, АдресХранилища) Экспорт
	...
	РезультатПроверки.Вставить("УправлениеМаршрутнымиЛистамиИспользуется",    НЕ Результат[2].Пустой());
	
	РезультатПроверки.Вставить("ПооперационноеРасписаниеИспользуется",        НЕ Результат[3].Пустой());
	
	РезультатПроверки.Вставить("ИспользуетсяКакДиспетчер",                    НЕ Результат[4].Пустой());
	
	РезультатПроверки.Вставить("ИспользуетсяДляПроизводстваПоЗаказам",        НЕ Результат[5].Пустой());
	
	РезультатПроверки.Вставить("ИспользуетсяПооперационноеУправлениеЭтапами", НЕ Результат[6].Пустой());
	
	РезультатПроверки.Вставить("ИспользуютсяСменныеЗадания",                  НЕ Результат[7].Пустой());
	//-- НЕ УТКА
