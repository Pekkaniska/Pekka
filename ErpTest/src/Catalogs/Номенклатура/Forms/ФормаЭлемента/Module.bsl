...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Отказ
		...
		ПараметрыФормы.Вставить("ЕстьСвязьСКлассификаторомПродукцииВЕТИСВОбъектеКопирования", ЕстьСвязьСКлассификаторомПродукцииВЕТИСВОбъектеКопирования);
		...
		ПараметрыФормы.Вставить("КопироватьСвязьСКлассификаторомПродукцииВЕТИС", КопироватьСвязьСКлассификаторомПродукцииВЕТИС);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ИначеЕсли КопироватьСвязьСКлассификаторомПродукцииВЕТИС Тогда
		...
		ИнтеграцияВЕТИСУТ.КопироватьСвязьСКлассификаторомПродукцииВЕТИС(
			ИсточникКопирования,
			Объект.Ссылка,
			УстановитьСвязьСХарактеристикамиПродукцииВЕТИС,
	...
	Если ИзображенияИзСервиса.Количество() > 0 Тогда
		РаботаСНоменклатуройУТ.ПрикрепитьИзображения(ТекущийОбъект, ИзображенияИзСервиса);
		Прочитать();
	...
	Если ЭтоАдресВременногоХранилища(ШтрихкодыСервис) Тогда
		...
		Для каждого Элемент Из ШтрихКоды Цикл
			Элемент.Номенклатура = ТекущийОбъект.Ссылка;
		КонецЦикла; 
		РаботаСНоменклатуройУТ.ЗаписатьШтрихкоды(Штрихкоды);
		Прочитать();
	КонецЕсли;
	// Конец ЭлектронноеВзаимодействие.РаботаСНоменклатурой
...
Процедура ЗаполнитьПоПараметрам()
	...
	Если (Параметры.Свойство("ВидАлкогольнойПродукцииЕГАИС")
			И ЗначениеЗаполнено(Параметры.ВидАлкогольнойПродукцииЕГАИС))
		Или (Параметры.Свойство("Параметры.ИмпортерЕГАИС")
			И ЗначениеЗаполнено(Параметры.ИмпортерЕГАИС))
		Или (Параметры.Свойство("Параметры.ОбъемЕГАИС")
			И ЗначениеЗаполнено(Параметры.ОбъемЕГАИС))
		Или (Параметры.Свойство("Параметры.ПроизводительЕГАИС")
			И ЗначениеЗаполнено(Параметры.ПроизводительЕГАИС))
		Или (Параметры.Свойство("Параметры.КрепостьЕГАИС")
			И ЗначениеЗаполнено(Параметры.КрепостьЕГАИС))Тогда
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.ВидНоменклатуры)
		И (Параметры.Свойство("НаименованиеНоменклатурыВЕТИС")
			Или Параметры.Свойство("АртикулНоменклатурыВЕТИС")
			Или Параметры.Свойство("ШтрихкодВЕТИС")
			Или Параметры.Свойство("ЕдиницаИзмеренияВЕТИС")) Тогда
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		//++ НЕ УТ
		Объект.ДрагоценныеМатериалы.Загрузить(ИсточникКопирования.ДрагоценныеМатериалы.Выгрузить());
		//-- НЕ УТ
		...
		ИначеЕсли Параметры.Свойство("НаименованиеНоменклатурыВЕТИС")
			И ЗначениеЗаполнено(Параметры.НаименованиеНоменклатурыВЕТИС) Тогда
			
			НаименованиеЗаполнение = Параметры.НаименованиеНоменклатурыВЕТИС;
		...
		ИначеЕсли Параметры.Свойство("АртикулНоменклатурыВЕТИС")
			И ЗначениеЗаполнено(Параметры.АртикулНоменклатурыВЕТИС) Тогда
			
			Объект.Артикул = Параметры.АртикулНоменклатурыВЕТИС;
			
		КонецЕсли;
		
		Если Параметры.Свойство("ШтрихкодВЕТИС")
			И ЗначениеЗаполнено(Параметры.ШтрихкодВЕТИС) Тогда
			
			Штрихкод = Параметры.ШтрихкодВЕТИС;
			
		КонецЕсли;
		
		Если Параметры.Свойство("ЕдиницаИзмеренияВЕТИС")
			И ЗначениеЗаполнено(Параметры.ЕдиницаИзмеренияВЕТИС) Тогда
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Параметры.Свойство("АртикулНоменклатурыВЕТИС")
			И ЗначениеЗаполнено(Параметры.АртикулНоменклатурыВЕТИС) Тогда
		...
		Если Параметры.Свойство("НаименованиеНоменклатурыВЕТИС")
			И ЗначениеЗаполнено(Параметры.НаименованиеНоменклатурыВЕТИС) Тогда
