...
Процедура ИсторияИзмененийОрганизация(Команда)
	...
	ПараметрыОтбора.Вставить("СтруктурнаяЕдиница", Организация);
	
	ПараметрыОткрываемойФормы = Новый Структура();
	ПараметрыОткрываемойФормы.Вставить("Отбор",       ПараметрыОтбора);
	ПараметрыОткрываемойФормы.Вставить("ОтборПоТипу", "Организации");
...
Процедура ЗаписатьНалоговыйОрганДляПодразделения(ПараметрыЗаписи)
	...
	Иначе
		...
		Пока ВыборкаПодразделения.Следующий() Цикл
			...
			ЯвляетсяОбособленным = ОсновнаяРегистрация <> ПараметрыЗаписи.НалоговыйОрган И ЗначениеЗаполнено(ПараметрыЗаписи.НалоговыйОрган);
			Справочники.Организации.УстановитьПризнакОбособленногоПодразделения(ВыборкаПодразделения.СтруктурнаяЕдиница, ЯвляетсяОбособленным, ИсторияРегистрацийВНалоговомОргане);
...
Процедура ЗаписатьНалоговыйОрганДляОрганизации()
	...
	|ГДЕ
	|	История.Период < &Период
	|	И История.СтруктурнаяЕдиница = &Организация
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ПодразделенияОрганизаций.РегистрацияВНалоговомОргане = &РегистрацияВНалоговомОргане");
	
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("РегистрацияВНалоговомОргане", ОсновнаяРегистрация);
	Запрос.УстановитьПараметр("Период", НачалоМесяца(ТекущаяДатаСеанса()));
	
	Результат = Запрос.ВыполнитьПакет();
	
	История = Результат[0].Выгрузить();
	...
	Если История.Количество() > 0 Тогда
		НаборЗаписей.Загрузить(История);
		НаборЗаписей.Записать();
	КонецЕсли;
	
	ОбособленныеПодразделения = Результат[1].Выгрузить();
	
	Для Каждого Строка Из ОбособленныеПодразделения Цикл
		Справочники.Организации.УстановитьПризнакОбособленногоПодразделения(
			Строка.Подразделение, Ложь);
	КонецЦикла;
