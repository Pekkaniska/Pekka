...
Процедура УправлениеДоступностьюДополнительныеСведения()
	...
	Элементы.ФактическийАдресЧастноеЛицо.АвтоОтметкаНезаполненного = УказыватьЮридическиеРеквизиты Или ФактАдресКонтрагентаОбязателен;
...
Функция ДополнительныеСведенияЗаполненыНеправильно()
	...
	Иначе
		...
		Если УказыватьЮридическиеРеквизиты
			И (ЮрАдресКонтрагентаОбязателен
			И Не ЗначениеЗаполнено(ЮридическийАдрес))
			Или (ФактАдресКонтрагентаОбязателен
			И Не ЗначениеЗаполнено(ФактическийАдрес)) Тогда
...
Процедура ЗаписатьПартнераИСвязанныеДанные(СписокСозданных)
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		Если ЗначенияПолейВСписокЗначений(ЭтаФорма[ИмяРеквизитаТелефонЗначенияПолей]).Количество()> 0 Тогда
			...
			ДозаполнитьСтрокуТелефона(НоваяСтрокаКИ, ЭтаФорма[ИмяРеквизитаТелефонЗначенияПолей]);
		КонецЕсли;
		...
		Если ЭтоКомпания = 0 И НЕ ПустаяСтрока(ТелефонПартнера) Тогда
			...
			Иначе
				...
				Если ИНН = ИННИсточникДанныхЗаполнения Тогда
					...
					ДозаполнитьСтрокуТелефона(НоваяСтрокаКИ, ТелефонКонтрагентаЗначенияПолей);
				КонецЕсли;
