...
Процедура НастроитьФорму()
	...
	Элементы.НадписьДляПродукцииПодконтрольнойВЕТИСОдинСкладБезОрдерности.Видимость = Объект.ПодконтрольнаяПродукцияВЕТИС
																			И Не ИспользоватьНесколькоСкладов
																			И Не ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач;
	Элементы.НадписьДляПродукцииПодконтрольнойВЕТИСНетОрдерныхСкладов.Видимость = Объект.ПодконтрольнаяПродукцияВЕТИС
			 																И ИспользоватьНесколькоСкладов
																			И Не ИспользоватьОрдерныеСклады;
	Элементы.НадписьДляПродукцииПодконтрольнойВЕТИСЕстьОрдерныеСклады.Видимость = Объект.ПодконтрольнаяПродукцияВЕТИС
...
Процедура УстановитьНастройкуИспользованияСерий()
	...
	Если Объект.ИспользоватьСерии Тогда
		...
		Если Объект.ПодконтрольнаяПродукцияВЕТИС Тогда
			Объект.НастройкаИспользованияСерий = Перечисления.НастройкиИспользованияСерийНоменклатуры.ПартияТоваров;
		Иначе
...
Процедура УдалитьНеКорректныеПолитики()
	...
	Если ЗначениеЗаполнено(Объект.ПолитикаУчетаСерий)
		...
		Если Не ОбщегоНазначенияУТ.СсылкаСоотвествуетОтбору(Объект.ПолитикаУчетаСерий, ПараметрыОтбораПолитики.Отбор) Тогда
	...
	Для Каждого СтрТабл Из Объект.ПолитикиУчетаСерий Цикл
		...
		Если СтрТабл.ПолитикаУчетаСерий <> Справочники.ПолитикиУчетаСерий.СерииНеИспользуются Тогда
			...
			Если Не ОбщегоНазначенияУТ.СсылкаСоотвествуетОтбору(СтрТабл.ПолитикаУчетаСерий, ПараметрыОтбораПолитики.Отбор) Тогда
