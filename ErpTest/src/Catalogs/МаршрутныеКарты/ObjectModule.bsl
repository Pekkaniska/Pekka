...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Статус = Перечисления.СтатусыМаршрутныхКарт.Действует Тогда
		...
		Если ИспользоватьОперации Тогда
			...
			ПроверитьОперацииВыполняютсяВТекущемПодразделении(Отказ);
		КонецЕсли;
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПараметризациюРесурсныхСпецификаций") ИЛИ 
		(НЕ Справочники.МаршрутныеКарты.ЕстьАвтовыборНоменклатуры(ВыходныеИзделия) 
		И НЕ Справочники.МаршрутныеКарты.ЕстьАвтовыборНоменклатуры(ВозвратныеОтходы) 
		И НЕ Справочники.МаршрутныеКарты.ЕстьАвтовыборНоменклатуры(МатериалыИУслуги)) Тогда
...
#Область Автовыбор
	...
	МассивПроверок = Новый Массив;
	Если ИспользоватьВыходныеИзделия Тогда
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "ВыходныеИзделия", НСтр("ru = 'Выходные изделия'"), "Номенклатура"));
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "ВозвратныеОтходы", НСтр("ru = 'Промежуточный и побочный выпуск'"), "Номенклатура"));
	КонецЕсли;
	Если ИспользоватьМатериалы Тогда
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "МатериалыИУслуги", НСтр("ru = 'Материалы и работы'"), "Номенклатура"));
	КонецЕсли;
...
#Область ОтборПоСвойствам
	...
	Если ИспользоватьТрудозатраты Тогда
		МассивПроверок.Добавить(Новый Структура("ИмяТЧ, Представление, Реквизит", "Трудозатраты", НСтр("ru = 'Трудозатраты'"), "ВидРабот"));
	КонецЕсли;
...
Процедура ПроверитьОперацииВыполняютсяВТекущемПодразделении(Отказ)
	
	Если Ссылка.Пустая() ИЛИ НЕ ЗначениеЗаполнено(Подразделение) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	ТехнологическиеОперации.Владелец = &Ссылка
		|	И ТехнологическиеОперации.ПометкаУдаления = ЛОЖЬ
		|	И (ЕСТЬNULL(ВидыРабочихЦентров.Подразделение, &Подразделение) <> &Подразделение
		|			ИЛИ ЕСТЬNULL(ВладельцыРабочихЦентров.Подразделение, &Подразделение) <> &Подразделение)
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ШаблонТекстаСообщения = НСтр("ru = 'Рабочий центр операции ""%1"" не принадлежит подразделению ""%2""'");
		ПодразделениеПредставление = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Подразделение, "Представление");
		
		Выборка = РезультатЗапроса.Выбрать();
		
		Пока Выборка.Следующий() Цикл
			
			ТекстСообщения = СтрШаблон(ШаблонТекстаСообщения, Выборка.Представление, ПодразделениеПредставление);
