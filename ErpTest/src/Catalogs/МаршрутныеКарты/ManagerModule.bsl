...
//  ДанныеПоНоменклатуре         - Структура								 - См. ДанныеПоНоменклатуре().
// 
// Возвращаемое значение:
//  Структура - содержит данные маршрутной карты: операции, выходные изделия, материалы, трудозатраты.
//
Функция ДанныеОперацииМаршрутнойКарты(Операция, КоличествоОпераций, КоэффициентМаршрутнойКарты, ДанныеПоНоменклатуре) Экспорт
	...
	Для каждого КлючИЗначение Из ИндексыТаблиц Цикл
		...
		Если ИмяТаблицы = "ВыходныеИзделия" ИЛИ ИмяТаблицы = "ВозвратныеОтходы" ИЛИ ИмяТаблицы = "Материалы" Тогда
	...
	ОбработатьРасчетКоличестваПоФормулам(ПараметрыАвтовыбора, СтруктураДанных);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|		ПО Материалы.Ссылка = МаршрутныеКарты.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ТехнологическиеОперации КАК ТехнологическиеОперации
		|		ПО Материалы.Операция = ТехнологическиеОперации.Ссылка
		|ГДЕ
		|	МаршрутныеКарты.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыМаршрутныхКарт.Действует)
		|	И Материалы.Операция <> ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|	И Материалы.ОперацияРедактирование = ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТрудозатратыМК.Ссылка
		|ИЗ
		|	Справочник.МаршрутныеКарты.Трудозатраты КАК ТрудозатратыМК
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МаршрутныеКарты КАК МаршрутныеКарты
		|		ПО ТрудозатратыМК.Ссылка = МаршрутныеКарты.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ТехнологическиеОперации КАК ТехнологическиеОперации
		|		ПО ТрудозатратыМК.Операция = ТехнологическиеОперации.Ссылка
		|ГДЕ
		|	МаршрутныеКарты.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыМаршрутныхКарт.Действует)
		|	И ТрудозатратыМК.Операция <> ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|	И ТрудозатратыМК.ОперацияРедактирование = ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|	И ТехнологическиеОперации.НомерОперации <> 1
		|
		|ОБЪЕДИНИТЬ
		|
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МаршрутныеКартыВыходныеИзделия.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.МаршрутныеКарты.ВыходныеИзделия КАК МаршрутныеКартыВыходныеИзделия
		|ГДЕ
		|	(МаршрутныеКартыВыходныеИзделия.КлючСвязи = &ПустойКлючСвязи
		|			ИЛИ МаршрутныеКартыВыходныеИзделия.СпособАвтовыбораНоменклатуры = ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораНоменклатуры.ПустаяСсылка)
		|			ИЛИ МаршрутныеКартыВыходныеИзделия.СпособАвтовыбораХарактеристики = ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораХарактеристики.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МаршрутныеКартыВозвратныеОтходы.Ссылка
		|ИЗ
		|	Справочник.МаршрутныеКарты.ВозвратныеОтходы КАК МаршрутныеКартыВозвратныеОтходы
		|ГДЕ
		|	(МаршрутныеКартыВозвратныеОтходы.КлючСвязи = &ПустойКлючСвязи
		|			ИЛИ МаршрутныеКартыВозвратныеОтходы.СпособАвтовыбораНоменклатуры = ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораНоменклатуры.ПустаяСсылка)
		|			ИЛИ МаршрутныеКартыВозвратныеОтходы.СпособАвтовыбораХарактеристики = ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораХарактеристики.ПустаяСсылка))
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МаршрутныеКартыМатериалыИУслуги.Ссылка
		|ИЗ
		|	Справочник.МаршрутныеКарты.МатериалыИУслуги КАК МаршрутныеКартыМатериалыИУслуги
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МаршрутныеКарты.ОтборПоСвойствам КАК МаршрутныеКартыОтборПоСвойствам
		|		ПО МаршрутныеКартыМатериалыИУслуги.Ссылка = МаршрутныеКартыОтборПоСвойствам.Ссылка
		|			И МаршрутныеКартыМатериалыИУслуги.УдалитьОтборСвойство = МаршрутныеКартыОтборПоСвойствам.Свойство
		|			И МаршрутныеКартыМатериалыИУслуги.КлючСвязи = МаршрутныеКартыОтборПоСвойствам.КлючСвязи
		|ГДЕ
		|	МаршрутныеКартыМатериалыИУслуги.УдалитьОтборСвойство <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПустаяСсылка)
		|	И МаршрутныеКартыОтборПоСвойствам.Свойство ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.МаршрутныеКарты.ОтборПоСвойствам КАК МаршрутныеКартыОтборПоСвойствам
		|		ПО МаршрутныеКартыТрудозатраты.Ссылка = МаршрутныеКартыОтборПоСвойствам.Ссылка
		|			И МаршрутныеКартыТрудозатраты.УдалитьОтборСвойство = МаршрутныеКартыОтборПоСвойствам.Свойство
		|			И МаршрутныеКартыТрудозатраты.КлючСвязи = МаршрутныеКартыОтборПоСвойствам.КлючСвязи
		|ГДЕ
		|	(МаршрутныеКартыТрудозатраты.КлючСвязи = &ПустойКлючСвязи
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МаршрутныеКартыСоответствиеСвойств.Ссылка
		|ИЗ
		|	Справочник.МаршрутныеКарты.СоответствиеСвойств КАК МаршрутныеКартыСоответствиеСвойств
		|ГДЕ
		|	МаршрутныеКартыСоответствиеСвойств.СпособПодбораЗначения = ЗНАЧЕНИЕ(Перечисление.СпособыПодбораЗначенияСвойстваНоменклатуры.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	МаршрутныеКартыВыходныеИзделия.Ссылка
		|ИЗ
		|	Справочник.МаршрутныеКарты.ВыходныеИзделия КАК МаршрутныеКартыВыходныеИзделия
		|ГДЕ
		|	МаршрутныеКартыВыходныеИзделия.Номенклатура.ИспользованиеХарактеристик <> ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать)
		|	И МаршрутныеКартыВыходныеИзделия.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|	И МаршрутныеКартыВыходныеИзделия.СпособАвтовыбораХарактеристики В (ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораХарактеристики.УказываетсяВНСИ), ЗНАЧЕНИЕ(Перечисление.СпособыАвтовыбораХарактеристики.ПустаяСсылка))
		|");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Запрос = Новый Запрос(ТекстЗапросаПерваяОперация);
			Запрос.УстановитьПараметр("Владелец", Выборка.Ссылка);
			ВыборкаПерваяОперация = Запрос.Выполнить().Выбрать();
			Если ВыборкаПерваяОперация.Следующий() Тогда
				ПерваяОперация = ВыборкаПерваяОперация.Ссылка;
			Иначе
				ПерваяОперация = Справочники.ТехнологическиеОперации.ПустаяСсылка();
			КонецЕсли;
			
			ОбъектИзменен = Ложь;
			
			Если СправочникОбъект.Статус = Перечисления.СтатусыМаршрутныхКарт.Действует Тогда
				...
				Для каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
					Для каждого Строка Из СправочникОбъект[ТабличнаяЧасть] Цикл
						...
						Если Строка.Операция.Пустая() Тогда
							
							Если НЕ Строка.ОперацияРедактирование.Пустая() Тогда
								
								Строка.Операция = Строка.ОперацияРедактирование;
								ОбъектИзменен = Истина;
								
							ИначеЕсли НЕ ПерваяОперация.Пустая() Тогда
						...
						Если НЕ Строка.Операция.Пустая()
							И Строка.Операция <> ПерваяОперация
							И Строка.ОперацияРедактирование.Пустая() Тогда
							
							Строка.ОперацияРедактирование = Строка.Операция;
							ОбъектИзменен = Истина;
			...
			Для каждого ТабличнаяЧасть Из СтрРазделить("ВыходныеИзделия,ВозвратныеОтходы,МатериалыИУслуги,Трудозатраты",",") Цикл
				Для каждого Строка Из СправочникОбъект[ТабличнаяЧасть] Цикл
					...
					Если (ТабличнаяЧасть = "МатериалыИУслуги" ИЛИ ТабличнаяЧасть = "Трудозатраты")
						...
						И СправочникОбъект.ОтборПоСвойствам.Найти(Строка.КлючСвязи, "КлючСвязи") = Неопределено Тогда
						
						НоваяСтрока = СправочникОбъект.ОтборПоСвойствам.Добавить();
			...
			НайденныеСтроки = СправочникОбъект.СоответствиеСвойств.НайтиСтроки(
				Новый Структура("СпособПодбораЗначения", Перечисления.СпособыПодбораЗначенияСвойстваНоменклатуры.ПустаяСсылка()));
				
			Для каждого Строка Из НайденныеСтроки Цикл
				
				Если СправочникОбъект.МатериалыИУслуги.Найти(Строка.КлючСвязи, "КлючСвязи") = Неопределено Тогда
					СправочникОбъект.СоответствиеСвойств.Удалить(Строка);
			...
			Для каждого Строка Из СправочникОбъект["ВыходныеИзделия"] Цикл
				
				Если Строка.Номенклатура.Пустая() Тогда
					Продолжить;
				КонецЕсли;
				
				ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Строка.Номенклатура, "ВидНоменклатуры,ИспользованиеХарактеристик");
				
				Если ЗначенияРеквизитов.ИспользованиеХарактеристик <> Перечисления.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать
					И Строка.Характеристика.Пустая()
					И (Строка.СпособАвтовыбораХарактеристики.Пустая() 
						ИЛИ Строка.СпособАвтовыбораХарактеристики = Перечисления.СпособыАвтовыбораХарактеристики.УказываетсяВНСИ) Тогда
					
					Если ЗначенияРеквизитов.ВидНоменклатуры = СправочникОбъект.ДляВидаИзделий Тогда
						Строка.СпособАвтовыбораХарактеристики = Перечисления.СпособыАвтовыбораХарактеристики.ПодбираетсяПоСвойствамПродукции;
					Иначе
						Строка.СпособАвтовыбораХарактеристики = Перечисления.СпособыАвтовыбораХарактеристики.УточняетсяПриПроизводстве;
					КонецЕсли;
...
Функция ДанныеДляРасчетаПоФормулам(НаборДанных, ПараметрыАвтовыбора, КешированныеЗначения = Неопределено)
...
#Область ДанныеМаршрутнойКарты
	...
	Если ПереченьДанных.ВГраница() <> -1 Тогда
		...
		ДанныеМаршрутнойКарты = ДанныеМаршрутнойКарты(
									ПараметрыАвтовыбора.Спецификация,
									ПараметрыАвтовыбора.Количество,
									ПараметрыАвтовыбора.Номенклатура,
									ПараметрыАвтовыбора.Характеристика,
									СписокДанных);
...
#Область ИсточникиДанных
	...
	ИсточникиДанных.ПараметрыАвтовыбора = ПараметрыАвтовыбора;
	
	Для каждого КлючИЗначение Из СтруктураДанных Цикл
		Если ЗначениеЗаполнено(КлючИЗначение.Значение) Тогда
			Для каждого Строка Из СтруктураДанных[КлючИЗначение.Ключ] Цикл
				ЗаполнитьЗначенияСвойств(ИсточникиДанных[КлючИЗначение.Ключ].Добавить(), Строка);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
#КонецОбласти
...
Функция СтруктураИсточниковДляРасчетаПоФормулам() Экспорт
	...
	ИсточникиДанных.Вставить("ПараметрыАвтовыбора");
...
Функция ОписаниеИсточниковДанныхОперандов(Объект = Неопределено) Экспорт
	...
	// [ОсновноеИзделие]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
		НСтр("ru = 'ОсновноеИзделие'"),
		НСтр("ru = 'Основное изделие'"));
	...
	Если Объект = Неопределено
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			НСтр("ru = 'ВыходныеИзделия'"), 
			НСтр("ru = 'Выходные изделия'"));
	...
	Если Объект = Неопределено
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			НСтр("ru = 'Материал'"), 
			НСтр("ru = 'Материал (текущая строка)'"));
...
Функция ИсточникДанныхДляРасчетаПоФормулам(ПутьКДанным, ИсточникиДанных) Экспорт
	...
	ИначеЕсли ПутьКДанным.Ключ = "ОсновноеИзделие" Тогда
		...
		ЗаполнитьЗначенияСвойств(Источник, ИсточникиДанных.ПараметрыАвтовыбора);
	...
	Иначе
		...
		ИначеЕсли ПутьКДанным.Ключ = "Распоряжение" Тогда
			...
			ЗаполнитьЗначенияСвойств(Структура, ИсточникиДанных.ПараметрыАвтовыбора);
...
Процедура ОбработатьРасчетКоличестваПоФормулам(ПараметрыАвтовыбора, ДанныеМаршрутнойКарты, ИспользоватьДанныеМаршрутнойКарты = Истина)
	...
	Если ИспользоватьДанныеМаршрутнойКарты Тогда
		ДанныеДляРасчета = ДанныеДляРасчетаПоФормулам(НаборДанных, ПараметрыАвтовыбора, ДанныеМаршрутнойКарты);
	Иначе
		ДанныеДляРасчета = ДанныеДляРасчетаПоФормулам(НаборДанных, ПараметрыАвтовыбора);
	КонецЕсли;
...
Функция ПолучитьПараметрыАвтовыбора(МаршрутнаяКарта, КоличествоРезультата, ДанныеПоНоменклатуре)
...
Процедура ДобавитьДанныеМаршрутнойКартыРекурсивно(МаршрутнаяКарта, КоличествоРезультата, Номенклатура, Характеристика, СтруктураСвязей, СтруктураДанных)
	...
	ПараметрыАвтовыбора = ПолучитьПараметрыАвтовыбора(МаршрутнаяКарта, КоличествоРезультата, ДанныеПоНоменклатуре);
	...
	ОбработатьРасчетКоличестваПоФормулам(ПараметрыАвтовыбора, ДанныеМаршрутнойКарты);
...
Процедура ДобавитьДанныеМаршрутнойКартыДляПечатиРекурсивно(МаршрутнаяКарта, КоличествоРезультата, Номенклатура, Характеристика, СтруктураСвязей, СтруктураДанных)
	...
	ПараметрыАвтовыбора.Количество = КоличествоРезультата;
	ОбработатьРасчетКоличестваПоФормулам(ПараметрыАвтовыбора, ДанныеМаршрутнойКарты, Ложь);
...
Функция ТекстЗапросаОтборПоСвойствам()
	...
	|		ПО МаршрутныеКартыОтборПоСвойствам.Свойство = СвойстваНоменклатуры.Свойство
	...
	|		ПО МаршрутныеКартыОтборПоСвойствам.Свойство = СвойстваХарактеристики.Свойство
...
Процедура РаспаковатьСоставНаборов(ДанныеМаршрутнойКарты, ИмяТЧ)
	...
	Для каждого СтрокаНабораДанных Из НаборДанных Цикл
		...
		Если СтрокаНабораДанных.ТипНоменклатуры = ТипНоменклатурыНабор Тогда
			...
			Если ПараметрыВариантаКомплектации = Неопределено ИЛИ НЕ ПараметрыВариантаКомплектации.Комплектующие.Количество() Тогда
				ЗаполнитьЗначенияСвойств(НаборДанных.Вставить(ИндексСтроки), СтрокаНабораДанных, , "Номенклатура, Характеристика");
			Иначе
				Для каждого СтрокаКомплектующие Из ПараметрыВариантаКомплектации.Комплектующие Цикл
					...
					НоваяСтрока.КоличествоУпаковок  = СтрокаКомплектующие.КоличествоУпаковок *
						СтрокаНабораДанных.КоличествоУпаковок;
