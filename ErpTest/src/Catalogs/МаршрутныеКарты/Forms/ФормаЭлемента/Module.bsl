...
Процедура ВыходныеИзделияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ПриВводеНовойСтрокиСАвтовыбором(Элемент.Имя, Копирование);
...
Процедура ВыходныеИзделия_НастроитьАвтовыборНоменклатуры(Команда)
	...
	Если Объект.ДляВидаИзделий.Пустая() Тогда
		ТекстСообщения = НСтр("ru = 'Для настройки автовыбора номенклатуры необходимо указать вид изделий.'");
...
Процедура КомандаНастроитьАвтовыборМатериалов(Команда)
	...
	Если Объект.ДляВидаИзделий.Пустая() Тогда
		ТекстСообщения = НСтр("ru = 'Для настройки автовыбора материала необходимо указать вид изделий.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ДляВидаИзделий", "Объект");
		Возврат;
	КонецЕсли; 
...
Процедура ПолучитьСтрокиИзБуфераОбменаНаСервере(ИмяТЧ)
	...
	Для каждого СтрокаИзБуфера Из СтрокиИзБуфера Цикл
		...
		Если ПодборТоваров Тогда
			...
			Если ИмяТЧ = "ВыходныеИзделия" ИЛИ ИмяТЧ = "ВозвратныеОтходы" ИЛИ ИмяТЧ = "МатериалыИУслуги" Тогда
				ТекущаяСтрока.КлючСвязи = Новый УникальныйИдентификатор;
				УправлениеДаннымиОбИзделияхКлиентСервер.ПриВводеНовойСтрокиСАвтовыбором(ТекущаяСтрока);
			КонецЕсли;
...
Процедура ОткрытьНастройкуОтбораПоСвойствамИРасчетаПоФормулам(ТекущиеДанные, ИмяТЧ)
	...
	Если Объект.ДляВидаИзделий.Пустая() Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для настройки потребления необходимо указать вид изделий.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ДляВидаИзделий", "Объект");
...
&НаКлиенте
Процедура ПриВводеНовойСтрокиСАвтовыбором(ИмяТЧ, Копирование)
	...
	Исключение
		...
		ОтменитьТранзакцию();
 		...
 		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Заполнение маршрутной карты'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПроверитьЗаполнениеПринудительно()
	...
	Отказ = НЕ СправочникОбъект.ПроверитьЗаполнение();
...
Процедура УправлениеВидимостью(Форма, Действие = "")
	...
	Если ВсеДействия ИЛИ Действие = "РассчитыватьКоэффициент" Тогда
		...
		ОтображатьКоэффициент = НастройкиПодсистемы.ИспользуетсяПроизводство22
			И НЕ НастройкиПодсистемы.ИспользуетсяПроизводство21;
		
		Элементы.ГруппаКоэффициентПрименения.Видимость = ОтображатьКоэффициент;
		Элементы.ГруппаКоэффициентФормула.Видимость = ОтображатьКоэффициент
			И Объект.РассчитыватьКоэффициент;
		
	КонецЕсли;
	
	Если ВсеДействия Тогда
		
		Элементы.ГруппаРазбиватьМаршрутныеЛисты.Видимость = НастройкиПодсистемы.ИспользуетсяПроизводство21;
		Элементы.ГруппаПланированиеРаботыКлючевыхВидовРабочихЦентров.Видимость =
			НастройкиПодсистемы.ИспользуетсяПроизводство21;
			
		Элементы.ВыходныеИзделияДоляСтоимости.Видимость = НастройкиПодсистемы.ИспользуетсяПроизводство21;
...
Процедура ПриИзмененииВидаИзделийВызовСервера()
	...
	УправлениеДаннымиОбИзделиях.ПроверитьСвойстваАвтовыбораИОтбораПоСвойствам(
		Объект,
		Объект.СоответствиеСвойств,
		Объект.ОтборПоСвойствам,
		Объект.ДляВидаИзделий);
