	...
	Если ПлатежиПо275ФЗ Тогда
		...
		Если ДоговорСУчастникомГОЗ Тогда
			Если ЗначениеЗаполнено(БанковскийСчет) И ЗначениеЗаполнено(ГосударственныйКонтракт)
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ШаблонСообщения, ЭтотОбъект, "БанковскийСчет", , Отказ);
			КонецЕсли;
			Если ЗначениеЗаполнено(БанковскийСчетПолучателя) И ЗначениеЗаполнено(ГосударственныйКонтракт)
				И ГосударственныйКонтракт <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчетПолучателя, "ГосударственныйКонтракт") Тогда
				
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ШаблонСообщения, ЭтотОбъект, "БанковскийСчетПолучателя", , Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
