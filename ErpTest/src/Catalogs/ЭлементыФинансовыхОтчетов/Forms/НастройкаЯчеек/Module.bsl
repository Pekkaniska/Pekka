...
Процедура Вставить(Команда)
	...
	Если Не ОбластьВЗонеРедактирования(ЭтаФорма, ТекущаяОбласть) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно заменить выбранную область'"));
		Возврат;
	КонецЕсли;
	...
	Если Отказ Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно заменить выделенную область'"));
		Возврат;
...
Процедура КопироватьЗначения(Вырезать = Ложь)
	...
	Если Не ОбластьВЗонеРедактирования(ЭтаФорма, ТекущаяОбласть) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Невозможно скопировать выбранную область'"));
		Возврат;
	КонецЕсли;
...
Процедура ВставитьНаСервере(Массив, ВысотаОбласти, ШиринаОбласти)
	...
	ТекущаяТаблицаЭлементов = ПолучитьИзВременногоХранилища(ЭлементыОтчета());
...
Процедура ПредставлениеОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЭтоПростаяТаблица Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Выбор значений ячеек доступен только в сложной таблице'"));
		Возврат;
	КонецЕсли;
...
Процедура РедактироватьЭлементОтчета(Область)
	...
	Если Расшифровка.ВидЭлемента = ПредопределенноеЗначение("Перечисление.ВидыЭлементовФинансовогоОтчета.ФормулаПоГруппе")
		...
		ПоказатьПредупреждение( , НСтр("ru = 'Настройка свойств элемента доступна в виде бюджета!'"));
		Возврат;
	КонецЕсли;
