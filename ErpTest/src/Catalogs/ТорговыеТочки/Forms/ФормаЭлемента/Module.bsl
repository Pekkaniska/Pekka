...
Процедура УстановитьВидимостьКодаОКТМО(ЭтотОбъект)
	...
	ЭтотОбъект.Элементы.КодПоОКТМО.Видимость = 
		...
		И НЕ ЗначениеЗаполнено(ЭтотОбъект.ПараметрыТорговыхТочек.КодПоОКТМО)
...
Процедура УстановитьЦветГиперссылкиАдрес()
	...
	Если АдресЗаполнен() Тогда
		ЦветТекста = ЦветаСтиля.ГиперссылкаТабличнойЧасти;
	Иначе
...
Процедура СохранитьОКТМО()
	...
	НовыйАдрес = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов[0].ЗначенияПолей;
	
	КодыАдреса = АдресныйКлассификатор.КодыАдреса(НовыйАдрес);
	Если ЗначениеЗаполнено(КодыАдреса) Тогда
...
Процедура РазместитьКонтактнуюИнформацию()
	...
	ПараметрыКИ = УправлениеКонтактнойИнформацией.ПараметрыКонтактнойИнформации();
	ПараметрыКИ.ПоложениеЗаголовкаКИ = ПоложениеЗаголовкаЭлементаФормы.Лево;
	ПараметрыКИ.ИсключаемыеВиды      = ИсключаемыеВиды;
	
	УправлениеКонтактнойИнформацией.ПриСозданииНаСервере(ЭтотОбъект, Объект, ПараметрыКИ);
