...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Попытка
		...
		НовыйСпособ.Наименование = НСтр("ru='Начислено сдельно (создан при обновлении ИБ на 2.4.2)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru = 'Не удалось создать способ отражения для сдельных работ по причине: %Причина%'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
