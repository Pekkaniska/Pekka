...
Функция ТекстЗапросаНеобеспеченныхНазначенийОбеспечено(ПараметрыФормированияЗапроса) Экспорт
	...
	|	ИЛИ НЕ Назначения.ТипНазначения В (ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21),
...
Функция ТекстЗапросаНазначенийРасширенный() Экспорт
	...
	ТекстЗапроса =
		...
		|			ИЛИ (
		|				&ТолькоДавальцы2_2
		|				И Набор.Назначение.Партнер <> ЗНАЧЕНИЕ(Справочник.Партнеры.ПустаяСсылка)
		|				И Набор.Назначение.ТипНазначения <> ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Собственное))
		|			)
		//-- НЕ УТКА
		|ГДЕ
...
Процедура ДобавитьУсловиеИсключенияПартнера(Условия, Источник) Экспорт
	...
	//++ НЕ УТКА
	ЕстьНазначенияСЗаполненнымПартнером = ЕстьНазначенияСЗаполненнымПартнером Или ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья");
	//-- НЕ УТКА
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//++ НЕ УТКА
	|		ИЛИ Заказ <> НЕОПРЕДЕЛЕНО И Заказ Ссылка Документ.ЗаказДавальца
	//-- НЕ УТКА
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				Если Объект.ТипНазначения = Перечисления.ТипыНазначений.ПустаяСсылка() Тогда
					...
					//++ НЕ УТКА
					Если Объект.Партнер.Пустая() И ТипЗнч(Объект.Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
						Объект.ТипНазначения = Перечисления.ТипыНазначений.Давальческое21;
					ИначеЕсли НЕ Объект.Партнер.Пустая() И ТипЗнч(Объект.Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
						Объект.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеПродукция22;
					ИначеЕсли НЕ Объект.Партнер.Пустая() И ТипЗнч(Объект.Заказ) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
						Объект.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22;
					ИначеЕсли НЕ Объект.Партнер.Пустая() И НЕ ЗначениеЗаполнено(Объект.Заказ) Тогда
						Объект.ТипНазначения = Перечисления.ТипыНазначений.ДавальческоеМатериалы22;
						Объект.Наименование = Справочники.Назначения.ПредставлениеНазначения(Объект, Неопределено);
					КонецЕсли;
					//-- НЕ УТКА
				...
				Если Объект.КонтролироватьТолькоНаличие <> 
						...
						//++ НЕ УТКА
						ИЛИ ЗначениеЗаполнено(Объект.Заказ) И ТипЗнч(Объект.Заказ) = Тип("ДокументСсылка.ЗаказДавальца")
						//-- НЕ УТКА
				...
				// Установка признака ДвиженияПоСкладскимРегистрам для неиспользуемых назначений
				ЗапросПроверки.УстановитьПараметр("Назначение", Объект.Ссылка);
				Если Не Объект.ДвиженияПоСкладскимРегистрам И ЗапросПроверки.Выполнить().Пустой() Тогда
					
					Объект.ДвиженияПоСкладскимРегистрам = Истина;
					ОбъектИзменен = Истина;
