...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	СобытияФормВЕТИСКлиентПереопределяемый.ОбработкаВыбораКонтрагента(
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	РегистрыСведений.НастройкиУполномоченногоГашенияВЕТИС.ВыполнитьЗаписьПоГрузоотправителю(
		Объект.Ссылка, УполномоченноеГашение.Выгрузить().ВыгрузитьКолонку("Грузополучатель"));
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ХозяйствующиеСубъектыВЕТИС", Объект.Ссылка);
...
Процедура КонтрагентСоздание(Элемент, СтандартнаяОбработка)
	...
	Реквизиты.Вставить("Наименование",            Объект.НаименованиеПолное);
	Реквизиты.Вставить("СокращенноеНаименование", Объект.Наименование);
	Реквизиты.Вставить("ЮридическийАдрес",        Объект.АдресПредставление);
...
Процедура Подключаемый_ВыполнитьПереопределяемуюКоманду(Команда)
	...
	СобытияФормВЕТИСКлиентПереопределяемый.ВыполнитьПереопределяемуюКоманду(ЭтаФорма, Команда);
...
Процедура СинхронизироватьСписокПредприятий(Команда)
	...
	Если Модифицированность Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения("ВопросПриСинхронизацииЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещенияВопрос, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		
	Иначе
		
		ВыполнитьСинхронизациюПредприятий();
...
Процедура СвязатьСПредприятием(Команда)
	...
	ОткрытьФорму("Справочник.ХозяйствующиеСубъектыВЕТИС.Форма.ИзменениеСвязиПредприятияХозяйствующегоСубъекта", ПараметрыФормы, ЭтотОбъект,
...
Процедура СоответствуетОрганизацииПриИзмененииСервер()
	...
	Если Объект.СоответствуетОрганизации Тогда
		...
		Если ИнтеграцияВЕТИСПереопределяемый.ИспользуетсяПереработкаНаСтороне() Тогда
	...
	Элементы.ПредприятияПроизводственныйОбъект.Видимость = (СобственныйПроизводственныйОбъектЗначениеПоУмолчанию <> Неопределено);
...
Процедура УправлениеЭлементамиФормы(Форма)
	...
	Элементы.ПредприятияСинхронизировать.Доступность = Форма.ЕстьПравоИзменения;
	...
	Если Объект.СоответствуетОрганизации Тогда
		Элементы.ПредприятияСинхронизировать.Заголовок = НСтр("ru = 'Синхронизировать'");
	Иначе
		Элементы.ПредприятияСинхронизировать.Заголовок = НСтр("ru = 'Актуализировать'");
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ОбновитьЗначениеЭтоАдминистраторХозяйствующегоСубъекта(Форма)
...
Процедура ЗаполнитьДанныеСопоставления()
	...
	Если ЗначениеЗаполнено(Объект.Контрагент)
		...
		ДанныеСопоставления = ИнтеграцияВЕТИСПереопределяемый.ИННКППСопоставленногоХозяйствующегоСубъекта(Объект.Контрагент);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ЕстьПравоСозданияКонтрагента = ИнтеграцияИСПереопределяемый.ЕстьПравоСозданияКонтрагента();
...
&НаСервере
Процедура ВыполнитьСинхронизациюПредприятий()
	...
	Попытка
		...
		Объект.Предприятия.Загрузить(Запрос.Выполнить().Выгрузить());
...
Процедура ВыполнитьПодборПредприятий()
	...
	ОткрытьФорму("Справочник.ХозяйствующиеСубъектыВЕТИС.Форма.ДанныеКлассификатора", ПараметрыФормы, ЭтотОбъект,,,,
	             ОповещениеОкончанияПодбора , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Функция СопоставитьПоИННКППСервер()
	...
	СопоставлениеНайдено = Ложь;
	
	Если Объект.СоответствуетОрганизации Тогда
		
		НайденноеСоответствие = ИнтеграцияВЕТИСПереопределяемый.ОрганизацияПоИННКПП(Объект.ИНН, Объект.КПП);
		
	Иначе
		
		НайденноеСоответствие = ИнтеграцияВЕТИСПереопределяемый.КонтрагентПоИННКПП(Объект.ИНН, Объект.КПП);
...
Процедура СоздатьНовогоКонтрагента(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ДанныеКонтрагента.Вставить("Наименование",            Объект.Наименование);
		ДанныеКонтрагента.Вставить("СокращенноеНаименование", Объект.НаименованиеПолное);
		ДанныеКонтрагента.Вставить("ИНН",                     Объект.ИНН);
		ДанныеКонтрагента.Вставить("КПП",                     Объект.КПП);
		ДанныеКонтрагента.Вставить("ЮридическийАдрес",        Объект.АдресПредставление);
