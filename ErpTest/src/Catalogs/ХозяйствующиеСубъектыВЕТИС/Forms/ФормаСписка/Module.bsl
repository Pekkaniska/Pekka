...
Процедура ОбработатьПереданныеПараметры(Отказ)
	...
	ИначеЕсли ЗначениеЗаполнено(Контрагент)
		И Метаданные.ОпределяемыеТипы.ОрганизацияКонтрагентВЕТИС.Тип.Типы().Найти(ТипЗнч(Параметры.Контрагент)) <> Неопределено Тогда
		
		ДанныеКонтрагента = ИнтеграцияВЕТИСПереопределяемый.ДанныеКонтрагентаДляПоискаХозяйствующегоСубъекта(Параметры.Контрагент);
...
Процедура ОбработатьНайденныеХозяйствующиеСубъекты(НомерСтраницы)
	...
	ИначеЕсли ПараметрыПоиска.Свойство("GLN") Тогда
		ПоискПоGLNОтХозяйствующегоСубъекта = Истина;
