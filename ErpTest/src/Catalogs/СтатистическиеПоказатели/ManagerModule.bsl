...
Функция ДанныеДляЗаполненияXML(ПроверятьДатуИзменения = Истина) Экспорт
	...
	КонтейнерОбъектов.ВерсияФормата = "3.0.1.0";
	...
	КонтейнерФорм.ВерсияФормата = "3.0.1.0";
...
//
// Параметры:
//	Отбор - ОтборКомпоновкиДанных - Отбор СКД.
//
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	ЗаполнитьПоставляемымиПравилами();
...
Функция НовыйПоставляемаяНастройка(Настройка)
	...
	//
	// Такое преобразование не обеспечивается механизмами компоновки данных, поэтому конвертор реализован в конфигурации - 
...
Функция ИмяСобытияВЖурналеРегистрации()
	...
	Возврат НСтр("ru = 'Правила заполнения форм статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗагрузитьПравилаЗаполненияФормСтатистики(ПравилаЗаполнения)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Обновление поставляемых данных статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
