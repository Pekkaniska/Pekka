...
Функция НайтиШтрихкодыУпаковокПоСпискуШтрихкодов(ЗначенияШтрихкодов, ОрганизацияЕГАИС, ТипыУпаковок = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ
	|	АкцизныеМаркиЕГАИС" +?(ЗначениеЗаполнено(ОрганизацияЕГАИС), "поОрганизации", "") + "
	|ИЗ
	|	РегистрСведений.АкцизныеМаркиЕГАИС КАК АкцизныеМаркиЕГАИС
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
	...
	Иначе
		...
		|ВЫБРАТЬ
		|	АкцизныеМаркиЕГАИС.АкцизнаяМарка    КАК АкцизнаяМарка,
		|	МАКСИМУМ(АкцизныеМаркиЕГАИС.Статус) КАК Статус
		|ПОМЕСТИТЬ
		|	СтатусыАкцизныхМарокЕГАИС
		|ИЗ
		|	АкцизныеМаркиЕГАИС КАК АкцизныеМаркиЕГАИС
		|СГРУППИРОВАТЬ ПО
		...
		|	АкцизныеМаркиЕГАИС.АкцизнаяМарка              КАК АкцизнаяМарка,
		|	АкцизныеМаркиЕГАИС.Статус                     КАК Статус,
		|	МАКСИМУМ(АкцизныеМаркиЕГАИС.ОрганизацияЕГАИС) КАК ОрганизацияЕГАИС
		|ПОМЕСТИТЬ
		|	ОрганизацииСтатусыАкцизныхМарокЕГАИС
		|ИЗ
		|	АкцизныеМаркиЕГАИС КАК АкцизныеМаркиЕГАИС
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ СтатусыАкцизныхМарокЕГАИС КАК СтатусыАкцизныхМарокЕГАИС
		|	ПО АкцизныеМаркиЕГАИС.АкцизнаяМарка = СтатусыАкцизныхМарокЕГАИС.АкцизнаяМарка
		|		И АкцизныеМаркиЕГАИС.Статус = СтатусыАкцизныхМарокЕГАИС.Статус
		|СГРУППИРОВАТЬ ПО
		|	АкцизныеМаркиЕГАИС.АкцизнаяМарка,
		...
		|	АкцизныеМаркиЕГАИС.АлкогольнаяПродукция КАК АлкогольнаяПродукция
		|ПОМЕСТИТЬ
		|	АкцизныеМаркиЕГАИСпоОрганизации
		|ИЗ
		|	АкцизныеМаркиЕГАИС КАК АкцизныеМаркиЕГАИС
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОрганизацииСтатусыАкцизныхМарокЕГАИС КАК ОрганизацииСтатусыАкцизныхМарокЕГАИС
		|	ПО АкцизныеМаркиЕГАИС.АкцизнаяМарка = ОрганизацииСтатусыАкцизныхМарокЕГАИС.АкцизнаяМарка
		|		И АкцизныеМаркиЕГАИС.ОрганизацияЕГАИС = ОрганизацииСтатусыАкцизныхМарокЕГАИС.ОрганизацияЕГАИС
		|		И АкцизныеМаркиЕГАИС.Статус = ОрганизацииСтатусыАкцизныхМарокЕГАИС.Статус";
	КонецЕсли;
	...
	|			
	|		ЛЕВОЕ СОЕДИНЕНИЕ АкцизныеМаркиЕГАИСпоОрганизации КАК АкцизныеМаркиЕГАИС
	|		ПО АкцизныеМаркиЕГАИС.АкцизнаяМарка = ШтрихкодыУпаковокТоваров.Ссылка
	|";
	...
	Для Каждого СтрокаТЧ Из ТаблицаШтрихкодовУпаковок Цикл
		...
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаТЧ.АлкогольнаяПродукция)
			...
			И СтрокаТЧ.ТипШтрихкода = Перечисления.ТипыШтрихкодов.PDF417
			И АкцизныеМаркиВызовСервера.ЭтоШтрихкодАкцизнойМарки(СтрокаТЧ.ЗначениеШтрихкода) Тогда
			
			СтрокаТЧ.КодАлкогольнойПродукции = АкцизныеМаркиВызовСервера.КодКлассификатораНоменклатурыЕГАИС(СтрокаТЧ.ЗначениеШтрихкода, КэшКодовАлкогольнойПродукции);
			КодыАлкогольнойПродукции.Добавить(СтрокаТЧ.КодАлкогольнойПродукции);
	...
	Для Каждого СтрокаТЧ Из ТаблицаШтрихкодовУпаковок Цикл
		...
		Если Результат[ВРег(СтрокаТЧ.ЗначениеШтрихкода)] <> Неопределено Тогда
			Результат[ВРег(СтрокаТЧ.ЗначениеШтрихкода)].Добавить(ДанныеШтрихкодаУпаковки);
		Иначе
			МассивУпаковок = Новый Массив;
			МассивУпаковок.Добавить(ДанныеШтрихкодаУпаковки);
			Результат.Вставить(ВРег(СтрокаТЧ.ЗначениеШтрихкода), МассивУпаковок);
		КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(СтрокаДерева.ШтрихкодУпаковки) Тогда
				...
				Иначе
					...
					Если СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
						...
						Если СтрДлина(СтрокаДерева.ЗначениеШтрихкода) = 150 Тогда
...
Функция ВложенныеШтрихкодыУпаковок(СписокШтрихкодов, ОрганизацияЕГАИС, ИндексыМаркированныхТоваров = Неопределено) Экспорт
	
	Если ИндексыМаркированныхТоваров = Неопределено Тогда
		ИндексыМаркированныхТоваров="ШтрихкодУпаковки";
	КонецЕсли;
	
	ДеревоУпаковок = ПустоеДеревоУпаковок();
	...
	Если ТипЗнч(ИндексыМаркированныхТоваров) = Тип("Строка") Тогда
		МаркированныеТовары.Индексы.Добавить(ИндексыМаркированныхТоваров);
	ИначеЕсли ТипЗнч(ИндексыМаркированныхТоваров) = Тип("Массив") Тогда
		Для Каждого НужныйИндекс Из ИндексыМаркированныхТоваров Цикл
			МаркированныеТовары.Индексы.Добавить(НужныйИндекс);
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьСгенерироватьАкцизнуюМарку(КодАкцизнойМарки, Номенклатура, Характеристика, ЗаписьПриОбновленииИБ = ЛОЖЬ) Экспорт
	...
	Иначе
		Возврат СгенерироватьАкцизнуюМарку(КодАкцизнойМарки, Номенклатура, Характеристика, ЗаписьПриОбновленииИБ);
	КонецЕсли;
...
Функция СгенерироватьАкцизнуюМарку(КодАкцизнойМарки, Номенклатура, Характеристика, ЗаписьПриОбновленииИБ = ЛОЖЬ) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Создание новой акцизной марки'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
// вложенных уже в них штрихкодов 2ого уровня, т.к. они при записи также проверяются по вложенным упаковкам.
...
Функция ПустоеДеревоУпаковок() Экспорт
...
Функция СформироватьПечатнуюФормуШтрихкодыУпаковок(МассивОбъектов, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ Выборка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC
			...
			И НЕ Выборка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.PDF417
			И НЕ Выборка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СтрДлина(Выборка.ЗначениеШтрихкода) = 150
			И Выборка.ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix Тогда
			
			ПараметрыШтрихкодовУпаковок.СерияМарки = Сред(Выборка.ЗначениеШтрихкода, 4, 3);
			ПараметрыШтрихкодовУпаковок.НомерМарки = Сред(Выборка.ЗначениеШтрихкода, 7, 8);
...
Процедура ДобавитьШтрихкодВТабличныйДокумент(ТабличныйДокумент, ПараметрыМакетовДляПечати, ПараметрыШтрихкодовУпаковокДляПечати) Экспорт
	...
	ИначеЕсли ТипШтрихкодаУпаковки = Перечисления.ТипыШтрихкодов.Code128 Тогда
		...
		НомерФорматаCode128 = ШтрихкодыУпаковокКлиентСервер.ФорматШтрихкодаCode128(ЗначениеШтрихкода);
...
Процедура ЗаполнитьСтрокуДереваУпаковок(СтрокаДерева, Выборка, Уровень, МаркированныеТовары, ДополнительныеПараметры)
	...
	Если СтрокаДерева.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
		...
		Если Не ЗначениеЗаполнено(СтрокаДерева.АлкогольнаяПродукция)
			...
			СтрокаДерева.КодАлкогольнойПродукции = АкцизныеМаркиВызовСервера.КодКлассификатораНоменклатурыЕГАИС(
				СтрокаДерева.ЗначениеШтрихкода,
				ДополнительныеПараметры.КэшКодовАлкогольнойПродукции);
		КонецЕсли;
