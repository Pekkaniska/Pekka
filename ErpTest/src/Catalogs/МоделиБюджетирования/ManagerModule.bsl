...
//
// Параметры:
//  МодельБюджетирования  			 - Справочник.МоделиБюджетирования - Модель для которой строится расписание
...
// заполняет новый реквизит РегламентноеЗадание.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	И НЕ МоделиБюджетирования.ПометкаУдаления
	|	И МоделиБюджетирования.АвтоматическиФормироватьЗадачи
	|	И &ИспользоватьБюджетныйПроцесс
	|	И НЕ МоделиБюджетирования.ЭтоГруппа
	|	И МоделиБюджетирования.РегламентноеЗадание = &ПустойУникальныйИдентификатор");
	Запрос.УстановитьПараметр("ИспользоватьБюджетныйПроцесс", ПолучитьФункциональнуюОпцию("ИспользоватьБюджетныйПроцесс"));
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Справочник.МоделиБюджетирования";
	МетаданныеОбъекта = Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта);
	
	УстановитьПривилегированныйРежим(Истина);
	Поиск = Новый Структура("Метаданные", Метаданные.РегламентныеЗадания.ФормированиеБюджетныхЗадач);
	РеглЗадания = РегламентныеЗаданияСервер.НайтиЗадания(Поиск);
	РегЗаданиеЭталон = Неопределено;
	Для каждого РеглЗадание Из РеглЗадания Цикл
		Если РеглЗадание.Параметры.Количество() = 0 Тогда
			РегЗаданиеЭталон = РеглЗадание;
		КонецЕсли;
	КонецЦикла; 
	Если РегЗаданиеЭталон = Неопределено Тогда
		Расписание = Новый РасписаниеРегламентногоЗадания;
		Расписание.ВремяНачала = '00010101220000'; // в 10:00 вечера
		Расписание.ПериодПовтораДней = 1;
	Иначе
		Если ТипЗнч(РегЗаданиеЭталон) = Тип("РегламентноеЗадание") Тогда
			Расписание = РегЗаданиеЭталон.Расписание;
		Иначе
			Расписание = РегламентныеЗаданияСервер.РасписаниеРегламентногоЗадания(РегЗаданиеЭталон.УникальныйИдентификатор, Ложь);
		КонецЕсли;
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
	...
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуСсылокДляОбработки(
		Параметры.Очередь, "Справочник.МоделиБюджетирования", МенеджерВременныхТаблиц);
	...
	ВыборкаПоОбъектам = РезультатЗапроса.Выбрать();
	Пока ВыборкаПоОбъектам.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки = Блокировка.Добавить("Справочник.МоделиБюджетирования");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ВыборкаПоОбъектам.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			Блокировка.Заблокировать();
			
			Модель = ОбновлениеИнформационнойБазыУТ.ПроверитьПолучитьОбъект(ВыборкаПоОбъектам.Ссылка,
			                                                                ВыборкаПоОбъектам.ВерсияДанных,
			                                                                Параметры.Очередь);
			
			Если НЕ Модель = Неопределено Тогда
				Модель.Заблокировать();
				
				Модель.ОбменДанными.Загрузка = Истина;
				Модель.ДополнительныеСвойства.Вставить("Расписание", Расписание);
				БюджетированиеСервер.ПроверитьЭкземплярРегламентногоЗадания(Модель, Ложь);
				ОбновлениеИнформационнойБазы.ЗаписатьОбъект(Модель, , , );
				БюджетированиеСервер.УстановитьПараметрыЭкземплярРегламентногоЗадания(Модель, Ложь);
			КонецЕсли;
		...
		Исключение
			ОтменитьТранзакцию();
			ОбновлениеИнформационнойБазыУТ.СообщитьОНеудачнойОбработке(ИнформацияОбОшибке(), ВыборкаПоОбъектам.Ссылка);
			Продолжить;
		КонецПопытки;
	КонецЦикла;
	
	ОбработкаЗавершена = Не ОбновлениеИнформационнойБазы.ЕстьДанныеДляОбработки(Параметры.Очередь, ПолноеИмяОбъекта);
	
	УстановитьПривилегированныйРежим(Истина);
	Если ОбработкаЗавершена Тогда
		Для каждого РеглЗадание Из РеглЗадания Цикл
			Если РеглЗадание.Параметры.Количество() = 0 Тогда 
				РегламентныеЗаданияСервер.УдалитьЗадание(РеглЗадание.УникальныйИдентификатор);
			КонецЕсли;
		КонецЦикла; 
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
