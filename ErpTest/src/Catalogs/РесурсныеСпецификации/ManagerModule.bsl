...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСоздатьНаОсновании, Параметры) Экспорт
	...
	Команда = СозданиеНаОснованииПереопределяемый.ДобавитьКомандуСоздатьНаОснованииБизнесПроцессЗадание(КомандыСоздатьНаОсновании);
	Команда.ВидимостьВФормах = ВидимостьВФормах;
...
Процедура ПроверитьВложенныеСпецификации(Объект, Отказ) Экспорт
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаТЧ = СоответствиеСтрок[Выборка.Ссылка][0];
...
Функция ПроверитьЗацикливаниеСпецификаций(Спецификация, ВложенныеСпецификации = Неопределено, НачалоДуги = Неопределено, КонецДуги = Неопределено) Экспорт
	...
	|ИЗ
	|	( %1 ) КАК Таблица");
	Запрос.Текст = СтрШаблон(Запрос.Текст, ТекстЗапросаВложенныеСпецификации());
...
Процедура ДобавитьВложеннуюСпецификациюВСписок(Список, СоответствиеСтрок, ИмяТЧ, Строка, ИмяПоля = "Спецификация")
	...
	Иначе
		СоответствиеСтрок[Строка[ИмяПоля]].Добавить(ПутьКСтроке);
	КонецЕсли;
...
Функция ДанныеСпецификации(ДанныеПоНоменклатуре, ПараметрыВыборки)
	...
	Если НЕ ПараметрыВыборки.Свойство("ПереченьДанных", ПереченьДанных)
		ИЛИ ПереченьДанных = Неопределено
		Тогда
...
#Область ВтВходящиеИзделия
	Если ПереченьДанных.Найти("ВходящиеИзделия") <> Неопределено ИЛИ ПараметрыВыборки.ОбъединитьМатериалыИВходящиеИзделия Тогда
		...
		|	1              КАК НомерСтроки,
		|
		|	Таблица.НоменклатураВходящегоИзделия КАК Номенклатура,
		|	&ИсточникХарактеристика              КАК Характеристика,
		|
		|	Таблица.УпаковкаВходящегоИзделия           КАК Упаковка,
		|	Таблица.КоличествоУпаковокВходящегоИзделия КАК КоличествоУпаковок,
		|	Таблица.КоличествоВходящегоИзделия         КАК Количество,
		|
		|	ВЫБОР
		|		КОГДА Таблица.ТипПроизводственногоПроцесса = ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.Ремонт)
		...
		|
		|	ЕСТЬNULL(СсылкиНаЭтапы.Ссылка,ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)) КАК Этап,
		|	ЕСТЬNULL(СсылкиНаЭтапы.Ссылка,ЗНАЧЕНИЕ(Справочник.ЭтапыПроизводства.ПустаяСсылка)) КАК ЭтапРедактирование,
		|
		|	ЗНАЧЕНИЕ(Перечисление.СпособыПолученияМатериаловВСпецификации.Обеспечивать) КАК СпособПолученияМатериала
		|
		|ПОМЕСТИТЬ ВтВходящиеИзделия
		|ИЗ
		|	Справочник.РесурсныеСпецификации КАК Таблица
		...
		|			И Т.НомерЭтапа = 1) КАК СсылкиНаЭтапы
		|		ПО (ИСТИНА)
		|
		|ГДЕ
		|	(Таблица.Ссылка = &Спецификация
		|		И  Таблица.ТипПроизводственногоПроцесса В (ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.Ремонт),
		...
		ТекстПоляВыражениеХарактеристика = ТекстПоляВыражениеХарактеристикаОсновногоИзделия("Таблица", "НоменклатураВходящегоИзделия", "ХарактеристикаВходящегоИзделия");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИсточникХарактеристика", ТекстПоляВыражениеХарактеристика);
...
#Область ВТВремяРаботыВидовРабочихЦентров
	...
	Если ПереченьДанных.Найти("ВидыРабочихЦентров") <> Неопределено
		...
		СоздатьВТВремяРаботыВидовРабочихЦентров(МенеджерВременныхТаблиц, ДанныеПоНоменклатуре.Спецификация);
...
#Область ВТОтборПоСвойствам
	...
	|	ЕСТЬNULL(ЕСТЬNULL(СвойстваХарактеристики.Значение, СвойстваНоменклатуры.Значение), ВЫБОР
	...
	|
	|ГДЕ
	|	РесурсныеСпецификацииОтборПоСвойствам.Ссылка = &Спецификация
	|	И &ИспользоватьПараметризацию
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЭтапыПроизводстваОтборПоСвойствам.КлючСвязи,
	|	ЭтапыПроизводстваОтборПоСвойствам.Свойство,
	|	ЭтапыПроизводстваОтборПоСвойствам.Условие,
	|	ЭтапыПроизводстваОтборПоСвойствам.Значение,
	|	ЕСТЬNULL(ЕСТЬNULL(СвойстваХарактеристики.Значение, СвойстваНоменклатуры.Значение), ВЫБОР
	...
	ИндексРезультата = ИндексРезультата + 2;
...
#Область ВыходныеИзделия
	Если ПереченьДанных.Найти("ВыходныеИзделия") <> Неопределено Тогда
		...
		|	ПО ВыходныеИзделия.КлючСвязи = ВТОтборПоСвойствам.КлючСвязи
		//-- НЕ УТКА
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НормативнаяДлительностьЭтаповПроизводства КАК НормативнаяДлительностьЭтапов
...
#Область ВходящиеИзделия
	Если ПереченьДанных.Найти("ВходящиеИзделия") <> Неопределено ИЛИ ПараметрыВыборки.ОбъединитьМатериалыИВходящиеИзделия Тогда
		...
		ТекстЗапроса = ТекстЗапроса + "
		|ВЫБРАТЬ
		|	ВходящиеИзделия.Ссылка         КАК Ссылка,
		|	ВходящиеИзделия.НомерСтроки    КАК НомерСтроки,
		|
		|	ВходящиеИзделия.Номенклатура   КАК Номенклатура,
		|	ВходящиеИзделия.Характеристика КАК Характеристика,
		|
		|	ВходящиеИзделия.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
		|	ВЫБОР 
		|		КОГДА ВходящиеИзделия.Номенклатура.ИспользованиеХарактеристик В (
		|					ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры),
		|					ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеСДругимВидомНоменклатуры),
		|					ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ИндивидуальныеДляНоменклатуры))
		|			ТОГДА ИСТИНА
		...
		|	КОНЕЦ                                            КАК ХарактеристикиИспользуются,
		|	ВЫБОР
		|		КОГДА ВходящиеИзделия.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ                                            КАК ЛюбаяХарактеристика,
		|
		|	ВходящиеИзделия.Номенклатура.ЕдиницаИзмерения    КАК ЕдиницаИзмерения,
		|	ВходящиеИзделия.Упаковка                         КАК Упаковка,
		|	ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1)    КАК КоэффициентУпаковки,
		|
		|	ВходящиеИзделия.Количество                       КАК Количество,
		|	ВходящиеИзделия.КоличествоУпаковок               КАК КоличествоУпаковок,
		|	ВходящиеИзделия.КоличествоУпаковок               КАК КоличествоУпаковокНаЕдиницуПартииВыпуска,
		|	ЕСТЬNULL(ОсновноеВыходноеИзделие.Количество, 0)  КАК КоличествоИзделие,
		|
		|	ЛОЖЬ                                             КАК Производится,
		|	ВходящиеИзделия.СпособПолученияМатериала         КАК СпособПолученияМатериала,
		|
		...
		|
		|	ВходящиеИзделия.СтатьяКалькуляции                КАК СтатьяКалькуляции,
		|
		|	ВходящиеИзделия.Этап                             КАК Этап,
		|	ВходящиеИзделия.ЭтапРедактирование               КАК ЭтапРедактирование,
		|
		|	ВходящиеИзделия.Этап.Подразделение               КАК ПодразделениеЭтапа,
		|	ВходящиеИзделия.Этап.Подразделение               КАК Подразделение,
		|
		|	ВЫБОР ВходящиеИзделия.Ссылка.ТипПроизводственногоПроцесса
		|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.Разборка)
		|			ТОГДА ЕСТЬNULL(НормативнаяДлительностьЭтапов.ДнейОтНачала, 0)
		|		ИНАЧЕ - ЕСТЬNULL(НормативнаяДлительностьЭтапов.ДнейДоОкончания, 0)
		|	КОНЕЦ                                            КАК ДнейОтПотребности,
		...
		|
		|ИЗ
		|	ВтВходящиеИзделия КАК ВходящиеИзделия
		|	
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОсновноеИзделие КАК ОсновноеВыходноеИзделие
		|		ПО ИСТИНА
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НормативнаяДлительностьЭтаповПроизводства КАК НормативнаяДлительностьЭтапов
		|		ПО НормативнаяДлительностьЭтапов.Этап = ВходящиеИзделия.Этап
		|
		...
		ТекстЗапросаКоэффициентУпаковки = Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
			"ВходящиеИзделия.Упаковка",
			"ВходящиеИзделия.Номенклатура");
		...
		ПоляСоединения = "ВходящиеИзделия.Номенклатура, ВходящиеИзделия.Характеристика, СпрСклады.Ссылка";
		...
		ИндексыРезультата.Вставить("ВходящиеИзделия", ИндексРезультата);
		ИндексРезультата = ИндексРезультата + 1;
	КонецЕсли;
#КонецОбласти
//-- НЕ УТКА
...
#Область ВидыРабочихЦентров
	Если ПереченьДанных.Найти("ВидыРабочихЦентров") <> Неопределено Тогда
		...
		|	ВТВремяРаботы.Занятость                         КАК Занятость,
		|	ВТВремяРаботы.Занятость                         КАК ЗанятостьНаЕдиницуПартииВыпуска,
		|
		...
		|
		|////////////////////////////////////////////////////////////////////////////////";
		ИндексыРезультата.Вставить("ВидыРабочихЦентров", ИндексРезультата);
		ИндексРезультата = ИндексРезультата + 1;
	КонецЕсли;
#КонецОбласти
...
#Область АльтернативныеВидыРабочихЦентров
	Если ПереченьДанных.Найти("АльтернативныеВидыРабочихЦентров") <> Неопределено Тогда
		...
		|УПОРЯДОЧИТЬ ПО
		|	ТабличнаяЧасть.НомерСтроки";
		ИндексыРезультата.Вставить("АльтернативныеВидыРабочихЦентров", ИндексРезультата);
		ИндексРезультата = ИндексРезультата + 1;
	КонецЕсли;
	#КонецОбласти
//-- НЕ УТКА
...
//++ НЕ УТКА
	СписокТаблиц.Добавить("ВТОтборПоСвойствамНастройки");
	СписокТаблиц.Добавить("ВТОтборПоСвойствам");
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ВтВходящиеИзделия") <> Неопределено Тогда
		СписокТаблиц.Добавить("ВтВходящиеИзделия");
	КонецЕсли;
	Если МенеджерВременныхТаблиц.Таблицы.Найти("ВТВремяРаботыВидовРабочихЦентров") <> Неопределено Тогда
		СписокТаблиц.Добавить("ВТВремяРаботыВидовРабочихЦентров");
	КонецЕсли;
//-- НЕ УТКА
...
//++ НЕ УТКА
			ОбработатьНастройкиАвтовыбораИПолученияНоменклатуры(
				НаборДанных,
				ИмяНабораДанных,
				ДанныеПоНоменклатуре,
				ПараметрыВыборки);
...
//-- НЕ УТКА
	...
	КонецЦикла;
//++ НЕ УТКА
	Если ПараметрыВыборки.ОбъединитьМатериалыИВходящиеИзделия И СтруктураДанных.Свойство("МатериалыИУслуги") Тогда
		ОбъединитьМатериалыИВходящиеИзделия(СтруктураДанных.МатериалыИУслуги, МассивРезультатов[ИндексыРезультата["ВходящиеИзделия"]].Выгрузить());
	КонецЕсли;
//-- НЕ УТКА
...
//++ НЕ УТКА
	Если ДанныеСпецификации.Свойство("ВходящиеИзделия")
		...
		РассчитатьКоличество(
			ПартияВыпуска,
			ДанныеСпецификации.ВходящиеИзделия,
			Истина,
			Ложь,
			Ложь);
		РассчитатьДнейОтПотребности(ПараметрыРасчета.ДанныеПоНоменклатуре, ДанныеСпецификации.ВходящиеИзделия);
	КонецЕсли;
//-- НЕ УТКА
...
//++ НЕ УТКА
	Если ДанныеСпецификации.Свойство("ВидыРабочихЦентров")
		...
		РассчитатьЗанятость(
			...
			ПараметрыРасчета,
			ДанныеДляРасчета);
		РассчитатьДнейОтПотребности(ПараметрыРасчета.ДанныеПоНоменклатуре, ДанныеСпецификации.ВидыРабочихЦентров);
	КонецЕсли;
...
Процедура РассчитатьВремяРаботыВСтроке(ПартияВыпуска, Строка, ДанныеДляРасчета = Неопределено)
	
	Если ПустаяСтрока(Строка.АлгоритмРасчетаКоличества) Тогда
	
		ОдновременноПроизводимое =
			?(Строка.ОдновременноПроизводимоеКоличествоЕдиницПартийИзделий = 0,
			1,
...
Процедура РассчитатьЗанятость(ПартияВыпуска, ИсходнаяТаблица, ПараметрыРасчета, ДанныеДляРасчета = Неопределено)

	Для каждого Строка Из ИсходнаяТаблица Цикл
		
		Если ПустаяСтрока(Строка.АлгоритмРасчетаКоличества) Тогда
		
			Если ПараметрыРасчета.ИспользоватьОдновременноПроизводимоеКоличество Тогда
				...
				Строка.Занятость = Строка.ЗанятостьНаЕдиницуПартииВыпуска * Коэффициент;
				
			Иначе
				
				Строка.Занятость = Строка.ЗанятостьНаЕдиницуПартииВыпуска * ПартияВыпуска.Числитель / ПартияВыпуска.Знаменатель;
...
Функция ВыгрузитьДанныеДляКонструктораФормул(ОбъектИлиСсылка, ИмяТЧ, ДанныеСтроки = Неопределено) Экспорт
	...
	ОписаниеИсточников = Справочники.РесурсныеСпецификации.ОписаниеИсточниковДанныхОперандов();
	
	ОсновноеИзделие = УправлениеДаннымиОбИзделияхКлиентСервер.ДанныеОсновногоИзделияСпецификации(ОбъектИлиСсылка);
...
Функция ОписаниеИсточниковДанныхОперандов(Объект = Неопределено) Экспорт
	...
	// [Основное изделие]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'ОсновноеИзделие'"), НСтр("ru = 'Основное изделие'"));
	
	НовыйРеквизит = УправлениеДаннымиОбИзделиях.СоздатьОписанияРеквизитаИсточникаДанныхОперандов(НСтр("ru = 'Количество'"));
	...
	Если Объект = Неопределено
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'ВыходныеИзделия'"), НСтр("ru = 'Выходные изделия'"));
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
			НСтр("ru = 'ВыходноеИзделие'"), 
			НСтр("ru = 'Выходное изделие (текущая строка)'"));
	...
	// [МатериалыИУслуги]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'МатериалыИУслуги'"), НСтр("ru = 'Материалы и работы'"));
	...
	// [Материал]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'Материал'"), НСтр("ru = 'Материал (текущая строка)'"));
	...
	// [ВозвратныеОтходы]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
		НСтр("ru = 'ВозвратныеОтходы'"), 
		НСтр("ru = 'Побочный выход'"));
	...
	// [ВозвратныйОтход]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(
		НСтр("ru = 'ВозвратныйОтход'"), 
		НСтр("ru = 'Побочный выход (текущая строка)'"));
	...
	// [Подразделение]
	Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'Подразделение'"), НСтр("ru = 'Подразделение'"));
	
	Описание.СодержитСвойства = Истина;
	Описание.НаборСвойств     = Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_СтруктураПредприятия;
	...
	Если НастройкиПодсистемыПроизводство.ИспользуетсяПроизводство22 И Не НастройкиПодсистемыПроизводство.ИспользуетсяПроизводство21 Тогда
		...
		Описание = УправлениеДаннымиОбИзделиях.СоздатьОписанияИсточникаДанныхОперандов(НСтр("ru = 'Распоряжение'"), НСтр("ru = 'Заказ на производство'"));
		
		Описание.СодержитСвойства = Истина;
		Описание.НаборСвойств     = Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ЗаказНаПроизводство2_2;
...
Функция ИсточникДанныхДляРасчетаПоФормулам(ПутьКДанным, ИсточникиДанных) Экспорт
	...
	Иначе
		...
		ИначеЕсли ПутьКДанным.Ключ = "Распоряжение" Тогда
...
Процедура ДобавитьСтрокиДляРасчетаПоФормулам(НаборДанных, ДанныеСпецификации, ПереченьДанных)
	...
	СоставТЧ = 
		СтрРазделить("ВыходныеИзделия,ВозвратныеОтходы,МатериалыИУслуги,Трудозатраты,ВидыРабочихЦентров,АльтернативныеВидыРабочихЦентров",",");
...
Процедура ОбработатьНастройкиАвтовыбораИПолученияНоменклатуры(НаборДанных, ИмяНабораДанных, ДанныеПоНоменклатуре, ПараметрыВыборки)
	
	СтрокиАвтоВыбора = УправлениеДаннымиОбИзделиях.ОбработатьНастройкиАвтовыбораНоменклатуры(
																НаборДанных,
																ДанныеПоНоменклатуре,
	...
	Если СтрокиАвтоВыбора.Количество() > 0 Тогда
		...
		ЗаполнитьНастройкиПолученияМатериалов(СтрокиАвтоВыбора, ИмяНабораДанных, ДанныеПоНоменклатуре, ПараметрыВыборки);
...
Функция РазузловатьПотребностиИЗаполнитьСрокиОбеспечения(ВходящиеПотребности, ПараметрыВыборки, КэшированныеСпецификации, ОписаниеОшибки)
	...
	//++ НЕ УТКА
	Иначе
		ЗаполнитьСрокиОбеспеченияПролеживанияПотребностей(
			Потребности,
...
Процедура ЗаполнитьСрокиОбеспеченияПролеживанияПотребностей(Потребности, Распоряжения)
	
	НормативныйГрафик = НормативныйГрафикПоТаблицеПотребностей(Потребности, Распоряжения);
	
	НормативныйГрафик.Индексы.Добавить("НомерЭтапа, КлючСвязиПотребность");
	НормативныйГрафик.Индексы.Добавить("НомерСледующегоЭтапа, КлючСвязиПотребность");
	НормативныйГрафик.Индексы.Добавить("Этап, КлючСвязиПотребность");
	
	СтруктураПоискаПервыйЭтап    = Новый Структура("НомерЭтапа, КлючСвязиПотребность", 1);
	СтруктураПоискаПоследнийЭтап = Новый Структура("НомерСледующегоЭтапа, КлючСвязиПотребность", 0);
	СтруктураПоискаЭтап          = Новый Структура("Этап, КлючСвязиПотребность");
	
	Для каждого Потребность Из Потребности Цикл
		
		Если Потребность.ЭтоЗависимаяПотребность Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураПоискаПервыйЭтап.КлючСвязиПотребность = Потребность.КлючСвязи;
		ПервыеЭтапы = НормативныйГрафик.НайтиСтроки(СтруктураПоискаПервыйЭтап);
		
		СтруктураПоискаПоследнийЭтап.КлючСвязиПотребность = Потребность.КлючСвязи;
		ПоследниеЭтапы = НормативныйГрафик.НайтиСтроки(СтруктураПоискаПоследнийЭтап);
		...
		Если Потребность.РазмещениеВыпуска = Перечисления.СпособыПривязкиОперацийПроизводства.КНачалу Тогда
			...
			Начало = Границы.КНачалу + ?(ПервыеЭтапы.ВГраница() <> -1, ПервыеЭтапы[0].ДлительностьДоЗапуска, 0);
			
			Окончание = '00010101';
			Для каждого Этап Из ПоследниеЭтапы Цикл
				Окончание = Макс(Окончание, Границы.КНачалу + Этап.ДлительностьДоЗапуска + Этап.Ресурсоемкость);
			КонецЦикла;
			
		Иначе
			
			Начало = '39991231';
			Для каждого Этап Из ПервыеЭтапы Цикл
				Начало = Мин(Начало, Границы.КОкончанию - Этап.ДлительностьДоВыпуска - Этап.Ресурсоемкость);
			КонецЦикла;
			
			Окончание = Границы.КОкончанию - ?(ПоследниеЭтапы.ВГраница() <> -1, ПоследниеЭтапы[0].ДлительностьДоВыпуска, 0);
		...
		Если Потребность.ОграниченСрокПролеживания Тогда
			...
			ИначеЕсли Потребность.ЭтоРемонт И ЭтоРазузлованнаяПотребность(Потребность) Тогда
				...
				СтруктураПоискаЭтап.КлючСвязиПотребность = Потребность.КлючСвязиПотребность;
				СтруктураПоискаЭтап.Этап = Потребность.ЭтапПолучательИзделия;
				ЭтапПолучатель = НормативныйГрафик.НайтиСтроки(СтруктураПоискаЭтап)[0];
				
				Если Потребность.РазмещениеВыпуска = Перечисления.СпособыПривязкиОперацийПроизводства.КНачалу Тогда
					Потребность.СрокПролеживания = Границы.КНачалу + ЭтапПолучатель.ДлительностьДоЗапуска;
				Иначе
			       	Потребность.СрокПролеживания = Границы.КОкончанию - ЭтапПолучатель.ДлительностьДоВыпуска - ЭтапПолучатель.Ресурсоемкость;
				КонецЕсли;
		...
		ЗаполнитьСвойстваЗависимойПотребности(Потребность, "Начало, Окончание, СрокПролеживания");
...
Процедура ДобавитьИндексыВТаблицуЭтаповДляРасчетаНормативногоГрафика(Этапы) Экспорт
	...
	Этапы.Индексы.Добавить("СвязанСЭтапомПроизводства");
...
Процедура ЗаполнитьЭтапыПоТаблицеПотребностей(Потребности, Этапы)
	...
	Для каждого Потребность Из Потребности Цикл
		...
		Для каждого Этап Из Потребность.Этапы Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Потребность, "Распоряжение, ЭтоРазборка");
			...
			Если Этап.НомерЭтапа = 1 ИЛИ Потребность.Этапы.Найти(Этап.НомерЭтапа, "НомерСледующегоЭтапа") = Неопределено Тогда
				
				СтруктураПоиска = Новый Структура;
				СтруктураПоиска.Вставить("КлючСвязиПотребность", Потребность.КлючСвязи);
				СтруктураПоиска.Вставить("ПланироватьНеРанее", Справочники.ЭтапыПроизводства.ПустаяСсылка());
				СтруктураПоиска.Вставить("ЭтоРазборка", Ложь);
				СтруктураПоиска.Вставить("ЭтоРемонт", Ложь);
				...
				Если НайденныеСтроки.Количество() = 0 Тогда
					...
					Если Потребность.ЭтоРазборка Тогда
						...
						НоваяСтрока.ПервыйЭтап = НЕ ЗначениеЗаполнено(Потребность.ЭтапИсточник)
							И (Потребность.КлючСвязиПотребность = 0);
						
					ИначеЕсли Потребность.ЭтоРемонт Тогда
						
						НоваяСтрока.ПервыйЭтап = (Потребность.КлючСвязиПотребность = 0);
						
					Иначе
						
						НоваяСтрока.ПервыйЭтап = Потребность.ПланироватьНеРанее.Пустая();
						
					КонецЕсли;
					
				КонецЕсли;
				
				Если Потребность.ЭтоРазборка И ЗначениеЗаполнено(Потребность.ЭтапИсточник) Тогда
			...
			Если Этап.НомерСледующегоЭтапа = 0 Тогда
				...
				Если Потребность.ЭтоРазборка Тогда
					...
					СтруктураПоиска = Новый Структура("КлючСвязиПотребность, ЭтоРазборка", Потребность.КлючСвязи, Истина);
					НайденныеСтроки = Потребности.НайтиСтроки(СтруктураПоиска);
				...
				Иначе
					...
					Если ЗначениеЗаполнено(Потребность.ЭтапИсточник) Тогда
					...
					Иначе
						...
						НоваяСтрока.ПоследнийЭтап = (Потребность.КлючСвязиПотребность = 0);
...
Процедура ЗаполнитьВнутренниеЗависимостиЭтапов(Этапы, Зависимости) Экспорт
...
Процедура ЗаполнитьВнешниеЗависимостиПоТаблицеПотребностей(Потребности, Потребность, Зависимости)
	...
	Если ЗначениеЗаполнено(Потребность.КлючСвязиПотребность) Тогда
		
		СвязаннаяПотребность = Потребности.Найти(Потребность.КлючСвязиПотребность, "КлючСвязи");
		Если НЕ СвязаннаяПотребность = Неопределено Тогда
			
			Если Потребность.ЭтоРемонт Тогда
				
				СтрокаОтправитель = Потребность.Этапы.Найти(Потребность.ЭтапОтправительИзделия, "Этап");
				СтрокаПолучатель = СвязаннаяПотребность.Этапы.Найти(Потребность.ЭтапПолучательИзделия, "Этап");
				
				Если СтрокаОтправитель <> Неопределено И СтрокаПолучатель <> Неопределено Тогда
					
					НоваяСтрока = Зависимости.Добавить();
					НоваяСтрока.Этап = СтрокаОтправитель.КлючСвязи;
					НоваяСтрока.СледующийЭтап = СтрокаПолучатель.КлючСвязи;
					
				КонецЕсли;
				
				ЗависимаяПотребность = Потребность.ЗависимаяПотребность;
				Если ЗначениеЗаполнено(ЗависимаяПотребность) Тогда
					
					СтрокаОтправитель = СвязаннаяПотребность.Этапы.Найти(ЗависимаяПотребность.ЭтапОтправительИзделия, "Этап");
					СтрокаПолучатель = Потребность.Этапы.Найти(ЗависимаяПотребность.ЭтапПолучательИзделия, "Этап");
			...
			Иначе
				...
				Отправитель = ?(Потребность.ЭтоРазборка, СвязаннаяПотребность, Потребность);
				СтрокаОтправитель = Отправитель.Этапы.Найти(Потребность.ЭтапОтправительИзделия, "Этап");
				
				Получатель = ?(Потребность.ЭтоРазборка, Потребность, СвязаннаяПотребность);
				СтрокаПолучатель = Получатель.Этапы.Найти(Потребность.ЭтапПолучательИзделия, "Этап");
				...
				Если НЕ Потребность.ПланироватьНеРанее.Пустая() Тогда
					...
					СтрокаОтправитель = СвязаннаяПотребность.Этапы.Найти(Потребность.ПланироватьНеРанее);
					СтрокиПолучатели = Потребность.Этапы.НайтиСтроки(Новый Структура("НомерЭтапа", 1));
...
//++ НЕ УТКА
Процедура ОбъединитьМатериалыИВходящиеИзделия(МатериалыИУслуги, ВходящиеИзделия)
	...
	Для каждого Колонка Из МатериалыИУслуги.Колонки Цикл
		Если ВходящиеИзделия.Колонки.Найти(Колонка.Имя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
//++ НЕ УТКА
Процедура СоздатьВТВремяРаботыВидовРабочихЦентров(МенеджерВременныхТаблиц, Спецификация)
	...
	ТекстЗапроса = 
		...
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ЭтапыПроизводства.ПорядокРаботыВидовРабочихЦентров = ЗНАЧЕНИЕ(Перечисление.ПорядокРаботыВидовРабочихЦентров.Одновременно)
		|ИЗ
		...
		|			ТОГДА 0
		|		ИНАЧЕ ТабличнаяЧасть.Занятость
		|	КОНЕЦ КАК Занятость
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТСинхроннаяРабота
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТМаксВремяРаботы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТВидыРЦ
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТСинхроннаяОдновременнаяРабота";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РесурсныеСпецификации.ОтборПоСвойствам КАК РесурсныеСпецификацииОтборПоСвойствам
	|		ПО РесурсныеСпецификацииТрудозатраты.Ссылка = РесурсныеСпецификацииОтборПоСвойствам.Ссылка
	|			И РесурсныеСпецификацииТрудозатраты.УдалитьОтборСвойство = РесурсныеСпецификацииОтборПоСвойствам.Свойство
	|			И РесурсныеСпецификацииТрудозатраты.КлючСвязи = РесурсныеСпецификацииОтборПоСвойствам.КлючСвязи
	|ГДЕ
	|	РесурсныеСпецификацииТрудозатраты.УдалитьОтборСвойство <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПустаяСсылка)
	|	И РесурсныеСпецификацииОтборПоСвойствам.Свойство ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ
	|
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				Если Объект.ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.ПустаяСсылка() Тогда
					...
					Объект.ИспользоватьНесколькоВыходныхИзделий = Выборка.ИспользоватьНесколькоВыходныхИзделий ИЛИ (Объект.ВыходныеИзделия.Количество() > 1);
				...
				Для каждого ТабличнаяЧасть Из СтрРазделить("ВыходныеИзделия,ВозвратныеОтходы,МатериалыИУслуги,Трудозатраты",",") Цикл
					Для каждого Строка Из Объект[ТабличнаяЧасть] Цикл
						...
						Если (ТабличнаяЧасть = "МатериалыИУслуги" ИЛИ ТабличнаяЧасть = "Трудозатраты")
				...
				НайденныеСтроки = Объект.СоответствиеСвойств.НайтиСтроки(
					Новый Структура("СпособПодбораЗначения", Перечисления.СпособыПодбораЗначенияСвойстваНоменклатуры.ПустаяСсылка()));
				...
				Для каждого Строка Из НайденныеСтроки Цикл
					...
					ИначеЕсли Строка.УдалитьПредопределенноеЗначение Тогда
						Строка.СпособПодбораЗначения = Перечисления.СпособыПодбораЗначенияСвойстваНоменклатуры.ПоЗначению;
