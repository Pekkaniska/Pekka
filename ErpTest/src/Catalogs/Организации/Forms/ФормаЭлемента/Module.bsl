...
Процедура ЗаписатьДанныеУчетнойПолитикиИНалогов(ТекущийОбъект, Отказ)
	...
	Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.СтавкиНалогаНаИмущество)
	 И (ЕстьЗаписиРегистрСтавкиНалогаНаИмущество(ТекущийОбъект.Ссылка)
		...
		Если Не ЗначениеЗаполнено(СтавкаНалогаНаИмуществоПериод) Тогда
			СтавкаНалогаНаИмуществоПериод = УчетнаяПолитикаОрганизаций.Период;
		КонецЕсли;
		
		Набор = РегистрыСведений.СтавкиНалогаНаИмущество.СоздатьНаборЗаписей();
		Набор.Отбор.Организация.Установить(ТекущийОбъект.Ссылка);
		Набор.Отбор.Период.Установить(НачалоДня(СтавкаНалогаНаИмуществоПериод));
		
		СтрокаНабора = Набор.Добавить();
		СтрокаНабора.Организация									 = ТекущийОбъект.Ссылка;
		СтрокаНабора.Период	 	 									 = НачалоДня(СтавкаНалогаНаИмуществоПериод);
...
Процедура СтавкаНалогаНаИмуществоДатаНачалоВыбораЗавершение(ВыбранныйПериод, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(ВыбранныйПериод) Тогда
		...
		СтавкаНалогаНаИмуществоПериод = ВыбранныйПериод;
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка)
		...
		СтавкаНалогаНаИмуществоПериод = НачалоМесяца(ОбщегоНазначенияКлиент.ДатаСеанса());
