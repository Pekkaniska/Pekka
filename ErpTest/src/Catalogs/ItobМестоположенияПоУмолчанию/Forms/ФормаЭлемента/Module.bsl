...
Процедура ЗадатьМестоположение(Команда)
	...
	Если НЕ ItobОперативныйМониторингКлиентСервер.ПроверитьДоступностьСервисаCsmSvc(ItobОперативныйМониторингКлиентСервер.ПолучитьАдресСервисаCsmSvcСервер()) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Сервис CsmSvc недоступен! Необходимо воспользоваться мастером настройки службы CsmSvc.'"));
		Возврат;
