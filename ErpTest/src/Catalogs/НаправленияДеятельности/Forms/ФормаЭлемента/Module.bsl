...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаРежимНалогообложения.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьРаздельныйУчетПоНалогообложению");
...
Процедура УстановитьТипНаправленияНаСервере()
	...
	//++ НЕ УТ
	МожноВестиУчетЗатрат = ПолучитьФункциональнуюОпцию("УчитыватьСебестоимостьТоваровПоВидамЗапасов") 
							И ПолучитьФункциональнуюОпцию("ИспользоватьОбособленноеОбеспечениеЗаказов");
	Если НЕ МожноВестиУчетЗатрат Тогда
	...
	Если МожноВестиУчетЗатрат И УчетСредствПоКонтрактуГОЗ Тогда
		...
		КонецЕсли;
		Если НЕ УчетДенежныхСредств Тогда
			УчетДенежныхСредств = Истина;
			Объект.УчетДенежныхСредствРаздельно = Истина;
			СпособУчетаДС = "РаздельныйУчет";
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыФормыЦелевыеПараметрыКонтрактаГОЗ()
	...
	ПараметрыФормы = Отчеты.ИсполнениеКонтрактовГОЗ.СтруктураЦелевыхПараметровКонтрактаГОЗ();
