...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//++ НЕ УТ
	|	ИЛИ ТаблицаОбъекта.ТипНаправленияДеятельности = ЗНАЧЕНИЕ(Перечисление.ТипыНаправленийДеятельности.ПустаяСсылка)
	//-- НЕ УТ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Объект <> Неопределено Тогда
				...
				//++ НЕ УТ
				Если Объект.ТипНаправленияДеятельности = Перечисления.ТипыНаправленийДеятельности.ПустаяСсылка() Тогда
