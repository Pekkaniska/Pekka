...
Процедура ПередЗаписью(Отказ)
	...
	Если Показатели.Количество() < 2 Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В таблице преобразования должно быть как минимум две строки!'"));		
	КонецЕсли;
	...
	Для Каждого СтрокаТЗ Из ТЗКонтроль Цикл
		Если СтрокаТЗ.Количество > 1 Тогда
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'В таблице преобразования дублируются одинаковые строки!'"));	
