...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСертификатыПодписейОрганизации
...
Процедура ИсходящиеДокументыВерсияФорматаНачалоВыбораЗавершить(ВыбранныйЭлемент, ТекущиеДанные) Экспорт 
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ПриИзмененииФорматаВСтрокеНастроек(ТекущиеДанные);
...
Процедура ОбновитьФлагиРегламентаПоСтроке(Строка, СпособОбмена, ИспользоватьУПД, ИспользоватьУКД)
	...
	Если Не ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(СпособОбмена)
		...
		Если (Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура И Не ИспользоватьУПД)
			Или (Строка.ИсходящийДокумент = Перечисления.ВидыЭД.КорректировочныйСчетФактура 
		...
		Иначе
			...
			Если Строка.ИсходящийДокумент = Перечисления.ВидыЭД.СчетНаОплату
...
Процедура ВыбратьАдрес(ТолькоДляПросмотра = Ложь)
	...
	ОткрытьФорму("Обработка.ОбменСКонтрагентами.Форма.АдресУчастникаОбменаЭД",
		ПараметрыФормы,
		,
		,
		,
...
Процедура НастройкаУведомленийПродолжить(РасшифрованныйМаркер, ДополнительныеПараметры) Экспорт
	...
	ПрофильНастроекЭДО       = ДополнительныеПараметры.Профиль;
	ИдентификаторОрганизации = ДополнительныеПараметры.Идентификатор;
	
	Если Не ЗначениеЗаполнено(РасшифрованныйМаркер) Тогда
		Возврат;
	КонецЕсли;
	...
	НастройкиУведомления.Вставить("ИдентификаторОрганизации", ИдентификаторОрганизации);
	НастройкиУведомления.Вставить("ПрофильНастроек",          ПрофильНастроекЭДО);
...
Процедура ПриИзмененииИсходящиеДокументы(Знач ИмяКолонки, Знач ИдентификаторСтроки)
	...
	Если ТекущиеДанные.ИсходящийДокумент = Перечисления.ВидыЭД.ПроизвольныйЭД
		...
		Если ИмяКолонки = "ИсходящиеДокументыИспользоватьЭП" И НЕ ТекущиеДанные.ИспользоватьЭП Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
				ТекущиеДанные.ИсходящийДокумент);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
		
		ТекущиеДанные.ИспользоватьЭП = ТекущиеДанные.Формировать;
		Если Не ТекущиеДанные.Формировать Тогда 
			ТекущиеДанные.ТребуетсяОтветнаяПодпись = ТекущиеДанные.Формировать;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТекущиеДанные.ИсходящийДокумент = Перечисления.ВидыЭД.СчетФактура
		...
		Если ТекущиеДанные.ВерсияФормата = "ФНС 5.01 (УПД:Счет-фактура и первичный документ)" Тогда
	...
	Если ИмяКолонки = "ИсходящиеДокументыДокументУчета" Тогда
		Если Не ТекущиеДанные.ИспользоватьЭП Тогда
			ТекущиеДанные.ТребуетсяОтветнаяПодпись = Ложь;
		КонецЕсли;
		
		ОбновитьФлагиРегламентаПоСтроке(ТекущиеДанные, Объект.СпособОбменаЭД, Объект.ИспользоватьУПД, Объект.ИспользоватьУКД);
		
		ОбменСКонтрагентамиСлужебныйКлиентСервер.ЗаполнитьПредставлениеРегламентаЭДОВСтроке(ТекущиеДанные);
	КонецЕсли;
...
Процедура РазрешитьРедактированиеРеквизитовСпискаВидыЭДЗавершить(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ТекущийЭлемент = Элементы.ИдентификаторОрганизации;
	КонецЕсли;
...
Процедура РазрешитьРедактированиеДополнительныхНастроекЗавершить(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СвойстваФормы.Вставить("СпособОбменаЭД", Объект.СпособОбменаЭД);
