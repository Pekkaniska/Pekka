...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("Операция", Операция); // "Подписание" или "Отправка"
	...
	Иначе
		...
		НастройкаНапоминаний = ХранилищеСистемныхНастроек.Загрузить(
			"Справочник.ПрофилиНастроекЭДО.Форма.ПомощникРегистрацииСертификатов", "ОтложенныеНапоминания");
...
Процедура НеНапоминать(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ДобавитьПрофильВНастройкуНапоминаний(ТекущиеДанные.Ссылка);
	КонецЕсли;
	
	Если Не ПрофилиНастроекЭДО.Количество() Тогда
		Закрыть();
	КонецЕсли;
...
Процедура ПриЗакрытииНаСервере()
	...
	ХранилищеСистемныхНастроек.Сохранить("Справочник.ПрофилиНастроекЭДО.Форма.ПомощникРегистрацииСертификатов",
...
Функция СформироватьТаблицуДанных(Профили)
	...
	Таблица.Колонки.Добавить("ПрофильНастроекЭДО", Новый ОписаниеТипов("СправочникСсылка.ПрофилиНастроекЭДО"));
	...
	Для Каждого Элемент Из Профили Цикл
		...
		Строка.ПрофильНастроекЭДО = Элемент;
	КонецЦикла;
...
Процедура ПрименитьНастройкиОтложенныхНапоминаний(ТаблицаДанных)
	...
	Если ОтложенныеНапоминания.Количество() Тогда
		...
		ХешиСертификатов = ХешСертификатовПрофиля(ТаблицаДанных.ВыгрузитьКолонку("ПрофильНастроекЭДО"));
		
		Для Каждого Строка Из ТаблицаДанных Цикл
			
			СтрокиНастроек = ОтложенныеНапоминания.НайтиСтроки(Новый Структура("ПрофильНастроек", Строка.ПрофильНастроекЭДО));
			Если СтрокиНастроек.Количество() Тогда
				...
				ТекущийХешСертификатов    = ХешиСертификатов[Строка.ПрофильНастроекЭДО];
...
&НаСервере
Процедура ДобавитьПрофильВНастройкуНапоминаний(ПрофильНастроекЭДО)
	
	СтрокиНастроек = ОтложенныеНапоминания.НайтиСтроки(Новый Структура("ПрофильНастроек", ПрофильНастроекЭДО));
	Если СтрокиНастроек.Количество() Тогда
	...
	ХешСертификатов = ХешСертификатовПрофиля(ПрофильНастроекЭДО);
	
	СтрокаНастройки.ПрофильНастроек = ПрофильНастроекЭДО;
	СтрокаНастройки.ХешСертификатов = ХешСертификатов[ПрофильНастроекЭДО];
	СтрокаНастройки.ДатаОтсчета     = НачалоДня(ТекущаяДатаСеанса());
	
	УдаляемыеСтроки = ПрофилиНастроекЭДО.НайтиСтроки(Новый Структура("Ссылка", ПрофильНастроекЭДО));
	Для Каждого Строка Из УдаляемыеСтроки Цикл
		ПрофилиНастроекЭДО.Удалить(Строка);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Функция ХешСертификатовПрофиля(ПрофилиНастроекЭДО)
	
	ХешиОтпечатков = Новый Соответствие;
	
	СписокПрофилей = Новый Массив;
	Если ТипЗнч(ПрофилиНастроекЭДО) <> Тип("Массив") Тогда
		СписокПрофилей.Добавить(ПрофилиНастроекЭДО);
	Иначе
		СписокПрофилей = ПрофилиНастроекЭДО;
	КонецЕсли;
	...
	|ИЗ
	|	Справочник.ПрофилиНастроекЭДО КАК ПрофилиНастроекЭДО
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПрофилиНастроекЭДО.СертификатыПодписейОрганизации КАК СертификатыПрофиля
	|		ПО ПрофилиНастроекЭДО.Ссылка = СертификатыПрофиля.Ссылка
	|ГДЕ
	...
	Пока ВыборкаПрофиль.Следующий() Цикл
		...
		ХешиОтпечатков.Вставить(ВыборкаПрофиль.Профиль, ХешОтпечатков);
