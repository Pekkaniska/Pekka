	...
	Если Параметры.Свойство("СпособыОбменаЭД", СпособыОбменаЭД) И ЗначениеЗаполнено(СпособыОбменаЭД) Тогда
		Если СпособыОбменаЭД.Количество() = 1 Тогда
			...
			Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском Тогда
				...
				Заголовок = Элементы.СтраницаПодключенияК1СТакском.Заголовок;
...
Процедура ПолучитьИдентификаторУчастникаОбменаЭДЧерез1СЭДО()
	...
	ИначеЕсли Не ЗначениеЗаполнено(КодРегиона) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "АдресОрганизации");
		ВозниклиОшибки = Истина;
	КонецЕсли;
		...
		Если ДеревоXML <> Неопределено Тогда
			...
			Если УзелОператорыЭДО <> Неопределено Тогда
				Для Каждого УзелОператорЭДО Из УзелОператорыЭДО.Строки Цикл
					...
					НовыйОператор = ОператорыЭДО.Добавить();
					НовыйОператор.Наименование = УзелОператорЭДОНаимОрг.Значение;
					НовыйОператор.Идентификатор = УзелОператорЭДОИд.Значение;
					НовыйОператор.ИНН = УзелОператорЭДОИНН.Значение;
					НовыйОператор.КПП = УзелОператорЭДОКПП.Значение;
					Если УзелОператорЭДООГРН <> Неопределено Тогда
						НовыйОператор.ОГРН = УзелОператорЭДООГРН.Значение;
					КонецЕсли;
					Если УзелОператорЭДОСертификат <> Неопределено Тогда
						НовыйОператор.Сертификат = УзелОператорЭДОСертификат.Значение;
					КонецЕсли;
					...
					Если УзелОператорЭДОНаимОрг.Значение = ОператорЭДО Тогда
						...
						ОператорЭДОИд = НовыйОператор.Идентификатор;
						ОператорЭДОИНН = НовыйОператор.ИНН;
						ОператорЭДОКПП = НовыйОператор.КПП;
						ОператорЭДООГРН = НовыйОператор.ОГРН;
						ОператорЭДОСертификат = НовыйОператор.Сертификат;
...
Процедура СохранитьПараметры(ДанныеСохранены)
	...
	Попытка
		...
		НастройкаИспользоватьЭП = ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи()
			И (НЕ ОбменСКонтрагентамиСлужебныйКлиентСервер.ЭтоПрямойОбмен(СпособОбменаЭД) ИЛИ ИспользоватьЭП);
		ВидыЭлектронныхДокументов = ОбменСКонтрагентамиСлужебный.ИспользуемыеВидыЭлектронныхДокументов();
		Для Каждого ЗначениеПеречисления Из ВидыЭлектронныхДокументов Цикл
			...
			ОбменСКонтрагентамиСлужебный.ЗаполнитьНастройкуВидаЭлектронногоДокумента(
				НоваяСтрока, ЗначениеПеречисления, СпособОбменаЭД, НастройкаИспользоватьЭП);
			
		КонецЦикла;
		
		ТаблицаФорматов = Неопределено;
		...
		Для Каждого ПрикладнойВид Из ПрикладныеВидыЭлектронныхДокументов Цикл
			...
			ОбменСКонтрагентамиСлужебный.ЗаполнитьНастройкуПрикладногоВидаЭлектронногоДокумента(
				НоваяСтрока, ПрикладнойВид, НастройкаИспользоватьЭП);
