...
//++ НЕ УТКА
	...
	|	И СпособыОбеспеченияПотребностей.Подразделение <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
//-- НЕ УТКА
	|";
...
//++ НЕ УТКА
				Если Объект.ТипОбеспечения = Перечисления.ТипыОбеспечения.Производство
					...
					Если Не ЗначениеЗаполнено(Объект.ИсточникОбеспеченияПотребностей) Тогда
						Объект.ИсточникОбеспеченияПотребностей = Объект.Подразделение;
					КонецЕсли;
