...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		//++ НЕ УТ
		УчетУСНСервер.ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации(ПервыйПериод, СписокОрганизаций, Ложь, КоличествоПовторяющихсяДанных);
		ЗакрытиеМесяца.ОбновитьРезервыСомнительныхДолговПослеРаспределенияРасчетов(ПервыйПериод, СписокОрганизаций);
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		ВызватьИсключение НСтр("ru = 'При новой архитектуре расчетов с партнерами не требуется отложенное распределение расчетов.'");
	КонецЕсли;
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		//++ НЕ УТ
		УчетУСНСервер.ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации(ПервыйПериод, СписокОрганизаций,, КоличествоПовторяющихсяДанных);
		ОценкаПроизводительности.ЗафиксироватьЗамерДлительнойОперации(ОписаниеЗамера, КоличествоПовторяющихсяДанных, "ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации");
		КоличествоПовторяющихсяДанных = 0;
		//-- НЕ УТ
...
Процедура РассчитатьРасчетыСКлиентами(НачалоПериода, ОкончаниеПериода, ИсходныеДанные, КоличествоЗаписей = 0)
	...
	//++ НЕ УТ
	РеглУчетПроведениеСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВРеглУчете(РасчетныеРасчетыСКлиентами);
	//-- НЕ УТ
...
Процедура РассчитатьРасчетыСПоставщиками(НачалоПериода, ОкончаниеПериода, ИсходныеДанные, КоличествоЗаписей = 0)
	...
	//++ НЕ УТ
	РеглУчетПроведениеСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВРеглУчете(РасчетныеРасчетыСПоставщиками);
	//-- НЕ УТ
...
//Корректировку не будем пересчитывать, она самодостаточна
		|		И ТИПЗНАЧЕНИЯ(Расчеты.Регистратор) <> ЗНАЧЕНИЕ(Документ.КорректировкаРегистров)
...
//Рарус Владимир Подрезов 20.02.2017
//Корректировку не будем пересчитывать, она самодостаточна
		|		И ТИПЗНАЧЕНИЯ(РасчетыПоДокументам.Регистратор) <> ЗНАЧЕНИЕ(Документ.КорректировкаРегистров)
//Рарус Владимир Подрезов Конец
		|	) КАК Расчеты
		|
...
Процедура РассчитатьПартииПоЦепочкамИзВыборки(ОписаниеДвижений, РезультатЗапроса, РасчетныеПартии, ЦепочкиДвижений, Регистраторы,  Тест = Ложь, КоличествоЗаписей = 0)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если РасчетныеПартии.Количество() > 1000
			...
			КонецЕсли;
			//++ НЕ УТ
			РеглУчетПроведениеСервер.ЗарегистрироватьДокументыРасчетовСПартнерамиКОтражениюВРеглУчете(РасчетныеПартии);
			//-- НЕ УТ
...
Функция СформироватьЗаданияКРасчетуНалогов(МенеджерРегистра, Расчеты, Регистраторы)
	...
	Если МенеджерРегистра = РегистрыНакопления.РасчетыСПоставщикамиПоДокументам Тогда
		...
		//++ НЕ УТ
		ВнеоборотныеАктивы.ДополнитьТекстыЗапросовПриЗаписиДвиженийРаспределенияВзаиморасчетов(Запрос, ТекстыЗапросов);
		//-- НЕ УТ
		...
		ВнеоборотныеАктивы.СформироватьЗаданияПриЗаписиДвиженийРаспределенияВзаиморасчетов(
			РезультатЗапроса, 4, Запрос.МенеджерВременныхТаблиц);
		//-- НЕ УТ
...
Процедура ПроверитьБлокировкуВходящихДанныхПриОбновленииИБ(ПредставлениеОперации)
	...
	//++ НЕ УТ
	ВходящиеДанные.Вставить(Метаданные.Документы.ПриобретениеУслугПоЛизингу);
	ВходящиеДанные.Вставить(Метаданные.Документы.ОтчетОператораСистемыПлатон);
	//-- НЕ УТ
