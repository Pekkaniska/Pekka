...
// Серверная часть обработчика события изменения организации формы документа.
//
// Параметры:
// 	Форма - УправляемаяФорма.
//
Процедура ОрганизацияПриИзмененииНаСервере(Форма) Экспорт
	
		ЗначенияДляЗаполнения = ЗначенияДляЗаполненияОтветственныхЛиц(Форма);
...
Процедура ЗаполнитьПервоначальныеЗначения(Форма) Экспорт
	...
	Для Каждого ЗначениеДляЗаполнения Из ЗначенияДляЗаполненияОтветственныхЛиц(Форма) Цикл
...
Процедура ПриПолученииДанныхСтрокиСоставаНДФЛ(Форма, СтрокаСостава)
	...
	Если Форма.СпособВыплаты.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Аванс 
		Или Форма.СпособВыплаты.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Зарплата  Тогда
	...
	Для Каждого СтрокаНДФЛРаботника Из СтрокиНДФЛРаботника Цикл
		...
		СтрокаСостава.НДФЛСумма = СтрокаСостава.НДФЛСумма + СтрокаНДФЛРаботника.Сумма;
		
		Если ЗначениеЗаполнено(СтрокаНДФЛРаботника.ДокументОснование) Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
				ТипыОснований,
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТипЗнч(СтрокаНДФЛРаботника.ДокументОснование)),
				Истина);
		КонецЕсли;	
	...
	Если Форма.СпособВыплаты.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Аванс И ТипыОснований.Количество() > 0 Тогда
...
Функция СписокОплат(Форма)
	...
	РеквизитыРегистратора = ОбщегоНазначенияБЗК.ЗначенияРеквизитовОбъектов(Регистраторы, "Номер, Дата");
...
Функция ЗначенияДляЗаполненияОтветственныхЛиц(Форма)
