...
//
Функция НедоступныеДальнейшиеДействия(ДокументВЕТИС, ДальнейшиеДействия, ОперацииДопустимыхДействий, ХозяйствующийСубъект, ПользовательВЕТИС = Неопределено) Экспорт
...
Функция ПродукцияСоответствуетПриказам(ДокументВЕТИС)
	...
	Результат = Новый Структура("ЕстьПродукцияПриказ647, ЕстьПродукцияПриказ646, ЕстьПродукцияПриказ647Приказ646, ЕстьПродукцияПриказ648, ЭтоВСДНаСыроеМолоко, ЭтоСертификацияУлововВБР");
	Результат.ЕстьПродукцияПриказ647 = Ложь;
	Результат.ЕстьПродукцияПриказ646 = Ложь;
	Результат.ЕстьПродукцияПриказ648 = Ложь;
	Результат.ЕстьПродукцияПриказ647Приказ646 = Ложь;
	Результат.ЭтоВСДНаСыроеМолоко = Ложь;
	Результат.ЭтоСертификацияУлововВБР = Ложь;
	...
	ИначеЕсли ТипЗнч(ДокументВЕТИС) = Тип("ДокументОбъект.ИсходящаяТранспортнаяОперацияВЕТИС") Тогда
		ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Таблица.Ссылка КАК Продукция
		|ПОМЕСТИТЬ
		|	ВтПродукция
		|ИЗ
		|	Справочник.ПродукцияВЕТИС КАК Таблица
		|ГДЕ
		|	Таблица.Ссылка В (&МассивПродукция);
		|";
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда
		...
		Если ЭтоСсылка Тогда
			Запрос.УстановитьПараметр("ДокументСсылка", ДокументВЕТИС);
		Иначе
			МассивПродукция = ДокументВЕТИС.Товары.ВыгрузитьКолонку("Продукция");
			Запрос.УстановитьПараметр("МассивПродукция", МассивПродукция);
		КонецЕсли;
...
Функция ИдентификаторыСырогоМолока()
...
Функция ОперацияДоступнаПользователюСРасшифровкой(ОперацияВЕТИС, ДокументВЕТИС = Неопределено, ХозяйствующийСубъект = Неопределено, ПользовательВЕТИС = Неопределено, ПраваДоступа = Неопределено, УполномоченноеГашение = Неопределено)
	...
	Если ДействиеДоступно = Истина И ОперацияВЕТИС = Перечисления.ВидыОперацийВЕТИС.ЗапросНаОформлениеТранспортнойПартии
		...
		Если СоответствиеПриказам.ЕстьПродукцияПриказ648 Тогда
			
			ДействиеДоступно = НСтр("ru = 'Недостаточно прав оформление транспортной партии продукции 648 приказа'");
			
		ИначеЕсли ПраваДоступа.Найти(Перечисления.ПраваДоступаВЕТИС.ОформлениеВСДПриказ646) = Неопределено
			И ПраваДоступа.Найти(Перечисления.ПраваДоступаВЕТИС.АттестованныйСпециалист) = Неопределено Тогда
			
			ДействиеДоступно = НСтр("ru = 'Недостаточно прав оформление транспортной партии продукции 646 приказа'");
		
		ИначеЕсли ПраваДоступа.Найти(Перечисления.ПраваДоступаВЕТИС.ОформлениеВСДНаСыроеМолоко) = Неопределено
			И СоответствиеПриказам.ЭтоВСДНаСыроеМолоко Тогда
			
			ДействиеДоступно = НСтр("ru = 'Недостаточно прав оформление транспортной партии на сырое молоко'");
			
		ИначеЕсли ПраваДоступа.Найти(Перечисления.ПраваДоступаВЕТИС.СертификацияУлововВБР) = Неопределено
			И СоответствиеПриказам.ЭтоСертификацияУлововВБР Тогда
			
			ДействиеДоступно = НСтр("ru = 'Недостаточно прав оформление транспортной партии на рыбу'");
		...
		ИначеЕсли ПраваДоступа.Найти(Перечисления.ПраваДоступаВЕТИС.АттестованныйСпециалист) = Неопределено
			...
			ДействиеДоступно = НСтр("ru = 'Недостаточно прав оформление транспортной партии продукции 647 приказа'");
		...
		ИначеЕсли ПраваДоступа.Найти(Перечисления.ПраваДоступаВЕТИС.ОформлениеВСДПриказ646) = Неопределено
			...
			ДействиеДоступно = НСтр("ru = 'Недостаточно прав оформление транспортной партии продукции 646/647 приказа'");
