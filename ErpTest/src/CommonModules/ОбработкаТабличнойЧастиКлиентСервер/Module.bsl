...
//+++rarus-spb_zlov 25.09.2016    
	СтруктураЗаполненияЦены.Вставить("ОбъектСтроительства");
	СтруктураЗаполненияЦены.Вставить("Договор");
    ...
    Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") 
      ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
	    СтруктураЗаполненияЦены.Вставить("ОбъектСтроительства", Объект.пкОбъектСтроительства);
	    СтруктураЗаполненияЦены.Вставить("Договор",             Объект.Договор);
	    СтруктураЗаполненияЦены.Вставить("ТекущийДокумент",     Объект.Ссылка);
	    СтруктураЗаполненияЦены.Вставить("Дата",                Объект.Дата);        
    КонецЕсли;    
//---rarus-spb_zlov 25.09.2016 
...
Процедура ЗаполнитьПризнакЦеховаяКладовая(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Склад) Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			РеквизитыСклада = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущаяСтрока.Склад, "ЦеховаяКладовая, Подразделение");
			ЦеховаяКладовая = РеквизитыСклада.ЦеховаяКладовая;
			Подразделение = РеквизитыСклада.Подразделение;
		#Иначе
			ТекстИсключения = НСтр("ru = 'Попытка заполнения служебных реквизитов на основании выбранного склада на клиенте.'");
			ВызватьИсключение ТекстИсключения;
		#КонецЕсли
...
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыДействия;
	
	Если Не СтруктураДействий.Свойство("ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
...
// Заполняет назначение побочного выхода для текущей строки табличной части документа "Этап производства".
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПроверитьЗаполнитьНазначениеПобочногоВыхода(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ПараметрыДействия;
	
	Если Не СтруктураДействий.Свойство("ПроверитьЗаполнитьНазначениеПобочногоВыхода", ПараметрыДействия) Тогда
		Возврат;
	КонецЕсли;
	
	ОбеспечениеПроизводстваКлиентСервер.ЗаполнитьНазначениеПобочногоВыходаВСтрокеЭтапаПроизводства(
		ТекущаяСтрока,
		ПараметрыДействия);
