...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПересчитыватьВВалютуРегл = Истина) Экспорт
	...
	|			ТОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл, 0)
	|		ИНАЧЕ 0
...
#Область ВключениеИсключениеНДСВСтоимость // (Дт 2х :: Кт 19.04)
	...
	|ВЫБРАТЬ //// Включение/исключение НДС в стоимость (Дт 2х :: Кт 19.04)
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	АналитикаУчетаНоменклатуры.Склад КАК МестоУчетаДт,
	|	ВЫРАЗИТЬ(АналитикаУчетаНоменклатуры.Склад КАК Справочник.Склады).Подразделение КАК ПодразделениеДт,
