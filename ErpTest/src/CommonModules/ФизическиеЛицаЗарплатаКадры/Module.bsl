...
Функция РасшифровкаПодписи(Знач ФИОПолные) Экспорт
	...
	Если ФИО.Количество() >= 3 Тогда
		...
		РасшифровкаПодписи = ФизическиеЛицаЗарплатаКадрыКлиентСервер.ИнициалыИмени(ФИО[1]) + " "
			+ Лев(ФИО[2], 1) + ". "
			+ ФИО[0];
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если ВыборГруппИЭлементов <> ИспользованиеГруппИЭлементов.Группы Тогда
		...
		Если Параметры.Свойство("СтрокаПоиска") 
			...
			СоздатьВТПрежниеФИО(Запрос.МенеджерВременныхТаблиц, Ложь, Параметры.СтрокаПоиска);
...
Процедура СоздатьВТПрежниеФИО(МенеджерВременныхТаблиц, ТолькоРазрешенные, СтрокаПоиска, ИмяВТПрежниеФИО = "ВТПрежниеФИО") Экспорт
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ФИОФизическихЛицСрезПоследних.Период,
		...
		|	КОНЕЦ КАК Период,
		|	ВсеПохожиеЗаписиФИО.Фамилия,
		|	ВсеПохожиеЗаписиФИО.Имя,
		|	ВсеПохожиеЗаписиФИО.Отчество
...
Функция НаименованиеСлужебное(Наименование) Экспорт
	...
	Возврат СокрЛП(
		ФамилияСлужебная(СтруктураФИО.Фамилия) + " "
		+ ИмяСлужебное(СтруктураФИО.Имя) + " "
		+ ОтчествоСлужебное(СтруктураФИО.Отчество));
