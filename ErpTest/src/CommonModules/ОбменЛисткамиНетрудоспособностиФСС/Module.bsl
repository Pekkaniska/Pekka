...
Функция ВыгрузитьЗапросДляОтправкиРеестраЭЛН(Документ, ПомещатьВФайл, ИдентификаторФормы) Экспорт
	...
	request.regNum = УчетПособийСоциальногоСтрахованияКлиентСервер.РегистрационныйНомерДляОбменаФСС(Документ);
	...
	Результат.РегистрационныйНомерФСС = УчетПособийСоциальногоСтрахованияКлиентСервер.РегистрационныйНомерДляОбменаФСС(Документ);
	Результат.ТекстXML                = ЗаписьXML.Закрыть();
...
Функция ДанныеБольничногоДляПолученияЭЛН(БольничныйЛист, Отказ)
	...
	Результат.РегистрационныйНомерФСС = УчетПособийСоциальногоСтрахованияКлиентСервер.РегистрационныйНомерДляОбменаФСС(РеквизитыОрганизации);
...
Функция ЗагрузитьОтветСервисаФСС(Документ, Операция, АдресРасшифрованногоОтвета) Экспорт
	...
	Если WSResult.Свойства().Получить("MESS") <> Неопределено Тогда
		...
		Если Не ПустаяСтрока(ТекстСообщенияФСС) Тогда
			ТекстСообщенияФСС = НСтр("ru = 'При обмене с ФСС возникли ошибки. Ответ ФСС:'") + Символы.ПС + ТекстСообщенияФСС;
		КонецЕсли;
	КонецЕсли;
