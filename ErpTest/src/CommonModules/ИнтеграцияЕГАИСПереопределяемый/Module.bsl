...
Процедура ЗаполнитьТаблицуШтрихкодовНеМаркируемойПродукцииТТН_ЕГАИС(ДокументСсылка, ЗаполняемаяТаблица) Экспорт
	...
	|	ТТНВходящаяЕГАИСТовары.Номенклатура.ОбъемДАЛ КАК Коэффициент
	|ИЗ
...
Функция ОрганизацияПоИННКПП(ИНН, КПП) Экспорт
	...
	//++ НЕ ГОСИС
	МетаданныеИмя = Метаданные.Справочники.Организации.Имя;
	ОбменСКонтрагентамиПереопределяемый.СсылкаНаОбъектПоИННКПП(МетаданныеИмя, ИНН, КПП, ОрганизацияСсылка);
	//-- НЕ ГОСИС
...
Функция ЕстьПравоСозданияКонтрагента() Экспорт
	...
	//++ НЕ ГОСИС
	ОписаниеТиповКонтрагентГИСМ = Метаданные.ОпределяемыеТипы.КонтрагентГИСМ.Тип;
	
	Если ОписаниеТиповКонтрагентГИСМ.Типы().Количество() = 1 Тогда
		МетаданныеТипа = Метаданные.НайтиПоТипу(ОписаниеТиповКонтрагентГИСМ.Типы()[0]);
		Если МетаданныеТипа <> Неопределено Тогда
			Возврат ПравоДоступа("Добавление", МетаданныеТипа);
		КонецЕсли;
	КонецЕсли;
	//-- НЕ ГОСИС
...
Функция ПолеКоэффициентПересчетаНеупакованнойПродукции(ИмяТаблицы = "ТабличнаяЧасть") Экспорт
	...
	//++ НЕ ГОСИС
	Возврат ИнтеграцияЕГАИСУТ.ПолеКоэффициентПересчетаНеупакованнойПродукции(ИмяТаблицы);
	//-- НЕ ГОСИС
