...
//	МассивДокументов 		 - ОпределяемыйТип.ОснованиеСтатусыОформленияДокументовВЕТИС - документы-основание для документа ВЕТИС
//	МетаданныеДокументаВЕТИС - ОбъектМетаданных:Документ - метаданные документа из ОпределяемыйТип.ДокументыВЕТИСПоддерживающиеСтатусыОформления
//	МенеджерВТ 				 - МенеджерВременныхТаблиц - см. одноименный параметр СформироватьТаблицуТоварыДокументовОснования()
//
// Возвращаемое значение:
//	Соответствие
//		Ключ 	 - элемент параметра МассивДокументов
...
Функция ОпределитьСтатусыОформленияДокументов(МассивДокументов, МетаданныеДокументаВЕТИС, МенеджерВТ)
	...
	ЕстьТЧТоварыУточнение = (Метаданные.Документы[ИмяДокументаВЕТИС].ТабличныеЧасти.Найти("ТоварыУточнение") <> Неопределено);
	ЕстьТЧСырье 		  = (Метаданные.Документы[ИмяДокументаВЕТИС].ТабличныеЧасти.Найти("Сырье") <> Неопределено);
	
	Если ИмяДокументаВЕТИС = Метаданные.Документы.ИсходящаяТранспортнаяОперацияВЕТИС.Имя Тогда
		ЭтоДвижениеПриход = "ЛОЖЬ";
	ИначеЕсли ИмяДокументаВЕТИС = Метаданные.Документы.ИнвентаризацияПродукцииВЕТИС.Имя Тогда
		ЭтоДвижениеПриход =
	...
	Иначе
		...
		ЕстьКоличество = (Метаданные.Документы[ИмяДокументаВЕТИС].ТабличныеЧасти.Товары.Реквизиты.Найти("Количество") <> Неопределено);
		
		ТекстЗапросаОформленныеТовары = ТекстЗапросаОформленныеТовары
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				?(ЕстьТЧТоварыУточнение,
					ШаблонЗапросаОформленныеТоварыСУточнениями,
					ШаблонЗапросаОформленныеТовары),
				ИмяДокументаВЕТИС,
				"Товары",
...
Функция ОпределитьСовпадениеОтправителяИПолучателяВЕТИС(МассивДокументов, МетаданныеДокументаВЕТИС, ТаблицаРеквизитов, МенеджерВТ)
	...
	Запрос.УстановитьПараметр(
		...
		ИнтеграцияВЕТИС.МассивПустыхЗначенийОпределяемогоТипа(Метаданные.ОпределяемыеТипы.ТорговыйОбъектВЕТИС));
	Запрос.УстановитьПараметр(
		"ПустойПроизводственныйОбъект",
		ИнтеграцияВЕТИС.МассивПустыхЗначенийОпределяемогоТипа(Метаданные.ОпределяемыеТипы.ПроизводственныйОбъектВЕТИС));
				...
				Если НЕ ПредопределенныеЗначения.Свойство(КлючИЗначение.Ключ) Тогда
					...
					Если СоставныеРеквизиты.Свойство(КлючИЗначение.Ключ) Тогда
						...
						Для Каждого ТекущийРеквизит Из СоставныеРеквизиты[КлючИЗначение.Ключ] Цикл
							Если ЗначениеЗаполнено(ИсточникОбъект[ТекущийРеквизит]) Тогда
								СтрокаРеквизитов[КлючИЗначение.Ключ] = ИсточникОбъект[ТекущийРеквизит];
							КонецЕсли;
						КонецЦикла;
						
					Иначе
						СтрокаРеквизитов[КлючИЗначение.Ключ] = ИсточникОбъект[КлючИЗначение.Значение];
					КонецЕсли;
		...
		Иначе
			...
			Для Каждого РеквизитыДокумента Из ЗначенияРеквизитовОбъектов Цикл
				...
				Для Каждого ЗначенияРеквизитовДокумента Из РеквизитыДокумента.Значение Цикл
					...
					ИмяРеквизита = СтруктураРеквизитовОснования[ЗначенияРеквизитовДокумента.Ключ];
