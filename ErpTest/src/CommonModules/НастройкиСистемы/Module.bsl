...
Процедура СинхронизироватьЗначенияПодчиненныхКонстант(ИмяКонстанты, ЗначениеКонстанты, Отказ)
	...
	ИначеЕсли Найти(ИмяКонстанты, "ИспользоватьВнеоборотныеАктивы") > 0 Тогда
		...
		Если НЕ Учет2_2 И НЕ Учет2_4 Тогда
			УстановитьЗначениеКонстанты(Константы.ИспользоватьЛизинг, Ложь);
			УстановитьЗначениеКонстанты(Константы.ВестиУчетПлатежейВПлатон, Ложь);
		КонецЕсли;
		
		Если Учет2_4 И НЕ ЗначениеЗаполнено(Константы.ПорядокУчетаВНАВУпрУчете.Получить()) Тогда
			УстановитьЗначениеКонстанты(Константы.ПорядокУчетаВНАВУпрУчете, Перечисления.ПорядокУчетаВНАВУпрУчете.ПоСтандартамМУ);
		КонецЕсли; 
	//-- НЕ УТ
