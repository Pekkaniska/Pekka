...
#Область ТекстВозвратОплатыОтПоставщика
	...
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика)
	|	И Операция.ПроведеноБанком
	|	И (ЕСТЬNULL(РасчетыПоДокументам.ДолгРегл, 0) <> 0 ИЛИ ЕСТЬNULL(РасчетыПоДокументам.Долг, 0) <> 0)
	|";
#КонецОбласти
...
#Область ТекстРасчетыПоДепозиту                  // (Дт 51 :: Кт 55.03, 76.09)
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	
	|	ЕСТЬNULL(РасчетыПоДепозитам.Сумма,0) КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
...
#Область ТекстРасчетыПоПредоставленномуЗайму     // (Дт 51 :: Кт 58.03, 76.09)
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	
	|	ЕСТЬNULL(РасчетыПоЗаймам.Сумма, 0) КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстВозвратАвансаОтПоставщика);
	ТекстыОтражения.Добавить(ТекстВозвратОплатыОтПоставщика);
	...
	ТекстыОтражения.Добавить(ТекстВозвратАвансаОтДрОрганизации);
	ТекстыОтражения.Добавить(ТекстВозвратОплатыОтДрОрганизации);
