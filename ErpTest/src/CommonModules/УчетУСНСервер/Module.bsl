...
Функция ТекстЗапросаДоходовДляАнализаНеобходимостиСторноНоваяАрхитектураРасчетов()
	...
	|	РасчетыСКлиентами.ДолгРегл КАК Сумма
	|ПОМЕСТИТЬ ДоходыДляПроверкиНаСторно
	|ИЗ
...
Функция ПризнанныеРасходы(ПараметрыЗапроса)
	...
	|		КОГДА КОНЕЦПЕРИОДА(ДанныеПервичныхДокументов.ДатаРегистратора, ДЕНЬ) = ДанныеПервичныхДокументов.ДатаРегистратора
	|			ТОГДА ДанныеПервичныхДокументов.ДатаРегистратора
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(ДанныеПервичныхДокументов.ДатаРегистратора, СЕКУНДА, 1)
	|	КОНЕЦ КАК Период,
...
Функция СодержаниеРасхода(ВидРасхода, ХозяйственнаяОперация, Партнер, Договор)
	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		...
		КонецЕсли;
		МассивСодержания.Добавить(СтрШаблон(НСтр("ru = 'от %1 ""%2""'"), ПредставлениеПоставщика, Партнер));
	КонецЕсли;	
	Если ЗначениеЗаполнено(Договор) Тогда
		МассивСодержания.Добавить(СтрШаблон(НСтр("ru = 'по договору ""%1""'"), Договор));
	КонецЕсли;
...
Функция ТекстЗапросаРасходовПоСебестоимости()
	...
	|			ПО Аналитика.Склад = СкладыУчетаМатериалов.Склад
	...
	|			КОГДА Аналитика.Склад ССЫЛКА Справочник.СтруктураПредприятия ИЛИ НЕ СкладыУчетаМатериалов.Склад ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Материалы)
	|			КОГДА ВЫРАЗИТЬ(Партии.СтатьяРасходовАктивов КАК ПланВидовХарактеристик.СтатьиРасходов).ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|					И (Партии.АналитикаРасходов ССЫЛКА Справочник.НематериальныеАктивы
	|						ИЛИ Партии.АналитикаРасходов ССЫЛКА Справочник.ОбъектыЭксплуатации)
	...
	|		КОНЕЦ КАК ВидРасходов,
	|		ВЫБОР КОГДА Партии.Партия ССЫЛКА Документ.СборкаТоваров ТОГДА Партии.ДокументПоступления ИНАЧЕ Партии.Партия КОНЕЦ КАК Партия,
	|		Партии.ДокументПоступления КАК ДокументВозникновенияРасходов,
	|		ВЫБОР
	...
	|		Партии.ТипЗаписи В (ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ДопРасходы),ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ДопРасходыСПартией),ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ДопРасходыБезПартии)) КАК ДопРасход
	|	ИЗ
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Услуги)
	|			КОГДА НЕ Аналитика.Склад ЕСТЬ NULL И Аналитика.Склад ССЫЛКА Справочник.СтруктураПредприятия ИЛИ НЕ СкладыУчетаМатериалов.Склад ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Материалы)
	|			КОГДА ВЫРАЗИТЬ(Партии.СтатьяРасходовАктивов КАК ПланВидовХарактеристик.СтатьиРасходов).ВариантРаспределенияРасходовРегл = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|					И (Партии.АналитикаРасходов ССЫЛКА Справочник.НематериальныеАктивы
	|						ИЛИ Партии.АналитикаРасходов ССЫЛКА Справочник.ОбъектыЭксплуатации)
	...
	|			ПО (ЕСТЬNULL(СебестоимостьТоваров.АналитикаУчетаНоменклатуры, Партии.КорАналитикаУчетаНоменклатуры) = Аналитика.КлючАналитики)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВидыРасходовПорядкаОтражения КАК ВидыРасходовПорядкаОтражения
	|		ПО Аналитика.Номенклатура.ГруппаФинансовогоУчета = ВидыРасходовПорядкаОтражения.ГруппаФинансовогоУчета
	|			И ЕСТЬNULL(Аналитика.Склад, НЕОПРЕДЕЛЕНО) = ВидыРасходовПорядкаОтражения.Склад
	|		ЛЕВОЕ СОЕДИНЕНИЕ ГруппыФинансовогоУчетаНеТовары КАК ГруппыФинансовогоУчета
	|		ПО Аналитика.Номенклатура.ГруппаФинансовогоУчета = ГруппыФинансовогоУчета.ГруппаФинансовогоУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ СкладыУчетаМатериалов КАК СкладыУчетаМатериалов
	|		ПО ЕСТЬNULL(Аналитика.Склад, НЕОПРЕДЕЛЕНО) = СкладыУчетаМатериалов.Склад
	...
	|			ИНАЧЕ ЕСТЬNULL(ВидыРасходовПорядкаОтражения.ВидРасходов, ЕСТЬNULL(ГруппыФинансовогоУчета.ВидРасходов, ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Номенклатура)))
	|		КОНЕЦ,
	|		ВЫБОР КОГДА Партии.Партия ССЫЛКА Документ.СборкаТоваров ТОГДА Партии.ДокументПоступления ИНАЧЕ Партии.Партия КОНЕЦ,
	|		Партии.ДокументПоступления,
	|		ВЫБОР
	|			КОГДА Партии.КорВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|			ТОГДА Партии.ВидДеятельностиНДС
...
Функция ТекстЗапросаРасходовПоКредитамИЛизингу()
	...
	|	РасчетыПоФинансовымИнструментам.СтатьяАналитики КАК СтатьяРасходов,
	...
	|	И РасчетыПоФинансовымИнструментам.Договор ССЫЛКА Справочник.ДоговорыКредитовИДепозитов
	|	И НЕ РасчетыПоФинансовымИнструментам.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.ОсновнойДолг)
	|	И НЕ РасчетыПоФинансовымИнструментам.ТипСуммы = ЗНАЧЕНИЕ(Перечисление.ТипыСуммГрафикаКредитовИДепозитов.ПустаяСсылка)
	|	И РасчетыПоФинансовымИнструментам.Договор.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерыДоговоровФинансовыхИнструментов.КредитИлиЗайм)
...
Функция ТекстЗапросаПоОплатамНоваяАрхитектураРасчетов()
	...
	|		ИЛИ РасчетыСПоставщиками.ПредоплатаРегл > 0)
