...
Функция РезультатКомпоновкиМакетаПечатнойФормы(ОбъектОтчета, ДанныеРасшифровки = Неопределено, НастройкиОтчета = Неопределено, ВнешниеНаборыДанных = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("ИспользуютсяПользовательскиеНастройкиПечати") Тогда
	...
	КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("МакетПечатнойФормы", МакетПечатнойФормы);
	...
	Если КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("МакетКомпоновкиДанных")
		...
		МакетКомпоновки = КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.МакетКомпоновкиДанных;
	...
	Если КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("ФормированиеМакетаКомпоновкиДанных") Тогда
		...
		КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("СкомпонованныйМакетКомпоновкиДанных", МакетКомпоновки);
...
Функция ТарифнаяСтавкаНаПечать(Знач ТарифнаяСтавка) Экспорт
	
	Если ЗначениеЗаполнено(ТарифнаяСтавка) Тогда
		Возврат Формат(Цел(ТарифнаяСтавка), "ЧДЦ=0; ЧН=; ЧГ=") + " " + НСтр("ru='руб'") + ". " + Формат((ТарифнаяСтавка - Цел(ТарифнаяСтавка)), "ЧЦ=2; ЧДЦ=; ЧС=-2; ЧН=; ЧВН=") + " " + НСтр("ru='коп'") + ".";
	КонецЕсли;
...
Функция ФорматДатыЧислоВКавычкахМесяцПрописью(Знач ФорматируемаяДата) Экспорт
	
	Если ФорматируемаяДата = NULL Тогда
		ФорматируемаяДата = '00010101';
	КонецЕсли;
	
	Возврат Формат(ФорматируемаяДата, "Л=ru_RU; ДФ='''""''дд''""'' ММММ гггг ''г.'''; ДП='""___"" ______________ 20____ г.'");
	...
	Пока СтрокВыведено < СтрокНачислено Или СтрокВыведено < СтрокУдержаноВыплачено Цикл
		...
		Для Каждого СтрокаРасшифровкаНачисленийУдержаний Из РасшифровкаНачисленийУдержаний Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаРасшифровкаНачисленийУдержаний.РасшифровкаНачисления) Тогда
				...
				Для Каждого КлючИЗначениеСтрукутры Из СтрокаРасшифровкаНачисленийУдержаний.РасшифровкаУдержания Цикл
					РасшифровкаНачисленияУдержания.Вставить(КлючИЗначениеСтрукутры.Ключ);
				КонецЦикла;
			...
			Если Не ЗначениеЗаполнено(СтрокаРасшифровкаНачисленийУдержаний.РасшифровкаУдержания) Тогда
				...
				РасшифровкаНачисленияУдержания = Новый Структура;
				Для Каждого КлючИЗначениеСтрукутры Из СтрокаРасшифровкаНачисленийУдержаний.РасшифровкаНачисления Цикл
					РасшифровкаНачисленияУдержания.Вставить(КлючИЗначениеСтрукутры.Ключ);
				КонецЦикла;
...
Функция ДополненныеПараметры(ДанныеПечатнойФормы, ДополнительныеПараметры)
	...
	Иначе
		...
		КопияДанныхПечатнойФормы = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ДанныеПечатнойФормы);
...
Функция ДанныеПоНДФЛИВзносам(СтрокиСотрудников, МесяцНачисления, ВыводитьОсобенностиРасчетаНДФЛ, ВыводитьИнформациюОНачисленныхВзносахВПФР)
	...
	Если ВыводитьОсобенностиРасчетаНДФЛ Тогда
		...
		Запрос.Текст =
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудники КАК Сотрудники
			|		ПО (ИмущественныеВычетыНДФЛ.МесяцРегистрации МЕЖДУ Сотрудники.МесяцНачисления И Сотрудники.КонецМесяцаНачисления)
			...
			|	И ИмущественныеВычетыНДФЛ.Период < &КонецПериода
			|{ГДЕ
			|	ИмущественныеВычетыНДФЛ.МесяцРегистрации КАК МесяцНачисления,
			|	ИмущественныеВычетыНДФЛ.ФизическоеЛицо.*,
			|	ИмущественныеВычетыНДФЛ.ГоловнаяОрганизация.*}
			|
			|СГРУППИРОВАТЬ ПО
			...
			|	СведенияОДоходахНДФЛОбороты.МесяцНачисления КАК МесяцНачисления
...
Процедура УстановитьОтборПоследнейСсылкиНаОбъектПечати(ОтчетОбъект, ПользовательскиеНастройкиКД)
	...
	Если ПараметрСсылка <> Неопределено Тогда
		...
		Если ЗначениеПараметра = Неопределено
			...
			УстановитьОбъектыОтбораОтбора(ПараметрСсылка, ЗначениеПараметра, ПользовательскиеНастройкиКД);
...
Процедура УстановитьОбъектыОтбораОтбора(ПараметрОтбора, ОбъектыОтбора, ПользовательскиеНастройкиКД = Неопределено)
...
Процедура УстановитьПараметрОтбора(ОтчетОбъект, ОбъектыОтбора, ПользовательскиеНастройкиКД = Неопределено) Экспорт
	...
	ПараметрСсылка = ПараметрОтбораПечатнойФормы(ОтчетОбъект, ПользовательскиеНастройкиКД);
	Если ПараметрСсылка <> Неопределено Тогда
		УстановитьОбъектыОтбораОтбора(ПараметрСсылка, ОбъектыОтбора, ОтчетОбъект.КомпоновщикНастроек.ПользовательскиеНастройки);
	КонецЕсли;
