...
Функция ОбработатьВводШтрихкода(Форма, ИсходныеДанные, КэшированныеЗначения) Экспорт
	...
	Если ИсходныеДанные.Свойство("ШтрихкодУпаковки")
		И ЗначениеЗаполнено(ИсходныеДанные.ШтрихкодУпаковки)
		И ИсходныеДанные.ТипУпаковки = Перечисления.ТипыУпаковок.МаркированныйТовар Тогда
		
		ДанныеШтрихкода = ИсходныеДанные;
	...
	Если ПараметрыСканированияАкцизныхМарок.ИспользуютсяДанныеВыбораПоАлкогольнойПродукции Тогда
	...
	ИначеЕсли ДанныеШтрихкода.ТипУпаковки = ПредопределенноеЗначение("Перечисление.ТипыУпаковок.МаркированныйТовар") Тогда
		...
		РезультатДобавленияШтрихкодаВДокумент = РезультатОбработкиШтрихкодаАкцизнойМарки(Форма,
		                                                                                 МенеджерДокумента, 
		                                                                                 ДанныеШтрихкода, 
...
Функция АкцизныеМаркиВДокументе(Форма, ПараметрыСканированияАкцизныхМарок) Экспорт
...
Функция РезультатОбработкиШтрихкодаАкцизнойМарки(Форма, МенеджерДокумента, ДанныеШтрихкода, ПараметрыСканированияАкцизныхМарок) Экспорт
	
	УчитыватьДанныеВыбора = Ложь;
	ДанныеВыбора = ПараметрыСканированияАкцизныхМарок.ДанныеВыбораПоАлкогольнойПродукции;
	ТребуетсяВыборНоменклатуры = Ложь;
	...
	Если ПараметрыСканированияАкцизныхМарок.ЗапрашиватьНоменклатуру
		И (Не ЗначениеЗаполнено(ДанныеШтрихкода.Номенклатура)
		Или Не ЗначениеЗаполнено(ДанныеШтрихкода.АлкогольнаяПродукция)) Тогда
		
		ТребуетсяВыборНоменклатуры = Истина;
		
	КонецЕсли;
	
	Если ПараметрыСканированияАкцизныхМарок.ЗапрашиватьНоменклатуру
		И ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.ДокументОснование)
		И Не ТребуетсяВыборНоменклатуры 
		И Не ЗначениеЗаполнено(ДанныеШтрихкода.Серия) Тогда
		
		ДанныеДокументаОснования = ДанныеДокументаОснования(ПараметрыСканированияАкцизныхМарок);
	...
	Если ТребуетсяВыборНоменклатуры Тогда
		
		Если ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.ДокументОснование)
			И Не ЭтоАдресВременногоХранилища(ПараметрыСканированияАкцизныхМарок.АдресДанныхДокументаОснования) Тогда
		...
		РезультатДобавленияШтрихкодаВДокумент.ПараметрыВыбораНоменклатуры.Вставить("ЗначениеШтрихкода",             ДанныеШтрихкода.Штрихкод);
		РезультатДобавленияШтрихкодаВДокумент.ПараметрыВыбораНоменклатуры.Вставить("Номенклатура",                  ДанныеШтрихкода.Номенклатура);
		РезультатДобавленияШтрихкодаВДокумент.ПараметрыВыбораНоменклатуры.Вставить("Характеристика",                ДанныеШтрихкода.Характеристика);
		РезультатДобавленияШтрихкодаВДокумент.ПараметрыВыбораНоменклатуры.Вставить("ДокументОснование",             ПараметрыСканированияАкцизныхМарок.ДокументОснование);
		РезультатДобавленияШтрихкодаВДокумент.ПараметрыВыбораНоменклатуры.Вставить("АдресДанныхДокументаОснования", ПараметрыСканированияАкцизныхМарок.АдресДанныхДокументаОснования);
...
Функция ПолучитьДанныеПоШтрихкодам(СписокШтрихкодов, ПараметрыСканированияАкцизныхМарок, КэшированныеЗначения, АкцизныеМаркиВДокументе = Неопределено) Экспорт
	...
	ЕстьОшибкиПриСчитыванииАкцизныхМарок = Ложь;
	ДанныеШтрихкодовУпаковок             = Неопределено;
	...
	Если КодыАкцизныхМарокPDF417.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			...
			ПолучитьДанныеАкцизныхМарокПоШтрихкодам(КодыАкцизныхМарокPDF417, 
			                                        ПараметрыСканированияАкцизныхМарок, 
			                                        АкцизныеМаркиВДокументе, 
			                                        Перечисления.ТипыШтрихкодов.PDF417));
	КонецЕсли;
	
	Если КодыАкцизныхМарокDataMatrix.Количество() > 0 Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			Результат.ОбработанныеШтрихкоды,
			ПолучитьДанныеАкцизныхМарокПоШтрихкодам(КодыАкцизныхМарокDataMatrix, 
			                                        ПараметрыСканированияАкцизныхМарок, 
			                                        АкцизныеМаркиВДокументе, 
			                                        Перечисления.ТипыШтрихкодов.DataMatrix));
	КонецЕсли;
		
	Если Результат.ОбработанныеШтрихкоды.Количество() > 0 Тогда
		
		Для Каждого ОбработанныйШтрихКод Из Результат.ОбработанныеШтрихкоды Цикл
			
			Если Не ПустаяСтрока(ОбработанныйШтрихКод.Значение.ТекстОшибки) Тогда
				
				ЕстьОшибкиПриСчитыванииАкцизныхМарок = Истина;
				Прервать;
		...
		Если ЕстьОшибкиПриСчитыванииАкцизныхМарок Тогда
			
			ОбработанныеАкцизныеМарки = Новый Массив;
			Для Каждого ОбработанныйШтрихКод Из Результат.ОбработанныеШтрихкоды Цикл
				
				ОбработанныеАкцизныеМарки.Добавить(ОбработанныйШтрихКод);
	...
	Если ЕстьОшибкиПриСчитыванииАкцизныхМарок Тогда
		...
		Если ДанныеШтрихкодовУпаковок = Неопределено Тогда
			...
			ДеревоУпаковок = Справочники.ШтрихкодыУпаковокТоваров.ПустоеДеревоУпаковок();
			ДобавитьКолонкиОшибокКДеревуУпаковок(ДеревоУпаковок);
			Результат.Вставить("ВложенныеШтрихкоды", Новый Структура("ДеревоУпаковок", ДеревоУпаковок));
			
		Иначе
			ДеревоУпаковок = ДанныеШтрихкодовУпаковок.ВложенныеШтрихкоды.ДеревоУпаковок;
		КонецЕсли;
		
		Для Каждого ОбработаннаяАкцизнаяМарка Из ОбработанныеАкцизныеМарки Цикл
			
			Если ДеревоУпаковок.Строки.Найти(ОбработаннаяАкцизнаяМарка.Ключ, "ЗначениеШтрихкода", Истина) = Неопределено Тогда
				
				НоваяСтрока = ДеревоУпаковок.Строки.Добавить();
				
				НоваяСтрока.ТекстОшибки          = ОбработаннаяАкцизнаяМарка.Значение.ТекстОшибки;
				НоваяСтрока.ЕстьОшибки           = Не ПустаяСтрока(НоваяСтрока.ТекстОшибки);
				НоваяСтрока.ТипУпаковки          = ОбработаннаяАкцизнаяМарка.Значение.ТипУпаковки;
				НоваяСтрока.ТипШтрихкода         = ОбработаннаяАкцизнаяМарка.Значение.ТипШтрихкода;
				НоваяСтрока.АлкогольнаяПродукция = ОбработаннаяАкцизнаяМарка.Значение.АлкогольнаяПродукция;
				НоваяСтрока.Номенклатура         = ОбработаннаяАкцизнаяМарка.Значение.Номенклатура;
				НоваяСтрока.Характеристика       = ОбработаннаяАкцизнаяМарка.Значение.Характеристика;
				НоваяСтрока.Серия                = ОбработаннаяАкцизнаяМарка.Значение.Серия;
				НоваяСтрока.Справка2             = ОбработаннаяАкцизнаяМарка.Значение.Справка2;
				НоваяСтрока.ЗначениеШтрихкода    = ОбработаннаяАкцизнаяМарка.Значение.Штрихкод;
				
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ОбработаннаяАкцизнаяМарка.Значение);
...
Функция ПолучитьДанныеАкцизныхМарокПоШтрихкодам(СписокШтрихкодов, ПараметрыСканированияАкцизныхМарок, АкцизныеМаркиВДокументе, ТипШтрихкода)
	...
	ПолучитьДанныеСуществующихАкцизныхМарок(СписокШтрихкодов, АкцизныеМаркиВДокументе, ТипШтрихкода,
	                                       ПараметрыСканированияАкцизныхМарок, Результат,  КодыАлкогольнойПродукции, СоответствиеКодаАкцизнойМарке);
	...
	ПолучитьДанныеНеизвестныхАкцизныхМарок(СписокШтрихкодов, ТипШтрихкода, ПараметрыСканированияАкцизныхМарок,
	                                       Результат, КодыАлкогольнойПродукции, СоответствиеКодаАкцизнойМарке);
	
	Если ТипШтрихкода = Перечисления.ТипыШтрихкодов.PDF417 Тогда
	
		СоответствиеАлкогольнойПродукции = ИнтеграцияЕГАИС.СоответствиеАлкогольнойПродукции(КодыАлкогольнойПродукции);
		Для Каждого КлючИЗначение Из СоответствиеАлкогольнойПродукции Цикл
			МассивАкцизныхМарок = СоответствиеКодаАкцизнойМарке[КлючИЗначение.Ключ];
			Для Каждого ЭлементМассива Из МассивАкцизныхМарок Цикл
				ДанныеШтрихкода = Результат[ЭлементМассива];
				Если ДанныеШтрихкода <> Неопределено Тогда
					ДанныеШтрихкода.АлкогольнаяПродукция = КлючИЗначение.Значение
				КонецЕсли;
			КонецЦикла;
...
Процедура ПолучитьДанныеНеизвестныхАкцизныхМарок(СписокШтрихкодов, ТипШтрихкода, ПараметрыСканированияАкцизныхМарок,
	                                             Результат, КодыАлкогольнойПродукции, СоответствиеКодаАкцизнойМарке)
	...
	Для Каждого КодАкцизнойМарки Из СписокШтрихкодов Цикл
		...
		Если ТипШтрихкода = Перечисления.ТипыШтрихкодов.PDF417 Тогда
			АкцизныеМаркиВызовСервера.ЗаполнитьСопоставленнуюНоменклатуруПоАкцизнойМарке(КодАкцизнойМарки, ДанныеШтрихкода, КэшКодовАлкогольнойПродукции);
		ИначеЕсли ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.Номенклатура) Тогда
			ДанныеШтрихкода.Номенклатура   = ПараметрыСканированияАкцизныхМарок.Номенклатура;
			ДанныеШтрихкода.Характеристика = ПараметрыСканированияАкцизныхМарок.Характеристика;
		КонецЕсли;
		...
		Если ПараметрыСканированияАкцизныхМарок.ЗапретитьАкцизныеМаркиDataMatrixБезСтатуса
			И ТипШтрихкода = Перечисления.ТипыШтрихкодов.DataMatrix
			И ПустаяСтрока(ДанныеШтрихкода.ТекстОшибки) Тогда
			
			ДанныеШтрихкода.ТекстОшибки = НСтр("ru = 'Статус: Не определен'");
		...
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.КодАлкогольнойПродукции)
			...
			ДанныеШтрихкода.КодАлкогольнойПродукции = АкцизныеМаркиВызовСервера.КодКлассификатораНоменклатурыЕГАИС(ДанныеШтрихкода.Штрихкод, КэшКодовАлкогольнойПродукции);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.АлкогольнаяПродукция)
			И Не ПустаяСтрока(ДанныеШтрихкода.КодАлкогольнойПродукции) Тогда
			Если КодыАлкогольнойПродукции.Найти(ДанныеШтрихкода.КодАлкогольнойПродукции) = Неопределено Тогда
				КодыАлкогольнойПродукции.Добавить(ДанныеШтрихкода.КодАлкогольнойПродукции);
				МассивАкцизныхМарок = Новый Массив;
				МассивАкцизныхМарок.Добавить(ДанныеШтрихкода.Штрихкод);
				СоответствиеКодаАкцизнойМарке.Вставить(ДанныеШтрихкода.КодАлкогольнойПродукции, МассивАкцизныхМарок);
			Иначе
				ЭлементСоответствия = СоответствиеКодаАкцизнойМарке.Получить(ДанныеШтрихкода.КодАлкогольнойПродукции);
				ЭлементСоответствия.Добавить(ДанныеШтрихкода.Штрихкод);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПолучитьДанныеСуществующихАкцизныхМарок(СписокШтрихкодов, АкцизныеМаркиВДокументе, ТипШтрихкода,
	                                              ПараметрыСканированияАкцизныхМарок, Результат, КодыАлкогольнойПродукции, СоответствиеКодаАкцизнойМарке)
	...
	Для Каждого КлючЗначение Из ШтрихкодыУпаковок Цикл
		...
		Если Не ЗначениеЗаполнено(ДанныеШтрихкода.ТекстОшибки) 
			И ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.ДокументОснование) Тогда
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				ДанныеШтрихкода.ТекстОшибки = НСтр("ru = 'Не соответствует документу-основанию'");
			КонецЕсли;
...
Функция ПолучитьДанныеПоШтрихкодамУпаковок(ШтрихкодыУпаковок, ПараметрыСканированияАкцизныхМарок, АкцизныеМаркиВДокументе)
	...
	ДобавитьКолонкиОшибокКДеревуУпаковок(ВложенныеШтрихкоды.ДеревоУпаковок);
	...
	Для Каждого СтрокаВерхнегоУровня Из ВложенныеШтрихкоды.ДеревоУпаковок.Строки Цикл
		...
		Иначе
			...
			Для Каждого СтрокаМаркированныеТовары Из НайденныеСтроки Цикл
				...
				Если Не ЗначениеЗаполнено(ТекстОшибки) 
					...
					ПараметрыПоиска = Новый Структура;
					ПараметрыПоиска.Вставить("Номенклатура",  ДанныеМаркированногоТовара.Номенклатура);
					ПараметрыПоиска.Вставить("Характеристика",ДанныеМаркированногоТовара.Характеристика);
					ПараметрыПоиска.Вставить("Серия",         ДанныеМаркированногоТовара.Серия);
		...
		ПроверитьИЗаполнитьВложенныеШтрихкоды(СтрокаВерхнегоУровня.Строки, ДанныеШтрихкода.ВложенныеШтрихкоды, 
		                                      АкцизныеМаркиВДокументе, Результат.ОбработанныеШтрихкоды, ШтрихкодыУпаковок);
...
Процедура ПроверитьИЗаполнитьВложенныеШтрихкоды(ДеревоУпаковок, ВложенныеШтрихкоды, АкцизныеМаркиВДокументе,
	                                            ОбработанныеШтрихкоды = Неопределено, ШтрихкодыУпаковок = Неопределено, ПараметрыСканированияАкцизныхМарок = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок Цикл
		...
		Если ОбработанныеШтрихкоды <> Неопределено 
			...
			Если ШтрихкодыУпаковок.Найти(СтрокаДерева.ШтрихкодУпаковки) <> Неопределено Тогда
				...
				ДанныеШтрихкода = АкцизныеМаркиКлиентСервер.СтруктураДанныхШтрихкода(СтрокаДерева.ЗначениеШтрихкода);
		...
		Если ПараметрыСканированияАкцизныхМарок <> Неопределено
			И Не ПараметрыСканированияАкцизныхМарок.ИспользуетсяСоответствиеШтрихкодовСтрокДерева Тогда
...
Функция ДанныеДокументаОснования(ПараметрыСканированияАкцизныхМарок) Экспорт
	
	Если ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.ДокументОснование) Тогда
		Если Не ЭтоАдресВременногоХранилища(ПараметрыСканированияАкцизныхМарок.АдресДанныхДокументаОснования) Тогда
			...
			ИнтеграцияЕГАИСПереопределяемый.СформироватьДанныеДокументаОснования(
			        ПараметрыСканированияАкцизныхМарок.ДокументОснование, 
			        ПараметрыСканированияАкцизныхМарок.ДокументЕГАИС,
			...
			ПараметрыСканированияАкцизныхМарок.АдресДанныхДокументаОснования = ПоместитьВоВременноеХранилище(
			       ТаблицаДанныхДокументаОснования,
			       ПараметрыСканированияАкцизныхМарок.ИдентификаторФормы);
			
		Иначе
			
			ТаблицаДанныхДокументаОснования = ПолучитьИзВременногоХранилища(ПараметрыСканированияАкцизныхМарок.АдресДанныхДокументаОснования);
...
Процедура ДобавитьКолонкиОшибокКДеревуУпаковок(ДеревоУпаковок) 
	
	ДеревоУпаковок.Колонки.Добавить("ЕстьОшибки",  Новый ОписаниеТипов("Булево"));
	ДеревоУпаковок.Колонки.Добавить("ТекстОшибки", ОбщегоНазначения.ОписаниеТипаСтрока(300));
...
Процедура ЗаполнитьСлужебныеРеквизиты(Форма, ИмяКолонкиКоличество = "Количество") Экспорт
	...
	ДанныеДляЗаполненияСлужебныхРеквизитов = ДанныеДляЗаполненияСлужебныхРеквизитов(Товары);
...
Функция ДанныеДляЗаполненияСлужебныхРеквизитов(Товары)
	...
	Если ДанныеДляЗаполненияСлужебныхРеквизитов <> Неопределено Тогда
		Возврат  ДанныеДляЗаполненияСлужебныхРеквизитов;
	КонецЕсли;
...
Функция ЗаполнитьТоварИзСтроки(ДокументОбъект, СтрокаТовара, ПараметрыЗаполнения, Количество)
	...
	Если ВыполнитьПоискПоНоменклатуре Тогда
		...
		Если ГруппироватьНоменклатуруПоТипуШтрихкода(ДокументОбъект)
			...
			ПараметрыОтбора.Вставить("ТипШтрихкода", ТипШтрихкода);
		КонецЕсли;
