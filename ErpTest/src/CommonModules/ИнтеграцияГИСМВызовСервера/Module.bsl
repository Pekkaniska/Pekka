...
// Возвращает  структуру, содержащую ИНН и КПП контрагента.
//
// Параметры:
...
Функция ПоместитьТекстСообщенияXMLВКонвертSOAP(ТекстСообщенияXML) Экспорт
	...
	ТекстСообщенияXMLБезОтступов = ИнтеграцияГосИС.ФорматироватьXML(ТекстСообщенияXML, Ложь);
...
Функция ТекстВходящегоСообщенияXML(КонвертSOAP) Экспорт
	...
	ИначеЕсли ТипЗнч(ЭлементDOM) = Тип("ЭлементDOM") Тогда
		...
		ЗаписьXML.УстановитьСтроку(ИнтеграцияГосИС.ПараметрыФорматированияXML(Ложь));
