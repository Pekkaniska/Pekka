...
Процедура РассчитатьИтоговыеПоказателиРеализации(Форма) Экспорт
	...
	Если УчетНДСУПКлиентСервер.ПродажаОблагаетсяНДС(Форма.Объект.НалогообложениеНДС) Тогда
...
Функция СформироватьТекстДокументыНаОсновании(Форма) Экспорт
	...
	ИначеЕсли Форма.Элементы.Найти("ТекстДокументыНаОснованииПодвал") <> Неопределено Тогда
		...
		Форма.ТекстДокументыНаОснованииПодвал = Новый ФорматированнаяСтрока(Форма.ТекстСчетаФактурыВыданные, Отступ, Форма.ТекстТТН);
