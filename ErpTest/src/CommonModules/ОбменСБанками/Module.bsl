	...
	Если СоставКоманд.Найти(ПолноеИмя) <> Неопределено Тогда
		...
		Если ЗначениеФОИспользоватьОбменСБанками Тогда
			...
			Если ЕстьПравоОбменаЭД Тогда
				...
				Если Результат.Следующий() Тогда // есть настройка обмена с письмом
					ЕстьПисьма = Истина;
				КонецЕсли;
			КонецЕсли;

			Если ЕстьПисьма Тогда
				КомандаЭДО = КомандыЭДО.Добавить();
				КомандаЭДО.Обработчик = "ОбменСБанкамиСлужебныйКлиент.СоздатьПисьмоВБанкНаОсновании";
				КомандаЭДО.СписокФорм = "";
				КомандаЭДО.Идентификатор = "ПисьмоВБанк";
				КомандаЭДО.Представление = НСтр("ru = 'Письмо в банк'");
				КомандаЭДО.Порядок = 5;
				КомандаЭДО.РежимИспользованияПараметра = РежимИспользованияПараметраКоманды.Одиночный;
				КомандаЭДО.МестоРазмещения = "ФормаСоздатьНаОсновании";
				КомандаЭДО.Недоступна = Не ЕстьПравоПросмотраЭД;
