...
// Процедура - Открыть форму мастера заявления на подключение
//
// Параметры:
//  Организация				 - СпровочникСсылка.Организация - Если указана, мастер открывается с отбором по этой организации.
//  ВладелецОткрываемойФормы - УправляемаяФорма - форма, которая будет указана в качестве владельца для открываемой формы мастера.
...
Процедура ОткрытьФормуЗаявления_ПослеЗакрытияФормыБублика() Экспорт
	...
	ПараметрыФормы = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(Результат.Параметры);
