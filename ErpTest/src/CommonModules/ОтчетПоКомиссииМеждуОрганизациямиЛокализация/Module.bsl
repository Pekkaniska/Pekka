...
#Область ТекстКомитентНДСНаКомиссионноеВознаграждение // (Дт 19.04 :: Кт 60.01)
	...
	|ВЫБРАТЬ //// НДС на комиссионное вознаграждение по рег. НДСПредъявленный (Дт 19.04 :: Кт 60.01)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#Область ТекстКомиссионерВыручкаКомиссионноеВознаграждение // (Дт 62.01 :: Кт 90.01.1)
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСКлиентами) КАК ВидСчетаДт,
	|	Операция.ГруппаФинансовогоУчетаПолучателя КАК ГруппаФинансовогоУчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	Операция.Валюта КАК ВалютаДт,
	|	Операция.Подразделение КАК ПодразделениеДт,
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	МАКСИМУМ(Расчеты.АналитикаУчетаПоПартнерам.Контрагент) КАК Контрагент,
	...
	|	ВЫБОР КОГДА Расчеты.Регистратор = Расчеты.ЗаказКлиента ТОГДА NULL ИНАЧЕ Расчеты.ЗаказКлиента.ГруппаФинансовогоУчетаПолучателя КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка";
