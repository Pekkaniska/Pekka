...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ16(ПараметрыПечати, ДокументОснование) Экспорт
	...
	Соглашение                        = ПредварительныеДанныеРезультат.Соглашение;
	...
	Запрос.УстановитьПараметр("ЭтоСклад",              ТипЗнч(Склад) = Тип("СправочникСсылка.Склады"));
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	СписаниеПринятыхНаХранениеТоваров.Ссылка   КАК Ссылка,
	|	КОЛИЧЕСТВО(ИнвентаризационнаяОпись.Ссылка) КАК Количество
	|
	|ПОМЕСТИТЬ КоличествоИнвентаризационныхОписей
	|ИЗ
	|	Документ.СписаниеПринятыхНаХранениеТоваров КАК СписаниеПринятыхНаХранениеТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИнвентаризационнаяОпись КАК ИнвентаризационнаяОпись
	|		ПО (СписаниеПринятыхНаХранениеТоваров.Дата МЕЖДУ ИнвентаризационнаяОпись.ДатаНачала И ИнвентаризационнаяОпись.ДатаОкончания)
	|			И СписаниеПринятыхНаХранениеТоваров.МестоХранения = ИнвентаризационнаяОпись.Склад
	|			И СписаниеПринятыхНаХранениеТоваров.Организация = ИнвентаризационнаяОпись.Организация
	|			И (ИнвентаризационнаяОпись.Проведен)
	|ГДЕ
	|	СписаниеПринятыхНаХранениеТоваров.Ссылка = &ТекущийДокумент
	|
	|СГРУППИРОВАТЬ ПО
	|	СписаниеПринятыхНаХранениеТоваров.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|			И КоличествоИнвентаризационныхОписей.Ссылка.МестоХранения = ИнвентаризационнаяОпись.Склад
	...
	|	ДокументСписания.Контрагент                                         КАК Поставщик,
	|	ДокументСписания.Соглашение                                         КАК Соглашение,
	...
	|	ДокументСписания.ХозяйственнаяОперация                              КАК ХозяйственнаяОперация,
	...
	|	ДокументСписания.МестоХранения                                      КАК Подразделение,
	|	ПРЕДСТАВЛЕНИЕ(ДокументСписания.МестоХранения)                       КАК ПодразделениеПредставление,
	|	ВЫБОР
	|		КОГДА &ЭтоСклад
	|			ТОГДА ДокументСписания.МестоХранения.ТекущийОтветственный
	|		ИНАЧЕ """"
	|	КОНЕЦ                                                               КАК Кладовщик,
	|	ВЫБОР
	|		КОГДА &ЭтоСклад
	|			ТОГДА ДокументСписания.МестоХранения.ТекущаяДолжностьОтветственного
	|		ИНАЧЕ """"
	|	КОНЕЦ                                                               КАК ДолжностьКладовщика,
	|	ДокументСписания.Менеджер.ФизическоеЛицо                            КАК Ответственный,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СписаниеПринятыхНаХранениеТоваров КАК ДокументСписания
	...
	ИначеЕсли ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
		...
		Если ПредварительныйРасчет Тогда
			...
			|ИЗ
			|	Документ.СписаниеПринятыхНаХранениеТоваров.ВидыЗапасов КАК СписываемыеВидыЗапасов
			...
			|	ЦеныНоменклатуры.СтоимостьРегл              КАК Цена
			|
			|ПОМЕСТИТЬ ЦеныНоменклатуры
			|ИЗ
		...
		Иначе
			...
			|			ТОГДА 0
			|		ИНАЧЕ СУММА(ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.СтоимостьРегл, 0)) / СУММА(СебестоимостьТоваровПоРегистратору.Количество)
			|	КОНЕЦ                                                КАК Цена
			|
...
Функция ТаблицаКурсовВалют(ДокументОснование)
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СписаниеПринятыхНаХранениеТоваров.Ссылка                    КАК Ссылка,
	|	НАЧАЛОПЕРИОДА(СписаниеПринятыхНаХранениеТоваров.Дата, ДЕНЬ) КАК Дата,
	|	СписаниеПринятыхНаХранениеТоваров.ВидЦеныПоставщика.Валюта  КАК Валюта
	|ИЗ
	|	Документ.СписаниеПринятыхНаХранениеТоваров КАК СписаниеПринятыхНаХранениеТоваров
	|ГДЕ
	|	СписаниеПринятыхНаХранениеТоваров.Ссылка = &ДокументОснование
	|	И СписаниеПринятыхНаХранениеТоваров.ВидЦеныПоставщика.Валюта <> &ВалютаРегламентированногоУчета
	|
	|УПОРЯДОЧИТЬ ПО
	|	Валюта,
	|	Дата";
...
#Область СписаниеТоваровПринятыхНаОтветХранение                    // (Дт  :: Кт 002)
	ТекстЗапроса = "
	...
	|
	|	ВЫБОР КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровПереданныхВПроизводство)
