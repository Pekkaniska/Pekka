...
Процедура СоздатьВТДосрочноеНазначениеПенсииДляСЗВ_СТАЖ(МенеджерВременныхТаблиц, Организация, Год, ЗаписиОСтаже) Экспорт
	...
	ОснованияВыслугиЛетСотрудников.Колонки.Добавить("ОсобыеУсловияТруда", Новый ОписаниеТипов("СправочникСсылка.ОсобыеУсловияТрудаПФР,СправочникСсылка.ОсобыеУсловияТрудаДляСЗВКПФР,СправочникСсылка.ОснованияДосрочногоНазначенияПенсии,СправочникСсылка.ОснованияДосрочногоНазначенияПенсииДляСЗВК"));
	...
	Для Каждого СтрокаСотрудника Из ЗаписиОСтаже Цикл 
		Если Не ЗначениеЗаполнено(СтрокаСотрудника.ОсобыеУсловияТруда)
			...
			Продолжить;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаСотрудника.ОсобыеУсловияТруда) Тогда 
			ЗаполнитьЗначенияСвойств(ОснованияВыслугиЛетСотрудников.Добавить(), СтрокаСотрудника);
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаСотрудника.ОснованиеВыслугиЛет) Тогда
			НоваяСтрока = ОснованияВыслугиЛетСотрудников.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСотрудника);
			НоваяСтрока.ОсобыеУсловияТруда = СтрокаСотрудника.ОснованиеВыслугиЛет;
		КонецЕсли;
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьШтатноеРасписание") Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	КадровыеДанные.Подразделение,
		               |	КадровыеДанные.Должность,
		               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КадровыеДанные.ФизическоеЛицо),
		               |	ОснованияВыслугиЛет.ОсобыеУсловияТруда,
		               |	ОснованияВыслугиЛет.КодПозицииСписка
		               |ИЗ
		               |	ВТКадровыеДанные КАК КадровыеДанные
		               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОснованияВыслугиЛет КАК ОснованияВыслугиЛет
		               |		ПО КадровыеДанные.ФизическоеЛицо = ОснованияВыслугиЛет.ФизическоеЛицо
		               |			И КадровыеДанные.ОснованиеДосрочногоНазначенияПенсии = ОснованияВыслугиЛет.ОсобыеУсловияТруда
		               |
		               |СГРУППИРОВАТЬ ПО
		               |	КадровыеДанные.Подразделение,
		               |	КадровыеДанные.Должность,
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ПозицииШтатногоРасписания.Подразделение,
	               |	ПозицииШтатногоРасписания.Должность,
	               |	СУММА(ПозицииШтатногоРасписания.КоличествоСтавок),
...
Процедура ЗаполнитьДанныеДосрочногоНазначенияПенсии(Объект, Организация, ОтчетныйПериод) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьШтатноеРасписание") Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ДанныеСотрудников.КоличествоРаботающих,
		               |	ДанныеСотрудников.КоличествоРаботающих,
