...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	СоотвествиеДанных = РегламентированнаяОтчетностьУП.СоответствиеИменПолейОрганизации();
	
	Если ТипЗнч(СписокПоказателей) = Тип("СписокЗначений") Тогда
	
		Для каждого ИмяПоля Из СписокПоказателей Цикл
			Ключ = СоотвествиеДанных.Получить(ИмяПоля.Представление);
			Если Ключ <> Неопределено Тогда
				НовыеИмена.Добавить(Ключ);
				Продолжить;
			КонецЕсли;
			
		КонецЦикла;
		Поля = СтрСоединить(НовыеИмена, ",");
	
	КонецЕсли;
	
	Сведения = ОрганизацииСервер.СведенияОбОрганизации(Организация, Поля, ДатаЗначения);
	
	Возврат РегламентированнаяОтчетностьУП.КонвертироватьСведенияОбОрганизации(Сведения);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПрибыль" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2015Кв1" ИЛИ ИДРедакцииОтчета = "ФормаОтчета2016Кв4" Тогда	
			...
			Если КонтекстРасчета <> Неопределено Тогда
				...
				// часть показателей заполняется модулем НалоговыйУчет
				НалоговыйУчет.ЗаполнитьОтчетПрибыльФормаОтчета2015Кв1(
					Контейнер,
					ТаблицаРасшифровки,
					ИДРедакцииОтчета,
					КонтекстРасчета);
			...
			// Так как расшифровка составляется разными алгоритмами, то в общем случае порядок показателей в ней может оказаться
			// непонятным для пользователя. Поэтому отсортируем показатели расшифровки по возрастанию их номеров ячеек.
...
Функция ЭтоИностраннаяОрганизация(Организация) Экспорт
	...
	РегламентированнаяОтчетностьУП.ПолучитьЗначениеРеквизитаОрганизации(Организация, "ИностраннаяОрганизация", Значение, Ложь);
...
//
//  Возврат КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудники.ВыгрузитьКолонку("Сотрудник"),
//  "Фамилия,Имя,Отчество,СтраховойНомерПФР", ДатаЗапроса);
//
...
// ЭлектронныеПредставленияРегламентированныхОтчетов), соответствующий данному типу объекта. Неопределено, если такого
// вида отправляемого документа не существует.
//
