...
// Производится запуск выполнения фонового проведения.
//
// Параметры:
...
//
// Параметры:
//  МенеджерВременныхТаблиц	 - МенеджерВременныхТаблиц - менеджер с таблицей ВтДатыОтсчета,
//		содержащей данные для расчета. Колонки:
...
//  СценарийПланирования - СправочникСсылка.СценарииТоварногоПланирования - сценарий, календарь которого
//		используется для расчета дат запуска.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - таблица с колонками ДатаВыпуска, ЧислоДней, ДатаЗапуска (результат расчета).
//
Функция ПолучитьДатыПоГрафику(МенеджерВременныхТаблиц, СценарийПланирования, ИскатьВперед) Экспорт
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	&Календарь КАК Календарь,
		|	ВтДатыОтсчета.ДатаОтсчета КАК ДатаОтсчета,
		|	ВтДатыОтсчета.ЧислоДней КАК ЧислоДней
		|ПОМЕСТИТЬ ВтПараметрыПоиска
		|ИЗ
		|	ВтДатыОтсчета КАК ВтДатыОтсчета
		|;
		|"
		+ ОбеспечениеСервер.ТекстЗапросаДатГрафика(, ИскатьВперед)
		+ "
		|ВЫБРАТЬ
		|	Т.ДатаОтсчета КАК ДатаОтсчета,
		|	Т.ЧислоДней КАК ЧислоДней,
		|	ЕСТЬNULL(ДатыГрафика.Дата, ДОБАВИТЬКДАТЕ(Т.ДатаОтсчета, ДЕНЬ, Т.ЧислоДней * &К)) КАК ДатаПоГрафику
		|ИЗ
	...
	Запрос.УстановитьПараметр("К", ?(ИскатьВперед, 1, -1));
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	Если ЗначениеЗаполнено(СценарийПланирования) Тогда
		Календарь = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СценарийПланирования, "Календарь");
