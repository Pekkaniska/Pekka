...
Процедура ОтметитьДокументКОтражениюВБюджетированииПриЗаписиНабораЗаписей(Источник, Отказ, РежимЗаписи) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Бюджетирование.Отражение факта'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Для Каждого ОписаниеПравила Из ТаблицаПравил Цикл
		...
		Если ИменаВидовДокументов = Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru= 'Не выполнено правило бюджетирования'", КодОсновногоЯзыка),,,
				ОписаниеПравила.Правило,
				НСтр("ru = 'Не выполнено правило бюджетирования, т.к. не определена СКД получения данных'", КодОсновногоЯзыка));
			Продолжить;
		КонецЕсли;
