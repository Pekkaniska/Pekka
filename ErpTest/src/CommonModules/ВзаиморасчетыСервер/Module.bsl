...
Функция ПолучитьТекстЗапросаДокументыРасчетовСПоставщиками(ПараметрыОтбора) Экспорт
	...
	//++ НЕ УТ
	Если ПравоДоступа("Чтение", Метаданные.Документы.ОтчетОператораСистемыПлатон) Тогда
		ТекстЗапроса = ТекстЗапроса + "ОБЪЕДИНИТЬ ВСЕ" +
		"
		|ВЫБРАТЬ
		|	ДанныеДокумента.Ссылка КАК Ссылка,
		...
		ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(
			ТекстЗапроса, Тип("ДокументСсылка.ОтчетОператораСистемыПлатон"), ПараметрыОтбора);
	...
	//++ НЕ УТ
	Если ПравоДоступа("Чтение", Метаданные.Документы.ВыкупПринятыхНаХранениеТоваров) Тогда
		ТекстЗапроса = ТекстЗапроса + "ОБЪЕДИНИТЬ ВСЕ" + "
		|ВЫБРАТЬ
		|	ДанныеДокумента.Ссылка КАК Ссылка,
		|	ДанныеДокумента.Дата КАК Дата,
		...
		ДополнитьТекстЗапросаДокументыРасчетовСПоставщиками(
			ТекстЗапроса, Тип("ДокументСсылка.ВыкупПринятыхНаХранениеТоваров"), ПараметрыОтбора);
...
Функция ВидДоговораКонтрагента(ДокументРасчетов, Соглашение, ХозяйственнаяОперация) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументРасчетов) Тогда
		...
		ИначеЕсли ТипЗнч(ДокументРасчетов) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов") Тогда
			...
			ВидДоговора = "СПоставщиком";
		//++ НЕ УТ	
		ИначеЕсли ТипЗнч(ДокументРасчетов) = Тип("ДокументСсылка.ОтчетОператораСистемыПлатон") Тогда
...
//
// Параметры:
//	Форма					- УправляемаяФорма - Форма инициатор заполнения.
...
Функция ТекстПроведенияЗачетАвансовКлиентаПоНакладной(ТипДокументаСтрокой, Шаблон, ИмяТЧ = "РасшифровкаПлатежа", ТекстХозяйственнойОперацииПереноса = "") Экспорт
...
Функция ТекстПроведенияЗачетАвансовПоставщикуПоНакладной(ТипДокументаСтрокой, Шаблон, ИмяТЧ = "РасшифровкаПлатежа") Экспорт
...
Функция ПараметрыОтбораПриВыбореДокументаРасчетовСПоставщиками() Экспорт
	...
	ПараметрыОтбора.Вставить("ЗапретитьДоговорыПоДокументам",  Ложь);
	//++ НЕ УТ
	ПараметрыОтбора.Вставить("ПлатежиПо275ФЗ",                 Ложь);
	//-- НЕ УТ
...
Процедура ЗаполнитьТаблицуОстатковРасчетов(Реквизиты, АдресПлатежейВХранилище, ТаблицаОстатковРасчетов, УчитыватьФилиалы = Истина) Экспорт
	...
	//++ НЕ УТ
	Если Реквизиты.Свойство("ПлатежиПо275ФЗ") И Реквизиты.ПлатежиПо275ФЗ Тогда
		
		СхемаЗапросаОстатков = Новый СхемаЗапроса;
		СхемаЗапросаОстатков.УстановитьТекстЗапроса(Запрос.Текст);
		
		ОтборПлатежиПо275ФЗ = "ЕСТЬNULL(АналитикаПоПартнерам.Договор.ПлатежиПо275ФЗ, ЛОЖЬ)";
		Оператор = СхемаЗапросаОстатков.ПакетЗапросов[2].Операторы[0];
		Оператор.Отбор.Добавить(ОтборПлатежиПо275ФЗ);
		
		Если Реквизиты.Свойство("ДоговорСУчастникомГОЗ") Тогда
			Если Реквизиты.ДоговорСУчастникомГОЗ Тогда
				ОтборДоговорСУчастникомГОЗ = "ЕСТЬNULL(АналитикаПоПартнерам.Договор.ДоговорСУчастникомГОЗ, ЛОЖЬ)";
			Иначе
				ОтборДоговорСУчастникомГОЗ = "НЕ ЕСТЬNULL(АналитикаПоПартнерам.Договор.ДоговорСУчастникомГОЗ, ИСТИНА)";
			КонецЕсли;
			Оператор.Отбор.Добавить(ОтборДоговорСУчастникомГОЗ);
		КонецЕсли;
	...
	Для Каждого ВыбранноеПоле Из МассивВыбранныхПолей Цикл
		Если ВычисляемыеПоля.Найти(Строка(ВыбранноеПоле.Поле)) = Неопределено Тогда
			ТекстОтборов = ТекстОтборов + ?(ТекстОтборов = "","", "ИЛИ") +"
				|	"+ИмяТаблицы+"." + Строка(ВыбранноеПоле.Поле) + " <> 0 ";
		КонецЕсли;
	КонецЦикла;
