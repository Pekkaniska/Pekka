...
//
// Параметры:
//  ТаблицаРеквизиты - ТаблицаЗначений - Реквизиты документа.
//  Отказ			 - Булево	 - Флаг отказа проведения.
//  СписокОшибок	 - Массив	 - Ошибки, которые обнаружены при расчете налога.
// 
// Возвращаемое значение:
...
//
Функция ПодготовитьТаблицыРасчетНалога(ТаблицаРеквизиты, Отказ, СписокОшибок = Неопределено) Экспорт
	...
	ТаблицыРасчетНалога = ТаблицыРасчетНалога(Реквизиты);
...
// Определяет расходы на Платон.
//
// Параметры:
//  Организация						 - СправочникСсылка.Организации -  Организация, для которой выполняется расчет транспортного налога.
//  ПериодРасчета					 - Дата - Дата закрытия месяца.
//  ТаблицаРасчетТранспортногоНалога - ТаблицаЗначений - Содержит расчет транспортного налога.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Содержит расходы на Платон.
//
Функция ПолучитьРасходыНаПлатон(Организация, ПериодРасчета, ТаблицаРасчетТранспортногоНалога) Экспорт

	ТаблицаРасходыНаПлатон = СформироватьТаблицуРасходовНаПлатон(Организация, ПериодРасчета, ТаблицаРасчетТранспортногоНалога);

	УчестьРасходыНаПлатонВРасчетеТранспортногоНалога(ПериодРасчета, ТаблицаРасходыНаПлатон, ТаблицаРасчетТранспортногоНалога);
...
Процедура ДополнитьПараметрыРасчетаНалогаНаИмущество(Организация, ПериодРасчета, ДополнительныеПараметрыРасчета, ПараметрыРасчета) Экспорт

	Если ДополнительныеПараметрыРасчета <> Неопределено Тогда
		ПараметрыРасчета.МенеджерВременныхТаблиц = ДополнительныеПараметрыРасчета;
...
Процедура СоздатьВТКоэффициентыЕНВД(ПараметрыРасчета) Экспорт
	...
	|	СпособыОтраженияРасходовПоАмортизации.СтатьяРасходов КАК СтатьяРасходов,
	|	СпособыОтраженияРасходовПоАмортизации.СпособОтраженияРасходовЗаданДокументом КАК СпособОтраженияРасходовЗаданДокументом,
	|	СпособыОтраженияРасходовПоАмортизации.СпособОтраженияРасходов КАК СпособОтраженияРасходов
	...
	|	ПорядокУчетаОСБУ.СтатьяРасходов,
	|	ПорядокУчетаОСБУ.СпособОтраженияРасходов <> ЗНАЧЕНИЕ(Документ.ИзменениеПараметровОС2_4.ПустаяСсылка),
	|	ПорядокУчетаОСБУ.СпособОтраженияРасходов
	|ИЗ
...
Процедура СоздатьВТДвижимоеИмуществоПринятоеКУчетуПосле2013(ПараметрыРасчета) Экспорт
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(ПараметрыРасчета.ПериодРасчета) Тогда
	...
	Иначе	
		...
		|				(Организации.Ссылка = &ГоловнаяОрганизация
		|					ИЛИ Организации.ГоловнаяОрганизация = &ГоловнаяОрганизация))
		|	И НЕ СостоянияОСОрганизаций.ОсновноеСредство.ГруппаОС В (
		|				ЗНАЧЕНИЕ(Перечисление.ГруппыОС.Здания), 
		|				ЗНАЧЕНИЕ(Перечисление.ГруппыОС.Сооружения), 
		|				ЗНАЧЕНИЕ(Перечисление.ГруппыОС.МноголетниеНасаждения), 
		|				ЗНАЧЕНИЕ(Перечисление.ГруппыОС.ЗемельныеУчастки), 
		...
		|
		|ИМЕЮЩИЕ
		|	МИНИМУМ(СостоянияОСОрганизаций.Период) >= ДАТАВРЕМЯ(2013, 1, 1)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОС";
...
Процедура ДобавитьТекстЗапросаНеСнятыеСУчетаОС(ПараметрыРасчета, СписокЗапросов) Экспорт

	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	СостоянияОС.ОсновноеСредство КАК ОсновноеСредство,
	|	СостоянияОС.ОсновноеСредство.ГруппаОС КАК ГруппаОС,
	|	СостоянияОС.ОсновноеСредство.НедвижимоеИмущество КАК НедвижимоеИмущество
	|ПОМЕСТИТЬ ВТ_НеСнятыеСУчетаОС
	|ИЗ
	...
	Если ПараметрыРасчета.РасчетПоФактическойСтоимости Тогда
		ТекстОтбораОС = "И ОсновноеСредство В
		|					(ВЫБРАТЬ
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ОТБОР_ОС//", ТекстОтбораОС);
	КонецЕсли; 
...
Процедура ДобавитьТекстЗапросаСтоимостьИАмортизация(Период, ПараметрыРасчета, СписокЗапросов) Экспорт
	...
	Если ВнеоборотныеАктивы.ИспользуетсяУправлениеВНА_2_4(ДобавитьМесяц(Период, -1)) Тогда
		...
		Если Месяц(ПараметрыРасчета.мДатаКонцаПериодаОтчета) + 1 = Месяц(Период) 
			...
			// Это расчет в выбранном месяце, в нем еще не выполнен расчет стоимости поступивших ОС
			...
			|ГДЕ
			|	ПринятиеКУчетуОС.Организация = &Организация
			...
			|ОБЪЕДИНИТЬ ВСЕ
			|
			// Уменьшение стоимости исходного ОС при разукомплектации.
			|ВЫБРАТЬ
			|	-СтоимостьОС.СтоимостьБУ КАК СтоимостьРегл,
			|	РазукомплектацияОС.ОсновноеСредство КАК ОсновноеСредство
			|ИЗ
			|	Документ.РазукомплектацияОС КАК РазукомплектацияОС
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РазукомплектацияОС.ОС КАК СтоимостьОС
			|		ПО СтоимостьОС.Ссылка = РазукомплектацияОС.Ссылка
			|ГДЕ
			|	РазукомплектацияОС.Организация = &Организация
			...
			|ОБЪЕДИНИТЬ ВСЕ
			|
			// Стоимости новых ОС при разукомплектации.
			|ВЫБРАТЬ
			|	СтоимостьОС.СтоимостьБУ КАК СтоимостьРегл,
			|	СтоимостьОС.ОсновноеСредство КАК ОсновноеСредство
			|ИЗ
			...
			|ГДЕ
			|	МодернизацияОС2_4.Организация = &Организация
			...
			|		И СтоимостьОС.Регистратор ССЫЛКА Документ.СписаниеОС2_4
			|		И СтоимостьОС.ОсновноеСредство В
			|						(ВЫБРАТЬ
			|							СписокОС.ОсновноеСредство
			|						ИЗ
			...
			|	ИЗ
			|		СписокОС_РасчетПоПредварительнойСтоимости КАК СтоимостьОС) КАК СписокОС
			|
			|СГРУППИРОВАТЬ ПО
			|	СписокОС.ОсновноеСредство
			|
			...
			|		ПО (ПредварительнаяСтоимостьОС.ОсновноеСредство = СтоимостьОС.ОсновноеСредство)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	СтоимостьОС.ОсновноеСредство
			|;
			...
			|		И АмортизацияОС.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РазукомплектацияОС)) КАК АмортизацияОС
			|
			|СГРУППИРОВАТЬ ПО
			|	АмортизацияОС.ОсновноеСредство
			|
			...
			|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(АмортизацияОС.СуммаОстатокКт, 0) * ТаблицаОС.СтоимостьБУ / ЕСТЬNULL(СтоимостьОС.СтоимостьРегл, 0) КАК ЧИСЛО (15, 2))
			|		ИНАЧЕ 0
			...
			|		ПО СтоимостьОС.ОсновноеСредство = РазукомплектацияОС.ОсновноеСредство
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_АмортизацияОС_НаНачалоМесяца КАК АмортизацияОС
			|		ПО АмортизацияОС.ОсновноеСредство = РазукомплектацияОС.ОсновноеСредство
			|ГДЕ
			|	РазукомплектацияОС.Организация = &Организация
			|	И РазукомплектацияОС.Проведен
			|	И РазукомплектацияОС.ОтражатьВРеглУчете
...
Процедура ДобавитьТекстЗапросаИсторияРегистрацийВНалоговомОргане(СписокЗапросов) Экспорт
	...
	|ИЗ
	|	РегистрСведений.ИсторияРегистрацийВНалоговомОргане.СрезПоследних(&КонецПериода, СтруктурнаяЕдиница = &Организация) КАК ИсторияРегистрацийВНалоговомОргане";
...
Функция РасходыПоНалогу(Реквизиты, СправкаРасчет, РасходыНаПлатон, СписокОшибок, Отказ)
	...
	|	СправкаРасчет.СуммаНалогаКУплате * &КоэффициентПересчетаВУпр КАК СуммаБезНДС,
	|	СправкаРасчет.СуммаНалогаКУплате * &КоэффициентПересчетаВУпр КАК Сумма,
	|	ВЫБОР КОГДА &УправленческийУчетОрганизаций
	|		ТОГДА СправкаРасчет.СуммаНалогаКУплате * &КоэффициентПересчетаВУпр
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаУпр
	...
	|
	|	ВЫБОР 
	|		КОГДА &Период >= &ДатаПрекращенияДействияЛьготПлатон
	|			ТОГДА РасходыНаПлатон.СуммаРегл
	|		КОГДА РасходыНаПлатон.СуммаРегл > РасходыНаПлатон.СуммаНалога
	|			ТОГДА РасходыНаПлатон.СуммаРегл - РасходыНаПлатон.СуммаНалога 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК СуммаНУ,
	|
	|	ВЫБОР 
	...
	|			ТОГДА 0
	|		КОГДА РасходыНаПлатон.СуммаРегл > РасходыНаПлатон.СуммаНалога
	|			ТОГДА РасходыНаПлатон.СуммаНалога 
	|		ИНАЧЕ РасходыНаПлатон.СуммаРегл 
	|	КОНЕЦ КАК ПостояннаяРазница,
	|
	|	-РасходыНаПлатон.СуммаРегл КАК ВременнаяРазница
	|
	...
	Запрос.УстановитьПараметр("УправленческийУчетОрганизаций",
		УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.УправленческийУчетОрганизаций(НачалоМесяца(Реквизиты.Период)));
...
Функция ТаблицаРасходовПоНалогу(ВидНалога, ВыборкаРасходыБезРаспределения, ВыборкаРасходыСРаспределением, Отказ, СписокОшибок = Неопределено) Экспорт
	...
	Если ВыборкаРасходыСРаспределением <> Неопределено Тогда
		...
		Пока ВыборкаРасходыСРаспределением.Следующий() Цикл
			...
			Для Индекс = 0 По ТаблицаРаспределения.Количество()-1 Цикл
				...
				// ПрочиеРасходы
				Запись = ПрочиеРасходы.Добавить();
				ЗаполнитьЗначенияСвойств(Запись, ВыборкаРасходыСРаспределением, "Период, Организация, РегистрацияВНалоговомОргане");
	...
	КолонкиГруппировки = "Период, ВидДвижения, Организация, Подразделение, НаправлениеДеятельности, СтатьяРасходов, АналитикаРасходов, РегистрацияВНалоговомОргане, ХозяйственнаяОперация";
	КолонкиСуммирования = "Сумма, СуммаБезНДС, СуммаУпр, СуммаРегл, ПостояннаяРазница, ВременнаяРазница";
...
Функция ТаблицыРасчетНалога(Реквизиты)
	...
	Если Реквизиты.ВидНалога = Перечисления.ВидыИмущественныхНалогов.НалогНаИмущество Тогда
		...
		РасчетПоНалогу = РасчетИмущественныхНалогов.ПолучитьРасчетПоНалогуНаИмущество(Реквизиты.Организация, Реквизиты.Период);
		
	ИначеЕсли Реквизиты.ВидНалога = Перечисления.ВидыИмущественныхНалогов.ТранспортныйНалог Тогда
		
		РасчетыПоНалогу = РасчетИмущественныхНалогов.ПолучитьРасчетПоТранспортномуНалогу(Реквизиты.Организация, Реквизиты.Период);
	...
	ИначеЕсли Реквизиты.ВидНалога = Перечисления.ВидыИмущественныхНалогов.ЗемельныйНалог Тогда
		...
		РасчетПоНалогу = РасчетИмущественныхНалогов.ПолучитьРасчетПоЗемельномуНалогу(Реквизиты.Организация, Реквизиты.Период);
...
Функция ПустаяТаблицаРасходыНаПлатон() Экспорт
	...
	РасходыНаПлатон.Колонки.Добавить("СуммаРегл", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	РасходыНаПлатон.Колонки.Добавить("СуммаНалога", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	РасходыНаПлатон.Колонки.Добавить("База", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
...
Функция ПустаяСправкаРасчет(ИмяРегистраСведений) Экспорт
	...
	СправкаРасчет = РасчетИмущественныхНалогов.ПустаяСправкаРасчет(ИмяРегистраСведений);
...
Функция СформироватьТаблицуРасходовНаПлатон(Организация, ПериодРасчета, ТаблицаРасчетТранспортногоНалога)
		...
		Пока ВыборкаПлатежи.Следующий() Цикл
			...
			Для Сч = 0 По КоэффициентыОплаты.ВГраница() Цикл
				...
				Если Расход.УплачиваютсяАвансы Тогда
					Расход.СуммаРегл = ?(РаспределениеПлатежаЗаКвартал <> Неопределено, РаспределениеПлатежаЗаКвартал[Сч], 0);
				Иначе
					Расход.СуммаРегл = РаспределениеПлатежа[Сч];
				КонецЕсли; 
				...
				Если ГодовойРасчет Тогда
					...
					Для каждого СтрокаУчтенныеПлатежи Из СписокСтрокУчтенныхПлатежей Цикл
						Если НачалоКвартала(СтрокаУчтенныеПлатежи.Период) = ВыборкаПлатежи.Период
							...
							СуммаПлатежа = Мин(Расход.СуммаРегл, СтрокаУчтенныеПлатежи.СуммаПлатежа);
							Расход.СуммаРегл = Расход.СуммаРегл - СуммаПлатежа;
							СтрокаУчтенныеПлатежи.СуммаПлатежа = СтрокаУчтенныеПлатежи.СуммаПлатежа - СуммаПлатежа;
						КонецЕсли; 
					КонецЦикла; 
				КонецЕсли; 
				
				Если Расход.СуммаРегл = 0 Тогда
...
Процедура УчестьРасходыНаПлатонВРасчетеТранспортногоНалога(ПериодРасчета, ТаблицаРасходыНаПлатон, ТаблицаРасчетТранспортногоНалога)
	...
	ТаблицаРасчетТранспортногоНалога.Индексы.Добавить("ЗарегистрированоВРеестреСистемыПлатон");
		
	СтруктураПоиска = Новый Структура("ЗарегистрированоВРеестреСистемыПлатон", Истина);
	СтрокиРасчетаНалога = ТаблицаРасчетТранспортногоНалога.НайтиСтроки(СтруктураПоиска);
	
	Для Каждого СтрокаРасчетаНалога Из СтрокиРасчетаНалога Цикл
		
		СуммаНалогаКУплате = СтрокаРасчетаНалога.СуммаНалогаКУплате - СтрокаРасчетаНалога.СуммаНалоговогоВычета;
		...
		Если ТаблицаРасходыНаПлатон.Количество() <> 0 Тогда
			
			СтруктураПоиска = Новый Структура("ОсновноеСредство");
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, СтрокаРасчетаНалога);
			СписокСтрок = ТаблицаРасходыНаПлатон.НайтиСтроки(СтруктураПоиска);
			КоэффициентыОплаты = Новый Массив;
			Для каждого Расход Из СписокСтрок Цикл
				КоэффициентыОплаты.Добавить(Расход.СуммаРегл);
				СтрокаРасчетаНалога.СуммаПлатыОператоруСистемыПлатон = 
				СтрокаРасчетаНалога.СуммаПлатыОператоруСистемыПлатон + Расход.СуммаРегл;
			КонецЦикла;
			
			РаспределениеНалога = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(
			СуммаНалогаКУплате, КоэффициентыОплаты);
			
			Для Сч = 0 По КоэффициентыОплаты.ВГраница() Цикл
				
				Расход = СписокСтрок[Сч];
				Расход.СуммаНалога = РаспределениеНалога[Сч];
				
				Если Расход.СуммаНалога > Расход.СуммаРегл Тогда
					СтрокаРасчетаНалога.СуммаНалоговогоВычета = СтрокаРасчетаНалога.СуммаНалоговогоВычета + Расход.СуммаРегл;
				Иначе
					СтрокаРасчетаНалога.СуммаНалоговогоВычета = СтрокаРасчетаНалога.СуммаНалоговогоВычета + Расход.СуммаНалога;
				КонецЕсли; 
		...
		Если ГодовойРасчет Тогда
			СтрокаРасчетаНалога.СуммаНалогаКУплате = СтрокаРасчетаНалога.СуммаНалогаКУплате - СтрокаРасчетаНалога.СуммаНалоговогоВычета;
		Иначе
			СтрокаРасчетаНалога["СуммаАвансовыхПлатежей" + Формат(ПериодРасчета, "ДФ='q''Кв'''")] = 0;
			СтрокаРасчетаНалога.СуммаНалогаКУплате = 0;
		КонецЕсли;
...
//
Процедура СформироватьЗаданиеКРасчетуНалогаНаИмущество(ДокументСсылка, ДанныеТаблиц) Экспорт

	ТипОбъекта = ТипЗнч(ДокументСсылка);
	
	ЕстьОсновныеСредства = (ТипОбъекта <> Тип("ДокументСсылка.РегламентнаяОперация")
							И ДокументСсылка <> Неопределено);
								
	ПроверитьЧтоОСПринятоКУчету = (ТипОбъекта = Тип("ДокументСсылка.ВводОстатковВнеоборотныхАктивов")
									ИЛИ ТипОбъекта = Тип("ДокументСсылка.ВводОстатковВнеоборотныхАктивов2_4")
									ИЛИ ТипОбъекта = Тип("ДокументСсылка.ПринятиеКУчетуОС")
									ИЛИ ТипОбъекта = Тип("ДокументСсылка.РегистрацияТранспортныхСредств"));
									
	ПринятиеКУчетуЗаданоДокументом = (ТипОбъекта = Тип("ДокументСсылка.ВводОстатковВнеоборотныхАктивов")
										ИЛИ ТипОбъекта = Тип("ДокументСсылка.ВводОстатковВнеоборотныхАктивов2_4")
										ИЛИ ТипОбъекта = Тип("ДокументСсылка.ПринятиеКУчетуОС"));
	...
	ТекстОбъединенияДанных = ВнеоборотныеАктивы.СформироватьТекстОбъединенияДанныхДляФормированияЗадания(
									СписокТаблиц, ДанныеТаблиц, СписокДопПолей);
	...
	Если ЕстьОсновныеСредства Тогда
		...
		|		ИЛИ ОбъектыЭксплуатации.НедвижимоеИмущество)
		|	И НЕ ОбъектыЭксплуатации.ГруппаОС В (
		|						ЗНАЧЕНИЕ(Перечисление.ГруппыОС.ЗемельныеУчастки), 
		|						ЗНАЧЕНИЕ(Перечисление.ГруппыОС.ОбъектыПриродопользования))
		|
		|ИНДЕКСИРОВАТЬ ПО
	...
	Если ПроверитьЧтоОСПринятоКУчету Тогда
		...
		Если ПринятиеКУчетуЗаданоДокументом Тогда
			...
			Если ДанныеТаблиц.МенеджерВременныхТаблиц.Таблицы.Найти("СостоянияОСОрганизаций") <> Неопределено Тогда
...
//
Процедура СформироватьЗаданиеКРасчетуТранспортногоНалога(ДокументСсылка, ДанныеТаблиц, ЕстьОсновныеСредства = Неопределено) Экспорт

	ТипОбъекта = ТипЗнч(ДокументСсылка);
	
	ЕстьОсновныеСредства = (ТипОбъекта <> Тип("ДокументСсылка.РегламентнаяОперация")
							И ДокументСсылка <> Неопределено
							ИЛИ ЕстьОсновныеСредства <> Неопределено И ЕстьОсновныеСредства);
	
	ПроверитьЧтоОСЗарегистрировано = 
		(ТипОбъекта <> Тип("ДокументСсылка.ОтменаРегистрацииТранспортныхСредств")
			И ТипОбъекта <> Тип("ДокументСсылка.РегистрацияТранспортныхСредств"));
...
//  ДанныеТаблиц	 - Структура		 - Содержит список таблиц в которых произошли изменения.
//
Процедура СформироватьЗаданиеКРасчетуЗемельногоНалога(ДокументСсылка, ДанныеТаблиц) Экспорт
	...
	ПроверитьЧтоОСЗарегистрировано = 
		(ТипОбъекта <> Тип("ДокументСсылка.ОтменаРегистрацииЗемельныхУчастков")
			И ТипОбъекта <> Тип("ДокументСсылка.РегистрацияЗемельныхУчастков"));
...
// Определяет необходимость расчета налога на имущество в указанном периоде.
//
// Параметры:
...
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется расчет налога на имущество.
//
Функция ТребуетсяРасчетНалогаНаИмущество(Организация, Период) Экспорт
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&НачалоПериода, Организация = &Организация) КАК Т
	...
	|				ИЛИ &НачалоПериода >= ДАТАВРЕМЯ(2015, 1, 1))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
...
// Определяет необходимость расчета земельного налога в указанном периоде.
//
// Параметры:
//  Организация	 - СправочникСсылка.Организации - Организация для которой требуется определить необходимость расчета земельного налога.
//  Период		 - Дата - Период в котором нужно проверить необходимость расчета.
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется расчет земельного налога.
//
Функция ТребуетсяРасчетЗемельногоНалога(Организация, Период) Экспорт
	...
	|	1
	|ИЗ
	|	РегистрСведений.РегистрацияЗемельныхУчастков КАК Регистрация
	|ГДЕ
	|	Регистрация.ВключатьВНалоговуюБазу
	|	И Регистрация.Период МЕЖДУ НАЧАЛОПЕРИОДА(&НачалоПериода, ГОД) И &ОкончаниеПериода
	|	И Регистрация.Организация = &Организация";
...
// Определяет необходимость расчета транспортного налога в указанном периоде.
//
// Параметры:
//  Организация	 - СправочникСсылка.Организации - Организация для которой требуется определить необходимость расчета транспортного налога.
//  Период		 - Дата - Период в котором нужно проверить необходимость расчета.
// 
// Возвращаемое значение:
//  Булево - Истина, если требуется расчет транспортного налога.
//
Функция ТребуетсяРасчетТранспортногоНалога(Организация, Период) Экспорт
	...
	|					ИЗ
	|						Организации)
	|				И ВключатьВНалоговуюБазу) КАК РегистрацияТC
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	1
	|ИЗ
	|	РегистрСведений.РегистрацияТранспортныхСредств КАК РегистрацияТC
	|ГДЕ
	|	РегистрацияТC.ВключатьВНалоговуюБазу
	|	И РегистрацияТC.Период МЕЖДУ НАЧАЛОПЕРИОДА(&НачалоПериода, ГОД) И &ОкончаниеПериода
	|	И РегистрацияТC.Организация В
	|			(ВЫБРАТЬ
	|				Организации.Ссылка
	|			ИЗ
	|				Организации)";
...
Процедура ПолучитьСостоянияОСОрганизацийДляФормированияЗаданийКЗакрытиюМесяца(ДанныеТаблиц, Регистратор) Экспорт
	...
	|ГДЕ
	|	ПорядокУчетаОСБУ.Регистратор = &Регистратор
