...
#Область ТекстРазборкаТоваровТоваровНаСкладах // (Дт 41.01 :: Кт 41.01)
	...
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаВР, Строки.СуммаВР)) КАК СуммаВРДт,
	|	ВЫБОР
	|		КОГДА СтрокиРасход.Склад ССЫЛКА Справочник.Партнеры
	|				ИЛИ СтрокиРасход.Склад ССЫЛКА Справочник.Организации
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)
	|	КОНЕЦ КАК ВидСчетаКт,
	|	СтрокиРасход.ГруппаФинансовогоУчета КАК АналитикаУчетаКт,
	|	СтрокиРасход.Склад КАК МестоУчетаКт,
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	СтрокиРасход.Склад.Подразделение КАК ПодразделениеКт,
	...
	|		ИНАЧЕ СтрокиРасход.Номенклатура
	|	КОНЕЦ КАК СубконтоКт1,
	|	ВЫБОР
	|		КОГДА СтрокиРасход.Склад ССЫЛКА Справочник.Партнеры
	|				ИЛИ СтрокиРасход.Склад ССЫЛКА Справочник.Организации
	|			ТОГДА СтрокиРасход.Номенклатура
	|		ИНАЧЕ Операция.Склад
	|	КОНЕЦ КАК СубконтоКт2,
	|	ЕСТЬNULL(Стоимости.ГруппаПродукции, СтрокиРасход.ГруппаПродукции) КАК СубконтоКт3,
	|	0 КАК ВалютнаяСуммаКт,
	...
	|		ДокументыКОтражению.Ссылка = УчетнаяПолитикаОрганизаций.Ссылка
	|			И ДокументыКОтражению.Период = УчетнаяПолитикаОрганизаций.Период
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСтроки КАК СтрокиРасход ПО	
	|		(СтрокиРасход.Ссылка = Операция.Ссылка)
	|			И (СтрокиРасход.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход))
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСтроки КАК Строки ПО
	|		(Строки.Ссылка = Операция.Ссылка)
	|			И (Строки.Номенклатура = СтрокиРасход.КорНоменклатура)
	|			И (Строки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТВыборкаОднойЗаписиВТСтроки КАК ПерваяЗапись ПО
	|		(ПерваяЗапись.Номенклатура = Строки.Номенклатура
	|			И ПерваяЗапись.Ссылка = Строки.Ссылка
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтоимости КАК Стоимости ПО	
	|		(Строки.Ссылка = Стоимости.Ссылка)
	|			И (СтрокиРасход.Номенклатура = Стоимости.Номенклатура)
	|			И (СтрокиРасход.КорНоменклатура = Стоимости.КорНоменклатура)
	|			И (СтрокиРасход.Склад = Стоимости.Склад)
	|			И (СтрокиРасход.ГруппаФинансовогоУчета = Стоимости.ГруппаФинансовогоУчета)
	|			И (СтрокиРасход.НаправлениеДеятельности = Стоимости.НаправлениеДеятельности)
	|			И (Стоимости.ВидДвижения = СтрокиРасход.ВидДвижения)
	|			И (СтрокиРасход.РазделУчета = Стоимости.РазделУчета)
	|ГДЕ
	|	НЕ УчетнаяПолитикаОрганизаций.УчетнаяПолитика.СборкаТоваровЧерезСчет20
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе)
	|	КОНЕЦ,
	|	ЕСТЬNULL(Стоимости.ГруппаПродукции, СтрокиРасход.ГруппаПродукции),
	|	СтрокиРасход.ГруппаФинансовогоУчета,
	|	СтрокиРасход.Склад,
	|	СтрокиРасход.Склад.Подразделение,
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|			И ВидыЗапасовСписание.АналитикаУчетаНоменклатуры.Склад = Строки.Склад
	...
	|			И ВидыЗапасовОприходование.АналитикаУчетаНоменклатуры.Склад = Строки.Склад
