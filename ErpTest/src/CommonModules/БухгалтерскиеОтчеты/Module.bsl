...
//
Процедура СписокОтборовПоСубконто(Отбор, ОтбираемыеСубконто, СпособПодбора = "ВсеСубконто") Экспорт
	...
	Для каждого ЭлементОтбора Из Отбор Цикл
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			СписокОтборовПоСубконто(ЭлементОтбора.Элементы, ОтбираемыеСубконто, СпособПодбора);
			
		Иначе
			
			ИмяПоля = Строка(ЭлементОтбора.ЛевоеЗначение);
			Если СтрНачинаетсяС(ИмяПоля, "Субконто") Тогда
				
				Если СпособПодбора = "ТолькоДопСвойства" Тогда
					УсловиеПодбораВыполняется = Сред(ИмяПоля, 10, 2) = ".[";
				ИначеЕсли СпособПодбора = "РеквизитыСубконто" Тогда
					УсловиеПодбораВыполняется = Сред(ИмяПоля, 10, 1) = ".";
				Иначе
					УсловиеПодбораВыполняется = (СтрДлина(ИмяПоля) = 9 Или Сред(ИмяПоля, 10, 1) = ".");
				КонецЕсли;
				Если УсловиеПодбораВыполняется Тогда
			   
					НомерСубконто = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Сред(ИмяПоля, 9, 1));
...
Процедура ОпределитьСостояниеЭтаповРасчетаЗакрытияМесяца(Параметры, АдресХранилища) Экспорт
	...
	Пока Результат.ДатаАктуальности > НачалоМесяца(Параметры.НачалоПериода) И Результат.Состояние = "НеТребуется" Цикл
		...
		Если Результат.Состояние = "НеТребуется" Тогда
			Результат.ДатаАктуальности = ДобавитьМесяц(Результат.ДатаАктуальности, -1);
		КонецЕсли;
