	...
	Если РезультатДобавленияЗаписей.ВходящееСообщение.НовоеСообщение Тогда
		...
		Если ДанныеДокумента.Объект <> Неопределено Тогда
			...
			Если ОбработаныВсеДанные
				...
				Если ПараметрыОбмена.ПараметрыПреобразования.НайденныеСсылки.Свойство("ЗаписиСкладскогоЖурналаВЕТИС") Тогда
					Для каждого СтрокаСоответствия Из ПараметрыОбмена.ПараметрыПреобразования.НайденныеСсылки.ЗаписиСкладскогоЖурналаВЕТИС Цикл
						СписокПолученныхЗаписейСЖ.Добавить(СтрокаСоответствия.Значение);
					КонецЦикла;
				КонецЕсли;
...
Процедура СверткаРегистраСоответствиеНоменклатурыВЕТИС() Экспорт
	...
	|		ПО СоответствиеНоменклатурыВЕТИС.ЗаписьСкладскогоЖурнала = ЗаписиСкладскогоЖурналаВЕТИС.Ссылка
	|		 И ЗаписиСкладскогоЖурналаВЕТИС.КоличествоВЕТИС <> 0
	|ГДЕ
	|	СоответствиеНоменклатурыВЕТИС.ЗаписьСкладскогоЖурнала <> ЗНАЧЕНИЕ(Справочник.ЗаписиСкладскогоЖурналаВЕТИС.ПустаяСсылка)
	|	И ЗаписиСкладскогоЖурналаВЕТИС.Ссылка ЕСТЬ NULL
	|СГРУППИРОВАТЬ ПО
	...
	Если НЕ Результат.Пустой() Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Свертка регистра Соответствие номенклатуры ВЕТИС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
