...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	НоваяСтрока.ТекстВыполнить = НСтр("ru='Сформировать'");
	НоваяСтрока.ДействиеИспользование = ОписаниеДействия_СервернаяПроцедура(
		"ЗакрытиеМесяцаСервер.Использование_ФормированиеРезервовПоСомнительнымДолгам");
	НоваяСтрока.ДействиеВыполнить  = ОписаниеДействия_ВыполнитьРасчет(
		"ЗакрытиеМесяцаСервер.Выполнить_ФормированиеРезервовПоСомнительнымДолгам");
	НоваяСтрока.ДействиеПодробнее = ОписаниеДействия_ОткрытьСписокДокументовРегламентнаяОперация();
	НоваяСтрока.ТипыРегламентныхОпераций.Добавить(Перечисления.ТипыРегламентныхОпераций.РасчетДолейСписанияКосвенныхРасходов);
	НоваяСтрока.ТипыРегламентныхОпераций.Добавить(Перечисления.ТипыРегламентныхОпераций.ФормированиеРезервовПоСомнительнымДолгам);
...
Процедура Использование_ОформлениеПроизводстваБезЗаказов(ПараметрыОбработчика) Экспорт
	...
	|		ПО Аналитика.КлючАналитики = РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры
	...
	|		ПО КлючиАналитики.Склад = Работы.Подразделение
...
Процедура Использование_ФормированиеДвиженийПоРасчетамСПартнерамиИПереоценкаРасчетов(ПараметрыОбработчика) Экспорт
	...
	Иначе
		...
		КонецЕсли;
		ПериодВДиапазонеПересчета = ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика);
...
Процедура Использование_ПереоценкаДенежныхСредствИФинансовыхИнструментов(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,,,,ДействиеПодробнее) Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_НачислениеАмортизацииОС(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,,, "ЗаданияКРасчетуАмортизацииОС") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_НачислениеАмортизацииНМА(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,,, "ЗаданияКРасчетуАмортизацииНМА") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_ПереходНаУчетВнеоборотныхАктивовВерсии24(ПараметрыОбработчика) Экспорт
	...
	ИначеЕсли СведенияОПроблемах.КоличествоПроблем = 0
		...
		Если ПериодРегистрации = Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить() Тогда
...
Процедура Использование_ФормированиеДвиженийПоВнеоборотнымАктивам(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,,, "ЗаданияКФормированиюДвиженийПоВНА") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_ПризнаниеВНалоговомУчетеЛизинговыхПлатежей(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,, "ВТОрганизации") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_ПогашениеСтоимостиТМЦВЭксплуатации(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,,, "ЗаданияКПогашениюСтоимостиТМЦВЭксплуатации") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_РасчетСтоимостиВнеоборотныхАктивов(ПараметрыОбработчика) Экспорт
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика,,, "ЗаданияКРасчетуСтоимостиВНА") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_ФормированиеРезервовПоСомнительнымДолгам(ПараметрыОбработчика) Экспорт
	...
	|ПОМЕСТИТЬ ВТОрганизации
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&НачалоПериода, Организация В (&МассивОрганизаций)) КАК Т
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УчетныеПолитикиОрганизаций КАК СпрУчетнаяПолитика
	|		ПО Т.УчетнаяПолитика = СпрУчетнаяПолитика.Ссылка
	...
	Если ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, Ложь, "ВТОрганизации") Тогда
		Возврат;
	КонецЕсли;
...
Процедура Использование_РасчетКурсовыхРазниц(ПараметрыОбработчика) Экспорт
	...
	|		ИЛИ НЕ(&ВестиУУНаПланеСчетов И Хозрасчетный.ИсключитьСуммуУУИзПереоценкиПоПлануСчетов))
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|
	|ВЫБРАТЬ
...
Функция ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, ВыполнятьССамогоРаннего = Истина,
			ИмяТаблицыОтбораОрганизаций = "", ИмяРегистраЗаданий = "", ДействиеПодробнее = Неопределено)
...
Функция ПроверитьИспользованиеРегламентированногоУчета(ПараметрыОбработчика, ИзменятьСостояниеЭтапа = Истина)
	...
	Иначе
		...
		Если ПараметрыОбработчика.Свойство("ДанныеЭтапа") И ПараметрыОбработчика.ДанныеЭтапа.Наименование = "Формирование резервов по сомнительным долгам" Тогда
			ДатаНачалаВеденияРеглУчета  = Константы.пкДатаНачалаТиповогоРасчетаРСД.Получить();
		Иначе	
