...
Процедура НачалоВыбораСоглашенияСКлиентом(СтруктураПараметров, СтандартнаяОбработка) Экспорт
	...
	ПараметрыФормы.Вставить("Элемент", Элемент); 
	ПараметрыФормы.Вставить("ДанныеФормыСтруктура", ДанныеФормыСтруктура); 
	...
	Если Не ЗначениеЗаполнено(Партнер) Тогда
		...
		ОткрытьФорму("Справочник.Партнеры.ФормаВыбора", Новый Структура("Отбор", ОтборКлиент),,,,, 
			Новый ОписаниеОповещения("НачалоВыбораСоглашенияСКлиентомЗавершение", ЭтотОбъект, ПараметрыФормы), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		Возврат;
	КонецЕсли;
