...
#Область ТекстСторноНДС // (Дт 90.03 :: Кт 19.03)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ  //// Входящий НДС по продажам прошлых лет (сторно) по рег. НДСПредъявленный (Дт 91.02 :: Кт 19.03)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ТекстВозвратПереданнойТары // (Дт 41.03 :: Кт 76.09)
	...
	ТекстВозвратПереданнойТарыСкладПроизводства = "
	|ВЫБРАТЬ //// Себестоимость возвращаемой тары на склад производства (Дт 20 :: Кт 76.09)
	|	
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|ВЫБРАТЬ //// Отклонение в стоимости возвращаемой тары на расходы (Дт 91.02 :: Кт 76)
	|	
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|		КОГДА Строки.Сумма = 0 И НЕ Стоимости.Сумма ЕСТЬ NULL
	|			ТОГДА (Стоимости.Знак * Стоимости.Сумма)
	|				+ ЕСТЬNULL(СуммыВВалютеРегл.СуммаБезНДСРегл + СуммыВВалютеРегл.СуммаНДСРегл, ТаблицаВидыЗапасов.СуммаСНДС)
	|		ИНАЧЕ Стоимости.Знак * Стоимости.Сумма КОНЕЦ) КАК Сумма,
	|	(ВЫБОР
	|		КОГДА Строки.СуммаУУ = 0 И НЕ Стоимости.Сумма ЕСТЬ NULL
	|			ТОГДА (Стоимости.Знак * Стоимости.СуммаУУ)
	|				+ ЕСТЬNULL(СуммыВВалютеРегл.СуммаБезНДСУпр + СуммыВВалютеРегл.СуммаНДСУпр, ТаблицаВидыЗапасов.СуммаСНДС / КурсВалютыУпрУчета.Курс)
	...
	|ВЫБРАТЬ //// Отклонение в стоимости возвращаемой тары в доходы (Кт 76 :: Дт 91.01)
	|	
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстВозвратПереданнойТарыСкладПроизводства);
