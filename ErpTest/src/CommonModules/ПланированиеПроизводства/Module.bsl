...
// 
// Возвращаемое значение:
//   - Структура - см. метод СтруктураДанныхПродукции.
//
...
// Процедура дозаполняет назначение в строках спецификации заказа
//
// Параметры:
//  Объект - ДокументОбъект.ЗаказНаПроизводство - заказ на производство.
//
Процедура ДозаполнитьНазначениеВСпецификации(Объект) Экспорт
	...
	Если НайденныеСтроки.ВГраница() <> -1 Тогда
		...
		Для Каждого СтрокаПродукция Из НайденныеСтроки Цикл
			...
			Для Каждого ЭлементКоллекции Из Объект.ВыходныеИзделия.НайтиСтроки(ОтборВыходныеИзделия) Цикл
				ЭлементКоллекции.Назначение = Объект.Назначение;
			КонецЦикла;
	...
	Если Объект.ОбосабливатьПоНазначениюЗаказа Тогда
		...
		Для Каждого ЭлементКоллекции Из Объект.МатериалыИУслуги Цикл
			
			Если Не ЭлементКоллекции.ПроизводитсяВПроцессе
				И (ЭлементКоллекции.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Обособленно")
				ИЛИ ЭлементКоллекции.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно")) Тогда
...
Функция ЗаданиеРасчетГрафикаПроизводства() Экспорт
	...
	Иначе
		Задание = РегламентныеЗаданияСервер.Задание(МетаданныеПредопределенного);
	КонецЕсли;
...
Функция ЗаполнитьСпецификациюЗаказаПоСпискуНоменклатуры(Объект, 
	...
	МассивНоменклатуры = Новый Массив;
	МассивДанныхВыборки = Новый Массив;
	...
	Для ИндексТекущихДанных = 0 По МассивДанных.ВГраница() Цикл
		...
		ДанныеВыборки = Справочники.РесурсныеСпецификации.ДанныеПоНоменклатуреРасширенный();
		ЗаполнитьЗначенияСвойств(ДанныеВыборки, ДанныеПоНоменклатуре);
		МассивДанныхВыборки.Добавить(ДанныеВыборки);
	...
	Для ИндексТекущихДанных = 0 По МассивДанных.ВГраница() Цикл
		...
		ДанныеПоНоменклатуре.Вставить("ПоЗаказуДавальца", СвойстваНазначения <> Неопределено И СвойстваНазначения.ЭтоНазначениеДавальца);
