...
Процедура ЗаполнитьДанныеОснованияПлатежа(Знач ОснованиеПлатежа, ДанныеОснованияПлатежа) Экспорт
	...
	Для каждого ЭлементКоллекции Из ДанныеПоНоменклатуре Цикл
		...
		НоваяСтрока.СтавкаНДСКод = ПреобразоватьСтавкуНДС(НоваяСтрока.СтавкаНДС);
	КонецЦикла;
...
Функция ДанныеДокументаДляПолученияПлатежнойСсылки(ДокументСсылка, ТолькоДанныеЧека = Ложь)
	...
	ДанныеДокумента.ДанныеЧека =                 Результат[7].Выгрузить();
