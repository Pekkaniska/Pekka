...
Функция ДополнитьПредставлениеПредставлениемФискальнойОперацииВДокументеПродажи(Основание, СчетаФактурыВыданныеНаОсновании = Неопределено, СчетаФактурыПолученныеНаОсновании = Неопределено) Экспорт
	...
	ВозможноПробитиеЧека = ВозможноПробитиеЧека И ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование");
...
Функция ТипыСсылокДокументыРеализации() Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказДавальца)
		...
		ТипыСсылок.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Документы.ЗаказДавальца));
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура СформироватьЗапросВременнаяТаблицаТоварыДокументаПродажи(ТекстЗапроса,
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказДавальца" Тогда
		...
		ПредставлениеДопТабличнойЧасти = НСтр("ru = 'Материалы'");
	...
	ИначеЕсли ИмяТаблицы = "Документ.ПередачаСырьяПереработчику" Тогда
		...
		ПредставлениеТабличнойЧасти = НСтр("ru = 'Сырье и материалы'");
...
// Формирует текст запроса для проверки заполненности реквизита "Склад" в строках табличных частях Заказа давальца
...
Процедура СообщитьОбОшибкахКорректностьЗаполненияСкладаЗаказаДавальца(Знач Выборка,
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЕстьОшибкиЗаполненияСкладПоступления Тогда
			...
			ТекстОшибки = НСтр("ru='Поле ""Склад поступления"" не заполнено'");
