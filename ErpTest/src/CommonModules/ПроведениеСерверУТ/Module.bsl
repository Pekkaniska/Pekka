...
Процедура ПодготовитьНаборыЗаписейКРегистрацииДвижений(Объект, ЭтоНовый = Ложь, ДвиженияМетаданные = НеОпределено, ОтключитьПроверкуДатыЗапретаИзменения = Ложь) Экспорт
	...
	Если НЕ ЭтоНовыйДокумент Тогда
		...
		//++ НЕ УТКА
		// Движения делаются из набора записей Движение товаров.
		Если ТипДокумента = Тип("ДокументСсылка.ЗаказНаПроизводство2_2") Тогда
			ИсключаемыеРегистры.Добавить("ОбеспечениеЗаказов");
		КонецЕсли;
		//-- НЕ УТКА
		...
		Если Объект.ДополнительныеСвойства.Свойство("РежимЗаписи")
		 И НЕ Объект.ДополнительныеСвойства.РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
			...
			Если
			//++ НЕ УТКА
			 ТипДокумента = Тип("ДокументСсылка.МаршрутныйЛистПроизводства") Тогда
...
//-- НЕ УТКА
	...
	Если ФормироватьУправленческийБаланс Тогда
		УправленческийУчетПроведениеСервер.ОбновитьДвиженияАктивовПассивов(Объект);
	КонецЕсли;
...
Процедура ЗагрузитьТаблицыДвижений(ДополнительныеСвойства, Движения) Экспорт
	...
	Для Каждого Таблица Из ТаблицыДляДвижений Цикл
		...
		КонецЕсли;
		//++ НЕ УТКА
		Если ИмяРегистра = "ОтражениеДокументовВМеждународномУчете" Тогда
			// Формирование движений выполняется процедурой МеждународныйУчетПроведениеСервер.ЗарегистрироватьКОтражению().
			Продолжить;
		КонецЕсли;
		//-- НЕ УТКА
		//-- НЕ УТ
...
Процедура СформироватьЗаписиРегистровЗаданий(Объект) Экспорт
	...
	УчетНДСУП.ОтразитьЗаписиРегистраЗаданий(Объект, Объект.ДополнительныеСвойства);
	
	//++ НЕ УТ
	ВнеоборотныеАктивы.СформироватьЗаписиРегистровЗаданий(Объект.Ссылка, Объект.ДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы)
	//-- НЕ УТ
...
//++ НЕ УТКА
	ОбеспечениеПроизводства.ПриИзмененииОбособленногоОбеспечения(Объект, МассивЗаказов);
//-- НЕ УТКА
...
Процедура ВыполнитьКонтрольРезультатовПроведения(Объект, Отказ) Экспорт
	...
	Для Каждого Результат Из МассивРезультатов Цикл
		...
		//-- НЕ УТКА
		//+++rarus-spb_zlov 16.01.2017
		//ИначеЕсли ИмяКонтроля = Врег("ЗаказыКлиентов")
		//	  Или ИмяКонтроля = Врег("ЗаказыПоставщикам") Тогда
		ИначеЕсли ИмяКонтроля = Врег("ЗаказыКлиентов") Тогда
			Возврат;
		ИначеЕсли ИмяКонтроля = Врег("ЗаказыПоставщикам") Тогда
		//---rarus-spb_zlov 16.01.2017
		...
		ИначеЕсли ИмяКонтроля = Врег("ВТ_РаспоряженияНаВыпускПродукцииОперативныйКонтроль")
			...
			ИЛИ ИмяКонтроля = Врег("ВТ_РаспоряженияНаПередачуИзПроизводства")
...
// Иначе суммы расчитываются по курсу на дату документа.
// 
...
Процедура ИнициализироватьВтСуммыДокументовВВалютахУчета(Запрос, ТекстыЗапроса, ТекстЗапросаДанныхДокументов) Экспорт
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ВтДанныеДокументов", Истина);
	...
	|	ПО
	|		ДанныеДокументов.Дата >= КурсыВалют.Период
	...
	|	Дата
	|";
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ВтПериодыКурсовВалют", Истина);
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ПериодыКурсовВалют.Дата   КАК Дата,
	...
	|	КОНЕЦ КАК Курс
	|ПОМЕСТИТЬ ВтКурсыВалютНаДаты
	|ИЗ
	|	ВтПериодыКурсовВалют КАК ПериодыКурсовВалют
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.КурсыВалют КАК КурсыВалют
	|	ПО
	...
	|	Дата
	|";
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ВтКурсыВалютНаДаты", Истина);
	...
	|			      И ДанныеДокументов.ВалютаДокумента <> ДанныеДокументов.ВалютаВзаиморасчетов
	|				ТОГДА ДанныеДокументов.СуммаВзаиморасчетов
	|			ИНАЧЕ ВЫРАЗИТЬ((ДанныеДокументов.СуммаСНДС * КурсыВалютДокументов.Курс / КурсыВалютыУправленческогоУчета.Курс) КАК ЧИСЛО(31,2))
	...
	|			ИНАЧЕ ВЫРАЗИТЬ((ДанныеДокументов.СуммаБезНДС * КурсыВалютДокументов.Курс / КурсыВалютыУправленческогоУчета.Курс) КАК ЧИСЛО(31,2))
	...
	|			ИНАЧЕ ВЫРАЗИТЬ((ДанныеДокументов.СуммаНДС * КурсыВалютДокументов.Курс / КурсыВалютыУправленческогоУчета.Курс) КАК ЧИСЛО(31,2))
	...
	|			      И ДанныеДокументов.СуммаНДСВзаиморасчетов <> 0
	|				ТОГДА ДанныеДокументов.СуммаВзаиморасчетов - ДанныеДокументов.СуммаНДСВзаиморасчетов
	|			ИНАЧЕ ВЫРАЗИТЬ((ДанныеДокументов.СуммаБезНДС * КурсыВалютДокументов.Курс / КурсыВалютыРегламентированногоУчета.Курс) КАК ЧИСЛО(31,2)) 
	|		КОНЕЦ 
	|	) КАК СуммаБезНДСРегл,
	|
	...
	|			      И ДанныеДокументов.СуммаНДСВзаиморасчетов <> 0
	|				ТОГДА ДанныеДокументов.СуммаНДСВзаиморасчетов
	|			ИНАЧЕ ВЫРАЗИТЬ((ДанныеДокументов.СуммаНДС * КурсыВалютДокументов.Курс / КурсыВалютыРегламентированногоУчета.Курс) КАК ЧИСЛО(31,2))
	|		КОНЕЦ
	|	) КАК СуммаНДСРегл,
	|
	|	ЕСТЬNULL(Суммы.СуммаБезНДСРегл + СуммаНДСРегл,
	|		ВЫБОР
	|			КОГДА ДанныеДокументов.ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|			      И ДанныеДокументов.ВалютаДокумента <> ДанныеДокументов.ВалютаВзаиморасчетов
	|			      И ДанныеДокументов.СуммаВзаиморасчетов <> 0
	|				ТОГДА ДанныеДокументов.СуммаВзаиморасчетов
	|			ИНАЧЕ ВЫРАЗИТЬ((ДанныеДокументов.СуммаСНДС * КурсыВалютДокументов.Курс / КурсыВалютыРегламентированногоУчета.Курс) КАК ЧИСЛО(31,2))
	|		КОНЕЦ
	|	) КАК СуммаСНДСРегл,
	|
	...
	|		И ДанныеДокументов.ВалютаДокумента = Суммы.Валюта
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтКурсыВалютНаДаты КАК КурсыВалютыРегламентированногоУчета
	|	ПО
	|		ДанныеДокументов.Дата = КурсыВалютыРегламентированногоУчета.Дата
	|		И КурсыВалютыРегламентированногоУчета.Валюта = &ВалютаРегламентированногоУчета
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтКурсыВалютНаДаты КАК КурсыВалютыУправленческогоУчета
	|	ПО
	|		ДанныеДокументов.Дата = КурсыВалютыУправленческогоУчета.Дата
	|		И КурсыВалютыУправленческогоУчета.Валюта = &ВалютаУправленческогоУчета
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ВтКурсыВалютНаДаты КАК КурсыВалютДокументов
	|	ПО
	|		ДанныеДокументов.Дата = КурсыВалютДокументов.Дата
	|		И КурсыВалютДокументов.Валюта = ДанныеДокументов.ВалютаДокумента
	|";
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ВтСуммыДокументовВВалютахУчета", Истина);
...
Процедура СообщитьОбОшибкахПроведенияПоРегиструЗаказыМатериаловВПроизводство(Объект, Отказ, РезультатЗапроса)
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПередачаМатериаловВПроизводство")
		ИЛИ ТипЗнч(Объект) = Тип("ДокументОбъект.ДвижениеПродукцииИМатериалов") Тогда
