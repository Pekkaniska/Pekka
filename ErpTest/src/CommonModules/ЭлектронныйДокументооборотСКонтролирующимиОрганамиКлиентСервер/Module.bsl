	...
	Иначе
		...
		Если ЭлементФормыНаименованиеЭтапа <> Неопределено Тогда
			...
			Если НЕ (СтрНачинаетсяС(Форма.ИмяФормы, "Справочник.ЭлектронныеПредставленияРегламентированныхОтчетов")
				...
				ИЛИ СтрНачинаетсяС(Форма.ИмяФормы, "Справочник.ЗаявлениеОНазначенииПенсии")) Тогда
...
Процедура ОбработкаПроверкиЗаполненияЗаявленияОНазначенииПенсии(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если НЕ СведенияКорректны Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Вид пенсии""'"),,"УказательВидПенсии");
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ СведенияКорректны Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Паспорт, контакты""'"),,"УказательСведенияОПолучателе");
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ СведенияКорректны Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Начисление пенсии""'"),,"УказательДопСведения");
		Отказ = Истина;
	КонецЕсли;	
	...
	Если НЕ СведенияКорректны Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Представитель""'"),,"Объект.ЕстьПредставитель");
		Отказ = Истина;
	КонецЕсли;
	...
	Если НЕ СведенияКорректны Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Заполните обязательные реквизиты раздела ""Cпособ получения""'"),,"УказательСпособПолучения");
		Отказ = Истина;
	КонецЕсли;
...
Функция ДопСведенияУказаныКорректно(Форма, МастерДалее = Истина, ВыводитьСообщения) Экспорт
	...
	Если Форма.ЭтоЗаявлениеОНазначенииПенсии Тогда
		...
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений о детях'"));
		КонецЕсли;
		
		ИнвалидыПожилыеУказаныКорректно(Форма, МастерДалее, Ложь);
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений об инвалидах и пожилых'"));
		КонецЕсли;
		
		ВоеннаяСлужбаПоПризывуУказанаКорректно(Форма, МастерДалее, Ложь);
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений военной службы по призыву'"));
		КонецЕсли;
		
		ВоеннаяСлужбаУказаныКорректно(Форма, МастерДалее, Ложь);
		Если НЕ МастерДалее И ВыводитьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Проверьте корректность заполнения сведений военной службы'"));
		КонецЕсли;
