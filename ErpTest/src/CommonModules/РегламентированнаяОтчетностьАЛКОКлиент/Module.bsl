...
Процедура ОбработкаОповещенияАЛКО(Форма, ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если НРег(ИмяСобытия) = НРег("ОбновитьЗаявлениеОКорректировке") И (НЕ Форма.НомерАлкоОтчета = 0) Тогда
	...
	Если НРег(ИмяСобытия) = НРег("ОбновитьЗаявлениеОКорректировкеИзДекларации") И Форма.НомерАлкоОтчета = 0 Тогда
...
Процедура ПриЗакрытииАЛКО(Форма, ЗавершениеРаботы) Экспорт
	...
	СохраненныйДокумент = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
								"мСохраненныйДок", Форма.СтруктураРеквизитовФормы);
...
Процедура ТаблицаФормыРазделаПриАктивизацииСтрокиАЛКО(Форма, Элемент) Экспорт
	...
	Форма.АктивныйЭлементТаблицы = Элемент;
	// Нельзя использовать серверные методы. 
	ВыполнитьСМинимальнойЗадержкой(Форма, "ПриАктивизацииСтрокиНаКлиенте");
...
Процедура ОрганизацияОткрытиеАЛКО(Форма, Элемент = Неопределено, СтандартнаяОбработка = Ложь) Экспорт
	...
	ФормаСправочникаОрганизации = ОткрытьФорму("Справочник.Организации.ФормаОбъекта", ПараметрыОрганизации, Форма, 
									КлючУникальности, , , Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗаявлениеОКорректировкеНажатиеАЛКО(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если Значениезаполнено(Форма.ЗаявлениеОКорректировке) Тогда
		...
		НажатиеГиперссылкиАЛКО(Форма, ЭлементЗаявлениеОКорректировке, РежимОткрытияОкнаФормы.Независимый,
																				СтандартнаяОбработка);
		
	Иначе
		
		КраткоеИмяФормыЗаявления = "ФормаОтчета2017Кв3";
		ИмяФормыЗаявления = "Отчет.РегламентированныйОтчетАлкоЗаявлениеОКорректировке.Форма." + КраткоеИмяФормыЗаявления;
		...
		Если ФормаЗаявления = Неопределено Тогда
			...
			ОткрытьФорму(ИмяФормыЗаявления, ПараметрыФормы, Форма.ВладелецФормы, 
							Истина, , , , РежимОткрытияОкнаФормы.Независимый);
...
Процедура ВидДеятельностиПоЛицензииНажатиеАЛКО(Форма, Элемент = Неопределено, СтандартнаяОбработка = Ложь) Экспорт
...
Функция ОбработкаДействияСАлкоОтчетом(ОписаниеОтчета, Действие, НеСтандартнаяОбработка) Экспорт
	
	Если НЕ ТипЗнч(ОписаниеОтчета) = Тип("Структура") Тогда
	...
	Алко11.ФормаОтчета2014Кв1.Вставить("Утверждена", 
					"Постановлением Правительства РФ от 09.08.2012 № 815 (в ред. приказа ФСРАР от 08.04.2014 № 92)");
	...
	Алко12.ФормаОтчета2012Кв3.Вставить("Утверждена", 
					"Постановлением Правительства РФ от 09.08.2012 № 815 (в ред. приказа ФСРАР от 08.04.2014 № 92)");
	...
	УстановитьДляФормыДействие(СписокДействий, Алко13.ФормаОтчета2016Кв4, "ЗагрузкаИзФайла", Ложь);
	...
	УстановитьДляФормыДействие(СписокДействий, Алко14.ФормаОтчета2016Кв4, "ЗагрузкаИзФайла", Ложь);
	...
	УстановитьДляФормыДействие(СписокДействий, Алко15.ФормаОтчета2016Кв4, "ЗагрузкаИзФайла", Ложь);
	...
	УстановитьДляФормыДействие(СписокДействий, Алко16.ФормаОтчета2018Кв1, "ЗагрузкаИзФайла", Ложь);
	...
	УстановитьДляФормыДействие(СписокДействий, Алко17.ФормаОтчета2018Кв1, "ЗагрузкаИзФайла", Ложь);
	...
	УстановитьДляФормыДействие(СписокДействий, Алко18.ФормаОтчета2018Кв1, "ЗагрузкаИзФайла", Ложь);
	...
	Если СтруктураРазрешенийФормы.ТабличныйОтчет И СтруктураРазрешенийФормы[Действие] Тогда
	...
	ИначеЕсли НЕ СтруктураРазрешенийФормы[Действие] Тогда
		...
		Если ПериодНачалаДействия = Неопределено Тогда
			...
			Иначе
				...
				ТекстПредупреждения = НСтр("ru = '" + ПредставлениеДействия + " редакции формы, утвержденной 
									|" + СтруктураРазрешенийФормы.Утверждена + ", 
									|не предусмотрена.
									|" + ПредставлениеДействия + " будет реализована в будущих версиях программы.'");
		...
		Иначе
			...
			ТекстПредупреждения = НСтр("ru = '" + ПредставлениеДействия + " редакции формы, утвержденной 
												|" + СтруктураРазрешенийФормы.Утверждена + ",
												|реализована начиная с отчетного периода за " 
												+ ПериодНачалаДействия + ".
												|Создайте новый отчет за период " + ПериодНачалаДействия 
												+ " или позже.'");
...
Процедура ВызватьФормуВводаАдресаАЛКО(СохраненныеДанныеАдреса, Заголовок, 
	...
	Если ВидКонтактнойИнформации = Неопределено Тогда
		...
		СтруктураДоступныхВидовКонтактнойИнформацииВводаАдреса 
					= РегламентированнаяОтчетностьАЛКОВызовСервера.ОпределитьВидыКонтактнойИнформацииДляВводаАдреса();
...
Процедура ВызватьФормуВыбораЗначенийАЛКО(Форма, ФлажокОтклАвтоВыборКодов, СтруктураПараметров, ОписаниеОповещения) Экспорт
	...
	Если НЕ ФлажокОтклАвтоВыборКодов Тогда
		...
		Форма.Активизировать();
		Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ФормаВыбораЗначенияИзТаблицы", 
			ПараметрыФормы, Форма, КлючУникальности, , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	...
	Иначе
		...
		Форма.Активизировать();
		Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = ОткрытьФорму("ОбщаяФорма.АЛКОФормаВводаПарыСвязанныхЗначений", 
			ПараметрыФормы, Форма, КлючУникальности, , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
//								
//
Процедура ВыполнитьСМинимальнойЗадержкой(Форма, ИмяПроцедуры, ОбъектОповещения = Неопределено, 
															ИмяПроцедурыОповещения = Неопределено)
															
	ВыполнитьСЗадержкой(Форма, ИмяПроцедуры, 0.1, ОбъектОповещения, ИмяПроцедурыОповещения);
...
//
Функция ПроверитьПодключениеОрганизации(СтруктураРеквизитовФормы, ТолькоОтчетность = Ложь)
...
Процедура ПриОткрытииЗавершениеАЛКО(Отказ, Форма) Экспорт
	...
	Если Форма.ЭтоВебКлиент Тогда
		...
		ВыполнитьСЗадержкой(Форма, "Подключаемый_ОткрытьОтчетНаКлиенте", 0.2, 
...
Процедура ПриАктивизацииСтрокиАЛКО(Форма) Экспорт
	...
	// При активизации строки срабатывает при уходе на другую страницу.
	// Поскольку эта часть кода вызывается через ВыполнитьСЗадержкой(),
	// срабатывает через таймаут 0.1, и к этому моменту таблица формы,
	// сохраняемая в Форма.АктивныйЭлементТаблицы может не существовать,
	// поскольку при переходе старая таблица удаляется, новая создается.
	// Проверять по имени таблицы формы нельзя, для страниц одного раздела
	// имя формы одинаково, а Элемент уже не существует.
	// Поэтому вынужденная проверка через Попытка-Исключение, других способов нет.
	Попытка
		ТекущиеДанные = Элемент.ТекущиеДанные;
	Исключение
		Возврат;
	...
	Если ВыполняетсяДлительнаяОперация Тогда
		...
		Если НужноОткрытьФорму Тогда
			...
			ФормаДлительнойОперацииОтчета = ОткрытьФорму(ИмяФормыДлительнойОперации, ПараметрыФормы, 
						ВладелецФормыДлитОпер, КлючУникальности, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВводРеквизитовОП_АЛКО(Форма)
	...
	Форма.Активизировать();
	Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = ОткрытьФорму(ИмяФормыВводРеквизитовОП, ПараметрыФормы, Форма, КлючУникальности, , ,
...
Процедура ВыборДеятельностиПоЛицензииАЛКО(Форма)
	...
	ДополнительныеПараметры = Новый Структура("Форма, ИсходноеЗначениеКода, ИсходноеЗначениеНазвания", 
												Форма, ИсходноеЗначениеКода, ИсходноеЗначениеНазвания);
...
Процедура ВыборДеятельностиПоЛицензииЗавершение(РезультатВыбора, Параметры) Экспорт
	...
	ЧислоВыбранныйКод = ?(ЗначениеЗаполнено(ВыбранныйКод), Число(ВыбранныйКод), КодЗаведомоНелицензионный);
	...
	Если БылоИзменение Тогда
		...
		Если (КоличествоСтраницЛицензий > 1) и (ЧислоВыбранныйКод  >= ГраницаОпределенияЛицензионности) 
		...
		Иначе
			...
			Форма.ОбработкаПолейСтраницыЛицензийНаСервере( ЧислоВыбранныйКод < ГраницаОпределенияЛицензионности );
			...
			Если (Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20131101')				    
				И (ЧислоВыбранныйКод >= ГраницаОпределенияЛицензионности) Тогда
...
Функция ЛицензируемаяДеятельностьАЛКО(Форма)
	...
	// соответствующую в макете Списки в области Виды Деятельности кодам больше или равном 8.
	//
...
Процедура ВосстановитьУдаленныеСтраницыЗавершениеАЛКО(РезультатВопроса, Форма) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ВыполнитьСЗадержкой(Форма, "ПослеВосстановленияУдаленныхСтраниц");
...
Процедура КомандаОчиститьРегистрыОтМусораАЛКО(Форма)
	...
	ЖирныйНаклонныйШрифт 	= Новый Шрифт( , ,Истина, Истина);
		
	ТекстВнимание = Новый ФорматированнаяСтрока(НСтр("ru='Внимание!'"), ЖирныйНаклонныйШрифт, WebЦвета.Красный);
	ТекстПредупреждение = Новый ФорматированнаяСтрока(
		НСтр("ru='После применения данной процедуры восстановление удаленных страниц станет невозможным!'"), 
		ЖирныйНаклонныйШрифт);
	ТекстПредупреждение2 = Новый ФорматированнаяСтрока(
		НСтр("ru='Если Отчет изменен, перед очисткой изменения будут сохранены!'"), ЖирныйНаклонныйШрифт);
	ТекстВнимание = Новый ФорматированнаяСтрока(ТекстВнимание, Символы.ПС, ТекстПредупреждение, 
									Символы.ПС, ТекстПредупреждение2, Символы.ПС, Символы.ПС);
	
	ТекстЧастичнаяОчистка = Новый ФорматированнаяСтрока(НСтр("ru='Частичная очистка'"), ЖирныйШрифт);
	...
	ТекстПолнаяОчистка = Новый ФорматированнаяСтрока(НСтр("ru='Полная очистка'"), ЖирныйШрифт);
...
Процедура УстановитьДляФормыДействие(СписокДействий, СтруктураФормыАлкоОтчета, 
	...
	Если ЗначениеЗаполнено(Действие) И (НЕ СписокДействий.НайтиПоЗначению(Действие) = Неопределено) Тогда
...
Процедура ОткрытьФормуВыбораСтраницАЛКО(Форма, ПараметрыФормы, ТекущийРаздел, НомерСтраницыРаздела)
	...
	ФормаНастройкиОтчета = РегламентированнаяОтчетностьКлиент.ПолучитьОбщуюФормуПоИмени("НастройкиОтчета", 
									ПараметрыФормы, Форма, КлючУникальности);
...
Процедура ОткрытьФормуНавигацииПоОшибкамБезПараметровАЛКО(Форма) Экспорт
	...
	ФормаНавигацииПоОшибкам = ПолучитьФорму("ОбщаяФорма.АЛКОФормаСообщенийОбОшибках", ПараметрыФормы, , КлючУникальности);
...
Функция ОпределитьОткрытуюФормуНавигацииПоОшибкам(ФормаОтчета)
	
	Возврат ОпределитьОткрытуюВспомогательнуюФормуОтчета(ФормаОтчета, "ОбщаяФорма.АЛКОФормаСообщенийОбОшибках");
	
КонецФункции
	
Функция ОпределитьОткрытуюВспомогательнуюФормуОтчета(ФормаОтчета, ИмяВспомогательнойФормы)
	
	Если НЕ ТипЗнч(ФормаОтчета) = Тип("УправляемаяФорма") Тогда
	    Возврат Неопределено;	
	КонецЕсли;
...
Процедура НажатиеГиперссылкиАЛКО(Форма, Элемент, РежимОткрытияОкна, СтандартнаяОбработка = Ложь)
...
Процедура РезультатСохраненияАЛКО(Форма, Автосохранение = Ложь, КодИФНС, КПП, Вариант, 
	...
	Если Форма.БылаУдаленаСтраницаРаздела Тогда	
		ЗакрытьФормуОшибокПриИзмененииОтчета(Форма);	
	КонецЕсли;
	...
	Если Форма.ПользовательСохранилОтчет или Форма.ПользовательНажалСохранитьОтчет Тогда
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Сохраняется %1...'"), 
													Форма.Заголовок), , , БиблиотекаКартинок.Записать);
	Иначе
...
Процедура ЗакрытьФормуОшибокПриИзмененииОтчета(Форма)
	...
	Если (НЕ Форма.ФормаОшибок = Неопределено) и ТипЗнч(Форма.ФормаОшибок) = Тип("УправляемаяФорма") Тогда
...
Процедура УдалитьДокументОтчета_ДлительнаяОперацияАЛКО(Форма)
	...
	Если НЕ Форма.ПользовательСохранилОтчет и (НЕ Форма.СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено) Тогда
