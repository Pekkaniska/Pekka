...
//++ НЕ УТКА
	...
	ИначеЕсли ТипРеквизита = Тип("Булево")
		И ИмяРеквизита = "Согласован"
		И (
			ТипОбъекта = Тип("ДокументОбъект.ЗаказДавальца")
		) Тогда
		Если СостояниеДО = "Согласован" Тогда
			Результат = Истина;
		ИначеЕсли СостояниеДО = "НеСогласован" Тогда
...
Процедура ПриЗаполненииСвойстваОбъектаXDTOПоЗначению(Значение, Свойства, ТипОбъекта, ИмяСвойства, ТипСвойства,
	...
	//++ НЕ УТКА
		...
		ИначеЕсли Значение = Перечисления.СтатусыЗаказовДавальцев.Согласован Тогда
		...
		ИначеЕсли Значение = Перечисления.СтатусыЗаказовДавальцев.НеСогласован  Тогда
...
//rarus+ saveld 31.10.2016

	Если ВызовИзФормыОбъекта Тогда
		Возврат;
	КонецЕсли;

	ПерезаписатьОбъект = Ложь;
		
	Если ТипЗнч(ПредметСогласования) = Тип("ДокументСсылка.ЗаказНаВнутреннееПотребление") Тогда
		
		Если ПредметСогласования.Статус = Перечисления.СтатусыВнутреннихЗаказов.Закрыт Тогда
			Возврат;
		КонецЕсли;

		Объект = ПредметСогласования.ПолучитьОбъект();
		Если Состояние = Перечисления.СостоянияСогласованияВДокументообороте.НаСогласовании Тогда
			Если Объект.Статус <> Перечисления.СтатусыВнутреннихЗаказов.КОбеспечению Тогда
            	ПерезаписатьОбъект = Истина;
//Рарус Владимир Подрезов 19.12.2016
//				Объект.Статус = Перечисления.СтатусыЗаказовПоставщикам.КОбеспечению;
...
//Рарус Владимир Подрезов Конец
			КонецЕсли;	
		ИначеЕсли Состояние = Перечисления.СостоянияСогласованияВДокументообороте.Согласован Тогда
			Если Объект.Статус <> Перечисления.СтатусыВнутреннихЗаказов.КВыполнению Тогда
            	ПерезаписатьОбъект = Истина;
//Рарус Владимир Подрезов 19.12.2016
//				Объект.Статус = Перечисления.СтатусыЗаказовПоставщикам.КВыполнению;
				Объект.Статус = Перечисления.СтатусыВнутреннихЗаказов.КВыполнению;
//Рарус Владимир Подрезов Конец
			КонецЕсли;	
		КонецЕсли;

	ИначеЕсли ТипЗнч(ПредметСогласования) = Тип("ДокументСсылка.ЗаказНаПеремещение") Тогда
...
//rarus+ saveld 31.10.2016	
	КомандапПкИнтеграция = Метаданные.ОбщиеКоманды.пкИнтеграцияС1СДокументооборот;
	
	ТипыПараметраКоманды = КомандапПкИнтеграция.ТипПараметраКоманды.Типы();
