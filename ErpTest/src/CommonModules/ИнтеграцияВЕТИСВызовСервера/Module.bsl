...
//
Функция ПодготовитьКПередаче(ВходящиеДанные, ПараметрыПередачи, УникальныйИдентификатор = Неопределено, ХозяйствующиеСубъекты = Неопределено, НастройкаОбмена = Неопределено) Экспорт
...
Функция ОтменитьПоследнююОперацию(ДокументСсылка) Экспорт
	...
	ДанныеПоследнегоСообщения = ПоследнееСообщение(ДокументСсылка, "Передано");
...
Процедура ОтметитьДокументКакОбработанный(ДокументСсылка) Экспорт
...
//		** Ссылка - ОпределяемыйТип.ОрганизацияКонтрагентВЕТИС, Неопределено - ссылка на сопоставленный объект или Неопределено.
...
Функция ПредставленияСопоставлений(ХозяйствующийСубъект, Предприятие = Неопределено, ИмяХозяйствующегоСубъекта = "") Экспорт
	...
	СтруктураВозврата = Новый Структура("КонтрагентХозяйствующегоСубъекта, ТорговыйОбъект, ПроизводственныйОбъект");
	...
	Если Выборка.Следующий() Тогда
		Если ЗначениеЗаполнено(Выборка.Контрагент) Тогда
			СтруктураВозврата.КонтрагентХозяйствующегоСубъекта.Представление = Новый ФорматированнаяСтрока(
				Строка(Выборка.Контрагент),,,, ПолучитьНавигационнуюСсылку(Выборка.Контрагент));
			СтруктураВозврата.КонтрагентХозяйствующегоСубъекта.Ссылка = Выборка.Контрагент;
		Иначе
			СтруктураВозврата.КонтрагентХозяйствующегоСубъекта.Представление = Новый ФорматированнаяСтрока(
				НСтр("ru = '<не сопоставлено>'"),, ЦветаСтиля.ЦветОсобогоТекста,, ПолучитьНавигационнуюСсылку(ХозяйствующийСубъект));
		КонецЕсли;
		Если ЗначениеЗаполнено(Предприятие) Тогда
			Если ЗначениеЗаполнено(Выборка.ТорговыйОбъект) Тогда
				СтруктураВозврата.ТорговыйОбъект.Представление = Новый ФорматированнаяСтрока(
					Строка(Выборка.ТорговыйОбъект),,,, ПолучитьНавигационнуюСсылку(Выборка.ТорговыйОбъект));
				СтруктураВозврата.ТорговыйОбъект.Ссылка = Выборка.ТорговыйОбъект;
			Иначе
				СтруктураВозврата.ТорговыйОбъект.Представление = Новый ФорматированнаяСтрока(
			...
			КонецЕсли;
			Если ЗначениеЗаполнено(Выборка.ПроизводственныйОбъект) Тогда
				СтруктураВозврата.ПроизводственныйОбъект.Представление = Новый ФорматированнаяСтрока(
					Строка(Выборка.ПроизводственныйОбъект),,,, ПолучитьНавигационнуюСсылку(Выборка.ПроизводственныйОбъект));
				СтруктураВозврата.ПроизводственныйОбъект.Ссылка = Выборка.ПроизводственныйОбъект;
			Иначе
				СтруктураВозврата.ПроизводственныйОбъект.Представление = Новый ФорматированнаяСтрока(
					НСтр("ru = '<не сопоставлено>'"),, 
					ЦветаСтиля.ЦветОсобогоТекста,, 
...
Функция ВыполнитьСинхронизациюКлассификаторов(ХозяйствующиеСубъекты = Неопределено, ТипыВЕТИС = Неопределено, ПараметрыОбмена = Неопределено) Экспорт
	...
	ЗагрузитьНезагруженныеЭлементыКлассификаторов(ХозяйствующиеСубъекты, ТипыВЕТИС, ПараметрыОбмена);
	
КонецФункции

Функция ЗагрузитьНезагруженныеЭлементыКлассификаторов(ХозяйствующиеСубъекты = Неопределено, ТипыВЕТИС = Неопределено, ПараметрыОбмена = Неопределено) Экспорт
...
Функция ВыполнитьОбмен(ХозяйствующиеСубъекты = Неопределено, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если УникальныйИдентификатор <> Неопределено И Не РежимРаботыБезФоновыхЗаданий() Тогда
		
		ВозвращаемоеЗначение = Новый Структура;
		ВозвращаемоеЗначение.Вставить("Изменения",                 Новый Массив);
		ВозвращаемоеЗначение.Вставить("ДлительнаяОперация",        Неопределено);
		ВозвращаемоеЗначение.Вставить("Ожидать",                   ВремяОжидания(ПараметрыОбмена));
...
Функция ПродолжитьВыполнениеОбмена(АдресВоВременномХранилище) Экспорт
	...
	Если ПараметрыОбмена.УникальныйИдентификатор <> Неопределено И Не РежимРаботыБезФоновыхЗаданий() Тогда
...
Функция ВыполнитьОбменУниверсально(ПараметрыОбмена) Экспорт
	...
	ОбработатьОчередьСообщений(ПараметрыОбмена);
	...
	Если ТребуетсяОжидание Тогда
		...
		Если ПараметрыОбмена.ОжидатьНаСервере Тогда
			ИнтеграцияВЕТИС.Ожидать(ВремяОжидания);
	...
	Иначе
		...
		ИнтеграцияВЕТИСВызовСервера.ЗагрузитьНезагруженныеЭлементыКлассификаторов(,,ПараметрыОбмена);
...
Функция ПроверитьРезультатОбработкиЗаявок(ХозяйствующиеСубъекты = Неопределено, СообщениеДокумент = Неопределено, УникальныйИдентификатор = Неопределено) Экспорт
...
//	РезультатВМассиве - Булево - вернуть результат в виде массива
//
// Возвращаемое значение:
//	если РезультатВМассиве = Ложь, то Структура - документы ВЕТИС
//		Ключ - имя документа ВЕТИС из метаданных
//	если РезультатВМассиве = Истина, то Массив - документы ВЕТИС
//
Функция ИменаДокументовВЕТИСДляДокументаОснования(ДокументОснование, РезультатВМассиве = Ложь) Экспорт
	
	Если РезультатВМассиве Тогда
		ИменаДокументовВЕТИС = Новый Массив;
	Иначе
		ИменаДокументовВЕТИС = Новый Структура;
	КонецЕсли;
	
	Для Каждого ТипДокументаВЕТИС Из Метаданные.ОпределяемыеТипы.ДокументыВЕТИСПоддерживающиеСтатусыОформления.Тип.Типы() Цикл
		
		МетаданныеДокументаВЕТИС = Метаданные.НайтиПоТипу(ТипДокументаВЕТИС);
		ТипыОснованияВЕТИС       = ИнтеграцияВЕТИС.ДокументыОснованияДляДокументаВЕТИС(МетаданныеДокументаВЕТИС);
		
		Если ТипыОснованияВЕТИС.СодержитТип(ТипЗнч(ДокументОснование)) Тогда
			Если РезультатВМассиве Тогда
				ИменаДокументовВЕТИС.Добавить(МетаданныеДокументаВЕТИС.Имя);
			Иначе
				ИменаДокументовВЕТИС.Вставить(МетаданныеДокументаВЕТИС.Имя);
			КонецЕсли;
		КонецЕсли;
	...
	Если РезультатВМассиве Тогда
		...
		ЭлементВходящаяТТН  = ИменаДокументовВЕТИС.Найти(Метаданные.Документы.ВходящаяТранспортнаяОперацияВЕТИС.Имя);
		ЭлементИсходящаяТТН = ИменаДокументовВЕТИС.Найти(Метаданные.Документы.ИсходящаяТранспортнаяОперацияВЕТИС.Имя);
		
		Если ЭлементВходящаяТТН <> Неопределено И ЭлементИсходящаяТТН <> Неопределено Тогда
			
			// Упорядочим входящую и исходящую операции по приоритету - сначала входящая, потом исходящая
			ИменаДокументовВЕТИС.Удалить(ЭлементВходящаяТТН);
			ИменаДокументовВЕТИС.Удалить(ЭлементИсходящаяТТН);
			
			ИменаДокументовВЕТИС.Добавить(Метаданные.Документы.ВходящаяТранспортнаяОперацияВЕТИС.Имя);
			ИменаДокументовВЕТИС.Добавить(Метаданные.Документы.ИсходящаяТранспортнаяОперацияВЕТИС.Имя);
			
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат ИменаДокументовВЕТИС;
...
// Возвращаемое значение:
//	Структура - Структура со свойствами:
//		* НаименованиеНоменклатурыВЕТИС	- ОпределяемыйТип.СтрокаВЕТИС		- Наименование номенклатуры.
//		* АртикулНоменклатурыВЕТИС		- ОпределяемыйТип.СтрокаВЕТИС		- Артикул номенклатуры.
...
Функция ПараметрыСозданияНоменклатуры(ПродукцияВЕТИС, ЕдиницаИзмеренияВЕТИС) Экспорт
	...
	ПараметрыНоменклатуры = Новый Структура("НаименованиеНоменклатурыВЕТИС, АртикулНоменклатурыВЕТИС, ШтрихкодВЕТИС,"
	...
	Если РеквизитыПродукцииВЕТИС.Следующий() Тогда
		ПараметрыНоменклатуры.НаименованиеНоменклатурыВЕТИС = РеквизитыПродукцииВЕТИС.Наименование;
		ПараметрыНоменклатуры.АртикулНоменклатурыВЕТИС      = РеквизитыПродукцииВЕТИС.Артикул;
		ПараметрыНоменклатуры.ШтрихкодВЕТИС                 = РеквизитыПродукцииВЕТИС.Штрихкод;
	КонецЕсли;
...
Функция ДанныеАдресаПоАдресуXML(АдресXML, АдресПредставление) Экспорт
	...
	Если СведенияОбАдресе.Идентификаторы.Свойство("Город")
		...
		ДанныеАдреса.НаселенныйПунктGUID          = Строка(СведенияОбАдресе.Идентификаторы.Город);
		ДанныеАдреса.НаселенныйПунктПредставление = СтрШаблон("%1 %2", СведенияОбАдресе.ГородСокращение, СведенияОбАдресе.Город);
	ИначеЕсли СведенияОбАдресе.Идентификаторы.Свойство("НаселенныйПункт")
...
Функция ОтправитьДляОбработки(ТелоЗапроса, Порт, АдресРесурса, SOAPAction, НастройкаОбмена)
	...
	ЗащищенноеСоединение = ИнтеграцияВЕТИСПовтИсп.ЗащищенноеСоединение();
...
Процедура ОбработатьОчередьСообщений(ПараметрыОбмена)
	...
	Пока ПовторитьЦиклОбработкиСообщений Цикл
		...
		Для Каждого ЭлементОчереди Из ПараметрыОбмена.ОчередьСообщений Цикл
			...
			Если Не ЗначениеЗаполнено(ЭлементОчереди.ИдентификаторЗаявки) Тогда
				...
				Если СообщениеМожноПередавать(ЭлементОчереди, ПараметрыОбмена, НайденныеЗависимости) Тогда
			...
			Иначе
				...
				Если СообщениеМожноЗагружать(ЭлементОчереди, ПараметрыОбмена, НайденныеЗависимости) Тогда
					...
					Если КоличествоЭлементовОчередиПослеОбработки = КоличествоЭлементовОчередиДоОбработки Тогда
	...
	Если МинимальноеКоличествоИтераций <> Неопределено
		И МинимальноеКоличествоИтераций > 20 Тогда
...
Функция ВремяОжидания(ПараметрыОбмена)
	...
	Если ПараметрыОбмена.НомерИтерации < 15 Тогда
		ВремяОжидания = 1;
		Для НомерИтерации = 2 По ПараметрыОбмена.НомерИтерации Цикл
...
Функция ОтменитьОперацию(ДокументСсылка, ДанныеПоследнегоСообщения)
	...
	ПредыдущаяОперация = ИнтеграцияВЕТИС.ПредыдущаяОперация(ПоследовательностьОпераций, СтрокаПоследовательности);
	Если ПредыдущаяОперация = Неопределено Тогда
		
		ДанныеЗаписи = РегистрыСведений.СтатусыДокументовВЕТИС.ЗначенияПоУмолчанию(ДокументСсылка);
		РегистрыСведений.СтатусыДокументовВЕТИС.ВыполнитьЗаписьВРегистр(ДанныеЗаписи);
		
		НовыйСтатусПослеОбновления = ДанныеЗаписи.Статус;
	...
	Иначе
		...
		Если ПредыдущаяОперация.ТипСообщения = Перечисления.ТипыЗапросовИС.Исходящий Тогда
			ПараметрыОбновления = МенеджерОбъекта.СтатусПослеПередачиДанных(
				ДокументСсылка,
				ПредыдущаяОперация.Операция, Перечисления.СтатусыОбработкиСообщенийВЕТИС.ЗаявкаПринята, Неопределено);
		Иначе
			ПараметрыОбновления = МенеджерОбъекта.СтатусПослеПолученияДанных(
				ДокументСсылка,
				ПредыдущаяОперация.Операция, Неопределено);
		КонецЕсли;
...
Функция ПоследнееСообщение(ДокументСсылка, ТипПоследнегоСообщения = "Передано") Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЕТИСПрисоединенныеФайлы.Ссылка        КАК Сообщение,
	|	ВЕТИСПрисоединенныеФайлы.ТипСообщения  КАК ТипСообщения,
	|	ВЕТИСПрисоединенныеФайлы.ВладелецФайла КАК ХозяйствующийСубъект,
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВЕТИСПрисоединенныеФайлы КАК ОтветНаПередачуДанных
	|		ПО ВЕТИСПрисоединенныеФайлы.Ссылка = ОтветНаПередачуДанных.СообщениеОснование
	|		И (ВЕТИСПрисоединенныеФайлы.Операция = ОтветНаПередачуДанных.Операция)
	...
	|	ВЫБОР
	|		КОГДА ВЕТИСПрисоединенныеФайлы.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовИС.Исходящий) ТОГДА
	|			0
	|		ИНАЧЕ
	|			1
	|	КОНЕЦ УБЫВ
	...
	Если ТипПоследнегоСообщения = "Передано" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие",
			"
			|ВЫБОР КОГДА ВЕТИСПрисоединенныеФайлы.ТипСообщения = ЗНАЧЕНИЕ(Перечисление.ТипыЗапросовИС.Исходящий)
	...
	ИначеЕсли ТипПоследнегоСообщения = "СОшибкой" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Условие",
			"
			|ВЫБОР
			|	КОГДА ВЫРАЗИТЬ(ВЕТИСПрисоединенныеФайлы.Описание КАК Строка(255))= """"
			|		ТОГДА ЛОЖЬ
			|	ИНАЧЕ ИСТИНА
			|КОНЕЦ
			|");
...
//  Булево - Истина, если включен режим работы с тестовым контуром.
//
Функция РежимРаботыБезФоновыхЗаданий()
	
	ПараметрЗапускаПриложения = ПараметрыСеанса.ПараметрыКлиентаНаСервере.Получить("ПараметрЗапуска");
	
	Возврат СтрНайти(ВРег(ПараметрЗапускаПриложения), ВРег("РежимРаботыБезФоновыхЗаданийВЕТИС")) > 0;
