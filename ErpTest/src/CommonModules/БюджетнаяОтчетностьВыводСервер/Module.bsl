...
Функция НеобязательныеПараметрыПолученияВыборкиЯчеек() Экспорт
	Параметры = Новый Структура("Колонка, Строка, ИндексыПоиска, ДобавлятьФлаг");
...
Функция ПолучитьВыборкуЯчеек(ДанныеЯчеек, НакопленныйОтбор,
	...
	ДобавлятьФлаг = Ложь;
	Если НЕ НеобязательныеПараметры = Неопределено Тогда
...
Процедура ВыводПолучениеДанныхДляБюджетногоОтчетаВыводБюджетногоОтчета(ПараметрыЭтаповРасчета, Результат, ПродолжитьРасчет)
	...
	ДанныеЯчеекДобавленныеСтроки = ПараметрыВывода.ДанныеЯчеек.СкопироватьКолонки(); // Таблица для добавления строк.
	...
	ПараметрыВывода.Удалить("КэшРеквизитовЭлементовБюджета");
...
Функция ПолучитьИзмененияВЯчейках(ВидБюджета,
	...
	РезультатИзменений = РассчитатьИзмененияВОтчете(СтруктураОписанияОтчета.ПараметрыЯчеек,
	                                                ПредыдущиеДанныеЯчеек,
	                                                НовыеДанныеЯчеек,
	                                                СтрокиКРасчету,
	                                                МассивКПересчету,
	                                                ПараметрыПериодов,
	                                                ИндексыТаблицыДанныхЯчеек);
...
Функция ПолучитьДанныеПоИсточникам(ВидБюджета, СКДДанных, ПараметрыДанных, ИсточникиДанных, СтруктураЯчеек, КэшДанных)
	...
	Если КэшДанных.Свойство("КэшРеквизитовЭлементовБюджета") Тогда
		КэшРеквизитовЭлементовБюджета = КэшДанных.КэшРеквизитовЭлементовБюджета;
	Иначе
		КэшРеквизитовЭлементовБюджета = Новый Структура;
		КэшРеквизитовЭлементовБюджета.Вставить("КэшДопРеквизитов",
			ФинансоваяОтчетностьСервер.КэшДополнительныеРеквизиты(ВидБюджета));
		КэшРеквизитовЭлементовБюджета.Вставить("КэшРеквизитов",
			БюджетнаяОтчетностьРасчетКэшаСервер.КэшРеквизитовЭлементовВидаБюджета(ВидБюджета));
	...
	Если ИсточникиПериодовГруппировок.Количество() > 0 Тогда
		...
		Для Каждого Источник Из ИсточникиПериодовГруппировок Цикл
			...
			Запрос.Текст = "Выбрать "
				+ СтрЗаменить(Источник.ВыражениеНачалоПериодаГруппировки,
				              "ЛеваяТаблицаПериодов.Период" + МинимальнаяПериодичность,
				              "&НачалоПериода")
				+ " КАК НачалоПериода, "
				+ СтрЗаменить(Источник.ВыражениеКонецПериодаГруппировки,
				              "ЛеваяТаблицаПериодов.Период" + МинимальнаяПериодичность,
				              "&КонецПериода")
				+ " КАК КонецПериода ";
...
Процедура РазбитьЯчейкиПоПериодам(ТекстЗапроса, ОписаниеОтчета, ВспомогательныеДанные, ИмяТаблицы)
	...
	Для Каждого Измерение Из ОписаниеОтчета.Периодичности Цикл
		ИмяИзмерения = ОбщегоНазначения.ИмяЗначенияПеречисления(Измерение);
		Если Измерение = Перечисления.Периодичность.Неделя Тогда
			ТекстСоединенияПериодичностей = ТекстСоединенияПериодичностей + "
			|	И ( СтруктураЯчеек.ФлагПериод" + ИмяИзмерения + " = """"
			|			ИЛИ
			|			СтруктураЯчеек.ВыражениеПериода" + ИмяИзмерения + " = -1
			|			ИЛИ
			|			(СтруктураЯчеек.ВыражениеПериода" + ИмяИзмерения + " = 0
			|				И ДобавитьКДате(&НачалоПериода, " + ИмяИзмерения + ", СтруктураЯчеек.ВыражениеПериода" + ИмяИзмерения + ") = 
			|					ТаблицаПериодов.Период" + ИмяИзмерения + "Отчет)
			|			ИЛИ
			|			(СтруктураЯчеек.ВыражениеПериода" + ИмяИзмерения + " > 0
			|				И ДобавитьКДате(НачалоПериода(&НачалоПериода," + ИмяИзмерения + "), " + ИмяИзмерения + ", СтруктураЯчеек.ВыражениеПериода" + ИмяИзмерения + ") = 
			|					ТаблицаПериодов.Период" + ИмяИзмерения + "Отчет)
			|		)";
		Иначе
			ТекстСоединенияПериодичностей = ТекстСоединенияПериодичностей + "
		...
		Иначе
			...
			Для Каждого Измерение Из ОставшиесяИзмерения Цикл
				...
				Если Измерение = "Валюта" Тогда
					ТекстЗапроса = ТекстЗапроса + " ВЫБОР КОГДА ВЫРАЗИТЬ(ТаблицаДанных.НефинансовыйПоказатель КАК Справочник.НефинансовыеПоказателиБюджетов).ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыНефинансовыхПоказателей.Денежный)
					|		ТОГДА ТаблицаДанных." + Измерение + " = ТаблицаЗаполнения." + Измерение + "
					|		ИНАЧЕ ИСТИНА
					|	КОНЕЦ
					|";
				ИначеЕсли Измерение = "ЕдиницаИзмерения" Тогда
					ТекстЗапроса = ТекстЗапроса + " ВЫБОР КОГДА ВЫРАЗИТЬ(ТаблицаДанных.НефинансовыйПоказатель КАК Справочник.НефинансовыеПоказателиБюджетов).ВидПоказателя = ЗНАЧЕНИЕ(Перечисление.ВидыНефинансовыхПоказателей.Количественный)
					|		ТОГДА ТаблицаДанных." + Измерение + " = ТаблицаЗаполнения." + Измерение + "
					|		ИНАЧЕ ИСТИНА
					|	КОНЕЦ
					|";
...
Процедура ВыполнитьСоединениеСтруктурыСДанными(ТекстЗапроса, ОписаниеОтчета, ВспомогательныеДанные, ИмяТаблицы, ВспомогательныеПараметры, РежимФормирования)
	...
	Для Каждого Измерение Из ОписаниеОтчета.Измерения Цикл
		...
		Если Измерение = "Сценарий" 
			...
			ТекстЗапроса = ТекстЗапроса + "
			|	И (СтруктураЯчеек.Флаг" + Измерение + " = ""заполнить"" ИЛИ СтруктураЯчеек.Флаг" + Измерение + " = """"
	...
	Если РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Отчет Тогда
		...
		Для Каждого Измерение Из ОписаниеОтчета.ВидыАналитик Цикл
			...
			ПустоеЗначение = БюджетированиеКлиентСервер.ПустоеЗначениеАналитики();
...
Функция ПараметрыГруппировокКолонок(ПараметрыВывода, ИндексыПоискаИзмерений, ДеревоВывода,
	...
	Для Каждого Колонка Из ДеревоВывода.Строки Цикл
		...
		Иначе
			...
			Результат = ПолучитьВыборкуЯчеекИерархически(ДанныеЯчеек, Колонка.ИмяИзмерения, НакопленныйОтбор, НеобязательныеПараметры);
			...
			Для Каждого Выборка Из Результат Цикл
				...
				Если БюджетнаяОтчетностьКлиентСервер.ЛеваяЧастьИмениСовпадает(Колонка.ИмяИзмерения, "Период")
...
Функция ПолучитьЗначениеОперанда(ДанныеЯчеек, ПараметрыЯчеек, ПараметрыПериодов, ПараметрыРасчета,
                                 ВидЭлемента = Неопределено, ЛогРасчета = Неопределено)
		...
		ИначеЕсли ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПроизводныйПоказатель
			...
			Для каждого НаборИзмерений Из НаборыИзмерений Цикл
				...
				Если ВидЭлементаСтроки = Перечисления.ВидыЭлементовФинансовогоОтчета.ФормулаПоГруппе Тогда
					...
					Иначе
						ВидЭлементаСтроки = БюджетнаяОтчетностьРасчетКэшаСервер.ЗначениеРеквизитаЭлементаВидаБюджета(СтрокаДанных.Колонка,
							"ВидЭлемента", КэшРеквизитовЭлементовБюджета.КэшРеквизитов);
					КонецЕсли;
...
//	 ** ДанныеЯчеек           - ТаблицаЗначений - Таблица данных ячеек.
//	 ** ПараметрыЯчеек        - ТаблицаЗначений - Таблица параметров ячеек.
//	 ** ПараметрыПериодов     - Структура       - Структура параметров периодов.
//	 ** СтрокаДанных          - СтрокаТаблицыЗначений - Рассчитываемая строка данных (таблицы данных ячеек).
...
//	 ** КэшРеквизитовЭлементовБюджета - Структура       - Кэш настроек элементов вида бюджета.
//	                                  - Неопределено    - Значение по умолчанию. Кэш настроек элементов вида бюджета отсутствует.
//	* ЛогРасчета              - Массив          - Массив для записи лога расчета.
//	                          - Неопределено    - Значение по умолчанию. Запись лога не производится.
//
// Возвращаемое значение:
...
Функция РассчитатьСвязанныеСтатьиБюджетов(ПараметрыВыводаВидаБюджета, АдресДанныхЯчеек, ПараметрыДанных,
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(ИмяОбласти) = Тип("Структура") Тогда
			...
			КонецЕсли;
			Если РезультатОтражения.Свойство("Примечание")
	...
	Пока Итерация <= 2 Цикл
		...
		Для Каждого ЯчейкаОтчета Из СтрокиКПересчету Цикл
			...
			Если Не НайденныеНовыеСтроки.Количество() Тогда
				НоваяСтрока = НовыеДанныеЯчеек.Добавить();
				Если ЯчейкаОтчета.РасчетЗаполнение = "итог" Тогда
...
Процедура ОбходДанныхТаблицыОтчета(ПараметрыВывода, СтрокиОтчета, НакопленныйОтбор = Неопределено, Уровень = 1)
...
Процедура ОбходЯчеекТаблицыОтчета(ПараметрыВывода, СтрокаОтчета, СтруктураКолонок, НакопленныйОтбор)
	...
	Для Каждого Колонка Из СтруктураКолонок.Строки Цикл
		...
		Если Колонка.Строки.Количество() Тогда
			ОбходЯчеекТаблицыОтчета(ПараметрыВывода, СтрокаОтчета, Колонка, НакопленныйОтбор);
		Иначе
...
Процедура ВывестиЯчейкиВОтчет(ПараметрыВывода, СчетчикТаблиц, ОбластьЗначение, ОбластьПримечание, СтрокаОтчета, СтруктураКолонок, НакопленныйОтбор, СчетчикКолонок = 1)
						...
						Если МожноРедактировать И Не ВалютаВводаОтличаетсяОтВалюты Тогда
							...
							Если Не СтрокаОтчета.Строки.Количество() Тогда
								...
								Если Строки[0].ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.РедактируемоеЗначение
									...
									КонецЕсли;
								Иначе
									Если ПараметрыФлагов.ФлагКоличество
									   И НЕ ПараметрыФлагов.ФлагСумма
									   И НЕ ПараметрыФлагов.ФлагЗначение Тогда
									...
									ИначеЕсли ПараметрыФлагов.ФлагКоличество Тогда
										...
										ПараметрыЧислаТипа = ОписаниеТипаДенежногоПоля.КвалификаторыЧисла;
										// Увеличиваем разрядность и разрядность дробной части на 1.
										Область.ТипЗначения = ОбщегоНазначенияУТ.ПолучитьОписаниеТиповЧисла(ПараметрыЧислаТипа.Разрядность + 1, 3);
									Иначе
...
Функция ПолучитьБазовуюКомпоновкуДляРасчетаОформления(ЭлементыОформления, ТипОформляемойОбласти, ДанныеОтчетаДляОформления)
	...
	ВычисляемоеПоле = ФинОтчеты.НовоеВычисляемоеПоле(КомпоновкаПоЯчейкам,
	                                                 "ЗначениеЯчейки",
	                                                 "
	                                                 |ВЫБОР 
...
Функция Итог(ДанныеЯчеек, ПараметрыЯчеек, ПараметрыПериодов, Префикс,
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		ПараметрыРасчета.Вставить("КэшРеквизитовЭлементовБюджета", Неопределено);
		Значение = ПолучитьЗначениеОперанда(ДанныеЯчеек, ПараметрыЯчеек, ПараметрыПериодов, ПараметрыРасчета,
			НайденнаяСтрока.ВидОперанда);
	...
	Для Каждого ВидАналитики Из ОписаниеОтчета.ВидыАналитик Цикл
		...
		Если Не ИсточникВсеЭлементы Тогда
			...
			Для Каждого ВидАналитикиУточняемый Из ОписаниеОтчета.ВидыАналитик Цикл
				...
				КонецЕсли;
				ТекстЗапроса = ТекстЗапроса + ТекстУсловияПоСледующейАналитике;
			КонецЦикла;
		...
		Иначе
			...
			Если ПараметрыЗаполненияИзмерений.РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Заполнение
			 ИЛИ ПараметрыЗаполненияИзмерений.РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Документ Тогда
				ТекстПодзапросаПоАналитике =
				"ВЫБРАТЬ
			...
			ТекстПодзапросаПоАналитике = СтрЗаменить(ТекстПодзапросаПоАналитике, "&ПустоеЗначениеАналитики", ПустоеЗначениеАналитики);
			ТекстПодзапросаПоАналитике = СтрЗаменить(ТекстПодзапросаПоАналитике, "&ИмяПоляТаблицыДанных", "ТаблицаДанных." + ИмяПоля);
