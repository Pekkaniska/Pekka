...
Функция КодыАдреса(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Передано недопустимое значение Адреса для метода АдресныйКлассификаторСлужебный.КодыАдреса.'");
	КонецЕсли;
...
//
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина, ЗагружатьИсториюИзменений = Ложь, ЗагружатьПорциями = Ложь) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Попытка
			...
			ЗагрузитьАдресныеОбъекты(СубъектРФ, КаталогФайлов, ЗагружатьПорциями);
			ЗагрузитьДомаЗданияСтроения(СубъектРФ, КаталогФайлов, ЗагружатьПорциями);
			
			Если ЗагружатьИсториюИзменений Тогда
				ЗагрузитьИсториюАдресныхОбъектов(СубъектРФ, КаталогФайлов, ЗагружатьПорциями);
	...
	Для Каждого ЭлементПроверки Из АдресаДляПроверки Цикл
		...
		Если ОдиночнаяПроверка.Ошибки.Количество() > 0
			...
			И ЭтоМуниципальныйАдрес(ЭлементПроверки.Адрес.addressType) Тогда
				...
				Адрес = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ЭлементПроверки.Адрес);
				Адрес.munDistrict     = "";
...
// 
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораВнутр(Результат, ПочтовыйИндекс, ДополнительныеПараметры)
...
|		ПО (ДополнительныеСведения.Идентификатор = АдресныйОбъект.ДополнительныеАдресныеСведения)";
	...
	Для каждого АдресСтрока Из Адреса.Выгрузить() Цикл
		...
		Если КакиеУровниПроверять["Distict"] <> Неопределено И АдресСтрока["Наименование" + КакиеУровниПроверять["Distict"]] = NULL Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Район ""%1"" отсутствует в ФИАС'"), Адрес.District + " " + Адрес.DistrictType));
		КонецЕсли;
	...
	// Земельные участки не проверяем
	Если СтрСравнить(Адрес.HouseType, НаименованиеЗемельногоУчастка()) = 0 Тогда
		Возврат Результат;
	КонецЕсли;
					...
					Если СтрСравнить(НомерДомаАдрес, Дом) = 0 Тогда
						...
						Если СтрСравнить(ТипДомаАдрес, НазваниеВладения) = 0 Тогда
							...
							Иначе
								...
								ПохожиеВарианты.Добавить(ПредставлениеДомаИСтроений(НазваниеВладения, Дом, Корпус, НазваниеСтроения, Строение));
	...
	Если НЕ ДомаКорректный Тогда
		...
		Если ПохожиеВарианты.Количество() > 0 Тогда
			...
			ПервыйВариант = ПохожиеВарианты[0];
			...
			Иначе
				...
				Если ПохожиеВарианты.Количество() = 2 Тогда
					...
					ВторойВариант = ПохожиеВарианты[1];
...
Функция НаименованиеЗемельногоУчастка()
	
	Возврат НСтр("ru='зем. участок'");
...
Процедура ЗагрузитьАдресныеОбъекты(КодСубъектаРФ, КаталогФайлов, ЗагружатьПорциями)
...
Процедура ЗагрузитьДомаЗданияСтроения(КодСубъектаРФ, КаталогФайлов, ЗагружатьПорциями)
...
Процедура ЗагрузитьИсториюАдресныхОбъектов(КодСубъектаРФ, КаталогФайлов, ЗагружатьПорциями)
...
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Представление = "") Экспорт
	...
	Если ТипЗнч(КонтактнаяИнформация) = Тип("Строка") Тогда
		...
		ИначеЕсли ЭтоКонтактнаяИнформацияВJSON(КонтактнаяИнформация) Тогда
			АдресИзJSON = СтрокаJSONВСтруктуру(КонтактнаяИнформация);
...
Процедура УстановитьИдентификаторыДляМуниципальногоАдреса(Адрес)
	...
	КопияАдреса = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(Адрес);
