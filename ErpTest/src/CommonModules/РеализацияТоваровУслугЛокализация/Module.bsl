...
Процедура ПередЗаписью(Объект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности Тогда
...
//+++rarus-spb_zlov 13.01.2017
	|	ТаблицаДокумента.пкДатаНачалаАренды,
	|	ТаблицаДокумента.пкДатаОкончанияАренды,
	|	ТаблицаДокумента.пкТехника,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Номенклатура.ВидНоменклатуры.пкЭтоУслугаТранспортировки
	|			ТОГДА ТаблицаДокумента.НомерСтроки
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК НомерСтрокиТранспортировки,
	|	ПОДСТРОКА(ТаблицаДокумента.пкПримечаниеДляПечати, 1, 250) КАК пкПримечаниеДляПечати,
//---rarus-spb_zlov 13.01.2017
//++ Рарус Лимаренко 28.12.2018
...
#Область РеализацияБезПереходаПраваСобственности // (Дт 45.01 :: Кт 41.01)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Отгрузка без перехода права собственности (Дт 45.01 :: Кт 41.01)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
Функция ТекстВыручкаОтРеализацииКомиссионногоТовара()
	...
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	ВидыЗапасовДокумента.Договор.Подразделение КАК ПодразделениеКт,
	|	ВидыЗапасовДокумента.Договор.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|
...
Функция ТекстНДССОтгрузкиБезПереходаПраваСобственности()
	...
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС),
	|			ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ))
	|	И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
	|	И Операция.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.КПредоплате)
	|	И Строки.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|	И (
	|		ВЫБОР КОГДА Операция.ВернутьМногооборотнуюТару ТОГДА
...
#Область ЗалоговаяСтоимостьПереданнойТарыСкладПроизводства // (Дт 76 :: Кт 20)
	...
	|	Аналитика.Номенклатура КАК СубконтоКт1,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.МатериальныеЗатраты) КАК СубконтоКт2,
	|	Аналитика.Номенклатура.ГруппаАналитическогоУчета КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
...
#Область ОтклонениеВСтоимостиРасходы // (Дт 44.01 :: Кт 41.01)
	ТекстЗапроса = "
...
#Область ОтклонениеВСтоимостиРасходыСкладПроизводства // (Дт 44.01 :: Кт 20)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Отклонение в стоимости возвращенных товаров на складе производства (Дт 44.01 :: Кт 20)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#Область ОтклонениеВСтоимостиДоходы // (Дт 41.01 :: Кт 91.01)
	ТекстЗапроса = "
...
#Область ОтклонениеВСтоимостиДоходыСкладПроизводства // (Дт 20 :: Кт 91.01)
	ТекстЗапроса = "
	|ВЫБРАТЬ //// Отклонение в стоимости возвращенных товаров на складе производства (Дт 20 :: Кт 91.01)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
