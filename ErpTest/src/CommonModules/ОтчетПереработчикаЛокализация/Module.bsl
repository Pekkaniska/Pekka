...
#Область ЗакупкаУслуги // (Дт 08, 2Х, 44, 91 :: Кт 60, 71, 76)
	...
	|	ПО
	//++ НЕ УТКА
	|		ЕСТЬNULL(СпрПартииПроизводстваЭтапа.Назначение,
	//-- НЕ УТКА
...
#КонецОбласти
	...
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетПереработчика.ВозвратныеОтходы КАК ТаблицаПродукции
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаУчетаНоменклатуры.Склад КАК Справочник.СтруктураПредприятия), Операция.Подразделение) КАК ПодразделениеДт,
	...
	|			ЕСТЬNULL(ВЫРАЗИТЬ(АналитикаУчетаНоменклатуры.Склад КАК Справочник.СтруктураПредприятия), Операция.Подразделение)			
