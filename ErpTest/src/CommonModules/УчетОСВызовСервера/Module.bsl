...
Процедура ИнициализироватьПараметрыЗапросаПриОтраженииАмортизации(Запрос, ДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("НачалоУчета2_4", Константы.ДатаНачалаУчетаВнеоборотныхАктивов2_4.Получить());
...
// Вызывается когда ОС принимается к учету или снимается с учета.
//
Процедура СформироватьЗаданиеАмортизацияОС(ДокументСсылка, ДанныеТаблиц) Экспорт
	...
	|	0 КАК НомерПакета,
	|	МИНИМУМ(ВЫБОР 
	|				КОГДА ТИПЗНАЧЕНИЯ(ИзмененныеДанные.Документ) = ТИП(Документ.РегистрацияНаработок)
...
// Вызывается когда НМА принимается к учету или снимается с учета.
// Задание формируется всегда.
//
Процедура СформироватьЗаданиеАмортизацияНМА(ДокументСсылка, ДанныеТаблиц) Экспорт
...
Процедура СформироватьЗаданиеПризнаниеВНУЛизинговыхПлатежей(ДанныеТаблиц, ОпределитьПериод = Ложь, ЭтоИзменениеУчетнойПолитики = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОпределитьПериод Тогда
			ПериодЗадания = СледующийПериодПризнанияВНУЛизинговыхПлатежей(Выборка.Организация, Выборка.Месяц);
		Иначе
...
Функция ПодготовитьТаблицуАрендныхПлатежей(ТаблицаОС, Реквизиты, КоличествоОбработанныхДанных, Отказ)
	...
	Иначе
		Запрос.УстановитьПараметр("ПериодСостояния", КонецМесяца(Реквизиты.Период));
	...
	|	-РасчетыПоФинансовымИнструментам.СуммаРеглНачальныйОстаток КАК ВалютнаяСуммаНачальныйОстаток,
	|	РасчетыПоФинансовымИнструментам.СуммаРеглРасход КАК ВалютнаяСуммаОборотКт
	...
	|				ИЗ
	|					ВТ_ОтборРасчетов)) КАК РасчетыПоФинансовымИнструментам
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент,
	|	ДоговорКонтрагента
