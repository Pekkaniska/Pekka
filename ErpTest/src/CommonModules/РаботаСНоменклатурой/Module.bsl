...
Процедура ПриСозданииНаСервереФормаНоменклатуры(Форма, 
	...
	ДобавитьЭлементыФормыСервиса(Форма, "ПредставлениеНоменклатурыСервиса", 
		ГруппаРодитель, "1С:Номенклатура", РежимПредставленияОбъектаСервиса);
		
	Если ЗначениеЗаполнено(ДанныеИнтерактивногоЗаполнения) Тогда
		ЗаполнитьРеквизитыИнтерактивногоСоздания(Форма, ДанныеИнтерактивногоЗаполнения);
	Иначе
		ЗаполнитьДанныеПоНоменклатуре(Форма, СсылкаНаОбъект, 
			РежимПредставленияОбъектаСервиса, ОтображатьДанныеХарактеристик);			
	КонецЕсли;
...
Процедура ПриЗаписиНаСервереФормаНоменклатуры(Форма, ТекущийОбъект, Отказ) Экспорт
	...
	ЗаписатьСоответствиеНоменклатурыИХарактеристик(ТекущийОбъект.Ссылка, Неопределено, ПараметрыЗаписи);	
...
Процедура ПриСозданииНаСервереФормаВидаНоменклатуры(Форма, 
	...
	ЗаполнитьДанныеПоКатегории(Форма, СсылкаНаОбъект, РежимПредставленияОбъектаСервиса);
	
	// Интерактивное заполнение вида номенклатуры.
	
	Если ЭтоИнтерактивноеСоздание(Форма, СсылкаНаОбъект) Тогда
		ЗаполнитьРеквизитыВидаНоменклатурыВФорме(Форма);		
	КонецЕсли;
...
Функция ПараметрыЗаписиСоответствияНоменклатуры() Экспорт
	...
	Параметры.Вставить("РежимЗагрузкиХарактеристик", 
		Перечисления.РежимыЗагрузкиХарактеристикРаботаСНоменклатурой.ПустаяСсылка());
...
//
Процедура ЗаписатьСоответствиеНоменклатурыИХарактеристик(Знач НоменклатураСсылка, Знач ХарактеристикаСсылка, ДанныеЗаполнения) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения.ИдентификаторНоменклатурыСервиса) Тогда
		...
		НоваяЗапись.ИдентификаторХарактеристикиСервиса  = ДанныеЗаполнения.ИдентификаторХарактеристикиСервиса;

		НоваяЗапись.ПредставлениеНоменклатурыСервиса    = ДанныеЗаполнения.ПредставлениеНоменклатурыСервиса;
		НоваяЗапись.ПредставлениеХарактеристикиСервиса  = ДанныеЗаполнения.ПредставлениеХарактеристикиСервиса;

		НоваяЗапись.РежимЗагрузкиХарактеристик          = ДанныеЗаполнения.РежимЗагрузкиХарактеристик;
...
//  только по номенклатуре.
//
// Параметры:
//  СсылкиНаОбъекты	 - Структура - структура ссылок на объекты.
//    Ключи:
...
Функция ИдентификаторыНоменклатурыИХарактеристикПоСсылкам(Знач СсылкиНаОбъекты) Экспорт
	...
	|	СоответствиеНоменклатурыРаботаСНоменклатурой.ИдентификаторХарактеристикиСервиса КАК ИдентификаторХарактеристикиСервиса,
	|	1 КАК Приоритет
	|ПОМЕСТИТЬ СсылкиИИдентификаторы
	|ИЗ
	|	РегистрСведений.СоответствиеНоменклатурыРаботаСНоменклатурой КАК СоответствиеНоменклатурыРаботаСНоменклатурой
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	СсылкиИИдентификаторы.Номенклатура КАК Номенклатура,
	|	СсылкиИИдентификаторы.Характеристика КАК Характеристика,
	|	СсылкиИИдентификаторы.ИдентификаторНоменклатурыСервиса КАК ИдентификаторНоменклатуры,
	|	СсылкиИИдентификаторы.ИдентификаторХарактеристикиСервиса КАК ИдентификаторХарактеристики
	|ИЗ
	|	СсылкиИИдентификаторы КАК СсылкиИИдентификаторы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Приоритет";
	
	Запрос.УстановитьПараметр("Номенклатура",         СсылкиНаОбъекты.Номенклатура);
	Запрос.УстановитьПараметр("Характеристика",       СсылкиНаОбъекты.Характеристика);
	Запрос.УстановитьПараметр("ПустаяХарактеристика", ПустаяСсылкаНаХарактеристику());
...
//
Функция ПолучитьСоответствиеНоменклатурыПоИдентификаторам(Знач ИдентификаторыНоменклатуры) Экспорт
...
Процедура ПолучитьДанныеНоменклатурыСервиса(ПараметрыВызова, Знач АдресРезультата) Экспорт
	...
	Если ТипЗнч(ПараметрыВызова.ИдентификаторыНоменклатуры) = Тип("Массив") 
		ИЛИ ТипЗнч(ПараметрыВызова.ИдентификаторыНоменклатуры) = Тип("Строка") Тогда
		
		// Если переданы идентификаторы номенклатуры - запрос идет только по номенклатуре.
		
		ДанныеНоменклатуры = ДанныеНоменклатурыСервиса(ПараметрыВызова.ИдентификаторыНоменклатуры);
	...
	ИначеЕсли ТипЗнч(ПараметрыВызова.ИдентификаторыНоменклатуры) = Тип("Соответствие") Тогда	
		...
		Для каждого ЭлементКоллекции Из ПараметрыВызова.ИдентификаторыНоменклатуры Цикл
		...
		Для каждого ЭлементКоллекции Из ДанныеНоменклатуры Цикл
			...
			Если ПараметрыВызова.ИдентификаторыНоменклатуры[ЭлементКоллекции.Идентификатор] = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ПараметрыЗапросаХарактеристик = РаботаСНоменклатуройСлужебныйКлиентСервер.ПараметрыЗапросаХарактеристик();
			
			ПараметрыЗапросаХарактеристик.ИдентификаторыХарактеристик = ПараметрыВызова.ИдентификаторыНоменклатуры[ЭлементКоллекции.Идентификатор];
...
//
Функция ДанныеНоменклатурыСервиса(Знач ИдентификаторыНоменклатуры) Экспорт
...
Процедура ПолучитьНоменклатуруПоШтрихкодам(Штрихкоды, Знач АдресРезультата) Экспорт
	...
	ПоместитьВоВременноеХранилище(ТаблицаНоменклатуры, АдресРезультата);
...
Процедура НайтиКатегорииПоСтрокеПоиска(Знач ПоисковаяСтрока, Знач АдресРезультата) Экспорт
	...
	ПоместитьВоВременноеХранилище(Новый Структура("Категории", ТаблицаКатегорий), АдресРезультата);
...
//
Функция ПолучитьПолноеОписаниеКатегорииПоИдентификатору(Знач ИдентификаторыКатегорий, Знач АдресРезультата = Неопределено) Экспорт
...
Процедура СформироватьПредставлениеКарточкиКатегории(Знач ИдентификаторыКатегорий, Знач АдресРезультата) Экспорт
	...
	ДанныеПоКатегории = ПолучитьПолноеОписаниеКатегорииПоИдентификатору(ИдентификаторыКатегорий);
	...
	Для каждого ЭлементКоллекции Из ДанныеПоКатегории Цикл
		
		ТабличныйДокумент = Новый ТабличныйДокумент;
		
		ЗаполнитьКарточкуВидаНоменклатуры(ТабличныйДокумент, ЭлементКоллекции);	
		
		Данные = Новый Структура("Идентификатор, ПредставлениеКатегории");
		
		Данные.Идентификатор = ЭлементКоллекции.Идентификатор;
...
Процедура СформироватьПредставленияКарточекНоменклатуры(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт 
	...
	Если ДанныеПоНоменклатуре = Неопределено 
		...
		СформироватьТехническоеПредставлениеНоменклатуры(ИдентификаторНоменклатурыВМассиве, 
			ДанныеПоНоменклатуре, ХранилищеКэшей, ПредставлениеНоменклатуры);
	Иначе
		...
		СформироватьКарточкуНоменклатуры(ЭлементКоллекции, ДанныеХарактеристики, ПутьКТабличномуДокументу, ЭтоРежимПросмотра);
...
Функция РежимыЗагрузкиИКоличествоХарактеристик(Номенклатура) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РежимыЗагрузки.ИдентификаторНоменклатурыСервиса КАК ИдентификаторНоменклатуры,
	|	РежимыЗагрузки.РежимЗагрузкиХарактеристик КАК РежимЗагрузкиХарактеристик,
	|	ЕСТЬNULL(СУММА(ВЫБОР
	|				КОГДА СоответствиеНоменклатурыРаботаСНоменклатурой.ИдентификаторХарактеристикиСервиса = """"
	|					ТОГДА 0
	|				ИНАЧЕ 1
	|			КОНЕЦ), 0) КАК КоличествоХарактеристик
	|ИЗ
	|	РежимыЗагрузки КАК РежимыЗагрузки
	...
	|		ПО РежимыЗагрузки.ИдентификаторНоменклатурыСервиса = СоответствиеНоменклатурыРаботаСНоменклатурой.ИдентификаторНоменклатурыСервиса
	|
	|СГРУППИРОВАТЬ ПО
	|	РежимыЗагрузки.ИдентификаторНоменклатурыСервиса,
	|	РежимыЗагрузки.РежимЗагрузкиХарактеристик";
...
Функция ТаблицаШтрихкодов(ДанныеЗаполнения, СсылкаНаОбъект)
...
Процедура ЗагрузкаКатегорий(Знач ПараметрыЗагрузки, ДолжноБытьСоздано, Знач АдресРезультата, Знач АдресДополнительногоРезультата)
	...
	Если ПараметрыЗагрузки.РежимЗагрузкиКатегорий = "Привязка" Тогда
		...
		Результат.Вставить("Создано",           ДанныеКатегорий.Количество());
		Результат.Вставить("ДолжноБытьСоздано", ДанныеКатегорий.Количество());
	...
	ИначеЕсли ПараметрыЗагрузки.РежимЗагрузкиКатегорий = "Объединение" Тогда	
		...
		Результат.Вставить("Создано",               0);
		Результат.Вставить("ДолжноБытьСоздано",     0);
		Результат.Вставить("НовыеЭлементы",         Новый Массив);
		Результат.Вставить("АдресДанныхЗаполнения", ПоместитьВоВременноеХранилище(ДанныеКатегорий, АдресДополнительногоРезультата));
		
	ИначеЕсли ПараметрыЗагрузки.РежимЗагрузкиКатегорий = "ПоОтдельности" Тогда	
		
		// Загрузка категорий по-отдельности.
		...
		Если СохранятьИерархиюКатегорийПриЗагрузке()
			И ПараметрыЗагрузки.КоличествоУровнейИерархии > 0 Тогда
			
			СоздатьРодительскиеКатегории(ДанныеКатегорий, СозданныеРодительскиеКатегории, ПараметрыЗагрузки.КоличествоУровнейИерархии);
		КонецЕсли;
...
Процедура ПодготовитьДанныеДляСозданияНоменклатуры(
	...
	Для каждого ЭлементКоллекции Из ПараметрСоздания.Идентификаторы Цикл
		...
		Если Не НастройкиПодсистемы().ИспользоватьХарактеристики Тогда
...
Процедура СоздатьНоменклатуруИХарактеристики(Знач ДанныеТекущейНоменклатуры, ПараметрыТекущейНоменклатуры, СозданнаяНоменклатура, ТекущаяХарактеристика = Неопределено)
	...
	РаботаСНоменклатуройПереопределяемый.ЗаписатьШтрихкоды(ТаблицаШтрихкодов(ДанныеТекущейНоменклатуры, СсылкаНаОбъект));
...
Процедура ЗаполнитьДанныеПоКатегории(Форма, СсылкаНаОбъект, РежимПредставленияОбъектаСервиса)
...
Процедура ЗаполнитьДанныеПоНоменклатуре(Форма, СсылкаНаОбъект, РежимПредставленияОбъектаСервиса, ОтображатьДанныеХарактеристик)
	...
	КоличествоХарактеристикСопоставлено = 0;
	...
	Если Выборка.Следующий() Тогда
		...
		Если ОтображатьДанныеХарактеристик 
			И Выборка.РежимЗагрузкиХарактеристик 
				= Перечисления.РежимыЗагрузкиХарактеристикРаботаСНоменклатурой.ЗагружатьХарактеристикиКакХарактеристики Тогда
			
			СформироватьЗаголовокГиперссылкиХарактеристик(Форма);
		КонецЕсли;
...
Процедура УстановитьСтатусыПроверкиНоменклатуры(ДанныеПоНоменклатуре, ИдентификаторыСПараметрами)
	...
	Для каждого СтрокаНоменклатуры Из ДанныеПоНоменклатуре Цикл
		...
		ДополнитьДанныеИзПараметровСоздания(СтрокаНоменклатуры, ДополнительныеПараметры);
		...
		Если СтрокаНоменклатуры.СтатусПроверки.Количество() > 0
			И ЕстьХарактеристики Тогда
...
Функция ПолучитьФильтрыКатегорииВСервисе(Знач ИдентификаторКатегории, Отказ = Ложь)
	...
	Для каждого ЭлементКоллекции Из ТаблицаКатегорий[0].МерныеХарактеристики Цикл
		Если ЗначениеЗаполнено(ЭлементКоллекции.Значение.ЕдиницаИзмерения.Наименование) Тогда
			...
			НоваяСтрока.Наименование  = ЭлементКоллекции.Ключ;
...
Процедура ДополнитьДанныеИзПараметровСоздания(СтрокаНоменклатуры, ДополнительныеПараметры)
	...
	Если ПараметрыПоиска.Свойство("ДополнительныеРеквизиты") Тогда 
		...
		Для Каждого Реквизит Из ПараметрыПоиска.ДополнительныеРеквизиты Цикл 
			Если ЭтоМернаяХарактеристика(Реквизит) Тогда
				ПараметрыЗапроса.МерныеХарактеристики[Реквизит.Идентификатор] = Реквизит.Значение;
			Иначе
...
Функция СписокНоменклатурыИХарактеристик(Знач ПараметрыПоиска, Отказ = Ложь)
	...
	Если ПараметрыПоиска.РежимОтображенияХарактеристик = "Линейный" Тогда
		...
		ЗаполнитьСписокНоменклатурыЛинейно(ТаблицаНоменклатуры, ПараметрыПоиска.НомерСтраницыДанных);	
		
	ИначеЕсли ПараметрыПоиска.РежимОтображенияХарактеристик = "Группировка" Тогда	
		// Заполнение номенклатуры с группировкой по номенклатуре
		ЗаполнитьСписокНоменклатурыСГруппировкой(ТаблицаНоменклатуры);	
		
	ИначеЕсли ПараметрыПоиска.РежимОтображенияХарактеристик = "БезХарактеристик" Тогда	
...
Процедура ЗаполнитьСписокНоменклатурыЛинейно(ТаблицаНоменклатуры, НомерСтраницыДанных)
		...
		Если ТаблицаНоменклатуры[Счетчик].КоличествоХарактеристик > 0 Тогда
			...
			Для каждого ЭлементКоллекции Из ТаблицаНоменклатуры[Счетчик].Характеристики Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТаблицаНоменклатуры[Счетчик], 
					"Наименование, Идентификатор, ПредставлениеЕдиницыИзмерения, КоличествоХарактеристик");
				...
				НоваяСтрока.Наименование = НаименованиеПоНоменклатуреИХарактеристике(
					НоваяСтрока.Наименование, НоваяСтрока.НаименованиеХарактеристики);
	...
	|	СоответствиеНоменклатурыРаботаСНоменклатурой.Номенклатура КАК Номенклатура,
	|	СоответствиеНоменклатурыРаботаСНоменклатурой.Характеристика КАК Характеристика,
	|	ЕСТЬNULL(ИдентификаторыИСсылкиНоменклатуры.РежимЗагрузкиХарактеристик, НЕОПРЕДЕЛЕНО) КАК РежимЗагрузкиХарактеристик
	|ИЗ
	|	НоменклатураИХарактеристикиСервиса КАК НоменклатураИХарактеристикиСервиса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыРаботаСНоменклатурой КАК СоответствиеНоменклатурыРаботаСНоменклатурой
	|		ПО НоменклатураИХарактеристикиСервиса.ИдентификаторНоменклатуры = СоответствиеНоменклатурыРаботаСНоменклатурой.ИдентификаторНоменклатурыСервиса
	...
	|ВЫБРАТЬ
	|	НоменклатураИХарактеристикиСервиса.ИдентификаторНоменклатуры,
	|	НоменклатураИХарактеристикиСервиса.ИдентификаторХарактеристики,
	|	СоответствиеНоменклатурыРаботаСНоменклатурой.Номенклатура,
	|	СоответствиеНоменклатурыРаботаСНоменклатурой.Характеристика,
	|	ЕСТЬNULL(СоответствиеНоменклатурыРаботаСНоменклатурой.РежимЗагрузкиХарактеристик, НЕОПРЕДЕЛЕНО)
	|ИЗ
	|	НоменклатураИХарактеристикиСервиса КАК НоменклатураИХарактеристикиСервиса
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыРаботаСНоменклатурой КАК СоответствиеНоменклатурыРаботаСНоменклатурой
	|		ПО НоменклатураИХарактеристикиСервиса.ИдентификаторНоменклатуры = СоответствиеНоменклатурыРаботаСНоменклатурой.ИдентификаторНоменклатурыСервиса
	|			И НоменклатураИХарактеристикиСервиса.ИдентификаторХарактеристики = СоответствиеНоменклатурыРаботаСНоменклатурой.ИдентификаторХарактеристикиСервиса
	|ГДЕ
	|	НЕ СоответствиеНоменклатурыРаботаСНоменклатурой.Номенклатура ЕСТЬ NULL";
	...
	Для каждого ЭлементКоллекции Из ТаблицаНоменклатуры Цикл
		...
		СтрокиНоменклатуры = ДанныеНоменклатуры.НайтиСтроки(Новый Структура("ИдентификаторНоменклатуры, ИдентификаторХарактеристики",
			ЭлементКоллекции.Идентификатор, ЭлементКоллекции.ИдентификаторХарактеристики));
		...
		Иначе
			...
			Если СтрокаНоменклатуры.РежимЗагрузкиХарактеристик 
				= Перечисления.РежимыЗагрузкиХарактеристикРаботаСНоменклатурой.НеЗагружатьХарактеристики Тогда
				
				// Если номенклатура загружается без характеристик - характеристики не доступны для загрузки.
...
Процедура ЗаполнитьСписокНоменклатурыСГруппировкой(ТаблицаНоменклатуры)
	...
	Запрос.УстановитьПараметр("Идентификаторы", ТаблицаНоменклатуры.ВыгрузитьКолонку("Идентификатор"));
	...
	Для каждого ЭлементКоллекции Из ТаблицаНоменклатуры Цикл
		...
		СтрокаНоменклатуры = ДанныеНоменклатуры.Найти(ЭлементКоллекции.Идентификатор, "ИдентификаторНоменклатуры");
...
Процедура ВывестиИзображения(Изображения, ОбластьКартинка, ТабличныйДокумент)
	
	Если Не ЗначениеЗаполнено(Изображения) Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеИзображения = РаботаСНоменклатуройСлужебный.ДвоичныеДанныеПоАдресуURL(Изображения[0].ИзображениеURL);
	
	ОбластьКартинка.Рисунки.Рисунок.Картинка = Новый Картинка(ДанныеИзображения);
...
Процедура СформироватьКарточкуНоменклатуры(Знач ДанныеНоменклатуры, Знач ДанныеХарактеристики, Знач АдресРезультата, ЭтоРежимПросмотра)
	...
	Если ПлатныеРеквизиты.Свойство("Изображения") Тогда
		...
		ТабличныйДокумент.Вывести(ОбластьКартинка);
	Иначе
		ВывестиИзображения(ДанныеНоменклатуры.Изображения, ОбластьКартинка, ТабличныйДокумент);
	КонецЕсли;
		
	ЗаполнитьДополнительныеРеквизитыИХарактеристикиНоменклатуры(ДанныеНоменклатуры, ДанныеХарактеристики, 
...
Процедура ЗаполнитьДополнительныеРеквизитыИХарактеристикиНоменклатуры(ДанныеНоменклатуры, ДанныеХарактеристики, ТабличныйДокумент, Макет, КарточкаКуплена)
	...
	Если НЕ КарточкаКуплена Тогда
		...
		// Если карточка не была куплена - выводятся заглушки реквизитов.
	...
	Иначе
		...
		Иначе
			...
			Если ДанныеНоменклатуры.Характеристики.Количество() > 0 Тогда
				...
				// Гиперссылка на оставшиеся характеристики.	
...
Процедура ЗагрузитьНоменклатуруИХарактеристикиПродолжение(ПараметрыПроцедуры, АдресРезультата, АдресДополнительногоРезультата, КоличествоСозданныхЭлементов)
	...
	Если ИдентификаторыНоменклатуры.Количество() = 0 Тогда
		...
		// Если создавались характеристики и больше объектов нет - метод завершается
	...
	Если ВедетсяУчетВидовНоменклатуры() Тогда
		...
		Для каждого СтрокаДанныхПоНоменклатуре Из ДанныеПоНоменклатуре Цикл
			ЗаполнитьСоответствиеРеквизитовИЗначенийНоменклатуры(
				СоответствиеРеквизитовИЗначений, СтрокаДанныхПоНоменклатуре, 
					ИдентификаторыСПараметрами[СтрокаДанныхПоНоменклатуре.Идентификатор].ПараметрыСоздания.ВидНоменклатуры, Ложь);
		КонецЦикла;
...
Функция ВеткаКатегорийНоменклатуры(Знач ИдентификаторыКатегорий)
...
Процедура СформироватьЗаголовокГиперссылкиХарактеристик(Форма, КоличествоХарактеристикСопоставлено = Неопределено)
...
//
// Параметры:
//  Номенклатура - Ссылка, Строка - ссылка на номенклатуру или идентификатор номенклатуры сервиса.
// 
// Возвращаемое значение:
//  Ссылка, Неопределено - Перечисление.РежимыЗагрузкиХарактеристикРаботаСНоменклатурой.
//
Функция РежимЗагрузкиХарактеристик(Номенклатура)
	
	Результат = Ложь;
	
	Если ТипЗнч(Номенклатура) = Тип("Строка") Тогда
		
		// Передан идентификатор номенклатуры
		...
		Запрос.УстановитьПараметр("ИдентификаторНоменклатуры", Номенклатура);
		Запрос.УстановитьПараметр("ПустаяХарактеристика",      ПустаяСсылкаНаХарактеристику());
		
	ИначеЕсли ТипЗнч(Номенклатура) = Метаданные.ОпределяемыеТипы.НоменклатураРаботаСНоменклатурой.Тип.Типы()[0] Тогда
		
		// Передана ссылка на номенклатуру	
...
Процедура ЗаполнитьПризнакЗагрузкиХарактеристик(ИдентификаторНоменклатуры, ДанныеХарактеристик)
	...
	Если РежимЗагрузки = Перечисления.РежимыЗагрузкиХарактеристикРаботаСНоменклатурой.НеЗагружатьХарактеристики Тогда
...
Процедура ЗаполнитьРеквизитыИнтерактивногоСоздания(Форма, ДанныеЗаполнения)
	
	Форма.РаботаСНоменклатурой_АдресДанныхЗаполнения 
		= Форма.Параметры.РаботаСНоменклатурой_АдресДанныхЗаполнения;
	
	ПредставлениеОбъектаСервиса = Новый ФорматированнаяСтрока(ДанныеЗаполнения[0].Наименование,,,, "Ссылка");
	
	Форма.РаботаСНоменклатурой_ИдентификаторОбъектаСервиса = ДанныеЗаполнения[0].Идентификатор;
...
Процедура ЗаполнитьРеквизитыФормыВидаНоменклатуры(Форма, ПараметрыЗакрытияФормы)
	...
	// Если поменялся перечень категорий или изменен режим обновления, объект помечается как измененный.
...
Процедура ЗаполнитьРеквизитыФормыНоменклатуры(Форма, ПараметрыЗакрытияФормы)
	...
	РаботаСНоменклатуройПереопределяемый.ЗаполнитьНоменклатуруВФорме(Форма, 
		ДанныеДляЗаполнения.Скопировать(
			Новый Структура("Пометка, ЭтоДополнительныйРеквизит", Истина, Ложь)));
...
Процедура ЗаполнитьРеквизитыВидаНоменклатурыВФорме(Форма)
