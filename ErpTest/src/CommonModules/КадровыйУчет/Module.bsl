...
//
Процедура СформироватьДвиженияВидовЗанятостиСотрудников(Движения, КоллекцияВидовЗанятости) Экспорт
			...
			Если Отборы <> Неопределено И Отборы.Количество() > 0 Тогда
				...
				Для Каждого ОписаниеОтбора Из Отборы Цикл
					...
					Если ВРег(ОписаниеОтбора.ЛевоеЗначение) <> ВРег("Сотрудник")
						И ВРег(ОписаниеОтбора.ЛевоеЗначение) <> ВРег("ГоловнаяОрганизация") Тогда
...
Процедура СоздатьВТОсновныеСотрудникиФизическихЛиц(МенеджерВременныхТаблиц, ТолькоРазрешенные, СписокФизическихЛиц, Организация, НачалоПериода, ОкончаниеПериода = '00010101') Экспорт
	...
	Запрос.УстановитьПараметр("ОкончаниеПериода", ?(ЗначениеЗаполнено(ОкончаниеПериода), ОкончаниеПериода, ЗарплатаКадрыПериодическиеРегистры.МаксимальнаяДата()));
	Запрос.УстановитьПараметр("СписокФизическихЛиц", СписокФизическихЛиц);
...
Функция ТаблицаНачисленийСотрудниковПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод = "Период", ИмяПоляСотрудник = "Сотрудник", ИмяПоляДатаОкончания = "", ИспользоватьКраткиеПредставления = Истина, ВыводитьДенежныеПоказателиСВалютой = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты") Тогда
		...
		ОписанияНачислений = ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод, ИмяПоляСотрудник, ИмяПоляДатаОкончания, ИспользоватьКраткиеПредставления, ВыводитьДенежныеПоказателиСВалютой);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.3.72";
	Обработчик.Процедура = "КадровыйУчет.УдалитьСпецсимволыИзСлужебныхИдентификаторовФизическихЛиц";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7145a31e-8592-429e-b58f-526553f0ed6f");
	Обработчик.Комментарий = НСтр("ru='Заполняет служебные наименования физических лиц'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.5.4";
	Обработчик.Процедура = "КадровыйУчет.ПеренестиВидыЗанятостиИзКадроваяИсторияСотрудниковВВидыЗанятостиСотрудников";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("83880fa0-22a8-4c3f-8812-c3d01f0ff813");
	Обработчик.Комментарий = НСтр("ru='Переносит виды занятости в специализированный регистр сведений'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.5.5";
	Обработчик.Процедура = "КадровыйУчет.ЗаполнитьОсновныхСотрудниковФизическихЛиц";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("aaf15f17-b599-46b8-81d9-73feda27d259");
	Обработчик.Комментарий = НСтр("ru='Заполняет основных сотрудников физических лиц'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.5.4";
	Обработчик.Процедура = "КадровыйУчет.ЗаполнитьОформленПоТрудовомуДоговоруТекущихКадровыхДанныхСотрудников";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("94bfc18d-4af4-42f8-b693-54e83794dea3");
	Обработчик.Комментарий = НСтр("ru='Заполняет признак оформленности трудовых отношений'");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.5.81";
	Обработчик.Процедура = "РегистрыСведений.ДокументыФизическихЛиц.ОбновитьПредставленияПаспортовРФ";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c03c48b0-06ab-4559-9102-746374145fbc");
	Обработчик.Комментарий = НСтр("ru='Исправляет представления Паспортов РФ'");
...
Процедура УдалитьСпецсимволыИзСлужебныхИдентификаторовФизическихЛиц(ПараметрыОбновления = Неопределено) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "Справочник.ФизическиеЛица", "Ссылка", Выборка.Ссылка) Тогда
			...
			ФизическоеЛицоОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ФизическоеЛицоОбъект.НаименованиеСлужебное = ФизическиеЛицаЗарплатаКадры.НаименованиеСлужебное(Выборка.Наименование);
			ФизическоеЛицоОбъект.ОбменДанными.Загрузка = Истина;
			ФизическоеЛицоОбъект.Записать();
...
Процедура ПеренестиВидыЗанятостиИзКадроваяИсторияСотрудниковВВидыЗанятостиСотрудников(ПараметрыОбновления = Неопределено) Экспорт
	
	КадровыйУчетВнутренний.ПеренестиВидыЗанятостиИзКадроваяИсторияСотрудниковВВидыЗанятостиСотрудников(ПараметрыОбновления);
	
КонецПроцедуры

Процедура ЗаполнитьОсновныхСотрудниковФизическихЛиц(ПараметрыОбновления = Неопределено) Экспорт
	
	КадровыйУчетВнутренний.ЗаполнитьОсновныхСотрудниковФизическихЛиц(ПараметрыОбновления);
	
КонецПроцедуры

Процедура ЗаполнитьОформленПоТрудовомуДоговоруТекущихКадровыхДанныхСотрудников(ПараметрыОбновления = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		ВидыСобытийНачалаУчета = Новый Массив;
		ВидыСобытийНачалаУчета.Добавить(Перечисления.ВидыКадровыхСобытий.Прием);
		ВидыСобытийНачалаУчета.Добавить(Перечисления.ВидыКадровыхСобытий.НачальныеДанные);
		
		ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(
			ПараметрыПостроения.Отборы, "ВидСобытия", "В", ВидыСобытийНачалаУчета);
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Запись = НаборЗаписей.Добавить();
			ЗаполнитьЗначенияСвойств(Запись, Выборка);
			Запись.ОформленПоТрудовомуДоговору = Истина;
...
Процедура ЗаполнитьНаборыЗначенийДоступаСотрудников() Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СотрудникиНаборыЗначенийДоступа.Ссылка КАК Ссылка,
		|	СотрудникиНаборыЗначенийДоступа.НомерНабора КАК НомерНабора
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СотрудникиСПризнакомОбновления.Сотрудник КАК Сотрудник
		|ИЗ
...
Процедура ЗаполнитьОбособленныеПодразделенияКадровыхПереводов() Экспорт
	...
	СоздатьВТТекущиеКадровыеДанныеСотрудников(Запрос.МенеджерВременныхТаблиц, Ложь);
...
Процедура ЗаполнитьФамилиюИмяИОтчествоПоДаннымРегистра(Параметры) Экспорт
	...
	Если Параметры.ПрогрессВыполнения.ВсегоОбъектов = 0 Тогда
		...
		Если Выборка.КоличествоФизическихЛиц = 0 Тогда
...
Функция ТекстСоединенияДляПолученияДатНачалаУчета(ИмяПоляСотрудник) Экспорт
	...
	Возврат КадровыйУчетВнутренний.ТекстСоединенияДляПолученияДатНачалаУчета(ИмяПоляСотрудник);
