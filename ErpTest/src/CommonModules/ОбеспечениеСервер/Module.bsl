...
//-- НЕ УТКА
		...
		|		ПО Т.Этап = ЭтапПроизводства2_2.Ссылка
		//-- НЕ УТКА
		|
		|ГДЕ
		|	Т.Заказ <> &Ссылка
...
Функция СформироватьВтПараметровОбеспеченияТоваров(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст = Справочники.ФорматыМагазинов.ТекстЗапросаВтФорматыСкладов() +
		...
		|;
		|/////////////////////////////////
		|" + ОбеспечениеСервер.ТекстЗапросаСтатистикиПотребления("ВтТоварыПоСтатистике")
...
// Возвращает текст запроса формирующий временную таблицу со статистикой потребления (ВтСтатистикаПотребления).
//
// Параметры:
//   Источник - Строка - Имя временной таблицы с товарами, по умолчанию: ВтТовары.
//
// Возвращаемое значение:
//   Строка - Текст запроса.
//
Функция ТекстЗапросаСтатистикиПотребления(Источник = Неопределено) Экспорт
	
	ТекстЗапроса =
		// 1. Дополняем таблицу товаров складами обеспечиваемыми распределительными центрами.
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТоварыНаСкладах.Номенклатура   КАК Номенклатура,
		|	ТоварыНаСкладах.Характеристика КАК Характеристика,
		|	ТоварыНаСкладах.Склад          КАК Склад,
		|	ТоварыНаСкладах.УдаленныйСклад КАК УдаленныйСклад
		|ПОМЕСТИТЬ ВтТоварыКУчету
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|	
		|	ВЫБРАТЬ
		|		Способы.Номенклатура,
		|		Способы.Характеристика,
		|		ВЫРАЗИТЬ(Способы.СпособОбеспеченияПотребностей.ИсточникОбеспеченияПотребностей КАК Справочник.Склады),
		|		Способы.Склад
		|	ИЗ
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|//#ВтСезонныеКоэффициенты
		|
		// 2. Получаем все рабочие дни в периоде расчета статистики (от начала периода по конец периода включительно).
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВтГрафикРаботыПоКалендарю
		|ИЗ
		|	ВтТоварыКУчету КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклад
		|		ПО Товары.УдаленныйСклад = СпрСклад.Ссылка
		...
		|	КОЛИЧЕСТВО(График.Дата) КАК Количество
		|ПОМЕСТИТЬ ВтКоличествоРабочихДнейГрафика
		|ИЗ
		|	ВтГрафикРаботыПоКалендарю КАК График
		|СГРУППИРОВАТЬ ПО
		|	График.Календарь
		|;
		|
		// 3. Для каждого дня, в котором было движение товара по складу, определяем
		// были ли остатки в начале и в конце дня или только в конце дня.
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТоварыНаСкладах.Склад                       КАК УдаленныйСклад,
		|	ТоварыНаСкладах.Номенклатура                КАК Номенклатура,
		|	ТоварыНаСкладах.Характеристика              КАК Характеристика,
		...
		|				Товары.Номенклатура   КАК Номенклатура,
		|				Товары.Характеристика КАК Характеристика,
		|				Товары.УдаленныйСклад КАК Склад
		|			ИЗ
		|				ВтТоварыКУчету КАК Товары)) КАК ТоварыНаСкладах
		|ГДЕ
		...
		|				ТОГДА ВЫРАЗИТЬ(ТоварыНаСкладах.Регистратор КАК Документ.ПеремещениеТоваров).СкладПолучатель
		|			КОГДА ТоварыНаСкладах.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыНаСкладах.Регистратор КАК Документ.ПеремещениеТоваров).СкладОтправитель
		|		КОНЕЦ                                       КАК КорСклад,
		|		НАЧАЛОПЕРИОДА(ТоварыНаСкладах.Период, ДЕНЬ) КАК Дата,
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтТоварыКУчету КАК Товары
		|			ПО ТоварыНаСкладах.Номенклатура      = Товары.Номенклатура
		|				И ТоварыНаСкладах.Характеристика = Товары.Характеристика
		|				И ТоварыНаСкладах.Склад          = Товары.УдаленныйСклад
		|	ГДЕ
		|		ТоварыНаСкладах.Период МЕЖДУ &ПериодКлассификацииДатаНачала И &ПериодКлассификацииДатаОкончания
		|		И ТоварыНаСкладах.Активность
		|		И ТоварыНаСкладах.Регистратор ССЫЛКА Документ.ПеремещениеТоваров
		|		И ТоварыНаСкладах.Склад.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.ОптовыйСклад)
		|	
		//++ НЕ УТКА
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		...
		|		ВЫБОР
		|			КОГДА ТоварыНаСкладах.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыНаСкладах.Регистратор КАК Документ.ДвижениеПродукцииИМатериалов).Получатель
		|			КОГДА ТоварыНаСкладах.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|				ТОГДА ВЫРАЗИТЬ(ТоварыНаСкладах.Регистратор КАК Документ.ДвижениеПродукцииИМатериалов).Отправитель
		|		КОНЕЦ                                       КАК КорСклад,
		|		НАЧАЛОПЕРИОДА(ТоварыНаСкладах.Период, ДЕНЬ) КАК Дата,
		|		ТоварыНаСкладах.ВидДвижения                 КАК ВидДвижения,
		|		ТоварыНаСкладах.ВНаличии                    КАК Количество
		...
		|		ТоварыНаСкладах.Период МЕЖДУ &ПериодКлассификацииДатаНачала И &ПериодКлассификацииДатаОкончания
		|		И ТоварыНаСкладах.Активность
		|		И ТоварыНаСкладах.Регистратор ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
		|		И ВЫРАЗИТЬ(ТоварыНаСкладах.Регистратор КАК Документ.ДвижениеПродукцииИМатериалов).ХозяйственнаяОперация В
		...
		|	Перемещения.Склад,
		|	Перемещения.КорСклад,
		|	Перемещения.ВидДвижения,
		|	Перемещения.Дата
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		...
		|;
		|
		// 5. Получаем расход по дням за период расчета статистики.
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РасходТоваров.Номенклатура      КАК Номенклатура,
		|	РасходТоваров.Характеристика    КАК Характеристика,
		|	РасходТоваров.УдаленныйСклад    КАК УдаленныйСклад,
		|	РасходТоваров.Дата              КАК Дата,
		|	СУММА(РасходТоваров.Количество) КАК Количество,
		|	СУММА(РасходТоваров.Возврат)    КАК Возврат
		...
		//++ НЕ УТКА
		|		И НЕ ТоварыНаСкладах.Регистратор ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
		//-- НЕ УТКА
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		...
		|		И ТоварыНаСкладах.Активность
		|		И ТИПЗНАЧЕНИЯ(ТоварыНаСкладах.Регистратор) В (ТИП(Документ.ВозвратТоваровОтКлиента),
		//++ НЕ УТ
		...
		//++ НЕ УТ
		|		И ВЫБОР
		|			КОГДА ТоварыНаСкладах.Регистратор ССЫЛКА Документ.ДвижениеПродукцииИМатериалов ТОГДА
		|				ВЫРАЗИТЬ(ТоварыНаСкладах.Регистратор КАК Документ.ДвижениеПродукцииИМатериалов).ХозяйственнаяОперация =
		...
		|		Перемещения.Склад,
		|		Перемещения.Дата,
		|		Перемещения.Количество,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(КорПеремещения.Количество, 0) > Перемещения.Количество ТОГДА
		|				Перемещения.Количество
		|			ИНАЧЕ
		|				ЕСТЬNULL(КорПеремещения.Количество, 0)
		|		КОНЕЦ
		|	ИЗ
		|		ВтПеремещенияПоСкладам КАК Перемещения
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВтПеремещенияПоСкладам КАК КорПеремещения
		|			ПО Перемещения.Номенклатура = КорПеремещения.Номенклатура
		|				И Перемещения.Характеристика = КорПеремещения.Характеристика
		|				И Перемещения.Склад          = КорПеремещения.Склад
		|				И Перемещения.КорСклад       = КорПеремещения.КорСклад
		|				И КорПеремещения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|	ГДЕ
		|		Перемещения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|		И НЕ Перемещения.КорСклад В
		|					(ВЫБРАТЬ
		|						Товары.УдаленныйСклад
		|					ИЗ
		|						ВтТоварыКУчету КАК Товары
		|					ГДЕ
		|						&ОптимизироватьЗапасыРаспределительногоЦентра)) КАК РасходТоваров
		|	
		|СГРУППИРОВАТЬ ПО
		|	РасходТоваров.Номенклатура,
		|	РасходТоваров.Характеристика,
		|	РасходТоваров.УдаленныйСклад,
		|	РасходТоваров.Дата
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура, Характеристика, УдаленныйСклад, Дата
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтПеремещенияПоСкладам
		|;
		|
		// 6. Суммируем расход для расчета среднедневного по простому и продвинутому способу (исключаются дни с пробоем остатка).
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Расход.Номенклатура                                КАК Номенклатура,
		|	Расход.Характеристика                              КАК Характеристика,
		|	Расход.УдаленныйСклад                              КАК УдаленныйСклад,
		|
		|	СУММА(Расход.Количество - Расход.Возврат)           КАК ПотреблениеБезКорректировок,
		|	СУММА((Расход.Количество - Расход.Возврат)
		|		* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)) КАК КоличествоПотребления,
		|	СУММА((Расход.Количество - Расход.Возврат)
		|		* (Расход.Количество - Расход.Возврат)
		|		* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)
		|		* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)) КАК КвадратКоличестваПотребления,
		|	СУММА(ВЫБОР КОГДА Остатки.ЕстьОстатки ТОГДА
		|			(Расход.Количество - Расход.Возврат)
		|			* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)
		|		ИНАЧЕ
		|			- Расход.Возврат
		|			* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)
		|		КОНЕЦ)                                          КАК КоличествоПолногоПотребления,
		|	СУММА(ВЫБОР КОГДА Остатки.ЕстьОстатки ТОГДА
		|			(Расход.Количество - Расход.Возврат)
		|			* (Расход.Количество - Расход.Возврат)
		|			* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)
		...
		|			* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)
		|			* ЕСТЬNULL(СезонныеКоэффициенты.Коэффициент,1)
		|		КОНЕЦ)                                          КАК КвадратКоличестваПолногоПотребления
		|ПОМЕСТИТЬ ВтПотребление
		|ИЗ
		|	ВтРасходТовараПоДням КАК Расход
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
		|		ПО Расход.УдаленныйСклад.Календарь = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтГрафикРаботыПоКалендарю КАК РабочиеДни
		|		ПО ЕСТЬNULL(Константы.ОсновнойКалендарьПредприятия, Расход.УдаленныйСклад.Календарь) = РабочиеДни.Календарь
		|			И Расход.Дата = РабочиеДни.Дата
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОстаткиТовараПоДням КАК Остатки
		|		ПО Расход.Дата           = Остатки.Дата
		|			И Расход.Номенклатура   = Остатки.Номенклатура
		|			И Расход.Характеристика = Остатки.Характеристика
		|			И Расход.УдаленныйСклад = Остатки.УдаленныйСклад
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСезонныеКоэффициенты КАК СезонныеКоэффициенты
		|		ПО ИСТИНА
		|//#СоединениеВтСезонныеКоэффициенты
		|
		|СГРУППИРОВАТЬ ПО
		|	Расход.Номенклатура,
		|	Расход.Характеристика,
		|	Расход.УдаленныйСклад
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Характеристика,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтСезонныеКоэффициенты;
		|
		...
		|	Товары.Номенклатура                     КАК Номенклатура,
		|	Товары.Характеристика                   КАК Характеристика,
		|	Товары.УдаленныйСклад                   КАК УдаленныйСклад,
		|	МАКСИМУМ(Товары.Дата)                   КАК Дата,
		|	МАКСИМУМ(Товары.Дата = РабочиеДни.Дата) КАК ОстатокДеньВДень
		|ПОМЕСТИТЬ ВтДатыОстатка
		|ИЗ
		|	ВтОстаткиТовараПоДням КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК Константы
		|		ПО Товары.УдаленныйСклад.Календарь = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтГрафикРаботыПоКалендарю КАК РабочиеДни
		|		ПО ЕСТЬNULL(Константы.ОсновнойКалендарьПредприятия, Товары.УдаленныйСклад.Календарь) = РабочиеДни.Календарь
		|			И Товары.Дата <= РабочиеДни.Дата
		|
		|СГРУППИРОВАТЬ ПО
		|	Товары.Номенклатура,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтГрафикРаботыПоКалендарю;
		|
		...
		|	ДатыОстатка.УдаленныйСклад КАК УдаленныйСклад,
		|	СУММА(ВЫБОР
		|		КОГДА ДатыОстатка.ОстатокДеньВДень И ЕСТЬNULL(Расход.Количество - Расход.Возврат, 0) <> 0
		|				ИЛИ Остатки.ЕстьКонечныйОстаток
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ)                     КАК ЧислоДнейПотребления,
		|	СУММА(ВЫБОР
		|		КОГДА ДатыОстатка.ОстатокДеньВДень И Остатки.ЕстьОстатки
		|				ИЛИ НЕ ДатыОстатка.ОстатокДеньВДень И Остатки.ЕстьКонечныйОстаток
		|			ТОГДА 1
		|		ИНАЧЕ 0
		|	КОНЕЦ)                     КАК ЧислоДнейПолногоПотребления
		|ПОМЕСТИТЬ ВтКоличествоДнейКУчету
		|ИЗ
		|	ВтДатыОстатка КАК ДатыОстатка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОстаткиТовараПоДням КАК Остатки
		...
		|			И ДатыОстатка.УдаленныйСклад = Остатки.УдаленныйСклад
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтРасходТовараПоДням КАК Расход
		|		ПО ДатыОстатка.Дата              = Расход.Дата
		|			И ДатыОстатка.Номенклатура   = Расход.Номенклатура
		|			И ДатыОстатка.Характеристика = Расход.Характеристика
		|			И ДатыОстатка.УдаленныйСклад = Расход.УдаленныйСклад
		|
		|СГРУППИРОВАТЬ ПО
		|	ДатыОстатка.Номенклатура,
		|	ДатыОстатка.Характеристика,
		|	ДатыОстатка.УдаленныйСклад
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура,
		|	Характеристика,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтРасходТовараПоДням;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтДатыОстатка;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтОстаткиТовараПоДням;
		|
		// 9. Получаем итоговые статистические показатели.
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НаборДанных.Склад                        КАК Склад,
		|	НаборДанных.УдаленныйСклад               КАК УдаленныйСклад,
		|	НаборДанных.Номенклатура                 КАК Номенклатура,
		|	НаборДанных.Характеристика               КАК Характеристика,
		|	
		|	НаборДанных.ПотреблениеБезКорректировок  КАК Потребление,
		|	ВЫБОР
		|		КОГДА НаборДанных.СреднедневноеПотребление >= НаборДанных.СреднедневноеПолноеПотребление
		|			ТОГДА НаборДанных.СреднедневноеПотребление
		|		ИНАЧЕ НаборДанных.СреднедневноеПолноеПотребление
		|	КОНЕЦ                                    КАК СреднедневноеПотребление,
		|	ВЫБОР
		|		КОГДА НаборДанных.СреднедневноеПотребление >= НаборДанных.СреднедневноеПолноеПотребление
		|			ТОГДА НаборДанных.ДисперсияПотребления
		|		ИНАЧЕ НаборДанных.ДисперсияПолногоПотребления
		|	КОНЕЦ                                    КАК ДисперсияПотребления
		|ПОМЕСТИТЬ ВтСтатистикаПотребленияУдаленныхСкладов
		|ИЗ
		|	(ВЫБРАТЬ
		|		Товары.Склад                                          КАК Склад,
		|		Товары.УдаленныйСклад                                 КАК УдаленныйСклад,
		...
		|		Товары.Характеристика                                 КАК Характеристика,
		|		
		|		ЕСТЬNULL(Потребление.ПотреблениеБезКорректировок, 0)  КАК ПотреблениеБезКорректировок,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПотребления, 0) = 0
		|				ТОГДА 0
		|			ИНАЧЕ ЕСТЬNULL(Потребление.КоличествоПотребления, 0) / ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПотребления, 0)
		|		КОНЕЦ                                                 КАК СреднедневноеПотребление,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПолногоПотребления, 0) = 0
		|				ТОГДА 0
		|			ИНАЧЕ ЕСТЬNULL(Потребление.КоличествоПотребления, 0) / ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПолногоПотребления, 0)
		|		КОНЕЦ                                                 КАК СреднедневноеПолноеПотребление,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПотребления, 0) > 1
		|				ТОГДА (ВЫРАЗИТЬ(ЕСТЬNULL(Потребление.КвадратКоличестваПотребления, 0) / (ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПотребления, 0) - 1) КАК ЧИСЛО(15, 3)))
		|					- (ВЫРАЗИТЬ(ЕСТЬNULL(Потребление.КоличествоПотребления, 0) / (ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПотребления, 0) - 1)
		|						* ЕСТЬNULL(Потребление.КоличествоПотребления, 0) / ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПотребления, 0) КАК ЧИСЛО(15, 3)))
		|			ИНАЧЕ 0
		|		КОНЕЦ                                                 КАК ДисперсияПотребления,
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПолногоПотребления, 0) > 1
		|				ТОГДА (ВЫРАЗИТЬ(ЕСТЬNULL(Потребление.КвадратКоличестваПотребления, 0) / (ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПолногоПотребления, 0) - 1) КАК ЧИСЛО(15, 3)))
		|					- (ВЫРАЗИТЬ(ЕСТЬNULL(Потребление.КоличествоПолногоПотребления, 0) / (ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПолногоПотребления, 0) - 1)
		|						* ЕСТЬNULL(Потребление.КоличествоПолногоПотребления, 0) / ЕСТЬNULL(КоличествоДнейКУчету.ЧислоДнейПолногоПотребления, 0) КАК ЧИСЛО(15, 3)))
		|			ИНАЧЕ 0
		|		КОНЕЦ                                                 КАК ДисперсияПолногоПотребления
		|	ИЗ
		|		ВтТоварыКУчету КАК Товары
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВтКоличествоДнейКУчету КАК КоличествоДнейКУчету
		|			ПО Товары.УдаленныйСклад    = КоличествоДнейКУчету.УдаленныйСклад
		|				И Товары.Номенклатура   = КоличествоДнейКУчету.Номенклатура
		|				И Товары.Характеристика = КоличествоДнейКУчету.Характеристика
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВтПотребление КАК Потребление
		|			ПО Товары.УдаленныйСклад    = Потребление.УдаленныйСклад
		|				И Товары.Номенклатура   = Потребление.Номенклатура
		|				И Товары.Характеристика = Потребление.Характеристика
		|	) КАК НаборДанных
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтТоварыКУчету;
		|
		// 10. Вычисляем среднедневное для РЦ, рассчитываем признак стабильности потребления.
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Статистика.Склад                  КАК Склад,
		|	Статистика.Номенклатура           КАК Номенклатура,
		|	Статистика.Характеристика         КАК Характеристика,
		|
		|	&ПериодКлассификацииДатаНачала    КАК ДатаНачалаПериода,
		|	&ПериодКлассификацииДатаОкончания КАК ДатаОкончанияПериода,
		|	СУММА(Статистика.Потребление)     КАК Потребление,
		|
		|	ВЫБОР
		|		КОГДА КалендарьСклад.Количество = 0 ТОГДА
		|			0
		|		ИНАЧЕ
		|			СУММА(Статистика.СреднедневноеПотребление * КалендарьУдаленныйСклад.Количество) / КалендарьСклад.Количество
		|	КОНЕЦ                             КАК СреднедневноеПотребление,
		|	ВЫБОР
		|		КОГДА КалендарьСклад.Количество = 0 ТОГДА
		|			0
		|		ИНАЧЕ
		|			СУММА(Статистика.ДисперсияПотребления * КалендарьУдаленныйСклад.Количество * КалендарьУдаленныйСклад.Количество)
		|				/ (КалендарьСклад.Количество * КалендарьСклад.Количество)
		|	КОНЕЦ                             КАК ДисперсияПотребления,
		|	ВЫБОР
		|		КОГДА КалендарьСклад.Количество = 0 ИЛИ СУММА(Статистика.СреднедневноеПотребление * КалендарьУдаленныйСклад.Количество) = 0 ТОГДА
		|			ЛОЖЬ
		|		ИНАЧЕ
		|			СУММА(Статистика.ДисперсияПотребления * КалендарьУдаленныйСклад.Количество * КалендарьУдаленныйСклад.Количество)
		|				/ СУММА(Статистика.СреднедневноеПотребление * КалендарьУдаленныйСклад.Количество) / КалендарьСклад.Количество < 0.0025
		|	КОНЕЦ                             КАК ПотреблениеСтабильно // вариация меньше 5%
		|ПОМЕСТИТЬ ВтСтатистикаПотребления
		|ИЗ
		|	ВтСтатистикаПотребленияУдаленныхСкладов КАК Статистика
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклад
		|		ПО Статистика.Склад = СпрСклад.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК КонстантыСклад
		|		ПО СпрСклад.Календарь = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтКоличествоРабочихДнейГрафика КАК КалендарьСклад
		|		ПО ЕСТЬNULL(КонстантыСклад.ОсновнойКалендарьПредприятия, СпрСклад.Календарь) = КалендарьСклад.Календарь
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрУдаленныйСклад
		|		ПО Статистика.УдаленныйСклад = СпрУдаленныйСклад.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Константы КАК КонстантыУдаленныйСклад
		|		ПО СпрУдаленныйСклад.Календарь = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтКоличествоРабочихДнейГрафика КАК КалендарьУдаленныйСклад
		|		ПО ЕСТЬNULL(КонстантыУдаленныйСклад.ОсновнойКалендарьПредприятия, СпрУдаленныйСклад.Календарь) = КалендарьУдаленныйСклад.Календарь
		|
		|СГРУППИРОВАТЬ ПО
		|	Статистика.Склад,
		|	Статистика.Номенклатура,
		|	Статистика.Характеристика,
		|	КалендарьСклад.Количество
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Склад,
		|	Номенклатура,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтСтатистикаПотребленияУдаленныхСкладов;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтКоличествоРабочихДнейГрафика;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтКоличествоДнейКУчету;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВтПотребление
		|";
	
	УчетСезонности = Константы.ВариантУчетаСезонныхКолебаний.Получить();
	Если ПолучитьФункциональнуюОпцию("ИспользоватьРасширенноеОбеспечениеПотребностей")
		И УчетСезонности = Перечисления.УчетСезонныхКолебаний.ПоСезоннымКоэффициентам Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//#ВтСезонныеКоэффициенты",
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	СпрНоменклатура.СезоннаяГруппа КАК СезоннаяГруппа,
			|	ВЫБОР
			|		КОГДА НЕ Товары.Склад.БизнесРегион.СезоннаяГруппа ЕСТЬ NULL
			|				И НЕ Товары.Склад.БизнесРегион.СезоннаяГруппа = ЗНАЧЕНИЕ(Справочник.СезонныеГруппыБизнесРегионов.ПустаяСсылка)
			|			ТОГДА Товары.Склад.БизнесРегион.СезоннаяГруппа
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СезонныеГруппыБизнесРегионов.Прочие)
			|	КОНЕЦ                          КАК СезоннаяГруппаБизнесРегиона
			|ПОМЕСТИТЬ ВтСезонныеГруппы
			|ИЗ
			|	ВтТоварыКУчету КАК Товары
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
			...
			|ПОМЕСТИТЬ ВтСезонныеКоэффициенты
			|ИЗ
			|	ВтСезонныеГруппы КАК Группы
			...
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВтСезонныеГруппы;");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//#СоединениеВтСезонныеКоэффициенты",
			"			И Расход.Номенклатура.СезоннаяГруппа = СезонныеКоэффициенты.СезоннаяГруппа
			|			И ВЫБОР
			|				КОГДА НЕ Расход.УдаленныйСклад.БизнесРегион.СезоннаяГруппа ЕСТЬ NULL
			|					И НЕ Расход.УдаленныйСклад.БизнесРегион.СезоннаяГруппа = ЗНАЧЕНИЕ(Справочник.СезонныеГруппыБизнесРегионов.ПустаяСсылка)
			|				ТОГДА Расход.УдаленныйСклад.БизнесРегион.СезоннаяГруппа
			|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СезонныеГруппыБизнесРегионов.Прочие)
			|			КОНЕЦ = СезонныеКоэффициенты.СезоннаяГруппаБизнесРегиона");
		
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользоватьРасширенноеОбеспечениеПотребностей")
		И УчетСезонности = Перечисления.УчетСезонныхКолебаний.ПоАналогичномуПериодуПрошлогоГода Тогда
		
		// Текст запроса статистики по товарной группе за указанный период
		ТекстУП = ТекстЗапроса;
		ТекстУП = СтрЗаменить(ТекстУП, "ТоварыНаСкладах.Номенклатура", "ТоварыНаСкладах.Номенклатура.ТоварнаяКатегория");
		ТекстУП = СтрЗаменить(ТекстУП, "ТоварыНаСкладах.Характеристика", "НЕОПРЕДЕЛЕНО");
		ТекстУП = СтрЗаменить(ТекстУП, "(Номенклатура, Характеристика, Склад)", "(Номенклатура.ТоварнаяКатегория, НЕОПРЕДЕЛЕНО, Склад)");
		ТекстУП = СтрЗаменить(ТекстУП, "//#ВтСезонныеКоэффициенты",
			"ВЫБРАТЬ
			|	1 КАК Коэффициент
			|ПОМЕСТИТЬ ВтСезонныеКоэффициенты;");
		
		// Текст запроса статистики по товарной группе за период прошлого года аналогичный указанному
		ТекстУППГ = ТекстУП;
		ТекстУППГ = СтрЗаменить(ТекстУППГ, "&ПериодКлассификацииДатаНачала",    "ДОБАВИТЬКДАТЕ(&ПериодКлассификацииДатаНачала, ГОД, -1)");
		ТекстУППГ = СтрЗаменить(ТекстУППГ, "&ПериодКлассификацииДатаОкончания", "ДОБАВИТЬКДАТЕ(&ПериодКлассификацииДатаОкончания, ГОД, -1)");
		
		// Текст запроса статистики по товарам за период прошлого года аналогичный очередному
		ТекстОППГ = ТекстЗапроса;
		ТекстОППГ = СтрЗаменить(ТекстОППГ, "&ПериодКлассификацииДатаНачала",    "ДОБАВИТЬКДАТЕ(&РасчетныйПериодДатаНачала, ГОД, -1)");
		ТекстОППГ = СтрЗаменить(ТекстОППГ, "&ПериодКлассификацииДатаОкончания", "ДОБАВИТЬКДАТЕ(&РасчетныйПериодДатаОкончания, ГОД, -1)");
		ТекстОППГ = СтрЗаменить(ТекстОППГ, "//#СоединениеВтСезонныеКоэффициенты",
			"			И Расход.Номенклатура.ТоварнаяКатегория = СезонныеКоэффициенты.ТоварнаяКатегория
			|			И Расход.УдаленныйСклад = СезонныеКоэффициенты.Склад");
		
		ТекстыЗапроса = Новый Массив;
		ТекстыЗапроса.Добавить(ТекстУП);
		ТекстыЗапроса.Добавить(
			"ВЫБРАТЬ
			|	СтатистикаТекущегоПериода.Склад                    КАК Склад,
			|	СтатистикаТекущегоПериода.Номенклатура             КАК ТоварнаяКатегория,
			|	СтатистикаТекущегоПериода.СреднедневноеПотребление КАК СреднедневноеПотребление
			|ПОМЕСТИТЬ ВтСтатистикаТекущегоПериода
			|ИЗ
			|	ВтСтатистикаПотребления КАК СтатистикаТекущегоПериода
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВтСтатистикаПотребления");
		ТекстыЗапроса.Добавить(ТекстУППГ);
		ТекстыЗапроса.Добавить(
			"ВЫБРАТЬ
			|	СтатистикаПрошлогоПериода.Склад                    КАК Склад,
			|	СтатистикаПрошлогоПериода.Номенклатура             КАК ТоварнаяКатегория,
			|	ВЫБОР
			|		КОГДА СтатистикаПрошлогоПериода.СреднедневноеПотребление = 0 ТОГДА
			|			1
			|		ИНАЧЕ
			|			СтатистикаТекущегоПериода.СреднедневноеПотребление / СтатистикаПрошлогоПериода.СреднедневноеПотребление
			|	КОНЕЦ                                              КАК Коэффициент
			|ПОМЕСТИТЬ ВтСезонныеКоэффициенты
			|ИЗ
			|	ВтСтатистикаПотребления КАК СтатистикаПрошлогоПериода
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВтСтатистикаТекущегоПериода КАК СтатистикаТекущегоПериода
			|		ПО СтатистикаПрошлогоПериода.Номенклатура = СтатистикаТекущегоПериода.ТоварнаяКатегория
			|			И СтатистикаПрошлогоПериода.Склад = СтатистикаТекущегоПериода.Склад
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВтСтатистикаПотребления;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВтСтатистикаТекущегоПериода");
		ТекстыЗапроса.Добавить(ТекстОППГ);
	...
	Если ЗначениеЗаполнено(Источник) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВтТовары", Источник);
	КонецЕсли;
	
	Возврат РегистрыСведений.СхемыОбеспечения.ВременнаяТаблицаСпособыОбеспеченияВсехСкладов(Перечисления.ТипыОбеспечения.Перемещение)
		+ ТекстЗапроса +
		";
		|
		|////////////////////////////////////////////////////////////////////////////////
...
Процедура УстановитьПараметрыЗапросаРасчетаСтатистики(Запрос, ПериодКлассификации = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ГрафикБезКалендаря",
		СоздатьПустойГрафик(ПериодКлассификации.ДатаНачала, ПериодКлассификации.ДатаОкончания));
