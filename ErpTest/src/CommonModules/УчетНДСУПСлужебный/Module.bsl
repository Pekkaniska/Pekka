...
// Формирует список доступных значений по настройкам учета и отражаемой операции
//
// Параметры:
// 	ПараметрыЗаполнения - Структура - См. УчетНДСУПКлиентСервер.ПараметрыЗаполненияВидаДеятельностиНДС().
//
// Возвращаемое значение:
// 	Результат - Массив - Доступные виды деятельности НДС.
// 
Функция ДоступныеВидыДеятельностиНДС(ПараметрыЗаполнения) Экспорт
	...
	Если ПараметрыУчетаОрганизации.РаздельныйУчетНДС Тогда
		Если ПараметрыУчетаОрганизации.ОсновнойВидДеятельностиНДСЗакупки = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
			ДоступныеПоНастройкам.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС);
		КонецЕсли;
		Если ПараметрыУчетаОрганизации.ОсновнойВидДеятельностиНДСЗакупки = Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС Тогда
			ДоступныеПоНастройкам.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
		КонецЕсли;
		Если ПараметрыУчетаОрганизации.ОсновнойВидДеятельностиНДСЗакупки = Перечисления.ТипыНалогообложенияНДС.ПоФактическомуИспользованию Тогда
			ДоступныеПоНастройкам.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС);
			ДоступныеПоНастройкам.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС);
		КонецЕсли;
		Если ПараметрыУчетаОрганизации.ПрименяетсяЕНВД Тогда
			ДоступныеПоНастройкам.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД);
		КонецЕсли;
		Если ПараметрыУчетаОрганизации.ОсобыйПорядокУчетаНДСПриПроизводствеДЦ Тогда
	...
	Если ПараметрыУчетаОрганизации.РаздельныйУчетНДС И ПараметрыУчетаИБ.ПродажиНаЭкспорт Тогда
...
// Формирует список доступных значений по отражаемой операции
//
// Параметры:
...
Процедура РассчитатьНДСЗаписиКнигиПокупокПоНДСПредъявленный(ПараметрыРасчета)
	...
	|		И НДСПредъявленный.НаправлениеДеятельности = НДСПредъявленный.НаправлениеДеятельности
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ИсправительныеСчетаФактуры КАК ИСФ
	|	ПО
...
Процедура ВернутьДокументыКОтражениюВУчете(ПараметрыРасчета)
	...
	|	НДСЗаписиКнигиПокупок.Регистратор,
	|	НДСЗаписиКнигиПокупок.Организация,
	|	НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПокупок.Период, ДЕНЬ)
	|ИЗ
	|	НДСЗаписиКнигиПокупокПоАвансамВыданным КАК НДСЗаписиКнигиПокупок
	|ГДЕ
	|	НДСЗаписиКнигиПокупок.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НАЧАЛОПЕРИОДА(ТекущиеРегистраторыАвансы.Период, ДЕНЬ) КАК Период,
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСЗаписиКнигиПокупок.Период,
	|	НДСЗаписиКнигиПокупок.Документ,
	|	НДСЗаписиКнигиПокупок.Организация,
	|	НДСЗаписиКнигиПокупок.ДатаОтражения
	|ИЗ
	|	ДокументыКОтражениюАвансы КАК НДСЗаписиКнигиПокупок
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыВыборочнойРегистрации КАК ДокументыВыборочнойРегистрации
	|		ПО НДСЗаписиКнигиПокупок.Документ = ДокументыВыборочнойРегистрации.Документ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура РассчитатьЗачетАвансовПолученных(ПараметрыРасчета)
	...
	|	АвансыПолученные.ДокументОплаты                                         КАК СчетФактура,
	|	ВЫБОР
	|		КОГДА РасчетыСКлиентами.ХозяйственнаяОперация =
...
#Область ФормированиеИсправительныхСчетовФактур
