...
Функция ПоляКоторыеНеВсегдаТребуетсяЗаполнять(ИмяРеглОтчета, ИмяФормы, Период) Экспорт
	...
	Если ИмяРеглОтчета = "РегламентированныйОтчетСтатистикаФормаП3" 
		И (ИмяФормы = "ФормаОтчета2016Кв1"
			ИЛИ ИмяФормы = "ФормаОтчета2017Кв1"
			ИЛИ ИмяФормы = "ФормаОтчета2018Кв1") Тогда
		...
		Если НЕ ПоследнийМесяцКвартала Тогда
			...
			НеТребуютсяПоляФормы.Добавить("П030004901");
			НеТребуютсяПоляФормы.Добавить("П030004902");
			НеТребуютсяПоляФормы.Добавить("П030005001");
			НеТребуютсяПоляФормы.Добавить("П030005002");
			НеТребуютсяПоляФормы.Добавить("П030005003");
			НеТребуютсяПоляФормы.Добавить("П030005004");
	...
	ИначеЕсли ИмяРеглОтчета = "РегламентированныйОтчетСтатистикаФормаП6" Тогда
		...
		Если Месяц(Период) <> 6 Тогда
			НеТребуютсяПоляФормы.Добавить("П030070003");
			НеТребуютсяПоляФормы.Добавить("П030070004");
			НеТребуютсяПоляФормы.Добавить("П030071003");
			НеТребуютсяПоляФормы.Добавить("П030071004");
			НеТребуютсяПоляФормы.Добавить("П030072003");
			НеТребуютсяПоляФормы.Добавить("П030072004");
			НеТребуютсяПоляФормы.Добавить("П030073003");
			НеТребуютсяПоляФормы.Добавить("П030073004");
			НеТребуютсяПоляФормы.Добавить("П030074003");
			НеТребуютсяПоляФормы.Добавить("П030074004");
			НеТребуютсяПоляФормы.Добавить("П030075003");
			НеТребуютсяПоляФормы.Добавить("П030075004");
			НеТребуютсяПоляФормы.Добавить("П030076003");
			НеТребуютсяПоляФормы.Добавить("П030076004");
			НеТребуютсяПоляФормы.Добавить("П030077003");
			НеТребуютсяПоляФормы.Добавить("П030077004");
			НеТребуютсяПоляФормы.Добавить("П030078003");
			НеТребуютсяПоляФормы.Добавить("П030078004");
			НеТребуютсяПоляФормы.Добавить("П030079003");
			НеТребуютсяПоляФормы.Добавить("П030079004");
			НеТребуютсяПоляФормы.Добавить("П030080003");
			НеТребуютсяПоляФормы.Добавить("П030080004");
		КонецЕсли;
...
//     * Организация - СправочникСсылка.Организации - организация, для которой будет
//                     выполняться поиск отчетов "Расчет по страховым взносам";
//     * ДатаОкончания - Дата - дата окончания отчетного периода для поиска отчетов
//                     "Расчет по страховым взносам".
//
// Возвращаемое значение:
...
Функция ПерсонифицированныеСведенияОЗастрахованныхЛицахИзРасчетовПоСтраховымВзносам(СтруктураПараметров) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИсточникОтчета", "РегламентированныйОтчетРасчетПоСтраховымВзносам");
...
Функция СоответствиеЗаменяемыхКодовОргановФСГС(Знач ОпрашиватьСервис = Ложь) Экспорт
	...
	Если ТекущаяДатаСеанса() > '2018-12-24' Тогда		
	...
	Если ОпрашиватьСервис Тогда
		...
		Исключение
			...
			НСтр("ru = 'Электронный документооброт с контролирующими органами.Получение изменений кодов органов ФСГС на ЦЕМПОС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура РассчитатьПоказателиРеглОтчета(ПараметрыРасчета, Данные) Экспорт
	...
	Если ТипЗнч(Отч) = Тип("ОтчетМенеджер.РегламентированныйОтчетЗемельныйНалог") Тогда
...
Функция РеглОтчетыИсполнителиТребований() Экспорт
	...
	ИсполнителиТребований.Вставить("ДекларацияОВоздействииНаОкружающуюСреду",                            "ДекларацияОВоздействииНаОкружающуюСреду");
...
Функция ОписаниеФормыРеглОтчета(Требование, Организация, РегистрацияВНалоговомОргане, Знач НачалоПериода, Знач КонецПериода, Знач Периодичность) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОписаниеФормыРеглОтчета.Имя) Тогда
		...
		ИначеЕсли Требование = "СведенияОСреднесписочнойЧисленности.2016_РегистрацияОрганизации" Тогда
			...
			ОргСведения = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация, , "ДатаРегистрации");
...
//  * Ссылка                   - ДокументСсылка.РегламетированныйОтчет
...
//
//  Параметры:
//   ПараметрыРеглОтчетов - ТаблицаЗначений:
...
//   * РегистрацияВНалоговомОргане - СправочникСсылка.РегистрацииВНалоговомОргане
//   * Правило                     - СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов - Необязательная колонка.
...
Процедура ЗаполнитьСсылкиНаРеглОтчеты(ПараметрыРеглОтчетов, ОтборПоВыбраннойФормеРеглОтчета = Ложь) Экспорт
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПараметрыРеглОтчетов.Индекс КАК Индекс,
	|	ЕСТЬNULL(РегламентированныйОтчет.Ссылка, ЗНАЧЕНИЕ(Документ.РегламентированныйОтчет.ПустаяСсылка)) КАК Ссылка,
	|	ЕСТЬNULL(РегламентированныйОтчет.ДатаПодписи, ДАТАВРЕМЯ(1, 1, 1)) КАК Дата,
	|	ПараметрыРеглОтчетов.ИсточникОтчета КАК ИсточникОтчета,
	|	ПараметрыРеглОтчетов.Организация КАК Организация,
	|	ПараметрыРеглОтчетов.ДатаОкончания КАК ДатаОкончания,
	|	ПараметрыРеглОтчетов.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ЕСТЬNULL(РегламентированныйОтчет.ВыбраннаяФорма, """") КАК ВыбраннаяФорма,
	|	ПараметрыРеглОтчетов.Правило КАК Правило,
	...
	|		ПО (НЕ РегламентированныйОтчет.ПометкаУдаления)
	|			И ПараметрыРеглОтчетов.Организация = РегламентированныйОтчет.Организация
	|			И ПараметрыРеглОтчетов.ДатаОкончания = РегламентированныйОтчет.ДатаОкончания
	|			И ПараметрыРеглОтчетов.ИсточникОтчета = РегламентированныйОтчет.ИсточникОтчета
	|			И (ПараметрыРеглОтчетов.РегистрацияВНалоговомОргане = ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
	|				ИЛИ ПараметрыРеглОтчетов.РегистрацияВНалоговомОргане.Код = РегламентированныйОтчет.КодНалоговогоОргана)
	|
	|УПОРЯДОЧИТЬ ПО
	|	РегламентированныйОтчет.Дата УБЫВ, РегламентированныйОтчет.Вид УБЫВ
	|ИТОГИ ПО
	|	Индекс";
	...
	Пока ВыборкаИндекс.Следующий() Цикл
		...
		ПараметрыРеглОтчета = ПараметрыРеглОтчетов[ВыборкаИндекс.Индекс];
...
//
Функция ВыгрузитьРегламентированныйОтчет(СохраненныйОтчет) Экспорт
...
//     * Форма - УправляемаяФорма - экранная форма объекта, отображаемого на закладке "Отчеты"
//                        формы "1С-Отчетность".
//     * Статус - Строка
//
...
//                                                      Истина, если требуется выполнение ст. 55 НК РФ в части
//                                                      сокращения периода.
//
...
Функция БлижайшийНалоговыйПериод(Организация, Знач ПроверяемыйПериод, ВариантРасширенногоПервогоПериода,
	...
	ПроверяемыйПериод, ВариантРасширенногоПервогоПериода, СтандартнаяДлительностьНалоговогоПериода, ДатаРегистрации);
...
Функция АдресВФорматеФИАС(Адрес) Экспорт
	
	Возврат РегламентированнаяОтчетность.АдресВФорматеФИАС(Адрес);
...
Процедура СохранитьДанныеРазделаРеглОтчетЕНВДФормаОтчета2018Кв4(НаимРаздела, ТабличныйДокумент, РазделыОтчета, ДопПараметры)
	...
	ИначеЕсли НаимРаздела = "Раздел2" Тогда
		...
		ЗначениеК1 = ?(Год(ДопПараметры["ДатаКонцаПериодаОтчета"]) >= 2019, 1.915, 1.868);
