...
//			Сотрудник
//			ФизическоеЛицо
//			ДатаДохода - дата отражения дохода в учете страховых взносов
//			Начисление – перечисление ВидыОсобыхНачисленийИУдержаний
...
Процедура ЗафиксироватьСтажВРеестрах(ПараметрыОбновления = Неопределено) Экспорт
	...
	ИмяТЧ = "СведенияНеобходимыеДляНазначенияПособий";
	Выборка = Запрос.Выполнить().Выбрать();
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ФлажокСтажЛет Или Выборка.ФлажокСтажМесяцев Тогда
				ФиксацияВторичныхДанныхВДокументах.ЗафиксироватьРеквизитОбъекта(
					ДокументОбъект, "СтажЛет",     ИмяТЧ, СтрокаДокумента.ИдентификаторСтрокиФикс);
				ФиксацияВторичныхДанныхВДокументах.ЗафиксироватьРеквизитОбъекта(
					ДокументОбъект, "СтажМесяцев", ИмяТЧ, СтрокаДокумента.ИдентификаторСтрокиФикс);
			КонецЕсли;
			Если Выборка.ФлажокСтажРасширенныйЛет Или Выборка.ФлажокСтажРасширенныйЛет Тогда
				ФиксацияВторичныхДанныхВДокументах.ЗафиксироватьРеквизитОбъекта(
					ДокументОбъект, "СтажРасширенныйЛет",     ИмяТЧ, СтрокаДокумента.ИдентификаторСтрокиФикс);
				ФиксацияВторичныхДанныхВДокументах.ЗафиксироватьРеквизитОбъекта(
					ДокументОбъект, "СтажРасширенныйМесяцев", ИмяТЧ, СтрокаДокумента.ИдентификаторСтрокиФикс);
			КонецЕсли;
		КонецЦикла;
