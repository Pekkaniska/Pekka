...
Функция НайтиСоздатьНоменклатуруПоставщика(Контрагент, Идентификатор, Наименование, Артикул) Экспорт
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Иначе
			...
			НоменклатураПоставщика.Артикул = Артикул;
...
Функция ПодготовитьСтруктуруДляПриобретенияТоваровУслугПоПередачеТоваров(ДеревоДанных) Экспорт
	...
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		...
		ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект("ЕдиницыИзмерения", НоваяСтрока.Упаковка,
		...
		Если Сопоставление <> Неопределено Тогда
			ИдентификаторНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
			...
			НаименованиеНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
				"ТаблицаТоваров.НомерСтроки.Сопоставление.Наименование");
			АртикулНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
				"ТаблицаТоваров.НомерСтроки.Сопоставление.Артикул");
		КонецЕсли;
		
		НоменклатураПоставщика = НайтиСоздатьНоменклатуруПоставщика(ДанныеОбъекта.Контрагент, ИдентификаторНоменклатуры, НаименованиеНоменклатуры, АртикулНоменклатуры);
...
Функция ПодготовитьСтруктуруДляПриобретенияТоваровУслугПоПередачеРабот(ДеревоДанных) Экспорт
	...
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		...
		Если Сопоставление <> Неопределено Тогда
			...
			НаименованиеНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
				"ТаблицаУслуг.НомерСтроки.Сопоставление.Наименование");
			АртикулНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
				"ТаблицаУслуг.НомерСтроки.Сопоставление.Артикул");
		КонецЕсли;
...
Процедура ЗаполнитьДанныеПервичногоДокументаУПД(СтруктураДанных, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
	...
	Если ЭтоКомиссия Тогда
		...
		СведенияОКомиссионере = ЭлектронноеВзаимодействиеУТ.ПолучитьДанныеЮрФизЛица(СтруктураЭД.Контрагент, ,ДанныеШапки.Дата);
	...
	Пока СтрокаТовары.Следующий() Цикл
		...
		Если ИспользоватьНаборы Тогда
			Если СтрокаТовары.ЭтоНабор Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Сопоставление.Вставить("НоменклатураИБ", СтрокаТовары.Номенклатура);
		Сопоставление.Вставить("ХарактеристикаИБ", СтрокаТовары.Характеристика);
		Сопоставление.Вставить("УпаковкаИБ", СтрокаТовары.ЕдиницаИзмерения);
		НовСтрока.Сопоставление = Сопоставление;
		...
		Если СчетФактура.ДокументыОснования.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Основание.НомерИсправленияИсходногоДокумента) Тогда
				ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "НомерИсправленияИсходногоДокумента",
					Число(Основание.НомерИсправленияИсходногоДокумента));
...
Функция ПодготовитьСтруктуруДляПриобретенияТоваровУслугУПД(ДеревоДанных) Экспорт
	...
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		...
		Если Сопоставление <> Неопределено Тогда
			...
			НаименованиеНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
				"СведенияОТоварах.НомерСтроки.Сопоставление.Наименование");
			АртикулНоменклатуры = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре,
				"СведенияОТоварах.НомерСтроки.Сопоставление.Артикул");
		КонецЕсли;
		...
		Если Признак = "1" Тогда
			...
			Если СведенияОТаможеннойДекларации <> Неопределено И СведенияОТаможеннойДекларации.Строки.Количество() > 0 Тогда				
				...
				Если ЗначениеЗаполнено(НомерТаможеннойДекларации) Тогда
					...
					ЭлектронноеВзаимодействиеПереопределяемый.НайтиСсылкуНаОбъект("ТаможенныеДекларации", ТаможеннаяДекларация, НомерТаможеннойДекларации);
...
Функция НайтиСоглашениеОбИспользованииЭД(СтрокаОбъекта, ДеревоРазбора) Экспорт
	...
	Запрос.УстановитьПараметр("Контрагент",	ПолучитьРеквизитШапкиЭД(СтрокаОбъекта, "Контрагент", ДеревоРазбора));					  
	Запрос.УстановитьПараметр("Партнер",	ПолучитьРеквизитШапкиЭД(СтрокаОбъекта, "Партнер", ДеревоРазбора));
...
Процедура ДобавитьАдресПоСтруктуреАдресаФНС(Объект, ВидАдреса, СтруктураАдресаФНС)
	...
	НазваниеРегиона = "";
	ОбменСКонтрагентамиПереопределяемый.НазваниеРегиона(СтруктураАдресаФНС.КодРегион, НазваниеРегиона); 
	...
	СтруктураАдреса.Вставить("Регион",           НазваниеРегиона);
