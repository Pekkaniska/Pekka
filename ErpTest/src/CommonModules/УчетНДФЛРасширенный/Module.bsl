...
Процедура ЗарегистрироватьДоходыИСуммыНДФЛПоВременнойТаблицеНачислений(Регистратор, Движения, Отказ, Организация, Дата, ПериодРегистрации, ПорядокВыплаты, ПланируемаяДатаВыплаты, ДанныеДляПроведения, Записывать = Ложь, РегистрироватьСуммыНалога = Неопределено, ДоходПолученНаТерриторииРФ = Ложь, ПеречислитьНалогПриВыплатеЗарплаты = Ложь, ИмяТаблицыФизлиц = "ВТФизическиеЛица") Экспорт 
	...
	Если РегистрироватьСуммыНалога Тогда
		...
		Если Движения.Найти("ДокументыУчтенныеПриРасчетеНДФЛ") <> Неопределено Тогда
			УчетНДФЛ.СформироватьДокументыУчтенныеПриРасчетеДляМежрасчетногоДокументаПоВременнойТаблице(Движения, Отказ, Организация, ДанныеДляПроведения.МенеджерВременныхТаблиц, Регистратор, ИмяТаблицыФизлиц, Записывать); 	
		КонецЕсли;
		...
		УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛ(Движения, Отказ, Организация, ПериодРегистрации, ДанныеДляПроведения.НДФЛПоСотрудникам, ДанныеДляПроведения.МенеджерВременныхТаблиц, ПорядокВыплаты, Записывать,, Истина,, ПеречислитьНалогПриВыплатеЗарплаты);
...
Процедура ВосстановитьДатыПолученияДохода(Набор, ПараметрыКорректировки)
	...
	ДанныеКорректировки = Новый Структура(ПараметрыКорректировки.ПоляКорректировки);
	ДанныеКорректировки.Вставить("УстаревшаяДатаПолученияДохода");
	
	Имя = "ДополнительнаяПроверкаФиксированныхСтрок";
	ДополнительнаяПроверкаФиксированныхСтрок = ПараметрыКорректировки.Свойство(Имя) И ПараметрыКорректировки[Имя];
	
	// Подготовка вспомогательной таблицы для корректировки строк набора.
	СтрокиНабора = Набор.ВыгрузитьКолонки();
	СтрокиНабора.Колонки.Добавить("СтрокаНабора"); // Ссылка на строку исходного набора, изменяться будут эти строки.
	Для Каждого Стр Из Набор Цикл
		
		Если Не Стр.Сторно Тогда
			
			Если ДополнительнаяПроверкаФиксированныхСтрок Тогда
				ДатаПолученияДоходаФиксирована = Стр.ДатаПолученияДоходаФиксирована
					И УчетНДФЛ.ДатаПолученияДоходаДолжнаБытьЗафиксирована(Стр.КатегорияДохода);
			Иначе
				ДатаПолученияДоходаФиксирована = Стр.ДатаПолученияДоходаФиксирована;
			КонецЕсли;
			
			Если Не ДатаПолученияДоходаФиксирована Тогда
				НовСтр = СтрокиНабора.Добавить();
				ЗаполнитьЗначенияСвойств(НовСтр, Стр);
				НовСтр.СтрокаНабора = Стр;
			КонецЕсли;
		...
		Для Каждого СтрКорректировка Из ДатыПолученияДохода Цикл
			...
			Для Каждого Стр Из СтрокиНабора Цикл
				...
				Для Каждого Колонка Из КолонкиСравнения Цикл
					Если Стр[Колонка] <> СтрКорректировка[Колонка] Тогда
				...
				Если ЕстьКорректировка Тогда
					...
					СтрНабора = Стр.СтрокаНабора;
					СтрНабораДоп = Неопределено;
					...
					Для Каждого Ресурс Из Ресурсы Цикл
						...
						Иначе
							...
							Если СтрНабораДоп = Неопределено Тогда
								СтрНабораДоп = НаборДоп.Добавить();
								ЗаполнитьЗначенияСвойств(СтрНабораДоп, Стр,, КолонкиСуммирования);
							КонецЕсли;
			...
			Если РаспределениеВыполнялось И Не РаспределенаПолностью Тогда
				Для Каждого Ресурс Из Ресурсы Цикл
					СтрКорректировка[Ресурс] = 0; // Обнуление остатков корректировки.
				КонецЦикла;
			КонецЕсли;
