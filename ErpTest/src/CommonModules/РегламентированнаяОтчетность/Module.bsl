...
Функция АдресВФорматеФИАС(Адрес) Экспорт
	...
	СтруктураАдресаЗаписи = РаботаСАдресами.СведенияОбАдресе(Адрес, ДополнительныеПараметры);
	...
	//
	ДополнительныеПараметры = Новый Структура("КодыАдреса,ПолноеНаименованиеСокращений", Истина, Истина);
...
Процедура ЗаполнитьАдресВФорматеФИАСНаСтраницеРеглОтчета(СтраницаРаздела, ПараметрыЗаполнения) Экспорт
	...
	СтруктураАдреса = АдресВФорматеФИАС(ПараметрыЗаполнения.Адрес);
...
Процедура ВыполнитьДействияПередУдалениемПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	МассивФайловВладельца = МодульРаботаСФайламиСлужебныйВызовСервера.ПолучитьВсеПодчиненныеФайлы(ВладелецФайлов);
...
Процедура ВставитьПоказатели_3НДФЛ(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2015кв1"
...
Функция ФормаРеглОтчета(Наим, ВыбраннаяФорма = Неопределено, Владелец = Неопределено, КлючУникальности = Неопределено) Экспорт
	...
	Если Наим = "БухгалтерскаяОтчетностьВБанк" Тогда
		Возврат Отчеты[Наим];
	КонецЕсли;
...
Функция ВывестиНаПечатьЛистРаздела(ЗаголовокФормы, СтруктураПараметровЛиста, ТабличноеПоле)
	...
	Для Каждого Область Из ТабличноеПоле.Области Цикл
		...
		Если НЕ Тип(Область) = Тип("РисунокТабличногоДокумента")
...
Функция ОбщаяЧастьСсылкиНаИзмененияЗаконодательства(ИсточникОтчета) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		ОбщаяЧастьСсылкиНаИзмененияЗаконодательства = СтрШаблон(
			"http://v8.1c.ru/lawmonitor/lawchanges.jsp?&regReportForm=%1&regReportOnly=true&userConfiguration=%2",
...
Процедура ОбновитьШаблоныЧерезВебСервисНаСервереВФоне(ПараметрыКоманды, АдресРезультата = Неопределено) Экспорт
	Попытка
		...
		ОтобранныеСтроки = ТаблицаФорматов.НайтиСтроки(Новый Структура("Код, ОКУД", КодШаблона, ОКУД));
		Если ОтобранныеСтроки.Количество() <> 1 Тогда 
			Возврат;
		КонецЕсли;
		
		Если ОтобранныеСтроки[0].Версия = ВерсияШаблона Тогда 
			Возврат;
		КонецЕсли;
		...
		Если Таблица.Количество() = 1 И Таблица[0].РазностьДат < 3600 И Таблица[0].РазностьДат > -3600 Тогда 
			Возврат;
		КонецЕсли;
...
Процедура ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(Форма, ИмяМакетаАттрибутов, ИмяМакетаШаблона, ПараметрыВыгрузки) Экспорт
	Попытка
		...
		Если Не УниверсальныйОтчетСтатистики.СравнитьСтруктурыШаблонов(Выборка.Шаблон, РегламентированнаяОтчетностьВызовСервера.ОбъектОтчета(Форма.ИмяФормы).ПолучитьМакет(ИмяМакетаШаблона)) Тогда 
			Возврат;
		КонецЕсли;
		...
		ПараметрыВыгрузки.Вставить("ПроизведенаЗаменаФормата_КодШаблона", ДопАтрибуты.code);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Версия формата выгрузки была изменена на '") + Выборка.Версия);
