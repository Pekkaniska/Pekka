...
Процедура РегистрацияПерерасчетовПоПредварительнымДанным(Регистратор = Неопределено, Организация = Неопределено) Экспорт
	...
	Попытка
		...
		УдалитьПредварительныеПерерасчеты(Регистратор, Организация);
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Регистрация перерасчетов по предварительным данным'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция РегистрыИсточникиПредварительныхПерерасчетов() Экспорт
	...
	Регистры.Вставить(Метаданные.РегистрыНакопления.ВыполненныеРаботыСотрудников.Имя, Истина);
...
Процедура СвернутьНачисленияПерерасчеты(НачисленияПерерасчет) Экспорт
	...
	СворачиваемаяКоллекция = НачисленияПерерасчет.Выгрузить();
	СворачиваемаяКоллекция.Очистить();
	...
	СворачиваемаяКоллекция.Сортировать("ПериодДействия,ДатаНачала,Сотрудник,Начисление,Сторно", Новый СравнениеЗначений);
	
	КлючТекущейЗаписи = Новый Структура("Сотрудник,Подразделение,Начисление,ДокументОснование,КодВычета,ПериодДействия");
	ТекущаяЗапись = Неопределено;
	
	Для Каждого Строка Из СворачиваемаяКоллекция Цикл
...
Процедура ЗаполнитьСотрудниковПерерасчетаДокумента(Объект) Экспорт
	
	МетаданныеДокумента = Объект.Ссылка.Метаданные();
	ЕстьПерерасчетУдержаний = Ложь;
	...
	Если МетаданныеДокумента.ТабличныеЧасти.Найти("УдержанияПерерасчет") <> Неопределено Тогда
		...
		ЕстьПерерасчетУдержаний = Истина;
	КонецЕсли;
	
	ТаблицаСотрудников = Объект.СотрудникиПерерасчет.Выгрузить();
	ТаблицаСотрудников.Очистить();
	
	Для Каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
		Для Каждого Строка Из ТабличнаяЧасть Цикл
			НоваяСтрока = ТаблицаСотрудников.Добавить();
			Если ЕстьПерерасчетУдержаний
				И ТабличнаяЧасть = Объект.УдержанияПерерасчет Тогда
	...
	ТаблицаСотрудников.Свернуть("Сотрудник,ПериодДействия,ВидРасчета,ДокументОснование,ПерерасчетНачислений,ПерерасчетУдержаний");
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаСотрудников, Объект.СотрудникиПерерасчет);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЛьготыСотрудников") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ЛьготыСотрудников");
		Модуль.ДополнитьСотрудникиПерерасчетыЛьготами(Объект, МетаданныеДокумента);
	КонецЕсли;
...
Функция КраткоеПредставлениеДокумента(СсылкаНаОснование)
	...
	ИмяТаблицы = МетаданныеОснования.ПолноеИмя();
	ИмяМетаданных = МетаданныеОснования.Имя;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаОснование);
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ТаблицаОснования.Ссылка = &Ссылка";
	
	ИменаПолей = Новый Массив;
	Если МетаданныеОснования = Метаданные.Справочники.ГрафикиРаботыСотрудников Тогда
		ИменаПолей.Добавить("ТаблицаОснования.Наименование КАК Наименование");
	Иначе
		ИменаПолей.Добавить("ТаблицаОснования.Номер КАК Номер");
		ИменаПолей.Добавить("ТаблицаОснования.Дата КАК Дата");
	КонецЕсли;	
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПоляОснования", СтрСоединить(ИменаПолей, ","));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаОснования", ИмяТаблицы);
	...
	Если Выборка.Следующий() Тогда
		...
		Если МетаданныеОснования = Метаданные.Справочники.ГрафикиРаботыСотрудников Тогда
			КраткоеПредставление = КраткоеПредставление + " " + Выборка.Наименование;
		Иначе
...
Процедура ЗаполнитьДанныеПерерасчетаНачислений(ДанныеПерерасчета, Организация, Подразделение = Неопределено, МесяцНачисления, Сотрудники, СотрудникиПериодДействияПерерасчет, ИсключаемыеРегистраторы, РежимДоначисления, МенеджерРасчета)
	...
	Если ЗарплатаКадры.ВТСодержитСтроки(МенеджерВременныхТаблиц, "ВТСторнируемыеРегистраторы") Тогда
		...
		ЗаполнитьСторнированиеНачислений(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели);
...
Функция ДанныеПерерасчетаПриУвольненииСотрудника(ДокументУвольнения, ИсправленныйДокумент, Сотрудники, Организация, ДатаУвольнения, ПериодПерерасчетов, ИсключаемыеРегистраторы, МенеджерРасчета) Экспорт
	...
	Пока МесяцПерерасчетов <= ОкончаниеПериода Цикл
		Для Каждого Сотрудник Из Сотрудники Цикл
			...
			Если СтрокиТаблицаСотрудников.Количество() = 0
				И МесяцПерерасчетов <= ПериодПерерасчетов.ДатаОкончания Тогда
	...
	Если ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
		...
		ПерерасчитанныеНачисления = Запрос.Выполнить().Выгрузить();
		...
		Пока МесяцПерерасчетов <= ПериодПерерасчетов.ДатаОкончания Цикл
			Для Каждого Сотрудник Из Сотрудники Цикл
				...
				Для Каждого СтрокаПерерасчетов Из СтрокиПерерасчетов Цикл
					ПерерассчитанныеСтроки = ПерерасчитанныеНачисления.НайтиСтроки(Новый Структура("Сотрудник,Начисление,ПериодДействия", СтрокаПерерасчетов.Сотрудник, СтрокаПерерасчетов.Начисление, СтрокаПерерасчетов.ПериодДействия));
...
Процедура УдалитьПредварительныеПерерасчеты(Регистратор = Неопределено, Организация = Неопределено)
	
	НаборЗаписей = РегистрыСведений.ПредварительныеПерерасчеты.СоздатьНаборЗаписей();
	Если Регистратор <> Неопределено Тогда
		НаборЗаписей.Отбор.РегистраторПерерасчета.Установить(Регистратор);
	КонецЕсли;
...
Процедура ЗаписатьПредварительныеПерерасчетыПоИзменениямВНаборах(ИзмененияВНаборахИсточников, Регистратор)
	...
	ДобавитьПредварительныеПерерасчетыПоИзменениямВыполненныхРабот(ПредварительныеПерерасчеты, ИзмененияВНаборахИсточников, Регистратор);
...
Процедура ДобавитьПредварительныеПерерасчетыПоИзменениямВыполненныхРабот(ПредварительныеПерерасчеты, ИзмененияВНаборахИсточников, РегистраторПерерасчета)

	ИзменившиесяДанныеНабора = ИзмененияВНаборахИсточников[Метаданные.РегистрыНакопления.ВыполненныеРаботыСотрудников.Имя]; 
	...
	Для каждого СтрокаИзменения Из ИзменившиесяДанныеНабора Цикл
		...
		НоваяСтрока.ТипИзменения = Перечисления.ТипИзмененийПерерасчетов.ВыполненныеРаботы;
	КонецЦикла;
...
Процедура СоздатьВТПерерасчетыПособийПоОрганизации(МенеджерВременныхТаблиц)
	...
	Для каждого СтрокаТаблицы Из ТаблицаОрганизаций Цикл
		СтрокаТаблицы.ПерерасчетПособий = ПерерассчитыватьПособия(СтрокаТаблицы.Организация);	
	КонецЦикла;
	
	ЗарплатаКадры.СоздатьВТПоТаблицеЗначений(МенеджерВременныхТаблиц, ТаблицаОрганизаций, "ВТПерерасчетыПособийПоОрганизации");
...
Процедура СоздатьВТСотрудникиПоПоказателямПодразделений(МенеджерВременныхТаблиц)
	...
	ПараметрыПостроения = ЗарплатаКадрыОбщиеНаборыДанных.ПараметрыПостроенияДляСоздатьВТИмяРегистраСрез();
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(ПараметрыПостроения.Отборы, "Подразделение", "В", Подразделения);
...
Процедура СоздатьВТСотрудникиПериодыУдержаний(МенеджерВременныхТаблиц)
	...
	ТекстЗапросаПерерасчетовБезПроверки = 
		...
		|	НЕ ПредварительныеПерерасчеты.ТипИзменения В (&ТипыИзмененийТребующихПроверки)";
	ЧастиЗапроса.Добавить(ТекстЗапросаПерерасчетовБезПроверки);
	РасчетЗарплатыРасширенный.ДобавитьЧастьЗапросаПроверкиПерерасчетовУдержанийПоПоказателям(МенеджерВременныхТаблиц, ЧастиЗапроса);
	РасчетЗарплатыРасширенный.ДобавитьЧастьЗапросаПроверкиПерерасчетовУдержанийПоИсполнительнымДокументам(МенеджерВременныхТаблиц, ЧастиЗапроса);
	Запрос.Текст = СтрСоединить(ЧастиЗапроса, Разделитель);
...
Процедура СоздатьВТНеобходимыеПерерасчетыНачисленийПриНачалеУчета(МенеджерВременныхТаблиц)
	...
	Запрос.Текст =
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПерерасчетыПособийПоОрганизации КАК ПерерасчетыПособийПоОрганизации
		|		ПО (ПерерасчетыПособийПоОрганизации.Организация = СотрудникиПериоды.Организация)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИсходныеДанныеПерерасчетов КАК ИсходныеДанныеПерерасчетов
		...
		|				ГДЕ
		|					(ПерерасчетыПособийПоОрганизации.ПерерасчетПособий
		|						ИЛИ НЕ ОтобранныеНачисления.ЭтоПособие)))
		|
		|СГРУППИРОВАТЬ ПО
		|	СотрудникиПериоды.Организация,
...
Процедура СоздатьВТНеобходимыеПерерасчетыНачислений(МенеджерВременныхТаблиц, ИсключаемыйРегистратор = Неопределено)
	...
	УдалитьВТ.Добавить("ВТПерерасчетыПособийПоОрганизации");
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьЗапросыУничтоженияВременныхТаблиц(Запрос.Текст, УдалитьВТ);
	Запрос.Выполнить();
...
Процедура ЗарегистрироватьПерерасчетыНачислений(МенеджерВременныхТаблиц, ИсключаемыйРегистратор = Неопределено)
	...
	СоздатьВТПерерасчетыПособийПоОрганизации(МенеджерВременныхТаблиц);
...
Процедура ЗарегистрироватьПерерасчетыУдержаний(МенеджерВременныхТаблиц)
...
Процедура ЗаполнитьСторнированиеНачислений(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьБазовыйПериод(НоваяСтрокаСторноНачисленийПерерасчет, Выборка);
...
Процедура ЗаполнитьСторнированиеПособий(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьБазовыйПериод(НоваяСтрокаСторноПособийПерерасчет, Выборка);
...
Процедура СоздатьВТСторнируемыеРегистраторы(МенеджерВременныхТаблиц, ИсключаемыеРегистраторы)
	...
	Запрос.Текст =
		...
		|			И (ВЫБОР
		|				КОГДА ИсходныеДанныеПерерасчетов.Начисление.СпособВыполненияНачисления В (ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ЕжемесячноПриОкончательномРасчете), ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ВЗаданныхМесяцахПриОкончательномРасчете))
		|					ТОГДА ИСТИНА
		...
		|ПОМЕСТИТЬ ВТПерерассчитываемыеРегистраторы
		|ИЗ
		...
		|	ПерерассчитываемыеРегистраторы.Организация КАК Организация,
		...
		|	ПерерассчитываемыеРегистраторы.ИдентификаторСтрокиВидаРасчета КАК ИдентификаторСтрокиВидаРасчета,
		|	ПерерассчитываемыеРегистраторы.Сторно КАК Сторно,
		...
		|ИЗ
		|	ВТПерерассчитываемыеРегистраторы КАК ПерерассчитываемыеРегистраторы
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПерерассчитываемыеРегистраторы КАК ПерерассчитываемыеРегистраторыСторно
		...
		|			И ПерерассчитываемыеРегистраторы.ВидРасчета = ПерерассчитываемыеРегистраторыСторно.ВидРасчета
		...
		|ГДЕ
		|	ПерерассчитываемыеРегистраторыСторно.Сотрудник ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	И НЕ СторнируемыеРегистраторы.Сторно";
