...
//rarus+ saveld 08.11.2016	
	ШК_EAN13 = пкОбщийМодульСервер.ПолучитьШтрихкодEAN13(Ссылка, "Заявка на аренду техники");
	Если ШК_EAN13 <> Неопределено Тогда
		ПараметрыШтрихкода.Вставить("ТипКода",         1); // EAN13
		ПараметрыШтрихкода.Вставить("Штрихкод", ШК_EAN13);
		//ПараметрыШтрихкода.СодержитКС = СтрДлина(ПараметрыШтрихкода.Штрихкод) = 13;
	Иначе
//rarus- saveld 08.11.2016	
...
//rarus+ saveld 08.11.2016	
Процедура кнВывестиШтрихкодEANВТабличныйДокумент(ТабличныйДокумент, Макет, Знач ОбластьМакета, Ссылка, НазваниеПФ = "") Экспорт
	...
	ШК_EAN13 = пкОбщийМодульСервер.ПолучитьШтрихкодEAN13(Ссылка, НазваниеПФ);
	Если ШК_EAN13 <> Неопределено Тогда
		ПараметрыШтрихкода.Вставить("ТипКода",         1); // EAN13
		ПараметрыШтрихкода.Вставить("Штрихкод", ШК_EAN13);
