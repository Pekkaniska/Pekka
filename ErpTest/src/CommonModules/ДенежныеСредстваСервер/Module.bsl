...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты) Экспорт
	...
	//Рарус Владимир Подрезов 11.10.2017 5301
	|	Заявка.пкКурсДокумента КАК пкКурсДокумента,
	|	Заявка.пкДатаКурса КАК пкДатаКурса,
...
Процедура ДанныеОбОплатеВедомостей(МенеджерВременныхТаблиц, Параметры) Экспорт
	...
	|			И (ВедомостиЗаявки.Ссылка.ХозяйственнаяОперация В (&МассивОпераций) ИЛИ ВедомостиЗаявки.Ссылка.ХозяйственнаяОперацияПоЗарплате В (&МассивОпераций))
...
Функция ТекстСуммаНДСПлатежа(ВалютаДокумента, РасшифровкаПлатежа, ДобавлятьТекстВТомЧисле) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТекстНазначениеПлатежаНДС) Тогда
		//Рарус Владмир Подрезов 15.03.2017
		ЕстьДоговор = Ложь;
		Для Каждого текСтрока Из РасшифровкаПлатежа Цикл
			Если ТипЗнч(текСтрока.Заказ) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				ЕстьДоговор = Истина;
			КонецЕсли;
		КонецЦикла;
		Если ЕстьДоговор Тогда
			СуммаНДС = ТаблицаНДС.Итог("СуммаНДС");//Окр(СуммаДокумента / (1 + 0.18) * 0.18, 2);
			ТекстНазначениеПлатежаНДС = НСтр("ru='В т.ч. НДС (18%)'") + " " + Формат(СуммаНДС, "ЧЦ=15; ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ=");
		Иначе
		//Рарус Владмир Подрезов Конец
...
Функция ДокументыКСозданию(СтрокиГрафика, ТипДокумента, Группировать = Истина) Экспорт
	...
	//-- НЕ УТ
		...
		|	ИСТИНА                                                                      КАК ОплатаПоЗаявкам,
		|	ВЫБОР
		|		КОГДА СУММА(ЛицевыеСчета.Сумма) > ТаблицаОплаты.СуммаКОплате ТОГДА
		|			ТаблицаОплаты.СуммаКОплате
		|		ИНАЧЕ
	...
	Пока ВыборкаСтроки.НайтиСледующий(Новый Структура("Группируется", Ложь)) Цикл
		...
		Если ТипЗнч(ВыборкаСтроки.ОбъектОплаты) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств")
...
//
Функция ПолучитьНДФЛПеречисленныйПоВедомостям(Объект, МассивВедомостей, Отказ = Ложь) Экспорт
	...
	|	ЛОЖЬ КАК ИсчисленоПоДивидендам,
	|	ВедомостьНаВыплатуЗарплатыВБанкНДФЛ.Сумма КАК Сумма,
	|	ВедомостьНаВыплатуЗарплатыВБанкНДФЛ.Ссылка.ПериодРегистрации КАК ПериодРегистрации,
	|	ВедомостьНаВыплатуЗарплатыВБанкНДФЛ.Ссылка.Организация КАК Организация
	|ПОМЕСТИТЬ ВТ_ДанныеВедомостей
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыВБанк.НДФЛ КАК ВедомостьНаВыплатуЗарплатыВБанкНДФЛ
	|ГДЕ
	|	ВедомостьНаВыплатуЗарплатыВБанкНДФЛ.Ссылка В(&МассивВедомостей)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВедомостьНаВыплатуЗарплатыВКассуНДФЛ.Ссылка.ПериодРегистрации,
	|	ВедомостьНаВыплатуЗарплатыВКассуНДФЛ.Ссылка.Организация
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыВКассу.НДФЛ КАК ВедомостьНаВыплатуЗарплатыВКассуНДФЛ
	|ГДЕ
	|	ВедомостьНаВыплатуЗарплатыВКассуНДФЛ.Ссылка В(&МассивВедомостей)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.ФизическоеЛицо,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.СтавкаНалогообложенияРезидента,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.МесяцНалоговогоПериода,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.РегистрацияВНалоговомОргане,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.ВключатьВДекларациюПоНалогуНаПрибыль,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.КодДохода,
	|	ЛОЖЬ,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.Сумма,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.Ссылка.ПериодРегистрации,
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.Ссылка.Организация
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыПеречислением.НДФЛ КАК ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ
	|ГДЕ
	|	ВедомостьНаВыплатуЗарплатыПеречислениемНДФЛ.Ссылка В(&МассивВедомостей)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.ФизическоеЛицо,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.СтавкаНалогообложенияРезидента,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.МесяцНалоговогоПериода,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.РегистрацияВНалоговомОргане,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.ВключатьВДекларациюПоНалогуНаПрибыль,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.КодДохода,
	|	ЛОЖЬ,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.Сумма,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.Ссылка.ПериодРегистрации,
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.Ссылка.Организация
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыРаздатчиком.НДФЛ КАК ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ
	|ГДЕ
	|	ВедомостьНаВыплатуЗарплатыРаздатчикомНДФЛ.Ссылка В(&МассивВедомостей)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДанныеВедомостей.Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Перечисление.НДФЛСтавки.Ставка13) КАК Ставка,
	|	ВТ_ДанныеВедомостей.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ВТ_ДанныеВедомостей.СтавкаНалогообложенияРезидента КАК СтавкаНалогообложенияРезидента,
	|	ВТ_ДанныеВедомостей.МесяцНалоговогоПериода КАК МесяцНалоговогоПериода,
	|	ВТ_ДанныеВедомостей.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ВТ_ДанныеВедомостей.ВключатьВДекларациюПоНалогуНаПрибыль КАК ВключатьВДекларациюПоНалогуНаПрибыль,
	|	ВТ_ДанныеВедомостей.ИсчисленоПоДивидендам КАК ИсчисленоПоДивидендам,
	|	СУММА(ВТ_ДанныеВедомостей.Сумма) КАК Сумма,
	|	ВТ_ДанныеВедомостей.ПериодРегистрации КАК Период
	|ИЗ
	|	ВТ_ДанныеВедомостей КАК ВТ_ДанныеВедомостей
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ДанныеВедомостей.ИсчисленоПоДивидендам,
	|	ВТ_ДанныеВедомостей.МесяцНалоговогоПериода,
	|	ВТ_ДанныеВедомостей.Организация,
	|	ВТ_ДанныеВедомостей.РегистрацияВНалоговомОргане,
	|	ВТ_ДанныеВедомостей.ФизическоеЛицо,
	|	ВТ_ДанныеВедомостей.ВключатьВДекларациюПоНалогуНаПрибыль,
	|	ВТ_ДанныеВедомостей.ПериодРегистрации,
	...
	ПеречисленныйНалог = Запрос.Выполнить().Выгрузить();
	...
	Для Каждого СтрокаНалога Из ПеречисленныйНалог Цикл
		ЗаполнитьЗначенияСвойств(ОтборСтавок, СтрокаНалога);
		ВыборкаСтавок.Сбросить();
		Если ВыборкаСтавок.НайтиСледующий(ОтборСтавок) Тогда
			СтрокаНалога.Ставка = ВыборкаСтавок.СтавкаНДФЛ
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПеречисленныйНалог.Организация КАК Организация,
	|	ПеречисленныйНалог.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ПеречисленныйНалог.Ставка КАК Ставка,
	|	ПеречисленныйНалог.МесяцНалоговогоПериода КАК МесяцНалоговогоПериода,
	|	ПеречисленныйНалог.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ПеречисленныйНалог.ВключатьВДекларациюПоНалогуНаПрибыль КАК ВключатьВДекларациюПоНалогуНаПрибыль,
	|	ПеречисленныйНалог.ИсчисленоПоДивидендам КАК ИсчисленоПоДивидендам,
	|	ПеречисленныйНалог.Сумма КАК Сумма,
	|	&РеквизитыПлатежногоПоручения КАК РеквизитыПлатежногоПоручения,
	|	ПеречисленныйНалог.Период КАК Период
	|ПОМЕСТИТЬ ВТНалогПеречисленный
	|ИЗ
	|	&ПеречисленныйНалог КАК ПеречисленныйНалог";
	
	Запрос.УстановитьПараметр("ПеречисленныйНалог", ПеречисленныйНалог);
	Запрос.УстановитьПараметр("РеквизитыПлатежногоПоручения",
		РеквизитыПлатежногоПорученияПоДатеНомеру(Объект.ДатаПроведенияБанком, ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Объект.Номер)));
...
Процедура РассчитатьИЗарегистрироватьУдержанныеНалоги(Объект, ДополнительныеСвойства, Отказ) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.РасходныйКассовыйОрдер") Тогда
		...
		МассивВедомостей.Добавить(Объект.Ведомость);
		
		МассивФизЛиц = Объект.ВыплатаЗаработнойПлаты.ВыгрузитьКолонку("ФизическоеЛицо");
		
		УдержанныйНалог = ПолучитьНДФЛУдержанныйПоВедомостям(МассивВедомостей, Отказ, МассивФизЛиц);
		
		УчетНДФЛ.СформироватьУдержанныйНалогПоТаблицеЗначений(Объект.Движения, Отказ, Объект.Организация, Объект.Дата, УдержанныйНалог, , Истина);
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.ДепонированиеЗарплаты") Тогда
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.СписаниеБезналичныхДенежныхСредств") Тогда
		...
		МассивФизЛиц.Добавить(Объект.ПодотчетноеЛицо);
		
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета Тогда
			УдержанныйНалог = ПолучитьНДФЛУдержанныйПоВедомостям(МассивВедомостей, Отказ, МассивФизЛиц);
		Иначе
			УдержанныйНалог = ПолучитьНДФЛУдержанныйПоВедомостям(МассивВедомостей, Отказ);
		КонецЕсли; 
...
Функция ДанныеВедомостейНаВыплатуЗарплаты(МассивВедомостей) Экспорт
	...
	Если РеквизитыШапки.КоличествоХозОпераций > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается форма оплаты. Ввод одного документа на основании выделенных ведомостей невозможен.'");
	...
	Если РеквизитыШапки.КоличествоЗарплатныхПроектов > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается поле ""Зарплатный проект"". Ввод одного документа на основании выделенных ведомостей невозможен.'");
	...
	Если РеквизитыШапки.КоличествоБанков > 1 Тогда
		Текст = НСтр("ru='У выделенных ведомостей отличается поле ""Банк"". Ввод одного документа на основании выделенных ведомостей невозможен.'");
...
Процедура ЗаполнитьТекстНазначенияПлатежа(Объект) Экспорт
	...
	Если ТекстНазначение = "" Тогда
		...
		КонецЕсли;
		//-- НЕ УТ
	//++ Рарус Лимаренко 29.11.2017
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		ТекстНазначение = ТекстНазначение + Символы.ПС + НазначениеПлатежа(Объект);
	//-- Рарус Лимаренко 29.11.2017
	Иначе
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетаДенежнымиСредствами.ВозвратРасходаДенежныхСредств)
	|		ИНАЧЕ
	...
	|ВЫБРАТЬ
	|	ВЫБОР ДанныеДокумента.ХозяйственнаяОперация
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОплатаПоставщику) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетаДенежнымиСредствами.РасходДенежныхСредств)
	...
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствВДругуюОрганизацию) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетаДенежнымиСредствами.ВозвратДенежныхСредств)
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПрочаяВыдачаДенежныхСредств) ТОГДА
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетаДенежнымиСредствами.РасходДенежныхСредств)
	|	КОНЕЦ КАК ТипРасчета,
	|	ЕСТЬNULL(ДанныеДокумента.Кассир.ФизическоеЛицо.Наименование, """")           КАК Кассир,
	|	ЕСТЬNULL(ДанныеДокумента.Кассир.ФизическоеЛицо.ИНН, """")                    КАК КассирИНН,
	...
	|			ЗНАЧЕНИЕ(Перечисление.ТипыРасчетаДенежнымиСредствами.ВозвратРасходаДенежныхСредств)
	|	КОНЕЦ КАК ТипРасчета,
	|	ЕСТЬNULL(NULL, """")                   КАК Кассир,
	|	ЕСТЬNULL(NULL, """")                   КАК КассирИНН,
	|	
	|	ДанныеДокумента.Организация    КАК Организация,
	|	ДанныеДокумента.СуммаДокумента КАК СуммаДокумента,
	|	ДанныеДокумента.Валюта         КАК Валюта,
...
Процедура ЗаполнитьПозицииИТаблицуОплатЧека(ПараметрыФискализацииЧека, ДанныеДокумента, СуммаПредоплатыКорректировка)
	...
	|	ВЫБОР
	|		КОГДА ЖурналФискальныхОпераций.ДокументОснование ЕСТЬ NULL ТОГДА
	|			ВЫБОР
	|				КОГДА ЕСТЬNULL(ВЫБОР
	|						КОГДА Расшифровка.ВалютаОснования = Расшифровка.ВалютаОплаты ТОГДА
	|							ЕСТЬNULL(Расшифровка.СуммаДокумента, 0) = Расшифровка.СуммаОплаты
