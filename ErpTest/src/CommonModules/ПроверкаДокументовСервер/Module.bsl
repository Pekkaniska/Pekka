...
// Проверяет, можно ли удалять документ (отказывает в удалении, в случае если документ был проверен и стоит жесткая
// блокировка объекта при проверке).
//	
//	Параметры:
//		Источник - ДокументОбъект - см. подписку на событие "ПроверкаДокументовПередЗаписьюДокумента";
...
Процедура ПередИзменениемДокумента(Источник, Отказ, Удаление = Ложь) Экспорт
	...
	Если ДанныеПроверки.Проверен И Не ИзменениеПроверенных Тогда
		...
		ЗаписьЖурналаРегистрации(НСТр("ru = 'Операция не выполнена.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Источник.Метаданные(), Источник.Ссылка, СообщениеОбОшибке);
...
Функция ИсключаемыеТипыДляПроверки() Экспорт
	...
	МассивВозврата.Добавить(Тип("ДокументСсылка.РаспределениеДоходовИРасходовПоНаправлениямДеятельности"));
	...
	Иначе
		...
		Иначе
			Если ДанныеОПроверке.Проверен Тогда
				...
				КартинкаКнопки = БиблиотекаКартинок.ИсключитьИзУчетнойПолитики;
			Иначе
				ПодсказкаКнопки = НСтр("ru = 'Документ не проверен.'");
				КартинкаКнопки = БиблиотекаКартинок.ВключитьВУчетнуюПолитику;
			КонецЕсли;
		КонецЕсли;		
	КонецЕсли;
...
Функция МассивОрганизацийДокумента(ДокументИсточник, УчитыватьУправленческуюОрганизацию = Ложь) Экспорт
	...
	Если Справочники.Организации.ИспользуетсяНесколькоОрганизаций() Тогда
		...
		Если ТипЗнч(ДокументИсточник) = Тип("ДокументОбъект.СчетФактураВыданный")
			ИЛИ ТипЗнч(ДокументИсточник) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
