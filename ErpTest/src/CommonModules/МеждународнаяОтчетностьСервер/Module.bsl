...
Процедура ДобавитьТекстовыйБлок(Параметры, ЭлементОтчета)
	...
	ТекстШаблона = "["+НСтр("ru='Комплект отчетов'")+"]";
	...
	ТекстШаблона = "["+НСтр("ru='Конечная дата периода отчета'")+"]";
	Если СтрНайти(ТекстЗаголовка, ТекстШаблона) > 0 Тогда
	...
	ТекстШаблона = "["+НСтр("ru='Текущая дата и время'")+"]";
	Если СтрНайти(ТекстЗаголовка, ТекстШаблона) > 0 Тогда
		ДатаФормированияОтчета = Формат(Параметры.ДатаФормированияОтчета, "ДЛФ=DT");
		ТекстЗаголовка = СтрЗаменить(ТекстЗаголовка,ТекстШаблона,ДатаФормированияОтчета);
	КонецЕсли;
	
	ТекстШаблона = "["+НСтр("ru='Организация'")+"]";
	Если СтрНайти(ТекстЗаголовка, ТекстШаблона) > 0 Тогда
...
Процедура ДобавитьЭлементТаблицыИзмерение(ЭлементыТаблицы, Элемент, ДопСуффикс, ЭтоКолонки)
	...
	Если ЭтоПериод Тогда
		...
		ИмяКолонки = "Период" + Строка(Элемент.Периодичность);
...
Процедура ВывестиШапкуТаблицы(Параметры, ЭлементВывода, ДокументРезультат)
	...
	Если Параметры.ВыводитьКодСтроки Тогда
		...
		ОбъединитьПоВертикали(ДокументРезультат, ПерваяСтрокаШапки, КоличествоУровнейШапки, ПерваяКолонкаШапки, НСтр("ru = 'Код строки'"), Истина);
	КонецЕсли;
	Если Параметры.ВыводитьПримечание Тогда
		ПерваяКолонкаШапки = ПерваяКолонкаШапки + 1;
		ОбъединитьПоВертикали(ДокументРезультат, ПерваяСтрокаШапки, КоличествоУровнейШапки, ПерваяКолонкаШапки, НСтр("ru = 'Примечание'"), Истина);
	КонецЕсли;
...
Функция РасшифровкаЯчейки(ОписаниеТаблицы, ДанныеСтрокиТаблицы, ИмяКолонки)
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		Расшифровка = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(Расшифровка);
		Если ОписаниеТаблицы.ТаблицаПоказателиВКолонках Тогда
...
Процедура РасшифровкаПроизводногоПоказателя(ПараметрыОтчета)
	...
	ВывестиСтроку(ТабличныйДокумент, ОбластьСекции, НСтр("ru = 'Период'")+": "+ПериодСтрокой);
	// Валюта
	СтрокаВалюты = ?(ПараметрыОтчета.Ресурс = "Сумма",Валюта.Функциональная,Валюта.Представления);
	Шаблон = НСтр("ru = 'Валюта'")+": %1";
...
Функция ПустоеЗначениеПоказателя(ИнтервалыОтчета, Измерения = Неопределено)
...
Процедура УстановитьВалюту(ДокументРезультат, Валюта, ПредставлениеВалюты) Экспорт
	...
	Если СтрНайти(Область.Текст, ": " + НСтр("ru = 'Да'")) > 0 Тогда
...
Процедура ОбновитьДеревоНовыхЭлементов(Форма, Параметры) Экспорт
	...
	ЗаголовокПоказателяМежд = НСтр("ru = 'Монетарный показатель международного учета'");
	ЗаголовокПоказателяРегл = НСтр("ru = 'Монетарный показатель регламентированного учета'");
...
Процедура ОбновитьДеревоСохраненныхЭлементов(Форма, Параметры) Экспорт
	...
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Счет плана счетов'")+"]", НСтр("ru = 'Счет плана счетов'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Тип итога'")+"]", НСтр("ru = 'Тип итога'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Начальное сальдо'")+"]", НСтр("ru = 'Начальное сальдо'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Немонетарный показатель'")+"]", НСтр("ru = 'Немонетарный показатель'"));
	ФинОтчеты.НовоеПолеВыбора(НастройкиКД,"СвязанныйЭлемент.["+НСтр("ru = 'Выводить заголовок элемента'")+"]", НСтр("ru = 'Выводить заголовок элемента'"));
...
Процедура ДобавитьИзмеренияВДеревоНовыхЭлементов(ДеревоЭлементов, ВидПоказателей, БыстрыйПоиск)
	...
	ДобавитьСтрокуИзмерения(СтрокаПериод, НСтр("ru = 'Год'")      , ПредопределенноеЗначение("Перечисление.Периодичность.Год"));
	ДобавитьСтрокуИзмерения(СтрокаПериод, НСтр("ru = 'Полугодие'"), ПредопределенноеЗначение("Перечисление.Периодичность.Полугодие"));
	ДобавитьСтрокуИзмерения(СтрокаПериод, НСтр("ru = 'Квартал'")  , ПредопределенноеЗначение("Перечисление.Периодичность.Квартал"));
	ДобавитьСтрокуИзмерения(СтрокаПериод, НСтр("ru = 'Месяц'")    , ПредопределенноеЗначение("Перечисление.Периодичность.Месяц"));
	
	// Добавление видов субконто
	ЗаголовокМежд = НСтр("ru = 'Виды субконто международные'");
	ЗаголовокРегл = НСтр("ru = 'Виды субконто хозрасчетные'");
	Если ВидПоказателей = ВидыПоказателейОтчетности.Международные Тогда
...
Процедура ЗаполнитьВалютуОтчета(ЭлементРесурс, ВидПоказателей) Экспорт
	...
	Если ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.Международные Тогда
		
		СписокВыбора.Добавить("Сумма",НСтр("ru = 'В функциональной валюте'") + СтрШаблон(" (%1)", ВалютаМУ.Функциональная));
		СписокВыбора.Добавить("СуммаПредставления",НСтр("ru = 'В валюте представления'") + СтрШаблон(" (%1)", ВалютаМУ.Представления));
		
	ИначеЕсли ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.Регламентированные Тогда
		
		ДоступностьРесурса = Ложь;
		ВидимостьРесурса = Ложь;
		СписокВыбора.Добавить("Сумма", НСтр("ru = 'Бухгалтерский учет'") + СтрШаблон(" (%1)", ВалютаРегл));
		Если ПолучитьФункциональнуюОпцию("ВестиУУНаПланеСчетовХозрасчетный") Тогда
			СписокВыбора.Добавить("СуммаУУ", НСтр("ru = 'Управленческий учет'") + СтрШаблон(" (%1)", ВалютаУУ));
			ДоступностьРесурса = Истина;
			ВидимостьРесурса = Истина;
		КонецЕсли;
		Если ПолучитьФункциональнуюОпцию("ВестиУчетНаПланеСчетовХозрасчетныйВВалютеФинОтчетности") Тогда
			СписокВыбора.Добавить("СуммаФО", НСтр("ru = 'В валюте финансовой отчетности'") + СтрШаблон(" (%1)", ВалютаФО));
			ДоступностьРесурса = Истина;
			ВидимостьРесурса = Истина;
		КонецЕсли;
		ЭлементРесурс.ТолькоПросмотр = НЕ ДоступностьРесурса;
	...
	ИначеЕсли ВидПоказателей = Перечисления.ВидыПоказателейОтчетности.МеждународныеИРегламентированные Тогда
		...
		Если ВалютаРегл = ВалютаМУ.Функциональная Тогда
			СписокВыбора.Добавить("Сумма",НСтр("ru = 'В функц. и регл. валюте'") + СтрШаблон(" (%1)", ВалютаМУ.Функциональная));
		ИначеЕсли ВалютаРегл = ВалютаМУ.Представления Тогда
			СписокВыбора.Добавить("Сумма",НСтр("ru = 'В представл. и регл. валюте'") + СтрШаблон(" (%1)", ВалютаМУ.Представления));
		Иначе
			СписокВыбора.Добавить("Сумма",НСтр("ru = 'В функц. и регл. валюте'") + СтрШаблон(" (%1, %2)", ВалютаМУ.Функциональная, ВалютаРегл));
		КонецЕсли;
		ЭлементРесурс.Видимость = Истина;
		ЭлементРесурс.ТолькоПросмотр = Истина;
