...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала.РазборРезюме") Тогда
		МодульРазборРезюме = ОбщегоНазначения.ОбщийМодуль("РазборРезюмеВызовСервера");
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КонфигурацииЗарплатаКадры") Тогда
		...
		МодульКонфигурацииЗарплатаКадры.ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды);
	КонецЕсли;
...
//
// Параметры:
//  КодКатегории - Строка - код категории застрахованного лица, должен соответствовать одному из значения перечисления КатегорииЗастрахованныхЛицДляПФР;
//  Год - Число - год, для которого следует определить тариф;
//
// Возвращаемое значение:
//  Число, Неопределено. Неопределено возвращается в том случае, когда переданых неподходящие значения параметров..
//
...
Функция ПолучитьСведенияОСотруднике(Знач Сотрудник, Знач МассивПоказателей = Неопределено, Знач ДатаЗначения = Неопределено) Экспорт
	...
	Если КадровыеДанные.Количество() > 0 Тогда
		...
		Для каждого ОписаниеПоказателя Из Сведения Цикл
			...
			Сведения.Вставить(ОписаниеПоказателя.Ключ, ДанныеСотрудника[ИмяДанных]);
...
Процедура ПолучитьСтруктуруКлючевыхРеквизитовОбъекта(ИмяОбъекта, СтруктураКлючевыхРеквизитов) Экспорт
	ОбменСБанкамиПоЗарплатнымПроектам.ПолучитьСтруктуруКлючевыхРеквизитовОбъекта(ИмяОбъекта, СтруктураКлючевыхРеквизитов);
КонецПроцедуры
...
Процедура ЭлектронноеВзаимодействиеПриОпределенииГотовностиИсточников(ДокументыМассив, ФормаИсточник = Неопределено) Экспорт
	...
	Для каждого ДокументМассива Из ДокументыМассив Цикл
		ДокументОбъект = ДокументМассива.ПолучитьОбъект();
		Если Не ДокументОбъект.ПроверитьЗаполнение() Тогда
			МассивДляУдаления.Добавить(ДокументМассива);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого ДокументДляУдаления Из МассивДляУдаления Цикл
		ДокументыМассив.Удалить(ДокументДляУдаления);
	КонецЦикла;
...
Функция ЭлектронноеВзаимодействиеПриОпределенииДанныхЮрФизЛица(ЮрФизЛицо) Экспорт
	...
	СведенияОбОрганизации.Вставить("ИНН", СведенияОбОрганизации.ИННЮЛ);
	СведенияОбОрганизации.Вставить("КПП", СведенияОбОрганизации.КППЮЛ);
	СведенияОбОрганизации.Вставить("ПолноеНаименование", СведенияОбОрганизации.НаимЮЛПол);
...
// Открывает транзакцию, в случае если транзакция еще не активна
//
//	Возвращаемое значение - булево
//		Истина - если транзакция открыта
//		Ложь - если на момент вызова транзакция уже была открыта.
//
...
Процедура ЗаполнитьПервоначальныеЗначенияВФорме(Форма, ЗапрашиваемыеЗначения) Экспорт
	...
	Для Каждого ЗапрашиваемоеЗначение Из ЗапрашиваемыеЗначения Цикл
		// Поддерживваетмо значение по умолчанию "Ответственный" имеет особое поведение - заполняется безусловно.
...
Функция КраткийСоставСотрудниковПоСпискуФизическихЛиц(ФизическиеЛица, ДатаАктуальности) Экспорт
	...
	Для Каждого Физлицо Из ПервыеФизлица Цикл
		...
		КраткийСостав = КраткийСостав + ФизическиеЛицаЗарплатаКадрыКлиентСервер.ФамилияИнициалы(ФИО);
...
// Предполагаем, чтосписок поставляемых/"чужих" подсистем более стабилен, 
...
// Возвращает Истина, если используется учет хозрасчетных организаций.
Функция ИспользуетсяУчетХозрасчетныхОрганизаций() Экспорт
	Возврат ЗарплатаКадрыВнутренний.ИспользуетсяУчетХозрасчетныхОрганизаций();
КонецФункции
...
// Заполняет ответсвенных лиуц организации, сохраняет запрашиваемые значения в реквизите формы СведенияОбОтветственныхРаботниках.
//
// Параметры:
//	Форма					- УправляемаяФорма
...
//
Процедура ОбновитьРегистрРассчитанныхПериодовСостояний(ОписаниеРегистраПервичныхДанных, ОписаниеРегистраВторичныхДанных, КлючиОбновляемыхДанных, РежимЗагрузки = Ложь) Экспорт
...
Процедура ЗаписатьНаборыВторичногоРегистраСостояний(ОписаниеРегистраВторичныхДанных, РассчитанныеДанные, КлючиОбновленныхНаборов, РежимЗагрузки)
...
Функция ПолучитьДанныеФайла(Знач ВладелецФайла, 
	...
	Возврат РаботаСФайлами.ДанныеФайла(ФайлыВладельца[0], ИдентификаторФормы, ПолучатьСсылкуНаДвоичныеДанные);
...
Процедура ДобавитьПараметрОтбора(СтруктураПараметровОтбора, ИмяПараметра, ОписаниеТипа,	Представление = Неопределено,
	...
	ОписаниеПараметра = Новый Структура("ТипПараметра, ИмяПараметра", ОписаниеТипа, ИмяПараметра);
	Если Представление = Неопределено Тогда
		Представление = ИмяПараметра;
	КонецЕсли;
	ЗарплатаКадрыВнутренний.ДополнитьОписаниеМодифицирующегоПараметраОтбора(ОписаниеПараметра, ИмяМодификации);
	СтруктураПараметровОтбора.Вставить(Представление, ОписаниеПараметра);
...
Процедура ПриСозданииНаСервереФормыСДинамическимСписком(Форма, НаименованиеРеквизитаСписок, ГруппаНастроек = "СписокНастройкиОтбора",
	...
	Если ПараметрыКритерияОтбора <> Неопределено Тогда
		...
		Для каждого ПараметрКритерияОтбора Из ПараметрыКритерияОтбора Цикл
			...
			ИмяРеквизитаФормы = ПараметрКритерияОтбора.Значение.ИмяПараметра + "_" + ИдПараметра;
			...
			ПараметрКритерияОтбора.Значение.Свойство("ИмяМодификации", НоваяСтрокаПараметра.ИмяМодификации);
			
			ИмяРеквизитаФормы = ПараметрКритерияОтбора.Значение.ИмяПараметра + "_Использование_" + ИдПараметра;
	...
	Если ПараметрыКритерияОтбора <> Неопределено И ГруппаПараметровКритерияОтбора <> Неопределено Тогда
		...
		Для каждого ПараметрКритерияОтбора Из ПараметрыКритерияОтбора Цикл
			...
			ГруппаТекущегоПараметра.Заголовок = ЗарплатаКадрыКлиентСервер.ПредставлениеИмениБезПробелов(ПараметрКритерияОтбора.Ключ);
			...
			ИмяЭлементаФормы = ПараметрКритерияОтбора.Значение.ИмяПараметра + "_Использование_" + ИдПараметра;
			...
			ИмяЭлементаФормы = ПараметрКритерияОтбора.Значение.ИмяПараметра + "_" + ИдПараметра;
...
Процедура УничтожитьВТ(МенеджерВременныхТаблиц, Знач ИменаВТ, ПроверятьНаличие = Ложь) Экспорт
	...
	Для Каждого ИмяВТ Из ИменаВТ Цикл
		Если ПроверятьНаличие Тогда
			Если Не ВТСуществует(МенеджерВременныхТаблиц, ИмяВТ) Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
...
//		* Пераметры - Структура - значения параметров запроса.
...
Функция ДоступныеОрганизации() Экспорт
	...
	Если Не ПривилегированныйРежим()
		...
		ЗарплатаКадрыПовтИсп.ЗаполнитьДоступныеОрганизации(ДоступныеОбъекты);
...
Функция ЗапросВТПервичныеДанные(МенеджерВременныхТаблиц, ОписаниеРегистра, ЗначенияИзмерений)
	...
	ЗаменитьШаблонНаПоляИсточника(ТекстЗапроса, "ЗначенияИзмерений.Измерение_", "Измерение_", ОписаниеРегистра.Измерения);
...
Функция ЗапросРассчитанныеДаты(МенеджерВременныхТаблиц, ОписаниеРегистра)
	...
	ТекстЗапроса = 
		...
		|				КОГДА СостоянияДействующиеНаДаты.Начало >= СостоянияДействующиеНаДатыВытесняющие.Начало
		|						И СостоянияДействующиеНаДаты.ДатаНачалаСобытия < СостоянияДействующиеНаДатыВытесняющие.ДатаНачалаСобытия
		|						И СостоянияДействующиеНаДатыВытесняющие.Окончание = ДАТАВРЕМЯ(1, 1, 1)
		|					ТОГДА ИСТИНА
	...
	МетаданныРегистра = Метаданные.РегистрыСведений[ОписаниеРегистра.ИмяРегистра];
	МассивБулевоРесурсов = Новый Массив;
	Для Каждого Ресурс Из ОписаниеРегистра.Ресурсы Цикл
		МетаданныеРесурса = МетаданныРегистра.Ресурсы.Найти(Ресурс);
		Если МетаданныеРесурса = Неопределено Тогда
			МетаданныеРесурса = МетаданныРегистра.Измерения.Найти(Ресурс);
		КонецЕсли;
...
Процедура ЗаполнитьГодЗаписиРегистраСведений(ИмяРегистра, ИзмерениеОтбора, ИмяПоляПериод, ПараметрыОбновления = Неопределено) Экспорт
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	*,
	|	НАЧАЛОПЕРИОДА(Регистр.#ИмяПоляПериод, ГОД) КАК ГодЗаполнение
	|ИЗ
	|	#ИмяРегистра КАК Регистр
	|ГДЕ
	|	Регистр.Год = ДАТАВРЕМЯ(1, 1, 1)
	|УПОРЯДОЧИТЬ ПО
	|	Регистр.#ИзмерениеОтбора";
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ИзмерениеОтбора", ИзмерениеОтбора);
	Запрос.Текст = ТекстЗапроса;
	Результат = Запрос.Выполнить();
	...
	Пока Выборка.СледующийПоЗначениюПоля(ИзмерениеОтбора) Цикл
		...
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, ПространствоБлокировки, ИзмерениеОтбора, Выборка[ИзмерениеОтбора]) Тогда
			Продолжить;
		КонецЕсли;
		НаборЗаписей.Отбор[ИзмерениеОтбора].Установить(Выборка[ИзмерениеОтбора]);
		Пока Выборка.Следующий() Цикл
			НоваяСтрока = НаборЗаписей.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
			НоваяСтрока.Год = Выборка.ГодЗаполнение;
		КонецЦикла;
		НаборЗаписей.Записать();
...
//
// Параметры:
//   ТекстXML - Строка - Сведения о классификаторе.
//
// Возвращаемое значение:
//   Булево - Истина, если данные удалось записать в классификатор.
//
Функция ОбновитьКлассификатор(ТекстXML) Экспорт
	...
	ТаблицаССсылками = ПолныеИменаВСсылки(ТаблицаБезСсылок, НаборЗаписей.ВыгрузитьКолонки(), ТаблицаОшибок);
	...
	КоличествоБудетЗагружено = ТаблицаССсылками.Количество();
	...
	// Загрузка сведений.
	НаборЗаписей.Загрузить(ТаблицаССсылками);
	НаборЗаписей.ДополнительныеСвойства.Вставить("ЗаписьОбщихДанных");
	ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(НаборЗаписей);
	
	// Запись результатов в журнал регистрации.
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Работа с классификаторами'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурнала,
		ОбъектМетаданных,
		,
		ТекстЖурнала);
...
//   ТаблицаССсылками - ТаблицаЗначений - Пустой шаблон результирующей таблицы.
...
//
Функция ПолныеИменаВСсылки(ТаблицаБезСсылок, ТаблицаССсылками, ТаблицаОшибок = Неопределено) Экспорт
	...
	Для Каждого Колонка Из ТаблицаССсылками.Колонки Цикл
	...
	Для Каждого СтрокаТаблицыБезСсылок Из ТаблицаБезСсылок Цикл
		СтрокаТаблицыССсылками = ТаблицаССсылками.Добавить();
		...
		Для Каждого ИмяКолонки Из ИменаКолонокДляПреобразованияСсылок Цикл
			...
			Если СтрНачинаетсяС(ЗначениеБезСсылок, "ПредопределенноеЗначение:") Тогда
				...
				Иначе
					ЗначениеССсылками = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент(ПолноеИмяПредопределенного);
				КонецЕсли;
			...
			Если ЗначениеССсылками = Неопределено Тогда
				...
				КонецЕсли;
				ТаблицаССсылками.Удалить(СтрокаТаблицыССсылками);
				Прервать;
			КонецЕсли;
			// Установка ссылки.
...
Функция СсылкиВПолныеИмена(ТаблицаССсылками) Экспорт
	...
	Для Каждого Колонка Из ТаблицаССсылками.Колонки Цикл
		...
		Если ВычитаемыеТипы.Количество() > 0 Тогда
			...
			ТаблицаБезСсылок.Колонки.Вставить(ТаблицаССсылками.Колонки.Индекс(Колонка), Колонка.Имя, ОписаниеТиповВНовойТаблице, Колонка.Заголовок, Колонка.Ширина);
	...
	Для Каждого СтрокаТаблицыССсылками Из ТаблицаССсылками Цикл
...
Процедура ОтменитьЗаявлениеНаВычеты(Регистратор, Сотрудник) Экспорт
	...
	ЗарплатаКадрыВнутренний.ОтменитьЗаявлениеНаВычеты(Регистратор, Сотрудник);
