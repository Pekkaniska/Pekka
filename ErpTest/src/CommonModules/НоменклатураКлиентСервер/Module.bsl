...
Процедура ПересчитатьСтатусУказанияСерийПриОбработке(ПараметрыУказанияСерий, ТекущийСтатус, СерииУказаныПолностью, КоличествоСерий, ВариантОбеспечения = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	ЭтоМатериалыМЛИлиПроизвОперации = 
		...
		И (ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.МаршрутныйЛистПроизводства"
			ИЛИ ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ПроизводственнаяОперация2_2");
	//-- НЕ УТКА
	
	ИспользоватьСтатусСерийМожноУказатьГОСИС = 
		ГосударственныеИнформационныеСистемыКлиентСерверПереопределяемый.ИспользоватьСтатусСерийМожноУказать(ПараметрыУказанияСерий);
