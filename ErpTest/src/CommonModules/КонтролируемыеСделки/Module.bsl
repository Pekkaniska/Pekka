...
Функция ВернутьЗаполненныйМакетДляЛиста1Б(Лист1Б, НомерСтраницы, СтруктураШапки) Экспорт 
	...
	Если СтруктураШапки.ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012() Тогда
		СтрокаВПараметрыМакета(Лист1Б.Строка043КодПоОКП, "П043_", Макет.Параметры, 6);
	Иначе
		СтрокаВПараметрыМакета(КодПоОКПИзКодаПоОКПД2(Лист1Б.Строка043КодПоОКПД2), "П043_", Макет.Параметры, 6);
	КонецЕсли;
	
	Если СтруктураШапки.ВерсияУведомления = Документы.УведомлениеОКонтролируемыхСделках.ВерсияУведомления_2012() Тогда
		СтрокаВПараметрыМакета(РазложитьОКВЭДВСтроку(Лист1Б.Строка045КодОКВЭД), "П045_", Макет.Параметры, 6);
	Иначе
		СтрокаВПараметрыМакета(РазложитьОКВЭДВСтроку(Лист1Б.Строка045КодОКВЭД2), "П045_", Макет.Параметры, 6);
	КонецЕсли;
...
Функция ВернутьЗаполненныйМакетДляРаздела3(Раздел3, ДанныеРаздела3, НомерСтраницы, СтруктураШапки) Экспорт 
	...
	Если НЕ ЗначениеЗаполнено(СведенияОФизическомЛице.Строка030ИНН) Тогда
		...
		КонецЕсли;
		СтрокаВПараметрыМакета(СведенияОФизическомЛице.ДокументКемВыдан, "ОрганВыдавшийДокумент_", Макет.Параметры, 80);
		...
		АдресСтруктурой = РаботаСАдресами.СведенияОбАдресе(СведенияОФизическомЛице.КонтактнаяИнформацияЗначенияПолей);
...
Функция ВернутьЗаполненныйМакетДляЛиста2(СведенияОбУведомлении, НомерСтраницы) Экспорт 
	...
	Если ОсновныеСведенияВыгрузки.Свойство("НПФЛКодВидДок") Тогда
		...
		СтрокаВПараметрыМакета(ОсновныеСведенияВыгрузки.НПФЛВыдДок,    "ОрганВыдавшийДокумент_", Макет.Параметры, 80);
	КонецЕсли;
