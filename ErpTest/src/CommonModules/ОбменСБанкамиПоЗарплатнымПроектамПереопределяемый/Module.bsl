...
Процедура ВедомостиПлатежногоДокументаПеречисленияЗарплаты(ПлатежныйДокумент, МассивВедомостей, СтандартнаяОбработка = Истина) Экспорт
	...
	|	И ВедомостиЗаявки.Ведомость Ссылка Документ.ВедомостьНаВыплатуЗарплатыВБанк
	|	И ВедомостиЗаявки.Ведомость <> ЗНАЧЕНИЕ(Документ.ВедомостьНаВыплатуЗарплатыВБанк.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ВедомостиСписания.Ведомость Ссылка Документ.ВедомостьНаВыплатуЗарплатыВБанк
	|	И ВедомостиСписания.Ведомость <> ЗНАЧЕНИЕ(Документ.ВедомостьНаВыплатуЗарплатыВБанк.ПустаяСсылка)");
...
Процедура ДополнитьПлатежныйДокумент(ПлатежныйДокумент, МассивВедомостей, СтандартнаяОбработка) Экспорт
	...
	Для Каждого Ведомость Из МассивВедомостей Цикл
		НоваяСтрока = ВедомостьОбъект.РасшифровкаПлатежа.Добавить();
		НоваяСтрока.Ведомость = Ведомость;
		НоваяСтрока.Сумма = СуммыПоВедомостям[Ведомость].СуммаПоДокументу;
	КонецЦикла;
	
	ВедомостьОбъект.СуммаДокумента = ВедомостьОбъект.РасшифровкаПлатежа.Итог("Сумма");
...
Процедура ОбработкаПроверкиЗаполнения(Документ, Отказ, ПроверяемыеРеквизиты, СтандартнаяОбработка = Истина) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументОбъект.ПодтверждениеЗачисленияЗарплаты") Тогда
		...
		Если Не ЗначениеЗаполнено(Документ.ПервичныйДокумент) Тогда
			СтандартнаяОбработка = ложь;
		КонецЕсли;
...
//
// Параметры:
//		МенеджерВременныхТаблиц - МенеджерВременныхТаблиц - в него должна быть помещена таблица 
//					ВТСоответствиеВедомостейДокументамОплаты с колонками:
//					  * ВедомостьВБанк - ОпределяемыйТип.ВедомостьВБанкЗарплатаКадры - ведомость в банк;
//					  * ДокументОплаты - ДокументСсылка - документ оплаты по ведомости в банк;
...
Процедура СформироватьДвиженияПоДокументамОплаты(МенеджерВременныхТаблиц, ВременнаяТаблицаОпределена) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДД.Ведомость КАК ВедомостьВБанк,
	|	ДД.Ссылка КАК ДокументОплаты,
	|	ДД.Ссылка.Дата КАК Дата
	|ПОМЕСТИТЬ ВТСоответствиеВедомостейДокументамОплаты
	|ИЗ
	|	Документ.СписаниеБезналичныхДенежныхСредств.РасшифровкаПлатежа КАК ДД
	|ГДЕ
	|	ДД.Ссылка.Проведен";
