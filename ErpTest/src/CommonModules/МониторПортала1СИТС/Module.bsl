...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Монитор Портала 1С:ИТС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПередВызовомОперацииСервиса(ИмяПрограммы, ДанныеАутентификации, РезультатОперации)
	...
	Если ИмяПрограммы = "Unknown" Тогда
		...
		РезультатОперации.ИнформацияОбОшибке =
			НСтр("ru = 'Не заполнено имя программы в методе ИнтернетПоддержкаПользователейПереопределяемый.ПриОпределенииИмениПрограммы().'");
	КонецЕсли;
