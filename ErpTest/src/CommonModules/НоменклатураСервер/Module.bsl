...
// Обработчик подписки на событие СделатьДополнительныйРеквизитОбщим
//
Процедура СделатьДополнительныйРеквизитНоменклатурыОбщимПередЗаписью(Объект, Отказ) Экспорт
	...
	Если Не Объект.ЭтоНовый()
		Или Не ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
		Возврат;
	КонецЕсли;
	
	РодительНабора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.НаборСвойств, "Родитель");
	
	Если РодительНабора = Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_Номенклатура
		Или РодительНабора = Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_ХарактеристикиНоменклатуры
		Или РодительНабора = Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_СерииНоменклатуры Тогда
		
		СсылкаНового = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ПолучитьСсылку();
		Объект.УстановитьСсылкуНового(СсылкаНового);
		
		ОбъектНабор = Объект.НаборСвойств.ПолучитьОбъект();
		
		Если Объект.ЭтоДополнительноеСведение Тогда
			ТЧНабора = ОбъектНабор.ДополнительныеСведения;
		Иначе
			ТЧНабора = ОбъектНабор.ДополнительныеРеквизиты;
		КонецЕсли;
		
		НоваяСтрока = ТЧНабора.Добавить();
		НоваяСтрока.Свойство = СсылкаНового;
		ОбъектНабор.Записать();
		
		Объект.НаборСвойств = Справочники.НаборыДополнительныхРеквизитовИСведений.ПустаяСсылка();
...
Процедура НастроитьОтборВПараметрахПолитикУчетаСерий(Параметры) Экспорт
	...
	Если Параметры.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС Тогда
	...
	Если ЗначениеЗаполнено(ВидНоменклатуры)
		...
		Или ПолучитьФункциональнуюОпцию("ИспользоватьТоварыВПутиОтПоставщиков")) Тогда
...
Функция ДанныеДляЗаполненияСерийПоFEFOВЭтапе(Объект, ПараметрыУказанияСерий, Запрос)
	...
	|	ДанныеДокумента.Характеристика КАК Характеристика,
	|	ВЫРАЗИТЬ(ДанныеДокумента.Упаковка КАК Справочник.УпаковкиЕдиницыИзмерения) КАК Упаковка,
	|	ДанныеДокумента.ВариантОбеспечения КАК ВариантОбеспечения,
	...
	|	ДанныеДокумента.Характеристика КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно)
