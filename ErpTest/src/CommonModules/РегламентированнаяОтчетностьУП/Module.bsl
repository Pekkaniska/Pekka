...
Процедура ЗаполнитьОтчетАвансыИмуществоФормаОтчета2011Кв3(ПараметрыОтчета, Контейнер) Экспорт
	...
	|	И НЕ СостоянияОСОрганизаций.ОсновноеСредство.НедвижимоеИмуществ
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока Период <= НачалоМесяца(ДобавитьМесяц(ПараметрыОтчета.мДатаКонцаПериодаОтчета, 1)) Цикл
		...
		Если НЕ ПрименениеУСН Тогда
			...
			|	ЕСТЬNULL(СчетаУчета.ГруппаФинансовогоУчета.СчетУчета, СчетаУчета.СчетУчета) КАК СчетУчета,
			|	ЕСТЬNULL(СчетаУчета.ГруппаФинансовогоУчета.СчетУчетаАмортизации, СчетаУчета.СчетНачисленияАмортизации) КАК СчетНачисленияАмортизации
	...
	Пока Период <= НачалоМесяца(ДобавитьМесяц(ПараметрыОтчета.мДатаКонцаПериодаОтчета, 1)) Цикл
		...
		Если НЕ ПрименениеУСН Тогда
			...
			Иначе
				...
				|		ИНАЧЕ ЕСТЬNULL(СтавкиНалогаНаИмуществоСрезПоследних.ОсвобождениеОтНалогообложения, ЛОЖЬ)
				|	КОНЕЦ КАК ОсвобождениеОтНалогообложения,
				|	СписокОС.ОсновноеСредство.ЭтоНедвижимость КАК ЭтоНедвижимость,
				|	ВЫБОР
				|		КОГДА СтавкиНалогаНаИмуществоПоОтдельнымОсновнымСредствамСрезПоследних.ПорядокНалогообложения = ЗНАЧЕНИЕ(Перечисление.ПорядокНалогообложенияИмущества.ОсвобождаетсяОтНалогообложения)
				|			ТОГДА СтавкиНалогаНаИмуществоПоОтдельнымОсновнымСредствамСрезПоследних.КодНалоговойЛьготыОсвобождениеОтНалогообложения
				|		ИНАЧЕ СтавкиНалогаНаИмуществоСрезПоследних.КодНалоговойЛьготыОсвобождениеОтНалогообложения
...
Процедура ЗаполнитьРегламентированныйОтчетАлкоПриложение11ФормаОтчета2012Кв3ПоПодразделениям(ПараметрыОтчета, Контейнер) Экспорт
	...
	|		КОГДА СведенияОбАлкогольнойПродукции.ПроизводительИмпортерДляДекларацийАлко.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
	|			ТОГДА СведенияОбАлкогольнойПродукции.ПроизводительИмпортерДляДекларацийАлко.ИНН
	|		ИНАЧЕ СведенияОбАлкогольнойПродукции.ПроизводительИмпортерДляДекларацийАлко.НалоговыйНомер
	|	КОНЕЦ КАК П000020000104,
	|	СведенияОбАлкогольнойПродукции.ПроизводительИмпортерДляДекларацийАлко.КПП КАК П000020000105,
	|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СубконтоКт1.НаименованиеПолное КАК СТРОКА(200)) КАК П000020000106,
	|	ВЫБОР
...
|					ИЛИ (ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПересортицаТоваров)
	...
	|	КОНЕЦ КАК П000020000107,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.СубконтоКт1 КАК Справочник.Контрагенты).КПП, НЕОПРЕДЕЛЕНО) КАК П000020000108,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПриобретениеТоваровУслуг).ДатаВходящегоДокумента, ХозрасчетныйОбороты.Период) КАК П000020000109,
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПриобретениеТоваровУслуг).НомерВходящегоДокумента, НЕОПРЕДЕЛЕНО) КАК П000020000110,
...
Процедура ЗаполнитьРегламентированныйОтчетСтатистикаФорма4ДМФормаОтчета2012Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоНачальныйОстаток, 0) 
		|				* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|				/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3)) КАК ОСНачальныйОстаток,
		|	ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоПоступило, 0)
		|				* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|				/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3)) КАК ОСПоступило,
		|	ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоИзрасходовано, 0)
		|				* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|				/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3)) КАК ОСИзрасходовано,
		|	ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоКонечныйОстаток, 0)
		|				* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|				/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3)) КАК ОСКонечныйОстаток,
		|	ВЫБОР
		|		КОГДА ОсновныеСредства.ОС.ОбъектБытовогоНазначения
		|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоНачальныйОстаток, 0)
		|							* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|							/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3))
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ОСБытНачальныйОстаток,
		|	ВЫБОР
		|		КОГДА ОсновныеСредства.ОС.ОбъектБытовогоНазначения
		|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоПоступило, 0)
		|							* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|							/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3))
		|		ИНАЧЕ 0
		...
		|	ВЫБОР
		|		КОГДА ОсновныеСредства.ОС.ОбъектБытовогоНазначения
		|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоИзрасходовано, 0)
		|							* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|							/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3))
		|		ИНАЧЕ 0
		...
		|	ВЫБОР
		|		КОГДА ОсновныеСредства.ОС.ОбъектБытовогоНазначения
		|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ОССодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ОсновныеСредства.КоличествоКонечныйОстаток, 0)
		|							* ОССодержаниеМеталлов.КоэффициентЧислитель * ОССодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|							/ (ОССодержаниеМеталлов.КоэффициентЗнаменатель * ОССодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3))
		|		ИНАЧЕ 0
		...
		|	ВЫРАЗИТЬ(ЕСТЬNULL(СодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ХозрасчетныйОстаткиИОбороты.КоличествоНачальныйОстаток, 0)
		|				* СодержаниеМеталлов.КоэффициентЧислитель * СодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|				/ (СодержаниеМеталлов.КоэффициентЗнаменатель * СодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3)),
		|	ВЫРАЗИТЬ(ЕСТЬNULL(СодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ХозрасчетныйОстаткиИОбороты.КоличествоОборотДт, 0)
		|				* СодержаниеМеталлов.КоэффициентЧислитель * СодержаниеМеталлов.КоэффициентЗнаменательБазовая 
		|				/ (СодержаниеМеталлов.КоэффициентЗнаменатель * СодержаниеМеталлов.КоэффициентЧислительБазовая) КАК ЧИСЛО(15,3)),
		|	ВЫРАЗИТЬ(ЕСТЬNULL(СодержаниеМеталлов.Количество, 0) * ЕСТЬNULL(ХозрасчетныйОстаткиИОбороты.КоличествоОборотКт, 0)
...
// Функция возвращает соотвествие имен полей организации.
// 
// Возвращаемое значение:
//	 Структура 
//
Функция СоответствиеИменПолейОрганизации() Экспорт
	...
	СоответствиеДанных.Вставить("СтрИО", "СтранаРегистрации");
...
// Функция возвращает структуру со сведениями об организации в формате БРО.
// 
// Возвращаемое значение:
//	 Структура 
//
Функция КонвертироватьСведенияОбОрганизации(ДанныеОрганизации) Экспорт 

	СоответствиеДанных = СоответствиеИменПолейОрганизации();	
	
	ДанныеОрганизацииКонв = Новый Структура;
	
	Для каждого ТекЭлемент Из ДанныеОрганизации Цикл
		Для каждого ТекСоответствие Из СоответствиеДанных Цикл
			Если ТекЭлемент.Ключ = ТекСоответствие.Значение Тогда
				 ДанныеОрганизацииКонв.Вставить(ТекСоответствие.Ключ, ТекЭлемент.Значение);
				 Продолжить;
	...
	Возврат ДанныеОрганизацииКонв;
	
КонецФункции // КонвертироватьСведенияОбОрганизации()
...
Функция УстаревшаяПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если СписокПоказателей = Неопределено Тогда
		...
		СписокПоказателей.Добавить("СтранаРегистрации"); //СтрИО
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "СтранаРегистрации" Тогда
