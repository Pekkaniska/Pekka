...
// Синхронизирует пометку удаления между элементом справочника ключей и справочником, входящим в ключ.
//
// Параметры:
//  ПолноеИмяСправочникаКлючей	 - Строка 
//  ИзменяемоеЗначение			 - СправочникСсылка - ссылка на элемент изменяемый элемент справочника 
//  ДополнительныеСвойства		 - Структура
//		*ПометкаУдаленияДоЗаписи - Булево - значение пометки удаления элемента до записи измененного объекта в базу
//		*ЭтоНовый - Булево
//  ПометкаУдаления				 - Булево - текущее значение пометки удаления элемента
//  ЭтоГруппа					 - Булево - текущее значение свойства ЭтоГруппа (для групп ключи не ведутся).
//
Процедура УстановитьПометкуУдаленияКлюча(ПолноеИмяСправочникаКлючей, ИзменяемоеЗначение, ДополнительныеСвойства, ПометкаУдаления, ЭтоГруппа) Экспорт

	Если ЭтоГруппа
		Или ДополнительныеСвойства.ЭтоНовый
		Или ПометкаУдаления = ДополнительныеСвойства.ПометкаУдаленияДоЗаписи Тогда
		Возврат;
	КонецЕсли;
	...
	|ГДЕ
	|	&УсловиеУстановкиПометки
	|	И (НЕ Таблица.ПометкаУдаления
	|				И &ПометкаУдаления
	|			ИЛИ Таблица.ПометкаУдаления
	|				И НЕ &ПометкаУдаления
	|				И &УсловиеОтсутствияПометкиЗначения)";
	
	ТипЗначенияРеквизита = ТипЗнч(ИзменяемоеЗначение);
	
	РеквизитыСодержащиеТипЗначения = Новый Массив;
	...
	Для Каждого Реквизит Из МетаданныеСправочникаКлючей.Реквизиты Цикл
		...
		Если Реквизит.Тип.СодержитТип(ТипЗначенияРеквизита) Тогда
			РеквизитыСодержащиеТипЗначения.Добавить("Таблица." + Реквизит.Имя + " = &ИзменяемоеЗначение ");
			ЭлементБлокировки = Блокировка.Добавить(ПолноеИмяСправочникаКлючей); 
			ЭлементБлокировки.УстановитьЗначение(Реквизит.Имя, ИзменяемоеЗначение);
		КонецЕсли;
		...
		Для Каждого ТипРеквизита Из Реквизит.Тип.Типы() Цикл
			...
			Если ОбщегоНазначения.ЭтоСсылка(ТипРеквизита)
				...
				РеквизитыСсылочногоТипа.Добавить("НЕ ЕСТЬNULL(Таблица."+ Реквизит.Имя + ".ПометкаУдаления, ЛОЖЬ)");
		    	Прервать;
			КонецЕсли;
		КонецЦикла;
	...
	Если РеквизитыСодержащиеТипЗначения.Количество() = 0 Тогда
		ТекстИсключения = НСтр("ru = 'Справочник %ИмяСправочника% не отражается в ключе %ИмяКлюча% (передано значение %Значение%)'");
		ТекстИсключения = СтрЗаменить(ТекстИсключения, "%ИмяСправочника%", Метаданные.НайтиПоТипу(ТипЗначенияРеквизита).ПолноеИмя());
		ТекстИсключения = СтрЗаменить(ТекстИсключения, "%ИмяКлюча%", ПолноеИмяСправочникаКлючей);
		ТекстИсключения = СтрЗаменить(ТекстИсключения, "%Значение%", Строка(ИзменяемоеЗначение));
		
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	УсловиеУстановкиПометки = "(" + СтрСоединить(РеквизитыСодержащиеТипЗначения, " ИЛИ ") + ")";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеУстановкиПометки", УсловиеУстановкиПометки); 
	
	УсловиеОтсутствияПометкиЗначения = СтрСоединить(РеквизитыСсылочногоТипа, " И ");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтсутствияПометкиЗначения", УсловиеОтсутствияПометкиЗначения); 
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолноеИмяСправочника", ПолноеИмяСправочникаКлючей); 
	
	Запрос.УстановитьПараметр("ПометкаУдаления", ПометкаУдаления);
	Запрос.УстановитьПараметр("ИзменяемоеЗначение", ИзменяемоеЗначение);
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбъектКлюча = Выборка.Ссылка.ПолучитьОбъект();
		ОбъектКлюча.ДополнительныеСвойства.Вставить("СинхронизацияКлючаСоЗначением");
		
		ОбъектКлюча.УстановитьПометкуУдаления(ПометкаУдаления);
...
//  Булево 
//
Функция СсылкаСоотвествуетОтбору(Ссылка, Отбор) Экспорт
...
Функция ОписаниеТипаДенежногоПоля(ЗнакПоля = Неопределено) Экспорт
	...
	ОписаниеПоля = Метаданные.ОпределяемыеТипы.ДенежноеПолеЛюбойЗнак.Тип;
	Если ЗнакПоля = ДопустимыйЗнак.Неотрицательный Тогда
		ОписаниеПоля = Метаданные.ОпределяемыеТипы.ДенежноеПолеНеотрицательное.Тип;
	КонецЕсли;
...
Функция ПолучитьПрефиксДляУИП(Ссылка)
	...
	//++ НЕ УТКА
	Соответствие.Вставить("ЗаказДавальца",						"ЗКД");
	//-- НЕ УТКА
...
//
// Параметры:
//  Форма			 - УправляемаяФорма	- форма, с которой удаляются элементы
...
Процедура УстановитьОтборИзмерениюСКлючами(Список, ИмяОтбора, ЗначенияОтбора) Экспорт
	...
	Если ТипЗнч(ЗначенияОтбора) = Тип("СписокЗначений") Тогда
		Для каждого ЭлементСписка Из ЗначенияОтбора Цикл
			Если ЗначениеЗаполнено(ЭлементСписка.Значение) Тогда
				...
				Иначе
					ДляПоискаКлючей.Добавить(ЭлементСписка.Значение);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ЗначениеЗаполнено(ЗначенияОтбора) Тогда
		...
		Иначе
			ДляПоискаКлючей.Добавить(ЗначенияОтбора);
		КонецЕсли;
	КонецЕсли;
	...
	Если ДляПоискаКлючей.Количество() > 0  Тогда
		...
		|ГДЕ
		|	КлючиРеестраДокументов.Ключ В(&Ключи)";
		...
		Пока Выборка.Следующий() Цикл
			ЗначенияОтбораДляУстановки.Добавить(Выборка.Ссылка);
		КонецЦикла;
...
// Обработчик подписки СохранитьВДополнительныхСвойствахПроставитьПризнакЭтоНовыйПередЗаписью
//
Процедура СохранитьВДополнительныхСвойствахПризнакЭтоНовыйПередЗаписью(Объект, Отказ) Экспорт
...
// Обработчик подписки СоздатьКлючРеестраДокументовПриЗаписиСправочника
//
Процедура СоздатьКлючРеестраДокументовПриЗаписиСправочника(Объект, Отказ) Экспорт
	
	Если Объект.ОбменДанными.Загрузка Тогда
		Возврат;
	КонецЕсли;
	
	Справочники.КлючиРеестраДокументов.СоздатьОбновитьКлючРеестра(Объект, Объект.ДополнительныеСвойства.ЭтоНовый);
