...
Функция ПересечениеДопустимыхЦелейПоПродукциям(МассивПродукции, НизкокачественнаяПродукция) Экспорт
	...
	Иначе
		...
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПродукцияВЕТИС.Ссылка) КАК Ссылка
		|ИЗ
		|	РегистрСведений.ДопустимыеЦелиПоГруппамВЕТИС КАК ДопустимыеЦелиПоГруппамПриказаВЕТИС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ВидыПродукцииПоГруппамВЕТИС КАК ВидыПродукцииПоГруппамПриказаВЕТИС
		|		ПО ВидыПродукцииПоГруппамПриказаВЕТИС.ГруппаПриказа = ДопустимыеЦелиПоГруппамПриказаВЕТИС.ГруппаПриказа
		...
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЦелиВЕТИС.Идентификатор КАК Цель,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПродукцияВЕТИС.Ссылка) КАК Ссылка
		|ИЗ
		|	Справочник.ПродукцияВЕТИС КАК ПродукцияВЕТИС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРолиПользователяВЕТИС КАК РолиПользователяВЕТИС
		...
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПродукцияВЕТИС.Ссылка) = &КоличествоПродукции";
...
//  ДеревоКэш	 - ДеревоЗначений - дерево с кэшем полученных ранее целей
// 
// Возвращаемое значение:
//  Массив - массив допустимых целей
//
Функция ДопустимыеЦелиПоПродукции(Продукция, ДеревоКэш, НизкокачественнаяПродукция = Ложь) Экспорт
	...
	Если ДеревоКэш.Строки.Количество() = 0 Тогда
		...
		КонецЕсли;
		ТекущаяСтрока = ДеревоКэш.Строки.Получить(0);
		
		Возврат ТекущаяСтрока.Строки.ВыгрузитьКолонку("Цель");
		
	КонецЕсли;
	
	НайденнаяСтрока = ДеревоКэш.Строки.Найти(Продукция, "Продукция");
	Если ЗначениеЗаполнено(НайденнаяСтрока) Тогда
		Возврат НайденнаяСтрока.Строки.ВыгрузитьКолонку("Цель");
	Иначе
		ДеревоЦелей = ДопустимыеЦелиПоПродукциям(Продукция);
		Если ДеревоЦелей.Строки.Количество() = 0 Тогда
			Возврат Новый Массив;
		КонецЕсли;
		
		ТекущаяСтрока = ДеревоЦелей.Строки.Получить(0);
		МассивЦелей = ТекущаяСтрока.Строки.ВыгрузитьКолонку("Цель");
		...
		Для каждого ТекСтрока Из ТекущаяСтрока.Строки Цикл
			НовСтрока = ДобавляемаяСтрока.Строки.Добавить();
			НовСтрока.Продукция = Продукция;
			НовСтрока.Цель = ТекСтрока.Цель;
		КонецЦикла;
...
Функция ЦелиАттестованныхСпециалистов(ГруппыПродукции)
	...
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		...
		МасГруппаПриказа  = СтрРазделить(Макет.Область(НомерСтроки, 1).Текст,".");
		СтрТипПродукции   = СокрЛП(МасГруппаПриказа[2]);
		СтрГруппаПриказа  = СокрЛП(МасГруппаПриказа[1]);
		СтруктураПроверкиДублей.ЦельНаименование  = СокрЛП(Макет.Область(НомерСтроки, 2).Текст);
		СтруктураПроверкиДублей.ЦельИдентификатор = СокрЛП(Макет.Область(НомерСтроки, 3).Текст);
		...
		Если ГруппаПриказа <> СтрГруппаПриказа Тогда
			...
			Если ГруппаПродукции = Неопределено Тогда
				ГруппаПродукции = ГруппыПродукции.Получить(СтрГруппаПриказа + " / " + СтрТипПродукции);
			КонецЕсли;
...
Функция ПереченьПродукцииУполномоченныхЛиц(ГруппыПродукции)
	...
	Для НомерСтроки = 2 По КоличествоСтрок Цикл
		...
		ГруппаПриказа             = СокрЛП(Макет.Область(НомерСтроки, 1).Текст);
		...
		НоваяСтрока.ГруппаПриказа             = ГруппыПродукции.Получить(ГруппаПриказа);
