...
Функция ПостроитьДеревоОператоров() Экспорт
	...
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Степень'"),           НСтр("ru = 'Pow(,)'"),  2);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Квадратный корень'"), НСтр("ru = 'Sqrt()'"),  1);
	
	ГруппаОператоров = РаботаСФормулами.ДобавитьГруппуОператоров(Дерево, НСтр("ru = 'Тригонометрические функции'"));
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Sin'"),  НСтр("ru = 'Sin()'"), 1);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Cos'"),  НСтр("ru = 'Cos()'"), 1);
	РаботаСФормулами.ДобавитьОператор(Дерево, ГруппаОператоров, НСтр("ru = 'Tan'"),  НСтр("ru = 'Tan()'"), 1);
...
Функция ВыборХарактеристикиПоАлгоритму(ДанныеПоНоменклатуре,
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Подбор характеристики по алгоритму'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ДобавитьСоответствиеДляАвтоРасчета(ДанныеДляРасчета, ПутьКДанным, Источник)
	...
	Если (ПутьКДанным.Ключ = "ВыходныеИзделия" 
		...
		И ПутьКДанным.ИмяСвойства = НСтр("ru = 'Количество'") Тогда
	...
	Для каждого КлючИЗначение Из ОписаниеИсточников Цикл
		...
		Если ОписаниеИсточника.Коллекция Тогда
			...
			Для каждого Строка Из Объект[Ключ] Цикл
				...
				ПредставлениеСтроки = СтрШаблон(НСтр("ru = 'Строка[%1].'"), Формат(Строка.НомерСтроки, "ЧН=; ЧГ="));
