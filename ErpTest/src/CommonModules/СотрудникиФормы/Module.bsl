...
//
// Параметры:
//	ФизическоеЛицоСсылка - СправочникСсылка.ФизическиеЛица, ссылка на физлицо, которое уже сопоставлено с сотрудником.
//	Фамилия - Строка - фамилия человека, которого ищем,
//	Имя - Строка - имя человека, которого ищем,
//	Отчество - Строка - отчество человека, которого ищем,
//	Уточнение - Строка - уточнение ФИО человека, которого ищем.
//
// Возвращаемое значение - Структура - структура с полями:
//	ФизическоеЛицоУникально - Булево - Истина, если найден только один человек или не найдено ни одного,
...
Процедура ДобавитьКомандуПечатиСправкиОСреднемЗаработкеДляПособияПоБезработице(КомандыПечати) Экспорт
	
	Если КадровыйУчетФормы.ДоступенПросмотрПлановыхНачислений() Тогда
		
		КомандаПечати = КадровыйУчетФормы.КомандаПечатиСправкиОСреднемЗаработкеДляПособияПоБезработице(КомандыПечати);
	...
	Если Пользователи.РолиДоступны("ЧтениеНалоговИВзносов,ДобавлениеИзменениеНалоговИВзносов") Тогда
		...
		Для ИндексРезультата = 0 По 3 Цикл
			...
			Если ИндексРезультата > 0 Тогда
				...
				ИменаРеквизитовРегистраторов = "Дата,Номер,Сотрудник,Организация";
				...
				РеквизитыРегистраторов = ОбщегоНазначенияБЗК.ЗначенияРеквизитовОбъектов(МассивРегистраторов, ИменаРеквизитовРегистраторов, Истина);
...
Процедура ПечатьТ1ПоСправочникуСотрудники(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_Т1") Тогда
		...
		Для Каждого ДанныеСотрудника Из ДанныеСотрудников Цикл
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ДанныеСотрудника.Сотрудник);
...
Процедура ПечатьТ5ПоСправочникуСотрудники(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_Т5") Тогда
		
		СписокПриказов = Новый Массив;
		
		ПоследниеКадровыеПереводы = ПоследниеКадровыеПереводыСотрудников(МассивОбъектов);
		Для Каждого СотрудникПеревод Из ПоследниеКадровыеПереводы Цикл
			...
			Иначе
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, СотрудникПеревод.Сотрудник);
...
Процедура ПечатьТ8ПоСправочникуСотрудники(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
...
Функция ПоследниеКадровыеПереводыСотрудников(МассивСотрудников)
	
	Возврат СотрудникиФормыВнутренний.ПоследниеКадровыеПереводыСотрудников(МассивСотрудников);
