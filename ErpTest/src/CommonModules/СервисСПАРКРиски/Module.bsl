...
#Область ЗаполнениеГрадациямиИндексовИРеквизитами
	...
	Для Каждого ТекущаяСтрока Из Результат.ЗначенияИндексов Цикл
		ТекущаяСтрока.СводныйИндикаторГрадация = СПАРКРискиКлиентСервер.ГрадацияИндекса(
			ТекущаяСтрока.СводныйИндикатор,
			"СводныйИндикатор");
		ТекущаяСтрока.ИДОГрадация = СПАРКРискиКлиентСервер.ГрадацияИндекса(
			ТекущаяСтрока.ИндексДолжнойОсмотрительности,
			"ИндексДолжнойОсмотрительности");
		ТекущаяСтрока.ИФРГрадация = СПАРКРискиКлиентСервер.ГрадацияИндекса(
			ТекущаяСтрока.ИндексФинансовогоРиска,
			"ИндексФинансовогоРиска");
		ТекущаяСтрока.ИПДГрадация = СПАРКРискиКлиентСервер.ГрадацияИндекса(
	...
	Если Результат.ВидОшибки.Пустая() Тогда
		Если (ТипЗнч(Результат.ЗначенияИндексов) = ТипТаблицаЗначений) И (Результат.ЗначенияИндексов.Количество() > 0) Тогда
			...
			Если Не ОбщегоНазначения.РазделениеВключено() Тогда
				...
				ТНачало = ТекущаяУниверсальнаяДатаВМиллисекундах();
				// Классификатор типов событий и статусов в одинаковый, поэтому для хранения используется единый справочник "ТипыСобытийСПАРКРиски".
				...
				ОписаниеРезультата = ОписаниеРезультата
					...
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						...
						ТаблицаИдентификаторов.Количество(),
						ТКонец - ТНачало);
				...
				Если МассивИдентификаторов.Количество() > 0 Тогда
					...
					Если ТипыСобытийСохраненныеВИБ.Количество() < МассивИдентификаторов.Количество() Тогда
						...
						СПАРКРискиМониторингСобытий.ЗагрузитьНовыеТипыСобытий(МассивИдентификаторов, ТипыСобытийСохраненныеВИБ, ВидОшибкиДобавленияТиповСобытий);
...
Процедура ПослеВызоваОперацииСервиса(Результат)
	...
	Если Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ТребуетсяОплатаИлиПревышенЛимит Тогда
					...
					Если ТекущийРезультат.Свойство("inn") Тогда
						...
						Для Каждого СтрокаЗначенияИндекса Из СтрокиЗначенияИндекса Цикл
							...
							Если ТекущийРезультат.Свойство("aggregatedIndex")
								...
								ПрочитатьЗначениеИзСтруктуры(СтрокаЗначенияИндекса, ИндексыСПАРККонтрагента, "СводныйИндикатор", "totalIndex", ТипЧисло, -1);
								ПрочитатьЗначениеИзСтруктуры(СтрокаЗначенияИндекса, ИндексыСПАРККонтрагента, "Активен", "acting", ТипБулево, Ложь);
								ПрочитатьЗначениеИзСтруктуры(СтрокаЗначенияИндекса, ИндексыСПАРККонтрагента, "ОтображатьСтатус", "showStatus", ТипБулево, Истина);
								ПрочитатьЗначениеИзСтруктуры(СтрокаЗначенияИндекса, ИндексыСПАРККонтрагента, "ДатаСтатуса", "statusDate", ТипДата, '00010101');
								ПрочитатьЗначениеИзСтруктуры(СтрокаЗначенияИндекса, ИндексыСПАРККонтрагента, "ИдентификаторСтатуса", "statusGuid", ТипСтрока, "");
