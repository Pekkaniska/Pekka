...
Процедура ЗаполнитьХэшЭЛН(ПараметрыОбновления = Неопределено) Экспорт
	...
	|		ПО РеестрыЭЛН.НомерЛисткаНетрудоспособности = ХэшиЭЛН.НомерЛисткаНетрудоспособности
	|ГДЕ
	|	РеестрыЭЛН.УдалитьХеш <> """"
	|	И РеестрыЭЛН.Ссылка.ПометкаУдаления = ЛОЖЬ
	|	И ХэшиЭЛН.Хэш ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ХэшиЭЛН КАК ХэшиЭЛН
	|		ПО Больничные.НомерЛисткаНетрудоспособности = ХэшиЭЛН.НомерЛисткаНетрудоспособности
	|ГДЕ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		РегистрыСведений.ХэшиЭЛН.ЗаписатьХэш(Выборка.НомерЛисткаНетрудоспособности, Выборка.УдалитьХеш);
	КонецЦикла;
...
Функция НаименованиеПодсистемы(КодЯзыка = Неопределено)
	Возврат НСтр("ru = 'Пособия социального страхования'", ?(КодЯзыка = Неопределено, ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка(), КодЯзыка));
КонецФункции
