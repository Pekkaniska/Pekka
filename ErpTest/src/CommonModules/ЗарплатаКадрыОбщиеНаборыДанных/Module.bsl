...
Процедура ЗаполнитьОбщиеЗапросы(НаборыДанных, ТолькоРазрешенные)
...
Процедура ЗаменитьПредставленияЗапросов(НаборыДанных, Объект)
...
// Возвращает структуру, описывающую соединения для схемы запроса
//
// Возвращаемое значение:
...
Процедура ВывестиДополнительныеПоляПредставленийВОтчет(Объект, ДополнительныеПоляПредставлений)
	...
	Если ДополнительныеПоляПредставлений <> Неопределено
		...
		Объект.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("СообщатьОРасхожденияхВПутяхКДанным", СообщатьОРасхожденияхВПутяхКДанным);
		
		ВывестиДополнительныеПоляПредставленийВСКД(Объект.СхемаКомпоновкиДанных, ДополнительныеПоляПредставлений, СообщатьОРасхожденияхВПутяхКДанным);
...
Процедура ДобавитьПолеВНаборДанныхСКД(НаборДанных, ИмяПоля, ОписаниеПоля, УстанавливатьПутиСКД, КэшСведенийНаборовДанных, СообщатьОРасхожденияхВПутяхКДанным)
	...
	Если Поле = Неопределено Тогда
		...
		Если ПутиНабора.Получить(ОписаниеПоля.ПутьПоляСКД) = Неопределено Тогда
			...
			ДобавитьВНаборПапкуПоПутиКПолю(НаборДанных, ОписаниеПоля.ПутьПоляСКД);
	...
	Иначе
		...
		ДобавитьВНаборПапкуПоПутиКПолю(НаборДанных, Поле.ПутьКДанным);
		
		Если СообщатьОРасхожденияхВПутяхКДанным = Истина
			И ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда
...
Процедура ДобавитьВНаборПапкуПоПутиКПолю(НаборДанных, ПутьКПолю)
	
	ЧастиПути = СтрРазделить(ПутьКПолю, ".");
	Если ЧастиПути.Количество() > 1 Тогда
		
		ЧастиПути.Удалить(ЧастиПути.Количество() - 1);
		ПутьКПапке = СтрСоединить(ЧастиПути, ".");
		
		Папка = НаборДанных.Поля.Найти(ПутьКПапке);
		Если Папка = Неопределено Тогда
			Папка = НаборДанных.Поля.Добавить(Тип("ПапкаПолейНабораДанныхСхемыКомпоновкиДанных"));
			Папка.ПутьКДанным = ПутьКПапке;
		КонецЕсли;
		
		ДобавитьВНаборПапкуПоПутиКПолю(НаборДанных, ПутьКПапке);
