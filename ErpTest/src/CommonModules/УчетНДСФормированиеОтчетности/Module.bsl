...
Функция ЭлектронноеПредставлениеКнигиПокупок_503(ВыгружаемыйДокумент) Экспорт
	...
	Для Каждого СтрокаСведений Из СведенияДокумента.Записи Цикл
		...
		Если СтрокаСведений.СвПосредн <> Неопределено Тогда
			ВывестиУзел503_СвПосредн(КомпоновщикXML, СтрокаСведений.СвПосредн);
		КонецЕсли;
	...
	ВывестиУзел503_Подписант(КомпоновщикXML, ПараметрыВыгрузки);
...
Функция ЭлектронноеПредставлениеКнигиПродаж_503(ВыгружаемыйДокумент) Экспорт
	...
	ВывестиУзел503_СвПродав(КомпоновщикXML, ПараметрыВыгрузки);
...
#Область ПодготовкаОбщихДанных
	...
	ВидыЦенностейСтр180 = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ВидыЦенностейСтр180,
		Перечисления.ВидыЦенностей.МассивВидовЦенностиНалоговыйАгент());
	...
	Для Каждого ВидЦенности Из Перечисления.ВидыЦенностей Цикл
		Если ВидыЦенностейСтр130.НайтиПоЗначению(ВидЦенности) = Неопределено
			...
			И ВидыЦенностейСтр180.Найти(ВидЦенности) = Неопределено Тогда
...
#Область Раздел2
	...
	Если Контейнер.Свойство("Раздел2") Тогда
		...
		Если УчетНДС.ВедетсяУчетНДСПоФЗ335(ПараметрыОтчета.мДатаКонцаПериодаОтчета)
		   И ОрганизацияНеПлательшикНДС Тогда
...
Процедура ВывестиУзел503_СвПокуп(КомпоновщикXML, ПараметрыВыгрузки)
	...
	ВывестиЭлементыУзловНалогоплательщика503(КомпоновщикXML, ПараметрыВыгрузки);
	
	КомпоновщикXML.ЗаписатьКонецЭлемента(); // СвПокуп
	
КонецПроцедуры

Процедура ВывестиУзел503_СвПродав(КомпоновщикXML, ПараметрыВыгрузки)
...
Процедура ВывестиЭлементыУзловНалогоплательщика503(КомпоновщикXML, ПараметрыВыгрузки)
...
// сведения о посреднике. Процедура неприменима к соответствующему элементу журнала учета счетов-фактур.
//
Процедура ВывестиУзел503_СвПосредн(КомпоновщикXML, СвПосредн)
...
Процедура ВывестиУзел503_Подписант(КомпоновщикXML, ПараметрыВыгрузки)
