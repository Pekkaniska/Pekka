...
//
Функция ТаблицаКоэффициентовРаспределенияСохраняемогоДС(РаспределениеСохраняемыхНачислений, НачисленияДляРаспределения, НазначениеРасчета, Сотрудник, Подразделение, Организация, ПериодРасчета, ДатаНачалаСобытия) Экспорт
	...
	КонецЦикла;
	ОтражениеЗарплатыВБухучете.СвернутьРезультатыРаспределения(ТаблицаКоэффициентов);
	...
	Начисления = НоваяТаблицаНачисленияДляБухучета();
	...
	Для каждого СтрокаТЗ Из НачисленияДляРаспределения Цикл
		...
		НоваяСтрока.Сотрудник 				= Сотрудник;
		НоваяСтрока.ПериодРегистрации 		= НачалоМесяца(ПериодРасчета);
		НоваяСтрока.ПериодДействия 			= НачалоМесяца(ДатаНачалаСобытия);
	...
	|	Начисления.Территория КАК ТерриторияВыполненияРаботВОрганизации,
...
//			* Сумма
//
Функция БухучетНДФЛСотрудниковПоДокументамОснованиям(ТаблицаУсловийОтбора, ИсключаемыйРегистратор) Экспорт
	...
	|			И ТаблицаУсловийОтбора.КатегорияДохода = БухучетНачисленияУдержания.КатегорияДохода
	...
	|	ТаблицаУсловийОтбора.КатегорияДохода,
	...
	|	СУММА(ЕСТЬNULL(Бухчет.Сумма, 0)) КАК Сумма
	|ИЗ
	...
	|			И СписокУсловий.КатегорияДохода = Бухчет.КатегорияДохода
	|
	|СГРУППИРОВАТЬ ПО
	|	СписокУсловий.ФизическоеЛицо,
	|	СписокУсловий.Подразделение,
	|	СписокУсловий.ДокументОснование,
	|	СписокУсловий.МесяцНалоговогоПериода,
	|	СписокУсловий.КатегорияДохода,
	|	ЕСТЬNULL(Бухчет.СтатьяФинансирования, ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)),
	|	ЕСТЬNULL(Бухчет.СтатьяРасходов, ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка))";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ТаблицыКУдаления = Новый Массив();
	ТаблицыКУдаления.Добавить("ВТСписокУсловий");
	ТаблицыКУдаления.Добавить("ВТБухучет");
	ЗарплатаКадры.УничтожитьВТ(Запрос.МенеджерВременныхТаблиц, ТаблицыКУдаления);
...
//
Процедура СформироватьДвиженияБухучетНачисленияУдержанияПоСотрудникам(Движения, Отказ, Организация, ПериодРегистрации, Начисления, Удержания, РезультатыРасчетаНДФЛ, НеУчитыватьВРаспределенииПриОкончательномРасчете = Ложь, ЗаписыватьДвижения = Ложь, УчитыватьНачислениеВРаспределенииНДФЛ = Неопределено) Экспорт
	...
	Если Начисления <> НеОпределено Тогда
		...
		Для Каждого Строка Из Начисления Цикл
			...
			Если УчитыватьНачислениеВРаспределенииНДФЛ = Истина Тогда
				НоваяСтрока.УчитыватьВРаспределенииНДФЛ = Истина;
			КонецЕсли;
...
Процедура СоздатьВТБухучетНачисленийПоСтатьямФинансирования(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, МассивФизическихЛиц = Неопределено,
	...
	Если ДляНДФЛ И ИсключатьМежрасчетныеВыплаты Тогда
		
		ТекстЗапросаУчитыватьВРаспределенииНДФЛ = ТекстЗапроса;
		ТекстЗапросаУчитыватьВРаспределенииНДФЛ = СтрЗаменить(ТекстЗапросаУчитыватьВРаспределенииНДФЛ, "ПОМЕСТИТЬ ВТБухучетНачисленийПоСтатьямФинансирования", "");
		
		УсловиеДляЗамены = "И БухучетНачисления.НеУчитыватьВРаспределенииПриОкончательномРасчете
		|И БухучетНачисления.УчитыватьВРаспределенииНДФЛ";
		ТекстЗапросаУчитыватьВРаспределенииНДФЛ = СтрЗаменить(ТекстЗапросаУчитыватьВРаспределенииНДФЛ,"И &УсловиеИсключатьМежрасчетныеВыплаты",УсловиеДляЗамены);
...
Процедура СведенияОБухучетеНачисленийИВзносовПоСтатьямФинансирования(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, Сотрудники = Неопределено) Экспорт
	...
	СоздатьВТБухучетНачисленийПоСтатьямФинансирования(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, МассивФизическихЛиц);
...
Процедура СформироватьДвиженияКоэффициентыРаспределенияДенежногоСодержания(Движения, КоэффициентыРаспределенияДенежногоСодержания) Экспорт
	...
	Если КоэффициентыРаспределенияДенежногоСодержания.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого СтрокаТЗ Из КоэффициентыРаспределенияДенежногоСодержания Цикл
		НоваяЗапись = Движения.КоэффициентыРаспределенияДенежногоСодержания.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяЗапись, СтрокаТЗ);
		Движения.КоэффициентыРаспределенияДенежногоСодержания.Записывать = Истина;
	КонецЦикла;
...
Процедура СоздатьВТНастройкиНачислений(Запрос, НастройкиФактическихНачисленийСохраняемогоДенежногоСодержания = Ложь)
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда
			// Уточнение получения поля:
			// "НЕОПРЕДЕЛЕНО КАК НазначениеРасчетаСохраняемогоДенежногоСодержания".
			Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержания");
			Модуль.УточнитьТекстЗапросаСоздатьВТНастройкиНачислений(Запрос.Текст);
		КонецЕсли;
...
Процедура СоздатьВТНастройкиБухучетаСотрудников(Запрос, ИспользоватьОбособленныеТерритории, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении, ИмяВТНачисленияИсходная, РаспределениеСохраняемогоДС)
	...
	ОтражениеЗарплатыВБухучете.СоздатьВТСведенияОБухучетеЗарплатыСотрудников(Запрос.МенеджерВременныхТаблиц, ИмяВТНачисленияИсходная, ИменаПолейВременнойТаблицы);
...
Процедура СоздатьВТБухучетОсновногоЗаработка(Запрос, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная, РаспределениеСохраняемогоДС)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТНачисленияДляРаспределения", ИмяВТНачисленияИсходная);
	Если РаспределениеСохраняемогоДС Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПО Начисления.Сотрудник = РаспределениеОсновногоЗаработка.Сотрудник", "ПО Начисления.ВременныйСотрудник = РаспределениеОсновногоЗаработка.Сотрудник");
	КонецЕсли;
	Результат = Запрос.ВыполнитьПакет();
...
Процедура СоздатьВТНачисленияКакЗаданоВидуРасчетаДляБухучета(Запрос, ИмяВТНачисленияИсходная, ИспользоватьОбособленныеТерритории)
	...
	|		И СписокНачислений.ТерриторияВыполненияРаботВОрганизации = НастройкиБухучетаСотрудников.ТерриторияВыполненияРаботВОрганизации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиНачислений КАК НастройкиНачислений
	|		ПО СписокНачислений.Начисление = НастройкиНачислений.Начисление
	|ГДЕ
	|	СтрокиКУдалению.ИдентификаторСтроки ЕСТЬ NULL
	...
	Если НЕ ИспользоватьОбособленныеТерритории Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И СписокНачислений.ТерриторияВыполненияРаботВОрганизации = НастройкиБухучетаСотрудников.ТерриторияВыполненияРаботВОрганизации", "");
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТНачисленияДляРаспределения", ИмяВТНачисленияИсходная);
	Запрос.Выполнить();
...
Процедура СоздатьВТНачисленияПоБазе(Запрос, УчитыватьСторноЗаписи, ИмяВТНачисленияИсходная)
	...
	|	ВЫБОР
	|		КОГДА СписокНачислений.ТерриторияВыполненияРаботВОрганизации ЕСТЬ НЕ NULL 
	|				И СписокНачислений.ТерриторияВыполненияРаботВОрганизации <> НЕОПРЕДЕЛЕНО
	|				И СписокНачислений.ТерриторияВыполненияРаботВОрганизации <> ЗНАЧЕНИЕ(Справочник.ТерриторииВыполненияРабот.ПустаяСсылка)
...
Процедура СоздатьВТБухучетНачисленияПоБазе(Запрос, ИспользоватьОбособленныеТерритории, УчитыватьСторноЗаписи, МенеджерКадровогоУчета, МенеджерДанныхУчетаВремени)
	...
	Если Не РезультатОчередностьРаспределенияПоБазе.Пустой() Тогда
		...
		ОписаниеПоляТерритория = "ЗНАЧЕНИЕ(Справочник.ТерриторииВыполненияРабот.ПустаяСсылка)";
		Если ИспользоватьОбособленныеТерритории Тогда
			ОписаниеПоляТерритория = 
			"	ВЫБОР
			|		КОГДА БухучетНачисленийВременная.ТерриторияВыполненияРаботВОрганизации ЕСТЬ НЕ NULL 
			|				И БухучетНачисленийВременная.ТерриторияВыполненияРаботВОрганизации <> НЕОПРЕДЕЛЕНО
			|				И БухучетНачисленийВременная.ТерриторияВыполненияРаботВОрганизации <> ЗНАЧЕНИЕ(Справочник.ТерриторииВыполненияРабот.ПустаяСсылка)
			|			ТОГДА БухучетНачисленийВременная.ТерриторияВыполненияРаботВОрганизации
			|		ИНАЧЕ БухучетНачисленийВременная.Подразделение
			|	КОНЕЦ";
		КонецЕсли;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "БухучетНачисленийВременная.ТерриторияВыполненияРаботВОрганизации", ОписаниеПоляТерритория);
		Запрос.Выполнить();	
...
Процедура СоздатьВТБухучетНачисленийБезДоговоровГПХ(ИсходныеДанные, ИмяВТБухучетНачислений) Экспорт
	...
	// РаспределениеСохраняемогоДС - признак того, что выполняется получение бухучета начислений, входящих в расчет сохраняемого ДС
	...
	СоздатьВТНастройкиБухучетаСотрудников(Запрос, ИспользоватьОбособленныеТерритории, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении, ИмяВТНачисленияИсходная, РаспределениеСохраняемогоДС);
	...
	СоздатьВТНачисленияКакЗаданоВидуРасчетаДляБухучета(Запрос, ИмяВТНачисленияИсходная, ИспользоватьОбособленныеТерритории);
	...
	СоздатьВТБухучетОсновногоЗаработка(Запрос, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная, РаспределениеСохраняемогоДС);
...
Процедура СоздатьВТБухучетНачисленийПоБазеСохраняемогоДС(Запрос, КоэффициентыРаспределенияДенежногоСодержания, СтрокиКоэффициентыСохраняемогоДС, ИмяВТНачисленияИсходная)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		НачисленияПоБазе = РезультатЗапроса.Выгрузить();
		Отбор = Новый Структура("НазначениеРасчета");
...
Функция ДанныеДляРаспределенияНДФЛ(МенеджерВременныхТаблиц, ИмяВТБухучетНачислений, МассивФизическихЛиц, МассивРаспределяемыхУдержаний, ПериодРегистрации, Организация, ОкончательныйРасчет, ИсключаемыйРегистратор, РегистраторыНДФЛОбновленияБухучета)
	...
	Если ОкончательныйРасчет Тогда
		...
		СоздатьВТБухучетНачисленийПоСтатьямФинансирования(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, МассивФизическихЛиц, ИсключаемыеРегистраторы, Истина, Истина, Истина);
...
Процедура РаспределитьНДФЛПоБазе(РезультатыРаспределения, ТаблицаДляРаспределения, База, БазаВсеНачисления, СтрокиУжеУдержаноПоФизическимЛицам, Организация, ПериодРегистрации, ОтборТолькоПоФизлицу = Ложь)
	...
	ОтражениеЗарплатыВБухучете.СвернутьРезультатыРаспределения(РезультатыРаспределения);
...
Процедура РаспределитьУдержанияПоБазе(РезультатыРаспределения, ТаблицаУдержаний, БазаУдержаний, СтрокиУжеУдержаноПоФизическимЛицам, Организация, ПериодРегистрации, БазовыеВидыРасчета = Неопределено, ТочностьОкругленияВТаблице = Ложь, РаспределятьТолькоПоРабочимМестам = Ложь)
