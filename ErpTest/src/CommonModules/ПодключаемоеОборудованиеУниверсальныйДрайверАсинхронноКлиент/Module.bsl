...
Процедура НачатьПечатьЧекаКоррекции(ОповещениеПриЗавершении, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	ФискальныйПризнак  = "";
	АдресСайтаПроверки = "";
	ДанныеЧека = МенеджерОборудованияВызовСервера.ПолучитьXMLПакетДляЧекаКоррекции(ВходныеПараметры, ВходныеПараметры, Истина);
	Попытка
		Оповещение = Новый ОписаниеОповещения("НачатьПечатьЧекаКоррекции_Завершение", ЭтотОбъект, ДополнительныеПараметры);
		ОбъектДрайвера.НачатьВызовСформироватьЧекКоррекции(Оповещение, ПараметрыПодключения.ИДУстройства, ДанныеЧека, НомерЧека, НомерСмены, ФискальныйПризнак, АдресСайтаПроверки); 
	Исключение
		ВыходныеПараметры.Очистить();
		ВыходныеПараметры.Добавить(999);
...
Процедура НачатьФискализациюЧека(ОповещениеПриЗавершении, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	ИначеЕсли ПараметрыПодключения.ТипОборудования = "ККТ" Тогда 
		...
		НомерСмены = 0;
		НомерЧека  = 0;
		ДанныеЧека = МенеджерОборудованияВызовСервера.ПолучитьXMLПакетДляФискализацияЧека(ОбщиеПараметры, ВходныеПараметры, СуммаЧека, РевизияИнтерфейса, ШиринаСтроки);
		...
		Попытка
			...
			Если ПараметрыПодключения.РевизияИнтерфейса = 2002 Тогда  // Версия стандарта 2.2 (ОФД 1.0.5)
...
Процедура НачатьФискализациюЧека_Завершение(РезультатВыполнения, Параметры, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения Тогда
		...
		Если Отправляет1СSMS Или Отправляет1СEmail Тогда
			...
			АтрибутыЧека = МенеджерОборудованияКлиент.ЗаполнитьАтрибутыИдентификацииЧека();
			АтрибутыЧека.РегистрационныйНомерККТ = РегистрационныйНомерККТ;
			АтрибутыЧека.ЗаводскойНомерФН = ЗаводскойНомерФН;
			АтрибутыЧека.НомерЧекаФН = НомерЧека;
			АтрибутыЧека.ФискальныйПризнак = ФискальныйПризнак;
			АтрибутыЧека.СуммаЧека = СуммаЧека;
			АтрибутыЧека.АдресСайтаПроверки = АдресСайтаПроверки;
			АтрибутыЧека.ДатаСеанса = ДатаСеанса;
			ТекстСообщения = МенеджерОборудованияКлиент.ПолучитьШаблонАтрибутовЧекаДляОтсылки(АтрибутыЧека);
