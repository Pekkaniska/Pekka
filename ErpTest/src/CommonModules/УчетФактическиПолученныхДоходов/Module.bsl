...
Процедура СоздатьВТНалогУдержанный(МенеджерВременныхТаблиц, ИсточникНДФЛ, ДатаПолученияДохода, ИмяВТ = "ВТНалогУдержанный") Экспорт
	...
	// Необходимо уточнить дату полкчения дохода
	...
	Если ЕстьДоудержанныеНалоги Тогда
		...
		УдаляемыеТаблицы.Добавить("ВТНепереносимыеОснованияНалогов");
		УдаляемыеТаблицы.Добавить("ВТПредварительноНалогУдержанный");
	...
	Если УдаляемыеТаблицы.Количество() > 0 Тогда
		ЗарплатаКадры.УничтожитьВТ(МенеджерВременныхТаблиц, УдаляемыеТаблицы);
	КонецЕсли;
	
КонецПроцедуры

// Переносит ранеее зарегистрированный доход на новую дату получения дохода
// Параметры:
...
Процедура ЗаполнитьКатегориюДоходаВОперацияНалоговогоУчетаПоНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	|		И ОперацияНалоговогоУчетаПоНДФЛСведенияОДоходах.КодДохода <> ЗНАЧЕНИЕ(Справочник.ВидыДоходовНДФЛ.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Процедура ИсправитьДвиженияОперацияНалоговогоУчетаПоНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	МАКСИМУМ(Регистраторы.Доходы) КАК Доходы
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|							ВсеРегистраторы.Регистратор
	|						ИЗ
	|							ВТВсеРегистраторы КАК ВсеРегистраторы)) КАК Доходы
	|		
	|		СГРУППИРОВАТЬ ПО
	|			Доходы.Ссылка,
	|			Доходы.Сотрудник,
	...
	|				ОперацияНалоговогоУчетаПоНДФЛНДФЛИсчисленныйПоСтавке13.Ссылка,
	|				ВЫБОР
	|					КОГДА ОперацияНалоговогоУчетаПоНДФЛНДФЛИсчисленныйПоСтавке13.МесяцНалоговогоПериода < &ДатаИзмененияПорядкаИсчисленияНалогаДляИностранцев
	|						ТОГДА НАЧАЛОПЕРИОДА(ОперацияНалоговогоУчетаПоНДФЛНДФЛИсчисленныйПоСтавке13.МесяцНалоговогоПериода, МЕСЯЦ)
	|					ИНАЧЕ ОперацияНалоговогоУчетаПоНДФЛНДФЛИсчисленныйПоСтавке13.МесяцНалоговогоПериода
	|				КОНЕЦ,
	|				ВЫБОР
	...
	|				ВЫБОР
	|					КОГДА ОперацияНалоговогоУчетаПоНДФЛНДФЛУдержанный.МесяцНалоговогоПериода < &ДатаИзмененияПорядкаИсчисленияНалогаДляИностранцев
	|						ТОГДА НАЧАЛОПЕРИОДА(ОперацияНалоговогоУчетаПоНДФЛНДФЛУдержанный.МесяцНалоговогоПериода, МЕСЯЦ)
	|					ИНАЧЕ ОперацияНалоговогоУчетаПоНДФЛНДФЛУдержанный.МесяцНалоговогоПериода
	|				КОНЕЦ КАК МесяцНалоговогоПериода,
	|				ВЫБОР
	...
	|СГРУППИРОВАТЬ ПО
	|	Регистраторы.Ссылка,
	|	Регистраторы.Ссылка.Организация,
	|	Регистраторы.Ссылка.ДатаОперации";
	Если ПараметрыОбновления = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, " ПЕРВЫЕ 1000", "");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПараметрыОбновления <> НеОпределено Тогда
			...
			Исключение
				ОтменитьТранзакцию();
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось исправить движения %1 по причине %2'"),
					Выборка.Ссылка,
		...
		Если Выборка.Доходы Тогда
			Доходы = РезультатЗапроса.Выгрузить();
			СведенияОДоходахНДФЛ = РегистрыНакопления.СведенияОДоходахНДФЛ.СоздатьНаборЗаписей();
			СведенияОДоходахНДФЛ.Отбор.Регистратор.Установить(Выборка.Ссылка);
		...
		Если Выборка.Доходы Тогда
			УчетНДФЛ.СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(Движения, Отказ, Выборка.Организация, Выборка.ДатаОперации, Доходы);
		КонецЕсли;
		...
		Если Не Отказ Тогда
			...
			Если Выборка.Доходы Тогда
				ОбновлениеИнформационнойБазы.ЗаписатьДанные(СведенияОДоходахНДФЛ);
			КонецЕсли;
		КонецЕсли;
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ЗавершитьОбновлениеДанных(ПараметрыОбновления);
