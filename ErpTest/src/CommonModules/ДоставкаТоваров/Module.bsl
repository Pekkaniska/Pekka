...
//+++rarus-spb_zlov 30.04.2017    
		Или ДокументОбъект.СпособДоставки = Перечисления.СпособыДоставки.пкПерекатНаОбъекте
//---rarus-spb_zlov 30.04.2017 
...
Функция ПоследниеРеквизитыДоставкиИзРаспоряжений(ОбъектСсылка, ПолучательОтправитель = Неопределено, Перевозчик = Неопределено)
	...
	Если СвязанныеДокументы[ИмяОбъекта] = Неопределено Тогда
		...
		Если СвязанныеДокументы[ИмяОбъекта] = Неопределено Тогда
			...
			//++ НЕ УТ
			СвязанныеДокументы.Вставить("Документ.ПриемкаТоваровНаХранение", Документы.ПриемкаТоваровНаХранение.ПустаяСсылка());
			//-- НЕ УТ
			
			Если СвязанныеДокументы[ИмяОбъекта] = Неопределено Тогда
				СвязанныеДокументы.Очистить();
				//++ НЕ УТ	
				СвязанныеДокументы.Вставить("Документ.ЗаказПереработчику",Документы.ЗаказПереработчику.ПустаяСсылка());
				СвязанныеДокументы.Вставить("Документ.ПередачаСырьяПереработчику", Документы.ПередачаСырьяПереработчику.ПустаяСсылка());
				//-- НЕ УТ
				Если СвязанныеДокументы[ИмяОбъекта] = Неопределено Тогда
					СвязанныеДокументы.Очистить();
					СвязанныеДокументы.Вставить(ИмяОбъекта,ОбъектСсылка);
					//++ НЕ УТКА
					Если ИмяОбъекта = "Документ.ПередачаДавальцу" Тогда
						СвязанныеДокументы.Очистить();
						СвязанныеДокументы.Вставить("Документ.ЗаказДавальца", Документы.ЗаказДавальца.ПустаяСсылка())
					КонецЕсли;
					//-- НЕ УТКА
				КонецЕсли;
			КонецЕсли;
...
Функция ПолучитьПараметрыПриемки(ОбъектСсылка, Знач ВариантПриемкиТоваров = Неопределено, НакладнаяПоЗаказам = Ложь)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПриемкаТоваровНаХранение") И Не НакладнаяПоЗаказам Тогда
		...
		СтруктураВозврата.РаспоряжениеНаПриемку			= Тип("ДокументСсылка.ПриемкаТоваровНаХранение");
	//-- НЕ УТ
