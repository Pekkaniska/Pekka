...
Функция РасчетыВУсловныхЕдиницах(ДоговорКонтрагента) Экспорт
	...
	Возврат ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВРублях;
...
//	ПараметрыКонтекста - Структура - Содержит поля:
//		* Дата 				- Дата - обязательный
//		* Организация 		- СправочникСсылка.Организации - обязательный
//		* Склад		 		- СправочникСсылка.Склады - не обязательный
//		* ТипЦен 			- СправочникСсылка.ВидыЦен - не обязательный
//		* ВалютаДокумента	- СправочникСсылка.Валюты - не обязательный
//		* Курс	 			- Число - не обязательный
//		* Кратность			- Число - не обязательный
//		* СуммаВключаетНДС	- Число - не обязательный
//		* СтавкаНДС			- ПеречислениеСсылка.СтавкиНДС - не обязательный. 
//			Если параметр передан, то будет использована переданная ставка, иначе ставка из номенклатуры.
//
// Возвращаемое значение:
//	Структура - Содержит ключи:
//		* Номенклатура		- СправочникСсылка.Номенклатура - Позиция номенклатуры.
//		* Наименование		- Строка - Наименование номенклатуры.
//		* СтавкаНДС			- ПеречислениеСсылка.СтавкиНДС - Ставка НДС при продаже номенклатуры.
...
//		* Услуга			- Булево - Признак услуги.
//		* Цена				- Число - Цена по типу цен.
//		* ЦенаВРознице		- Число - Заполняется только в БП.
//		* СчетаУчета		- Структура - Счета учета.
//		* ОтражениеВУСН		- ПеречислениеСсылка.ОтражениеВУСН - Отражение в УСН.
//		* Комиссионный		- Булево - Признак комиссионного товара.
//		* ДоходЕНВД			- Булево - Признак того, что доход относится к ЕНВД.
//		* Субконто			- Произвольный - Субконто учета дохода от реализации номенклатуры (Номенклатурная группа).
//
Функция ПолучитьСведенияОНоменклатуре(Номенклатура, ПараметрыКонтекста) Экспорт
	...
	СведенияОНоменклатуре.Вставить("СтранаПроисхождения",  Справочники.СтраныМира.ПустаяСсылка());
	СведенияОНоменклатуре.Вставить("НомерГТД",             Справочники.НомераГТД.ПустаяСсылка());
	...
	Иначе
		...
		СтруктураПараметровЦены.Вставить("Период", ПараметрыКонтекста.Дата);
		СтруктураПараметровЦены.Вставить("ТипЦен", ТипЦен);
		СтруктураПараметровЦены.Вставить("Валюта", ВалютаДокумента);
		СтруктураПараметровЦены.Вставить("Курс", КурсДок);
		СтруктураПараметровЦены.Вставить("Кратность", КратностьДок);
		...
		Цена = СоответствиеНоменклатураЦена[Номенклатура];
	...
	СведенияОНоменклатуре.Вставить("ДоходЕНВД", Ложь);
	...
	Если ЗначениеЗаполнено(СчетДоходов) Тогда 
		...
		Если СвойстваСчетаДоходов.ВидСубконто1 = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы
		 ИЛИ СвойстваСчетаДоходов.ВидСубконто2 = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы
		 ИЛИ СвойстваСчетаДоходов.ВидСубконто3 = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы Тогда
			СведенияОНоменклатуре.Вставить("Субконто", РеквизитыНоменклатуры[ИмяРеквизитаНоменклатурнаяГруппа]);
		КонецЕсли;
...
//
// Возвращаемое значение:
//	Булево - Истина, если номенклатура является услугой.
//
...
//
// Параметры:
//	ПолеНоменклатура - Строка - Выражение для получения ссылки на номенклатуру.
//
// Возвращаемое значение:
//	Строка - Выражение для получения признака услуги.
//
...
//
// Возвращаемое значение:
//	Булево - Истина, если организация является обособленным подразделением.
//
...
Функция ПолучитьСтавкуНалогаНаПрибыль(СтруктураШапкиДокумента, ИФНС = Неопределено, СтавкаФБ = 0, СтавкаРФ = 0, СтавкаМестный = 0) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ПрименяютсяРазныеСтавкиНалогаНаПрибыль") Тогда
		...
		Иначе
			ИмяРегистра = "СтавкиНалогаНаПрибыльВБюджетСубъектовРФ";
			Если Метаданные.РегистрыСведений.Найти(ИмяРегистра) <> Неопределено Тогда
				Отбор    = Новый Структура("Организация, РегистрацияВНалоговомОргане", СтруктураШапкиДокумента.Организация, ИФНС);
				Ставки   = РегистрыСведений.СтавкиНалогаНаПрибыльВБюджетСубъектовРФ.ПолучитьПоследнее(СтруктураШапкиДокумента.Дата, Отбор);
				СтавкаНалогаПоИФНС = ?(Ставки = Неопределено, 0, Ставки.СтавкаСубъектРФ / 100);
			КонецЕсли;
		КонецЕсли;
...
Функция ПодготовитьСоответствиеНоменклатураЦена(СтруктураПараметров) Экспорт
	...
	СтарыйКурс = Новый Структура();
	СтарыйКурс.Вставить("Валюта");
	СтарыйКурс.Вставить("Курс", 1);
	СтарыйКурс.Вставить("Кратность", 1);

	НовыйКурс = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СтарыйКурс);
	НовыйКурс.Валюта    = Валюта;
	НовыйКурс.Курс      = Курс;
	НовыйКурс.Кратность = Кратность;

	Пока Выборка.Следующий() Цикл
		
		ЗаполнитьЗначенияСвойств(СтарыйКурс, Выборка);
	
		ЦенаНоменклатуры = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
			Выборка.Цена,
			СтарыйКурс,
			НовыйКурс);
