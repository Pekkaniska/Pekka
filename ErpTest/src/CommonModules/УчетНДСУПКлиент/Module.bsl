...
Процедура СформироватьИсправительныеСчетаФактурыПоИзмененнымДокументам(ТаблицаИзмененныхДокументов) Экспорт
	...
	Если ТаблицаИзмененныхДокументов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Формирование исправительных счетов-фактур не требуется'"));
		Возврат;
	КонецЕсли;
	
	УчетНДСУПСлужебныйКлиент.СформироватьИсправительныеСчетаФактурыПоИзмененнымДокументам(ТаблицаИзмененныхДокументов);
	//-- Локализация
	
КонецПроцедуры

// Открывает форму журнала регистрации с отбором по событию формирования исправительных счетов-фактур.
//
// Параметры:
// 	Форма - УправляемаяФормы - Форма-владелец, и которой открывается журнал.
//
Процедура ОткрытьИсправительныеСчетаФактурыСформированныеПоИзмененнымДокументам(Форма) Экспорт
	
	//++ Локализация
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СобытиеЖурналаРегистрации", НСтр("ru = 'Сформирован исправительный счет-фактура'"));
	ПараметрыФормы.Вставить("ДатаНачала", НачалоДня(ОбщегоНазначенияКлиент.ДатаСеанса()));
	ПараметрыФормы.Вставить("ДатаОкончания", КонецДня(ОбщегоНазначенияКлиент.ДатаСеанса()));
	
	ОткрытьФорму("Обработка.ЖурналРегистрации.Форма", 
		ПараметрыФормы, Форма, , , , Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	//-- Локализация
	
КонецПроцедуры

// Вызывается в обработчике события ОбработкаОповещения формы, устанавливает заголовок гиперссылки перехода к документам.
// 
// Параметры:
// 	ИмяСобытия - Строка - Имя события оповещения.
// 	Параметр - Произвольный - Значение параметра оповещения.
// 	ЭлементОткрытьИсправительныеСчетаФактуры - ЭлементФормы - Элемент формы перехода к сформированным счетам-фактурам.
// 	
Процедура ЗаконченоФормированиеИсправительныхСчетовФактурПоИзмененнымДокументам(ИмяСобытия, Параметр, ЭлементОткрытьИсправительныеСчетаФактуры) Экспорт
	
	//++ Локализация
	Если ИмяСобытия = "СформированыИсправительныеСчетаФактурыПоИзмененнымДокументам" Тогда
		ЭлементОткрытьИсправительныеСчетаФактуры.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сформировано %1'"), Параметр);
		ЭлементОткрытьИсправительныеСчетаФактуры.Видимость = Истина;
	КонецЕсли;
	//-- Локализация
	
КонецПроцедуры

// Формирует задания на печать исправительных счетов-фактур на ответственных за документы.
//
// Параметры:
// 	ТаблицаИзмененныхДокументов - ТаблицаЗначений - Измененные документы продажи и их реквизиты.
// 	                              Колонки таблицы:
...
Процедура СформироватьЗаданияНаПечатьИсправительныхСчетовФактурПоИзмененнымДокументам(ТаблицаИзмененныхДокументов) Экспорт
	...
	//++ Локализация
	Если ТаблицаИзмененныхДокументов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Формирование заданий не требуется'"));
		Возврат;
	КонецЕсли;
	
	УчетНДСУПСлужебныйКлиент.СформироватьЗаданияНаПечатьИсправительныхСчетовФактурПоИзмененнымДокументам(ТаблицаИзмененныхДокументов);
	//-- Локализация
	
КонецПроцедуры

// Открывает форму журнала регистрации с отбором по событию формирования заданий на печать исправительных счетов-фактур.
//
// Параметры:
// 	Форма - УправляемаяФормы - Форма-владелец, и которой открывается журнал.
//
Процедура ОткрытьЗаданияНаПечатьИсправительныхСчетовФактурИзмененнымДокументам(Форма) Экспорт
...
// Вызывается в обработчике события ОбработкаОповещения формы, устанавливает заголовок гиперссылки перехода в журнал регистрации.
// 
// Параметры:
// 	ИмяСобытия - Строка - Имя события оповещения.
// 	Параметр - Произвольный - Значение параметра оповещения.
// 	ЭлементОткрытьЗаданияНаПечать - ЭлементФормы - Элемент формы перехода к заданиям на печать.
// 	
Процедура ЗаконченоФормированиеЗаданийНаПечатьИсправительныхСчетовФактурПоИзмененнымДокументам(ИмяСобытия, Параметр, ЭлементОткрытьЗаданияНаПечать) Экспорт
	
	//++ Локализация
	Если ИмяСобытия = "СформироватьЗаданияНаПечатьИсправительныхСчетовФактурПоИзмененнымДокументамЗавершение" Тогда
		ЭлементОткрытьЗаданияНаПечать.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Сформировано %1'"), Параметр);
		ЭлементОткрытьЗаданияНаПечать.Видимость = Истина;
	КонецЕсли;
	//-- Локализация
