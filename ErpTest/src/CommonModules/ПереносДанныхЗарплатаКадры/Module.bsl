...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ПереносДанныхЗарплатаКадры.УстановитьДоступенПереносДанныхИзПрежнихПрограмм";
...
Процедура ВыполнитьОбработчикиПослеЗагрузкиПараметров(ОбработкаОбъект, Параметры) Экспорт

	// Выключение контроля уникальности табельных номеров перед загрузкой сотрудников.
	Если Параметры.ИзменитьНастройкуУникальностьТабельныхНомеров Тогда
		ИзменитьНастройкуУникальностьТабельныхНомеровПередЗагрузкойДанных(Параметры);
	КонецЕсли;
	
	// Обслуживание загрузки присоединенных файлов.
	ОбработкаОбъект.Параметры.Вставить("ПутьКПрисоединеннымФайлам", "");
	Если ТипЗнч(ОбработкаОбъект.Данные) = Тип("Структура") И ОбработкаОбъект.Данные.Свойство("ПутьКПрисоединеннымФайлам") Тогда
		ОбработкаОбъект.Параметры.ПутьКПрисоединеннымФайлам = ОбработкаОбъект.Данные.ПутьКПрисоединеннымФайлам;
	КонецЕсли;
...
Процедура ВыполнитьОбработчикиПослеЗагрузкиДанных(Параметры) Экспорт
	...
	Если Параметры.ИзменитьНастройкуУникальностьТабельныхНомеров Тогда
		ИзменитьНастройкуУникальностьТабельныхНомеровПослеЗагрузкиДанных();
	КонецЕсли;
	...
	Если Параметры.ЗаполнитьРеквизитыПодчиненныхПодразделений Тогда
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ЗаполнитьРеквизитыПодчиненныхПодразделений();
	КонецЕсли;
...
Процедура ПодразделенияОрганизаций_ПослеЗагрузки(Объект) Экспорт
...
Процедура ДокументыФизическихЛиц_ПослеЗагрузки(НаборЗаписей) Экспорт
...
Процедура ЗаполнитьМестаВыплатыЗарплатыСотрудников(Параметры) Экспорт
	...
	Если Не Параметры.Свойство("ЗагруженныеЛицевыеСчетаСотрудников") Или Не Параметры.Свойство("ЗагруженныеСотрудники") Тогда
		//	
	Иначе
		
		Если Параметры.ЗагруженныеЛицевыеСчетаСотрудников.Количество() = 0 Или Параметры.ЗагруженныеСотрудники.Количество() = 0 Тогда
			//	
		Иначе	
...
Процедура ЛицевыеСчетаСотрудников_ПослеЗагрузки(НаборЗаписей, Параметры) Экспорт
	...
	Если Не Параметры.Свойство("ЗагруженныеЛицевыеСчетаСотрудников") Или Параметры.ЗагруженныеЛицевыеСчетаСотрудников.Количество() = 0 Тогда
...
Процедура Сотрудники_ПослеЗагрузки(Объект, Параметры) Экспорт
	
	Если Не Параметры.Свойство("ЗагруженныеСотрудники") Тогда
		Параметры.Вставить("ЗагруженныеСотрудники", Новый Массив);
	КонецЕсли;
...
Процедура НастройкиРасчетаЗарплаты_ПослеЗагрузки(НаборЗаписей, Отказ) Экспорт
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Если Настройка.Количество() > 0 Тогда
			...
			Настройка[0].ИспользоватьНачисленияПоДоговорам = Настройка[0].ИспользоватьНачисленияПоДоговорам Или НаборЗаписей[0].ИспользоватьНачисленияПоДоговорам;
		...
		Иначе
			...
			НоваяЗапись.ИспользоватьНачисленияПоДоговорам = НаборЗаписей[0].ИспользоватьНачисленияПоДоговорам;
...
Процедура ПрисвоениеВоинскогоЗвания_ПослеЗагрузки(Объект) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ВоинскиеСпециальныеЗвания") Тогда
...
Процедура ПрисвоениеКлассногоЧинаРанга_ПослеЗагрузки(Объект) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КлассныеЧиныРанги") Тогда
		ВремяРегистрации = ЗарплатаКадрыРасширенный.ВремяРегистрацииДокумента(Объект.Ссылка, Объект.ДатаПрисвоения);
		Документы[Объект.Метаданные().Имя].ЗаполнитьНачисленияСотрудника(Объект, ВремяРегистрации);
		Документы[Объект.Метаданные().Имя].РассчитатьФОТПоДокументу(Объект, ВремяРегистрации);
	КонецЕсли;
...
Процедура ОтпускПоУходуЗаРебенкомV7_ПослеЗагрузки(Объект) Экспорт
	...
	Если ОсновныеСотрудники.Количество() > 0 Тогда
		...
		ДанныеДляРасчета = УчетПособийСоциальногоСтрахованияРасширенный.ДанныеДляРасчетаСреднегоЗаработкаФСССотрудника(ОсновнойСотрудникФизЛица, ДополнительныеПараметры);
...
Процедура ИзменитьНастройкуУникальностьТабельныхНомеровПередЗагрузкойДанных(Параметры)
	...
	Если Настройка.Количество() > 0 Тогда
		...
		Иначе
			// Уникальность выключена, сбросим параметр, что бы после загрузки данных не обрабатывать его.
			Параметры.ИзменитьНастройкуУникальностьТабельныхНомеров = Ложь;
			Записывать = Ложь;
		КонецЕсли;
	Иначе
		НоваяЗапись = Настройка.Добавить();
...
Процедура ИзменитьНастройкуУникальностьТабельныхНомеровПослеЗагрузкиДанных()
