...
Функция ДеревоЭлементовБюджета(ВидБюджета, ВыводитьЭлементыСложнойТаблицы = Ложь) Экспорт
	...
	ВидЭлементаКэша = Перечисления.ВидыКэшаБюджета["ДеревоЭлементов" + Постфикс];
...
// Ключевая операция: "Бюджетирование.РасчетКэшаПолучениеСтруктурыОписания"
//
...
Функция ИсточникиЗначенийЭлемента(Кэш, СтрокаДерева, Заполнение = Ложь, БезПроизводных = Ложь) Экспорт
	...
	Если Не СложнаяТаблица Тогда
		...
		ПодчиненныеИсточникиЗначенийЭлементаПростойТаблицы(
			ФинансоваяОтчетностьКлиентСервер.ПодчиненныеСтроки(СтрокаДерева),
			ТаблицаИсточников,
		...
		Если Таблица <> Неопределено Тогда //Измерения могут быть выбраны без таблицы
			...
			Если ЭтоСтроки Тогда
				ИсточникПодчиненных = ФинансоваяОтчетностьКлиентСервер.ПодчиненныйЭлемент(
					Таблица,
					"ВидЭлемента",
			...
			ПодчиненныеИсточникиЗначенийЭлементаПростойТаблицы(
				ФинансоваяОтчетностьКлиентСервер.ПодчиненныеСтроки(ИсточникПодчиненных),
				ТаблицаИсточников,
				ВидыЭлементовЗначений);
		КонецЕсли;
	...
	Иначе
		...
		НастройкаЯчеек = ФинансоваяОтчетностьКлиентСервер.ПодчиненныйЭлемент(
			Таблица,
			"ВидЭлемента",
	...
	Для Каждого Элемент Из ТаблицаИсточников Цикл
		...
		Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НефинансовыйПоказатель Тогда
			...
			СписокРеквизитов = "ВидАналитики1, ВидАналитики2, ВидАналитики3, ВидАналитики4, ВидАналитики5, ВидАналитики6,
	...
	Результат = ТаблицаИсточников.СкопироватьКолонки();
...
Функция НайтиОтборПоИмени(ИсточникОтбор, ИмяЭлемента, НайденныйОтбор = Неопределено, СУчетомВложенных = Ложь) Экспорт
...
//
// Параметры:
// 	Источник - Структура - Источник данных вида бюджета.
		...
		Если ЗначенияОтбора.Количество() = 1 Тогда
			Для Каждого ЗначениеОтбора Из ЗначенияОтбора Цикл
				...
				Если ЗначениеОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
					...
					И НРег(ЗначениеОтбора.ПравоеЗначение) = "<заполнить_сценарий>"
...
Функция ВыражениеПериодаГруппировкиИсточникаДанных(ЭлементОтчета, СтруктураКолонок, КэшРеквизитов)
	...
	Если СтруктураКолонок.Периодичности.Количество() Тогда
		...
		Если МинимальнаяПериодичность = Перечисления.Периодичность.Неделя Тогда
			...
			ТекстПоля = "ВЫБОР КОГДА НАЧАЛОПЕРИОДА(" + ТекстПоля + ", " + МинимальнаяПериодичность + ") < &НачалоПериода
						|ТОГДА &НачалоПериода ИНАЧЕ НАЧАЛОПЕРИОДА(" + ТекстПоля + ", " + МинимальнаяПериодичность + ") КОНЕЦ";
			
		Иначе
			
			ТекстПоля = "НАЧАЛОПЕРИОДА(" + ТекстПоля + ", " + МинимальнаяПериодичность + ")";
...
// Заполняет источники данных по виду элемента
//
// Параметры:
// 	Источники - Массив - Массив описаний источников данных.
// 	СтруктураКэша - Структура закэшированных данных.
...
// 	ВидЭлемента     - ПеречислениеСсылка.ВидыЭлементовФинансовогоОтчета - Вид элемента, по которому заполняются источники.
// 	ПараметрыЗаполненияИсточников - Структура - Параметры заполнения источников.
// 	 *РекурсивноеПолучениеИсточниковПоСтатьям - Булево - См. описание ПолучитьОписаниеИсточниковДанных
// 	 *ПроверятьНеобходимостьФактаПоСтруктуреОтчета - Булево - Для вида бюджета без возможности ввода плана можно указать сценарий в измерениях,
// 	                                                 тогда, необходимость получения фактических данных по источнику может быть ограниченно измерением.
// 	                                                 Т.е. если указан вывод по плановому сценарию, то получать фактические данные не нужно.
// 	                                                 Если вид бюджета не для ввода плана и в структуре есть измерение "Сценарий", то Истина - выполняется дополнительная проверка.
...
Процедура ЗаполнитьИсточникиПоВидуЭлемента(Источники, СтруктураКэша, ОписаниеСтруктурыЭлементовБюджета, ВидЭлемента, ПараметрыЗаполненияИсточников)
	...
	ТаблицаНастроек        = ОписаниеСтруктурыЭлементовБюджета.ТаблицаНастроек;
	...
	Для Каждого ОтобраннаяСтрока Из ОтобранныеСтроки Цикл
		...
		НайденныйИсточник = НайтиИсточник(СтруктураКэша, Источники, ОтобраннаяСтрока, СтруктураКолонок, ДополнительныеПараметрыИсточника);
...
Процедура ДополнитьПризнакиЗаданияСценариевСтруктуройБюджета(ПризнакиЗаданияИсточника, ОтобраннаяСтрока, ОписаниеСтруктурыЭлементовБюджета, СтруктураКэша)
	...
	Если ПродолжатьАнализ Тогда
		...
		Если КорневойЭлемент <> Неопределено Тогда
			...
			Если НЕ ПараметрыЗаданияСценариев.ЕстьСценарийДляИсточника
			   И НЕ КолонкиПоОтношениюКПоказателю = Неопределено Тогда
				...
				Иначе
					...
					СвойстваКолонокПростойТаблицы = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ПараметрыЗаданияСценариев);
			...
			Если НЕ ПараметрыЗаданияСценариев.ЕстьСценарийДляИсточника Тогда
				// Сценарии быть заданы или только в колонках или только в строках.
				// Если сценарии указаны вместе с показателем, то они могут быть или только выше, или только ниже текущего показателя.
				ЭлементыДереваСодержатСценарии(ОтобраннаяСтрока, ПараметрыЗаданияСценариев, "Вниз", СтруктураКэша);
			КонецЕсли;
	...
	Если ПродолжатьАнализ Тогда
		Если НЕ ТаблицаНастроек = Неопределено Тогда
			...
			КорневойЭлемент = ФинансоваяОтчетностьКлиентСервер.КорневойЭлемент(ОтобраннаяСтрока, Перечисления.ВидыЭлементовФинансовогоОтчета.ТаблицаСложная);
					...
					Если РазрешитьРедактирование = Истина Тогда
						...
						Если РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Заполнение
							...
							Если НЕ РежимФормирования = Перечисления.РежимыФормированияБюджетныхОтчетов.Заполнение 
								...
								КонецЦикла;
							Иначе
								ТипыАналитики = ВидАналитики.ТипЗначения.Типы();
								МассивИсточников.Добавить(Новый Структура("Источник, Объект", "ВсеЭлементы", Неопределено));
							КонецЕсли;
						...
						Иначе
							...
							ИсточникВсеЭлементы = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(Элемент, "ИсточникВсеЭлементы");
							Если ИсточникВсеЭлементы = Неопределено
							 ИЛИ НЕ ИсточникВсеЭлементы Тогда
								ИсточникиЗначений = КэшИсточникиЗначений.НайтиСтроки(
									Новый Структура("ЭлементОтчета, ДобавляемыеЗначенияДокумента", Элемент, Истина));
...
Функция ЗаполнитьЯчейкиОперандовФормулы(Параметры, НоваяСтрока, ИсточникиДанных, ИсточникиПериодовГруппировок, Операнды)
	...
	Для Каждого ДанныеОперанда Из Операнды Цикл
		...
		Для Каждого СтрокаОперанда Из МассивСтрокОперандов Цикл
			...
			ВидЭлемента = ДанныеОперанда.Операнд.ВидЭлемента;
...
Процедура ЗаполнитьСведенияОбИсточникеДанных(НоваяСтрока, ИсточникиДанных, ИсточникиПериодовГруппировок, Элемент, 
	...
	ИначеЕсли Не ЗначениеЗаполнено(Элемент) Тогда // расчет по подчиненным строкам
		...
		Для Каждого Строка Из ПодчиненныеСтроки Цикл
			...
			СтрокаОперанда = НоваяСтрока.Владелец().Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаОперанда, НоваяСтрока, "Строка, Колонка");
			СтрокаОперанда.ИмяОперанда = НСтр("ru = 'Вложенная строка'") + " #" + ПорядковыйНомерСтроки;
			СтрокаОперанда.ВидОперанда = Перечисления.ВидыЭлементовФинансовогоОтчета.ЯчейкаТаблицы;
			СтрокаОперанда.ОсновнойЭлемент = Ложь;
			СтрокаОперанда.ОперандСтрока = Строка.Строка;
			СтрокаОперанда.ОперандКолонка = Строка.Колонка;
...
Процедура ДобавитьПоляТаблицыДанныхПоОписанию(Набор,
	...
	Для Каждого КлючИЗначение Из ПоляРесурсов Цикл
		...
		ИначеЕсли КлючИЗначение.Ключ = "ЗначениеПоказателя" Тогда
			...
			МассивТипов.Добавить(Тип("Строка"));
			МассивТипов.Добавить(Тип("Булево"));
			ПолеНабораДанных(Набор, КлючИЗначение.Ключ, КлючИЗначение.Ключ, , Новый ОписаниеТипов(МассивТипов, Новый КвалификаторыЧисла(15, 2), Новый КвалификаторыСтроки(150)));
		Иначе
...
Процедура ДобавитьПодчиненныйНаборПоБюджетномуРегистру(ПараметрыПриемника,
	...
	Источник                      = ПараметрыИсточника.Источник;
	...
	Если ДляПоказателя Тогда
		ПоляНСИПоказателяДляОтбора = Новый Массив;
		ПоляНСИПоказателяДляОтбора.Добавить("НСИ.КоличествоИспользуемыхАналитик");
		...
		Для Сч = 1 По МаксимальноеКоличествоАналитик Цикл
			...
			ТекстЗапросаНСИ = ТекстЗапросаНСИ + СтрШаблон(ШаблонВставки, Сч);
			
			ПоляНСИПоказателяДляОтбора.Добавить("НСИ.ВидАналитики" + Сч);
		КонецЦикла;
		
		ТекстЗапросаНСИ = ТекстЗапросаНСИ + "
		...
		Если Источник.Объекты <> Неопределено Тогда
			...
			ТекстЗапросаНСИ = ТекстЗапросаНСИ + "
			|ГДЕ
			|	НСИ.СвязанныйПоказательБюджетов в (&" + ИмяПараметра + ")";
	...
	Если ДляПоказателя Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	" + СтрСоединить(ПоляНСИПоказателяДляОтбора, "," + Символы.ПС) + ",
		|";
	КонецЕсли;
	...
	Если ХранимыеФактическиДанные Тогда
		...
		Если СтруктураКолонокТаблицыДанных.Периодичности.Количество() Тогда
			ПериодичностьТаблицы = БюджетнаяОтчетностьКлиентСервер.МинимальнаяПериодичность(СтруктураКолонокТаблицыДанных.Периодичности);
		КонецЕсли;
		ТекстЗапроса = ТекстЗапроса + "
	...
	Если Источник.Объекты <> Неопределено Тогда
		...
		Если ДляПоказателя Тогда
			ТекстЗапроса = ТекстЗапроса + ?(Первый, " ", " И ") + "СтатьяБюджетов В
			|	(ВЫБРАТЬ
			|		ВлияющиеСтатьи.СтатьяБюджетов
			|	ИЗ
			|		ВлияющиеСтатьи)
	...
	Если Не ДляПоказателя Тогда
		Для Каждого Аналитика Из Аналитики Цикл
			...
			Если НайтиОтборПоИмени(Источник.ДополнительныйОтбор, ИмяПараметра,, Истина) Тогда
				...
				ТекстЗапроса = ТекстЗапроса + ?(Первый, "{", ",") + "
				|	" + ТекстПоля + " КАК " + ИмяПараметра + "_" + Источник.Идентификатор;
				Первый = Ложь;
	...
	|//{ПоляКомпоновки}
	|ИЗ (
	| " + ТекстЗапроса + ") КАК ПредварительныйРасчет
	|";
		...
		Для Каждого Измерение Из СтруктураКолонокТаблицыДанных.ВидыАналитик Цикл
			...
			Если Источник.Свойство(ИмяАналитики, ОтборПоИзмерениям)
			   И ЗначениеЗаполнено(ОтборПоИзмерениям) Тогда
				...
				ТекстЗапроса = ТекстЗапроса + ?(Первый, "{ГДЕ", ",") + "
				|	" + ТекстАналитики + " В (&" + ИмяПараметра + ")
				|";
				Первый = Ложь;
	...
	Если РежимПолученияДанных = Перечисления.РежимПолученияДанныхБюджетирования.Разрешенные Тогда
...
Функция ПолучитьСКДИтоговыеДанныеОбороты(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, ЕстьХранимыеПравила, ЕстьПривилегированныйРежим)
	...
	Если ЕстьПривилегированныйРежим Тогда
		ПлановыеДанные = ФинОтчеты.НовыйНабор(БазовыйОбъединенныйНабор, Тип("НаборДанныхОбъектСхемыКомпоновкиДанных"), "ПлановыеДанныеПривилегированно");
		ПоляРесурсов = Новый Структура("СуммаПриход, СуммаРасход,
										|СуммаВалПриход, СуммаВалРасход,
										|КоличествоПриход, КоличествоРасход");
		УдалитьИзСтруктурыНеиспользуемыеКлючи(ПоляРесурсов, ПоляИсточников);
	...
	Если ЕстьХранимыеПравила Тогда
		...
		Если ЕстьПривилегированныйРежим Тогда 
			ХранимыеФактическиеДанные = ФинОтчеты.НовыйНабор(БазовыйОбъединенныйНабор, Тип("НаборДанныхОбъектСхемыКомпоновкиДанных"), "ХранимыеФактическиеДанныеПривилегированно");
			ПоляРесурсов = Новый Структура("СуммаПриход, СуммаРасход,
											|СуммаВалПриход, СуммаВалРасход,
											|КоличествоПриход, КоличествоРасход");
			УдалитьИзСтруктурыНеиспользуемыеКлючи(ПоляРесурсов, ПоляИсточников);
...
Функция ПолучитьСКДДокумента(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, 
	...
	Для Каждого Источник Из ИсточникиДанных Цикл
		...
		Если Имя = "ДанныеДокумента_Документ" Тогда
			...
			ПодчиненныйНабор = СКДИтоговыеДанные.НаборыДанных[0].Элементы.Найти(Имя);
		КонецЕсли;
...
Функция РасчетКэшаПолучениеСКДИсточников(ВидБюджета, СтруктураОписанияОтчета, СтруктураКолонокТаблицыДанных, РежимФормирования, ВидЭлементаКэша, Постфикс)
	...
	СКДПлановыеДанные = ПолучитьСКДПлановыеДанные(ИсточникиДанных, ПоляИсточников,
													...
													ПараметрыКУстановке, РежимыФормирования);
													
	Если ЕстьПривилегированныйРежим Тогда
		
		РежимыФормирования.Вставить("ПривилегированныйРежим", Истина); 
		СКДПлановыеДанныеПривилегированно = ПолучитьСКДПлановыеДанные(ИсточникиДанных, ПоляИсточников,
														СтруктураКолонокТаблицыДанных, Аналитики, 
														ПараметрыКУстановке, РежимыФормирования);
	КонецЕсли;
	...
	Если ЕстьХранимыеПравила Тогда
		...
		Если ЕстьПривилегированныйРежим Тогда																
			РежимыФормирования.Вставить("ПривилегированныйРежим", Истина); 
			СКДФактическиеДанныеПривилегированно = ПолучитьСКДФактическиеДанные(ИсточникиДанных, ПоляИсточников,
																СтруктураКолонокТаблицыДанных, Аналитики, 
																ПараметрыКУстановке, РежимыФормирования);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЕстьОстатки Тогда
		СКДПредварительныйРасчет = ПолучитьСКДПредварительныйИтоговыйРасчет(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, 
																			ЕстьХранимыеПравила, ЕстьПривилегированныйРежим);
	...
	Поле = ФинансоваяОтчетностьСервер.НовоеВычисляемоеПоле(СКДИтоговыеДанные, "РедактируемоеЗначение", 
					"ВЫБОР КОГДА НЕ ЗначениеЗаполнено(РедактируемоеЗначение) ТОГДА """" ИНАЧЕ РедактируемоеЗначение КОНЕЦ");
			...
			ИначеЕсли ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.ФиксированнаяАналитика Тогда
				...
				Иначе
					...
					ЗначениеФильтра    = БюджетированиеКлиентСервер.ПриведенноеЗначениеАналитики(
						Элемент.ЭлементОтчета.ЗначениеАналитики, ТипЗначенияФильтра);
				КонецЕсли;
...
Функция РасчетКэшаПолучениеДереваДляВыводаОтчета(ВидБюджета, СтруктураЯчеек, ПараметрыЯчеек, ВидЭлементаКэша)
...
Функция ПараметрыЭлементаСпискаВвода(СведенияОБюджетеДляРасчета, ЭлементОтчета, ВышележащиеКолонки)
	...
	Если ЭлементОтчета.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
		Заголовок = НСтр("ru = 'Статья/показатель бюджетов'");
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(Тип("СправочникСсылка.СтатьиБюджетов"));
		МассивТипов.Добавить(Тип("СправочникСсылка.ПоказателиБюджетов"));
	...
	Если ЭлементОтчета.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.Измерение Тогда
		...
		Если ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Период Тогда
			...
			ИначеЕсли Периодичность = Перечисления.Периодичность.Полугодие Тогда
				...
				ПериодичностьТекст = НСтр("ru = 'Полугодие с'");
			ИначеЕсли Периодичность = Перечисления.Периодичность.Квартал Тогда
				Формат = "ДФ='к ''%1'''";
				ПериодичностьТекст = НСтр("ru = 'квартал'");
			ИначеЕсли Периодичность = Перечисления.Периодичность.Месяц Тогда
				Формат = "ДФ='ММММ гггг'";
			ИначеЕсли Периодичность = Перечисления.Периодичность.Декада Тогда
				Формат = "ДФ='''%1'' дд.ММ.гг'";
				ПериодичностьТекст = НСтр("ru = 'Декада с'");
			ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда
				Формат = "ДФ='''%1'' дд.ММ.гг'";
				ПериодичностьТекст = НСтр("ru = 'Неделя с'");
			...
			Возврат Новый Структура("Заголовок, ТипЗначения, Формат", Заголовок, ТипЗначения, Формат);
			
		ИначеЕсли ТипИзмерения = Перечисления.ТипыИзмеренийФинансовогоОтчета.Валюта Тогда
			
			ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.Валюты");
			Возврат Новый Структура("Заголовок, ТипЗначения", "Валюта", ТипЗначения);
		...
		Если ЗначениеЗаполнено(ЭлементОтчета.ФиксированноеЗначение)
			...
			Если ЗначениеЗаполнено(ЭлементОтчета.ВидАналитики) Тогда
				ЗаписываемоеЗначение = БюджетированиеКлиентСервер.ПриведенноеЗначениеАналитики(
					ЗаписываемоеЗначение, ЭлементОтчета.ВидАналитики.ТипЗначения);
			КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Выборка Цикл
			...
			ИначеЕсли СтрокаТаблицы["Флаг" + Измерение] = "прочие" Тогда
				Если СтруктураКолонок.Измерения.Найти(Измерение) <> Неопределено Тогда
					МассивВыбираемыхЗначений.Добавить(БюджетированиеКлиентСервер.ПустоеЗначениеАналитики(),
					                                  "<прочие>");
				Иначе
...
Процедура ПодчиненныеИсточникиЗначенийЭлементаСложнойТаблицы(ПодчиненныеЭлементы, СписокЭлементов)
	...
	Для Каждого СтрокаДерева Из ПодчиненныеЭлементы Цикл
		...
		ПодчиненныеИсточникиЗначенийЭлементаСложнойТаблицы(
			ФинансоваяОтчетностьКлиентСервер.ПодчиненныеСтроки(СтрокаДерева), СписокЭлементов);
...
Процедура ПодчиненныеИсточникиЗначенийЭлементаПростойТаблицы(ПодчиненныеЭлементы, СписокЭлементов, ВидыЭлементовЗначений)
	...
	Для Каждого СтрокаДерева Из ПодчиненныеЭлементы Цикл
		...
		ПодчиненныеИсточникиЗначенийЭлементаПростойТаблицы(
			ФинансоваяОтчетностьКлиентСервер.ПодчиненныеСтроки(СтрокаДерева), СписокЭлементов, ВидыЭлементовЗначений);
...
//
// Возвращаемое значение:
//	Произвольное - Значение реквизита элемента вида бюджета.
//
