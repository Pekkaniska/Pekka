...
#Область УстановитьСвойстваФормыПоПараметрам
	...
	#Область УстановитьСвойстваФормыПодбораПоНалогообложениюНДС
		Если Параметры.Свойство("НалогообложениеНДС") Тогда
			Ограничения = УчетНДСУПКлиентСервер.ОграничениеТоваровПоНалогообложению(Параметры.НалогообложениеНДС);
...
//
// Параметры:
//	Форма - УправляемаяФорма - форма подбора, форма помощника продаж.
//
...
//Рарус Владимир Подрезов 18.04.2017	
	Если Форма.ИмяФормы = "Обработка.ПодборТоваровВДокументПродажи.Форма.Форма" Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			...
			"ВНаличииВБазовыхЕдиницах", 0, ВидСравненияКомпоновкиДанных.Больше,,Форма.ТолькоВНаличии или Форма.ТолькоВНаличииПоЯчейкеПользователя);
			//-- Рарус IA
	Иначе
...
//Рарус Владимир Подрезов Конец
//++ Рарус IA
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.СписокНоменклатура, "ВключитьОтборВЯчейкеСвободныеОстаткиОстатки", Форма.ТолькоВНаличииПоЯчейкеПользователя);
	ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.СписокНоменклатура, "ЯчейкаОтбора", Форма.пкСкладскаяЯчейка);
//-- Рарус IA
//Рарус Владимир Подрезов 18.04.2017	
	КонецЕсли;
//Рарус Владимир Подрезов Конец
...
//Рарус Владимир Подрезов Конец
		//++ Рарус IA
		ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.СписокХарактеристики, "ВключитьОтборВЯчейкеСвободныеОстаткиОстатки", Форма.ТолькоВНаличииПоЯчейкеПользователя);
		ПодборТоваровКлиентСервер.УстановитьПараметрДинамическогоСписка(Форма.СписокХарактеристики, "ЯчейкаОтбора", Форма.пкСкладскаяЯчейка);
		//-- Рарус IA
//Рарус Владимир Подрезов 18.04.2017	
		КонецЕсли;
//Рарус Владимир Подрезов Конец
...
//++ Лимаренко К.А. 18.06.18
		Если не Форма.ИмяФормы  = "Обработка.пкАРМСклад.Форма.ФормаСписка" Тогда
...
//++ Рарус IA 2017.03.15
							...
							|	ВЫБОР
							|		КОГДА &ВключитьОтборВЯчейкеСвободныеОстаткиОстатки = ИСТИНА
							...
							|		КОГДА &ВключитьОтборВЯчейкеСвободныеОстаткиОстатки = ИСТИНА
							|			ТОГДА 0
							|		ИНАЧЕ СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток
							|	КОНЕЦ КАК ВРезервеСоСкладаОстаток,
							|	ВЫБОР
							|		КОГДА &ВключитьОтборВЯчейкеСвободныеОстаткиОстатки = ИСТИНА
							|			ТОГДА 0
							|		ИНАЧЕ СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток
							|	КОНЕЦ КАК ВРезервеПодЗаказОстаток
							|ИЗ
							|	РегистрНакопления.СвободныеОстатки.Остатки(,{УсловиеОтбораПоСкладу}{(Номенклатура).* КАК Номенклатура}) КАК СвободныеОстаткиОстатки
							|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыВЯчейках.Остатки(, Ячейка = &ЯчейкаОтбора) КАК ТоварыВЯчейкахОстатки
							|		ПО СвободныеОстаткиОстатки.Номенклатура = ТоварыВЯчейкахОстатки.Номенклатура
							//|			И СвободныеОстаткиОстатки.Характеристика = ТоварыВЯчейкахОстатки.Характеристика
							//|			И СвободныеОстаткиОстатки.Склад = ТоварыВЯчейкахОстатки.Ячейка.Владелец
							|			) КАК СвободныеОстатки
							|ПО (СвободныеОстатки.Номенклатура = СправочникНоменклатура.Ссылка)
...
//++ Рарус IA 2017.03.15
							...
							|	КОНЕЦ КАК ВРезервеПодЗаказОстаток
							|ИЗ
							|	РегистрНакопления.СвободныеОстатки.Остатки(,{УсловиеОтбораПоСкладу} И Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка) {(Номенклатура).* КАК Номенклатура}) КАК СвободныеОстаткиОстатки
							|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыВЯчейках.Остатки(, Ячейка = &ЯчейкаОтбора) КАК ТоварыВЯчейкахОстатки
							|		ПО СвободныеОстаткиОстатки.Номенклатура = ТоварыВЯчейкахОстатки.Номенклатура
							|			И СвободныеОстаткиОстатки.Характеристика = ТоварыВЯчейкахОстатки.Характеристика
							//|			И СвободныеОстаткиОстатки.Склад = ТоварыВЯчейкахОстатки.Ячейка.Владелец
							|			) КАК СвободныеОстатки
							|ПО (СвободныеОстатки.Номенклатура = СправочникНоменклатура.Ссылка) И (СвободныеОстатки.Характеристика = ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка))
							//|	И (СвободныеОстатки.Склад = &Склад)
							|";
//-- Рарус IA 2017.03.15
						КонецЕсли;
...
//++ Рарус IA 2017.03.15
							...
							|		ПО СвободныеОстаткиОстатки.Номенклатура = ТоварыВЯчейкахОстатки.Номенклатура
							//|			И СвободныеОстаткиОстатки.Характеристика = ТоварыВЯчейкахОстатки.Характеристика
							|			И СвободныеОстаткиОстатки.Склад = ТоварыВЯчейкахОстатки.Ячейка.Владелец) КАК СвободныеОстатки
							|ПО (СвободныеОстатки.Номенклатура = СправочникНоменклатура.Ссылка)
							|	И (СвободныеОстатки.Склад = &Склад)
							|";
//-- Рарус IA 2017.03.15
...
//++ Рарус IA 2017.03.15
						...
						|	КОНЕЦ КАК ВРезервеПодЗаказОстаток
						|ИЗ
						|	РегистрНакопления.СвободныеОстатки.Остатки(,{УсловиеОтбораПоСкладу} И Номенклатура = &Номенклатура) КАК СвободныеОстаткиОстатки
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыВЯчейках.Остатки(, Ячейка = &ЯчейкаОтбора) КАК ТоварыВЯчейкахОстатки
						|		ПО СвободныеОстаткиОстатки.Номенклатура = ТоварыВЯчейкахОстатки.Номенклатура
						|			И СвободныеОстаткиОстатки.Характеристика = ТоварыВЯчейкахОстатки.Характеристика
						//|			И СвободныеОстаткиОстатки.Склад = ТоварыВЯчейкахОстатки.Ячейка.Владелец
...
//++ Рарус IA
Процедура УстановитьОтборТолькоВНаличииПоЯчейкеПользователя(Форма) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ГруппаОтбораТолькоВНаличииНоменклатура,
		"ВНаличииОстаток", 0, ВидСравненияКомпоновкиДанных.Больше,,Форма.ТолькоВНаличииПоЯчейкеПользователя или Форма.ТолькоВНаличии);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
		ГруппаОтбораТолькоВНаличииХарактеристики,
		"ВНаличииОстаток", 0, ВидСравненияКомпоновкиДанных.Больше,,Форма.ТолькоВНаличииПоЯчейкеПользователя);
