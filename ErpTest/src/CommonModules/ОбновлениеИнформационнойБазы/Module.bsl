...
//Рарус Владимир Подрезов 06.07.2017
	Попытка
...
//Рарус Владимир Подрезов 06.07.2017
	Исключение
		ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось получить информацию по объекту ""%1"" по причине: %2'"),
				Данные,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Информация по данным'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
//Рарус Владимир Подрезов
		Если ТипЗнч(Объект) = Тип("ДокументОбъект.ЗаказКлиента") Тогда
			Объект.ОбменДанными.Загрузка = Объект.пкЗакрыт;
		КонецЕсли;
//Рарус Владимир Подрезов Конец
...
//                    Если данных для обновления нет, то возвращается пустая таблица значения без колонок.
//
...
Функция ИмяСоответствуетТребованиямИменованияСвойств(Имя)
	...
	Буквы = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ_";
	Цифры = "1234567890";
	
	Если Имя = "" Или СтрНайти(Буквы, ВРег(Лев(Имя, 1))) = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат СтрРазделить(ВРег(Имя), Буквы + Цифры, Ложь).Количество() = 0;
...
Процедура УстановитьПорядокПоСтраницам(Запрос, ПараметрыПостроения)
	...
	Направления = ПараметрыПостроения.Направления;
	ДополнительныеПараметры = ПараметрыПостроения.ДополнительныеПараметры;
