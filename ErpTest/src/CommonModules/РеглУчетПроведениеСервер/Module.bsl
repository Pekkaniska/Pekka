...
Процедура ОтразитьВсеРегламент() Экспорт
	...
	УчетНДСУП.ВыполнитьЗаданияПоФормированиюКнигиПокупокПродаж(КонецМесяца(ТекущаяДата));
...
Процедура ОтразитьВсеВФоне(Параметры, АдресХранилища) Экспорт
	...
	УчетНДСУП.ВыполнитьЗаданияПоФормированиюКнигиПокупокПродаж(КонецМесяца(Параметры.ПериодРасчета), Параметры.Организация);
	ОтразитьВсе(
...
//-- Рарус Лимаренко 25.12.2018
	...
	//++ Рарус Лимаренко 25.12.2018
	//|	НовыеСтатусы.Комментарий КАК Комментарий
	|	ВЫРАЗИТЬ(НовыеСтатусы.Комментарий КАК СТРОКА(200)) КАК Комментарий
	//-- Рарус Лимаренко 25.12.2018
	|ИЗ
	|	НовыеСтатусы КАК НовыеСтатусы
	|
...
Процедура ВыполнитьОффлайновыеРасчеты(Документы)
	...
	ВнеоборотныеАктивы.ОтложенноеФормированиеДвиженийПриФормированииПроводок(МассивСсылок);
	ВнеоборотныеАктивы.РассчитатьСтоимостьВнеоборотныхАктивовПриФормированииПроводок(МассивСсылок);
...
//++ Рарус Лимаренко 19.12.2018
	Если ИмяДокумента = "РасчетКурсовыхРазниц" Тогда
		ЗапросХозрасчетный.Текст = СтрЗаменить(ЗапросХозрасчетный.Текст,"ОшибкиПроверки.Ссылка ЕСТЬ NULL","ОшибкиПроверки.Ссылка ЕСТЬ NULL И НЕ ХЗ.СчетДт.ИсключитьСуммуБУИзПереоценкиПоПлануСчетов И НЕ ХЗ.СчетКт.ИсключитьСуммуБУИзПереоценкиПоПлануСчетов")		
	КонецЕсли;
//-- Рарус Лимаренко 19.12.2018
...
Функция СформироватьТаблицыОтбораДанных(ТипДокумента, ВременныеТаблицы, Организация = Неопределено, ПериодОтражения = Неопределено, МассивСсылок = Неопределено, МассивИсключаемыхСсылок = Неопределено)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Данные.Организация = &Организация", "ИСТИНА");
	Иначе
		Запрос.УстановитьПараметр("Организация", Организация);
	КонецЕсли;
