...
Процедура ЗаполнитьРегламентированныйОтчет(Контейнер, Расшифровка, ИмяРедакции, КонтекстРасчета) Экспорт
	...
	СхемаРасчета        = СхемаРасчета(КонтекстРасчета, ОписаниеСуммНалоговыйУчет());
	...
	Если НалоговыйУчетОбособленныхПодразделений.УчетВРазрезеНалоговыхОрганов() Тогда
		...
		Если Не ЗначениеЗаполнено(КонтекстРасчета.КППОбособленногоПодразделения) Тогда
			НалоговаяБаза = 0; // При заполнении по организации в целом, расчет базы можно выполнить позже - механизмами БРО
...
Функция СхемаРасчета(КонтекстРасчета, ОписаниеСумм)
	...
	// Подп. 8 п. 1 ст. 265 - см. также Расходы.Внереализационные.Затраты.ЛиквидацияОсновныхСредств.
	...
	ДобавитьРасчет(СхемаРасчета, "Расходы.Внереализационные.Затраты.ЛиквидацияОсновныхСредств");
...
Процедура СоздатьВременнуюТаблицуСтатьиПрочихДоходовРасходов(ВидыДоходовРасходов, ПроцессорПолученияДанных)
	...
	|	СтатьиПрочихРасходов.ВидПрочихРасходов КАК ВидСтатьи,
	...
	|ГДЕ
	|	СтатьиПрочихРасходов.ВидПрочихРасходов В(&ВидыДоходовРасходов)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СтатьиПрочихДоходов.Ссылка,
	|	СтатьиПрочихДоходов.ВидПрочихДоходов,
	...
	|ГДЕ
	|	СтатьиПрочихДоходов.ВидПрочихДоходов В(&ВидыДоходовРасходов)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДобавитьОтборПрочихДоходовРасходовВНастройкиОтчета(Настройки, ИмяИлиОписаниеПоля, ЭтапРасчета, ВидСтатьи, Счет = Неопределено)
	...
	Если Счет <> Неопределено И Счет.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ПрочиеДоходы) Тогда
		ДобавитьОтборВНастройкиОтчета(Настройки, ИмяПоля + "." + "ВидПрочихДоходов", ВидСтатьи);
	Иначе
		ДобавитьОтборВНастройкиОтчета(Настройки, ИмяПоля + "." + "ВидПрочихРасходов", ВидСтатьи);
	КонецЕсли;
...
Функция НовыйРасчетРаспределенияНалога()
	...
	// Ограничение разрядности определено в формате представления в электронном виде - Приказ ФНС от 26 ноября 2014 г. N ММВ-7-3/600@.
	...
	Расчет.Колонки.Добавить("СтавкаНалогаРегиональныйБюджет",   ОбщегоНазначения.ОписаниеТипаЧисло( 4,  2, ДопустимыйЗнак.Неотрицательный));
...
Функция СхемаРасчетаНалоговойБазы(КонтекстРасчета)
	...
	СхемаРасчета = НовыйСхемаРасчета(КонтекстРасчета, ОписаниеСуммНалоговыйУчет());
	...
	Для Каждого Показатель Из ОписаниеФормыОтчета Цикл
		...
		Для Каждого ЭлементРасшифровкиПоказателя Из РасшифровкаПоказателя Цикл
			...
			Если ЗнакОперации = "-" Тогда
				// В форму РасшифровкаПоказателейРегламентированныхОтчетов суммы передаются так, чтобы их сложение дало значение
				// расшифровываемого показателя.
				Сумма = - Сумма;
			КонецЕсли;
...
Процедура ВключитьСтраницу(СтраницаБРО, Показатель, ИмяРедакции)
	...
	// 1. С технической точки зрения, оно показывает, используется ли конкретная страница - будет ли она представлена в
	// составе декларации. Поэтому, если есть данные для заполнения на этой странице, то нужно убедиться, что поле также заполнено.
