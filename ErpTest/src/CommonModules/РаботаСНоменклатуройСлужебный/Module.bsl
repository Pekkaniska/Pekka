...
Функция ОписаниеПараметровЗапросаНоменклатура() Экспорт
	...
	Для каждого ЭлементКоллекции Из СоответствиеИменМерныхХарактеристик() Цикл
		МерныеХарактеристики.Вставить(ЭлементКоллекции.Ключ, 0);
	КонецЦикла;
	...
	Если Не ПоискЗаполнен Тогда
		...
		Для каждого ЭлементКоллекции Из ПараметрыЗапроса.МерныеХарактеристики Цикл
			...
			Если ЗначениеЗаполнено(ЗначениеМернойХарактеристики) Тогда
				...
				МернаяХарактеристика.min = ЭлементКоллекции.Значение;
				МернаяХарактеристика.max = ЭлементКоллекции.Значение;
...
//
Процедура ОбновитьВидНоменклатуры(ВидНоменклатуры, ИдентификаторыКатегории) Экспорт
	
	РазобратьИзмененияВидовНоменклатуры(ИдентификаторыКатегории, Неопределено, ВидНоменклатуры, Истина);	
...
Процедура РазобратьИзмененияВидовНоменклатуры(ИдентификаторыКатегорий, 
	...
	Для каждого ТекущийВидНоменклатуры Из ЗначенияРеквизитовВидовНоменклатуры Цикл
		...
		РазобратьРеквизитыВидовНоменклатуры(СтрокаДанных, ТекущийВидНоменклатуры, РезультатАнализа, РежимОбновления);
...
Процедура РазобратьРеквизитыНоменклатуры(СтрокаДанныхПоНоменклатуре, СтрокаНоменклатурыИБ, ДополнительныеРеквизитыНоменклатуры, СоответствиеРеквизитовИЗначений, РезультатАнализа, РеквизитыДляОбновления, РежимОбновления = Ложь)
	...
	Если РаботаСНоменклатурой.ВедетсяУчетВидовНоменклатуры() Тогда
		...
		Если ВидыНоменклатурыИнформационнойБазы.Найти(СтрокаНоменклатурыИБ.ВидНоменклатуры) = Неопределено 
			...
			СтрокиПоВидамНоменклатуры = ТаблицаОтличий.НайтиСтроки(
				Новый Структура("ТекущееЗначение, НовоеЗначение, Пометка", СтрокаНоменклатурыИБ.ВидНоменклатуры, ВидыНоменклатурыИнформационнойБазы[0], Истина));
...
Процедура РазобратьРеквизитыВидовНоменклатуры(ДанныеПоКатегории, СтрокаВидаНоменклатуры, РезультатАнализа, РежимОбновления = Ложь)
	...
	Если Не ПривязаноНесколькоКатегорий Тогда
