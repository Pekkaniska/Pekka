...
Функция НачислитьНалогНаПрибыль(Движения, Знач Реквизиты, Знач ПараметрРаспределениеНалогаНаПрибыль) Экспорт
	...
	НачалоНалоговогоПериода = НалоговыйУчет.НачалоНалоговогоПериода(Реквизиты.Период, Реквизиты.Организация);
...
Процедура ДобавитьПроводкуСписанияОтклоненийПриОкругленииНалогаНаПрибыль(
	...
	Проводка.Период      = Реквизиты.Период;
	Проводка.Организация = Реквизиты.Организация;
	Проводка.Содержание  = НСтр("ru = 'Перенос сальдо счета 68.04.2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
...
//  РаспределениеНалогаНаПрибыль - см. НовыйРаспределениеНалогаНаПрибыль() - исходные данные для распределения налога
...
Функция НачислениеНалогаНаПрибыль(Период, Организация, НалоговаяБаза, СтавкаФедеральныйБюджет, СтавкаРегиональныйБюджет, РаспределениеНалогаНаПрибыль, ПрименяетсяПБУ18 = Истина)
	...
	Если НачалоМесяца(Период) > НачалоНалоговогоПериода Тогда
...
Процедура ДобавитьНачислениеНалогаНаПрибыль(НачислениеНалога, УровеньБюджета, РегистрацияВНалоговомОргане = Неопределено, СуммаНалога)
...
Процедура ДобавитьПроводкиУменьшениеНаСуммуТорговогоСбора(Движения, НачислениеНалога, Период, Организация, ПрименяетсяПБУ18)
	...
	Если СуммаПроводок > 0 Тогда // ранее был признан расход. Надо его сторнировать
		...
		СодержаниеПроводок = НСтр("ru = 'Сторнированы расходы в виде сумм уплаченного торгового сбора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
	Иначе // восстанавливаем ранее отсторнированный расход
		...
		СодержаниеПроводок = НСтр("ru = 'Восстановлены расходы в виде сумм уплаченного торгового сбора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
	...
	Для Каждого ЗаписьРасходы Из Расходы Цикл
		...
		НастройкиОтраженияТорговогоСбора = РегистрыСведений.ПорядокОтраженияРасходов.НастройкаОтраженияСтатьиРасходов(Организация, СтатьяТорговогоСбора, ЗаписьРасходы.Подразделение);
		// Деятельность, облагаемая торговым сбором, не облагается ЕНВД:
		СчетСписанияОСНО = НастройкиОтраженияТорговогоСбора.СчетСписанияОСНО;
	...
	Для Каждого ДанныеПроводки Из УменьшениеНалога Цикл
		...
		Если Проводка.Сумма > 0 Тогда
			Проводка.Содержание  = НСтр("ru = 'Налог на прибыль уменьшен на сумму торгового сбора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
		Иначе
			Проводка.Содержание  = НСтр("ru = 'Восстановлен налог на прибыль, ранее уменьшенный на сумму торгового сбора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
		КонецЕсли;
...
Функция ПодготовитьТаблицуРасчетНалогаНаПрибыль(ТаблицаРеквизиты, СписокОрганизаций, Отказ) Экспорт
	...
	Если Выборка.Следующий() Тогда
		Если Выборка.СуммаУсловногоНалога > 0 Тогда
			...
			Запись.ПоДебету           = "Д";
			Запись.СодержаниеПроводки = НСтр("ru = 'Условный расход по налогу на прибыль'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
		Иначе
			Запись = ТаблицаНалогаНаПрибыль.Добавить();
			Запись.СуммаНалога        = -Выборка.СуммаУсловногоНалога;
			Запись.СуммаЕНВД          = 0;
			...
			Запись.ПоДебету           = "К";
			Запись.СодержаниеПроводки = НСтр("ru = 'Условный доход по налогу на прибыль'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
		КонецЕсли;
	КонецЕсли;
...
Функция ПодготовитьТаблицуРасчетПостоянныхРазниц(ТаблицаРеквизиты, СписокОрганизаций, Отказ, КоличествоОбрабатываемыхДанных) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.СуммаДт > 0 Тогда
			...
			Запись.ПоДебету           = "Д";
			Запись.СодержаниеПроводки = НСтр("ru = 'Постоянное налоговое обязательство'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
			ЕстьПНО = Истина;
		КонецЕсли;
		...
		Если Выборка.СуммаДт < 0 Тогда
			...
			Запись.КоррСчет           = Выборка.КоррСчет;
			Запись.ПоДебету           = "К";
			Запись.СодержаниеПроводки = НСтр("ru = 'Постоянный налоговый актив'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
		КонецЕсли;
...
Процедура ОтражениеОНАиОНО(ТаблицаРасчетВременныхРазниц, ВидАктивовОбязательств, ВременныеРазницы, СчетаАктивовИОбязательств = Неопределено)
	...
	Если СуммаДт09 <> 0 Тогда
		...
		Запись.СодержаниеПроводки = НСтр("ru = 'Признание отложенного налогового актива'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
		Запись.ПоДебету           = "Д";
	КонецЕсли;
	...
	Если СуммаКт09 <> 0 Тогда
		...
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.ПоДебету           = "К";
		Запись.СодержаниеПроводки = НСтр("ru = 'Погашение отложенного налогового актива'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
	...
	Если СуммаКт77 <> 0 Тогда
		...
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.ПоДебету           = "К";
		Запись.СодержаниеПроводки = НСтр("ru = 'Признание отложенного налогового обязательства'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
	...
	Если СуммаДт77 <> 0 Тогда
		...
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.ПоДебету           = "Д";
		Запись.СодержаниеПроводки = НСтр("ru = 'Погашение отложенного налогового обязательства'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
