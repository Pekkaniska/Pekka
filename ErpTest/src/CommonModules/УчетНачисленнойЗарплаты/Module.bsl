...
//			Начисление - ссылка на план видов расчета.
//			Сумма
//		ПорядокВыплаты - ПеречислениеСсылка.ХарактерВыплатыЗарплаты
...
//
Процедура ЗарегистрироватьНачисленияУдержания(Движения, Отказ, Организация, ПериодРегистрации, Начисления, Удержания, БазаУдержаний, ПрочиеДоходы, ПорядокВыплаты = Неопределено, ЗаписыватьДвижения = Ложь, ОграничениеВзысканий = Ложь, ВыплатитьКакАванс = Ложь) Экспорт
	
	СтрокиНачислений = Новый Массив;
	УдержанияПоРабочимМестам = ТаблицаРаспределенияПоРабочимМестам();
	ДанныеМежрасчетногоПериода = ЭтоДанныеМежрасчетногоПериода(ПорядокВыплаты);
...
//		Допустимо присутствие других колонок в передаваемых таблицах значений.
//
Процедура ЗарегистрироватьОтработанноеВремя(Движения, Отказ, Организация, ПериодРегистрации, Начисления, ПорядокВыплаты = Неопределено, ЗаписыватьДвижения = Ложь) Экспорт
...
//		Допустимо присутствие других колонок в передаваемых таблицах значений.
//
Процедура ЗарегистрироватьНДФЛ(Движения, Отказ, Организация, ПериодРегистрации, РезультатыРасчетаНДФЛ, МВТ, ПорядокВыплаты = Неопределено, ЗаписыватьДвижения = Ложь, РаспределятьПоРабочимМестамБезусловно = Ложь, Знач ДанныеМежрасчетногоПериода = НеОпределено, Авансом = Ложь, ВыплатитьКакАванс = Ложь) Экспорт
	...
	Если Распределять Тогда
		...
		КонецЕсли;
		// Если выполняется регистрация результата авансового расчета, тогда дополняем базу расчета НДФЛ
		// данными о начислениях и помещаем их в ВТРегистрНакопления_СведенияОДоходахНДФЛ, необходимую УчетНДФЛ.БазаРасчетаНДФЛ 
		Если Авансом Тогда
			СформироватьНачисленияКакСведенияОДоходахНДФЛ(МВТ, Организация, ПериодРегистрации);
		КонецЕсли;
	...
	// Взаиморасчеты
	Если ЗначениеЗаполнено(ПорядокВыплаты) Или Авансом Или ВыплатитьКакАванс Тогда
		УдержанияДляВзаиморасчетов = ВзаиморасчетыССотрудниками.НоваяТаблицаНачисленнойЗарплаты();
...
//			если Истина - наборы записей будут записаны после заполнения.
//
Процедура ЗарегистрироватьКорректировкиВыплаты(Движения, Отказ, Организация, ПериодРегистрации, КорректировкиВыплаты, МВТ, ПорядокВыплаты = Неопределено, ЗаписыватьДвижения = Ложь, РаспределятьПоРабочимМестамБезусловно = Ложь) Экспорт
...
//
Процедура ЗарегистрироватьВозвратНДФЛ(Движения, Отказ, Организация, ПериодРегистрации, РезультатыРасчетаНДФЛ, ПорядокВыплаты = Неопределено, ЗаписыватьДвижения = Ложь) Экспорт
...
Функция ЭтоДанныеМежрасчетногоПериода(ПорядокВыплаты)
	Если ПорядокВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Зарплата Или ПорядокВыплаты = Неопределено Тогда
		Возврат Ложь
	Иначе
		Возврат Истина
	КонецЕсли	
...
Процедура СформироватьНачисленияКакСведенияОДоходахНДФЛ(МВТ, Организация, ПериодРегистрации)
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ПериодРегистрации", ПериодРегистрации);
	Запрос.УстановитьПараметр("КодДохода", КодДоходаПоУмолчанию);
	Запрос.Выполнить();
