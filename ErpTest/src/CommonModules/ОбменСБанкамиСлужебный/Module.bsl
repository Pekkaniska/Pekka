			...
			Если РеквизитыНастройкиОбмена.ИспользуетсяКриптография Тогда
				...
				Если НЕ ЭлектронноеВзаимодействиеСлужебный.ЗначениеСвойстваXDTO(Тикет, "Sign") = Неопределено Тогда
					...
					Если Не ДанныеСертификата = Неопределено Тогда
						...
						ПрисоединенныйФайл = ОбменСБанкамиСлужебныйВызовСервера.ПрисоединенныйФайл(СообщениеКвитанция);
...
//       ** ИтогКоличествоПолученных - Число - количество полученных документов.
//
Процедура СинхронизироватьДокументыСбербанк(Параметры, АдресРезультата) Экспорт
	
	Результат = Новый Структура("Успех, ТребуетсяАутентификация, ИтогКоличествоПолученных", Ложь, Ложь, 0);
	...
	Тикет = Тикеты.Получить(0); // должен быть возвращен только один тикет
	
	Если ЗначениеЗаполнено(Тикет) Тогда
		СохранитьТикетСбербанк(Параметры.НастройкаОбмена, Тикет);
	КонецЕсли;
	...
	ПолучитьИзвещенияПоТикетамСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии,
		ИдентификаторОрганизации, МассивТикетов, Результат.ИтогКоличествоПолученных);
	
	ПолучитьСтатусыДокументовСбербанк(
		Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии, Результат.ИтогКоличествоПолученных);
...
//    * ТекущаяСессия - Структура - параметры установленной сессии
//  АдресРезультата - Строка - адрес, по которому будут помещены результаты выполнения. Содержит структуру со следующими полями:
//    * МассивНеобработанныхТикетов - Массив - в элементах строки с тикетами, которые еще не обработал банк
//    * ТребуетсяАутентификация - Булево - если Истина, то сессия "протухла" и нужно повторить аутентификацию.
//
	...
	Для Каждого Ответ Из МассивОтветов Цикл
		...
		Иначе
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбработатьОтветСбербанка(
				Ответ, Параметры.НастройкаОбмена, МассивНовыхСообщенийОбмена, ИдентификаторЗапроса, КоличествоТомов);
...
Процедура ОтправитьДокументыВСбербанкБазоваяАутентификация(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	Результат.Вставить("ИтогКоличествоПолученных", 0);
	...
	Если Параметры.ПолучитьСтатусыДокументов Тогда // для платежных поручений и писем
		МассивТикетов = Новый Массив;
		ПолучитьСтатусыДокументовСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии,
			Результат.ИтогКоличествоПолученных, Параметры.МассивСообщенийОбмена);
...
Функция ОбработатьЭлектронныеДокументы(Параметры, АдресРезультата) Экспорт
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Подписать") Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СертификатыИзНастроек.СертификатЭП.Отпечаток КАК ОтпечатокСертификата,
		|	ВТ_СертификатыИзНастроек.НастройкаОбмена.ПрограммаБанка КАК ПрограммаБанка
		|ИЗ
		|	ВТ_СертификатыИзНастроек КАК ВТ_СертификатыИзНастроек";
		...
		ИначеЕсли (Действия = "Подписать" ИЛИ Действия = "ПодписатьОтправить") И МассивСсылокНаОбъект.Количество() = 1 Тогда
			...
			РезультатДляДиагностики = Результат[8].Выгрузить();
			ОтпечаткиВНастройке = РезультатДляДиагностики.ВыгрузитьКолонку("ОтпечатокСертификата");
			РазностьМассивов = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивОтпечатков, ОтпечаткиВНастройке);
			Если РезультатДляДиагностики.Количество()
				И (РезультатДляДиагностики[0].ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
					ИЛИ РезультатДляДиагностики[0].ПрограммаБанка = Перечисления.ПрограммыБанка.АльфаБанкОнлайн)
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		...
		Если ТЗ_ЭД_КОтправке.Количество() > 0 Тогда
			...
			Если МассивСообщенийОбменаКОтправкеССервераБезПодписи.Количество() > 0 Тогда
				...
				КоличествоОтправленных = КоличествоОтправленных + СтРезультата.КоличествоОтправленных;
...
Функция ОтправкаПакетовЭДО(Знач МассивПакетовЭДО, Знач СоотвНастроекОбменаИПараметровСертификатов, ТекстСообщения = "") Экспорт
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетовЭДО Цикл
		...
		Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			Если СоотвНастроекОбменаИПараметровСертификатов = Неопределено
			...
			ИдентификаторСессии = СоотвНастроекОбменаИПараметровСертификатов.Получить(НастройкаОбмена).ИдентификаторСессии;
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				ОбработатьОшибкуПередачиПакета(СообщенияОбмена, ВидОперации, ТекстОшибки, Неопределено, ПакетОбменСБанками);
				Продолжить;
			КонецЕсли;
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ПодробноеПредставлениеОшибки, , "ОбменСБанками", ПакетОбменСБанками);
		...
		Исключение
			...
			ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
				ВидОперации, ТекстОшибкиВЖурнале, , "ОбменСБанками", ПакетОбменСБанками);
...
Процедура ПолучитьНовыеДокументыИзБанка(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	Результат = ПолучениеФайловИзИнтернета.СкачатьФайлВоВременноеХранилище(
		АдресСервера + АдресРесурса, ПараметрыПолучения);
	...
	Для Каждого Идентификатор Из МассивИдентификаторов Цикл
		...
		СохранитьПолученныйПакет(Параметры.НастройкаОбмена, Результат.Путь, ДанныеВозврата);
...
Процедура ОтправкаДокументовВБанкАсинхронныйОбмен(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	ОбменСБанкамиСлужебныйВызовСервера.ОтправитьЭДВБанк(
		Параметры.НастройкаОбмена, Параметры.ИдентификаторСессии, РезультатОтправки);
...
//
Процедура СохранитьПолученныйПакет(НастройкаОбмена, Путь, ДанныеВозврата) Экспорт
...
//
Процедура ПрочитатьИдентификаторыПакетов(НастройкаОбмена, Путь, МассивИдентификаторов) Экспорт
...
Функция ИнформацияОВнешнейКомпоненте(АдресВнешнейКомпоненты) Экспорт
	...
	Если Не НайденаИнформация Тогда
		...
		ЭлектронноеВзаимодействиеСлужебныйВызовСервера.ОбработатьОшибку(
			Операция, ТекстОшибки, , "ОбменСБанками");
		УдалитьФайлы(ВременныйКаталог);
		УдалитьФайлы(ВремФайл);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура СформироватьПлатежноеПоручениеAsync(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	...
	Если ЭтоПеречислениеВБюджет Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОКТМО) ИЛИ ПустаяСтрока(ОКТМО) Тогда
			ОКТМО = "0";
		КонецЕсли;
		ЗаполнитьСвойствоXDTO(ПлатежиВБюджетCML, "OKTMO", СокрЛП(ОКТМО), Истина, ТекстОшибки);
		ПоказательОснования = ЗначениеРеквизитаВДереве(ДеревоДанных, "ПлатежиВБюджет.ПоказательОснования");
		Если НЕ ЗначениеЗаполнено(ПоказательОснования) ИЛИ ПустаяСтрока(ПоказательОснования) Тогда
			ПоказательОснования = "0";
		КонецЕсли;
...
Процедура ОтправитьВБанк(Настройки, Данные, Результат, ТекстОшибки)
	...
	РезультатВыполнения = ЭлектронноеВзаимодействиеСлужебный.ОтправитьPOSTЗапрос(
...
Процедура ПрочитатьВалютнуюВыпискуБанка(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, НовоеСообщениеОбмена)
...
Процедура ПрочитатьВыпискуБанка(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, НовоеСообщениеОбмена)
	...
	Попытка
		Statement = ФабрикаXDTO.ПрочитатьXML(ОбъектXML);
		ВерсияФормата = ЗначениеСвойстваXDTO(Statement, "formatVersion");
		Фабрика = ОбменСБанкамиСлужебныйПовтИсп.ФабрикаAsyncXDTO(ВерсияФормата);
		Если Фабрика = Неопределено Тогда
	...
	Иначе
		НазваниеДокумента = НСтр("ru = 'Выписка банка от %1'");
		НазваниеДокумента = СтрШаблон(НазваниеДокумента, Формат(ТекущаяДатаСеанса(), "ДЛФ=D"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДатаОкончанияСтрокой) Тогда
		ДатаОкончания = XMLЗначение(Тип("Дата"), ДатаОкончанияСтрокой);
...
Процедура ПрочитатьДокументИзПакета(НастройкаОбмена, ПакетОбменСБанками, Document, МассивВнешнихИдентификаторов, ДанныеВозврата)
	...
	ИначеЕсли КодДокумента = "15" Тогда
		ПрочитатьВыпискуБанка(
			НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, НовоеСообщениеОбмена);
	...
	ИначеЕсли КодДокумента = "20" Тогда
		ПрочитатьПодтвержениеОткрытияСчетовПоЗарплатномуПроекту(
...
Процедура ПрочитатьПодтвержениеОткрытияСчетовПоЗарплатномуПроекту(НастройкаОбмена, ПакетОбменСБанками, ДвоичныеДанныеФайла, Document, НовоеСообщениеОбмена, ДанныеВозврата)
...
Процедура УдалитьСтаруюВерсиюСообщения(ЗаписьНабора)
	...
	СтруктураВерсии = СтруктураВерсииЭД(ЗаписьНабора.СсылкаНаОбъект);
...
Процедура СформироватьПисьмо(СсылкаНаОбъект, НастройкиОбменаЭД, СтруктураПараметров)
	...
	СтруктураЭД.Вставить("Основной", Истина);
	СтруктураЭД.Вставить("ПредставлениеДокумента", Строка(СсылкаНаОбъект));
	СтруктураЭД.Вставить("Статус", Перечисления.СтатусыОбменСБанками.Сформирован);
	СтруктураЭД.Вставить("АдресФайлаВоВременномХранилище", АдресФайлаВоВременномХранилище);
	СтруктураПараметров.Вставить("СтруктураЭД", СтруктураЭД);
	СтруктураПараметров.Вставить("Организация", НастройкиОбменаЭД.Организация);
...
Процедура ПолучитьСтатусыДокументовСбербанк(НастройкаОбмена, ИдентификаторСессии, КоличествоПолучено, МассивСообщенийОбмена = Неопределено)
