...
//  Массив
//
Функция ДанныеДляОформленияПоЗаказам(ДанныеРаспоряжений, ИдентификаторыСтрок, ПоляГруппировки, СписокОшибок, ПолеОшибки, МетаданныеНакладных) Экспорт
...
//  Массив
//
Функция ДанныеДляОформленияПоОрдерам(ДанныеРаспоряжений, ИдентификаторыСтрок, ПоляГруппировки, СписокОшибок, ПолеОшибки, МетаданныеНакладных) Экспорт
	
	ГруппировкиРаспоряжений = Новый Массив;
	ОшибкиПоНакладным = Новый Массив;
	СтрокаНакладной = Неопределено;
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если ПустаяСтрока(ТекстОшибки) Тогда
			Если Строка.СостояниеНакладной = 4 Тогда
				...
				ОшибкиПоНакладным.Добавить(ТекстОшибки);
			Иначе
	...
	Если ОшибкиПоНакладным.Количество() > 1 Или ЗначениеЗаполнено(ГруппировкиРаспоряжений) Тогда
		Для каждого ТекстОшибки Из ОшибкиПоНакладным Цикл
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(СписокОшибок, ПолеОшибки, ТекстОшибки, Неопределено);
		КонецЦикла;
	ИначеЕсли СтрокаНакладной <> Неопределено Тогда
		Группировка = ПолучитьГруппировку(ГруппировкиРаспоряжений, СтрокаНакладной, ПоляГруппировки, МетаданныеНакладных, Истина);
		Группировка.МассивЗаказов.Добавить(СтрокаНакладной.Ссылка);
