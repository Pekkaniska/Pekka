...
Процедура СформироватьДвиженияПоДокументу(Движения, Отказ, Организация, ПериодРегистрации, ДанныеДляПроведения, СтрокаСписокТаблиц = "") Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОценочныеОбязательстваЗарплатаКадры") Тогда
		...
		СтраховыеВзносы = Движения.Найти("СтраховыеВзносыПоФизическимЛицам");
		Если СтраховыеВзносы <> Неопределено Тогда
			СтраховыеВзносы = СтраховыеВзносы.Выгрузить();
			ТаблицаВзносов  = ОтражениеЗарплатыВУчете.ДанныеДляОтражениеВУчетеВзносовПоТаблице(СтраховыеВзносы);
		Иначе
			ТаблицаВзносов = ОтражениеЗарплатыВУчете.НоваяТаблицаСтраховыеВзносы();
		КонецЕсли;
		
		НачисленияУдержания = ОбщегоНазначенияБЗК.ТаблицаЗначенийПоИмениРегистраНакопления("НачисленияУдержанияПоСотрудникам");
		
		НачисленияУдержанияПоСотрудникам = Движения.Найти("НачисленияУдержанияПоСотрудникам");
		Если НачисленияУдержанияПоСотрудникам <> Неопределено Тогда
			НачисленияУдержанияПоСотрудникам = НачисленияУдержанияПоСотрудникам.Выгрузить();
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(НачисленияУдержанияПоСотрудникам.НайтиСтроки(Новый Структура("Сторно", Ложь)), НачисленияУдержания);
		КонецЕсли;
		
		НачисленияУдержанияПоКонтрагентамАкционерам = Движения.Найти("НачисленияУдержанияПоКонтрагентамАкционерам");
		Если НачисленияУдержанияПоКонтрагентамАкционерам <> Неопределено Тогда
			НачисленияУдержанияПоКонтрагентамАкционерам = НачисленияУдержанияПоКонтрагентамАкционерам.Выгрузить();
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(НачисленияУдержанияПоКонтрагентамАкционерам, НачисленияУдержания);
		КонецЕсли;
		...
		НачисленнаяЗарплатаИВзносы = ОтражениеЗарплатыВБухучете.ТаблицаНачисленийИВзносовДляБухучета(МенеджерВременныхТаблиц);
		...
		Если ЕстьДвижениеВыплатаОтпусковЗаСчетРезерва Тогда
			...
			Если НастройкиРезервовОтпусков.ФормироватьРезервОтпусковБУ Тогда
				...
				МодульРезервОтпусков.СписатьРасходыПоОплатеОтпускаЗаСчетОценочныхОбязательств(Организация, 
					ПериодРегистрации, ДокументСсылка, НачисленнаяЗарплатаИВзносы, НачисленныеОтпуска, КолонкиСуммирования);
...
// Процедура формирует дополнительные движения по регистрам финансового учета для документа НачислениеОценочныхОбязательствПоОтпускам.
//
// Параметры:
...
Процедура ОбработкаПроведенияНачислениеОценочныхОбязательствПоОтпускам(ЭтотОбъект, Отказ, РежимПроведения) Экспорт
	...
	Документы.НачислениеОценочныхОбязательствПоОтпускам.ИнициализироватьДанныеДокумента(ЭтотОбъект.Ссылка, ЭтотОбъект.ДополнительныеСвойства);
	...
	ОтражениеЗарплатыВФинансовомУчетеУП.ОтразитьТрудозатратыНезавершенногоПроизводства(ЭтотОбъект.ДополнительныеСвойства, ЭтотОбъект.Движения, Отказ);
	
	//Движения по отражению зарплаты в финансовом учете
	УправленческийУчетПроведениеСервер.ОтразитьДвиженияОтражениеЗарплатыВФинансовомУчете(ЭтотОбъект.ДополнительныеСвойства, ЭтотОбъект.Движения, Отказ);
...
Функция РаспределитьНачисленияПоЭтапамПроизводства(Параметры) Экспорт
	...
	ВидыВзносов                   = ВидыСуммВзносов(Параметры.Ссылка);
...
Функция ПараметрыРаспределенияСтроки(ВидСуммы, СтрокаБазы, ДанныеСпособа)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВидСуммы.ТипНалога) Тогда
...
Функция ВидыСуммВзносов(Ссылка)
