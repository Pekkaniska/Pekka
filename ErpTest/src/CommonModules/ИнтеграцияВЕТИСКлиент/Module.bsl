...
//
// Параметры:
//  Форма - УправляемаяФорма - форма объекта документа ВетИС к передаче
...
Процедура ПодтвердитьКорректностьДокументаСозданногоЧерезWeb(ДокументСсылка) Экспорт
	...
	ИнтеграцияВЕТИСВызовСервера.ОтметитьДокументКакОбработанный(ДокументСсылка);
...
Процедура ПодготовитьСообщенияКПередаче(ДинамическийСписок, ПараметрыПередачи, ПравоИзменения) Экспорт
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		Если ДанныеСтроки.ДальнейшееДействиеВЕТИС1 = ПараметрыПередачи.ДальнейшееДействие
			Или ДанныеСтроки.ДальнейшееДействиеВЕТИС2 = ПараметрыПередачи.ДальнейшееДействие
			Или ДанныеСтроки.ДальнейшееДействиеВЕТИС3 = ПараметрыПередачи.ДальнейшееДействие Тогда
	...
	Если НепроведенныеДокументы.Количество() > 0 Тогда
		...
		// Проверим, есть ли право на проведение документов.
		Если НЕ ПравоИзменения Тогда
			ПоказатьПредупреждение(, НСтр("ru='У Вас недостаточно прав для обработки непроведенных документов.'"));
			Возврат;
		КонецЕсли;
...
Процедура ПодготовитьСообщенияКПередачеЗавершение(Контекст) Экспорт
	...
	Если ТипЗнч(Контекст) = Тип("Структура")
		...
		Форма = ПолучитьФормуПоЭлементуФормы(Контекст.ДинамическийСписок);
...
Процедура ОткрытьФормуВыбораОрганизацийВЕТИС(Форма, Префикс, Префиксы = Неопределено, ОповещениеПриЗавершении = Неопределено, ПрефиксыСписков = "") Экспорт
...
// Функция проверяет необходимость указания серий в строке, если возможно, открывает форму указания, если форма 
// указания не требует контекстного вызова сервера.
//
// Параметры:
//	Форма					- УправляемаяФорма	- Форма в которой инициировано указание серий.
...
//
// Возвращаемое значение:
//	Булево, Истина - Нужен серверный вызов.
//
Функция ДляУказанияСерийНуженСерверныйВызов(Форма,
	...
	Возврат ИнтеграцияВЕТИСКлиентПереопределяемый.ДляУказанияСерийНуженСерверныйВызов(Форма,
		ПараметрыУказанияСерий, Текст, СтандартнаяОбработка, ТекущиеДанные);
...
Процедура ОткрытьФормуСозданияДокументаВЕТИС(ПолноеИмяДокументаВЕТИС, ДокументОснование = Неопределено, Владелец = Неопределено) Экспорт
	
	ПараметрыФормы = Новый Структура("Основание", ДокументОснование);
	
	ОткрытьФорму(
		ПолноеИмяДокументаВЕТИС + ".Форма.ФормаДокумента",
		ПараметрыФормы,
		Владелец);
...
Процедура ОткрытьФормуРедактированияУпаковокШтрихкодовПоСтроке(Форма, Объект, ИдентификаторСтроки,
	ОписаниеОповещения, ТолькоПросмотр = Ложь) Экспорт
	
	ДанныеУпаковокШтрихкодов = ПолучитьДанныеУпаковокШтрихкодовПоСтроке(Объект, ИдентификаторСтроки);
...
// Параметры:
//  Форма						 - УправляемаяФорма - форма в которой происходит выбор
//  ИмяСписка					 - Строка - имя элемента формы связанного с динамическим списком для выбора
...
//
Функция ВыборСтрокиДинамическогоСпискаКорректен(Форма, ИмяСписка, ПоТекущейСтроке = Ложь, СообщатьОшибкуПользователю = Истина) Экспорт
	
	ТипГруппировка = Тип("СтрокаГруппировкиДинамическогоСписка");
	Если ПоТекущейСтроке Тогда
		
		Если Форма.Элементы[ИмяСписка].ТекущиеДанные <> Неопределено И ТипЗнч(Форма.Элементы[ИмяСписка].ТекущаяСтрока) <> ТипГруппировка Тогда
			Возврат Истина;
		КонецЕсли;
		
	Иначе
		
		Если Форма.Элементы[ИмяСписка].ВыделенныеСтроки.Количество() Тогда 
			Результат = Истина;
	...
	Если СообщатьОшибкуПользователю Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Операция недоступна для выбранного объекта'"),,ИмяСписка);
	КонецЕсли;
...
Функция ПолучитьФормуПоЭлементуФормы(ЭлементФормы)
...
//   * Форма                   - УправляемаяФорма   - форма, в которой происходит выбор основания,
//   * ОповещениеПриЗавершении - ОписаниеОповещения - действие при выборе основания,
//   * РедактированиеДоступно  - Булево, Истина     - признак доступности объекта для редактирования.
Функция ПараметрыЗавершенияВыбораОснования() Экспорт
	...
	Результат.Вставить("РедактированиеДоступно", Истина);
	Возврат Результат;
...
Процедура ОбработкаОповещенияВФормеДокументаОснования(Форма, Объект, ИмяСобытия, Параметр, Источник, СобытиеОбработано) Экспорт
	
	ИнтеграцияВЕТИСКлиентПереопределяемый.ОбработкаОповещенияВФормеДокументаОснования(
		Форма,
		Объект,
		ИмяСобытия,
		Параметр,
	...
	ИначеЕсли ИнтеграцияИСКлиентСервер.ЭтоСобытиеОповещенияИзмененОбъект(ОписаниеСобытия) Тогда
		...
		Если ЧастиИмениОбъекта.Количество() = 2 И ЧастиИмениОбъекта[0] = "Документ" Тогда
			...
			СобытиеОбработано = Истина; // будет обработано далее по коду
			
			ИменаДокументовВЕТИС = ИнтеграцияВЕТИСВызовСервера.ИменаДокументовВЕТИСДляДокументаОснования(Объект.Ссылка);
			
			Если НЕ ИменаДокументовВЕТИС.Свойство(ЧастиИмениОбъекта[1]) Тогда
				Возврат; // документ не может быть основанием для измененного документа ВЕТИС - ничего делать не нужно
			КонецЕсли;
	...
	Если НЕ СобытиеОбработано Тогда
		...
		// Имя события сформировано не функцией ИнтеграцияИСКлиентСервер.ИмяСобытия...()
		УточнениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='""%1"": некорректное имя события оповещения ВетИС ""%2""'"),
			Форма.ИмяФормы,
			ИмяСобытия);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ИнтеграцияИСКлиентСервер.ТекстОшибки(ИнтеграцияВЕТИСКлиентСервер.ПредставлениеПодсистемы(), УточнениеОшибки),
			Объект.Ссылка);
	Иначе
...
Процедура ВыполнитьКомандуГиперссылкиВФормеДокументаОснованияВЕТИС(ДокументОснование, НавигационнаяСсылкаФорматированнойСтроки, Форма, СобытиеОбработано) Экспорт
	...
	Если ИнтеграцияИСКлиентСервер.ЭтоКомандаНавигационнойСсылкиСоздатьОбъект(ОписаниеКоманды) Тогда
		...
		ОткрытьФормуСозданияДокументаВЕТИС(ПолноеИмяДокументаВЕТИС, ДокументОснование, Форма);
