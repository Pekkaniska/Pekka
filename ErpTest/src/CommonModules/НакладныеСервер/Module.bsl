...
Функция ТекстЗапросаСостояний(ИмяТаблицыСостояний, ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению = Ложь) Экспорт
	...
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияПередачВПроизводство" Тогда
		Возврат ТекстЗапросаСостоянияПередачВПроизводство(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению);
	...
	ИначеЕсли ИмяТаблицыСостояний = "СостоянияПередачИзПроизводства" Тогда
		Возврат ТекстЗапросаСостоянияПередачИзПроизводства(ПараметрыОтбора, Распоряжения, ТолькоНаличиеКОформлению);
...
Функция СостоянияПоХозОперациям(ХозОперации) Экспорт
	...
	Для каждого ХО Из ХозОперации Цикл
		...
		ИначеЕсли ХО = Перечисления.ХозяйственныеОперации.ПоступлениеИзПроизводства
			Или ХО = Перечисления.ХозяйственныеОперации.ВозвратИзЭксплуатации
...
Процедура ДобавитьСостояние(Состояния, Состояние, Документ)
...
Функция ТекстЗапросаСостоянияРасходныхОрдеров(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	Если ПараметрыОтбора.ХозОперация Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеестрДокументов.ХозяйственнаяОперация",
			"ВЫБОР КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.ЗаказНаПроизводство ТОГДА
...
Функция ТекстЗапросаСостоянияПередачВПроизводство(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|				И ЗаказыМатериаловВПроизводство.КОформлению > 0
	...
	ДополнитьТекстОтборомПоРееструДокументов(ТекстЗапроса, "Распоряжение", ПараметрыОтбора.ХозОперация,
		ПараметрыОтбора.Организация,,, ПараметрыОтбора.Менеджер);
	//++ НЕ УТКА
	Если ПараметрыОтбора.ХозОперация Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РеестрДокументов.ХозяйственнаяОперация",
			"ВЫБОР КОГДА РеестрДокументов.Ссылка ССЫЛКА Документ.ЗаказНаПроизводство ТОГДА
...
Функция ТекстЗапросаСостоянияОтчетовПереработчика(ПараметрыОтбора, ТолькоНаличиеКОформлению)
	...
	|				И ТоварыПолученныеОтПереработчика.Количество > 0
	...
	|		Остатки.КодСтроки КАК КодСтроки,
	|		Остатки.ТипСтоимости КАК ТипСтоимости,
	|		Остатки.АналитикаУчетаНоменклатуры.Склад КАК Партнер
	|	ИЗ
	...
	ДополнитьТекстОтборомПоРееструДокументов(ТекстЗапроса, "Распоряжение", ПараметрыОтбора.ХозОперация,
		ПараметрыОтбора.Организация,, ПараметрыОтбора.Подразделение, ПараметрыОтбора.Менеджер);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//&Второе", "//&");
	ДополнитьТекстОтборомПоРееструДокументов(ТекстЗапроса, "ЗаказПоставщику", ПараметрыОтбора.ХозОперация,
		ПараметрыОтбора.Организация,, ПараметрыОтбора.Подразделение, ПараметрыОтбора.Менеджер);
...
Функция ТекстЗапросаСостоянияПередачИзПроизводства(ПараметрыОтбора, ТипыРаспоряжений, ТолькоНаличиеКОформлению)
	...
	|				И РаспоряженияНаПередачуИзПроизводства.Количество > 0
	...
	ДопУсловие = "Получатель ССЫЛКА Справочник.Склады";
	Если ПараметрыОтбора.ХозОперация Тогда
	...
	КонецЕсли;
	ДополнитьТекстОтборомПоОстаткам(ТекстЗапроса, "Распоряжение", ТипыРаспоряжений,, ДопУсловие);
	ДополнитьТекстОтборомПоРееструДокументов(ТекстЗапроса, "Распоряжение",, ПараметрыОтбора.Организация,,
		ПараметрыОтбора.Подразделение, ПараметрыОтбора.Менеджер);
...
Функция ТекстЗапросаСостоянияОтчетовДавальцам(ПараметрыОтбора, ТолькоНаличиеКОформлению)
	...
	|				И УслугиДавальцуКОформлению.КОформлению > 0
	...
	|	ВтЕстьРаспоряжения КАК ВтЕстьРаспоряжения
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЕстьОтчеты КАК ВтЕстьОтчеты
	|		ПО ВтЕстьРаспоряжения.ЗаказДавальца = ВтЕстьОтчеты.ЗаказДавальца
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтЕстьРаспоряжения
	...
	ДополнитьТекстОтборомПоРееструДокументов(ТекстЗапроса, "ЗаказДавальца", ПараметрыОтбора.ХозОперация,
		ПараметрыОтбора.Организация,, ПараметрыОтбора.Подразделение, ПараметрыОтбора.Менеджер);
