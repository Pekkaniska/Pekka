...
//-- НЕ УТКА
	...
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|	ПО СпрПартииПроизводства.Ссылка = Реквизиты.ПартияПроизводства
	|
	...
	|	И Реквизиты.Статус В (ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Начат),
	|						ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Завершен))
	|	И (Реквизиты.ФактическоеНачалоЭтапа МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|		ИЛИ Реквизиты.ФактическоеОкончаниеЭтапа МЕЖДУ &НачалоПериода И &ОкончаниеПериода
	|		ИЛИ ((Реквизиты.ФактическоеОкончаниеЭтапа = ДатаВремя(1, 1, 1) ИЛИ Реквизиты.ФактическоеОкончаниеЭтапа > &ОкончаниеПериода)
	|			И Реквизиты.ФактическоеНачалоЭтапа < &НачалоПериода))
//-- НЕ УТКА
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
//-- НЕ УТКА
	...
	|					ИЛИ АналитикаУчетаНоменклатуры.Склад ССЫЛКА Справочник.СтруктураПредприятия) И АналитикаУчетаНоменклатуры.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)) КАК ДД
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК Аналитика
	|		ПО ДД.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ СкладыПодразделения КАК СкладыПодразделения
	|		ПО (Аналитика.Склад = СкладыПодразделения.Склад)
...
//-- НЕ УТКА
	...
	|				ИЛИ АналитикаУчетаНоменклатуры.Склад ССЫЛКА Справочник.СтруктураПредприятия)
...
Функция НаборыЗаписейОтложенногоПроведенияЭтапаПроизводства(Реквизиты, ПараметрыЗаполнения, ПараметрыЗадания)
	...
	ИменаРегистров = Новый Структура("ТоварыОрганизаций, ТоварыКОформлениюОтчетовКомитенту, СебестоимостьТоваров, РезервыТоваровОрганизаций");
...
Функция СобытиеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Расход материалов на производство (фоновое задание)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
