...
//
// Параметры:
//  СтруктураОчереди - Структура со свойствами. см. функцию ИнтеграцияВЕТИСКлиентСервер.СтруктураОчередиПередачиДанных()
//  НастройкиОбменаВЕТИС - Соответствие - Настройки обмена с ВЕТИС.
//
Функция ЗагрузитьРезультатОбработкиЗаявки(ЭлементОчереди, ПараметрыОбмена) Экспорт
	...
	Для Каждого СообщениеXML Из СообщенияXML Цикл
		...
		ИначеЕсли ДанныеДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийВЕТИС.ЗаявкаОтклонена Тогда
			...
			Если ЕстьОшибкиТребующиеПовторногоЗапроса
		...
		ИначеЕсли ДанныеДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийВЕТИС.ЗаявкаОбрабатывается Тогда
			...
			ВремяОжидания = ТекущаяДатаСеанса() - ДанныеВыполненияЗаявки.issueDate;
			...
			Иначе
				...
				Если ПараметрыОбмена.ОтправлятьЗависшиеЗапросыПовторно
		...
		НайденныеСсылкиПриОткатеТранзакции = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ПараметрыОбмена.ПараметрыПреобразования.НайденныеСсылки);
		...
		Попытка
			...
			ИначеЕсли Операция = Перечисления.ВидыОперацийВЕТИС.ОтветНаЗапросОформленияВходящейПартии Тогда
		...
		Если ТранзакцияЗафиксирована
			...
			Или ДанныеДокумента.СтатусОбработки = Перечисления.СтатусыОбработкиСообщенийВЕТИС.ОшибкаПоиска) Тогда
...
Функция ЗапросПолученияРезультатаОбработкиЗаявкиXML(ХозяйствующийСубъект, ИдентификаторЗаявки, НастройкаОбмена) Экспорт
	...
	ЗапросКвитанции = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, ИмяТипа);
...
Функция ПодготовитьЗапросыДляЗагрузкиИзмененныхДанных(ПараметрыОбмена) Экспорт
	...
	|ГДЕ
	|	Таблица.Ссылка В(&ХозяйствующиеСубъекты)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЗапрашиватьИзмененныеЗаписиСкладскогоЖурнала Тогда
...
Функция ПередатьСообщениеВСервисОбработкиЗаявок(ИсходящееСообщение, РеквизитыСообщения, НастройкаОбмена, ПараметрыОбмена) Экспорт
	...
	Иначе
		ТекстСообщенияXML = ИнтеграцияВЕТИС.ТекстСообщенияXMLИзПротокола(ИсходящееСообщение);
	КонецЕсли;
	...
	Если РезультатОтправкиЗапроса.ТекстСообщенияXMLПолучен Тогда
		...
		Если ХозяйствующийСубъект = "ПроверкаПодключенияКВЕТИС" Тогда
...
Функция ПодготовитьЗапросИзмененныхВетеринарноСопроводительныхДокументов(ХозяйствующийСубъект, Предприятие, КоличествоЭлементов, ПараметрыОбмена, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИнтеграцияВЕТИС.ПодготовитьСообщенияКПередаче(СообщенияXML, Истина, ПараметрыОбмена);
...
Функция ПодготовитьЗапросИзмененныхЗаписейСкладскогоЖурнала(ХозяйствующийСубъект, Предприятие, КоличествоЭлементов, ПараметрыОбмена, ДополнительныеПараметры = Неопределено) Экспорт
...
Функция ЗаявкаXML(Запрос, НастройкаОбмена, ХранилищеВременныхДат) Экспорт
	...
	Заявка = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, ИмяПакета);
	Заявка.apiKey = НастройкаОбмена.КлючAPI;
	
	Заявка.application = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Заявка, "application");
	...
	Заявка.application.data = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Заявка.application, "data");
...
Функция ЗапросНаРегистрациюИзменениеПредприятияXML(ХозяйствующийСубъект, ДанныеПредприятия, ПараметрыОбмена) Экспорт
	...
	Запрос = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПространствоИмен, ИмяТипа);
	
	Запрос.initiator = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "initiator");
	...
	Запрос.modificationOperation = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "modificationOperation");
	...
	Запрос.modificationOperation.resultingList = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.modificationOperation, "resultingList");
	...
	enterprise = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.modificationOperation.resultingList, "enterprise");
	...
	enterprise.activityList = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(enterprise, "activityList");
	
	Для Каждого ВидДеятельности Из ДанныеПредприятия.ВидыДеятельности Цикл
		
		activity = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(enterprise.activityList, "activity");
	...
	Если ДанныеПредприятия.Свойство("СобственникПредприятия")
		...
		enterprise.owner = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(enterprise, "owner");
...
Функция ЗапросНаРегистрациюИзменениеХозяйствующегоСубъектаXML(ХозяйствующийСубъект, ДанныеХозяйствующегоСубъекта, ПараметрыОбмена) Экспорт
	...
	businessEntity = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.modificationOperation.resultingList, "businessEntity");
	...
	businessEntity.incorporationForm = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(businessEntity, "incorporationForm");
...
Функция ЗапросНаСозданиеУдалениеСвязиМеждуХозяйствующимСубъектомИПредприятиемXML(ХозяйствующийСубъект, Предприятие, СпособИзменения, GLN, ПараметрыОбмена) Экспорт
	...
	Запрос.modificationOperation.businessEntity = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.modificationOperation, "businessEntity");
	Запрос.modificationOperation.businessEntity.guid = НастройкаОбмена.Идентификатор;
	
	activityLocation = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.modificationOperation, "activityLocation");
	...
	activityLocation.enterprise = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(activityLocation, "enterprise");
...
Функция ЗапросНаАннулированиеВетеринарноСопроводительногоДокументаПоUUIDXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Запрос.specifiedPerson = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "initiator");
...
Функция ЗапросВетеринарноСопроводительныхДокументовXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Если ПараметрыЗапроса.ПараметрыОтбора <> Неопределено Тогда
		...
		Если ПараметрыОтбора.Свойство("Интервал") И ТипЗнч(ПараметрыОтбора.Интервал) = Тип("Структура") Тогда
			Запрос.issueDateInterval = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "issueDateInterval");
		...
		Если ПараметрыОтбора.Свойство("ГрузоотправительПредприятие") И ЗначениеЗаполнено(ПараметрыОтбора.ГрузоотправительПредприятие) Тогда
			ЭлементConsignor  = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "consignor");
			ЭлементEnterprise = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементConsignor, "enterprise");
			...
			Если ПараметрыОтбора.Свойство("ГрузоотправительХозяйствующийСубъект")
			   И ЗначениеЗаполнено(ПараметрыОтбора.ГрузоотправительХозяйствующийСубъект) Тогда
				ЭлементBusinessEntity = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементConsignor, "businessEntity");
		...
		Если ПараметрыОтбора.Свойство("СвязанныйДокумент") И ТипЗнч(ПараметрыОтбора.СвязанныйДокумент) = Тип("Структура") Тогда
			Запрос.referencedDocument = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "referencedDocument");
...
Функция ЗапросЗаписейСкладскогоЖурналаXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Запрос = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПараметрыЗапросаВЕТИС.ПространствоИмен, ПараметрыЗапросаВЕТИС.ТипВЕТИС);
	...
	Запрос.searchPattern = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениСвойства(ПараметрыЗапросаВЕТИС.ПространствоИмен, "searchPattern", Запрос);
...
Функция ЗапросНаСозданиеИзменениеПродукцииXML(ХозяйствующийСубъект, ДанныеПродукции, ПараметрыОбмена) Экспорт
	...
	productItem = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.modificationOperation.resultingList, "productItem");
	...
	productItem.product    = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem, "product");
	productItem.subProduct = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem, "subProduct");
	...
	Если ЗначениеЗаполнено(ДанныеПродукции.ХозяйствующийСубъектПроизводитель) Тогда
		productItem.producer = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem, "producer");
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(ДанныеПродукции.ХозяйствующийСубъектСобственникТорговойМарки) Тогда
		productItem.tmOwner = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem, "tmOwner");
	...
	Для Каждого Производитель Из ДанныеПродукции.Производители Цикл
		...
		ЭлементДанных = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem, "producing");
		ЭлементДанных.location = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементДанных, "location");
	...
	productItem.packaging = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem, "packaging");
	
	Если ЗначениеЗаполнено(ДанныеПродукции.ФасовкаУпаковка) Тогда
		productItem.packaging.packagingType = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem.packaging, "packagingType");
	...
	Если ЗначениеЗаполнено(ДанныеПродукции.ФасовкаЕдиницаИзмерения) Тогда
		productItem.packaging.unit = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(productItem.packaging, "unit");
...
Функция ЗапросДанныхПользователяXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Запрос.user = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "user");
...
Функция ЗапросНаИзменениеПравПользователей(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	ЭлементUser = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "user");
	...
	Для Каждого ДанныеПользователя Из ПараметрыЗапроса.ДанныеПользователей Цикл
		...
		ЭлементAuthorityList = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементUser, "authorityList");
		...
		Для Каждого ПравоДоступаВЕТИС Из ДанныеПользователя.ПраваДоступа Цикл
			...
			Если ИдентификаторРоли <> Неопределено Тогда
				ЭлементAuthority = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементAuthorityList, "authority");
...
Функция ЗапросНаИзменениеЗонОтветственностиПользователейXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Для Каждого ДанныеПользователя Из ПараметрыЗапроса.ДанныеПользователей Цикл
		...
		Если ДанныеПользователя.Предприятия.Количество() > 0
			...
			ЭлементWorkingAreaList = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементUser, "workingAreaList");
			...
			Для Каждого Предприятие Из ДанныеПользователя.Предприятия Цикл
				...
				ЭлементWorkingArea = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементWorkingAreaList, "workingArea");
				ЭлементWorkingArea.enterprise = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементWorkingArea, "enterprise");
...
Функция ЗапросНаРегистрациюИПривязкуПользователейХозяйствующегоСубъектаXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Для Каждого ДанныеПользователя Из ПараметрыЗапроса.ДанныеПользователей Цикл
		...
		Иначе
			...
			ЭлементIdentity = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементUser, "identity");
			...
			ЭлементCitizenship = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементUser, "citizenship");
			ЭлементCountry     = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(ЭлементCitizenship, "country");
...
Функция ЗапросНаВнесениеСведенийОВетеринарныхОперацияхXML(ХозяйствующийСубъект, ПараметрыЗапроса, ПараметрыОбмена) Экспорт
	...
	Запрос.enterprise = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "enterprise");
	...
	Для каждого СведенияОЗаписиЖурнала Из ПараметрыЗапроса.ЗаписиСкладскогоЖурнала Цикл
		...
		stockEntry = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "stockEntry");
		...
		Если (СведенияОЗаписиЖурнала.Свойство("ЛабораторныеИсследования")
		 ...
		 ИЛИ (СведенияОЗаписиЖурнала.Свойство("Иммунизации")
			...
			stockEntry.vetEventList = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(stockEntry, "vetEventList");
			...
			Если СведенияОЗаписиЖурнала.Свойство("ЛабораторныеИсследования")
			   ...
			   И СведенияОЗаписиЖурнала.ЛабораторныеИсследования.Количество() > 0 Тогда
				...
				Для каждого СведенияОЛабораторномИсследовании Из СведенияОЗаписиЖурнала.ЛабораторныеИсследования Цикл
					...
					laboratoryResearch = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(stockEntry.vetEventList, "laboratoryResearch");
					...
					Если СведенияОЛабораторномИсследовании.Свойство("НаименованиеЛаборатории")
					   И ЗначениеЗаполнено(СведенияОЛабораторномИсследовании.НаименованиеЛаборатории) Тогда
						laboratoryResearch.operator = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(laboratoryResearch, "operator");
						laboratoryResearch.operator.name = СведенияОЛабораторномИсследовании.НаименованиеЛаборатории;
					КонецЕсли;
					
					referencedDocument = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(laboratoryResearch, "referencedDocument");
					...
					Если СведенияОЛабораторномИсследовании.Свойство("НаименованиеПоказателя")
					   ...
					   И СведенияОЛабораторномИсследовании.ТипПоказателя = "Показатель" Тогда
						laboratoryResearch.indicator = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(laboratoryResearch, "indicator");
					...
					Если СведенияОЛабораторномИсследовании.Свойство("НаименованиеПоказателя")
					   ...
					   И СведенияОЛабораторномИсследовании.ТипПоказателя = "Болезнь" Тогда
						laboratoryResearch.disease = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(laboratoryResearch, "disease");
					...
					Если СведенияОЛабораторномИсследовании.Свойство("МетодИсследования")
					   И ЗначениеЗаполнено(СведенияОЛабораторномИсследовании.МетодИсследования) Тогда
						laboratoryResearch.method = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(laboratoryResearch, "method");
			...
			Если СведенияОЗаписиЖурнала.Свойство("Иммунизации")
			   ...
			   И СведенияОЗаписиЖурнала.Иммунизации.Количество() > 0 Тогда
				...
				Для каждого СведенияОбИммунизации Из СведенияОЗаписиЖурнала.Иммунизации Цикл
					...
					immunization = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(stockEntry.vetEventList, "immunization");
					...
					immunization.disease = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(immunization, "disease");
					...
					Если (СведенияОбИммунизации.Свойство("НазваниеИПроизводительВакциныПрепарата")
					 ...
					 ИЛИ (СведенияОбИммунизации.Свойство("НомерСерииВакциныПрепарата")
						...
						immunization.medicinalDrug = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(immunization, "medicinalDrug");
