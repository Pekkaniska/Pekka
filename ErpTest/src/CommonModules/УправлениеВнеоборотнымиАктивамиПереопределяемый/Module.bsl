...
Процедура СформироватьДвиженияНачислениеАмортизации(ТаблицаЗатрат, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаЗатрат Цикл
		...
		УчетОСВызовСервера.ОпределитьПостоянныеРазницыВРасходах(Проводка, ПоддержкаПБУ18);
...
Функция ПодготовитьПараметрыНачислениеАмортизации(ТаблицаЗатрат, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаЗатрат",
		ПараметрыПроведения(ТаблицаЗатрат, СписокОбязательныхКолонок));
	...
	Параметры.Вставить("Реквизиты",
		ПараметрыПроведения(ТаблицаРеквизиты, СписокОбязательныхКолонок));
...
//
// Параметры:
//  ТаблицаАмортизации			 - ТаблицаЗначений - таблица значений с рассчитанной амортизацией со следующими колонками:
//  КоличествоОбработанныхДанных - Число - количество обработанных данных (используется для замера производительности).
...
//
Функция ПодготовитьТаблицуРаспределениеАмортизацииПоНаправлениямРегл(ТаблицаАмортизации, КоличествоОбработанныхДанных = 0, Отказ = Ложь) Экспорт
	
	ТаблицаЗатрат = ПустаяТаблицаАмортизации();
	
	СоответствиеПолей = Новый Соответствие;
	СоответствиеПолей.Вставить("СчетАмортизации", "СчетНачисленияАмортизации");
	ОбщегоНазначенияУТ.ДобавитьСтрокиВТаблицу(ТаблицаЗатрат, ТаблицаАмортизации, СоответствиеПолей);
