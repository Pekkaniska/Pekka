...
Функция ПроводкиЗакрытиеСчета90(Период, Организация) Экспорт
	...
	Проводки.Свернуть(
		"ЭтоПрибыль, СчетДт, СчетКт, ПрибылиИУбытки, НаправлениеДеятельности", 
...
Функция ПрибыльОтПродаж(НачалоПериода, КонецПериода, Организации)
	...
	|	СУММА(ЕСТЬNULL(Хозрасчетный.СуммаУУОборотКт, 0) - ЕСТЬNULL(Хозрасчетный.СуммаУУОборотДт, 0)) КАК СуммаУУ,
...
Функция ПодготовитьТаблицыЗакрытиеСчета91(ТаблицаРеквизиты, СписокОрганизаций, Отказ) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ Выборка.Прибыль = 0 ИЛИ НЕ Выборка.ПрибыльПР = 0 ИЛИ НЕ Выборка.ПрибыльВР = 0 Тогда
...
Процедура СформироватьДвиженияЗакрытиеГода(ТаблицаЗакрытие, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	СформироватьДвиженияПереносУбыткаНаБудущее(Параметры, Реквизиты, ПоддержкаПБУ18, Движения, Отказ);
...
Функция ПодготовитьТаблицуЗакрытие20_23_25_26НУиНЕ(ТаблицаРеквизиты, Отказ) Экспорт
	...
	|			&КонГраница,
	|			Счет В (&СписокСчетовНУ),
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат),
	|			Организация = &Организация
	|				И ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРасходов В (&ВидыРасходовНормируемые)
	|	) КАК НалоговыйОстатки
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	НалоговыйОстатки.Подразделение,
	|	НалоговыйОстатки.НаправлениеДеятельности,
	|	СУММА(ЕСТЬNULL(НалоговыйОстатки.СуммаОстатокКт, 0) - ЕСТЬNULL(НалоговыйОстатки.СуммаОстатокДт, 0)),
	|	СУММА(ЕСТЬNULL(НалоговыйОстатки.СуммаУУОстатокКт, 0) - ЕСТЬNULL(НалоговыйОстатки.СуммаУУОстатокДт, 0)),
	|	СУММА(ЕСТЬNULL(НалоговыйОстатки.СуммаНУОстатокКт, 0) - ЕСТЬNULL(НалоговыйОстатки.СуммаНУОстатокДт, 0)),
	|	СУММА(ЕСТЬNULL(НалоговыйОстатки.СуммаПРОстатокКт, 0) - ЕСТЬNULL(НалоговыйОстатки.СуммаПРОстатокДт, 0)),
	|	СУММА(ЕСТЬNULL(НалоговыйОстатки.СуммаВРОстатокКт, 0) - ЕСТЬNULL(НалоговыйОстатки.СуммаВРОстатокДт, 0)),
	|	НалоговыйОстатки.Субконто1
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(
	|			&КонГраница, 
...
Функция ПодготовитьТаблицуЗакрытие44НУ(ТаблицаРеквизиты, Отказ) Экспорт
	...
	// закрытие счета 44 по нормируемым статьям затрат
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("КонГраница",  КонГраница);
	Запрос.УстановитьПараметр("Организация", Реквизиты.Организация);
	...
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат),
	|			Организация = &Организация
	|				И ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРасходов В (&ВидыРасходовНормируемые)) КАК НалоговыйОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	НалоговыйОстатки.Счет,
	|	НалоговыйОстатки.Подразделение,
...
Процедура СформироватьДвиженияПереносУбыткаНаБудущее(Параметры, Реквизиты, ПоддержкаПБУ18, Движения, Отказ) 
	
	ПрибыльВБУ = Ложь;
	
	// проводки в Дт 84.02
	СтрокиУбытка = Параметры.ТаблицаЗакрытие.НайтиСтроки(Новый Структура("СчетДт,СчетКт",
																		ПланыСчетов.Хозрасчетный.УбытокПодлежащийПокрытию,
																		ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиНеЕНВД));
	Если СтрокиУбытка.Количество() = 0 Тогда
		
		// может быть ситуация, когда в БУ - прибыль, а в НУ - убыток
		// тогда проводки будут по Кт 84.01 и сумма НУ в них будет отрицательная
		ПрибыльВБУ = Истина;
	...
	СуммаУбытка = 0;
	Для Каждого СтрокаУбытка Из СтрокиУбытка Цикл
		СуммаУбытка = СуммаУбытка + ?(ПрибыльВБУ, -СтрокаУбытка.СуммаНУ, СтрокаУбытка.СуммаНУ);
	КонецЦикла;
	Если СуммаУбытка <= 0 Тогда // в НУ прибыль, переносить не надо
		Возврат;
	КонецЕсли;
	
	Содержание = НСтр("ru = 'Перенос убытка на будущее'");
	СубконтоУбытка = СоздатьОбновитьУбытокПредыдущегоПериода(КонецГода(Реквизиты.Период)+1, Отказ);
	
	Для Каждого СтрокаУбытка Из СтрокиУбытка Цикл
		СтрокаУбытка.СуммаНУ = 0; // обнуляем, чтобы реформация не закрывала эту сумму
		...
		Если ПланыСчетов.Хозрасчетный.УбыткиПрошлыхЛет.УчетПоПодразделениям Тогда
			Проводка.ПодразделениеДт = СтрокаУбытка.Подразделение;
		КонецЕсли;
		Если ПланыСчетов.Хозрасчетный.УбыткиПрошлыхЛет.УчетПоНаправлениямДеятельности Тогда
			Проводка.НаправлениеДеятельностиДт = СтрокаУбытка.НаправлениеДеятельности;
		КонецЕсли;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СубконтоУбытка);
		
		Проводка.СчетКт                    = ?(ПрибыльВБУ,СтрокаУбытка.СчетДт,СтрокаУбытка.СчетКт);
		Проводка.ПодразделениеКт           = СтрокаУбытка.Подразделение;
		Проводка.НаправлениеДеятельностиКт = СтрокаУбытка.НаправлениеДеятельности;
...
//Рарус Владимир Подрезов 11.05.2017
			Если ЗначениеЗаполнено(Запись.ДоговорКонтрагента) И ЗначениеЗаполнено(Запись.ДоговорКонтрагента.Подразделение) Тогда
				Проводка.ПодразделениеКт	= Запись.ДоговорКонтрагента.Подразделение;
			КонецЕсли;
...
//Рарус Владимир Подрезов 11.05.2017
			Если ЗначениеЗаполнено(Запись.ДоговорКонтрагента) И ЗначениеЗаполнено(Запись.ДоговорКонтрагента.Подразделение) Тогда
				Проводка.ПодразделениеДт	= Запись.ДоговорКонтрагента.Подразделение;
			КонецЕсли;
//Рарус Владимир Подрезов Конец
...
// Процедура вызывается после распределения расчетов с клиентами для записи в регистр сведений "ЗаданияКЗакрытиюМесяца"
//	данных о необходимости пересчета резервов.
// Параметры:
...
//
Процедура ОбновитьРезервыСомнительныхДолговПослеРаспределенияРасчетов(Период, СписокОрганизаций) Экспорт
	...
	|ИЗ
	|	РегистрСведений.УчетнаяПолитикаОрганизаций.СрезПоследних(&Период, Организация В (&СписокОрганизаций)) КАК УчетнаяПолитикаОрганизацийСрезПоследних
	|ГДЕ
	|	УчетнаяПолитикаОрганизацийСрезПоследних.УчетнаяПолитика.ФормироватьРезервыПоСомнительнымДолгамБУ
	|	ИЛИ УчетнаяПолитикаОрганизацийСрезПоследних.УчетнаяПолитика.ФормироватьРезервыПоСомнительнымДолгамНУ";
...
Функция ПодготовитьТаблицуУбытков(ТаблицаРеквизиты, Отказ) Экспорт
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоГода, &КонДата, , Счет В (&Счета99), , Организация = &Организация, , ) КАК ХозрасчетныйОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ПрибыльНУ = Результаты[0].Выгрузить().Итог("ПрибыльНУ");
	
	ТаблицаУбытков      	= Результаты[3].Выгрузить();
