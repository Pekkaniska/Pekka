...
Процедура ПослеЗаписиНаСервере(Форма) Экспорт
	...
	Для каждого ИмяРеквизитаОрганизация Из ОрганизацииДокумента Цикл
		...
		ЗапомнитьПодписиДокументовВФорме(Форма, ОписаниеФормыОбъекта);
	КонецЦикла; 
...
Процедура ЗаполнитьПодписиДокументаВФорме(Форма, ОписаниеФормыОбъекта) 
	...
	СведенияОПодписях = ПодписиДокументов.СведенияОПодписяхДокументов(ОписаниеПолейПодписантов, Организация);
...
Процедура ЗапомнитьПодписиДокументовВФорме(Форма, ОписаниеФормыОбъекта)
	...
	Если СведенияОбОтветственныхРаботникахВФорме <> Неопределено Тогда
		СведенияОбОтветственныхРаботниках = ОбщегоНазначенияКлиентСервер.СкопироватьСоответствие(СведенияОбОтветственныхРаботникахВФорме);
	Иначе		
...
Процедура СохранитьСоставПодписантовОрганизацииВНастройках(Форма, ОписаниеФормыОбъекта)
	...
	НадоЗаписатьНастройки = СформироватьНастройкиПодписейПоОрганизации(НастройкиПоОрганизации, Форма, ОписаниеФормыОбъекта);
	Если Не НадоЗаписатьНастройки Тогда
		Возврат;
	КонецЕсли;
...
Функция СформироватьНастройкиПодписейПоОрганизации(ТекущиеНастройки, Форма, ОписаниеФормыОбъекта)
	...
	ИзмененныеПодписанты = Новый Соответствие;
	...
	ЗначенияДо = СведенияОбОтветственныхРаботниках.Получить(ОписаниеФормыОбъекта.ИмяРеквизитаОрганизация);
	
	// Выберем только измененных.
	Для каждого КлючИЗначение Из ОписаниеПолейПодписантов Цикл
		СтруктураОписанияПодписанта = КлючИЗначение.Значение;
		ФизическоеЛицо = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, СтруктураОписанияПодписанта.ФизическоеЛицо);
		
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЗначенияДо, КлючИЗначение.Ключ) = ФизическоеЛицо Тогда
			Продолжить;
		КонецЕсли;
		
		ИзмененныеПодписанты.Вставить(КлючИЗначение.Ключ, ФизическоеЛицо);
	КонецЦикла;
	
	Если ИзмененныеПодписанты.Количество() = 0 Тогда
		Возврат НастройкиИзменены;
	КонецЕсли;
	
	// Проверим на совпадение с ответственными лицами организаций и с текущими настройками.
	Организация = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(
		Форма, ОписаниеФормыОбъекта.ПрефиксФормыОбъект + ОписаниеФормыОбъекта.ИмяРеквизитаОрганизация);
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.СведенияОбОтветственныхЛицах") Тогда
		МассивПодписантов = ОбщегоНазначения.ВыгрузитьКолонку(ИзмененныеПодписанты, "Ключ", Истина);
		СтрокаСведений = СтрСоединить(МассивПодписантов, ",");
			
		Модуль = ОбщегоНазначения.ОбщийМодуль("СведенияОбОтветственныхЛицах");
		ОтветственныеЛицаОрганизации = Модуль.ОтветственныеЛицаОрганизации(Организация, СтрокаСведений);
	КонецЕсли;
	
	Для каждого ИзмененныйПодписант Из ИзмененныеПодписанты Цикл
		Если Не ЗначениеЗаполнено(ИзмененныйПодписант.Значение)
			ИЛИ ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОтветственныеЛицаОрганизации, ИзмененныйПодписант.Ключ) = ИзмененныйПодписант.Значение Тогда
			// Если измененный подписант или очищен
			// или равен подписанту из ответственных лиц - удаляем настройку пользователя.
			Если ТекущиеНастройки.Свойство(ИзмененныйПодписант.Ключ) Тогда
				ТекущиеНастройки.Удалить(ИзмененныйПодписант.Ключ);
				НастройкиИзменены = Истина;
			КонецЕсли;
		Иначе
			// Иначе добавляем измененного подписанта в настройки
			ТекущиеНастройки.Вставить(ИзмененныйПодписант.Ключ, ИзмененныйПодписант.Значение);
			НастройкиИзменены = Истина;
		КонецЕсли;
	КонецЦикла; 
