...
// Отправляет ранее подготовленную сервисом подготовки машиночитаемых бланков форму на email.
//
// Параметры:
//  ПараметрыОтправки - Структура - стодержит параметры отправки (см. описание процедуры РаботаСПочтовымиСообщениями.ОтправитьПочтовоеСообщение).
...
Функция ПравилаЗагрузкиДляФормы(ПутьКФорме, НастройкиЗагрузки = Неопределено) Экспорт
	...
	Если ОбъектОтчет.Метаданные().Макеты.Найти(ИмяМакета) <> Неопределено Тогда
...
Процедура ОтразитьВЖурналеОтчетов(ПараметрыОтчета) Экспорт
	...
	Попытка
		...
		ОбъектОтправляетсяВКонтролирующиеОрганы = ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОбъектОтправляетсяВКонтролирующиеОрганы(ДокСсылка);		Если ОбъектОтправляетсяВКонтролирующиеОрганы Тогда
		...
		ИначеЕсли ВидКонтролирующегоОргана = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") Тогда
			...
			КонецЕсли;
		ИначеЕсли НеотправляемыйРеглОтчетИмеетВозможностьИзмененияСтатусаОтправкиИзФормыОтчетаВручную(ДокСсылка.ИсточникОтчета) Тогда
			ТекущийСтатус = РегламентированнаяОтчетность.СохраненныйСтатусОтправкиРеглОтчета(ДокСсылка);
			Если ТекущийСтатус <> Неопределено Тогда
				СтатусОтправки = ТекущийСтатус;
			КонецЕсли;
...
Функция ВыборкаОбъектовНеЗаписанныхВРегистрЖурналОтчетовСтатусы()
	...
	Для каждого ТипОбъекта Из МассивТипов Цикл
		...
		ИначеЕсли ПолноеИмяТипа = "Справочник.КомплектыОтчетностиПерсучета" Тогда
			Запрос.Текст = Запрос.Текст + "
...
Функция НеотправляемыйРеглОтчетИмеетВозможностьИзмененияСтатусаОтправкиИзФормыОтчетаВручную(ИмяОбъекта)
	...
	МассивИмен.Добавить("РегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ");
	...
	МассивИмен.Добавить("РегламентированныйОтчетПодтверждениеВидаДеятельности");
