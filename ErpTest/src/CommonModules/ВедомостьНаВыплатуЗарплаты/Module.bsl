...
Процедура ОбработкаПроведения(Ведомость, Отказ) Экспорт
	...
	ВедомостьНаВыплатуЗарплатыВнутренний.ЗарегистрироватьВыплату(Ведомость, Отказ);
...
Процедура РассчитатьЗарплату(Зарплата, ПараметрыРасчета)
	...
	// Сумму группы разносим по строкам группы
	// При необходимости удаляем строки с нулевой или отрицательной суммой к выплате
	...
	Для Каждого Группа Из Группы Цикл
		...
		Если ПараметрыРасчета.ТолькоПоложительные И Группа.КВыплате <= 0 Тогда
		...
		Если ПараметрыРасчета.ПроцентВыплаты <> 100 Тогда
			СуммаГруппы = СуммаГруппы * ПараметрыРасчета.ПроцентВыплаты/100;
