...
// Выполняет проверку заполнения отборов для отчетов, содержащих дополнительные наборы данных по ОС, НМА или физлицам.
//
// Параметры:
...
Процедура ДобавитьОтборДляПоказателяКонтроль(ПараметрыОтчета, КомпоновщикНастроек, ВПользовательскиеНастройки = Истина) Экспорт
	...
	Если ПараметрыОтчета.ПоказательКонтроль Тогда 
		...
		Если ПараметрыОтчета.Свойство("ПоказательКоличество") И ПараметрыОтчета.ПоказательКоличество Тогда
...
Процедура ИнициализацияФормыОтчета(Отчет, РеквизитыФормы, Форма)
	...
	СписокМакетовОформления = БухгалтерскиеОтчетыВызовСервера.ПолучитьСписокМакетовОформления();
