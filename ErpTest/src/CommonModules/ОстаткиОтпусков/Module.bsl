...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.8.41";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("544872c1-1929-419e-9413-2d2b43af4807");
	Обработчик.Процедура = "Документы.ГрафикОтпусков.УстановитьИспользованиеПараметровНаборовСвойств";
	Обработчик.Комментарий = НСтр("ru = 'Установка использования параметров наборов свойств документов ""График отпусков"".'");
	...
	Пока Выборка.Следующий() Цикл 
		...
		Для Каждого СтрокаСотрудника Из ГрафикОтпусков.Сотрудники Цикл 
			Если ЗначениеЗаполнено(СтрокаСотрудника.ВидОтпуска) 
				...
				И СтрокаСотрудника.ВидОтпуска <> ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.Основной") Тогда
...
Процедура РассчитатьКоличествоДнейНакопительногоОтпуска(СотрудникБезПрава, СтрокаВидаОтпуска, РабочиеГода, МассивПравНаОтпуск)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ДанныеПлановогоВремени.Дата КАК Дата,
	               |	ДанныеПлановогоВремени.ЧасыПлан КАК ЧасыПлан,
	               |	ЕСТЬNULL(ДанныеУчетаВремени.Часы, 0) КАК Часы,
	               |	ВЫБОР
	               |		КОГДА ЕСТЬNULL(ДанныеУчетаВремени.УсловияТруда, ЗНАЧЕНИЕ(Справочник.УсловияТруда.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.УсловияТруда.ПустаяСсылка)
	               |			ТОГДА ЛОЖЬ
	               |		ИНАЧЕ ИСТИНА
	               ...
	               |		ПО ДанныеПлановогоВремени.Сотрудник = СотрудникиПериоды.Сотрудник
	               |			И ДанныеПлановогоВремени.Дата >= СотрудникиПериоды.ДатаНачала
	               |			И ДанныеПлановогоВремени.Дата <= СотрудникиПериоды.ДатаОкончания
	               |			И (ДанныеПлановогоВремени.ЧасыПлан > 0)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеУчетаВремени КАК ДанныеУчетаВремени
	               |		ПО ДанныеПлановогоВремени.Сотрудник = ДанныеУчетаВремени.Сотрудник
	               |			И ДанныеПлановогоВремени.Дата = ДанныеУчетаВремени.Дата
	...
	Пока Выборка.СледующийПоЗначениюПоля("Год") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ДнейОтпускаВМесяц = 0 Тогда 
				Продолжить;
			КонецЕсли;
			ОтработаноЗаДень = ?(Выборка.ЧасыПлан = 0, 0, ?(Выборка.Дата >= Дата(2017, 4, 1), 
