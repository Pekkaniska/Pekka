	...
	Попытка
		...
		Если ВидЭД <> Перечисления.ВидыЭД.АктЗаказчик
			...
			Пока Результат.Следующий() Цикл
				...
				ЭтоПрикладнойВидЭД = ОбменСКонтрагентамиСлужебный.ЭтоПрикладнойВидЭД(ВидЭД);
				...
				Если СтрокаВидЭД = Неопределено Тогда
					...
					Если ЭтоПрикладнойВидЭД Тогда
						...
						НастройкиРегламента = ОбменСКонтрагентамиСлужебный.НастройкиРегламентаЭДОПрикладныхФорматов(
							ВидЭД, ВерсияФормата);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ОбменСКонтрагентамиСлужебный.ЭтоПрикладнойВидЭД(ВидЭД) Тогда
...
#Область Справочники_ПрофилиНастроекЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("a110a099-21af-4169-9d30-0175a5aedf0f");
	...
	Обработчик.ЧитаемыеОбъекты = "Справочник.МаршрутыПодписания,"
		+ "Справочник.ПрофилиНастроекЭДО,"
		+ "РегистрСведений.ПодписываемыеВидыЭД";
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.ПрофилиНастроекЭДО.ОбработатьДанныеДляПереходаНаНовуюВерсию";
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбработатьДанныеДляПереходаНаНовуюВерсию

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = Версия;
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "Справочники.СоглашенияОбИспользованииЭД.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("864ded76-c686-4942-ba05-3dd51501bdf3");
	...
	Обработчик.ЧитаемыеОбъекты = "Справочник.СоглашенияОбИспользованииЭД";
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.СоглашенияОбИспользованииЭД.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "Любой";
...
Процедура КлючевыеРеквизитыТекстСостоянияЭДОПриИзменении(ПараметрыПриИзменении) Экспорт
	...
	Если ПараметрыПриИзменении.Организация <> Реквизит.Организация
		...
		|	СоглашенияОбИспользованииЭД.СтатусПодключения КАК СтатусПодключения,
		|	ВЫБОР
		|		КОГДА СоглашенияОбИспользованииЭД.ДоговорКонтрагента = &Договор
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК Порядок
		|ИЗ
		|	Справочник.ПрофилиНастроекЭДО КАК ПрофилиНастроекЭДО
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СоглашенияОбИспользованииЭД КАК СоглашенияОбИспользованииЭД
		|		ПО ПрофилиНастроекЭДО.Ссылка = СоглашенияОбИспользованииЭД.ПрофильНастроекЭДО
		...
		Если Выборка.Следующий() Тогда
			...
			ЭДОНастроен = ЗначениеЗаполнено(Выборка.Соглашение)
				И Выборка.СтатусПодключения = Перечисления.СтатусыУчастниковОбменаЭД.Присоединен;
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.Справочники.ПрофилиНастроекЭДО, Истина);
	Списки.Вставить(Метаданные.Справочники.СоглашенияОбИспользованииЭД, Истина);
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбновитьВидыДокументов
...
#Область Справочники_СоглашенияОбИспользованииЭД_ЗаполнитьВерсииФорматов
...
#Область Справочники_СоглашенияОбИспользованииЭД_ПеренестиСертификатАвторизацииВТЧ
...
#Область Справочники_СоглашенияОбИспользованииЭД_ЗаполнитьВерсииФорматовИсходящихЭДИПакета
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбновитьВерсииФорматовИсходящихЭДИПакета
...
#Область Справочники_СоглашенияОбИспользованииЭД_ЗаполнитьИспользованиеКриптографии
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбновитьВерсиюФорматаИсходящихЭД207_208
...
#Область Справочники_СоглашенияОбИспользованииЭД_ОбновитьВерсиюФорматаИсходящихЭД501_502
...
#Область Справочники_СоглашенияОбИспользованииЭД_УдалитьОтветныеТитулы
...
#Область Справочники_ПрофилиНастроекЭДО_ЗаполнитьРегламентЭДО
...
#Область Справочники_ПрофилиНастроекЭДО_СнятьФлагОтветнойПодписиУСчетаНаОплату
...
#Область Справочники_ЭДПрисоединенныеФайлы_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ИзменяемыеОбъекты = "Документ.ЭлектронныйДокументВходящий,"
		...
		+ "Документ.ЭлектронныйДокументИсходящий,"
		+ "РегистрСведений.СостоянияЭД";
	Обработчик.БлокируемыеОбъекты = "Справочник.ЭДПрисоединенныеФайлы,"
		+ "Документ.ЭлектронныйДокументВходящий,"
		+ "Документ.ЭлектронныйДокументИсходящий";
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений";
	НоваяСтрока.Порядок = "После";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "РаботаСФайламиСлужебныйВызовСервера.ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы";
	НоваяСтрока.Порядок = "После";
...
#Область РегистрыСведений_КонтрольОтраженияВУчетеЭДО_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ПроцедураПроверки = "";
	Обработчик.ЧитаемыеОбъекты = "РегистрСведений.КонтрольОтраженияВУчетеЭДО,"
		+ "Справочник.ЭДПрисоединенныеФайлы,"
		+ "Документ.ЭлектронныйДокументВходящий";
...
Процедура УстановитьСоответствиеНоменклатуры(Знач НоменклатураКонтрагента, Знач НоменклатураИБ = Неопределено) Экспорт
	...
	ОбменСКонтрагентамиСлужебный.УстановитьСоответствиеНоменклатуры(НоменклатураКонтрагента, НоменклатураИБ);
...
//
// Параметры:
//  Форма - УправляемаяФорма - форма элемента номенклатуры.
//
