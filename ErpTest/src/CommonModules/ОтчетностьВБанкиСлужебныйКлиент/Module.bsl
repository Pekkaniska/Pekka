...
Процедура ПродолжитьОтправкуОтчетаПослеПодключенияИнтернетПоддержки(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.СоглашениеПринято Тогда
		ПолучитьАдресВнешнейКомпоненты(ДополнительныеПараметры);
	Иначе
...
Процедура ПолучитьАдресВнешнейКомпоненты(ДополнительныеПараметры)
	
	// Берется внешняя компонента из кэша, если она там есть
	ПодключаемыйМодуль = ВнешняяКомпонентаИзКэша("VPNKeyTLS");
	
	Если ПодключаемыйМодуль <> Неопределено Тогда
		ПослеПодключенияВКСбербанка(ПодключаемыйМодуль, ДополнительныеПараметры);
		Возврат;
	КонецЕсли;

	АдресВК = АдресВК("VPNKeyTLS");
	
	Если АдресВК = Неопределено Тогда
		ЗагрузитьВнешнююКомпонентуЧерезИнтернет(ДополнительныеПараметры);
	Иначе
		ПодключитьВнешнююКомпонентуСбербанка(АдресВК, ДополнительныеПараметры);
		ОбновитьВнешнююКомпоненту("VPNKeyTLS");
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗагрузитьВнешнююКомпонентуЧерезИнтернет(ДополнительныеПараметры) Экспорт
	...
	Результат = ОтчетностьВБанкиСлужебныйВызовСервера.СкачатьВнешнююКомпоненту(
		ДополнительныеПараметры.Банк, ПараметрыКлиента);
	
	Если Результат.Статус = "Выполняется" Тогда
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(Неопределено);
		ПараметрыОжидания.ВыводитьСообщения = Истина;
		Оповещение = Новый ОписаниеОповещения("ПослеЗагрузкиВнешнейКомпоненты", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ПослеЗагрузкиВнешнейКомпоненты(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПодключитьВнешнююКомпонентуСбербанка(АдресВК, ДополнительныеПараметры)
	
	Оповещение = Новый ОписаниеОповещения("ПослеПодключенияВКСбербанка", ЭтотОбъект, ДополнительныеПараметры);
	ПодключитьВнешнююКомпонентуПоСсылке(Оповещение, АдресВК, "VPNKeyTLS");
...
Процедура ПослеПолученияДанныхПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ОперацияВыполненаУспешно = ОтчетностьВБанкиСлужебныйВызовСервера.ОтправитьДанныеВБанк(
		...
		ДополнительныеПараметры.ИмяФайла, ЭлектроннаяПодписьДвоичныеДанные, ПараметрыКлиента, ДополнительныеПараметры.Отчет);
...
Функция ПодсистемаДополнительныеВнешниеКомпонентыСуществует() Экспорт
	Возврат ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСБанками.ДополнительныеВнешниеКомпоненты");
КонецФункции

Функция ВнешняяКомпонентаИзКэша(ИмяМодуля) Экспорт
	
	Если ПодсистемаДополнительныеВнешниеКомпонентыСуществует() Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДополнительныеВнешниеКомпонентыКлиент");
		Возврат Модуль.ВнешняяКомпонентаИзКэша(ИмяМодуля);
	Иначе
...
Процедура ПодключитьВнешнююКомпонентуПоСсылке(Оповещение, АдресВК, ИмяМодуля) Экспорт
	...
	Если ПодсистемаДополнительныеВнешниеКомпонентыСуществует() Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДополнительныеВнешниеКомпонентыКлиент");
		Модуль.ПодключитьВнешнююКомпонентуПоСсылке(Оповещение, АдресВК, ИмяМодуля);
	КонецЕсли;
...
Функция АдресВК(Знач ИмяКомпоненты, Версия = Неопределено) Экспорт
	...
	Если ПодсистемаДополнительныеВнешниеКомпонентыСуществует() Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДополнительныеВнешниеКомпонентыВызовСервера");
		Возврат Модуль.АдресВК(ИмяКомпоненты, Версия);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОбновитьВнешнююКомпоненту(Знач ИмяМодуля) Экспорт
	...
	Если ПодсистемаДополнительныеВнешниеКомпонентыСуществует() Тогда
		Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ДополнительныеВнешниеКомпонентыВызовСервера");
		Модуль.ОбновитьВнешнююКомпоненту(ИмяМодуля);
	КонецЕсли;
