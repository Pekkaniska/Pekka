...
Функция ПолучитьВерсиюЧислом(СтрокаВерсии) Экспорт
	...
	ПервоеЧисло = 0; ВтороеЧисло = 0; ТретьеЧисло = 0; ЧетвертоеЧисло = 0;
	
	Если ТипЗнч(СтрокаВерсии) = Тип("Строка") И ЗначениеЗаполнено(СтрокаВерсии) Тогда
	...
	ЧислоСтрокаВерсии = ПервоеЧисло*1000000000 + ВтороеЧисло*1000000 + ТретьеЧисло*1000 + ЧетвертоеЧисло;
...
// 
Процедура ВыполнитьМетодКонфигурацииАЛКО(Знач ИмяМетода, Знач Параметры = Неопределено, ВерсияБСПЧислом = Неопределено) Экспорт
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ru='Не найден общий модуль " + ИмяОбщегоМодуляВычислитьВыполнить + "!'");
		Сообщение.Сообщить();
...
//  в порядке расположения элементов массива.
//
Процедура ВыполнитьМетодОбъектаАЛКО(Знач Объект, Знач ИмяМетода, Знач Параметры = Неопределено, ВерсияБСПЧислом = Неопределено) Экспорт
...
//
Процедура ВыполнитьВБезопасномРежимеАЛКО(Знач Алгоритм, Знач Параметры = Неопределено, ВерсияБСПЧислом = Неопределено) Экспорт
...
//
Функция ВычислитьАЛКО(Знач Выражение, Знач Параметры = Неопределено, ВерсияБСПЧислом = Неопределено) Экспорт
...
//		Строка.
//
Функция ПолучитьИмяФормыОбъектаЗначенияЭлементаФормы(Форма, ИмяЭлементаСсылки, ЗначениеСсылки = Неопределено) Экспорт
...
Функция ОпределитьКонтрагентаАЛКО(ИНН, КПП, ПолноеИлиКраткоеНаименование = Неопределено) Экспорт
	...
	Контрагент = ОпределитьКонтрагентаИзЗапросаАЛКО(РезультатЗапроса, ИНН, КПП, ПолноеИлиКраткоеНаименование, 
												"Контрагент", ЕстьСтранаРегистрации);
	
	Возврат Контрагент;

КонецФункции

Функция ОпределитьКонтрагентаИзЛицензийПоставщиковАЛКО(ИНН, КПП, ПолноеИлиКраткоеНаименование = Неопределено, 
	...
	Если ЗначениеЗаполнено(ТипАлкоголя) и (ТипЗнч(ТипАлкоголя) = Тип("Строка")) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПеречислениеВидыЛицензийНаПродажуАлкоголяПиво) Тогда
			...
			УсловиеВидЛицензии = СтрЗаменить(УсловиеВидЛицензии, "&ВидыЛицензийНаПродажуАлкоголя", 
											СтрокаПеречислениеВидыЛицензийНаПродажуАлкоголяПиво);			
		КонецЕсли; 
	...
	Если ЗначениеЗаполнено(СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции) Тогда
		...
		ВыборКонтрагента = "ЛицензииПоставщиковАлкогольнойПродукции." + ИзмерениеПоставщик;
		УсловиеИНН = "ЛицензииПоставщиковАлкогольнойПродукции." + ИзмерениеПоставщик + ".ИНН";
		
		УсловиеНалоговыйНомер = ?(ЕстьНалоговыйНомер, "&УсловиеНалоговыйНомер2 = &ИНН", "Ложь");		
		УсловиеНалоговыйНомер2 = "ЛицензииПоставщиковАлкогольнойПродукции." + ИзмерениеПоставщик + ".НалоговыйНомер";
...
Функция ТекстЗапросаВыбораКонтрагентаАЛКО(ОсновнаяТаблица, НужныЛицензии = Истина, 
											ТипАлкоголя = Неопределено, НайтиКонтрагента = Ложь) Экспорт
	...
	Если НужныЛицензии ИЛИ ( ЗначениеЗаполнено(ТипАлкоголя) и (ТипЗнч(ТипАлкоголя) = Тип("Строка")) ) Тогда
		РегламентированнаяОтчетностьПереопределяемый.СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции(
													СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции);	
	КонецЕсли; 
	...
	Если ЗначениеЗаполнено(СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции) Тогда
		...
		Если ЗначениеЗаполнено(ТипАлкоголя) и (ТипЗнч(ТипАлкоголя) = Тип("Строка")) Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПеречислениеВидыЛицензийНаПродажуАлкоголяПиво) Тогда
				...
				Если ВРег(ТипАлкоголя) = ВРег("Слабоалкогольная") 
					...
					УсловиеВидЛицензии = ПсевдонимТаблицаКонтрагентов + ".ВидЛицензии = ЗНАЧЕНИЕ(&ВидыЛицензийНаПродажуАлкоголя)";
				Иначе
					УсловиеВидЛицензии = ПсевдонимТаблицаКонтрагентов + ".ВидЛицензии <> ЗНАЧЕНИЕ(&ВидыЛицензийНаПродажуАлкоголя)";
				КонецЕсли;	
		...
		КонтрагентСсылка = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеПоставщик + ".Ссылка";
		
		КонтрагентНаименованиеПолное = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеПоставщик 
										+ "." + РеквизитНаименованиеПолное;
		
		Если ЕстьСтранаРегистрации Тогда
		    КонтрагентСтранаРегистрации = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеПоставщик + ".СтранаРегистрации";
		Иначе
			КонтрагентСтранаРегистрации = """""";
		КонецЕсли; 
		
		КонтрагентИНН = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеПоставщик + ".ИНН";
		
		Если ЕстьНалоговыйНомер Тогда
		    КонтрагентНалоговыйНомер = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеПоставщик + ".НалоговыйНомер";
		КонецЕсли;
		
		КонтрагентКПП = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеПоставщик + ".КПП";
		...
		КонтрагентСерияНомер = ПсевдонимТаблицаКонтрагентов + "." + ИзмерениеСерияНомер;
	...
	Иначе
		...
		КонтрагентНаименованиеПолное = ПсевдонимТаблицаКонтрагентов + "." + РеквизитНаименованиеПолное;
		
		Если ЕстьСтранаРегистрации Тогда
		    КонтрагентСтранаРегистрации = ПсевдонимТаблицаКонтрагентов + ".СтранаРегистрации";
		Иначе
			КонтрагентСтранаРегистрации = """""";
		КонецЕсли;
		...
		Если ЕстьНалоговыйНомер Тогда
		    КонтрагентНалоговыйНомер = ПсевдонимТаблицаКонтрагентов + ".НалоговыйНомер";
		КонецЕсли;
		...
		УсловиеКонтрагент = ПсевдонимТаблицаКонтрагентов + "." + РеквизитНаименованиеПолное + " ЕСТЬ НЕ NULL";
	...
	Если НЕ НужныЛицензии Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "КАК СтранаРегистрации,", "КАК СтранаРегистрации");
		
	КонецЕсли;
	
	Если НужныЛицензии ИЛИ (ТаблицаКонтрагентов = "Справочник.Контрагенты") Тогда
...
Функция ТекстЗапросаВыбораПроизводителяИмпортераАЛКО(ВставитьКодПродукции, ТипАлкоголя = Неопределено) Экспорт
...
Функция ОпределитьПроизводителяИмпортераАЛКО(ИНН, КПП, ПолноеНаименование, КодПродукции, 
															ТипАлкоголя = Неопределено) Экспорт
	...
	ПроизводительИмпортер = ОпределитьКонтрагентаИзЗапросаАЛКО(РезультатЗапроса, ИНН, КПП, ПолноеНаименование,
																		"Контрагент", ЕстьСтранаРегистрации);
...
Функция ИзвлечьСтруктуруXMLАЛКО(ВерсияФормата, ИмяФормыОбъекта, ОбъектЭтогоОтч = Неопределено) Экспорт
...
Функция ПолучитьПустуюСтруктуруАдреса() Экспорт
	...
	СтруктураАдреса.Вставить("ДополнительныеКоды",       Новый Структура);
	
	СтруктураАдреса.Вставить("КодСтраны",       "643");
	СтруктураАдреса.Вставить("Страна",       	  "РОССИЯ");
	СтруктураАдреса.Вставить("Индекс",          "");
	СтруктураАдреса.Вставить("КодРегиона",      "00");
	...
	СтруктураАдреса.Вставить("АдресXML", "");
	
	Представление = "";
	
	ПустойАдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(
			СтруктураАдреса, Представление, Перечисления.ТипыКонтактнойИнформации.Адрес);
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(ПустойАдресXML) Тогда
		...
		СтруктураАдреса.ПредставлениеАдреса = ПредставлениеАдреса;
...
Функция ПолучитьПустуюСтруктуруСведенийОбОП() Экспорт
	...
	СтруктураСведенияОбОП = ПолучитьПустуюСтруктуруАдреса();
...
Функция ПолучитьСтруктуруАдресаИзСтандартногоПредставленияИлиXML(знач Адрес) Экспорт
	...
	АдресВФорматеОтчета = ПолучитьПустуюСтруктуруАдреса();
	...
	Исключение
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не найден общий модуль " + ИмяОбщегоМодуляРаботаСАдресами + "!'");
		Сообщение.Сообщить();
	...
	СтруктураАдресаСМассивами = ОбщийМодульРаботаСАдресами.СведенияОбАдресе(АдресXML, ДополнительныеПараметры);
	...
	Если СтруктураАдресаСМассивами.Свойство("Здание") Тогда
		...
		Если СтруктураАдресаСМассивами.Здание.Количество() > 0 Тогда
			СтрокаДома = СтрокаДома + 
						?(ЗначениеЗаполнено(СтруктураАдресаСМассивами.Здание.ТипЗдания), Строка(СтруктураАдресаСМассивами.Здание.ТипЗдания) + " ", "") 
						+ Строка(СтруктураАдресаСМассивами.Здание.Номер);
		КонецЕсли;	
	...
	Если СтруктураАдресаСМассивами.Свойство("Корпуса") Тогда
		...
		Для Каждого ЭлМассива Из МассивКорпусов Цикл
			...
			Если СокрЛП(ЭлМассива.ТипКорпуса) <> "Литера"  Тогда
				...
				ТекКорпус = ?(ЗначениеЗаполнено(ЭлМассива.ТипКорпуса), Строка(ЭлМассива.ТипКорпуса) + " ", "") + Строка(ЭлМассива.Номер);				
	...
	Если СтруктураАдресаСМассивами.Свойство("Помещения") Тогда
		...
		Для Каждого ЭлМассива Из МассивПомещений Цикл
			...
			ТекПомещение = ?(ЗначениеЗаполнено(ЭлМассива.ТипПомещения), Строка(ЭлМассива.ТипПомещения) + " ", "") + Строка(ЭлМассива.Номер);			
	...
	// Добавим сокращения.
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.Район) Тогда
		
		Если СтруктураАдресаСМассивами.Свойство("РайонСокращение") Тогда
		
			АдресВФорматеОтчета.Район = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.РайонСокращение),
											Строка(АдресВФорматеОтчета.Район),
											Строка(АдресВФорматеОтчета.Район) + " " + Строка(СтруктураАдресаСМассивами.РайонСокращение));	
	...
	Если НЕ ЗначениеЗаполнено(АдресВФорматеОтчета.Город) Тогда
		
		// Поскольку могли по ошибке не указать город для "обычного" региона,
		// проверяем конкретные регионы.
		Если 	АдресВФорматеОтчета.КодРегиона = "77"		// Москва
			или АдресВФорматеОтчета.КодРегиона = "78" 		// Санкт-Петербург
			или АдресВФорматеОтчета.КодРегиона = "92"		// Севастополь
			или АдресВФорматеОтчета.КодРегиона = "99" 		// Байконур и иные территории
			Тогда
		    АдресВФорматеОтчета.Город = РегламентированнаяОтчетностьВызовСервера.ПолучитьНазваниеРегионаПоКоду(АдресВФорматеОтчета.КодРегиона);		
		КонецЕсли;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.Город) Тогда
		
		Если СтруктураАдресаСМассивами.Свойство("ГородСокращение") Тогда
			
			АдресВФорматеОтчета.Город = ?( НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.ГородСокращение),
										Строка(АдресВФорматеОтчета.Город),
										Строка(АдресВФорматеОтчета.Город) + " " + Строка(СтруктураАдресаСМассивами.ГородСокращение));
		КонецЕсли;
		 
	КонецЕсли;
									
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.НаселенныйПункт) Тогда
		
		Если СтруктураАдресаСМассивами.Свойство("НаселенныйПунктСокращение") Тогда
			
			АдресВФорматеОтчета.НаселенныйПункт = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.НаселенныйПунктСокращение),
										Строка(АдресВФорматеОтчета.НаселенныйПункт),
										Строка(АдресВФорматеОтчета.НаселенныйПункт) + " " + Строка(СтруктураАдресаСМассивами.НаселенныйПунктСокращение));
		КонецЕсли;								
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(АдресВФорматеОтчета.Улица) Тогда
		
		Если СтруктураАдресаСМассивами.Свойство("УлицаСокращение") Тогда
			
			АдресВФорматеОтчета.Улица = ?(НЕ ЗначениеЗаполнено(СтруктураАдресаСМассивами.УлицаСокращение),
										Строка(АдресВФорматеОтчета.Улица),
										Строка(АдресВФорматеОтчета.Улица) + " " + Строка(СтруктураАдресаСМассивами.УлицаСокращение));
		КонецЕсли;
...
Функция СтруктураПредопределенныхВидовКонтактнойИнформацииАдреса() Экспорт
	...
	Для каждого СтрокаЗапроса Из РезЗапросаПредопределенных Цикл
		...
		ПредопределенныйЭлементСправочника = ПредопределенноеЗначение(
									"Справочник.ВидыКонтактнойИнформации." + СтрокаЗапроса.ИмяПредопределенныхДанных);
		...
		Если СтрНайти(ИмяПредопределенныхДанныхВРег, ВРег("Адрес")) > 0 Тогда
			...
			Если ЕстьРеквизитТолькоНациональныйАдрес и ЗначениеЗаполнено(ПредопределенныйЭлементСправочника.ТолькоНациональныйАдрес) Тогда
...
Функция ОпределитьВидыКонтактнойИнформацииДляВводаАдреса() Экспорт
	...
	СписокПочтовыхВидовКонтактнойИнформации =  СтруктураДоступныхВидовКонтактнойИнформации.Почтовый;
	...
	Если НЕ ИмяВидКонтактнойИнформации = Неопределено Тогда
		...
		ВидКонтактнойИнформации = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации." + ИмяВидКонтактнойИнформации);
...
Функция ПолучитьАдресXMLОбъекта(знач Ссылка, ПредставлениеАдреса = Неопределено, 
								...
								знач ДатаАдреса = Неопределено) Экспорт
		...
		Для каждого ЭлементСпискаВидов Из СписокИменВидовКонтактнойИнформации Цикл
			...
			Если ТипЗнч(ТекущееИмяВидаКонтактнойИнформации) = Тип("Строка") 
				...
				Если СписокДоступныхАдресныхВидов.НайтиПоЗначению(ТекущееИмяВидаКонтактнойИнформации) = Неопределено Тогда
					Продолжить;
				Иначе		 
			    	СписокРабочихВидовКонтактнойИнформации.Добавить(ПредопределенноеЗначение(
									"Справочник.ВидыКонтактнойИнформации." + ТекущееИмяВидаКонтактнойИнформации));
				КонецЕсли;
			КонецЕсли;
	...
	Если (СтрокаТаблицаКонтактнойИнформации = Неопределено) И ВыдаватьЛюбойЕслиНетНужногоВида Тогда
		...
		Если ТаблицаКонтактнойИнформации.Количество() > 0 Тогда
			...
			Для Каждого СтрокаТаблицы Из ТаблицаКонтактнойИнформации Цикл
				...
				Если ВидКонтактнойИнформации.Предопределенный Тогда
					...
					НаименованиеВидаКонтактнойИнформации = ВидКонтактнойИнформации.ИмяПредопределенныхДанных;
...
Процедура ОбновитьМестоОсуществленияДеятельности(Форма, Результат) Экспорт
	...
	ОбрабатываемыеДанные.Вставить("КодСтраны", "643");
	...
	Форма.Модифицированность = Форма.Модифицированность или (СтароеМестоДеятельности <> СокрЛП(ПредставлениеАдреса));
...
Функция КонтрольноеЧислоИНН(ИНН) Экспорт
	...
	Если СтрДлина(СокрЛП(ИНН)) = 10 Тогда
		...
		КонтрольноеЧисло = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(КонтрольнаяСумма);
	...
	ИначеЕсли СтрДлина(СокрЛП(ИНН)) = 12 Тогда
		...
		КонтрольноеЧисло = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(КонтрольнаяСумма11)
		                 + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(КонтрольнаяСумма12);
...
Функция ЭтоТочноРоссийскийКонтрагент(Контрагент, ЕстьСтранаРегистрацииВКонфигурации = Неопределено, 
									СтранаРоссия = Неопределено, ИНН, КПП) Экспорт 
	...
	Если (РоссийскийКонтрагент = Неопределено) и (ДлинаИНН = 12) Тогда
		...
		Если ВСтрокеВсеЦифры(ИНН) Тогда
			...
			РоссийскийКонтрагент = (КонтрольноеЧислоИНН = Прав(ИНН, 2));
			
		Иначе
			// Неверный ИНН, по умолчанию считаем российским.
			РоссийскийКонтрагент = Истина;
		КонецЕсли;
		
	КонецЕсли;
	
	Если (РоссийскийКонтрагент = Неопределено) и (ДлинаКПП = 9) Тогда
		// Считаем как неверный ИНН российского контрагента.
		РоссийскийКонтрагент = Истина;
	КонецЕсли;
	
	Если (РоссийскийКонтрагент = Неопределено) и (НЕ (ДлинаИНН = 12 или ДлинаИНН = 10)) Тогда
		РоссийскийКонтрагент = Ложь;
	КонецЕсли;
	
	Возврат РоссийскийКонтрагент;

КонецФункции

Функция ПроверкаЗаполненияЛицензийВСтрокахТаблицАЛКО(ДокументСсылка, ТаблицаСообщенийОбОшибках, СтатусПроверки,
									 НетПроблемИННКПП, СтруктураСведенийЛицензий, РоссийскийКонтрагент) Экспорт

	Перем ИмяРаздела, СтрВКакомРазделе, НомерЛиста, НомерСтрокиСтроковый, 
		ИмяСерияЛицензии, СерияЛицензии, ИмяНомерЛицензии, НомерЛицензии,
		ИмяДатаВыдачиЛицензии, ДатаВыдачиЛицензии, ИмяДатаОкончанияЛицензии, ДатаОкончанияЛицензии,
	    ИмяКемВыданаЛицензия, КемВыданаЛицензия, РодительскийПадежЧьяЛицензия;
	...
	Если ТипЗнч(РоссийскийКонтрагент) = Тип("Булево") Тогда
	    МожетБытьЛицензия = РоссийскийКонтрагент;
	Иначе	
	...
	ИмяПоказателяСерияЛицензии 			= ИмяСерияЛицензии + "_" + НомерСтрокиСтроковый;	
	ИмяПоказателяНомерЛицензии 			= ИмяНомерЛицензии + "_" + НомерСтрокиСтроковый;	
    ИмяПоказателяДатаВыдачиЛицензии 	= ИмяДатаВыдачиЛицензии + "_" + НомерСтрокиСтроковый;	
	ИмяПоказателяДатаОкончанияЛицензии 	= ИмяДатаОкончанияЛицензии + "_" + НомерСтрокиСтроковый;	
    ИмяПоказателяКемВыданаЛицензия 		= ИмяКемВыданаЛицензия + "_" + НомерСтрокиСтроковый;
	
	Если ЗначениеЗаполнено(СерияЛицензии + НомерЛицензии) Тогда
	...
	Если ЛицензияЗаполнена Тогда
		...
		Если МожетБытьЛицензия Тогда
			...
			Если НЕ (ИмяПоказателяНеЗаполнено = Неопределено) Тогда
				...
				ТекстОписанияОшибки = СтрВКакомРазделе + " декларации, на листе " + НомерЛистаСтроковый 
										...
										+ ", не все поля сведений о лицензии " 
				...
				СообщитьОбОшибкеЗаполнения(ДокументСсылка, ТаблицаСообщенийОбОшибках, СтатусПроверки, СтруктураОшибки);
				
			КонецЕсли;
			
			// Проверка на минимальную длину Кем Выдана.
			Если ЗначениеЗаполнено(КемВыданаЛицензия) И (СтрДлина(КемВыданаЛицензия) < 3) Тогда
		...
		Иначе
			...
			ТекстОписанияОшибки = СтрВКакомРазделе + " декларации, на листе " + НомерЛистаСтроковый 
										+ ", в строке " + НомерСтрокиСтроковый  
										+ ", заполнены сведения о лицензии для "
...
Функция ПроверкаИННКППвСтрокахТаблицАЛКО(ДокументСсылка, ТаблицаСообщенийОбОшибках,
	...
	Перем ИмяРаздела, СтрВКакомРазделе, НомерЛиста, НомерСтрокиСтроковый, 
		...
		ИмяПоказателяНаименования, Наименование,
		МожетБытьПустойКПП, РоссийскийТолькоЮЛ, РоссийскийТолькоФЛ, КПП56СимволыМогутБытьБуквами;
	...
	Наименование = ?(ЗначениеЗаполнено(Наименование), СокрЛП(Строка(Наименование)), "");
	...
	КПП56СимволыМогутБытьБуквами = ?(КПП56СимволыМогутБытьБуквами = Неопределено, Ложь, КПП56СимволыМогутБытьБуквами);
	...
	Если СтрДлина(Наименование) < 3 Тогда
		...
		ТекстОписанияОшибки = СтрВКакомРазделе + " декларации, на листе " + НомерЛистаСтроковый 
										...
										+ ", наименование " + РодПадежЧейИННКПП + " содержит менее 3-х символов!";
		...
		Если ДлинаКПП > 0 Тогда
			...
			Иначе
				...
				Если НЕ ВСтрокеВсеЦифры(КПП) Тогда
					...
					ТекстОписанияОшибки = СтрВКакомРазделе + " декларации, на листе " + НомерЛистаСтроковый 
											+ ", в строке " + НомерСтрокиСтроковый + ВТаблице 
											+ ", строка КПП " + РодПадежЧейИННКПП + " " + КПП + " должна состоять только из цифр!";
					...
					СтруктураОшибки.Вставить("Показатель", ИмяПоказателяКПП);
					СтруктураОшибки.Вставить("ЛистРаздела", НомерЛиста);
					СтруктураОшибки.Вставить("ТипОшибки", "НедопустимыеСимволыКППТаблица2");
...
//		Булево						Истина, если ошибки не найдены.
//
Функция ПроверитьЗаполнениеТитульный(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, 
										ПараметрыВыгрузки, КПП56СимволыМогутБытьБуквами = Ложь) Экспорт
	...
	Если ПараметрыВыгрузки.ЭтоПБОЮЛ Тогда
		...
		Если НЕ ВСтрокеВсеЦифры(ИННФЛ) Тогда
		...
		Если СтрДлина(ИННФЛ) = 12 Тогда
			КонтрольноеЧислоИНН = КонтрольноеЧислоИНН(ИННФЛ);
		...
		Если СтрДлина(КПП) = 9 Тогда
			...
			Иначе
				...
				Если НЕ ВСтрокеВсеЦифры(КПП) Тогда
					...
					ТекстОписанияОшибки = "На Титульном листе неверно задан КПП организации! КПП должен состоять только из цифр.";
					...
					СтруктураОшибки.Вставить("Показатель", "Организация");
					СтруктураОшибки.Вставить("ЛистРаздела", Неопределено);
					СтруктураОшибки.Вставить("ТипОшибки", "НедопустимыеСимволыКППДекларант2");
	...
	// Место деятельности.
	Если    (ПараметрыВыгрузки.Свойство("ИндексМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.ИндексМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("КодРегионМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.КодРегионМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("РайонМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.РайонМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("ГородМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.ГородМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("НаселПунктМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.НаселПунктМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("УлицаМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.УлицаМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("ДомМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.ДомМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("КорпусМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.КорпусМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("ЛитераМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.ЛитераМестаОсуществления))
		или (ПараметрыВыгрузки.Свойство("КвартМестаОсуществления") и НЕ ПустаяСтрока(ПараметрыВыгрузки.КвартМестаОсуществления))
		Тогда
	
		Если ПустаяСтрока(ПараметрыВыгрузки.КодРегионМестаОсуществления) Тогда
			ТекстОписанияОшибки = "На Титульном листе в адресе места осуществления деятельности организации не указан код региона!";
			
			СтруктураОшибки = Новый Структура;
			СтруктураОшибки.Вставить("ТекстОписанияОшибки", ТекстОписанияОшибки);
			СтруктураОшибки.Вставить("Раздел", "Титульный");
			СтруктураОшибки.Вставить("Показатель", "МестоОсуществленияДеятельности");
			СтруктураОшибки.Вставить("ЛистРаздела", Неопределено);
			СтруктураОшибки.Вставить("ТипОшибки", "НеУказанКодРегионаДекларант");
			
			СообщитьОбОшибкеЗаполнения(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, СтруктураОшибки);
					
		ИначеЕсли СтрДлина(СокрЛП(ПараметрыВыгрузки.КодРегионМестаОсуществления))<> 2 Тогда
			ТекстОписанияОшибки = "На Титульном листе в адресе места осуществления деятельности организации неверно указан код региона!";
		...
		КонецЕсли;
	Иначе
		// Если место деятельности не заполнено, очищаем код страны - он может быть присвоен.
		Если (ПараметрыВыгрузки.Свойство("КодСтраныМестаОсуществления") 
			и НЕ ПустаяСтрока(ПараметрыВыгрузки.КодСтраныМестаОсуществления)) Тогда
		    ПараметрыВыгрузки.КодСтраныМестаОсуществления = "";		
		КонецЕсли; 
	КонецЕсли;
	
	// Телефон.
	Если ПустаяСтрока(ПараметрыВыгрузки.ТелОрг) Тогда
		ТекстОписанияОшибки = "На Титульном листе в данных организации не указан телефон организации!";
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.EmailОтпр) Тогда
		ТекстОписанияОшибки = "На Титульном листе в данных организации не указан Email организации!";
		...
		СообщитьОбОшибкеЗаполнения(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, СтруктураОшибки);
	Иначе
		ПравильныйАдрес = ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(
																			ПараметрыВыгрузки.EmailОтпр);
		Если НЕ ПравильныйАдрес Тогда
		
			ТекстОписанияОшибки = "На Титульном листе в данных организации указан неверный Email организации!";
...
//		Число	- 0 - нет ошибок
//      		- 1 - есть ошибки
//				- 2 - нет ошибок, но есть сообщения
//				- 3 - непредвиденная ошибка (в норме не должно возвращаться никогда).
...
Функция УдалитьИзВременногоХранилищаАдресаАЛКО(Форма, ОсновнойАдрес, ВспомогательныйАдрес, АдресДвоичныхДанных) Экспорт
...
Процедура ОбработатьУсловныеЭлементыАЛКО(Знач ПараметрыВыгрузки, Узел, 
						ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, ВерсияБСПЧислом = Неопределено) Экспорт
	...
	Для Инд = 1 По КоличествоСтрок Цикл
		...
		Если НЕ ПустаяСтрока(ТекСтр.Условие) Тогда
			Если НЕ УсловиеВыполненоАЛКО(ПараметрыВыгрузки, ТекСтр.Условие, ,
							ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, ВерсияБСПЧислом) Тогда
		...
		ОбработатьУсловныеЭлементыАЛКО(ПараметрыВыгрузки, ТекСтр,
...
Процедура ОтсечьНезаполненныеНеобязательныеУзлыАЛКО(Узел) Экспорт
	...
	Если ТипЗнч(Узел) <> Тип("ДеревоЗначений") Тогда
		...
		Если (Найти(Узел.Обязательность, "Н") <> 0 ИЛИ СтрНайти(Узел.Обязательность, "H") <> 0) И УзелПустАЛКО(Узел) Тогда // учтем оба варианта: кириллицу и латиницу
		...
		ИначеЕсли (Найти(Узел.Обязательность, "М") <> 0 ИЛИ СтрНайти(Узел.Обязательность, "M") <> 0) // учтем оба варианта: кириллицу и латиницу
				И УзелПустАЛКО(Узел) 
				И ?(Найти(Узел.Обязательность, "О") <> 0 ИЛИ СтрНайти(Узел.Обязательность, "O") <> 0, 
...
Процедура УстановитьЗначениеЭлементаАЛКО(УзелРодитель, ИмяЭлемента, ЗначениеЭлемента) Экспорт
...
Функция ПроверитьДанныеАЛКО(Узел, ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки) Экспорт
...
// Возвращает общий хэш SHA256 переданных в списке данных.
//
// Параметры:
//
...
Функция ХешИдКодПродПроцСпирта(КодПродукции = Неопределено, ПроцентСпирта = Неопределено) Экспорт
...
Функция ХешИдТранспортногоСредства(ТипТС = Неопределено, РегистрационныйНомерТС = Неопределено, ГрузоподъемностьТС = Неопределено) Экспорт
...
Процедура ЗаполнитьТитульныйАЛКО(Форма) Экспорт
	...
	Форма.Элементы.ГлавныйБухгалтер.Видимость = НЕ (Форма.ЭтоПБОЮЛ ИЛИ (Форма.НомерАлкоОтчета = 0) ИЛИ (Форма.НомерАлкоОтчета >= 1000));
	
	Форма.Элементы.КПП.Видимость = НЕ (Форма.ЭтоПБОЮЛ ИЛИ (Форма.НомерАлкоОтчета = 1000));
	...
	Если НЕ Форма.Элементы.Найти("МестоОсуществленияДеятельности") = Неопределено Тогда
		...
		Если НЕ Форма.ДополнительныеДанные.Титульный.Свойство("АдресМестаДеятельности") Тогда
			...
			Если ЗначениеЗаполнено(Форма.СтруктураДанныхТитульный.МестоДеятельности) Тогда
			    ...
			    АдресМестаДеятельности = ПолучитьСтруктуруАдресаИзСтандартногоПредставленияИлиXML(
													Форма.СтруктураДанныхТитульный.МестоДеятельности);
			Иначе
				АдресМестаДеятельности = ПолучитьПустуюСтруктуруАдреса();
			КонецЕсли; 
	...
	Если Форма.НомерАлкоОтчета = 0 Тогда
		...
		РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗагрузитьРекивизитыЗаявленияОКорректировкиАЛКО(
						Форма.СтруктураРеквизитовФормы, Форма.СтруктураДанныхТитульный, ПринудительноЗаполнить);
		...
		Если ЗначениеЗаполнено(НомерПриложения) Тогда
			...
			Форма.НомерПриложенияПредставление = "Приложение № " + НомерПриложения; 
		КонецЕсли;
	...
	Если Форма.ПользовательСохранилОтчет 
		И (НЕ Форма.НомерАлкоОтчета = 0) И (Форма.НомерАлкоОтчета < 1000)
...
Функция ПолучитьЗаявлениеОКорректировкеПоЖурналу(ДокументСсылкаАлкоОтчет) Экспорт
	...
	НаборЗаписей = ПолучитьНаборЗаписейРегистраЖурнала(ДокументСсылкаАлкоОтчет, "ЗаявлениеОКорректировке", 0, 
...
//
Процедура УстановитьВидимостьРазделовАЛКО(Форма, НаименованиеРаздела, ИндексСтраницы = Неопределено, 
...
Процедура ПриСозданииНаСервереАЛКО(Форма, НомерАлкоОтчета = Неопределено) Экспорт

	СформироватьСтруктуруРеквизитовФормыАЛКО(Форма.СтруктураРеквизитовФормы);
	
	Форма.УстановитьПараметрыДекларации();
	
	СформироватьВспомогательныеСтруктурыОтчетаАЛКО(Форма, НомерАлкоОтчета);
...
Процедура ОчиститьНаСервереАЛКО(Форма) Экспорт
	...
	ИначеЕсли Форма.НомерАлкоОтчета = 1000 Тогда
		...
		Форма.ПредставлениеАдресаВыездногоОбслуживания = "Заполнить";
...
//						страница которого выводится.
//
Процедура УдалитьСтраницуОтчетаАЛКО(Форма, Раздел = Неопределено, ИндексСтраницы = Неопределено) Экспорт
...
Процедура ПередСохранениемАЛКО(Форма, СтруктураПараметров) Экспорт
	...
	Если НЕ (Форма.ПользовательСохранилОтчет или Форма.ПользовательНажалСохранитьОтчет) Тогда
...
Функция СохранитьДанныеОтчетаАЛКО(Форма, СтруктураПараметров) Экспорт
	...
	Если (Форма.ПользовательСохранилОтчет или Форма.ПользовательНажалСохранитьОтчет)
		...
		Если ЗначениеЗаполнено(Форма.СданныйАлкоОтчет) Тогда
			...
			Если ЗначениеЗаполнено(ЗаявлениеОКорректировке) Тогда
				...
				Если НЕ СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка = ЗаявлениеОКорректировке Тогда
					...
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = НСтр("ru='Очищена ссылка на ""Отчет, корректировка сведений которого необходима""
	...
	Если Форма.ПользовательСохранилОтчет или Форма.ПользовательНажалСохранитьОтчет Тогда
	...
	СтрокаНовойВременнойМеткиКомментария = "(Временный отчет, используемый для технических целей.)";
	...
	Если Форма.НомерАлкоОтчета = 0 И ЗначениеЗаполнено(Форма.НомерПриложенияПредставление) Тогда		
	...
	Если НЕ ПозицияСтаройВременнойМеткиКомментария = 0 Тогда
	    ...
	    Если НЕ ПозицияЗакрывающейСкобки = 0 Тогда
			...
			ДлинаВременнойМеткиКомментария = ПозицияЗакрывающейСкобки - ПозицияСтаройВременнойМеткиКомментария + 1;
	...
	// Для Заявления о корректировке - фиксируем в комментарии номер Приложения.
	Если Форма.НомерАлкоОтчета = 0 И ЗначениеЗаполнено(Форма.НомерПриложенияПредставление) Тогда		
		Комментарий = СокрЛП(СокрЛП(Форма.НомерПриложенияПредставление) + " " + Комментарий);			
	КонецЕсли; 
	...
	Если Форма.НомерАлкоОтчета = 0 Тогда
		...
		Если ЗначениеЗаполнено(Форма.УдаленныйСданныйАлкоОтчет) И (Вариант = 0) Тогда
	...
	Если (НЕ СтруктураРеквизитовФормы.мБезОткрытияФормы)
	   ...
	   И (ИдетЗаписьНовогоДокументаСНовымНомеромКорректировки ИЛИ Форма.ЭтоНовыйДокумент) Тогда
...
//
Функция ОтменитьФоновоеЗадание(ИдентификаторЗадания) Экспорт
...
Функция ДобавитьВРегистрЖурнала(ДокументСсылкаАлкоОтчет, ИмяРегистра, ИндексСтраницыРегистра, 
	...
	Если НЕ Ресурсы = Неопределено Тогда
		...
		Если Ресурсы.Свойство("ЗаявлениеОКорректировке") Тогда
			...
			Результат = Результат и ДобавитьВРегистрЖурналаНапрямую(ДокументСсылкаАлкоОтчет, "ЗаявлениеОКорректировке", 0, 
									0, СоответствиеДействий.Получить("Сервис"), РесурсыОбщиеСведения);
...
Процедура ДобавитьТаблицуЗначенийВРегистрСведенийАлко(ДокументСсылкаАлкоОтчет, ИмяФормыОбъекта, ИмяРегистраСведений, 
														ИндексСтраницыРегистра, ТЗ, КоличествоТекСтрок) Экспорт
	...
	Если ТранзакцияАктивна() Тогда
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистраСведений + "
	...
	Для Каждого СтрокаТЗ Из ТЗ Цикл
		...
		ОбъектОтчета.ОбработкаСтрокиТаблицыСтарогоФормата(ИмяРегистраСведений, СтрокаТЗ, ВышеБРО_1_1_11_0);
...
Функция ПереходНаСтрокуТаблицыФормыАЛКО(Форма, ИдГруппы, НомерСтроки, ИмяПоля = Неопределено) Экспорт
	...
	Если НомерСтроки > КоличествоСтрок Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Номер строки оказался больше максимально возможного.
				|будет произведена попытка перехода на последнюю строку'"));
	...
	Если ТаблицаФормыРаздела.ТекущаяСтрока = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю( НСтр("ru = 'Строка № " + НомерСтроки 
...
Функция ЗаполнитьПоСтраницеРазделаАЛКО(ИдГруппы, ИндексСтраницы,  
	...
	Перем КоличествоПервых, ТекущийИндексСтроки;
	...
	Если ИндексСтраницы >= 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ИДДокИндСтраницы) Тогда
			...
			Если ТипЗнч(СтруктураСервисРегистров) = Тип("Структура") Тогда
				...
				ИДДокИндСтраницы = ПолучитьИдДокИндСтраницы(ДокОтчета, ИндексСтраницыРегистра);	
		...
		СтрокаПервые = ?(НЕ КоличествоПервых = Неопределено, 
						" ПЕРВЫЕ " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(КоличествоПервых), "");
		...
		УсловиеПоИндексуСтроки = ?(НЕ ТекущийИндексСтроки = Неопределено, 
								"(АлкоПрилДанные.ИндексСтроки > &ТекущийИндексСтроки)", "Истина");
...
Функция ОтобратьСтрокиПоГрафам(ИДДокИндСтраницы, ИмяРегистра, ИдГруппы, СписокГраф) Экспорт
	...
	Для каждого ЭлементМассива Из МассивГраф Цикл
		...
		УсловиеПоГрафам = УсловиеПоГрафам + РазделительУсловия + "АлкоПрилДанные." + ИмяКолонки + " > 0";
...
Функция НайтиСданныйАлкоОтчетОпределенногоИсточникаПоОбразцу(ОбразецДокументСсылкаАлкоОтчет, ИсточникОтчета) Экспорт
	...
	ДатаНачалаПериодаОтчета = ОписаниеОтчета.ДатаНачалаПериодаОтчета;
	...
	Если РезультатЗапроса.Количество() > 0 Тогда
		...
		Для каждого СтрокаРезультата Из РезультатЗапроса Цикл
			...
			ИначеЕсли СтрокаРезультата.СостояниеСдачиОтчетности = Перечисления.СостояниеСдачиОтчетности.ДокументооборотНачат 
				Тогда
...
Функция ПолучитьСтруктуруСведенийГрафПоОПФармацевтическихДеклараций(ДокументСсылкаАлкоОтчета, ТаблицаСтраницыРаздела, СервисРегистровРаздела, 
								                                     ИмяРегистра, ИдГруппы, СписокГраф) Экспорт
	...
	Для каждого ЛистРаздела Из ТаблицаСтраницыРаздела Цикл
		...
		Если (НЕ ПустаяСтрока(ДанныеОП)) и (НЕ СокрЛП(ДанныеОП) = "Заполнить") Тогда
			...
			Если ТипЗнч(ДополнительныеДанные) = Тип("Структура") 
				...
				ИндексСтраницыРегистра = СервисРегистровРаздела[НомерЛиста - 1].ИндексСтраницыРегистра;
				...
				Для каждого ЭлементМассива Из МассивГраф Цикл					
					СтруктураСведенийПООП.Вставить("СтруктураГрафы" + ЭлементМассива, Новый Структура);					
				КонецЦикла;
				...
				Для каждого СтрокаТаблицы Из ТаблицаЗапроса Цикл
					...
					Для каждого ЭлементМассива Из МассивГраф Цикл
						...
						Если СтрокаТаблицы[ИмяКолонки] > 0 Тогда
							...
							Если НЕ СтруктураСведенийПООП["СтруктураГрафы" + ЭлементМассива].Свойство(ИдКодПродПроцСпирта) Тогда
							    СтруктураСведенийПООП["СтруктураГрафы" + ЭлементМассива].Вставить(ИдКодПродПроцСпирта, Новый Структура);							
							КонецЕсли; 
							
	                        СтруктураСведенийГрафы = СтруктураСведенийПООП["СтруктураГрафы" + ЭлементМассива][ИдКодПродПроцСпирта];
...
Процедура ПроверитьКонтрольныеСоотношенияФармацевтическихДеклараций(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтруктураПараметров) Экспорт
	...
	ИсточникСверяемойДекларации = "РегламентированныйОтчетАлкоПриложение" + НомерСверяемойДекларации;
	СтруктураИнформацииСверяемойДекларации = НайтиСданныйАлкоОтчетОпределенногоИсточникаПоОбразцу(
												ДокументСсылкаАлкоОтчет, ИсточникСверяемойДекларации);
	...
	Если ЗначениеЗаполнено(СтруктураИнформацииСверяемойДекларации.ДокументСсылкаАлкоОтчет) Тогда
		...
		ИмяРегистраСверяемойДекларации = "СведенияРеглОтчетАлкоПрил" + НомерСверяемойДекларации + "Декларация";
		СервисРегистровРазделаСверяемойДекларации = СтруктураСервисРегистровСверяемойДекларации[ИмяРегистраСверяемойДекларации];
		
		Если ТаблицаСтраницыДекларация = Неопределено ИЛИ СтруктураСервисРегистров = Неопределено Тогда
		...
		ИмяРегистраДекларации = "СведенияРеглОтчетАлкоПрил" + НомерДекларации + "Декларация";
		СервисРегистровРазделаДекларации = СтруктураСервисРегистров[ИмяРегистраДекларации];
		
		СтруктураГрафыСверяемойДекларации = ПолучитьСтруктуруСведенийГрафПоОПФармацевтическихДеклараций(
												ДокументСверяемойДекларации, ТаблицаСтраницыСверяемойДекларации, СервисРегистровРазделаСверяемойДекларации, 
												ИмяРегистраСверяемойДекларации, ИдГруппы, НомерКолонкиСверяемойДекларации);		
		
	    СтруктураГрафыДекларации = ПолучитьСтруктуруСведенийГрафПоОПФармацевтическихДеклараций(
											ДокументСсылкаАлкоОтчет, ТаблицаСтраницыДекларация, СервисРегистровРазделаДекларации, 
											ИмяРегистраДекларации, ИдГруппы, НомерКолонкиДекларации);
		...
		Для каждого ЭлементСтруктуры Из СтруктураГрафыДекларации Цикл
			...
			НомерЛистаОПДекларацииСтроковый = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(НомерЛистаОПДекларации);
			
			СтрокаОПДекларации = Наименование_ОП + ?(НЕ ПустаяСтрока(КПП_ОП), ", (КПП " + КПП_ОП + ")", "") 
									+ " адрес: " + ПредставлениеАдресаОПДекларации;
			
			СтруктураГрафыПоОПДекларации 	= СтруктураСведенийПООПДекларации["СтруктураГрафы" + НомерКолонкиДекларации];
			...
			Если НЕ СтруктураГрафыСверяемойДекларации.Свойство(ИдОПДекларации, СтруктураСведенийПООПСверяемойДекларации) Тогда
				...
				ТекстОписанияОшибки = СтрВКакомРазделе + ", на листе " + НомерЛистаОПДекларацииСтроковый + ", по обособленному подразделению 
					|" + СтрокаОПДекларации	+ "
					|есть заполненные строки по Графе " + НомерКолонкиДекларацииСтр + " ""израсходовано на собственное производство.."", 
					|но в ранее " + СостояниеСверяемойДекларации + " декларации " 
					+ ПредставлениеДокументаСверяемойДекларации + "
					|нет строк с заполненной Графой " + НомерКолонкиСверяемойДекларацииСтр + " на странице по этому же ОП!
				...
				СообщитьОбОшибкеЗаполнения(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, , СтруктураОшибки);
				
			Иначе
				
				СтруктураГрафыПоОПСверяемойДекларации = СтруктураСведенийПООПСверяемойДекларации["СтруктураГрафы" + НомерКолонкиСверяемойДекларации];
				...
				Для каждого ЭлементСтруктурыГрафыДекларации Из СтруктураГрафыПоОПДекларации Цикл
					...
					Если НЕ СтруктураГрафыПоОПСверяемойДекларации.Свойство(ИдКодПродПроцСпирта, СведенияГрафыСверяемойДекларации) Тогда
						...
						ТекстОписанияОшибки = СтрВКакомРазделе + ", на листе " + НомерЛистаОПДекларацииСтроковый + ", по обособленному подразделению 
							|" + СтрокаОПДекларации	+ ",
							|в строке " + НомерСтрокиСтроковый  + ", для значения ""Содержание этилового спирта в объеме готовой продукции"" " + ПроцентСпирта + " заполнена Графа " + НомерКолонкиДекларацииСтр + ",
							|но в ранее " + СостояниеСверяемойДекларации + " декларации " + ПредставлениеДокументаСверяемойДекларации + "
							|для этого же значения содержания спирта " + ПроцентСпирта + "
							|нет строк с заполненной Графой " + НомерКолонкиСверяемойДекларацииСтр + " на странице по этому же ОП!
							|Согласно правилам заполнения Графа 19 Приложения №1 должна соответствовать значению Графы 8 "
							+ "Приложения №2 по каждому обособленному подразделению организации!";
							
						ИмяПоказателя = ИдГруппы + НомерКолонкиДекларации + "_" + НомерСтрокиСтроковый;
					...
					Иначе
						...
						НомерСтрокиСверяемойДекларацииСтроковый 	= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(СведенияГрафыСверяемойДекларации.НомерСтроки);
						...
						Если СведенияГрафыСверяемойДекларации.Графа <> ЗначениеГрафыДекларации Тогда
							...
							ТекстОписанияОшибки = СтрВКакомРазделе + ", на листе " + НомерЛистаОПДекларацииСтроковый + ", по обособленному подразделению 
								|" + СтрокаОПДекларации	+ ",
								|в строке " + НомерСтрокиСтроковый  + ", для значения ""Содержание этилового спирта в объеме готовой продукции"" " + ПроцентСпирта + " 
								|значение Графы " + НомерКолонкиДекларацииСтр + " " + ЗначениеГрафыДекларацииСтроковый + ",
								|но в ранее " + СостояниеСверяемойДекларации + " декларации " + ПредставлениеДокументаСверяемойДекларации + "
								|в строке " + НомерСтрокиСверяемойДекларацииСтроковый  + ", для этого же значения содержания спирта " + ПроцентСпирта + " 
								|значение Графы " + НомерКолонкиСверяемойДекларацииСтр + " " + СведенияГрафыСверяемойДекларацииСтроковый + "
				...
				Если СтруктураГрафыПоОПСверяемойДекларации.Количество() > 0 Тогда
					...
					Для каждого ЭлементСтруктурыГрафыСверяемойДекларации Из СтруктураГрафыПоОПСверяемойДекларации Цикл
						...
						СтрокаПроцентСпирта = СтрокаПроцентСпирта + ?(СтрокаПроцентСпирта = "", "", Символы.ПС)
					...
					ТекстОписанияОшибки = СтрВКакомРазделе + ", на листе " + НомерЛистаОПДекларацииСтроковый + ", по обособленному подразделению 
						|" + СтрокаОПДекларации	+ "
						|нет заполненных строк по Графе " + НомерКолонкиДекларацииСтр + " ""израсходовано на собственное производство.."", 
		...
		Если СтруктураГрафыСверяемойДекларации.Количество() > 0 Тогда
			...
			Для каждого ЭлементСтруктурыГрафыСверяемойДекларации Из СтруктураГрафыСверяемойДекларации Цикл
				...
				СтрокаОПСверяемойДекларации = Наименование_ОП + ?(НЕ ПустаяСтрока(КПП_ОП), ", (КПП " + КПП_ОП + ")", "") 
										+ " адрес: " + ПредставлениеАдресаОПСверяемойДекларации + " - лист " + НомерЛистаОПСверяемойДекларации;
				
				СтрокаСпискаОП = СтрокаСпискаОП + ?(СтрокаСпискаОП = "", "", Символы.ПС) + СтрокаОПСверяемойДекларации;
			
			КонецЦикла; 
			
			ТекстОписанияОшибки = СтрВКакомРазделе + " в ранее " + СостояниеСверяемойДекларации + " декларации " + ПредставлениеДокументаСверяемойДекларации + "
				|по обособленным подразделениям:
				|" + СтрокаСпискаОП + "
				|имеются строки с заполненной Графой " + НомерКолонкиСверяемойДекларацииСтр + ",
				|но в проверяемой декларации нет строк по этим же обособленным подразделениям с заполненной Графой " + НомерКолонкиДекларацииСтр + "!				
				|Согласно правилам заполнения Графа 19 Приложения №1 должна соответствовать значению Графы 8 "
				+ "Приложения №2 по каждому обособленному подразделению организации!";
...
Процедура ВывестиНаПечатьТитульный(	СтруктураПараметровСтраницы, 
									...
									ОбъектОтчета  = Неопределено ) Экспорт
	...
	НастройкиСтраницы = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"НастройкиСтраницы", СтруктураПараметровСтраницы);
	...
	// Версия печати обычно дата формы, если есть.
	ВерсияМакетаПечати	= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ВерсияМакетаПечати", СтруктураПараметровСтраницы);
	
	МакетПечати = ПолучитьМакетПечатнойФормы(ИмяФормыОбъекта, ИмяРаздела, ВерсияМакетаПечати, ОбъектОтчета);
	...
	НомерКорректировки = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
															"НомКорр", СтруктураДанныхТитульный);
	...
	Для Кв = 1 По 4 Цикл
		...
		ТекОтчетныйПериод = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
												"ОтчетныйПериод" + Кв + "Кв", СтруктураДанныхТитульный); 
	    Если ТекОтчетныйПериод = "V" Тогда				
			ВывестиПолеМакета(Область_ОсновнаяЧасть, "ОтчетныйПериод" + Кв + "Кв", "V");								
		КонецЕсли;
	
	КонецЦикла; 
	
	// Отчетный Год, хранится уже в виде строки, форматировать не надо. (4 клетки).
	ОтчетныйГод = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ОтчетныйГод", СтруктураДанныхТитульный);	
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "ОтчетныйГод", ОтчетныйГод);
	
	// ОргИМНС.
	ОрганизацияИМНС = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ОргИМНС", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "ОргИМНС", ОрганизацияИМНС);	
						
	// ОргНазв.
	ОрганизацияНазв = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ОргНазв", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "ОргНазв", ОрганизацияНазв);
	
	// ОргНазвСокращ.
	ОрганизацияНазвСокращенное = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ОргНазвСокращ", СтруктураДанныхТитульный);
	...
	ОргПочтовыйИндекс = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"ПочтовыйИндекс", СтруктураДанныхТитульный);
	...
	// КодРегиона (2 клетки).
	ОргКодРегиона = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"КодРегиона", СтруктураДанныхТитульный);
	...
	// АдресРайон.
	ОрганизацияАдресРайон = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"АдресРайон", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "АдресРайон", ОрганизацияАдресРайон);
	
	// АдресГород.
	ОрганизацияАдресГород = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"АдресГород", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "АдресГород", ОрганизацияАдресГород);
	
	// АдресУлица.
	ОрганизацияАдресУлица = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"АдресУлица", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "АдресУлица", ОрганизацияАдресУлица);
	
	// АдресДом.
	ОрганизацияАдресДом = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"АдресДом", СтруктураДанныхТитульный);
	...
	// Адрес организации.
	ПредставлениеАдресаОрганизации = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ПредставлениеАдресаОрганизации", СтруктураДанныхТитульный);
	Если СтруктураДанныхТитульный.ЭтоПБОЮЛ Тогда	
		АдресВыведен = ВывестиПолеМакета(Область_ОсновнаяЧасть, 
									"АдресИндивидуальногоПредпринимателя", ПредставлениеАдресаОрганизации);
		АдресВыведен = АдресВыведен ИЛИ ВывестиПолеМакета(Область_ОсновнаяЧасть, 
									"АдресМестаЖительстваИП", ПредставлениеАдресаОрганизации);
	...
	ПредставлениеАдресаМестаДеятельности = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"МестоДеятельности", СтруктураДанныхТитульный);
	...
	// Номер лицензии.
	НомерЛицензии = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"НомерЛицензии", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "НомерЛицензии", НомерЛицензии);
	
	// ТелОрганизации.
	ТелОрганизации = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ТелОрганизации", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "ТелОрганизации", ТелОрганизации);
	
	// НомерКонтактногоТелефона.
	НомерКонтактногоТелефона = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"НомерКонтактногоТелефона", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "КонтактныйТел", НомерКонтактногоТелефона);
	
	// Добавочный Номер.
	ДобНомер = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ДобНомер", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "ДобТел", ДобНомер);
	
	// Наименование удостоверяющего документа.
	ДокПолномочий = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"НаименованиеДокумента", СтруктураДанныхТитульный);
	ВывестиПолеМакета(Область_ОсновнаяЧасть, "ДокПолномочий", ДокПолномочий);
	
	// ЭлектроннаяПочта.
	ЭлектроннаяПочта = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ЭлектроннаяПочта", СтруктураДанныхТитульный);
	...
	ДатаПодписи = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"ДатаПодписи", СтруктураДанныхТитульный);
	...
	МесяцПрописьюРодПадежДатыПодписи = СтрЗаменить(ДатаПодписиПрописью, ПолныйГодДатыПодписи + " г.", "");
	МесяцПрописьюРодПадежДатыПодписи = СокрЛП(СтрЗаменить(МесяцПрописьюРодПадежДатыПодписи, ДеньДатыПодписи, ""));
	
	НомерАлкоОтчета = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"НомерАлкоОтчета", СтруктураРеквизитовФормы);
	...
	Если Число(НомерАлкоОтчета) = 1000 Тогда
		...
		ЭтоОтмена =  РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
														"ЭтоОтмена", СтруктураДанныхТитульный);
		Если НЕ ЗначениеЗаполнено(ЭтоОтмена) Тогда
		    ЭтоОтмена = Ложь;		
		КонецЕсли;
		
		НомерКорректировкиУведомления = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
														"НомерКорректировкиУведомления", СтруктураДанныхТитульный);
		...
		ДатаНачалаУведомления = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
														"ДатаНачалаУведомления", СтруктураДанныхТитульный);
		...
		ДатаКонцаУведомления = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
		 												"ДатаКонцаУведомления", СтруктураДанныхТитульный);
		...
		ВремяНачалаУведомления = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
														"ВремяНачалаУведомления", СтруктураДанныхТитульный);
		...
		ВремяКонцаУведомления = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
														"ВремяКонцаУведомления", СтруктураДанныхТитульный);
		...
		Представитель = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
														"Представитель", СтруктураДанныхТитульный);
		ПереключательПредставителя = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
														"ПереключательПредставителя", СтруктураДанныхТитульный);
		
		ЭтоРуководитель = ""; ЭтоПредставитель = "";
	...
	НомерПриложения = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"НомерПриложения", СтруктураДанныхТитульный);
	...
	Если НЕ НомерПриложения = Неопределено Тогда
		...
		Если Число(НомерПриложения) >= 13 И Число(НомерПриложения) <= 15 Тогда
			
			Очистить = Истина;
			
			Для Кв = 1 По 4 Цикл								
				ВывестиПолеМакета(Область_ОсновнаяЧасть, "ОтчетныйПериод" + Кв + "Кв", "V", Очистить);								
			КонецЦикла;
			
		КонецЕсли; 
		
		// Специфические поля Заявления о предоставлении корректирующих деклараций после установленного срока.
		НомерПриложения = Строка(НомерПриложения);
		НомерПриложения = "Приложение № " + НомерПриложения;
	    ВывестиПолеМакета(Область_ОсновнаяЧасть, "НомерПриложения", НомерПриложения);
		...
		ИсходящийНомер = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"ИсходящийНомер", СтруктураДанныхТитульный);
		...
		ИсходящийНомер = "Исх. № " + ИсходящийНомер + " от " + Формат(ДатаПодписи, "ДЛФ=ДД");
		ВывестиПолеМакета(Область_ОсновнаяЧасть, "ИсходящийНомер", ИсходящийНомер);
				
	КонецЕсли;
	
	Обоснования = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"Обоснования", СтруктураДанныхТитульный);
	...
	ТаблицаПеречень = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"ТаблицаПеречень", СтруктураПараметровСтраницы);
	...
	Если СтруктураДанныхТитульный.ЭтоПБОЮЛ Тогда
		...
		ОргДиректор = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ОргДиректор", СтруктураДанныхТитульный);
	...
	Иначе
		...
		// ОргБухгалтер.
		ОргБухгалтер = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
														"ОргБухгалтер", СтруктураДанныхТитульный);
...
Процедура ВывестиНаПечатьРазделЛицензии(СтруктураПараметровСтраницы,
	...
	ИдГруппы = РегламентированнаяОтчетностьАЛКОКлиентСервер.ПолучитьИдГруппыПоРазделуАЛКО(
														ИмяРаздела, СтруктураРеквизитовФормы);
	ИндексЛиста = -1;
	Для Каждого ЛистЛицензий Из ТаблицаСтраницыЛицензии Цикл
		
		ИндексЛиста = ИндексЛиста + 1;
				
		ТаблицаДопСтрок =  ЗаполнитьПоСтраницеРазделаАЛКО(ИдГруппы,
							ИндексЛиста, СтруктураРеквизитовФормы, СтруктураСервисРегистров);
		...
		Для ИндПечЛиста = 1 По КолПечатныхЛистов Цикл
			...
			Для Ном = 1 По 5 Цикл
				...
				ЗначениеРеквизита = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
																	ИмяРеквизита, СтруктураДанныеЛиста);
...
Процедура ВывестиНаПечатьРазделДекларации(СтруктураПараметровСтраницы,
	...
	Перем ПечататьВозвраты, ПечататьДекларация;
	...
	НастройкиАлгоритмаПечатиСтрокОрг = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"НастройкиАлгоритмаПечатиСтрокОрг", НастройкиАлгоритмаПечати);
	...
	ВыводитьВозвратыЗаДекларацией = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"ВыводитьВозвратыЗаДекларацией", НастройкиАлгоритмаПечати);
	...
	// Обычно "СтрокаОП", "РеквизитыОрганизации".
	ИмяОбластиСтрокаОрг 		= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ИмяОбластиСтрокаОрг", НастройкиАлгоритмаПечати);
			
	// Обычно "ЗаголовокРеквизитов".
	ИмяОбластиЗаголовокРеквизитовОП 		= НастройкиАлгоритмаПечати.ИмяОбластиЗаголовокРеквизитовОП; 
	// Обычно "ЗаголовокРеквизитов".
	ИмяОбластиЗаголовокРеквизитовОрг 		= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ИмяОбластиЗаголовокРеквизитовОрг", НастройкиАлгоритмаПечати);
	...
	ЗаголовокРеквизитовОрг 			= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ЗаголовокРеквизитовОрг", НастройкиАлгоритмаПечати);
	...
	// Обычно "Реквизиты" или "ОП".
	ИмяОбластиРеквизитовОрг 		=  РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"ИмяОбластиРеквизитовОрг", НастройкиАлгоритмаПечати);
	...
	КолонкиГруппировокОрг 	= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"КолонкиГруппировокОрг", НастройкиАлгоритмаПечати);
	КолонкиСуммированияОрг 	= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"КолонкиСуммированияОрг", НастройкиАлгоритмаПечати);
	КолонкиНеВыводитьОрг 	= РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрЗначениеПоКлючуСтруктуры(
													"КолонкиНеВыводитьОрг", НастройкиАлгоритмаПечати);
		
	ИдГруппыРаздела = РегламентированнаяОтчетностьАЛКОКлиентСервер.ПолучитьИдГруппыПоРазделуАЛКО(
															ИмяРаздела, СтруктураРеквизитовФормы);
	Если ВыводитьВозвратыЗаДекларацией Тогда
		...
		ИдГруппыРазделаВозвраты = РегламентированнаяОтчетностьАЛКОКлиентСервер.ПолучитьИдГруппыПоРазделуАЛКО(
														ИмяРазделаВозвраты, СтруктураРеквизитовФормы);
		// Обычно "СтрокаОП", "РеквизитыОрганизации".
	...
	ОсталосьСтрок = ОсталосьСтрок - Высота_ЗаголовокЛистаВСтроках - Высота_НомерСтраницыВСтроках;
	...
	Если ВыводитьСводнуюТаблицуПоОрганизацииВЦелом Тогда
		...
		ТекстРеквизитовОрганизации = Наименование + ", ИНН " + ИНН + ?(ЗначениеЗаполнено(КПП), ", КПП " + КПП, "") + "
									|" + Адрес;
		...
		ДанныеПоОрганизации = ЗаполнитьПоВсейОрганизации(ИдГруппыРаздела, СтруктураРеквизитовФормы, 
									СтруктураСервисРегистров, КолонкиГруппировокОрг, 
									КолонкиСуммированияОрг, КолонкиНеВыводитьОрг);
		...
		Если НужноОбработатьСтрок > 0 Тогда
			...
			СообщитьПроцентВыполнения(
				"Печать раздела " + ИмяРаздела + ".", ОбработаноСтрок, НужноОбработатьСтрок, ПроцентБезКонвертацииАрхивации);
		...
		Если ВыводитьВозвратыЗаДекларацией Тогда
			...
			Если ОсталосьСтрок < (Высота_ПустаяСтрокаВСтроках + Высота_СтрокаОПВСтроках + 3) Тогда
				...
				ОсталосьСтрок = ОсталосьСтрок - Высота_ЗаголовокТаблицыВСтроках - Высота_НомерСтраницыВСтроках;
			...
			ВывестиПолеМакета(Область_ЗаголовокПоОрганизацииВозвраты, ИмяОбластиЗаголовокРеквизитовОргВозвраты, 
							ЗаголовокРеквизитовОргВозвраты);
		    				
			ВывестиПолеМакета(Область_ЗаголовокПоОрганизацииВозвраты, 
							ИмяОбластиРеквизитовОргВозвраты, ТекстРеквизитовОрганизации);
			...
			ДанныеПоОрганизации = ЗаполнитьПоВсейОрганизации(ИдГруппыРазделаВозвраты, СтруктураРеквизитовФормы, 
										СтруктураСервисРегистров, КолонкиГруппировокОрг, 
										КолонкиСуммированияОрг, КолонкиНеВыводитьОрг);
	...
	Если ВыводитьВозвратыЗаДекларацией Тогда
		...
		ДанныеОП_Возвраты = СтруктураСобратьДанныеПоОП(
								ИдГруппыРазделаВозвраты, ИмяРазделаВозвраты, ТаблицаСтраницыВозвраты, ЭтоПБОЮЛ, ИНН);
	...
	Для Каждого Лист Из ДанныеОП_Раздела Цикл
		...
		Для каждого СведОрг Из СтруктураОП.СведОрг Цикл
			...
			ДанныеДвиженийДекларация = ЗаполнитьПоСтраницеРазделаАЛКО(ИдГруппыРаздела, ИндексСтраницы, 
												СтруктураРеквизитовФормы, СтруктураСервисРегистров);
			...
			Если ВыводитьВозвратыЗаДекларацией Тогда
				...
				Если ЗначениеЗаполнено(ПодробныеСведения.НомерЛистаДругогоРаздела) Тогда
					...
					ДанныеДвиженийДекларация = ЗаполнитьПоСтраницеРазделаАЛКО(ИдГруппыРазделаВозвраты, ИндексСтраницыВозвратов, 
													СтруктураРеквизитовФормы, СтруктураСервисРегистров);			
				Иначе
					// Передаем отрицательный индекс - получим пустую таблицу с нужными колонками.								
					ДанныеДвиженийДекларация = ЗаполнитьПоСтраницеРазделаАЛКО(ИдГруппыРазделаВозвраты, -1, 
													СтруктураРеквизитовФормы, СтруктураСервисРегистров);		
				КонецЕсли;
				...
				Если КоличествоВозвратовБезДекларации = 0 Тогда
					...
					ВывестиНаПечатьСтроки(СтруктураПараметров, ПечатнаяФорма, 
											ОсталосьСтрок, ОбработаноСтрок, ТекЛист, ВсегоЛистов);
		...
		Для каждого Лист Из ДанныеОП_Возвраты Цикл
			...
			Для каждого СведОргВозвратов Из СтруктураОПВозвратов.СведОрг Цикл
				...
				Если ПодробныеСведенияВозвратов.НомерЛистаДругогоРаздела = Неопределено Тогда
					...
					ИндексСтраницыВозвратов = ПодробныеСведенияВозвратов.НомерЛистаДекларация - 1;
					...
					ВывестиНаПечатьСтроки(СтруктураПараметров, ПечатнаяФорма, ОсталосьСтрок, 
										ОбработаноСтрок, ТекЛист, КоличествоВозвратовБезДекларации);
...
Функция ПолучитьИмяОбщегоМодуляРаботаСАдресами(ВерсияБСПЧислом = Неопределено)
...
Функция ПолучитьИдДанных(Данные, Разделитель = "_")
	...
	Для каждого ЭлементСписка Из Данные Цикл
		...
		ПреобразованнаяСтрокаДанных = ?(ПреобразованнаяСтрокаДанных = Неопределено, "", 
																	ПреобразованнаяСтрокаДанных);	
		ПреобразованнаяСтрокаДанных = ПолучитьДопустимуюСтроку(ПреобразованнаяСтрокаДанных);
		
		Результат = Результат + ?(Индекс = 0, "", Разделитель) + ПреобразованнаяСтрокаДанных;
...
Процедура ИзменитьЗаписьРегистраСведенийАЛКО(Форма, ИдГруппы, ИндексСтраницы, ИндексСтроки, 
	...
	ДокументСсылкаАлкоОтчет = Форма.СтруктураРеквизитовФормы.мСохраненныйДок;
	ИндексСтраницыРегистра = РегламентированнаяОтчетностьАЛКОКлиентСервер.ПолучитьИндексСтраницыРегистра(
																		Форма, ИдГруппы, ИндексСтраницы);
...
Функция СформироватьСтруктуруПоказателейИСтруктуруПараметровАЛКО(Форма)
	...
	Если Форма.ДополнительныеДанные.Титульный.Свойство("АдресМестаДеятельности") Тогда
		...
		// Адрес обязан быть российским.
		Если ЗначениеЗаполнено(Форма.СтруктураДанныхТитульный.МестоДеятельности) Тогда
		    ПараметрыВыгрузки.Вставить("КодСтраныМестаОсуществления",  "643");
			ПараметрыВыгрузки.Вставить("КодРегионМестаОсуществления",  АдресМестаДеятельности.КодРегиона);
		Иначе	
		    ПараметрыВыгрузки.Вставить("КодСтраныМестаОсуществления",  "");
			ПараметрыВыгрузки.Вставить("КодРегионМестаОсуществления",  "");
		КонецЕсли; 
...
Функция ОпределитьКонтрагентаИзЗапросаПоНаименованию(ТаблицаРезультатаЗапроса, ПолноеИлиКраткоеНаименование = Неопределено,
...
Функция ОпределитьКонтрагентаИзЗапросаАЛКО(ТаблицаРезультатаЗапроса, ИНН, КПП, ПолноеИлиКраткоеНаименование = Неопределено,
	...
	Если ТаблицаРезультатаЗапроса.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(КПП) Тогда
			...
			Для каждого СтрокаТЗ Из ТаблицаРезультатаЗапроса Цикл
				...
				Если СокрЛП(СтрокаТЗ[ИмяКолонкиКонтрагент].КПП)= КПП Тогда
					...
					Если (НЕ ЕстьСтранаРегистрации) 
						ИЛИ
						(ЕстьСтранаРегистрации И СтрокаТЗ[ИмяКолонкиКонтрагент].СтранаРегистрации = СтранаРоссия )
						Тогда
			...
			Если ТаблицаРезультатаЗапросаКППСтрогая.Количество() > 0 Тогда
			    ...
			    Контрагент = ОпределитьКонтрагентаИзЗапросаПоНаименованию(ТаблицаРезультатаЗапросаКППСтрогая, 
									ПолноеИлиКраткоеНаименование, ИмяКолонкиКонтрагент, Истина);				
			Иначе
				
				Контрагент = ОпределитьКонтрагентаИзЗапросаПоНаименованию(ТаблицаРезультатаЗапросаКПП, 
									ПолноеИлиКраткоеНаименование, ИмяКолонкиКонтрагент, Истина);						
			КонецЕсли; 
		...
		Если (Контрагент = Неопределено) И (НЕ ЗначениеЗаполнено(КПП))  Тогда
			...
			Для каждого СтрокаТЗ Из ТаблицаРезультатаЗапроса Цикл
				...
				Если (НЕ ЗначениеЗаполнено(СтрокаТЗ[ИмяКолонкиКонтрагент].КПП))
					ИЛИ ( ЕстьСтранаРегистрации 
						И (НЕ СтрокаТЗ[ИмяКолонкиКонтрагент].СтранаРегистрации = СтранаРоссия) )
					Тогда
			...
			Если ТаблицаРезультатаЗапросаБезКПП.Количество() > 0 Тогда
				...
				Контрагент = ОпределитьКонтрагентаИзЗапросаПоНаименованию(ТаблицаРезультатаЗапросаБезКПП, ПолноеИлиКраткоеНаименование,
																	ИмяКолонкиКонтрагент, Истина);
		...
		Если Контрагент = Неопределено Тогда
			...
			Контрагент = ОпределитьКонтрагентаИзЗапросаПоНаименованию(ТаблицаРезультатаЗапроса, ПолноеИлиКраткоеНаименование,
																	ИмяКолонкиКонтрагент, Ложь); 
	...
	Если НЕ Контрагент = Неопределено Тогда
		...
		ЭтоРоссийскоеЮрЛицо = (ЕстьСтранаРегистрации 
								   ...
								   И (ДлинаИНН = 10))
								ИЛИ 
								  ((ДлинаИНН = 10) И (НЕ ЕстьСтранаРегистрации));		
								  
		КПП_Контрагента = ?(ЭтоРоссийскоеЮрЛицо, Контрагент.КПП, "");
		
	  	Если ЭтоРоссийскоеЮрЛицо 
		  	И (СтрДлина(КПП_Контрагента) = 9) И (СтрДлина(НачальныйКПП) = 0) Тогда
...
Функция НомерАлкоПриложения(Форма = Неопределено, ИмяФормыОбъекта = Неопределено)
...
Процедура ПриОбновленииИБЗаполнитьИзмерениеОрганизацияАлко(Параметры = Неопределено) Экспорт
	...
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоЖурнал");
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоЛицензии");
	
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил11Раздел1");
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил11Раздел2");
	
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил12Раздел1");
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил12Раздел2");
	
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил13Декларация");
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил14Декларация");
	
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил15Поставщики");
	СписокРегистров.Добавить("СведенияРеглОтчетАлкоПрил15Производство");
	...
	Если ОбщийРезультат.Количество() > 0 Тогда
		...
		Для каждого ЭлементСписка Из ОбщийРезультат Цикл		
			ИтоговаяИнформацияОбОшибке = ИтоговаяИнформацияОбОшибке + ЭлементСписка.Значение + "
										|";		
		КонецЦикла;
									
	    ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение измерения Организация регистров отчетов ФСРАР" 
								+ " - ошибка блокировки при заполнении измерения Организация'"), 
								УровеньЖурналаРегистрации.Ошибка,,, ИтоговаяИнформацияОбОшибке);
...
Функция ЗаполнитьИзмерениеОрганизацияРегистраАлко(ИмяРегистра)
	...
	Если РезЗапросаНепустыхОрганизаций.Пустой() Тогда
		...
		Если РезЗапросаНепустыхОрганизаций.Пустой() Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистра + " - при заполнении измерения Организация обнаружено что все организации в ИБ пустые.'"), 
										УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ ЭтоФайловаяБаза Тогда
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистра + " - ошибка блокировки при заполнении измерения Организация'"), 
									УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	РегистрСведенийРеглОтчет = РегистрыСведений[ИмяРегистра]; 						
		
	СписокСоставаРегистра = Новый СписокЗначений;
	СписокСоставаРегистра.Добавить("Измерения");
	Отбор = ПолучитьСтруктуруДанныхЗаписиРегистраСведений(ИмяРегистра, СписокСоставаРегистра);
...
Функция ВысотаЯчейкиВСтроках(Область_Строка, ИмяПоля, ПомещаетсяВСтрокуПоля)
	...
	КолСтрокВСтрокеПереносимой = Цел(СтрДлина(СтрокаПереносимая)/ПомещаетсяВСтрокуПоля)
								+ ?(СтрДлина(СтрокаПереносимая)%ПомещаетсяВСтрокуПоля > 0, 1, 0);
	КолСтрокВСтрокеПереносимой = ?(КолСтрокВСтрокеПереносимой = 0, 1, КолСтрокВСтрокеПереносимой);
		
	Возврат КолСтрокВСтрокеПереносимой;
									
КонецФункции

Функция ВысотаСтрокиВСтроках(Область_Строка, СписокПереносимыхПолей, ИдГруппыОбластиСтрокаМакета, ИдГруппыПолейТаблицы)
	...
	Для каждого ЭлементСписка Из СписокПереносимыхПолей Цикл
		...
		ТекКолСтрокВСтрокеПереносимой = ВысотаЯчейкиВСтроках(
										Область_Строка, ИмяПоляПереносимойСтроки, ПомещаетсяВСтрокуПереносимую);
...
Функция СтрокаШаблонИзАгрегатныхДанных(ТекстШаблона, СтрокаДанных, ИдГруппыТаблицы, ОграничительНачало = "%#", ОграничительКонец = "#%")
	...
	Пока ПозицияНачала > 0 Цикл
		...
		Результат = Результат + СокрЛП(Строка(СтрокаДанных[ИдГруппыТаблицы + ИмяПоляШаблона]));
...
Процедура ВывестиНаПечатьСтроки(СтруктураПараметров, 
								ПечатнаяФорма, ОсталосьСтрок, ОбработаноСтрок, ТекЛист = 0, ВсегоЛистов = 1)
		  
	
	// Группа переменных для извлечения данных о высотах из НастройкиАлгоритмаПечати.
	Перем СписокСтруктурИнформацииИтогов, СписокПереносимыхПолей;
	...
	Для Каждого СтрокаДанных Из ТаблицаСтрок Цикл
		...
		Для Каждого Колонка Из ТаблицаСтрок.Колонки Цикл
			...
			ВывестиПолеМакета(Область_Строка, ИмяКолонкиМакета, СтрокаДанных[ИмяКолонки]);
		...
		КолСтрокВСтрокеПереносимой = ВысотаСтрокиВСтроках(
						Область_Строка, СписокПереносимыхПолей, ИдГруппыОбластиСтрокаМакета, ИдГруппыПолейТаблицы);
	...
	Для каждого ЭлементСписка Из СписокСтруктурИнформацииИтогов Цикл
		...
		Для каждого ЭлементМассива Из МассивКолонокСуммирования Цикл
			...
			СтрокаКолонокСуммирования = СтрокаКолонокСуммирования + РазделительКолонкиСуммирования 
										+ ИдГруппыПолейТаблицы + ЭлементМассива;
			РазделительКолонкиСуммирования = ",";
		...
		Для каждого ЭлементМассива Из МассивКолонокГруппировок Цикл
			...
			СтрокаКолонокГруппировок = СтрокаКолонокГруппировок + РазделительКолонкиГруппировок 
										+ ИдГруппыПолейТаблицы + ЭлементМассива;
			РазделительКолонкиГруппировок = ",";
		...
		Если (ОсталосьСтрок < КоличествоСтрокИтогов + 3) и (КоличествоСтрокИтогов > 0) Тогда
		...
		Если ТекЛист = ВсегоЛистов Тогда
			...
			Если ОсталосьСтрок < (3 + КоличествоСтрокИтогов + Высота_ПодвалВСтроках) Тогда
		...
		Для Каждого СтрокаДанных Из ТаблицаСтрок Цикл
			...
			// Расчитываем текст пояснения, если там есть элементы шаблона %#ИмяПоля#%.
			ТекстПоясненияИтога = СтрокаШаблонИзАгрегатныхДанных(ТекстПояснения, СтрокаДанных, 
																ИдГруппыПолейТаблицы, "%#", "#%");
...
Функция ПолучитьМакетПечатнойФормы(ИмяФормыОбъекта, ИмяРаздела, Версия = "", ОбъектОтчета  = Неопределено)
	
	ИмяПечатнойФормыРаздела = "Печатная" + Сред(Строка(ИмяФормыОбъекта), СтрНайти(Строка(ИмяФормыОбъекта), "ФормаОтчета")) 
								+ "_" + Строка(ИмяРаздела);
								
	Если ЗначениеЗаполнено(Версия) Тогда	
		ИмяПечатнойФормыРаздела = ИмяПечатнойФормыРаздела + "_" + Строка(Версия);	
	КонецЕсли; 
...
Функция ВывестиПолеЦеликом(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить = Ложь)
...
Функция ВывестиВКвадратикиОтдельноеПоле(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить = Ложь)
	...
	Если НЕ ОбластьМакета.Области.Найти(ИмяОбластиМакетаВариант1) = Неопределено Тогда
		...
		СодержитЗначение = ОбластьМакета.Области[ИмяОбластиМакетаВариант1].СодержитЗначение;
		...
		Если Не ДлинаВыводимогоЗначения = 0 Тогда
			...
			Для Ном = 1 По ДлинаВыводимогоЗначения Цикл				
				ВывестиПолеЦеликом(ОбластьМакета, ИмяОбласти + "_" + Ном, 
									Сред(СтрокаВыводимогоЗначения, Ном, 1), Очистить);
			КонецЦикла;	
	...
	ИначеЕсли НЕ ОбластьМакета.Области.Найти(ИмяОбластиМакетаВариант2) = Неопределено Тогда
		...
		СодержитЗначение = ОбластьМакета.Области[ИмяОбластиМакетаВариант2].СодержитЗначение;
		
		Если Не ДлинаВыводимогоЗначения = 0 Тогда
			
			// Например вывод дат подписания.
			Для Ном = 1 По ДлинаВыводимогоЗначения Цикл				
				ВывестиПолеЦеликом(ОбластьМакета, ИмяОбласти + Ном, 
									Сред(СтрокаВыводимогоЗначения, Ном, 1), Очистить);
			КонецЦикла;
...
Функция ВывестиВКвадратикиСтрокуТаблицы(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить = Ложь)
	...
	ИмяОбластиМакетаВариант3 = ИмяПоляСПодчеркиванием + "1" + НомерСтрокиСПодчеркиванием;
	
	Если НЕ ОбластьМакета.Области.Найти(ИмяОбластиМакетаВариант3) = Неопределено Тогда
		
		Результат = Истина;
		
		СодержитЗначение = ОбластьМакета.Области[ИмяОбластиМакетаВариант3].СодержитЗначение;
		...
		Если Не ДлинаВыводимогоЗначения = 0 Тогда
			...
			Для Ном = 1 По ДлинаВыводимогоЗначения Цикл
				...
				НомерСимвола = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(Ном);				
				ИмяОбластиМакета = ИмяПоляСПодчеркиванием + НомерСимвола + НомерСтрокиСПодчеркиванием;
				
				ВывестиПолеЦеликом(ОбластьМакета, ИмяОбластиМакета, 
									Сред(СтрокаВыводимогоЗначения, Ном, 1), Очистить);
...
Функция ВывестиВКвадратики(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить = Ложь)
	...
	Если ПозицияПодчеркивания = 0 Тогда
		...
		Результат = ВывестиВКвадратикиОтдельноеПоле(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить);
		
		Если НЕ Результат Тогда
		    // Пробуем вариант строки таблицы.
			Результат = ВывестиВКвадратикиСтрокуТаблицы(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить);		
		КонецЕсли; 
...
Функция ВывестиПолеМакета(ОбластьМакета, ИмяОбластиВывода, ВыводимоеЗначение, Очистить = Ложь)
...
Функция ЗаполнитьПоВсейОрганизации(ИдГруппы, СтруктураРеквизитовФормы, СтруктураСервисРегистров,
									КолонкиГруппировок = "", КолонкиСуммирования = "", КолонкиНеВыводить = "")
	...
	Если ВРег(КолонкиГруппировок) = ВРег("Не сворачивать") ИЛИ ВРег(КолонкиГруппировок) = ВРег("НеСворачивать") Тогда
	    Возврат Результат;	
	КонецЕсли;
	...
	Если НеЗаполненаСтрокаКолонокСуммирования или НеЗаполненаСтрокаКолонокГруппировок Тогда
		...
		Для каждого Колонка Из Результат.Колонки Цикл
            ...
            Если Колонка.ТипЗначения.СодержитТип(Тип("Число")) Тогда
				...
				Если НеЗаполненаСтрокаКолонокСуммирования Тогда
	                ...
	                КолонкиСуммирования = КолонкиСуммирования + РазделительКолонкиСуммирования + НомерКолонки;
			...
			Иначе
				...
				Если НеЗаполненаСтрокаКолонокГруппировок Тогда
	                ...
	                КолонкиГруппировок = КолонкиГруппировок + РазделительКолонкиГруппировок + НомерКолонки;
...
Функция СтруктураСобратьДанныеПоОП(ИдГруппы, ИмяРаздела, ТаблицаСтраницыРаздела, ЭтоПБОЮЛ, ИНН)
	...
	Для Каждого ЛистРаздела Из ТаблицаСтраницыРаздела Цикл
		...
		Представление = РегламентированнаяОтчетностьАЛКОКлиентСервер.СформироватьОбособленноеПодразделениеАЛКО(
		...
		ДанныеОП_Раздела[ИдОП].СведОрг.Вставить("Х"+ НомерЛистаСтроковый, Новый Структура);
...
//		Булево - Истина, если длительная операция выполнена.
//
Функция НачатьВыполнениеДлительнойОперацииАЛКО(ВидДлОперации, ПараметрыДлительнойОперации)
...
Функция ЗапуститьВыполнениеДлительнойОперацииАЛКО(ИмяЭкспортнойПроцедуры, ПараметрыЭкспортнойПроцедуры, 
							НаименованиеЗадания, РежимДлительнойОперации, УникальныйИдентификаторЗадания)
...
Процедура ВыполнитьДлительнуюОперациюСТранзакцией(ПараметрыДокумента, АдресХранилища) Экспорт
	...
	Исключение
		...
		СообщениеЖурнала = НСтр("ru = 'При выполнении длительной операции в процедуре " + ИмяПроцедурыОперации 
...
Процедура ВыполнитьДлительнуюОперациюБезТранзакции(ПараметрыДокумента, АдресХранилища) Экспорт
...
Процедура ОпределитьПараметрыОбработчикаОжиданияАЛКО(МаксимальныйИнтервал, КоэффициентУвеличенияИнтервала) Экспорт
	...
	Если КоличествоСеансов < 6 или БазаФайловая Тогда
		...
		МаксимальныйИнтервал = 7;
		КоэффициентУвеличенияИнтервала = 1.15;
...
Функция СохранитьРегистры(ПараметрыДлительнойОперации)
	...
	РежимДлительнойОперации = НЕ (СтруктураРеквизитовФормы.мБезОткрытияФормы или (НЕ РежимДлительнойОперации));
	...
	ЗаданиеВыполнено = ЗапуститьВыполнениеДлительнойОперацииАЛКО(ИмяЭкспортнойПроцедуры, ПараметрыЭкспортнойПроцедуры, 
							НаименованиеЗадания, РежимДлительнойОперации, УникальныйИдентификаторЗадания);
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистра + "
												|- ошибка блокировки при удалении записей при копировании.'"), 
										УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ИмпортИзДанныхСтарогоФормата(ДокументСсылкаАлкоОтчетПриемник, СтруктураСведенийОтчета)
	...
	Перем ИмяФормыОбъекта, НомерАлкоОтчета, СписокСохранения, СписокИменРегистровСведений, СтруктураИдГрупп, 
		СтруктураИдГруппИменРегистровСведений, СервисРегистров, СервисРегистровУдаленные, МаксИндексСтраницы;
						...
						Если ТипЗнч(СтрокаТаблЗнач.ДанныеДопСтрок) = Тип("Структура") Тогда
							...
							Для Каждого ЭлСтруктуры Из СтрокаТаблЗнач.ДанныеДопСтрок Цикл
								...
								Для Каждого ЭлементСпискаИдГруппРаздела Из СписокИдГруппРаздела Цикл
									...
									Если НомерАлкоОтчета = 6 
										или НомерАлкоОтчета = 7 Тогда
										
										СуфиксАдресногоПоля = "07";
										ИмяАдресногоПоля = ИдГруппы + СуфиксАдресногоПоля;
										
										// Для адресной информации нужно наличие поля ИдГруппы + "07XML",
			                            ...
			                            Если НЕ ЭлементСтруктуры.Значение.Колонки.Найти("ДополнительныеДанные") = Неопределено Тогда
											...
											ИмпортАдресныхДанныхТаблицИзДополнительныхДанных(
													ДополнительныеДанныеСтраницы, ИмяАдресногоПоля, ТаблЗнач);
										КонецЕсли;
...
Процедура ИмпортАдресныхДанныхТаблицИзДополнительныхДанных(ДополнительныеДанныеСтраницы, ИмяАдресногоПоля, ТаблЗнач,
								ИмяАдресногоПоляТаблицы = Неопределено, ИмяАдресногоПоляXMLТаблицы = Неопределено)
	...
	Для Каждого ЭлементДопДанных Из ДополнительныеДанныеСтраницы Цикл
		...
		Если СтрНайти(ИмяЯчейки, ИмяАдресногоПоля + "_") = 1 Тогда
			...
			АдресXML = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(
								АдресМестаДеятельности, Представление, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации);
...
Функция ВосстановитьРегистры(ПараметрыДлительнойОперации)
	...
	Если ЭтоКопирование и (НЕ ЭтоСтарыйФормат) Тогда
		...
		РежимДлительнойОперацииПриКопировании = (КоличествоСтрок > 500);
	...
	Для Каждого ЗаписьЖурналаДобавлениеСтроки Из НаборЗаписейЖурналаДобавленыхСтрок Цикл
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
				...
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистра + "
												|- ошибка блокировки при удалении добавленных, но не сохраненных записей.'"), 
										УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		ИДДокИндСтраницы = ПолучитьИдДокИндСтраницы(ДокументСсылкаАлкоОтчет, Число(ИндексРегистра));
	...
	Для Каждого ЗаписьЖурналаРедактирование Из НаборЗаписейЖурналаРедактирование Цикл
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
				...
				ОтменитьТранзакцию();
				ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистра + "
												|- ошибка блокировки при восстановлении измененных записей.'"), 
										УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПересчетИтоговПоГруппеАЛКО(ДокументСсылкаАлкоОтчет, СтруктураСведенийОтчета)
	...
	Если ТранзакцияАктивна() Тогда
		Попытка
			...
			БлокировкаДанных = Новый БлокировкаДанных;
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений." + ИмяРабочегоРегистра);
			ЭлементБлокировкиДанных.УстановитьЗначение("Документ", ДокументСсылкаАлкоОтчет);
			ЭлементБлокировкиДанных.Режим = РежимБлокировкиДанных.Разделяемый;
			БлокировкаДанных.Заблокировать();
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРабочегоРегистра + "
											|- ошибка блокировки при пересчете записей регистра.'"), 
									УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПечатьОтчета(ПараметрыДлительнойОперации)
	...
	ТаблицаПеречень = РегламентированнаяОтчетностьАЛКОКлиентСервер.ЗначениеПоКлючуСтруктуры(
													"ТаблицаПеречень", ПараметрыДлительнойОперации);
	...
	Если ПечатьУспешна Тогда
		...
		Для Индекс = 0 по ВесаСтраниц.ВГраница() Цикл
			...
			Если ОбщийВес > 0 Тогда			
				ВесаСтраниц[Индекс] = (ВесаСтраниц[Индекс] * ПроцентНаКонвертациюАрхивацию) / ОбщийВес;
			Иначе
		...
		Для Каждого Элемент Из РезультатПечати.СписокПечатаемыхЛистов Цикл
			...
			ТекущийПроцентВыполнения = ТекущийПроцентВыполнения + ВесаСтраниц[НомерЛиста-1];
			
			СообщитьПроцентВыполнения("Конвертация в выбранный формат и архивация.", 
										ТекущийПроцентВыполнения, 100, 100);
...
Процедура ОчиститьРегистрыСведенийАЛКООтМусора(ПараметрыДокумента, АдресХранилища)
...
Процедура ОчиститьРегистрСведенийАЛКООтМусора(ДокументСсылкаАлкоОтчет, СтруктураСведенийОтчета)
	...
	Если ТранзакцияАктивна() Тогда
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений." + ИмяРегистраСведений + "
											|- ошибка блокировки при очистке от мусора записей регистра.'"), 
									УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Для каждого ИндексРегистраАктивнойСтраницы Из МассивИндексовСтраниц Цикл
		...
		ИДДокИндСтраницы = ПолучитьИдДокИндСтраницы(ДокументСсылкаАлкоОтчет, ИндексРегистраАктивнойСтраницы);
		МассивИДДокИндСтраницы.Добавить(ИДДокИндСтраницы);
...
Процедура ПроверитьВыгрузкуОтчетаАЛКО(ПараметрыДокумента, АдресХранилища)
	...
	Если РезультатПроверки.ТаблицаСообщений.Количество() > 0  Тогда
		
		// Сначала объединяем ошибки и сообщения для сведения,
		// чтобы не выводить сообщения для сведения отдельно.
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатПроверки.ТаблицаСообщенийДляСведения, РезультатПроверки.ТаблицаСообщений);
		РезультатПроверки.ТаблицаСообщенийДляСведения.Очистить();
		
		ПредставлениеОшибок = ОбъектОтчета.СформироватьПредставлениеОшибок(
										РезультатПроверки.ТаблицаСообщений, ИмяФормыОбъекта);
		РезультатОперации = 1;
		
	ИначеЕсли РезультатПроверки.ТаблицаСообщенийДляСведения.Количество() > 0  Тогда
		
		ПредставлениеСообщенийДляСведения = ОбъектОтчета.СформироватьПредставлениеОшибок(
								РезультатПроверки.ТаблицаСообщенийДляСведения, ИмяФормыОбъекта, Истина);
		РезультатОперации = 2;
...
Процедура ВыгрузитьОтчетАЛКО(ПараметрыДокумента, АдресХранилища)
	...
	Если РезультатВыгрузки.ТаблицаСообщений.Количество() > 0  Тогда
		
		// Сначала объединяем ошибки и сообщения для сведения,
		// чтобы не выводить сообщения для сведения отдельно.
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатВыгрузки.ТаблицаСообщенийДляСведения, РезультатВыгрузки.ТаблицаСообщений);
		РезультатВыгрузки.ТаблицаСообщенийДляСведения.Очистить();
		
		ПредставлениеОшибок = ОбъектОтчета.СформироватьПредставлениеОшибок(
											РезультатВыгрузки.ТаблицаСообщений, ИмяФормыОбъекта);
		РезультатОперации = 1;
		
	Иначе
											
		Если РезультатВыгрузки.ТаблицаСообщенийДляСведения.Количество() > 0  Тогда
			
			ПредставлениеСообщенийДляСведения = ОбъектОтчета.СформироватьПредставлениеОшибок(
									РезультатВыгрузки.ТаблицаСообщенийДляСведения, ИмяФормыОбъекта, Истина);
			РезультатОперации = 2;
	...
	Если (ТипЗнч(ИмяФайлаXML) = Тип("Строка")) И ЗначениеЗаполнено(ИмяФайлаXML) Тогда
...
Функция ЗаполнитьПоДаннымИБ(ПараметрыДлительнойОперации)
	...
	НаименованиеЗадания = НСтр("ru = 'Автоматическое заполнение по данным ИБ отчета Алко № " + НомерАлкоОтчета + ".'");
	УникальныйИдентификаторЗадания = Неопределено;
...
Функция УдалениеДокументаОтчета(ПараметрыДлительнойОперации)
	...
	НаименованиеЗадания = НСтр("ru = 'Удаление временного документа отчета Алко № " + НомерАлкоОтчета + ".'");
	УникальныйИдентификаторЗадания = Неопределено;
...
Процедура ЗаполнитьСведенияОбОрганизацииАЛКО(Форма, ТолькоАдрес = Ложь)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаXML) Тогда
		...
		СтрокаXML = ПолучитьАдресXMLОбъекта(РабочаяОрганизация, ПредставлениеАдреса, 
								СписокВидовКонтактнойИнформации, Ложь, Форма.ДатаПодписи);
		...
		ИначеЕсли (НЕ ЗначениеЗаполнено(СтрокаXML)) и (НЕ Форма.ЭтоПБОЮЛ) Тогда
...
Процедура ЗаполнитьПоляАдресаОрганизацииАЛКО(Форма, Адрес)
	...
	СтрокаДома = СтрокаДома + ?(ПустаяСтрока(Адрес.Литера), "", ", " + Адрес.Литера);
...
Процедура ИнициализацияНаСервереАЛКО(Форма, БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Если ЕстьПереключательОП  Тогда
			...
			СписокВыбораРадиоБаттона.Добавить(0, ?(Форма.ЭтоПБОЮЛ,
													"По месту нахождения ИП",
													"По месту нахождения головной организации") );	
			
			ЗаголовокОП_ПБОЮЛ = ?(ЭтоВиноградная, "По месту нахождения виноградника", 
												  "По месту нахождения объекта торговли");										
			СписокВыбораРадиоБаттона.Добавить(1, ?(Форма.ЭтоПБОЮЛ,
													ЗаголовокОП_ПБОЮЛ,
	...
	Иначе
		...
		Форма.СтруктураРеквизитовФормы.мДокументИсточник = РегламентированнаяОтчетностьАЛКОКлиентСервер.СкопироватьЗначение(
...
Процедура ИнициализацияЭлементовТаблицыФормыАЛКО(Форма)
	...
	Если Форма.НомерАлкоОтчета >= 13 И Форма.НомерАлкоОтчета <= 15 Тогда
...
Процедура СформироватьВспомогательныеСтруктурыОтчетаАЛКО(Форма, НомерАлкоОтчета = Неопределено)
	...
	НомерАлкоОтчетаСтрока = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(Форма.НомерАлкоОтчета);
	...
	Для каждого ЭлементСтруктуры Из Форма.СтруктураРеквизитовФормы.мСтруктураИдГрупп Цикл
		...
		Форма.мСтруктураМногостраничныхРазделов.Вставить(Раздел, "ТаблицаСтраницы" + Раздел);
		...
		Для каждого ЭлементСписка Из СписокИдГруппРаздела Цикл
			...
			Иначе	
			    ИмяРегистра = "СведенияРеглОтчетАлкоПрил" + НомерАлкоОтчетаСтрока + ИмяТаблицы;
			КонецЕсли; 
	...
	Для каждого ЭлементСтруктуры Из Форма.СтруктураРеквизитовФормы.мСоответствиеИдГруппыРаздел Цикл
		// Если несколько групп в разделе, основной считается указанная первой в мСоответствиеИдГруппыРаздел.
		// Например 15-й алко.
		Если Форма.СтруктураРеквизитовФормы.мСоответствиеРазделИдГруппы.Получить(ЭлементСтруктуры.Значение) = Неопределено Тогда
		    Форма.СтруктураРеквизитовФормы.мСоответствиеРазделИдГруппы.Вставить(ЭлементСтруктуры.Значение, ЭлементСтруктуры.Ключ);		
		КонецЕсли; 
...
Функция ОпределитьМаксИндексСтраницы(Форма)
	...
	Для каждого ЭлементСтруктуры Из СтруктураМногострочныхРазделов Цикл
		...
		ИдГруппы = ЭлементСтруктуры.Ключ;
		СервисРегистровГруппы = Форма["СервисРегистров" + ИдГруппы];
		СервисРегистровУдаленныеГруппы = Форма["СервисРегистров" + ИдГруппы + "Удаленные"];
		...
		Если НЕ ДанныеВариановАвтоЗаполнения = Неопределено Тогда
			...
			Для Каждого ЭлементСтруктуры Из ДанныеВариановАвтоЗаполнения Цикл
				...
				Для Каждого ЭлементСтруктурыИдГрупп Из СтруктураИдГрупп Цикл
					...
					РазделОтчета = ЭлементСтруктурыИдГрупп.Ключ;
					...
					Для Каждого ЭлементСпискаИДРазделаОтчета Из СписокИДРазделаОтчета Цикл
						...
						Если ЭлементСпискаИДРазделаОтчета.Представление = ИмяТаблицыОтчета Тогда
		...
		Если ДанныеСтраницы.Количество()  > 0 Тогда
			...
			Если ТипЗнч(СтруктураДанных) = Тип("Структура") Тогда
				...
				Если ТипЗнч(СтруктураДополнительныхДанных) = Тип("Структура") Тогда
					...
					ИмпортАдресныхДанныхТаблицИзДополнительныхДанных(
								СтруктураДополнительныхДанных, "АдресМестаДеятельности", ТабП0000000002ТЗ,
																		"П0000000Адрес", "П0000000VNUTR");
...
Процедура ЗаполнитьОтчетныйПериодСГодомАЛКО(Форма)
	...
	Если Форма.НомерАлкоОтчета = 0 Тогда
	    ...
	    Если НомерПриложения >= 13 и НомерПриложения <= 15 Тогда
		    ЭтоЗаявлениеПоВиноградной = Истина;		
		КонецЕсли;
...
Процедура ОбновитьЦветСсылкиВидДеятельностиАЛКО(Форма)
	...
	Форма.Элементы.ВидДеятельности.ЦветТекста = ОбновитьЦветСсылкиАЛКО(
			Форма.ВидДеятельности, Форма.ЦветСтиляНезаполненныйРеквизит, Форма.ЦветСтиляЦветГиперссылкиБРО);
		
КонецПроцедуры

Процедура СформироватьПредставлениеВидаДеятельностиАЛКО(Форма, ИмяРеквизитаЗначенияВидаДеятельности)

	 Форма.ВидДеятельности = ?(ЗначениеЗаполнено(Форма[ИмяРеквизитаЗначенияВидаДеятельности]), 
	 							Форма[ИмяРеквизитаЗначенияВидаДеятельности], "Заполнить");
...
Функция УсловиеВыполненоАЛКО(ПараметрыВыгрузки, Условие, ИмяРаздела = "Титульный", 
							ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки, ВерсияБСПЧислом = Неопределено)
...
Функция ЗначениеСоответствуетФорматуАЛКО(Стр, ДокументСсылкаАлкоОтчет, ТаблицаСообщений, СтатусПроверки)
...
Процедура ОчиститьРегистрЖурнала(ДокументСсылкаАлкоОтчет, ИмяРегистра = Неопределено, 
	...
	Если ТранзакцияАктивна() Тогда		
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений.СведенияРеглОтчетАлкоЖурнал - ошибка блокировки при очистке Журнала.'"), 
									УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ДобавитьВРегистрЖурналаНапрямую(ДокументСсылкаАлкоОтчет, ИмяРегистра, ИндексСтраницыРегистра, 
	...
	Если ТранзакцияАктивна() Тогда				
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru = 'РегистрСведений.СведенияРеглОтчетАлкоЖурнал - ошибка блокировки при записи в Журнал.'"), 
									УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если НЕ Ресурсы = Неопределено Тогда
		...
		Если ТипЗнч(Ресурсы) = Тип("Структура") или ТипЗнч(Ресурсы) = Тип("ФиксированнаяСтруктура") Тогда
			...
			Если Ресурсы.Свойство("НачальноеЗначение") И (НЕ ИмяРегистра = "ЗаявлениеОКорректировке") Тогда
				...
				КонецЕсли;
			ИначеЕсли Ресурсы.Свойство("НачальноеЗначение") И (ИмяРегистра = "ЗаявлениеОКорректировке") Тогда
				НоваяЗаписьЖурналаДействие.НачальноеЗначение = Ресурсы.НачальноеЗначение;
			КонецЕсли; 
