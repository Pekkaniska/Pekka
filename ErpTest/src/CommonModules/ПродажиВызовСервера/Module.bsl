...
Функция СформироватьДанныеЗаполненияОтчетаДавальцу(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Партнер'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияКонтрагент Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Контрагент'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияОрганизация Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Организация'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПодразделение Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Подразделение'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияДоговор Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Договор'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияВалютаВзаиморасчетов Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Валюта взаиморасчетов'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияНалогообложениеНДС Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Налогообложение НДС'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияЦенаВключаетНДС Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Цена включает НДС'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияСтавкаНДС Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Ставка НДС'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияНоменклатура Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Номенклатура'"), Отказ);
	КонецЕсли;
	
	Если ВыборкаРеквизитыШапки.ЕстьОтличияХарактеристика Тогда
		СообщитьОРазличииПолейЗаказаДавальца(НСтр("ru='Характеристика'"), Отказ);
	КонецЕсли;
