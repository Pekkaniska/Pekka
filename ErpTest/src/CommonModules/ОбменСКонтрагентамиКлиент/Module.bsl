...
Процедура ОткрытьДеревоЭД(СсылкаНаОбъект, ПараметрыОткрытия = Неопределено, ЭД = Неопределено) Экспорт
	...
	Если ОбменСКонтрагентамиСлужебныйВызовСервера.ЕстьДОПоДокументу(СсылкаНаОбъект)
		ИЛИ ОбменСКонтрагентамиСлужебныйВызовСервера.НастройкаЭДСуществует(СсылкаНаОбъект) Тогда
...
Процедура ОткрытьАктуальныйЭД(ПараметрКоманды, Источник = Неопределено, ПараметрыОткрытия = Неопределено) Экспорт
	...
	ОбменСКонтрагентамиСлужебныйКлиент.ОткрытьАктуальныйЭД(ПараметрКоманды, ПараметрыОткрытия);
...
Процедура ДекорацияСостояниеЭДОНажатие(Форма, СтандартнаяОбработка = Ложь) Экспорт
	...
	Если Форма.Модифицированность Тогда
		...
		ИначеЕсли НРег(Реквизит.ТекстСостоянияЭДО) = НРег("ПригласитьКонтрагента") Тогда
			ТекстВопроса = НСтр("ru='Для того чтобы пригласить контрагента к обмену, нужно записать документ. Записать?'");
			
		ИначеЕсли НРег(Реквизит.ТекстСостоянияЭДО) = НРег("ЕстьДействующийДокумент") Тогда
			// Есть действующий документ, открываем дерево.
			ОткрытьДеревоЭД(Форма.Объект.Ссылка);
			
		ИначеЕсли НРег(Реквизит.ТекстСостоянияЭДО) = НРег("ПодключитьОрганизацию") Тогда
			ПредложениеОформитьЗаявлениеНаПодключение(Реквизит);
...
//  ВладелецФормы - Форма, УправляемаяФорма, ПолеФормы - владелец открываемой формы, которому будет отправлено оповещение о выборе.
//
Процедура ОткрытьВыборНоменклатурыКонтрагента(Знач ВладелецНоменклатуры, Знач ВладелецФормы) Экспорт
	
	ОбменСКонтрагентамиСлужебныйКлиент.ОткрытьВыборНоменклатурыКонтрагента(ВладелецНоменклатуры, ВладелецФормы);
...
//    * Форма - УправляемаяФорма - Форма учетного документа, из которого вызван диалог.
//
Процедура ДекорацияСостояниеЭДОНажатиеПродолжить(Результат, Параметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.ОК Тогда
		Параметры.Форма.Записать();
		ОткрытьДеревоЭД(Параметры.Форма.Объект.Ссылка);
	КонецЕсли;
...
Процедура ОбработкаНавигационнойСсылкиВФормеОбъектаИБ(СсылкаНаОбъект)
	...
	Если ПараметрыЭД.Свойство("НастройкаЭДО") И ЗначениеЗаполнено(ПараметрыЭД.НастройкаЭДО) Тогда
		...
		КонецЕсли;
	Иначе
		ПредложениеОформитьЗаявлениеНаПодключение(ПараметрыЭД);
	КонецЕсли;
