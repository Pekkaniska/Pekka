...
#Область ЗаголовокПротокола
	...
	Для Каждого ТекущаяОрганизация Из ОрганизацииДляПротокола Цикл
		Протокол.ДобавитьСтроку("	- "	+ ПредставлениеЗначения(ТекущаяОрганизация)
			...
			+ ПредставлениеЗначения(ПараметрыРасчета.УчетныеПолитики.МетодыОценкиСтоимости[ТекущаяОрганизация]));
	КонецЦикла;
...
#Область ЭтапыРасчета
	...
	Для Каждого ОписаниеЭтапа Из ПротоколРасчета.Этапы Цикл
		...
		Если ЗначениеЗаполнено(ОписаниеЭтапа.ВременныеТаблицы) Тогда
			...
			ВремяФормированияВсехТаблиц = Новый Структура("ВремяНачала, ВремяОкончания");
			...
			Для Каждого ОписаниеТаблицы Из ОписаниеЭтапа.ВременныеТаблицы Цикл
				...
				Для Каждого ОписаниеДействия Из ОписаниеТаблицы.Значение Цикл
					...
					Если ЗначениеЗаполнено(ОписаниеДействия.ВремяНачала) И ЗначениеЗаполнено(ОписаниеДействия.ВремяОкончания) Тогда
						Если НЕ ЗначениеЗаполнено(ВремяФормированияВсехТаблиц.ВремяНачала) Тогда
							ВремяФормированияВсехТаблиц.ВремяНачала = ОписаниеДействия.ВремяНачала;
						КонецЕсли;
						ВремяФормированияВсехТаблиц.ВремяОкончания = ОписаниеДействия.ВремяОкончания;
					КонецЕсли;
			...
			Если ЗначениеЗаполнено(ВремяФормированияВсехТаблиц.ВремяНачала) Тогда
...
#Область СохранениеПротоколаРасчета
	...
	ТекстПротокола = СтрЗаменить(ТекстПротокола, СокрЛП(УниверсальныеМеханизмыПартийИСебестоимости.СлужебныйСимвол_ТекстДляРегистрацииПроблемы()), "");
	ТекстПротокола = СтрЗаменить(ТекстПротокола, СокрЛП(УниверсальныеМеханизмыПартийИСебестоимости.СлужебныйСимвол_ПроблемаУжеЗарегистрирована()), "");
...
#КонецОбласти
	...
	Если БылиОшибкиДвижений ИЛИ БылиОшибкиЗаписи Тогда
		...
		ТекстОшибки =
			УниверсальныеМеханизмыПартийИСебестоимости.СлужебныйСимвол_ПроблемаУжеЗарегистрирована()
...
Процедура ОкончаниеЭтапаРасчета(ПараметрыРасчета) Экспорт
	...
	Для Каждого ОписаниеДвижения Из ОписаниеЭтапа.СформированныеСтрокиВТ Цикл
		...
		// Подсчитаем количество уникальных движений, сформированных на текущем этапе
		ВсегоДвижений = УниверсальныеМеханизмыПартийИСебестоимости.РазмерВременнойТаблицы(ПараметрыРасчета, ОписаниеДвижения.Ключ);
...
//
Процедура ОкончаниеФормированиеВременнойТаблицы(ПараметрыРасчета, УвеличиватьСчетчикЗамераПроизводительности = Ложь) Экспорт
	...
	Если УниверсальныеМеханизмыПартийИСебестоимости.ВременнаяТаблицаСуществует(ПараметрыРасчета, ОписаниеЗамера.ИмяВременнойТаблицы) Тогда
		КоличествоСтрок = УниверсальныеМеханизмыПартийИСебестоимости.РазмерВременнойТаблицы(ПараметрыРасчета, ОписаниеЗамера.ИмяВременнойТаблицы);
	Иначе
		КоличествоСтрок = 0;
	КонецЕсли;
