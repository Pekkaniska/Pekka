...
Процедура ОбработкаПолученияДанныхВыбора(РезультатЗапроса, ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	Если РезультатЗапроса <> Неопределено И Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				ПредставлениеСотрудника = Строка(Выборка.Сотрудник) + " (" + Выборка.Код + ")";
	...
	Если Параметры.Отбор.Свойство("ГоловнаяОрганизация") Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Отбор.ГоловнаяОрганизация) Тогда
			...
			Параметры.Отбор.Вставить("ГоловнаяОрганизация", ЗарплатаКадры.ГоловнаяОрганизация(Параметры.Отбор.ГоловнаяОрганизация));
	...
	Если НеобходимыДанныеДляПодбора
		Или НеобходимыВидыЗанятостиСотрудников Тогда
...
Процедура ОбновитьСостояниеСотрудника(Форма)
	...
	СостоянияСотрудников.СоздатьВТСостоянияСотрудниковНаДаты(Запрос.МенеджерВременныхТаблиц, Истина, ОписаниеФильтра, "ВТСведенияОСостоянииСотрудников");
	
	ПараметрыСостояния = Новый Структура;
	ПараметрыСостояния.Вставить("Сотрудник", Форма.СотрудникСсылка);
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СведенияОСостоянииСотрудников.Сотрудник КАК Сотрудник,
	|	СведенияОСостоянииСотрудников.Период КАК Начало,
	|	СведенияОСостоянииСотрудников.ПериодЗаписи КАК ПериодЗаписи,
	|	СведенияОСостоянииСотрудников.Состояние КАК Состояние,
	|	СведенияОСостоянииСотрудников.ДействуетДо КАК Окончание,
	|	СведенияОСостоянииСотрудников.ОкончаниеПредположительно КАК ОкончаниеПредположительно
	|ИЗ
	|	ВТСведенияОСостоянииСотрудников КАК СведенияОСостоянииСотрудников
	|ГДЕ
	|	СведенияОСостоянииСотрудников.Состояние В(&СостоянияОтклонений)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПараметрыСостояния.СостояниеСотрудника = Выборка.Состояние;
		ПредставлениеСостоянияСотрудника = СостоянияСотрудников.ПредставлениеСостоянияСотрудника(Выборка.Состояние);
		Если Не ПустаяСтрока(ПредставлениеСостоянияСотрудника) Тогда
			ПараметрыСостояния.ЗаголовокНадписи = ПредставлениеСостоянияСотрудника + " %2 %3";
		КонецЕсли;
		ПараметрыСостояния.ДатаОкончанияСостояния = ?(ЗначениеЗаполнено(Выборка.ОкончаниеПредположительно), Выборка.ОкончаниеПредположительно, Выборка.Окончание);
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(ПараметрыСостояния.СостояниеСотрудника) И Не ПараметрыСостояния.ЗаменитьЗаголовокНадписи Тогда
		ПараметрыСостояния.ЗаголовокНадписи = СтрШаблон(ПараметрыСостояния.ЗаголовокНадписи,
			...
			?(ЗначениеЗаполнено(ПараметрыСостояния.ДатаОкончанияСостояния), НСтр("ru='до'"), ""),
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ИспытательныйСрокСотрудников") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ИспытательныйСрокСотрудников");
		Модуль.ДобавитьПодменюПоИспытательномуСроку(ОписаниеМеню);
	КонецЕсли;
