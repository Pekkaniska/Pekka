...
Функция ПоясняющаяНадписьКМестуРаботыСотрудника(ДанныеСотрудника, РеквизитыОрганизации, ДатаСведений) Экспорт
	...
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(ДанныеСотрудника.ДатаЗавершенияРаботы) Тогда 
			СтрокаПериодРаботы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Уволен с %1'"), Формат(ДанныеСотрудника.ДатаЗавершенияРаботы, "ДЛФ=D"));
...
Процедура ПрочитатьТекущиеДанныеСотрудника(Форма)
	...
	Для Каждого ИмяЗаказываемыхКадровыхДанных Из ЗаказываемыеКадровыеДанные Цикл
		...
		ИначеЕсли ИмяЗаказываемыхКадровыхДанных = "ТекущаяТарифнаяСтавка" Тогда
			СоответсвиеКадровыхДанныхОбОплатеТруда.Вставить(ИмяЗаказываемыхКадровыхДанных, "ТарифнаяСтавка");
		ИначеЕсли ИмяЗаказываемыхКадровыхДанных = "ТекущийФОТ" Тогда
			СоответсвиеКадровыхДанныхОбОплатеТруда.Вставить(ИмяЗаказываемыхКадровыхДанных, "ФОТ");
		ИначеЕсли ИмяЗаказываемыхКадровыхДанных = "ТекущийСпособРасчетаАванса" Тогда
			СоответсвиеКадровыхДанныхОбОплатеТруда.Вставить(ИмяЗаказываемыхКадровыхДанных, "СпособРасчетаАванса");
		ИначеЕсли ИмяЗаказываемыхКадровыхДанных = "ТекущийАванс" Тогда
			СоответсвиеКадровыхДанныхОбОплатеТруда.Вставить(ИмяЗаказываемыхКадровыхДанных, "Аванс");
		Иначе
	...
	Если ТаблицаКадровыхДанных.Количество() > 0 Тогда
		...
		Для Каждого ОписаниеКадровыхДанных Из СоответсвиеКадровыхДанных Цикл
		...
		Если Не ЗначениеЗаполнено(ТаблицаКадровыхДанных[0][СоответсвиеКадровыхДанных.Получить("ТекущаяОрганизация")]) Тогда
	...
	ТаблицаКадровыхДанных = КадровыйУчет.КадровыеДанныеСотрудников(
		Истина,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Форма.Сотрудник.Ссылка),
		ОбщегоНазначения.ВыгрузитьКолонку(СоответсвиеКадровыхДанныхОбОплатеТруда, "Значение"),
		ОбщегоНазначения.ТекущаяДатаПользователя());
	
	Если ТаблицаКадровыхДанных.Количество() > 0 Тогда
		
		Для Каждого ОписаниеКадровыхДанных Из СоответсвиеКадровыхДанныхОбОплатеТруда Цикл
