...
Функция ПредставлениеПериодаВЕТИС(Знач ТочностьПериода, Знач НачалоПериода = Неопределено, Знач КонецПериода = Неопределено, Знач ПредставлениеПериода = Неопределено) Экспорт 
	...
	Если ЗначениеЗаполнено(КонецПериода)Тогда 
		ШаблонФорматнойСтроки = "ru = '%1 - %2'";
	Иначе
		ШаблонФорматнойСтроки = "ru = '%1'";
	КонецЕсли;
	...
	Возврат НСтр(ШаблонФорматнойСтроки);
...
Функция СоответствиеПолейДокументовОснованийИДокументовВЕТИС(ТипДокумента, ТипОснования) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.ИсходящаяТранспортнаяОперацияВЕТИС")Тогда
		Возврат СоответствиеПоТипу(ИнтеграцияВЕТИСКлиентСерверПереопределяемый.СоответствиеПолейДокументовОснованийИИсходящейТранспортнойОперации(),ТипОснования);
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС")Тогда
		Возврат СоответствиеПоТипу(ИнтеграцияВЕТИСКлиентСерверПереопределяемый.СоответствиеПолейДокументовОснованийИВходящейТранспортнойОперации(),ТипОснования);
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ИнвентаризацияПродукцииВЕТИС")Тогда
		Возврат СоответствиеПоТипу(ИнтеграцияВЕТИСКлиентСерверПереопределяемый.СоответствиеПолейДокументовОснованийИИнвентаризацииПродукции(),ТипОснования);
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ПроизводственнаяОперацияВЕТИС")Тогда
		Возврат СоответствиеПоТипу(ИнтеграцияВЕТИСКлиентСерверПереопределяемый.СоответствиеПолейДокументовОснованийИПроизводственнойОперации(),ТипОснования)
	КонецЕсли;
...
// Получает хозяйствующие субъекты из настроек обмена.
//
// Параметры:
...
Функция КоличествоСтраниц(ОбщееКоличество) Экспорт
	
	КоличествоСтраниц = ОбщееКоличество/РазмерСтраницы();
...
//   * Шаблон      - Строка - шаблон вывода строки коллекции (по умолчанию "%1");
...
Функция ПараметрыПредставленияТабличнойЧастиПроизводителей(ПустоеКоличество = Неопределено) Экспорт
	...
	Результат = ПараметрыПредставленияТабличнойЧасти("Производитель");
...
Функция СформироватьНадписьПоДаннымТабличнойЧасти(Данные, ПараметрыНадписи) Экспорт
	...
	Для НомерВыводимойСтроки = 1 По КоличествоСтрок Цикл
		...
		Для Ит = 0 По ИменаКолонок.ВГраница() Цикл
			ЗначениеПараметра = ПараметрыНадписи.НеЗаполнено;
...
Процедура НастроитьОтборПоОрганизацииВЕТИС(Форма,Знач Результат, Префикс = Неопределено, Знач ЗначениеПрефиксы = Неопределено) Экспорт
	...
	Если ОбновлятьПредставление Тогда 
		...
		Если ТипЗнч(Результат) = Тип("СправочникСсылка.ХозяйствующиеСубъектыВЕТИС") Тогда
			...
			КонецЦикла;
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") И ИнтеграцияИСКлиентСервер.ЕстьРеквизитОбъекта(Результат,"ВыбранныеЗначения") Тогда
...
Процедура ОрганизацияВЕТИСОтборПриИзменении(Форма, ПрефиксСписков) Экспорт 
	...
	Если ИнтеграцияИСКлиентСервер.ЕстьРеквизитОбъекта(Форма, "Список") Тогда
		УстановитьОтборОрганизацияПредприятиеВЕТИС(Форма.Список, Форма.ОрганизацииВЕТИС, ПрефиксСписков);
	КонецЕсли;
	
	Если ИнтеграцияИСКлиентСервер.ЕстьРеквизитОбъекта(Форма, "СписокКОформлению") Тогда
		УстановитьОтборОрганизацияПредприятиеВЕТИС(Форма.СписокКОформлению, Форма.ОрганизацииВЕТИС, ПрефиксСписков);
	КонецЕсли;
	
	Если ИнтеграцияИСКлиентСервер.ЕстьРеквизитОбъекта(Форма, "СписокТТНКОформлению") Тогда
		УстановитьОтборОрганизацияПредприятиеВЕТИС(Форма.СписокТТНКОформлению, Форма.ОрганизацииВЕТИС, ПрефиксСписков);
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьОтборОрганизацияПредприятиеВЕТИС(Список, ДеревоОтбора, Префиксы) Экспорт 
	
	КорневаяГруппа = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
	...
	Если МассивПрефиксов.Количество() = 1 Тогда 
		...
		Для Каждого Организация Из ДеревоОтбора.ПолучитьЭлементы() Цикл
			...
			Если Организация.ПолучитьЭлементы().Количество() Тогда 
...
Функция ДеревоОтбораОрганизацииВЕТИСВМассивСтруктур(ДеревоХозяйствующихСубъектовПредприятийВЕТИС, ВсеСтроки = Истина, ФормироватьХэш = Истина, ВсеПредприятияВыбраны = Истина) Экспорт 
	...
	Для Каждого СтрокаОрганизация Из ЭлементыДерева Цикл
		Если ВсеСтроки ИЛИ СтрокаОрганизация.Выбрана Тогда 
			НовыйЭлемент = Новый Структура("Организация,Предприятия,ВсеПредприятияВыбраны",СтрокаОрганизация.ХозяйствующийСубъектПредприятиеВЕТИС, Новый Массив, ВсеПредприятияВыбраны);
			...
			Если ФормироватьХэш Тогда 
				...
				Пока СтрДлина(ХэшСтроки)>32 Цикл 
					ХэшБуфер.ЗаписатьПобитовоеИсключительноеИли(0, ПолучитьБуферДвоичныхДанныхИзСтроки(Лев(ХэшСтроки,32),КодировкаТекста.UTF8));
...
Функция ПредставлениеВыбранныхНастроек(ДеревоОтбора,ХэшСтроки="",СписокВыбора = Неопределено) Экспорт 
	...
	Если ТипЗнч(СписокВыбора) = Тип("СписокЗначений")Тогда 
		Для Каждого ЭлементСписка Из СписокВыбора Цикл 
			Если ИнтеграцияИСКлиентСервер.ЕстьРеквизитОбъекта(ЭлементСписка.Значение,"ХэшСтроки")Тогда 
	...
	Возврат "" + ПредставлениеОрганизации + " (" + ПредставлениеПредприятия + ")";
...
КонецФункции // ТочностьДатПродукцииИзСвязаннойТаблицы()
