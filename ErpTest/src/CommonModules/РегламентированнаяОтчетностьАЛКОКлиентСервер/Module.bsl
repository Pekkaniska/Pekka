...
Функция СформироватьОбособленноеПодразделениеАЛКО(СведенияОбОП, ЭтоПБОЮЛ, ОрганизацияИНН) Экспорт
	...
	ВыводитьРеквизиты = ЗначениеЗаполнено(Наименование)	                 
	                ИЛИ ЗначениеЗаполнено(КПП)
	                ИЛИ ЗначениеЗаполнено( СтрЗаменить(ПредставлениеАдреса, ",", "") );
	
	Если ВыводитьРеквизиты Тогда
		НаименованиеОП = "";
		Если НЕ ПустаяСтрока(Наименование) Тогда
			НаименованиеОП = Наименование;
		КонецЕсли;
				
		Если НЕ (ПустаяСтрока(ОрганизацияИНН) или (ОрганизацияИНН = "Заполнить") ) Тогда
			НаименованиеОП = НаименованиеОП + ?(ПустаяСтрока(НаименованиеОП), "", ", ") + "ИНН " + ОрганизацияИНН;
		КонецЕсли;
		
		Если НЕ (ЭтоПБОЮЛ  или ПустаяСтрока(КПП)) Тогда		
			НаименованиеОП = НаименованиеОП + ?(ПустаяСтрока(НаименованиеОП), "", ", ") + "КПП " + КПП;					
		КонецЕсли; 
		
		Если НЕ ПустаяСтрока( СтрЗаменить(ПредставлениеАдреса, ",", "") ) Тогда
			НаименованиеОП = НаименованиеОП + 
						?(ПустаяСтрока(НаименованиеОП), "", Символы.ПС) + ПредставлениеАдреса;
		КонецЕсли;
	Иначе		
		НаименованиеОП = "";
	КонецЕсли;
...
Процедура ЗагрузитьРекивизитыЗаявленияОКорректировкиАЛКО(СтруктураРеквизитовФормы, СтруктураДанныхТитульный,
														ПринудительноЗаполнить = Ложь) Экспорт
