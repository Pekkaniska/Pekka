...
//		* мДатаКонцаПериодаОтчета 			  - Дата - Дата конца периода отчета;
//		* Периодичность			 			  - ПеречислениеСсылка.Периодичность - Периодичность формирования отчета.
// 
Процедура ЗаполнитьФормуСтатистики(Контейнер, Знач ИмяОтчета, Знач ИмяРедакцииОтчета, ПараметрыОтчета) Экспорт
	...
	// Некоторые формы статистики заполняются отдельно по каждому обособленному подразделению,
	...
	РассчитатьЗначенияПоказателей(Показатели, ТаблицаРасшифровки, ОтборПоОбособленнымПодразделениям);
...
//
Процедура РассчитатьЗначенияПоказателей(Показатели, ТаблицаРасшифровки, ОтборПоОбособленнымПодразделениям = Неопределено) Экспорт
	...
	Для Каждого ОписаниеЗапроса Из ЗапросыДляВыполнения Цикл
		...
		Если ОписаниеЗапроса.Группировка Тогда
			...
			БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметрВывода(КомпоновщикНастроек.Настройки, "ВертикальноеРасположениеОбщихИтогов", РасположениеГруппировкиКомпоновкиДанных.Нет);
...
Функция СериализоватьРезультатНастройки(КомпоновщикНастроек) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Сохранение настроек форм статистики'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
