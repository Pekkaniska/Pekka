...
Процедура ЗаполнитьДеревоСебестоимости(ПараметрыДерева, ПараметрыУзла)
	...
	Для Каждого ТекПартия Из ВыпущеннаяПродукция Цикл
		...
		ИначеЕсли ТекПартия.ТребуетсяРазузлование Тогда
			...
			Отборы.Знаменатель = ТекПартия.Знаменатель;
...
Процедура РазузловатьУзелДереваСебестоимости(ПараметрыДерева, ПараметрыУзла, ТаблицаРезультата, ОписаниеПродукции)
	...
	Для Каждого ТекПартия Из СебестоимостьУзла Цикл
		...
		ПривестиЧислительИЗнаменатель(ТекПартия);
...
Функция ТекстЗапросаЗатратыНаПродукцию()
	...
	ТекстМатериальныеЗатраты =
		...
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНЗП
		|		ПО СебестоимостьТоваров.АналитикаУчетаНоменклатуры = АналитикаНЗП.КлючАналитики
		|
		|ГДЕ
		|	НЕ СебестоимостьТоваров.Партия = НЕОПРЕДЕЛЕНО
		|	И СебестоимостьТоваров.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	И НЕ АналитикаНЗП.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК АналитикаНЗП
		|		ПО СебестоимостьТоваров.АналитикаУчетаНоменклатуры = АналитикаНЗП.КлючАналитики
		|ГДЕ
		...
		|	И СебестоимостьТоваров.ТипЗаписи В (ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия),
	...
	ТекстМатериалыОбъединение =
		...
		|	КлючиАналитикиЗатрат.Номенклатура					КАК Затрата,
		|	СпрНоменклатура.ЕдиницаИзмерения					КАК ЕдиницаИзмеренияЗатрата,
		|	КлючиАналитикиЗатрат.Характеристика					КАК ХарактеристикаЗатрата,
		|	КлючиАналитикиЗатрат.Серия							КАК СерияЗатрата,
		|	КлючиАналитикиЗатрат.Назначение						КАК НазначениеЗатрата,
		|	КлючиАналитикНЗП.СтатьяКалькуляции					КАК СтатьяКалькуляции,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикНЗП.Склад) = ТИП(Справочник.СтруктураПредприятия)
		|			ТОГДА КлючиАналитикНЗП.Склад
		...
		|			/ ПартииМатериаловНЗП.Знаменатель) 			КАК Числитель,
		|	ЕСТЬNULL(ПартииПолуфабрикатов.ОбщееКоличество, 0)
		|		* ИтогоПартииМатериаловНЗП.КоличествоЗатрата 	КАК Знаменатель,
		|	ПартииМатериалов.КоличествоЗатрата
		|			* (ПартииМатериаловНЗП.КоличествоЗатрата
		|				* ПартииМатериаловНЗП.Числитель)
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ИтогоПартииМатериаловНЗП КАК ИтогоПартииМатериаловНЗП
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикНЗП
		|		ПО КлючиАналитикНЗП.КлючАналитики = ПартииМатериалов.АналитикаЗатратаНЗП
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК КлючиАналитикиЗатрат
		|		ПО КлючиАналитикиЗатрат.КлючАналитики = ПартииМатериалов.АналитикаЗатрата
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
		|		ПО КлючиАналитикиЗатрат.Номенклатура = СпрНоменклатура.Ссылка
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СпрСклады
		|		ПО КлючиАналитикНЗП.Склад = СпрСклады.Ссылка
		|";
	...
	ТекстТрудозатратыОбъединение =
		...
		|	ЛОЖЬ											КАК ТребуетсяРазузлование,
		|	""Трудозатраты""								КАК ТипЗатрат,
		|	МАКСИМУМ(&Числитель)							КАК Числитель,
		|	МАКСИМУМ(&Знаменатель)							КАК Знаменатель,
		|	СУММА(ТрудозатратыНЗП.Количество
		|		* &Числитель
		|		/ &Знаменатель)								КАК КоличествоЗатрата,
		|
		|// Стоимости согласно параметру ""ДанныеПоСебестоимости""
		|	0												КАК Материальные,
		|	СУММА(ВЫБОР
		...
		|			ТОГДА ТрудозатратыНЗП.СтоимостьРегл
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)							КАК СуммаЗатрата,
		|
		|// Стоимости с НДС
		|	0												КАК МатериальныеСНДС,
		|	СУММА(ТрудозатратыНЗП.Стоимость
		|			* &Числитель
		|			/ &Знаменатель)							КАК ТрудозатратыСНДС,
		|	0												КАК ПостатейныеПостоянныеСНДС,
		|	0												КАК ПостатейныеПеременныеСНДС,
		|	0												КАК ДопРасходыСНДС,
		|
		...
		|	СУММА(ТрудозатратыНЗП.СтоимостьРегл
		|			* &Числитель
		|			/ &Знаменатель)							КАК ТрудозатратыРегл,
		|	0												КАК ПостатейныеПостоянныеРегл,
		|	0												КАК ПостатейныеПеременныеРегл,
		|	0												КАК ДопРасходыРегл,
		|	0												КАК СтоимостьЗабалансоваяРегл
	...
	ТекстПрочиеРасходыОбъединение =
		...
		|			ТОГДА ПрочиеРасходыНЗП.СтоимостьРегл
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПостоянные,
		|	СУММА(ВЫБОР
		|		КОГДА СтатьиРасходов.ХарактерПроизводственныхЗатрат = ЗНАЧЕНИЕ(Перечисление.ХарактерПроизводственныхЗатрат.Постоянные)
		|			ТОГДА 0
		|		КОГДА &ДанныеПоСебестоимости = 1
		...
		|			ТОГДА ПрочиеРасходыНЗП.СтоимостьРегл
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПеременные,
		|	0											КАК ДопРасходы,
		|	0											КАК СуммаЗатратаЗабалансовая,
		|	СУММА(ВЫБОР
		|		КОГДА &ДанныеПоСебестоимости = 1
		...
		|		ИНАЧЕ ПрочиеРасходыНЗП.Стоимость
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПостоянныеСНДС,
		|	СУММА(ВЫБОР
		|		КОГДА СтатьиРасходов.ХарактерПроизводственныхЗатрат = ЗНАЧЕНИЕ(Перечисление.ХарактерПроизводственныхЗатрат.Постоянные)
		|			ТОГДА 0
		|		ИНАЧЕ ПрочиеРасходыНЗП.Стоимость
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПеременныеСНДС,
		|	0											КАК ДопРасходыСНДС,
		|
		|// Стоимости без НДС
		|	0											КАК МатериальныеБезНДС,
		...
		|		ИНАЧЕ ПрочиеРасходыНЗП.СтоимостьБезНДС
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПостоянныеБезНДС,
		|	СУММА(ВЫБОР
		|		КОГДА СтатьиРасходов.ХарактерПроизводственныхЗатрат = ЗНАЧЕНИЕ(Перечисление.ХарактерПроизводственныхЗатрат.Постоянные)
		|			ТОГДА 0
		|		ИНАЧЕ ПрочиеРасходыНЗП.СтоимостьБезНДС
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПеременныеБезНДС,
		|	0											КАК ДопРасходыБезНДС,
		|
		|// Стоимости Регл
		|	0											КАК МатериальныеРегл,
		...
		|		ИНАЧЕ ПрочиеРасходыНЗП.СтоимостьРегл
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПостоянныеРегл,
		|	СУММА(ВЫБОР
		|		КОГДА СтатьиРасходов.ХарактерПроизводственныхЗатрат = ЗНАЧЕНИЕ(Перечисление.ХарактерПроизводственныхЗатрат.Постоянные)
		|			ТОГДА 0
		|		ИНАЧЕ ПрочиеРасходыНЗП.СтоимостьРегл
		|	КОНЕЦ * &Числитель
		|			/ &Знаменатель)						КАК ПостатейныеПеременныеРегл,
		|	0											КАК ДопРасходыРегл,
		|	0											КАК СтоимостьЗабалансоваяРегл
		|ИЗ
		|	РаботыДавальца КАК РаботыДавальца
...
Процедура ПривестиЧислительИЗнаменатель(Узел)
	
	Если Узел.Знаменатель = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если Узел.Числитель % Узел.Знаменатель = 0 Тогда
		
		Узел.Числитель = Узел.Числитель / Узел.Знаменатель;
		Узел.Знаменатель = 1;
	...
	Если Узел.Знаменатель % Узел.Числитель = 0 Тогда
		
		Узел.Знаменатель = Узел.Знаменатель / Узел.Числитель;
		Узел.Числитель = 1;
	...
	Число1 = Узел.Числитель;
	Число2 = Узел.Знаменатель;
	...
	Если Число1 + Число2 < 1 Тогда
		Возврат;
	КонецЕсли;
	
	Узел.Числитель = Узел.Числитель / (Число1 + Число2);
	Узел.Знаменатель = Узел.Знаменатель / (Число1 + Число2);
