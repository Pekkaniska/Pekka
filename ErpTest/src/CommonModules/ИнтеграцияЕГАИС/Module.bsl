...
// Устарело. Можно использовать имя библиотечного типа.
...
Функция СправочникВидовАлкогольнойПродукции() Экспорт
	...
	Возврат Метаданные.Справочники.ВидыАлкогольнойПродукции.Имя;
...
Функция ЗначениеОпределяемогоТипаПоНаименованию(ИмяТипа, Наименование, ТочноеСоответствие, Родитель = Неопределено, Владелец = Неопределено) Экспорт
	...
	НайденноеЗначение     = ПустоеЗначениеОпределяемогоТипа(ИмяТипа);
	...
	Для Каждого ТипОпределяемогоТипа Из ТипыОпределяемогоТипа Цикл
		...
		ПолноеИмяТипа     = Метаданные.НайтиПоТипу(ТипОпределяемогоТипа).ПолноеИмя();
		МенеджерОбъекта   = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмяТипа);
		НайденноеЗначение = МенеджерОбъекта.НайтиПоНаименованию(Наименование, ТочноеСоответствие, Родитель, Владелец);
...
Функция ЗагрузитьТТНВходящуюЕГАИС(ДанныеДокументаТТН, ДанныеДокументаУведомлениеОРегистрации, ОрганизацияЕГАИС, ДанныеДокументаИсторияПоСправкам2) Экспорт
	...
	Если РезультатДобавленияЗаписиТТН.НовоеСообщение Тогда
		...
		Если ДанныеДокументаИсторияПоСправкам2 <> Неопределено Тогда
			...
			РезультатДобавленияЗаписиИстории = ИнтеграцияЕГАИС.ДобавитьЗаписьВПротоколОбмена(
...
//
Функция ЗагрузитьОрганизацию(ДанныеОрганизацииXDTO) Экспорт
...
Функция ЗагрузитьКвитанциюПолученЕГАИС(ДанныеДокумента, ДанныеОснованияКвитанции, ИдентификаторЕГАИС, ОрганизацияЕГАИС) Экспорт
	...
	Если РезультатДобавленияЗаписи.НовоеСообщение Тогда
		...
		Если ОтразитьФиксацию И ЗначениеЗаполнено(ДанныеОснованияКвитанции.Ссылка) Тогда
			...
			МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ОснованиеКвитанцииМетаданные.ПолноеИмя());
			...
			Если ЗаписыватьДокумент Тогда
				...
				Если ПерезаписатьИдентификаторЕГАИС(ОснованиеКвитанцииМетаданные.Имя) Тогда
					...
					КонецЕсли;
				ИначеЕсли Не ЗначениеЗаполнено(ДокументОбъект.ИдентификаторЕГАИС) Тогда
					ДокументОбъект.ИдентификаторЕГАИС = ИдентификаторЕГАИС;
				КонецЕсли;
...
Процедура ЗаполнитьСвойствоXDTO(ОбъектXDTO, ИмяСвойства, ЗначениеСвойства = Неопределено, КешОшибок = Неопределено, Глубина = Неопределено) Экспорт
	...
	ДанныеДляРасшифровкиОшибок = ИнтеграцияГосИС.ДанныеДляРасшифровкиОшибок("ОбщийМакет.ПредставленияПолейЕГАИС", Глубина);
...
Процедура УстановитьВидимостьКомандыВыполнитьОбмен(Форма, ИмяЭлемента) Экспорт
	...
	Если НЕ ИнтеграцияЕГАИСПереопределяемый.ЕстьПравоИзмененияДокументовЕГАИС() Тогда
		Кнопка = Форма.Элементы.Найти(ИмяЭлемента);
		Если Кнопка <> Неопределено Тогда
			Кнопка.Видимость = Ложь;
		КонецЕсли;
...
Функция РезультатПроверкиДанныхТСД(МассивСчитанныхДанных, ПараметрыПроверки) Экспорт
	
	Если ПараметрыПроверки.ЭтоФормаДокумента Тогда
		СоответствиеШтрихкодовСтрокДерева  = Неопределено;
		ДетализацияСтруктурыХранения       = Неопределено;
		ПараметрыСканированияАкцизныхМарок = АкцизныеМаркиКлиентСервер.ПараметрыСканированияАкцизныхМарок(ПараметрыПроверки.Форма);
		ДеревоМаркированнойПродукции       = Неопределено;
	Иначе
		ПараметрыСканированияАкцизныхМарок = ПараметрыПроверки.ПараметрыСканированияАкцизныхМарок;
		СоответствиеШтрихкодовСтрокДерева  = ПараметрыПроверки.СоответствиеШтрихкодовСтрокДерева;
		ДетализацияСтруктурыХранения       = ПараметрыПроверки.ДетализацияСтруктурыХранения;
		ДеревоМаркированнойПродукции       = ПараметрыПроверки.Форма.ДеревоМаркированнойПродукции;
	КонецЕсли;
	
	ОрганизацияЕГАИС = ПараметрыПроверки.ОрганизацияЕГАИС;
	
	ЗагрузкаДанныхТСД = Новый Структура;
	ЗагрузкаДанныхТСД.Вставить("ЕстьОшибки",                                 Ложь);
	ЗагрузкаДанныхТСД.Вставить("ТекстОшибки",                                "");
	...
	ЗагрузкаДанныхТСД.Вставить("ЕстьОшибкиВДеревеУпаковок",                  Ложь);
	ЗагрузкаДанныхТСД.Вставить("АдресДереваУпаковок",                        "");
	ЗагрузкаДанныхТСД.Вставить("АдресВложенныеШтрихКоды",                    "");
	ЗагрузкаДанныхТСД.Вставить("МаркируемаяАлкогольнаяПродукция",            Новый Массив);
	...
	ЗагрузкаДанныхТСД.Вставить("АдресДанныхДокументаОснования",              Новый Массив);
	ЗагрузкаДанныхТСД.Вставить("ДокументОснование",                          Новый Массив);
	...
	ТаблицаНеАкцизныеМарки.Колонки.Добавить("Количество",                        ОбщегоНазначения.ОписаниеТипаЧисло(15, 3, ДопустимыйЗнак.Неотрицательный));
	...
	Для Каждого ЭлементМассива Из МассивСчитанныхДанных Цикл
		
		Если ЭлементМассива.Свойство("ШтрихкодМаркиАлкогольнойПродукции")
			И Не ПустаяСтрока(ЭлементМассива.ШтрихкодМаркиАлкогольнойПродукции) Тогда
			
			ДанныеМаркируемойПродукции = Новый Структура;
			ДанныеМаркируемойПродукции.Вставить("ШтриховойКодТовара",         ЭлементМассива.Штрихкод);
			ДанныеМаркируемойПродукции.Вставить("АкцизнаяМарка",              ЭлементМассива.ШтрихкодМаркиАлкогольнойПродукции);
			ДанныеМаркируемойПродукции.Вставить("ДанныеШтрихкода",            Неопределено);
			
			ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Добавить(ДанныеМаркируемойПродукции);
			
			СоответствиеСчитанныхАкцизныхМарок.Вставить(ЭлементМассива.ШтрихкодМаркиАлкогольнойПродукции, "");
			
		Иначе
			
			Если АкцизныеМаркиВызовСервера.ЭтоШтрихкодАкцизнойМарки(ЭлементМассива.Штрихкод) Тогда
				
				ДанныеМаркируемойПродукции = Новый Структура;
				ДанныеМаркируемойПродукции.Вставить("ШтриховойКодТовара",        "");
				ДанныеМаркируемойПродукции.Вставить("АкцизнаяМарка",             ЭлементМассива.Штрихкод);
				ДанныеМаркируемойПродукции.Вставить("ДанныеШтрихкода",            Неопределено);
			...
			Иначе
				...
				СоответствиеСчитанныхДанныхНеАкцизныхМарок.Вставить(ЭлементМассива.Штрихкод, ЭлементМассива);
				
				НоваяСтрока = ТаблицаНеАкцизныеМарки.Добавить();
				НоваяСтрока.ШтриховойКод = ЭлементМассива.Штрихкод;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНеАкцизныеМарки Цикл
		...
		Если СтрокаТаблицы.ЭтоУпаковка Тогда
		...
		ИначеЕсли СтрокаТаблицы.ЭтоИмеющаясяВБазеУпаковка Тогда
		...
		ИначеЕсли СтрокаТаблицы.ЭтоПартионнаяАлкогольнаяПродукция Тогда
			...
			Если ПараметрыПроверки.ЭтоФормаДокумента Тогда
				...
				ЗагрузкаДанныхТСД.ПартионнаяАлкогольнаяПродукция.Добавить(СоответствиеСчитанныхДанныхНеАкцизныхМарок[СтрокаТаблицы.ШтриховойКод]);
			...
			Иначе
				...
				ДанныеПартионнойПродукции.Вставить("ДанныеШтрихкода", Неопределено);
				
				ЗагрузкаДанныхТСД.ПартионнаяАлкогольнаяПродукция.Добавить(ДанныеПартионнойПродукции);
			КонецЕсли;
	...
	КоличествоНовыхУпаковок            = НовыеУпаковки.Количество();
	КоличествоИмеющихсяУпаковок        = СуществующиеУпаковки.Количество();
	КоличествоСтрокПартионнойПродукции = ЗагрузкаДанныхТСД.ПартионнаяАлкогольнаяПродукция.Количество();
	
	Если ДетализацияСтруктурыХранения = Неопределено Тогда
		
		НовыеУпаковки.Очистить();
		
		Если ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() = 0 
			И КоличествоСтрокПартионнойПродукции = 0
			И КоличествоИмеющихсяУпаковок = 0 Тогда
			
			ЗагрузкаДанныхТСД.ЕстьОшибки = Истина;
			ЗагрузкаДанныхТСД.ТекстОшибки = НСтр("ru = 'В считанных штриховых кодах нет данных ни по упаковкам, ни по алкогольной продукции.'");
			Возврат ЗагрузкаДанныхТСД;
			
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗагрузкаДанныхТСД.Упаковки, СуществующиеУпаковки, Истина);
		
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Бутылки Тогда
		
		НовыеУпаковки.Очистить();
		
		Если КоличествоИмеющихсяУпаковок <> 0 Тогда
			
			ДанныеИмеющихсяУпаковок = Справочники.ШтрихкодыУпаковокТоваров.ВложенныеШтрихкодыУпаковок(СсылкиНаСуществующиеУпаковки, ОрганизацияЕГАИС);
			
			Для Каждого СтрокаТаблицы Из ДанныеИмеющихсяУпаковок.МаркированныеТовары Цикл
				
				Если СоответствиеСчитанныхАкцизныхМарок.Получить(СтрокаТаблицы.ЗначениеШтрихкода) = Неопределено Тогда
					
					ДанныеМаркируемойПродукции = Новый Структура;
					ДанныеМаркируемойПродукции.Вставить("ШтриховойКодТовара", "");
					ДанныеМаркируемойПродукции.Вставить("АкцизнаяМарка",      СтрокаТаблицы.ЗначениеШтрихкода);
					ДанныеМаркируемойПродукции.Вставить("ДанныеШтрихкода",    Неопределено);
					
					ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Добавить(ДанныеМаркируемойПродукции);
					
					СоответствиеСчитанныхАкцизныхМарок.Вставить(СтрокаТаблицы.ЗначениеШтрихкода, "");
		...
		Если ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() = 0 
			И КоличествоСтрокПартионнойПродукции = 0 Тогда
			
			ЗагрузкаДанныхТСД.ЕстьОшибки = Истина;
			ЗагрузкаДанныхТСД.ТекстОшибки = НСтр("ru = 'В считанных штриховых кодах нет данных по бутылкам и партионной алкогольной продукции.'");
			Возврат ЗагрузкаДанныхТСД;
	...
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.КоробаСБутылками Тогда
		...
		Если ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() = 0 Тогда
			
			Если КоличествоСтрокПартионнойПродукции   = 0 
				И КоличествоИмеющихсяУпаковок       = 0
				И КоличествоНовыхУпаковок           = 0 Тогда
		...
		Иначе
			...
			Если (КоличествоНовыхУпаковок + КоличествоИмеющихсяУпаковок) > 1 Тогда
					
					ЗагрузкаДанныхТСД.ЕстьОшибки = Истина;
					ЗагрузкаДанныхТСД.ТекстОшибки = НСтр("ru = 'Одновременно считана маркируемая алкогольная продукция и несколько упаковок. Данные обработаны не будут.'");
					Возврат ЗагрузкаДанныхТСД;
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗагрузкаДанныхТСД.Упаковки, НовыеУпаковки);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗагрузкаДанныхТСД.Упаковки, СуществующиеУпаковки);
		
	ИначеЕсли ДетализацияСтруктурыХранения = Перечисления.ДетализацияСтруктурыХраненияАлкогольнойПродукции.Полная Тогда
		...
		Если ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция.Количество() = 0 Тогда
			...
			ИначеЕсли КоличествоИмеющихсяУпаковок + КоличествоНовыхУпаковок > 1 Тогда
				
				ПроверитьКорректностьСчитанныхТСДУпаковок(ЗагрузкаДанныхТСД, ДеревоМаркированнойПродукции, НовыеУпаковки,
				                                          СуществующиеУпаковки, СоответствиеШтрихкодовСтрокДерева);
				
				Если ЗагрузкаДанныхТСД.ЕстьОшибки Тогда
					Возврат ЗагрузкаДанныхТСД;
				КонецЕсли;
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗагрузкаДанныхТСД.Упаковки, НовыеУпаковки, Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗагрузкаДанныхТСД.Упаковки, СуществующиеУпаковки, Истина);
		
	КонецЕсли;
	
	ПолучитьДанныеПоШтрихКодамТСД(ЗагрузкаДанныхТСД, ПараметрыСканированияАкцизныхМарок, ПараметрыПроверки);
	
	Если ПараметрыПроверки.ЭтоФормаДокумента
		И ПараметрыСканированияАкцизныхМарок.ЗапрашиватьНоменклатуру 
		И Не ЗагрузкаДанныхТСД.ЕстьОшибкиВДеревеУпаковок Тогда
		ПроверитьНеобходимостьУказанияДополнительныхДанныхДляАкцизныхМарок(ЗагрузкаДанныхТСД, ПараметрыСканированияАкцизныхМарок);
	КонецЕсли;
	
	АкцизныеМаркиЕГАИС.ДанныеДокументаОснования(ПараметрыСканированияАкцизныхМарок);
	
	ЗагрузкаДанныхТСД.АдресДанныхДокументаОснования = ПараметрыСканированияАкцизныхМарок.АдресДанныхДокументаОснования;
	ЗагрузкаДанныхТСД.ДокументОснование             = ПараметрыСканированияАкцизныхМарок.ДокументОснование;
	
	Возврат ЗагрузкаДанныхТСД;
	
КонецФункции

Процедура ПроверитьНеобходимостьУказанияДополнительныхДанныхДляАкцизныхМарок(ЗагрузкаДанныхТСД, ПараметрыСканированияАкцизныхМарок)
	
	МассивТребуетсяУказаниеДанных   = Новый Массив;
	МассивНеТребуетсяУказаниеДанных = Новый Массив;
	
	ДанныеДокументаОснования = Неопределено;
	
	Если ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.ДокументОснование) Тогда
		ДанныеДокументаОснования = АкцизныеМаркиЕГАИС.ДанныеДокументаОснования(ПараметрыСканированияАкцизныхМарок);
	КонецЕсли;
	
	Для Каждого ЭлементМассива Из ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция Цикл
		
		ТребуетсяУказаниеДопДанных = Ложь;
		
		Если Не ЗначениеЗаполнено(ЭлементМассива.ДанныеШтрихкода.Номенклатура) Тогда
			ТребуетсяУказаниеДопДанных = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПараметрыСканированияАкцизныхМарок.ДокументОснование)
			И ДанныеДокументаОснования <> Неопределено
			И Не ТребуетсяУказаниеДопДанных 
			И Не ЗначениеЗаполнено(ЭлементМассива.ДанныеШтрихкода.Серия) Тогда
			
			ПараметрыПоиска = Новый Структура;
			ПараметрыПоиска.Вставить("Номенклатура",   ЭлементМассива.ДанныеШтрихкода.Номенклатура);
			ПараметрыПоиска.Вставить("Характеристика", ЭлементМассива.ДанныеШтрихкода.Характеристика);
		...
		Если ТребуетсяУказаниеДопДанных Тогда
			МассивТребуетсяУказаниеДанных.Добавить(ЭлементМассива);
		Иначе
			МассивНеТребуетсяУказаниеДанных.Добавить(ЭлементМассива);
		КонецЕсли;
		
	КонецЦикла;
	
	ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукция            = МассивНеТребуетсяУказаниеДанных;
	ЗагрузкаДанныхТСД.МаркируемаяАлкогольнаяПродукцияДопУказание = МассивТребуетсяУказаниеДанных;
	
КонецПроцедуры

Процедура ПолучитьДанныеПоШтрихкодамТСД(РезультатПроверки, ПараметрыСканированияАкцизныхМарок, ПараметрыПроверки)
	...
	Для Каждого ЭлементМассива Из РезультатПроверки.МаркируемаяАлкогольнаяПродукция Цикл
		
		МассивШтрихкодов.Добавить(ЭлементМассива.АкцизнаяМарка);
	...
	Если Не ЭтоФормаДокумента Тогда
		...
		Для Каждого ЭлементМассива Из РезультатПроверки.ПартионнаяАлкогольнаяПродукция Цикл
			...
			МассивШтрихкодов.Добавить(ЭлементМассива.ШтриховойКод);
	...
	ДанныеПоШтрихКодам = АкцизныеМаркиЕГАИС.ПолучитьДанныеПоШтрихкодам(МассивШтрихкодов,
	                                                                   ПараметрыСканированияАкцизныхМарок,
	                                                                   Неопределено);
	
	РезультатПроверки.АдресВложенныеШтрихКоды = ПоместитьВоВременноеХранилище(ДанныеПоШтрихКодам.ВложенныеШтрихкоды, Форма.УникальныйИдентификатор);
	
	Если ДанныеПоШтрихКодам.ЕстьОшибкиВДеревеУпаковок Тогда
		
		РезультатПроверки.ЕстьОшибкиВДеревеУпаковок = Истина;
		РезультатПроверки.АдресДереваУпаковок       = ПоместитьВоВременноеХранилище(ДанныеПоШтрихКодам.ВложенныеШтрихкоды.ДеревоУпаковок, Форма.УникальныйИдентификатор);
		
	КонецЕсли;
	
	Для Каждого ЭлементМассива Из РезультатПроверки.МаркируемаяАлкогольнаяПродукция Цикл
		
		ЭлементМассива.ДанныеШтрихкода = ДанныеПоШтрихКодам.ОбработанныеШтрихкоды.Получить(ЭлементМассива.АкцизнаяМарка);
		Если ЭлементМассива.ДанныеШтрихкода = Неопределено Тогда
			ЭлементМассива.ДанныеШтрихкода = ДанныеПоШтрихКодам.НеобработанныеШтрихкоды.Получить(ЭлементМассива.АкцизнаяМарка);
		КонецЕсли;
	...
	Если Не ЭтоФормаДокумента Тогда
		...
		Для Каждого ЭлементМассива Из РезультатПроверки.ПартионнаяАлкогольнаяПродукция Цикл
			...
			ЭлементМассива.ДанныеШтрихкода = ДанныеПоШтрихКодам.ОбработанныеШтрихкоды.Получить(ЭлементМассива.ШтриховойКод);
			Если ЭлементМассива.ДанныеШтрихкода = Неопределено Тогда
				ЭлементМассива.ДанныеШтрихкода = ДанныеПоШтрихКодам.НеобработанныеШтрихкоды.Получить(ЭлементМассива.ШтриховойКод);
			КонецЕсли;
			
			Если ЭлементМассива.ДанныеШтрихкода <> Неопределено Тогда
				ЭлементМассива.ДанныеШтрихкода.Количество = ЭлементМассива.Количество;
			КонецЕсли;
	...
	Для Каждого ЭлементМассива Из РезультатПроверки.Упаковки Цикл
		...
		ДанныеУпаковки.Вставить("ДанныеШтрихкода", ДанныеПоШтрихКодам.ОбработанныеШтрихкоды.Получить(ЭлементМассива));
		Если ДанныеУпаковки.ДанныеШтрихкода = Неопределено Тогда
			ДанныеУпаковки.ДанныеШтрихкода = ДанныеПоШтрихКодам.НеобработанныеШтрихкоды.Получить(ЭлементМассива);
		КонецЕсли;
...
Процедура ПроверитьКорректностьСчитанныхТСДУпаковок(РезультатПроверки, ДеревоМаркированнойПродукции, НовыеУпаковки, СуществующиеУпаковки, СоответствиеШтрихкодовСтрокДерева)
	...
	Если МассивПредполагаемыхРодителей.Количество() > 1 Тогда
		...
		РезультатПроверки.ТекстОшибки = НСтр("ru = 'Для считанных упаковок не удалось упаковку, в которой содержатся другие упаковки.'");
...
Процедура СохранитьНастройкиВыбораАлкогольнойПродукции(Форма, Ссылка) Экспорт
	
	СохраняемыеНастройки = Новый Структура;
	СохраняемыеНастройки.Вставить("СохраненВыборПоАлкогольнойПродукции", Форма.СохраненВыборПоАлкогольнойПродукции);
	СохраняемыеНастройки.Вставить("ДанныеВыбораПоАлкогольнойПродукции",  Форма.ДанныеВыбораПоАлкогольнойПродукции);
	
	ОбщегоНазначения.ХранилищеНастроекДанныхФормСохранить(Ссылка, "ДанныеВыбораАлкогольнойПродукции", СохраняемыеНастройки);
	
КонецПроцедуры

Процедура ВосстановитьНастройкиВыбораАлкогольнойПродукции(Форма, Ссылка) Экспорт
	
	НастройкиВыбораАлкогольнойПродукции = ОбщегоНазначения.ХранилищеНастроекДанныхФормЗагрузить(Ссылка, "ДанныеВыбораАлкогольнойПродукции");
...
Процедура ЗаполнитьТабличнуюЧастьТовары(ДокументОбъект, РезультатЗапроса, ДанныеЗаполнения, БезСопоставления = Ложь) Экспорт
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Если ИнтеграцияЕГАИСКлиентСервер.ЕстьРеквизитОбъекта(ВыборкаТовары, "ЕстьОшибкиНеупакованнаяПродукция") 
