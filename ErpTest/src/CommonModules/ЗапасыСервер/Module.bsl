...
//		оно не меняется.
//	* ОтборыВидовЗапасов - Структура - По данным отборам строится временная таблица ДоступныеВидыЗапасов.
...
Процедура ЗаполнитьВидыЗапасовПоТоварамОрганизаций(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов) Экспорт
	...
	Если ПараметрыЗаполненияВидовЗапасов.ИмяТаблицыОстатков <> "ТоварыОрганизаций" Тогда
		ТекстИсключения = НСтр("ru = 'Процедуру ЗаполнитьВидыЗапасовПоТоварамОрганизаций можно вызывать только с таблицей остатков ТоварыОрганизаций'");
...
Процедура ПроведениеДокументовПоРегиструСебестоимостьТоваров(Параметры="", АдресХранилища="") Экспорт
	...
	ДопСвойстваНабора = Новый Структура("НеСохранятьРасчетныеДвижения", Истина);
...
//  Форма				 - УправляемаяФорма	 - форма объекта, у которого есть табличная часть ВидыЗапасов
//
...
Процедура ИсправитьОстаткиПоТоварамКПередаче(ПараметрыВызова) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		|	ВЫРАЗИТЬ(Товары.Регистратор КАК Документ.ПередачаТоваровМеждуОрганизациями).УказыватьНомераГТД КАК УказыватьНомераГТД
		|ИЗ
		...
		|	ВЫРАЗИТЬ(Товары.Регистратор КАК Документ.ПередачаТоваровМеждуОрганизациями).УказыватьНомераГТД
		|ИЗ
		|	РегистрНакопления.ТоварыОрганизацийКПередаче КАК Товары
		|ГДЕ
		|	Товары.ОрганизацияВладелец = &ОрганизацияВладелец
			...
			Пока ВыборкаВидЗапасов.Следующий() Цикл
				...
				Пока ВыборкаНомерГТД.Следующий() Цикл
					...
					Пока ТекущиеИтоги.Следующий() Цикл
						...
						Если КонтролироватьОтрицательныеОстатки И ТекущиеИтоги.Остаток + ТекущиеИтоги.Резерв < 0 Тогда
							...
							КонецЕсли;
							ЗаполнитьОбщиеПараметрыТекстеСообщенияОбОшибкахПроведенияПоТоварамОрганизацийИРезервам(ТекстСообщения, Объект.Ссылка, ТекущиеИтоги);			
...
Процедура УстановитьИспользованиеРегламентногоЗаданияСверткиРезервов(Параметры) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru = 'Не удалось установить использование регламентного задания свертки резервов товаров организаций по причине: %Причина%'");
...
Функция ЗапросРаспределенияРезервовПоТаблицеОстатков(МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|		Ключи.Номенклатура,
	|		Ключи.Номенклатура.ЕдиницаИзмерения,
...
Процедура СформироватьВТСторноРезервов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	Если ТаблицаСторноДвижений.Количество() > 0 Тогда
		ГлубинаРекурссии = 0;
		ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ТаблицаСторноДвижений, ТаблицаСторноДвижений, ГлубинаРекурссии);
	КонецЕсли;
...
Процедура ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ИсходнаяТаблицаСторноДвижений, РезультирующаяТаблицаСторноДвижений, ГлубинаРекурссии)
	
	Если ИсходнаяТаблицаСторноДвижений.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ГлубинаРекурссии = ГлубинаРекурссии + 1;
	
	Если ГлубинаРекурссии > 100 Тогда
		
		ТекстИсключения = НСтр("ru = 'Превышена максимальная глубина рекурссии при формировании сторно резервов по расходу. Сохраните текущее состояние информационной базы и передайте ее разработчикам программы.'");
		ВызватьИсключение ТекстИсключения;
	...
	Пока Выборка.Следующий() Цикл
		...
		СторноПриходаРезерваДляРаспределения = Выборка.Количество;
	...
	Если ТаблицаСторноДвижений.Количество() > 0 Тогда
		...
		ДобавитьВТаблицуСторноРезервовСторноПоРасходу(ДокументОбъект, ТаблицаСторноДвижений, РезультирующаяТаблицаСторноДвижений, ГлубинаРекурссии);
...
Процедура ЗаполнитьВидыЗапасовРазныхПериодов(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	|	ОстаткиПоПериодам.КоличествоОстаток - ЕСТЬNULL(ВидыЗапасов.Количество, 0) КАК КоличествоОстаток,
	...
	|	И ОстаткиПоПериодам.КоличествоОстаток - ЕСТЬNULL(ВидыЗапасов.Количество, 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ЗапросРаспределенияБезУчетаПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц, ДополнительныеСвойства, ПараметрыЗаполнения)
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(АналитикаКомиссионера.СтатьяКалькуляции, Аналитика.СтатьяКалькуляции) КАК СтатьяКалькуляцииОстатков,
	//-- НЕ УТ
	|	Аналитика.Номенклатура КАК Номенклатура,
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(АналитикаКомиссионера.СтатьяКалькуляции, Аналитика.СтатьяКалькуляции),
	//-- НЕ УТ
	|	Аналитика.Номенклатура,
	...
	//++ НЕ УТ
	|		И ТаблицаДокумента.СтатьяКалькуляцииОстатков = Остатки.СтатьяКалькуляции
	//-- НЕ УТ
	...
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки Возр,
	|	Остатки.ДокументРеализации = ТаблицаДокумента.ДокументРеализации Убыв,
	...
	ЗаменитьОтсутствующиеВТВТекстеЗапроса(ТаблицаТоваров,
											Запрос.Текст,
											"ДокументРеализации",
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц, ДополнительныеСвойства)
	...
	//++ НЕ УТ 
	|			И (ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаВозвратОтКомиссионера.СтатьяКалькуляции)
	//-- НЕ УТ 
	...
	//++ НЕ УТ 
	|			И (ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаВозвратОтПереработчика.СтатьяКалькуляции)
	//-- НЕ УТ 
	...
	//++ НЕ УТ 
	|			И (ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = АналитикаВозвратОтКлиента.СтатьяКалькуляции)
	//-- НЕ УТ 
	|ГДЕ
	|	ТаблицаТоваров.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
	|
	...
	|			И (Остатки.СкладОтгрузки = ТаблицаВидыЗапасовСводно.СкладОтгрузки
	|				ИЛИ ТаблицаВидыЗапасовСводно.СкладОтгрузки В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|				ИЛИ Остатки.СкладОтгрузки В (ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), НЕОПРЕДЕЛЕНО))
	|			И (Остатки.ДокументРеализации = ТаблицаВидыЗапасовСводно.ДокументРеализации
	|				ИЛИ ТаблицаВидыЗапасовСводно.ДокументРеализации В (ЗНАЧЕНИЕ(Документ.РеализацияТоваровУслуг.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	...
	|	ТаблицаВидыЗапасовСводно.СкладОтгрузки,
	|	ТаблицаВидыЗапасовСводно.ДокументРеализации,
	|	Аналитика.Номенклатура,
	|	Аналитика.Характеристика,
	|	Аналитика.Серия,
	|	Аналитика.Назначение,
	...
	//++ НЕ УТ 
	|		И АналитикаОтгрузки.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	//-- НЕ УТ
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК ДатыПоступления
...
Процедура ЗаменитьОтсутствующиеВТВТекстеЗапроса(ВременнаяТаблица, ТекстЗапроса, ИмяПоля, ТекстВыражениеВыборки, ТекстЗначенияПоУмолчанию)
...
Процедура СформироватьРезервыПоОрганизациямДляКонтроля(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	Если ЗначениеЗаполнено(ПараметрыЗаполненияВидовЗапасов.ПериодЗаполнения) Тогда
...
Функция РаспределитьРезервыТоваровОрганизацийПоПериодам(КоличествоПериодовПриФормированииРезервовОрганизаций, ТаблицаНовыеРезервыИсходная, МенеджерВременныхТаблиц) Экспорт
	...
	|	ТаблицаНовыеРезервы.КорВидЗапасов КАК КорВидЗапасов,
	|	ТаблицаНовыеРезервы.Количество КАК Количество,
	|	ТаблицаНовыеРезервы.Количество КАК КоличествоНеРаспределеноеПоПериодам,
	|	ТаблицаНовыеРезервы.ЭтоСторно КАК ЭтоСторно,
	|	ТаблицаНовыеРезервы.ВидДвижения КАК ВидДвижения
	|ИЗ
	|	ВТТаблицаНовыеРезервы КАК ТаблицаНовыеРезервы
	...
	Пока ВыборкаОстатков.Следующий() Цикл
		...
		Пока Индекс >=0 Цикл
			...
			РезервВЭтотМесяцВЭтуЗапись = Мин(НераспределенныйРезерв.КоличествоНеРаспределеноеПоПериодам, РезервВЭтотМесяцВсегоНераспределено);				
			Если РезервВЭтотМесяцВЭтуЗапись = НераспределенныйРезерв.КоличествоНеРаспределеноеПоПериодам Тогда
				НераспределенныйРезерв.КоличествоНеРаспределеноеПоПериодам = 0;
			...
			Иначе
				НераспределенныйРезерв.КоличествоНеРаспределеноеПоПериодам = НераспределенныйРезерв.КоличествоНеРаспределеноеПоПериодам - РезервВЭтотМесяцВЭтуЗапись;
				...
				НоваяСтрока.КоличествоНеРаспределеноеПоПериодам = 0;
	...
	Если ЕстьИзмененияВТаблице Тогда
		...
		ТаблицаНовыеРезервы.Колонки.Удалить("КоличествоНеРаспределеноеПоПериодам");
...
Процедура СообщитьОКонтролеНалогообложенияПриПодбореВидовЗапасов(Знач ДокументОбъект, Знач ПараметрыЗаполненияВидовЗапасов)
	...
	Если КонтролироватьНалогообложениеНДС(ПараметрыЗаполненияВидовЗапасов) Тогда
		...
		НалогообложениеНДСОрганизации = Справочники.Организации.НалогообложениеНДС(
...
Процедура ТаблицаОстатковРеализованныхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	Если ДокументОбъект.Метаданные().Реквизиты.Найти("ХозяйственнаяОперация") <> Неопределено Тогда
		...
		//++ НЕ УТ
		ИначеЕсли ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОтПереработчика Тогда
...
Процедура ТаблицаОстатковДляВозвратаПереданныхПереработчикуТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков)
	...
	|		И Аналитика.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		И Аналитика.Склад = &Партнер
	|		И АналитикаДокумента.Назначение = Аналитика.Назначение
	|		И Аналитика.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|	
	|ГДЕ
	|	ТоварыОрганизаций.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	...
	|	ТоварыОрганизаций.НомерГТД
	|УПОРЯДОЧИТЬ ПО
	|	Аналитика.Номенклатура, Аналитика.Характеристика, Аналитика.Назначение, Аналитика.Серия, ВидЗапасов, ДатаПоступления
	|	
	|;
	|///////////////////////////////////////////////////////////////////////////////
	...
	|	&Организация КАК Организация,
	|	ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка) КАК Склад,
	|	КлючиАналитики.Склад КАК СкладОтгрузки,
	|	ЗНАЧЕНИЕ(Справочник.СделкиСКлиентами.ПустаяСсылка) КАК Сделка,
	|	ТабТовары.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
...
Функция ДополнитьЗапросКонтролемОстатковНаКаждыйМесяц(Запрос, ТекстЗапроса, ПараметрыДополнения, МинимальнаяДатаОстатков, ДатыОстатков = Неопределено) Экспорт
	...
	Для Каждого МесяцКонтроля из МесяцыКонтроля Цикл
		...
		ЭтоПослениеДвижения = КонецМесяцаДатыПоследнегоДвижения = ТекущийМесяц;
		...
		Если ПараметрыДополнения.ПодбиратьЗапасыРазныхПериодов Тогда
			...
			Если ПараметрыДополнения.ЕстьТаблицаПриходуемыхВидовЗапасов Тогда
				...
				|ГДЕ
				|	&ТекстЭтоПослениеДвижения ИЛИ ТаблицаВидыЗапасов.Период <= &ТекстПериод";
			...
			Для Каждого СтрМас из ДатыОстатков Цикл
				...
				// Контролиремый месяц больше месяца этой итерации
		...
		ТекстЗапросаВТОстаткиНаТекущийМесяц = СтрЗаменить(ТекстЗапросаВТОстаткиНаТекущийМесяц, "&ТекстЭтоПослениеДвижения", ?(ЭтоПослениеДвижения, "ИСТИНА", "ЛОЖЬ"));
...
//
// Параметры:
//  Запрос					 - Запрос - запрос с устновленными параметрами, вляющие на отбор.
//										Если параметры не устновлены, то им будут присвоены значения по умолчанию.
...
//										* КонецПерида - Дата - если не установлен, то конец текущего месяца 
...
Функция ТекстЗапросаОформленияПоРезервамТоваровОрганизаций(Запрос, ТипРезультата = "ТаблицаЗначений", Знач ИмяВТОтборыНоменклатуры = "") Экспорт
	...
	|		И (&ПоВсемСкладам
	|				ИЛИ РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры.Склад = &Склад)
	...
	|				И РезервыТоваровОрганизаций.КорОрганизация = РезервыТоваровОрганизаций.Организация
	|			ТОГДА ВЫБОР
	|					КОГДА РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры.Склад ССЫЛКА Справочник.Склады
	...
	|	КОНЕЦ КАК ВидЦены,
	|	РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры.Склад КАК Склад,
	|	РезервыТоваровОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
	...
	Если Не Запрос.Параметры.Свойство("Склад") Тогда
		Запрос.УстановитьПараметр("Склад", Неопределено);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ПоВсемСкладам", Не ЗначениеЗаполнено(Запрос.Параметры.Склад));
