...
Функция ПериодичностьОтчетногоПериода(Период, Организация) Экспорт
	...
	ПараметрыПолитики 	 = РегистрыСведений.УчетнаяПолитикаОрганизаций.ПараметрыУчетнойПолитики(Организация, Период);
	ПорядокУплатыАвансов = ПараметрыПолитики.ПорядокУплатыАвансовПоНалогуНаПрибыль;
...
Функция НалоговаяСтавкаРегиональныйБюджет(Период, Организация, ФормаПредставленияРезультата = "Доля") Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ПрименяютсяРазныеСтавкиНалогаНаПрибыль") Тогда
		...
		// Поле ОбщаяСтавка позволяет преимущественно получить ставку, установленную для организации в целом, т.е. с пустой
		// регистрацией в налоговом органе.
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
...
Функция ПолучитьТаблицуВидовАктивовИОбязательств() Экспорт
	...
	// удаляем счет 97.11 из вида активов "РБП"
	СтрокаРБП = ТаблицаВидовАктивовИОбязательств.Найти(Перечисления.ВидыАктивовИОбязательств.РасходыБудущихПериодов,"ВидАктивовОбязательств");
	Если СтрокаРБП <> Неопределено Тогда
		ИндексСчета = СтрокаРБП.Счета.Найти(ПланыСчетов.Хозрасчетный.УбыткиПрошлыхЛет);
		Если ИндексСчета <> Неопределено Тогда
			СтрокаРБП.Счета.Удалить(ИндексСчета);
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоПроводкаРасходыНеУчитываемыеДляНалогаНаПрибыль(СчетДт, СубконтоДт) Экспорт
	...
	Если БухгалтерскийУчетПовтИсп.СчетВИерархии(СчетДт, ПланыСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ДоляТранспортныхРасходовТекущегоМесяца(Период, Организация)
	...
	|		И ВыручкаИСебестоимостьПродаж.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)
	...
	|			ГДЕ
	|				Аналитики.КлючАналитики.Склад = НастройкиИсключений.Склад
	...
	|				+ ВыручкаИСебестоимостьПродаж.ПостатейныеПеременныеРегл
	...
	|			АналитикиТоваров КАК АналитикиТоваров
	|		ПО 
	|			ВыручкаИСебестоимостьПродаж.ВидЗапасов = АналитикиТоваров.ВидЗапасов
	|			И ВыручкаИСебестоимостьПродаж.АналитикаУчетаНоменклатуры = АналитикиТоваров.Ключ
	|			И ВыручкаИСебестоимостьПродаж.АналитикаУчетаПоПартнерам.Организация = АналитикиТоваров.Организация
	|	ГДЕ
	|		ВыручкаИСебестоимостьПродаж.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|		И ВыручкаИСебестоимостьПродаж.РасчетСебестоимости
