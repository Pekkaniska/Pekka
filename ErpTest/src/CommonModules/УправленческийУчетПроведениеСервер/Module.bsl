...
//
// Параметры:
//  ОбъектДокумента - ДокументОбъект - документ для которого необходимо сформировать и записать движения по управленческому балансу.
//
Процедура ОбновитьДвиженияАктивовПассивов(ОбъектДокумента) Экспорт
	
	Обработки.ДвиженияАктивовПассивов.ОбновитьДвиженияДокумента(ОбъектДокумента);
...
Процедура ПолучитьРасчетыСКлиентами(МенеджерВременныхТаблиц, СписокДокументов)
	...
	#Область ТекстЗапросаПоРасчетамСКлиентамиПоСрокам
		...
		|	РасчетыСПартнерами.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
		|	РасчетыСПартнерами.ОбъектРасчетов КАК Заказ,
		|	РасчетыСПартнерами.РасчетныйДокумент КАК РасчетныйДокумент,
		|	РасчетыСПартнерами.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
		|	ВЫБОР КОГДА НЕ РасчетыСПартнерами.ХозяйственнаяОперация В (&ХозОперацииБезДвойнойЗаписиДолга)
		|					И РасчетыСПартнерами.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
		|		ТОГДА 0
		|		ИНАЧЕ РасчетыСПартнерами.Долг
		|	КОНЕЦ КАК Долг,
		|	ВЫБОР КОГДА НЕ РасчетыСПартнерами.ХозяйственнаяОперация В (&ХозОперацииБезДвойнойЗаписиДолга)
		...
		|ИМЕЮЩИЕ
		|	СУММА(РасчетыСКлиентами.Долг) <> 0 ИЛИ СУММА(РасчетыСКлиентами.Предоплата) <> 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор,
		|	Организация,
	...
	Запрос.УстановитьПараметр("ХозОперацииБезДвойнойЗаписиДолга", ОперативныеВзаиморасчетыСервер.ХозяйственныеОперацииНеОтгрузка());
	Запрос.Выполнить();
...
Процедура ПолучитьРасчетыСПоставщиками(МенеджерВременныхТаблиц, СписокДокументов)
	...
	#Область ТекстЗапросаПоРасчетамСПоставщикамиПоСрокам
		...
		|ИМЕЮЩИЕ
		|	СУММА(РасчетыСПоставщиками.Долг) <> 0 ИЛИ СУММА(РасчетыСПоставщиками.Предоплата) <> 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Регистратор,
		|	Организация,
...
Функция ТекстЗапросаДвиженияКонтрагентДоходыРасходы()
	...
	#Область ТекстЗапросаОбновить
		...
		Тексты = Новый Массив;
		Тексты.Добавить(ТекстЗапросаОбновить);
