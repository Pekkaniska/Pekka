...
Функция ПолучитьСертификаты(Знач Хранилище) Экспорт
	...
	Для Каждого ЗначениеХранилища Из Хранилище Цикл
		Для Каждого ПолученныйСертификат Из ХранилищеСертификатов.Получить(ПривестиКЕдиномуТипуХранилища(ЗначениеХранилища)) Цикл
			...
			Сертификат.Вставить(
				"ПоставщикСтруктура", 
				Новый ФиксированнаяСтруктура(ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ПолученныйСертификат.Издатель)));
			Сертификат.Вставить(
				"ВладелецСтруктура", 
				Новый ФиксированнаяСтруктура(ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ПолученныйСертификат.Субъект)));
...
Функция ИзвлечьКриптопровайдер(Сертификат, ПроанализироватьАлгоритм = Ложь)
	...
	Если ПроанализироватьАлгоритм И Криптопровайдер <> Неопределено Тогда
		...
		ПутьМодуляКриптографии = ?(ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент(),
