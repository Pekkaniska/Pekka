...
Процедура ЗарегистрироватьНовуюДатуПолученияДохода(Регистратор, Движения, МенеджерВременныхТаблиц, ДатаВыплаты, ДатаОперации, Отказ, Записывать = Ложь) Экспорт
	...
	Запрос.Текст = ТекстЗапроса;
	Запрос.Выполнить();
	ТаблицыКУдалению = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ВТДанныеДляОтбора");
	...
	|	СУММА(Доходы.ОбщаяСуммаДохода) КАК ОбщаяСуммаДохода
	...
	|	ГДЕ
	|		ПеренесенныеДоходы.УстаревшаяДатаПолученияДохода <> ДАТАВРЕМЯ(1, 1, 1)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		РегистрНакопления.СведенияОДоходахНДФЛ КАК СведенияОДоходахНДФЛ
	|	ГДЕ
	|		(СведенияОДоходахНДФЛ.ФизическоеЛицо, СведенияОДоходахНДФЛ.Регистратор) В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|					ПеренесенныеДоходы.ФизическоеЛицо,
	|					ПеренесенныеДоходы.ДокументОснование
	|				ИЗ
	|					ВТПеренесенныеДоходы КАК ПеренесенныеДоходы)) КАК Доходы
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ИЗ
	|			ВТДанныеДляОтбора КАК ДанныеДляОтбора) КАК Основания
