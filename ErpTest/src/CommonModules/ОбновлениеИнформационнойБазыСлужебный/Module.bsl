...
Процедура НайтиПорциюДляОбновления(ПараметрыПоиска, АдресРезультата) Экспорт
	...
	Пока ИтераторОбхода <> Неопределено Цикл
		...
		ПараметрыВыборки.ДополнительныеИсточникиДанных = ОбновлениеИнформационнойБазы.ПолучитьИсточникиДанных(
...
Процедура ОбработатьФрагментДанныхВПотоке(Фрагмент, ОписаниеПотоков, КонтекстОбработчика)
	...
	КонтекстОбработчикаДляПотока = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(КонтекстОбработчика);
...
//
Функция ДобавляемоеЧислоПопыток(ОбработчикОбновления, КонтекстОбработчика)
	...
	Если ОбработчикОбновления.Многопоточный Тогда
		...
		Если ОбновляемыеДанные <> Неопределено
		   И ОбновляемыеДанные.ПерваяЗапись = Неопределено
		   И ОбновляемыеДанные.ПоследняяЗапись = Неопределено Тогда
			Возврат 0;
		КонецЕсли;
	КонецЕсли;
