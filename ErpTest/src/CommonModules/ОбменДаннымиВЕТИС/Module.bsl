...
Процедура ЗаполнитьОбъект_Batch(Данные, ПолеКоличество, ВспомогательныеДанные, ОбъектXDTO, СообщениеXML) Экспорт
	...
	// Отключено до доработки Меркурием.
...
Процедура ЗаполнитьОбъект_VetDocument(Данные, Шапка, ВспомогательныеДанные, ДополнительныеПараметры, ОбъектXDTO, СообщениеXML) Экспорт
	...
	Если ДополнительныеПараметры.Свойство("Регионализация") Тогда
		Для каждого ДанныеРегионализации Из ДополнительныеПараметры.Регионализация Цикл
			...
			Регионализация = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(РезультатыИсследованийПартии, "r13nClause");
	...
	Если ДополнительныеПараметры.Свойство("ЛабораторныеИсследования") Тогда
		...
		Для каждого ДанныеИсследований Из СписокСтрок Цикл
			...
			ДанныеИсследованийXDTO = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(РезультатыИсследованийПартии, "laboratoryResearch");
	...
	Если ДополнительныеПараметры.Свойство("Иммунизация") Тогда
		...
		Для каждого ДанныеОбработки Из СписокСтрок Цикл
			...
			ДанныеОбработкиXDTO = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(РезультатыИсследованийПартии, "immunization");
...
Процедура СформироватьДокументыПоДате(ОбщиеДанные, ТаблицаЭлементов, ПараметрыОбмена, ИзмененныеОбъекты) Экспорт
	...
	Для Каждого СтрокаТЧ Из ТаблицаЭлементов Цикл
		...
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыВерсионныхОбъектовВЕТИС.СозданВРезультатеОперацииГашения
			...
			Если Не СтрокаДанныеДокументов.ТипВСД = Перечисления.ТипыВетеринарныхДокументовВЕТИС.Возвратный Тогда
				...
				СформироватьДокумент_ВходящаяТранспортнаяОперацияВЕТИС(ОбщиеДанные, КСозданиюДокументов, СтрокаТЧ, СтрокаДанныеДокументов, ПараметрыОбмена, СтрокаРазделителиТО, ИзмененныеОбъекты);
	...
	Для Каждого СтрокаВТО из КСозданиюДокументов.ВходящаяТранспортнаяОперацияВЕТИС Цикл
		Если ИзмененныеОбъекты.Найти(СтрокаВТО.Ссылка) = Неопределено Тогда
...
Процедура СформироватьДокумент_ВходящаяТранспортнаяОперацияВЕТИС(ОбщиеДанные, КСозданиюДокументов, СтрокаТаблицыЭлементов, СтрокаДанныеДокументов, ПараметрыОбмена, СтрокаРазделителиТО, ИзмененныеОбъекты)
	...
	Если ДокументОбъект = Неопределено Тогда
		...
		Для каждого СтрокаИзмененных Из ИзмененныеОбъекты Цикл
			Если ТипЗнч(СтрокаИзмененных) = Тип("ДокументСсылка.ВходящаяТранспортнаяОперацияВЕТИС") Тогда
				ДокументОбъект = СтрокаИзмененных.ПолучитьОбъект();
				ПоляСверки = СтрРазделить(СтрокаРазделителиТО, ",");
				Для каждого ПолеСверки Из ПоляСверки Цикл
					Если СтрокаИзмененных[СокрЛП(ПолеСверки)] <> СтрокаДанныеДокументов[СокрЛП(ПолеСверки)] Тогда
						ДокументОбъект = Неопределено;
						Прервать;
					КонецЕсли;
				КонецЦикла;
		...
		Если ДокументОбъект = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(СтрокаДанныеДокументов.ВетеринарноСопроводительныйДокумент) Тогда
				...
				Если СтрокаДанныеДокументов.ВетеринарноСопроводительныйДокумент.Тип
					= Перечисления.ТипыВетеринарныхДокументовВЕТИС.Возвратный Тогда
	...
	Если ИзмененныеОбъекты.Найти(ДокументОбъект.Ссылка) = Неопределено Тогда
