...
Функция ПолучитьЗначенияВсехПравНастроек(Знач ТекущийПользователь) Экспорт
	...
	ЗапросПраваНастройки.УстановитьПараметр("ГруппаПВХНастройкаДоступностиСправочники", уатПраваИНастройки.уатПолучитьСсылкуПВХПравИНастроек("ПРАВА ДОСТУПА СПРАВОЧНИКОВ"));
	ЗапросПраваНастройки.УстановитьПараметр("ГруппаПВХНастройкаДоступностиДокументы", уатПраваИНастройки.уатПолучитьСсылкуПВХПравИНастроек("ПРАВА ДОСТУПА ДОКУМЕНТОВ"));
	ВыборкаПраваНастройки = ЗапросПраваНастройки.Выполнить().Выбрать();
	
	НЗЗначенияПравИНастроек = РегистрыСведений.уатПраваИНастройки.СоздатьНаборЗаписей();
