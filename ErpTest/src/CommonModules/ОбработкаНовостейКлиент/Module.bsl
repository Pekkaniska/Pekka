...
Процедура ОбработкаНажатияВТекстеНовости(лкНовости, ДанныеСобытия, СтандартнаяОбработка, Форма, ЭлементФормы) Экспорт
	...
	Если (ДанныеСобытия.Anchor <> Неопределено) Тогда
		...
		Иначе
			...
			Попытка
				...
				ИнформацияПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='ДанныеСобытия.Anchor.nameProp = %1'"),
						ТолькоСсылка));
			Исключение
				ТолькоСсылка = "";
				...
				ИнформацияПереходаПоСсылке.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка получения ДанныеСобытия.Anchor.nameProp по причине:
							|%1'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
			КонецПопытки;
		КонецЕсли;
		...
		Если (ТолькоСсылка = Неопределено) ИЛИ ПустаяСтрока(ТолькоСсылка) Тогда
			...
			ИнформацияПереходаПоСсылке.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='СтрокаСоединенияИнформационнойБазы() = %1'"),
					СтрокаСоединенияСИБ));
			Если ГдеПротокол > 0 Тогда
				...
				Если ГдеРазделительВСтрокеСоединения3 > ГдеРазделительВСтрокеСоединения2 Тогда
					...
					ИнформацияПереходаПоСсылке.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Веб-адрес базы в СтрокаСоединенияИнформационнойБазы() = %1'"),
					...
					Если ГдеРазделительВСсылке3 > ГдеРазделительВСсылке2 Тогда
						...
						ИнформацияПереходаПоСсылке.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Веб-адрес базы в Ссылке = %1'"),
								ВебАдресБазыВСсылке));
		...
		ИнформацияПереходаПоСсылке.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Только ссылка без адреса базы = %1'"),
				ТолькоСсылка));
