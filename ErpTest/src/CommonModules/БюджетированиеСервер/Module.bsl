...
//	ЗадачиПервойОчереди - Массив - ЗадачаСсылка.БюджетнаяЗадача - Массив задач (ЗадачаСсылка.БюджетнаяЗадача) 
//	                      или ЗадачаСсылка.БюджетнаяЗадача, которые будут обрабатываться в первую очередь,
...
Процедура ДополнитьСКДАналитикойПоВиду(СхемаКомпоновкиДанных, ИсточникАналитики) Экспорт
	...
	Для каждого ВидАналитики Из ВидыАналитик Цикл
		...
		НовыйПараметр.Имя = ИмяПараметраВидаАналитики;
		НовыйПараметр.Значение = ВидАналитики.Ссылка;
		НовыйПараметр.ОграничениеИспользования = Истина;
		НовыйПараметр.ТипЗначения = Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.АналитикиСтатейБюджетов");
...
Процедура УстановитьСвойстваПолейДляНастройкиПравила(СхемаКомпоновкиДанных, Правило) Экспорт
	...
	Для каждого ПараметрКомпоновки Из СхемаКомпоновкиДанных.Параметры Цикл
		Если Не СлужебныеПоля.Найти(ПараметрКомпоновки.Имя) = Неопределено Тогда
...
Процедура ЗначенияНефинансовогоПоказателяПоПравилу(Правило, Параметры, ТаблицаЗначений) Экспорт
	...
	Если Не СхемаКомпоновкиДанных.НаборыДанных.Количество() Тогда
		ШаблонСообщения = НСтр("ru = 'Отсутствует набор данных в схеме заполнения для ""%1""'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
	
	ПоляНабора = СхемаКомпоновкиДанных.НаборыДанных[0].Поля;
	
	Если ПоляНабора.Найти("ЗначениеПоказателя") = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле ""ЗначениеПоказателя""'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
	
	Если ПоляНабора.Найти("Период") = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле ""Период""'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
	
	Если Правило.ЗаполнениеВалюты = "ОпределяетсяПоИсточникуДанных" И ПоляНабора.Найти("Валюта") = Неопределено Тогда
		ШаблонСообщения = НСтр("ru = 'В схеме компоновки данных нефинансового показателя ""%1"" отсутствует поле ""Валюта""'");
		ВызватьИсключение СтрЗаменить(ШаблонСообщения, "%1", Правило.НефинансовыйПоказатель);
	КонецЕсли;
		...
		Иначе
			...
			Если ПолучатьФактическиеДанные Тогда
				...
				Для Каждого ЧастьОдногоВыражения Из ЧастиВыражений Цикл 
					...
					Если ЧастьОдногоВыражения.Ключ = "ВыражениеПериод" Тогда
		...
		ИначеЕсли СтрокаДопДанных.ЗаполнениеВалюты = "ОпределяетсяПоИсточникуДанных" 
			...
			Если СтрокаДопДанных.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.РегламентированныйУчет 
				...
				Тогда
					...
					ВВалютеИсточника = Ложь;
				//++ НЕ УТКА
			ИначеЕсли СтрокаДопДанных.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.МеждународныйУчет 
				И Не СтрокаДопДанных.ИсточникВалютный Тогда
					ВыражениеЗаполненияПоля = "&ВалютаМеждународ";
					ВВалютеИсточника = Ложь;
				//-- НЕ УТКА
...
Функция ВыражениеЗаполненияПоказателяСумма(Знач ПараметрыПолученияФакта, Знач СтрокаДопДанных)
	...
	//++ НЕ УТКА
	ИначеЕсли ПараметрыПолученияФакта.ВалютаОтчета = ПараметрыПолученияФакта.ВалютаМеждународ 
		И СтрокаДопДанных.ВыраженияПоказателейСуммы.Свойство("ВалютаМеждународ", Выражение) Тогда
...
Процедура ОбновитьПараметрыИдентификацииВыражений(
	...
	Если Не ВыражениеИспользовалосьДляПоля Тогда
		...
		ЧастиВыражений.Добавить(
			...
			+ " ТОГДА " 
			+ ПреобразоватьВВыражениеСКД(Выражение, ЧисловоеВыражение, ВыражениеАналитики));
...
Процедура ДобавитьКолонкиДополнительныхДанных(ТаблицаДополнительныхДанных, ИндексИменИВидовАналитик)
	...
	Если ТребуетсяЗаполнение Тогда
		...
		Для НомерАналитики = 1 По МаксимальноеКоличествоАналитик Цикл
			КолонкиТаблицыДанных.Добавить("ВидАналитики" + НомерАналитики);
			...
			КолонкиТаблицыДанных.Добавить("ЗначениеАналитики" + НомерАналитики);
		КонецЦикла;
...
//
// Параметры:
// 	Форма - УправляемаяФорма - форма элемента справочника Модели бюджетирования.
//
