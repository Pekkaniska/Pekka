...
//			
//		ЗаписыватьДвижения - Булево - если Истина, то наборы записей будут записаны после заполнения.
...
//
Процедура ЗарегистрироватьПогашениеЗаймов(Движения, Отказ, Организация, ПериодРегистрации, ПогашениеЗаймов, ПорядокВыплаты = Неопределено, ЗаписыватьДвижения = Ложь) Экспорт
...
Функция НачисленныеОтпускаПоПериодамИВидам(Сотрудник, ВидыОтпусков, ИсключаемыеРегистраторы = Неопределено) Экспорт
	...
	РегистрыРасчета.Начисления.СоздатьВТРезультирующийПериодДействия(Запрос.МенеджерВременныхТаблиц,
		"ВидРасчета В (
		|	ВЫБРАТЬ
		|		ВидыРасчетовНачислений.Ссылка
		|	ИЗ
		|		ПланВидовРасчета.Начисления КАК ВидыРасчетовНачислений
		|	ГДЕ
		|		ВидыРасчетовНачислений.ВидОтпуска В (&ВидыОтпусков))
	...
	|		ИНАЧЕ ЗначенияПоказателейНачислений.Значение
	|	КОНЕЦ КАК Значение
	|ИЗ
