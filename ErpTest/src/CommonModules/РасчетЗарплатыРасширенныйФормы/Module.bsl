...
Процедура РасчетЗарплатыНачисленияСотрудниковВДанныеФормы(ТаблицыНачислений, ТаблицаИсточник, ПараметрыСтрок, РежимДоначисления)
	...
	Если ТаблицыНачислений.НачисленияПерерасчетНулевыеСторно <> Неопределено Тогда
		НачисленияПерерасчетНулевыеСторноСотрудниковВДанныеФормы(ТаблицыНачислений.НачисленияПерерасчетНулевыеСторно, ТаблицаИсточник, ПараметрыСтрок, РежимДоначисления);
	КонецЕсли;
...
Процедура НачисленияПерерасчетНулевыеСторноСотрудниковВДанныеФормы(ТаблицаПриемник, ТаблицаИсточник, ПараметрыСтрок, РежимДоначисления)
	...
	ПозицииСотрудников = ТаблицаПозицииСотрудников(НачальныеПозицииВставки["НачисленияПерерасчетНулевыеСторно"]);
	...
	Для Каждого СтрокаПозиций Из ПозицииСотрудников Цикл
		...
		Для Каждого СтрокаНачисления Из СтрокиНачислений Цикл
			...
			Если СтрокаНачисления.Владелец = "НачисленияПерерасчетНулевыеСторно" Тогда
...
Процедура РасчетЗарплатыНДФЛВДанныеФормы(ТаблицаНДФЛ, ТаблицаВычетов, ТаблицаИсточник, НачальныеПозицииВставки = Неопределено, ТаблицаДокументов = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	НДФЛ = УчетНДФЛ.ТаблицаНДФЛбезМесяцаПериодаПредоставленияВычета(ТаблицаИсточник.Скопировать(), ОписаниеКолонок);
	Если НачальныеПозицииВставки <> Неопределено Тогда
		...
		Для Каждого СтрокаПозиций Из ПозицииСотрудников Цикл
			...
			Для Каждого СтрокаИсточник Из СтрокиНДФЛ Цикл
				...
				СтрокаНДФЛВДанныеФормы(СтрокаНДФЛ, ТаблицаВычетов, СтрокаИсточник, СтрокиВычетов, ПараметрыДляПроверкиРезультатаРаспределения);
				КоличествоСтрок = КоличествоСтрок + 1;
				ПозицияВставки = ПозицияВставки + 1;
				СтрокиКУдалению.Добавить(СтрокаИсточник);
...
Процедура СтрокаНДФЛВДанныеФормы(СтрокаНДФЛ, ТаблицаВычетов, СтрокаИсточник, СтрокиВычетов, ПараметрыДляПроверкиРезультатаРаспределения)
...
Процедура ПоказателиВМенеджерРасчета(СтрокаПриемник, СтрокаФормы, ИмяПВР = "Начисления", КоличествоПоказателей = Неопределено) Экспорт
...
Процедура НачисленияПерерасчетСНулевымиСторноВДанныеМенеджераРасчета(ТаблицаИсточник, ТаблицаДополнение, МенеджерРасчета, Организация, Отбор = Неопределено, НачальныеПозицииВставки = Неопределено, ДанныеРезультатовРаспределения = Неопределено, ДанныеРаспределенияПоТерриториямУсловиямТруда = Неопределено) Экспорт
	НачисленияПерерасчетВДанныеМенеджераРасчета(ТаблицаДополнение, МенеджерРасчета, Организация, Отбор, НачальныеПозицииВставки, ДанныеРезультатовРаспределения, ДанныеРаспределенияПоТерриториямУсловиямТруда, "НачисленияПерерасчетНулевыеСторно");
...
Процедура НачисленияПерерасчетВДанныеМенеджераРасчета(ТаблицаИсточник, МенеджерРасчета, Организация, Отбор = Неопределено, НачальныеПозицииВставки = Неопределено, ДанныеРезультатовРаспределения = Неопределено, ДанныеРаспределенияПоТерриториямУсловиямТруда = Неопределено, ИмяТаблицыПерерасчета = "НачисленияПерерасчет") Экспорт
	...
	УдалитьСтрокиРасчета(ТаблицаИсточник, СтрокиРасчета, Отбор = Неопределено);
	Если НачальныеПозицииВставки <> Неопределено Тогда
		НачальныеПозицииВставки.Вставить(ИмяТаблицыПерерасчета, НачальныеПозицииСотрудников);
	КонецЕсли;
