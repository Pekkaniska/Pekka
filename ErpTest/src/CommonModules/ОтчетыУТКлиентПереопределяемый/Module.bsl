...
//
// Параметры:
//  ЭтаФорма							 - УправляемаяФорма	 - форма, на которой происходит обработка расшифровки.
...
Процедура ОбработатьРасшифровкуКонтрольОформленияДокументовТовародвижения(
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеВозвратТоваровОтКлиента Тогда
		ОтчетыУТВызовСервераПереопределяемый.ЗаполнитьОтборыКонтрольОформленияДокументовТовародвижения(
			ПараметрыОбработкиРасшифровки, "Распоряжения", ОтборыДляФормированияОтчетаКонтроля);
		ПараметрыОткрытияФормы = Новый Структура;
		ПараметрыОткрытияФормы.Вставить("Распоряжения", ПараметрыОбработкиРасшифровки.Распоряжения);
		ПараметрыОткрытияФормы.Вставить("ИмяТекущейСтраницы", "РаспоряженияНаОформление");
	...
	ИначеЕсли СтруктураРекомендаций.ОформитеВозвратМатериаловИзПроизводства Тогда
		...
		ПараметрыОткрытияФормы.СтруктураБыстрогоОтбора.ОтборХозяйственныеОперации.Добавить(
		...
		ПараметрыОткрытияФормы.СтруктураБыстрогоОтбора.ОтборТипыДокументов.Добавить(
...
Процедура МасштабироватьСхему(ЭтаФорма)
	...
	ЭтаФорма.Элементы.МасштабСхемы.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Масштаб %1 %'"),
		Строка(Масштаб));
