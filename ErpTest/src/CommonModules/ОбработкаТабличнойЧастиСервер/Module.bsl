...
Процедура ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	//++ НЕ УТКА
	ПроверитьЗаполнитьЦенуПобочногоВыпускаВСтрокеТЧ(        										ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	//-- НЕ УТКА
	//++ НЕ УТ
	...
	//++ НЕ УТКА
	ОбработкаТабличнойЧастиКлиентСервер.ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(  ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ОбработкаТабличнойЧастиКлиентСервер.ПроверитьЗаполнитьНазначениеПобочногоВыхода(                ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	//-- НЕ УТКА
...
//++ НЕ УТКА
	...
	ПроверитьОтметитьНаличиеАналогов(                                                               ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
...
//++ Рарус IA 03.10.2016
	пкМодификацияСервер.пкОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
//-- Рарус IA 03.10.2016
...
//+++rarus-spb_zlov 25.09.2016  	
    ...
    Если (ТекущаяСтрока.Номенклатура.пкЭтоТехника ИЛИ ВидНоменклатуры.пкЭтоУслугаТранспортировки) И НЕ ЗначениеЗаполнено(ТекущаяСтрока.ВидЦены) Тогда
        ОбъектСтроительства = Неопределено;
        Если СтруктураПараметровДействия.Свойство("ОбъектСтроительства") Тогда
            ОбъектСтроительства = СтруктураПараметровДействия.ОбъектСтроительства;    
        КонецЕсли; 
        
        Договор = Неопределено;
        Если СтруктураПараметровДействия.Свойство("Договор") Тогда
            Договор = СтруктураПараметровДействия.Договор;    
        КонецЕсли; 
        
        ТекущийДокумент = Неопределено;
        
        Если СтруктураПараметровДействия.Свойство("ТекущийДокумент") Тогда
            ТекущийДокумент = СтруктураПараметровДействия.ТекущийДокумент;    
        КонецЕсли; 
        ...
        Если ТекущаяСтрока.Номенклатура.пкЭтоТехника Тогда
			//++ Рарус Лимаренко 24.10.2018 № 14625
				Если ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ЗаказКлиента") И ТекущаяСтрока.Свойство("пкЗамененнаяМодель") И ЗначениеЗаполнено(ТекущаяСтрока.пкЗамененнаяМодель) Тогда
		        СтруктуруЦены = пкУправлениеТехникойСервер.ПолучитьСтруктуруЦеныАрендыТехники(ТекущаяСтрока.пкЗаявкаНаАрендуТехники, ОбъектСтроительства,
				ТекущаяСтрока.пкТехника, ТекущаяСтрока.пкЗамененнаяМодель, Договор, ТекущаяСтрока.Количество, ТекущийДокумент, Дата, ТекущаяСтрока.пкВидТарифа, ТекущаяСтрока.Упаковка,ТекущаяСтрока.пкЗамененнаяМодель);	 
			Иначе
				СтруктуруЦены = пкУправлениеТехникойСервер.ПолучитьСтруктуруЦеныАрендыТехники(ТекущаяСтрока.пкЗаявкаНаАрендуТехники, ОбъектСтроительства, 
                ТекущаяСтрока.пкТехника, ТекущаяСтрока.Номенклатура, Договор, ТекущаяСтрока.Количество, ТекущийДокумент, Дата, ТекущаяСтрока.пкВидТарифа, ТекущаяСтрока.Упаковка);  
				КонецЕсли;
			//-- Рарус Лимаренко 24.10.2018 № 14625	
			                 
            Цена = СтруктуруЦены.Цена; 
        Иначе
//Рарус Владимир Подрезов 14.01.2018 7744
			...
			Если ТекущийДокумент <> Неопределено 
				...
				И ЗначениеЗаполнено(ТекущийДокумент.ЗаказКлиента)
				//И Цена = 0
			Тогда
			    ...
			    ЗапросТ.УстановитьПараметр("Заказ", 		ТекущийДокумент.ЗаказКлиента);
			    ЗапросТ.УстановитьПараметр("Номенклатура",	ТекущаяСтрока.Номенклатура);
			                                                   
			    РезультатТ = ЗапросТ.Выполнить().Выбрать();
			    Если РезультатТ.Следующий() Тогда
					Цена = РезультатТ.Цена;    
			    КонецЕсли;
			КонецЕсли;
			Если ТекущаяСтрока.Цена = 0 И ВидНоменклатуры.пкЭтоУслугаТранспортировки Тогда
//Рарус Владимир Подрезов Конец
            	Цена = пкУправлениеТехникойСервер.ПолучитьЦенуТранспортировки(ТекущаяСтрока.Номенклатура, Договор, ОбъектСтроительства); 
//Рарус Владимир Подрезов 14.01.2018 7744
			ИначеЕсли ТекущаяСтрока.Цена > 0 И ВидНоменклатуры.пкЭтоУслугаТранспортировки Тогда
				Цена = ТекущаяСтрока.Цена;
			КонецЕсли;
//Рарус Владимир Подрезов Конец
        КонецЕсли;    
            
        //Цена = РаботаСКурсамиВалют.ПересчитатьВВалюту(Цена, Договор.ВалютаВзаиморасчетов, СтруктураПараметровОтбора.Валюта, СтруктураПараметровОтбора.Дата); 
        
        Если ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ЗаказКлиента") И НЕ ЗначениеЗаполнено(ТекущаяСтрока.пкВидТарифа)
          И ТекущаяСтрока.Номенклатура.пкЭтоТехника Тогда
            ТекущаяСтрока.пкВидТарифа = СтруктуруЦены.ВидТарифа;   	
		КонецЕсли;
...
//+++rarus-spb_zlov 19.10.2016    
	Если СтруктураДействий.Свойство("пкТипТехники", СтруктураПараметровДействия)
		...
		СтруктураДействийЗаполнения.Вставить("пкТипТехники", СтруктураПараметровДействия);
	КонецЕсли;
//---rarus-spb_zlov 19.10.2016 
...
Процедура ПроверитьЗаполнитьЦенуПобочногоВыпускаВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если Не СтруктураДействий.Свойство("ПроверитьЗаполнитьЦенуПобочногоВыпуска", СтруктураПараметровДействия) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьОтметитьНаличиеАналогов(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)

	Перем ПараметрыВыбораАналогов;

	Если НЕ СтруктураДействий.Свойство("ПроверитьОтметитьНаличиеАналогов", ПараметрыВыбораАналогов) Тогда
		Возврат;
	КонецЕсли;
	
	Если ПараметрыВыбораАналогов.Свойство("ИмяПоляЕстьАналоги") Тогда
		ИмяПоля = ПараметрыВыбораАналогов.ИмяПоляЕстьАналоги;
	Иначе
		ИмяПоля = "ЕстьАналогиМатериала";
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		ТекущаяСтрока[ИмяПоля] = УправлениеПроизводством.ПроверитьНаличиеАналогов(ТекущаяСтрока.Номенклатура,
																					ТекущаяСтрока.Характеристика,
																					ТекущаяСтрока.КлючСвязиСпецификация,
...
Процедура ПроверитьЗаполнитьОбеспечениеВСтрокеТЧЭтапаПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	ОбеспечениеПроизводства.ЗаполнитьОбеспечениеВСтрокеЭтапаПроизводства(ТекущаяСтрока, ПараметрыДействия);
	
КонецПроцедуры

Процедура ПроверитьЗаполнитьСкладПроизводстваВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
