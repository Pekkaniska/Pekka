		...
		Для Каждого ЭлементОчереди Из Очередь Цикл
			...
			Исключение
				...
				Если Не (Не ИнтеграцияЕГАИС.СообщатьОбОшибкахПриЗагрузкеДанных()
					...
					ИнтеграцияЕГАИСКлиентСервер.СообщитьПользователюВФорму(ИдентификаторВладельца, ТекстОшибки);
...
Процедура ИнформироватьОбОшибкеВДокументе(Шаблон, Ссылка, ПредставлениеОшибки) Экспорт
	...
	ТекстСообщения = СтрШаблон(НСтр(Шаблон, ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Ссылка, ПредставлениеОшибки);
	ЗаписатьОшибкуВЖурналРегистрации(ТекстСообщения);
	
	ТекстСообщения = СтрШаблон(НСтр(Шаблон), Ссылка, ПредставлениеОшибки);
