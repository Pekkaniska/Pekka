...
//
// Параметры:
//   Форма - УправляемаяФорма - Форма, в которой расположена команда вызывающая команда.
...
//         с условием ИмяСобытия = "ВосстановленыНачальныеЗначения".
//       Примечание. Обработка ошибок выполнется автоматически.
...
Процедура ВосстановитьНачальныеЗначенияПослеЗавершенияФоновогоЗадания(Задание, Контекст) Экспорт
	...
	Если Статус = "Ошибка" Тогда
		...
		ВывестиИнформациюОбОшибке(Кратко, Задание.ПодробноеПредставлениеОшибки);
	КонецЕсли;
...
//
// Параметры:
//   КраткийТекст - Строка - Краткий текст ошибки.
//   ПодробныйТекст - Строка - Необязательный. Тексты ошибок, которые при желании может просмотреть пользователь.
//   Заголовок - Строка - Необязательный. Заголовок предупреждения.
//
Процедура ВывестиИнформациюОбОшибке(КраткийТекст, ПодробныйТекст = "", Заголовок = "") Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("ВывестиИнформациюОбОшибкеЗавершение", ЭтотОбъект, Контекст);
	ПоказатьВопрос(Обработчик, КраткийТекст, Кнопки, , 0, Заголовок);
КонецПроцедуры

// Продолжение процедуры ВывестиИнформациюОбОшибке.
Процедура ВывестиИнформациюОбОшибкеЗавершение(Ответ, Контекст) Экспорт
	Если Ответ = "Подробнее" Тогда
		...
		НастройкиДиалога.Вставить("Заголовок", ?(ЗначениеЗаполнено(Контекст.Заголовок), Контекст.Заголовок, НСтр("ru = 'Подробнее'")));
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(0, НСтр("ru = 'Закрыть'"));
		
		Обработчик = Новый ОписаниеОповещения("ВывестиИнформациюОбОшибкеЗавершение", ЭтотОбъект, Контекст);
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(Обработчик, Контекст.ПодробныйТекст, Кнопки, НастройкиДиалога);
	КонецЕсли;
КонецПроцедуры
...
Процедура ФормаОрганизацииПередЗаписьюОбработатьПФР(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ДополнительныеОбработки.Количество() > 0 Тогда
		...
		ДополнительныеПараметры.Отказ = Истина;
		ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "СохранениеИзмененийВЗарплатеОрганизации");
