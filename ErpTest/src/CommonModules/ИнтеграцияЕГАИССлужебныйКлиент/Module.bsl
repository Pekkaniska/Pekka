...
Процедура ПослеПередачиСообщений()
...
Процедура ПолучитьДокументыПоОчереднойОрганизацииИзОчередиОбмена(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПослеПолученияСписковСообщений();
...
Процедура ПослеПолученияСписковСообщений()
	...
	Если ЗначениеЗаполнено(ДанныеОбменаЕГАИС.ТекстОшибки) Тогда
		ИнтеграцияЕГАИСКлиентСервер.СообщитьПользователюВФорму(ДанныеОбменаЕГАИС.Контекст.ИдентификаторВладельца, ДанныеОбменаЕГАИС.ТекстОшибки);
	КонецЕсли;
...
Процедура ПослеПолученияСообщений()
...
Процедура ПослеПодтвержденияСообщений()
...
//
Функция СохраняемыеДанныеОбмена(ПакетыОбмена, Контекст, ОбработчикОповещения)
...
Функция ОбменУжеЗапущен(Контекст, ВыводитьСообщения = Истина)
	
	Контекст.Вставить("Форма",Неопределено);
	Если Контекст.ВОсновнойФорме = Ложь Тогда
		Контекст.Форма = ОткрытьФорму("ОбщаяФорма.ДлительнаяОперация",
			Новый Структура("ТекстСообщения",НСтр("ru = 'Выполняется обмен с  ЕГАИС'")),
			,"EGAIS3",,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	КонецЕсли;
		
	ИдетОбменСЕГАИС = ПараметрыПриложения["ЕГАИС.ДанныеСеансаОбмена"];
	Если ИдетОбменСЕГАИС <> Неопределено Тогда 
		Если ВыводитьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Обмен с УТМ ЕГАИС уже запущен, дождитесь завершения предыдущего сеанса'"));
		КонецЕсли;
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗакрытьФормуВыполненияОбмена(Форма)
...
Функция ОбщиеПараметрыОбменаНаКлиенте(Источник)
