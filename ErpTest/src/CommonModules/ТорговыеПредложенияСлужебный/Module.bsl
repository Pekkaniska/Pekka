...
Функция ВыполнитьЗапрос(ЗапросСервиса, Отказ) Экспорт
	...
	ПараметрыСоединения = БизнесСетьПовтИсп.ПараметрыСоединения("ТорговыеПредложения");
...
Функция ПолучитьНовыйТикет(Тикет, Соединение, ПараметрыСоединения, ЗапросСервисаТикета, Отказ)
	...
	Попытка
		...
		Запрос = Новый HTTPЗапрос(ЗапросСервисаТикета.Адрес);
		Запрос.Заголовки.Вставить("vnd.1cbn.endpoint-id", ИдентификаторПрограммы);
		Запрос.Заголовки.Вставить("Content-Type", ЗапросСервисаТикета.Контент);
		Запрос.УстановитьТелоИзСтроки(БизнесСеть.ЗначениеВJSON(ТикетИТС));
...
Функция ОбработатьОшибкуСервиса(Соединение, ЗапросСервиса, КодСостояния, Данные, Отказ)
	...
	КонецЕсли;
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Торговые предложения.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())
...
Процедура СформироватьПредставлениеТорговогоПредложения(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Если Отказ Или Не ЗначениеЗаполнено(Результат) Тогда
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Чтение торгового предложения.'"),
			ПодробнаяИнформация, ТекстСообщения, "БизнесСеть");
		Возврат;
	КонецЕсли;
	...
	ПараметрыВывода.Вставить("АдресаКартинок", "");
	Если ЗначениеЗаполнено(Результат.ИдентификаторНоменклатурыСервиса) Тогда
		...
		НаименованиеКатегории = Результат.НаименованиеКатегорииСервиса;
		Свойства = Результат.СвойстваСервиса;
	Иначе
		ИдентификаторКатегории = Результат.ИдентификаторКатегории;
		НаименованиеКатегории  = Результат.НаименованиеКатегории;
	...
	Если Отказ ИЛИ Не ЗначениеЗаполнено(РеквизитыКонтрагента) Тогда
		ТекстСообщения = НСтр("ru = 'Ошибка получения данных поставщика.'");
		ПодробнаяИнформация = ТекстСообщения + Символы.ПС + СтрШаблон(НСтр("ru = 'Идентификатор %1.'"), Идентификатор)
			 + Символы.ПС + СтрШаблон(НСтр("ru = 'Поставщик ИНН/КПП %1/%2.'"),
...
Процедура ПоискСоответствийНоменклатурыСервиса(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Попытка
		...
		Для каждого ЭлементКоллекции Из ТаблицаПоиска Цикл
			...
			Если ЗначениеЗаполнено(Запись.ИдентификаторНоменклатурыСервиса) Тогда
				Продолжить;
			ИначеЕсли (Не ЗначениеЗаполнено(Результат) ИЛИ Результат.Количество() = 0) И Запись.Подтверждено = Ложь Тогда
			...
			Иначе
				...
				Если КоличествоВариантов <= 10 Тогда
					Запись.ПредставлениеНоменклатурыСервиса = СтрШаблон(НСтр("ru = '<Несколько вариантов (%1)>'"), КоличествоВариантов);
				Иначе
	...
	Исключение
		ОтменитьТранзакцию();
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Поиск эталонов 1С:Номенклатура.'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
			НСтр("ru = 'Ошибка поиска эталонов 1С:Номенклатура для публикации торговых предложений.'"), "БизнесСеть");
	КонецПопытки;
...
Процедура СинхронизацияТорговыхПредложений(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Пока ВыборкаПрайсЛисты.Следующий() Цикл
		...
		Если ВыборкаПрайсЛисты.Отключено
			И ВыборкаПрайсЛисты.ДействиеСинхронизации = Перечисления.СостоянияСинхронизацииТорговыеПредложения.Синхронизировано Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Не Отказ Тогда
		УдалитьУстаревшиеПрайсЛистыВСервисе(Отказ);
	КонецЕсли;
...
Функция СформироватьТабличныйДокументТорговогоПредложения(ПараметрыВывода)
	...
	Если ЗначениеЗаполнено(ПараметрыВывода.АдресаКартинок)Тогда
		ОбластьКартинка.Рисунки.Рисунок.Картинка = Новый Картинка(
			ДвоичныеДанныеПоАдресуURL(ПараметрыВывода.АдресаКартинок[0]));
...
Процедура УдалитьУстаревшиеПрайсЛистыВСервисе(Отказ)
	...
	Для каждого ЭлементКоллекции Из Результат.Список Цикл
		...
		// Обработка прайс-листов организаций, не зарегистрированных в информационной базе не производится.
		Если Идентификаторы.Найти(ЭлементКоллекции.ИдентификаторОрганизации, "ИдентификаторОрганизации") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Идентификаторы.Найти(ЭлементКоллекции.Идентификатор, "ИдентификаторПрайсЛиста") = Неопределено Тогда
			...
			ПараметрыЗапроса = НовыеПараметрыПолученияПрайсЛистов(ЭлементКоллекции.Идентификатор);
			ЗапросСервиса = ЗапросСервисаУдалениеПрайсЛиста(ПараметрыЗапроса);
			ВыполнитьЗапрос(ЗапросСервиса, Отказ);
			Если Отказ Тогда
				Прервать;
...
Функция НовыеПараметрыПолученияПрайсЛистов(Идентификатор = "")
...
Функция НовыйЗапросСервиса()
	...
	ЗапросСервиса.Вставить("Обработка");        // Описание обработки данных результата.
...
Функция НовыеПараметрыПоискаТорговыхПредложенийСОтборами() Экспорт
	...
	ТаблицаТоваровСервиса = Новый ТаблицаЗначений;
	ТаблицаТоваровСервиса.Колонки.Добавить("ИдентификаторНоменклатурыСервиса",
		Новый ОписаниеТипов("Строка")); // nomenclatureId
	ТаблицаТоваровСервиса.Колонки.Добавить("ИдентификаторХарактеристикиСервиса",
	...
	Результат.Вставить("НоменклатураСервиса",    ТаблицаТоваровСервиса); // productSpecificationIds
...
Функция НовыеПараметрыПоискаТорговыхПредложенийПоСписку() Экспорт
	...
	Товары.Колонки.Добавить("РеквизитыСервиса",  Типы().Таблица,
		"Идентификатор (Строка), Значение (Строка)"); // categoryRequisitesFilter
	Товары.Колонки.Добавить("ТоварыСервиса", Типы().Таблица,
		"ИдентификаторНоменклатурыСервиса (Строка), ИдентификаторХарактеристикиСервиса (Строка)"); // productSpecificationIds
...
//
Функция ЗапросСервисаПолучениеИдентификатораОрганизации(Организация)
...
Функция ЗапросСервисаПоискТорговыхПредложенийСОтборами(ПараметрыЗапроса) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыЗапроса.НоменклатураСервиса) Тогда
		...
		Для каждого ЭлементНоменклатурыСервиса Из ПараметрыЗапроса.НоменклатураСервиса Цикл
			СтруктураНоменклатурыСервиса = Новый Структура;
			СтруктураНоменклатурыСервиса.Вставить("nomenclatureId",
				ЭлементНоменклатурыСервиса.ИдентификаторНоменклатурыСервиса);
			СтруктураНоменклатурыСервиса.Вставить("nomenclatureCharacteristicId",
				ЭлементНоменклатурыСервиса.ИдентификаторХарактеристикиСервиса);
		КонецЦикла;
...
//
// Параметры:
//  ПараметрыЗапроса - Структура - параметры поиска, см. НовоеТорговоеПредложение.
// 
// Возвращаемое значение:
//  Структура - данные запроса.
//
Функция ЗапросСервисаСозданиеОбновлениеНоменклатурыПоставщика(ПараметрыЗапроса, Идентификатор, Отказ)
	...
	НаименованиеНоменклатуры = ПараметрыЗапроса.НаименованиеНоменклатуры
		+ ?(ПустаяСтрока(ПараметрыЗапроса.НаименованиеХарактеристики), "", ", " + ПараметрыЗапроса.НаименованиеХарактеристики);
...
//
// Параметры:
//  ПараметрыЗапроса	 - Структура - см. НовыеПараметрыСозданияТорговогоПредложения.
// 
// Возвращаемое значение:
//  Структура - данные запроса.
//
Функция ЗапросСервисаСозданиеОбновлениеТорговогоПредложения(ПараметрыЗапроса, Идентификатор, Отказ)
	...
	Заголовок = "" + ПараметрыЗапроса.Номенклатура
		+ ?(ЗначениеЗаполнено(ПараметрыЗапроса.Характеристика), " ," + ПараметрыЗапроса.Характеристика, "");
...
Процедура ДобавитьКолонкиТаблицыОписания(ТаблицаИсточник, Имя, Префикс = "")
	...
	ИначеЕсли Имя = "Search/TradeOfferV1_Product_1CNomenclature" Тогда
		...
		Таблица.Колонки.Добавить("ИдентификаторХарактеристикиСервиса", Типы().Строка, "characteristicId");
		Таблица.Колонки.Добавить("ПредставлениеХарактеристикиСервиса", Типы().Строка, "characteristicName");
...
Функция КонвертироватьЗначениеСервиса(Значение, Правило, КодСостояния = 200)
	...
	ИначеЕсли Правило = "ДесериализацияВалюты" Тогда
		...
		Для каждого ЭлементКоллекции Из Значение Цикл
			Если ЗначениеЗаполнено(ЭлементКоллекции) Тогда
				Результат.Добавить(ТорговыеПредложенияПовтИсп.ПолучитьВалюту(ЭлементКоллекции));
			Иначе
				Результат.Добавить(Неопределено);
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбработатьТорговыеПредложения(ПрайсЛист, ТекстОшибки, Отказ)
	...
	ЗагрузитьМанифестТорговыхПредложений(МанифестИзмененныхПредложений, МанифестУдаленныхПредложений);
	...
	ЗагрузитьМанифестНоменклатуры(МанифестИзмененийНоменклатуры, МанифестУдаленнойНоменклатуры);
	...
	Для каждого СтрокаТовары Из ПрайсЛист.Товары Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТовары.ИдентификаторНоменклатурыСервиса) Тогда
			...
			ПроверитьНоменклатуруПоставщика(СтрокаТовары, ПрайсЛист.Штрихкоды, ПрайсЛист.Свойства,
				МанифестИзмененийНоменклатуры, Обновлено, Отказ);
			Если Отказ Тогда
				Возврат;
			КонецЕсли;
...
Процедура ЗагрузитьМанифестТорговыхПредложений(МанифестИзмененныхПредложений, МанифестУдаленныхПредложений)
...
Процедура ЗагрузитьМанифестНоменклатуры(МанифестИзмененийНоменклатуры, МанифестУдаленнойНоменклатуры)
...
Процедура ПроверитьНоменклатуруПоставщика(СтрокаТовары, Штрихкоды, Свойства, МанифестИзмененийНоменклатуры, Обновлено, Отказ)
	...
	Если ТребуетсяОбновление Тогда
		СохранитьНоменклатуруПоставщика(СтрокаТовары.ИдентификаторНоменклатуры, ХешНовый, ДанныеНоменклатуры, Отказ);
		Если Не Отказ Тогда
...
Процедура СохранитьНоменклатуруПоставщика(Идентификатор, ХешНовый, ДанныеНоменклатуры, Отказ)
	...
	Если Отказ Тогда
		ТекстОшибки = НСтр("ru = 'Ошибка обновления номенклатуры поставщика'");
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьТорговоеПредложение(ПрайсЛист, СтрокаТовары, МанифестИзмененныхПредложений, ТекстОшибки, Отказ)
	...
	Если ХешНовый <> ХешПредложения Тогда
		ТребуетсяОбновление = Истина;
	КонецЕсли;
...
Процедура СохранитьОбновитьТорговоеПредложение(ДанныеПредложения, Идентификатор, ХешНовый, ТекстОшибки, Отказ)
	...
	Если Не ЗначениеЗаполнено(Идентификатор) Тогда
		...
		Если Отказ И Результат = 422 Тогда
			...
			Результат = ВыполнитьЗапрос(ЗапросСервиса, Отказ);
			Если Не Отказ И ЗначениеЗаполнено(Результат) И Результат.Список.Количество() Тогда 
				ПолучитьСообщенияПользователю(Истина); // Удалим сообщение об ошибке, т.к. у нас получилось найти предложение.
				Идентификатор = Результат.Список[0].Идентификатор;
...
Процедура ОчиститьУстаревшиеИдентификаторыТорговыхПредложений(Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Число(Результат) = 404 Тогда
			Отказ = Ложь;
		ИначеЕсли Отказ Тогда
			Возврат;
		КонецЕсли;
...
Процедура ИзменитьСостояниеПубликацииПрайсЛистов(ПараметрыМетода) Экспорт
	...
	ЭтоОперацияУдаленияОтключения = Ложь;
