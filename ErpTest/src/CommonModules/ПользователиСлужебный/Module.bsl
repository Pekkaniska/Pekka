...
//Рарус Владимир Подрезов 26.07.2017
	Если РольДоступна(Метаданные.Роли.пкПолныеПрава) Тогда // Не заменять на РолиДоступны.
		Возврат "";
	КонецЕсли;
...
//Поищим среди пользователей МП
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	пкПользователиМП.Пользователь КАК Ссылка
	|ИЗ
	|	Справочник.пкПользователиМП КАК пкПользователиМП
	|ГДЕ
	|	пкПользователиМП.ИдентификаторПользователяИБ = &ИдентификаторПользователяИБ
	|	И пкПользователиМП.Пользователь <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	пкПользователиМП.Пользователь
	|ИЗ
	|	Справочник.пкПользователиМП КАК пкПользователиМП
	|ГДЕ
	|	пкПользователиМП.Наименование = &ПолноеИмя
	|	И пкПользователиМП.Пользователь <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)";
	Запрос.УстановитьПараметр("ПолноеИмя", Результат.ИмяПользователя);
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция СсылкаПользователяПоПолномуНаименованию(ПолноеИмя)
	...
	|ГДЕ
	|	Пользователи.Наименование = &ПолноеИмя
//++ Рарус Лимаренко 29.06.2018	
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	пкПользователиМП.Пользователь.Ссылка,
	|	пкПользователиМП.Пользователь.ИдентификаторПользователяИБ
	|ИЗ
	|	Справочник.пкПользователиМП КАК пкПользователиМП
	|ГДЕ
	|	пкПользователиМП.Наименование = &ПолноеИмя	
