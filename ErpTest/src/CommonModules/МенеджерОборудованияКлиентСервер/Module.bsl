...
Функция РасшифроватьQRКодЧекаККТ(ЗначениеQRКода) Экспорт
	...
	ДанныеШтрихкода.ПризнакРасчета = ПолучитьТипРасчетаДенежнымиСредствамиПоКоду(ДанныеШтрихкода.ПризнакРасчетаКод);
...
// 
Функция ПолучитьКодТипаМаркировкиККТ(ТипМаркировкиККТ) Экспорт
	
	Если ТипМаркировкиККТ = ПредопределенноеЗначение("Перечисление.ТипыМаркировкиККТ.ИзделияИзМеха") Тогда
		ТипМаркировкиККТКод = "02"
	ИначеЕсли ТипМаркировкиККТ = ПредопределенноеЗначение("Перечисление.ТипыМаркировкиККТ.ЛекарственныеПрепараты") Тогда
		ТипМаркировкиККТКод = "03"
	ИначеЕсли ТипМаркировкиККТ = ПредопределенноеЗначение("Перечисление.ТипыМаркировкиККТ.ТабачнаяПродукция") Тогда
		ТипМаркировкиККТКод = "05"
	ИначеЕсли ТипМаркировкиККТ = ПредопределенноеЗначение("Перечисление.ТипыМаркировкиККТ.ОбувныеТовары") Тогда
		ТипМаркировкиККТКод = "1520"
	КонецЕсли;
	
	Возврат ТипМаркировкиККТКод;
...
Функция ПараметрыФискальнойСтрокиЧека() Экспорт;
	...
	ПараметрыСтроки.Вставить("СуммаНДС"  , 0);            // Число,Обязательно - Сумма НДС
...
Функция ПараметрыНормализуемыхФискальныхДанных() Экспорт
	...
	СтруктураРезультат.Вставить("НеобходимаСтрочнаяЗаменаФН");     // Признак необходимости срочной замены ФН.
	СтруктураРезультат.Вставить("ПамятьФНПереполнена");            // Признак переполнения памяти ФН.
	СтруктураРезультат.Вставить("РесурсФНИсчерпан");               // Признак исчерпания ресурса ФН.
