...
// 	   * ЭкспортНесырьевыхТоваров - Булево - Признак того, что используется экспорт несырьевых товаров.
// 	   * ЭкспортСырьевыхТоваровУслуг - Булево - Признак того, что используется экспорт несырьевых товаров. 
...
Процедура ЗаполнитьВидДеятельностиНДС(ВидДеятельностиНДС, ПараметрыЗаполнения, КэшированныеЗначенияПараметров = Неопределено) Экспорт
	...
	ИначеЕсли (ИзмененныеПараметры.Свойство("Организация") 
			    И ЗначениеЗаполнено(ПараметрыЗаполнения.Организация))
...
//
Процедура АктуализироватьСчетаФактурыПолученныеПередЗаписью(ПараметрыРегистрации, РежимЗаписи, ПометкаУдаления) Экспорт
	
	//++ Локализация
	СчетФактураНеТребуется = УчетНДСУПСлужебный.СчетФактураПолученныйНеТребуется(ПараметрыРегистрации);
	Проведен = (РежимЗаписи = РежимЗаписиДокумента.Проведение);
...
Процедура ЗаполнитьНалогообложениеНДСПродажи(НалогообложениеНДС, ПараметрыЗаполнения, КэшированныеЗначенияПараметров = Неопределено) Экспорт
	...
	ИзмененныеПараметры = УчетНДСУПСлужебный.ИзмененныеПараметры("ПараметрыЗаполненияНалогообложенияНДСПродажи", 
							ПараметрыЗаполнения, КэшированныеЗначенияПараметров);
	...
	Если ЗначениеЗаполнено(НалогообложениеНДС) И МассивДоступных.Найти(НалогообложениеНДС) = Неопределено Тогда
		// Значение указанное в НСИ не доступно по учетной политике
		НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка();
	КонецЕсли;
		
	УчетНДСУПСлужебный.ОбновитьКэшированныеЗначенияПараметров(
...
//
Процедура АктуализироватьСчетаФактурыВыданныеПередЗаписью(ПараметрыРегистрации, РежимЗаписи, ПометкаУдаления) Экспорт
...
//
Процедура АктуализироватьСчетаФактурыКомиссионеруПередЗаписью(ПараметрыРегистрации, РежимЗаписи, ПометкаУдаления) Экспорт
...
Процедура ОписаниеОперацийИТиповДокументовДляЖурналаДокументовЗакупки(ХозяйственныеОперацииИДокументы) Экспорт
	...
	Строка = ХозяйственныеОперацииИДокументы.Добавить();
	ЗаполнитьЗначенияСвойств(Строка,СтрокаСчетФактураПолученный);
	Строка.ХозяйственнаяОперация 		= Перечисления.ХозяйственныеОперации.ВыкупТоваровХранящихсяНаСкладе;
	
	Строка = ХозяйственныеОперацииИДокументы.Добавить();
	ЗаполнитьЗначенияСвойств(Строка,СтрокаСчетФактураПолученный);
	Строка.ХозяйственнаяОперация 		= Перечисления.ХозяйственныеОперации.ВыкупТоваровПереданныхВПроизводство;
	//-- НЕ УТ
