...
Процедура СоздатьМенеджерКриптографииПослеУстановкиЗначенияСвойстваНеВыводитьСообщенияОбОшибках(ВходящийКонтекст) Экспорт
	...
	Если ВходящийКонтекст.ПараметрыКриптографии <> Неопределено Тогда
		РезультатВыполнения = Новый Структура("Выполнено, Алгоритм", Истина, ПараметрыВызова.Алгоритм);
...
Процедура СоздатьМенеджерКриптографииПослеОпределенияАлгоритма(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		РезультатЗаполнения = ЗаполнитьПараметрыКриптографии(Алгоритм);
...
Процедура СоздатьМенеджерКриптографииПослеПолученияСпискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если КриптопровайдерУстановленВСистеме Тогда
			...
			КонецЕсли;
			РезультатВыполнения = КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "МенеджерКриптографии", ВходящийКонтекст.МенеджерКриптографии,
				Новый Структура("Алгоритм", ТекущийКриптопровайдер.Алгоритм));
			ВыполнитьОбработкуОповещения(
				ВходящийКонтекст.ОповещениеОЗавершении,
				РезультатВыполнения);
		Иначе
...
Процедура ПолучитьСертификаты(ОповещениеОЗавершении, ХранилищеСертификатов, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Контекст.ХранилищеСертификатов.ЛокальныеХранилища) Тогда
		Параметры = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(Параметры);
		...
		СоздатьМенеджерКриптографии2(
			Оповещение,
			КриптографияЭДКОКлиентСервер.КриптопровайдерMicrosoftBaseCryptographicProvider(), 
			Параметры);
	Иначе
...
Процедура ПолучитьСертификатыИзЛокальногоХранилища(ВходящийКонтекст)
	...
	Если ВходящийКонтекст.ТекущееЛокальноеХранилище < ВходящийКонтекст.ХранилищеСертификатов.ЛокальныеХранилища.Количество() Тогда
		// Нужно получить сертификаты по всем хранилищам.
		// В этом случае можно получить все сертификаты за один вызов, если не указывать хранилище.
		Если ВходящийКонтекст.ХранилищеСертификатов.ЛокальныеХранилища.Количество() = 4 Тогда
			Хранилище = "";
			ВходящийКонтекст.ТекущееЛокальноеХранилище = 4;
		Иначе
...
Процедура ПроверитьСертификатПослеПоискаСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено И Результат.СертификатНайден Тогда
		// обход нереализованности проверки без учета даты во внешней компоненте
		Если ВходящийКонтекст.ВходящиеПараметры[1] = "ПроверитьТолькоПоСпискуУстановленныхУЦ" Тогда
			ТекДата = ОбщегоНазначенияКлиент.ДатаУниверсальная();
			Если ТипЗнч(Результат.СвойстваСертификата.ДействителенС) <> Тип("Дата") ИЛИ ТипЗнч(Результат.СвойстваСертификата.ДействителенПо) <> Тип("Дата")
			ИЛИ ТекДата - 49 * 3600 < Результат.СвойстваСертификата.ДействителенС ИЛИ ТекДата + 49 * 3600 > Результат.СвойстваСертификата.ДействителенПо Тогда
				ВыполнитьОбработкуОповещения(
					ВходящийКонтекст.ОповещениеОЗавершении,
...
Процедура ВыбратьСертификат(ОповещениеОЗавершении, ЭтоЭлектроннаяПодписьВМоделиСервиса, НачальноеЗначениеВыбора, ХранилищеСертификатов, 
		ПоказыватьПросроченные, МножественныйВыбор, Отпечатки, ЗапретитьВыборПросроченных, БезВозможностиВыбора) Экспорт
	
	ПараметрыФормы = Новый Структура("Хранилище, ПоказыватьПросроченные, НачальноеЗначениеВыбора, МножественныйВыбор, Отпечатки, ЗапретитьВыборПросроченных, БезВозможностиВыбора",
									ХранилищеСертификатов, ПоказыватьПросроченные, НачальноеЗначениеВыбора, МножественныйВыбор, Отпечатки, ЗапретитьВыборПросроченных, БезВозможностиВыбора);
...
Процедура ПроверитьПодписьPKCS7ПослеПоискаСертификатов(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ВходящийКонтекст.Подписанты = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ВходящийКонтекст.Подписанты);
		Для ИндексПодписанта = 0 По ВходящийКонтекст.Подписанты.Количество() - 1 Цикл
			ВходящийКонтекст.Подписанты[ИндексПодписанта] = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(
				ВходящийКонтекст.Подписанты[ИндексПодписанта]);
...
Процедура ЗашифроватьPKCS7ПослеСозданияМенеджераКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если Результат.Алгоритм = "GOST R 34.10-2012-256" ИЛИ Результат.Алгоритм = "GOST R 34.10-2012-512" Тогда
...
Процедура РасшифроватьPKCS7(
	...
	Если Сертификат = Неопределено Или КриптографияЭДКОКлиентСервер.СертификатВЗащищенномХранилищеНаСервере(Сертификат) Тогда
...
Процедура РасшифроватьФайл(
	...
	Если СертификатПолучателя = Неопределено ИЛИ КриптографияЭДКОКлиентСервер.СертификатВЗащищенномХранилищеНаСервере(СертификатПолучателя) Тогда
...
Процедура ХешироватьДанные(
	...
	Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		...
		ПредварительноеОповещениеОЗавершении = Новый ОписаниеОповещения("ХешироватьДанныеСервисПередЗавершением", ЭтотОбъект, ПараметрыОповещения);
		
		СервисКриптографииКлиент.ХешированиеДанных(ПредварительноеОповещениеОЗавершении, ИмяФайлаДанных);
...
Процедура ОпределитьАлгоритм(
	...
	Если ПараметрыВызова.Алгоритм <> Неопределено ИЛИ ПараметрыВызова.Криптосообщение = Неопределено
		...
		ВыполнитьОбработкуОповещения(
			ОповещениеОЗавершении,
			КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "Алгоритм", Алгоритм));
...
Процедура ОпределитьАлгоритмПослеЧтенияФайлаКриптосообщения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ЗначениеЗаполнено(АлгоритмКриптосообщения) Тогда
			ВыполнитьОбработкуОповещения(
				ОповещениеОЗавершении,
				КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "Алгоритм", АлгоритмКриптосообщения));
		Иначе
...
Процедура ОпределитьАлгоритмПослеЭкспортаСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если ЗначениеЗаполнено(СвойстваСертификата.АлгоритмПубличногоКлюча) Тогда
			ВыполнитьОбработкуОповещения(
				ОповещениеОЗавершении,
				КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "Алгоритм", СвойстваСертификата.АлгоритмПубличногоКлюча));
		Иначе
			ОписаниеОшибки = ОписаниеОшибкиНеУдалосьОпределитьКриптографическийАлгоритм();
			ВыполнитьОбработкуОповещения(
				ОповещениеОЗавершении,
...
Процедура СоздатьМенеджерКриптографии2ПослеУстановкиВнешнейКомпоненты(ВходящийКонтекст) Экспорт
	...
	ДополнительныеПараметры = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ВходящийКонтекст.ДополнительныеПараметры);
...
Процедура СоздатьМенеджерКриптографии2ПослеПолученияКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
			...
			Если ЗначениеЗаполнено(ВходящийКонтекст.Криптопровайдер) Тогда
				...
				Для Каждого ДоступныйКриптопровайдер Из Результат.Криптопровайдеры Цикл
					Если ДоступныйКриптопровайдер.Имя = ВходящийКонтекст.Криптопровайдер.Имя
						...
						Криптопровайдер = ДоступныйКриптопровайдер;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			Иначе
		...
		Если ЗначениеЗаполнено(Криптопровайдер) Тогда
			...
			ВходящийКонтекст.Вставить("Криптопровайдер", Криптопровайдер);
			СвойстваКриптопровайдера = КриптографияЭДКОКлиентСервер.СвойстваКриптопровайдера(Криптопровайдер.Имя, Криптопровайдер.Тип);
