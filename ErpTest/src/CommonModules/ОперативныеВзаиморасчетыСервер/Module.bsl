...
Процедура ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры) Экспорт
	...
	Если ЭтоРасчетыСКлиентами Тогда
		...
		//++ НЕ УТКА
		РасчетыПоЗаказу = РасчетыПоЗаказу ИЛИ ТипЗнч(ОбъектРасчетов) = Тип("ДокументСсылка.ЗаказДавальца");
		//-- НЕ УТКА
	Иначе
		РасчетыПоЗаказу = ТипЗнч(ОбъектРасчетов) = Тип("ДокументСсылка.ЗаказПоставщику");
		//++ НЕ УТ
		РасчетыПоЗаказу = РасчетыПоЗаказу ИЛИ ТипЗнч(ОбъектРасчетов) = Тип("ДокументСсылка.ЗаказПереработчику")
		//-- НЕ УТ
	КонецЕсли;
			...
			Иначе
				...
				Если НЕ СтрокаОплаты.ЭтоКорректировка Тогда
					...
					Если СтрокаОтгрузкиПоставки.Сумма <> 0 Тогда
						...
						НовСтр_Долг_Расход.ДатаПлановогоПогашения        = СтрокаОтгрузкиПоставки.ДатаПлановогоПогашения;
						НовСтр_Долг_Расход.ПорядокЗачета                 = СтрокаОтгрузкиПоставки.ПорядокОперации;
				...
				//Корректировка реализации в минус.
				Иначе
					...
					//Если это корректировка и реализация еще не оплачена полностью, то расчетным документом должна являться накладная.
					ИначеЕсли СтрокаОплаты.СвязанныйДокумент = СтрокаОтгрузкиПоставки.Регистратор
						...
						//Уменьшим план отгрузок, если есть.
						Если НЕ СтрокаОтгрузкиПоставки.ПланОтгрузокПоставокСписан Тогда
							СписатьПланОтгрузкиПоставки(ГлобальныеПеременные, ТаблицаПланОтгрузокПоставок, СтрокаОтгрузкиПоставки, ПланыОтгрузокПоставок);
						КонецЕсли;
						...
						Если СтрокаОтгрузкиПоставки.Сумма <> 0 Тогда
							...
							//ДанныеНакладной = ГлобальныеПеременные.ДанныеНакладных[СтрокаОплаты.СвязанныйДокумент];
							//ДанныеНакладной.СуммаОстаток = ДанныеНакладной.СуммаОстаток - СуммаСторно;
	...
	Если НЕ ОсновныеПараметры.ТолькоПланы Тогда
		ТаблицаРасчетовПоСрокам.ЗаполнитьЗначения(АналитикаУчетаПоПартнерам, "АналитикаУчетаПоПартнерам");
		ТаблицаРасчетовПоСрокам.ЗаполнитьЗначения(ОбъектРасчетов,            "ОбъектРасчетов");
		ТаблицаРасчетовПоСрокам.ЗаполнитьЗначения(ВалютаРасчетов,            "Валюта");
		ТаблицаРасчетовПоСрокам.Свернуть("Период, ВидДвижения, АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта, ДокументРегистратор,
			|РасчетныйДокумент, ХозяйственнаяОперация, ДатаПлановогоПогашения, ДатаВозникновения, ПорядокЗачета, ПорядокОперации,
			|ВалютаДокумента, СвязанныйДокумент, СтатьяДвиженияДенежныхСредств, КорОбъектРасчетов",
	...
	ТаблицаПланОплат.ЗаполнитьЗначения(АналитикаУчетаПоПартнерам, "АналитикаУчетаПоПартнерам");
	ТаблицаПланОплат.ЗаполнитьЗначения(ОбъектРасчетов,            "ОбъектРасчетов");
	ТаблицаПланОплат.ЗаполнитьЗначения(ВалютаРасчетов,            "Валюта");
	
	НаборЗаписейПланОплат.Загрузить(ТаблицаПланОплат);
	НаборЗаписейПланОплат.Записать();
	
	ТаблицаПланОтгрузокПоставок.ЗаполнитьЗначения(АналитикаУчетаПоПартнерам, "АналитикаУчетаПоПартнерам");
	ТаблицаПланОтгрузокПоставок.ЗаполнитьЗначения(ОбъектРасчетов,            "ОбъектРасчетов");
	ТаблицаПланОтгрузокПоставок.ЗаполнитьЗначения(ВалютаРасчетов,            "Валюта");
	
	НаборЗаписейПланОтгрузокПоставок.Загрузить(ТаблицаПланОтгрузокПоставок);
	НаборЗаписейПланОтгрузокПоставок.Записать();
	
	Если НаборЗаписейПланОплат.Количество() = 0 
		И НаборЗаписейПланОтгрузокПоставок.Количество() = 0 
		И НаборЗаписейПоСрокам.Количество() = 0 
		И НЕ ОсновныеПараметры.ТолькоПланы Тогда
		РегистраторРасчетовОбъект.Удалить();
	КонецЕсли;
	
	Если НЕ ОсновныеПараметры.ТолькоПланы И ОсновныеПараметры.ДополнительныеСвойстваПроведения <> Неопределено Тогда
		ОсновныеПараметры.ДополнительныеСвойстваПроведения.ДляПроведения.Вставить(
			?(ЭтоРасчетыСКлиентами,"РасчетыСКлиентами","РасчетыСПоставщиками")+"ПоСрокам",
			ТаблицаРасчетовПоСрокам);
	КонецЕсли;
...
Процедура РассчитатьПоИзменениям(МенеджерВременныхТаблиц, ЭтоРасчетыСКлиентами, Регистратор, ДополнительныеСвойства = Неопределено) Экспорт
	...
	|ИЗ (
	|	//В пределах дня порядок зачета документа-регистратора может быть меньше чем порядок операции
	|	ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА РасчетыСКлиентамиПоСрокам.ПорядокОперации <  РасчетыСКлиентамиПоСрокам.ПорядокЗачета
	|				ТОГДА РасчетыСКлиентамиПоСрокам.ПорядокОперации
	|			ИНАЧЕ РасчетыСКлиентамиПоСрокам.ПорядокЗачета
	|		КОНЕЦ                               КАК Порядок,
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	//Минимальный порядок операции самого документа регистратора, порядок зачета не берется, т.к. может уходить в закрытые периоды (оплата долга).
	|	ВЫБРАТЬ
	...
	|	ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА РасчетыСКлиентамиПланОплат.ПорядокОперации <  РасчетыСКлиентамиПланОплат.ПорядокЗачета
	|				ТОГДА РасчетыСКлиентамиПланОплат.ПорядокОперации
	|			ИНАЧЕ РасчетыСКлиентамиПланОплат.ПорядокЗачета
	|		КОНЕЦ,
	|		Изменения.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|		Изменения.ОбъектРасчетов            КАК ОбъектРасчетов,
	|		Изменения.ВалютаРасчетов            КАК ВалютаРасчетов 
	...
	|	ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА РасчетыСКлиентамиПланОтгрузок.ПорядокОперации <  РасчетыСКлиентамиПланОтгрузок.ПорядокЗачета
	|				ТОГДА РасчетыСКлиентамиПланОтгрузок.ПорядокОперации
	|			ИНАЧЕ РасчетыСКлиентамиПланОтгрузок.ПорядокЗачета
	|		КОНЕЦ,
	|		Изменения.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|		Изменения.ОбъектРасчетов            КАК ОбъектРасчетов,
	|		Изменения.ВалютаРасчетов            КАК ВалютаРасчетов 
	...
	|	
	|	ВЫБРАТЬ
	|		РасчетыПоСрокам.ПорядокОперации     КАК Порядок,
	|		Изменения.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|		Изменения.ОбъектРасчетов            КАК ОбъектРасчетов,
	|		Изменения.ВалютаРасчетов            КАК ВалютаРасчетов
	|	ИЗ
	...
	|				И РасчетыПоСрокам.ПорядокЗачета > Изменения.ПорядокЗачетаПоДатеПлатежа
	|				И РасчетыПоСрокам.ПорядокОперации < Изменения.ПорядокОперации
	|	ГДЕ
	...
	|					И РасчетыСКлиентамиПоСрокам.Валюта = Изменения.ВалютаРасчетов
	|					И РасчетыСКлиентамиПоСрокам.РасчетныйДокумент = Изменения.Документ
	|					И НЕ РасчетыСКлиентамиПоСрокам.КорОбъектРасчетов = НЕОПРЕДЕЛЕНО) ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
	...
	|УПОРЯДОЧИТЬ ПО
	|	ВЫБОР 
	|		КОГДА ТИПЗНАЧЕНИЯ(ВложенныйЗапрос.ОбъектРасчетов) В (&ТипыПлатежныхДокументов)
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ
	|;
...
Процедура ВыполнитьПереоценкуНаДату(МассивОрганизаций, Период, ЭтоРасчетыСКлиентами = Неопределено) Экспорт
	...
	ДетальныеЗаписи.Индексы.Добавить("Валюта");
	СтруктураПоиска = Новый Структура("ЭтоРасчетыСКлиентами, АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта");
	...
	Для Каждого СтрокаПереоценки Из ОбъектыКПереоценке Цикл
		...
		ДанныеРегистратора = РегистраторРасчетов(СтрокаПереоценки.ОбъектРасчетов,
													СтрокаПереоценки.АналитикаУчетаПоПартнерам,
													СтрокаПереоценки.Валюта,
													СтрокаПереоценки.ЭтоРасчетыСКлиентами);
		Регистратор = ДанныеРегистратора.Ссылка;
		...
		Для Каждого СтрокаДетальныхЗаписей Из СтрокиДетальныхЗаписей Цикл
			...
			Если СтрокаДетальныхЗаписей.ДолгРеглОстаток < СтрокаДетальныхЗаписей.ДолгРеглОстатокРасчетный 
				...
				НовСтр.Регистратор           = Регистратор;
...
// 			* ТолькоПланы - Булево - Истина, если необходимо заполнить только плановые оплаты и плановые отгрузки/поставки.
...
//Возвращает ссылку на служебный документ РегистраторРасчетов по переданным параметрам, а так же дату документа.
//
// Параметры:
...
//		* Дата - Дата - дата найденного служебного документа.
//
Функция РегистраторРасчетов(ОбъектРасчетов, АналитикаУчетаПоПартнерам, Валюта, ЭтоРасчетыСКлиентами, Организация = Неопределено) Экспорт
	...
	ТипРасчетов = ?(ЭтоРасчетыСКлиентами, Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом, Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком);
...
Процедура ЗаполнитьРегистрыПриВключенииНовойАрхитектуры(Параметры, УникальныйИдентификатор) Экспорт
	...
	Попытка
		...
		|	МИНИМУМ(РасчетыСКлиентами.Период) КАК ДатаВозникновения,
		...
		|	ВЫРАЗИТЬ(РасчетыСКлиентамиПоДокументам.Регистратор КАК Документ.КорректировкаРеализации).ДокументОснование КАК СвязанныйДокумент,
		|
		...
		|		ЗачетыОплат.СвязанныйДокумент                                    КАК СвязанныйДокумент,
		...
		|		РасчетыПоДокументам.СвязанныйДокумент             КАК СвязанныйДокумент,
		...
		ТаблицаАналитик = ТаблицаДвижений.Скопировать(,"АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта");
		ТаблицаАналитик.Свернуть("АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта");
		
		Для Каждого СтрокаАналитики Из ТаблицаАналитик Цикл
			НаборЗаписей = РегистрыНакопления.РасчетыСКлиентамиПоСрокам.СоздатьНаборЗаписей();
			Регистратор = РегистраторРасчетов(СтрокаАналитики.ОбъектРасчетов, СтрокаАналитики.АналитикаУчетаПоПартнерам, СтрокаАналитики.Валюта, Истина).Ссылка;
			НаборЗаписей.Отбор.Регистратор.Установить(Регистратор);
			НаборЗаписей.Загрузить(ТаблицаДвижений.Скопировать(Новый Структура("АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта",
		...
		|	МИНИМУМ(РасчетыСПоставщиками.Период)          КАК ДатаВозникновения,
		...
		|		ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		|	КОНЕЦ                                                           КАК ВидДвижения,
		|	ВЫРАЗИТЬ(РасчетыСПоставщикамиПоДокументам.Регистратор КАК Документ.КорректировкаПриобретения).ДокументОснование КАК СвязанныйДокумент,
		|
		...
		Для Каждого СтрокаАналитики Из ТаблицаАналитик Цикл
			НаборЗаписей = РегистрыНакопления.РасчетыСПоставщикамиПоСрокам.СоздатьНаборЗаписей();
			Регистратор = РегистраторРасчетов(СтрокаАналитики.ОбъектРасчетов, СтрокаАналитики.АналитикаУчетаПоПартнерам, СтрокаАналитики.Валюта, Ложь).Ссылка;
			НаборЗаписей.Отбор.Регистратор.Установить(Регистратор);
			НаборЗаписей.Загрузить(ТаблицаДвижений.Скопировать(Новый Структура("АналитикаУчетаПоПартнерам, ОбъектРасчетов, Валюта",
				СтрокаАналитики.АналитикаУчетаПоПартнерам, СтрокаАналитики.ОбъектРасчетов, СтрокаАналитики.Валюта)));
			НаборЗаписей.ОбменДанными.Загрузка = ИСТИНА;
		...
		Для Каждого Стр из ТаблицаОбъектов Цикл
			...
			ОсновныеПараметры.ТолькоПланы = Истина;
			ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры);
		КонецЦикла;
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Включение онлайн взаиморасчетов с партнерами.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОчиститьРегистрыПриВыключенииНовойАрхитектуры(Параметры = Неопределено, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Выключение онлайн взаиморасчетов с партнерами.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ЗаполнитьПоВсемРасчетам(ТолькоПланы = Ложь) Экспорт
	...
	Для Каждого Стр из ТаблицаОбъектов Цикл
		ОсновныеПараметры = СтруктураПараметровЗаполненияВзаиморасчетов();
		ЗаполнитьЗначенияСвойств(ОсновныеПараметры,Стр);
		ОсновныеПараметры.ТолькоПланы = ТолькоПланы;
		ЗаполнитьОперативныеВзаиморасчеты(ОсновныеПараметры);
	КонецЦикла;
...
Процедура СформироватьВТДанныхРегистровДоНачалаРасчетов(Запрос, ЭтоРасчетыСКлиентами)
	
	//Данные расчетов до начала пересчета.
	Запрос.Текст = "
	|ВЫБРАТЬ *
	|ПОМЕСТИТЬ ВТРасчетыПоСрокамДо
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК РасчетыПоСрокам
	|ГДЕ
	|	РасчетыПоСрокам.ОбъектРасчетов              = &ОбъектРасчетов
	...
	|ПОМЕСТИТЬ ВТПорядкиОперацийОтгрузокПоставок
	|ИЗ
	|	ВТРасчетыПоСрокамДо КАК РасчетыПоСрокам
	|ГДЕ
	|	РасчетыПоСрокам.ОбъектРасчетов              = &ОбъектРасчетов
	|	И РасчетыПоСрокам.АналитикаУчетаПоПартнерам = &АналитикаУчетаПоПартнерам
	|	И РасчетыПоСрокам.Валюта                    = &ВалютаРасчетов
	...
	|	РасчетыПоСрокамДо.СвязанныйДокумент         КАК СвязанныйДокумент,
	...
	|	
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.Предоплата ИНАЧЕ РасчетыПоСрокамДо.Предоплата КОНЕЦ)         КАК Предоплата,
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ПредоплатаРегл ИНАЧЕ РасчетыПоСрокамДо.ПредоплатаРегл КОНЕЦ) КАК ПредоплатаРегл,
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ПредоплатаУпр ИНАЧЕ РасчетыПоСрокамДо.ПредоплатаУпр КОНЕЦ)   КАК ПредоплатаУпр,
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.Долг ИНАЧЕ РасчетыПоСрокамДо.Долг КОНЕЦ)                     КАК Долг,
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ДолгРегл ИНАЧЕ РасчетыПоСрокамДо.ДолгРегл КОНЕЦ)             КАК ДолгРегл,
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ДолгУпр ИНАЧЕ РасчетыПоСрокамДо.ДолгУпр КОНЕЦ)               КАК ДолгУпр
	|	
	|ПОМЕСТИТЬ ВТРасчетыПоСрокамОстатки
	|ИЗ
	|	ВТРасчетыПоСрокамДо КАК РасчетыПоСрокамДо
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПорядкиОперацийОтгрузокПоставок КАК ПорядкиОпераций
	...
	|	РасчетыПоСрокамДо.СвязанныйДокумент,
	|	РасчетыПоСрокамДо.ПорядокЗачета
	|ИМЕЮЩИЕ
	|	СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.Предоплата ИНАЧЕ РасчетыПоСрокамДо.Предоплата КОНЕЦ) > 0 
	|	ИЛИ СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ПредоплатаРегл ИНАЧЕ РасчетыПоСрокамДо.ПредоплатаРегл КОНЕЦ) > 0 
	|	ИЛИ СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ПредоплатаУпр ИНАЧЕ РасчетыПоСрокамДо.ПредоплатаУпр КОНЕЦ) > 0 
	|	ИЛИ СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.Долг ИНАЧЕ РасчетыПоСрокамДо.Долг КОНЕЦ) > 0
	|	ИЛИ СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ДолгРегл ИНАЧЕ РасчетыПоСрокамДо.ДолгРегл КОНЕЦ) > 0
	|	ИЛИ СУММА(ВЫБОР КОГДА РасчетыПоСрокамДо.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -РасчетыПоСрокамДо.ДолгУпр ИНАЧЕ РасчетыПоСрокамДо.ДолгУпр КОНЕЦ) > 0
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
	|;
	//Планы оплат.
	|ВЫБРАТЬ *
	|ПОМЕСТИТЬ ВТРасчетыПланОплатДо
	|ИЗ
	...
	|ИЗ
	|	ВТРасчетыПланОплатДо КАК РасчетыПланОплатДо
	|СГРУППИРОВАТЬ ПО
	...
	//Планы отгрузок.
	|ВЫБРАТЬ *
	|ПОМЕСТИТЬ ВТРасчетыПланОтгрузокПоставокДо
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПланОтгрузок КАК РасчетыПланОтгрузок
	|ГДЕ
	|	РасчетыПланОтгрузок.ОбъектРасчетов              = &ОбъектРасчетов
	|	И РасчетыПланОтгрузок.АналитикаУчетаПоПартнерам = &АналитикаУчетаПоПартнерам
	|	И РасчетыПланОтгрузок.Валюта                    = &ВалютаРасчетов
	|	И РасчетыПланОтгрузок.ПорядокОперации           < &Порядок
	|	И НЕ РасчетыПланОтгрузок.ДокументРегистратор    = &Регистратор
	|;
	|ВЫБРАТЬ
	...
	|ИЗ
	|	ВТРасчетыПланОтгрузокПоставокДо КАК РасчетыПланОтгрузокДо
	|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьПланыОплат(Запрос, Порядок, ЭтоРасчетыСКлиентами)
...
Функция ПолучитьПланыОтгрузокПоставок(Запрос, Порядок, ЭтоРасчетыСКлиентами)
...
Функция ПолучитьОплаты(Запрос, Порядок)
	...
	|	//РасчетыСКлиентами
	|	ВЫБРАТЬ
	|	//Для учета границы начала пересчета, в остатки должно попасть то, что раньше проведено.
	|		Расчеты.Период                                          КАК Период,
	...
	|		Расчеты.СвязанныйДокумент                               КАК СвязанныйДокумент,
	|		ВЫБОР
	|			КОГДА Расчеты.КорОбъектРасчетов <> Неопределено
	...
	|		Расчеты.Регистратор ССЫЛКА Документ.КорректировкаРеализации       КАК ЭтоКорректировка,
	
	|		СУММА(ВЫБОР	КОГДА Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|					ТОГДА -Расчеты.Сумма
	|				ИНАЧЕ Расчеты.Сумма
	|		КОНЕЦ)                                                            КАК Сумма,
	|		СУММА(ВЫБОР	КОГДА Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|		Расчеты.ХозяйственнаяОперация                                 КАК ХозяйственнаяОперация,
	|		Расчеты.КорОбъектРасчетов                                     КАК КорОбъектРасчетов,
	|		Расчеты.Регистратор ССЫЛКА Документ.КорректировкаПриобретения КАК ЭтоКорректировка,
	|		СУММА(ВЫБОР	КОГДА Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|					ТОГДА -Расчеты.Сумма
	|				ИНАЧЕ Расчеты.Сумма
	|		КОНЕЦ)                                                  КАК Сумма,
	|		СУММА(ВЫБОР	КОГДА Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	...
	|
	|	ТИПЗНАЧЕНИЯ(ОстаткиОплат.РасчетныйДокумент) = ТИП(Документ.КорректировкаРеализации)
	|	ИЛИ ТИПЗНАЧЕНИЯ(ОстаткиОплат.РасчетныйДокумент) = ТИП(Документ.КорректировкаПриобретения) КАК ЭтоКорректировка,
	...
	|		ИЛИ &ВалютаРасчетов =&ВалютаРегл
	|		ИЛИ ОстаткиОплат.РасчетныйДокумент ССЫЛКА Документ.ВзаимозачетЗадолженности КАК РучнойКурсРегл,
	|	ОстаткиОплат.ВалютаДокумента=&ВалютаУпр 
	|		ИЛИ &ВалютаРасчетов =&ВалютаУпр
	|		ИЛИ ОстаткиОплат.РасчетныйДокумент ССЫЛКА Документ.ВзаимозачетЗадолженности КАК РучнойКурсУпр,
	...
	|	ЕСТЬNULL(ДвиженияКорОбъекта.ПредоплатаРегл, РасчетыОплаты.СуммаРегл) КАК СуммаРегл,
	|	ЕСТЬNULL(ДвиженияКорОбъекта.ПредоплатаУпр, РасчетыОплаты.СуммаУпр) КАК СуммаУпр,
	|
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК ДвиженияКорОбъекта
	|			ПО &АналитикаУчетаПоПартнерам            = ДвиженияКорОбъекта.АналитикаУчетаПоПартнерам
	|				И НЕ РасчетыОплаты.КорОбъектРасчетов = Неопределено
	|				И РасчетыОплаты.Регистратор          = ДвиженияКорОбъекта.ДокументРегистратор
	|				И РасчетыОплаты.КорОбъектРасчетов    = ДвиженияКорОбъекта.ОбъектРасчетов
	|УПОРЯДОЧИТЬ ПО
	|	ПорядокЗачета ВОЗР";
...
Функция ПолучитьОтгрузкиПоставки(Запрос, Порядок)
	
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Расчеты.ДатаРегистратора                            КАК Период,
	|	НАЧАЛОПЕРИОДА(ВЫБОР
	...
	|		ИЛИ &ВалютаРасчетов =&ВалютаРегл
	|		ИЛИ РасчетыОтгрузкиПоставки.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности КАК РучнойКурсРегл,
	|	РасчетыОтгрузкиПоставки.ВалютаДокумента = &ВалютаУпр 
	|		ИЛИ &ВалютаРасчетов =&ВалютаУпр
	|		ИЛИ РасчетыОтгрузкиПоставки.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности КАК РучнойКурсУпр,
	|	ВЫБОР
	...
	|		ИЛИ &ВалютаРасчетов =&ВалютаРегл
	|		ИЛИ РасчетыПоСрокамОстатки.РасчетныйДокумент ССЫЛКА Документ.ВзаимозачетЗадолженности КАК РучнойКурсРегл,
	|	РасчетыПоСрокамОстатки.ВалютаДокумента = &ВалютаУпр 
	|		ИЛИ &ВалютаРасчетов =&ВалютаУпр
	|		ИЛИ РасчетыПоСрокамОстатки.РасчетныйДокумент ССЫЛКА Документ.ВзаимозачетЗадолженности КАК РучнойКурсУпр,
	|	ВЫБОР 
	...
	Если Порядок <> "" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,"//УсловиеПорядок","И Расчеты.ПорядокОперации          >= &Порядок");
		Запрос.УстановитьПараметр("ДатаПереоценки",НачалоДня(Дата(Лев(Порядок,8))-1));
	Иначе
...
Функция ПолучитьДанныеНакладных(Запрос)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРасчетыПоСрокамДо КАК ВТРасчетыПоСрокамДо
...
Функция ХозяйственныеОперацииНеОтгрузка() Экспорт
	...
	//В версиях 2.4.7 и выше не нужны
