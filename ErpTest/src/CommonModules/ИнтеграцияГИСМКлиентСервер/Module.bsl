...
Функция ПредставлениеПодписи(СертификатКриптографии, Сообщение, ВыводитьГиперссылки, СтруктураЦветаСтиля) Экспорт
	...
	Если СертификатКриптографии = Неопределено Тогда
		Подпись = Новый ФорматированнаяСтрока(НСтр("ru = 'Не подписано'"),,СтруктураЦветаСтиля.ЦветТекстаТребуетВниманияГИСМ);
		Возврат Подпись;
	КонецЕсли;
...
Функция ОтправитьДляОбработки(ТелоЗапроса, Порт, АдресРесурса, SOAPAction)
	...
	Исключение
		...
		#Если НЕ ТонкийКлиент И НЕ МобильныйКлиент Тогда
