	...
	Для каждого Реквизит Из Реквизиты Цикл
			...
			Для каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
				...
				Для каждого РеквизитТаблицы Из Реквизит.Значение Цикл
					Если (Заполнение) И (РеквизитТаблицы.Значение <> 2) И (РеквизитТаблицы.Значение > 0) 
						...
						Результат = Ложь;
						Ошибки = Ошибки + "Таблица <" + уатПолучитьСинонимРеквизита(ЭтотОбъектМетаданные, , Реквизит.Ключ) + 
							"> значение колонки <" + уатПолучитьСинонимРеквизита(ЭтотОбъектМетаданные, РеквизитТаблицы.Ключ, 
				...
				Если Уникальность И СтруктураОтбора.Количество() > 0 
					...
					Если НайденныеСтроки.Количество() > 1 Тогда
						...
						КонецЦикла;
						Ошибки = Ошибки + "Таблица <" + уатПолучитьСинонимРеквизита(ЭтотОбъектМетаданные, , Реквизит.Ключ) +
							"> значения колонки <" + уатПолучитьСинонимРеквизита(ЭтотОбъектМетаданные, РеквизитТаблицы.Ключ, 
		...
		Иначе // обычный реквизит справочника / ПВХ (не табличная часть)
			...
			Если Заполнение 
				...
				Результат = Ложь;
				Ошибки = Ошибки + "Реквизит """ + уатПолучитьСинонимРеквизита(ЭтотОбъектМетаданные,Реквизит.Ключ) + 
...
Процедура уатПроверкаПраваДоступаКДокументам(Объект, Отказ, ЭтаФорма = Неопределено, 
...
Процедура уатПроверкаПраваДоступаКСправочникам(Объект, Отказ, ЭтаФорма = Неопределено, ЗначенияПрав = Неопределено) Экспорт
	...
	ИначеЕсли Результат = Перечисления.уатВидыПравДляСправочников.РедактированиеПоГруппам Тогда
		Если ЗначенияПрав = Неопределено Тогда
			ЗначенияПрав = уатПолучитьПраваИНастройкиПользователя(ПользователиКлиентСервер.ТекущийПользователь());
		КонецЕсли;
		...
		Иначе //если не в корне, то определяем по дереву доступа
			...
			Если (ЗначениеДоступа = Неопределено) ИЛИ (НЕ ЗначениеДоступа) Тогда
				...
				КонецЕсли;
				Сообщить("Нет прав на редактирование " + стрРодитель + " объектов вида "  + Объект.Метаданные().Представление(),
