...
Функция ЭтоНомерКарты(НомерКарты) Экспорт
	Если Не ЗначениеЗаполнено(НомерКарты) Тогда
		Возврат Ложь; // Пустая строка - не номер карты.
	КонецЕсли;
	
	Длина = СтрДлина(НомерКарты);
	ДлинаЧисла = 0;
	Для НомерСимвола = 1 По Длина Цикл
		КодСимвола = КодСимвола(НомерКарты, НомерСимвола);
		Если КодСимвола > 47 И КодСимвола < 58 Тогда // Число.
			ДлинаЧисла = ДлинаЧисла + 1;
		Иначе
			Возврат Ложь; // Посторонние символы.
		КонецЕсли;
	КонецЦикла;
	
	Если ДлинаЧисла = 16 Или ДлинаЧисла = 19 Тогда
		Возврат Истина;
	КонецЕсли;
	Возврат Ложь;
КонецФункции
...
Функция ПредставлениеДней(Дней)
