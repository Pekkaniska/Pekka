...
//
Процедура СинхронизироватьСБанком(Оповещение, НастройкаОбмена, ВыводитьОкноОжидания = Ложь) Экспорт
	...
	Если РеквизитыНастройкиОбмена.ПрограммаБанка = СбербанкОнлайн Тогда
		Если РеквизитыНастройкиОбмена.ИспользуетсяКриптография Тогда
			...
			УстановитьСоединениеИАутентифицироватьсяНаСервереСбербанк(Оповещение, НастройкаОбмена);
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен")
	...
	ИначеЕсли ПарольКСертификатуПолучен(НастройкаОбмена, ДанныеАвторизации)
...
//
Процедура АутентификацияПоСертификатуСбербанк(Оповещение, ИдентификаторВК, ДанныеСертификата, НастройкаОбмена = Неопределено) Экспорт
...
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//
Процедура АутентифицироватьсяПоСертификатуСбербанк(Оповещение, ИдентификаторВК, НастройкаОбмена)
	...
	ОпределитьСертификатПодписиСбербанк(ОповещениеПослеОпределенияСертификата, ИдентификаторВК, НастройкаОбмена);
...
//    ЭтоТест - Булево - если Истина, то будет недоступно сохранение пароля.
//
Процедура АутентифицироватьсяНаТокенеСбербанка(ОповещениеОВыполнении, ИдентификаторВК, НастройкаОбмена = Неопределено, ПринудительнаяАутентификация = Ложь, ЭтоТест = Ложь) Экспорт
...
//
Процедура ПроверитьПодписьНаТокенеСбербанк(Оповещение, ПодписанныеДанные, Подпись, Сертификат, ИдентификаторВК) Экспорт
...
//
Процедура ПодписатьДанныеСбербанк(ОписаниеОповещения, ИдентификаторВК, СтрокаПодписи, ИдентификаторСертификата, Представление) Экспорт
...
//
Процедура ОпределитьСертификатПодписиСбербанк(Оповещение, ИдентификаторВК, НастройкаОбмена, ОграничениеПоСертификатам = Неопределено) Экспорт
...
//
Процедура ВыполнитьБазовуюАутентификациюСбербанк(Оповещение, ИмяВнешнегоМодуля, КлючСессии, НастройкаОбмена = Неопределено, ЛогинПароль = Неопределено, ТекстПояснения = "") Экспорт
...
Процедура ОтправитьЧерезДополнительнуюОбработку(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Число") Тогда
		Параметры.ИтогКоличествоОтправленных = Параметры.ИтогКоличествоОтправленных + Результат;
	КонецЕсли;
	...
	Иначе
		Для Каждого КлючИЗначение Из ДанныеДляОтправки Цикл
			...
			СтруктураОтправки.Вставить("ИтогКоличествоОтправленных", 0);
...
// Команда создания письма в банк.
Процедура СоздатьПисьмоВБанкНаОсновании(ПараметрКоманды, ПараметрыВыполненияКоманды) Экспорт
	...
	Параметры = Новый Структура;
	Параметры.Вставить("Основание", ПараметрКоманды);
	ОткрытьФорму("Документ.ПисьмоОбменСБанками.Форма.ПисьмоВБанк", Параметры);
...
Процедура ПровестиТестНастройки(Обработчик, НастройкаОбмена) Экспорт
	...
	Если ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.АсинхронныйОбмен") Тогда
		...
		Иначе
			ПослеПолученияОтпечатковПроверитьСвязь(Неопределено, Параметры);
		КонецЕсли;
...
//                      - ИтогКоличествоОтправленных - Число - количество отправленных пакетов.
//    * ДополнительныеПараметры - значение, которое было указано при создании объекта ОписаниеОповещения
...
Процедура ОтправитьДокументыЧерезВК(Оповещение, ДанныеОтправки, ПолучитьНовыеДокументы = Истина) Экспорт
	...
	ДополнительныеПараметры.Вставить("ИтогКоличествоОтправленных", 0);
...
//                            * ИтогКоличествоПолученных - Число - количество полученных пакетов.
//     * ДополнительныеПараметры - значение, которое было указано при создании объекта ОписаниеОповещения.
//  ПодключаемыйМодуль - AddIn - внешняя компонента банка.
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - настройка обмена.
//
Процедура ПолучитьНовыеДокументыВК(Оповещение, ПодключаемыйМодуль, НастройкаОбмена) Экспорт
	...
	ДополнительныеПараметры.Вставить("ПодключаемыйМодуль", ПодключаемыйМодуль);
	ДополнительныеПараметры.Вставить("НастройкаОбмена", НастройкаОбмена);
	ДополнительныеПараметры.Вставить("ИтогКоличествоПолученных", 0);
...
Процедура ОтправитьДанныеПослеБазовойАутентификацииСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успех Тогда
		...
		Если РезультатЗадания.Статус = "Выполняется" Тогда
			...
			ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
		Иначе
			Результат = Новый Структура("Успех, РезультатОперации", Истина, РезультатЗадания);
...
//  ПодключаемыйМодуль - AddIn - подключаемый модуль банка.
//
...
Процедура ПроверитьВерсиюВКСбербанк(Оповещение, ИмяВнешнегоМодуля)
	...
	Если ИнформацияОВК = Неопределено
		ИЛИ НЕ ПоддерживаетсяВерсияКомпонентыСбербанк(ИнформацияОВК.Версия) Тогда
		ВыполнитьОбработкуОповещения(Оповещение, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ПодобратьСертификатПослеПолученияСпискаСТокенаСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораСертификатаСбербанк", ЭтотОбъект, Параметры);
...
Процедура ПослеОтправкиПлатежногоДокументаСбербанк(Тикет, Параметры) Экспорт
	...
	ОбменСБанкамиСлужебныйВызовСервера.ПослеОтправкиПлатежногоДокументаСбербанк(
		Параметры.НастройкаОбмена, Параметры.СообщениеОбмена, Тикет);
	
	Параметры.ИтогКоличествоОтправленных = Параметры.ИтогКоличествоОтправленных + 1;
...
Процедура ПолучитьРезультатыОбработкиЗапросовСбербанк(Знач НастройкаОбмена, Параметры)
	...
	МассивИдентификаторов = ОбменСБанкамиСлужебныйВызовСервера.ТикетыЗапросовСтатусовСбербанк(
...
Процедура ПослеВыбораСертификатаСбербанк(ВыбраннаяСтрока, Параметры) Экспорт
	...
	Иначе
		...
		ДанныеСертификата = Параметры.СоответствиеСертификатовСбербанка.Получить(ВыбраннаяСтрока.Значение);
		СтруктураВозврата.Вставить("СертификатСсылка", ДанныеСертификата.СертификатСсылка);
		СтруктураВозврата.Вставить("ДвоичныеДанныеСертификата", ДанныеСертификата.ДвоичныеДанныеСертификата);
...
Процедура ПослеСинхронизацииСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе // выполнено
		...
		Если РезультатОперации.ТребуетсяАутентификация Тогда
			...
			КонецЕсли
		Иначе
			ПоказатьРезультатСинхронизации(0, РезультатОперации.ИтогКоличествоПолученных);
...
Процедура ОтправитьДокументыВСбербанкБазоваяАутентификация(Оповещение, ДанныеДляОтправки, ДополнительныеПараметры = Неопределено)
	...
	СтруктураВозврата = Новый Структура("ИтогКоличествоОтправленных, ИтогКоличествоПолученных");
	ЗаполнитьЗначенияСвойств(СтруктураВозврата, ДополнительныеПараметры);
	СтруктураВозврата.Вставить("ИтогКолПодготовленных", СтруктураВозврата.ИтогКоличествоОтправленных);
...
Процедура ОбработатьРезультатОтправкиДокументовПоНастройкеСбербанк(РезультатОтправки, ДополнительныеПараметры)
	...
	ДополнительныеПараметры.ИтогКоличествоОтправленных =
		ДополнительныеПараметры.ИтогКоличествоОтправленных + ДополнительныеПараметры.МассивСообщенийОбмена.Количество();
	ДополнительныеПараметры.ИтогКоличествоПолученных =
		ДополнительныеПараметры.ИтогКоличествоПолученных + РезультатОтправки.ИтогКоличествоПолученных;
...
Процедура ПослеОтправкиДанныхВСбербанк(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗадания = Неопределено Тогда // задание было отменено
		...
		Если ИспользоватьЖурналирование Тогда
			СохранитьЖурналВФайл(ДополнительныеПараметры.НастройкаОбмена, КаталогДляЖурналирования);
		КонецЕсли;
		Результат = Новый Структура("Успех", Ложь);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.Оповещение, Результат);
		Возврат;
	...
	Если ДополнительныеПараметры.ИспользоватьЖурналирование Тогда
		СохранитьЖурналВФайл(ДополнительныеПараметры.НастройкаОбмена, ДополнительныеПараметры.КаталогДляЖурналирования);
	КонецЕсли;
	...
	Иначе // выполнено
		...
		Если РезультатОперации.ТребуетсяАутентификация Тогда
			...
			Иначе
				...
				ВыполнитьБазовуюАутентификациюСбербанк(Обработчик, ДополнительныеПараметры.ИмяВнешнегоМодуля,
					ДополнительныеПараметры.НастройкаОбмена, ДополнительныеПараметры.НастройкаОбмена);
			КонецЕсли
...
Процедура ПроверитьПодписьПослеПодключенияВКСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Результат.ПодключаемыйМодуль.НачатьВызовПроверитьПодписьДанныхЧерезVPNKeyTLS(Оповещение,
		ДополнительныеПараметры.ПодписанныеДанные, ДополнительныеПараметры.Подпись, ДополнительныеПараметры.Сертификат);
...
Процедура ОтправитьСообщениеВБанкСАутентификациейЛогинПароль(ПараметрыАутентификации, Параметры) Экспорт
	...
	Иначе
		...
		ПодготовитьИОтправитьПЭД(
			МассивСообщенийОбменаКОтправке, СоотвНастроекОбменаИСтруктурСертификатов, , ОписаниеОповещения);
	КонецЕсли;
...
Процедура ОтправитьСообщениеВБанкПослеПолученияМаркера(Маркер, Параметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"НачатьОтправкуСообщенийВБанкСАвторизациейЛогинПароль", ЭтотОбъект, Параметры);
	ПодготовитьИОтправитьПЭД(
		МассивСообщенийОбменаКОтправке, СоотвНастроекОбменаИСтруктурСертификатов, Параметры, ОписаниеОповещения);
	
КонецПроцедуры

Процедура РасшифроватьМаркерБанка(Результат, ДополнительныеПараметры) Экспорт

	Если НЕ ДополнительныеПараметры.Свойство("СоответствиеВозврата") Тогда
		ДополнительныеПараметры.Вставить("СоответствиеВозврата", Новый Соответствие);
	КонецЕсли;
	
	СоответствиеВозврата = ДополнительныеПараметры.СоответствиеВозврата;
		...
		Для Каждого Элемент Из СоответствиеНастроекОбменаИСертификатов Цикл
			...
			Для Каждого Сертификат Из Сертификаты Цикл
				...
				Если ПараметрыСтруктура.Свойство("ИдентификаторСессии", Маркер) Тогда
					СоответствиеВозврата.Вставить(НастройкаОбмена, ПараметрыСтруктура);
					Прервать;
				Иначе
...
Процедура НачатьОтправкуСообщенийВБанкСАвторизациейЛогинПароль(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		КонецЕсли;
		Если Результат.Свойство("КоличествоОтправленных", КоличествоОтправленных) И ТипЗнч(КоличествоОтправленных) = Тип("Число") Тогда
			Параметры.ИтогКоличествоОтправленных = Параметры.ИтогКоличествоОтправленных + КоличествоОтправленных;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОткрытьФормуЗапросаВыписки(ПараметрыАутентификации, Параметры) Экспорт
	...
	ВладелецФормы = Параметры.Владелец;
	ПараметрыФормы = Новый Структура("ДанныеСертификата, ДатаНачала, ДатаОкончания, МассивСообщенийОбмена,
		|НастройкаОбмена, НомерСчета, Пароль, Логин, СообщениеОбмена, ГотовыеВыписки,
		|ПринудительноеПолучениеВыписки");
	ЗаполнитьЗначенияСвойств(ПараметрыФормы, Параметры);
	ПараметрыФормы.Вставить("ВидОперации", "ПолучениеВыписки");
...
Процедура ОтправитьДокументыВБанк(ДанныеАутентификации, Параметры) Экспорт
	...
	Если НЕ Параметры.ТребуетсяАвторизация ИЛИ ЗначениеЗаполнено(ДанныеАутентификации) Тогда
		РеквизитыНастройкиОбмена = Новый Структура(
			"АдресСервера, ИдентификаторОрганизации, ПрограммаБанка, ИмяВнешнегоМодуля, ВерсияФормата, ДополнительнаяОбработка");
		ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(
			Параметры.НастройкаОбмена, РеквизитыНастройкиОбмена);
		...
		Иначе // АльфаБанкОнлайн
			СтруктураВозврата = Новый Структура;
			ОбменСБанкамиСлужебныйВызовСервера.ОтправитьЭДВБанк(Параметры.НастройкаОбмена, Неопределено, СтруктураВозврата);
...
Процедура ОтправитьИПолучитьДокументыВБанкПослеРасшифровкиМаркера(Результат, Параметры) Экспорт
	...
	ОтправитьИПолучитьДокументыВБанк(ДанныеСтрокой, Параметры)
...
Процедура ОтправитьИПолучитьДокументыВБанк(ИдентификаторСессии, Параметры)
	...
	Результат = ОбменСБанкамиСлужебныйВызовСервера.ЗапускЗаданияОтправкиДокументовАсинхронныйОбмен(
		Параметры.НастройкаОбмена, ИдентификаторСессии);
	...
	Если Результат.Статус = "Выполняется" Тогда
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(Результат, Оповещение, ПараметрыОжидания);
	Иначе
		ПослеОтправкиДокументовВБанкАсинхронныйОбмен(Результат, Параметры)
	КонецЕсли;
...
Процедура ВыполнитьДействияПослеОтправки(Параметры)
	...
	Если Параметры.ИтогКолНовыхЭД + Параметры.ИтогКолПодписанных + Параметры.ИтогКолПодготовленных + Параметры.ИтогКоличествоОтправленных Тогда
		ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, Параметры.ИтогКолНовыхЭД, Параметры.ИтогКолПодписанных,
			Параметры.ИтогКолПодготовленных, Параметры.ИтогКоличествоОтправленных);
	КонецЕсли;
...
//
Процедура ПодготовитьИОтправитьПЭД(СообщенияОбмена, СоответствиеНастроекОбменаИПараметровСертификатов = Неопределено, Параметры = Неопределено, ОбработчикПослеОтправкиПЭД = Неопределено)
...
Процедура ПослеСинхронизацииПоОчереднойНастройкеОбмена(Результат, ДополнительныеПараметры)
	
	Если Не Результат Тогда
		ДополнительныеПараметры.ЕстьОшибка = Истина;
	КонецЕсли;
	
	Если ДополнительныеПараметры.МассивНастроекОбмена.Количество Тогда
		ТекущаяНастройкаОбмена = ДополнительныеПараметры.МассивНастроекОбмена.Получить(0);
		ДополнительныеПараметры.МассивНастроекОбмена.Удалить(0);
		ОповещениеПослеСинхронизации = Новый ОписаниеОповещения(
			"ПослеСинхронизацииПоОчереднойНастройкеОбмена", ЭтотОбъект, ДополнительныеПараметры);
		СинхронизироватьСБанком(ОповещениеПослеСинхронизации, ТекущаяНастройкаОбмена);
	Иначе
		ВыполнитьОбработкуОповещения(
...
Процедура ПродолжитьОтправкуСообщенийОбменаПослеРасшифровкиМаркера(Результат, Параметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершитьОтправкуСообщенийОбмена", ЭтотОбъект, Параметры);
	ПодготовитьИОтправитьПЭД(Параметры.МассивКОтправке, СоотвНастроекОбменаИСтруктурСертификатов, Параметры, ОписаниеОповещения);
...
Процедура ВыполнитьДействияПослеОтправкиПЭДЗавершить(Результат, ДопПараметры)
	...
	Если ТипЗнч(РезультатОтправкиПЭД) = Тип("Структура") Тогда
		...
		Параметры.ИтогКоличествоОтправленных = Параметры.ИтогКоличествоОтправленных + РезультатОтправкиПЭД.КоличествоОтправленных;
		Если РезультатОтправкиПЭД.Свойство("ДанныеДляОтправкиЧерезДопОбработку")
			И РезультатОтправкиПЭД.ДанныеДляОтправкиЧерезДопОбработку.Количество() > 0 Тогда
...
//
Процедура ВывестиИнформациюОбОбработанныхЭД(ТекстЗаголовка, КолСформированных, КолПодписанных, КолПодготовленных, КоличествоОтправленных = 0)
	
	Если КолПодготовленных + КоличествоОтправленных > 0 Тогда
		ДопТекст = ?(КоличествоОтправленных > 0, НСтр("ru = 'отправлено'"), НСтр("ru = 'подготовлено к отправке'"));
		Количество = ?(КоличествоОтправленных > 0, КоличествоОтправленных, КолПодготовленных);
		Если КолПодписанных > 0 Тогда
...
Процедура ПослеОбработкиДокументов(РезультатЗадания, Параметры) Экспорт
	...
	Иначе // выполнено
		...
		Если Результат <> Неопределено Тогда
			...
			Если НЕ Результат.Свойство("КоличествоОтправленных", ИтогКоличествоОтправленных) Тогда
				ИтогКоличествоОтправленных = 0;
			КонецЕсли;
			...
			Параметры.Вставить("ИтогКоличествоОтправленных", ИтогКоличествоОтправленных);
...
Процедура ПослеОправкиПакетовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ИтогКоличествоОтправленных", Результат.ИтогКоличествоОтправленных);
...
Процедура ПродолжитьСинхронизациюПослеПолученияНовыхДокументовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат.Результат) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Результат.Результат);
	КонецЕсли;
	ПоказатьРезультатСинхронизации(ДополнительныеПараметры.ИтогКоличествоОтправленных, Результат.ИтогКоличествоПолученных);
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеСинхронизации, Истина);
...
Процедура ЗапуститьПроцессПолученияВыпискиЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.ИтогКоличествоОтправленных = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьРекурсивноСообщенияОбменаПоНастройкамЧерезВК(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ДанныеОтправки.Количество() = 0 Тогда // все настройки обмена обработаны
		Результат = Новый Структура("ИтогКоличествоОтправленных, ИтогКолПодготовленных", 0, 0);
...
Процедура ПродолжитьОтправкуПакетовПоНастройкамОбменаЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.ИтогКоличествоОтправленных = ДополнительныеПараметры.ИтогКоличествоОтправленных + Результат.ИтогКоличествоОтправленных;
	Если ТипЗнч(Результат.Результат) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Результат.Результат);
	КонецЕсли;
...
Процедура УстановитьСоединениеПослеПодключенияВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") ИЛИ Результат = Неопределено Тогда
		...
		ДанныеВозврата.Вставить("ИтогКоличествоОтправленных", ДополнительныеПараметры.ИтогКоличествоОтправленных);
...
Процедура ОтправитьПакетыПослеУстановкиСоединенияЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") ИЛИ Результат = Неопределено ИЛИ Результат = Ложь Тогда
		...
		ДанныеВозврата.Вставить("Результат", Результат);
		ДанныеВозврата.Вставить("ИтогКоличествоОтправленных", ДополнительныеПараметры.ИтогКоличествоОтправленных);
		ДанныеВозврата.Вставить("ИтогКоличествоПолученных", 0);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеОтправкиПакетов, ДанныеВозврата);
		Возврат;
	КонецЕсли;
...
Процедура ОтправитьСледующийПакетЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.ИтогКоличествоОтправленных = ДополнительныеПараметры.ИтогКоличествоОтправленных + 1;
...
Процедура ПолучитьПакетыПослеПолученияИдентификаторовЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") Или Результат = Неопределено Тогда
		...
		СтруктураВозврата.Вставить("ИтогКоличествоПолученных", 0);
...
Процедура ПолучитьОчереднойПакетВК(МассивИдентификаторов, ДополнительныеПараметры)
	...
	Если МассивИдентификаторов.Количество() = 0 Тогда
		...
		КонецЕсли;
		СтруктураВозврата.Вставить("ИтогКоличествоПолученных", ДополнительныеПараметры.ИтогКоличествоПолученных);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПолученияНовыхДокументов, СтруктураВозврата);
		Возврат;
	КонецЕсли;
...
Процедура СохранитьПакетПослеПолученияЧерезВК(Результат, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.ИтогКоличествоПолученных = ДополнительныеПараметры.ИтогКоличествоПолученных + 1;
