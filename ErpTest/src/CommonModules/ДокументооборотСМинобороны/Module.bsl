...
Функция ПроверитьПараметрыНаСервереИнтернетПоддержки(Логин = "", Пароль = "")
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		Если Не ПустаяСтрока(СтрокаОтвет) Тогда
			...
			Если СтатусПоддержки <> "OK" Тогда
				Результат.СтатусПодпискиИТС = Данные.supportConditionsImplStatus.itsStatus                 = "OK";
				Результат.СтатусРегистрации = Данные.supportConditionsImplStatus.productRegistrationStatus = "OK";
				Возврат Результат;
			Иначе
