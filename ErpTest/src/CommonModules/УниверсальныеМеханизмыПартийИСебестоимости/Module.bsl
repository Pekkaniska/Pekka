...
Процедура ИнициализироватьОбщиеПараметрыРасчета(ПараметрыИнициализации, ПараметрыРасчета, ПараметрыОтладки = Неопределено) Экспорт
	...
	Если ПараметрыИнициализации.Свойство("РасшифровкаРаспределения") Тогда
		ПараметрыРасчета.Вставить("РасшифровкаРаспределения"); // вызывается при формировании отчета
	КонецЕсли;
	...
	Для Каждого ПараметрыУчетнойПолитики Из УчетныеПолитики Цикл
		...
		Если ПараметрыРасчета.ПартионныйУчетВерсии22
		 И (ПараметрыУчетнойПолитики.РаздельныйУчетТоваровПоНалогообложениюНДС
		 	ИЛИ ПараметрыУчетнойПолитики.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная) Тогда
		...
		Если ПараметрыРасчета.ПартионныйУчетВерсии22
		 И (ПараметрУчетнойПолитикиПрошлогоПериода.РаздельныйУчетТоваровПоНалогообложениюНДС
		 	ИЛИ ПараметрУчетнойПолитикиПрошлогоПериода.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная) Тогда
...
Процедура СформироватьДвиженияПоРегиструПоДаннымЗапроса(ПараметрыРасчета, ИмяРегистра, Запрос) Экспорт
	...
	Выборка = ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, Истина);
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			Иначе
				...
				//++ НЕ УТКА
				Если ТипЗнч(НаборЗаписей) = Тип("РегистрНакопленияНаборЗаписей.СебестоимостьТоваров")
				 И ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЭтапПроизводства2_2")
				 И ЭтоПараметрыРасчетаПУ22 Тогда
				...
				ТаблицаПериодов = ДополнительныеСвойства.ТаблицыДляДвижений["Таблица" + ИмяРегистраНакопления].Скопировать(,"Период");
				ТаблицаПериодов.Свернуть("Период", "");
				Если ТаблицаПериодов.Количество() > 1 Тогда
					НаборЗаписей.ДополнительныеСвойства.Удалить(ИмяСлужебногоДополнительногоСвойстваОбъекта());
				КонецЕсли;
				ДатыЗапретаИзмененияУТ.ОтключитьПроверкуДатыЗапретаИзменения(НаборЗаписей);
...
Функция ВыполнитьЗапросСЗамеромДляПротокола(ПараметрыРасчета, Запрос, УвеличиватьСчетчикЗамераПроизводительности = Ложь,
			ИменаВременныхТаблицДляЗамера = "", ВозвращатьРезультатыЗапросов = Ложь) Экспорт
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, Реквизиты) Экспорт
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Реквизиты, "Организация")
	 И ЗначениеЗаполнено(Реквизиты.Организация) Тогда
		...
		ПараметрыУчетнойПолитики = ЗначениеНастроекПовтИсп.ПараметрыУчетнойПолитики(Реквизиты.Организация, НачалоМесяца(Период)); 
		...
		Запрос.УстановитьПараметр("РаздельныйУчетТоваровПоНалогообложениюНДС",
	...
	Запрос.УстановитьПараметр("ЭтоВводОстатковВНА_2_4", ЭтоВводОстатковВНА_2_4);
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		...
		НСтр("ru='В регистрах ""Прочие расходы незавершенного производства""  и ""Трудозатраты незавершенного производства"" не должно быть остатков по завершенным партиям производства.'"));
	//-- НЕ УТ
...
// Пример вызова: УниверсальныеМеханизмыПартийИСебестоимости.ВыгрузитьВременнуюТаблицуВФайл(ПараметрыРасчета,
// "ВТКэшСебестоимостьТоваров", "C:\Temp\Таблица.mxl").
//
...
// Пример вызова: УниверсальныеМеханизмыПартийИСебестоимости.ВыбратьСтрокиВременнойТаблицы(ПараметрыРасчета,
// "ВТКэшСебестоимостьТоваров", 1).
//
