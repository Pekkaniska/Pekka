...
//				* СтавкиСтраховыхВзносов - таблица значений с колонками: Сотрудник, Ставк.
//
...
//
//	Возвращаемое значение:
//			Структура, см. конструктор НоваяСтруктураДанныеДляОтраженияЗарплатыВБухучете() 
//				* НачисленнаяЗарплатаИВзносы - таблица значений, описание см. НоваяТаблицаБухучетНачисленнаяЗарплатаИВзносы()
//					* ФизическоеЛицо
//					* Сотрудник
//					* Подразделение
...
//				* НачисленныйНДФЛ - таблица значений, описание см. НоваяТаблицаБухучетНачисленныйНДФЛ()
//					* ФизическоеЛицо
//					* Сотрудник
//					* ВидОперации
//					* СтатьяФинансирования
...
//					* Сумма
//				* УдержаннаяЗарплата – таблица значений, описание см. НоваяТаблицаБухучетУдержаннаяЗарплата()
//					* ФизическоеЛицо
//					* Сотрудник
//					* Подразделение
//					* ПодразделениеУчетаЗатрат
...
Функция ДоступноИзменениеПроцентаЕНВДДляУчетаЗатратНаЗарплату() Экспорт
...
// Сворачивает переданную таблицу значений с результатами распределения.
//
// Параметры
//		РезультатыРаспределения - таблица значений.
//
Процедура СвернутьРезультатыРаспределения(РезультатыРаспределения) Экспорт

	Колонки = "";
	ЕстьКолонкаРезультат = Ложь;
	Для Каждого Колонка Из РезультатыРаспределения.Колонки Цикл
		Если Колонка.Имя = "Результат" Тогда
			ЕстьКолонкаРезультат = Истина;
			Продолжить;
		КонецЕсли;
		Колонки = Колонки + "," + Колонка.Имя;
	КонецЦикла;
	
	Если ЕстьКолонкаРезультат Тогда
		Колонки = Сред(Колонки, 2);
...
// Рассчитывает остатки отпусков по сотруднкиам.
//
// Параметры:
// 	Организация – тип СправочникСсылка.Организации.
// 	Период – тип Дата.
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.3.78";
	Обработчик.Процедура = "ОтражениеЗарплатыВБухучете.ЗаполнитьРегистрациюВНалоговомОрганеДляБухучетаНДФЛ";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();   
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("74fc14a6-c3c0-4e8e-8db1-22786fd4eba1");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение регистрации в налоговом органе для бухучета НДФЛ.'");
...
Процедура ЗаполнитьРегистрациюВНалоговомОрганеДляБухучетаНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы") Тогда
		ИменаРегистров.Добавить("БухучетНачисленияУдержанияПоСотрудникам");
		ИменаРегистров.Добавить("БухучетНачисленияУдержанияПоКонтрагентамАкционерам");
		ИменаРегистров.Добавить("НачисленияУдержанияПоКонтрагентамАкционерам");
	КонецЕсли;
	ИменаРегистров.Добавить("НачисленияУдержанияПоСотрудникам");
	...
	|ИЗ
	|	#ИмяРегистра КАК НачисленияУдержания
	|ГДЕ
	|	НачисленияУдержания.НачислениеУдержание В(&ВидыОсобыхНачисленийИУдержанийНДФЛ)
	|	И НачисленияУдержания.РегистрацияВНалоговомОргане = ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НачисленияУдержания.ТерриторияВыполненияРаботВОрганизации КАК Подразделение,
	|	НачисленияУдержания.Период КАК Период
	|ИЗ
	|	ВТРегистраторы КАК Регистраторы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ #ИмяРегистра КАК НачисленияУдержания
	...
	|	ВЫБОР
	|		КОГДА НачисленияУдержания.НачислениеУдержание В (&ВидыОсобыхНачисленийИУдержанийНДФЛ)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК СтрокаНДФЛ,
	|	*
	|ИЗ
	|	ВТРегистраторы КАК Регистраторы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ #ИмяРегистра КАК НачисленияУдержания
	...
	Для Индекс = 0 По ИменаРегистров.ВГраница() Цикл
		
		ИмяРегистра  = ИменаРегистров[Индекс];
		Запрос.Текст = СтрЗаменить(ТекстЗапроса, "#ИмяРегистра","РегистрНакопления."+ИмяРегистра);
		РезультатЗапроса = Запрос.ВыполнитьПакет();
		Если НЕ РезультатЗапроса[2].Пустой() Тогда
			
			ОбработкаВыполнялась = Истина;
			
			СоответствиеПериодРегистрации = Новый Соответствие;
			...
			Пока Выборка.СледующийПоЗначениюПоля("Период") Цикл
				...
				КонецЦикла;
				СоответствиеПодразделенийИРегистраций = УчетНДФЛ.ПолучитьРегистрацииПодразделенийВНалоговомОргане(Подразделения, ДатаАктуальности);
				СоответствиеПериодРегистрации.Вставить(ДатаАктуальности, СоответствиеПодразделенийИРегистраций);
			КонецЦикла;
			
			Выборка = РезультатЗапроса[2].Выбрать();
			Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
				
				ИмяПространстваБлокировки = "РегистрНакопления." + ИмяРегистра + ".НаборЗаписей";
				...
				Пока Выборка.СледующийПоЗначениюПоля("Период") Цикл
					...
					СоответствиеПодразделенийИРегистраций = СоответствиеПериодРегистрации[Период];
					Если СоответствиеПодразделенийИРегистраций = Неопределено Тогда
						СоответствиеПодразделенийИРегистраций = Новый Соответствие;
					КонецЕсли;
					
					Организация = Выборка.Организация;
					РегистрацияВНалоговомОргане = ЗарплатаКадры.РегистрацияВНалоговомОргане(Организация, Период);
					Пока Выборка.Следующий() Цикл
						НоваяСтрока = НаборЗаписей.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
						Если Выборка.СтрокаНДФЛ Тогда
							РегистрацияВНалоговомОрганеПодразделения = СоответствиеПодразделенийИРегистраций.Получить(Выборка.ТерриторияВыполненияРаботВОрганизации);
							УчетНДФЛ.ПроставитьРегистрациюВНалоговомОрганеВСтроке(НоваяСтрока, РегистрацияВНалоговомОрганеПодразделения, РегистрацияВНалоговомОргане);
						КонецЕсли;
					КонецЦикла;
