...
Функция ЗапросДанных(ДокументИмя, ЭтоОбъектРасчетов = Истина) Экспорт
	...
	Иначе
		ТекстыСозданияВременныхТаблиц.Добавить(ТекстПустыеВТ("РасчетыСКлиентамиНоваяАрхитектура,РасчетыСПоставщикамиНоваяАрхитектура"));
	КонецЕсли;
...
#Область ВтСтроки
	...
	|		КОГДА (Аналитика.Склад ССЫЛКА Справочник.Партнеры ИЛИ Аналитика.Склад ССЫЛКА Справочник.Организации) И НЕ ВидыЗапасов.Договор ЕСТЬ NULL
	|		ТОГДА ВидыЗапасов.Договор.Подразделение
	|	ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ КАК ПодразделениеАналитики,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КорАналитика.Склад, Аналитика.Склад) ССЫЛКА Справочник.Склады
	|		ТОГДА ЕСТЬNULL(КорАналитика.Склад.Подразделение, Аналитика.Склад.Подразделение)
	|		КОГДА ЕСТЬNULL(КорАналитика.Склад, Аналитика.Склад) ССЫЛКА Справочник.СтруктураПредприятия
	|		ТОГДА ЕСТЬNULL(КорАналитика.Склад, Аналитика.Склад)
	|		КОГДА НЕ КорАналитика.Склад ЕСТЬ NULL И НЕ ВидыЗапасов.Договор ЕСТЬ NULL И
	|			(КорАналитика.Склад ССЫЛКА Справочник.Партнеры ИЛИ КорАналитика.Склад ССЫЛКА Справочник.Организации)
	|		ТОГДА КорВидыЗапасов.Договор.Подразделение
	|		КОГДА НЕ ВидыЗапасов.Договор ЕСТЬ NULL И (Аналитика.Склад ССЫЛКА Справочник.Партнеры ИЛИ Аналитика.Склад ССЫЛКА Справочник.Организации)
	|		ТОГДА ВидыЗапасов.Договор.Подразделение
	...
	|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.СтоимостьРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ДопРасходыРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ТрудозатратыРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	|	) КАК Сумма,
	|	СУММА(
	...
	|			КОГДА РасчетСебестоимостиТоваровОкончательный.Ссылка ЕСТЬ NULL
	|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.СтоимостьУпр, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ДопРасходыУпр, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ТрудозатратыУпр, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеУпр, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеУпр, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	|	) КАК СуммаУУ,
	|	СУММА(
	...
	|			КОГДА РасчетСебестоимостиТоваровОкончательный.Ссылка ЕСТЬ NULL
	|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.СтоимостьЗабалансоваяРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	...
	|			КОГДА РасчетСебестоимостиТоваровОкончательный.Ссылка ЕСТЬ NULL
	|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.СтоимостьЗабалансовая, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	...
	|			КОГДА РасчетСебестоимостиТоваровОкончательный.Ссылка ЕСТЬ NULL
	|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостояннаяРазница, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	|	) КАК СуммаПР,
	|	СУММА(
	...
	|			КОГДА РасчетСебестоимостиТоваровОкончательный.Ссылка ЕСТЬ NULL
	|				ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ВременнаяРазница, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	|	) КАК СуммаВР,
	|	СУММА(
	...
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПостоянныеРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					+ ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостатейныеПеременныеРегл, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					- ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ПостояннаяРазница, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|					- ВЫРАЗИТЬ(ЕСТЬNULL(СтоимостьТоваров.ВременнаяРазница, 0) * Движения.Количество КАК ЧИСЛО(31,2))
	|			ИНАЧЕ 0
	|		КОНЕЦ
	|	) КАК СуммаНУ
	|
...
Функция ТекстРасчетовПоНовойАрхитектуре()
	...
	|	ИНАЧЕ Расчеты.ОбъектРасчетов.Подразделение КОНЕЦ КАК Подразделение,
	|	ВЫБОР КОГДА Расчеты.ДокументРегистратор = Расчеты.ОбъектРасчетов ТОГДА NULL
	|	ИНАЧЕ Расчеты.ОбъектРасчетов.ГруппаФинансовогоУчета КОНЕЦ КАК ГруппаФинансовогоУчета,
	|	ВЫБОР КОГДА Расчеты.ДокументРегистратор = Расчеты.ОбъектРасчетов ТОГДА NULL
	|	ИНАЧЕ Расчеты.ОбъектРасчетов.ГруппаФинансовогоУчетаПолучателя КОНЕЦ КАК ГруппаФинансовогоУчетаПолучателя,
	|	
...
Функция ТекстЗачетАвансов(ДокументИмя) Экспорт
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.ПредоплатаРегл ИНАЧЕ 0 КОНЕЦ) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.ПредоплатаУпр ИНАЧЕ 0 КОНЕЦ) КАК СуммаУУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.АвансыПолученные) КАК ВидСчетаДт,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) И Расчеты.ЗаказКлиента = Расчеты.РасчетныйДокумент ТОГДА НЕОПРЕДЕЛЕНО
	...
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.Валюта ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК ВалютаДт,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) И Расчеты.ЗаказКлиента = Расчеты.РасчетныйДокумент ТОГДА НЕОПРЕДЕЛЕНО
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КОНЕЦ) КАК ПодразделениеДт,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Аналитика.НаправлениеДеятельности ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК СчетДт,
	|
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Аналитика.Контрагент ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоДт1,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Аналитика.Договор ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказКлиента <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.Предоплата ИНАЧЕ 0 КОНЕЦ) КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	...
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА Операция.Ссылка = Расчеты.ЗаказКлиента ТОГДА Операция.ГруппаФинансовогоУчета
	|		КОГДА НЕ Договоры.ГруппаФинансовогоУчета ЕСТЬ NULL ТОГДА Договоры.ГруппаФинансовогоУчета
	|		КОГДА НЕ ДоговорыОрганизаций.ГруппаФинансовогоУчета ЕСТЬ NULL И Аналитика.Организация = ДоговорыОрганизаций.Организация
	|		ТОГДА ДоговорыОрганизаций.ГруппаФинансовогоУчета
	...
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	|	
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Расчеты.Валюта ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК ВалютаКт,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА НЕ Договоры.Подразделение ЕСТЬ NULL ТОГДА Договоры.Подразделение
	|		КОГДА НЕ ДоговорыОрганизаций.Подразделение ЕСТЬ NULL ТОГДА ДоговорыОрганизаций.Подразделение
	...
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Аналитика.НаправлениеДеятельности ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО КАК СчетКт,
	|
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Аналитика.Контрагент ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоКт1,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Операция.Договор ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Расчеты.Предоплата ИНАЧЕ 0 КОНЕЦ) КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	...
	|СГРУППИРОВАТЬ ПО
	|	Операция.Ссылка,
	|	ЕСТЬNULL(РеализацияБезПереходаПраваСобственности.ДатаПереходаПраваСобственности, Расчеты.Период),
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.ПредоплатаРегл ИНАЧЕ 0 КОНЕЦ) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.ПредоплатаУпр ИНАЧЕ 0 КОНЕЦ) КАК СуммаУУ,
	|
	...
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаДт,
	|
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Расчеты.Валюта ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК ВалютаДт,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА НЕОПРЕДЕЛЕНО
	...
	|		КОГДА НЕ ПервичныйДокумент.Подразделение ЕСТЬ NULL ТОГДА ПервичныйДокумент.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КОНЕЦ) КАК ПодразделениеДт,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Аналитика.НаправлениеДеятельности ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК СчетДт,
	|
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Аналитика.Контрагент ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоДт1,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Аналитика.Договор ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ТОГДА Расчеты.Предоплата ИНАЧЕ 0 КОНЕЦ) КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	...
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) И Расчеты.ЗаказПоставщику = Расчеты.РасчетныйДокумент ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА Операция.Ссылка = Расчеты.ЗаказПоставщику ТОГДА Операция.ГруппаФинансовогоУчета
	|		КОГДА НЕ Договоры.ГруппаФинансовогоУчета ЕСТЬ NULL ТОГДА Договоры.ГруппаФинансовогоУчета
	|		КОГДА НЕ ДоговорыОрганизаций.ГруппаФинансовогоУчета ЕСТЬ NULL И Аналитика.Организация = ДоговорыОрганизаций.Организация
	|		ТОГДА ДоговорыОрганизаций.ГруппаФинансовогоУчета
	...
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	|	
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.Валюта ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК ВалютаКт,
	|	МАКСИМУМ(ВЫБОР
	|		КОГДА Расчеты.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) И Расчеты.ЗаказПоставщику = Расчеты.РасчетныйДокумент ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА Операция.Ссылка = Расчеты.ЗаказПоставщику ТОГДА Операция.Подразделение
	|		КОГДА НЕ Договоры.Подразделение ЕСТЬ NULL ТОГДА Договоры.Подразделение
	...
	|		КОГДА НЕ ПервичныйДокумент.Подразделение ЕСТЬ NULL ТОГДА ПервичныйДокумент.Подразделение
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КОНЕЦ) КАК ПодразделениеКт,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Аналитика.НаправлениеДеятельности ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК НаправлениеДеятельностиКт,
	|	НЕОПРЕДЕЛЕНО КАК СчетКт,
	|
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Аналитика.Контрагент ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоКт1,
	|	МАКСИМУМ(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Аналитика.Договор ИНАЧЕ НЕОПРЕДЕЛЕНО КОНЕЦ) КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	СУММА(ВЫБОР КОГДА Расчеты.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереносАванса) ИЛИ Расчеты.ЗаказПоставщику <> Расчеты.РасчетныйДокумент ТОГДА Расчеты.Предоплата ИНАЧЕ 0 КОНЕЦ) КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	0 КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	...
	Если ДокументИмя = "Документ.ОтчетПоКомиссииМеждуОрганизациями" ИЛИ ДокументИмя = "Документ.ПередачаТоваровМеждуОрганизациями" Тогда
		...
		ВыражениеДляИнтеркомпани = СтрЗаменить(ВыражениеДляИнтеркомпани, "Аналитика", "Расчеты");
		ТекстЗачетАвансовКлиентовНоваяАрхитектура = СтрЗаменить(ТекстЗачетАвансовКлиентовНоваяАрхитектура, "Операция.ГруппаФинансовогоУчета", ВыражениеДляИнтеркомпани);
		ТекстЗачетАвансовПоставщикамНоваяАрхитектура = СтрЗаменить(ТекстЗачетАвансовПоставщикамНоваяАрхитектура, "Операция.ГруппаФинансовогоУчета", ВыражениеДляИнтеркомпани);
	КонецЕсли;
...
#Область Распределение	 // Отражение доп. расходов (Дт 10, 15, 20, 41 :: Кт 10.ДР, 41.ДР, 19.03, 19.11)
	...
	ТекстРаспределения =
	"ВЫБРАТЬ //// Отражение доп. расходов (Дт 10, 15, 20, 41 :: Кт 10.ДР, 41.ДР, 19.03, 19.11)
	|	Операция.Ссылка КАК Ссылка,
...
Функция ТекстНДСПоНДСПредъявленный() Экспорт
	...
	Возврат 
		...
		|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)                               КАК ВалютаКт,
		|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)                 КАК ПодразделениеКт,
		|	НДСПредъявленный.КорНаправлениеДеятельности                            КАК НаправлениеДеятельностиКт,
		|
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)                         КАК СчетКт,
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ // Изменение направления деятельности (Дт <19.Х> - Кт <19.Х>)
		|
		|	НДСПредъявленный.Регистратор КАК Ссылка,
		|	НДСПредъявленный.Период      КАК Период,
		|	НДСПредъявленный.Организация КАК Организация,
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ // Вычет НДС (Дт <68.02, 68.52> - Кт <19.Х>)
		|
		|	НДСПредъявленный.Регистратор КАК Ссылка,
		|	НДСПредъявленный.Период      КАК Период,
		|	НДСПредъявленный.Организация КАК Организация,
		...
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ // Восстановление НДС (Кт <19.Х> - Дт <68.02, 68.52>)
		|
		|	НДСПредъявленный.Регистратор КАК Ссылка,
		|	НДСПредъявленный.Период      КАК Период,
		|	НДСПредъявленный.Организация КАК Организация,
...
Функция СтрокаМестоАналитика(МестоУчета, АналитикаУчета) Экспорт
	...
	Если ЗначениеЗаполнено(МестоУчета) Тогда
		...
		ИначеЕсли ТипМеста = Тип("ПеречислениеСсылка.ВидыЦенностей") Тогда
			Место = НСтр("ru='по виду ценности ""%МестоУчета%""'");
		Иначе
			Место = НСтр("ru='по месту учета ""%МестоУчета%""'");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(АналитикаУчета) Тогда
		...
		ИначеЕсли ТипАналитики = Тип("ПеречислениеСсылка.ТипыНалогообложенияНДС") Тогда
			Аналитика = НСтр("ru='для налогообложения ""%АналитикаУчета%""'");
		Иначе
			Аналитика = НСтр("ru='для аналитики учета ""%АналитикаУчета%""'");
		КонецЕсли;
...
Функция ИспользуемыеТаблицыДляОтраженияТипа(ИмяТипаДокумента) Экспорт
	...
	// ПоступлениеТоваров
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.ПоступлениеТоваров");
	ТаблицыДокумента.Добавить("РегистрНакопления.СебестоимостьТоваров");
	ТаблицыТиповДокументов.Вставить("ПоступлениеТоваров", ТаблицыДокумента);
	...
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.РаспределениеДоходовИРасходовПоНаправлениямДеятельности");
	ТаблицыДокумента.Добавить("РегистрНакопления.ПрочиеРасходы");
	ТаблицыДокумента.Добавить("РегистрСведений.ПорядокОтраженияРасходов");
	ТаблицыТиповДокументов.Вставить("РаспределениеДоходовИРасходовПоНаправлениямДеятельности", ТаблицыДокумента);
...
Функция ПараметрыОтраженияРеглУчетаПоУмолчанию() Экспорт
	...
	СтруктураПараметров.Вставить("ТипыНеОблагаемыеНДС",      УчетНДСУП.ТипыНалогообложенияНДСУчитываетсяВСтоимости());
	СтруктураПараметров.Вставить("ТипыОблагаемыеНДС",        УчетНДСУП.ТипыНалогообложенияНДСНеУчитываетсяВСтоимости());
