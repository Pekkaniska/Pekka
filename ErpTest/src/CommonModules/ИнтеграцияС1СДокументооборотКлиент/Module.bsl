...
//Рарус Владимир Подрезов 15.03.2017
	тПараметрыПодключения = пкОбщийМодульСервер.пкПолучитьПараметрыПользователяДО(Неопределено);
	Если тПараметрыПодключения.ПарольСохранен Тогда
		ИмяПользователя	= тПараметрыПодключения.ПользовательДО;
		Пароль			= тПараметрыПодключения.ПарольДО;
		ПарольСохранен	= тПараметрыПодключения.ПарольСохранен;
		Возврат;
	КонецЕсли;
...
//Рарус Владимир Подрезов 15.03.2017
	Если ПарольСохранен Тогда
		пкОбщийМодульСервер.пкПроверитьЗаписатьПараметрыПользователяДО(Неопределено, ИмяПользователя, Пароль);
	КонецЕсли;
//Рарус Владимир Подрезов Конец
