...
Процедура ЗагрузитьСообщение(Источник) Экспорт
	...
	Исключение
		...
		РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если Не СостояниеВыгрузки.Выбран() Тогда
		...
		РегистрацияОшибки(НСтр("ru = 'Выгрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПредставлениеОшибки);
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийВыгрузка);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Шаблон = НСтр("ru = 'Не удалось получить состояние загрузки, возможно сервис временно недоступен: %1 %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Комментарий = СтрШаблон(Шаблон, Символы.ПС, ПредставлениеОшибки);
		РегистрацияПримечания(НСтр("ru = 'Выгрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
	...
	Исключение
		...
		Иначе
			...
			РегистрацияОшибки(НСтр("ru = 'Выгрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ВыгрузитьДанныеВКаталог(КаталогВыгрузки, УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически) Экспорт
	...
	Если ВыгруженоОбъектов < ПараметрыВыгрузки.РазмерПорцииДанных Тогда
		Если ЗавершитьМиграциюАвтоматически Тогда
			...
			Исключение
				Комментарий = НСтр("ru = 'Не удалось установить монопольный режим.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Если Не СостояниеЗагрузки.Выбран() Тогда
		Комментарий = НСтр("ru = 'Состояние загрузки не найдено. Загрузка прервана'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
		// Самоудаление.
		УдалитьОчередь();
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийЗагрузка);
		Возврат;
	...
	Исключение
		...
		Иначе 
			РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ЗагрузитьДанныеИзКаталога(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки) Экспорт
	...
	Если Манифест.Свойство("ОбъектыВыгруженныеПолностью", ОбъектыВыгруженныеПолностью) Тогда
		Шаблон = НСтр("ru = 'Не удалось выгрузить порциями объекты: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СервисПоддерживаетМиграцию(ИмяСервера, АдресПрограммногоИнтерфейса, АдресРегистрации, АдресВосстановления, РегистрацияРазрешена) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Сервис не поддерживает миграцию'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция Зарегистрироваться(ИмяСервера, Наименование, Логин, ЭлектроннаяПочта, Пароль, Телефон) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось выполнить запрос, код ответа: %1'"), Ответ.КодСостояния));
		Возврат Ложь;
	КонецЕсли;
...
Функция Загрузить(ОписаниеДанных) Экспорт
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			РегистрацияОшибки(НСтр("ru = 'Загрузка сообщения'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
...
Функция ВыгрузитьПользовательскиеДанные(УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ПараметрыВыгрузки)
	...
	ШаблонНачалоВыгрузки = НСтр("ru = 'Выгрузка объекта: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ШаблонОкончаниеВыгрузки = НСтр("ru = 'Выгружен объект: %1, Количество: %2, Длительность (сек): %3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВсегоОбъектов = 0;
	...
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				РегистрацияОшибки(НСтр("ru = 'Выгрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
				Продолжить;
			КонецПопытки;
...
Функция ЗагрузитьПользовательскиеДанные(ПараметрыЗагрузки)
	...
	ШаблонНачалоЗагрузки = НСтр("ru = 'Загружается объект: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ШаблонОкончаниеЗагрузки = НСтр("ru = 'Загружен объект: %1, Количество: %2, Длительность (сек): %3'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ТипПоляПоиска(ОбъектМетаданных, ИмяПоля)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
		|поле объекта не обнаружено!'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяПоля,
		ОбъектМетаданных.ПолноеИмя());
...
Процедура ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru = 'Выгрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура РегистрацияОшибки(Событие, Комментарий)
	...
	ИмяСобытия = СтрШаблон(НСтр("ru = 'Миграция приложений.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Событие);
