...
//           В таких случах рекомендуется изменять не состав фиксируемых реквизитов, а только данный флажок.
//
...
// Регистрирует описание фиксируемого реквизита в коллекции фиксируемых реквизитов объекта.
//
// Параметры:
//   ФиксируемыеРеквизиты - Соответствие - Коллекция, в которую добавляется описание фиксируемого реквизита
//       и которая далее используется как первый параметр функции ФиксацияВторичныхДанныхВДокументах.ПараметрыФиксацииВторичныхДанных().
//   Шаблон - Структура - Описание группы фиксируемых реквизитов, полученное при помощи функции ОписаниеФиксируемогоРеквизита().
//   ИмяРеквизита - Строка - Имя реквизита объекта.
//   ОтображатьПредупреждение - Булево - Не обязательный. Признак отображения предупреждения при редактировании
//       автоматически заполненного (незафиксированного) реквизита.
//       По умолчанию Истина. Если Ложь, то при начале редактирования реквизита не будет выводиться предупреждение.
...
//       В частности, рекомендуется отключать для реквизитов, которые заполняются из последнего выбранного значения.
//
Процедура ДобавитьФиксируемыйРеквизит(ФиксируемыеРеквизиты, Шаблон, ИмяРеквизита, ОтображатьПредупреждение = Истина) Экспорт
	Шаблон.ИмяРеквизита = ИмяРеквизита;
	Шаблон.ОтображатьПредупреждение = ОтображатьПредупреждение;
...
// ОснованиеЗаполнения из ОписаниеФиксируемогоРеквизита.
//    	ПараметрыФиксацииВторичныхДанных - структура см.
//    	                                   ФиксацияВторичныхДанныхВДокументахКлиентСервер.ПараметрыФиксацииВторичныхДанных.
//
...
//   ИмяТЧ - Строка - Необязательный. Имя табличной части с реквизитом.
//   ИдентификаторСтрокиФикс - Число - Необязательный. Идентификатор фиксируемой строки табличной части.
//
Процедура ЗафиксироватьРеквизитОбъекта(Объект, ИмяРеквизита, ИмяТЧ = "", ИдентификаторСтрокиФикс = 0) Экспорт
	
	ФильтрДляПоиска = Новый Структура("ИмяРеквизита, Путь", ИмяРеквизита, ИмяТЧ);
	
	// Для не ТЧ ИмяТЧ = "", а ИдентификаторСтрокиФикс = 0.
	Если ИмяТЧ <> "" Тогда
		ФильтрДляПоиска.Вставить("ИдентификаторСтроки", ИдентификаторСтрокиФикс);
	КонецЕсли;
	
	Найденные = Объект.ФиксацияИзменений.НайтиСтроки(ФильтрДляПоиска);
	Если Найденные.Количество() = 0 Тогда
		ЗаполнитьЗначенияСвойств(Объект.ФиксацияИзменений.Добавить(), ФильтрДляПоиска);
	КонецЕсли;
