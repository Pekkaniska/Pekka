...
Процедура ПрочитатьРеквизитыИсправления(Форма, РежимИсправления = "РасчетЗарплаты", ИмяОсновногоРеквизита = "Объект", ПолеОрганизация = "Организация", ПолеПериодРегистрации = "ПериодРегистрации", ПроверятьНаличиеНачислений = Истина) Экспорт
	...
	Если Форма.ИспользоватьИсправлениеДокумента И РежимИсправления = "РасчетЗарплаты"
		И Не Объект.ИсправленныйДокумент.Пустая() Тогда
		Форма.ПериодРегистрацииИсправленногоДокумента =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ИсправленныйДокумент, ПолеПериодРегистрации);
	КонецЕсли;
	
	// Исходим из того, что механизм исправления не подключается к непроведенным документам.
...
Функция ДоступноИсправлениеРасчетногоДокумента(Документ, Организация, ПериодРегистрации, ПроверятьНаличиеНачислений) Экспорт 
	...
	ИсправлениеДоступноДажеЕслиНеВыполненыНачисления = Не ПроверятьНаличиеНачислений;
	...
	РезультатПроверки = Новый Структура("ИсправлениеДоступно,ПроведенаВыплатаЗарплаты,ПроизведеноОтражение", Ложь, НеОпределено, НеОпределено);
	...
	Иначе
		...
		Если (	БылаВыплатаВПериоде 
				Или БылоОтражениеЗарплатыВБухучете 
				Или ИсправлениеВТекущемПериодеВозможно 
				Или ИсправлениеДоступноДажеЕслиНеВыполненыНачисления) Тогда
			РезультатПроверки.ИсправлениеДоступно = Истина;
...
Функция ПараметрыСторнирования() Экспорт
	...
	ПараметрыСторнирования.Вставить("ИсправлениеВТекущемПериоде", Ложь);
	ПараметрыСторнирования.Вставить("ОтменаДокумента", Ложь);
	ПараметрыСторнирования.Вставить("ПериодРегистрации", Неопределено);
	ПараметрыСторнирования.Вставить("РеквизитыДляПроведения", Неопределено);
		
	Возврат ПараметрыСторнирования;
...
//    * ИсправлениеВТекущемПериоде - Булево - Истина когда сторнирование выполняется в периоде регистрации сторнируемого документа.
...
Процедура СторнироватьДвиженияПоВсемУчетам(Движения, ИсправленныйДокумент, ПараметрыСторнирования, СтруктураВидовУчета) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		ИсправлениеДокументовЗарплатаКадры.СторнироватьДвиженияБезСпецификиУчетов(
			Движения, ИсправленныйДокумент, ПараметрыСторнирования);
...
// Выполняет проверку наличия существующего документа исправления или сторнирования по исправляемому документу.
// Параметры:
//		ИсправленныйДокумент - исправленный документ
//		ИсключаемыйДокумент - исключаемый из проверки документ.
//		Отказ - булево
Процедура ПроверитьНаличиеИсправленияСторнирования(ИсправленныйДокумент, ИсключаемыйДокумент = Неопределено, Отказ = Ложь) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИсправленныйДокумент", ИсправленныйДокумент);
	Запрос.УстановитьПараметр("Ссылка", ИсключаемыйДокумент);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаДокумента.Ссылка КАК Ссылка,
	|	""исправлен"" КАК ТипДействия
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ДокументСторно.Ссылка,
	|	""сторнирован"" КАК ТипДействия
	|ИЗ
	|	Документ.СторнированиеНачислений КАК ДокументСторно
	|ГДЕ
	|	ДокументСторно.СторнируемыйДокумент = &ИсправленныйДокумент
	|	И ДокументСторно.Ссылка <> &Ссылка
	|	И ДокументСторно.Проведен";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "#ТаблицаДокумента", ОбщегоНазначения.ИмяТаблицыПоСсылке(ИсправленныйДокумент));
	
	ШаблонСообщения = НСтр("ru = 'Документ %1 уже %2 документом %3'");
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ИсправленныйДокумент, Выборка.ТипДействия, Выборка.Ссылка);
...
Функция ДокументыИсправляемыеВТекущемПериоде()
