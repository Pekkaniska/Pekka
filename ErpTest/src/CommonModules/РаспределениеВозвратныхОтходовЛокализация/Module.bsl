...
#Область ТекстВыпускСобственнойПродукции // (Дт 10 :: Кт 2Х)
	...
	|	ЕСТЬNULL(СкладЦеховаяКладовая.Подразделение, Операция.Подразделение) КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	ЕСТЬNULL(СкладЦеховаяКладовая.Подразделение, Операция.Подразделение) КАК ПодразделениеКт,
	...
	|			ПО КлючиАналитики.Склад = СкладЦеховаяКладовая.Ссылка
...
#Область ТекстВыпускВозвратныхОтходов // (Дт 2Х :: Кт 2Х)
	...
	// Проводки формируются только для цеховых кладовых и подразделений.
	|	И ЕСТЬNULL(СпрКорСклады.ЦеховаяКладовая, ИСТИНА)
	|";
#КонецОбласти
...
#Область ТекстРаспределениеПоРасходам // (Дт Счет учета возвратных отходов :: Кт СтатьяРасходов.СчетУчета)
	...
	|
	|	СуммыДокументовВВалютеРегл.СуммаБезНДСРегл КАК Сумма,
	|	СуммыДокументовВВалютеРегл.СуммаБезНДСУпр КАК СуммаУУ,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Производство) КАК ВидСчетаДт,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаДт,
	...
	|			ТОГДА СуммыДокументовВВалютеРегл.СуммаБезНДСРегл
	...
	|			ПО ТаблицаРасходов.Ссылка = СуммыДокументовВВалютеРегл.Регистратор
	|			И ТаблицаРасходов.ИдентификаторСтроки = СуммыДокументовВВалютеРегл.ИдентификаторСтроки
...
#Область ТекстРаспределениеПоПрочимАктивамПассивам // (Дт Счет учета возвратных отходов :: Кт Документ.СчетУчета)
	...
	|	Операция.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|
	|	ТаблицаРасходов.СчетУчета КАК СчетКт,
	|	ТаблицаРасходов.Субконто1 КАК СубконтоКт1,
	|	ТаблицаРасходов.Субконто2 КАК СубконтоКт2,
	|	ТаблицаРасходов.Субконто3 КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	ВЫБОР
