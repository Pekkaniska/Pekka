...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ТекстПередачаАктиваДоПереходаПраваСобственности());
	ТекстыОтражения.Добавить(ТекстРасходыОтСписанияАктиваДоПереходаПраваСобственности());
	ТекстыОтражения.Добавить(ТекстРасходыОтСписанияАктиваПослеПереходаПраваСобственности());
	ТекстыОтражения.Добавить(ТекстРасходыОтСписанияАктиваСОтложеннымПереходомПрав());
	ТекстыОтражения.Добавить(ТекстВосстановитьАмортизационнуюПремию());
...
Функция ТекстРасходыОтСписанияАктива()
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ //// Расходы от списания актива (Дт <91.02> :: Кт <01.09>)
	|	
	...
	|	
	|	""Расходы от списания основного средства"" КАК Содержание
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|ГДЕ
	|	Строки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СебестоимостьРеализацииОС)
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ //// Расходы от списания основного средства (Дт <91.02> :: Кт <01.09>)
	|	
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Расходы) КАК ВидСчетаДт,
	|	СтоимостьОС.КорСтатьяРасходов КАК АналитикаУчетаДт,
	|	СтоимостьОС.КорПодразделение КАК МестоУчетаДт,
	|	
	|	НЕОПРЕДЕЛЕНО КАК ВалютаДт,
	|	СтоимостьОС.КорПодразделение КАК ПодразделениеДт,
	|	СтоимостьОС.КорНаправлениеДеятельности КАК НаправлениеДеятельностиДт,
	|	
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетДт,
	|	
	|	СтоимостьОС.КорСтатьяРасходов КАК СубконтоДт1,
	|	СтоимостьОС.КорАналитикаРасходов КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|	
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	Строки.СуммаАмортизационнойПремии КАК СуммаНУДт,
	|	-Строки.СуммаАмортизационнойПремии КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|	
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ВыбытиеВНА) КАК ВидСчетаКт,
	|	СтоимостьОС.ГруппаФинансовогоУчета КАК АналитикаУчетаКт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	|	
	|	НЕОПРЕДЕЛЕНО КАК ВалютаКт,
	...
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	|	
	|	СтоимостьОС.ОсновноеСредство КАК СубконтоКт1,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|	
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	Строки.СуммаАмортизационнойПремии КАК СуммаНУКт,
	|	-Строки.СуммаАмортизационнойПремии КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	
	|	""Расходы от списания основного средства"" КАК Содержание
	|	
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияУслугПрочихАктивов.Расходы КАК Строки
	|	ПО Строки.Ссылка = СтоимостьОС.Регистратор
	|		И Строки.ВнеоборотныйАктив = СтоимостьОС.ОсновноеСредство
	|	
	|ГДЕ
	|	Строки.ВосстановитьАмортизационнуюПремию
	|	И СтоимостьОС.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СебестоимостьРеализацииОС)
	|	И СтоимостьОС.КорСтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|";
...
Функция ТекстПередачаАктиваДоПереходаПраваСобственности()
	...
	|	
	|ГДЕ
	|	Строки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав)
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	Расходы.СуммаАмортизационнойПремии КАК СуммаНУДт,
	|	-Расходы.СуммаАмортизационнойПремии КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|	
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
	|
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	Расходы.СуммаАмортизационнойПремии КАК СуммаНУКт,
	|	- Расходы.СуммаАмортизационнойПремии КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	
	|	""Передача актива до перехода права собственности"" КАК Содержание
	|	
	...
	|	ПО Строки.Регистратор = ДокументыКОтражению.Ссылка
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияУслугПрочихАктивов.Расходы КАК Расходы
	|	ПО Расходы.Ссылка = Строки.Регистратор
	|		И Расходы.ВнеоборотныйАктив = Строки.ОсновноеСредство
	|	
	|ГДЕ
	|	Расходы.ВосстановитьАмортизационнуюПремию
	|	И Строки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав)
	|	И Строки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И Строки.КорСтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|";
...
Функция ТекстРасходыОтСписанияАктиваСОтложеннымПереходомПрав()
	...
	|	Строки.СтоимостьВР + Строки.СтоимостьВРЦФ КАК СуммаВРКт,
	|	
	|	""Расходы от списания актива с отложенным переходом прав"" КАК Содержание
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
	...
	|	
	|ГДЕ
	|	Строки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РасходыОтСписанияАктиваСОтложеннымПереходомПрав)
	...
	|ГДЕ
	|	Расходы.ВосстановитьАмортизационнуюПремию
	|	И Строки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РасходыОтСписанияАктиваСОтложеннымПереходомПрав)
	|	И Строки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И Строки.КорСтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|";
...
Функция ТекстВосстановитьАмортизационнуюПремию()
	...
	|	Строки.ВнеоборотныйАктив КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|	
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
