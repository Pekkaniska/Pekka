...
//  Структура - см. функцию ИнтеграцияВЕТИСКлиентСервер.РезультатВыполненияЗапросаЭлементовКлассификатора().
//
Функция ИсторияИзмененийПредприятийРФ(Интервал, НомерСтраницы = 1, КоличествоЭлементовНаСтранице = Неопределено) Экспорт
...
//  Структура - см. функцию ИнтеграцияВЕТИСКлиентСервер.РезультатВыполненияЗапросаЭлементовКлассификатора().
//
Функция ИсторияИзмененийЗарубежныхПредприятий(Интервал, НомерСтраницы = 1, КоличествоЭлементовНаСтранице = Неопределено) Экспорт
...
//  Структура - см. функцию ИнтеграцияВЕТИСКлиентСервер.РезультатВыполненияЗапросаЭлементовКлассификатора().
//
Функция ИсторияИзмененийХозяйствующихСубъектов(Интервал, НомерСтраницы = 1, КоличествоЭлементовНаСтранице = Неопределено) Экспорт
...
Функция ЗапросЭлементовКлассификатораХозяйствующихСубъектовXML(ПараметрыОтбора, НомерСтраницы)
	...
	Если ПараметрыОтбора <> Неопределено Тогда
		...
		Запрос.businessEntity = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос, "businessEntity");
		...
		Если ПараметрыОтбора.Свойство("ДанныеАдреса")
			...
			Если    (ДанныеАдреса.Свойство("РегионGUID")          И ЗначениеЗаполнено(ДанныеАдреса.РегионGUID))
				...
				Запрос.businessEntity.juridicalAddress = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.businessEntity, "juridicalAddress");
				
				Если ДанныеАдреса.Свойство("СтранаGUID")
					И ЗначениеЗаполнено(ДанныеАдреса.СтранаGUID) Тогда
					Запрос.businessEntity.juridicalAddress.country = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.businessEntity.juridicalAddress, "country");
				...
				Если ДанныеАдреса.Свойство("РегионGUID")
					...
					Запрос.businessEntity.juridicalAddress.region = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.businessEntity.juridicalAddress, "region");
				...
				Если ДанныеАдреса.Свойство("РайонGUID")
					...
					Запрос.businessEntity.juridicalAddress.district = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.businessEntity.juridicalAddress, "district");
				...
				Если ДанныеАдреса.Свойство("НаселенныйПунктGUID")
					...
					Запрос.businessEntity.juridicalAddress.locality = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.businessEntity.juridicalAddress, "locality");
				...
				Если ДанныеАдреса.Свойство("УлицаGUID")
					...
					Запрос.businessEntity.juridicalAddress.street = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.businessEntity.juridicalAddress, "street");
...
Функция ЗапросЭлементовКлассификатораПредприятийРФXML(ПараметрыОтбора, НомерСтраницы)
	...
	Если ПараметрыОтбора <> Неопределено Тогда
		...
		Если ПараметрыОтбора.Свойство("НомерПредприятия")
			...
			Запрос.enterprise.numberList = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise, "numberList");
		...
		Если ПараметрыОтбора.Свойство("ДанныеАдреса")
			...
			Если    (ДанныеАдреса.Свойство("РегионGUID")          И ЗначениеЗаполнено(ДанныеАдреса.РегионGUID))
				...
				Запрос.enterprise.address = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise, "address");
				
				Если ДанныеАдреса.Свойство("РегионGUID")
					И ЗначениеЗаполнено(ДанныеАдреса.РегионGUID) Тогда
					Запрос.enterprise.address.region = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise.address, "region");
					Запрос.enterprise.address.region.guid = ДанныеАдреса.РегионGUID;
				КонецЕсли;
				
				Если ДанныеАдреса.Свойство("РайонGUID")
					И ЗначениеЗаполнено(ДанныеАдреса.РайонGUID) Тогда
					Запрос.enterprise.address.district = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise.address, "district");
					Запрос.enterprise.address.district.guid = ДанныеАдреса.РайонGUID;
				КонецЕсли;
				
				Если ДанныеАдреса.Свойство("НаселенныйПунктGUID")
					И ЗначениеЗаполнено(ДанныеАдреса.НаселенныйПунктGUID) Тогда
					Запрос.enterprise.address.locality = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise.address, "locality");
					Запрос.enterprise.address.locality.guid = ДанныеАдреса.НаселенныйПунктGUID;
				КонецЕсли;
				
				Если ДанныеАдреса.Свойство("УлицаGUID")
					И ЗначениеЗаполнено(ДанныеАдреса.УлицаGUID) Тогда
					Запрос.enterprise.address.street = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise.address, "street");
	...
	Если ПараметрыОтбора <> Неопределено Тогда
		...
		Если ПараметрыОтбора.Свойство("Страна")
			...
			Если ПараметрыОтбора.Свойство("Страна") Тогда
				Запрос.enterprise.address.country = ИнтеграцияВЕТИС.ОбъектXDTOПоИмениТипа(Запрос.enterprise.address, "country");
