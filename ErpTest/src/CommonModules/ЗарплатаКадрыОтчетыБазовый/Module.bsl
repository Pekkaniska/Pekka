...
Функция ЗапросДанныеДокументаФизическихЛиц() Экспорт
	...
	Запрос.Текст =
		...
		|			И КадровыеДанныеСотрудниковСПриоритетами.Подразделение = ВысшиеПриоритетыПодразделенийСотрудников.ПодразделениеСортировки
		|
		|СГРУППИРОВАТЬ ПО
		...
		|			ТОГДА ВЫРАЗИТЬ(НачисленияИУдержания.ВидРасчета КАК ПланВидовРасчета.Начисления).РеквизитДопУпорядочивания * 100
		|		КОГДА ТИПЗНАЧЕНИЯ(НачисленияИУдержания.ВидРасчета) = ТИП(ПланВидовРасчета.Удержания)
		|			ТОГДА ВЫРАЗИТЬ(НачисленияИУдержания.ВидРасчета КАК ПланВидовРасчета.Удержания).РеквизитДопУпорядочивания * 100
