...
//  Предикат                - Строка                  - Выражение для получения ФПД.
//  ПараметрыПредиката      - Структура               - Значения параметров используемых в выражении параметра "Предикат".
//  Реквизиты               - Строка, Массив          - Имена получаемых полей. Поддерживается значение "*".
//  ИмяВТ                   - Строка                  - Имя временной таблицы по умолчания "ВТРезультирующийПериодДействия".
//
Процедура СоздатьВТРезультирующийПериодДействия(МенеджерВременныхТаблиц,
			Знач Предикат, ПараметрыПредиката = Неопределено, Реквизиты = "", ИмяВТ = "ВТРезультирующийПериодДействия") Экспорт
				
	ДопУсловие = "СторноТекущегоПериода = НЕОПРЕДЕЛЕНО";
	Предикат = ?(ПустаяСтрока(Предикат), ДопУсловие, Предикат + Символы.ПС + "И " + ДопУсловие);
	
	РПД = ЗарплатаКадрыРасширенный.РезультирующийПериодДействия(
		Метаданные.РегистрыРасчета.Начисления, Предикат, ПараметрыПредиката, Реквизиты);
